...
#     不是 ⇒ 它會把人推向更暗的路。
#
# 天花板/範圍: 只把【這次 staged 的】 migration 複製出來餵給 scripts/migration-post-commit-guard.sh;
#   守門邏輯全在那支,本檔不重寫。只看 staged 不掃全庫(掃全庫會被既有違規擋死所有 commit、且隨分母變慢)。
#   這份清單是我想得到的那些, 而我最可能漏掉的是「staged 清單與 index 要提交的那份不一致的那種掉包」。
# 天花板/量具: 本檔自己【不驗 migration 對不對】,只驗「有沒有把對的檔餵過去」;而 -f 掉包(M4)只證
#   工作樹有一支檔,不證 index 要提交的那一份是同一支。
#   這份清單是我想得到的那些, 而我最可能漏掉的是「餵對了檔、但那支守門本身的射程漏掉的形狀(見那支的覆蓋界線)」。
