...
#      ~~**2026-08-25 起 EXPECT=47**~~ ⇒ **現值 50**(index-aware +7 / code-reviewer R1 +8 /
#      互相遮蔽 +2 / codex R1 +15 / codex R2 +1 / **F4 證人 +3**)。
#      🔴 **這一行是同一格第【四】次要改** —— 前三次都寫錯。病不在粗心:
#      **這個數字被抄在兩個檔裡, 而只有一個會被跑到。**
#      ⇒ codex R3 F8 開的更硬修法(**還沒做, 屬範圍決定**):每格配唯一 ID、
#        harness 比對「預期 ID 集合 vs 實際跑到的 ID 集合」, 廢掉第二份數字。
...
# ✅ **2026-08-25:證人補上了。**
#    ~~「這一道還沒有證人:harness 裡沒有任何一格會讓這裡的 grep 回 rc>1 —— 我構造不出一個
#      『不改本檔的碼、只靠 fixture』就能讓它壞掉的世界(它讀的是 pipe, 不是檔)。」~~ ⇒ **劃掉, 不刪。**
#    🔴 **構造得出來** —— 繞開的是那句話的【前提】不是結論:`grep` 是一支**外部指令**,
#       用 PATH shim 換掉它就好, 本檔的碼一個字沒改。
#
# 🔴🔴 **而【怎麼壞】會改變結論 —— 這是本輪最貴的一課(codex R3 F5 逼出來的)。**
#    兩種故障各量一次, 同一個 fixture, 只差有沒有這個 if:
#
#      (甲) grep **每一發都壞**(harness `格33`)
#           有 if ⇒ rc=2 印「副檔名篩選的 grep 回 rc=2」   ← 指著 grep
#           拿掉  ⇒ rc=2 印「分母是 0 ⇒ 枚舉機制壞了」
#                  ← 🔴 **只回報下游症狀, 沒有指出真正失敗的那一發 grep**
#                    (codex R2 F3 訂正:~~「指著一個沒壞的東西」~~ 不精確 ——
#                     副檔名過濾失敗確實讓枚舉鏈壞了, 錯的是它沒說出根因。)
#           ⇒ 這種故障會**連下游的副檔名 grep 一起打壞** ⇒ 分母變 0 ⇒ 下游自檢接住。
#
#      (乙) grep **只壞第一發** —— 為了覆蓋**只影響單次呼叫的故障模型**(harness `格33c`)
#           (codex R2 F3 訂正:~~「真實的 grep 故障更常是暫時性的」~~ **這組量測撐不起那句話**,
#            而記憶體 / 檔案描述子耗盡也不一定只影響一次呼叫。⇒ 改寫成故障模型, 不宣稱頻率。)
#           有 if ⇒ rc=2
#           拿掉  ⇒ 🔴 **rc=0, 印「全部有歸屬」= 真的放行。下游沒有第二道網子。**
#
#    ⇒ **這道 if 買到的是 fail-closed, 不只是病名。**
#    ⚠️ ~~我第一版只量了(甲), 就寫下「它買的不是防恆綠、是病名」~~ —— **那個結論是錯的**,
#       而它錯的方式很典型:**我只做了一種故障, 而那一種剛好會觸發別人的網子。**
#       📌 判別句:**我構造的故障, 是不是剛好是「會被別的東西接住」的那一種?**
#
# 📌 `格33` 的判準**不能只看 rc**(甲世界兩邊 rc 都是 2)⇒ 判別力在訊息字串上;
#    `格33c` 則是 rc 本身就分得開(2 vs 0)⇒ **它是這兩格裡承重的那一格。**
