...
# 🔴🔴 **2026-09-08 Sean 拍甲調高**(逐字「甲 現在調上限先通關, 重開之後派一個窗【真的】重搬一次
#    + 立一條『新東西不准進常載』的閘」)。⛔ ~~WARN=22000 / HARD=26000~~ 作廢。
#    量測當下 CLAUDE.md = **24,279 字元**(距舊 HARD 只剩 1,721 = **2.8 個中位改動**)。
#    沿用同一把尺(中位 614 / p90 1,301):
#      WARN=27000 ⇒ 餘裕 2,721 = 4.4 個中位改動 ⇒ 不會在下一次改動就叫
#      HARD=30000 ⇒ 餘裕 5,721 = 9.3 個中位 / 4.4 個 p90
#      WARN 到 HARD 之間 3,000 = 4.9 個中位改動 ⇒ 看到警告之後仍有時間搬
# 🛑 **而調高【不是】修法, 它是買時間 —— 兩件成對, 不可只取前半**:
#    Sean 同一句拍的另一半是「重開之後派一個窗真的重搬一次 + 立一條新東西不准進常載的閘」。
#    📌 理由是量到的:2026-09-02 搬過一次 43,326 ⇒ 19,650, 而**六天後回到 24,279**
#       ⇒ 🔴 **病灶不是總量, 是「每學到一件事就往常載加一段」而沒有東西擋著。**
#       ⇒ 調上限只會把同一天往後推;**沒有那道閘, 下一次一樣會撞。**
#    🔵 而那 70% 不是廢話:tidy 2026-09-08 量到義務佔 7,337 / 24,279 ⇒ 其餘是拍板紀錄與病史
#       ⇒ ⛔ **不得以刪註解/刪舊字面當降行手段**(鐵則 6 同款);要搬去 casebook, 不是刪。
WARN=27000
HARD=30000
