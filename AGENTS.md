...
> 🔴 **重複段主從(2026-08-09 Sean Q-F=C)**:本檔與 `CLAUDE.md` ~~逐字重複~~**部分重疊**的八段(⚠️ 2026-09-02 實測「逐字重複」不成立:本檔四段各有 24%~50% 的內容 `CLAUDE.md` 從來沒有過 ⇒ **照抄任一邊都會毀掉另一邊**;逐段數字 📎 `docs/patterns/routing-casebook.md` §26)(鐵則 1-12/Git 紀律/Busboy/Bash 紀律/Server 端鐵則/風格速查/突發狀況/Phase 1 精神)**以 `CLAUDE.md` 為準**;兩檔字面歧異時照 CLAUDE.md 執行並回報修齊,改任一段必同 commit 改兩檔。
...
**發現 Claude Code 在做這些 → 點出**:把 design「翻譯成 Tailwind 風格」/ 為「保留既有 storefront 結構」改 design 內容 / 憑記憶描述 design·畫預覽 HTML / 把 9 大藍圖 schema 加進 Phase 1 / 一個 slice 跨 3+ 檔卻未提 plan(鐵則 8)/ 未經 Sean 批准啟動多代理實作(鐵則 7)/ 即將自動 push。
