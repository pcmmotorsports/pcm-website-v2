...
> 🔴 **重複段主從(2026-08-09 Sean Q-F=C)**:本檔與 `CLAUDE.md` ~~逐字重複~~**部分重疊**的八段(⚠️ 2026-09-02 實測「逐字重複」不成立:本檔四段各有 24%~50% 的內容 `CLAUDE.md` 從來沒有過 ——鐵則 35% / Bash 45% / 自檢清單 50% / Git 24%,10 字視窗 ⇒ **照抄任一邊都會毀掉另一邊**)(鐵則 1-12/Git 紀律/Busboy/Bash 紀律/Server 端鐵則/風格速查/突發狀況/Phase 1 精神)**以 `CLAUDE.md` 為準**;兩檔字面歧異時照 CLAUDE.md 執行並回報修齊,改任一段必同 commit 改兩檔。
