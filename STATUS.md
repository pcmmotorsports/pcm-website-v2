...
**🆕 M-3 migration 待 Sean `supabase db push`**(**僅剩 ⑥ 3DS-0a** payment_webhook_events 表+record RPC;**②-③a 已上 prod 20260612150000**〔審查側複驗、不再待 push〕;角色〔payment_confirmer〕既存零新密碼/env;3DS-0a push 後 webhook route 才能端到端,純 code+unit tests + 0b/0c migration MCP 模擬皆不卡)。②-③ 拍板已記 plan v6 §0(Q1=A/Q2=A/Q3=B + 完整修好=雙軌+token);3DS 拍板記 master plan v5(對帳脊椎 + D4 override + S1/S2/S3=B)。前序:②-② ✅(pg A1=A、真連線 PASS)/ ②-① ✅ 上 prod。
