...
**下一階段=M-4a 第一期收口(Sean 07-16 拍優先序:①客戶線→②Email→③最新商品、④小件穿插)**。**①客戶線五片 ✅ 全收案上 production**(列表/明細-a `30659b8`/明細-b `675b949`/儲值金編輯 `afea2b7`/tier 編輯 `13ce3a9`;migrations 至 `20260717010000` 全 apply、Sean 肉眼驗過)。**②Email 通知片=規劃階段 ✅ 完成、🔴 等 Sean 拍板才動工**(v1 雙審 FAIL→Sean 四拍板→**架構改用 Supabase pg_cron 免費解**→v2→R2 雙審 FAIL→**v3 銷案**;plan 審 2 輪上限已用盡、不開 R3)。實作**另開視窗**(Sean 交代),交接=`docs/handoff/2026-07-16-m4a-email-notify-plan-r2-kickoff.md`、plan 真權威=`pcm-tools/review-inbox/m4a-email-notify-plan.md`(v3)。
**Branch:** dev
...
| Hash | 訊息 | 時間 |
|---|---|---|
| `529a7d8` | docs(docs): 客戶線收工交接+STATUS 對齊 上正式站 [m-4a] | 2026-07-16 |
| `afea2b7` | feat(admin): 儲值金編輯 加值扣款 owner RPC+明細表單 [m-4a] | 2026-07-16 |
| `675b949` | feat(admin): 客戶明細-b 訂單歷史+地址+車庫 [m-4a] | 2026-07-16 |

## 下一步
**M-4a 第一期收口(①客戶線 ✅ 全完→②Email→③最新商品、④小件穿插)**:②**Email 通知片**——規劃已完成、**🔴 gate=Sean 拍 plan §8 三題**(★S1 是否過度工程〔薄 outbox vs 極簡 await+人工補〕/★S2 **`order_shipped` 業務語意**〔現況無此事件、未拍則 E4 blocked〕/★S3 退款取消後是否仍寄+極低重複風險認可)→ 拍完**另開視窗**依序做 E1a(migration)→E1b(port/adapter)→E2a(sweeper+對帳+dead-man)→E2b(pg_cron/Vault/連通)→E3(order_created)→E4(blocked)。開工依據=`docs/handoff/2026-07-16-m4a-email-notify-plan-r2-kickoff.md`;plan v3=`pcm-tools/review-inbox/m4a-email-notify-plan.md`。③**最新商品**(storefront 例行 UI)。④小件穿插=Q3d 佔位圖/Q3a 佔位頁/Q3e 結帳內嵌地址。另:`/pcm-roadmap`+`/graphify --update` 值班台 07-16 已代跑(欠帳銷)。獨立線(未排):**LINE 補資料線**(Q1 拍板、4-5 slice、動工前 Sean 須先拍「真實 email 存哪」);搜尋 S2 lightech #275→S3 MVP;M-3 真刷卡 gate;step-up 全套另片(詳 memory `project_m4a-tier-edit-decisions`)。M-4a 收尾後開 Sean 交代「流程再優化」正式題。

## Sean 待決策
**🔴 ①Email 片動工 gate=plan §8 三題**(全文=`pcm-tools/review-inbox/m4a-email-notify-plan.md` §8):**★S1** 是否過度工程(薄 outbox+sweeper〔PRD §6.6 明文要求、plan 預設〕vs 極簡「await 送+failed 告警+人工補」;判準=是否承諾客人可靠收到、有無人天天巡檢後台)/ **★S2 `order_shipped` 業務語意**(🔴 現況**不存在**「整單出貨」事件——D2 後訂單 workflow 不代表出貨;未拍→E4 blocked)/ **★S3** 補寄前已退款取消是否仍寄原信 + 外部寄信非原子的極低重複風險認可(無法消滅只能收窄)。②**(非阻擋、與 Email 片無因果)Vercel 升 Pro?** 官方明載 Hobby 禁商業用途、PCM 賣場已命中(「Advertising the sale of a product or service」+「processing payment」),$20/月=**既存合規風險**、Sean 已知悉未拍。③**(Email S4、非必拍)** ACL 採 GRANT service_role(Fable 裁)而非 RPC-only(codex 裁)——兩審分歧、Claude 已判、Sean 可否決(理由=RPC-only 不縮小 blast radius,持 key 者本就能讀 customers 全表;全紀錄 plan §4.3)。④**(LINE 補資料線動工前)** 真實 email 存哪、如何與 auth.users 同步、如何不破壞既有 LINE 防冒登入設計。⑤(非阻擋)車款搜尋繼承件卡片是否併 inherited 年份=R3、日後另議。(Email Q1/Q2/Q4 + tier Q1/Q2——07-16 已拍、銷案,詳 memory。)
