...
## 當前狀態
**Phase:** Phase 1 / **Milestone:** **M-3 結帳**(階段① ✅;階段②-① confirm RPC + payment_confirmer ✅ 上 prod;**階段②-② adapter+use-case 地基 ✅**〔②-②a charge + ②-②b confirm/編排〕;②-③ charge action 開工)
**當前 slice:** **M-3 階段②-③ charge server action**(接 ②-② adapter+use-case 地基)— 組 建單→charge→confirm 鏈、server read-back orders.total 單一金額來源、PF-X1〔孤兒/charge-attempt〕+PF-X2〔per-order 鎖防雙扣〕+PF-X3〔已落 use-case〕落地、cardholder 從 server session/地址組裝〔MUST-FIX 3〕。**②-② ✅**(②-②a TapPayChargeAdapter + ②-②b PaymentConfirmerAdapter〔pg Supabase session pooler + 完整 CA 驗證、真連線端到端實測 PASS〕+ confirmPayment 孤兒契約;鐵則8 pg 已批 A1=A;BLOCKER+3MUST-FIX+8SHOULD 全落實)。

**Branch:** dev

## 最後更新
2026-06-12 — Claude Code 執行 session(寫審分離 ROLE=A 執行側)[**階段②-② adapter+use-case 地基 ✅**(2 子片):②-②a TapPayChargeAdapter(c8b5da0、pay-by-prime + TapPayChargeResult 補 amount〔BLOCKER PF-X3〕+ phoneNumber 必填 + order_number 對帳);②-②b PaymentConfirmerAdapter(pg、finally end、連線/RPC RAISE 分類)+ IPaymentConfirmer port + confirmPayment use-case(孤兒契約 outcome、confirmed:false fail-closed)+ composition root 受控注入 + pg 依賴 + master plan §3.2 webhook override。鐵則8 pg 已批 A1=A;BLOCKER+3MUST-FIX+8SHOULD 全落實;各片三綠 + code-reviewer PASS + codex 關卡2 PASS(②-②a/②-②b round1 findings 全修 round2 PASS)。**②-②b-fix 連線模型 + SSL 修正**(Sean 端真連線測試觸發、codex 三輪〔Sean 授權破 2 輪、payment SSL〕):原「直連 5432」作廢〔IPv6-only Vercel 連不到〕→ Supabase **session pooler + 完整 CA 驗證**(內嵌 Supabase Root 2021 CA);codex 抓出「CA 驗證假象」〔連線字串 sslmode 覆蓋 ssl 物件〕→ buildPgConfig 離散欄位+剝 SSL 參數+唯一 CA 驗證;**追加 host 釘死 pooler DNS allowlist + 顯式 servername**〔審查側 codex 抓 pg 只對 DNS host 設 servername、MITM 縱深、Sean 拍 A〕;Sean 重設 payment_confirmer 密碼;端到端實測 CA+登入+SECDEF+窄權全 PASS 零訂單異動、反向實證 bogus CA + IP/非-pooler host 必被擋 → 下一步 ②-③ charge action]
前序 2026-06-11 — [階段②-① confirm RPC + payment_confirmer 窄權角色 ✅ db push 上 production + env 就緒]

## 最近 3 commit
> 下表列近期可達 commit(挑有意義的、非機械 git log -3;表頂 `3d87a5d`〔②-②b、已推 origin/dev〕為本 ②-②b-fix slice commit 提交時 dev 可達父、本 slice commit 自身 hash 不入表〔避 off-by-one orphan〕)。
| Hash | 訊息 | 時間 |
|---|---|---|
| `3d87a5d` | feat(adapters): M-3 ②-②b PaymentConfirmer pg + confirmPayment [m-3] | 2026-06-12 |
| `c8b5da0` | feat(adapters): M-3 ②-②a TapPayChargeAdapter pay-by-prime [m-3] | 2026-06-12 |
| `c11ec9c` | docs(status): 階段②-① db push 上 prod + env 就緒、下一步 ②-② [m-3] | 2026-06-11 |

## 下一步
**🆕 M-3 階段②-③ charge server action**(接 ②-② adapter+use-case 地基):storefront server action 組「建單(既有 placeOrder)→ getTapPayAdapter().charge → getPaymentConfirmer 經 confirmPayment 編排」鏈;🔴 **server read-back orders.total 為單一金額來源**餵 charge 與 confirm(client 零價、禁浮點);PF-X1〔charge-attempt 紀錄供孤兒對帳〕+ PF-X2〔per-order charge 鎖防雙扣、跨請求 DB/紀錄層〕落地〔PF-X3 已在 use-case〕;cardholder 從 server session/結帳地址組裝〔MUST-FIX 3、phone 取地址〕;outcome 映 UI(paid→完成頁 / charge_failed→可重試 / orphan·charge_unknown→「付款已收勿重刷」)。✅ **payment_confirmer 真連線已實測**(session pooler + CA 驗證端到端 PASS、見 Blocker)。續 ②-④ 前端 TapPay Fields / ②-⑤ 完成頁 / ②-⑥ webhook。詳 docs/specs/2026-06-12-m3-stage2-2-tappay-adapter-plan.md §4 + kickoff §7
...
**🆕 M-3 階段②-② adapter+use-case 地基 ✅**(pg 已批 A1=A、BLOCKER+3MUST-FIX+8SHOULD 全落實;**payment_confirmer 真連線端到端實測 PASS**=session pooler + 完整 CA 驗證、Sean 已重設密碼+更新 env);**無待決策**。前序:②-① ✅ 上 prod。
...
**M-3 階段②-② 前置 ✅ 就緒 + payment_confirmer 真連線端到端實測 PASS**(2026-06-12):`PAYMENT_CONFIRMER_DB_URL` = **Supabase session pooler**(`aws-1-<region>.pooler.supabase.com:5432`、user `payment_confirmer.<ref>`;原「直連 5432」作廢=直連 host IPv6-only、Vercel 連不到)+ adapter 完整 CA 驗證(Supabase Root 2021 CA、rejectUnauthorized:true、**host 釘死 pooler DNS allowlist + 顯式 servername**=MITM 縱深);實測 CA 驗證 + payment_confirmer 登入 + SECDEF confirm RPC + 窄權(直查 orders denied)全 PASS、零訂單異動;反向實證 bogus CA + IP/非-pooler host 必被擋。Sean 已重設 payment_confirmer 密碼 + 更新 env。TapPay sandbox 金鑰(Partner Key/Merchant/APP_ID/APP_KEY)已設。**非 blocker**。
