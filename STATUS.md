...
**當前 slice:** **g-6c ✅(愛車編輯/刪除/設主車、CRUD 閉環、鏡像 g-5c、PCM 自驅、未 push)**(g 第十三片;g-6 拆三片第三片 = 編輯/刪除/設主車。VehiclesTab 重構單一 vehEdit state〔null=關 / 無id=新增 / 有id=編輯、對齊 design L385/L601/L607/L617〕+ .acc-addr-actions 編輯〔toggle〕/刪除鈕〔搬 design L600-603、複用 g-5c address actions class + inline style marginTop:12〕+ 刪除前 confirm('確定要刪除這輛車？')〔直接搬 design L400〕+ 兩 inline-form 包裹層 scrollIntoView ref + 編輯重用 InlineVehicleForm〔veh=該筆完整 CustomerVehicle、heading「編輯車輛」、onSubmit=(input)=>updateVehicleAction(id,input)、id 綁 parent closure〕+ vehicle/actions.ts 加 updateVehicleAction/deleteVehicleAction〔鏡像 update/deleteAddressAction 信任邊界、updateAction 僅 name fieldError、deleteAction ownership 靠 use-case〔listByCustomer 驗本人+遞補首台主車〕+RLS、plain-update 同 address #199 僅 RLS 守〕。設主車無獨立鈕〔design 卡片僅編輯/刪除、改主車靠編輯表單「設為主要車輛」勾選→updateVehicle isPrimary swap、setPrimaryVehicle use-case 本 UI 不接〕。session-write 跑 code-reviewer + codex 關卡2。三綠 typecheck 7/7 + lint 10/10 + build /account ƒ + vitest 73 檔/461〔+vehicle/actions.test +8 + VehiclesTab +5〕。**愛車 CRUD 閉環:g-6a 唯讀 / g-6b 新增 / g-6c 編輯刪除設主車 = Sean 一次完整肉眼驗**。**g-7 wallet 推延**〔台灣儲值法規未定、deposit 不做、backlog #202 + memory project_wallet-deposit-taiwan-legal-hold〕。manifest 擴 g-6c + open_drift(愛車 CRUD 完成、g-7 HOLD)。**前序 g-6b〔g 第十二片〕= 新增車輛表單、g-6a〔g 第十一片〕= 唯讀列表、見 git history**。原 g-6b/g-6a 詳述移此前序:g-6 拆三片第二片 = 新增。VehiclesTab→'use client' + ＋新增車輛鈕(design L584) + 新 InlineVehicleForm.tsx〔搬 design L760-798、6 純文字欄〔車型必填/年份/引擎號/里程/已改裝/最近保養 type=date〕+ 設主車勾選、**無發票三 tab**(比 g-5b 簡單)、#181 雙通道僅 name、新增不預填〔車型是車非人、design L584 name:''〕〕 + 新 vehicle/actions.ts addVehicleAction〔五層信任邊界鏡像 addAddressAction:getUser user.id / VehicleInput safeParse 僅 name 必填無巢狀 / addVehicle isPrimary→unsetCurrentPrimaryExcept swap / RLS vehicles_insert_own / catch 不洩〕。原 g-6a 詳述移此前序:g-6 拆三片第一片 = 唯讀列表。composition getVehicleRepo〔鏡像 getAddressRepo、SupabaseVehicleAdapter authenticated client 走 RLS vehicles_*_own、不持 service_role〕+ page getVehicleRepo→listByCustomer 退化空陣列 + AccountView vehicles prop + VehiclesTab 搬 design L580-620 .acc-bikes/.acc-bike 卡〔ap-mono Primary/Secondary + h3 車型 name + .acc-bike-meta 年份·引擎號 + .acc-bike-stats 里程/已改裝/最近保養條件渲染〕+ 空狀態「尚未新增愛車—新增後可記錄改裝履歷。」+ .acc-bike CSS 11+2 條〔搬 design L739-781 + mobile L913-914 @media/data-mobile 雙寫〕。唯讀不渲染寫入鈕〔新增 g-6b、編輯/刪除/設主車 g-6c〕、design L678 VehicleModal dead code(return null)不搬、不搬 design localStorage mock 愛車。車款維持 design 自由文字 name;Sean 拍 ③.5=A〔→ products filter 連動 backlog #200 綁 Phase 2 結構化 vehicles、graphify 證 Identity↔Catalog 跨 bounded context 零邊〕。鏡像 g-5a 唯讀地基、無 session-write 不跑 codex。三綠 typecheck 7/7 + lint 10/10 + build /account ƒ + vitest 72 檔/441〔+VehiclesTab.test 7〕。肉眼驗 Sean 拍空狀態+tab 渲染夠〔沿用 g-5a 已驗 .acc-empty pattern〕、卡片視覺留 g-6b 一起驗。manifest 擴 g-6a + open_drift 更新。前序 g-5c〔g 第十片〕= 收件地址編輯/刪除 CRUD 閉環、見 git history)

**Branch:** dev

## 最後更新
2026-05-31 — Claude Code [g-6c ✅(愛車編輯/刪除/設主車、CRUD 閉環、鏡像 g-5c、PCM 自驅;三綠 typecheck 7/7 + lint 10/10 + build /account ƒ + vitest 73 檔/461;session-write 跑 code-reviewer + codex 關卡2;愛車 CRUD 閉環 g-6a/b/c Sean 一次肉眼驗;g-7 wallet 推延〔台灣儲值法規 Sean 拍不做、backlog #202 + memory〕;manifest 擴 g-6c;g-6a 已 push〔origin/dev=44394b5〕、未 push 2 commit g-6b/c〔等 Sean 手動推〕)]

## 最近 3 commit
> 下表列近期 3 個有意義的可達 commit(挑有意義的、非機械 git log -3;本 g-6c slice commit〔HEAD〕記可達祖先 e9cfd5d 為表頂、g-6c commit 自身 hash 不入表〔避 busboy 雙 amend orphan、見 memory project_status-top-hash-off-by-one-normal + backlog #180〕;busboy 步驟 7 例外:STATUS 已在 slice 主 commit 手動更新、不再 amend)
| Hash | 訊息 | 時間 |
|---|---|---|
| `e9cfd5d` | feat(storefront): g-6b 新增車輛表單 + InlineVehicleForm + addVehicleAction [M-1-14e-g-6b] | 2026-05-31 |
...
## 下一步
**g-6c ✅(愛車 CRUD 閉環、未 push)→ Sean 一次愛車肉眼驗 + g-7 推延**:愛車 tab CRUD 完成(g-6a 唯讀 / g-6b 新增 / g-6c 編輯刪除設主車)。**下一步 = Sean 一次完整愛車肉眼驗**(等 push 後實機):登入 /account → 我的愛車 → ＋新增車輛(車型必填+其餘選填+可勾設主車 → 儲存 → 卡片即現)/ 每卡「編輯」(帶入舊值〔車型/年份/引擎號/里程/已改裝/最近保養〕→改值儲存→不刷新即更新)/「刪除」(confirm「確定要刪除這輛車？」→確定卡片消失〔刪主車則首台遞補 Primary〕/取消不刪)/ 編輯勾「設為主要車輛」儲存 → 該卡變 Primary 黑底、原主車降 Secondary;桌機+手機。**g-7 wallet 推延**(台灣儲值法規未定、Sean 拍 deposit 不做、backlog #202)。**會員中心除 wallet 外完成**;g 段後續待 Sean 排(M-1-15 / M-1-16)。**g-5 全部 + g-6a 空狀態 肉眼驗 ✅**(2026-05-31 Sean 驗過)。
