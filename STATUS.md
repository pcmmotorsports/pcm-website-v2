...
**下一階段=M-4a 第一期收口(Sean 07-16 拍優先序:①客戶線→②Email→③最新商品、④小件穿插)**。客戶線進行中:列表(唯讀)先前已上線;明細-a ✅ `30659b8`(基本資料+儲值金餘額/流水;🔴 Sean 07-16 拍板 admin 後台顯示儲值金=override 05-31 前台 hold、範圍僅 admin、memory 已記);**當前 slice=儲值金編輯 ✅ 本 commit**(🔴 動錢;`admin_adjust_wallet` owner RPC migration `20260716210000`〔**未 apply、commit 壓住**〕+明細頁加值/扣款表單;關卡1 PASS→code-reviewer R1 PASS→codex 兩輪 4 findings 全修〔含 PG `E'\v'` 字面陷阱〕、round2 後未再跑=硬上限、殘項轉值班台模擬;🔴 **db push 前必拍待決策①D1**)。續行=tier 編輯(🔴 高風險件#3、plan 關卡1)。
**Branch:** dev
...
## 下一步
**M-4a 第一期收口(Sean 07-16 拍:①客戶線→②Email→③最新商品、④小件穿插)**:①**客戶線**進行中——列表 ✅ → 明細-a ✅ `30659b8` → 明細-b ✅ `675b949` → **儲值金編輯 ✅ 本 commit(code 收工;🔴 部署硬閘未走=待決策①D1 拍 → Sean db push `20260716210000` → 值班台驗 ACL+交易模擬 → 才放行推)** → **tier 編輯**(🔴 高風險件#3=service_role 寫 customers.tier+audit+step-up、鐵則 8;偵察已完=audit_log 已 apply 在用/actor 現為過渡 cookie 待接真 SSO session/step-up 門檻待拍,plan 時逐項處理;tier 後台寫不需 #215 前置=07-13 分析)。②**Email 通知片**(plan=`docs/handoff/2026-07-13-email-notification-slice-plan.md`;🔴 動工前先給 Sean 決策題「觸發點碰不碰金流 RPC」)。③**最新商品**(storefront 例行 UI)。④小件穿插=Q3d 佔位圖/Q3a 佔位頁/Q3e 結帳內嵌地址。獨立線(未排):搜尋 S2 lightech #275→S3 MVP;M-3 真刷卡 gate。M-4a 收尾後開 Sean 交代「流程再優化」正式題。

## Sean 待決策
①**🔴 D1 儲值金 double-submit 防護層級(擋 db push `20260716210000`、apply 前必拍)**:A=現狀夠(前端鈕 pending disable+PRG+流水可視可沖銷;執行台+值班台原建議)/ B=再加 DB 級去重(ledger 加 idempotency key+UNIQUE=schema 改動、另開一片;codex 第三眼強烈建議、動錢不宜只靠沖銷)。**推薦 A 先上、B 開 backlog 隨 tier 片一併評**。②D2 儲值金單筆上界:A=維持 1000 萬(值班台建議)/ B=收緊 100 萬(可與①同拍;改=migration 一行)。③**Email 片觸發點碰不碰金流 RPC**(plan 內建議=outbox 不進訂單交易、不碰 create_order;動 Email 片前拍)。④(到 tier 編輯片時拍)step-up 二次驗證門檻。⑤(非阻擋)車款搜尋繼承件卡片是否併 inherited 年份=R3、日後另議。
