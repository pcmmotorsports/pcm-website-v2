...
| `14373b9` | feat(storefront): M-3 ②-③e chargePaymentAction 成交鏈+六態映射 [m-3] | 2026-06-12 |

## 下一步
**🆕 M-3 3DS Phase I 續**(master plan v5 §2):3DS-0b(cross-tab DB 合約:begin 偵測同 cart_session_id orphan-pending → `needs_settle`〔D4 override〕、需 ②-③a 上 prod 先)→ 3DS-0c(bank_transaction_id 欄 + 待開票 durable 表)→ 3DS-1a(Record API adapter)→ 3DS-1b(🔴 settleCharge use-case、鐵則12 codex 關卡2 須覆蓋 retry 三仲裁測)→ 3DS-2(webhook route 祕密路徑+rate-limit+對不上本機 order 丟)→ 3DS-3(callback route + 成交清車、**吸收原 ②-⑤ 完成頁**〔git stash WIP〕)→ 3DS-4(sweeper cron)。**Phase II**:3DS-5a/5b/6/7〔charge adapter 3DS+initiate〔禁 delay_capture_in_days:-1〕、client redirect、cart_session_id〕 feature-flag `TAPPAY_3DS_ENABLED`;**Phase I+II + flag on 前 prod 結帳不開**(誠實中間態)。3DS-0a ✅(本 commit、待 db push)。🔴 前置:Sean `supabase db push`〔②-③a + 3DS-0a 都堆著〕。詳 master plan v5 / plan v6
...
**🆕 M-3 migration 待 Sean `supabase db push`**(② ②-③a payment_charge_attempts 表+4 RPC + ⑥ 3DS-0a payment_webhook_events 表+record RPC;角色〔payment_confirmer〕既存零新密碼/env;push 後 ②-③b 真連線實測 + 3DS-1b settleCharge / 3DS-2 webhook route 才能對 session pooler 端到端,純 code+unit tests 不卡)。②-③ 拍板已記 plan v6 §0(Q1=A/Q2=A/Q3=B + 完整修好=雙軌+token);3DS 拍板記 master plan v5(對帳脊椎 + D4 override + S1/S2/S3=B)。前序:②-② ✅(pg A1=A、真連線 PASS)/ ②-① ✅ 上 prod。
...
**M-3 ②-③b 真連線實測卡 ②-③a db push**(非硬 blocker:②-③b 純 code+unit tests 可先行;主軌 RPC literal 實呼〔pooled MCP 限制、migration 頭註解誠實標記〕由 ②-③b adapter 對 session pooler 真連線補測、需 ②-③a 先上 prod)。**②-② 前置 ✅**:`PAYMENT_CONFIRMER_DB_URL` = session pooler + adapter CA 驗證 + host 釘死(真連線端到端 PASS、反向實證 bogus CA/IP/非-pooler 必擋);TapPay sandbox 金鑰已設。
