...
## 當前狀態
**Phase:** Phase 1 / **Milestone:** M-1(M-0 ✅ + M-1-01~12 ✅ + M-1-13a~d ✅ + M-1-13e-pre-1/2/3 ✅ + M-1-13e-a ✅ + M-1-13e-b ✅ + M-1-13e-b-2 ✅ + M-1-13f-1 ✅ + M-1-13f-2 ✅ + **M-1-13H ✅(商品頁全面改版 6 slice 完成 + Codex Review findings 4 處 fix)** + 13g(Toast 推延)+ **Stage-3-onboarding ✅ + Stage-3-codex-fix ✅(工作流升級 + Codex 4 findings 應對)** + **M-1-13I ✅(車種跨頁傳遞 3 bug 修 + V1 manifest audit、新工作流第 2 次實證)** + **M-1-13Z ✅** + M-1-14-recon ✅ + **M-1-14a ✅(Supabase customers 4 表 + RLS + 5 trigger / 3 function + 對帳 view)** + **M-1-14b ✅(domain Customer 10 欄 + address/vehicle/wallet 3 子 entity + ports ICustomerRepo 改寫 + 3 子 port)** + **M-1-14a-patch ✅(Codex FAIL 處置:invoice NOT NULL + ledger COMMENT Q1=B + PRD 入 repo + #170/#171)** + **M-1-14c ✅(packages/schemas 6 組 zod v4 表單驗證 schema + z.infer + zod 入 catalog)** + **M-1-14d ✅(packages/adapters customer/address/vehicle 3 Supabase adapter + 3 mapper + 3 mapper test;wallet 拆 M-1-14d-2)** + **M-1-14d-2 ✅(SupabaseWalletAdapter 雙 client〔讀 authenticated / 寫 service_role〕+ wallet mapper + 7 test、server-only export)**、餘 M-1-14e~16)
**當前 slice:** **M-1-14d-2 ✅**(packages/adapters:`mappers/wallet.ts`〔SupabaseWalletLedgerRow + mapSupabaseWalletEntryToDomain + mapWalletEntryToInsertRow;related_order_id string|null 直送不 coalesce、note NOT NULL string、amount signed int 純傳遞不變號〕+ `SupabaseWalletAdapter.ts` implements IWalletRepository〔listEntries〔穩定排序 entry_date desc, created_at desc〕/ addEntry / getBalance〕+ `mappers/wallet.test.ts`〔7 案例:重命名 + related_order_id null/非null 直送 + amount sign 保留〕。**雙 client DI(codex 關卡1〔d slice〕must-fix)**:readClient authenticated〔RLS own、listEntries/getBalance〕+ writeClient service_role〔RLS INSERT only-service_role、addEntry〕;每查詢明確 `.eq` filter 防繞 RLS。getBalance:balance/totalDeposit 來源 customers 餘額欄〔Q1=B trigger 同步〕、查無 row throw〔不 silent 0〕、lastEntryAt = ledger MAX(created_at)〔view 不對 authenticated GRANT、直查 ledger 等價〕。addEntry 靠 DB trigger 同步餘額、adapter 不自算;ledger immutable。**export 從 `@pcm/adapters/server`〔server-only、金流敏感、codex 關卡2 must-fix〕、非 root public**。**codex 關卡1** FAIL → 自修 6 findings〔lastEntryAt 補 .eq / 雙 client JSDoc 強化〔readClient 必帶 session 非裸 anon〕/ addEntry 授權歸 caller / 查無錯誤訊息 / 偏離歸 domain WalletBalance〕。**codex 關卡2** round1 FAIL 2 must-fix〔#1 wallet adapter 不該 root public export → 移 server.ts;#2 domain WalletBalance JSDoc lastEntryAt 精準化〕→ 修 round2 PASS。**超「4 檔」透明標注**:除 wallet.ts/adapter/test/index.ts,codex must-fix 另動 `server.ts`〔server-only export〕+ `domain/identity/wallet.ts`〔JSDoc 精準化、純註解不改型別〕。三綠 typecheck 7/7 + lint 10/10 + build 1/1 + wallet test 7/7;**code-reviewer PASS 0 must-fix**;未 push)。前序 **M-1-14d ✅**(customer/address/vehicle 3 adapter + 3 mapper + 23 test、已 push 870821d)。前序 M-1-14c/a/a-patch/b ✅(zod schema / Supabase 4 表 + RLS + GRANT / domain 10 欄 + ports 4 介面、Codex 終審 PASS;詳見 PROGRESS)。前序 M-1-14-recon / M-1-13Z / M-1-13I/13H ✅(詳見 PROGRESS)

**Branch:** dev

## 最後更新
2026-05-23 — Claude Code [M-1-14d-2 ✅ 收尾(SupabaseWalletAdapter 雙 client + wallet mapper + 7 test、server-only export;自驅 SOP 全跑:codex 關卡1〔6 findings〕+ 關卡2〔round1 2 must-fix→round2 PASS〕+ code-reviewer PASS;codex must-fix 致超「4 檔」另動 server.ts + domain/wallet.ts、已透明標注);+ 產 M-1-14e handoff;Sean 先前指示已 push M-1-14d(870821d);d-2 停等 Sean push;下一段 M-1-14e use-cases(接 DB/auth/金流、codex 雙關卡不可跳、範圍大先評估拆)]

## 最近 3 commit
| Hash | 訊息 | 時間 |
|---|---|---|
| `1882e57` | feat(adapters): M-1-14d-2 SupabaseWalletAdapter 雙 client + wallet mapper + test [M-1-14d-2] | 2026-05-23 |
