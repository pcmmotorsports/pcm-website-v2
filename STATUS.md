...
## 當前狀態
**Phase:** Phase 1 / **Milestone:** **M-3 結帳**(階段① ✅;②-① ✅ 上 prod;②-② ✅ 已推;②-③ ✅;②-④ ✅ 已推 f0c359b;**🔴 3DS 重設計 = 主線**〔production 4 merchant 全要 3D 驗證、charge async redirect → 對帳脊椎+webhook+cross-tab 折入、master plan v5 Sean 批〕;**Phase I 3DS-0a webhook inbox ✅ + 3DS-0b cross-tab dedup ✅ + 3DS-0c bank_txn/待開票 ✅**)
**當前 slice:** **🔴 M-3 3DS 重設計 Phase I**(master plan v5:3DS 必 async redirect、同步 confirmPayment 拆「啟動半段 + 結算半段」、新增 shared **settleCharge** 對帳 use-case〔callback/webhook/sweeper/retry 共呼、Record API〔record_status=1+is_captured〕唯一權威〕為脊椎;cross-tab D4 override + webhook 加固 C 折入)。**3DS-0b ✅**(cross-tab DB 契約:`orders.cart_session_id` uuid + dedup index + begin cart-instance dedup〔D4:同 cart 已 paid→duplicate、有扣款跡象〔charged/pending-未-paid〕→needs_settle 交 settleCharge 裁決;dedup 移 user_in_flight 閘「前」=關卡1 fix1〕+ create_order DROP 4p→CREATE 5p〔p_cart_session_id null fail-closed + GRANT 硬化補現有 service_role over-grant〕+ asserts〔5p 矩陣/4p DROP/begin ACL 回歸〕;零漂移 diff 驗 create_order 本體 + MCP 模擬 S1-S8〔dedup 三態/異 cart user_in_flight/null fail-closed/order_locked/多 sibling charged 優先〕+ C-null/C-happy PASS 零留痕 + 三綠 + code-reviewer PASS + codex 關卡2 PASS〔3🟡:#3 charged 優先排序已修、#1 bank_txn 0c 補、#2 退款片 backlog〕;待 db push bundle)。**3DS-0a ✅**:payment_webhook_events durable inbox 表 + record_webhook_event 去重 RPC(rec_trade_id 主鍵 ON CONFLICT、白名單欄+raw sha256 hash 不落 PII、processed/attempt_count/next_retry_at 供 sweeper〔非死信〕、notify 無簽章不可信僅稽核、SECURITY DEFINER+search_path=''、payment_confirmer 表層零權限 assert、三欄長度 CHECK+RPC 入口防灌;MCP 模擬 PASS×3 零留痕 + 三綠 + code-reviewer PASS + codex 關卡2 r1/r2 各 FAIL→修、r3〔Sean 拍 B 授權〕PASS;待 db push)。前序 **②-④ ✅ 已推 f0c359b**(TapPay Fields 六態 + 雙扣 4 向量〔同 tab/回應遺失 fail-closed unknown/webhook rec_trade_id UNIQUE/cross-tab 待 3DS-0b〕)。**②-③ 全五片 ✅**(a~e:鎖+簿記+雙軌+findTotal+織入+cardholder+action 六態)。**②-④ ✅**:TapPay Fields 前端接線(useTapPayCard SDK v5.19.2+TapPayCardFields iframe 卡欄〔卡資料零進 state、安全 override〕+ useChargePayment 六態+清車政策 + View getPrime→chargePaymentAction、canGetPrime gate 雙鈕 + CheckoutSuccess processing 變體 + 抽 CheckoutSummaryAside + 🔴審查側 FAIL 回修:client throw=回應遺失層 fail-closed unknown 終態〔清車+持鎖+勿重複付款、防雙扣 BLOCKER〕+ active=false 清 canGetPrime〔stale 鈕 MUST-FIX〕;900 tests + manifest 同步)。**②-③a ✅**(payment_charge_attempts 表+4 RPC+token helper;MCP Call A-D 全綠;**已上 prod 20260612150000**〔審查側 list_migrations+catalog 複驗、版本對齊 repo 無漂移〕)。**②-③b ✅**(IChargeAttemptStore port + PgChargeAttemptAdapter 主軌〔複用 buildPgConfig 縱深〕+ SupabaseChargeAttemptFallbackAdapter 備軌 + ChargeAttemptStoreWithFallback 複合〔主×3 退避100/300→備×2 退避100、P0001 早停、errLabel 防洩、events trace 鎖順序〕;820 tests)。

**Branch:** dev

## 最後更新
2026-06-13 — Claude Code 執行 session(寫審分離 ROLE=A 執行側)[**🔴 3DS-0c ✅ migration**(attempt 加 bank_transaction_id〔3DS-5b 送 charge 前寫、前向欄〕+ begin CREATE OR REPLACE 補回 existing_bank_transaction_id〔codex 0b #1;zero-regression awk diff 嚴格 3 處〕+ pending_invoices 待開票表〔S1=B、order_id UNIQUE 冪等、record_pending_invoice SECDEF payment_confirmer、paid 防呆 fail-closed〕;MCP 模擬 begin S1-S8 零回歸 + P1-P5 PASS 零留痕 + 三綠 + code-reviewer PASS 零 must-fix + codex 關卡2 PASS〔consider:bank_txn CHECK 延 3DS-5b;nit:字面已修〕;待 db push bundle)。前序 **3DS-0b ✅ migration**(cross-tab DB 契約 + D4 needs_settle):審查側關卡1 PASS + 3 fixes 全落地(dedup 移 user_in_flight 閘前 / 部署紀律白紙黑字〔0b 必與 cart_session_id 前端+adapter+mapper 同 bundle 推、0c 連帶卡、0a 可獨立先推〕/ begin 新 outcome TS 同步閘〔parseBeginResult 對未列 reason throw → 3 TS 改項同 bundle、begin 不單獨上 prod〕);zero-drift diff 驗 create_order 本體 + MCP 模擬 S1-S8 + C-null/C-happy PASS 零留痕 + 三綠 + code-reviewer PASS 零 must-fix + codex 關卡2 PASS(3🟡 consider:#3 多 sibling charged 優先排序「已修」+ S8 re-sim 驗、#1 needs_settle bank_transaction_id「0c 補」、#2 refunded sibling 狀態矩陣「退款片 backlog」);待 db push bundle。docs 入庫 + STATUS 更正 ②-③a 已上 prod + Sean 拍 Q1=A/Q2=乙/Q3=A(跳 codex 關卡1)。前序 **🔴 3DS 重設計啟動 + Phase I 3DS-0a ✅**:production 4 TapPay merchant 全開 3D 驗證〔Sean 確認 merchant_id 自帶、env 已寫〕→ 3DS = async redirect〔charge 回 payment_url〔跳轉非結果〕→ 銀行 OTP → frontend_redirect + backend_notify〕→ 同步 confirmPayment 必拆「啟動半段 + 結算半段」、新增 shared **settleCharge** 對帳 use-case〔callback/webhook/sweeper/retry 共呼冪等、Record API〔record_status=1+is_captured〕唯一權威〕為脊椎。先做業界紮根研究〔3 workflow:全球 Stripe/Shopify/Adyen + 台灣綠界/藍新/蝦皮 + PCM 稽核〕grounding 六決策 → **master plan v5**〔codex 關卡1 三輪 8→4→1 收斂、HMAC/record_status 兩處我方假設被官方文件糾正、notify 無簽章權威改 Record API、D4 override 改 orphan-pending 為 retry-now settleCharge 仲裁、S1=B 發票 fast-follow/S2=B 退款 P2/S3=B ATM P2〕Sean 批。**3DS-0a migration**(payment_webhook_events durable inbox + record RPC):MCP 交易模擬 PASS×3 零留痕 + 三綠 + code-reviewer PASS + codex 關卡2 r1〔must-fix payment_confirmer 表層漏關門+回歸 assert、consider 公開入口字串無長度上限〕→修→r2〔must-fix bank_transaction_id 入口漏空字串 → 落表層 CHECK 露內部錯〕→修→r3〔Sean 拍 B 授權加輪〕PASS。前序 **②-④ TapPay Fields ✅ 已推 origin/dev=f0c359b**(六態 + 審查側鐵則12 雙扣 BLOCKER 回修 fail-closed unknown + active=false stale 鈕 MUST-FIX);②-③ 全五片 + ②-③a/b ✅(plan v6 六輪收斂、雙軌+token+誠實文案)]

## 最近 3 commit
> 下表列近期可達 commit(挑有意義的;表頂 `b50bd62`〔3DS-0a〕為已 commit 之可達 HEAD、本輪 docs 入庫 commit 之可達父;本輪 housekeeping(docs/gitignore/STATUS)hash 不入表〔避 off-by-one orphan〕)。
| Hash | 訊息 | 時間 |
|---|---|---|
| `b50bd62` | feat(schemas): M-3 3DS-0a ②-⑥ webhook durable inbox 表+record RPC [m-3] | 2026-06-13 |
| `f0c359b` | feat(storefront): M-3 ②-④ TapPay Fields 前端接線 [m-3] | 2026-06-12 |
| `49e7156` | docs(specs): 記 Sean 拍 A — begin 斷線卡單接受、靠 #224/②-⑥ 對帳解 [m-3] | 2026-06-12 |

## 下一步
**🆕 M-3 3DS Phase I 續**(master plan v5 §2):3DS-0b(cross-tab DB 合約:begin 偵測同 cart_session_id orphan-pending → `needs_settle`〔D4 override〕、照搬 cross-tab plan §3.1/§4.1)→ 3DS-0c(bank_transaction_id 欄 + 待開票 durable 表)→ 3DS-1a(Record API adapter)→ 3DS-1b(🔴 settleCharge use-case、鐵則12 codex 關卡2 須覆蓋 retry 三仲裁測)→ 3DS-2(webhook route 祕密路徑+rate-limit+對不上本機 order 丟)→ 3DS-3(callback route + 成交清車、**吸收原 ②-⑤ 完成頁**〔git stash WIP〕)→ 3DS-4(sweeper cron)。**Phase II**:3DS-5a/5b/6/7〔charge adapter 3DS+initiate〔禁 delay_capture_in_days:-1〕、client redirect、cart_session_id〕 feature-flag `TAPPAY_3DS_ENABLED`;**Phase I+II + flag on 前 prod 結帳不開**(誠實中間態)。**3DS-0b ✅**(20260613130000)+ **3DS-0c ✅**(20260613140000:bank_transaction_id 欄 + begin 補回傳 + pending_invoices 待開票表)、皆待 db push bundle → 下一動 = **3DS-1a 小 plan 等 Sean 批**(Record API adapter:ITapPayAdapter.recordQuery + wire parse〔top status/trade_records/record_status/is_captured/number_of_transactions〕+ 單元測;**純 TS code**〔非 migration〕、鐵則 8 動共用 port、不卡 db push)。**1b** = 🔴 settleCharge use-case(脊椎、鐵則12)。🔴 **db push bundle**(關卡1 fix2):0b(+0c)必與 cart_session_id 前端/adapter(create_order 5p 傳 key)/mapper(begin 新 outcome parse)+ 1b settleCharge **同一次 db push**(begin 新 outcome 不單獨上 prod);3DS-0a 時戳早、無 adapter 依賴 → 可獨立先推。1a/1b 純 code + 0c migration MCP 模擬皆不卡。詳 master plan v5 §2
...
**🆕 M-3 migration 待 Sean `supabase db push`**(②-③a 已上 prod 20260612150000〔不再待 push〕;待推 = **3DS-0a**〔webhook_events、時戳早、無 adapter 依賴 → **可獨立先推**〕+ **3DS-0b**〔cart_session_id dedup、🔴 **必與 cart_session_id 前端/adapter/mapper + 1b settleCharge 同一次 db push bundle、勿單推**;DROP 4p 斷現有 adapter + begin 新 outcome parseBeginResult throw〕;0c 時戳在 0b 後連帶卡同閘;角色 payment_confirmer 既存零新密碼/env;純 code+unit tests + 0b/0c migration MCP 模擬皆不卡)。②-③ 拍板已記 plan v6 §0(Q1=A/Q2=A/Q3=B + 完整修好=雙軌+token);3DS 拍板記 master plan v5(對帳脊椎 + D4 override + S1/S2/S3=B)。前序:②-② ✅(pg A1=A、真連線 PASS)/ ②-① ✅ 上 prod。
