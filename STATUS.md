...
## 最後更新
2026-07-27(**Eazi-Grip 每日同步修復,本機 code + migration、零 production DB、未 push**)=根因為同商品群 spec 換位在 bulk upsert 中途撞 `pv_spec_unique`、非爬蟲漏抓;已分流 transition hazard 到單群 atomic RPC(鎖定→重驗→sentinel→孤兒刪除→完整 upsert→終態 assert),保留唯一鍵並鎖 service_role。真 PostgreSQL 已驗 HOSEYAM005、雙向 swap、異常 payload、post-mutation rollback、雙連線 TOCTOU;獨立唯讀複查 PASS。🔴 上線順序固定=migration apply + ACL/signature 驗證 → 才能 push code/重跑 sync,兩者皆待 Sean 明確批准。**同日較早規劃/文件 session**=①STATUS 主表 **357→30 行**(`dc46f14`;全文存 `PROGRESS.md`)②E8-B plan v3③完整後台規劃交接。
