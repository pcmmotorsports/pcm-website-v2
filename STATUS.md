...
🏁 **2026-08-13 四窗全部收工並 merge 入 dev**(`origin/dev..HEAD` 共 27 顆,**全部未 push、等 Sean 手動推**):
· **B** `#376` 撞號重編號 → `2f91a7b3`(worktree `/Users/sean_1/pcm-backlog-renumber` 可回收)
· **S** OD 詳情線四片(備註搬家 / 客人 id 資料線 / 取數抽共用 / 面板客人卡入口)→ `9fec15ce`
· **D** `#452` 片 2a-1 採購作廢欄 → 已 merge + **已 apply 正式庫**(`6bcba338` 台帳)
· **E** 訂單列表 L2(雙 markup 收斂)+ L0(shipped_quantity 進投影)→ `6f371758`;
  **L1(狀態八值純函式,零 UI)= `f745e04e` 停在 branch `od-order-list` 上未 merge**
🔴 **夜跑新視窗第一件事**:讀 `docs/handoff/2026-08-13-night-run-handover.md`(四線交接 + 各窗未落檔判斷)。
🔴 **E 線 L3 開工前有兩題待裁**(E-381-HANDOFF §2,含 E 窗的建議與論證):
  ①狀態欄接上去 = 訂貨欄與付款膠囊下架(Sean 拍 Q2=A),要一片做完還是拆兩顆?
  ②13 欄寬 + 窄版收欄要不要同片?(換 container query 會讓「面板開著時列表變窄」觸發收欄 = **桌機視覺會變**)
  ⚠️ `order-list-view.ts` 現為 **668 行**(400 線 +268),L3 要動它 ⇒ 鐵則 6 的判斷要單獨提。
## Sean 待決策
...
🔴 **E 線 L2 兩件請 Sean 過目**(已申報、他尚未表態;退掉成本 = 退 `025a7e7e` 一顆,不影響其他片):①多品項單第一列 38→62px、表格總高 +10.4%(**密度是他最在意的**)②rowSpan 拆除與 `design-brief` §0-B「多品項的合併呈現他認可、保留…**不要改掉它**」字面衝突 —— 但 OD 定案成品(他第 4-14 輪反覆看過)`rowspan` 實測 0 ⇒ 判定為「舊條的載體被新拍板取代」,非違規實作。截圖 4 張在 E 窗 worktree `.playwright-mcp/l2-*.png`。
