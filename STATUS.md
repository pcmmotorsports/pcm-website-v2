...
## 當前狀態
**Phase:** Phase 1 / **Milestone:** M-1(M-0 ✅ + M-1-01~12 ✅ + M-1-13a~d ✅ + M-1-13e-pre-1/2/3 ✅ + M-1-13e-a ✅ + M-1-13e-b ✅ + M-1-13e-b-2 ✅ + M-1-13f-1 ✅ + M-1-13f-2 ✅ + **M-1-13H ✅(商品頁全面改版 6 slice 完成 + Codex Review findings 4 處 fix)** + 13g(Toast 推延)+ **Stage-3-onboarding ✅ + Stage-3-codex-fix ✅(工作流升級 + Codex 4 findings 應對)** + **M-1-13I ✅(車種跨頁傳遞 3 bug 修 + V1 manifest audit、新工作流第 2 次實證)** + **M-1-13Z ✅(裝 eslint-plugin-react-hooks v7.1.1 + 開 2 規則 + 修 4 違規 + CLAUDE 校正)**、餘 M-1-14~16)
**當前 slice:** **M-1-13Z ✅**(裝 eslint-plugin-react-hooks v7.1.1〔Sean 2 次重拍:原拍 v5、Code 偵察 raise v5 退役 + 不支援 ESLint 10 → 重拍 v7.1.1 只開 rules-of-hooks + exhaustive-deps 兩條老規則、v7 新規則留 #168〕;eslint.config.js 加 React-only block〔storefront/**/*.tsx + ui/**/*.tsx〕;修 4 條既有 exhaustive-deps 違規:ProductsPage mount effect disable / ProductPage withVehicle 升 useCallback〔外部 reviewer Q2〕/ ProductInfo L54+L61 刪多餘 product.id;CLAUDE.md React hooks 段字面校正 v6 beta→v7.1.1 實況 + v7 演進路徑;backlog #168〔v7 新規則評估〕+ #169〔next-env gitignore〕;三綠 + 22 component test 綠;階段 C code-reviewer PASS〔0 must-fix〕+ 外部 reviewer FAIL 3 must-fix 已全數處置;Codex Packet 2026-05-23 已產;ahead 待 push)。前序:M-1-13I ✅(車種跨頁 3 bug + V1 manifest audit);Stage-3-codex-fix ✅;Stage-3-onboarding ✅;M-1-13H ✅ 商品頁全面改版(詳見 PROGRESS)

**Branch:** dev

## 最後更新
2026-05-23 — Claude Code [M-1-13Z 完成:裝 eslint-plugin-react-hooks v7.1.1 + 開 2 規則 + 修 4 違規 + CLAUDE 校正、三綠 + 22 test 綠、階段 C code-reviewer PASS(0 must-fix)+ 外部 reviewer 3 must-fix 已全數處置、Codex Packet 已產、ahead 待 Sean push]

## 最近 3 commit
| Hash | 訊息 | 時間 |
|---|---|---|
| `6b2835b` | chore(config): 裝 eslint-plugin-react-hooks v7.1.1 + 修 4 違規 + CLAUDE 校正 [M-1-13Z] | 2026-05-23 |
| `34ed94e` | docs(roadmap): 刷新進度地圖至 M-1-13 完成 [M-1-13I] | 2026-05-22 |
| `1d82425` | fix(storefront): 修車種狀態跨頁傳遞 3 bug + V1 manifest audit [M-1-13I] | 2026-05-22 |

## 下一步
**M-1-13Z 待 Sean push**(本 slice 純 lint infra、無肉眼驗需求;若想保險可貼 Codex Packet `docs/reviews/2026-05-23-eslint-react-hooks-install-packet.md` 再審、回「再驗」即跑外部 reviewer)。**M-1-13I 肉眼驗**(待 Sean 驗):首頁選車按搜尋 → 列表車種不丟 / 選車進商品頁 → 麵包屑回列表帶車 / vehiclePill 本體點→列表帶車、× 點→留本頁清車(雙格式 `?vehicle=yamaha:mt-07:2024` 與 `?brand=&model=&year=` 皆驗;dev 驗 tier 用 cookie `pcm-tier` 繞 env、見 #163)。**M-1-13H 收尾**(若未完成):肉眼驗商品頁完整流程 + 業務流程(加入購物車 / tier 條件渲染)+ Claude Design 端 explorations 刪除(Q6、push pcm-website-design → submodule update)。接著 **M-1-14** Customer schema(audit raise #156 店家申請 PRD + #158 MobileTabBar)/ **M-1-15** LoginPage·RegisterPage(順帶 #156 + 強推 #158)/ **M-1-16** 200 SKU 種子(#157 促銷 PRD + Supabase findBySlug + toUIProduct(p, tier) 處理 #161 經銷價 + #162 brand.country + Phase 2 supabase 6 表)。M-1 收尾跑 premortem step-2。Stage 3 收尾殘項:Cowork app 貼 bundle-docs §J(Sean、不入 repo)
