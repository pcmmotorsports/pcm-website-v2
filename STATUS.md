...
## 下一步
**M-1-16b-1 ✅(seed 品牌+分類落地、未 push)→ 16b-2 匯入腳本**:下一步 = **16b-2**〔寫 `scripts/rpm-import.ts`、唯讀讀來源〔dllwkkfanaebrsuyuedy〕`product_groups_mv`〔**過濾 supplier_slug='rpm' 933 群**;mv 混 6 供應商 7057〕+ `products` 7277 變體 → 轉換 → bulk upsert 寫商城 products+product_variants;**先單群/小量 dry-run 驗轉換、不灌全量**〕。🔴 紅線:supplier_slug='rpm' 過濾、price_general 只取 `products.price_listing` 群最低〔mv 無 listing 欄〕、brand_id=RPM CARBON〔已 seed〕、join 用當前 sku〔special 64 近期改〕、price_by_tier 兩 key、images 抽 .url 存 string[]、fitment **全群聯集去重**〔41 群分歧〕+ unconfirmed 標記〔16b-2 加 domain `FitmentSpec.unconfirmed?` optional〕、external_id=`upper('rpm-'\|\|main_sku)`/handle=rpm-lower、金額 round 禁浮點。**Q-D=A 腳本納三綠**〔新增 tsconfig.scripts.json + root lint/typecheck glob、動 root config〕。**16b-2 動 price 轉換=鐵則 12、跑 codex 關卡2**。16b plan 全文 `/tmp/m-1-16b-plan.md`。16b-3 = 全量灌〔計時例外〕。g-7 wallet HOLD #202 不變。
