...
## 下一步
**M-4a 第一期收口(①客戶線 ✅ 全完→②Email→③最新商品、④小件穿插)**:②**Email 通知片**——規劃 ✅ 完成、**Sean 已拍 S1=A 做完整版/S2=B 每出一批寄一封/S3=A 退款後不寄+認可無法保證只寄一次 → 動工 gate 全解除**。**另開視窗**依序做 E1a(migration;🔴 S2=B 連鎖=`UNIQUE(event_type,order_id)` 作廢→改 `dedup_key`)→E1b(port/adapter)→E2a(sweeper+對帳+dead-man check)→E2b(pg_cron/pg_net/Vault/連通實證)→E3(order_created、前置=親驗全部 paid transition 發生地)→E4(order_shipped、前置=偵察「一批」定義)。開工依據=`docs/handoff/2026-07-16-m4a-email-notify-plan-r2-kickoff.md`;plan v3=`pcm-tools/review-inbox/m4a-email-notify-plan.md`。③**最新商品**(storefront 例行 UI)。④小件穿插=Q3d 佔位圖/Q3a 佔位頁/Q3e 結帳內嵌地址。另:`/pcm-roadmap`+`/graphify --update` 值班台 07-16 已代跑(欠帳銷)。獨立線(未排):**LINE 補資料線**(Q1 拍板、4-5 slice、動工前 Sean 須先拍「真實 email 存哪」);搜尋 S2 lightech #275→S3 MVP;M-3 真刷卡 gate;step-up 全套另片(詳 memory `project_m4a-tier-edit-decisions`)。M-4a 收尾後開 Sean 交代「流程再優化」正式題。
