...
**下一階段=M-4a 第一期收口(Sean 07-16 拍優先序:①客戶線→②Email→③最新商品、④小件穿插)**。**客戶線五片 code 全收工**:列表/明細-a `30659b8`/明細-b `675b949`/儲值金編輯 `afea2b7` 已上 production;**第五片 tier 編輯 ✅ code 收工(本 commit)**——🔴 **migration `20260717010000` 未 apply=硬閘壓住不推**:Sean db push → 值班台驗(ACL+交易模擬清單=migration 檔尾)→ 放行代推。交接=`docs/handoff/2026-07-16-m4a-wallet-close-production-push-kickoff.md`。
**Branch:** dev
...
## 下一步
**M-4a 第一期收口(Sean 07-16 拍:①客戶線→②Email→③最新商品、④小件穿插)**:①**tier 編輯硬閘收尾**=Sean db push `20260717010000` → 值班台驗(ACL 含 §2b customers 欄級收斂+交易模擬清單=migration 檔尾)→ 放行代推(客戶線五片全上 production)。②**Email 通知片**(plan=`docs/handoff/2026-07-13-email-notification-slice-plan.md`;🔴 動工前先給 Sean 決策題「觸發點碰不碰金流 RPC」)。③**最新商品**(storefront 例行 UI)。④小件穿插=Q3d 佔位圖/Q3a 佔位頁/Q3e 結帳內嵌地址。另:`/pcm-roadmap`+`/graphify --update` 07-16 尚未跑(交接欠帳④)。獨立線(未排):搜尋 S2 lightech #275→S3 MVP;M-3 真刷卡 gate;step-up 全套另片(前置=報價單側不變 auth_time,詳 memory `project_m4a-tier-edit-decisions`)。M-4a 收尾後開 Sean 交代「流程再優化」正式題。

## Sean 待決策
①**db push `20260717010000`**(tier RPC+customers 欄級 ACL 收斂;push 後值班台驗才放行推)。②**Email 片觸發點碰不碰金流 RPC**(plan 內建議=outbox 不進訂單交易、不碰 create_order;動 Email 片前拍)。③(非阻擋)車款搜尋繼承件卡片是否併 inherited 年份=R3、日後另議。(tier Q1=A 不 step-up/Q2=A 備註必填——07-16 已拍、銷案,詳 memory。)
