...
const COUNT_QUALIFIER: Partial<Record<NavItem['key'], string>> = {
  ...
  //    🔵 **字面 2026-08-30 改回「待處理」** —— 而**這一顆是主視窗暫定的,不是 Sean 的字**:
  //      他拍的是「數什麼」(甲),**沒有拍「叫什麼」**。放成單獨一顆 commit,是為了他要改字時
  //      不必動別的東西。⇒ **要改這個字不需要回頭問這一片。**
  //      📌 **「這個字的理由不成立了」與「這個字可以改了」是兩件事** —— 上一顆 commit 只做了前者。
  // 🔴 **「待處理」→「卡住」(2026-08-22,Sean 拍板乙)** —— 那顆數字**沒有說謊,說謊的是這個字**。
  //    ⛔ ~~而那個理由(②那半永遠不會自己離開清單)2026-08-30 起不成立了 —— 見上面 🔵 段。~~
  //    ⇒ 理由不成立之後,那個字回到「待處理」—— 它現在數的**就是**待處理。
  ...
  const inner = (
    <>
      ...
      {/*
        ...
        卡住    文字 26.00   容器 73   高 16.25 = 1 行     ← 2026-08-22 實量(字面已於 08-30 改)
        待處理  文字 39.00(推)容器 73   高 —          ← 🔵 2026-08-30 新字面
          ⚠️ **這一格是【推的】不是量的**:本表自己量到三格全部落在 13.0 px/字
             (未訂貨 39=3×13、卡住 26=2×13、缺貨 26=2×13)⇒ 3 字 = 39,與「未訂貨」同寬,
             而「未訂貨」是**實量過** ≤73 且不折行的。⇒ 同寬 ⇒ 同樣塞得下。
             🔴 **但它終究沒有被真的量過** —— 要當結論的人自己開瀏覽器跑一次下面那個量法。
