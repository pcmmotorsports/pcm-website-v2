...
describe('AppSidebar 導覽項', () => {
  it('should list every nav entry exactly once, including the supplier settings page', () => {
    ...
    // 🔴 2026-09-13 晚 Sean 答甲:11 → 6(軌上 5 + 設定群組 4 + 稽核)。**順序 = 軌上 → 群組**;
    //    這一格改期望值的理由:清單的【內容】被他改了,不是守門放寬 —— 它仍然釘「每一條恰好一次、字面形狀一致」。
    expect(navEntries()).toEqual([
      ['總覽', '/'],
      ['訂單', '/orders'],
      // ⛔ ~~['退款異常', '/orders/refund-exceptions']~~ 2026-09-13 晚 Sean 答甲:退款異常不再是側欄入口
      //    (計數搬到總覽、頁面仍在)。這一行不是漏了,是他推翻的;做回來就是違反那一拍。
      ['出貨清單', '/shipments'],
      ['客戶', '/customers'],
      // M-4b #20 片1a:商品列表(唯讀)已接上頁面 ⇒ 這一格從此可點(plan 驗收 6)。
      ['商品', '/products'],
      // 🔴 2026-09-16 首頁大圖:軌上 5 → 6(Sean Q7 乙 放主側欄)。改期望值的理由:清單【內容】多了一條可點的,不是守門放寬。
      ['首頁大圖', '/home-banners'],
      // M-4b 券片 2b-2:券列表(唯讀)已接上頁面 ⇒ 這一格可點。
      // 🔴 本行是**跟著 `nav-items.ts` 一起改的** —— 檔內 :97 逐字寫著
      //    「日後加 nav 要同步改本行 —— 那本來就該是一個有意識的動作」。
      //    ⚠️ 而我 2026-08-29 加那一項時【沒有同步】⇒ 這支與隔壁那支 rail 一起紅在 dev 上,
      //       是線A 重跑三綠時發現的。**我自己那一發只跑了我新建的三支測試檔。**
      //    📌 而那正是鐵則 11 那句的實體:**分母由「誰碰得到這段碼」決定, 不是由「我做了什麼」決定。**
      ['員工管理', '/settings/staff'],
      ['供應商', '/settings/suppliers'],
      // 🔴 2026-09-13 晚:優惠券收進「設定」群組(唯讀頁、一週改 0 次 ⇒ 鐵則 9 判準下是設定)⇒ 順序跟著群組走。
      ['優惠券', '/coupons'],
      ...
      // 🔴 2026-09-13 匯率:設定群組 4 → 5(plan 2026-09-13-fx-rate-settings-plan.md,主視窗裁獨立一頁)。
      //    改期望值的理由:清單【內容】多了一條可點的,不是守門放寬;本檔仍釘「每一條恰好一次」。
      ['匯率', '/settings/fx'],
      // 🔴 2026-09-15 事故紀錄(稽核 P2-7):設定群組 5 → 6。改期望值的理由同上一行:清單【內容】多了一條可點的,不是守門放寬。
      ['事故紀錄', '/settings/incidents'],
...
describe('#350a 側欄寬度(Sean:窄到跟文字對齊、放大訂單空間)', () => {
  /**
   * 🔴🔴 **2026-09-13:這一格原本在守一個【死旋鈕】,而它是恆綠的。**
   *
   * 原本釘的是 `ui/sidebar.tsx` 的 `SIDEBAR_WIDTH`(`9rem` = 144px)。
   * **而那支檔已經不在渲染樹上** —— `app-sidebar.tsx:72` 記著為什麼不再用 shadcn 的 `<Sidebar>`;
   * 真正在畫的是 `app-sidebar.tsx:229` 一個寫死的 `w-[84px]`。
   * ⇒ **畫面上是 84,而這一格說 144,兩個數字同時「正確」。**
   *
   * 🔬 **怎麼抓到的**(值得留,因為它不是任何一道綠抓到的):
   * 主視窗 2026-09-13 派「側欄 144 → 92」,我改了 `SIDEBAR_WIDTH`、改了這一格的期望值、
   * 三綠全綠 ⇒ **然後 build 完開真瀏覽器量,量到 84,CSS 變數確實是 5.75rem。**
   * **變數改到了,畫面沒動。**
   * 📌 **⇒ 抓到它的只有「做完打開瀏覽器按一下」。**
   *
   * 🔴 **而這是同一支檔的第二次** —— `app-sidebar.tsx:215-228` 記著 2026-08-20 那次逐字:
   * 「**元件被換掉,規則留在被淘汰的那一支**…而守門是恆綠的:原本釘的是『那支檔裡有那個字面』
   * —— 那支檔還在、字面還在、**而它已經不在渲染樹上**。」
   * ⇒ 📌 **同一個形狀在同一支檔出現兩次,而第二次是照著第一次的修法之外的那條路再走一遍。**
   *
   * ✅ **修法(主視窗 2026-09-13 裁):改成釘【真正在渲染的那個值】,不要刪。**
   *    刪掉 = 少一道守門;留著舊的 = **一道恆綠的守門**,而後者更糟 —— 它讓人以為有人在守。
   * ⚠️ `SIDEBAR_WIDTH` 家族**零活消費端**(全 repo grep,只剩測試與註解在提它)
   *    ⇒ 它是死碼。**而 `ui/sidebar.tsx` 是 fork 進來的 vendor 檔,刪它是另一件事** ⇒ 只標註。
   */
  it('🔴 桌機側欄寬 = 84px —— **釘的是真正在渲染的那一支**(`app-sidebar.tsx` 的 `w-[84px]`)', () => {
    // 🔴 抽的是**渲染樹上那支檔**的字面。抽不到 ⇒ 回 null ⇒ 紅(不是靜默跳過)。
    const m = /className='[^']*\bw-\[(\d+)px\]/.exec(SOURCE);
    expect(m, '抽不到 <aside> 的寬度字面 —— 是不是改用別的寫法了?那這一格要跟著搬').not.toBeNull();
    expect(Number(m![1])).toBe(84);
  });

  it('⚠️ `SIDEBAR_WIDTH`(死旋鈕)仍然存在,而它**不控制畫面** —— 這一格只是標註,不是要求', () => {
    // 🔴 **刻意寫成「它還在」而不是「它等於幾」**:釘住值等於復活那道恆綠守門。
    //    這一格的用途是:有人哪天把 `ui/sidebar.tsx` 刪掉或改名時,**這裡會紅一次**,
    //    而他會讀到上面那段、知道它本來就不控制畫面 ⇒ **紅的意思是「來讀這段」,不是「你弄壞了」。**
