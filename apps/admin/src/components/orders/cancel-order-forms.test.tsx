...
describe('A13b E1 ①挑數量', () => {
  it('🔴 可取消量 > 1 ⇒ 有數量欄、預設 = 可取消量', () => {
    const { container } = render(
      <PartialCancelForm orderId={ORDER_ID} items={[itemView({ maxCancellable: 3 })]} />,
    );
  ...
  it('🔴 可取消量 === 1 ⇒ **沒有**數量欄(不是渲染一個永遠填 1 的欄位)', () => {
    const { container } = render(
      <PartialCancelForm orderId={ORDER_ID} items={[itemView({ maxCancellable: 1 })]} />,
    );
    expect(
  ...
  it('🔴 送出去的 FormData:checkbox 帶 `<id>:<max>`、數量欄另外一格', () => {
    const { container } = render(
      <PartialCancelForm orderId={ORDER_ID} items={[itemView({ maxCancellable: 5 })]} />,
    );
...
describe('A13b E1 ②說明欄只有選「其他」才在畫面上', () => {
  ...
  it('🔴 零 JS(server 靜態輸出)⇒ textarea 恆在、且不帶 required', () => {
    ...
    const html = renderToStaticMarkup(<FullCancelForm orderId={ORDER_ID} />);
