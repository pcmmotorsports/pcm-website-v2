...
export function ItemAmountRow({
   ...
   *
   * ─────────────────────────────────────────────────────────────────────
   * 🔴🔴 **`variant='table-row'` 那個舊殼今天是【死碼】,而我們刻意留著它。**
   *
   * 2026-08-19 W3 當場量:
   * ```
   * grep -rn "variant='table-row'" apps/admin/src   ⇒ 0
   * 生產端唯一呼叫點 order-detail-items-table.tsx    ⇒ 帶的是 variant='card-line'
   * ```
   * **不刪的理由是量出來的,不是懶**(主視窗 2026-08-19 裁定,用 W3 的理由):
   * `item-amount-row.test.tsx` 有 **24 行**掛在那個表格殼上(`colSpan` / `<tr>` / `<td>`),
   * 而那幾格守的是 `blockedReason` ——「**已收款 ⇒ 不得改金額**」那道**錢的閘**。
   * ⇒ **重寫【守錢的測試】的風險,大於刪一段不會被執行的死碼的收益。**
   * 📌 判準可搬走:**「刪死碼」預設被當成零風險,而風險不在被刪的東西上,
   *    在【為了刪它而必須動的東西】上。**
   *
   * 🔴 **而它不是永遠不做 —— 觸發條件寫成可判斷的**(不寫「未來可考慮」):
   * > **哪天有人要動 `blockedReason` 那道閘、或發現兩個殼的行為已經漂了
   * > ⇒ 那時順手把 `table-row` 殼刪掉,因為【那時測試本來就要重寫】。**
   * ⚠️ **這句刻意寫在這裡、不開 backlog** —— 它要出現在**動那道閘的人**眼前,
   *    而不是躺在一張沒有人在讀的清單裡。
   * ─────────────────────────────────────────────────────────────────────
   */
  const trigger = (
    ...
    <button
      ...
      onClick={(e) => {
         ...
         * ⚠️ **這句字面過強,下面六行才是完整版**(片C 審查時 W5 引了這句就標 must-fix、
         *    被 W2 真瀏覽器三種點法量到「沒發生」推翻,兩人一起查出來的):它成立的前提是
         *    **`<button>`**——`<input type="checkbox">` 是另一個可啟動元素,activation 跑在
         *    離 target 最近的那一個,不會沿路再跑 `<summary>` 的 toggle。這句只對本檔這顆
         *    button 成立,不能套用到別的可啟動元素上。
