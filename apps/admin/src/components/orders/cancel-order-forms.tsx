          ...
          {pendingRefund.kind === 'unknown' ? (
            ...
            <>
              {/* 🔴🔴 **句子拆成【事實】與【後果】兩句, 而只有後果那句跟 `mode` 走。**
                  ...
                  —— 部分取消不寫 `cancelled_at` ⇒ `20260901080000` 那個 trigger 不會醒。
                  ⛔ ~~⇒ **不會開任何待退款**~~(2026-09-05 的事實)
                  ✅ 2026-09-14 起 `20260914070000` `pcm_partial_cancel_recompute` **會開**:
                     非卡淨收 − 剩餘應收(Sean `Q13`)> 0 才開;有刷卡收款 / 稅算不出不開。
                     舊字面「部分取消不會自動列待退款,請自行處理。」因此變成假話,員工照做會退兩次(2026-09-15 走查 路 3 實測)。
                  🛑 新字面**不承諾**「登記退款後待退款會被關掉」—— 今天沒有任何寫入端會設 `settled_at`(走查 路 3 問題 H,待 Sean 拍)。
                  ⇒ 📌 **事實對兩種模式都成立, 後果不同 —— 所以只有後果那句分岔。** */}
              {/* 🔴🔴 **Sean 2026-09-05 逐字:「甲:但是更簡短 精簡(我員工不是小孩子他看得懂)」**
                  ⛔ ~~兩句 + 解釋~~ ⇒ ✅ **一句 + 逐軌括號**。
                  🛑 **而「短」不等於「少講一件事」**:部分取消那句講的是**不一樣的事**
                     (它要看剩下的金額才決定開不開;後台建的含稅單 `pcm_order_remaining_receivable` 回 NULL ⇒ 不開、只在退款異常頁報 tax_uncomputable;
                      有刷卡收款的單根本取消不了(`20260914050000:541-545`)⇒ 字面不提刷卡 —— 審查 R2 F2 / N1)⇒ 它跟著縮, 但**不能併進整單那句**。
