  ...
  return (
    <div ref={hostRef} id={noticeId}>
      <div
        ...
        data-cutoff-notice=''
      >
        {/* 🔴 **這句話的射程要講準**(2026-09-10):`cutColumns` 數的是【被推到右邊看不到】的欄,
            那一種【捲動真的看得到】。而另有一種是【字被「…」切在格子裡】(今天是客戶 / 料號 /
            物品名稱)—— 那一種**捲到底也不會出現**, 這個數字從來沒有把它算進去。
            ⚠️ 舊版只說「可以左右捲動, 或把左邊的側邊欄收起來」⇒ Sean 照做了(截圖為證), 而字還是缺。
            📌 **一句提示如果講的方法解決不了它描述的問題, 它比沒有更糟。**
            🛑 **而「沒有完整顯示」這五個字不准換掉** —— `orders-cutoff-notice.test.tsx:113` 是一條
               明文拍板:半切的欄講成「看不到」是**說過頭**。我第一版寫成「要捲才看得到」,
               當場被那條守門擋下 ⇒ 📌 **它擋對了, 而我是在加東西的時候順手把別人的裁決改掉。** */}
        → 右邊還有 {cutColumns.length} 欄沒有完整顯示:
        <span className='font-medium'>{cutColumns.join('、')}</span>。 這一區可以左右捲動,或把左邊的側邊欄收起來。
        另外有些格子的字太長,會用「…」縮短;那一種捲動也不會變完整,要點進那張單看。
      </div>
    </div>
  );
}
