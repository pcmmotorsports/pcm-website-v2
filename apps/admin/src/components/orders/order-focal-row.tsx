...
export const QTY_MISSING_NOTE = {
  ...
  // 🔴🔴 **2026-09-04 訂貨→到貨走查:⛔ ~~「下訂之後這裡就會出現數字」~~ 會叫做對事的人去報修。**
  //    🔬 走查實測(admin-probe, 真瀏覽器):一張兩品項的單, 我訂了其中一項
  //       ⇒ **那一項的卡片立刻出現 `1/1`** ✅ 而**這一格仍然印「未知」**。
  //    🔬 而那是【對的】—— `goodsQuantityHeadline`(`lib/orders/order-status-axes.ts:495`)逐字:
  //       `if (lines.some((l) => l.quantitySummary === null)) return null;`
  //       ⇒ 🎯 **⇒ 只要【任何一項】還沒有數量摘要, 整格就是 null。這一格是【整張單】的合計。**
  //    ⇒ 🛑 **⇒ 所以舊句子對「只訂了一部分」那個世界是【假的】** ——
  //       而它下一句正好是「數字卻一直沒出現 ⇒ 那時才需要通知系統維護」
  //       ⇒ ⇒ 📌 **⇒ 一個做對事的人被指去報修, 而成本落在【去查那個不存在的 bug 的人】身上。**
