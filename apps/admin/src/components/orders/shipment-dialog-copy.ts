...
//
// 🔴🔴 **2026-09-17:本檔的 `staleDeploymentMessage()` 有 8 處 Markdown `**`, 而員工真的看得到。**
//    ⛔ 逐字如 ~~「請**重新整理頁面**」~~ —— 而這一串走的是
//    `shipment-dialog.tsx:377` `setResult({ …, code: null })`
//    ⇒ `parseShipmentError(null, …)` 回 `{ copy: null }`(`shipment-error-view.ts:93`)
//    ⇒ `shipment-dialog.tsx:681` `return <p>{result.message}</p>` ⇒ **純文字**
//    ⇒ 📌 **星號原樣印在彈窗上, 而那正好是「部署中途換版」那個本來就夠亂的時刻。**
// ✅ 改用「」括號 —— 這條路**沒有 `<strong>` 可用**(整段是一個字串塞進 `<p>`),
//    而「」在中文裡本來就是強調的寫法。🛑 **一個字都沒刪, 只換強調符號。**
// ⚠️ **這一族今天第四次了**(前三次:cancel-result-panel / audit page / 本檔)——
//    而前三次的守門都只掃它自己那幾個常數 ⇒ 換一支檔就沒有閘。
//    ⇒ 🎯 射程放到整個 admin 的那道閘在 `scripts/admin-copy-no-markdown-stars.test.ts`。
//
...
export function emptySelectionMessage(candidates: readonly ShipmentCandidateItem[]): string {
  ...
  return nothingShippable && anyAwaiting
    ? '這些品項目前都無法出貨，其中有品項尚未到貨。收到商品後，請展開「更多」，在對應品項旁按「到貨登記」，完成後即可選擇出貨數量。'
...
export function staleDeploymentMessage(maybeCreatedBox: boolean): string {
  ...
  const escape = '（若重新整理後仍出現此訊息，請截圖回報，暫勿繼續重試。）';
  return maybeCreatedBox
    ? '目前頁面無法繼續送出，請重新整理。剛才這次操作未建立包裹，' +
        '但先前的送出可能已建箱，只是尚未收到回應，或尚未完成加入品項、標記出貨。' +
        '重新整理後，請勿直接為同一批商品再次建箱，以免重複。' +
        // 🔴 兩個地方都要指(R3 F3):有品項的箱在**出貨卡**上;
        //    沒有品項的空箱**只**出現在「未收尾的空箱」區,出貨卡是由品項反查箱畫的、看不到它。
        //    只指其中一邊,員工在另一型的情況下會找不到而卡住。
        '請先到訂單頁的出貨卡查看是否已有這批商品的包裹；若沒有，再查看下方「未收尾的空箱」。' +
        '確認包裹後，再決定繼續加入品項或將包裹作廢。' +
        escape
    : '目前頁面無法繼續送出。這次操作未建立任何包裹，' +
        '請重新整理頁面後再試，不會因此重複建箱。' +
        escape;
}
