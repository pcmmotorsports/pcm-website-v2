...
//
// ✅✅ **而上面那四格【不是「已知限制」】—— 有人看過它們並選了這一邊, 而那個人是 Sean。**
//    📎 `~/pcm-mailbox/等Sean拍的題-20260903.md:1971`(2026-09-04 01:4x),他逐字:
//       **「甲 夠 —— 先擋住那四種, 之後有需要再說」**
//    🔴 而他是在【知道前兩格】之後選的 —— 主視窗把「面板內導航完全不觸發」與
//       「同上帶進來的姓名電話結構上永遠不會被判髒」兩件原字端給他, 他看過才答。
//    🛑 **⇒ 所以不要把這一節讀成「還沒做完」** —— 它是**被決定成這樣的**。
//       要擴大射程 ⇒ 那是一次新的拍板, 不是補完一件半成品。
//
// 🔵 **鐵則 8(裁決者:主視窗 -0,2026-09-04)**:本片 3 支檔逐字命中「跨 3+ 檔」,
//    而**主視窗沒有為這一片提過 plan**(Sean 拍的是 Q32 甲那個【選項】)。
//    ⇒ 主視窗 -0 裁**不補提 plan**,理由:3 支檔 · 零 schema/API/共用元件/config/部署 ·
//      送出契約檔 `manual-order-form.ts` 零改動 · 本元件 `render null` 不進 FormData。
//    🎯 **⇒ 這句裁決有名字, 不是一句無主的判斷。**
//
// 📌 **而 `9ef97219` 那顆 commit 的 body 說的是【那一刻的真相】**(它逐字寫
//    「這是我的判斷不是已獲批准」—— 而當時確實還沒有這一板)。**本段是訂正, 不是取代。**
//    ⇒ 🛑 兩者合起來才完整;**單讀任一邊都會少一半。**
