...
export const VERDICT_TEXT: Record<CancelLedgerVerdict, { title: string; hint: string; tone: PanelTone }> = {
  unreadable: {
    ...
    hint: '目前無法確認是否已送出。請重新整理本單確認；若仍無法讀取，請通知系統維護，先不要重送。',
    tone: 'error',
  },
  match_same_actor: {
    title: '已找到本次取消紀錄',
    hint: '本次取消已登記，無須再次送出。請展開下方「影響範圍與取消紀錄」查看明細。',
    tone: 'ok',
  },
  match_other_actor: {
    title: '已找到相符的取消紀錄，但無法確認由你登記',
    ...
    hint: '可能由同事登記，或目前無法辨識你的身分。請先與同事確認，不要直接再次送出。右上角選單的選擇不一定已生效；請查看後台首頁「具名身分」的說明。',
    tone: 'warn',
  },
  miss_truncated: {
    title: '無法確認本次取消是否已登記',
    hint: '目前只顯示最近的取消紀錄，本次紀錄可能未列出。請通知系統維護協助確認，先不要重送。',
    tone: 'error',
  },
  miss_complete: {
    title: '目前查不到這筆取消',
    hint: '目前未找到本次取消紀錄。請重新整理本單再確認一次；仍未找到時，才重新送出一次。',
    tone: 'warn',
  },
};
