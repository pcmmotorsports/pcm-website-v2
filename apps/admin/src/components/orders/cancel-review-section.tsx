...
> = {
  ...
  payment_refunded: {
    title: '這張單的錢已經退回去了',
    hint: '款項已經退還,不用再退一次。取消鈕這期還不能用(要等退款線開通),但這張單的錢已經結清 —— 現在不需要為它做任何事。',
  },
  payment_partially_refunded: {
    title: '這張單退了一部分款',
    hint: '還有一部分款留在這張單上,跟「已全退」不一樣、不能當結案處理。請先確認剩下的金額要退還是要收,再決定怎麼處置;這期不能在這裡按取消。',
  },
  payment_partially_paid: {
    title: '這張單只收到一部分款',
    hint: '收了一部分的單不能直接取消(取消要連著退款一起做,退款線之後才開通)。請先確認尾款是要收齊還是要退,再走人工流程。',
  },
