...
> = {
  ...
  payment_not_unpaid: {
    ...
    hint: '請先到本頁上方「退款」確認退款金額與方式，完成退款後再取消訂單。',
  },
  ...
  //    ⇒ 照本檔上面那條紀律改成**指位置**:「本頁上方的『退款』」(字面在 order-detail.tsx 那一塊)。
  //    🔴 走查 0915 問題 C:原本寫「本頁最下方」,而收款 · 退款分頁的順序是 收款 → 退款 → 申請取消(取消區在最下面)
  //       ⇒ 員工往下捲找不到退款。改成「上方」。
  //    📌 值得記:**我整晚在別處講「訊息不得指向不存在的動作」,而寫這一句時自己又犯了** ——
  //       而這次抓到我的不是人,是這張表的守門。
  payment_card_rail: {
    title: '這張單是刷卡付款的,要先把錢退回原卡片才能取消',
    hint: '刷卡的錢不能只把訂單取消掉就算數,要退回原本那張卡。請到本頁上方的「退款」裡處理;錢退完之後這張單才能取消。現金或匯款收的單則可以直接在這裡取消。',
  },
  ...
  payment_rail_unverifiable: {
    ...
    hint: '目前無法取得收款明細，可能讀取失敗或尚無紀錄，因此無法判斷收款方式及是否可取消。請重新整理一次；若仍相同，請通知系統維護。',
  },
  // 🔴 `#494` 的三條:**下一步各不相同**,所以不共用上面那句(Sean 2026-08-14 拍板 B)。
  //    起因是 Sean 對著一張**已經退過款**的單,看到上面那句「已付款…請走人工退款流程」。
  // 🔴🔴 **2026-09-05 Sean 拍甲改掉這一格**(⟦0a-CARDCANCELNOREFUND⟧ 片②):
  //    ⛔ ~~'這張單的錢已經結清 —— 現在不需要為它做任何事。'~~
  //    🔴 **那句話是錯的, 而它錯了三個星期**:那張單**還沒取消, 而且取消不了**
  //       (`admin_cancel_order` 的述詞要 `payment_status = 'paid'`, 而這裡是 `refunded`)。
  //       ⇒ 📌 **員工照那句話走, 會讓一張退完錢的單永遠掛在未取消, 而畫面告訴他這是對的。**
  //    ⚠️ **而它當時是對的** —— 2026-08-14 那一刻, 取消鈕還沒有路可走。
  //       ⇒ 🎯 **它不是錯, 是過期。而過期的文案與寫錯的文案, 在畫面上長得一樣。**
  //    🔵 新句子裡的「按下面的」**指向一個真的存在的東西**(同檔 `MarkCancelledForm`)——
  //       這一格的檔頭紀律逐字寫著「訊息不得指向不存在的動作」, 而我差點又犯一次。
  ...
  payment_refunded: {
    ...
    hint:
      // 🔴 **這裡不可以寫 Markdown 的 `**`** —— JSX 直接輸出純字串,
      //    員工會看到兩組實體星號。codex 2026-09-05 抓到, 而**我在同一份檔案裡
      //    到處寫 `**` 當強調**(那是給讀碼的人看的註解語法), 一不小心就寫進了對客字串。
  ...
  payment_partially_refunded: {
    title: '這張單退了一部分款',
    hint: '還有一部分款留在這張單上,跟「已全退」不一樣、不能當結案處理。這張單目前不能在這裡取消:剩下的金額要退,用上方的「退款」;要收,用最上面的「收款」。',
  },
  payment_partially_paid: {
    title: '這張單只收到一部分款',
    hint: '收了一部分款的單目前不能在這裡取消。尾款要收齊,用最上面的「收款」;要退,用上方的「退款」。',
  },
  ...
  charge_attempt_blocked: {
    ...
    title: '這張單有一筆刷卡還沒有結束',
    hint: '請等待這筆刷卡交易確認成功或失敗後，再取消訂單。重新整理不會改變交易結果；若持續未完成，請到 TapPay 後台查詢。',
  },
  charge_attempt_stuck: {
    ...
    // 🔴🔴 **2026-08-22 修正:上一版寫「不要坐著等它自己變」,而那句話【不準】。**
    //   當時我把 `CRON_SWEEPER_ENABLED` 標成「量不到、兩個世界都可能」,所以刻意寫成兩邊都為真。
    //   **Sean 2026-08-22 去 Vercel 看了,逐字回報 `CRON_SWEEPER_ENABLED : true`**
    //   ⇒ 那個世界收斂成一個:B1a 低頻再確認**是活的**,被舉手的單 TapPay 一回 paid 就會自己變。
    //   ⇒ 「不要坐著等」會讓員工**多跑一趟其實可能不必跑的**,而那正是這一族要防的**反面**
    //      （上一版防的是「叫他等一件不會來的事」，這一版要防「叫他別等一件可能會來的事」）。
    //   ⇒ 新寫法**兩邊都不替他決定**:講清楚「可能會自己好、但不保證」，把決定權連同查法一起給他。
    //   📌 改字前跑過 `bash scripts/literal-sweep.sh '不要坐著等它自己變'` ⇒ **總計 1 命中**（只有本檔)
    //      ⇒ 沒有任何測試在斷言那句話。而 `停止自動重試` 有 **5 命中、其中 3 條是斷言** ⇒ **那五個字不動。**
    hint: '系統試到上限就停了,之後只剩每隔幾小時一次的低頻再確認 —— 它有可能自己變成已付款,但不保證,而這張單已經卡很久了。最快的做法是去 TapPay 後台查那一筆的實際狀態,查到再決定要不要等。這張單目前在後台還不能取消、也還不能退款。',
  },
  charge_attempt_unknown: {
    title: '付款狀態沒有讀完整',
    hint: '刷卡紀錄未完整載入，無法確認是否仍有交易進行中。請重新整理；若仍相同，請通知系統維護。',
  },
  items_truncated: {
  ...
  quantity_summary_missing: {
    ...
    hint: '採購或取消數量不一致，或採購紀錄未完整載入。請通知系統維護，確認前不要重複送出取消。',
  },
  ledger_unhealthy: {
    // 🔴 三種病理共用一碼(`cancel-view.ts` 的 `ledger_unhealthy` docstring),文案不可只描述其中一種
    //    ——員工會照著向維護回報錯的症狀(R1 F5)。
    title: '取消紀錄異常',
    hint: '取消紀錄有異常，可能為數量不一致、缺少品項或訂單關閉資料異常。請通知系統維護，不要重試。',
  },
  cancellations_unreadable: {
    title: '取消紀錄沒有讀完整',
    hint: '取消紀錄未完整載入，無法確認已取消數量。請重新整理；若仍相同，請通知系統維護。',
  },
  nothing_cancellable: {
    ...
    //    🔵 2026-09-27 退貨收回第 2 片:退貨區塊已上線, 而它只收【已出貨】的品項 ⇒ 句子只指向出貨後那一段,
    //       已到貨、還沒出貨的部分維持「不能在這裡取消」, 不假裝有路。
    hint: '下表每個品項的「還能取消」都是 0。已到貨的部分不能在這裡取消;已出貨的品項可以在「收款 · 退款」分頁的「退貨」區塊登記退貨。若你確定還有沒到貨的數量,請重新整理,仍相同就通知系統維護。',
  },
};
...
export function CancelReviewSection({
  ...
  return (
    <section className={CARD}>
      ...
      {/*
        🔴🔴 **第二條路的按鈕**(Sean 2026-09-05 拍甲 · ⟦0a-CARDCANCELNOREFUND⟧ 片②)。
        上面 `payment_refunded` 那句話說「請按下面的『把這張單結掉』」——
        **這就是那個「下面」**。🛑 本檔的檔頭紀律逐字寫著「訊息不得指向不存在的動作」,
        而那一句與這顆鈕**必須成對存在**:少了鈕, 那句話就變成一個做不到的指示。
      */}
      {/*
        🔴🔴 **那顆「把這張單結掉」的鈕【不在本檔】, 而那不是風格** ——
        它要 import server action, 而**本檔走 client 路徑**(被 `order-cancel-block` 收進去)
        ⇒ import 的當下就紅:`This module cannot be imported from a Client Component module`。
        📌 **是一道守門告訴我這件事的, 不是我先查清楚的。**
        ⇒ 鈕在 `order-cancel-block.tsx`(server), **緊接在本區塊之後渲染**
           —— 上面那句「請按下面的」指的就是它, 兩者的順序是那句話的一部分。
      */}
