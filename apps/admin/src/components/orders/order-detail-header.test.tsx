...
//       (該檔 `:288-296` 就是一個 3 行的 `render(<OrderDetail shipmentWarning={NO_SHIPMENT} …/>)`,而且自帶正向對照)
...
describe('片2 標頭列 · render 層', () => {
  ...
  it('🔴 chip 畫出來的是【既有標籤表的字】,不是設計稿那個「未收齊」', () => {
    const { container } = render(
      <OrderDetail shipmentWarning={NO_SHIPMENT} refundsTruncated={false} stuckVerdicts={new Map()} detail={HEAD_DETAIL} returnTo='/orders' payments={{ status: 'ok', rows: [] }} />,
    );
