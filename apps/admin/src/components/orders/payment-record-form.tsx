...
export function PaymentRecordForm({
   ...
   *
   * 🔴🔴 **未驗前提(片2a code-reviewer Important 3)**:這顆鈕釘住的是**當下的 `stamp` prop**,
   *    而「按下去真的拿得到一把**新**鍵」繫於**失敗路徑的 revalidate 有沒有讓 server 重新鑄章**。
   *    `payment-actions.ts:123` 確實在失敗路徑呼叫 `revalidateOrderViews`,但那條路
   *    **本 repo 沒有端到端跑過** —— 單元測試是用 `rerender` 換 prop **假設**它成立。
   *    ⇒ 在 OP5 真往返煙測拿出「失敗 → 開始下一筆 → 送出,兩次 `request_id` 不同」那一格之前,
   *      本段一律標**假設**,不得寫成已驗。
   *    ⓘ 失敗形狀落在 fail-safe 那一邊:沒換到新鍵 ⇒ 第二筆被 G8 當成重送而**合併**
   *      (員工看到「先前已登錄過」),不是雙重入帳。
   */
