...
describe('V1 — 表頭欄數與內容欄數一致', () => {
  ...
  it('表頭恰為 14 欄,且欄名與期望一致(P7:收款欄加回來 ⇒ 13 → 14)', () => {
  ...
  it('單品項單:該列 <td> 數 = 14(訂單層與品項層都在同一列)', () => {
    ...
    // P7:+1 收款欄(訂單層); P5:−1 發票欄(變客戶格的第三層 tag); P3:−1 單號欄(併進日期格);
  ...
  it('🔴 L2 收斂後:三品項單的**每一列**都是 14 格(訂單層欄在第二列之後渲染成空格)', () => {
    ...
    // P7:+1 收款欄; P5:−1 發票欄(變客戶格的 tag); P3:−1 單號欄(併進日期格); P4:+1 來源欄
...
describe('V5 — 空 lines', () => {
  it('lines 為空仍渲染一列佔位、訂單層格不消失', () => {
    ...
    // P7:+1 收款欄; P5:−1 發票欄(變客戶格的第三層 tag); P3:−1 單號欄(併進日期格);
...
describe('L2 — 手機卡片模式的 DOM 契約(卡片化由 CSS 做,本區守 CSS 靠什麼認得欄位)', () => {
  ...
  describe('L2 — `globals.css` 卡片化區塊(用 postcss 走 AST,不用字串比對)', () => {
    ...
    const CARD_ORDER_TIED_GROUP = {
      ...
      //    ⇒ 這一組從五欄變四欄。
      // 🆕 **`col-pay` 於 P7(2026-09-13)加入** ⇒ 四欄 → **五欄**。globals.css 那條規則已同步。
      cols: ['col-unit', 'col-vehicle', 'col-brand', 'col-source', 'col-pay'],
    } as const;
...
describe('L3 片4 — 密度掛勾(`data-den`)', () => {
  ...
  it('🔴 CSS 三檔選擇器都在,而且密度值只住 globals.css(不得有第二份)', () => {
    ...
    // 🏁 **2026-08-22 Sean 拍板:「寬鬆、標準、緊湊功能就保持寬鬆吧」⇒ 切換拿掉、固定寬鬆。**
    //    (2026-08-23 線A 落地;OD 改版稿 `FIX-25 密度切換固定為「寬鬆」` 同一條。)
    //
    // 🔴🔴 **本格【翻面了】:原本要求兩檔選擇器【在】,現在要求它們【不在】。**
    //    ⚠️ 上面那整段推理(選擇器靜靜消失 = 密度切換失效而不紅)**仍然成立、刻意留著** ——
    //       它是「這一格為什麼要讀 CSS」的說明書。**被推翻的是要守哪一邊,不是那條推理。**
    //
    // 🔴 **為什麼是「規則刪掉」而不是「只拿掉按鈕」**:`orders-table.tsx` 仍吐 `data-den={density}`,
    //    而 `[data-den='…']` 特異性 (0,2,0) 永遠贏過裸 `.orders-grid` (0,1,0)
    //    ⇒ 只拿掉按鈕的話,**舊書籤帶 `?den=tight` 進來還是會變 26px,而畫面上沒有開關可以調回去**。
    for (const den of ['std', 'tight']) {
      expect(
        css,
        `${den} 那檔的選擇器又回來了 ⇒ 2026-08-22 的「固定寬鬆」被推翻,要重裁不是靜靜加回去`,
      ).not.toContain(`.orders-grid[data-den='${den}']`);
    }

    // 從 CSS 抽出**當下**的列高值(不是重打)
    ...
    // ⚠️ **鎖成寬鬆之後,這個數字從 3 變成 1** —— 而它仍然是分母:
    //    抽到 0 ⇒ 下面的迴圈不跑;抽到 >1 ⇒ 密度值又有第二份了(2026-08-23 移植時真的發生過,
    //    照文件順序搬 OD 那批 ⇒ 一口氣變成 11 份,**就是這一格抓到的**)。
    expect(rowHeights.length, '`--od-row-h` 不是恰好一份 ⇒ 要嘛抽不到(本格失去判別力), 要嘛密度值又分裂了').toBe(1);
