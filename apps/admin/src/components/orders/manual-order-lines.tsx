  ...
  return (
    <fieldset className='space-y-2 rounded-md border p-3' data-testid='manual-order-lines'>
      ...
      {/* 🔵 2026-09-14 Sean 看彈窗:「這些文字可以精簡扼要嗎」⇒ 原本三段(代購留白 / 單價填未稅 / 🔵 含稅切右邊 / 🛑 不切會多課稅)
          ...
          「不切會多課一次稅」那句拿掉 —— 它是「做錯會怎樣」, 而稅別那顆下拉現在看得到「未稅 / 含稅」兩個字, 選錯是看得見的。
          ⛔ ~~網站上沒有的東西(代購)就把「商品編號」留白,品名跟金額自己打。料號每一列都要填。~~
          ⛔ ~~單價這一格請填未稅金額 —— 經銷價單上多少就填多少。稅由系統自己算,不要自己加上去。~~
          ⛔ ~~🔵 手上只有含稅價?把右邊那格切成含稅,系統會自己換算回未稅。🛑 而不切就直接填含稅數字 ⇒ 勾了開發票會多課一次稅。~~ */}
      ...
      {rows.map((row, index) => (
        <div key={row.id} className='grid grid-cols-1 gap-2 sm:grid-cols-12' data-testid='manual-order-line-row'>
          ...
          {/* 2026-09-14:col-span 1 → 2(1440 量到那顆下拉只露出「未」);12 欄本來只用到 11, 拿那一格補它。 */}
          <label className='text-sm sm:col-span-2'>
          ...
          <label className='text-sm sm:col-span-2'>
            ...
            <input
              ...
              // 2026-09-14:不露 uuid 半截;那格只有 ~115px, 「商品編號(代購留白)」會被切 ⇒ 短版(sr-only label 仍是全名)。
              placeholder='編號(代購留白)'
              className='block w-full rounded-md border px-2 py-1'
            />
          </label>
