...
describe('🔴 14 欄在 Sean 的真實視窗下裝不裝得下真值(同尺量測)', () => {
  ...
  const KNOWN_CLIP_GAP: Record<string, number> = {
    // 🔴 2026-09-14 B2 對稿:廠牌 pin 回稿的 84(主視窗轉 Sean「其餘照稿」)⇒ 可用 70,最長真值 176.7 ⇒ 差距 106.7。
    //    ⛔ ~~171.3 − 127.4 = 43.9(08-23「依照 OD」)~~。這一筆是「拍過的裁切」,不是順手放行:
    //    稿的 84 是 Sean 自己的 Sheet 欄寬,長廠牌名在列上截字、全名在展開區看得到。要放寬 ⇒ 改 `globals.css .col-brand`,這裡跟著重量。
    'col-brand': 176.7 - 70,
    // 🔴 同一拍:料號 pin 回稿的 134 ⇒ 可用 120,最長真值 157.1 ⇒ 差距 37.1(它之前是 `width:auto;min-width` 吸收欄,從沒被裁過)。
    'col-sku': 157.1 - 120,
    'col-unit': 87.4 - 83.4,
    'col-amount': 100.1 - 96,
  ...
  it('CI 可攜 — DOM 分母與登記表守門(不碰任何像素比較)', async () => {
    ...
    expect(
      ...
      // 2026-09-14 B2:3 → 4(`col-sku` 進表)—— 這一次【是】放行新的截斷,而它有拍板:
      //    廠牌 84 / 料號 134 是稿 v22(Sean 的 Sheet 欄寬)的值,主視窗轉「其餘照稿」;長料號在列上截字、全名在展開區。
    ).toBe(4);
