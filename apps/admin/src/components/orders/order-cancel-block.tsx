...
//    ⚠️ **「未超過硬上限」那句已過期**(#350d-2 實查):`order-detail.tsx` 現在 **404 行**(2026-08-16 片1c-2 抽走 `ItemsTable` 之後量)、
 ...
 * 🛑 **這顆鈕【不保證】按下去會成功**:那支 RPC 還有本層不重打的閘
 *    (plan §10:以 DB 為準, UI 判定只是預告)。⇒ 被拒**不是 bug**, 畫面會顯示那條路自己的訊息。
 *
 * 🔴🔴 **2026-09-16:上面那句在放寬之前對【非刷卡單】是假的, 而那是這一片修的病。**
 *    ⛔ ~~「兩道閘(只開放刷卡單 / 取消過就擋)」~~ —— 只開放刷卡那道已經放寬掉。
 *    📌 **而真正要記住的是那句話的形狀**:「被拒不是 bug」的前提是【偶爾被拒(競態)】;
 *       對非刷卡單它是**永遠**被拒 ⇒ 同一句註解在兩種世界裡一個對一個錯,
 *       而**畫面上那個紅框仍然每一次都叫他按**(`cancel-view.ts` 只看 `payment_status === 'refunded'`)
 *       ⇒ 那不是「不保證成功」, 那是**指了一條死路**。
 *    ⇒ ⇒ 放寬之後它回到「偶爾被拒」那個世界, 這句話才重新是真的。
 *    🛑 **下次再有人往那支 RPC 加【永遠成立】的閘, 要同時回來看這個紅框**, 否則死路會再長一條。
 */
