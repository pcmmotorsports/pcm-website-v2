        ...
        {/* 🔴 ⛔ ~~「建好之後就跟網站上的單一樣,可以出貨、開發票。」~~ —— 那句對【出貨】是假的。
            **出貨必先到貨、無直送**(Sean 2026-08-05 拍板;`components/print/picking-doc.tsx:80` 逐字)
            ...
            🎯 板列 `⟦b4-MANUALORDERDEADEND⟧` 逐字說那是**設計不是 bug, 只是畫面從來沒說出來** ——
               ...
               而員工照它做完九個步驟之後才會撞到牆。
            ⛔ ~~「貨到了」四個字是按鈕上的字面 —— 照抄, 不要換成「登錄到貨」之類的同義詞~~
            ✅ **2026-09-11 Sean 逐字「登錄到貨=> 到貨登記」**:訂單頁與出貨視窗那兩個名字統一成「到貨登記」,
               本句只換了按鈕名、其他字不動(他 09-09 答「好」的那句)。`shipment-launcher.tsx` 與
               `shipment-dialog-copy.ts` 用同一個名字指同一顆鈕, 三處要一致。
