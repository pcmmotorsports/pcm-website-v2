...
describe('片6a-2 缺料自動展開', () => {
  it('🔴 「卡住」的判斷來自 `item-stuck.ts`,**本檔零判斷邏輯**', () => {
    ...
    /**
     * 🔴🔴 **白名單一句(W6 `W6-058` 建議,寫進【正文】不是寫進 commit)**:
     * `order-detail-items-table.tsx` 的 `hasProcurementRows`(= `(item.procurements ?? []).length > 0`)
     * 問的是**存在性**,**不是卡住狀態** ⇒ **刻意排除在本格之外**,不是漏網。
     * ⚠️ 沒有這一句,下一個人把下面的 regex 加寬(例如補上 `procurements\s*\?\?`)時,
     *    會把那一行掃進來 —— **而他會以為自己在補洞。**
     * 📌 「卡住」的定義仍然**只有** `lib/orders/item-stuck.ts` 一處,本檔零判斷。
     */
    const 自己判 = [
