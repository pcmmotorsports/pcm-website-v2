    ...
    <div className='mt-2 rounded-md border border-dashed p-2.5 text-xs'>
      ...
      {(state.status === 'idle' || state.status === 'failed' || state.status === 'blocked') && (
        <form action={formAction} className='flex flex-wrap items-center gap-2'>
          ...
          {/* 🔴 界線要寫在畫面上,不能只寫在 commit body(見檔頭)。
            ...
            *    (codex must-fix:測試只斷言含「到貨紀錄」四個字, 所以這句反話照樣全綠)。 */}
          <span className='text-muted-foreground w-full'>
            這一列只撤得掉剛剛那一筆。更早的到貨紀錄,到下面的「到貨紀錄」清單裡各自撤。
          </span>
        </form>
      )}
      ...
      {state.status === 'blocked' && (
        <p role='alert' className='text-destructive whitespace-pre-line'>
          {state.message}
          {/* 🔴🔴 **這一句補的不是資訊, 是一個【被承諾而不存在的回程】。**
              上面 DB 那段原文逐字叫他「要先把那些包裹作廢…**才能刪掉**這筆到貨紀錄」
              ⇒ 他會離開這一頁去作廢包裹。
              🛑 而撤銷入口的鑰匙是 `undoKey`(`receipt-record-form.tsx:127` 的 React state)
                 ⇒ **導航或重整就沒了** ⇒ 他回來時整條撤銷列不在, 而畫面沒有一個字提過這件事。

              🎯 病名不是我發明的 —— OD `pcm-524f` 的 `HANDOFF-orders-ui.md`(FIX-76)逐字:
              「**過期的說明比沒有說明更糟, 它會主動關掉使用者的下一個動作,**
               **而且『他按了沒反應』與『他讀完沒按』在我這邊長得一模一樣**」
              ⇒ 📌 **這是那個病的第二個現場**:「他去作廢了然後放棄」與「他根本沒去」,
                 在我們這邊也長得一模一樣 —— **兩個世界都是靜悄悄的。**

              ✅ 判準(今天量出來的):**這句話在兩個世界裡都要是真的** ——
                 「他還在這一頁」(入口還在, 這句話是提醒)與「他離開了」(入口沒了, 這句話是預告)。
              🔵 **為什麼是這一句而不是【把入口做成不會消失】**:後者要把一把**已消費的冪等鍵**
                 放進網址列 ⇒ 那是安全面, 不是本片能自己決定的。而這一句**在那件事做好之前一直是對的**。
              ⚠️ **暫用字面, 定稿是 Sean 的板**(主視窗 2026-09-03 批 甲案為暫用)。

              🔴🔴 **2026-09-03 同日:`#450` 做掉了 ⇒ 舊字面【變成假的】, 已改。**
              ⛔ 舊字面逐字:~~「這個撤銷入口只在這一頁有效 —— 離開或重新整理之後就沒有了,
                 **到時候要找工程師處理**。」~~
              🛑 **而它變假之後留著會【比沒有這句話更糟】** —— 那正是本檔上方引的 `FIX-76`
                 (`HANDOFF-orders-ui.md` 逐字):
                 **「過期的說明比沒有說明更糟, 它會主動關掉使用者的下一個動作,
                   而且『他按了沒反應』與『他讀完沒按』在我這邊長得一模一樣」**
              ⇒ 📌 員工讀到「要找工程師」就**不會去試**那個現在做得到的事。
              ✅ 而**舊字面逐字留在這段註解裡**, 讓拿它去搜的人同一發撞到這裡。
              🧪 守門:`receipt-undo-bar.test.tsx` 一格斷言畫面**不得**再出現「到時候要找工程師處理」。 */}
          {'\n\n'}
          🔵 這一列是「剛剛那一次」的快捷撤銷。離開之後,到下面的到貨紀錄裡一樣撤得掉。
        </p>
      )}
