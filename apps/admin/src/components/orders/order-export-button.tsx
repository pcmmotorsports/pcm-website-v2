...
//
// 🔴🔴🔴 **上面那句話是假的, 2026-08-26 codex 抓到(`#24` 片C)。**
//    它收到的 `csv` prop **就是整份訂單資料**, 含金額、客戶名, 而 08-26 起還含
//    **收件人姓名 / 電話 / 地址**。「不碰結構化資料」≠「不碰資料」——
//    📌 **換了序列化格式, 不等於換了內容。** 而原句的寫法會讓下一個人以為這裡是乾淨的。
//    ⚠️ 具體代價:那串 CSV 在**每一次**訂單列表的 RSC payload、瀏覽器記憶體與 Router Cache 裡,
//       **員工沒按匯出、甚至被 `blockedReason` 擋住時也在**。
//    ⇒ 日後接前端錯誤回報 / telemetry 時, **整包會被送出去**。codex 已掃過:
//      目前 repo 內**沒有** Sentry 或其他外送路徑(2026-08-26 量)—— 而那是「現在沒有」, 不是「不會有」。
//
// 下面這段講的是**結構化 vs 字串**那一件事, 它仍然成立 —— 只是它守的範圍比原句宣稱的窄:
