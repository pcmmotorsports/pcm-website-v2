...
function Section({
  ...
  title: string;
  /**
   * 這一區的一句人話。**2026-08-30 起選用** —— Sean 拿掉了「本次出貨」那一區的
   * (逐字「我不要這些奇怪標語」),而另外兩區他沒有點名 ⇒ 那兩區照舊有。
   * 🔴 **省略與空字串不一樣**:省略 ⇒ 那個 `<span>` 整個不渲染;
   *    給空字串 ⇒ 會留下一個空的 `<span>`,而 `.pd-sech` 是 flex ⇒ 它會吃掉一格 gap。
   */
  note?: string;
  qtyHeader: string;
      ...
      {blocked !== null ? (
        ...
        <>
          ...
          {/* ⛔ ~~`note='這個箱子裡屬於這張訂單的品項'`~~ —— 2026-08-30 Sean 拿掉
              (逐字「我不要這些奇怪標語」)。🔴 **只拿掉這一區的**:
              「尚未出貨」與「訂單取消」那兩區的 note 他【沒有點名】⇒ 留著,不要順手一起清。
            ...
            /* ⛔ ~~`尚未出貨:無 —— 這張訂單沒有還欠客人的品項。`~~
                **2026-08-30 Sean 拿掉**,原話逐字:
                ...
                ⚠️ **Sean 追問「有欠品的時候呢?」答「照舊」** ⇒ 下面 `else` 那一整區的
                   **文字內容零改動**(標題 / note / 清單 / 狀態欄 / 金額格,逐項比對過)。
