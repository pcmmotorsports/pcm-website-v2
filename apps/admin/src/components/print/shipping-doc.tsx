...
export function shippingDocBlocker(args: {
  ...
  if (reportedTotal === null || !Number.isFinite(reportedTotal)) {
    return '系統無法取得訂單品項總數，因此無法確認明細是否完整。請聯絡負責人處理，請勿使用本單出貨。';
  }
  if (items.length !== reportedTotal) {
    ...
    return `訂單品項數量不一致：已載入 ${items.length} 項，系統記錄共 ${reportedTotal} 項。出貨明細單可能缺少品項，請聯絡負責人處理，請勿使用本單出貨。`;
  }
  ...
  if (lines.length === 0) {
    return '這個包裹不包含本訂單的品項。請回到訂單頁，從正確包裹的出貨卡開啟明細單，請勿自行修改網址。';
  }
 ...
 *
 * ⛔ ~~`note` 是給員工看的一句人話,說明**這一區的母體是什麼** ——
 *    兩區的母體不同(一箱 vs 整張單),不寫清楚就會被讀成同一個東西。~~
 * 🔴 **2026-08-30 `note` 整個退場**(Sean 答【甲 兩句都拿掉】;分兩步的全文見下方型別區)。
 *    ⇒ 上面那個顧慮**今天沒有被解決,是被他決定不解決** —— 三張表仍然長得一樣、
 *      母體仍然不同,而紙上不再有任何一句話分開它們。**留著這句,不留白。**
 */
function Section({
  title,
  qtyHeader,
  ...
  title: string;
  /**
   * ⛔ ~~`note?: string` —— 這一區的一句人話~~ **2026-08-30 整個退場。**
   *
   * 兩步:①傍晚 Sean 拿掉「本次出貨」那一區的(逐字「我不要這些奇怪標語」),
   * 而另外兩區他當時**沒有點名** ⇒ 我保留了那兩區;②主視窗把那兩句的**逐字原文**端給他問
   * 「這兩句也拿掉嗎」⇒ **他答【甲 兩句都拿掉】** ⇒ 三區都沒有了 ⇒ prop 退場。
   *
   * 🔴 **為什麼分兩步而不是第一次就全拿掉**:他第一次點名的是**兩句**(一個清單),
   * 而他的理由是「我不要這些奇怪標語」(一個概括)。**概括與清單不一致時不能由實作者選一邊**
   * ⇒ 保留是那一步的正確預設,而正確的下一步是**把原文拿去問他**,不是替他推論。
   * 📌 ⇒ 那兩句現在不在紙上,是因為**他看過原文之後答了**,不是因為我們覺得它們同族。
   */
  qtyHeader: string;
  ...
  return (
    <section className={variant === undefined ? 'pd-items' : `pd-items pd-${variant}`}>
      <h2 className='pd-sech'>
        {title}
        {/* ⛔ ~~`<span>{note}</span>`~~ —— 三區的說明小字 2026-08-30 全部退場(見 `Section` 的 docstring)。
            ⚠️ **`.pd-sech` 是 flex + `gap:3mm`** ⇒ 留一個空的 `<span>` 會吃掉一格 gap,
               所以是整個節點不渲染,不是給空字串。 */}
      </h2>
      <table className='w-full border-collapse'>
        <thead>
          ...
          {/* ⛔ ~~`<tr className='pd-contbar'><th …>訂單 <b>…</b>　箱號 <b>…</b></th></tr>`~~
              **2026-08-30 Sean 拿掉**,原話逐字:
              「左上角的出貨單明細 訂單編號xxxxx 這邊也拿掉 / 我不要這些奇怪標語」。
              🔴 **這是【第二個】同型的東西** —— 他 2026-08-30 稍早拍【乙】拿掉的是
                 `.pd-runhead`(外層跨頁頁首,只在第 2 頁起看得到);
                 **而這一條在【每一頁都印,包含第 1 頁】** ⇒ 截圖上還在的就是它。
                 📌 兩條長得幾乎一樣、住在兩個地方,而只拿掉一條的人會以為做完了。
              🛑 **代價他已經知道並接受**(拍乙那次主視窗事先講過「箱號會不見,倉庫拿紙時
                 認不出是哪一箱」)⇒ 現在續頁上**訂單編號與箱號都不在**,而那是他的板。
              ⚠️ **`orderDisplayId` / `shipmentReference` 兩個 prop 因此沒有消費端了** ——
                 它們留著是刻意的:兩區的呼叫端仍在傳,而**下一次要把識別資訊放回續頁時,
                 接線已經在那裡**。若要刪,連呼叫端一起刪,不要只刪這一半。 */}
  ...
  return (
    ...
    >
      ...
      <table className='pd-run'>
        {/* ⛔ **續頁頁首那一條副標也拿掉了**(Sean 2026-08-30 拍【乙】,原話逐字:
            「乙 連截圖裡『訂單編號 PCM-2026-0104 · 箱號 5X4F9J』那整條也拿掉」)。
            ⇒ 原本是 `<thead className='pd-runhead'>`,印「出貨明細單 訂單編號 … · 箱號 …」,
              螢幕上 `display:none`、**只有紙的第 2 頁起看得到**。
            🔴 **他是在知道代價之後選的乙** —— 主視窗事先告訴過他「箱號會不見,倉庫拿紙時認不出是哪一箱」,
              他看過那句才拍。⇒ **這不是漏講,是他的板。不要再提醒一次。**
            📌 **⇒ 而它推翻的是同一天早些時候「丁」那一片的意圖**(把單據名與訂單編號搬進 `thead`
              讓每一頁都認得出來)⇒ **那一片沒有做錯,是需求變了。** */}
        {/* 🔴 `tfoot` 寫在 `tbody` 之前是 HTML 的既有規矩(瀏覽器仍印在底部),不是筆誤。 */}
        {/* ⛔ **頁尾「列印時間」那一行已拿掉**(Sean 2026-08-30 逐字:
            「出貨明細單的 列印時間 2026-08-30 11:39 跟左上角的這個部分還是不要好了,有點奇怪」)。
            ⇒ 🔴 **連 `<tfoot className='pd-runfoot'>` 整塊一起拿掉,不是只刪那一行字** ——
              `print-a4.css` 的 `.pd-runfoot > tr > td { padding-top: 3mm }` 會讓一個空的 `<tfoot>`
              **在每一頁都留 3mm 空白**,而那在螢幕上幾乎看不出來、只有紙上看得出來。
            📌 **⇒ 它推翻的是 `Q-⑨`=甲(Sean 2026-08-24「頁尾一行淡色小字『列印時間 …』」)** ——
              同一個人,六天後改變主意。**舊拍板不是被違反,是被取代。**
            ⚠️ 而 2026-08-30「丁」把它從 `.pd-bottom` 搬進 `<tfoot>`、讓它**每一頁都印**
              ⇒ 那個改動讓它出現的次數變多,而那可能正是他這次覺得「有點奇怪」的原因。
              **那段搬移的說明留在下方 `.pd-flow` 註解與 `print-a4.css`,不刪。** */}
        <tbody>
          <tr>
            <td>
        ...
        <>
          ...
          <div className='pd-col'>
              ...
              {/* ⛔ ~~`<div className='pd-multi'>同一箱可能還裝著其他訂單的商品。</div>`~~
                  **2026-08-30 Sean 拿掉**,原話逐字:「**我不要這些奇怪標語**」
                  (同一則裡他另外點名「這個箱子裡屬於這張訂單的品項」,兩句一起拿掉)。
                  🔴 **它原本是照稿的 `.pd-multi` 逐字搬來的** ⇒ **這一改是【偏離稿】,不是修 bug**
                     —— 而偏離的授權是他本人,寫在這裡免得下一個人「照樣張補齊」把它加回來。
                  ⚠️ 而它原本要提醒的那件事**沒有消失**:一箱兩單仍然印兩張紙(本檔 `:19`),
                     箱子裡仍然可能有別單的貨。⇒ **拿掉的是那句話,不是那個事實。** */}
          ...
          {/* ── 區塊一:這箱裡、屬於這張訂單的東西 ── */}
          {/* ⛔ ~~`note='這個箱子裡屬於這張訂單的品項'`~~ —— 2026-08-30 Sean 拿掉
              (逐字「我不要這些奇怪標語」)。
              ⛔ ~~🔴 **只拿掉這一區的**:「尚未出貨」與「訂單取消」那兩區的 note
                 他【沒有點名】⇒ 留著,不要順手一起清。~~
              🔴 **那句話只活了幾個小時** —— 主視窗把那兩句的**逐字原文**端給他問
                 「這兩句也拿掉嗎」⇒ **他答【甲 兩句都拿掉】** ⇒ **三區現在一句都沒有,
                 `note` 這個 prop 也退場了。**⇒ 舊字面留著,是因為它逐字寫著「留著」,
                 而照它做的人會把兩句補回去。
              📌 註解放在標籤【外面】,因為 JSX 的開標籤裡不能放大括號註解。
                 ⚠️ 而第二個坑更安靜:我把那個註解語法【當成文字寫進註解裡】,
                    它的結束符號當場把註解提前關掉 ⇒ TS1381。
                 ⇒ **註解裡不要引用註解的結束符號。** */}
          <Section
            title='本次出貨'
            qtyHeader='本次出貨'
            ...
            /* ⛔ ~~`尚未出貨:無 —— 這張訂單沒有還欠客人的品項。`~~
                **2026-08-30 Sean 拿掉**,原話逐字:
                「這個提示拿掉, 改成在本區合計 1,080 下面那條分隔線中間寫著 以下空白
                 如 --------------------- 以下空白 ------------------」
                🔴 **原本那句話的顧慮沒有消失,只是不需要用一句話講**:這一區空掉有兩個成因
                   (真的都出了 / 剩下的全被取消了)⇒ **「以下空白」剛好【兩個成因都成立】**,
                   它只陳述紙上的事實,不宣稱原因 ⇒ 舊註解那條「不可以說都寄出去了」仍然有效,
                   而現在的字面連那個坑都碰不到。
                ⚠️ **Sean 追問「有欠品的時候呢?」答「照舊」** ⇒ 下面 `else` 那一整區的
                   **文字內容零改動**(標題 / 清單 / 狀態欄 / 金額格,逐項比對過)。
                   ⛔ ~~該列舉原本含 `note`~~ —— **同日稍晚 Sean 答甲把那兩句也拿掉了**
                   ⇒ 那一項今天不成立,從列舉裡劃掉。
                🔴 ⛔ ~~「下面 `else` 那一整區**一個字都不改**」~~ —— **那句話不精確**
                   (code-reviewer 2026-08-30 抓):三個區塊**共用同一個 `Section` 元件**,
                   而本片把 `tr.pd-contbar` 從**元件裡**拿掉 ⇒ **尚未出貨與訂單取消兩區的
                   `<thead>` 也從兩列變一列**。
                   ⇒ 📌 **【文字沒改】與【結構沒改】是兩件事, 而「一個字都不改」把它們當成同一件。**
                   ✅ 那個結構改動**在他的授權範圍內**(同一則裡他說「訂單編號 xxxxx 這邊也拿掉」),
                      所以碼留著;寫在這裡是因為**「照舊」那兩個字會被讀成「那張紙完全沒動」**。
                🔴 **線是 `border-t` 不是底色** —— 同 `.pd-box` 那條:底色不印時會整個消失,
                   而單色印表機照樣印得出邊框。 */
            <div className='text-muted-foreground flex items-center gap-3 pt-3 text-sm'>
