...
describe('#661 ProductKeywordSearch', () => {
  it('🔴 送出方式是 GET、目的地是 /products —— 這一格擋的是「改成 POST 之後網址不帶 q」', () => {
    const { container } = render(<ProductKeywordSearch filter={NONE} />);
  ...
  it('輸入框的 name 是 q,並回填目前的搜尋詞', () => {
    render(<ProductKeywordSearch filter={{ setBy: undefined, keyword: 'brembo' }} />);
  ...
  it('🔴🔴 有分類時要有 set_by 的 hidden —— 沒有它,搜尋一次就把分類洗掉', () => {
    const { container } = render(
      <ProductKeywordSearch filter={{ setBy: 'staff', keyword: undefined }} />,
    );
  ...
  it('🔴 沒有 page 的 hidden —— 換搜尋詞要回第 1 頁,而「沒有那個欄位」就是做法', () => {
    const { container } = render(
      <ProductKeywordSearch filter={{ setBy: 'sync', keyword: 'x' }} />,
    );
    expect(container.querySelector('input[name="page"]')).toBeNull();
  });

  it('沒搜尋時不畫「清除搜尋」', () => {
    render(<ProductKeywordSearch filter={NONE} />);
    expect(screen.queryByText('清除搜尋')).toBeNull();
  });

  it('🔴 清除搜尋只拿掉 q、保留分類 —— 拿錯會把員工踢回全部商品', () => {
    render(<ProductKeywordSearch filter={{ setBy: 'staff', keyword: 'brembo' }} />);
  ...
  it('目前搜尋的詞要顯示出來(員工要看得到自己在哪個篩選狀態)', () => {
    const { container } = render(
      <ProductKeywordSearch filter={{ setBy: undefined, keyword: '煞車皮' }} />,
    );
