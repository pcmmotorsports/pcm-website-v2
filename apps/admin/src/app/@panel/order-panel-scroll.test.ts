...
describe('訂單面板的高度必須扣掉表頭', () => {
  it('🔴 兩處面板都用 calc 扣掉表頭高,不得是裸的 max-h-svh', () => {
    const c = panelClasses();
    const calc = [...c.matchAll(/max-h-\[calc\(100svh-3\.5rem\)\]/g)];
    // 🔴 **數量要對**:這支檔有**兩個** return 分支(有客人卡 / 沒客人卡),
    //    只修其中一個的話,另一條路上的員工照樣看不到取消區,而**測試會綠**。
    expect(calc.length, '兩個 return 分支都要扣;只修一個 = 另一條路照樣看不到取消區').toBe(2);
