...
describe('#10 片2b — 版面', () => {
  ...
  it('🔴🔴 片4:LOGO 與 QR 必須是 `<img>`、不得掛在 background 上 —— **改成 background 在螢幕上一模一樣、三綠全綠, 而【紙上那四樣會不見】;改我之前, 你要知道你在關掉什麼**', async () => {
...
describe('#10 片2b — 三區(Sean 2026-08-16 逐字:本次出貨 / 尚未出貨 / 訂單取消)', () => {
  /**
   * 區名。
   * ⛔ ~~**片2/片3 之後 `h2.pd-sech` 裡是「標題文字 + `<span>` 說明」兩個節點**~~
   *    ~~⇒ `h2.textContent` 會變成 `本次出貨這個箱子裡屬於這張訂單的品項`。~~
   * 🔴 **2026-08-30 起 `<span>` 說明整個退場**(Sean 答甲)⇒ `h2` 現在只有一個文字節點
   *    ⇒ `firstChild` 與 `textContent` 今天等價。**`firstChild` 留著是刻意的**:
   *    它讓「哪天說明回來了」不會把區名清單撐壞。
   * ⚠️ ~~說明文字另有一格在看(見下方「說明也要印出來」那格)~~ —— 那一格已改寫成
   *    「三句都不在」的斷言,不再是「說明也要印出來」。
   */
  ...
  it('🔴 三區的說明小字 Sean 2026-08-30 全部拿掉 —— 而「母體不同」是被決定不解決,不是被解決', async () => {
    ...
    const t = (await renderPage()).container.textContent ?? '';
    // ⛔🔴 **本格原本的宣稱【整個不成立了】,而它是分兩步失效的 —— 兩步都留著:**
    //    ① 2026-08-30 傍晚 Sean 拿掉「本次出貨」那一區的說明(逐字「我不要這些奇怪標語」)
    //       ⇒ 當時另外兩區他**沒有點名** ⇒ 我保留了,而本格當時只收窄成「兩區在講」。
    //    ② 同日主視窗把那兩句的**逐字原文**端給他問「這兩句也拿掉嗎」
    //       ⇒ **他答【甲 兩句都拿掉】** ⇒ **三區現在一句說明都沒有。**
    // 🔴 **⇒ 所以「三區的母體各不相同,紙上要各自講出來」這個顧慮【今天沒有被解決】** ——
    //    它是被 Sean 決定不解決的。三張表仍然長得一樣、母體仍然不同,
    //    而紙上不再有任何一句話分開它們。**寫在這裡,不留白。**
    // ✅ 本格改成釘住那個【新狀態】:三句都不在,而三個區塊標題還在
    //    (標題是那一區叫什麼,他拿掉的是解釋母體的那句)。
    expect(t).not.toContain('這個箱子裡屬於這張訂單的品項');
    expect(t).not.toContain('這張訂單還欠客人的東西');
    expect(t).not.toContain('這張訂單裡已經取消的品項');
    // 🔵 **正對照:三個區塊本身還在** —— 少了這三行,上面那三個 `not` 會與
    //    「整張紙沒渲染」印同一個綠。
    // 🔴 **走 `titles()`(只取 `.pd-items h2` 的標題文字節點),不是整頁 textContent** ——
    //    `toContain('本次出貨')` 打在整頁上**沒有判別力**:頁首那行
    //    `<span>本次出貨:{lines.length} 項</span>` 是無條件渲染的
    //    ⇒ **整個「本次出貨」區塊被刪掉它照樣命中**(code-reviewer 2026-08-30 抓)。
    //    📌 一個正對照打在一個【比它要證的東西更大】的分母上, 就不再是正對照。
    expect(titles((await renderPage()).container)).toEqual(['本次出貨', '尚未出貨', '訂單取消']);
  });
});
