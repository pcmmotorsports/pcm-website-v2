...
describe('/orders/[id] — RW2d 退款入口顯示鏈', () => {
   ...
   * 而**同一張畫面上的取消區正在叫員工來這裡**(逐字「請到本頁上方的「退款」裡處理」;2026-09-16 前寫「最下方」,位置指錯已改)
...
// ─────────────────────────────────────────────────────────────────────────────
// 危險操作沉底:取消 / 退款(2026-08-16;~~原寫「取消 / 退貨 / 退款」~~ 2026-08-19 改,見下方那格)
// ─────────────────────────────────────────────────────────────────────────────
//
...
describe('危險操作沉底:取消排在出貨之後', () => {
  ...
  /**
   * 🔴 **面板上不得出現一顆寫著「退貨」的鈕** —— 退貨整條功能在本 repo 零落點,
   *    而那顆鈕原本寫「退貨 / 退款」⇒ 它把員工指向一條不存在的流程。
   *    權威 = Aug-17 18:39「720 側邊欄確認稿」`:432` 逐字
   *    「退貨 —— 整條功能後台沒有。畫面已經改成講明『退貨功能目前還沒有』,不會叫你去走。」
   *
   * 🔴 **`not.toContain` 單獨用會恆綠**(面板整個沒渲染時它也過)⇒ 先釘兩個【一定會渲染】
   *    的正向錨:那兩顆鈕的字面本身。**先證得出面板在,那條否定式才有判別力。**
   *    (雙向表演:把 order-detail.tsx 的字面改回「退貨 / 退款」⇒ 第三條必紅。)
   */
  it('🔴 那顆鈕寫「退款」不寫「退貨」(退貨功能後台沒有)', async () => {
    const { container } = await renderPage();
    const text = container.textContent ?? '';
    expect(text, '取消鈕沒渲染 ⇒ 面板沒出來,下面那條否定式會恆綠').toContain('申請取消整張單');
    expect(text, '退款鈕沒渲染 ⇒ 同上').toContain('退款');
...
describe('#787:非卡退款登記入口 —— 🟢 2026-09-08 開封後,健康輸入要【看得見】它', () => {
  ...
  // `order-detail-money-tab.tsx` 的 `const hasStuckRefundVerdict = refunds.some(…)` 改成 `= false`
  // (2026-08-24 拆檔片:那一行隨 money content 自 `order-detail.tsx` 搬檔;突變目標跟著走)
