...
describe('S2:換單必須換掉整個表單實例(key={orderId})', () => {
  ...
  it('A 單失敗後導覽到 B 單,送出的不是 A 單那把鍵', async () => {
    ...
    const { container, rerender } = render(
      <PaymentSection orderId={ORDER_A} returnTo={RETURN_TO} payments={payments} amountDue={0} refundedTotal={0} cancelled={false} />,
    );
    await failWith(container, 'error', failureValues(STAMP_A));

    rerender(<PaymentSection orderId={ORDER_B} returnTo={RETURN_TO} payments={payments} amountDue={0} refundedTotal={0} cancelled={false} />);
...
describe('S3:明細讀不到 ⇒ 表單留著、只停用送出', () => {
  ...
  it.each(['unreadable', 'order_not_found'] as const)('%s:表單在、送出停用', (status) => {
    const { container } = render(
      <PaymentSection orderId={ORDER_A} returnTo={RETURN_TO} payments={{ status }} amountDue={0} refundedTotal={0} cancelled={false} />,
    );
  ...
  it('失敗態 + 明細讀不到:畫面上不得出現叫他「重新整理」的話', async () => {
    ...
    const { container, rerender } = render(
      <PaymentSection orderId={ORDER_A} returnTo={RETURN_TO} payments={ok} amountDue={0} refundedTotal={0} cancelled={false} />,
    );
    await failWith(container, 'error', failureValues(STAMP_A));
  ...
  it('失敗後明細一度讀不到、再恢復 —— 送出的仍是失敗那次的舊鍵', async () => {
    ...
    rerender(<PaymentSection orderId={ORDER_A} returnTo={RETURN_TO} payments={ok} amountDue={0} refundedTotal={0} cancelled={false} />);
