...
describe('BMW M:表格內文色 --fg-2(片5)', () => {
  ...
  it('🔴 單號保留主文字色(OD :181)—— 它是整列的主識別', () => {
    // 🔴🔴 **2026-09-13 修一格【已經紅了一段時間】的守門(P3 留下的債,不是本片弄壞的)。**
    //    P3 把單號併進日期格 ⇒ `td.col-oid` 那條規則連同整欄一起消失,
    //    而這一格仍在找 `td.col-oid` ⇒ **它從那一刻起就是紅的,而紅的理由與它要守的東西無關。**
    //    📌 **⇒ 搬走一個元素時,守它的那格也要跟著搬 —— 不搬的話它不會失效,它會【誤紅】,
    //       而誤紅久了就會被當成雜訊。**
    // 🔴 **要守的事情一個字都沒變**:單號的顏色是主文字色、不是次要色(它是整列的主識別)。
    //    變的只是它住在哪 —— 現在是日期格裡的 `.oid-sub`(P3 建的 class)。
    expect(CSS_CODE2).toMatch(/\.orders-grid\s+\.oid-sub\s*\{[^}]*color:\s*var\(--foreground\)/s);
  });
