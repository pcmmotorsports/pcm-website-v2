...
//
// ═══ 🔴🔴 多值軸的分隔符 —— **一張表,不是各自寫在各自的 parser 旁**(2026-08-20 W5 建,W3 提議)═══
//
// 🔴 **為什麼要有這張表**:多值軸會住在**同一條網址**上,而每一軸的分隔符
//    **取決於那個欄位的字元集** —— 不是風格選擇。
//    ⇒ 下一個加軸的人最可能的行為是**照抄鄰居**,而抄到錯的那一個 = 一個安靜的切法錯誤
...
//    (證明尺會動)與一個保證不命中的負對照(證明尺會回零)。做法見 `parseProductSkus` 檔頭。
//
// 📌 **同一條規則在不同欄位上有不同的難度,而難度低的那一邊反而更危險**(W5 2026-08-19):
//    分類含空白 74% ⇒ 用錯切法**幾乎不可能不踩**,寫的人當場就會發現;
//    料號含空白 0.7% ⇒ **大部分時候都對** ⇒ 寫的人測三個料號全過,
//    而那 347 個要等真人去查才會爆。**「大部分時候都對」是最難被抓到的那種。**
//
 ...
 *
 * 🔴🔴 **依賴(不是守門 —— 寫在這裡是要給【會踩到它的人】看,不是要在別人改別的東西時紅)**:
 * ```
 * 逗號分隔的前提 = product_variants.sku 零逗號(半形與全形都是)。
 * 2026-08-19 正式庫實測全表 51,645 筆、16 個供應商:
 *   半形逗號 0 ／ 全形逗號 0
 * 🔴 而【只量到 0 不算數】—— 同一發把尺的兩個方向都表演了:
 *   含空白      ⇒ 347      (正對照,與 parser 那條「不得用 /\s+/ 切」同一個數)
 *   含破折號    ⇒ 32,727   (正對照,大數)
 *   含一個保證不存在的字串 ⇒ 0 (負對照)
 *   ⇒ 這把 LIKE 尺在這一欄上會動、也會回零 ⇒ 逗號那個 0 是【世界】不是【尺】
 * 數法(一發跑完,可重跑):
 *   SELECT count(*) FILTER (WHERE sku LIKE '%,%'), count(*) FILTER (WHERE sku LIKE '%，%'),
 *          count(*) FILTER (WHERE sku LIKE '% %'), count(*) FILTER (WHERE sku LIKE '%-%'),
 *          count(*) FILTER (WHERE sku LIKE '%__NEVER_EXISTS__%')
 *   FROM public.product_variants;
 * ```
 * ⚠️🔴 **而這個 0 是【今天的】,且這一欄【不在我們控制之下】** ——
 *    同一發量到 `max(updated_at)::date` = **2026-08-19(就是今天)** ⇒ **這張表現在正在被寫**。
 *    `sku` 帶 `supplier_slug`,值來自供應商的資料源。**下一次餵進來的料號就可能帶逗號**,
 *    而症狀是:那個料號被切成兩半 ⇒ 兩半都查無 ⇒ **員工以為那個商品不存在**。零訊號。
 * ⇒ **這比同族的分類那一軸脆**:分類名是我們自己建的,料號不是。
 * ⇒ 真的出現逗號的那天,修法是換分隔符(或改走同名多鍵),**不是在這裡加一道檢查** ——
 *   檢查擋得住「切壞了」但擋不住「使用者查不到東西」。
 * 📎 這條的形狀來自 W3 2026-08-19 對分類那一軸的做法(把依賴寫在 parser 旁邊,不加守門)。
 */
