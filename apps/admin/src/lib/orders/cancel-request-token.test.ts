...
describe('#363 登記面:client 端亂數必須逐檔逐點登記', () => {
  // 🔴🔴 **這一格要顯式 timeout —— 而理由是【它會在錯的地方叫】。**
  //   `scanClientRandomness()` 走遍 `apps/` + `packages/` 讀每一支檔,而 vitest 預設只給 15s。
  //   🛑 **它紅出來的樣子是「登記面失敗」** —— 一個看起來像「有人加了沒登記的亂數」的紅,
  //     而真相是 `Error: Test timed out in 15000ms`。⇒ 📌 **撞到的人會去查一個不存在的亂數點。**
  //
  //   🔬 **量到的(2026-09-07 線【資料】`-db`,同一台機器七個窗在跑)**:
  //     同一支測試連跑五發 ⇒ **25.0s · 18.8s · 8.65s · 6.42s · 5.05s** —— **五倍的散佈**。
  //   🔴 **⇒ 我一開始把它歸因成「我把型別檔加大了 5% 所以超時」,而那個歸因【立不住】**:
  //     我的兩世界測試(還原 ⇒ 綠 / 放回 ⇒ 紅)**沒有控制機器負載**,
  //     而負載造成的差異(5s ⇒ 25s)遠大於 5% 的檔案成長。
  //     ⇒ 📌 **兩個世界印不同的東西, 不代表我改的那件事就是原因** —— 中間還站著一個沒被固定的變數。
  //   ✅ 正確的說法:**15s 落在這一格的自然變異帶【裡面】** ⇒ 它遲早會在任何人身上紅。
  //   🔬 **而「負載」這個成因後來被【第三方量到】了**:主視窗 `-f1` 2026-09-07 跑 `uptime`,
  //     **15 分鐘平均 load = 158**(五個窗同時在跑全套測試)。
  //     ⇒ 📌 我原本只有「散佈很大」這個間接證據, 那句「是負載」仍然是**推的**;
  //       這一發把它變成**量到的** —— 而量它的不是我。
  //   ⇒ 連帶的隊規(主視窗當場定):**全套測試一次只跑一個窗**, 起跑前先 `uptime` + 登記艦隊表。
  //
  //   ⇒ 給一個明顯寬的值,而**不是**把掃描範圍縮小(縮範圍 = 把守門的分母改小)。
  //   ⚠️ 它仍然只是把牆往後推。**真正的修法是讓掃描結果可快取/增量**,那是另一片。
  //     撞到 60s 的人請開一列,不要再往上加。
