...
const FAILURE_MESSAGES: Record<ProcurementFailureCode, string> = {
  ...
  //    畫面那一面現在有**三種**說法(讀不到 / 觸及上限 / 訂單層清單沒載完),各自的下一步不同;
  //    而 hidden `stale` 只帶得上來一個 `1` ⇒ **這裡分不出是哪一種**。
  //    ⇒ 硬複述其中一種必然會在另外兩種說謊(舊字面「請重新整理這張單再操作」就是這樣壞的),
  //      改成**指向那一則**。少一句話,而不再有第二個會漂的真相源。
  stale:
    '這個品項的採購清單這次沒有完整載入,現在送出會用不完整的內容覆蓋既有資料。' +
    '請看畫面上那則說明(它會告訴你這是哪一種情況、下一步該做什麼)。',
  not_hydrated:
