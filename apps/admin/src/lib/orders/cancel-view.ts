...
export type OrderCancelView = {
   ...
   * 🔵 判準刻意比 RPC 寬(不重打它的閘)⇒ **以 DB 為準, 這裡只是預告。**
   * 🔴 **2026-09-16 更正**:⛔ ~~「只開放刷卡」~~ 那道 `20260916210000` 放寬掉了;
   *    逐條的對應表在同檔 `buildOrderCancelView` 裡 `markCancelAllowed` 那一段。
   */
...
export function buildOrderCancelView(order: CancelViewOrder): OrderCancelView {
  ...
  //
  // 🔵 **判準【刻意】比 RPC 寬**,而**本層不重打它的閘**:
  //    照 plan §10 的不變式 —— **以 DB 為準, UI 判定只是預告。**
  //    ⇒ 重打一份會製造第二份規格, 而兩份會分岔(R3 F11 講的正是這個)。
  //
  // 🔴🔴 **2026-09-16:上面那句「還有兩道閘(只開放刷卡單 · 取消過就擋)」已經過期, 逐條對一次。**
  //    ⛔ ~~只開放刷卡單~~ —— 那道閘 `20260916210000` 放寬掉了(Sean 拍甲:非刷卡單也收)。
  //       🔬 **而它非放寬不可的理由是實查出來的**:正式庫「已全額退款而還沒取消」的單共 3 張,
  //          其中 **2 張**(4JTJG9 手動 1,785 · X5F8WG **網站單** 10,500)卡在那道閘上
  //          ⇒ 而 `admin_cancel_order` 那條路對 `refunded` 也擋 ⇒ **它們是收不掉的狀態。**
  //    ⇒ 📌 **放寬之後, RPC 的四道閘與本層的對應長這樣**:
  //       ```
  //       RPC ① cancelled_at IS NOT NULL       ⇐ 本層【有】對:order.cancelledAt === null
  //       RPC ② payment_method 有值而非 tappay ⇐ 本層【不對】:那一欄不在 adapter 的 SELECT 裡
  //                                              (要收窄得把它一路加進去 ⇒ 第二份規格 ⇒ R3 F11)
  //                                              🔵 而放寬之後它只擋 zero_total 那一族 ⇒ 實務上不會撞
  //       RPC ③ payment_status <> 'refunded'   ⇐ 本層【有】對:order.paymentStatus === 'refunded'
  //       RPC ④ 先前被部分取消過               ⇐ 本層【刻意不對】—— 理由在下一段
  //       ```
  //
  // 🛑🛑 **④ 為什麼刻意不鏡射(這是被要求鏡射之後, 查完決定不做的)**:
  //    同檔 `partialCancelAllowed` 那一段寫死了鏡射的**唯一條件** ——
  //    「**當 RPC 的拒絕訊息無法告訴使用者下一步時, UI 必須先擋。**」
  //    而 ④ 被拒時 RPC 的訊息是具體的:「這張單先前被部分取消過, 只標記那條路不收」。
  //    ⇒ 它要的不是「把鈕藏起來」, 是**把那句話印到他按的地方**(那是本片另一半在做的事)。
  //    ⚠️ **鏡射的代價不是零**:要鏡射得靠 `items[].cancelledQuantity`, 而那一欄**算不出來時是 null**
  //       ⇒ 一旦讀不到就會把鈕藏掉, 而**藏掉的鈕不會解釋自己** ——
  //       📌 那是把「可按而被拒(有話講)」換成「按不到而沒有話」, 對那個人更糟。
  //    🔴 **而這裡有一個【今天到不了、日後要回來】的洞, 明寫不假裝沒有**:
  //       一張「已全額退款 + 先前被部分取消過 + 還沒取消」的單, 兩條路**都**不通
  //       (mark 被 ④ 擋、cancel 被 `payment_refunded` 擋)⇒ 它仍然是收不掉的。
  //       🔬 今天正式庫**零筆**(2026-09-16 唯讀實查那 3 張的「曾被部分取消」全部是 f)——
  //          **「今天零筆」是我查的;「日後會不會出現」不是。** 出現了要開第三條路, 那是另一片。
  //
