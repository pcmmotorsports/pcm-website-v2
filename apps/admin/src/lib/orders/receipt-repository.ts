...
export async function listProcurementChoices(
  ...
  const { data, error } = await createSupabaseServiceClient()
    ...
    //    🔴🔴 **~~「甲片尚未 apply ⇒ 本 filter 與片3 是僅有的兩道」~~ 2026-08-21 更正,而它曾經是對的:**
    //    原句量法 `grep -c 20260814100000 supabase/APPLIED.tsv` = **0**;**當場重跑 ⇒ 1**
    //    (`supabase/APPLIED.tsv:230`,apply 於 2026-08-14;負對照 `grep -c 29991231235959` ⇒ 0)。
    //    ⇒ 寫入端那道守門(`20260814100000` 步 6b,回 `PROCUREMENT_VOIDED`)**已經上線**,
    //      而它是**權威**:本 filter 與片3 的「作廢列不給到貨鈕」都只是 UI 前置。
    //    ⇒ **結論沒變、理由換了**:仍然不要把任一道拿掉,但代價是【可用性退化】
    //      (員工填完整張表才被拒 / 選單裡多一個選了必被拒的選項),**不是資料寫壞**。
