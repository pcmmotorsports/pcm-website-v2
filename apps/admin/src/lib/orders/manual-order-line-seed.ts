...
export type ManualOrderLineSeed = {
   ...
   * 商品編號(型錄品項才有;代購留白)。
   *
   * ⚠️ **種它會讓一條【本片之前實務不可達】的拒絕路變成日常可達**(code-reviewer R1 MF4):
   *    建單 RPC 對那支變體的 `spec` 有兩道 —— **值必須全是字串**、**不得含價格鍵** ——
   *    不合就 **整張單被拒**(不是那一列被跳過)。本片之前員工要**手打一模一樣的 uuid**
   *    才碰得到那條路;現在按一下就碰得到。
   * 🔬 **而分母我去量了**(2026-09-06 對正式庫唯讀):
   *    變體總數 **59,841** · 有非字串值 **0** · 含價格鍵 **0**(`spec` 為 NULL 0、空物件 13,950)
   *    ⇒ 📌 **現有商品按下去不會被攔。**
   * 🛑 **而那不保證未來** —— 新上架的商品若帶一個非字串的 `spec` 值,
   *    那張單會**整張被拒**, 而畫面上只說「失敗」。⇒ 這一格是**已知的、量過的殘餘風險**,
