...
describe('cancel-action-state — 失敗文案(全樹唯一釘死 B 類四碼的地方)', () => {
  ...
  it('🔴 六句文案與 plan §4.2 的表逐字相同(對調兩句也要紅)', () => {
    const expected: Record<CancelFailureCode, string> = {
      ...
      // 🔵 2026-09-03 加(Sean 拍甲:取消已出貨的單要擋一次)。
      // 🔴 這一則刻意**說出下一步**(打電話給貨運), 而**不准說「攔不下來」**——
      //    新竹有取消託運的介面(`-ship` 實測伺服器自列 24 支含 TransDataCancel_Json),
      //    只是我們沒接線 ⇒ 寫「攔不了」會讓員工不去打那通電話, 而他本來還來得及攔。
