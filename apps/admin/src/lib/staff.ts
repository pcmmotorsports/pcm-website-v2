...
// 🔴 這不是登入 / 授權邊界。
// ⚠️ 2026-08-29 訂正(線F 量到, `-c8` 複驗):~~原句「也沒有驗證『目前使用者是誰』;
//    真實身分驗證待個人帳號接上」~~ **是一句沒有限定詞的平述, 而它已經不準**。
//    正確的說法【本檔 :181 早就寫對了】:效力綁在 `ADMIN_REQUIRE_REAL_IDENTITY` 上 ——
//    **旗標關著時 actor 是自選的;旗標開著時票上沒身分就不回退。**
//    🔴 而【線上那個旗標的值從 repo 讀不到】⇒ 兩個世界都要留著, 不要斷言其中一個。
// 📌 同一支檔兩種說法, 相距 178 行 —— 而【沒有限定詞的那一句在檔頭】, 它先被讀到。
