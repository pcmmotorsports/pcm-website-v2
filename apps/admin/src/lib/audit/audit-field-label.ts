...
//         ⛔ ~~41 / 29~~ **2026-09-10 重量 = 字面 71 段、納入 59 段**(掃描器改成解析前
//            先砍整行註解之後量的)。上面那兩個數字是 2026-08-22 的歷史值,保留不改寫,
//            但**不要拿它們當現況** —— 📌 一個沒有任何東西在對的數字,過期了也不會有人發現。
...
export const AUDIT_FIELD_LABEL: Record<string, string> = {
  ...
  // 換商品(20260922100000 admin_swap_order_item):before 是原商品、after 是新商品。
  item_id: '品項編號',
  variant_id: '商品規格編號',
  sku: '料號',
  title: '品名',
  ...
  // 🔴🔴 **2026-09-16 改字, 而【改的方向】比字本身重要。**
  //    ⛔ ~~原本是「送新竹狀態」, 理由逐字是「用詞對齊後台既有的那顆鈕」~~
  //    ⇒ 那個對齊法**本身就是錯的**:那顆鈕當天改名成「跟新竹要託運單號」,
  //      而這一欄**跟著就過期了** —— 📌 **把欄位標籤綁在【鈕的名字】上, 鈕一改名它就說謊。**
  //    ✅ 改用**狀態名**不用**動作名**:這一欄描述的是【那張新竹託運單現在的狀態】,
  //      與「哪一顆鈕按過」無關 ⇒ 下次再改鈕名, 這裡不必跟著動。
  //    🔵 而它仍然對得上操作頁:那顆鈕要的就是「託運單號」, 員工讀得出是同一件事。
  //    ⚠️ 代價明寫:這是**顯示層對映** ⇒ 改了之後**舊稽核紀錄的那一列也會跟著顯示新字**。
  ...
  // 🔴 這五欄是**新增的,不是漏翻的** —— `admin_soft_delete_order_note` 把它們寫進 audit 的
  //    before/after 快照,而**那道分母集合比對當場就紅了**(它等的就是有人新增稽核欄位那一天)。
  // ⚠️ 口氣照本檔既有約定:**寫員工看到那一列時想知道的事**,不是把英文欄名翻過來。
  note_id: '備註編號',
  note_type: '備註種類',
  deleted_at: '刪除時間',
  // 🔵 為什麼稽核裡放的是**摘要與長度**而不是備註全文:PII 最小化
  //    (`20260913020000` 與 A6 `20260802150000` 同一條規矩)。
  //    ⇒ 員工在稽核頁看得到「刪掉的是哪一則、多長」,看不到內容 —— 內容在備註時間軸上還在。
  body_sha256: '備註內容指紋',
  body_length: '備註字數',
  // ── M-4b-03 改金額審核(`20260915050000`)+ #956 車種(`20260914140000`)+ 收件三鍵(`20260915070000`)──
  // 🔴 這十欄是**新增的,不是漏翻的** —— 三支 RPC 2026-09-14/15 才開始把它們寫進 audit,
  //    而那道分母集合比對當場就紅了(它等的就是有人新增稽核欄位那一天)。
  // ⚠️ 口氣照本檔既有約定:**寫員工看到那一列時想知道的事**,不是把英文欄名翻過來
  //    —— 例:`status` 不寫「狀態」寫「這筆申請現在到哪」,因為稽核頁上同時有訂單狀態,只寫「狀態」會讀成那個。
  vehicle_source: '車種是選的還是手打的',
  request_row_id: '改金額申請編號',
  from_unit_price: '原本的單價',
  ...
  //    before/after 快照。而**那道分母集合比對當場就紅了**(它等的就是有人新增稽核欄位那一天)。
  // ⚠️ 口氣照本檔既有約定:**寫員工看到那一列時想知道的事**,不是把英文欄名翻過來。
  //    ⇒ 它真正的意思是「這次取消【沒有】去動品項數量」,而不是「這張單從來沒動過」。
  cancel_items_untouched: '這次取消沒有動品項數量',
  ...
  // ── 稅(手動建單寫的三欄)──────────────────────────────────
  // 🔴 **這三欄在字典裡漏了,而沒有任何東西紅過** —— 那道分母掃描器看不到它們
  //    (鍵的上方有整行註解 ⇒ 被吃掉)。2026-09-10 修好掃描器之後才現形。
  //    寫入端:`20260905360000:806-807` 與 `20260909030000:906-907`、`:922`,
  //    兩處的 action 都是 `order.manual_create`。
  tax_total: '這張單的稅',
  // ⚠️ 值是封閉字集(`exclusive` / `inclusive`),而**值的中文本次不做** ——
  //    那要動 `AUDIT_VALUE_LABEL`,不在 2026-09-10 Sean 拍的範圍內
  //    ⇒ 📌 **員工現在會看到「價錢含不含稅:exclusive」這種半套。** 已知,不是漏掉。
  price_tax_mode: '價錢含不含稅',
  // 🔵 它是「事後查得到員工當時填的原始意思」的落點(查法寫在 `20260909030000:915-921`)。
  // 🔴 **名字不能叫「拿多少去算稅」**(codex nit,它對):它是一個陣列,每筆含
  //    `order_item_id` / `tax_basis` / `unit_price`,而 `tax_basis` 是 `untaxed` / `taxed`
  ...
  // 🔴 [A1] 改價當下有沒有一筆付款正在進行(`20260909080000:326-327` 寫的)。
  //    ⇒ 客人正在做 3DS 驗證、而員工同時改了價 ⇒ 銀行照原價扣、單子對不上。
  //    🛑 這兩欄**不防止**那件事,它們讓對帳失敗時查得出來為什麼。
  //    ⚠️ `…_status` 可能是 `lookup_failed` —— 那是「查不到」不是「沒有」。
  active_charge_attempt_id: '改價當下那筆付款的編號',
