...
const FAILURE_MESSAGES: Record<RefundFailureCode, string> = {
  ...
  stuck_verdict_unknown:
    '目前無法取得完整退款紀錄或判定更正紀錄，本次退款沒有發起、錢沒有動。' +
    '可以先重新整理試一次;若仍然一樣,請通知系統維護(這張單的退款紀錄可能已超過顯示上限,重整不會改善)。',
  // 🔴 ⟦5b-RETRYNOEXIT1⟧ 2026-09-20:舊句以「稍後可再試一次。」結尾 ⇒ **沒有出口**
  //    ⇒ 員工會一直按, 而系統永遠給同一句話。補的是三件他答不出來的事:
  //    **等多久**(幾分鐘)· **試幾次還算合理**(若仍然是同一句話就停)· **停了找誰**(系統維護)。
  //    🛑 出口字彙沿用既有那道欠債守門的同一張表(`refund-action-state.test.ts:198`), 不自創第三套。
  //    🛑 不用 Markdown 星號 —— 這裡是純文字輸出, 星號會原樣印給員工(⟦b4-PCM03STARS⟧ 同族)。
  //    🔵 而這個碼【住在兩張表】(本檔 + 另一支), 兩句一起改 —— 只改一張的話,
  //       另一條路照樣說同一句沒出口的話, 而那正是本列要修的形狀。
  record_unavailable:
    '查不到 TapPay 交易現況(連線失敗或逾時),退款沒有發起、錢沒有動。' +
    '等幾分鐘再試一次;若仍然是同一句話,請勿反覆重試 —— 那多半是 TapPay 或網路那一端,請通知系統維護。',
  record_shape_bad:
  ...
  config:
    '金流設定不完整或不一致，退款沒有發起。重試無法解決，請通知管理者檢查設定。',
