 ...
 *       **不是換成事實**。
 *  ② ⛔ ~~🔴 **本地沒有金額上限這道閘** —— 今天的金額上界是 TapPay 自己的帳;~~
 *     ~~而它只蓋卡片那一渠道 ⇒ 卡退 + 匯退加起來可以超過訂單總額,而沒有東西會響。~~
 *     ✅ **2026-08-31:這一格【已經關了】,而沒有任何東西通知這段字**
 *        (codex 對抗審查 must-fix 4 抓到,我開兩支 migration 逐段複量):
 *        · `order_refunds`(卡)      ⇒ `445b` `BEFORE INSERT` 閘(`20260830210000:333`),超額 `RAISE PCM04`
 *        · `order_manual_refunds`   ⇒ `#866` `BEFORE INSERT OR UPDATE OR DELETE`(`20260824011000:259`)
 *        · 🔵 **而關鍵在上限函式本身兩渠道都扣**:`pcm_order_refundable_remaining`
 *          (`20260820100000:224`)減掉 `order_refunds`(processing+confirmed)**與**
 *          `order_manual_refunds`(未作廢)⇒ **「卡退+匯退加起來超過總額」那個缺口不成立了。**
 *     ⚠️ **而【剩下的邊】要一起帶,不然這格會被讀太寬**:`445b` **只有 `BEFORE INSERT`**
 *        (`20260830210000:312` 自己逐字寫著)⇒ **把既有列的 `refund_amount` 改大的 `UPDATE` 沒有被擋。**
 *     🔴 **這一格值得記的是它的方向**:殘餘風險**變小了**,而**沒有人回頭改這段字** ——
 *        📌 **一個往壞的方向走的數字會有人來吵;一個往好的方向走的,沒有任何人有動機回來查。**
 *        ⇒ 而它的後果不是難看:**下一片的金額安全判斷會建立在一個已經過期的架構上。**
