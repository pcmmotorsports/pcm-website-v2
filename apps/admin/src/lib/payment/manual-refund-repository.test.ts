...
describe('🔴 #866 軌別上限那三個碼 —— 它們原本落進 error,而 error 叫員工重試', () => {
  ...
  //      PCM02 ⇒ 要 `pcm_manual_refund_rail_cap` 回 NULL,今天構造不出來(我沒有演)。
  ...
  //       (🔵 正對照 `INSERT INTO …` ⇒ **3**)。
  //    ⇒ 🔵 **那為什麼還接?** 因為 map 的成本是一行,而**沒接的代價是落進 `error`
  //       那句「可以用同一張表單稍後再試」** —— 而未來任何人加一條 DELETE 路徑時,
  //       這一行已經在那裡了。⚠️ **但下面這幾格驗的是【map 的行為】,不是「員工今天看得到」。**
  it.each([
