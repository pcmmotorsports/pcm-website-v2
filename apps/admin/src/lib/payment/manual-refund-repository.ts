...
const SQLSTATE_CLASSIFICATION = new Map<string, ManualRefundFailureCode>([
  ...
  // 🔴 **⟦b4-PCM05SPLIT⟧ 2026-09-02:兩條路要對同一個碼講同一件事。**
  //    `20260901200000:310`(手動退款那支 trigger)已經在吐 `PCM07`=查無訂單,
  //    而這張表沒有它 ⇒ 它落進 fallback 的 `error`, 而 `error` 說「可以用同一張表單稍後再試」
  //    ⇒ 🔴 **那對「訂單不在了」是錯的下一步** —— 稍後再試一百次也不會有那張訂單。
  //    ⇒ 走 `rejected`(與 PCM01/02/03 同):那句 RAISE 本身就是寫給員工看的指示,
  //      罐頭化只會把「找不到這筆退款要掛的訂單…請重新整理後確認」換成一句更含糊的話。
  //    ⚠️ 而 `20260901200000` **還沒 apply** ⇒ 這一格今天接好而沒有流量, 不是壞的。
  // 🔴🔴 **【codex R2 新洞 —— 這一格是【已知而沒關】的, 不要讀成已經處理好了】**
  //    `rejected` 會把 **DB 的原句原樣顯示給員工**(本檔 `staffMessage`, 見
  //    `manual-refund-action-state.ts:44` 那段刻意的設計)。
  //    而 `20260901200000:310` 那句原話逐字是「…**請重新整理後確認。**」——
  //    ⇒ 🔴 **那是一個【沒有終點】的指示**:訂單如果真的不在了, 重整一百次也不會有,
  //      而員工會一直按。(同一輪 codex 對卡片那條路開的第一條 must-fix, 就是這件事。)
  //    ⇒ 而卡片那條路我修得掉(它走罐頭訊息 `order_not_found`, 本片已加上出口);
  //      **這條路修不掉 —— 它顯示的是 DB 的字, 而那支 migration 是線 `-b4` 的。**
  //    🔵 **今天不是活的**:`20260901200000` 在 `APPLIED.tsv` 命中 **0**(未 apply)
  //      ⇒ 正式庫今天不會吐 `PCM07` ⇒ 這一格是【接好而沒有流量】。
  //    ⇒ 🛑 **已開列 `⟦5b-MANUALPCM07NOEXIT⟧` 並交給 `-b4`** ——
  //      修法是在那支的 RAISE 訊息裡補出口(「若仍然找不到, 請勿反覆重試, 並通知系統維護」),
  //      **而它必須在那支 apply 之前做完。**
  //    ⇒ 📌 而本片【不改別人未 apply 的 migration】—— 那是他們的在製品。
  ['PCM07', 'rejected'], // 查無訂單:「找不到這筆退款要掛的訂單…請重新整理後確認」
