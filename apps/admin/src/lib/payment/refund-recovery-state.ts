...
const FAILURE_MESSAGES: Record<RecoveryFailureCode, string> = {
  ...
  //    ⚠️ **第三版才對**(兩份審查逐層逼出來的,記錄在此免得被改回去):
  //       v1「請重新整理清單確認現況」= 舊文案,叫人做沒有結果的事。
  //       v2「清單已更新為最新狀態」= 仍然誤導 —— 結案後**這一列會從異常清單消失**
  //          (`refund-read.ts:126` `.eq('status','processing')` 硬過濾),他在那裡看不到結果。
  //       v3(現行)= 講**真正看得到結果的地方**:該筆訂單頁的退款紀錄。
  already_finalized:
    '這筆退款已經結案,沒有更動任何退款資料。這一列會從異常清單消失,' +
