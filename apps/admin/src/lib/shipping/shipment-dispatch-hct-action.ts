...
export async function dispatchShipmentAction(args: {
  ...
  try {
    ...
    if (plan.kind === 'all_unknown') {
      auditLog('shipment.hct_dispatch', auth, 'fail', { shipment_id: args.shipmentId });
      // 🔴🔴 **2026-09-16:原本這句是「請人確認」—— 三句都對, 而沒有一句告訴他【確認什麼、找誰】。**
      //    🔬 Sean 真的撞到一次(`rtn_code_120000004`, 箱 45NJ3Y):他看完這句之後**只能乾瞪眼**。
      //    ⇒ 📌 **誠實顯示不確定之後, 要接一個他做得到的動作** —— 否則那份誠實只是把難題丟回給他。
      // 🔵 貨號寫進句子裡:他要**唸給客服聽**。
      // ⚠️ **電話號碼要標明來源與未確認** —— 那支號碼取自 `docs/reference/hct-logistics-api-reference.md`,
      //    而該檔第 8 節「未確認清單」逐字列著「聯絡電話 02-2837-1122 #5123 是否仍有效」。
      //    🔴 **不要把一個沒驗過的號碼印得像確定的事實。**
      return {
        ok: false,
        kind: 'needs_human',
