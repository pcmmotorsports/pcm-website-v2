...
export const HOME_BANNER_RESULT_MESSAGES = {
  ...
  // 🔴 這句話 R1 改過一次, 理由要留著:原本寫「舊的那張沒有動」——
  //    那在**這個情境**讀起來像【放心】, 而它實際上是【你還有一步沒做】。
  //    發布不會動別張(Q9 乙)+ 首頁吃四張 ⇒ 他改好發布之後, **錯的那張還在首頁**,
  //    一路掛到它自己的 ends_at(預設 14 天)。⇒ 一句真話要把下一步講出來。
  duplicated: {
    text: '已經複製成一張新草稿,現在可以改了。⚠️ 改好發布之後,記得回去把舊的那張「下架」——'
      + '首頁可以同時掛多張,不下架的話新舊兩張會一起出現。',
    tone: 'ok',
  },
...
/**
 * 按「發布」之前的一句提醒:**首頁現在掛著幾張,而發布不會把它們收掉。**
 * (Sean 2026-09-17 Q3 甲;主視窗 a5 轉達。)
 *
 * 🔴 **為什麼要有這一句**:2026-09-16 那條路是「複製 → 改 → 發布」,而
 *    「舊的不會自動收掉」只出現在**按複製**那一步的結果條上,**到按發布時早就被蓋掉了**。
 *    📌 **話說過了,而時機不對 —— 那與沒說一樣。**
 *
 * 三個判準(a5 給,而其中兩個是我自己昨晚踩過的):
 * 1. **不用「這張 / 那張」** —— 用身分講(`要收的那一張`)。整段住在草稿面板裡,
 *    指示代名詞在這裡沒有錨點,讀的人會推到**自己正在編的這一張**,那正是要收的**反面**。
 * 2. **張數是參數,不是寫死的字** —— 2026-09-16 那句假話就是文案自己講了一個數字而碼是別的。
 * 3. **`liveCount < 1` 回 `null`** —— 首頁本來就空的時候,發布不會造成同時掛兩張,那時印了是噪音。
 *
 * ⚠️ **它刻意只講【現在】,不預測發布之後的總數** ——
 *    草稿的上架時間可以填未來(那時會變 `scheduled`、不會立刻上首頁),
 *    而「現在掛著幾張」與「發布不收掉舊的」這兩件事**在每一種排程下都成立**。
 *    📌 少講一句預測,就少一個會錯的斷言。
 */
export function publishKeepsLiveHint(liveCount: number): string | null {
