...
//    ⚠️ **但這句只在呼叫端同時用了 `anyDuplicated` 入口擋門時才成立**:只換 `readSingleString`
//    的話,`invalid` 會被收斂成 `null`,而「這欄是空的就跳過某個互斥檢查」那族守門會因此被繞過
//    ⇒ **同一欄送兩份反而比送一份更容易通過**(關卡2 審查實跑在退款線抓到,見 `anyDuplicated`)。
//    ⇒ 呼叫端必須兩件都做:入口擋門 + 把 `null` 當形狀錯處理(逐檔看過,不假設)。
