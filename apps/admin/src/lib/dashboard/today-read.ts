...
//
// 🔴 **顯式 `.limit()`**(reviewer MF4):PostgREST 有 `max-rows=1000`,不帶 `.limit`
//    **不是**「無截斷」而是「1000 筆處被平台靜默截斷」——house 字面在 `../payment/refund-read.ts:67-70`。
//    對**加總**而言截斷 = **金額只會偏小、且不報錯**,正是本檔最怕的壞法。
//    ⇒ 取 N+1 判斷,超過就把旗標送到畫面,**寧可說「這個數字不完整」也不給一個安靜的錯數字**。

// ══ 🪦 `TODAY_REFUNDS_LIMIT` 隨「今日退款」那一格一起拆掉(2026-08-15)═══════
//
// 它是**退款**加總查詢的顯式上限(取 500,嚴格低於 production 實測的 `max-rows=1000`),
