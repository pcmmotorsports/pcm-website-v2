...
describe('白名單這張表本身', () => {
   ...
   */
  // 🔴 **限定句放進【標題】, 而那是刻意的**(主視窗 2026-09-07 定形):
  //   vitest 綠的時候只印 `it()` / `describe` 標題, `expect` 的訊息**只在紅的時候印**
  //   ⇒ 📌 **一句限定若只寫在 expect 訊息裡, 它在【這道守門通過的時候】看不見** ——
  //     而那正是它最需要被看到的時刻(本格的問題不是它會紅, 是它**綠得太容易**)。
  //   🛑 **而不得為了讓它印東西加一格恆真斷言**(`expect(true, '…')` 之類)——
  //     那是「恆真守門族」的病:一格永遠綠的測項讓四個數多 1 而**零判別力**。
  //   ⇒ ✅ 標題是綠的時候唯一會出現的位置, 所以那句話寫在那裡。
  it('🔴 白名單 == migrations 裡真的排過的那幾支(兩個方向都比)—— 而它擋不到只在 SQL Editor 排的, 那是 live 那半', () => {
