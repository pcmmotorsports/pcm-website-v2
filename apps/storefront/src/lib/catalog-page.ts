...
export function catalogRowToUIProduct(row: CatalogListRow): CatalogCardProduct {
  return {
    ...
    //     · `0`(贈品 / 買一送一的那個「送」/ 試用品)⇒ **應該**印「NT$ 0」(他早先拍的乙案)
    //   ⇒ **兩個拍板各要一半,而一行 `?? 0` 把兩半黏在一起。**
    //   🔴🔴 **而「應該」兩個字是承重的,不要讀成現況(codex R2 nit)**:
    //      本片(片A)**沒有動 `Price.tsx`** ⇒ 它的判準仍是 `… && v > 0`
    //      ⇒ **此刻 `0` 仍然印「—」,不是「NT$ 0」。** 那一半要等片B。
    //      ⇒ 片A 對客人看到的畫面**零改變**;它做的是把 `null` 與 `0` 在資料層分開,
    //        好讓片B 有東西可以分辨。**片B 落地後請回來把這一段的「應該」拿掉。**
