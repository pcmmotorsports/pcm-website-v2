 ...
 */
/**
 * 🔴 **查不到出處的「我們賣什麼」宣稱。** 與上面那組(我們是誰)分開,而**病因相同**:
 * 為了讓一句話更具體,多宣稱了一件沒查過的事。
 *
 * 📌 `日系`:2026-09-10 實查 `data/brand-content.ts` 的 21 家 ——
 *   義大利 9 · 英國 5 · **泰國 2** · 西班牙 / 盧森堡 / 斯洛維尼亞 / 德國 / 希臘 各 1
 *   ⇒ **日本 0 家。** 而首頁 description 曾經寫著「專營歐系與日系重機改裝部品」並且**上線過**。
 *   ⇒ 「歐系」同樣蓋不住泰國那兩家 ⇒ 現在一律寫「進口」(21 家全是國外品牌,這句查得到)。
 *
 * 🛑 **這兩組守門【擋不住什麼】—— 寫在這裡免得下一個人以為守住了:**
 *   它們是**字面黑名單**。擋得住「總代理」「日系」這幾個**已經犯過**的字再出現,
 *   **擋不住下一句新編的宣稱**(例如「北美原廠件」「全台最大」「日本直送」)。
 *   ⇒ 📌 **真正的守門是寫的人先去站上找出處**;這兩個常數只是把「已經踩過的坑」釘住。
 *   ⇒ 要真的擋住整類,得反過來做白名單(只准出現查得到出處的宣稱)——
 *     而那需要一份「站上既有字面」的權威清單,現在沒有。**沒有就是沒有,不要假裝這裡有。**
 */
export const UNSUPPORTED_ORIGIN_CLAIMS = ['日系', '歐系與日系'] as const;
...
export function buildLlmsTxt(base: string | undefined): string {
  ...
  return [
    `# ${SITE_NAME}`,
    '',
    `> 台灣的重機改裝部品線上商店,販售進口品牌的機車部品,可依車款查詢適用零件。營運方為${LEGAL_NAME}(${LEGAL_NAME_EN},統一編號 ${TAX_ID})。`,
    '',
    `${AUTHENTICITY_LINE}`,
    '',
