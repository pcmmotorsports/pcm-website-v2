...
//
// 🛑 **`/products` 那一頁【刻意不做】BreadcrumbList —— 這不是「還沒排到」。**
//   ⛔ 不要留成 TODO,不然下一個人會照著去做。理由兩條:
//   ① 它的麵包屑住在 `components/ProductsPageHeader.tsx`,**那支現在有別人在動**
//     ⇒ 要做只有「動那支」或「在這邊維護第二份推導」,而後者正是本檔在避免的東西。
//   ② **投報率差三個數量級**:商品詳情頁 25,843 頁,而 `/products` **只有一頁**;
//     且那一頁的麵包屑本來就只有兩階(首頁 › 商品目錄)⇒ 結構化資料加了幾乎不增加資訊。
//   ⇒ 主視窗 2026-09-09 判:不做。
//
