...
const DEFAULT_TITLE = `商品目錄 — ${SITE_SUFFIX}`;
...
export function buildCatalogPageText(
  ...
  if (categories.length === 1) {
    ...
    return {
      title: `${category} — ${SITE_SUFFIX}`,
      description: `PCM 的${category}選品 · 依車款 / 分類 / 品牌篩選`,
    };
  }
  // 有車款(或多顆分類)⇒ 通用。理由見檔頭兩段 🛑。
  if (hasVehicle || categories.length > 1) return { title: DEFAULT_TITLE, description: DEFAULT_DESCRIPTION };
  if (isNewArrivals) {
    return {
      title: `${NEW_ARRIVALS_LABEL} — ${SITE_SUFFIX}`,
