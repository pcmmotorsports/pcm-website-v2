...
describe('一發拿完:我拿到的是不是全部', () => {
  // ⛔⛔ **[2026-09-06 10:1x 訂正] 這一格的【標題曾經是錯的】, 而錯法值得留著。**
  //   ⛔ ~~「這是本片唯一擋得住【靜默截斷】的東西」~~ —— **它現在證不了那件事。**
  //   🔴 db 線把 `n` 從「另一次獨立的 `count(*)`」改成 `jsonb_array_length(rows)`
  //     (opus 審查:同一句 SQL = 同一個快照 ⇒ 兩個數近乎必然相等 ⇒ 同一把尺量兩次)
  //     ⇒ **正式資料上 `rows.length !== n` 這個世界【產不出來】。**
  //   🛑 **所以這一格今天是【契約鎖】不是【守門】** —— 它守的是「碼有照著 `n` 的語意處理」,
  //     而不是「資料沒被截斷」。**兩者的差別是:後者今天沒有人在守, 而它曾經被寫成有人在守。**
  //   ✅ **真正擋資料不完整的是下面那三格【逐列驗形狀】** —— 那三格的突變是真的殺得死的。
  //   📌 **留著這一格的唯一理由**:哪天有人把 `n` 改回獨立來源, 這一格不必重寫就恢復判別力。
  it('🔵 契約鎖(**不是**防截斷守門):`rows.length` 與 `n` 對不上 ⇒ throw', async () => {
    const spy = vi.spyOn(console, 'error').mockImplementation(() => {});
    // ⚠️ 這個 fixture 演的是一個**今天的 DB 產不出來**的世界(`n` 由 `rows` 推導)
    //   ⇒ 🛑 **它綠, 不代表正式資料上有任何東西被守住。**
