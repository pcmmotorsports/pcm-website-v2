...
describe('⟦search-PREFIXWRONGCAT⟧ 落點規則', () => {
  ...
  it('🔴🔴 R3 抓到的【我造成的退步】—— 釘住它, 因為它現在是壞的', () => {
    ...
    // 🔴🔴 **[2026-09-10 訂正 —— 那句話是【推的】, 而它正好錯在我們要決定的那個詞上]**
    //    🔬 對正式庫的 101 個分類實跑三條規則(量測表 `docs/evidence/2026-09-10-分類膠囊三條規則對照.md`):
    //      **前綴優先對 `煞車` 完全無效** ⇒ 仍然是 `拉桿與把手 · 煞車離合器拉桿`
    //      成因很簡單:**`煞車離合器拉桿` 本來就【以「煞車」開頭】** ⇒ 它照樣是前綴命中, 而且 count 最大。
    //    ✅ 那句話對 `齒盤` 是真的(前綴優先 ⇒ 齒盤與傳動), 而**對 `煞車` 是假的**。
    //    ⇒ 📌 **一句寫在測試檔裡、看起來像量過的話, 而它是推的** —— 而下一個人會拿它當前提。
    //    🔵 而「兩邊互斥」那半仍然成立:前綴優先實測會弄壞 5 個
    //      (`濾芯` 661⇒8 · `管束` 690⇒11 · `後照鏡` 723⇒25 · `排氣管` 376⇒99 · `後照` 同上)。
    //
    // ✅ **[2026-09-10 已修, 而【不是】改規則]** —— 改的是**俗稱字典**:
    //    `煞車` 加兩列(煞車系統 + 煞車離合器拉桿, 聯集)· `齒盤` 加一列(齒盤與傳動)。
    //    🔵 階段② 排在③ 前面且【整詞完全相等】才命中 ⇒ **只影響那兩個詞**,
    //      量過:111 個「命中 >= 2 個分類」的詞裡其餘 109 個一個都不動。
    //    🛑 `儀表` **刻意不加** —— 候選四個而「本體」那類裡是轉接盒 ⇒ 判不了, 理由在字典檔。
    //
    // 🔴🔴 **[2026-09-10 · 而我在同一發裡又寫錯一句, 照實留著]**
    //    ⛔ ~~我原本寫「這一格讀的是 `pickReal`, 只走階段③, 所以期望值不變」~~
    //    🔬 **跑了才知道是假的**:`pickReal = (q) => parseSearchFacets(q, REAL_SRC).category`
    //      —— 它走的是**整條管線**, 階段② 的字典【在它裡面】。⇒ 這一格當場紅了。
    //    ⇒ 📌 **又一次「看起來像量過的話, 而它是推的」** —— 而這次相隔【十分鐘】,
    //      就在我訂正上一句同類錯誤的正下方。🎯 **那不是巧合, 那是這個坑的深度。**
    //
    // ✅ **所以這個期望值【改了】, 而它不是為了讓測試過** ——
    //    行為是**刻意改的**(字典兩列, 見 `search-synonyms.ts`), 而這一格現在釘的是**新的正確行為**。
    // 🛑 **而舊的那個壞行為【沒有消失, 只是不再由這一格守】** ——
    //    階段③ 對 `煞車` 仍然挑 `拉桿與把手 · 煞車離合器拉桿`(1,152 > 450),
    //    量測在 `docs/evidence/2026-09-10-分類膠囊三條規則對照.md`。
    //    ⇒ 🔵 **那條規則本身沒被動過** —— 本片一個字都沒改 `pickCategory`。
    // 🔵 原本那句「一個沒有被釘住的退步會消失在 24/25 裡」**做到了它說要做的事**:
    //    這個缺陷真的被下一個人在畫面上重新撞到了(2026-09-10 走查),而且是**先讀到這一格才知道它是已知的**。
    expect(pickReal('煞車')).toBe('煞車系統');
  });

  it('🔴🔴 而客人看到的那一格:打「煞車」要拿到【兩顆】膠囊, 不是只有拉桿', () => {
    // 🔬 走查 2026-09-10 正式站實見:只有「拉桿與把手 · 煞車離合器拉桿」⇒ 想買來令片的人找不到。
    // ✅ 修法在【字典】(階段②)不在規則(階段③)⇒ 所以這一格要用 parseSearchFacets 不是 pickReal。
    // 🔵 兩顆 = 聯集, 照 Sean 2026-09-04 拍甲(一個詞對得上兩類 ⇒ 兩個都給客人)。
    const r = parseSearchFacets('煞車', REAL_SRC);
    expect(r.categories).toEqual(['煞車系統', '拉桿與把手 · 煞車離合器拉桿']);
    // 🛑 而【順序】也釘住:第一顆是舊答案漏掉的那一個。`category` 是舊呼叫端讀的單顆欄位。
    expect(r.category).toBe('煞車系統');
    // ⚪ 反對照:那個詞【被用掉了】⇒ 不得同時留在 leftover 裡(否則畫面會說「這個字沒用到」)。
    expect(r.leftover).toEqual([]);
  });

  it('🔵 而「齒盤」只給【一顆】—— 聯集的前提是兩邊都合理, 而這一對不是', () => {
    // 🔬 唯讀抽正式庫的商品名:護蓋那一類裡是 `前齒外蓋` / `鏈條蓋 搖臂護蓋` / `後內土除`
    //    ⇒ **沒有一件是齒盤** ⇒ 打「齒盤」的人要的是 `後齒盤 Z40` 那一族。
    // 🛑 所以這一格釘的是「**沒有**把護蓋那一類也塞進來」—— 聯集不是「不確定就都給」。
    const src = {
      ...REAL_SRC,
      categories: [
        { id: 'ft', name: '腳踏後移與傳動', count: 1781, children: [
          { id: 'spr', name: '齒盤與傳動', count: 146 },
        ] },
        { id: 'cf2', name: '碳纖維部品', count: 2642, children: [
          { id: 'cov', name: '鏈條蓋與齒盤護蓋', count: 167 },
        ] },
      ] as unknown as MockCategory[],
    };
    expect(parseSearchFacets('齒盤', src).categories).toEqual(['腳踏後移與傳動 · 齒盤與傳動']);
  });

  it('🛑 「儀表」刻意【不進字典】—— 判不了的那一格不准偷偷替客人決定', () => {
    // 🔬 候選有四個而不是兩個, 而「看起來像本體」那一類裡是 `motoscope pro J1850 轉接盒`:
    //    儀表保護貼(430) · 儀表與風鏡外蓋(97) · 儀表護蓋(45) · 儀表與控制器(93)
    // ⇒ 📌 這一格釘的是「字典裡【沒有】這個詞」—— 哪天有人手滑加進去, 這裡會紅並讀到理由。
    const hits = SEARCH_SYNONYMS.filter((x) => x.from === '儀表');
    expect(hits, '儀表進字典了 ⇒ 那要先有真實搜尋語料(見查證檔 §16), 不是手滑').toEqual([]);
  });
