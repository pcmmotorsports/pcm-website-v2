...
//
// 🔴 **2026-08-29(線G,`⟦b4-MONEY4⟧` ② 那一批)⑦ 補了一行【正向同伴】**,而它**不在上面那張靶表裡**:
//   `expect(resolveMock).toHaveBeenCalledTimes(2)` —— 它守的**不是**任何一個實作突變,
//   守的是**量具自己安靜死掉**:⑦ 的唯一斷言是 `not.toHaveBeenCalled()`,
//   而那一行在「兩個實例根本沒跑起來」的世界裡**一樣綠**。
//   📌 **靶表證的是「這一格抓得到那個 bug」;這一行證的是「這一格今天真的跑了」。兩個宣稱,不可互換。**
//   ⚠️ 上面那張靶表是**一次性的實跑紀錄**,沒有人會重跑它 ⇒ fixture 退化時它不會叫,而這一行會。
...
describe('useResolvedCart 自我修復(#375/#455)', () => {
  ...
  it('⑦ 🔴 同頁兩個實例:被較新解析推翻的舊 found:false 不准動 cart', async () => {
    ...
    // 🔴 **正向同伴**(`⟦b4-MONEY4⟧` ② 那一批,2026-08-29 線G)。下面那行是**負向**的 ——
    //    它在「兩個實例根本沒跑起來」的世界裡**一樣綠**,而那個世界與「跨實例守衛有效」
    //    在畫面上印同一個東西。
    // ⚠️ 這一行**不是**要重複 `MX5`(那把靶已經實跑量到會紅 ⑦,見檔頭)——
    //    兩者守的是**不同的失效**:`MX5` 守「守衛被拿掉」,這一行守「**量具自己安靜死掉**」。
    // 🔴 為什麼值得一行:日後有人動 fixture、讓兩個實例不再各解析一次,⑦ 會**恆綠**,
    //    **而 `MX5` 那一發是寫在檔頭的一次性紀錄,沒有人會重跑它** ⇒ 那個退化零訊號。
    expect(
      resolveMock,
      '兩個實例沒有各解析一次 ⇒ 下面那行的「沒被呼叫」不算數(可能只是沒跑)',
