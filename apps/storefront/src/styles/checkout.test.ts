...
describe('手機結帳欄位寬度(2026-08-22 挑 B:拆掉表單自己那層框)', () => {
  ...
  it('🔴 第三層:表單內距也要收(少了它就比 Sean 挑的那張圖窄 20px)', () => {
    ...
    //
    // 🔴🔴 **`m` 旗標不是選配**(線C `-95` 2026-08-29 量到,線G 複驗後補上)。
    //    沒有 `m` 時 `^` **只匹配整串的第 0 個位置** ⇒ 這條正則真正在守的只有 `}` 與 `;` 兩個字元,
    //    而那個 `^` 讓它**讀起來**涵蓋行首 —— **它一次都沒有涵蓋過。**
    //    線C 的五個世界,線G 逐字複製重跑(兩把尺並排):
    //    ```
    //    世界                    無 m      加 m
    //    A 前面是 }              抓到      抓到
    //    B 在字串最開頭           抓到      抓到
    ...
    //    D 前面是選擇器逗號       🔴 漏     抓到
    //    E 前面是註解             🔴 漏     抓到
    //    負對照:合法的後代選擇器  沒抓      沒抓   ← 加 m 不會誤抓
    //    ```
    //    ⇒ **五個世界裡三個假綠**,而上面那句「沒有人會發現」講的正是它自己。
    //    ⚠️ **射程**:本次只改這一處。同批另外 56 處未開檔、屬未確認
    //       —— **線C 明說「57 處裡只有 1 處是實證的」,不要把那個 1 讀成 57。**
    expect(seg, '把 .acc-inline-form-inner 裸寫在手機段 ⇒ 會員中心跟著變')
      .not.toMatch(/(^|[};])\s*\.acc-inline-form-inner\s*\{/m);
  });
