...
describe('🔴 標點遷移(Sean 2026-08-05 拍板 A · 全站遷移)', () => {
  ...
  // 🔴 **這 51 處是已知債、不是「可接受」**(R1 抓到我原本對它們全盲)。
  //    形狀只有兩種:①隔著 `</strong>`(47 處)②在全形右括號/右引號之後(4 處)——
  //    🔵 **2026-09-16:53 → 51,結掉 materya 那兩筆**(Sean 要求把品牌頁中文潤成專業書面語,
  //       順手把 `</strong>,之後才決定` 與 `</strong>;型錄因此` 兩處補成全形)。
  //       改的是 **OD 來源** `brand-content-data.js`,本檔是重產出來的 —— 符合下面那句「修正點在 OD 來源」。
  ...
  const KNOWN_BOUNDARY_MISSES = 51;
