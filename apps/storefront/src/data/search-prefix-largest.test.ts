...
describe('⟦search-PREFIXWRONGCAT⟧ 短詞不得落到小分類', () => {
  ...
  // 🔴🔴 **[2026-09-10 · `齒盤` 從這張清單移出去了, 而【不是】因為它紅了]**
  //    這一格量的是**階段③ 那條規則**(名字含該詞 ⇒ 取 count 最大)。
  //    而 `齒盤` 2026-09-10 起走**階段②(俗稱字典)**, 那一階段排在③ 前面
  //    ⇒ 📌 **它不再受這條規則管** ⇒ 留在這裡會拿【它已經不適用的性質】去量它。
  //    ✅ **而它沒有被放走** —— 下面緊接著一格單獨釘它的新落點。
  //    🔬 為什麼改:客人打「齒盤」被送到「鏈條蓋與齒盤護蓋」(167)而不是「齒盤與傳動」(146)——
  //      只差 21 件就讓【蓋子】贏【本體】。抽商品名核過:護蓋那類裡是 `前齒外蓋` / `鏈條蓋 搖臂護蓋`,
  //      **沒有一件是齒盤**。理由與量測:`docs/evidence/2026-09-10-分類膠囊三條規則對照.md`。
  it.each(['離合', '離合器', '拉桿', '管束', '濾芯', '後照', '後照鏡', '保護', '保護貼', '排氣', '排氣管', '定風', '定風翼', '碳纖', '碳纖維', '水管', '車架', '車架與', '土除', '服飾', '傳動', '大燈', '止滑', '外觀'])(
    '🔴 打「%s」要落到【候選裡件數最大的那個】',
    (w) => {
  ...
  it('🔴 `齒盤` 改由俗稱字典接手 —— 而它要落到【本體】不是【蓋子】', () => {
    // 🛑 這一格是上面那張清單移走 `齒盤` 的**配對**:移走一個詞就要有人接住它,
    //    否則「從清單裡拿掉」與「這個詞沒人管了」在檔案上長得一模一樣。
    const out = parseSearchFacets('齒盤', SRC) as { category: string | null };
    expect(out.category!.split(CATEGORY_URL_SEPARATOR).pop()!.trim()).toBe('齒盤與傳動');
    // ⚪ 反對照:那個【件數比較大】的蓋子類不得再是答案(否則字典沒生效, 而上面那格也不會紅)。
    expect(out.category).not.toContain('鏈條蓋與齒盤護蓋');
  });
