 ...
 *
 * 🔴🔴 **[2026-09-08 · 這一段的【前提換掉了】, 而判準本身仍然成立]**
 *    ⛔ ~~`dealerPrice` 只在 `tier === 'store'` 時由 route 端蓋上(`products/page.tsx`)~~
 *    🔴 **2026-09-08 Sean 裁甲**, 逐字「**甲 不掛了 —— 一個來源、一個快照**」:
 *       目錄頁的經銷會員改走經銷目錄 RPC(它讀 `products_list_dealer`, 該 view 把
 *       `price_store` 映進**同一個** `price_general` 欄)⇒ **`price` 本身就是經銷價**
 *       ⇒ 目錄頁**不再蓋** `dealerPrice`(再蓋一次 = 兩支 RPC 兩個快照 ⇒ 同一份 props 兩個價)。
 *    📌 **舊拍板不是錯的 —— 是【它問的那個世界不存在了】。**
 *    🔵 **而【商品詳情頁】仍然會蓋 `dealerPrice`**(`products/[slug]/page.tsx:162`)
 *       —— 那一頁沒走經銷目錄 RPC ⇒ 📌 **兩頁今天處在兩個不同的世界。**
 *
 *    ⛔ ~~「這支函式的兩個消費端今天處在兩個不同的世界, 而 `??` 對兩邊都對」~~
 *    🔴🔴 **那句是假的, 而我是在讀本檔檔頭的同一次編輯裡寫下它的**(codex R2 nit)。
 *       🔬 `grep -rn 'effectiveUnitPrice' apps packages` 的非測試命中 **只有本檔自己**
 *          (`:137` `:142` `:204` `:205` —— 全在 `filterProducts` / `sortProducts` 裡,
 *           而那兩支**在正式頁零呼叫者**, 本檔檔頭第 3 行就寫著)。
 *       ⇒ 🎯 **它今天【一個】正式消費端都沒有** —— 那兩頁的行為與這支函式無關。
 *    📌 **⇒ 本檔檔頭警告的正是這個坑, 而我一邊引用它一邊踩進去。**
 *       成因:我把「**這個概念**有兩個地方在用」寫成了「**這支函式**有兩個消費端」——
 *       🛑 **前者是對的(目錄頁與詳情頁確實對經銷價有不同做法),而受詞不是這支函式。**
 *
 * 🔴 **判準是「有沒有 `dealerPrice` 這個欄位」, 不是「它大不大」**(主視窗 B 2026-09-07 裁甲;
 *    **這一句今天仍然成立** —— 它守的是「不要用 `> 0` 當判準」, 與欄位由誰蓋無關):
