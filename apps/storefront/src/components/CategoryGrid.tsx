...
const CATEGORY_CHIPS: Readonly<Record<string, { cat: number; icon: ReactElement }>> = {
  ...
  // 🔴 第 12 顆:**不是 OD 畫的**,是 2026-08-22 補的(Sean 逐字授權「你想辦法生一顆就好」,
  //    看過三顆候選拍在真磚牆上的圖之後挑「乙」)。上面 `:59` 那句「新增分類時不要套公式」
  //    仍然成立 —— 他授權的是「生一顆」,不是授權套公式 ⇒ 風格規則是**從既有 11 顆 dump 出來**的,
  //    不是憑感覺像不像:viewBox 一律 `0 0 24 24`、元素 2-4 個、只用 path/rect/circle、
  //    **元素上不寫任何 stroke/fill**(全部由 `home.css` 的 `.b-cat-icon svg` 給)。
  // 🔴 **第一版被自己擋下來的實錘,留著**:我原本畫的是「橫置 rect + 直向鰭片」,在 24px 的格子裡
  //    看起來完全合理 —— 而它就是 `引擎與冷卻`(cat-7)那顆水冷排少了兩根水管接頭。
  //    ⚠️ **並排看沒看出來**,是把 11 顆的元素逐個 dump 出來比字面才撞到的。
  //    ⇒ 重畫成**兩軸都相反**(直立窄罐 + 橫向摺紋),不是「換個角度」。
  進氣系統: {
    cat: 12,
    // 直立圓筒濾芯:罐身 + 三道橫向摺紋
    icon: (
      <svg viewBox="0 0 24 24">
        <rect x="6.6" y="3.6" width="10.8" height="16.8" rx="3" />
        <path d="M6.6 8.2h10.8M6.6 12h10.8M6.6 15.8h10.8" />
      </svg>
    ),
  },
};
