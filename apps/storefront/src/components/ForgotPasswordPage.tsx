      ...
      <main className="auth-main">
        ...
        {stage === 'sent' && (
          <div className="auth-card">
            ...
            {/* 🔴🔴 **⟦auth-FORGOTVERBOSE⟧ 2026-09-09 · Sean 拍甲砍兩句。** 他逐字:「的文字說明真多..真囉唆..」
                ⛔ ~~`<li>連結 <b>1 小時內有效</b>，過期就再要一次。</li>`~~
                ⛔ ~~`<li>信裡的連結<b>只能用一次</b>，用過就失效。</li>`~~
                🔬 **砍它們的理由不是「太長」, 是【講了也用不上】**:那兩件事**信裡自己都寫了** ——
                  2026-09-09 正式站實際收到的那封信逐字有「這個連結在 1 小時內有效」(Sean 截圖)。
                  而客人站在**這一頁**的時候**還沒拿到連結** ⇒ 📌 這裡講連結的規矩,他無從使用。
                ✅ **留下的兩句是【他現在就用得上】的**:去哪裡找信(垃圾郵件)、以及「你可能根本沒有密碼」。

                🛑 **帳號列舉防護一格沒碰**(`login/forgot/actions.ts:41-45`):這四條 `<li>` 是**寫死的靜態文字**,
                  不隨 `outcome`(成功 / 帳號不存在 / rate_limited)分支 ⇒ 砍兩條**不會**製造出回應差異。
                  ⇒ 這一頁對三種世界仍然長得一模一樣。 */}
