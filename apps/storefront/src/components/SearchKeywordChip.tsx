  ...
  return (
    <div className="ac-bar">
      ...
      {/* 🔴🔴 **[2026-09-10 · 這裡原本有一行字, 而它【今天起是假的】—— 整段拿掉, 不補新的]**
        ...
        *
        * 🔵 **它在 `cc8a79188` 之前【是對的】** —— 那時 facet 與關鍵字是兩條互斥的資料路。
        * 🔴 而 `cc8a79188`(⟦db-SEARCHFACETMUTEX⟧ 合路)把兩條路併成一條 ⇒ **障礙消失**,
        *   而**這句話住在另一支元件, 沒有跟著走** ⇒ 📌 **它不是錯誤, 它是【過期】。**
        * 🔬 **2026-09-10 對正式站實測兩半, 兩半都失效**:
        *     篩選  `?search=carbon` ⇒ 1,135 件 · 加 `&pbrands=rizoma` ⇒ **2 件**
        *     排序  同一個 `carbon`, `recommend` / `price-asc` / `price-desc`
        *           第一筆分別是 `rpm-bmsx-04` / `rpm-bms1k2k305` / `rpm-kah2-29`
        *           而筆數三者都是 **1,135**(排序有效, 而它【不改變筆數】—— 那一格也對)
        * ✅ **為什麼「拿掉」不用問 Sean, 而「補一句新的」要**:
        *   拿掉一句假話不是文案決定;**補一句新的才是**。⇒ 所以整段拿掉, 不補。
        * 🎯 **而這一格自己預言過**:上一版註解逐字寫著「**而它有到期日**」——
        *   它猜的到期原因是 ⟦search-MULTIWORDVEHICLE⟧, **而真正到期的原因是合路**。
        *   ⇒ 📌 **一句話會過期這件事被看到了, 而【它會被什麼弄過期】沒有被猜中。**
        * 🛑 **而留下這句話的是【拿掉障礙的同一個人, 在同一天】** ——
        *   它不需要有人偷懶, 只需要**修的人與寫字的人在不同的檔案裡**。
        * 🔵 那顆膠囊(`搜尋:{keyword} ×`)留著 —— 它講的是「這批商品的來源」, 那件事沒有變。 */}
    </div>
  );
}
