...
export function SearchKeywordChip({
  ...
  return (
    <div className="ac-bar">
      ...
      {/* 🔴🔴 **這句話是本片的驗收核心,不是補充說明。**
        * 它要同時講三件事,少一件就會誤導:
        *   ① 現在看到的是什麼 ② 為什麼篩選點了沒用 ③ 他可以怎麼辦
        * ⛔ 只寫「關鍵字搜尋結果」= 標籤,不是宣稱 —— 那句話在「facet 有效」與
        *    「facet 無效」兩個世界裡是同一個字串。
        * 🔴🔴 **[2026-09-08 `front`]⛔ ~~「【左側】篩選與排序」~~ ⇒ 拿掉那個方位詞。**
        *   🔬 **量到的**(本機鑽機真瀏覽器, 402×874):**手機版沒有左側** ——
        *      篩選在**頂端**(「類別 / 品牌價格 / 排序」三顆), 而 Sean 2026-09-07 就是在手機上看的。
        *   ⇒ 📌 **那句話在描述一個他螢幕上不存在的位置, 而它讀起來完全通順**
        *     ⇒ 這是他逐字說「這句話在說什麼我看不懂」的一半。
        *   🛑 **通則**:對外文案裡凡是描述【當下這個系統長什麼樣】的詞 ——
        *     **方位**(左側/上方/右下角)、**順序**(先/再)、**步驟數** ——
        *     都會在版型或流程改掉之後**安靜地變成假的**, 而**沒有任何測試會紅**。
        *   ⚠️ **而「要【先】移除關鍵字」那個順序詞我【留著】** —— 它今天仍然是**事實**
        *     (facet 與關鍵字兩條資料路互斥, 見 `app/products/page.tsx` 那段註解)。
        *     🔴 **而它有到期日**:⟦search-MULTIWORDVEHICLE⟧ 之後車款會走 facet 路,
        *     ⇒ **客人打車款時根本不會看到這顆膠囊** ⇒ 那時要重新判這整句還要不要留。
        *     **整句重寫是文案品味 ⇒ Sean 拍板, 我不自己改。** */}
      <span className="ac-note">
