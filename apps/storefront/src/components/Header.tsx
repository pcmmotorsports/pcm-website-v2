  ...
  /* ═══ 搜尋入口暫時隱藏(2026-08-16,Sean 拍 `Q-SEARCH-0` = 乙)═══════════════════
     ...
     **Sean 逐字**:「要建立好這個搜尋功能,之前有討論過,但是一直沒進行。」
     ...
     📎 搜尋線遲遲沒動的**真正原因**不是忘了:`ADR-0004` 定的技術路線(pg_jieba)在 Supabase
        裝不起來(2026-07-25 實測推翻)。
        ⛔ ~~而**那個更正只寫進 memory、沒有回到那份決策檔** ⇒ 現在讀那份 ADR 的人會拿到
           一條蓋不起來的路線。~~
        🔴🔴 **2026-08-30:那半句話【是假的】, 而它從寫下來的那一天起就是假的。**
           `docs/decisions/0004-m1-pre-launch-decisions.md` **早就補好了** ——
           `§2.1-a` 是一整節更正(含兩個來源、PGroonga 替代路線、`grep -c 'pg_jieba'` ⇒ 6 的
           分母盤點、以及「`Q3` 的分詞路線**需要 Sean 重新拍**」那條待辦);
           連 `:35` 那一列與 `:133` 的 M-6 checklist 都各自帶著警告。
           落點:`ae1ca3cd`(2026-08-16)—— **與寫下上面那半句的 `c6e9826d` 是【同一天】。**
        📌 **⇒ 而線索一直就在這一句自己的括號裡:「另有窗在補那份 ADR」** ——
           那個窗補完了, **而這句話沒有回來把自己改掉。**
           ⇒ 一句「那邊還沒補」的註解, 在那邊補完之後**不會有任何東西讓它變紅**;
