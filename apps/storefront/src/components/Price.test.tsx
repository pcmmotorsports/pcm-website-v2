 ...
 *   現行雙向突變見各組標題旁的註記。
 *
 * 🔴🔴 **本檔有兩層守門, 突變之後【一格都不紅】** —— 它們執行期冗餘、意圖承重:
 *   `Number.isFinite`               ⇒ `Number.isInteger` 已經擋掉 Infinity / NaN
 *   `isRenderableOriginalPrice` 的 `> 0` ⇒ 三個呼叫點都跟著 `originalPrice > price`, 那個比較先擋掉 0
 *   ⇒ **兩層都留著**(哪天有人放寬 `Number.isInteger` 或拿掉那個 `> price`, 它們就是唯一防線),
 *     **而它們的自述必須說實話。** 一個「該紅而沒紅」的突變, 最省事的處置是不寫 ——
 *     它不會出現在任何報表上。
 */
