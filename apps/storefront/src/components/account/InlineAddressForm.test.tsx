...
//    突變 A:拿掉元件裡那個 <div>          ⇒ **`出現` 與 `跟著變` 兩格都紅**
//    突變 B:灰字寫死一個字串、不跟著 input ⇒ **只有 `跟著變` 那格紅**(打第二筆地址時值不會變)
//    (三格的簡稱 = 它們測試名的關鍵字:
//       `出現`   = `it('打了地址 → 下方灰字出現,且內容逐字等於輸入值')`
//       `跟著變` = `it('🔴 換一筆地址 → 灰字**跟著變** …')`
//       `空白`   = `it('地址空白 / 只有空格 → 不掛空灰字')`)
//
//    🔴 **A 那一行 2026-08-27 訂正過** —— 原本寫「⇒ 第 1 格紅」, 而實跑是**兩格都紅**
//       (`npx vitest run` 本檔 ⇒ `Tests 2 failed | 23 passed (25)`)。
//       ⇒ 原本那張 1:1 對應圖(A→格1 / B→格2)**比實際乾淨**:
//         實際是 **`跟著變` 抓得到 A 也抓得到 B;`出現` 只抓得到 A**
//         ⇒ 在這兩發突變的範圍內, **`出現` 被 `跟著變` 涵蓋**。
//       ⚠️ **而 `出現` 那格不刪**:它斷的是「首次輸入就出現且逐字相等」= 一個**與突變無關的正向行為**。
//          📌 **「被另一格涵蓋」與「沒有用」是兩件事** —— 前者是量測結果, 後者是價值判斷。
//       ⚠️ 上面那兩個「N 格紅」**會隨著本 describe 增減測試而變** ⇒ 它們是
//          **在 commit `88a9fca8` 當下量的**, 不是永久事實。
//    ⚠️ 只寫「有沒有這個節點」擋不住 B —— **一個永遠顯示同一句話的灰字,畫面上完全正常**。
//       所以 `跟著變` 那格**換過一次值再斷言**;只斷言一次的話,寫死的那版照樣綠。
//    `空白` 那格守空字串:沒輸入時不該掛一行空灰字(那會在表單裡留一道莫名其妙的間距)。
//
// 🔴 **2026-08-27:本段原本用「第 1 / 2 / 3 格」指涉, 已全部改成【測試名的關鍵字】。**
//    位置會因為「有人在中間插一格」而變 —— 而名字不會。
...
describe('InlineAddressForm 手機欄輸入法屬性', () => {
