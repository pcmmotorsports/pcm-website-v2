...
describe('BrandPageMedia · 圖說與外開連結', () => {
  ...
  it('🔴 三種來源都沒有 → 連結整個不建(不是印一個 ?v=undefined 的死連結)', () => {
    // 設計稿 :1890 的 else 分支無條件組 youtube watch 網址;真資料 15 家都有來源
...
describe('BrandPageMedia · 15 家實資料', () => {
  it('每家都渲染得出封面 + 圖說,且封面圖路徑指得到資產', () => {
    ...
    // 2026-09-16:14 → 15,`ohlins` 補上官方品牌影片(Sean 0916「影片記得拿來用」;出處見 sources 檔同日段)。
    expect(videos.length, '前提:實測 15 家有影片').toBe(15);
...
// 15 家走這條分流(2026-09-16 重數;原 14 —— ohlins 上影片。更早:2026-09-08 原 13,WRS 上影片 `525717793`)。
