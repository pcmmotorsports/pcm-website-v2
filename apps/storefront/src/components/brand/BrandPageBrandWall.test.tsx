...
describe('BrandPageBrandWall · 前提(資料形狀)', () => {
  it('🔴 26 家、slug 不重複、logoScale 恰 7 家非 1(範圍 0.88-1.08)', () => {
    expect(SLUGS).toHaveLength(26);
    expect(new Set(SLUGS).size, 'slug 重複會讓 React key 撞號').toBe(26);
...
describe('BrandPageBrandWall · 版型', () => {
  ...
  it('🔴 每一家當作當前品牌時,磚都還是 26 塊(不把自己濾掉)', () => {
    for (const slug of SLUGS) {
      const { container } = render(<BrandPageBrandWall currentSlug={slug} availableSlugs={ALL_SLUGS} />);
      expect(container.querySelectorAll('.bp-others-list > a'), slug).toHaveLength(26);
      cleanup();
    }
  });
  ...
  it('slug 不在名單裡時,26 塊全部不標記(不是丟錯或標到第一家)', () => {
    ...
    expect(container.querySelectorAll('.bp-others-list > a')).toHaveLength(26);
