...
describe('Header', () => {
  ...
  /* 🔴 **R2-3 那條可及性驗收【沒有作廢,只是暫時不適用】**(入口隱藏中)。
     ...
     🔴🔴 **2026-08-25 `.skip` → `.fails`(`#869` 存量掃描,cf 線)。上面那段推理【對了一半】:**
     `.skip` 確實比註解掉好(看得見),**而「看得見」不等於「會叫」** ——
     ```
     入口回來的那天 ⇒ aria-label 出現 ⇒ 這條斷言會【通過】
     而 .skip 讓它【根本不會跑】⇒ 沒有任何東西告訴任何人「可以把它打開了」
     ⇒ 「skipped 數 = 1」在【入口還藏著】與【入口已經回來】兩個世界印【同一個數字】
     ```
     ✅ **`it.fails` 把那一刻接上**:入口回來 ⇒ 斷言通過 ⇒ `it.fails` 判定「它竟然沒失敗」⇒ **紅**。
        **紅在有人能處理的那一刻**(同 `applied-migration-pairing.test.ts:17-24` 記載的機制)。
     **⇒ 復原方式改成:入口回來時它會【自己紅】,那時把 `.fails` 拿掉即可,不必有人記得。**

     🔴🔴 **而 `it.fails` 有一個洞,已實測、不假裝沒有(2026-08-25 三個世界,1b 指定要驗)**:
     ```
     ① 現況（入口藏著、找不到 label）      ⇒ 28 passed | 1 expected fail   🟢 綠
     ② 讓斷言通過（指向真的存在的標籤）    ⇒ 1 failed | 28 passed          🔴 紅且指名 ← 它存在的理由
     ③ 選擇器換成【永遠不存在】的字串      ⇒ 28 passed | 1 expected fail   🟢 【也綠】
     ```
     ⇒ **`it.fails` 不在乎【為什麼】失敗。** 若哪天是元件改名 / 選擇器失效 / render 拋錯
  ...
  /* 🔴🔴 上一格的**配套**,不是可有可無 —— 它把「為什麼失敗」的一半釘住。
     上一格是 `it.fails`,而 `it.fails` **對失敗的原因是盲的**(實測世界③:選擇器換成永遠不存在的
     字串,它**照樣綠**)⇒ 若哪天 render 拋錯、或 `getByLabelText` 這條路整個失效,
     上一格會**安靜地繼續綠**,而它守的那件事早就無關了。
     ✅ **本格證明的是「上一格的查詢器在這個 render 底下是活的」** ——
        它找得到別的 label ⇒ 上一格的失敗**只可能來自「搜尋」這個 label 不在**。
     ⚠️ **它證不到的**:`getByLabelText('搜尋')` 這一條**本身**有沒有被寫錯(拼字/全形半形)。
        那一半沒有守門,而入口回來時上一格會紅、有人會回來看,那時就會發現。 */
  it('🔴 上一格的量具自檢:同一個 render 下,查詢器找得到【別的】label', () => {
    renderWithCart(<Header isMobile={false} />);
    // 正向對照:這三個 label 是桌機 Header 一定會渲染的(見 Header.tsx 的 aria-label)
    expect(screen.getByLabelText('購物車')).toBeTruthy();
    expect(screen.getByLabelText('會員')).toBeTruthy();
    // 負向對照:一個永遠不存在的 label 必須找不到 —— 否則這把查詢器在亂答
    expect(() => screen.getByLabelText('這個標籤永遠不存在zzz')).toThrow();
  });
