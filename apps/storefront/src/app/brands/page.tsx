...
//
// 這條 route 自 D2b 起就被指著、但一直不存在(= 死連結)。D3c-3 先把 route 生出來,
// **sitemap 於 D3c-4 補、消費端收口與 #314 redirect 於 D3c-5 補**(見檔尾清單)。
//
...
//       (只判 `=== 'home'`)不受影響。
//    ✅ 那顆的 **href 也已於 D3c-5 改回 `/brands`**(當時延後是因為它動到全站每一頁的導覽
//       目的地 + `Header.test.tsx` 的對照表,值得自己一片自己驗)。
//
...
// **這條 route 的相關工作已全部完成**(D3c-5 收尾;這張清單每次有人動到相關檔案就要回頭改 ——
//  D3c-4 與 D3c-5 的關卡2 R1 各抓到它過期一次)
// ✅ ① 消費端收口 —— **D3c-5 完成**:站台 `Header` 的「品牌」、`HomeFooter` 的「品牌專區」、
//      `BrandIndex` 表頭的「品牌專區」(這第三顆是 D3c-5 R1 抓到的漏網)全部指 `/brands`;
//      兩支麵包屑本來就指對、不用改。
// ✅ ② `#314` 的 client redirect —— **D3c-5 完成**(`components/brand/BrandAboutRedirect.tsx`)。
// ✅ ③ sitemap —— **D3c-4 完成**(總覽進 `STATIC_SITEMAP_PATHS`、20 個介紹頁由 `brandSlugs` 產生)。
