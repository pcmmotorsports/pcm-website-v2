...
//    【沒有 `.pdf`】。**~~
// 🔴🔴 **2026-09-06 訂正:上面那一整段【過期了】,而它現在會把人指反方向。**
//    `.pdf` 那條路 2026-08-31 就落地了(`../statement.pdf/route.ts`,片 C3,伺服器產檔),
//    而 Sean 2026-09-06 在**正式站**對一張真單下載過、主視窗親讀 PDF 中文全字。
//    ⇒ ✅ **那顆鈕現在指的就是 `.pdf`**(主視窗裁甲;依據=稿 `:288` 逐字, 鐵則 1)。
//    ⇒ 🔵 **而這一頁【留著不刪】** —— 有人書籤了也不會壞, 只是訂單明細頁不再連它。
//    📌 **舊字面留刪除線**:拿「沒有 `.pdf`」去搜的人要在同一發撞到這裡。
//    (原文續下,它講的「不要接成發票」那一半**仍然成立**:)
// 🔴 **給片 C(那顆鈕)的人:這一頁的 URL 是 `/account/orders/<displayId>/statement`,** 而 `OrderDetailView.tsx:9` 那段「刻意不做」的註解寫的是
