...
//    ✅ **2026-09-06 訂正:本檔【早就改好了】** —— `30cfe8374`「新鮮度閘升硬斷言」
//      **同一顆就把後台與顧客站兩支一起改了**, 而上面那句「本檔還沒改」留在原地沒有跟著更新。
//      現況見下方那格逐字:`it('🔴 這份追蹤清單不比它守的原始碼舊 —— 舊了就紅, 不是印一行給你自己看')`
//      + `expect(stalenessNote()).toBeNull()`。
//    🔬 **而它不是「看起來改好了」—— 當場突變驗過**(`-ship` 2026-09-06):
//      `touch apps/storefront/next.config.ts`(= 改了碼沒重 build)⇒ **那一格當場紅**,
//      訊息逐字「這份追蹤清單比它守的原始碼舊 6 分鐘(1/9 支比它新:…next.config.ts)」
//      並直接給重 build 的指令;重 build 後複綠。
//    🛑🛑 **這一段留著不刪, 因為它自己就是那個病的標本**:
//      ⟦ship-STALEGUARDWARN⟧ 那一列(與這段註解)寫著「還沒做」, 而**碼裡早就做完了**
//      ⇒ 📌 **一句過期的「還沒做」會讓下一個人去做一件已經做完的事** ——
//        而他不會懷疑它, 因為那句話就寫在檔案自己身上。
//
