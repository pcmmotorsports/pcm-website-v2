...
export async function chargePaymentAction(input: unknown): Promise<ChargePaymentActionResult> {
  ...
  try {
    ...
    const placeOrderInput: PlaceOrderInput = {
      ...
      //       而它現在是候選① ⇒ 那張單所有通知信都寄去那裡, 而註冊信箱明明是好的。
      //       **舊順位下這個人收得到, 新順位下收不到。**
      //    🔴🔴 ⛔ ~~原本這裡寫「靜靜寄到一個【不存在的信箱】」~~ —— **那句低估了一號**
      //       (2026-09-20 R2 補正)。`@gmial.com` 這一類**不是不存在**, 它們是**真的有人在營運的
      //       仿冒網域(typosquat)** ⇒ 信會被【第三方收到】, 而信裡有:收件人姓名 + 電話 +
      //       完整地址 + 金額 + 訂單編號。
      //       📌 **⇒ 這不是「少一封信」, 是【PII 外洩】。兩件事, 不要讀成同一件。**
      //       🛑 **而 Sean 是在知道這個完整代價之後才拍甲的, 而且仍然不加防呆。**
      //    ⚠️ 上面兩張臉**都不要自己加防呆** —— 加了會把他拍的那件事(附註 + 地址優先)繞掉。
