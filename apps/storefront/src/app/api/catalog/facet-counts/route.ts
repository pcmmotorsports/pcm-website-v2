// app/api/catalog/facet-counts/route.ts — 「車 × 已選品牌 / 已選分類」下的各分類 / 各品牌件數(backlog #306)。
//
// 為什麼要有這支 route:件數目前是 `/products` 的 server props 一次帶下來的,面板內沒有取數管道;
//   而 Sean 拍板 A 案 = 選了才算(不拖慢 `/products` 首次載入)⇒ client 需要一個能事後問的入口。
//   2026-09-12 Sean 拍乙:車變成可選,只選品牌 / 分類也要連動(`?pbrands=` / `?categories=`,
//   與 `/products` 同一支 `parseCatalogQuery`)。
//
...
//
// 🔴 車輛參數三道白名單(公開端點,不可讓人用亂數字串驅動 DB 查詢;⛔ ~~放大係數 108~~
//   2026-09-12 起一次冷請求 = 一發 `catalog_facet_counts`,而有車時最壞一發約 4 秒):
...
export async function GET(request: Request) {
  ...
  const vehicleSlug = query.vehicle;
  // 🔴 帶了 vehicle 而形狀不合法 ⇒ 400,**不得**退成「沒車」去算全目錄:
  //    客人問的是那台車的件數,拿全目錄的數字回他 = #306 的病灶本身。
  if (url.searchParams.get('vehicle') !== null && !vehicleSlug) {
    return NextResponse.json({ error: 'invalid_vehicle' }, { status: 400, headers: NO_STORE });
  }
  // 2026-09-12(Sean 拍乙):車變成可選 —— 只選了品牌 / 分類也要連動。什麼都沒選 ⇒ 前端用
  //   server 帶下來的全站數、根本不會來問 ⇒ 這裡 400 且不碰 DB。
  if (!vehicleSlug && query.brandSlugs.length === 0 && query.categories.length === 0) {
    return NextResponse.json({ error: 'nothing_to_count' }, { status: 400, headers: NO_STORE });
  }

  let vehicle: FacetVehicle | null = null;
  if (vehicleSlug) {
    ...
    const parsed = parseVehicleFromUrl(
    ...
    if (modelId && parsed.model === undefined) {
  ...
  // ④ 已選篩選的字典白名單:品牌必須在品牌表、分類必須是分類樹裡的 key。
  // 🔴 認不得就 400,**不得**默默丟掉那一顆再算 —— 列表那一側會照單全收(例如舊書籤的下架品牌
  //    ⇒ 右邊 0 件),丟掉之後側欄會印出「沒有那個條件」的數字 = 面板與列表對不起來。
  const categoryKeys = categoryFacetKeys(categories);
  const brandSlugs = brands.map((b) => b.id);
  if (
    query.brandSlugs.some((slug) => !brandSlugs.includes(slug)) ||
    query.categories.some((c) => !categoryKeys.includes(c))
  ) {
    return NextResponse.json({ error: 'unknown_selection' }, { status: 400, headers: NO_STORE });
  }

  const counts = await fetchFacetCounts(vehicle, categoryKeys, brandSlugs, {
    categories: query.categories,
    brandSlugs: query.brandSlugs,
  });
  if (!counts) {
    // 取數失敗(已在 lib 層 console.error)⇒ 明確告訴 client「這次算不出來」,
