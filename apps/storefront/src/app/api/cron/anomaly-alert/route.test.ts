...
describe('安靜日心跳 —— 位置就是它的正確性', () => {
  it('🟢 沒踩門檻 + 全部檢查都過 ⇒ 寄一封心跳, 而 route 回 200', async () => {
     ...
     */
    /**
     * 🔴🔴 **[2026-09-07 · 白名單再開一行, 而【那也是他本人拍的】]** ⟦QB-10⟧
     *    選項逐字(`~/pcm-mailbox/端Sean-0905早上佇列.md:1785`):
     *    「甲=把死信數從『要不要響鈴』拿出來, **改成每天巡檢信裡一行數字**」;他的答案 `:1912`。
     *    ⇒ 上面那句「要加請重新問 Sean」**照做了**, 而他答了 ⇒ 白名單再窄地開一行。
     *
     * 🛑🛑 **而我在這裡量到一個【洞】, 不是放寬**:死信那一行的量詞是「**封**」不是「筆」
     *    ⇒ 我把那一行加進信裡之後, **這一格照樣全綠** —— 它根本沒看見那一行。
    ...
    // 🟢 死信那一行同理 —— 本 fixture `emailDeadLetterCount: null` ⇒ 它應該寫【查不到】。
    expect(msg?.text).toMatch(/^已經放棄、【永遠不會再寄】的信:查不到 封$/m);
  });

  /**
   * 🔴🔴 **[code-reviewer R1 nit 4:白名單那一半【沒有任何測試餵過】]** ⟦QB-10⟧
   *    上面那格的 fixture 是 `emailDeadLetterCount: null`(同檔 `:193`)
   *    ⇒ 真訊息那一行是「查不到 封」、**零個數字** ⇒ 白名單裡 `\d+` 那一半空轉,
   *      而它是**唯一**會被那把 `[筆封]` 尺量到的形狀。
   *    ⇒ 📌 **合成字串的正對照證明尺會咬人, 證明不了【真訊息】走過那條路。**
   *    ⇒ 本格讓真訊息帶一個數字走一次。
   */
  it('🔴 死信有數字那天:真訊息走一次 `N 封`, 而白名單放行它、其餘計數照樣擋', async () => {
    checkSpy.mockResolvedValue({ ...CLEAN_RESULT, emailDeadLetterCount: 4 });
    const res = await GET(makeReq(bearer(SECRET)));
    expect(res.status).toBe(200);
    expect(okNotify).toHaveBeenCalledTimes(1);
    const msg = (okNotify.mock.calls as unknown as { subject: string; text: string }[][])[0]?.[0];
    // 🟢 這仍然是那封安靜日心跳 —— 死信不再響鈴, 所以它不該變成告警信。
    expect(msg?.subject).toBe(ANOMALY_QUIET_HEARTBEAT_SUBJECT);
