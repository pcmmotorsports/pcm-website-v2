...
//      🔴 **更正(2026-08-22 線 C 正式庫唯讀實測)—— 原句一句話涵蓋了兩段,而它們的狀態不同:**
//        ① **認領段【已被行使】** —— `payment_charge_attempts` 實測:
//           `settle_attempt_count > 0` 的列 = 2(兩張皆 max 8)⇒ **sweeper 真的跑過 16 輪**
//           ⇒ 原句「未被行使」對這一段是**錯的**。
//        ② **`settled` 寫入段確實 0 次** —— 原句對這一段是對的。
//        ③ 🔴 **而 ② 的原因不是「它失敗了」,是【至今沒有一張單符合條件】**:
//           那兩張 `2SQH2P` / `GVRDMH` 在 TapPay 側分別是 PENDING(尚未授權)與查無
//           ⇒ **sweeper 不把它們推成 paid 是【正確行為】。**
//      ⇒ ⇒ 🔴 **所以現在的誠實句是**:「認領段已驗;`settled` 段未跑過,
//        而**『它沒救過任何一張單』與『它救不了』在紀錄上長得一樣,現有證據分不開這兩個**。」
//        要分開,唯一的路是**構造一個合格案例**(拋棄式 PG 做得到)⇒ 那是一片工。
//      ⚠️ 上述量測的限定:`settle_attempt_count` 只在 claim 時遞增;
//        **若有別的路徑會清掉它,那個 2 會偏低 —— 線 C 未查有無此路徑,標未確認。**
