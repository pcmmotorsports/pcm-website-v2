...
> # 🔴🔴 **先讀這句再往下看：下面的 `8173` / `223` / `2.66%` 【已作廢】**
> **只有 `8396` 可引用。** 作廢的理由在下一個框（那句 SQL 只查了 view `UNION ALL` 的上半）。
> **原文保留在下方是為了讓「當初就錯」看得出來，不是因為它還成立。**
...
⇒ ~~差 223 筆 = 2.66%。照原先定下的判準（差很少 ⇒ 純安全修補）⇒ 產品決策關解除。~~ 🔴 **已作廢，見下框**
