...
```
頁大小 < db-max-rows（200 < 現行值）✅  .range() 兩端皆含 ✅
中途失敗 throw 不 break ✅             count 不當終止判準 ✅
排序帶唯一鍵（.order('id')）✅
```
> 🔴 **2026-08-18 更正:這一格原本寫死「200 < 1000」,而那個 `1000` 已經過期。**
> **結論沒變**(200 在新舊兩個值之下都嚴格更小)—— **過期的只有理由裡那個數。**
> ⚠️ **新值我方未自驗**:I 窗轉述 V 窗對正式站實測
> (`products?select=id&limit=5000` ⇒ HTTP 206、`content-range 0-1999/19777` ⇒ 指向 **2000**)
> —— **主視窗與 I 窗均未自驗,我也沒有。標【轉錄,未自驗】。**
> 🔴 **所以這裡刻意不再寫死任何數字**:這個值**一年之內已經變過一次**,
> 寫死等於排定下一次過期。**要現值就去量,量法就是上面那一發請求。**
> 📌 判別句:**一個會變的數字寫進「證明」裡,那個證明就有保鮮期,而它不會自己告訴你過期了。**
