...
> # 🔴🔴 **本節的數字已作廢（2026-08-16，V 窗讀碼指出，我重跑確認）**
>
> **原本寫：控制組 3 列 / 300 元。那是【我這個 stub 的性質】，不是系統的性質。**
>
> **我的模型少了兩道真實系統有的後備**（我對著 `20260810170000` 逐項核過）：
>
> | 後備 | 真實系統 | 我原本的 stub |
> |---|---|---|
> | ① `FOR UPDATE` 臨界區 | ✅ | ✅ 有 |
> | ② **條件式 `UPDATE … WHERE payment_status='unpaid'`** | ✅ | 🔴 **沒有**（我只寫 `WHERE id=`） |
> | ③ **`UNIQUE` backstop** | ✅ | 🔴 **沒有** |
>
> **帶滿三道之後重跑（同樣三個入口同時結算一張 100 元的單）：**
>
> ```
> 🔴 控制組（只拿掉冪等樹，三道後備保留）
>      三個回傳：REJECT(非 unpaid) / REJECT(非 unpaid) / CHARGED
>      order_payments：1 列 / 合計 100 元        ← 🔴 【沒有】重複入帳
>
> ✅ 有冪等樹
>      三個回傳：IDEMPOTENT / CHARGED / IDEMPOTENT
>      order_payments：1 列 / 合計 100 元
> ```
>
> ## ⇒ 真正的結論（與原本相反）
>
...
> **冪等樹改變的是【回給呼叫端的值】**：
> ```
> 有樹  → 落敗者收到 IDEMPOTENT（成功）
> 無樹  → 落敗者收到 REJECT（失敗）—— 而那張單【其實已經付款成功了】
> ```
> ⇒ **拿掉它的真實後果是「上游把一張已成交的單當成失敗處理」，不是「重複入帳」。**
> 📎 **而那正是我自己在冪等規格 §2.1 寫過的那條**：「回失敗會讓上游把已成交的單當失敗處理。」
> **我寫了它，然後在框架裡用了相反的說法。**
>
> 🔴 **這次的病根**：**我的控制組把【要證的那道】和【本來就有的其他道】一起拿掉了**
> ⇒ 我以為在測「冪等樹的價值」，實際在測「三道全拿掉會怎樣」。
> **⇒ 突變要【只動一個變因】。**

~~原始輸出（保留供對照，數字已作廢）：控制組 3 列 / 300 元；有冪等樹 1 列 / 100 元。~~
