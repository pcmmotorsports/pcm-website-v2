...
### 🔴🔴 1.2-b 本檔有**兩個 `16`，單位不同** —— 引用前先看這一格

**它們在句子裡長得一模一樣，而對帳的時候會互相「驗證」。**

| 出現在 | 這個 16 是什麼 | 單位 | 配套數字 |
|---|---|---|---|
| §1.2（P5） | 全庫**成本／經銷／毛利類欄位**的總數 | **欄位** | anon 讀得到 **0**、對照 postgres **16** |
| §1.3（更正後） | `anon` 真正碰得到的**關聯**數 | **關聯**（表／view） | 表級版誤報為 15、欄級版 **16** |

🔴 **它們沒有任何關係，數值相同是巧合。**
⇒ 引用時**一律帶單位**：「16 個**欄位**」或「16 個**關聯**」，**不要寫「16」。**
📎 這是「數字離開量測現場要帶著範圍」的實例，而**同值不同單位是最毒的一種** ——
**因為對帳的人看到兩邊都是 16，會以為互相印證。**
