...
✅ **恢復對照(第三格)已補**:
```
07:46:25 UTC  查規則狀態 → rateLimit.action 仍為 deny（未改回 Log）
07:46:39 UTC  1 發        → 400 {"error":"invalid_vehicle"}
```
🔴 **這一格只有趁 `Deny` 還開著才做得成**:一旦改回 `Log`,**所有請求都回 400**(不論有無命中規則)⇒ 那時再打,「400」在「已恢復」與「規則根本不擋」兩個世界是**同一個值**,這格即作廢。
⇒ 在**規則仍為 `deny` 的前提下**打到 400 ⇒ 證明擋是**限流性、隨視窗自動恢復**的,**IP 未被永久列黑**。

⇒ **三格到齊,實驗閉環**:

| 格 | 時點(UTC) | 觀測 | 排除掉的世界 |
|---|---|---|---|
| ① 通過 | 07:35:06–14 | 19 發 400(app JSON) | 「規則把所有流量都擋掉」 |
| ② 被擋 | 07:35:14–15 | 6 發 403(邊緣 + `hnd1::`) | 「規則沒生效」 |
| ③ 恢復 | 07:46:39 | 1 發 400(規則仍 `deny`) | 「我的 IP 被永久封鎖」 |
