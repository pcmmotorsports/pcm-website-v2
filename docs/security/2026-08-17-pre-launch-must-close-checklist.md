...
> 🔴 **2026-08-24 加註(督導窗 pcm-website-v2-7c;原句一個字未改)· 前置 = `B5-a`**
> 🟢 **[2026-09-08 `-auth`]** 本條的前置 `B5-a` **2026-08-24 已落地**
> (`apps/admin/src/lib/session/session.ts:96` 逐字「`sub` 是【必填】, 不得寫成 `sub?:`」;
> spec `:814` 逐字「已於 2026-08-24 `B5-a` 落地時退場刪除」)。
> 🔵 而 `B5-b` 的**第四件**同樣已落地(`apps/admin/src/app/page.tsx:79-80` / `:100`)。
> 🛑 **而本註記【不宣稱本條可以關】** —— `:487` 說的是「三條一起**往前**」不是「一起關」,
> 而「已落地」是從**碼 + spec 退場紀錄**判的:**沒有人跑過一次真的登入**, 也**沒有逐條對 spec 規格**。
> 🔴 而 `B5-b` 是**繞過** `#17` 那個問題(「proxy 能不能載 server adapter 查 A 庫」)**不是解掉**,
> 而**怎麼繞的沒有人查** ⇒ 那是一個**開著的問題**。
> ⇒ 📌 它解掉的是【前置】, 不是【這一條】。
> **這三條收斂到同一件事:`E8-B` 的 `B5-a`。** ⑯ 的補齊需要「員工用自己的帳號登入」,而那正是 B5 那條線。
> ```
> ⑯ 共用密碼 · ⑰ session 踢不掉 · ⑲ 查不出誰做的
>    ⇒ 三條【不是三條線】。B5-a 做完,三條一起往前。
> ```
> ⚠️ **排工時不要當三條各派一個窗** —— 那會讓同一段權限敘述被三個人各改一次。
> 📌 而 B5 的狀態 2026-08-24 才被查清楚:**E8-B 七片已做四片(B1/B2/B3/B4),卡的是 B5,而 B5 在【我們這一邊】**。
>    B5 spec 內文那句「B4 零規格零實作」已過期(B4 於 2026-08-21 上線)——
>    更正與三發證據見 `docs/specs/2026-08-17-m4b-e8b-b5-spec.md` §B5-6 下方 2026-08-24 註記。
> 🔴 **本註記【不】宣稱這三條會被 B5-a 關掉** —— 它只說前置在哪。
>    ⑯ 的驗收(判別器 = **寫入擋不擋**,不是進不進得去)照本節 2026-08-18 更正後那版,不變。
...
> 而**怎麼繞的沒有人查** ⇒ 那是一個**開著的問題**。
> ⇒ 📌 它解掉的是【前置】, 不是【這一條】。
> **這三條收斂到同一件事:`E8-B` 的 `B5-a`。** ⑰ 要能「踢掉某一個人」,前提是系統先知道 session 屬於誰(= B5 的 `sub`)。
> ```
> ⑯ 共用密碼 · ⑰ session 踢不掉 · ⑲ 查不出誰做的
>    ⇒ 三條【不是三條線】。B5-a 做完,三條一起往前。
> ```
...
> 而**怎麼繞的沒有人查** ⇒ 那是一個**開著的問題**。
> ⇒ 📌 它解掉的是【前置】, 不是【這一條】。
> **這三條收斂到同一件事:`E8-B` 的 `B5-a`。** ⑲ 本節自己就寫著「⚠️ 它的前置是 E8-B(身分先要是真的)」。
> ```
> ⑯ 共用密碼 · ⑰ session 踢不掉 · ⑲ 查不出誰做的
>    ⇒ 三條【不是三條線】。B5-a 做完,三條一起往前。
> ```
...
| 條 | 判定 | 病 |
|---|---|---|
| ④ `E686` net 兩表 | 🟡 **曝露面已實測 · 權限面仍開著**(2026-09-02 線 `-0e` 量、`-0a` 裁、`-f3` 落檔) | ⛔ ~~「規格已落檔、已指派」是狀態~~ ⇒ ⛔ ~~現為 `scripts/check-anon-grants-prod.sh` 的欄級查詢;**但未對正式庫跑過**~~<br>🟡 **2026-09-02 訂正 —— 兩個面, 而它們的狀態【不同】**:<br>　✅ **曝露面**:2026-09-02 16:3x **第一次真的對正式站跑**(`scripts/probe-schema-exposure.sh`, 兩個庫各 15 格 / 共 **30 格全 PASS** / FAIL 0 / rc=0)⇒ **實測不可達**。<br>　🔴 **權限面收不掉**:`postgres` **非 superuser** ⇒ `REVOKE` 對 `supabase_admin` 的 grant 是**靜默 no-op**(規格 §2.5)。<br>🔴🔴 **⇒ 所以本項的關閉條件是【偵測有排程】, 不是【權限被收】。**<br>🛑 **⇒ 而這一句是用來擋住下一個人的**:規格 §2.5 逐字預言過那個動作 ——「它會**跑成功**而什麼都沒發生 ⇒ **那比失敗更糟, 因為會留下一份【我修好了】的錯誤紀錄。**」⇒ 📌 **不要寫 REVOKE migration。**<br>⚠️ **而「跑過一次」不等於「持續」** —— 規格 §3.1 逐字「**偵測頻率 = 暴露窗**」⇒ 那一格另開一列 `⟦0e-PROBENOSCHED⟧`。 |
| ③ `E683` 新表 anon | ✅ **主體 2026-08-18 已 apply · 而剩兩格【不是查過沒事, 是沒有人查過】**(2026-09-02 線 `-0e` 量、`-0a` 裁、`-f3` 落檔) | ⛔ ~~有方法、沒有可執行的斷言~~ ⇒ ⛔ ~~現為同一支腳本的 `pg_default_acl` 段(含對照組先驗量具);**未對正式庫跑過**~~<br>🔴🔴 **2026-09-02 訂正 —— 而【表格與內文互相矛盾】才是本次要修的東西**:本欄至今寫著「未對正式庫跑過」, **那是 08-18 的值**;而同檔 `:149-160` 的內文 **2026-08-23 就已經就地更正**「那一半已經清掉了」(附量法與負對照)。🛑 **⇒ 而主視窗 2026-09-02 就是照【這個表格】把人派出去的** ⇒ 📌 **一個過期的欄位, 派出了一趟工。**<br>✅ **現況**:`20260817060000_e683_1_public_default_privileges_revoke.sql` 已 apply(`grep -c '^20260817060000' supabase/APPLIED.tsv` ⇒ **1** · 🔵 負對照 `^19990101000000` ⇒ **0** ⇒ 尺會動);Sean 2026-08-23 **本人實跑** `scripts/check-anon-grants-prod.sh` 驗收, 兩發對照組皆過。<br>🔴 **而剩兩格 —— 【不是查過沒事, 是沒有人查過】**:<br>　**① sequence** —— E683 只 `REVOKE ALL ON TABLES`, **沒有碰 SEQUENCES**(該檔 `:44-45`)⇒ 那是 `⟦b4-SEQACL1⟧`(**有主**;Sean 2026-09-01 裁「戊」= 獨立安全片)。<br>　**② storage schema** ⇒ 見新列 `⟦0e-STORAGEACL⟧`。🛑 **而它【不得】併進 `⟦b4-SEQACL1⟧`** —— Sean 那個「戊」裁的是 **sequence**, 而 **storage 是另一個東西、他沒看過這個字** ⇒ 併進去 = **替他把裁定的射程放大**。<br>📌📌 **而本欄一度與 `:149-160` 的內文矛盾 ⇒ 【表格與內文要一起改, 而讀的人只讀表格。】**(`-0a` 逐字:**它是這份檔下一次會犯的同一個錯的唯一擋板**。)|
