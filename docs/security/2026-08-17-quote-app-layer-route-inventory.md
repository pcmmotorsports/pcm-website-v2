...
> ```
> 附錄 A-3  ❌ 原「token 永久有效」→ 推翻。307 張 never_expire = 0（Sean B4 實測輸出）
>              code 層的洞還在，但【資料沒有踩到】；窗長 30 或 60 天，真值未讀
...
資料層   今天沒有一列踩到（307 / never_expire 0，量到的）
```
⚠️ **窗有多長 = 未讀**:Sean A3 逐字「有，但是我忘記是30天還是60天」
⇒ 📌 真值可讀:去報價單專案讀**產快照時寫 `expires_at`** 的那一段(Sean 授權「你們可以去讀」)。
**在讀到之前,30 與 60 都只能寫【未確認】,不要挑一個寫進報告。**
...
· ~~永久有效（A-3）⇒ 一旦流出，沒有自然失效點~~
  🔴 已更正:307 張 never_expire = 0 ⇒ 有自然失效點，窗長 30 或 60 天（真值未讀，見 A-3）
  ⇒ 一旦流出，【在那個窗之內】沒有辦法主動作廢它（沒有撤銷機制）—— 這半仍然成立
```
...
token 是否出現在任何 log。
🔴 **新增一項未做**:讀報價單專案寫 `expires_at` 的那段 code,取回 30 或 60 天的真值(A-3)。
...
⚠️ 但它只證明**那一刻的正式庫**;**到期天數(30 或 60)仍是未讀**。
