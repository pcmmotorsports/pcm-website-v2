...
## A-3 🔴 有效期:**可以是永久,而那是設計允許的**

```
lib/quote-token.ts:16-21  isQuoteExpired(expiresAt)
  if (!expiresAt) return false;      ← 🔴 null = 【永不過期】
```
...
`expires_at IS NULL` 的實際張數(缺權限)、token 是否出現在任何 log。
...
**A-4 的「分不出來」= 推論,非量到。** `expires_at IS NULL` 張數 = **查不到,缺有權限的角色**。
...
✅ **而 2FA 開啟時 legacy 整條被拒**(`if (!state.require2fa)`)⇒ **2FA 是這條的解藥,已經在 code 裡。**
