...
### 6-b.4 🔴 修法要引的是**先例**，不是只引拍板

🔴 **【2026-08-17 更正】下面這條拍板已被 Sean 本人改判,而本節的結論【不受影響】—— 這正是本節在講的事,現在活生生示範了一次。**

~~Sean 2026-08-16 睡前答 `Q2` = **甲：撈不完整就整區失敗、不顯示任何一列**（＝**警告不是解法**）。~~ **← 已過期,勿引為先例。**

**現況(2026-08-17 下午)**:Sean 改判 **`Q2` = 乙 = 顯示已取得的 + 標示不完整**。
🔴 **不是他反覆**:08-16 答甲時**沒有前例可對照**;08-17 主視窗補上三個既有前例(訂單頁關鍵字截斷 amber banner 註解逐字「0 筆也要顯示」/ `itemCountTruncated` 印「?」`Q-EMBED-1` 08-16 他批的 / 退款例外清單=顯示部分+橫幅、且是**錢族**)後,他在**完整前提**下改判乙。**兩次都是他真心的,差別是有沒有前例。**(memory `project_0817-sean-nine-rulings-afternoon` Q-5)

🔴 **而本節的結論(facet-counts 上游失敗 → 503、不回半套)不因這個改判而變,兩個理由**:
1. **本節從頭就不是靠那條拍板** —— 靠的是下方既有寫法(facet-counts `route.ts:23-26`)。這正是本節標題「引先例不只引拍板」的意思:**拍板 08-16→08-17 就翻了,而先例還在跑**(§下方 line 251「拍板可以被下一次拍板改」現在成真了)。
2. **範圍不同**:`Q2`=乙 講的是「撈到了【部分資料】要顯示 + 標示」;facet-counts 是**上游字典【整個失敗】、根本沒有半筆資料可顯示** → 503。乙 不涵蓋「沒有資料」那種情形,所以它反轉的是**那條過期引用**,不是這個結論。

⚠️ **但不要只引那條(現已過期的)拍板** —— 這個系統**本來就有這條紀律,只是沒有人把它寫成一條**:

```
apps/storefront/src/app/api/catalog/facet-counts/route.ts:23-26（檔頭，早於 Q2）
  「上游字典失敗一律 503、不得回 200 半套…空字典 ≠ 沒有分類,是【這次查不到】」
同檔 :14-16、:120-126  三支 taxonomy 任一為空 ⇒ 503,不回半套
lib/vehicle-facet-counts.ts 檔頭  任一支查詢失敗 → throw ⇒ 不進快取,
                                  外層 catch 回 null ⇒ UI 退回「不顯示件數」不顯示錯數字
```

🔴 **先例比拍板更難被推翻**：拍板可以被下一次拍板改，
而「這個 repo 的既有寫法本來就是這樣」是一個**已經在跑的事實**。
⇒ 建議修法時**兩個都引，先例放前面**。
...
## 10. finding：`login/forgot` —— **防帳號列舉的那道設計，同時把「信寄不出去」變成看不見**

**嚴重度：中（可用性／帳號救援被 DoS）。歸屬：storefront 施工線。**
🔴 **鐵則 12⑤（對外不可回收：寄信）⇒ 我只出報告，修法要另派 + codex 對抗審查。**

### 10.1 現況（**量到的**）

```
apps/storefront/src/app/login/forgot/actions.ts   全檔 46 行
  無 auth（正確：登入前流程）
  無 code 層節流、無 captcha
    量法 grep -rliE 'rate.?limit|throttle|captcha|turnstile|recaptcha' apps/storefront/src/app/login/
      ⇒ 4 個檔命中，逐一開檔讀 ⇒ 全部是【註解與測試裡提到 rate_limited】，零個是實作
      🔴 正向對照：同一支 grep 對 apps/storefront/src 全樹 ⇒ 20 檔（節流機制在別處是存在的）
  節流 100% 在 Supabase 平台側（檔頭逐字提到 `429 rate_limited`）
```

### 10.2 🔴 兩個各自正確的設計，合起來產生一個沒有人選過的取捨

**設計 A（正確、而且是本片最高風險點的解法）** —— `actions.ts:6-8` 逐字：

> 「不論 Supabase 回成功／帳號不存在／AuthError（**含 429 rate_limited**），一律回同一個空物件
> —— 任何分支差異都會讓這頁變成帳號探測器。」

**設計 B（正確）** —— 節流交給平台，不在 code 裡自己做。

🔴 **合起來**：
攻擊者對這個**無需登入**的入口灌任意 email ⇒ 每一發都試圖寄一封信 ⇒ **燒掉專案的寄信額度**。
額度一旦耗盡，**真正的客人按「忘記密碼」也收不到信** ——
而 **A 讓那個狀態對所有人都是同一個畫面：「信已寄出」**。

> **⇒ 帳號救援整條掛掉，而我們與客人都看不到。**
> **客人會以為信在垃圾桶裡，我們會以為系統正常。**

⚠️ **這不是說 A 錯了。** A 是對的，而**沒有人寫下「A 也讓停機變成隱形」這件事**
—— 檔頭把 429 列進「要吞掉的」，理由只寫了列舉防護那一面。

### 10.3 建議的修法（**便宜，而且不必犧牲 A**）

```
對【客人】的回應維持一模一樣（列舉防護不動）
在【server 端】把 429／AuthError 記下來並告警  ← 現在連 catch 區塊都刻意不看 e 的內容
```

`actions.ts:38-41` 逐字：「**連錯誤本身都不看一眼、故意不判斷 `e` 的內容**」。
⇒ **那個「不看」是為了防列舉，而它防的是【回給 client 的東西】，不是【我們自己記什麼】。**
**兩者可以分開，而現在被綁在一起了。**

📎 引 `§6-b.4` 的先例邏輯：`facet-counts` 的上游失敗是 **503 讓它壞得看得見**；
`forgot` 的檔頭自己也有同款紀律 —— `resolveSiteUrl()` 回 undefined 時**刻意 throw**，
逐字理由：「**該讓它壞得看得見（500）**…吞掉的話每個客人都會看到『信寄出去了』而系統一封都沒寄」。
🔴 **同一個檔已經為【設定錯誤】選了「壞得看得見」，卻為【額度耗盡】選了「安靜」** ——
**而那兩種情況對客人的後果一模一樣：以為信寄了，其實沒有。**

### 10.4 我**沒有**做的

| 沒做 | 為什麼 |
|---|---|
| **實際灌信測額度** | 🔴 **鐵則 12⑤ 對外不可回收** —— 那會真的寄出信、真的燒額度。**不做，也不建議任何人做。** |
| 確認目前用哪個 SMTP 與額度數字 | 需要 Supabase Dashboard；memory 記著 08-08 曾設定 Gmail Workspace，**但我沒查證現況** ⇒ **未確認** |
| 確認 Supabase 端節流的實際窗與粒度（per-IP？per-email？全專案？） | 需查官方文件＋實測；**粒度決定這條的嚴重度**（per-email ⇒ 難燒；全專案 ⇒ 好燒） |

🔴 **所以嚴重度我標「中」而不是「高」，缺的就是上面第三格** ——
**而那一格【不需要動正式站】就能查（讀官方文件即可），只是我還沒查。**
