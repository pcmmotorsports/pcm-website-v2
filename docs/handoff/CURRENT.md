...
> 🔴🔴 **[2026-09-06 · 本檔【下面那一整節】停在 2026-08-28 —— 而 `CLAUDE.md:22` 規定你每個 session 都要讀它]**
>
> ## ✅ 要知道【現在誰在做什麼】⇒ 讀 `~/pcm-mailbox/現在誰在做什麼-<當天日期>.md`
> ```
> ls ~/pcm-mailbox/現在誰在做什麼-$(date +%Y%m%d).md
> 沒有那一支 ⇒ 當場建一個指標檔指到正本(不要搬正本)——
> 跨午夜檔名換一天而沒有東西會去建它, 而【查無】與【今天沒有人在做事】印同一個東西。
> ```
>
> 🛑 **而危險的不是「它舊了」,是【它舊得完全不會叫】** —— 逐條量過:
> ```
> git log -1 --date=short -- docs/handoff/CURRENT.md   ⇒ 2026-08-28  e78047a43   (9 天)
> 本檔下一節逐字寫著「2026-08-28 夜 · 五線在跑」並列出 6 支 進度-*-20260828.md
> 負對照 ls ~/pcm-mailbox/進度-建單面板-20260828.md    ⇒ 🔴 檔【還在】
> ```
> 📌 **⇒ 它的每一個指標都解析得到, 每一個檔都存在, 沒有任何一發檢查會紅。**
> 　 **錯的只有【日期】—— 而日期不會 exit 1。**
> ⇒ 🎯 **同族**:`docs/PHASE-1-MILESTONES.md` 那 45 個未勾框(2026-09-06 tidy 量到 0 個已勾)——
> 　 **一份沒人維護的清單, 與一份還沒做完的清單, 印同一個東西。**
>
> 🔵 **而路由那一半我不能自己改**(不動 `CLAUDE.md` / `AGENTS.md` 本體):
> 　 `grep -n '現在誰在做什麼' CLAUDE.md` ⇒ **0** · 同一條對 `AGENTS.md` ⇒ **0**
> 　 ⇒ **艦隊表只寫在 `MEMORY.md` 檔頭, 而 MEMORY 是主視窗的常載、不是每個施工窗的。**
> 　 ⇒ 已端主視窗:`CLAUDE.md:22`「每 session 必讀」那行要不要補艦隊表。**在他裁之前, 本段就是那個指標。**
>
> ⚠️ **下面那一節【不刪】** —— 它是 2026-08-28 的真實存檔點, 對考古有用。**它只是不再是「現在」。**


> # 🗂 2026-08-28 夜 · 存檔點位置(五線在跑 + 兩線今日已停)
>
> 🔴 **本節不寫任何 sha,也不寫行數** —— 沿用 08-27 `-de` 那句判詞:
> **「對一份【還在被編輯的檔】記 sha,記到的是一個時刻,不是一個身分。」**
> ⇒ **路徑才是身分。** 要判版本當場 `shasum -a 256 <檔> | cut -c1-12` 自己量。
>
> ```
> 線A 建單面板      ~/pcm-mailbox/進度-建單面板-20260828.md
> 線B 假守門盤點    ~/pcm-mailbox/進度-d3-管理者閘-20260828.md
> 線C 片1a 存檔     ~/pcm-mailbox/進度-線C-20260828.md
> 線D 排程與經銷價  ~/pcm-mailbox/進度-線D-20260828.md
> -c8 板子與查重    ~/pcm-mailbox/de-c8-存檔點-20260828-1156.md
> 主視窗            ~/pcm-mailbox/進度-主視窗-20260828.md · STOP-主視窗-20260828.md
> ⛔ 今日【已停】不要當成在跑:線E `~/pcm-mailbox/進度-線E-20260828.md`(mtime 12:03)
>                              下手窗 `~/pcm-mailbox/進度-下手窗-20260828.md`(mtime 10:21)
> 當場列法 ls -1 ~/pcm-mailbox/ | grep -c '20260828'   ⇒ 109(08-28 22:0x 量)
> 負對照   ls -1 ~/pcm-mailbox/ | grep -c 'zzz20260828' ⇒ 0
> ```
> ⚠️ **窗名每次重啟就變,以當下 `ListAgents` 為準;認人問工作樹路徑,不問名字。**
>
> ## ✅ 三件【Sean 2026-08-28 已回】—— ⚠️ ~~等 Sean,沒回之前那三條線不能動~~ **已作廢**
> ```
> Q1 ⇒ 甲  授權主視窗派 code-reviewer（他貼的字面是「甲」）
> Q2 ⇒ 他逐字「密碼看不到」= Vercel 遮值 ⇒ 那格【已問，答案是看不到】，不是「已問」
>       🔴 而 -c8 後續從稽核痕跡反推也走不通（getSessionActor 第1層與第3層最後呼叫同一支
>          resolveStaff ⇒ 兩個世界在那張表上不可能分開）⇒ 正確字面是【已量，而它答不出來】
> Q3 ⇒ 甲  換等級護欄【現在做】（他貼的字面是「ｑ３：甲」）
> ```
> 🔴 **本節 2026-08-28 一度在他回答之後仍寫著「等 Sean」數小時**,而那期間有一片的 code
>    註解引用「Sean 拍 Q3=甲」⇒ **SSoT 說在等,而碼說已經拍了,兩份文件互相否定**(code-reviewer 抓到)。
> 📌 **拍板落檔要與「拿它去用」同時發生 —— 晚一步,那一步裡的每個人讀到的都是相反的事實。**
> ⚠️ **引用拍板抄【題目文字】不抄題號** —— 同一天艦隊有多個 `Q3=甲`(reviewer 實 grep 到三個不同的)。
>
> ## ~~三件【等 Sean】~~（下方原文保留當歷史）
> ```
> Q1 主視窗能不能派 code-reviewer —— 一題卡三個窗
>    （線C 片1a commit 被 reviewer gate 擋、-c8 的 traps-neighbours.py、線B 的 naked-cells.py）
>    🔴 線C 那片寫不出跳審理由:命中鐵則 12③ migration + ②權限
> Q2 Vercel → pcm-admin → Production 的 ADMIN_REQUIRE_REAL_IDENTITY 值
>    🔴 兩端的 Vercel 工具都讀不到環境變數的【值】⇒ 只能 Sean 開 dashboard
>    ⚠️ 明天不要再 grep repo —— repo 裡那三句字面互相矛盾,再 grep 一百次也只會撈到它們
> Q3 「變更會員等級」護欄現在做還是等經銷價上線那天(plan 已寫,主視窗推薦現在做)
> ```
>
> ## 🔴 今晚三件會改變做法的
> ```
> ① scripts/literal-sweep.sh(CLAUDE.md 明文 commit 前必跑)有 markdown 標記盲區
>    ⚠️ ~~最硬的一格是 `~~刪除線~~` 不是 `**`,那把尺對它最主要的目標結構性失明~~
>       🔴 **超射,已收窄**(線D 量、`-c8` 四格獨立重跑後同意;超射那句的作者是 `-c8`,主視窗照落檔)
>       實測:片段**完全在 `~~` 內部**(`'真正認證化留 M-2-08'`)⇒ **1 命中,找得到**;
>             片段**跨越 `~~` 邊界**(`'⛔ ⏳ 待執行'`)⇒ **0 命中**
>       ⇒ **病灶是【跨標記邊界】,不是刪除線** —— `~~` 是 `**` / `` ` `` 那一種的**第三個成員**,不是新病
>       ✅ **正確的那句(要用的是這句)**:
>          **舊字面在我們這裡一定被 `~~` 包著 ⇒ 想【整行引用】它的人一定跨邊界 ⇒ 那個人一定拿到 0。**
>       🔴 **為什麼一定要收窄**:讀「結構性失明」的人會推出「這支工具對舊字面沒用」,
>          而它對**舊字面內部的片段是有用的** ⇒ **修法不變,而下一個人照這句話推出的東西不同。**
>       量級 docs 底下用刪除線的檔 **280 支 / 1,213 個片段**(負對照現造 `≈≈…≈≈` ⇒ 0)—— 這行不受影響
>       ⚠️ 剝標記後同一句而寫法不只一種:剝三種(`**`/`` ` ``/`~~`)⇒ **72**、只剝兩種 ⇒ **69**,
>          差 3 已定因(不是樹在長)—— 對 literal-sweep 而言 **72 才是對的數**
>    餵讀者字面 '已作廢 —— 修在' ⇒ 0 / 4,999 檔;餵原始字面 '**已作廢** —— 修在' ⇒ 1
>    負對照 ⚠️ ~~zzq-not-real-D28 ⇒ 0(尺是活的)~~ **這個字面已經死了 —— 把它寫進本檔的動作殺掉了它**
>       (`-c8` 當場重跑 ⇒ **1 命中**)。🔴 **而殺掉它的不是原作者**:線D 管得住「我不寫進正文」,
>       管不住「別人引用我的量測」—— **而引用是那個人的正確行為,他在留證據。**
>       ⇒ **這條規矩對單一作者可以靠紀律,對八個窗不行。**
>       ✅ **負對照一律現場現造**(`date +%s` 串進字面),**跑之前先確認它回 0**,不要寫死字面。
>    暴露面已量:docs/**.md 838 支 / 185,608 行,含 ** 或 ` 的 117,315 行 = 63%;
>    剝標記後同一句而原始寫法不只一種 ⇒ 72 組
>    🔴 致命的是時機:跑它的人【必然是剛改完那句話的人】⇒ 手上是腦子裡那句、不是檔案位元組
>       ⇒ 他會餵錯的那個,而那個 0 印出來的意思是「乾淨,可以 commit 了」
>    ⇒ 修它要動 .sh ⇒ 卡在 Q1 同一個字上。未修。
> ② 查重要跑【兩把尺】:traps-neighbours 排名 + 自己 grep 標題行(用機制關鍵字,不用自己的措辭)
>    實測同一則投稿:尺1 前 8 名最高 0.3211 而真同族三條【一條都沒進榜】;尺2 當場撈到三條全中
>    📌 兩把尺不是互相確認,是互相補洞 —— 只跑贏面大的那一把,漏掉的正是母題級的那種
> ③ 經銷價那條線的卡點【不是比對】:兩邊 sku 對得上(抽樣 500 ⇒ 496 且全部有經銷價)
>    卡在 ①那支 view 物理上沒有 price_store ②我們用 anon 公開金鑰讀它
>    ⇒ 加欄 = 經銷價對全世界公開 ⇒ 權限設計題,鐵則 12②
>    🔴 而 Q2=A「絕不接」的理由是【兩半】:安全那半仍成立、範圍那半已被 Q-C1c 推翻
>       ⇒ 不是推翻拍板,是那條拍板的兩半到期時間不同
> ```
...
>
> ## 🔴🔴 一之三、**Gilles 的價格會凍結,而來源每天在更新**(`-ed` 壓縮前最後一刻抓到)
>
> 🔴 **主視窗給它的 checkpoint 骨架裡有一句是錯的,而它照抄之前去查了**:
> 我逐字寫「`writeAllowed` 現在是 true ⇒ **每日同步會跑到它**」—— **那句不成立。**
> ```
> .github/workflows/rpm-sync.yml:72 的 matrix 是【寫死清單】, 不從設定檔衍生
> grep -c "gilles" .github/workflows/rpm-sync.yml   ⇒ 0   (主視窗當場複量)
> 負對照 grep -c "rpm" 同檔                          ⇒ 4   ⇒ 尺是活的
> 求值比對:writeAllowed=true 17 家 · matrix 14 家
>   🔴 已開寫而不在 matrix:['dna', 'extreme', 'gilles']   在 matrix 而沒開寫:[](空)
> ```
> **⇒ 顧客站的 Gilles 價格會凍結在首灌那一刻的快照,而來源每天更新、我們不會去拉。**
> **⇒ 時間拉長 = 我們賣的價格與來源脫節。** ⚠️ 交接文件 `:127` 說的「每日班」是**報價單側**的,不是網站側。
>
> **三家的判定不一樣**:`extreme` ✅ **刻意**(兩處明文);`gilles` 🔴 **真缺口**;
> `dna` 🔴 **全 repo 零命中,沒有任何檔案寫過為什麼** ⇒ 很可能是 2026-08-21 上架時漏的。
>
> 🔴 **`-ed` 沒有自己補,而理由是硬的**:①動 CI = **鐵則 12④ 平台設定** ②加進去 = **開始每夜自動寫正式庫**。
> ⇒ **這是他的決定。** 而 `-ed` 判它**比 `Q-Gilles-2`(「德國」那個字面錯)急得多** —— **那個沒人被擋,這個會漂價。**
>
> 📌 **而這一格的形狀比這一格本身值錢**(`-ed` 的話):
> **`dna` 在 manifest 與 sync matrix 兩個註冊點【都被漏掉】,而它們是兩個互不相干的檔。**
> **⇒「上架」這件事有多個註冊點,而沒有任何東西在數它們。**
> **而 `-ed` 今晚給 manifest 做了閘 —— `matrix` 沒有閘,而它比 manifest 貴:**
> **manifest 漏 ⇒ 少一段品牌介紹;`matrix` 漏 ⇒ 價格不同步。**
>
...
> ~~4 Q-Gilles-1  writeAllowed 翻不翻~~  ✅ 已回甲, 已灌, 1545 商品 live ⇒ 而它生出【兩題新的】: Q-同步清單(已回甲)/ Q-部署main(已回乙)
> 5 Q-sh進守門分母                       ← 真洞, 而沒有東西正在壞
> ~~6 Q-鐵則11分母~~ ✅ **已回 甲**(2026-08-28 09:4x)⇒ 已落地 `CLAUDE.md` + `AGENTS.md`
> 7 Q-路由表改既有列 ⏳ / ~~Q-零留痕判準~~ ✅ **已回 甲**(2026-08-28 09:4x)
...
> | **Q-鐵則11分母**(要 Sean 拍,主視窗不自改)鐵則 11 現行字面要「連跑兩發比檔數/測項數/紅的格數三個都相同」,而它**對「分母一開始就選窄」零判別力**。實錘 `7489aada` body 逐字「admin **126** passed · **2636** 測項 · 紅 0 —— 兩發完全相同」,而今天全套是 **282 檔 / 5117 測項** ⇒ 少掉的那半裡有 **6 格 TypeError**。🔴 **那三個數字全都在、兩發一致,它看起來比大多數收工紀錄都嚴謹。** ⚠️ **修法的代價要一起問** —— 每片都跑全套要付時間,今天已有窗撞過 `Another next build process is already running` | 08-27 `-21` 自爆、主視窗轉 | 全隊的收工紀律 | ✅ **已回 甲**(Sean 2026-08-28 09:4x, 主視窗端)⇒ 已落地 `CLAUDE.md` 鐵則 11 + `AGENTS.md` 同段。🔴 **強度**:他回的是**一個字**、讀的是主視窗的摘要 ⇒ 落檔只能寫「依主視窗摘要批准」。實測來源:線B `-d3` 02:1x 首報, 線D `-69` 09:4x、線A `-ea` 09:4x **各自在自己的分母上複現** ⇒ 三個獨立來源 |
> | 🔴 **`Q-Gilles-1`** `writeAllowed` 要不要翻 `true` 跑首灌?**甲=翻、跑監控式首灌 / 乙=先不翻**。⚠️ **這是寫進正式顧客站的 1,817 列、即時 live、不可逆(R3)⇒ 今晚一律不做**,`-ed` 已備到「他回一個字就能跑」。~~乾跑五關全綠、`--expect-groups=1545` 已驗~~ 🔴 **這句比實際乾淨,`-9e` 打掉**:首灌時 target=0 ⇒ **價格離群與來源消失對賬兩格恆綠**、handle 與 pv_spec **各半恆綠**、分群數**只印不擋**;而 `--expect-groups` 在**乾跑根本不驗**(`-ed` 餵 `9999` 實測 ⇒ 印 🔴 ALERT abort 不寫,**而 `rc=0`**)⇒ **「已驗」是假的,那是「已印」。** 真正有判別力的只有四格(分類 0 未對上 / handle 批內唯一 / pv_spec 批內撞鍵 / 新品驗價 M1) | 08-27 `-ed`(Sean 本人開的線)| Gilles 整條上架 | ✅ **已回 甲**(08-27)⇒ Sean 逐字「灌」。首灌已跑完:**1,545 商品 / 1,817 變體 現在 live 在顧客站**。🔴 **而上面那句「今晚一律不做」是寫給昨晚的,他早上就叫灌了 —— 這一列在他叫灌之後還印了幾小時的「未回」。** 🔴 **首灌完成 ≠ 這條線收工**:見下面兩列新題(同步清單缺兩家 / 品牌形象區要 merge 才看得見)。 |
> | **`Q-Gilles-2`** 來源側 2026-08-27 交接文件 §4 逐字寫「**德國** Gilles Tooling」= **錯的**。官網 imprint 寫 `L-6776 Grevenmacher, **Luxembourg**` / `R.C.S. Luxembourg: B 107.876`;商品頁逐字 `Made in **Luxembourg**`。**要不要改那份交接的字面?**🔴 **而不只交接文件** —— 報價單庫 `suppliers` 表實查 `slug='gilles'` ⇒ `origin='德國'`(`-9e` 指出、`-ed` 複查確認)⇒ **兩處都在報價單那側,一起改或一起不改。**🔴 **我方 `brand-content.ts` 寫「盧森堡」是對的、不用動** | 08-27 `-ed` | 報價單側文件 | ⏳ 未回 —— **而它排最後是對的:兩份文件字面錯,沒有人被擋、沒有東西在壞。**`-ed` 自己建議把它從決策表拿掉改進 backlog。 |
> | 🔴🔴 **`Q-同步清單`** `gilles` 與 `dna` 不在每日同步清單裡 ⇒ **顧客站價格凍結在首灌那一刻,而來源每天更新**。甲=兩家都加 / 乙=先不加。🔴 **`dna` 從 2026-08-21 起已經凍六天。** `extreme` 不在是刻意的(明文兩處)⇒ **是兩家不是三家**。量法(`-de` 親手複量,非轉述):`comm -23 <supplier-config 17 家> <rpm-sync.yml matrix 14 家>` ⇒ `dna` / `extreme` / `gilles`;`grep -c gilles .github/workflows/rpm-sync.yml` ⇒ **0**,正對照 `rpm` ⇒ **4**(尺是活的)。📌 **病灶(`-de` 判):** `scripts/supplier-config.ts:351` 逐字寫著「(交接文件第 4 點:這家 2026-08-27 起排入每日班、自己更新價格)」⇒ **決定寫進了【解釋它的檔】,而沒寫進【執行它的檔】,兩支之間零對帳。**
> 🔴🔴 **而 `dna` 那一半更難看,是 `-9e` 複量推翻主視窗的:** ~~「全 repo 沒有任何檔案寫過為什麼 `dna` 不在 matrix」~~ **這句我端錯了,而它讓事情看起來像個意外。**實況:①`~/pcm-mailbox/W3-Q-DNA商品匯入-plan-20260820.md:46` 逐字「把 dna 加進每日排程 matrix(獨立於首灌本身,首灌驗證過才做)」⇒ **是計畫好的延後,首灌 `739eb6ec`(08-21)之後沒有人接** ②🔴 **`docs/launch-todo.md:300` 從 08-23 起就掛著「DNA 不在每日同步名單」** ⇒ **板子上有這一列,而它在板子上躺了四天。**⇒ 📌 **真正的病灶不是「沒寫下來」,是【寫下來了、寫在板子上了,而還是沒有人做】。** 第 18 家不會因為多一份文件而被接住。(`-9e` 另更正:「全 repo 零命中」的字面會被讀成 `dna` 這個字零命中,實際 **137 檔**;成立的只有窄化後那句「repo 內沒人寫過【為什麼不在 matrix】」,四把尺、正對照 `extreme` 同尺 47 行、負對照 0。) | 08-27 `-de` 窗 | Gilles 與 DNA 的顧客站價格 | ✅ **已回 甲**(08-27,他在 `-de` 窗寫 `q1: A`)⇒ `-de` 動 `.github/workflows/rpm-sync.yml`、自己跑 codex(鐵則 12④)、自己 commit。🔴 **兩格誠實揭示**:①他寫的是 `A` 不是「甲」,**`A`→甲 是 `-de` 的解讀,不是他的字面**(選項只有甲/乙、甲列前;而這一題本身就是在問要不要加,方向沒有歧義)②**主視窗昨晚端給他的是「一家」,今天是「兩家」——我漏掉的那家凍最久。** |
> | 🔴 **`Q-部署main`** `origin/main..origin/dev` = **498 顆** ⇒ Gilles 品牌形象區要 merge 到 `main` 才看得見。甲=現在部署 / 乙=先不部署。⚠️ **那 498 顆不只是 Gilles** —— 是五天份的顧客站改動一起上。 | 08-27 `-de` 窗 | 顧客站的品牌形象區 | ✅ **已回 乙 = 先不部署**(08-27,他在 `-de` 窗寫 `q2: 乙`,條件是先查那段查詢)⇒ 態 = parked、誰 = Sean。🔴 **這兩題都是他在 `-de` 窗回的,主視窗未親見,而 `-de` 附了逐字引用塊** ⇒ 本表照此字面記,不寫成「全域拍板」。 |
> | ✅ **`Q-線3 片二審查債`**(不是題,是更正)~~線3 片二欠兩場審查(鐵則 12②)~~ 🔴 **已清,而主視窗今早還拿舊摘要去交辦它。**`線3-checkpoint-20260827.md:186-191` 寫著三場全跑完:R1 code-reviewer(opus)FAIL 7 / R2 codex CLI 唯讀 FAIL 5 / R3 code-reviewer(`-de` 窗獨立)2 ⇒ **15 條 must-fix 全折進 `bc61afe6`**(`:169`),而 `bc61afe6` 已推、在正式站。📌 **形狀(線1 今天在別處撞到同一條):一個被關掉的問題,不會自己去通知那些還在引用它的句子。** | — | — | ✅ 已清,列在此只為了關掉引用 |
...
> | **`Q-零留痕判準`**(要 Sean 拍,主視窗不自改)codex 唯讀審查的零留痕檢查是「跑前後 `git status --porcelain` 相同」——🔴 **八窗共用一棵樹時這個判準本身失效**:它把「codex 動了東西」與「別的窗剛 commit 掉東西」混進同一個 diff。線4 `-26` 本次實測:差異**全部來自別的窗**、它自己的檔零變動。⇒ 建議改成 **「我的檔有沒有變」**,不是「整棵樹有沒有變」。⚠️ 這是改 `docs/ops/AI_CONTRACT.md` §2 的判準 ⇒ 動判準是 Sean 的 | 08-27 `-26` 實測、主視窗轉 | 全隊的 codex 唯讀紀律 | ✅ **已回 甲**(Sean 2026-08-28 09:4x)⇒ 已落地:`CLAUDE.md` + `AGENTS.md`(commit 待收)、`docs/patterns/cowork-review-chain.md`、`.claude/skills/codex-adversary/SKILL.md`。🔴 **強度**:他回的是**一個字**、讀的是主視窗端的摘要 —— **不是他讀了那些檔** ⇒ 落檔字面只能寫「依主視窗摘要批准」,**不得寫成「Sean 審過」**;本題任何具體做法日後被推翻,**不得引用這次拍板當背書**。 |
