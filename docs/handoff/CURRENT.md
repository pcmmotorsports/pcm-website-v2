...
>
> **四窗與位址檔**(門鈴位址讀 `.{代號}-socket`,由主視窗依門鈴 from= 回寫;認不出=只落信不猜):**B**(.b-socket)=收款帳本塊 P1 施工中(B-340-Q plan 四題 A/A/A/A 已裁:混合受理分軌退/Q13 落 DB 具名函式/同單折抵/只 DB+RPC;confirm_order_payment 鎖=B;P4 backfill=Q10 例外問 Sean;P1→P2→P3/P5/P6→P7,全高風險)。**D**(.d-socket)=#350 工作區殼偵察+plan 落信待核(347-2a 已收割 `2891e2c8`;2b 硬閘=statement_timeout 待量+翻頁保留搜尋詞;Q-a=B POST+PRG)。**E**(.e-socket)=取消線 D2 施工中(D1 已收割;D3-D5 照 plan v3.2;D6 裁 A=元件層真瀏覽器,POST body 斷言取代 persisted)。**P**=二代已關窗,**三代待 Sean 開**(提示詞已交;首任務=雙扣 probe:P-271-STOP §4 五步+P-269-A 三條件;branch payment-lifecycle 領先 dev 兩顆 ed57f67a/d67aba05 不 rebase;🔴 口徑=「settle failed ⇒ 舊 URL 不成交」是**待實測前提、未被證明**,勿說已驗證;probe ①兩態都失敗=收割 L4b/②任一成功=不上、整理決策題/③構造不出=記未確認)。
...
> **完整化總 plan**=`docs/specs/2026-08-09-admin-completion-wave-plan.md`(塊序/匯流點/完成判準);決策題 Q1-Q16+建單六題+B 四題全清,**到總測試前 Sean 零決策**(例外=動錢/大量寫入 apply)。待辦小items:statement_timeout Studio SQL(2b 前)、D6-b 掛 #350 線下、#354/#355 backlog。(壓縮後讀本節,13:50 舊節僅備查)。第一動=date+find 全量掃 pcm-mailbox+確認 Monitor(brjcnghb9)活著+心跳 /loop 600s 照排(prompt 含 liveness+gh site-patrol issue 檢查;liveness 腳本=scratchpad/window-liveness.sh,重建法見 playbook §7-⑥)。**
>
