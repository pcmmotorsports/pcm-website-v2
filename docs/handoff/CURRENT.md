...
>    🔴🔴 **最硬的一格是 `~~刪除線~~`,不是 `**`**:我們整套「留痕不刪」的慣例就是靠 `~~` 寫的
>       ⇒ **舊字面正是它最該找到的東西,而舊字面在我們這裡【一定】被 `~~` 包著**
>       ⇒ **那把尺對它自己最主要的目標結構性失明。**(`**` 是偶然卡在中間;`~~` 是規則規定它一定在)
>       量級 docs 底下用刪除線的檔 **280 支 / 1,213 個片段**(負對照現造 `≈≈…≈≈` ⇒ 0)
>       ⚠️ 剝標記後同一句而寫法不只一種:剝三種(`**`/`` ` ``/`~~`)⇒ **72**、只剝兩種 ⇒ **69**,
>          差 3 已定因(不是樹在長)—— 對 literal-sweep 而言 **72 才是對的數**
>    餵讀者字面 '已作廢 —— 修在' ⇒ 0 / 4,999 檔;餵原始字面 '**已作廢** —— 修在' ⇒ 1
>    負對照 zzq-not-real-D28 ⇒ 0(尺是活的)
