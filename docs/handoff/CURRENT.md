...
> | **Q-27c** 片2 `20260826150000` 要不要補上跟片3a 同一道 `rolbypassrls` 前提斷言?甲=補、開小片重審 / 乙=先不補、記 backlog 等 Q15 那天一起。下手窗 `-de` 偏**甲** | 08-27 下手窗 `-de` 提、主視窗轉 | `-de`(它明說不自行改片2 = 鐵則 8) | ⏳ 未回 |
>
...
> | **Q-探針2** 貼 SQL Editor 跑 `~/pcm-mailbox/cf-拓撲探針-service_role成員與三表ACL-20260827.sql`(273 行)—— 問「正式庫是不是 `authenticator` 拓撲」⇒ **決定片2 與片3a 到底能不能 apply**。它的區 9 用正式庫的值模擬段A 會印什麼,**不用 apply 就知道** | 08-27 `cf` 產、`-de` 轉、主視窗落檔 | `-de` / `cf` 整條 RLS 線 | ✅ **已跑,而且是在主視窗問他之前就跑完的** ⇒ 輸出 `docs/probes/2026-08-27-production-topology-and-acl-results.md`(98 行,commit `9f9771b9`)。🔴 **主視窗把一件已經做完的事端成待辦** —— 成因是從轉述抄進本表時沒回頭量它的狀態。📌 **待辦表最容易長出來的東西就是已經做完的事,而它們在表上跟真待辦長得一樣。** |
...
> | **Q-段A-SET**(甲/乙)兩片段A 的角色閘要不要把 `SET` 拿掉、只留 `USAGE`?—— `authenticator` `SET ROLE` 之後**就是 `service_role` 本人**,不是多出來的第三方 ⇒ 現在的分類是錯的。🔴 **而兩種修法對「有人惡意建一個 SET-only 角色」那個世界印不同的東西** ⇒ `-de` 明文不自己動判準(動判準 = R4 換路訊號) | 08-27 `-de` | `-de` 的片2/片3a | ⏳ 未回 |
