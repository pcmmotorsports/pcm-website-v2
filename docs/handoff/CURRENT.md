...
> `-ed` 核出來:**37 列裡真正在等你的只有 8 列(22%)**,而**排第 1 的那件埋在第 62 行、在一個程式碼區塊裡、在一份標題叫「安全檢查」的東西底下**。
> 📌 **標題說「檢查」,內容說「快去止血」—— 你不會為了找止血,去點開一份叫檢查的東西。**
>
> ## 🛑 一、**現在唯一沒有人確認過的那一格**
> ### 🔴 **推完的驗收 —— 而推已經發生了,這一格【還沒有人做】**
> ```
> 登入後台 ⇒ 讓那個分頁【開著別關】⇒ 過 16 分鐘再回來看
> 還在後台裡     ⇒ ✅ 成功
> 被叫去重新登入 ⇒ ❌ 失敗 ⇒ 走退場路徑(見下), 而那是線3 備好的, 只等你一個字
> ```
> 📎 **退場路徑已備妥、兩案都實跑驗過、一個字都沒執行** ⇒ `docs/handoff/2026-08-27-bc61afe6-revert-path.md`(`5a0009f9`)
> 🔴 **而它檔裡最重要的是「跑之前先問一格」**:**失敗的形狀是什麼?**「每個人 15 分鐘被登出 / 任何動作都 403」⇒ 極可能是 Origin 閘 ⇒ 走 A(外科式,只關那道閘、四條修法全留);**其他形狀(例如登入頁本身進不去)⇒ 那可能不是這一片** —— **這一夜有 75 顆一起推,先確認是誰造成的再退。退錯東西的代價是:真正的病沒好,而我們少了四條修法。**
>
> ---
>
> ## 🗂 一之零、**以下原本的「先做這件」已作廢,原文照留不刪**
> 🔴🔴 **本節寫於 `9f9771b9` 當下(推之前)。2026-08-27 早上 Sean 已推到 `40c175f7` ⇒ §一 原文與 §二 第 1 題【已作廢】。**
> **當場對【遠端】重量**(`-26` 抓到、主視窗複跑):`5276411e` ⇒ **在**;`bc61afe6` ⇒ 🔴 **也在**(修好那版已經推了);`34c8fc5b` ⇒ **也在**(**原文拿它當負對照,而它現在是正例 ⇒ 那句「尺是活的」現在證不出任何東西**);負對照 `deadbeef` ⇒ **不在**(尺是活的)。
> 📌 **不靜靜改掉,是要讓你看得出「這一節曾經對,而世界動了」** —— 那比看到一節乾淨的新文字有用。
> 📌 **而 `-26` 指出的更深一格**:整節寫的是【推之前】的世界,**而它沒有任何一個時點錨** ⇒ **它是你早上讀到的第一個東西,而讀的人沒有理由懷疑它是舊的。**
>
> ### 🗂 原文(已作廢)
> ## ~~🛑 一、先做這件(**正在流血**)~~
> ```
> 有 bug 的後台換票版 5276411e ⇒ 已經在正式站上跑
> 修好的那版        bc61afe6 ⇒ 還沒推
> ⇒ 推是止血, 不推才是風險
> ```
> **`-ed` 獨立複量,帶負對照**:`git merge-base --is-ancestor 5276411e origin/dev` ⇒ 成立;`bc61afe6` ⇒ 不成立;負對照 `34c8fc5b` ⇒ 不成立(尺是活的)。
> 📎 **不修會怎樣的白話表** ⇒ `docs/handoff/2026-08-27-bc61afe6-impact-table.md`
> 📎 **整批 64 顆推之前的安全檢查** ⇒ `docs/handoff/2026-08-27-unpushed-batch-safety-check.md`
> ✅ **推完的驗收(用眼睛,不用指令)**:登入後台 ⇒ **讓那個分頁開著別關** ⇒ **過 16 分鐘再回來看**。**還在後台裡 = 成功;被叫去重新登入 = 失敗,叫線3 revert。**
>
> ## 🆕 一之二、**Gilles 首灌已完成 —— 而客人現在看到的是【半套】**
> **Sean 逐字「灌」⇒ 甲,`-ed` 執行完畢(`474c0063`)。1,545 商品 / 1,817 變體現在在正式顧客站上,客人點得到。**
> ```
> 寫入   products 1545/1545 · variants 1817/1817 · 未帶任何 --allow-* bypass
> 寫後驗 重跑唯讀乾跑 ⇒ 🔴 新商品 0 / 新變體 0  ← 最有力的一格:重跑找不到還要寫的東西 = 全部落地
>        已對上 1545 群 / 未對上 0 · 孤兒變體 0 · handle 批內唯一 · pv_spec 撞鍵 0
> ```
> 🔴🔴 **而有一半還沒上去**:
> ```
> 資料 ⇒ 寫進 DB ⇒ 【即時生效】⇒ 客人現在就看得到 1,545 件商品
> 而 GillesShowcase.tsx 是【程式碼】⇒ 要部署 main 才看得見
> ⇒ 現在點進 Gilles 商品頁, N°01「為什麼選 GILLES TOOLING」與 N°02 整段【不會出現】
> ```
> 📌 **這不是壞掉,是還沒部署。** 而 `-ed` 明說:**它沒有做「肉眼驗」** —— 那是 Sean 專屬用詞,形象區要等部署後由他點開商品頁看一眼。
>
> ⇒ 🔴 **這產生一題新的**:**`main` 要不要推?**(`main` ← production 顧客站,Sean 手動 merge)
> ⚠️ **而推 `main` 不只帶上那個形象區 —— 它會把 `dev` 上所有 storefront 的東西一起部署。** 那不是一個小動作,**要他自己判。**
>
> ## ❓ 二、在等你決定的(**8 題,依急迫排序**,細節在下面)
> ```
> ~~1 推不推 bc61afe6~~  🔴 已推, 這一題不成題 ⇒ 而它是排序的錨, 後面 7 題的「依急迫」要重看
> 2 Q-線3 的後續 —— 你【已回甲】而甲的前提後來被證偽 ⇒ 你以為結案的沒結案
> 3 Q-檢視-4/5/6/7(四題一起答)         ← 擋著線4 開工
> 4 Q-Gilles-1  writeAllowed 翻不翻     ← 全備妥、回一個字就能跑
> 5 Q-sh進守門分母                       ← 真洞, 而沒有東西正在壞
> 6 Q-鐵則11分母                         ← 連端都還沒端(主視窗還沒問你)
> 7 Q-路由表改既有列 / Q-零留痕判準       ← 制度題, 沒東西被擋
> 8 Q-Gilles-2 「德國」改不改             ← 🔴 -ed 自己建議把它拿掉:兩份文件字面錯,
>                                          沒有人被擋、沒有東西在壞 ⇒ 它該進 backlog
> ```
> ## 🖐 三、要你動手的(**這是工不是題,不用想,照做**)
> ```
> · 三支唯讀 SQL 探針要貼 Supabase SQL Editor(線1 正在確認它們貼得下去, 回來會補順序與期望長相)
> · vercel Firewall Traffic 貼三個數字 / SQL Editor 貼 pg_default_acl 一句(b4 要的)
> ⚠️ 撞名探針那支【要你先從 Dashboard 抄回網站庫的連線資訊填進去】—— 沒填它永遠印 ⚠️,那不是它壞了
> ```
> ## 📌 四、射程宣告(**這幾句刪掉,你會把一個【抽樣】讀成【窮舉】**)
> ```
> · 整批安全檢查:64 顆裡開過 body 的【不到十顆】, 其餘只看標題 ⇒ 那張清單是抽樣不是窮舉
> · 同一份:「沒找到不該推的, 而它也沒有看完 64 顆」
> · 正式庫的 RLS 政策跑了沒 ⇒ 沒有人量過, 標未確認
> · b4 刻意不列三類(要先有數字的 / 是工不是題的 / 已關的)—— 那是它的判斷, 不是漏
> · tokens.css 是解析器算的贏家值, 不是真瀏覽器的 computed value ⇒ 沒有人開過瀏覽器
> ```
> ## 🔎 五、b4 盤出來的 11 項(**在等你,而還沒被做成題目**)
> 在下面那節。🔴 **`b4` 的判準照抄:「不在表上的比表上多寫幾題貴 —— 表上多的他會跳過,表上少的【永遠不會發生】。」**
> ⚠️ 而主視窗**沒有把它們併進上面那 8 題** —— 理由:**它們是一個窗的盤點結果,還沒有被整理成「回一個字」的形狀**。`-ed` 判不了這一格,明說它不替你決定注意力的分配。**這一段是主視窗的決定。**
>
> ---
>

> ## 🔔 2026-08-27 · **等 Sean 回答**(端出時間 / 回了沒 / 誰在等)
>
> 🔴 **這一節是為了補 `CHECKPOINT-主視窗-96-收工-20260826.md` §0-1 點名的那個洞**:
> 「等待某人回答」這件事**沒有載體** ⇒ 而【窗在做事】與【窗停了】在主視窗這端是**同一個訊號:什麼都沒有**。
> 2026-08-26 兩個窗因此各停一段時間,是 Sean 自己講「3.4 都停著等你」才發現。
> **規矩:端出去就寫進本節;他回了就在該題後面標【已回 + 日期 + 他選的代號】,不要刪題。**
> ⚠️ **本節只是一個【看得見】的載體, 不是一個會主動提醒的機制** —— 它要有人來讀才有用。
>
> | 題 | 端出 | 誰在等 | 狀態 |
> |---|---|---|---|
> | **Q-27a** `20260826140000`(客戶列表加生日兩欄)要不要現在 apply 到正式庫?甲=現在處理 / 乙=先不推。主視窗推薦**甲** | 08-27 主視窗 | 未推的 14 顆全部 | ✅ **已回 甲**(08-27)⇒ Sean 親跑 SQL Editor,`Success`;主視窗給的驗證查詢回 **11 / 2 / 0 / 2 / 0** ⇒ 兩欄兩索引都在、兩發負對照皆 0。🔴 **主視窗把期待值寫成「應為 13」而正解是 11** —— 檔內三處自寫「9 欄 → 11 欄」(`:326` / `:390` / `:445`)而我沒讀就心算。**那個假期待值會讓 Sean 以為成功的事失敗了。** |
> | **Q-27b** cf(資料庫探針)與 b4(板子稽核)兩窗今天開不開?甲=現在開 / 乙=今天不開。推薦**甲** | 08-27 主視窗 | 下手窗 `-de` 的兩條線今天沒有人 | ✅ **已回**(08-27)⇒ Sean 直接把窗開了(七窗:`-de` 下手 / `-21` 線1 / `-26` 線4 / `-6e` 線3 / `-9e` 複量 / `-ce` b4 / `-52` cf),未走選項。 |
> | **Q-27c** 片2 `20260826150000` 要不要補上跟片3a 同一道 `rolbypassrls` 前提斷言?甲=補、開小片重審 / 乙=先不補、記 backlog 等 Q15 那天一起。下手窗 `-de` 偏**甲** | 08-27 下手窗 `-de` 提、主視窗轉 | `-de`(它明說不自行改片2 = 鐵則 8) | ⏳ 未回 🔴 **`b4` 2026-08-27 核出「該關沒關」**:✅ **已回 甲** —— 落點 memory `project_0827-sean-first-batch-rulings`、`b7af8f76` 整顆就是它。**而這一列在他答完之後還印了幾小時的「未回」。** |
>
> | **Q-探針1** 貼 SQL Editor 跑 `docs/probes/2026-08-26-email-sweep-enqueue-status-from-body.sql` —— 問「信到底有沒有被丟進去」(`email_outbox` 整表 1 列,08-22)。純唯讀 `BEGIN…ROLLBACK`。⚠️ **先看區 0,任一列非 ✅ ⇒ 底下全部作廢** | 08-26 `-de` 端過兩次(**只在對話裡**)、08-27 主視窗落檔 | 通知線的判斷 | ✅ **已跑**(08-27 Sean 親跑)⇒ 輸出逐格落 `docs/probes/2026-08-27-email-sweep-enqueue-status-results.md`。**閘開著、跑過了、符合條件 0 筆,不是壞掉**(四態落在 `completed` ⇒ `B4_DEPLOY_CUTOFF` 有設且格式合法)。🔴 **輸出裡兩個 🔴 是假紅**(`…from-body.sql:48` / `:145` 都是寫死字串,不論好壞都印)= `#942` 同族。⚠️ **`enqScanned:0` 分不出「沒有符合的訂單」與「cutoff 全濾掉」** ⇒ 第一張真實訂單進來後要重跑。 |
> | **Q-探針2** 貼 SQL Editor 跑 `~/pcm-mailbox/cf-拓撲探針-service_role成員與三表ACL-20260827.sql`(~~273 行 = **v1**~~ 🔴 **現行是 v2(317 行,`55f5210b4071`)—— 照舊路徑貼的人會貼到 v2 而以為是 v1**(`b4` 抓到);~~已跑完那一發用的是哪一版,未確認~~ ✅ **`-9e` 查到了**:結果檔 `docs/probes/2026-08-27-production-topology-and-acl-results.md:3-4` **寫死 v1**(273 行 / `8c37ced7a1f5`)⇒ **Sean 跑的是 v1**)—— 問「正式庫是不是 `authenticator` 拓撲」⇒ **決定片2 與片3a 到底能不能 apply**。它的區 9 用正式庫的值模擬段A 會印什麼,**不用 apply 就知道** | 08-27 `cf` 產、`-de` 轉、主視窗落檔 | `-de` / `cf` 整條 RLS 線 | ✅ **已跑,而且是在主視窗問他之前就跑完的** ⇒ 輸出 `docs/probes/2026-08-27-production-topology-and-acl-results.md`(98 行,commit `9f9771b9`)。🔴 **主視窗把一件已經做完的事端成待辦** —— 成因是從轉述抄進本表時沒回頭量它的狀態。📌 **待辦表最容易長出來的東西就是已經做完的事,而它們在表上跟真待辦長得一樣。** |
> | **Q-探針3** 貼 SQL Editor 跑 `~/pcm-mailbox/cf-寫入面分母探針-service_role寫入GRANT無政策-v2-20260827.sql`(172 行,sha 前 12 `8d1bd9ebf0f8`)—— 問「正式庫有幾張表中了 `#946` 那一格」(有寫入 GRANT 而缺 SELECT 政策 ⇒ 帶 `WHERE` 的 UPDATE 印 `UPDATE 0` 而不報錯)。⚠️ **那支只在十種 fixture 形狀上量過,沒有人拿它跑過正式庫** | 08-27 `cf` 產、`-de` 轉、主視窗落檔 | `-de` / `cf` 的「42 張怎麼補」整條 | ⏳ 未跑 |
> | **Q-線3-上片法**(甲/乙)`bc61afe6` 動到認證路徑,要怎麼上?甲=先推 preview 看一眼後台還能登入再推 dev / 乙=直接推 dev。🔴 **配錯的失敗形狀 = 續期永遠失敗、每個人 15 分鐘被登出一次**;`6e` 推薦甲,附「片一已經是【沒驗就上正式站】了,這是第二次」 | 08-27 **`6e` 直接端給 Sean**(主視窗不重端 —— 同一題兩種問法在飛,「他回了哪個窗」與「這題結了沒」會分不開)| `6e` 的片二 | ✅ **已回 甲**,🔴 **而甲的前提後來被證偽 —— 見下**。📎 **要決定推不推,先讀 `docs/handoff/2026-08-27-bc61afe6-impact-table.md`**(`5f2c5a6b`)—— `6e` 把 15 條 must-fix 換成**他看得懂的後果**,分四區:**✅ 正在壞 / ⏳ 埋著 / ⚪ 沒在壞 / 🔴 修法自己引進的新面**(第四區是**推的風險不是推的理由**)。🔴 **而那張表自己有一節自檢**,`6e` 當場抓到自己一格「比實際乾淨」(寫 5 處、實際 7 處,成因是它的尺多加一層過濾)——**它訂正了,而且寫進表裡不是偷偷改掉**。✅ **給 Sean 的驗收動作(兩個世界印不同的東西,用眼睛就看得到)**:推完 ⇒ 登入後台 ⇒ **讓那個分頁開著別關** ⇒ **過 16 分鐘再回來看**。**還在後台裡 = 成功;被叫去重新登入 = 失敗,叫 `6e` revert。**`6e` 正在把「甲」拆成 Sean 一步步照打的動作(指令 / 去哪看 / 🔴 兩個世界印不同東西的判準 / 不過怎麼辦 / 過了推 dev 的指令) |
> | **Q-鐵則11分母**(要 Sean 拍,主視窗不自改)鐵則 11 現行字面要「連跑兩發比檔數/測項數/紅的格數三個都相同」,而它**對「分母一開始就選窄」零判別力**。實錘 `7489aada` body 逐字「admin **126** passed · **2636** 測項 · 紅 0 —— 兩發完全相同」,而今天全套是 **282 檔 / 5117 測項** ⇒ 少掉的那半裡有 **6 格 TypeError**。🔴 **那三個數字全都在、兩發一致,它看起來比大多數收工紀錄都嚴謹。** ⚠️ **修法的代價要一起問** —— 每片都跑全套要付時間,今天已有窗撞過 `Another next build process is already running` | 08-27 `-21` 自爆、主視窗轉 | 全隊的收工紀律 | ⏳ 未端出(主視窗還沒問他)|
> | 🔴 **`Q-Gilles-1`** `writeAllowed` 要不要翻 `true` 跑首灌?**甲=翻、跑監控式首灌 / 乙=先不翻**。⚠️ **這是寫進正式顧客站的 1,817 列、即時 live、不可逆(R3)⇒ 今晚一律不做**,`-ed` 已備到「他回一個字就能跑」。~~乾跑五關全綠、`--expect-groups=1545` 已驗~~ 🔴 **這句比實際乾淨,`-9e` 打掉**:首灌時 target=0 ⇒ **價格離群與來源消失對賬兩格恆綠**、handle 與 pv_spec **各半恆綠**、分群數**只印不擋**;而 `--expect-groups` 在**乾跑根本不驗**(`-ed` 餵 `9999` 實測 ⇒ 印 🔴 ALERT abort 不寫,**而 `rc=0`**)⇒ **「已驗」是假的,那是「已印」。** 真正有判別力的只有四格(分類 0 未對上 / handle 批內唯一 / pv_spec 批內撞鍵 / 新品驗價 M1) | 08-27 `-ed`(Sean 本人開的線)| Gilles 整條上架 | ⏳ 未回 |
> | **`Q-Gilles-2`** 來源側 2026-08-27 交接文件 §4 逐字寫「**德國** Gilles Tooling」= **錯的**。官網 imprint 寫 `L-6776 Grevenmacher, **Luxembourg**` / `R.C.S. Luxembourg: B 107.876`;商品頁逐字 `Made in **Luxembourg**`。**要不要改那份交接的字面?**🔴 **而不只交接文件** —— 報價單庫 `suppliers` 表實查 `slug='gilles'` ⇒ `origin='德國'`(`-9e` 指出、`-ed` 複查確認)⇒ **兩處都在報價單那側,一起改或一起不改。**🔴 **我方 `brand-content.ts` 寫「盧森堡」是對的、不用動** | 08-27 `-ed` | 報價單側文件 | ⏳ 未回 |
> | ~~**`Q-Gilles-3`**~~ 🔴🔴 **2026-08-27 整題撤回 —— 不要端給他。它不是決策題,是現況。**`-ed` 查完:那 96 列的 `image_url` **不是空的,是 `no-photo.png`** —— 它抓下來開圖看了,是一張 **1000×750、PCM 品牌設計過的「暫無照片」卡**,不是破圖;顧客站不會再疊佔位(`scripts/rpm-transform.ts` 逐字 `image_url ?? images[0] ?? placeholder`,主視窗開檔複核相同)。🔴 **決定性的一步 = 對整支 view 分組數**:`lightech` 8,788 列 / 726 無照片 = **8.3%,早就在架上**;`gilles` 5.3%;`dna` 3.0%,**也早就在架上**。⇒ **gilles 夾在兩家已上架的中間** ⇒ 問他 = 請他重新決定一件**已被實作決定過兩次**的事;而「乙=擋掉」會讓三家標準不一致 = **引入不一致,不是提高品質**。📌 **`-ed` 補的判別句(照抄)**:**「把『要不要接受 X』端給他之前,先量【X 現在已經發生幾次了】。已經在線上跑著的不是決策題,是現況 —— 而把現況端成決策題,會讓他以為有個新問題要處理。」** |
> | ⏸ **`撞名探針`(還不是要他跑的題,先掛著)** ~~v2 / v3~~ ⇒ **現行 v4**:`~/pcm-mailbox/線1-21-撞名探針-片3至5-v4-20260827.sql`(**351 行**,`sha256` 前 12 `ff46a58bcda5`);自檢輸出 `~/pcm-mailbox/線1-21-撞名探針v4-世界自檢輸出-20260827.txt`,**輸出檔第 3 行的 sha 與定稿逐字相同**(`-21` 兩邊各算一次比對)—— 純 SELECT 包 `BEGIN…ROLLBACK`、只讀 `pg_catalog` 的名字、零 credential 零客戶資料;~~八個世界八種判定~~ ⇒ **輸出檔實際是 7 個世界 3 種判定**(`-9e` 複量)。**狀態 = 三輪審畢**:codex 審 **v1**(FAIL 16 must-fix)、**v2**(FAIL 3 must-fix),**v3 由 `-9e` 做第三輪【框架審】**(換角度換模型)⇒ 抓到 **② 那個判定不可能變成別的值**(區 0 沒有「已核對」出口 ⇒ 區 4 兩條分支是死碼)+ **自檢輸出的 sha 不是 v1/v2/v3 任何一版**。✅ **折完了 ⇒ 已備好、三輪審畢、待 Sean。**
> 🔴🔴 **而在他跑它之前,有一格【沒有人知道】的東西要先補**:v4 檔頭多了一格 `expected`(`exp_addr` / `exp_ref`),**貼之前要人填**。**沒填 ⇒ 它的結論永遠是「⚠️ 不算過:庫別未核對」** —— **那不是它壞了,是那一格還沒有人填。**
> ⇒ **要 Sean 從 Supabase Dashboard 抄回【網站庫的連線資訊】**(`exp_addr` / `exp_ref`)。`-21` 明說:**那個期望值現在沒有人知道,也沒有人查過。**
> 📌 **這是今晚唯一一個「探針已備好,而它還缺一個只有他拿得到的常數」的格** —— 不補,跑了也只會印 ⚠️。⇒ **主視窗裁定第三輪換人:交 `-9e`,而且問的是【框架】不是細節**(`00-work-rules §5`:第 3 輪起換角度換模型,同模型再審傾向在同一框架內找更細的問題)。⚠️ **在那一輪回來之前,這一列不是「可以叫 Sean 跑」的狀態** —— 我不會讓一支沒審過的 SQL 出現在他的待辦清單上。📌 而它自己在檔頭寫了**三個修不掉的殘餘風險**(`pg_stat_statements` 會留查詢文字 / 中途錯誤時尾端 `ROLLBACK` 可能不執行 / `search_path` 被動過時未限定的運算子仍可能被替身接走)—— **不假裝解掉。** | 08-27 `-21` | 退貨線片 3-5 的前置 | ⏸ 待審,**未端出** |
>
> 🔴 **主視窗自陳:這一列我在 03:2x 對 `-21` 說過「我進等待表了」,而【我沒有進】** —— 是它交 v2 時我才發現表上沒有。
> 📌 **今晚第三個「說了要轉而沒有當場發出去」的實例**(前兩個:`-de` 的 email 探針端過兩次只在對話裡、`-de` 收到 b4 的更正沒補送)。
> **常載 `§5` 轉述契約② 逐字寫著:說「我轉」的當下就發出去,發不出去就不准說那句。我說了,而我沒有當場做。**
> | 🔴 **`Q-sh進守門分母`**(甲/乙)`.sh` 完全不在 `scripts-whitelist-gate` 的分母裡(`EXTS='py js mjs cjs jsx mts cts'`)⇒ **一支新腳本可以零自檢而沒有人會知道**。量到:`scripts/*.sh` 共 **160** 支,而 `lint-staged` 逐字列名的只有 **12** 支(負對照 0、正對照命中)。🔴 **而 `.sh` 正是本 repo 用來寫守門與量具的語言**(`.husky/` 底下 **9 支** `.sh`;主視窗第一版寫 11 —— **目測 `ls` 的行數而沒有 `wc -l`**,`-9e` 抓到,已當場重量:`ls .husky/*.sh | wc -l` ⇒ 9,負對照 0)⇒ **沒有自檢的 `.sh` = 沒有自檢的守門。****題**:甲=把 `.sh` 加進分母 + 給豁免清單一個寫理由的欄位(⚠️ **裝上去很可能立刻紅**,148 支會一起亮 ⇒ plan 要先想過分批)/ 乙=先不動,記 `#949` 等排片。**主視窗傾向甲**,而**理由是「它現在既不能被守、也不能被正式豁免」** —— `-de` 那支 harness 就卡在這個死結裡,已 commit 進去了 | 08-27 `-de` 自陳、主視窗複量 | 全 repo 的 `.sh` | ⏳ 未回 |
> | 🔴 **`Q-檢視-4/5/6/7`(四題一起答,不要拆開)** —— `Q-檢視-1=乙`(可共用)+ `Q-檢視-3=乙`(共用那份沒有主人)合起來造出「**沒有主人、大家都看得到**」的檢視,而**誰能刪 / 誰能改 / 兩人同改 / 誰能建立共用**這四題沒有答案。📎 **三題的選項已備成「回三個字就能開工」**(`-26` `3ca19ae7`):三題各三個選項、**一樣詳細、沒有推薦**,每格三行(要多做的 / 你會看到什麼 / 🔴 代價),而代價寫成他看得懂的話。🔴 **跨題連動三格**:①`Q-檢視-4` 乙(只有建立者能刪)**與 `Q-檢視-3=乙` 相牴觸** —— 它等於把主人加回來只是換個欄位名;②而它第二個代價是**建立者離職後那張共用檢視誰都刪不掉**(`staff` 離職走 `is_active=false` 不物理刪除)⇒ 永遠留在每個人畫面上;③`Q-檢視-6` 的乙與丙 schema 完全一樣 ⇒ **這題不擋開工,可以晚一點決定**。⚠️ **而 `Q-檢視-7`(誰能建立共用)若答「只有管理者」,4/5 的丙就變得自然** ⇒ **四題一起端才問得對。**🔴 **另一格他該知道的**:4/5 的丙要讀 `staff.is_manager`,而 `-26` 當場重量 ⇒ 該欄有 **24 行**非測試檔命中(顯示 / 表單預設 / 寫入 / 稽核標籤 / 查詢欄位),而**做權限判斷的分支 0 個**(負對照 `is_active` ⇒ `staff.ts:144` 有真的權限分支 ⇒ 尺是活的)⇒ **選丙 = 這一欄第一次【拿來決定某人能不能做某件事】。一個從來沒有承過重的欄位,第一次承重。** | 08-27 `-26` | 線4 `#1` | ⏳ 未回 |
>
> 🔴 **主視窗自陳:這四題我在 03:5x 對 `-26` 說過「我明早併批端他」,而我【沒有進表】** —— 是它交備選項時我去找那三列,才發現表上沒有。
> 📌 **今晚第【四】個「說了要轉而沒有當場發出去」**(前三:`-de` 的 email 探針端過兩次只在對話裡 / `-de` 收到 b4 的更正沒補送 / 我說撞名探針進表了而沒進)。
> **常載 `§5` 轉述契約② 逐字:說「我轉」的當下就發出去,發不出去就不准說那句。我今晚說了四次,做到一次。**
> | 🔴 **`Q-路由表改既有列`**(一個字)主視窗 `fd0ef3cf` 改了 `CLAUDE.md` 路由表**既有的一列**(在「開新施工窗」那列的括號裡補 `§C-3` 指標),而 `~/.claude/rules/00-work-rules.md §4` 可自行改的字面是「路由表**新增一行**(diff 給 Sean 過目)」,**沒有涵蓋「改既有一列」** ⇒ `-9e` 複量指出這是主視窗越權。🔴 **主視窗已於 `<本顆>` 把那行【還原回原樣】** —— 不留著、不「揭露當授權」。**題**:甲=准(以後路由表**改既有列**也算可自行改,主視窗把 `§C-3` 那行加回去)/ 乙=不准(路由表只准新增行,改既有列一律先問)。**主視窗不推薦,這是他的權限線** | 08-27 `-9e` 打出來、主視窗自撤 | 主視窗的權限邊界 | ⏳ 未回 |
> | **`Q-零留痕判準`**(要 Sean 拍,主視窗不自改)codex 唯讀審查的零留痕檢查是「跑前後 `git status --porcelain` 相同」——🔴 **八窗共用一棵樹時這個判準本身失效**:它把「codex 動了東西」與「別的窗剛 commit 掉東西」混進同一個 diff。線4 `-26` 本次實測:差異**全部來自別的窗**、它自己的檔零變動。⇒ 建議改成 **「我的檔有沒有變」**,不是「整棵樹有沒有變」。⚠️ 這是改 `docs/ops/AI_CONTRACT.md` §2 的判準 ⇒ 動判準是 Sean 的 | 08-27 `-26` 實測、主視窗轉 | 全隊的 codex 唯讀紀律 | ⏳ 未回 |
> | **Q-段A-SET**(甲/乙)兩片段A 的角色閘要不要把 `SET` 拿掉、只留 `USAGE`?—— `authenticator` `SET ROLE` 之後**就是 `service_role` 本人**,不是多出來的第三方 ⇒ 現在的分類是錯的。🔴 **而兩種修法對「有人惡意建一個 SET-only 角色」那個世界印不同的東西** ⇒ `-de` 明文不自己動判準(動判準 = R4 換路訊號) | 08-27 `-de` | `-de` 的片2/片3a | ⏳ 未回 🔴 **`b4` 2026-08-27 核出「該關沒關」**:✅ **已回 乙**(Sean 在 `-de` 那個 session 的對話裡)—— 落點 memory `project_0827-sean-picked-b-for-segment-a-criterion`、`b7af8f76`。⚠️ **落檔字面只能寫「Sean 選乙」,那串措辭是提問的窗擬的。** |
> | **Q-G7射程**(一個字)片 3-5 的 dormant migration【撰寫】要不要等 Sean 關 G7?甲=不用等(撰寫前只查撞名;apply 與上線仍等)/ 乙=要等(片3 起整條停)。`-9e` 不推薦設計,只說「與 v5 內部一致的是甲」 | 08-27 `-9e` R7 | `-21` 退貨與庫存線 | ✅ **已回 甲**(08-27)⇒ `project_0827-sean-first-batch-rulings` 的 `Q-G7射程=甲`。🔴 **這一列在 memory 已甲之後仍印「未回」達數小時** —— 與 `Q-探針2` 同形狀:**待辦表上的一列,「還沒做」與「早就有答案了」印同一個 ⏳**。發現者是 **codex**(`R9-複量窗-9e-…-20260827.md` 引 v7 `:182`),不是主視窗、不是 `-9e`、不是 `-21` |
>
> | **`#945` submodule** 要不要讓 preflight 沙箱與 CI 也帶 `design-reference` | 08-27 主視窗 | 線4 `-26` | ✅ **已回「可以」** —— 🔴 **批的是方向不是 plan**;仍命中鐵則 12④ + 鐵則 8 ⇒ **要提 plan 等批才動手** |
> | **`Q-檢視-1`** 存起來的檢視要不要能給別人看到 | 08-27 線4 `-26` 提、主視窗轉 | 線4 `#1` 那片 | ✅ **已回 乙 = 自己的 + 共用的**(⚠️ 線4 傾向甲,他選了反面)|
> | **`Q-檢視-3`** 共用密碼登入的人怎麼辦 | 同上 | 同上 | ✅ **已回 乙 = 共用一份沒有主人的**(⚠️ 線4 傾向甲,他又選了反面)📌 **兩題都偏「共用」側 ⇒ 那是一個立場,不是隨手選** |
> | **`Q-檢視-2`** 存「今天」這種會動的日期,明天點開看到什麼 | 同上 | 同上 | 🔴 **他逐字「我看不懂,你決定」⇒ 主視窗決定 = 甲(存相對日期、多存一欄)。落檔一律寫「主視窗決定」,不得寫成「Sean 拍板」** —— 理由與代價全文在 memory `project_0827-sean-second-batch-and-night-run` §二,寫在那裡是為了讓它可以被推翻 |
> ### 🔴🔴 推之前先讀:**整批 64 顆的安全檢查**(`-21`,`a36b06d4`)
> `docs/handoff/2026-08-27-unpushed-batch-safety-check.md`(102 行)—— **今晚唯一一份看【整批合起來】的**(八個窗各自只驗了自己那幾顆)。
>
> **一句話**:**機械面全綠,而綠的射程只到「我跑過的那幾把尺」。本檔沒找到不該推的,而它也沒有看完 64 顆。**
>
> ```
> 三綠 TURBO_FORCE=1 typecheck / lint / build rc 皆 0
> admin 282 檔 5120 測項 紅 0 · storefront 256 檔 3692 測項 紅 0 · node 136 檔 2575 測項 紅 0
> ⚠️ 只跑一發 —— 鐵則 11 要連跑兩發比三個數, 它沒跑第二發, 自己寫在檔裡
> ① 順序依賴 沒找到 · ② 多窗共改 CLAUDE.md 兩顆【淨差異 0 行】· ④ env/secret 0 命中
> ```
>
> 🔴🔴 **⑤ 的答案是反過來的 —— 這是他早上最該先讀的一句**:
> ```
> 5276411e(有 bug 的後台換票版)⇒ 已在 origin/dev = 【正在正式站上跑】
> bc61afe6(修好的那版)        ⇒ 未推
> ⇒ 對這一顆而言,【推是止血, 不推才是風險】
> ```
>
> ⚠️ **它自己抓到五格「比實際乾淨」,而第 2 格最重要**:**64 顆裡它開過 body 的不到十顆,其餘只看標題** ⇒ **「body 被推翻」那張清單是抽樣不是窮舉。**
> 另四格:三個 project 只跑一發 / 順序依賴的尺只掃三個表名字面 / ~~`tokens.css` 是「未確認」不是「沒問題」~~ ✅ **已補量(`7670ef14`)**:逐檔比對 `origin/dev` 與 `HEAD` 的宣告集合 ⇒ **既有 token 一顆都沒被改**(`tokens.css` 68→69,**消失 0**;`product-page.css` 10→10;`wallet.css` 0→0;正對照餵一個真的少一項的集合 ⇒ 印得出來 ⇒ 尺會動)。唯一變化是**多一顆** `--gil-blue`,與其他 7 家品牌同形狀。而 `wallet.css` 被刪那行**是修不是退步**(那顆 token 全 repo 零定義,一直在走 fallback)。⚠️ **殘餘限制**:那是**解析器算出來的贏家值**,不是**真瀏覽器的 computed value** —— **沒有人開過瀏覽器**/ `20260826140000` 有沒有 code 依賴**完全沒查**。
>
> ### 🔴 2026-08-27 `b4` 核查本表完整性 ⇒ **11 項在等他而不在表上**
> **`b4` 的判準,照抄不改寫**:**「不在表上的比表上多寫幾題貴 —— 表上多的他會跳過,表上少的【永遠不會發生】。」**
> 🔴 **而這是主視窗同夜那句話的實測版**:我端給他的是**一張比實際乾淨的圖**,而**乾淨正是它讓人放心的原因**。
> 全文與每項的「誰在等 / 不回會怎樣」:`~/pcm-mailbox/b4-核查-等待表完整性-v1-20260827.md`
>
> | # | 在等他的 | 誰在等 |
> |---|---|---|
> | 1-4 | **`b4` 一頁表 A 堆四題**:`Q-net-RLS`(net 兩表補不補 RLS)/ `Q-真交易`(上線前刷小額真卡驗「記成已付」)/ `Q-心跳`(結算死了會不會有人知道 —— 上線前排片?)/ `Q-WCYCW5`(正式站那張空單) | b4 |
> | 5-7 | **只有他能動手的三件**:~~`vercel login` 本機 CLI 08-25 已過期~~ ⚠️ **`-9e` 實測 `vercel whoami` ⇒ 回 `pcmmotorsports`,登入有效** ⇒ **b4 那句的成因不成立**;而「全窗讀不到防火牆規則」這個現象**另有原因,沒有人查過**(`-9e` 明說它沒查) / 開 Firewall Traffic 貼三個數字 / SQL Editor 貼 `pg_default_acl` 一句 | b4 |
> | 8 | 🔴 **板子變更單 B 堆【9 格要他拍才能改態】**:`L223` CI done⇒open / `L226` 守門 done⇒open / `#866` doing⇒done / `#530` / `#841` / `:306` doing⇒parked / `B5-a` 併 `B7` / `#202` parked⇒doing / 33 項+roadmap。全文在 `docs/audits/2026-08-27-launch-todo-change-sheet-105-rows.md` §B | 主視窗(板子) |
> | 9 | **cf 第三面探針 v2**(`cf-第三面探針-政策內容與RESTRICTIVE-v2-20260827.sql`,181 行,`8cc5e21732c5`)—— 要他貼 SQL Editor | cf(42 張的第三面)|
> | 10 | 🔴 **`Q-Gilles-2` 的分母**:報價單庫 `public.suppliers` MCP 唯讀實查 `slug='gilles'` ⇒ **`origin = '德國'`** ⇒ **不只交接文件,DB 也是德國;拍改的話是兩處** | `-ed` |
> | 11 | **線1 `PLAN-v9` 的 `Q-開工`**(`:1224-1229`「請只回一個字」)📎 **先讀 `docs/handoff/2026-08-27-return-inventory-plan-for-sean.md`**(`716741da`,101 行)—— 六節,而**第二節是「你會看到什麼」、第四節是「你仍然不會有什麼」,第二節排前面**;第三節列**六格批准之後還會回來找他的事**。🔴 **那張表自己有一節自檢,`-21` 抓到自己四格「比實際乾淨」**,其中最狠的一格:**「一張為了讓他讀得完而寫的表,它『讀得完』這件事本身就是一種失真」**(它挑了 5 條「你不會有」,而 plan §11 自己列了 14 條)。🔴🔴 **沒有人給推薦**:`-21` 讀的是節選、主視窗沒讀過整份、`-9e` 被 Sean 明文禁止裁決 ⇒ **這一題他要自己判,而那是鐵則 8 本來的設計** | 線1 / `-9e` |
>
> ⚠️ **`b4` 刻意不列的三類**(它的判斷,我不推翻):`Q-AI-Bots`(要先有 Traffic 數字才問得出)/ 尺過不去那 3 格(**是工不是題**)/ `#947`(主視窗 `10452c52` 已關)。
>
> 🔴 **`Q-27a` 的射程 —— 🔴 本節寫下時是真的,而 2026-08-27 02:02 Sean 那發 push 之後就假了,而它在檔案裡原樣留了數小時**(發現者 `-9e` 複量,主視窗當場複跑確認)。
> **原文照留,不刪**:「接線碼 `f8f62cab` **未推** ⇒ 現在安全,而**那是運氣不是安排**(數法 `git merge-base --is-ancestor f8f62cab origin/dev` ⇒ 不成立)」。🔴 **同一條指令現在 `rc=0`**(`git log --oneline origin/dev | grep -c f8f62cab` ⇒ 1)⇒ **它已經上去了,而 DB 那兩欄也已經 apply ⇒ 順序沒出事。**📌 **這是同一張表今天第三個「狀態已變而表沒變」的格**(`Q-探針2` / `Q-G7射程` / 本格)——🔴 **前兩個是我沒去改;這一個更難:它是一句【當下為真】的描述,而讓它變假的是【別人做的事】** ⇒ 檔不會改、測試不會紅、`grep` 數不變。**複掃型守門對它天生無效。**⇒ 處置:**這種句子一律把數法寫在旁邊、並明說「引用前重跑」**,而不是把結論寫死。
> ⚠️ **正式庫到底跑了沒, 主視窗看不到、沒有量過** —— 這句是從 08-26 交接檔讀來的,標**未確認**。
>
> 🛑 **Q-27a 執行時的硬邊界(給 Sean 貼 SQL Editor 時看)**:
> **只跑 `supabase/migrations/20260826140000_m4b_admin_customer_list_view_birthday.sql` 這一支。**
> **`20260826160000`(片3a)** —— ⚠️ ~~它檔頭 `:7` 逐字「不要 apply。codex 判 FAIL」~~
> 🔴 **2026-08-27 深夜:上面那個引用已經指反了。** 片3a 被改過(`334da9fa`),`:7` 現在是拓撲那一行;
> 「不要 apply」四個字現在**只出現在說它已作廢的那句裡** ⇒ **跟著行號走的人會看到不相干的話,找到那句的人會看到相反的結論。**
> 📌 **不要再引行號 —— 引該檔的「審查紀錄」節。**
> **現在的兩個獨立條件**:①技術上**跑得過**(照正式庫量到的五角色形狀重建世界實跑 rc=0)②審查上**五輪 findings 全折**。
> 🔴 **兩者不互相蘊涵,而 apply 永遠是 Sean 的手(不可逆 ⇒ R3)。今晚仍然不跑。**`20260826150000`(片2)還沒推,現在不用管。
> 📌 **數法**:`ls supabase/migrations/ | grep -c '^20260826'` ⇒ **3**;
> ~~`grep -l '不要 apply' supabase/migrations/*.sql | wc -l` ⇒ **4**~~ 🔴 **這條守門今晚被證明量錯東西**:它數的是「**有幾支檔提到這個字串**」,而它想守的是「**那句禁令還在不在**」。2026-08-27 片3a 的禁令已作廢,而該檔**仍然命中**(因為作廢那句**引用了**那四個字)⇒ **守門一聲都沒出,值還是 4**。📌 **引用與宣告,對 `grep` 是同一個東西。** ⇒ 錨要改成**只有無條件禁令才會命中的字串**,這條待重寫;
> 負對照 `ls supabase/migrations/ | grep -c '^20260899'` ⇒ **0**。
> 🔴 **而這裡本身有一格教訓**:主視窗第一版只掃了「08-26 那三支」——
> **危險的分母是【會被貼進 SQL Editor 的任何一支】, 不是【今天做的那幾支】。這兩個分母沒有理由相等。**


> 🧭 **2026-08-21 01:42 Codex 成長優化規劃交接（docs-only、未實作）：** Sean 要把「AI 外部搜尋推薦、SEO 以外的成長與成交方法」交給 Claude 進一步安排；已重新核對當下 Git、`STATUS.md`、Phase 1／Phase 2 邊界、站內搜尋、GA4、商品資料與 2026 Google 官方更新。完整接手文件=`docs/handoff/2026-08-21-growth-discovery-conversion-claude-handoff.md`。**Claude 第一輪只複核與排程，不直接改產品程式**；先重跑該文件 §2.2、核對 dirty ownership，再按 §9 回報差異、3～4 個里程碑與 Sean 決策題。固定邊界：不做客戶 AI／LINE 自動客服、不公開庫存數、不把訂貨品假標現貨、不把 `llms.txt` 當 Google 成長重點、不在 ETA 資料不足時全量送 Merchant Center、不照過期 `pg_jieba`／Medusa 文件施工。此段只新增規劃指標；下方 2026-08-10 舊總帳仍是歷史備查，當下進度以 Git > `STATUS.md` > 本檔為準。

> 🧭🧭🧭 **2026-08-10 上午 夜跑收官・全窗換代交接(最新;下方各節僅備查)。** 夜跑代主視窗總帳:**14 片收割**(取消線 A13b 六片全完/收款帳本 OP1+OP2a/L5a 讓路兩片/350c 面板/347-2b 搜尋/347-3a 日期 RPC/#354+殘項 A 靶/#351① 白話表)、root **421 檔 6484 綠**、check **32/0**、**押推 46+ 顆等 Vercel 額度**(打滿中,額度回=推 dev+dev:main)。四窗全關(交接包=B-362/D-416/E-053-CHECKPOINT+P-293/295/296)。**新主視窗開工包=`/Users/sean_1/pcm-mailbox/PROMPT-MAIN.md`(日班版);四窗開工包 PROMPT-{B,D,E,P}.md 已更新,佇列閘在 Sean Q1-Q7**(誤沖恢復/取消三格 UX/cancel_mode/重試出口/殘餘風險認列/apply 六支批准/stash 清理;題文=主視窗對話晨報)。apply 六支序:L5a-M→L5a-1→OP1(PG<17 兩處)→OP2a(一處)→347-3a(**不可重跑**;apply 後 3b 前=真站 adapter smoke)。事故帳:turbopack fork 崩潰(上游未回報;多窗 dev 一律 --webpack、單窗照用,runbook §E-12)、兩次全機當機+VSCode 崩全復原零損失、#353 三重撞號+#358 撞號全改派(memory ambiguous-identifier)。
>
> 🧭🧭🧭 **2026-08-09 23:3x 主視窗換代交接(Sean 拍板全窗換新+夜跑到 09:00)**:新主窗開工包=`/Users/sean_1/pcm-mailbox/PROMPT-MAIN.md`(第一動/收割紀律/四窗佇列/帳面哈希/晨報清單全在);四施工窗開工包=PROMPT-{B,D,E,P}.md。本代收割:P probe 線 4 顆(3b887c5e)/E D2a(f186831e)+D2b(c421d90c)/D 350b(4509eaef);押推=origin/dev 停在 e9459aa3、本地至 c57a88a4;B 61d9e5b1 押著未收割(codex 審舊版=債)。Vercel 額度打滿=部署停 cc66721b。本節之下 22:35 當機復原節與更早檔頭=歷史備查。
> 🔴🔴 **2026-08-09 22:35 全機當機重開(電腦重啟)· 復原紀錄**:四施工窗被 VSCode --resume 救回、context 全存活(B/P 已自報,D/E 待報);主視窗 socket 換 `1298.sock`、各窗位址檔重寫中;信箱哨兵 Monitor 重掛=**bbtqv0yce**(舊 brjcnghb9 已死);scratchpad 重建(mail-baseline/window-liveness.sh)。**P 之 L4b+probe 四顆已收割 `3b887c5e`(root 6322 綠)、押著未推**;Vercel build 額度打滿=cc66721b 之後全部未部署(含 350a 側欄),額度回來一次推。B=修 OP1 斷言不一致→checkpoint→等 /compact;E=續折 R3 五條(E-037-A);D=續 350b;P=待交 L5a 片級 plan(P-279-STOP)。
> **四窗與位址檔**(門鈴位址讀 `.{代號}-socket`,由主視窗依門鈴 from= 回寫;認不出=只落信不猜):**B**(.b-socket)=收款帳本塊 P1 施工中(B-340-Q plan 四題 A/A/A/A 已裁:混合受理分軌退/Q13 落 DB 具名函式/同單折抵/只 DB+RPC;confirm_order_payment 鎖=B;P4 backfill=Q10 例外問 Sean;P1→P2→P3/P5/P6→P7,全高風險)。**D**(.d-socket)=#350 工作區殼偵察+plan 落信待核(347-2a 已收割 `2891e2c8`;2b 硬閘之一已解:statement_timeout 已實量(service_role=300s/authenticator=8s,落 20260809180000 檔頭 e310c814;2b 免重量、引用即可)+翻頁保留搜尋詞;Q-a=B POST+PRG)。**E**(.e-socket)=取消線 D2 施工中(D1 已收割;D3-D5 照 plan v3.2;D6 裁 A=元件層真瀏覽器,POST body 斷言取代 persisted)。**P**=二代已關窗,**三代待 Sean 開**(提示詞已交;首任務=雙扣 probe:P-271-STOP §4 五步+P-269-A 三條件;branch payment-lifecycle 領先 dev 兩顆 ed57f67a/d67aba05 不 rebase;🔴 口徑=「settle failed ⇒ 舊 URL 不成交」是**待實測前提、未被證明**,勿說已驗證;probe ①兩態都失敗=收割 L4b/②任一成功=不上、整理決策題/③構造不出=記未確認)。
> **Apply 現況**:pending=0(180000/190000/200000/210000 全 apply);L4b(app 層)未上線等 probe。**收割紀律**:只收乾淨樹、check(現 29/0)+root pnpm test 綠才推、新 migration 落檔=八支重釘+record all(b2s2b 有跨行條件式勿用機械 append 註解;record all 連跑有 REFUSE 瞬時紅=單獨重錄即可,B 待補 wait-and-recheck)。
> **今晨拍板**:Q16=A(L4a apply ✓)。**機制新增**:裁決信 10 分 ACK 信+內容必落信(runbook §C);.pending-replies.md 未回應帳(心跳⑥查逾 20 分);e2e-prod 單次紅=冷快取守門刻意中止不管、連續紅才管(已告 Sean)。
> **完整化總 plan**=`docs/specs/2026-08-09-admin-completion-wave-plan.md`(塊序/匯流點/完成判準);決策題 Q1-Q16+建單六題+B 四題全清,**到總測試前 Sean 零決策**(例外=動錢/大量寫入 apply)。待辦小items:~~statement_timeout Studio SQL~~(✅ 已量已落檔 e310c814)、D6-b 掛 #350 線下、#354/#355 backlog。(壓縮後讀本節,13:50 舊節僅備查)。第一動=date+find 全量掃 pcm-mailbox+確認 Monitor(brjcnghb9)活著+心跳 /loop 600s 照排(prompt 含 liveness+gh site-patrol issue 檢查;liveness 腳本=scratchpad/window-liveness.sh,重建法見 playbook §7-⑥)。**
>
...
>
> **拍板新增(全落 memory)**:Q9=A 出貨線開工/Q8=A 選車欄草稿接線片/額度=C 跑到歸零/**3 帳號輪轉不並行+一個月(至 09-07)完成整站**(`project_workmode-0807-account-rotation-one-month-goal`;佇列空=主視窗失職;月線序在 08-07 白天對話=週1 出貨+D 收工、週2 取消+退款、週3 認證+附件+上架、週4 未拆區塊清尾+總審)。
>
> **兩施工窗收束中(切換前最後狀態)**:**B**=W1(五支 RPC 骨架+ACL)施工中、已發收束令 `B-191-A`;**D**=Q8 接線片(高風險=共用選車欄、替身鏈不降級)+manifest H7 遺債段兩件、已發收束令 `D-178-A`。兩窗收束 STOP 可能落在信箱=**切換後第一件事收割它們**。E 窗已收線關閉(post-apply 已全清:型別重生欠著、排 B 的 W 線片間)。
>
> **立法候選新增**:①純 .sql/.sh 片三綠恆綠=鐵則 11 結構缺口 ②「守門畫在動作、不變量在狀態」族第 3/4 例(W0b F4 UPDATE 路+rm 活 cluster)③D 之探針誤讀=前提沒成立當實錘。收線立法(草稿 23 條+這些)仍待落檔。
>
> **肉眼驗清單(3111 現役)**:選車欄留字三點/商品頁編號 01/02/會員中心格狀+捲動+icon/品牌頁橫捲 10/N°05 排版/收藏鈕不跳頁。suite 疑一條時序敏感測試(兩筆觀察零紅,先記著)。codex 牆至 09-05(僅原帳號;**新帳號的 codex 額度未知,切換後可實測**)、補跑清單累加中。
...
> · dev 乾淨零未推(`f961462`);全套 344 檔 4620 綠(D3b 後;D3c-1 未併)。伺服器 3000=production build(D2f 版)。
>
...
> → Sean 拍板換路:**供應商改「主檔 + 下拉選單」,A5b 與 A5c 砍掉**,
> **並明確推翻 07-27 Q2=B「E12 供應商主檔這期不做」**(本次確認、非假設)。
>
> **Sean 今天八項拍板**(逐字見交接檔 §2):下拉選單 / **不可刪除、只可改名** / 停用開關 /
> **seed 26 家** / 字母排序 / **打字帶入候選** / **Q1=A 改名後歷史顯示新名字(不存快照)** /
...
> 這是 Sean 08-01 拍板 Q1=A 的產出;從今起進度度量 = **地圖上剩幾片**,不看 commit 數/守門數。
>
> 🔴🔴 **最刺眼的一條**:27 項裡能用的仍只有 **2 項**(看訂單明細、刷卡收款),**從 07-26 到今天沒變過**。
> 過去六天做的全部是資料層(七張表已 apply 正式站),而 `apps/admin/src` 對
> `order_notes` / `order_item_procurement` / `order_cancellations` / `order_refunds`
> **四個字串全部零命中**(本日實查)⇒ **資料層跑在 UI 層前面六天,而 27 項的驗收全發生在 UI 層。**
> 更正後小計 = **✅2 / 🟡6 / ⚠️2 / ❌17**(對照 07-29 A0c 的 2/5/20:**沒有任何一項從 ❌ 變 ✅**;
> 變的是 6 項從 ❌ 升到 🟡 = 資料層就位但員工按不到)。
>
> **三個數字**:已拆好可直接開工 **43 片** / 已知未拆區塊 **13 個**(開批才拆)/
> **連區塊都沒有的領域 5 塊**(商品線 20-24、客戶全欄位 25′、員工認證 26、稽核 UI 27、新需求 28-30)。
> 🔴 後兩者**刻意不預先拆片** —— 硬估就是重犯 7 月「巨型前置規劃」的錯(E10 69 片 + 關卡1 審 20 輪,
> 結果 A7b 105 道守門被一句換路作廢)。
>
> 🔴 **排程上最重要的事實**:43 片裡可夜跑的 21 片機械 + 4 片負測**大多是九碼退場與讀模型,
> 做完員工一項都不會多做完**;真正讓 27 項轉綠的是**高風險 10 片**(A2b1/A4a 守門、
> A5a/A6/A9v 權限、A8c1/A8c2/A8a1/A8a2 金流取消、A9d2)與**需肉眼 12 片**
> (A10a/A10b、A11a-c、A12a-b、A13a-b、A14a-c)—— **兩類都不能夜跑**。
>
> 🔴 **E10 完成 ≠ 後台完成**:§6.1 只涵蓋 27 項中的 17 項,商品線 / 客戶全欄位 / 認證三塊
> 完全在 E10 之外 ⇒ 依 Sean「後台是二元切換」的前提,**E10 做完員工還是不能上工**。
>
> **下一步不變**:今晚 22:00 後 sandbox 部分退款探測(見下一段 A7c 交接);
> 白天空檔若要動手,建議挑地圖 §3 的高風險片,不是機械片。
...
> 🔴 **R5(Fable,打 D9 是不是第三層名過其實)= FAIL,11 must-fix + 2 nit,v6 折入 13/13**
> (逐字 `docs/reviews/2026-07-31-e10-a7b-k1r5-fable.md`)。五輪合計 **82 must-fix + 14 nit**。
> 🔴🔴 **R5 證明「同一個病」已連三層**:R3 抓「證明成立但問題選錯」→ R4 抓「把 worker 紀律講成 DB 證明」
> → **R5 抓「v5 說 D9 讓錢已入帳必定被擋下,但當時沒有任何 CHECK 做那個比對」**
> ⇒ v6 起紀律:**每一句「⇒ 所以安全」都必須指得出具名 constraint,指不出來的改寫並進 §8**。
> 🔴🔴 **Sean 的 Q4=A 是根據被誇大的描述做的** ⇒ v6 §14 已用更正後的描述**重新提問**,尚未重答。
...
> ✅ **Q3=A 已由 Sean 完成肉眼驗收(2026-07-31,逐字「我的愛車ok」)** —— 這是本片唯一
> AI 無法自驗的項目(需登入、不讀 `.env*`);現由 Sean 背書通過,非 AI 宣稱。
...
>
> ✅ **2026-07-27 Eazi-Grip Supplier Daily Sync 修復已正式上線。**根因不是爬蟲漏資料，而是 HOSEYAM005 的現況 `base={}`／`BL=黑色` 要更新成 `base=黑色`／`BL=藍色`；最終資料唯一，但一般逐列 bulk upsert 會在 BL 尚未離開黑色前撞 `pv_spec_unique`。修法辨識 final collision 與 transition hazard；final 仍寫前 abort，hazard 群完整排除一般 orphan delete/bulk upsert，交給 `public.sync_product_variant_group` 單一交易完成 supplier lock、payload 重驗、唯一 sentinel 騰位、孤兒刪除、完整 desired upsert 與終態 assert。RPC 為 SECURITY INVOKER，僅 `service_role` 可 EXECUTE。
> ✅ **驗證證據:**Eazi-Grip 線上資料唯讀 dry-run 精準命中 HOSEYAM005 1 群 transition hazard、final collision=0；targeted Vitest 31/31、full suite 3029 passed + 1 todo、typecheck、lint、build、`git diff --check` 全綠；隔離 PostgreSQL 17 實跑 HOSE transition、既有兩 row A↔B、異常 primitive、post-mutation rollback、ACL 與雙連線 TOCTOU。`supabase/tests/database/atomic_variant_group_sync.test.sql` 留 25 項 pgTAP regression；本機臨時 cluster 無 pgTAP extension，故關鍵 SQL 路徑另以等價 assertion 實跑。獨立唯讀對抗審查最終 PASS、0 finding。
> ✅ **production release 已依 migration-first 順序完成:**① `20260727084801_atomic_variant_group_sync` 以原版本號 apply 到 `pcm-website-v2` production ②驗 function signature / SECURITY INVOKER / `search_path=''` / ACL（service_role=true、PUBLIC/anon/authenticated=false）/ `pv_spec_unique` 保留 / sentinel=0 ③push `246fedd` 到 `origin/dev` ④以新 commit 直接執行正式 `--supplier=eazigrip`（GitHub #59 單 job rerun 會沿用舊 SHA，故刻意不採）。結果=1686 群/5283 變體、一般 5280 + atomic HOSEYAM005 3/3、0 孤兒/0 下架/0 價格異常；production read-back=`HOSEYAM005=黑色`、`HOSEYAM005BL=藍色`、`HOSEYAM005RED=紅色`、全供應商 sentinel=0。
>
> 🔴🔴 **接手 session 的唯一入口 = `docs/handoff/2026-07-27-e10-order-closure-handoff.md`**(本檔以下是背景)。~~舊入口 `2026-07-27-admin-full-planning-handoff.md`~~ 的施工序已於 07-27 深夜被 Sean 推翻。總規劃本體 = **`docs/specs/2026-07-28-e10-order-closure-master-plan-v2.md`**(v1 經 codex 關卡1 FAIL 67 條、已重寫;逐條裁定 = `docs/reviews/2026-07-28-e10-k1-findings-triage.md`)。Sean 07-27 目標逐字=「我目標是把完整後台做好」。⚠️ 舊檔寫「由新 session 用 Fable 主導」—— 實際 07-27/07-28 兩輪跑的都是 `claude-opus-5`(harness 環境宣告為準,已向 Sean 揭露)。
> 🔴 **現行主線已於 2026-07-26 換軌 ＝ M-4b 後台重建「員工上工」線**(Sean 定調北極星:「可以完整上線給員工使用…他們不是工程師」)。~~接手第一件事 = 做全後台畫面預覽 artifact~~ ✅ **已完成**(46 畫面、Sean 驗收畢、U1-U7 全拍板;artifact 現行網址見 STATUS)。完整交接包 = `docs/handoff/2026-07-26-admin-backend-spec-handoff.md`;主規格 = `docs/specs/2026-07-25-admin-backend-rebuild-spec.md`(先讀 §0a 最新拍板)。🔴 **施工序已於 2026-07-27 深夜改為「E10 訂單閉環 → E8-B 認證(email+密碼)→ E8-C 2FA」**(Sean Q1=A、二次確認 Q6=A;E11 積木改為隨 E10 按需長出、不獨立成階段)。~~舊序 E11 積木 → E8 員工權限 → E10 訂單閉環~~;
>
...
- Updated: 2026-07-26, Asia/Taipei — 🔴 **換軌:M-4b 後台重建「員工上工」線開跑。本 session 零 code、零 migration、零 UI 變更,產出=規格 + **23 份研究底稿(5021 行)**。commit `b6908fb` / `6427616` / `8643c2c` + read-back 修正支;**push 狀態當場查、不看寫死數字**。**
  ...
  - **北極星(Sean 逐字)**:「可以完整上線給員工使用,操作,修改網站。而且他們不是工程師」⇒ 驗收標準 = 主規格 §1「員工的一天」**27 項**,實查 **✅2 / ⚠️6 / ❌19**(fresh read-back 實測更正)。
...
- Updated: 2026-07-24, Asia/Taipei（晚間）— 🎉🎉 **PCM 正式站完成史上第一筆真信用卡 3DS 刷卡、金流全鏈打通、Blocker 解除。** 全文 = memory `project_m3-first-real-charge-success-2026-07-24`。
  ...
  - **補差額用賣場 1 元商品**已上正式庫（品牌 PCM／分類 服務／其他／handle `pcm-balance-payment`／變體 `PCM-BALANCE-1`／`supplier_slug=manual`、seed 檔 `docs/specs/2026-07-24-m3-balance-payment-product-seed.sql`、對抗審 GO）。🔴 **缺免運**：客人補 NT$50 差額會被加 NT$100 運費 → 用途待解（下個 session）。
  - **下個 session 排隊(皆已拍板/備妥、不趕)**:①**已收未定 A 案**（付款↔品項狀態連動、純前台閘、Sean 拍 A+3 規則、落點 `apps/admin/src/lib/orders/workflow-select-options.ts`）②補差額商品免運 ③🔴 **退刷自動化線**（訂單取消+TapPay refund、5-7 片正經線、PRD `docs/specs/2026-07-24-refund-automation-line-prd.md`、鐵則 12 錢+權限+DB、需 Sean db push+admin TapPay env+拍 D1-D4）。
  - **Git**:本 session = ops + prod seed via MCP + Sean 設 env,**零業務 code 變更**;docs 記錄已 commit `4e78db0`（STATUS+檢查表+seed 檔）、**未 push 1 commit、等 Sean 手動推**。
- Updated: 2026-07-24, Asia/Taipei — 🔴🔴 **Sean 五拍板、方向大轉向(推翻 S3 sandbox 路線)**:**Q1=A** B-4 採統一三分支(方向已批、下一步 codex 關卡1)/ **Q2+Q3=來真的**——**放棄 sandbox 3DS 測試,金流全部用正式站 1 元商品 + Sean 真信用卡直接刷驗收**(`2026-07-24-m3-s3-sandbox-3ds-e2e-runbook.md` 已標作廢、改走 `2026-07-24-m3-go-live-real-charge-checklist.md`)/ **Q4=B** 退款線提前啟動第二段=後台取消訂單即自動打 TapPay refund(PRD `2026-07-24-refund-automation-line-prd.md`、5-7 片、需 Sean db push + admin Vercel TapPay env)/ **Q5=A** #269 排隊。Sean 總方針逐字:「**先把金流做到可以完整實際刷卡,要測試等等所有都用我自己真的信用卡+1 元賣場直接刷**」。
  ...
  - ✅ **#291 法律頁 2026-07-24 全鏈上線**:commit `5d5af4c` 已推 dev+main、production 部署 READY、`shop.pcmmotorsports.com/terms`+`/privacy` 實測 HTTP 200 + 內容正確;兩支 migration 皆 apply(DB hash=`eca6a241…` 與常數一致)。剩:Sean 肉眼驗渲染後 payload(drift `legalRenderedPayloadNotEyeballed`)、未經律師簽核(明示不用)。🔴 **提早部署的真實風險(2026-07-24 更正,原本寫的「結帳全斷」是錯的)**:版本列 `'2026-07-24'` **已存在**於正式 DB(首支 migration 已 apply)⇒ 現在部署**不會** FK 失敗、結帳不會斷。FK 只綁 `terms_version`、**不綁 `content_hash`** ⇒ 會**成功建單**,把客人對新條款畫面的同意掛到一列 `content_hash` 仍是舊值的版本上 —— **靜默的舉證錯配、無任何錯誤訊息**,危險性比「結帳全斷」**更高**(壞事發生時沒人會發現)。且期間一旦產生 consent,修正檔的守衛會拒絕覆寫,只能改開新版本鍵收拾。
...
- Updated: 2026-07-24, Asia/Taipei（凌晨）— **過夜自主 session 總結(Sean 睡前交代「推進不需他手動的活、要他批的排隊」)**：**未 push、未 apply DB、未開 flag、未動 `.env*`**;`dev` ahead `origin/dev` **4**(S2 `a5d7619` + 下列三 commit)。
  ...
  - 🔴 **#269「首頁 `?filter=new|sale` 接線」查證後改列排隊**(非今晚自主):它的解法要改 `products-url-state.tsx`=#287/#288 的高風險 URL-state 禁區(三個目錄 bug 來源、Next segment 碰撞)、sibling #287 明載鐵則 8 須先提 plan、且只在 production build 現形正需 #288-c/d/e 守門就位。→ 待 Sean 批 + #288 守門完成。
  - 🔴 **Sean 早上決策批次(詳 STATUS「Sean 待決策」)**:①**B-4-Q1**(排最前、架構)=B-4 三分支要不要改現行 flag-off 的 cardholder.email payload(A 統一/B flag-off byte-identical)②**S3-Q1**=staging 部署管道待查 ③**S3-Q2**=取 3DS-capable sandbox merchant ④**S6**=退款維持 SOP 或授權進「退款線第二段」實作 refund()(現手動可用、低急);⑤#269 是否照上述排隊。
- Updated: 2026-07-23, Asia/Taipei — **M-3 S2「sweeper 搬 Supabase pg_cron + anomaly-alert 備妥」✅ code + branch 驗證收工（本 session、**未 push**、**未 apply DB**）**：settle-sweep + anomaly-alert 兩支 cron 從 Vercel 每日搬 Supabase pg_cron + pg_net。新 migration `20260723120000_m3_s2_settle_sweep_pgcron.sql`（私有 schema `pcm_cron` + Vault-backed SECURITY DEFINER wrapper〔執行期讀 vault cron_base_url/cron_secret、`net.http_get` 帶 Bearer 打 route〕+ `cron.schedule` 兩 job〔settle `*/2`、anomaly `0 1`〕+ **vault secret 前置閘** + fail-closed 自閘斷言）+ `vercel.json` 移除兩 crons。🔴 **Q1=A：flag（`CRON_SWEEPER_ENABLED`/`ANOMALY_ALERT_ENABLED`）+ Resend/LINE env 全留 S4**、本片只建設。🔴 **實測 postgres 非 superuser → 不 REVOKE net/cron/vault**（物理不可行；**在 runbook 步4 確認未曝露後**才 dormant〔那些 schema 不在 Data API 曝露清單 = Dashboard 設定、DB 查不到、須人工核〕；連帶 **Database Webhooks 不受影響、Q_B moot**）；安全必要條件「曝露 schema 不含 net/cron/vault」= db push 前人工硬核。✅ **Supabase preview branch 兩次實跑**：套用綠 + 兩 job active + 端到端 `net.http_get` 200 + 突變（anon execute→3b RAISE、缺 secret→0b RAISE）。三綠（zero TS）、四層審不降級：codex 關卡1 FAIL→折入、code-reviewer(opus) PASS、Fable GO、codex 關卡2 R1 FAIL→折入→R2。🔴 **殘餘風險**：CRON_SECRET 三 route 共用（含無 flag 的 email-sweep）= backlog #294 拆 secret；service_role 可讀 vault（NOLOGIN、非持 key 即可）。plan 真權威=`docs/specs/2026-07-23-m3-s2-sweeper-pgcron-plan.md`（§11 定案）。🔴 **Sean 手動 runbook**（slice plan §11）：Vercel 設 CRON_SECRET → 啟用 pg_cron+pg_net → SQL Editor idempotent block 存 2 vault secret → db push（缺 secret 會 0b RAISE 拒套）→ 6h 內連通實證 → 確認 Vercel crons=0。**下一片 = S3 sandbox 3DS 全鏈實測。**（本片另存永久 TapPay 參考 `docs/reference/tappay-reference.md`。）
...
- ⚠️ **以下為 S2 開工前交接、S2 已於本 session code+branch 收工(見上)**;🔴 **Q1=A 更正:S2 只建設、不開 flag,flag 留 S4**(原「搬 + 開」字面已作廢)。黑洞即時對帳客人自助迴圈(S1a 逾時出口 + S1b 反查)已完整;S2 補**背景**網。現況缺口③=cron settle-sweep 每日一次 + `CRON_SWEEPER_ENABLED` false(與 3DS/anomaly flag 一起關)→ prod 黑洞零自動兜底;S2 已建好 pg_cron 分鐘級基礎設施(待 Sean apply)、**S4 開 flag 才真兜底**。🔴 **平台注意**:memory `reference_pcm-platform-plans-vercel-hobby-supabase-pro` 排程別預設綁 Vercel(Hobby 禁商業用途);Supabase Pro pg_cron 是方向。🔴 **Sean 手動步驟**:S2 若含 pg_cron migration → Sean db push;開 `CRON_SWEEPER_ENABLED` flag=Sean 手動。plan 真權威=`docs/specs/2026-07-23-m3-tappay-production-settle-line-plan.md` §3(go-live 開關硬清單)。
...
- 🔴 **Sean 的真刷驗收路徑（2026-07-22 明講）＝ 正式站開 1 元商品直接刷，不走本地區網**。規劃結帳驗收時**別再提「用手機連本機」**。連帶 backlog #293（本地 HTTPS 通道）已擱置。
  ⚠️ 該計畫的三個前置（②③為當時查證，**①已於 2026-08-17 夜推翻**）：①~~正式站結帳目前關著（3DS flag 全 false、`isThreeDSEnabled` 註解明寫「僅 sandbox/staging」）~~ 🔴 **反了：三個 flag 都是開的**。Sean 2026-08-17 夜給的字面值（原字）`TAPPAY_3DS_ENABLED:true` / `CRON_SWEEPER_ENABLED:true` / `ANOMALY_ALERT_ENABLED:true`，皆小寫，命中 `apps/storefront/src/lib/payment/three-ds-flag.ts:30` 的 `=== 'true'`。🔴 **而「已查證」三個字在這裡是錯的口徑** —— 正確的是「**查證方式未載明**」：來源＝**Sean 本人回報 → 主視窗轉述 → I 窗落檔，沒有任何人親眼看過 Vercel Production env 面板**⇒ 這是「他說的」不是「量到的」。（正本 `STATUS.md` Blocker 欄 + `docs/specs/2026-06-13-m3-3ds-webhook-master-plan.md` 檔頭；🔴 該 spec 原本那條量法 `grep -n '3DS flag 全 false' STATUS.md` **已失效、現在會命中刪除線**，不要照跑。）②退款無自動化（全樹無 TapPay refund 呼叫實作，依 07-17 拍板要 Sean 手動去 Dashboard 退）③1 元商品會被一般客人看到（正式站目錄公開、無「只有我看得到」機制）。詳 memory `project_sean-real-payment-verify-via-1nt-product`。
