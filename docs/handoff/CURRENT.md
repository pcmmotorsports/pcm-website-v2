...
>
> | **Q-探針1** 貼 SQL Editor 跑 `docs/probes/2026-08-26-email-sweep-enqueue-status-from-body.sql` —— 問「信到底有沒有被丟進去」(`email_outbox` 整表 1 列,08-22)。純唯讀 `BEGIN…ROLLBACK`。⚠️ **先看區 0,任一列非 ✅ ⇒ 底下全部作廢** | 08-26 `-de` 端過兩次(**只在對話裡**)、08-27 主視窗落檔 | 通知線的判斷 | ⏳ 未跑 |
> | **Q-探針2** 貼 SQL Editor 跑 `~/pcm-mailbox/cf-拓撲探針-service_role成員與三表ACL-20260827.sql`(273 行)—— 問「正式庫是不是 `authenticator` 拓撲」⇒ **決定片2 與片3a 到底能不能 apply**。它的區 9 用正式庫的值模擬段A 會印什麼,**不用 apply 就知道** | 08-27 `cf` 產、`-de` 轉、主視窗落檔 | `-de` / `cf` 整條 RLS 線 | ⏳ 未跑 |
