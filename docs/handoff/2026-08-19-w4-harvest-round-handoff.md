...
>
> ### 🔴🔴 **讀這份檔之前先知道:它會腐爛,而且已經腐爛過四次**
> ```
> 2026-08-20 一夜，同一份檔、同一個病【四次】:
>   章節標題 ／ 檔名 ／ 散文段 ／ §6 的【下一節】
> 而四次的共同形狀都是:**我核的那一節沒問題，而它住在隔壁。**
> 🔴 而「隔壁」的定義會一直往外移 ⇒ 所以**不要跳讀**。
> ```
> **⇒ 判別法(這一份檔的每一句都適用)**:
> ```
> 這句描述的是【一個已經結束的事件】 ⇒ 它不會過期，照讀
> 這句描述的是【一個仍在變的狀態】   ⇒ **不要相信它，去跑 §6 那四個命令**
> ```
> ⚠️ **而本檔已標記過的過期段落一律【不刪】** ——
> 留著是為了讓你看得到「一份交接檔是怎麼腐爛的」。看到 ~~劃線~~ 或
> 「已過期,留著是為了示範」就知道那一段不是現況。
>
> 📌 **開放項(2026-08-20 全檔掃過一次之後留的)**:
> **以上是我掃過的;而全檔還有沒有同類的東西,下一個人請自己再掃一次。**
> **一份寫得越具體的檢查清單,越會把「我列的」變成「全部」。**
...
🔴 **那個「1 支」是落檔時的數字,不要當現況** —— 當場跑 §6 那四個命令。
...
### 🔴🔴 **第 0 步(2026-08-20 新增):量「落後」,不是只量「領先」**
```
W1 的判別句:**一棵一直很乾淨的工作樹，不代表它接得上主線 —— 那是兩個不同的問題。**
🔴 而它為什麼可怕:「落後 N 顆」在那一側的【任何指令上都不會顯示】——
   `git status` 乾淨、四綠全過 ⇒ **所有會習慣性去看的訊號都是綠的**
⇒ ⇒ 而它爆的時刻是【有人正要推】，那是全場最不想 debug 的一刻。
```
**而這正是收割窗自己的盲點**:本檔第 1 步量的是 `dev..branch`(**領先**),
**從來沒有量過落後** —— 兩個方向都要。

### 🔴 而更精準的病名(W1 重量後收窄,比上面那句準)
```
~~一棵很乾淨的工作樹不代表它接得上主線~~ ← 仍然對，但不是這次的病
✅ 這次的病:**「未推」與「未收割」是兩個不同的問題，而同一個數字被拿去答了兩個**
   `origin/dev..HEAD` 答的是【有沒有推到 origin】（那是 Sean 的動作）
   而它一直被拿去暗示【還沒被收割】
   🔴 **兩者今晚一直碰巧一致，直到收割發生之後才分岔 —— 而分岔的那一刻沒有任何訊號**
⇒ ⇒ **報狀態的正確形狀**:
   「**N 顆已收進本機 dev；相對 origin 未推 M**」，**不是單一個「未推 N」**。
```

```bash
# 0. 領先與落後【一起量】—— 而落後要量【兩個基準】，不要只量一個
for b in $(git for-each-ref --format='%(refname:short)' refs/heads/); do
  ah=$(git rev-list --count dev..$b 2>/dev/null)          # 領先本機 dev
  bd=$(git rev-list --count $b..dev 2>/dev/null)          # 落後本機 dev ← 收割看這個
  bo=$(git rev-list --count $b..origin/dev 2>/dev/null)   # 落後 origin  ← 推的時候看這個
  printf '%-24s ahead=%-5s behind_dev=%-6s behind_origin=%s\n' "$b" "$ah" "$bd" "$bo"
done
```
### 🔴🔴 **而【落後本機 dev】與【落後 origin/dev】是兩個不同的問題**(2026-08-20 實錘,我當場踩)
```
我第一版只量了 $b..origin/dev ⇒ w1-order-panel = **68**
而收割是往【本機 dev】併，不是往 origin ⇒ 正確的量法 $b..dev = **108**
🔴 兩個數字都是真的，而它們答的是不同的問題:
   $b..dev        ⇒「這支分支落後【收割線】多少」   ← 收割窗看這個
   $b..origin/dev ⇒「這支分支落後【已推的線】多少」 ← 推的時候看這個
```
**首次實測(2026-08-20 03:5x,本樹)**:
```
w1-order-panel  ahead=0  behind_dev=108  behind_origin=68
（「領先」欄是 0 ⇒ **只看領先的話，它長得像「沒事」**）
```
⚠️ **而那 108 不代表「有東西沒收」** —— 我當場核過 W1 那幾顆
(`4884c1d5` / `613cf264` / `3671d3a3` / `86953978`)⇒ `merge-base --is-ancestor` **全部 ✅ 已在 `dev` 裡**。
🔴 **「分支落後主線」是常態,不是缺口** —— 它只有在**那支分支還要繼續往主線併**時才要處理。

⚠️ **而我上一版連【指令本身】都是壞的**:heredoc 把 `printf` 的 `\n` 吃掉,
**貼上去會斷成兩行、跑不動** ⇒ 📎 這正是本檔那條「**一條沒跑過的命令,是一句穿著命令外衣的宣稱**」。
**本版這段我實跑過才貼。**
⚠️ **而數字大不等於有事**:落後幾千顆的是**早就沒人動的舊分支**(`main` 落後 4354)。
🔴 **判別法不是看數字大小,是看【那支分支還有沒有人在上面工作】** ——
**一支活躍分支落後 68,比一支死分支落後 4000 危險得多。**

📌 **⇒ 這條的落點是【開窗第一件事】,不是收工清單**(主視窗提,我認)——
理由:它擋的是「**推之前才發現**」,而那時候已經來不及便宜地解決。
✅ **成本 = 兩個唯讀指令。**
...
**紀律**
```
· 🔴🔴 **解衝突之前先拿基線,不是解完再看**(2026-08-20 W4 差點犯 + 主視窗改硬)
```
解之前  grep -c '^<<<<<<<' <檔>   ⇒ N   ← **這一步是新加的**
解完再  grep -c '^<<<<<<<' <檔>   ⇒ 必須 0，**而且我知道我解掉了 N 塊**
🔴 差別:「解完再 grep 一次」只答得出【現在還有沒有】；
        先拿基線才答得出【我有沒有解完我以為的那些】。
```
**來由(實錘)**:2026-08-20 那次 traps 衝突有**兩塊**,而我只解了第一塊就去 commit。
**擋住我的是 git**(`fatal: cannot do a partial commit during a merge`),**不是我的紀律** ——
我的流程裡沒有「數一數有幾塊」這一步,**它一直隱含假設「衝突只有一塊」**。
🔴 **而那個假設活得久的方式是:前三次剛好都是一塊** —— 它不是被驗證的,**是沒有被反駁過。**
📎 同族:W5「事前寫期望值」,換到解衝突上就是這一條。

· 🔴🔴 **而「兩側各自 append」這個例外形狀,每次都要【重新驗】,不能靠慣例**
```
2026-08-20 同一支檔一夜衝突四次:前三次都是 append ⇒ 自解
第四次的【第二塊】comm -12 撈到**逐字重複 3 行** ⇒ 停下來看內容
⇒ 實查:分支側是同一段的【修訂版】，補的是一個當事人指定的限定
⇒ 若照前三次的慣例自解掉它，結果會是:
  **一條記錄某人犯錯的條目，會記成一個他沒犯的錯。**
```
**判別句**:`comm -12` **不是形式檢查,它是那個例外形狀成不成立的【唯一判準】** ——
**零重複才是 append;有重複就停下來讀內容,不論前面幾次都是 append。**
...
第一層｜全樹現況（記錄用，不是判準）
  🔴 **跑的時機 = 只在【收割告一段落】跑一次，不是每批**（2026-08-20 主視窗裁「乙」）
     · 每次 ~2 分鐘，而結果不可信 ⇒ 每批跑 = 每批花兩分鐘買一個不可信的東西
     · 而它仍有【一個】不可替代的用途:留下「這棵樹那個時段長什麼樣」的粗紀錄
     🔴 **而那個紀錄的用途是【事後對時間軸】，不是【當下判斷】**
       ⇒ 所以它可以粗、可以晚，**但不能沒有** ——
         少了它，之後有人問「那一批是什麼時候壞的」會答不出來
     · typecheck / lint **每批照跑**（不受負載影響、也快）
  TURBO_FORCE=1 全套跑一次
...
### 🔴🔴 而第一層【連紅的內容都不可信】—— 不只是「紅的不歸我」(2026-08-20 02:2x 實錘)

```
同一棵樹、同一顆 HEAD、什麼都沒改，只是重跑:
  第一層全樹    ⇒ **14 格紅 / 4 支檔**（1 支 admin + 3 支 storefront）
  把那 4 支【單獨重跑】 ⇒ **1 格紅**（13 格消失）
  再把剩那 1 格【單獨重跑】 ⇒ **0 格紅**（Tests 14 passed）
⇒ 🔴 **14 → 1 → 0。全部是假紅。**
而那唯一撐到第二輪的一格，訊息逐字是:
  `Error: Test timed out in 5000ms.`  ← **不是斷言失敗，是超時**
```
**成因(W5 同日獨立量到同款)**:七個窗同時在同一棵樹上跑測試 ⇒ **互相搶 CPU 與檔案**。
W5 那次:第一次 8 支紅(全 storefront)、第二次 1 支紅(admin),**兩批完全不重疊**。

### ⇒ 對收割窗的操作結論(比原本那條更硬)
```
❌ 不要拿第一層的【紅】去追 bug —— 先【單獨重跑那幾支】，多數會消失
❌ 更不要拿第一層的【綠】當保證 —— 🔴 **假綠比假紅貴**:
   某一次「全套 passed」可能只是那一刻剛好沒撞上
✅ 穩定的只有三格（W5 的話，我實測一致）:
   **typecheck / lint / 那一片自己的專屬測試檔**
⇒ 所以 §5e 第二層才是判準，而第一層的定位要再退一格:
   它不是「現況紀錄」而已，**它是一個已知會說謊的量具** —— 兩個方向都會。
```
**判別句**:第一層紅了 ⇒ **先問「單獨重跑還紅嗎?」**;
第一層綠了 ⇒ **不要問任何事,因為它答不出任何事。**

### 📎 而這個症狀有一個正確的名字 —— **用錯詞會去修一個沒壞的東西**
```
docs/patterns/guard-and-instrument-traps.md
  「『負載紅』與『flaky』**不可以共用一個詞**」（W3 2026-08-20 量到，Sean 同時獨立講出來）
🔴 處置完全相反:
  flaky（測試本身不穩）⇒ **去修測試**
  負載紅（機器忙、跑不完）⇒ **去修排程，不要碰測試**
```
**本節上面那組 14 → 1 → 0 是【負載紅】**(唯一撐到第二輪那格的訊息逐字 = `Test timed out in 5000ms`)。
⇒ 上面「已知會說謊的量具」講的是**症狀**,那條講的是**成因** —— **兩條要一起讀。**
⚠️ **而它有一個已經量到的代價**(那條條目內):有人在收到一個**真的**紅時,
第一反應是「又是負載紅嗎」——**負載製造的假紅,已經開始侵蝕讀【真紅】的能力。**

### 🔴🔴 **執行率實測:12 顆裡 4 顆 —— 我自己立的規矩,我自己執行了 33%**(2026-08-20 03:2x 自檢)

```
分母:§5e 立法（b69dbcbe，01:49）之後、由收割窗做的 merge commit ⇒ **12 顆**
量法（機械，可重跑）:
  for h in $(git log --format='%h' b69dbcbe..dev --merges); do
    git log -1 --format='%B' $h | grep -c '第一層' ...
逐顆結果:
  ✅ 兩層都寫的 = **4 顆**（8b380d6c / fd4d1585 / 9080752a / 8d3b1c52）
  🔴 完全沒寫的 = **8 顆**
  ✅ 連【髒檔數 + 樹的路徑】都寫的 = **1 顆**（fd4d1585）← 那是後來才加的要求
```
🔴 **而八顆缺的那些,不是「規矩立法之前」的** —— 立法在 01:49,而缺的八顆有六顆在那之後。
**我在立法之後,照舊寫了六顆不符合自己規矩的 commit body。**

#### 而處置是【不回頭補寫】
```
🔴 回頭補寫那八顆的 commit body ⇒ **會把一個真的漏執行，變成一個假的紀錄。**
✅ 正確的處置是把這件事本身寫下來（就是這一節）。
⇒ 而它同時是這條規矩【第一次被驗證有沒有被執行】的紀錄。
```

#### 📌 而這一格的通則,比那個數字有用
```
今晚立的規矩大多【只被執行過一次】——就是立它的那一顆 commit。
🔴 而「寫下規則」與「執行規則」之間沒有機制:
...
### 📌 而它有**反方向**的一次(2026-08-20 03:4x,同一晚)
```
preflight ⇒ **rc=4**「工作樹有未提交改動，正好是這次 merge 要動的檔」
而我照紀律停下來核 ⇒ `git status --porcelain <該檔>` **回空**
   `git diff --name-only` 回空 ／ `git diff --cached` 回空 ⇒ **三把尺都說乾淨**
⇒ 重跑 preflight ⇒ **rc=0** ⇒ merge 成功
🔴 成因:別的窗在那幾秒之間**把它 commit 掉了**。
```
⇒ **同一個機制、兩個方向**:
```
前一例:preflight 說乾淨 ⇒ merge 時已經髒了   （假綠）
本 例:preflight 說髒   ⇒ 核的時候已經乾淨了 （假紅）
🔴 **兩種都不是尺不準，是【尺量的是過去】** —— 而共用工作樹讓「過去」只有幾秒。
```
✅ **而處置一樣**:**核,不要相信單一時點的結論**。
⚠️ 但**核出來是乾淨也不能直接放行** —— 要**重跑一次 preflight**,而不是拿「我核過了」當通過。
**理由**:我核的是**那一支檔**,而 preflight 看的是**整個索引與工作樹** —— **範圍不同。**
...
🔴 **這句綁了一個【當時的待辦狀態】,而它現在查得出來** ——
不要相信這句,**當場查**:
```bash
# 哪些工作樹沒有 .husky/_（= 那棵樹上的 commit 沒有經過任何 pre-commit 閘）
git worktree list | awk '{print $1}' | while read t; do
  [ -d "$t/.husky/_" ] && echo "有閘  $t" || echo "🔴 無閘 $t"
done
```
#### ✅ 而我當場跑了一次(2026-08-20 04:5x)—— **押了一整晚的那件事,現在有答案**
```
總數 29 棵 ／ 有閘 25 ／ 🔴 **無閘 4**:
  /Users/sean_1/pcm-w2-dialog
  /Users/sean_1/pcm-w3-lit
  /Users/sean_1/pcm-w3-probe2
  /Users/sean_1/pcm-w3-traps
```
⚠️ **而「無閘」不等於「那棵樹上有裸 commit」** —— 兩個問題:
```
① 那棵樹**現在**沒有閘（上面量到的）
② 那棵樹上**有沒有 commit 是在無閘狀態下做的**（🔴 **這個沒量**）
⇒ 要答②得逐棵看它的 commit 時點 vs `.husky/_` 的生成時點，
  而 `.husky/_` 被 gitignore ⇒ **它的生成時點不在 git 裡** ⇒ 只有 mtime，而 mtime 會被後來的 install 覆寫
⇒ ⇒ **②可能已經永久答不出來了。** 這一格照實寫，不假裝它待辦。
```
🔴 **可行動的那一半**:那 4 棵**現在**就補得起來 —— `cd <樹> && pnpm install`
⇒ **而那要那幾棵的擁有者自己做**(收割窗不動別人的工作樹)。
...
## 5f. 📋 **本班自抓欄位**(2026-08-20 主視窗立;**艦隊慣例,不是規則**)

> 每個窗的 STOP／交件檔固定一行:
> **「本班我自己抓到的:N 件」**(各一句:**我的什麼壞了、我怎麼發現的、我重量了沒**)
>
> 🔴 **為什麼要有這一欄**:自查成功的樣態是**沒有 finding** ——
> 它永遠不會出現在任何 finding 清單裡 ⇒ **任何自評的自查率都系統性偏低。**
> ⇒ 修法不是「更常自查」,是**讓自查有一個落點**。

### 本班(2026-08-19 夜 ～ 08-20 晨)= **6 件**
```
1. 「未推 33」憑心算報，實際 32          ⇒ 重量後更正（下次每封 date + 重量）
2. 信上打了一個【沒量過的時間戳】(01:0x)  ⇒ 實際 00:52 ⇒ 改成每封發出前 date
3. 「我沒有正式庫 access」——**沒試就宣稱** ⇒ 實查通道在，是權限閘擋下 ⇒ 已落 traps
4. 只解了【第一塊】衝突就要 commit        ⇒ git 擋下 + 我自己 grep 才發現 ⇒ 立「先拿基線」紀律
5. 量 charge 有沒有逾時得到 3，而**對照組也是 3** ⇒ 尺無判別力
   ⇒ 逐行打開:3 個全在註解裡 ⇒ 結論仍成立，而**支持它的數字曾經是錯的**
6. 固定 60 行視窗掃長方法 ⇒ refund 的 signal 在第 85 行
   ⇒ **回了一個乾淨的 0，而事實是有**
```
🔴 **而 5 與 6 都留在 `W4-073` 檔裡沒有改掉數字** —— 讓下一個人看得到那個病長什麼樣。

### ⚠️ 這一欄的已知弱點(**照實記,不是待補**)
```
① **沒有觀察者** ⇒ 每 milestone 收尾要有人掃一次，**全 0 的那一班要被問一句**
② **可能長成交差欄位** ⇒ 判別法:**看有沒有人在那一欄寫過【對自己不利】的東西**
🔴 位階:這是【艦隊慣例】，**不是規則** —— 沒有動 CLAUDE.md / 00-work-rules
   （那兩份要 Sean 拍板）⇒ **它的存續只到這一班，除非之後被收編。**
```
...
> 🔴🔴 **本節刻意【不寫任何數字、不寫任何人名】** ——
> 舊版寫的是 ~~「未推 39」~~ ~~「下一批來源:W1 正在改的訂單面板」~~
> (**這兩句是【被推翻的原文】,引在這裡當理由 —— 不是現況,不要照它做事**),
> 而它們在寫下後幾小時內就全部過期了,**而過期時沒有任何訊號**。
> ⇒ **本節只給【當場跑得出答案】的命令。**
> ⚠️ 而本節**沒有時點** —— 那是刻意的:它只給命令,而命令沒有時效;
> **留一個時點會讓人以為那些命令也有時效。**
...
# ① 有沒有東西可以收（領先本機 dev 的分支）
for b in $(git for-each-ref --format='%(refname:short)' refs/heads/); do
  n=$(git rev-list --count dev..$b 2>/dev/null); [ "$n" != "0" ] && printf "%-32s %s\n" "$b" "$n"
done

# ② 已收進本機、但還沒推的有幾顆（🔴 這與①是【兩個不同的問題】，見 §5 開頭）
git rev-list --count origin/dev..dev

# ③ 各分支落後多遠（🔴 落後【本機 dev】與落後【origin】要分開看，見 §5 第 0 步）
for b in $(git for-each-ref --format='%(refname:short)' refs/heads/); do
  ah=$(git rev-list --count dev..$b 2>/dev/null)
  bd=$(git rev-list --count $b..dev 2>/dev/null)
  bo=$(git rev-list --count $b..origin/dev 2>/dev/null)
  printf '%-24s ahead=%-5s behind_dev=%-6s behind_origin=%s\n' "$b" "$ah" "$bd" "$bo"
done

# ④ 這棵樹現在有多髒（第一層四綠的效力取決於它，見 §5e）
git rev-parse --show-toplevel; git status --porcelain | wc -l
```

### 而這四個數答不出來的三件事 —— **要問人,不要猜**
```
· **那些分支背後的窗還活著嗎** ⇒ 「窗在做事」與「窗停了」在收割窗這端
  是【同一個訊號:什麼都沒有】⇒ 分不出來就問主視窗，不要自己填一個解釋
· **推的時機** ⇒ 收割窗永遠不 push（§5 紀律）；那是 Sean 的動作
· **哪些片欠著審查關** ⇒ 掃 commit body 自陳撈得到一部分，
  🔴 而它只撈得到【自己承認的】⇒ 那個數字永遠是【下界】，不是總數
```

### 🔴 押著的東西怎麼查(**不要照抄清單,清單會過期**)
```
· 未收割而押著的分支 ⇒ §2（附退役條件；退役 ≠ 刪分支）
· 推之前要讓 Sean 知道的 ⇒ `~/pcm-mailbox/W4-072-推送清單-20260820.md`
  🔴 而那份自己也會過期 ⇒ 它的檔頭寫了怎麼當場重量
· 未 apply 的 migration ⇒ `git diff --name-only origin/dev..dev | grep '^supabase/migrations/'`
  🔴 **push 不會 apply 它** ⇒ 有命中就是【程式先上、schema 後上】，
    而 apply 要 Sean 在場。**這一格每次推之前都要重跑，不要相信上一次的結果。**
```


### 📎 下一輪收割的來源 —— **🔴 以下這一節已經過期,留著是為了示範它怎麼壞的**

```
🔴 本節寫於 MAIN-066 剛落檔時，而那八條**當晚多數已經被做掉並收進 dev**
   （面板捲進視野、拒因文案指位置、客戶頁篩選、廠牌複選、貼料號…）
⇒ ⇒ 而它仍然寫著「下一輪的來源是這八條」
```
**⇒ 這一節的正確讀法**:
```
❌ 不要把它當【待辦清單】—— 它是一份【某個時點的快照】
✅ 要知道「現在還剩什麼」，只有一條路:**§6 那四個命令，當場跑**
🔴 而它為什麼沒有被更新:**它在 §6 的【後面】** ——
   我重寫 §6 時核的是「§6 這一節」，而它住在下一節
   ⇒ 📎 這正是本檔那條「**我剛剛掃的是這份檔的哪一部分?**」的第四例，
     **而這次是我在寫那條的同一天、同一份檔裡犯的。**
```
⚠️ **保留原文不刪** —— 刪掉之後,下一個人看不到「一份交接檔的最後一節會怎麼腐爛」。

---

#### (以下為原文,已過期)`MAIN-066` Sean 肉眼驗收第二輪(八條)
