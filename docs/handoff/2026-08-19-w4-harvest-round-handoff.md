...
**從來沒有量過 `branch..origin/dev`(落後)** —— 兩個方向都要。

```bash
# 0. 領先與落後【一起量】—— 落後那一欄是新加的
printf '%-22s %8s %8s
' 分支 領先dev 落後origin
for b in $(git for-each-ref --format='%(refname:short)' refs/heads/); do
  ah=$(git rev-list --count dev..$b 2>/dev/null)
  be=$(git rev-list --count $b..origin/dev 2>/dev/null)
  printf '%-22s %8s %8s
' "$b" "${ah:-?}" "${be:-?}"
done
```
**首次實測(2026-08-20 03:5x,本樹)** —— **活躍的窗全部落後**:
```
w1-order-panel   落後 **68**      w5-sku-filter    落後 **85**
w3-customer-sort 落後 **49**      w2-email-cron    落後 **267**
（而它們的「領先」欄多半是 0 ⇒ **只看領先的話，這些全部長得像「沒事」**）
```
