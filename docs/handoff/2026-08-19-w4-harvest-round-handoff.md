...
· 每批 merge 完跑四綠 `TURBO_FORCE=1` —— **而「四綠」對收割窗有專屬定義,見 §5e,不要照一般片的讀法**
· 🔴 **收割窗永遠不 push** —— 那道關的價值是「一雙沒寫過這段 code 的眼睛」，
  而收割窗與寫的窗都是 AI ⇒ 少了 Sean 那一下，關卡只是換一個 AI 蓋章
· 🔴 **不碰 `STATUS.md`** —— 收帳權在主視窗
· 🔴🔴 **`guard-commit-pathspec` 有一個看不到的方向**(2026-08-20 W5 挖到,寫成【閘的射程】不是【誰做錯了】)
```
那道閘防的是:我 commit 時帶走【別人 staged 的檔】
而反方向是:**我的檔被放進【別人正在進行的 merge】**
⇒ 閘比對的是「staged 有沒有超出我的快照」——
  別人的檔在【它自己的快照】裡 ⇒ **閘會放行**
⇒ 而這個方向【只在有 merge 正在進行時】可能發生
```
**判別句**:**我 `git add` 之前,有沒有看一眼 `.git/MERGE_HEAD` 在不在?**
⚠️ **而收割窗是最常製造這個窗口的角色** —— merge 中途去解衝突的那幾分鐘,
整個共用索引對別人是「看起來正常、而 add 進來會被我一起 commit」。
✅ **實例的處置(W5 做對的)**:先量「那顆 merge 有沒有碰到我的檔」
(`git diff --name-only HEAD MERGE_HEAD -- <自己的路徑>` ⇒ 0 命中),再 `git restore --staged` **只點名自己的路徑**。
✅ **而收割窗這一側的自核**:commit 之後跑 `git show --name-only <那顆>`,確認檔案清單只有預期的那些。
🔴 **這一格不能靠對方通知** —— 通知是好意,而**自核是機制**。
```

---

## 5e. 🔴🔴 收割窗的四綠 = **兩層**(2026-08-20 主視窗裁准;定義原封抄下,不要重新發明)

```
收割窗的四綠 = 兩層，兩層都要報，而【判準只有第二層】

第一層｜全樹現況（記錄用，不是判準）
  TURBO_FORCE=1 全套跑一次
  🔴 報的時候必附 git status --porcelain | wc -l **並附這棵樹的路徑**
    非 0 ⇒ 受測對象含別人未提交的東西 ⇒ **這一層的紅不歸收割窗**
    ⚠️ **路徑不可省**（W3 2026-08-20 實測提醒）:多棵工作樹同時在跑，
       髒檔數【很容易撞成同一個數字】—— 它量到自己那棵也是 9，與主樹的 9 是不同的 9。
       ⇒ 一個沒有出處的「9」，下一個人會把兩棵樹讀成同一件事。
  ⇒ 用途:留下「收割當下這棵樹長什麼樣」的紀錄，給下一個人對時間軸

第二層｜只跑本次收進來的那些檔（這才是判準）
  列出檔名（不是只報數量）+ Test Files / Tests 的實數 + rc
  ⇒ 綠 ⇒ 收割窗的責任範圍內無誤
  ⇒ 紅 ⇒ 這才是收割窗要處理的

🔴 而兩層都要寫進 commit body，不能只寫第二層 ——
  只寫第二層會讓「我收的東西是綠的」讀起來像「樹是綠的」
```

### 為什麼要有這條(理由是量出來的,不是想出來的)
```
2026-08-20 一夜【連續三輪】拿不到全樹全綠，而每一輪紅的都是【別人未提交的東西】:
  第一輪 note-compose-form.test.tsx      ← W2 的 WIP（後來它自己修掉）
  第二輪 note-compose-form 換成別的       ← 同一批 WIP 的新版本
  第三輪 refund-remaining-single-source  ← 一支【untracked、三分鐘前才寫出來】的 migration 觸發的守門
⇒ 在共用工作樹上，「全樹四綠」對收割窗而言**幾乎永遠拿不到**
⇒ 🔴 而**一個永遠拿不到的判準，實際效果是逼每個人自己發明說法** ——
  那比沒有標準更糟:每次的說法都不一樣，而讀的人無法比較。
```
📎 **原理面在** `docs/patterns/guard-and-instrument-traps.md:11196`
「多窗共用工作樹之下,『跑一次全套測試』**沒有一個確定的受測對象**」(W2/W4)。
**本節是操作面,那節是原理面,兩份互相引用。**

### ⚠️ 而第二層有一個容易踩的坑(2026-08-20 實錘兩次)
```
只跑幾支檔時，`rc≠0` 可能是【尺沒跑起來】而不是【測試紅】:
  npx vitest --reporter=basic  ⇒ vitest 4 載不起 ⇒ rc=1，零個測試
  在 apps/admin/ 底下用該目錄相對路徑 ⇒ No test files found ⇒ rc=1，Test Files=0
🔴 所以第二層規定要報【檔名 + Test Files/Tests 實數】，不是只報 rc ——
  「Tests 0」與「Tests 1 failed」是兩件事，而 rc 一模一樣。
```
📎 全文:`guard-and-instrument-traps.md`「別問它成功了嗎,問它做了幾件事」。
