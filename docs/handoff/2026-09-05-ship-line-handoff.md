...
>
> 🔴 **本檔的數字更新到 `agent/line-ship` HEAD = `a089565d9`(2026-09-05 下午)。**
> ⛔ ~~原寫 HEAD = `0b56b677d`~~ —— 留刪除線, 讓引用過那個 hash 的人同一發撞到這裡。
> 
> 🛑🛑 **而「未推 8」這個數【不含更新本檔的那一顆】** —— 寫下它的動作本身會讓它 +1。
> 📌 **一個寫進檔案的計數會數到它自己** ⇒ 所以本檔的規矩是:**數字只當【下限】讀。**
> ✅ **當場重數(唯一算數的做法)**:`git rev-list --count origin/dev..agent/line-ship`
> ⇒ 引用之前跑那一行, 不要抄本檔的數字。
...
### A. `agent/line-ship` —— ⛔ ~~**6 顆**~~ ⇒ **8 顆,可以推**
> 🔵 **主視窗第 17 批只收前 7 顆**(預告時 `a089565d9` 還不存在)⇒ 它排第 18 批。
> 📌 **「我合了那條分支」不等於「那條分支上每一顆都進來了」** —— 分支會繼續長,
>   而合的是**那一刻的分支頭**。⇒ 收割回核要用 `git cherry` 逐顆對, 不要數數量。
```
量法 git rev-list --count origin/dev..agent/line-ship  ⇒ **8**(不含更新本檔的那一顆)
a089565d9  docs(board): 同一個目錄同一個量法, 兩天差 36 倍 —— 而比值不是常數
afc354874  docs(handoff): 停著的事要寫【停在誰身上】(← 本檔自己)
...
3  本檔的 commit 清單是 HEAD=`a089565d9` 那一刻的。七個窗同時在寫 ⇒ 讀到時可能已經變了。
   🔴 **而本檔【已經自己過期過一次】** —— 第一版釘 `0b56b677d`, 而寫完它就變成 `afc354874`。
   ⇒ 📌 那不是疏忽, 是**結構性的**:交接檔釘 HEAD, 而寫交接檔會產生新的 HEAD。
   ⇒ ⇒ **所以本檔的每一個計數都只當【下限】讀, 要真值就跑 §一A 那一行。**
