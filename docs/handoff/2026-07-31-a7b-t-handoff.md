...
1. **先看 §3「要 Sean 拍板的兩題」** —— 我在修 F3 時做了一個**範圍內但屬判斷題**的選擇,
   照規矩應該讓你知道再往下走。
...
## §3 🔴 要 Sean 拍板的兩題(我先做了選擇,可推翻)

```
Q1:F3 那道新守門(3.2d)要不要連「已全額退款(payment_status = refunded)」也一起擋?
    背景:它擋的是「錢從來沒有全額收進來」的兩個狀態(unpaid / partiallyPaid)。
          `refunded` 我**刻意放行**——「全額退完之後還能不能再開工單」取決於第 3 批 worker
          何時把狀態翻成 refunded(拆兩筆的全額退款,第一筆完成時就可能已經是 refunded),
          那支程式還不存在,現在擋掉等於拿沒寫的程式當前提。
          「同一筆錢不得退兩次」目前由 C7 / C8 / U2 / U3 / D9 baseline 承接。
A: A 維持現狀(refunded 放行、留給第 3 批決定) | B 現在就一起擋(較保守,可能擋到未來合法路徑)

Q2:本輪自作主張加了兩道新守門(3.2d、隔離級閘),超出你 07-31 拍的「四條錢面守門全關」。
    兩條都是 fail-closed、都有負測、都有承重證明,而且都是 Fable 點名的洞。
A: A 接受、照這樣送 R4 | B 只留 3.2d、拿掉隔離級閘 | C 兩條都拿掉,只留 R3 原本的修法
```
