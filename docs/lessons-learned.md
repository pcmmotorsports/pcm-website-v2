...
**操作化(2026-08-22 加,主視窗落):要新增 `docs/patterns/guard-and-instrument-traps.md` 條目前,先跑 `python3 scripts/traps-neighbours.py <草稿>`,並把它印出來的 8 條【原樣貼進投稿】。**
- ❌ 「已數過副本 ✅」 —— 做對的 / 掃錯詞的 / 沒做的,三種人寫出來一模一樣。
- ✅ 「撈到的 8 條:<原樣貼>;其中 `:NNNN` 最近,而本條與它的差別是 X」。
- ⚠️ **那支工具是【候選】不是【判定】**:實測 top-5 命中 2/7(2026-08-22,分母=當夜七次重新發現)⇒ 它撈不到不代表沒有,**「差別是 X」那句才是被審查的東西**。
- 🔴 **只貼表不算。** 要貼輸出 **+ 寫出你開了哪幾條、為什麼判它不同族** —— **擋下來的是【開檔讀】那一步,不是排名。**
  實錘(2026-08-22 同夜):真正撞車的那一條被工具排**第 5 名、分數比第一名低快三倍** ⇒ **只看分數會跳過它**;把它擋下來的是有人把前幾名開檔讀了。
- 🔴🔴 **那支工具是【候選產生器】,不是【查重器】。跑完它之後,還要針對你這條的【機制關鍵字】自己再 grep 一次。**
  實錘(2026-08-22,工具上線一小時內):一條「我加的無障礙文字把我自己的視覺守門變成恆真的」——
  **它的近親在 top-8 裡完全沒出現**,是作者自己 `grep '^## ' <canonical> | grep '恆真\|恆綠'` 撈到的,
  而那一族當時 canonical 已有 **6 條**。**連兇手都一樣**(`textContent` 吞掉巢狀元素的字 ⇒ `toContain` 系斷言失去判別力)。
  ⇒ 工具自己就警告過「真正撞車的那條可能被排到後面」—— **這次它說對了自己。**
  🔴 **同日第二個實例把失效模式補成兩種**:①**排名低**(真正撞車的排第 5 名、分數低快三倍)
  ②**完全不在名單上** —— 一條條目與它的近親用的是兩套詞彙(「不給/渲染/員工/沉默」vs
  「守門/斷言/靜默出口/當事人」),**字元重疊極低 ⇒ 最高分只有 0.1311,而那還是一條無關的**。
  **擋下它的是審查者手動去解析錨點的母節,不是工具。**
  ⇒ 落點清單一律要寫明:**「沒有近親」的分母 = 工具 top-8 + 人工開的 1-2 條。已審 ≠ 已窮舉。**
- 🔴 **而「跑過工具、沒撞到」曾經兩個方向都不成立**:一邊是排名太低被跳過,一邊是**工具的母體少了 inbox 那批**(最近五天的產出,也就是最可能撞車的那一批)。母體已修(正本 + inbox 當場切,輸出標來源),**而這一格提醒你:下次也要問「它的母體是什麼」。**
- **立這一格的理由是量到的**:2026-08-22 一夜五個窗七次「我發現了一件新事」,七次都是那份檔裡早就寫過的;每一次那個人都做了字面 grep,每一次都回 0,**連審查的人自己也犯了一次**。⇒ 不是七次疏忽,是同一個結構問題。
...
### 12-41. 「這片做完沒」的判別法要綁 commit / 成品座標,不要綁「我預期它會長成什麼樣」

**事故脈絡:**
後台完成地圖 §-6(E 窗六代、2026-08-11 夜)結論「退款線只剩 `RF7` 沒落地」,並列了**三條互相獨立且一致**的證據:①`settle-charge.ts` 的 `case 2/3` 仍回 `refund_anomaly` ②`grep -c order_refunds …/settle-charge.ts` = 0 ③該檔註解自稱「2/3 **仍**走 `refund_anomaly`」。六代甚至在誠實界 §-6.5-2 主動留了一條**證偽方法**:「若它在別處被實作成另一個形狀,這格就是錯的 —— 判別法:`grep -rn order_refunds packages/use-cases`」。`RF7` 在**同日**落地(`3e542afc`)。七代 08-12 把四條逐條照原命令重跑:**四條字面全部仍真,結論已經是假的**。原因是四條共用同一個假設 ——「修法一定會讓 `settle-charge.ts` 去讀 `order_refunds` 帳本」;而實際修法落在**消費端**(`settle-charge.ts:183`)、commit 主旨逐字「**裁決不變**」,classifier 與帳本兩處都沒動。⇒ 那不是四條獨立證據,是**同一條假設的四個投影**;連作者自己準備的證偽測試也共用它,因此一起失效、而且**失效時完全不報錯**。與完成地圖 §-7.3「`grep 片號` 對『這片做完沒』零判別力」是同一個病的另一面:**那邊命中太寬(撈到別條線的片),這邊命中太窄(只認一種修法形狀)**。

**規則:**
- 寫「X 未實作 / 未落地 / 未覆蓋」的斷言時,證據要綁**不隨修法形狀改變的東西**:該片的 commit(`git log --grep` / `--stat`)、成品座標、或該片自己的驗收條件;**不得只綁「我預期它會改哪一行、會出現哪個識別字」**
- 一組證據若全部共用同一個「修法會長成什麼樣」的假設,**它們的證據數是 1 不是 N** —— 落筆前逐條標出各自假設,假設相同就併成一條
- **自備的證偽方法要另外過這一關**:它最常見的失效模式正是與正面證據共用同一個假設(本條事故即如此)
- 判準一句話:問「**如果它用另一種形狀做完了,我這條會不會照樣說『沒做』?**」——會 ⇒ 這條沒有判別力
- 盤點型產出(完成地圖 / 現況表 / 覆蓋率清單)每格至少要有**一個綁 commit 或成品座標**的證據欄,純 grep 命中不足以單獨支撐「未做」
- **enforce:** review 見到「N 條獨立證據一致」的宣稱,要求逐條標假設;標不出來或假設相同 = must-fix(§13 偵察類斷言同紀律)

**規範定位:** 對齊 memory `feedback_assertion-measures-the-wrong-thing`(量錯東西族,本條是它在「盤點斷言」域的形狀)+ `feedback_negative-test-observation-supplied-by-another-mechanism`(多症狀不等於多證據,本條是它的鏡像:多證據共用一個假設)+ §13 偵察 slice 方法論(凡結論將寫「X 未實作」)+ 完成地圖 §-7.3(命中太寬的另一面)+ 鐵則 10 三視角之「bug 可追蹤性」。🔴 **機制優先律(2026-07-22)那關的答案**:此病**做不成機制** —— 它不是 code 缺陷而是 docs 斷言與其推論之間的落差,grep 本身沒有錯、hook 也看不到「這條證據的假設是什麼」;⇒ 先立文字條,把「綁 commit / 成品座標」變成寫判別法時的檢查項,**機制留待下次踩到同族時再評**(主視窗 `E-338-A` §2 裁定)。

**教訓來源:** 完成地圖 §-6(E 窗六代 2026-08-11 夜,樹 `c8589a33`)vs §-8.1(E 窗七代 2026-08-12,commit `2e36d9c2`);`RF7` 落地於 `3e542afc`、修法座標 `packages/use-cases/src/settle-charge.ts:183`

**跨專案適用:** 適用所有「盤點 / 完成度 / 現況地圖 / 覆蓋率」型產出,不限 PCM;報價單側的 fetcher 覆蓋盤點、供應商上架清單同紀律

**首例:** 完成地圖 §-6 → §-8.1 翻面(`2e36d9c2`)為事故源;本立法 commit 即條文 anchor

### 12-42. 一份檔的「完備感」與它的「涵蓋範圍」是兩件事 —— 完備感會讓人停止追問範圍

**事故脈絡:**
`#20` 欄位歸屬盤點(B 窗、2026-08-15)要回答「哪些欄位員工改了會被同步蓋掉」。其中 `highlights` 的保護只有一層 —— 供應商級開關 `syncDescription`,沒有 per-row 第二層 ⇒ **該家開著,就是每天無條件覆寫**。要下結論就得知道「哪幾家開著」。`scripts/supplier-config.ts` 逐家列著 `syncDescription: true/false`,而且**有型別、有逐條註解、每家還附抓取統計數字**(如 `3,435/3,460`)—— 它看起來就是答案該在的地方,查到那裡很容易就停下來。**但那個檔答不了這題**:一家「設定為 true」不等於「每天真的會跑」——**是否排進每日 cron 在 `.github/workflows/rpm-sync.yml` 的一行 `matrix.supplier` 陣列裡**。只讀 config 會把 `extreme`(true 但為靜態一次性 fixture、刻意不排每日)算進去,得到 14;兩份檔交叉才得到真正每天會被覆寫的 **13**(每日排程 14 家中,`rpm` 為 false)。同一天同一條線還有第二次同形狀:判「無條件覆寫」時只做了型別層(介面欄位無 `?` ⇒ TypeScript 保證必進 payload),而**沒查載入層有沒有把 key 再拿掉**(`stripColumnIfMissing`,`rpm-import.ts:515`)—— 型別層是**真的**保證,但它保證的是「payload 裡有沒有這個 key」,不是「這個 key 有沒有活著送到 DB」。⇒ 兩次都不是「忘了查」,是**在一個看起來夠格的來源前面停住了**。

**規則:** 🔴 **全文在 memory `feedback_completeness-of-a-file-is-not-its-coverage`(一處全文、跨專案),本條不複述。**
判別句:**「這份檔的作者,有沒有理由知道我現在要問的這件事?」**——沒有 ⇒ 它再完備也答不了,去找那個有理由知道的檔。
- **本條(PCM 域)另立的兩件,memory 沒有**:
  - **enforce:** review 見到「N 家 / N 個會 …」這類數字,要求指出**它是從幾份檔交叉出來的**;只有一份 ⇒ 追問另一半住在哪裡,答不出來 = must-fix
  - **留痕體例**:劃掉過期字面時要分「**當初就錯**」與「**當初對、現在過期**」兩種標記。混成一種 ⇒ 所有被劃掉的東西看起來都像錯誤 ⇒ **而那會讓人不敢留痕**(2026-08-15 `#511` 那一行的處理即首例)

**規範定位:** 對齊 memory `feedback_claim-scope-exceeds-fact-three-shapes`(宣稱>事實族;本條是它的「來源選錯」形狀 —— 不是掃得不夠廣,是**問錯了檔**)+ `feedback_assert-scope-only-after-reading-source-file`(報事實前開原始檔;本條補的是「開了原始檔還不夠,要開**對的**那份」)+ §12-41(證據共用同一假設;本條是共用「答案在這個檔裡」這個假設)+ 鐵則 10 三視角之「bug 可追蹤性」。🔴 **機制優先律(2026-07-22)那關的答案**:**部分做得成、通用做不成**。已有的狹義機制是 `scripts/literal-sweep.sh`(強制跨全樹掃字面、並逐類報分母)—— 它能擋「掃得不夠廣」,**擋不住「問錯檔」**:那一步是判斷,而不同結論要交叉的檔對不同題目都不一樣,寫不成通用規則。⇒ 先立文字條,把「這份檔的作者有沒有理由知道」變成引用單一來源時的檢查項;**若日後同族再踩,再評估能不能對特定高頻題(如「哪幾家每日同步」)做成具名守門**。

**教訓來源:** `#20` 欄位歸屬盤點(B 窗 2026-08-15);`docs/specs/2026-08-15-product-field-ownership.md` §0(兩步觀測量法)與 §1a(13/14 分群與交叉量法);`scripts/supplier-config.ts` vs `.github/workflows/rpm-sync.yml` 的 `matrix.supplier`;第二次同形狀 = `rpm-import.ts:515` 的 `stripColumnIfMissing`

**跨專案適用:** 適用所有「設定檔 + 排程/呼叫端」分離的系統,不限 PCM;報價單側 fetcher 的「哪幾家啟用」同樣分散在 config 與 launchd/cron 兩處,同紀律

**首例:** `highlights` 13/14 那次(`e09d4cc4`)為事故源;本立法 commit 即條文 anchor

### 12-43. 「全樹」在有未收割分支的世界裡是不精確的 —— `git ls-files` 是【dev 的全樹】

**事故脈絡:**
2026-08-17,主視窗把一條出處路徑轉給 I 窗當依賴表的引用來源。I 窗落檔**之後**才查,
`ls` 與 `git ls-files` 皆零命中 ⇒ 寫下「**全樹不存在、正本位置未確認**」(有附量法與正向對照)。
🔴 **而那個檔一直都在** —— `docs/security/2026-08-17-supabase-recovery-throttle-granularity.md`
**當時**在 `customers` 分支上、**尚未進 dev**;`git ls-tree -r --name-only customers -- docs/security` 一跑就看得到。
⚠️ **今天重跑會看到它【已經在 dev 上】** —— 它於 2026-08-17 午後隨 `customers` 收割進來。
🔴 這一句刻意寫成過去式並附「現在會看到什麼」:**一條教訓的事實錨點若被第一個查證的人打臉,教訓就失效**
(R2 code-reviewer C8 抓到的,原句寫「從沒進過 dev」是現在式、已經是假的)。
⇒ **兩邊的量法都對,錯的是分母**:`git ls-files` / `test -e` 看的是**當前 checkout**,不是整個 repo。

**規則:** 🔴 **要下「查無 / 不存在 / 零命中」而對象是【一個檔案路徑】時,跑 `bash scripts/where-is.sh <path>`。**
它把四處分開報(工作樹 / dev / 所有分支 / 信箱)⇒ **一眼看得出是【哪一種查無】**。
- **`test -e` 失敗時的下一問是「它在別的分支上嗎」,不是「它不存在」。**
- **轉別人的 `檔案:路徑` 時,若那個檔還沒收割,一律寫成 `<branch>:<path>`。**
  🔴 主視窗自陳它那一半:它知道 E 窗在 `customers` 工作樹上,**而它沒有把分支寫進去。**

**規範定位:** 與 memory `feedback_absence-read-as-verified` 同族(「什麼都沒有」被讀成「檢查過了」),
而本條是它的**分母形狀**:不是沒檢查,是**檢查的範圍比宣稱窄**。
📎 同一天同一個人踩了兩次同族:另一次是比對 `docs/patterns/` 索引時,pattern **只認表格列**,
而同一份索引裡有兩種格式 ⇒ 差集裡有一個是自己的量法造出來的(`stalled-line-triage.md` 假陽性)。
⇒ **兩次都是「掃描字集比宣稱窄」** —— 一次窄在「只認表格列」,一次窄在「只認當前 checkout」。

**教訓來源:** 2026-08-17 I 窗依賴表落 `Minimum interval per user` 那一列;
更正經過在 commit `26ffc79e`(標未確認)與主視窗當日的 `git ls-tree` 實查。
🔴 **處置刻意不是「把路徑刪掉」**:下一個人可能也拿到同一個路徑(它在多封信裡)⇒
**刪掉他會再查一次;留著並標明,他一眼就知道已經被查過了。**
並寫了「不要照這個路徑去找,會找不到而以為自己搞錯」—— **路徑錯的時候,找不到的人第一反應是懷疑自己。**

**跨專案適用:** 適用任何有長命 feature branch / 多工作樹的 repo。
單一分支、每次都 rebase 到主線的專案不會踩到這一條。

**首例:** `26ffc79e`(標未確認)→ 本條與 `scripts/where-is.sh` 為修法 anchor

---

### 12-44. commit 包漏收一支檔有【三個出口】,而只有一個會紅 —— 驗包要比對【格數】不只 exit code

**事故脈絡:**
2026-08-24 夜跑,下手窗(協調窗 2)為四條線出 commit 包,一夜內三種「漏收」各撞到一次:
(1)**漏收一支【別人編譯時需要】的檔** ⇒ 乾淨樹 `typecheck` 紅並指名(`TS2307 Cannot find module`),
會吵 ⇒ 有人會發現;(2)🔴 **漏收一支【帶守門的】檔** ⇒ **`exit=0` 綠,而 `Tests` 從 177 掉到 165**
—— 12 格守門安靜消失,**不吵** ⇒ 沒有人會發現;(3)🔴 **被別條線的 commit 先收走了** ⇒
`git status` 少一行、**沒有任何訊號**(該支 `SupabaseOrderAdapter.ts` 原本要 `git add -p` 收三個 hunk,
而在作者寫下那句指示之後、協調窗讀到之前,它已被 `2e276a10` 一起收走)。
⚠️ 而同夜寫成的 `scripts/commit-pack-preflight.sh`(乾淨 worktree + 只放包內檔 + `typecheck`/`lint`/**`build`**/**包內測試檔跑一次 `vitest`**;~~原句只列 typecheck/lint~~ ⇒ 2026-08-24 起已含後兩項,審查點名括號過期)
**只抓得到第 (1) 種**;誤擋率乾跑實測:該放 1/1 通過(誤紅 0)、該紅 **1/2**(漏抓 1)——
而漏抓那一格正是提案人與協調窗**共同預測會紅**的示範靶(成因:問題與修正住在同一支檔,抽掉 = 兩個一起消失)。
🔴🔴 **第四個出口(同夜稍後,協調窗自己犯的):【我列 pathspec 的分母比這一包窄】** ——
它用 `git status --porcelain apps/admin/src/components/orders/ apps/admin/src/app/orders/` 撈一個
**跨目錄**的包,而漏掉的那支在 `apps/admin/src/app/print/orders/[id]/picking/`。
作者交件檔的 porcelain 清單**第一行就是它** ⇒ **清單對、路徑對,錯在收包那步的 pathspec。**
⚠️ 而該包的【格數比對】沒有抓到它 —— **因為那把尺比的也是同樣那兩個目錄**:
🔴 **一把尺與一個分母,若來自同一份錯的清單,它會自洽。**
⇒ 後果:`fa2e019e` 落地後 HEAD 上一支測試檔**必紅**(它仍讀舊檔,而字面已隨標頭搬走),
由作者在寫交接檔時當場量到(`grep -cF` 三發、兩個世界都給)。

📌 該腳本檔頭**從一開始就寫著**「抓不到不影響編譯的漏收 ⇒ 症狀是守門悄悄不見,不是紅」——
**它標對了,而要有人真的量一發才知道那句話有多重。**

**規則:** 🔴 **出 / 收 commit 包時,除了 `exit code` 必須比對【格數】。**
- **基準** = 主樹上跑該片相關測試的 `Test Files` / `Tests` 兩欄;
  **乾淨樹上只放這一包**跑同一批路徑 ⇒ **兩欄要相同**。格數變少 ⇒ 漏收了一支帶守門的檔。
- **乾淨樹的建法自帶兩道自檢**:建樹時 `git status --porcelain` 必須 **0 行**(髒基準量出來的綠沒有意義);
  複製後必須**剛好等於包內檔數**(多了代表搬到別線的東西 ⇒ 🔴 **它會綠,而綠在這裡是壞消息**)。
- **交件清單一律給【完整路徑】,收的人【不補目錄】** —— 補不出來就 `find` 或回問。
  (實例:收的人把 `app/orders/[id]/x.test.tsx` 補成 `components/orders/x.test.tsx`,
  `git status` 回「乾淨」⇒ 差點判成作者沒交。**補出來的座標與抄來的座標,在文件上長得一樣。**)
- **「收第 N 個 hunk」那種指示,有效期只到下一顆碰同一支檔的 commit,而過期無訊號** ⇒
  改寫成內容式檢查:「確認這三個字面在你要出的那棵樹裡各 1 命中」。
- 🔴 **pathspec 的來源必須是【作者交件檔那份 porcelain 原始輸出】,不是協調窗自己跑的目錄掃描。**
  自己跑一發是**複驗**(比對兩份是否相同),不是**產生**。
  **兩份不同 ⇒ 停下來問,不要選一份用。**
- **enforce:** 包裡沒有【主樹格數 vs 乾淨樹格數】兩欄 ⇒ 該包不出;
  只跑 `preflight` 綠就出包 = 違反(它是**必要不充分**,同夜實例:preflight 綠而 `vitest` 15 格紅)。

**規範定位:** 對齊 §12-43「掃描字集比宣稱窄」——本條是它在 **commit 包**這一層的形狀:
`preflight` 的射程比它讀起來的窄。並與 memory `feedback_a-missing-guard-file-goes-green-but-quieter`
(本條的第 2 出口)、`feedback_a-filter-silently-drops-what-you-listed`(過濾器安靜丟掉你列的目標)同族。
🔴 三者的共同判別點:**它們都通過了「我有沒有量」,而卡在「我量的是不是那件事」。**

**教訓來源:** 2026-08-24 夜跑五包 commit 包(`e299c5e0` / `f61da808` / `2abe5845` / `c287fc7c` / `fa2e019e`);
第 (2) 出口由補洞窗逐支退回檔案時量到(177 → 165);第 (3) 出口由協調窗查 `git diff --stat` 空的時發現;
誤擋率乾跑結果落檔 `~/pcm-mailbox/SUB2-019-preflight誤擋率乾跑-結果-20260824.md`。
🔴 **而 (3) 那次差點多出一顆沒必要的 commit** —— 擋住它的不是小心,是收訊端**沒有下結論**
(把 `0` 的兩個世界列出來、指名「只有你分得開」)**+ 附了負對照**。

**跨專案適用:** 適用任何**多人 / 多 agent 共用一棵工作樹**、而 commit 由單一人集中收的專案。
單人單樹、每次 `git add -A` 的專案不會踩到 (2)(3) 兩個出口。

**首例:** `SUB2-019`(誤擋率乾跑,該紅 1/2)與 `SUB2-021` / `SUB2-024`(首兩份含【格數比對】那一節的 commit 包)

### 12-45. 一把只認一種寫法的尺,對另一種寫法印的是「沒有」而不是「我不知道」 —— 守門要把自己的天花板寫在自己身上

**事故脈絡:**
2026-08-27 一天之內,**四個互不相干的窗各自撞到同一個形狀**,而四次都是靠「有人去餵一個新世界」才發現:

| 那把尺 | 它看不到的寫法 | 它對那個世界印什麼 |
|---|---|---|
| `git-isolation-denominator.py`(掃「有沒有人在別的樹跑 `git add -A`」) | Python tuple 形 `check_call(('git','commit',…))`、Node 分離參數形 `execFileSync('git',['add','-A'])` | **「沒有」**(不是「這種寫法我不會讀」) |
| `-ed` 的假 `mktemp` 注入器(要證明守門會擋) | Python 的 `tempfile.mkdtemp()` —— 它根本不呼叫 `mktemp` | **「攔到了」**(而它從來沒被呼叫過) |
| 早上用關鍵字判「這支有沒有寫限制」的存量掃描 | `scripts/452-verify.sh` 用的是別的字 | **「沒寫」** |
| 🔴 **同一把存量尺,對「量具限制被寫成編號清單裡的一條」** | 那條沒有「量不到 / 盲區」這種詞 | **「只寫了一段」**(而它兩段都有) |

🔴 **第三格是最刺的那個:`452-verify.sh` 是【全 repo 寫得最好的那一份】** ——
複量窗逐支開檔證實它 `:19`「能驗什麼 / 不能驗什麼(誠實邊界)」+ `:29`「這把尺量不到什麼」**兩段俱全**,
而**關鍵字掃描把它判成沒做**。
📌 **一份被誤判成「沒做」的,是那件做得最好的那一份** —— 做得好的人用自己的話寫,
而**關鍵字表只收得到抄範本的人**。

🔴🔴 **而第四格是本條最該被記住的那一格:那把尺自己犯的,正是本條要防的病。**
它自標「keyword-based、受同病影響、不當權威」—— **它標對了,而標對不等於沒發生。**

⚠️ 四次的共同點**不是**「regex 寫得不夠好」。是:
> **尺回報的是【我的字集裡沒有】,而讀的人收到的是【repo 裡沒有】。**
> 這兩句話在畫面上是同一個字:`0`。

🔴 **而「今天 repo 剛好 0 支那種寫法」不是不補的理由 —— 它正是那把尺看不到時會印的東西。**
(2026-08-27 實測:補進 Python tuple 與 Node 分離參數兩個 pattern 後,
**真實 repo 分母仍 20、零新增假陽**,6 個合成正世界全 True、負對照 `execFileSync('git',['status'])` False。
⇒ **這兩個 pattern 對現況一句話都沒改,而它們改的是【下一支腳本用哪種寫法時我們會不會知道】。**)

**規則:** 🔴 **任何守門 / 掃描 / 量具,把自己的天花板寫在自己的檔頭,用【固定標題】。**

- **兩段,標題逐字固定**(固定是為了讓它可被機械盤點 —— 見下一條為什麼):
  - `天花板/範圍:` —— **這片刻意不做什麼**(哪些目錄、哪些副檔名、哪些情境不在分母裡)
  - `天花板/量具:` —— **這把尺讀不到什麼寫法**(它認哪幾種形狀;不認的那些它會印成什麼)
- 🔴 **`天花板/量具:` 那段要寫【它會印什麼】,不只寫「不支援」。**
  「不支援 Python tuple 形」讀起來像是它會跳過;實際上它會把那支檔算進「乾淨」那堆。
  **兩者對下一個人的行為完全不同。**
- 🔴 **兩段要【分開】、要在【檔頭】。**
  實測 2026-08-27:`board-state` / `citation-rot` / `view-apply` 三支把兩件事**混在同一份編號清單裡**
  ⇒ 機械盤點讀不出來;`migration-static-checks.sh` 寫在 **`:422`** 而非檔頭
  ⇒ **只 grep 檔頭的盤點會漏掉一支寫得好的。**
- 🔴 **每一段的最後一句,要是那一段自己的限制。**
  逐字建議:「**這份清單是我想得到的那些,而我最可能漏掉的是 ______**」;答不出後半 ⇒ 前半照寫。
  理由(memory `feedback_an-honest-limits-list-is-read-as-a-complete-one`,2026-08-22 一夜兩例):
  **一份誠實的限制清單會被讀成【完整的】那一份** ⇒ **不在清單上的那一種,比完全沒有清單時更難被懷疑。**
  ⚠️ 本條要求寫天花板,因此**同時放大了這個副作用** —— 那一句不是客套,是它的解毒劑。
- **零命中要能自我否證**:尺印 `0` 時,必須同時印出**它掃了幾支檔、認得幾種形狀**。
  一個孤零零的 `0`,在「真的沒有」與「我看不到」兩個世界裡逐字相同。
- 🔴 **加 pattern 時必附【合成世界】** —— 拿真 repo 驗只證明它不吵,證明不了它抓得到
  (真 repo 今天 0 支,而那正是問題)。**正世界每種寫法一發 + 負世界至少一發**
  (負世界的用途:證明它不是「看到 `git` 就叫」)。
- ⚠️ **改 regex 時要同時報【舊分母有沒有變】** —— 加 pattern 最常見的壞法不是漏抓,
  是順手改了字集而**舊的那些悄悄不匹配了**,而畫面上只是數字小了一點。

**適用範圍與 retrofit(這一條寫死,不留模糊):**
> 🔴 **本條對【既有已接線守門】同樣生效,不是只管未來。**
> retrofit 排在 **`#952`**。**若三個月後(2026-11-27)那個編號還 `open`,
> 表示這條規則實際上只管了未來 —— 而那本身就是它該被重新檢討的訊號。**
📌 這句話的用途不是催進度,是把「規則寫了而沒有人做」從一個**沒有人會發現的狀態**,
變成一個**打開 backlog 就看得見的狀態**。

⚠️ **而「既有幾支」這個數,兩個窗量出兩個答案,而本條刻意不選一個當權威:**
```
-6e  42 支 —— 🔴 沒有落檔清單(設計文只點名 4 支)⇒ 不可複驗
-9e  37 支 —— wired 定義寫死:被 package.json(含 lint-staged)/ .husky/* / .github/workflows/*.yml
              【字面呼叫】的 scripts/*;遞移一層會到 60(那把尺把註解提到的也算)
⇒ 🔴 兩個數【不能相減】。它們不是同一個母體的兩次量測, 是兩個母體。
⇒ 而兩邊唯一一致的那句才是本條的立法基礎:**符合固定標題形狀的 = 0。**
```
📌 **⇒ 立法時要問的不是「哪個數對」,是「這兩個數在回答同一個問題嗎」。**
(細節與四層分佈落在 `#952` 與 `~/pcm-mailbox/複量-9e-守門天花板存量三層-20260827.md`,本條不抄 —— 抄了就會過期。)

**規範定位:** 對齊 §12-43「掃描字集比宣稱窄」—— 12-43 講**發現**它窄,本條講**讓它自己講出來有多窄**。
🔴 **與 `docs/patterns/guard-and-instrument-traps.md` ⑤「掃描的字集/範圍比你宣稱的窄」(`:393`)不是同一條**:
⑤ 是**病歷**(它窄了,怎麼發現);本條是**處方**(讓它自報射程,且**用機械數得出來的形狀**)。
同檔 `:7309` / `:5330` 已經立了「這一條要與⑤分開記,否則會被套用錯的修法」的先例,本條照那個先例走。
🔴 **與 memory `reference_a-broken-ruler-prints-an-empty-set-not-an-error` 也不同族**:
那份講**尺壞了**的七種成因(參數/排序/顯示上限/過期快照/建置產物…);
**本條的尺沒有壞 —— 它正確地執行了一個比讀者以為的更窄的定義。**
📌 **⇒ 修法完全相反:壞尺要修,窄尺要【講出來自己有多窄】。**
與 §12-42(完備感 ≠ 涵蓋範圍)同族:12-42 是**讀的人**被完備感騙,本條是**寫的人**沒留下範圍。
並與 memory `feedback_absence-read-as-verified`(零命中被讀成已驗證)、
`feedback_a-filter-silently-drops-what-you-listed` 同族。
🔴 **本族的共同判別句:錯的那一次和對的那一次,印同一個字。**

**教訓來源:** 2026-08-27 四窗同日(`-6e` `git-isolation-denominator.py` 兩發合成世界 `ef1e1953`;
`-ed` 假 `mktemp` 注入器攔不到 `tempfile.mkdtemp`;存量掃描誤判 `452-verify.sh`;
同一把存量尺把「編號清單裡的量具限制」判成缺一段 —— 由 `-9e` 逐支開檔 37 支複量發現)。
🔴 **而四次都不是靠審查抓到的** —— 是靠**有人動手餵一個新世界 / 有人不用關鍵字逐支開檔**。
**審查看得到 regex 寫了什麼,看不到它沒寫什麼。**

**跨專案適用:** 適用任何有自動守門 / 靜態掃描 / lint 規則的專案。
不適用純手動 review 的專案(那裡沒有「尺」這個角色,而人會說「我不確定」)。

**首例:** `git-isolation-denominator.py`(2026-08-27 補兩 pattern);
正範例 `scripts/452-verify.sh` `:19` + `:29`(**先於本條存在**,且曾被誤判成沒做)
...
### 12-46. 一條規矩的效力 = 它與【動手那一刻】的距離 —— 不是它寫得多對

**事故脈絡:**
2026-08-31 夜,報價單線提了一個提案(先把 `supplier_sync_log` 的接線碼寫好、用開關關著,等 apply 完翻開關),
然後**自己否決了它** —— 理由是它當天稍早才立的一條約定:「一段寫好了但沒跑過真路徑的碼躺在 repo 裡,
會讓下一個人以為它驗過了」(= 那條約定裡的「閘被當成紀錄」)。
🔴 **而它自己把功勞放對了位置**,逐字:
> 我提那個提案的時候**沒有想到那條約定**,是寫下「好處是翻一個開關」那句的**下一秒**才對上的。
> **⇒ 約定有效的證據不是我想起它,是【它剛好近到我撞得到】。**
> **⇒ 一條放在 `docs/ops/` 而三個月沒人讀的約定,不會有這個效果。**

同夜的第二個實例(`-30`,`#838` 丙 落地):它判斷**不加路由表那一行**,理由同構 ——
> 一條規矩要放在【違反它的人會經過的地方】,而違反它的人**正在寫 traps 條目**;
> 路由表是給【還不知道要開哪支檔】的人看的。
⇒ 它把分界寫進 **traps 正本檔頭第一屏**,不是路由表。

**規則:**
- 立一條新規矩(或決定既有規矩要放哪)時,**必答一句**:**「下一個會違反它的人,動手前會打開哪一支檔?」** ⇒ 放那裡。
- 答不出那支檔 ⇒ **那條規矩會被寫下來而不會生效** ⇒ 要嘛先找出那支檔,要嘛改成機制(hook / 測試 / CI 守門)。
- 🔴 **這條對它自己也成立**:本條放在 `lessons §12`,而 CLAUDE.md 路由表逐字要求「**要在任何規則檔新增條目(立法)前**」讀 §12-34/§12-36 ⇒ **立法者本來就會經過這裡。**
- ⚠️ **不取代**「機制優先律」(`~/.claude/rules/00-work-rules.md` §4):機制做不到才寫規則字;本條管的是**寫下來之後放哪**。

**同夜的量測(它讓本條不只是主張):**
`-30` 在寫「教訓該怎麼落檔」那一顆 commit 時,**第四次**踩到 markdown 表格溢出(反引號裡一根裸豎線)。
```
四次全部由工具(md-table-overflow.py)抓到 · 由人抓到 0 次 ⇒ 4:0
```
📌 **⇒ 那個 4:0 就是「機制優先律」的量測值** —— 而**正在書寫某條規則的當下,是它最容易被違反的時刻**
(同夜 `-15` 在寫「我被推翻了」那段文字時踩到未加引號 heredoc,是同一個形狀)。

**規範定位:** 對齊 `00-work-rules §4` 機制優先律(先機制、機制做不到才寫字)+ §12-36(寫立法字面前先看既有格式)+ traps 正本「寫對地方 ≠ 會被讀到」一族。

**教訓來源:** 2026-08-31 夜,報價單線自陳 + `-30` 路由表判斷 + 同夜 4:0 的溢出量測。
