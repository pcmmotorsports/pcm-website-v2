...
| open | — | 🔴 **`20260824020000` 已 apply 而帳本沒記,退場動作欠著** | 待派 | Sean 2026-08-24 已 apply `#858` 手動建單 RPC。而 `supabase/APPLIED.tsv` 那一列**仍在工作區未 commit**,理由寫在 `64eb9366` 的 body:一收進去,`database-types-apply-state.test.ts` 的 `it.fails` 守門**當場轉紅** —— 而那正是它被設計要做的事,逼人回去重 gen `database.types.ts`、逐字比對、刪掉 ⑬ 那一段、檔頭計數減一。🔴 **那需要 Supabase 存取** ⇒ 帳還沒記,**而不是沒有那筆帳**。下一班第一件 |
...
| open | — | 🔴 **退款告警的分母不含退款表** | 待派 | Sean 2026-08-24 實跑:`refunding_stuck_count` = 0 而畫面上有 5 筆卡住的退款 —— 因為它 `FROM payment_double_charge_anomalies`。✅ 排程與寄信**都活著**(pg_cron 每天台北 09:00,Sean 貼過信件截圖)。🔴 **所以這比「完全沒有通知」更難發現,因為信真的有來**。窗F 規格已定稿(`F-004`),新欄名 Sean 線裁甲 `order_refunds_stuck_count`(名字要唸得出分母的表),**門檻甲/乙/丙未裁** |
| open | — | 🔴 **那封每日告警叫的是一件後台做不到的事,已叫 15 天** | 待派 | Sean 2026-08-24 跑的定論 SQL:`2SQH2P` age **14 天 23 小時**、`GVRDMH` age **5 天**。而信自己寫著「這一類後台沒有手可以處理」。🔴 **第二筆是新的** ⇒ 它不只叫不動舊事,**連新事都叫不動** ⇒ 指向流程不指向資料。要嘛把那隻手做出來,要嘛這類單不進每日信 —— **Sean 拍板題** |
...
| open | — | 🔴 **B5-a 的 `20260824030000` 仍未 apply** | Sean | 加 `admin_sso_login_events.actor_kind` / `.actor_staff_id`。空窗期由 `login-event.ts` 一段會出聲的退回路徑接住。⚠️ **順序是硬的且與直覺相反**:先加欄後接線(欄位閒置、無事);先接線後加欄 ⇒ 每次登入靜默降級 |
