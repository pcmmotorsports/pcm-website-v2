-- 🔴 2026-09-09 14:5x UTC 從【正式庫】唯讀取的現行定義(pg_get_functiondef)。
-- 用途:⟦深掃-A1⟧ 跨流程時間競態那一片的【基底】與【可執行反向 SQL】。
-- 🛑 不要拿 supabase/migrations/20260905360000 當基底 —— 那是「當初貼的那一版」。
-- ⚪ 進版控前掃 vault/decrypted_secret/password/token/bearer ⇒ 0 命中。
-- ⚪ 線上這一版【零】charge_attempt 檢查(grep -c ⇒ 0)⇒ 沒有別人疊過。
-- 🛑 這份沒有交易封套;要當 rollback 用請包 BEGIN…COMMIT 並加 SET LOCAL lock_timeout。
