...
· 🔴 **但這一列的原始事件本來就不是走應用路徑** —— 板上(**節錄**)「**Sean 09-04 請窗刪掉測試單,23 ⇒ 1**」,那是**直接下 SQL**。
· 🛑 **而下面兩句我原本寫太滿,收窄**(codex nit,它對):
  ⛔ ~~「那個身分就是【唯一】做得到 TRUNCATE 的身分」~~ ⇒ 我量的只是**五個指定應用角色沒有直接權限**;
     我**沒有排除**其他角色、可切換的角色身分、管理連線、高權限函式。**DB 角色也不等於某一個人。**
  ⛔ ~~暗示當初那次就是 `TRUNCATE`~~ ⇒ **證不到**。`23 ⇒ 1` 反而不像單次 `TRUNCATE`(那會變成 0)。
· ⇒ 📌 **這道稽核擋得住「下次有人 `DELETE`」,擋不住「下次有人 `TRUNCATE`」。而兩者對 Sean 來說是同一件事:單子不見了。**

### 補法(**沒動,等 Sean**)

⛔ ~~加一支 `AFTER TRUNCATE … FOR EACH STATEMENT`,而 statement 級拿不到 `OLD` ⇒ 只記得到「發生過」,記不到「刪掉的是哪幾筆」,那是機制天花板~~

🔴 **上面整段是錯的,已在 plan §0 更正 —— 而那是我沒量就講的。**
· 【量的】改用 **`BEFORE` TRUNCATE**:它跑在資料消失**之前** ⇒ 函式裡讀得到每一列 ⇒ **整筆抄得下來**(單號、金額都在)。
  📌 我把 `AFTER` 的限制,講成了「TRUNCATE 這件事」的限制。
· 🔴 **而更重要的一格是【範圍】**(codex must-fix,我量了):`TRUNCATE orders CASCADE` 在正式庫會遞迴清到 **26 張表**,
  其中 **14 張碰錢**。三支 trigger 只涵蓋 **3 張** ⇒ **23 張照樣消失**。
  🛑 **而最小骨架的驗收會全綠**,因為骨架裡沒有那 23 張表。

⇒ 完整討論與三個選項在 **`docs/plans/2026-09-10-truncate-leaves-no-trace-plan.md`**(等 Sean 批)。
🛑 **動 schema ⇒ 鐵則 8。本份只回報,沒有動任何東西。**
