...
#### 🔴 已拍板的個案紀錄(**不是第四類例外** —— 逐案列出, 不放寬上面那條規則)

**① 結帳頁付款方式的「銀行匯款」那一格 —— 稿上沒有, 而它是 Sean 拍板加的。**

```
Sean 2026-09-04 拍板(逐字, 出處 ~/pcm-mailbox/Sean拍板-20260904-七題.md 檔尾第二批):
  Q-B 結帳頁沒有匯款選項 ⇒ 乙:授權自畫, 照信用卡那格, 事後補稿
Sean 2026-09-06 對「要不要補稿」的回覆(逐字, 主視窗 -f8 轉):
  「那....你們依照原有設計格式多一欄位而已不是嗎..」⇒ 裁:不補稿、不動 Design
```

🛑 **⇒ 對稿時不得把它拿掉。** 樣式**照既有的信用卡那一格**(那正是拍板的字面)。

🔬 **稿上真的沒有(2026-09-06 `-mail` 量, 分母附上, 可重跑)**:掃 OD **全部 12 個專案**
(磁碟 `ls ~/Library/Application\ Support/Open\ Design/namespaces/release-stable/data/projects | wc -l` ⇒ **12**)——
「匯款」與「結帳 或 checkout」出現在**同一支檔** ⇒ **0 檔**;「銀行轉帳」⇒ **6 檔**而**逐支開來全是
FAQ / 內容頁 / 商品頁 / 帳戶頁的資料檔**, 沒有一支是結帳頁的稿。
🟢 正對照 同一發問「信用卡」⇒ **33 檔**(尺是活的)· 🔵 負對照現造字面 ⇒ **0**。

🎯 **為什麼這一句要寫在【這裡】而不是只寫在板列上**:板列 `⟦design-ATMBACKFILL⟧` 自己寫過那個風險 ——
**下一個對稿的人手上那份稿沒有匯款選項 ⇒ 他會判我們畫錯 ⇒ 他會「修正」掉我們照拍板做的東西。**
📌 **而他不會去翻板子, 他會讀他該讀的這一節。**

⚠️ **這一格【不放寬】上面那條鐵則** —— 它記的是**一筆已經拍過板的個案**。
下一次再遇到「碼上有而稿上沒有」, **預設仍然是 storefront 對齊 design**;
要例外, 一樣要一筆拍板, 而**拍完就補到這張清單下面**, 不要靠記憶。
