...
5. 🔴 **經銷價守門**:`price_store` / `price_by_tier` / `cost` / `metadata` 四個 token 在本片新增的檔案裡**零命中**,
   且守門**掃整個 `app/products` 與 `components/products` 目錄樹**(片1a MF5 的修法已經是遞迴掃,
   `product-repository.test.ts:32-43`)⇒ **本片新檔一落地就自動被納入,不需要改守門清單**。
   **必須用突變驗**:故意在詳情頁寫一次 `price_store`,那格要紅。
