...
> ---
> ## 🔴🔴 **[2026-08-31 `-a0`] 上面那句「用 39」現在是【假的】—— 而它是本檔最容易被照抄的一句**
>
> **那支片【被撤下了】,從來沒有 apply 過。**舊字面留著不刪, 讓照抄 39 的人在同一發撞到這裡。
>
> ```
> 量到的（本窗當場跑, 樹 /Users/sean_1/pcm-wt-auth）:
>   ① 那個路徑 supabase/migrations/20260826000035_m4b_q15_customers_select_service_role_policy.sql
>      bash scripts/where-is.sh <該路徑>  ⇒ 工作樹 ✗ / dev ✗ / 所有 ref ✗（掃了 81 支, 含 tag）
>   ② 它的生命史（git log --all --follow）:
>        365a8f29 2026-08-26  加進來（315 行, 三輪 codex 審過）
>        f86fa3b5 2026-08-26  逐字「Q15 改做可重跑的全庫體檢 —— 那支只救一張表的 migration【先按住】」
>        dc6d1961 2026-08-26  刪除（片2 補 email_outbox 那一顆順手刪的）
>   ③ 帳本 supabase/APPLIED.tsv 含 'q15' ⇒ 0 列
>      🔵 負對照 帳本含 '20269999999999' ⇒ 0
>   🟢 正對照 git log --all -S'customers_select_own' ⇒ 11 顆（尺會動）
>      🔵 負對照 -S'zzq831nosuchpolicy' ⇒ 0 顆
> ```
>
> 🛑 **它不是弄丟的, 是【有人決定按住】** —— `f86fa3b5` 的訊息自己說了。
>    ⇒ 這一格照「看到規模大到不像意外的東西, 先問是不是有人決定的」處理:**問過了, 是決定的。**
>
> 🔴 **⇒ 所以正確的數是「本檔量測當下那兩個原數」, 不是 39**:
> ```
> 版控重播  47 張開 RLS / 【40】張缺  ← 用這個, 不是 39
> 正式庫    50 張開 RLS / 【42】張缺  ← 2026-08-26 Sean 親跑那支探針量到的
> ```
> ⚠️ **而這兩組都綁在 2026-08-26** —— 距今 5 天。**要現值就重跑本檔下面那支探針, 不要抄。**
>
> 🛑 **一格我證不到, 不要讀成我證了**:`customers` 那條政策**在正式庫裡到底有沒有**, 本窗答不出來
>    —— 那要連正式庫讀 `pg_policies`, 而我沒有那個授權。
>    我只證得到:**它不在 repo、也不在帳本上** ⇒ **不是經由我們的流程進去的。**
>    （若有人手貼進去過, 那會是路⑤, 見板 `⟦b4-ACL1⟧`。）
>
> 📌 **形狀**:一份 spec 的**存在理由**是「免得下一個人以為 Q15 關掉了」(本檔自己第 56 行逐字),
>    而它**自己**因為那支片被撤下而變成「讓下一個人以為 customers 已經補好了」。
>    ⇒ **一份防止誤讀的文件, 在它描述的東西被撤回時, 會變成誤讀的來源** —— 而**它不會知道。**
> ---
