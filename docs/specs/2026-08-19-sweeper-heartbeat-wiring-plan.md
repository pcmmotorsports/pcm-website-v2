...
#### 🔴🔴 而 N 不是「多久會知道」—— 那兩個數字差 24 倍,而本檔原本只寫了小的那個(2026-08-22 線 C 補)

```
N = 60 分鐘   決定的是【誤報】:多久沒說話才算死(吸掉單輪被砍、單輪 timeout、部署空窗)
而【多久會知道】由**讀取端的檢查頻率**決定 —— §3 把讀取端掛在 anomaly-alert
                ⇒ cron.job jobid=2  '0 1 * * *'  ⇒ **每天一次,01:00 UTC**

⇒ 🔴 sweeper 若在 01:05 UTC 死掉 ⇒ 下一次檢查是 24 小時後
   ⇒ **最壞情況:靜默死亡 23 小時 55 分才有人知道。**
```

**這不是本檔寫錯,是【N 這個數字沒有它自己的意義】** ——
讀 plan 的人看到「N=60 分鐘」,會很自然地以為**一小時內會知道**。**不會。**

> ✅ **要補的一句(建議逐字放進本節)**:
> 「N=60 分鐘是**判定門檻**;而**偵測延遲上限 = 24 小時**(讀取端掛在每日的 anomaly-alert)。
>  兩者不同,不可混用。**若 24 小時的延遲不可接受,要改的是【讀取端掛哪】,不是 N。**」

⚠️ 本補充**不提改到哪裡** —— 動排程 = 鐵則 12④,而且本 plan 本身還沒批。

📌 判準來源:`~/pcm-mailbox/C-803補充-現在那個修法抓不到它-20260822.md` 條件 1 ——
   **偵測的頻率必須【短於】證據的壽命 / 短於你能容忍的損失時間。**
   ✅ 而本片這一格**有一半是對的**:心跳的證據住在 `sweeper_heartbeat` 表 ⇒ **持久,不會被保留期吃掉**。
   `#803` 那條就是敗在這裡(證據住在只留 6 小時的 `net._http_response`)⇒ **兩片不要混著談。**
...
🔴 **而這條路的殘餘我不掩蓋**:
```
🔴🔴 **最前面這一條,因為它是本片【自己新增】的單點**(GR-060 nit;主視窗指定放最前):
  讀取端的活性掛在 ANOMALY_ALERT_ENABLED 後面 ⇒ **那個 flag 一關,監測靜默失效**
  ⇒ 它與「sweeper 死了」在畫面上**同樣是沒有訊息**
  ⇒ **一個監測系統自己有 kill switch,而 kill 掉之後的樣子與「一切正常」一樣。**

· 🔴 **CRON_SWEEPER_ENABLED 若被【刻意】關閉** ⇒ 章不會刷新 ⇒ **每天誤報一則**
  ⇒ 文案要能分辨「它被關掉了」與「它死了」,或讀取端先檢查那個 flag。

· **讀取端的 Supabase client 也吃同一條 lazy 不變式** —— 我前一版只在寫入端寫了(§2-b)。

· **§4 那行 console.error 印進我自己判定沒人讀的通道** ⇒ 可接受(GR-060 ④),
  **而要明寫:那行是【診斷】不是【偵測】** —— 偵測靠 staleness,不靠它。
...
~~· 🔴 **「sweeper 現在有沒有正在死」我答不出來**~~ ✅ **已答(2026-08-22 線 C 用正式庫唯讀量)**
  ```
  窗 1  08-20 21:16→03:15 UTC  178 發  ⇒ 🔴 2 發 503   (G 窗量,backlog #803 逐字)
  窗 2  08-21 12:46→18:45 UTC  180 發  ⇒ 🔴 1 發 503   (線 C)
  窗 3  08-21 14:50→20:48 UTC  180 發  ⇒ ✅ 0 發非 200 (線 C;全表 468 列零非 200)
  ```
  🟢 **它沒有在死** —— 三個窗的發數都對得上 `*/2`(6 小時 = 180 發)⇒ **沒有整段沒跑過**。
  ⇒ ⇒ **本片是【背景債】,不是【急件】。**
  🔴 而它**一直有小傷**:錯誤率約 0.5%–1.1%,每一發在 2 分鐘後的下一輪自己好。

  🔴🔴 **而那三發,本片一次都不會叫** ——
  `14:12 成功 → 14:14 失敗 → 14:16 成功` ⇒ `last_success_at` 在 14:16 就被刷新
  ⇒ **它從來沒有超過 60 分鐘。**
  ```
  問題 A  sweeper 整個死掉      ⇒ 本片解決   ⇒ 🟢 目前【沒有觀測到】發生過
  問題 B  活著而個別輪次失敗    ⇒ #803 解決  ⇒ 🔴 三個窗裡兩個真的發生
  ```
  ⚠️ **這不是說本片不用做** —— 它擋的是「發生了就沒人知道」的世界,而那個機率量不到
  (**它從來沒發生過 ≠ 它不會發生**)。
  ⇒ 但**排優先序時,兩片的證據等級不一樣**,而本檔與 `#803` 原本都沒有分開講。
  ⚠️ 限定:錯誤率的分母只有三個 6 小時窗,**其中兩個是別人量的**
  ⇒ 它答得出「有沒有在死」,**答不出「多久會壞一次」**。
```

---

## 附錄 A · 🔴 **要 Sean 拍的一題:每天要不要收到一則「一切正常」?**

> 由來:GR-060 ② 提的便宜第二層。主視窗已進 `MAIN-055 §K-2`(概念版),
> **本附錄補的是「他每天實際會收到什麼」** —— 他要判的是**那則訊息煩不煩**,不是概念。

### A-1 先講現況,因為它才是這題的理由
```
check-anomaly-alerts.ts:136-140 逐字:shouldAlert = 五個計數全部 > 0 才成立
:146 if (shouldAlert) 才推播
⇒ **一切正常 ⇒ 完全不寄。**
🔴 所以今天在 Sean 那端:
   「沒事」        ⇒ 安靜
   「告警器死了」  ⇒ 也是安靜
   ⇒ **兩者長得一模一樣。** 而心跳片只把這個問題往上推一層(誰看守看守者)。
```

### A-2 兩個選項,而下面是**他每天實際會收到的東西**

**甲(現況,不改):只有出事才響**
```
正常的日子:                (什麼都沒有)
出事的日子:
  • 雙扣候選(open,待查證)2 筆(最舊 3 天)
  • 人工待確認(sweeper 放棄:pending 孤兒 / 被讓路轉人工)1 筆 — …
  請登入後台/MCP 查 W1 退款候選報表…
  (本訊息零個資、僅計數)
```
**乙:每天一則,正常也寄**
```
正常的日子(每天一則,長這樣):
  ✅ PCM 金流守門 08/19 01:00 — 一切正常
  • 排程心跳:12 分鐘前(門檻 60 分)
  • 雙扣候選 0 / 退款卡逾時 0 / 人工待確認 0 / released 死卡 0
  (本訊息零個資、僅計數)

出事的日子:與甲相同,再加最上面那行變成 ⚠️
```

### A-3 兩邊的代價(我不美化任何一邊)
```
甲  ✅ 零噪音
    🔴 而【安靜】永遠是兩義的 —— 他無法從「沒收到」推出「沒事」
    🔴 心跳片做完也不改變這一點:心跳只在【出事時】說話

乙  ✅ 安靜變成有意義的訊號:**該來的沒來 ⇒ 有事**
       而那是唯一**不靠機器**的一層 —— 人自己會察覺「今天怎麼沒收到」
    🔴 每天一則,永遠。365 則/年
    🔴🔴 **而它有一個會自己壞掉的方式**:看久了會不看 ——
       一旦習慣性略過,**它缺席時也不會被察覺** ⇒ 那時它變成純噪音、零價值
       ⇒ 這一格**我沒有辦法用技術解**,它取決於他自己的習慣
```

### A-4 我的推薦與理由
**推薦乙**,而理由**只有一條、而且很窄**:
```
這條線上的每一道守門,壞掉的樣子都是【安靜】——
  sweeper 死 ⇒ 計數停在 0 ⇒ 告警說一切正常(這張表的 COMMENT 自己寫的)
  route 被砍 ⇒ 沒有回應、沒有 503、沒有 log
  心跳沒接線 ⇒ 表是空的 ⇒ 述詞零命中 ⇒ 不告警
⇒ 🔴 **在一條「所有故障都長得像安靜」的線上,唯一能對抗安靜的東西就是【預期中的聲音】。**
```
⚠️ **而我把反對自己的理由也寫出來**:A-3 那個「看久了會不看」是真的,
  而它會讓乙**退化成甲再加 365 則噪音** —— 比甲更差。
  ⇒ 所以這題**不是技術題**:它取決於 Sean 會不會真的每天瞄一眼。**只有他知道。**

### A-5 這題不擋本片
```
甲或乙,心跳的寫入端與讀取端都要做,做法一模一樣。
差別只在讀取端**要不要多一條「沒事也寄」的分支** ⇒ 那是幾行。
⇒ **不要等這題才開工。**
```
