...
| V9 | **(A11a-4)** 訂貨欄接的是 A9c 的**非 nullable** 型別,UI 端零 `?? 0`、零 join | **原始碼字面守門**(讀檔、剝註解後 grep `?? 0`)🔴 ~~型別層由 tsc 保證~~ **那句是錯的、A11a-4 實測證偽**:`x ?? 0` 對非 nullable 值在 TS **完全合法、零診斷**,本 repo 的 eslint 也無 type-aware 規則擋它 ⇒ **只有文字守門擋得住** | 在 UI 補 `?? 0`(母 plan 計數器摘要列 `:335` 明文禁止)|
...
| A11a-4/-5/-6 各自後 | ~~11 / 12 / 13~~ **10 / 11 / 12** | 逐欄補齊(訂貨 / 發票 / 出貨,順序視前置到位先後)。🔴 **原數字預設 A11a-3(操作欄)先做**,但十題全拍 A 已把它**整片延後到 A13** ⇒ 各片實際收工值各減 1。§3 的算式(「前一片值 +1」)才是權威、照它算即可;**A11a-4 實際收工 = 10**(2026-08-06 實得)| ✅ 每片都是完整的表 |
