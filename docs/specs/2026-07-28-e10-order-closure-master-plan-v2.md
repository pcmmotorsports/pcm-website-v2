...
| 28 | **A2b1** | T | `allocated_quantity` 跨列總量守門 constraint trigger + **先鎖 parent `order_items` 列**(R4 抓:不鎖時併發兩筆各讀舊合計仍會一起超量);鎖序固定 `order_items` → `order_item_procurement`。🔴 **R6 補 delta 守門**:新增或調升 allocation 時 assert **`合計 ≤ quantity − cancelled_quantity`**(取消後不得為已取消部分加開採購);**取消前已寫入的採購事實不動**。🔴🔴 **2026-07-31 連動(R3 抓,這條是設計層要求、不是寫法建議)**:`cancelled_quantity` 現在在
...
或呼叫具名的 lock+recompute 函式;**並對「真相非零但摘要列缺失」fail-closed**。
