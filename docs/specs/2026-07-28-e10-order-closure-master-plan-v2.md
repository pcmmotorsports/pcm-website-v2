...
| 57 | **A10b** | U | 明細頁:逐品項採購表單(含分配數量、到貨數量、異常原因、**供應商下拉選單**)。🔀 **2026-08-01 改**:~~相似值警告~~ 隨 A5c 作廢;供應商改為從主檔選(`S3a` 的讀模型供料、字母序、typeahead),**不再是自由文字**。🔴 本片是 `suppliers.is_active` 的**第一個真實消費端** —— 停用的供應商不得出現在這個選單裡(在此之前 `is_active` 是無作用旗標) | **5, 6, 7**(🔴 R5 抓:第 7 項 = **A10a+A10b 聯合驗收** —— 告知紀錄在 A10a、異常原因在 A10b,兩片皆完成才綠) |
