...
> 已跑:**R1 67**(審 v1)→ … → **R16 6+1** → **R17 5** → **R18 7 must-fix + 1 nit**,十八輪皆 FAIL,findings 各存 `docs/reviews/2026-07-28-e10-k1-r{2,…,18}-codex.md`(趨勢 67→33→31→32→26→24→15→17→16→7→5→4→7→10→10→6→5→**7**)。
...
> ⇒ 本檔為 **R18 折入後版本**(修正 = DAG U 段去重複節點 / A9w2 刪「或唯讀」/ A9w4 拆三(item writer / status-options writer / contract)且前二列高風險 / A9v 補撤 `order_status_options` 寫權 + ACL 終態斷言 / 結案 `resolution` 分流、僅 `retry_authorized` 可開下一代 / :722 刪 disabled 替代案)。全檔唯一待決清單 = **§8.6**。下一步送 **R19**。
...
第1批  A0a/A0b/A0c 現況重驗 (docs)
       ...
       A9w1/A9w2 (U: 九碼明細頁+週邊下架) → A9w3 (A: 投影收縮)
         ...
         ↓
       A10a/A10b/A10c2/A11a-c/A12a-b/A13a-b/A14a-c (U 主建設; 每片各一節點, 不重複列)
         ↓
