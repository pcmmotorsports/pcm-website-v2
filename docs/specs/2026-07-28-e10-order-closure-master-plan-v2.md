...
> ⚠️ **落地分兩片**:C9 的**約束本體**在小線 S2a(已寫、未 apply);「由 A4a 重算維護」要**大線 S2b** 接線才成立 ——
> 在那之前**沒有 writer 會把真實出貨量搬進第四軸**,所以 C9 攔不到「出貨超過到貨」——
> 🔴 但那不等於「第四軸恆為 0」:DB 未強制,owner / SECURITY DEFINER 寫得進去,A4a 也不會把它清掉。
...
> 施工歸屬(**2026-08-06 更新**:原 B2-S2 一份 plan 已作廢,拆兩線)=
> 小線 `docs/specs/2026-08-06-e10-b2-s2a-summary-columns-plan.md` / 大線 `…-s2b-recompute-wire-plan.md`。
> 🏁 **小線已落地** = migration `supabase/migrations/20260806100000_m4b_e10_b2_s2a_summary_shipped_quantity.sql`
>(摘要表 **6 欄 / 10 條 CHECK**;A1 契約債 ① 的三項清償聲明逐字寫在該檔檔頭,含第三項「刻意不做:不從 C6/C7 減 shipped」)。
> ⚠️ 該 migration **停在 `a4a-chain` 分支、尚未併 dev、尚未 apply**(Sean 2026-08-06 Q3=C)。
...
| 計數器摘要 | A1(**新表 `order_item_quantity_summary`**) | **A4a trigger**(由 A2/A7 明細驅動,無手填入口;負測 = A4b) | A9c 🔴 **必須用 PostgREST nested left embed**(`order_items(…, order_item_quantity_summary(…))`)—— `ADMIN_ORDER_LIST_SELECT` 是 select 字串、**寫不了 SQL `COALESCE`**;缺列會回 `null`,由 **mapper 在 TS 層正規化成四個 0**(🔴 2026-08-06 B2-S2a 加了第四軸 `shipped_quantity`,原文的三軸計數已過期),並補「缺列」單元測 | A11a-c 🔴 **只接非 nullable 型別**(正規化在 A9c 的 mapper 完成,UI 片不做 join、不做 COALESCE) | — |
...
2. 出貨 owner RPC(`shipment_reference` 由 N3a 產生、重試在本層;✅ **Q19=A(Sean 2026-07-28):P1 後綴正式作廢、不加 `-1/-2`** —— reference 本身全表唯一已足)
   ...
   ① 🏁 **`shipped` 的強制點 = 2026-08-05 Sean 拍板 Q1=A/Q2=A 已定案,本條 DoD 結清。**
      **終案** = 摘要表 CHECK `oiqs_shipped_le_instock`(C9)承重 + 出貨 RPC 訊息層 + break-glass oracle 四軸;
      ...
      施工歸屬 = **B2-S2a 小線**(`docs/specs/2026-08-06-e10-b2-s2a-summary-columns-plan.md`;
      原 B2-S2 一份 plan 已作廢)⇒ 🏁 已落地 migration `20260806100000`,未併 dev、未 apply。
      出處:`docs/specs/2026-08-05-shipped-enforcement-analysis.md` §10 + memory `project_m4b-b2-shipments-db-decisions`。
...
| **通知矩陣**(UX §3 #11) | ✅ **2026-08-09 Sean Q11=A**:照 07-26 草案表全收(8 事件;退貨收到=只走 LINE 不寄 Email,其餘 Email+LINE 皆發)| ~~開批閘~~ 已解 | 通知塊可開工 |
| **admin 建單六題** | ✅ **2026-08-09 Sean 全拍**:①價格=員工手填一口價含稅(A)②稅/折扣=**逐品項折扣+稅額分列**(B,對帳細)③客戶=必掛既有會員、無帳號先建(A)④付款=建單一律未付款、收款走收款帳本功能(A)⑤庫存=自由品項不進庫存採購鏈、出貨用「現貨補記到貨」#352(A)⑥經銷價=不自動帶、價格全手填、經銷價體系零接觸(A)。開批閘已解除 | ~~開批閘~~ 已解 | 手動建單可開工(納入 08-09「完整化」線) |
| **退款線兩題**(R8) | ✅ **2026-08-09 Sean 全拍**:①Q12=A 各自原路退(卡款系統自動退刷、匯款列待辦人工匯還)②Q13=Sean 自案(逐字「保留,看剩下訂單總金額多少,退掉多餘部分,不足的話也不用退款,就當作是先收多餘部分」)⇒ **退款額=max(0, 已收未退總額−取消後訂單剩餘應收總額)**;不退時已收款轉剩餘訂單部分款。落 memory `project_m4b-order-admin-complete-then-test` | ~~開批閘~~ 已解 | 退款線可開工 |
