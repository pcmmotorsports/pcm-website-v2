...
> 已跑:**R1 67**(審 v1)→ … → **R10 7** → **R11 5+1** → **R12 4 must-fix + 2 nit**,十二輪皆 FAIL,findings 各存 `docs/reviews/2026-07-28-e10-k1-r{2,…,12}-codex.md`(趨勢 67→33→31→32→26→24→15→17→16→7→5→**4**)。
...
> ⇒ 本檔為 **R12 折入後版本**(修正 = `check_fail_count` 入 schema + reconciling reclaim 轉移寫死 / `order_payments` 補 rail 欄·writer·卡款 backfill 的付款管道權威定義 / A7 明訂兩表同片 / **作用中主規格 `2026-07-25-…-rebuild-spec.md` §3.1/§3.4 舊模型原地標記由本檔取代** / 帳本 `bank_refund_id` 沿用既有欄)。全檔唯一待決清單 = **§8.6**。下一步送 **R13**。
...
1. `order_payments` 收款帳本(U3 多筆匯款 / 四格 / 催款 / 溢款處置)。🔴 **排本批最先**(R7:A8b 的 `partiallyPaid` 上限 =「實收額」,現行 schema 沒有實收金額真相,只有這張帳本能提供)。🔴 **R12 補「付款管道權威」完整定義**(§5.3 把本表當 rail 判定權威,原本只規格了匯款腿):每筆收款腿帶 **`rail` 欄**(值域以 **A0a 實查 `orders.payment_method` distinct 值**為準 —— 該欄 text nullable、註解範例 tappay/atm,`20260604120000:109`,不憑記憶列舉);**TapPay 卡款腿 backfill**(既有 paid 單依 charge attempts 回填 card 腿,否則舊單在本表查無管道);writer = 本批匯款登錄 RPC + backfill migration;**A8b 分流讀本表 rail,腿集合非純卡、非純匯款 ⇒ 混合,§8.6 拍板前 fail-closed**
