...
> ⇒ **R20 已 PASS(20 輪收官)**;本版另折入 **Fable 四線復盤 14 條 must-fix**(`docs/reviews/2026-07-28-e10-fable-retrospective.md`)。全檔唯一待決清單 = **§8.6**。閘序 = 關卡1 PASS ✅ → Fable 復盤 ✅ → **Sean 最終批准(現在這一步)** → A0a 動工。
> ✅ **動工前置之一已滿足**:完整視覺全貌已交付且 Sean 批准方向(artifact `ed7a6276-70fc-44f3-b09c-61c8991b5294`)。🔴 **剩餘前置 = 兩閘皆過**(R19 修字面:①codex 關卡1 PASS ②**關卡1 過後送 Sean 最終批准**(鐵則 8)—— 缺一不施工)。
>
...
| 16 | **D1t1** | runbook | 🔴 **D1 orchestrator 交易核心片**(R11 再拆:CLI/dry-run 另片):交易執行函式本體(BEGIN → 鎖序 → 步驟 8b-13 → COMMIT;任一 API 失敗 / 斷言不符 = `ROLLBACK` 後 exit 非零;`recordQuery` 逐次 30s AbortSignal,依賴 A15)。🔴 **復盤L3 修:不得直接 import `TapPayChargeAdapter`** —— 檔頭 `import 'server-only'` 在純 node/tsx 載入即 throw;runbook 用**獨立小 client**(複用 `wire.ts` 解析與 Record API 組裝、env 讀 Partner Key、不經 server-only 模組),D1t2 單元測含「node 環境可載入」斷言。🔴 **鎖序以 grep 既有金流 RPC 的實際鎖序為準對齊,不自創**(復盤L3 抓 ABBA deadlock 風險) | — |
...
| 19 | **D1b1** | runbook | 🔴 **D1 read-back 片**:TapPay read-back 六筆,**判定矩陣寫死(R9;數值親驗 tappay-reference.md:85-86)**:逐筆以 `payment_charge_attempts.rec_trade_id` 為查詢鍵、top `status ∈ {0,2}`(查詢成功語意)、**唯一命中**;三筆已退單(0052/0102/0104)斷言 `record_status = 3`(REFUNDED)且 `refunded_amount` = 授權金額;三筆 `pending` 單(0064/0090/0101)允許 `record_status ∈ {-1 (ERROR), 5 (CANCEL)}` —— 🔴 **`0`(AUTH)與 `4`(PENDING;三筆 3DS 已啟動、4 是最可能實際值 —— 預期分支非異常)皆不自動放行:出現 = 保留 cohort、輸出證據、停下等 Sean,不進刪除**(R21 同步片列)。🔴 **0052 出口同步(R21)**:0052 查無 ⇒ sandbox 推定成立、步驟 12 對 0052 走保持原值路徑,不擋其餘;0102/0104 查無必 abort。其餘零筆、多筆、狀態外、金額不符 = 一律 abort。🔴 **硬前置(A0a 實查)**:六筆 attempts 的 `rec_trade_id` **全部非 NULL**(欄位 nullable);任一 NULL = 停下 raise Sean,不得用寬條件替代 | — |
...
| 57 | **A10b** | U | 明細頁:逐品項採購表單(含分配數量、到貨數量、異常原因、相似值警告) | **5, 6, 7**(🔴 R5 抓:第 7 項 = **A10a+A10b 聯合驗收** —— 告知紀錄在 A10a、異常原因在 A10b,兩片皆完成才綠) |
| 58 | **A10c2** | U | 依供應商單號搜尋畫面(消費 A9b2) | — |
| 59 | **A11a** | U | 列表桌機:**12 欄骨架 + rowSpan 分組重算**(§7.2) | — |
...
**第 1 批 = 69 片。片型(`awk` 逐列數、非手算):U 18 / runbook 14 / A 16 / M 7 / R 6 / T 5 / docs 3。**

🔴 **高風險(判準:M / T / runbook / R 一律;A 型命中「service_role-only 表讀取投影」「server action 授權邊界」或「共用金流 adapter」才算)**:
M 7 + T 5 + runbook 14 + R 6 + A 10(A9a / A9b2 / A5c / A9g 讀 service_role-only 表;A9d1 / A9d2 / A9h / **A9w4a / A9w4b(R18 補列 —— server action 授權邊界)** 是授權邊界;A15 動共用金流 adapter)= **42 片**。
...
| 8 | **正式交易 + fail-fast**:🔴 **前置(復盤L3 + R21 補全路徑恢復)**:暫停 pg_cron 的 settle sweeper —— **單一方式定死 = `cron.alter_job(job_id, active := false)`**(不 unschedule、原排程設定原地保留)、**先等待進行中的執行排空**(查 `cron.job_run_details` 無 running)再開始;**恢復 = orchestrator 外層 `finally` + 獨立 `--recover-sweeper` 命令,成功/abort/crash/SIGINT 四路皆恢復並驗證 `active=true`**(cron 設定不受 DB ROLLBACK 保護,不能靠交易回滾);🔴 **dry-run(步驟 6)不動 cron** —— 全程 ROLLBACK 零寫入,sweeper 不需停、模擬時此步僅 log→ `BEGIN` → **`SET TRANSACTION ISOLATION LEVEL REPEATABLE READ`**(復盤L3:READ COMMITTED 下「非 cohort 逐 byte 不變」在併發結帳時必誤判)→ `SET LOCAL lock_timeout='5s'` / `statement_timeout='60s'` → 🔴 **鎖策略現在定死(R21:live repo 實存兩種順序 —— `confirm_order_payment` 先鎖 order、`mark_charge_attempt_charged` 先鎖 attempt ⇒「統一鎖序」不可得,改 fencing)**:①鎖序固定 ①`orders` ②`order_items` ③`payment_charge_attempts` ④`pending_invoices` `FOR UPDATE`(與 confirm 同向)②**鎖 orders 後立即 assert cohort 26 張的 attempts 全為終態(`failed`/`released`)、pending 三筆狀態未變** —— live payment 路徑(begin/mark_charge)只作用於 active attempt,cohort 斷言成立即證明無並發者會反向取鎖;斷言失敗 = `RAISE` abort ③sweeper 已停(前置)⇒ 唯二反向取鎖者皆被排除 → 交易內重驗 cohort 計數與金流狀態,漂移即 `RAISE` | 🔴 **鎖住 orders 列即同時擋住所有新 FK 子列**(FK 驗證需 KEY SHARE、與 FOR UPDATE 互斥)⇒ `email_outbox` 等 0 列表**不需列級鎖**,交易內重驗 = 0 即可(R5 抓「鎖清單與匯出清單不一致」的正確解法,附機制依據) |
...
🔴 **0052 專屬出口(復盤L3:sandbox 推定若成立,production 憑證查無 = 必然結果,原矩陣會把整條 D1 卡死)**:
**0052 零筆命中 ⇒ 判定 sandbox 推定成立、步驟 12 對 0052 跳過**(`payment_status` 保持原值、
remediation audit 記「production 查無、sandbox 推定、Sean 授權怎樣處理都好」),**不擋其餘步驟**;
