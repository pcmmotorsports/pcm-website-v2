...
| 18 | **D1t3** | runbook | 🔴 **D1 timeout / rollback 整合驗證片**:六筆總體 3 分鐘上限、`SET LOCAL idle_in_transaction_session_timeout = '5min'`(大於總體 HTTP 預算 + 餘裕);**負測(R23 落片):①逾時後交易確實 rollback、不留半掛交易 ②雙交易 NOWAIT(反向持鎖時立即 abort、不等待)③kill -9 後啟動 self-heal(依 ownership state 檔恢復 sweeper)**(隔離 DB 實跑) | — |
