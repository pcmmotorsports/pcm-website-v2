...
> 已跑:**R1 67**(審 v1)→ … → **R11 5+1** → **R12 4+2** → **R13 7 must-fix**,十三輪皆 FAIL,findings 各存 `docs/reviews/2026-07-28-e10-k1-r{2,…,13}-codex.md`(趨勢 67→33→31→32→26→24→15→17→16→7→5→4→**7**;R13 回升 = 首次全檔掃相鄰作用中規格抓到 4 條舊字面,v2 本體只剩 3 條)。
...
> ⇒ 本檔為 **R13 折入後版本**(修正 = `check_fail_count` 歸零+token-CAS 寫入 / canonical rail enum 定死、TapPay backfill 跨付款狀態、cash 建腿 none 不建、多實收管道才算混合 / D1 匯出逐表 selector(events 經 anomalies JOIN `old_order_id`)/ 主規格施工序·自由品項·N7-N9 與 UX §8 折入規則四處原地更新)。全檔唯一待決清單 = **§8.6**。下一步送 **R14**。
...
1. `order_payments` 收款帳本(U3 多筆匯款 / 四格 / 催款 / 溢款處置)。🔴 **排本批最先**(R7:A8b 的 `partiallyPaid` 上限 =「實收額」,現行 schema 沒有實收金額真相,只有這張帳本能提供)。🔴 **R12 補 + R13 閉環「付款管道權威」**:**canonical `rail` enum 定死 = `card / bank_transfer / cash`**(R13 抓:nullable `payment_method` distinct 當值域會漏管道;實查 distinct 仍列 A0a、用來驗 canonical 集合有沒有漏,不反過來當定義);**無收款(none)不建腿**;各管道 writer 具名 —— `card` 腿 = **TapPay backfill migration,依 `payment_charge_attempts` 交易事實回填、跨付款狀態**(🔴 R13:只回填 `paid` 會漏掉 D1 後三張 `refunded` TapPay 單)、`bank_transfer` 腿 = 本批匯款登錄 RPC、`cash` 腿 = 同 RPC 的現金選項;**A8b 分流讀本表 rail:唯有「多種實收管道並存」才算混合**(cash 單腿≠混合),混合 = §8.6 拍板前 fail-closed
