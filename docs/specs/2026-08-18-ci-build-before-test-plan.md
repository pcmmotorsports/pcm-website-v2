...
> ✅ 狀態(2026-08-18 16:2x G1 更新)：**已上線、已驗收。**
> `fbd5320e` 已推(`git push origin <sha>:dev`,主視窗執行,Sean 當日授權),
> CI `run 32115401537` ⇒ `success`,§7 五條驗收**逐條全過**(見 §7)。
> ~~13:4x 那版:Sean 已批、`ci.yml` 已動、**未推**~~ —— 那句在 16:1x 起為假。
...
> 理由與推翻過程在 **§9-c**(`grep -n '^### 9-c' <本檔>`)與 **§3 開頭那則裁定更新**(`grep -n '裁定更新' <本檔>`)。
> 🔴 ~~原本寫 `:427` 與 `:103` 兩個行號~~ —— **兩個都已漂掉**(GR 2026-08-18 實測:那兩行都落在 code fence 裡)。
> **指向本檔自己的行號,會被【本檔自己的編輯】弄假,而兩邊都不會有東西紅** ⇒ 一律用 grep 錨點。`ci.yml` = 鐵則 12④ 平台設定 ⇒ 需 codex 對抗審查 + Sean 批。
> 🔴 本檔所有「現況」句都附可重跑量法。量測時點 2026-08-18 11:1x CST、`dev = a482b428`。
> ⚠️ **例外:§9-a 與 §9-a-2 那幾條【不可重跑】** —— 它們指向的鑽機已拆除,詳見該節檔頭的紅字。
> **「可重跑」這三個字在本檔不是一個修辭,它是一個承諾;承諾兌現不了的那幾條已經逐一標出來。**
...
`.github/workflows/ci.yml`，在 **`Test` 那一步之前**插入一步(定位用 `grep -n 'name: Test' .github/workflows/ci.yml`,不寫行號)：
...
~~`.github/workflows/ci.yml`，在 `Test` 之前插入一步：~~
...
> **講的都是【甲：全 monorepo build】**，而現行方案是**【乙：只 build admin】**（§3-a、§9-c、`grep -n 'name: Build admin' .github/workflows/ci.yml`）。
> **零刪除保留**是因為它記著「丁為什麼被否決」那條理由（那條在乙底下仍然成立）。
> ⚠️ **但只讀本節的人會去改成 `pnpm build`** —— 那正是本檔記過的形狀：
> **改了方案而論證段留在原地沒跟著改。**
> ⇒ 引用本節前先讀 §9-c(`grep -n '^### 9-c' <本檔>`)與 §3-a。
...
**⚠️ 這一段是【11:1x 那一版】的紀錄,已被 11:5x 的【乙】與 Sean 13:20 的批准取代**
(GR 第三輪 must-fix:三層推翻紀錄裡唯一漏掉刪除線紀律的一段 ——
**§4 檔頭那個加註只攔得住從節首讀進來的人;`grep '裁【甲】'` 直達本段的人會讀到「甲是現行」**)。
**下面那段引文本身仍然成立**(它殺的是丁案,而丁在乙底下一樣沒被採用),保留原文:
...
3. 🔴 **Sean 推 `dev` 之後那一發 CI**：`gh run list --branch dev --limit 1 --json headSha,conclusion` 的 `headSha`
   ...
   ✅ **已驗收(2026-08-18 16:1x,G1 逐條核)**：
   ```
   run 32115401537   headSha fbd5320e534bd460d07d4a6b2881ee4c288d2ce3   conclusion=success
   Test Files 526 passed | 1 skipped (527)  ⇒ failed 數 0
   Build admin 那一步真的跑了（CI log 裡該字串 52 次；它是逐行前綴）
   ```
   🔴 **而它與同一顆的【本機拋棄式鑽機】那一發完全相同(526 / 1 / 527)** ——
   **那是本 repo 第一次拿到「本機」與「CI runner」兩個獨立環境的對照,而它們一致。**
   ⚠️ **一個資料點,不是一條曲線** —— 不要拿它去推「以後本機綠就等於 CI 綠」。
   ⚠️ ~~這條的 yes/no 不在施工窗手上 —— 本條在他推之前恆為「未驗」~~
   **那句已過期**(Sean 2026-08-18 把推不推授權給主視窗,由它 `git push origin <sha>:dev` 推那一顆)。
   **留痕的理由**:它記著「為什麼這條當時驗不了」,而那個限制**是被一個做法解掉的、不是被忽略的**。

   🔴 **順手記一個 CI 上的觀察,因為下一個人會想知道「CI 上那個數字到底是幾」**：
   ```
   本機零 env 鑽機   build.full.log 的 'not set'  ⇒ 162（= 81 × 兩行，四發皆同）
   CI runner        整份 log 的 'not set'        ⇒ 1
   ```
   **那 1 行的原文與它的位置(兩者都要帶,否則會被讀成同一件事)**：
   ```
...
4. 🔴 **負向對照（不可省）**：拿掉 Build ⇒ 那兩支**必須再度紅**，錯訊息仍是「找不到建置產物」。
   ...
   ✅ **本條【已經滿足】,不要另行製造**(GR 第三輪 nit / C2):`a482b428` 那一發 CI(run `32094343674`)
   就是「沒有 Build」的世界,而它 FAILURE、訊息正是「找不到建置產物」⇒ **這個負向對照今天已經免費發生過。**
   🔴 **照 §7-b 主視窗裁定:不推紅的。** 只讀本節逐條驗收的人若真的去 dev 拿掉那一步製造一發紅,
   **那正是 §7-b 要擋的動作** —— 兩節原本一節叫你做、一節說不必做,這一句是把它接起來。
...
| 負向（第二道，本機） | 在拋棄式鑽機上不 build 直接跑那兩支 ⇒ 必須紅 | 🔴 ~~`pcm-w1-fg2`，可重跑~~ **那棵鑽機已拆,這一發【無法重跑】**;要重做只能新開一棵(`git worktree add --detach` + `pnpm install`,約 30 秒) |
...
> 🔴🔴 **本節的證據載體【已經不存在了】(2026-08-18 GR 第三輪 must-fix,G1 折)**:
> 下面標「量法(可重跑)」的那幾條全部指向拋棄式鑽機 `/Users/sean_1/pcm-w1-fg2`,而**那棵樹已整棵拆除**
> (GR 14:3x 實測 `ls` ⇒ No such file、`git worktree list` 命中 0;我複跑同樣兩條,結果相同)。
> ⇒ 照 `00-work-rules §6-b` 第 4 條改寫:**下面的數字是【當時量到的】,載體已拆,【無法重跑】。**
>
> **canonical 在哪(誠實版,三段都要看)**:
> · W1 那三份 `build.full.log` 的原檔 ⇒ **沒有備份,已隨鑽機消失。** 不要再照著跑那幾條命令。
> · **但那個結論有【一份獨立的替代證據】**:G1 2026-08-18 在鑽機 `pcm-g1-fg2`(commit `55f391b5`,
>   **同樣零 `.env.local`**)跑的那一發,`build.full.log` 有 **162 行 `not set`(= 81 × 兩行)**、四項 `rc=0`
>   —— **81 這個數與 W1 的三份一致**。該 log 我在拆鑽機【之前】抄了出來:
>   `logs/four-greens/G1-55f391b5-20260818T133344/build.full.log`
>   (量法 `grep -c 'not set' <該路徑>` ⇒ **162**,我當場跑過)。
> · 🔴 **是「一份」不是「兩份」** —— G1 的**第一棵**鑽機(`pcm-g1-fg1` @ `52d83440`)那一發,
>   我只抄了 `console-tail.log`、**沒有抄 `build.full.log`**(`ls` 該目錄 ⇒ 只有 console-tail.log;
>   `grep -c 'not set' console-tail.log` ⇒ **0**,因為腳本只留尾 N 行)⇒ **那一發的 81 也無法重數。**
>   **我落筆時差點寫成「那兩份」,是回頭 `ls` 才發現的。**
> · ⚠️ **那份替代證據自己的限度**:①`logs/` 在 `.gitignore` 裡 ⇒ **它只活在這台機器的這棵主樹**,
>   重 clone 或清掉就沒了 ②它量的是**另一顆 commit**,不是 W1 當時那顆 ③**仍然沒有人量過 GitHub runner**。
...
| F6(b) bundle 與線上不同 | 🔴 storefront 那半會產一份 `NEXT_PUBLIC_*` 為空的 bundle;對這兩支測試無影響(它們量 CSS 與 chunks),但**多產一份沒人要、且與線上不同的東西** | 🔴 ~~不產 storefront bundle ⇒ 這個疑慮不存在~~ **假字面(GR 第三輪 must-fix)**:乙照樣在零 env 的 CI 產一份 **admin** bundle,而 `apps/admin/src` 有 **21 處 `NEXT_PUBLIC` 引用、分佈 8 個檔**(量法 `grep -rn NEXT_PUBLIC apps/admin/src` 計 21 行、`grep -rl` 計 8 檔,**我自己重跑過**)⇒ **同一個疑慮對 admin 一樣成立**,而那份 bundle 正是兩支測試要量的對象。✅ **今天無後果**(它們量 CSS 與 chunks、不吃 env)⇒ **裁定不用翻**;本欄改的是**記帳**:原本在一個乙也有的維度上只記了甲的帳。 |
...
驗收 3 現在寫的是「**Sean 推 dev 之後那一發 CI**」(用 grep 定位,不寫行號)。
