...
> **版本:** v4(R1 codex 26 條 + R2 換模型 23 條,兩輪全折後重寫)。
> **狀態:** **未經 Sean 批准 ⇒ 一行 code 都還沒寫**(鐵則 8)。主視窗裁示**不再開第三輪審查**,本版直接帶給 Sean 拍板。
> **片型:** 🔴 高風險片(鐵則 12③)。
...
⇒ v4 紀律:**引用任何函式的行為之前,先回這張表確認「最後一次改它的是哪一支」。**
...
1. **schema**:`order_item_procurement` 加 `voided_at timestamptz` / `void_reason text` /(`voided_by text`)
...
| G6 | **unvoid 有沒有真實需求,我沒問過 Sean** | 這不是查得到的,是要問的 ⇒ 成題 Q2 |
...
## §6 待 Sean 拍板(四案版;主視窗會把這段帶去)

```
Q1:採購撤銷用哪一案?
A: E|C'|A|B
  E = 照網站上「作廢包裹」那一套一模一樣的做法,幫採購也做一個「作廢」(推薦)
      好處:①原本訂幾件不會不見 ②撤銷理由存在那筆採購自己身上,不是只存在系統日誌
            ③這套做法我們半個月前就寫過一遍了,不是重新發明
      代價:要動兩處已經上線的計算,是這四案裡工最多的
  C'= 把件數改成 0、列留著
      🔴 我上一版推薦這個,現在撤回:原本訂 12 件的那個「12」會被抹掉、撤銷理由只能存在
         一個我自己剛判定不可信的地方,而且有四個地方會靜默讀錯 —— 例如撤銷後還能按「登錄到貨」,
         按下去畫面說「超過訂購量」,完全誤導
  A = 整列刪掉
      🔴 v2 推薦、已撤回:會讓另一支已上線函式的正確性論證失效,而且刪掉後用供應商單號搜不到那張單
  B = 自己另外設計一套作廢
      沒有理由 —— B 就是 E,而且已經有人寫好了

Q2:撤銷之後,要不要能「取消撤銷」救回來?
A: A|B
  A = 要(推薦;包裹那套就有,照抄成本低)
  B = 不要,撤錯了就重新建一筆
  🔴 這題我沒有資料替你判斷 —— 我不知道你實際上會不會撤錯了又想救回來
...
Q4:同一筆採購「作廢兩次」要怎樣?
A: A|B
  A = 拒絕,跟他說已經作廢了(推薦;包裹那套就是這樣拍板的,
      理由是第二次通常帶著不同的理由,靜默吞掉會弄丟那個資訊)
  B = 當作沒事發生
```
