...
## §10 🔴 上線後線上實測(2026-09-03 03:1x,線 `-mail`,`origin/main` 已含 `75491250`)

### 10-a 功能:**§5 那張表全綠,而其中兩格比預期好**
每發都帶 cache-buster + `cache:'no-store'`(f0 實測過:**被快取的舊答案與沒生效的修法印同一個數字**)。
```
rsv4                        8   🟢 不得回歸 ⇒ 沒回歸
油箱貼                       8   🟢
rpm                         8   🟢
rsv4 油箱貼                  2   ✅ 今天之前是 0
CARK9650 / cark9650      1 / 1   ✅ 真料號, 大小寫兩種同一個結果
PED-GP EVO MON SX RS660      1   ✅ **含空白的真料號** —— 切詞設計的關鍵測資
LIGHTECH 拉桿                8   ✅
zzqprb9137never              0   🔵 負對照
rpm rsv4                     0   ⚠️ **不是本片的驗收條件**(見下)
```
⚠️ **與主視窗-87 03:0x 那一發不一致的一格**:它量到 `rsv4 油箱貼` ⇒ **0**,我量到 **2**。
**我不宣稱它量錯** —— 我的這一發較晚、且帶 cache-buster;而部署完成時間我沒有量。
⇒ 📌 **兩個讀數都留著,而差異的成因未確認。**

🔵 **`rpm rsv4` ⇒ 0 的真因不在本片的碼裡**(主視窗-87 對正式庫唯讀查證):
全站**零支**商品在 `title/subtitle/description/external_id` 四欄裡同時含這兩個字
(含 `rsv4` 346 / 含 `rpm` 149 / **交集 0** / 全站 22,804)。
⇒ **`rpm` 是品牌名、`rsv4` 是車款名,而兩者都不在那四欄裡** ⇒ **那是下一刀的事,不是本片失敗。**

### 10-b 🔴 效能:**判準命中 ⇒ 片 B 的 GIN 索引優先於相關性排序**
**判準是跑之前寫下來的**(`scratchpad/perf-threshold.md`):①8 詞中位數 > 單詞的 3 倍 ②>1500ms ③任一發 >5000ms。

```
單詞 rsv4                中位數 ~140-302 ms   (8 筆)
單詞 CARK9650            中位數 ~842 ms       (1 筆)
兩詞 rsv4 油箱貼          中位數 ~1,660 ms     (2 筆)
兩詞 LIGHTECH 拉桿        中位數 ~1,707 ms     (8 筆)
八詞                     中位數 ~1,612 ms     (0 筆)
```
⛔ ~~🎯 ① 命中(1,612 / 140 ≈ 11.5 倍)· ② 命中(1,612 > 1,500)· ③ 未命中 ⇒ **索引優先。**~~
🔴🔴 **判準命中是真的,而「⇒ 索引優先」那半句 2026-09-03 03:2x 被推翻,原句留著。**
主視窗-87 對正式庫跑 `EXPLAIN (ANALYZE, BUFFERS)`:那一句 DB **只花 182ms**(Seq Scan、
`Rows Removed by Filter` 22,802、`Buffers: shared hit=4,977` 零磁碟讀)⇒ **DB 佔端到端 ~11%**
⇒ 建 GIN 最好的情況是 182 → 5ms ⇒ **1,660 → 1,483,客人感覺不出差別。**
📌 **⇒ 判準沒有錯,錯的是從「慢」推到「所以建索引」那一步 —— 那一步沒有來源。**
⇒ **後續拆解(空查詢當零 DB 對照、不是冷啟、不是每次重連、以及那個對不上的 1,400ms)
   全文在 `2026-09-03-storefront-search-brand-plan.md` §4。**

🔴🔴 **而量出來的形狀與我原本的假設【不同】,這一格比結論重要**:
```
我原本以為:詞越多越慢(線性)
實際量到  :**8 詞(1,612ms)與 2 詞(1,660ms)【一樣慢】** —— 斷崖在 1 ⇒ 2, 不在 8
```
🔵 **而我第一個解釋也錯了**:我先猜「慢是因為結果稀少 ⇒ 要掃完全表」,
所以又量了一組**把結果多寡與詞數拆開**:
```
             結果多            結果少
1 詞      302ms (8 筆)      842ms (1 筆)     ← 稀少確實有影響
2 詞    1,707ms (8 筆)    1,660ms (2 筆)     ← 而 2 詞不論結果多少都 ~1.7s
```
⇒ 📌 **兩個因子都在,而【詞數】壓過【稀少】。** ⇒ 我原本那個解釋只對了一半,
而**只量一組數字的話,我會把「稀少」當成全部的成因,然後去修錯的東西。**

🛑 **⇒ 這一格的業務意義要明寫,不要只留數字**:
**兩個詞是客人最常打的形狀**(Sean 自己撞到的三發裡有三發是兩個詞)⇒ **~1.7 秒是常態不是尾巴。**
⇒ ✅ **所以索引不是「效能優化」,它是把一個【每天都會發生】的 1.7 秒降下來。**

⚠️ **射程(先寫,免得被引用成別的東西)**:
· 這量的是**瀏覽器到回應**,含網路與 Vercel 冷啟 ⇒ **不是 DB 的 `EXPLAIN`**,
  **分不出慢在 DB 還是慢在網路/冷啟**。要分,得有人對正式庫跑 `EXPLAIN (ANALYZE)`。
· 每組 4-5 發、取中位數;**同一時段、同一條網路** ⇒ 不同時段可能不同。
· `x-vercel-cache` 每發都是 `MISS` ⇒ 至少不是在量快取。
