...
⚠️ **E11 的 security-label 兩欄事後更正(codex 關卡2 round2 #2)**:上表所附查詢用
`(select count(*) from pg_seclabel s where s.objoid=p.oid)` 與同款 `pg_shseclabel` 查詢。
量到的值(0 / 0)沒錯,但**查法不精確**:`pg_seclabel` 的識別鍵是 (objoid, **classoid**,
objsubid, provider),缺 `classoid` 會誤配**其他 catalog 中同一 OID 數值**的物件標籤;
`pg_shseclabel` 只存 cluster-shared 物件(role/database/tablespace),函式**依定義不會**
出現在該表。→ 現行指紋與斷言**只查 `pg_seclabel` 且帶 `classoid='pg_proc'` + `objsubid=0`**,
`shseclabel` 一項已移除。E11 本列保留為當時的取樣紀錄,**引用時以 §4 段 2.5 與補充檔 §1 為準**。
      ...
      **不得**用 IF EXISTS 強行覆寫未知版本。
      ...
      🔴 **「完整指紋」定義(codex R3 BLOCKER-1 更正 —— 原本只用 `md5(prosrc)` 不足)**:
        ...
        指紋 = 下列全部串接後取 md5:
          ...
          **`proargnames`** + `prosecdef` + `proconfig::text` + `proacl::text` +
          `pg_get_userbyid(proowner)` + `prorettype` + **`proretset`** + `lanname` +
          `provolatile` + `proparallel` + `proisstrict` + `proleakproof` + `procost` +
          `prorows` + **`prosupport`** + **security label**(見下方 round2 #2 更正) +
          `obj_description(oid,'pg_proc')`
        (= E11 全欄位,任一被動過指紋就變)

      🔴 **關卡2 M-2 更正(2026-07-19)**:上列**粗體四項是後補的**。原公式漏
        `proretset` / `prosupport` / security-label 狀態,而這三項**明明列在 E11**
        → 舊公式配「全屬性」稱呼是**名稱誇大實際能力**。補齊後基線指紋改變。

      🔴 **關卡2 round2 #1/#2 再更正 —— 指紋公式與常數共歷三代,前兩代皆作廢**:
        · **公式只寫一次**:改用 migration 內的 `pg_temp.b2_create_order_fp(oid)`,
          段 2.5 守門與檔尾斷言⑧**共用同一 helper**。原本抄兩份並宣稱「兩處不一致就
          fail-closed」**不成立**(斷言⑧只驗自己的公式對自己的常數,從不與段 2.5 比對;
          若有人同時弱化段 2.5 公式並重取該處常數,首次 apply 走狀態 A 根本用不到狀態 B
          常數 → ⑧照樣通過)。改為結構性單一來源後,漂移在型式上不可能發生。
        · **security label 精確化**:`pg_seclabel` 識別鍵是 (objoid, **classoid**, objsubid,
          provider),原本只比 `objoid` 會誤配他表同值 OID;`pg_shseclabel` 依定義只存
          cluster-shared 物件、函式不可能在其中 → **移除該表查詢**,改存排序後的
          `provider=label` 串接。
        · 現行常數(**兩者皆於 production 實測取得**,見補充檔 §4 第 3/11 條):
          8-param 基線 = **`77945871ed5d9f5dcac7f8d53c9f192c`**
          9-param 預期產出 = **`850e2e3cf5f503391df5fe6fe0067cce`**
        · 已作廢、勿再引用:`2b898129e49d194c30ab8039b857c0be`(初代)、
          `beca7444c4c29251940509d889fe0c74` / `8492abfc88930ecfd350e288a25d203c`(二代)

      🔴 **關卡2 M-3 更正:狀態 B 不再放寬**。原設計狀態 B 只驗 `md5(prosrc)` + 簽章字串,
        理由寫「ACL/COMMENT 本就要 canonicalize,故無風險」——**那正是守門存在要禁止的
        覆寫行為**。正常的 history 裂縫必發生在本檔完整 `COMMIT` 之後,現況本就該等於本檔的
        預期產出,放寬沒有正當理由。現改為**兩個狀態都驗完整指紋**。
        ⚠️ 9-param 預期產出指紋是**自指常數**(輸入含本檔下方的 COMMENT/ACL 字面)→
        代償控制 = **檔尾斷言⑧用同一條公式重算並比對同一常數**,任何人改了 COMMENT 卻忘了
        重取常數,**首次 apply 就會在斷言階段整批回滾並吵**,不會潛伏到某次重跑才誤擋。

      ⚠️ **殘餘:守門與 DROP 之間的競態窗口(codex R3 指出,誠實揭示、未消除)**
        讀 catalog 與執行 DROP 之間,理論上可有另一個 session 改動同一函式 → 守門不會重驗。
        · **緩解(關卡2 M-1 更正後的誠實版本)**:交易開頭取
          `pg_advisory_xact_lock(1201033732)` —— key 是**所有 `create_order` DDL 共用的固定常數**,
          日後動同一函式的 migration 一律沿用**這個字面值**。
          🔴 **原設計用「本 migration 自己的時戳」20260719120000 當 key = 假互斥**:下一支
          migration 會用它自己的時戳,兩把鎖永不衝突 → 前一視窗宣稱「已緩解」**是不成立的
          宣稱,已作廢**。常數來源(僅供追溯,apply 時不再計算):`select hashtext('public.create_order')`。
        · **未消除的部分**:不遵守此約定的外部 DDL session 仍可能插入 —— 此鎖**只**互斥
          「同樣寫死此常數」的 migration,真正的防線仍是下方 Q5=A 的有界假設。
...
段 7  DO $$ … $$;  fail-closed 斷言(任一不符即 RAISE):
      ...
      ⑤ 🔴 E11 全屬性逐項精確相等(codex R1 #2;非「含」、非抽樣):
         ...
         proconfig **整個陣列**相等 / **`pg_seclabel` 無列**(`classoid='pg_proc'` 且 `objsubid=0`;
         🔴 round2 #2:原字面「seclabel_count=0 / shseclabel_count=0」**已作廢** —— 前者缺 classoid
         會誤配他表同值 OID、後者查的表依定義不存函式)
...
- **apply 後需回退的內容**(🔴 codex R2 #6 更正,原字面作廢):另開 rollback migration ——
  ...
  **不含** COMMENT / ACL / owner / security label,只凍它會 rollback 不回去):
  ...
  7. security label 狀態(🔴 round2 #2 更正:只認 `pg_seclabel` 且須帶 `classoid`/`objsubid`;
     `shseclabel` 一項**已作廢**,函式依定義不會出現在該表)
  **rollback 只認這份凍結副本**;**沒有這份凍結副本就沒有合格的 rollback 路徑**,不得動工。
  ...
  🔴 **上述兩個缺口 + codex 關卡2 M-4 的第三個缺口 —— 已於 2026-07-19 全部補完**
  (原字面「**本片不修**」已作廢)。補法 = **新增 immutable 補充檔**
  `docs/reviews/2026-07-19-b2-preapply-snapshot-supplement.md`,**不動**原 snapshot(該檔自訂
  「產出後不得再編輯」,改它反而破壞凍結語意):
  ...
  「跑 rollback SQL → 指紋回到 `77945871…`」整條來回貫通,且守門負向測(竄改 ACL / 竄改 COMMENT)
  兩態皆被擋下。
...
2.5. 🔴 **語法預檢**(Sean 2026-07-19 拍 A;關卡2 後補做):起一個**本機拋棄式 PostgreSQL**,
   把 migration 檔**原封不動**套一次,只確認**語法**。理由:所有交易模擬用的都是「在 DB 內
   等價重建」的版本,**整份檔案從未被 PG 完整 parse 過** —— 未覆蓋面 = 檔頭註解 /
   `BEGIN`/`COMMIT` / `SET LOCAL` / `pg_advisory_xact_lock` / `COMMENT ON FUNCTION` 的 SQL 逸出。
   codex 明說這**不同於** Sean 拒絕的 failure-injection,是更便宜的 syntax preflight。
   ⚠️ 預檢**不可**改動檔案內容(改了就不是預檢那份)。結果見補充檔 §4。
3. 三綠(讀輸出內容判定)
...
## 15. 關卡 2 — Codex 審 diff 銷案表(2026-07-19,判定 NO-GO → 本輪全數處置)

**codex 關卡2 判定:NO-GO,5 must-fix**(其中 2 條是前一視窗的**假宣稱**)。逐條銷案:

| # | codex 判定 | 處置 | 落點 |
|---|---|---|---|
| M-1 | 🔴 advisory lock 是**假互斥** —— key 用本 migration 專屬時戳,下一支 migration 用它自己的時戳 → 兩把鎖不衝突。前一視窗宣稱「已緩解」= **假的** | **全收,採 codex 修法①+②合併**:key 改為所有 `create_order` DDL **共用的固定常數 `1201033732`**(來源 `hashtext('public.create_order')`,寫死並註明、apply 時不再計算);**同時**改寫全部宣稱字面,明確界定「只互斥同樣寫死此常數者、對外部 DDL 無效」,真正防線仍是 Q5=A 有界假設 | migration 段 2(檔內註解 12 行)、plan §4 段 2.5、本表 |
| M-2 | 🔴 狀態 A 指紋**不完整**卻自稱「全屬性」—— 漏 `proretset` / `prosupport` / security-label,三項**明明列在 E11** | **全收**。公式補齊四項(另補 `proargnames`)。「全屬性」字面全樹銷:migration 檔尾 NOTICE 改「E11 全欄」。<br>⚠️ **round2 #2 再收斂**:security label 查詢補 `classoid`/`objsubid` 並移除 `pg_shseclabel` → 公式再變一次,現行基線 = **`77945871ed5d9f5dcac7f8d53c9f192c`**(prod 實查) | migration 段 2.5 + 斷言⑧、plan §4 段 2.5、補充檔 §1/§2 |
| M-3 | 🔴 狀態 B 守門**過弱**,會靜默洗掉未知變更(只驗 `prosrc` md5 + 簽章;檔內註解「本來就要 canonicalize 所以沒風險」= **正是守門要禁止的覆寫行為**) | **全收**。狀態 B 改驗**完整預期產出指紋**,現行值 = **`850e2e3cf5f503391df5fe6fe0067cce`**(prod 實測)。🔴 **已實證**:竄改 9-param COMMENT 後重跑被擋下。<br>⚠️ **round2 #1 再收斂**:原「斷言⑧能證明兩處公式一致」為過度宣稱,已改為 `pg_temp` helper 單一定義來源 | migration 段 2.5 + 斷言⑧、補充檔 §4 第 8 條 |
| M-4 | 🔴 rollback **實際不可執行**(snapshot 只存 `prosrc` 本體、未明列 `proretset`、repo 內無可執行 rollback) | **全收**。新增 immutable 補充檔 `docs/reviews/2026-07-19-b2-preapply-snapshot-supplement.md`:完整 `pg_get_functiondef`(md5 與 prod **逐位元組相同**)+ E11 全欄 + **已 parse 且已實跑驗證**的 rollback SQL 與專屬斷言。**不改原凍結檔**。🔴 **已實證**:rollback 實跑後指紋精確回到基線 | 補充檔 §1/§3/§5、plan §9 |
| M-5 | schema/history 分離**確定會發生**(codex 讀 CLI v2.98.1 原始碼);可重跑策略合理但須補**失敗後 SOP** | **全收**。SOP 寫進 **migration 檔頭**(查 history → 查簽章 → 查完整指紋,四種判讀結果各自對應「可重跑 / 不需重跑 / 停下交人工」),明訂**不可直接再按一次 `db push`** | migration 檔頭(14 行) |
| nit | Q1=A 已知代價(跑完商品迴圈與 `nextval()` 才撞 CHECK → generic 500 + 單號跳號);硬條件 = B-4 flag 不得早於 B-3 六條鏡像驗證 | 已在 §7 Q1 記載;**硬條件補記於此**,B-4 動工時須逐條核 | §7 Q1、本列 |

**codex 判 OK、本輪未動**:狀態 A/B 二分無漏合法狀態、ACL 重建無漏復原、Q1=A 確實落地(RPC 內零 trim/驗證)、現行 app 仍送 8 參故 B-3/B-4 前不產髒值且建單先於 TapPay。

### 15.1 本輪新增的實證(非推論)

Sean 拍板的**語法預檢**與 M-4 的可執行性驗證合併於一次本機拋棄式 PG 實跑,九項結果見補充檔 §4。
重點:①migration **原文**整份 parse + 執行零語法錯 ②凍結副本重建的 8-param 指紋 == prod 基線
③狀態 A/B **正向各一、負向各一**(竄改 ACL、竄改 COMMENT)皆如預期 ④rollback 來回貫通。

⚠️ **本機驗證涵蓋不到**:`supabase db push` 的交易邊界(= M-5 SOP 處理的裂縫)、PostgREST
快取重載、prod 真實資料互動。本機 PG 17.10 vs prod 17.6 的版本差已由 byte-level 相符大幅降低,但非證明。

### 15.2 關卡 2 round 2(2026-07-19)—— 判定仍 NO-GO,3 must-fix + 1 nit

**codex 銷案:M-1 CLOSED / M-2 PARTIAL / M-3 PARTIAL / M-4 CLOSED / M-5 CLOSED。**

| # | codex 判定 | 我的複核 | 處置 | 落點 |
|---|---|---|---|---|
| r2-1 | 🔴 **斷言⑧無法保證兩份公式一致**:⑧只驗「⑧自己的公式算出的產物 == ⑧自己的常數」,從未與段 2.5 的公式比對。若有人同時弱化段 2.5 公式**並重取該處兩個常數**,首次 apply 走狀態 A、根本用不到狀態 B 常數 → ⑧照樣通過,之後 COMMENT 被外部改動就被弱化的守門放行 | **成立**。我上一版寫的「兩處不一致 = 斷言失敗,fail-closed」是**過度宣稱** —— 這正是本條線反覆出現的同一種錯:替一個控制措施取了它做不到的名字 | **全收,採 codex 修法**:公式收斂成單一 `pg_temp.b2_create_order_fp(oid)` helper,段 2.5 與斷言⑧共用 → 漂移在**結構上不可能**,不再倚賴斷言偵測。誇大字面全數改寫;COMMIT 前顯式 `DROP` helper(實測零殘留) | migration 段 2(helper 定義)、段 2.5、斷言⑧、檔尾 DROP;plan §4 段 2.5 |
| r2-2 | 🔴 **security-label 查詢未精確鎖定物件**:只用 `objoid`,但識別鍵含 `classoid`;`pg_shseclabel` 只存 cluster-shared 物件、函式不會在其中 | **成立**。查了一張依定義不可能命中的表,還把它算進「E11 全欄」 | **全收**。①只查 `pg_seclabel`、補 `classoid='pg_proc'::regclass AND objsubid=0` ②移除 `pg_shseclabel` ③由 count 改為**排序後 `provider=label` 串接**(可偵測「標籤被換掉」而非只偵測數量)。公式再變 → **兩個常數第三度重取,皆於 prod 實測** | migration helper + 斷言⑤;plan §2 E11 註、§4 段 2.5、§4 段 7 ⑤、§9;補充檔 §1/§2 |
| r2-3 | 🔴 **Packet 與 STATUS 仍是上一輪 NO-GO 版本**(含時戳鎖、舊 `2b898…` 指紋、弱狀態 B,並宣稱整份 migration 從未 parse),STATUS 卻仍指示 Sean 貼它 | **成立,且這條打臉我自己** —— §15 才剛寫「宣稱字面全樹銷」,Packet 就是漏網的那一處。**同款漏改在本條線已復發 10 次** | **全收**。Packet 全檔重生(對齊本輪 staged 版本、內嵌 SQL 與驗證證據全部更新);STATUS 同 commit 更新 | Packet 全檔、STATUS |
| r2-4 | nit:寫死 `1201033732` 可接受,但「object-scoped」用詞過強 —— advisory key 是應用層自訂數字資源,PG 並未綁定到 `pg_proc` 物件 | 成立 | **順手清**。改稱「application-defined create_order DDL 約定鎖」;並註明 32-bit key 碰撞的後果是**誤等待**(3s fail-fast)而非互斥失效 | migration 段 2 |

**codex 本輪明確判 CLOSED 且未再要求動的**:M-1(約定鎖)、M-4(補充檔的 rollback SQL 可執行、位於 Markdown 不會被自動套用、本機 17.10 驗證範圍有誠實標示)、M-5(失敗 SOP 符合 CLI 風險模型)。
另確認:**未發現新的經銷價外洩、浮點金額或 client tier 信任問題**;函式體反向移除 `notification_email` delta 後與前版相同。

### 15.3 🔴 輪數用盡 —— 交 Sean 決定是否破例開 round 3

PCM 規則:**關卡 2 每 slice 硬上限 2 輪,round2 仍 FAIL → 停、raise Sean,不准自行加輪。**
