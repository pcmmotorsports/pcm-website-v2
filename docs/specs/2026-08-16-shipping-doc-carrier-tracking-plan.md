...
> 🔴 **本 plan 落地後留下四題未決(`Q-C5`/`Q-C6`/`Q-C7`/`Q-C8`)⇒ `docs/specs/2026-08-16-shipping-doc-open-questions.md`。**
> 其中 `Q-C5`(主線路徑上那張紙永遠沒有追蹤碼)**會推翻本 plan §0 的價值主張**,接手實作前必讀。
...
| ~~**出貨日**~~ **`日期`** | `shippedAt` 有值印它;**沒有值印【列印當天】** | Sean 拍甲,見 §3.2;🔴 欄名已由 `Q-C6` 改成「日期」兩個字 |
...
```
情形①  箱還沒標出貨          ⇒ 追蹤碼還沒有（正常）  → 🔴【整列不印】（Q-C9b=乙）
        ⚠️ 原寫「印『尚未出貨，出貨後補』」——那個「補」的管道【不存在】
           （通知信暫緩／沒有會員訂單頁）⇒ 對客假承諾。Sean 逐字「什麼都不寫 ,空格」。
