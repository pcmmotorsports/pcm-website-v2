...
> # 🔴🔴 **部分作廢 —— 「追蹤碼」那三分之一已於 2026-08-17 拿掉紙面(`Q-C5`=丙)**
>
> **Sean 2026-08-17 睡前逐字 `q3: 丙`:出貨明細單不印追蹤碼欄位,追蹤碼只走LINE／Email 給客人。**
> `Q-C19`=乙(2026-08-17 早,逐字「**乙 只拿掉追蹤碼那列**」)⇒ **同一區的「貨運商」與「日期」留著。**
>
> ⇒ **本 plan 裡凡是講「紙上印追蹤碼」的段落一律作廢**(§0 的價值主張、§3.1 三種 `null` 印什麼、
> §4「三個號碼並排」、§6 驗收 ③)。**貨運商與出貨日那兩段仍然成立、已落地。**
> ⚠️ **「還沒出貨 / 缺漏 / `other` 免碼」三分法本身【沒有】作廢** —— 它搬去 `Q-C9`
> (客人訂單頁 + 出貨通知信);實作 `trackingDisplay` 與它的測試**一行都沒刪**,
> 只是**目前零消費端**(理由寫在該函式 docstring)。
> **數法(2026-08-17 當場跑,可重跑)** —— 🔴 **本段原本寫「`git diff --stat` ⇒ 空輸出(零改動)」,
> 那條是假的**:它不帶 `--cached` ⇒ 量的是 unstaged;而落筆之後我又替該檔加了檔頭語境註解。
> **會動的版本**:
> `git diff --cached --stat -- apps/admin/src/lib/shipping/shipping-doc-dispatch.test.ts` ⇒ **`+7`**(全是 `//` 註解);
> 只加註解的證明:`git diff --cached -U0 -- <該檔> | grep -E '^[+-]' | grep -vE '^(\+\+\+|---)' | grep -vcE '^\+//'` ⇒ **`0`**;
> `git grep -c '追蹤碼' -- apps/admin/src/lib/shipping/shipping-doc-dispatch.test.ts` ⇒ **12**(加註解前 11);
> `git grep -c '追蹤碼' -- apps/admin/src/lib/shipping/shipment-actions.test.ts` ⇒ **3**(寫入守門,本片一個字沒動)。
> ⇒ **「一行都沒刪」要讀成「斷言與測試名一格沒改」,不是「檔案零 diff」。**
> ⚠️ 上面「凡是…一律作廢」那句是**我逐段讀本檔判的,沒有機械數法** ⇒ 引用時當**人工判讀**看待。
> ⚠️ **「非 `other` 且追蹤碼空白 ⇒ 拒絕標記出貨」那道寫入守門也沒有作廢** —— 丙說的是
> 「不走這張紙」,不是「不用追蹤碼」。
>
> **執行紀錄**:`docs/specs/2026-08-17-qc5-tracking-off-paper-decommission-list.md`(只列不改的作廢清單)。
> 🔴 **本檔內文零刪除** —— 下一個人會讀它,而「東西不見了」與「東西被刻意拿掉了」在檔案上長得一樣。
