...
> **v13 = v12 + §16 折關卡2 codex(4 findings,全折;含立案 #433)**
...
- 不寫 `admin_audit_log`(沿用本線慣例);稽核靠帳本列本身 ⇒ **actor 欄是承重的**,不是裝飾。
  ...
  ⚠️ 但 **actor 欄本身的可信度另有缺口 ⇒ 見 #433**(存在性驗證不是身分驗證)——
...
**RPC-actor(不在本片射程,已立案 #433)** — `p_actor` 是**存在性驗證不是身分驗證**:
...
⇒ 立案 **#433** + 寫進 apply 檢查表 §六「`actor` 欄不是稽核依據」。
