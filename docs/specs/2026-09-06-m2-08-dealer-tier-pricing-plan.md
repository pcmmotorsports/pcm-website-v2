...
**⚠️ 兩格我沒答, 而它們會改算式**
· **捨入**:`round()` 是**整單一次**還是逐列?§A 那三格訂正逐字說「整單捨入一次」是**推的**,
　而法條 §32-1 的分母是【一期】不是【一張單】⇒ 🔴 **這一格仍未拍板**, 後半-B 要先問。
· **⟦b4-SHIPFEETAXBASIS⟧(運費本身是含稅還是未稅輸入)**:順手答一句 ——
　拍板 3 說「稅基 = 小計 + 運費」⇒ **那個運費必須是【未稅】的數**, 否則會對含稅運費再課 5%。
　⇒ 📌 **而「今天存進去的運費是哪一種」我沒量** ⇒ 那一列的第一件事是去量它, 不是改算式。
