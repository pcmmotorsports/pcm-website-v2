...
> 🔴🔴 **F3(窄 R2,替身 Fable)· 測試容器生命週期 —— 這條管下面所有格,寫在前面**
>
> §5-15 三子格與 §5-15b ① **直呼同一批就要刪掉的舊路徑**(`SupabaseOrderAdapter.ts:663-736`)
> ⇒ 它們是**一次性格**,片組 B 上線後必須跟著走。
> 但 §5-15b ②③③'④ 與 §16c 的 C-1 / C-2 是**永久格**。
> ⚠️ **兩者若住在同一個檔 / 同一個 `describe`**,日後有人「把舊路徑那族整段刪掉」時會**連鍋端**,
> 四道閘同時消失而**零訊號**(測試不會紅——因為它們根本不再存在)。
> 這正是 memory `feedback_decouple-dependency-but-forgot-to-move-out-of-dying-container`
> 記的 08-09 D2b 實錘形狀:「讓未來刪不到」要同時做兩半 —— **解耦值來源 + 物理搬離**。
>
> ⇒ **本片硬性要求兩條**:
> ① **一次性格與永久格物理分離**(不同檔,或至少不同 `describe`,檔名/區塊名標明
>    `…-legacy-parity`(一次性)與 `…-search`(永久));
> ② **刪除那批時做「數格對帳」**:刪前刪後各數一次測試數,差額必須**恰好等於**一次性格的數量,
>    對不上就是連鍋端了。數法寫進片組 B 的 commit body,不寫「應該只刪到那幾格」。

15. 🔴 **能力等價性(最重要,三格)**:拿掉舊欄**之前**,證明新搜尋找得到舊欄找得到的單。
    **①訂單編號(新號)②訂單編號(舊號 `legacy_display_id`)③供應商單號**
    (舊路徑座標:①②=`SupabaseOrderAdapter.ts:762` 的 `.or()` 兩欄對稱查;③=`:663-736` 兩段式路徑)。
...
15b. 🔴🔴 **等價性的「刻意不等價」那一格**(Sean 2026-08-12 拍 **Q-347-B1=B**,主視窗 `B-487-A` 轉)。
    ...
    **舊欄找得到、新搜尋故意找不到** —— 這是**拍板要的結果,不是回歸**。
    ...
    | ③ | 同一次查詢 ⇒ 提示「可能有未付款的單被隱藏,勾起來再查一次」**在搜尋控制區內可見**(`within(<搜尋控制區>)` + `toBeVisible()`) | B 案的**交付物**;沒有它,B 案就退化成「客服看到共 0 筆而不知道為什麼」= 選項 C |
    | ③' | **負向格**:corpus 換成「查無、但**沒有**被隱藏的未付款單」⇒ 該提示**不出現** | 🔴 沒有這條,**提示改恆顯**(或渲染在別處/被 CSS 藏)時 ③ 照樣綠 = 恆真格 |
    ...
    **突變**(三發、分開跑、各自只紅一條):
    ⓐ 拿掉隱藏規則那條 `payment_status <> 'unpaid'` ⇒ **② 必紅**(變成命中 1 筆);
    ⓑ 拿掉提示的渲染條件、改成**恆顯** ⇒ **③' 必紅**(③ 不會紅 —— 這正是要 ③' 的理由);
    ⓒ 把提示搬出搜尋控制區(或加 `hidden`)⇒ **③ 必紅**(這一發打的是 `within` + 可見性那半)。

    🔴 **F1 折點紀錄(窄 R2,替身 Fable)**:原 ③ 只寫「畫面出現提示」= **字串存在斷言、無負向格**,
    與我自己在 §16c C-1③ 立的原則(「未截斷時提示不出現」)**互相矛盾** ——
    同一份 plan 裡,同一個病我在一格防了、在另一格沒防。折點 4(§5-17)只改到 A5 文案那格,
    **沒掃到這格**。⇒ 教訓不是「補一格」,是**改驗收寫法時要全檔掃同型格**,不是只改被點名那處。
...
16. **舊欄零殘留**:①兩個 flag 名全樹 grep 零命中(含 `page.tsx` / `order-filter-controls.tsx` /
    ...
    「整段消失」這四個字會把**三件不同性質的事**包成一件。實查 `SupabaseOrderAdapter.ts:663-736`,
    ...
    **降級可見性的唯一載體**(`B-494-A` §1 逐字)。我上一版把它當殭屍撤掉,**那是基於 C 的判斷,
    ...
    | ③ | 🔴 **corpus 釘死「命中數 == 上限 且 `truncated=false`」⇒ 該提示不出現**(不是隨便挑一個遠低於上限的數) | 沒有這條,提示恆顯也會過 = 恆真格;**釘在邊界**才擋得住下一格那個壞法 |

    🔴 **F2 折點(窄 R2,替身 Fable)——為什麼 ③ 一定要釘在「剛好觸頂」**:
    若 UI **不讀 `truncated`**、改用 `ids.length >= 上限` 這種啟發式,三條斷言**照樣全綠**,
    但「命中數剛好等於上限」(`truncated=false`,結果其實是完整的)時**提示會誤顯** ——
    而 RPC 那個 `LIMIT v_limit + 1` 的 `+1` **存在的唯一理由**就是分辨「剛好觸頂」與「超過」
    (`20260810120000_m4b_347_3a_…:183`),UI 層用啟發式等於**把 `+1` 的價值整個抹掉**。
    ⇒ ③ 的 corpus 必須**恰好等於上限**。造價作法:**用 `p_limit` 參數把上限縮小**
    (例如 `p_limit=3` 再造 3 筆),不要真的去造 100 筆。
    ...
    **突變(寫成不變式、不寫成語法)**:讓查詢**每個命中商品吐一列、而不是每張訂單吐一列**
    ⇒ **必紅**(長度變 3)。
    - 對 §4.3 的 `UNION` 形狀,具體作法 = `UNION` 改 `UNION ALL` 並拿掉外層 `SELECT DISTINCT`。
    - 🔴 **不要把突變寫死成「改 `UNION ALL`」**:實作若改採現行 RPC 那種
      `OR EXISTS (SELECT 1 FROM order_items …)` 形狀(`20260810120000_m4b_347_3a_…:158-179`),
      那個語法**根本不存在** ⇒ 突變無從施加 ⇒ **這格就再也證不出判別力**(格子還在、靶沒了)。
      該形狀下的等價突變 = 把 `EXISTS` 子查詢改成對 `order_items` 的 `JOIN`。
    - 出處:2026-08-12 codex 診斷 probe 提「`UNION → UNION ALL` 與實際查詢結構無關」。
      ⚠️ **那條 finding 對本 plan 不成立**(§4.3 骨架 `:219`/`:226`/`:231` 確實用 `UNION`、
      `:245` 有外層 `DISTINCT`),它成立只因為**我餵給它的事實只有現行 RPC、漏了新骨架**。
      但它指出的**耦合**是真的:突變綁語法 ⇒ 換實作形狀就失效。故照此改寫。
...
19. 🔴 **已下架/隱藏商品的訂單仍搜得到**(A4b;2026-08-12 補,**2026-08-12 關卡1 R1 must-fix ⑥ 重寫**)。
    ...
    **真正的風險面只有 #10 品牌**——它是 12 條裡**唯一**走活的商品表的分支
    ...
    🔴 **fixture 硬性要求(窄 R2 N2)**:那個**品牌名(needle)必須與同一張單的
    `product_snapshot->>'title'` 與 spec 文字「零交集」** —— 商品品名快照**經常內含品牌字樣**
    (例如品牌 `Rizoma` 而品名快照是「Rizoma 後照鏡」),一旦相交,**#8 品名那條會搭便車命中**
    ⇒ 就算 #10 品牌分支被改壞、整格照樣綠 = **突變不紅**。
    ⇒ 造 fixture 時品牌名與品名快照**用刻意不重疊的字串**,並在該格註明這個理由
    (不然下一代會把它「修」成看起來更自然的資料而悄悄弄壞這格)。
