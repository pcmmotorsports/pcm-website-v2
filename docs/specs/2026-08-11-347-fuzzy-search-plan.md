...
16. **舊欄零殘留**:①兩個 flag 名全樹 grep 零命中(含 `page.tsx` / `order-filter-controls.tsx` /
    ...
    **突變**:把 RPC 的 `LIMIT v_limit + 1` 改成 `LIMIT v_limit` ⇒ ①必紅
    ...
    ### 16c ✅ 裁定 = **Q-347-B2=C**(Sean 2026-08-12,主視窗 `B-487-A`→`B-491-A` 轉;推翻推薦案 A)

    **三項全保留**:ⅰ供應商名帶出 + ⅱ多家去重 = **在新搜尋上重建**(#338 交付物不退場);
    ⅲ 截斷 = **維持擲錯**(「不默默降級」原語意不動)。

    🔴 **A 案的殭屍驗收格已撤**:上面 §16b 尾端那格「`truncated` 旗標 UI 真的顯示出來」是
    **A 案(ⅲ 退場)的前提**,隨 A 案落選一併作廢 —— 不留在驗收清單裡假裝還有人要驗它。
    (它的替代品是 16c-③ 的擲錯驗收。)

    #### 🔴🔴 ⅲ「維持擲錯」與**兩條既有硬規則正面衝突** —— 這格**還不能定稿**
    ...
    ⏸ **⇒ ⅲ 的落法要主視窗/Sean 再裁一次(題已擬,見 `B-492-NOTE`)。**
    我不自選的理由:三條路各自推翻一條**已核准的東西**(PII 規則 / 關鍵字截斷設計 / 本 plan §4),
    這不是連動調整,是**三個拍板互相打架**——照 `feedback_decision-option-must-be-traced-to-end-state`,
    選項要走到終態才准上桌,而這三條的終態都不在我的射程內。
