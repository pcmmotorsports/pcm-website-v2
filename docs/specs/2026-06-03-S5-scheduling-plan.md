...
- 🔴 **現況(2026-07-22 起)= 台灣 12:30(`cron: '30 4 * * *'` UTC 04:30)**;改動理由見 `.github/workflows/rpm-sync.yml` 檔頭與報價單 repo 的 `docs/handoff/REQ-2026-07-22-網站庫同步時間.md`(原排程比報價單側爬蟲開跑還早 30 分鐘 → 每天同步到前一天的資料)。**以下為 2026-06-03 當時的拍板紀錄、不回改**:
...
3. 設好後到 **Actions 頁 → Supplier Daily Sync → Run workflow**(workflow `name:` 實際字面;舊稱 RPM Daily Sync 已不存在)(workflow_dispatch)手動跑一次驗證(或等隔天台灣 12:30 自動;2026-07-22 前為 03:00)。
