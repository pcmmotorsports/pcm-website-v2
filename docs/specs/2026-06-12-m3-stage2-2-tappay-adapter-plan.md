...
- **②-②b(confirm + 編排側)**:`IPaymentConfirmer` port + confirm 型別 + `PaymentConfirmError` 分類 + `PaymentConfirmerAdapter`(pg 直連 5432)+ `confirmPayment` use-case(孤兒單契約 outcome)+ composition root(受控 server subpath 注入)+ `package.json` 加 pg + master plan §3.2 webhook override 更新 + 測。
...
1. **連線端點直連 5432**(非 pooler):`PAYMENT_CONFIRMER_DB_URL` 由 Sean 設為**直連 5432**(`db.<ref>.supabase.co:5432`、非 6543 pooler)。理由:migration L30 已 4 次證 pooler 呼 SECURITY DEFINER 必斷(memory `pooled-mcp-set-role-secdef-terminates`);pg + transaction-mode pooler 另有 prepared-statement 陷阱。adapter 不解析/不改寫連線字串(直收注入),**plan 階段直接拍**=連線字串內容由 Sean 端保證直連、composition root 註解標明「須直連 5432 非 pooler」+ 上線前驗。
2. **PF-A happy-path flip**:RPC happy unpaid→paid 已 migration MCP C1-C8 實證(review-log L34「C1-C8 全綠:happy unpaid→paid」);adapter 單元測用 fake pg client 覆蓋「成功回 JSON→parse {confirmed,idempotent}」路徑(補上 NOT-FOUND 早退以外的成功路徑解碼)。**真 payment_confirmer pg 連線 round-trip 延 ②-③**(real connection + Sean 肉眼驗、review-log 已書面 deferred;MCP SET ROLE+SECDEF 必斷無法跑)。
...
- `packages/adapters/src/payment/PaymentConfirmerAdapter.ts`(新、`import 'server-only'`、`import { Client } from 'pg'`):`implements IPaymentConfirmer`。constructor 收 `connectionString` + 可選 `clientFactory`(預設 `(c)=>new Client({connectionString:c, connectionTimeoutMillis:8000})`、測試注 fake `PgClientLike`)。`confirm`:`connect`→`query('SELECT public.confirm_order_payment($1::uuid,$2::integer,$3::text) AS result',[orderId, amount.amount, recTradeId])`→parse rows[0].result→`{confirmed,idempotent}`;catch 分類(`err.code==='P0001'`→rejected 否則 unreachable)throw `PaymentConfirmError`;`finally end()`。
