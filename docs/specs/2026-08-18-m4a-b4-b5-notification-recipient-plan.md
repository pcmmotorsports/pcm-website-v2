...
### 🔴 R1 之後又進兩條(W9 補查,2026-08-18 收工前)

審查者**自認沒讀本 plan 指名的兩份真權威**(PRD / B-4 plan 檔頭)⇒ 它只審得了 plan vs code,
**審不了 plan vs 契約**。W9 去把那一格補起來,結果又出兩條 must-fix:

```
V1  plan:133 「缺列由 C-1 對帳補寄兜底」 —— 而 C-1 與 A-1 告警【都沒有實作】
    (PRD §6 gate #2/#3 自己寫「五訊號目前查無實作」;W9 grep ⇒ 0 實作命中)
    ⇒ F3 的嚴重度【上修】:不是「C-1 是唯一救濟」,是【目前沒有救濟】
V2  plan:58 vs PRD:87 —— PRD §3.2 **明文規定** NULL fallback 的形狀:
    訂單欄 NULL → customers.email → isSyntheticEmail 閘 → 落 skipped_no_real_email
    (機制已在 SupabaseEmailOutboxAdapter.ts:156-163)
    ⇒ 本 plan §3「全不合格 ⇒ 不 enqueue + 一行 log」是【對真權威的未申報偏離】
    ⇒ F13 從 consider 升級 —— 它背後站著 PRD 的規定,不只是「比較不可觀測」
```
📌 **FRAME-1(下一班派審查前先讀)**:**把真權威直接貼進 prompt,不要指望審查者自己去開檔。**
一份沒讀契約的審查,**它的 PASS 與 FAIL 都只涵蓋一半**。

⚠️ **`F15` 的事實面由 W9 取代了我的打回**:`2SQH2P` **不是三手口述**,是**兩筆各自落檔的 prod 查詢**
(`STATUS.md:684` + `docs/phase-1-backlog.md:13045`)。
✅ **但結論仍成立**:prod 簽章的**最後一次直接觀察是 07-19 的 8 參**,之後全是推論
⇒ **「吻合但未證實」**。§12 拆不拆閘 = **拍板題**,而**代價不對稱**(猜錯 ⇒ 結帳整條斷)。

📌 **本節的狀態要一直寫在這裡,直到 R2 PASS 為止**:
**R1 FAIL(12 條)+ W9 補查 2 條 = 14 條待折、R2 未跑。** 「我修了」不等於「它過了」。
