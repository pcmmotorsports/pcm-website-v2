...
--
--  🔴🔴 而【還有第二條路,取決於時間戳序】(2026-08-18 G6 補;G1 實測,我轉述)
--  ```
--  現在 supabase/migrations/ 裡最大的版本號 ⇒ 20260818190000（G6 當場量）
--  甲 命名 > 20260818190000 ⇒ 正常排最後 ⇒ 就是上面寫的那個後果（會被一起推走）
--  乙 命名 < 20260818190000 ⇒ 🔴 所有人的 db push 直接 rc=1，
--     直到有人加 --include-all 或把它移走
--     ⇒ 症狀不是「我的東西被推了」，是【別人推不動了，而錯誤訊息指著我的檔】
--  ```
--  📌 **實測(G1 21:0x 跑的,我沒重跑 ⇒ 標轉述)**:不加旗標 ⇒ rc=1 + 訊息【指名那支檔】
--     + 告訴你要加 --include-all;`grep -c 'up to date'` ⇒ 0;拿掉那支才回 up to date。
--  ⇒ **原本擔心的「安靜地不被選中、還印 up to date」不存在** —— 它吵、它指名、不做偷偷的事。
--  ✅ **「加了 --include-all 之後會發生什麼」2026-08-18T21:4x 已量**(G6 跑,拋棄式 PG,已收攤)
--  ```
--  測資：三支 migration，每一支只把自己的名字寫進 exec_log（serial id = 真實執行序）
--        先推 A(…000000) 與 C(…000002)，再放進 B(…000001)——時間戳【夾在中間】
--  世界一 不加 --include-all ⇒ rc=1
--         "Found local migration files to be inserted before the last migration on remote database."
--  世界二 加 --include-all   ⇒ rc=0，"Applying migration …000001_b.sql"（只跑 B 一支）
--  🔴 而【執行順序】：exec_log = 1:A / 2:C / 3:B   ← B 最後才執行
--     帳本 schema_migrations 按 version 排 ⇒ 000000 / 000001 / 000002 ← B 看起來【在中間】
--  ```
--  🔴🔴 **⇒ 帳本順序(按檔名)與實際執行順序【永久不一致】,而沒有任何地方記著真實順序**
--     (`schema_migrations` 只有 version,沒有 applied 的序)。
--  ⇒ **本檔的裁定:搬檔時【不得】用 `--include-all` 繞過那個 rc=1。**
--  🔴 **理由不是「它會炸」,是【它不炸】**(G6 的話,我採用):
--     rc=0、印 Finished、帳本看起來整齊,而**檔名順序 ≠ 執行順序這件事在任何輸出上都看不到**。
--     · 那支 SQL 若【依賴】後面某支建的東西 ⇒ 它會炸 —— **那反而是好的**
--     · 若它【不依賴】⇒ 它靜靜地留下一個永久錯位的帳本
--  ⇒ **正解是把檔名時間戳排到最大號之後(甲),不是加旗標。**
--  ⚠️ **限定**:本機 PG 17.10 + supabase CLI 2.98.1;正式庫 17.6
--     ⇒ 證的是【CLI 的行為】,而 CLI 版本相同。**我沒有重跑 G6 的實驗 ⇒ 標轉述。**
--  ⚠️ **順帶(未確認,而重跑的人會撞到)**:不加 `--debug` 時 CLI 對本機 PG 直接報
--     `tls error (server refused TLS connection)` 連不上;**加了 `--debug` 就連得上**(兩台叢集都重現)。
--     G6 沒追成因 ⇒ 重跑本實驗記得帶 `--debug`。
--  🔴 **所以搬檔【一定要先講一聲】** —— 乙那條其實比甲安全(它擋住而不是偷做),
--     **而它擋的是【別人】**:全隊在你搬完之後、到有人發現之前,都推不動。
--
--  ⇒ **硬順序(兩個條件都成立才可以搬)**:
