...
#       不給參數 ⇒ 用 supabase/migrations/20260828080000_m4b_b4views1_saved_order_views.sql
...
SQL=${1:-supabase/migrations/20260828080000_m4b_b4views1_saved_order_views.sql}
FAILED=0
...
psqlq -c "SELECT public.admin_create_saved_order_view('clerk','並發測試','q',NULL,false,'cc1','req-cc')" > /dev/null
...
B1=$("$PGBIN/psql" -h "$SOCK" -U postgres -d postgres -Atq \
      ...
      -c "SELECT public.admin_update_saved_order_view('clerk',$VID,'B 改的',NULL,NULL,NULL,'req-cc')" 2>&1)
case "$B1" in
...
APID=$!
sleep 1
B2=$(psqlq -c "SELECT public.admin_update_saved_order_view('clerk',$VID,'B 又改的',NULL,NULL,NULL,'req-cc')" 2>&1)
wait $APID 2>/dev/null
...
psqlq -c "SELECT public.admin_create_saved_order_view('clerk','刪的並發','q',NULL,false,'cc3','req-cc')" > /dev/null
...
B4=$("$PGBIN/psql" -h "$SOCK" -U postgres -d postgres -Atq \
      -c "SET statement_timeout='1500ms'" \
      -c "SELECT public.admin_delete_saved_order_view('clerk',$VID3,'req-cc')" 2>&1)
case "$B4" in
...
APID=$!
sleep 1
B5=$(psqlq -c "SELECT public.admin_delete_saved_order_view('clerk',$VID3,'req-cc')" 2>&1)
wait $APID 2>/dev/null
...
# ── 世界 6:🔴 `list` 那支也要鎖住 staff(codex 複審 R2-2)────────────────────
# 我修 F4 時只修了三支【有寫入】的, 想的是「有寫入才有 TOCTOU」——**而讀也會**:
# READ COMMITTED 下 list 先看到員工 active ⇒ 另一交易停用他並提交
# ⇒ 它的 RETURN QUERY 拿新 snapshot ⇒ **仍然把檢視回傳給一個已被停用的員工**。
# 判別力來源:A 鎖住 staff 那一列不放手 ⇒ B 的 list 必須【擋在那裡】
#   有 FOR SHARE ⇒ 拿不到共享鎖(A 持有排他)⇒ 逾時
#   沒有 FOR SHARE ⇒ 它根本不碰鎖 ⇒ 立刻回一堆列
( psqlq -c "BEGIN; SELECT 1 FROM public.staff WHERE id='clerk' FOR UPDATE; SELECT pg_sleep(4); COMMIT;" > /dev/null 2>&1 ) &
APID=$!
sleep 1
B6=$("$PGBIN/psql" -h "$SOCK" -U postgres -d postgres -Atq \
      -c "SET statement_timeout='1500ms'" \
      -c "SELECT count(*) FROM public.admin_list_saved_order_views('clerk')" 2>&1)
case "$B6" in
  *canceling*|*timeout*) echo "ok   W6 A 鎖住 staff 時 list 擋在那裡(它有 FOR SHARE)" ;;
  *) echo "FAIL W6 list 沒有被擋 —— 它立刻回了 [$B6] ⇒ 它沒有鎖 staff ⇒ 授權 TOCTOU 還在"; FAILED=1 ;;
esac
wait $APID 2>/dev/null

# ── 負對照:一個【本來就不該擋】的世界必須通得過 ─────────────────────────────
# 少了這一發, 一個「什麼都擋」的壞掉 harness 也會讓 W1/W2 全綠
psqlq -c "SELECT public.admin_create_saved_order_view('clerk','沒人搶的','q',NULL,false,'cc2','req-cc')" > /dev/null
...
B3=$("$PGBIN/psql" -h "$SOCK" -U postgres -d postgres -Atq \
      -c "SET statement_timeout='1500ms'" \
      -c "SELECT public.admin_update_saved_order_view('clerk',$VID2,'改好了',NULL,NULL,NULL,'req-cc')" 2>&1)
if [ "$B3" = "UPDATED" ]; then
...
[ $FAILED -eq 0 ] && echo "=== 並發:六個世界都對 ===" || echo "=== 並發:有世界不對 ==="
exit $FAILED
