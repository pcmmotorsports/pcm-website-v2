# RF1 Slice Plan — 退款金額+運費重算引擎(純函式)(2026-07-25;**v8 = 五輪審查 findings 全折入**)
...
> **v8(本版)= R3 的第 5 條(R3-5、#216 gate anchor)折入**——該條在 `ccad329` 之後的後續 commit 修掉,不在 RF1 本體 commit 內。
> 🔴 **共五輪審查、累計 31 must-fix + 9 nit 全數折入;另 1 條 codex 誤判經實查駁回**。
> 🔴 **R3 為 Sean 2026-07-25 特別授權破例**(平時 codex 每片上限 2 輪);**R3-1~R3-4 已折入但未再複審**——3 條為文件字面同步、1 條為測試隔離缺口(已補 14h + 突變 M20 驗證);**R3-5 由主對話親自核實 + 負向突變實測後折入**(見 §10g)。
...
## 10g. codex 關卡2 R3 findings 銷案(**Sean 授權破例第三輪;判 FAIL;4 must-fix 折入 `ccad329`、第 5 條折入後續 commit**)

> 🔴 **本輪實際跑了兩次(兩個並行 Claude 視窗各跑一次 codex,Sean 同一句指令貼進兩個視窗)。**
> 兩邊 prompt 不同:一邊只問「找新問題」→ 得 R3-1~R3-4(已折入 `ccad329`);
> 另一邊多問「**前兩輪的修法是否真的成立**」→ 同樣的 4 條 + 對前兩輪 6 項修法逐條裁決
> (**4 HOLDS / 2 BROKEN**:一條 BROKEN 即 R3-1;另一條=下表 **R3-5**,兩邊只有這一輪抓到)。
> ⇒ 教訓已落 memory `feedback_single-review-session-avoid-parallel`:**重疊審查線唯一有價值的情況是「問不同的問題」**;
> 問一樣的問題就是純重工(白付一次 codex),而且並行視窗會即時改動未 commit 檔、使審查前的凍結快照失效。
...
| **R3-5** 🆕 | **#216 drift gate 的 anchor 錯**:`latestCreateOrderShipping()` 挑的是「最新一支**命中運費 CASE regex** 的 migration」,不是「最新一支**定義 `create_order`** 的 migration」⇒ 最新那支若把 CASE 寫成 regex 抓不到的形狀,gate **靜默退回已被取代的舊 migration**、永遠綠 | ✅ 屬實(主對話親自核實,非照抄 codex):實查 `supabase/migrations/` 有 **7 支**定義 `create_order` 且當時 **7 支全部命中 regex** ⇒ 回退鏈很深且完全靜默;另有 1 支(`20260612150000`)只在 DROP/GRANT 提到函式名、並未定義它 ⇒ anchor regex 必須要求 `CREATE [OR REPLACE] FUNCTION` | anchor 改為「最新一支**定義** `create_order` 的檔」;該檔**抓不到 CASE 就直接紅**並附指示訊息(叫維護者更新 regex 或改對照欄位 DEFAULT),**不准回退舊檔**;加斷言 `sql.file === latestCreateOrderFile()` 防未來又被改回回退式搜尋。**折入後續 commit(不在 `ccad329` 內)** |

🔴 **誠實邊界**:R3-1~R3-4 **已折入但未再複審**(Sean 授權的破例僅一輪)。其中 R3-1 已用突變 M20 機械驗證修法有效;R3-2~R3-4 為文件字面,可由讀者直接對照。

🔴 **R3-5 的負向驗證(不只「改完是綠的」)**:臨時放一支時戳最新、定義 `create_order` 但運費寫成 `v_subtotal >= v_free_threshold THEN 0 ELSE v_home_fee END`(= **RF2a-0/RF2b 即將採用的形狀**)→ **新版 gate 4 紅、舊版 gate 6 綠**(舊版假綠實測落地,不是推論)。臨時檔與臨時測試檔跑完即刪、`git status --porcelain` 驗零留痕。
🔴 **RF2a-0 連動**:該片要把 gate 擴成「TS 常數 ↔ RPC CASE ↔ 欄位 DEFAULT」三方,**必須沿用此 anchor 語意**(對照最新定義檔、抓不到就紅),不要退回「掃到哪支算哪支」。

### 🔴 五輪審查的形狀(值得記入制度)

- **31 must-fix 中,質疑「核心公式 / 金額正確性 / Q6=B 架構」的:0 條。** 公式面自 R1 起未被動搖。
- **反覆出現的兩種形狀**:
  ...
  3. **守門本身對照錯了對象 = 1 次**(R3-5:gate 挑「命中 regex 的檔」而非「最新定義檔」⇒ 對照已作廢的定義、永遠綠)。⇒ 教訓:**守門不只要問「有沒有測」,還要問「它拿什麼當對照基準、那個基準會不會被靜默換掉」**;找不到基準時要**紅**,不是往舊資料回退。
