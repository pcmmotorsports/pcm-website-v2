...
> ⛔ **2026-08-29 訂正:上面那句【已被 Sean 的新拍板取代】。**
> 他逐字拍「**甲 依情況換一句**」⇒ 落地是【換掉】不是【多加一句】:
> `next === '/checkout'` 時整句換成結帳那一句, 原句在那個世界完全不出現。
> 🔴 **留著原句不刪** —— 它記錄的是當時的判斷, 而推翻它的是 Sean 本人、不是我們發現它錯了。
> ⇒ 落點 `apps/storefront/src/components/LoginPage.tsx` 的 `AUTH_SUB_CHECKOUT`。
