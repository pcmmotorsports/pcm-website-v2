...
> ✅ **已回寫母 plan `:296`**(2026-08-12,主視窗 `P-534-A` §3 裁定**歸 2f 片**):該處已加註解區塊,
> 含下表實測與三條要求(先剝註解 / 兩支不共用字串 / 消融優先)。本節保留完整推導。
...
✅ **已立案 #427**(主視窗 `P-534-A` §3 裁定;2026-08-12)。**修法排進沖銷片收尾的 record 輪**——
那輪本來就要動 `l5b2-2d-verify.sh` 19 處 + 重釘 LINE_TIP 錨 ⇒ 同批做、單一寫者=P 窗,不另派人。
~~舊字面:回報給主視窗決定派給誰~~
