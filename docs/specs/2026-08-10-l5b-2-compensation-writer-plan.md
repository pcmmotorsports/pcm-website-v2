...
### 🛑 2g 的作者:動手前先做這一件(2026-08-29 線C 搬進來,原文在別的檔裡)

> **這一節不是新規定** —— 它 2026-08-12 就寫好了,**而它住在 `scripts/l5b2-2e-rollback.sql` 的第 49 行註解裡**。
> 🔴 **2g 的作者不會去開一支 rollback 腳本** ⇒ 所以那份義務在原地等於不存在。**搬到這裡,原文留在原地。**

**必做**:把 2g 那支 writer 的**函式名**填進**兩處空陣列**:
```
...
```
**為什麼**(`l5b2-2e-rollback.sql:49-51` 逐字):
> (b) 🔴 **加一份顯式函式名單**(下方 `p.proname = ANY (ARRAY[...])`,目前空陣列)——
>   **2g 實作時必須把自己的函式名填進去**,那才是真正的不變量。
>   ⇒ 這條是**寫給 2g 作者的前置義務**;正規式只是它沒被履行時的網子。

🔴 **不填的後果不是「少一道保險」** —— 那道閘現在只剩**啟發式文字掃描**那一半,
而它**自陳**會靜默放行 5 種形狀(動態 SQL `EXECUTE format(...)` / owner 在 psql 直接 DML /
其他 schema 的函式 / 只 `UPDATE` 不新增列 / 把關鍵字拆開)。
⇒ **閘的職務是「2g writer 在庫時, 阻止 2e 或 2f 被單獨 rollback」** ——
命中不到就**放行**,而放行的正是本 plan `:718` 自陳「比不做更危險」的那個狀態。

⚠️ **而 2026-08-29 量到的現況要一起知道**:`2e` **2026-08-12 已 apply**、`2f` **2026-08-13 已 apply**、
**`2g` 不存在**(三種命名變體掃 `supabase/migrations/` 查無 —— **那是「我的檢查沒看到」,不是「它不存在」**)。
⇒ 「三支綁同一個 apply 批次」**實際執行是:兩支、分兩天、第三支沒寫**。
✅ **而這是安全的那一邊** —— 照本 plan `:718` 自己的話「真正的危險不是『2e 上了 2f 沒上』(**那還沒有新 sender**)」。
🔴 **⇒ 但它反過來說明一件事:2g 一落地, 那個批次的不變量【當場】從「無所謂」變成「必須成立」。**
