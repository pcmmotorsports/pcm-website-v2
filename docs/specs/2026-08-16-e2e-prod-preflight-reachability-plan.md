...
> # 🔴🔴 v3 結論:**不要照本 plan 實作。作者(I 窗)自己撤回推薦。**
>
> **codex `-s read-only` 對抗審查判 FAIL:7 must-fix + 1 nit。**
> **而其中一條直接證明我的核心查證是【假的】,不是不精確 —— 是相反。**
>
> ```
> 我在 v2 寫:「exit code 死在 playwright.prod.config.ts:73 的 && 鏈,沒有任何消費端讀它」
> codex 反駁:`&&` 會保留左側的非零碼
> 我實測:  sh -c 'exit 2' && echo x ; echo $?   ⇒  2       ← codex 對，我錯
> ```
> 🔴 **我拿一個【我沒實測的 shell 行為】當成「查證過的事實」寫進 plan,還用它推翻了 v1。**
> **v2 那句「比它說的更糟」讀起來像更深入的查證 —— 而它是一個更自信的錯誤。**
>
> **⇒ 但 v1 也不對(exit code 分兩種確實沒用),只是【理由不是我寫的那個】。**
> **兩版的結論方向都不對,而我兩次都給了「聽起來查過」的理由。**
>
> ---
>
> ## 為什麼撤回,不只是修一修(這是 YAGNI 判斷,不是被打退堂鼓)
>
> codex 的另外 6 條合起來說明:**這道探針【擋不住它想擋的,又擋得住不該擋的】。**
> ```
> 擋不住：貼成 service key ⇒ 照樣 200 通過（plan 自己寫明不驗 key 類型）
>         貼成別家主機   ⇒ 任意反向代理回 generic 404 也通過，證不了對方是 PostgREST
> 擋得住：WAF / 限流 / 路徑規則單獨擋 root path ⇒ 真實商品請求明明會成功，卻提前硬紅
>         429 / 408 / 5xx 沒有判讀規則 ⇒ Supabase 閘道抖一下照樣紅（丙案宣稱要解決的正是這個）
> ```
> 🔴 **而它宣稱要防的那個失敗,本來就會失敗** —— 鑰匙壞掉時 e2e 一樣紅,
> 只是**晚幾十秒、訊息難看一點**。**買到的是訊息品質,不是新的防護。**
> ⇒ **用「這片值不值得存在」那把尺量:不值得。**(§6 那句「不會讓任何現有綠變紅」
> 也被 codex 判為**宣稱大於事實** —— 有效 key 遇到 DNS/逾時/限流一樣轉紅。)
>
> ## 真的要做的話,唯一值得的形狀(不是本 plan)
> **不要探 root path,直接讓 e2e 的第一個測試去打【測試本來就要用的那條資料路徑】並斷言形狀。**
> 那能同時回答「連得上」「key 有效」「key 權限對」——**而且它不是新增探針,是把既有測試往前排。**
> ⇒ **範圍更小、失敗模式更少、沒有新的對外依賴。** 這需要另寫一份 plan,本檔不改寫成那個。
>
> ## 我的建議
> **關掉本片。** 那兩把新鑰匙的驗證,靠**下一次 push 時 `e2e-prod` 自己跑**就會知道
> (它本來就會連 Supabase)—— **那是零成本的,而且今天已經在等 Sean 推。**
>
> ---
>
> **以下為 v1/v2 原文,逐字保留備查。⚠️ 內含已被證實為假的查證(v2 §2),不要引用。**
>
> **狀態:~~等批~~ → **撤回,不建議實作**。命中鐵則 12④(CI 設定)⇒ 只提 plan、未動任何檔。**
