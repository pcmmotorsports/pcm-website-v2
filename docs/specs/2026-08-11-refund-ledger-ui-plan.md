...
②**已拍板**(2026-08-12 Sean,`S-087-A`):新區塊叫 **「系統自動退款」**、舊區塊照舊叫「退款帳本」。
...
**區塊標題 = 「系統自動退款」**(Sean 2026-08-12 拍板 Q1=A,`S-087-A`)。
⚠️ **U3 要留意的一個殘留歧義(不改舊區塊、只在新區塊處理)**:舊區塊叫「退款帳本」,
字面聽起來像「所有退款都在這」,但它其實**不含**本區塊這些。⇒ 新區塊的副標建議一句
「由系統自動發起的退款,與上方人工退款分開記錄」之類,把範圍講白;
**字面待 Sean 肉眼定稿**(結構鎖字不鎖),我不自己定稿。
...
| 情境 | 畫面 | 為什麼不能合併 |
|---|---|---|
| 這單沒有這類退款 | 「這張訂單沒有系統自動退款紀錄」(Q1=A 定案後填入) | 正常態、不需要做任何事 |
...
| **既有 `order_refunds` 帳本區塊(M-3 A7c)** | **完全不動它**(名字照舊「退款帳本」)。本片只在其下方新增「系統自動退款」區塊。命名交會點已由 Q1=A 收掉 |
...
```
Q1【已拍板=A,2026-08-12 Sean,`S-087-A`】訂單頁上會同時出現兩個退款區塊。
   **結果:新區塊叫「系統自動退款」,舊區塊照舊叫「退款帳本」。**
   落點兩處已同批改完、不分批:§3(區塊標題)與 §3-1(空狀態文案)。
   🔴 **我原本推薦的「補償退款紀錄」被 Sean 一句反問打回**(「補償是什麼意思」,`S-084-A` §3),
   病根記在這裡免得再犯:我的推薦理由逐字是「L5b 的來源就是補償退款線」=**拿內部線名當客人看的字**,
   正是本 plan §3 開頭自己引的準則(memory `project_admin-ux-operation-intuitiveness`)明文禁的。
   **引了準則不等於套用了準則**——我在 §3 寫下它、在 §7 違反它,中間隔了 100 行。
   (原選項 B「一般退款／補償退款」、C「合併成一張時間軸」未獲採用,留字備查。)
