-- ============================================================================
-- 🔴🔴 2026-08-26:**本檔已從 `supabase/migrations/` 移出來, 現在是【草稿】不是 migration。**
--
-- **為什麼移**(codex 2026-08-26 對抗審查片2 的 must-fix 7, 而它是對的):
--   這支與片2 兩支都還沒記進 migration history。有人跑一般的 `supabase db push` ⇒
--   **CLI 會套用【所有 pending migrations】, 而且依 timestamp 先跑這一支**
--   ⇒ 它會在【解除條件未成立】時被套上去, 而片2 的 rollback 也撤不掉它。
--   🔴 **那不是 SQL 物件衝突, 是部署序列衝突** —— 而**檔頭那段「暫緩」註解攔不住 `db push`。**
--   📌 **一個只寫在註解裡的「不要套我」, 對自動化工具是隱形的。**
--      要它真的不被套, 唯一的辦法是【讓它不在那個目錄裡】。
--
-- **它還有沒有用**:有。`customers` 是那 42 張的其中一張, 屬於**片 3**。
--   片3 開工時把本檔的內容搬回去(重取檔名時戳), 而**下面那三個解除條件要逐條重新確認**,
--   不要因為它已經寫好了就直接搬。
--
-- **要回去的話**:`git mv` 回 `supabase/migrations/`, 檔名時戳當天用 `date -u` 重取。
-- ============================================================================

-- ============================================================
-- 🛑🛑 【暫緩 · 現在不要 apply 這一支】(Sean 2026-08-26 選【乙】)
--
--   本檔的碼本身是好的(三輪 codex + 拋棄式 PG 兩側世界都跑過, 見檔尾),
--   **卡的是【範圍】不是【品質】。**
--
--   codex R3 換角度後開的框架問題, 而 Sean 同意:
--     補一條政策**只救 customers 一張**, 而 orders 那張同型的沒救
--     ⇒ 拿掉 BYPASSRLS 那天, 畫面會變成【客戶列表有、每個人訂單數 0】
--     ⇒ **比整頁空白更像真資料。空白會有人叫, 0 不會。**
--
--   ⇒ 先做的是那支**可重跑的全庫體檢**:
--       docs/probes/2026-08-26-q15-rls-service-role-audit.sql
--     它問的是【線上真實狀態】, 而本檔的立論建在【版控字面重播】上。
--     ⇒ **先看清楚全貌, 再決定這一支要不要貼、還是併進一片更大的。**
--
--   🔴 解除條件(三個都要):
--     ① 有人跑過那支體檢並把輸出貼回來
--     ② Sean 對「範圍 = 只補 customers / 補一批 / 換做法」重新拍板
--     ③ 本檔的 timestamp 當天用 `date` 重取(檔名的 20260826 會過期)
--   🔴 在那之前, **本檔不進 `supabase/APPLIED.tsv`, 也不要貼進 SQL Editor。**
-- ============================================================
--
...
--    那是一個看起來更嚴格的改動, 而它會讓客戶後台出問題,
--    ⛔ ~~整個空白~~ 🔴 2026-09-01 唯讀實測更正:**不是空白, 是一半有一半沒有** ——
--       商品那幾張有政策而 customers/orders 都缺 ⇒ 商品頁正常, 客戶與訂單是 0
--       ⇒ **0 比空白更難被發現**(空白會有人叫, 0 不會)。板上 ⟦b9-RLSHARDEN⟧。
