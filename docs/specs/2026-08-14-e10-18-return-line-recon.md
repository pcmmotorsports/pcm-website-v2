...
重跑(照上表宣告的範圍逐字):`grep -rn "退貨" supabase/migrations packages apps/admin/src apps/storefront/src`
⇒ **41 行**;`| grep -v '\.test\.'` ⇒ **27 行**;其中 **`apps/storefront/src` 佔 15 行(排除測試)**。
...
**只有「該品項全數到貨」才觸發 `nothing_cancellable`**。
