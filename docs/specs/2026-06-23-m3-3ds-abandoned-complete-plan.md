...
| UX | 3DS 開新分頁、原結帳頁保留鎖定;三種離開事件分流(§6.2 A/B/C)→ 回購物車(車不丟)→ 馬上重結帳零死路頁 |
...
客人放棄 → 原頁保留鎖定 → 點「重新付款」→ chargePaymentAction(server):
  ...
  ④ begin(新單) → 舊單已 released 退 cart dedup/user_in_flight → 佔鎖 → initiatePayment → 新分頁(§6)
...
## §6 塊A 前端 — 新分頁 3DS UX(round5 #4/#5)

### §6.1 popup 契約(round4 #4)
- charge-actions redirect outcome **必含 `{redirect:true, redirectUrl, orderId, displayId}`**(原頁據 orderId 輪詢)。
- 🔴 **popup 必在非同步付款操作前同步開啟**:click handler 內先 `const popup = window.open('about:blank','_blank')`;成功後、導向 TapPay 前設 **`popup.opener = null`**(安全;父頁仍持有 WindowProxy 供 `popup.closed`/`popup.close()`);再 await 拿 redirectUrl → `popup.location.href = redirectUrl`;**非 redirect 分支關閉空白 popup**;`popup===null`(被攔)→ §6.4 fallback。
- 原結帳頁 = `CheckoutPendingThreeDS`:依 **orderId** own-only 輪詢 payment-status(複用既有只讀 API;S2b 未推時 fail-closed 只讀不炸)。
- 🔴 **只有確認 `paid` 才清購物車**;**pending/callback 不得清除與原頁共享的 localStorage / cart**。

### §6.2 三種離開事件分流(round5 #4,禁混寫)
通知一律 **BroadcastChannel(同源)**;**不使用 postMessage**;**任何通知都必須再經 own-only payment-status API 複查才解鎖**。
- **A. TapPay 彈窗內「取消」**(沙盒實證 CANCEL):TapPay 回 CANCEL → `frontend_redirect_url` 回 callback → callback settleCharge → **markFailed(pending)** → callback 經 BroadcastChannel 通知原頁 → 原頁 own-only 複查後解鎖。
- **B. 使用者關閉彈窗 / 上一頁**:Record 可能維持 PENDING → `popup.closed` 觸發原頁 own-only 複查 → 原頁解鎖(**不宣稱 TapPay 已 CANCEL**);下次結帳由 preflight settle + release CAS 處理。
- **C. PCM 原頁「停止等待 / 回購物車」**:純前端 abort polling + 可 `popup.close()` 手上 handle;**不宣稱 TapPay 回 CANCEL、不呼 release RPC、不清購物車**;下次結帳由 preflight 處理。

### §6.3 TapPay 語意(round5 #5,釘死)
- 🔴 **`frontend_redirect_url` 才是「交易完成返回」**(成功/失敗落地頁)。
- 🔴 **`go_back_url` 只是部分銀行 3DS Error 頁的返回連結,不得當成通用 cancel/close callback**。
- 桌機 = 彈窗 3D + 原頁背景等(§6.1/§6.2);**真機 + production build 實測**(dev bundle 手機 onClick 卡死前例 [[reference_pcm-mobile-device-verify-dev-vs-prod]])。

### §6.4 popup-blocked / 手機整頁 fallback(round5 #5,完整定義)
整頁 fallback **無存活的原頁 listener**,故行為自足:
- `popup===null`(被攔)→ 同頁 `window.location.assign(redirectUrl)`。
- callback 已有 orderId → 自行 settle / own-only 查詢:
  - **paid** → 清車 + regenerate(換 cart key)+ 成功頁。
  - **failed** → **不清車**,顯示「回購物車 / 重新付款」CTA。
  - **pending** → **不清車**,可持續有界輪詢 + 提供「回購物車」CTA。
- 回購物車後再次付款**仍經 preflight**;**BroadcastChannel 在整頁 fallback 僅可選通知、非必要依賴**。
- 驗收涵蓋:桌機 popup、popup blocked、iOS/Android production build。
...
>
> 🔴 **codex 觸發(round6 五修正)**:命中鐵則 12(payment/雙扣/migration/RLS/GRANT)→ K2 必跑;**A1/A2/A3 三片皆改 payment redirect/callback/購物車清理/retry/popup security → 全部 codex K2 必跑 + `security_review_required` + `code_review_required`,不得標「例行跳」**。
>
...
>
> 🔴 **前端片(A1/A2/A3)共同要求(round6 五)**:開工前 **grep `design-reference` 字面**;**若 design-reference 無此付款狀態畫面 → 記錄 business override / open drift、等 Sean 批准,不得憑想像設計**;Manifest Impact;**TSX/CSS 同 slice**;smoke test;三綠(typecheck + lint + build);肉眼驗收涵蓋**桌機 popup / popup blocked / iOS / Android production build**。
...
| A1 | popup 開啟/opener=null 安全/CheckoutPendingThreeDS 等待元件 | 前端元件(grep design+TSX/CSS) | ✗ | 40m | 同步開 popup、opener=null、依 orderId 輪詢、只 paid 清車;桌機/blocked/iOS/Android production 肉眼驗 | smoke test + 三綠 + 真機 | **K2 + security + code review** | L2(文案) |
| A2 | 三種離開事件 A/B/C + BroadcastChannel + own-only 複查 | 前端 + callback(grep design+TSX/CSS) | ✗ | 45m | A/B/C 行為分流、通知後必複查才解鎖、C 不清車不呼 release;桌機/blocked/iOS/Android production 肉眼驗 | smoke test + 三綠 + 真機 | **K2 + security + code review** | L2(文案) |
| A3 | popup-blocked / 手機整頁 fallback | 前端 + callback(grep design+TSX/CSS) | ✗ | 40m | popup===null→整頁 assign、paid/failed/pending 清車規則、回購物車經 preflight;桌機/blocked/iOS/Android production 肉眼驗 | smoke test + 三綠 + 真機 | **K2 + security + code review** | L2(文案) |
...
- **R1/R2/R3 全完成**;**anomaly lifecycle、append-only event、unknown fail-closed、W1 報表 + 7 步 runbook、A1/A2/A3 真機(桌機/popup blocked/iOS/Android production)、B1 全部符合既有 gate**。
