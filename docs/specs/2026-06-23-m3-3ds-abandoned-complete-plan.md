...
45. 開啟後執行 production smoke / 監控;**發現付款 / 雙扣 / callback / sweeper / anomaly 異常 → 第一動作把 flag 關回 `false`**,再依 rollback/runbook 處理。
    - 🔴 **關 flag 縱深防護(#252 GAP2 靜默雙扣盲窗、Sean 2026-07-01 拍 B+A)**:begin-dedup + user_in_flight 兜底守住「立即重付」主場景,但 **GAP2(客人換裝置/清 cookie 致 cart_session 不同 + 隔 >10 分鐘 + 純 pending 兄弟單)** 的雙扣走 `pending→charged`、**不觸發 anomaly genesis(只認 `released→charged`)→ #250/W1 看不見 = 靜默盲區**(驗證 `docs/reviews/2026-07-01-m3-252-begin-dedup-fallback-verification.md`)。處置:
      - **計畫性關 flag**(非緊急):**先跑一輪 `settle-sweep` 收斂 in-flight pending attempt → 終態**(paid → begin 走 duplicate 攔住;failed → 無兄弟單可撞),**再關 flag**;壓縮盲窗(非零窗:sweep 後至 late-settle 間仍有殘窗)。
      - **緊急關 flag**:flag 優先關(安全第一),關後**立即跑 `settle-sweep`** 收斂殘留 in-flight,並人工比對「同 user 短窗多筆 paid」直到 **#256(pending-based 雙扣偵測、GAP2 治本 A)** 上線。
      - 🔴 治本(A)= **#256**:補「同 user 短窗多筆 paid」pending-based 雙扣偵測進 #250 → 讓 GAP2 也會告警;上線後本縱深條目降為次要防線。
