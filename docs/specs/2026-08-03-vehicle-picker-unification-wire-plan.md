...
| `:75` | ~~`輸入你的車輛`~~ ⇒ **現字面 `輸入您的車輛`** + `:74` `01 ·` | **不變**(容器規則);⚠️ 編號撞號是 **D 段**的事,見 §8 |

> 🔴🔴 **2026-08-18 字面更正(W2)—— 本檔【全部】含「你」的格子都受影響,不只上面那一格。**
> Sean 當日拍板「**顧客站用「您」/ 後台用「你」**」⇒ storefront UI 的「你」已整批換成「您」。
> **舊字面全部留痕不刪**(它們記錄的是當時的決定);**引用本檔去 grep code 時一律用「您」版本。**
>
> ⚠️ **特別針對 `§2.6` 那格寫著「🔴 逐字不動(§7 正確性紅線:錯誤 ✓ 比空白更糟)」的四態判定文案** ——
> 那條紅線管的是**語意**(不可以把「不確定」講成「✓ 適用」),**不是敬語**。
> 本次只換人稱、四個狀態各自說的事情一個字都沒有改;守門 `ProductFitmentCheck.test.tsx` 15 格同步更新後全綠。
> 🔴 **會被誤讀成違反紅線,所以寫在這裡** —— 只看那一格的人會以為有人動了不能動的東西。
>
> 受影響的格子(本檔內,實掃):`:123` `輸入你的車輛` / `:136` 四態判定文案 / `:138` `先前的車款連結已失效…`
> / `:160` `清單裡找不到你的車?`。掃法:`bash scripts/literal-sweep.sh '<舊字面>'`。
