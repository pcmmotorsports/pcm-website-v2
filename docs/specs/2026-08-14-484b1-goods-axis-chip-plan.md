...
> 🔴 **本片命中鐵則 8(跨 3+ 檔)⇒ 動手前要 Sean 批准。** 主視窗會與 `#486`、`#485`、B-3 一起批次送他。
...
## 2. 🔴 一個要 Sean 拍的定義題(**本片唯一的決策點**)

「未到貨」對應哪幾個貨品軸值?

| 案 | `goodsAxes` | 白話 | 現行 13 張單會篩出 |
|---|---|---|---|
| **甲(推薦)** | `['none', 'ordered']` | **還沒到貨的單全部** —— 含「還沒跟廠商訂」與「訂了還沒到」 | 12 + 0 = **12**(扣掉已取消 5 張 ⇒ **7**) |
| 乙 | `['ordered']` | **只有「訂了還沒到」** —— 還沒去訂的不算 | **0** |
...
🔴 **這一題我不自己拍。** 若 Sean 選乙,我照做並在 chip 旁留一行說明它排除了什麼。
