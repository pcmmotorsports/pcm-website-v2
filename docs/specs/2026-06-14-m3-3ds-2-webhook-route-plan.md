...
> 🔵 **2026-08-30 06:32 更正(`-e9` 量、`-48` 轉、`-b9` 落;舊句留著不刪)** → ✅ **已設, 且真的會擋。**
>   Sean **2026-08-23** 加規則、**2026-08-30 02:2x** 改成 429;API 逐格驗過
>   (規則數 **3→4** · version **7→8** · path pre `/api/checkout/tappay-notify/` · `limit=300` · `window=60`)。
> 🛑 **而【不會誤擋】不在這一格的字面裡** ⇒ **這一格滿足 ≠ 誤擋風險關掉。**
>   **未讀值**:Firewall → Traffic 的 `Rate Limited` 欄(02:2x 改 429 之後是不是 0)——
>   **只有 Sean 打得開**, 已在他的待辦第 1 位;板上對應列 =「TapPay 通知端點沒有限流」。
>   🔴 **誤擋的代價**:誤擋 = **客人付了款, 而我們這邊沒標成已付(錢)。**
> 🔴 **而本格【不動下面那些 checkbox】** —— 下方那句「**checkbox 刻意都不勾**」是**整體政策**,
>   不是逐格進度條;勾一格會讓下一個人以為其餘格也可以逐格勾。
>   (`-48` 原本轉「缺口 C ② 可勾」, 讀了這支檔之後**自己收回**。)
