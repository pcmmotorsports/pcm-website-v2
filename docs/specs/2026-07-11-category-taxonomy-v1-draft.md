...
>
> 🔴🔴 **件數全部是快照, 不是現況(2026-09-05 · 線 `-auth` 加, 板列 `⟦spec-CATCOUNTSTALE⟧`)**
> **本檔所有件數是 2026-07-11 的快照;2026-09-05 抽驗 7 個, 7 個全不同(-66% ~ +20%, 兩個方向都有)
> ⇒ 引用任何一個數字前當場重量。**
> 🔬 **重量法**:正式站打 `?category=<分類名>`, 抽 RSC 裡的 `total`(見板列 `⟦search-COUNTROLLUP⟧`)。
> 🎯 **為什麼不直接把表重算一遍**:那只是把舊快照換成**新快照**, 而下一個人一樣不知道它是快照。
>    ⇒ 📌 **要治的是「數字旁邊沒有時點」, 不是「數字是舊的」。**
> ⚠️ **這一句證不到什麼**:①只抽驗 **7 個**, 而 §3/§5 有 14 大類 / 50 子類 ⇒ **其餘 57 個沒量**
>    (7/7 全漂 ⇒ 我推其餘也漂了, **那是推論不是量測**)②**沒查是什麼讓它漂的**
>    (上架 / 下架 / 清空價格 / 分類重掛都會動它)③**分類體系那一半沒有被質疑** ——
>    過期的只有件數, **樹的形狀是 Sean 2026-07-11 拍板的, 照舊有效。**
