...
實查 `lib/vehicle-url.ts:38-40`:`parseVehicleFromUrl` 對「**沒有** `vehicle` 參數」與
「**有但解析不出來**」(`?vehicle=garbage`、id 不在 taxonomy)**回同一個 `null`** ——
函式簽章上就分不出這兩格。

⇒ 照 §3 的 `??` 寫法,`?vehicle=garbage` 會**落到鏡**、把 session 裡的車套上,
並由 `useVehicleUrlSync` 把 URL 改寫成那台車(= 壞參數被自我修復掉)。

**我認為這是合意的**(壞參數視同無車、而且順手把 URL 修乾淨),但它是**設計決定不是自然結果**,
所以寫在這裡讓 Sean 一起看,而不是埋在實作裡。
⚠️ 若要改成「壞參數就是壞參數、不套鏡」,得先讓 `parseVehicleFromUrl` 回得出第三種狀態
(PDP 那側有 `urlInvalid` 的先例,但那是呼叫端自己傳的 prop、不是這支函式的能力)——
**那會是另一片的體積**,不建議塞進本片。
...
8. 🔴 **URL 帶壞車參數 + 鏡有車 ⇒ 套鏡的車、並把 URL 改寫乾淨**(§5-6 的設計決定要被釘住,
...
4. **§5-6** 壞 URL 參數折進「無車」同一格(主視窗初核補;我認為合意,一併讓 Sean 看見)
...
## §8-附記 · 2026-09-20 回填:Sean 的答案找回來了(上面 §8 原文一字未動)

🔵 **這一節是 2026-09-20 加的**,上面 §8「待批(四件)」那段**原文保留**,
因為它記錄的是**當時**的狀態。答案回填在這裡,不覆蓋原文。

### ① 逐字(Sean 2026-08-08 晨)

> **Q28① 已拍 A|A|A|A(08-08 晨「依照建議」)**:批准動工/接受裸列表自動套車+URL 改寫/
> 無車帶頁碼回第 1 頁/壞參數視同無車。
...
- **原始落檔**:`eba1a26e6` 的 **`STATUS.md`**(repo 根目錄,不是 `docs/handoff/`)。
  該顆 commit 訊息逐字:`docs(docs): Q28① 拍板 A|A|A|A 落檔 — STATUS 翻已拍+D 新窗開工令 D-226-A 備妥 [M-4b]`,
  diff 只有一行(`STATUS.md | 2 +-`)。
- **今天在哪**:🔴 **`PROGRESS.md:1373`——它【在】今天的工作區,不是只活在 git 歷史裡。**
  ⚠️ **這一格更正我 2026-09-20 稍早的回報**:我當時說「今天工作區查無」,那是**錯的**,
  成因是我只 grep 了 `docs/`,而 `PROGRESS.md` 在 **repo 根目錄**。
  📌 教訓同族:**搜尋範圍比結論範圍窄的時候,那個 0 不是「沒有」,是「我沒看到」。**
- **今天的 `STATUS.md` 裡查無**(`grep -c 'Q28' STATUS.md` ⇒ `0`)—— 這一半是對的。

### ③ 成因:`e45f420a8`

`e45f420a8`「docs(docs): STATUS 93 換代批次行+主表二次精簡 **550→24 行**」。
🔵 而它**不是刪掉**那一行,是**搬家**:同一顆 commit 裡 `STATUS.md -535`、`PROGRESS.md +543`。
⇒ 所以「在 STATUS 找不到」與「不存在了」是兩件事,而我上一次把它們讀成同一件。

### ④ 🔴 射程(這一格是本節最重要的)

那一題**從頭到尾講的是「套 session 車 + 改寫網址」**,
**一個字都沒有提到列表頁該顯示幾件商品。**

原題四小題逐字範圍:①批不批 ②裸 `/products` 自動套車+網址變 `?vehicle=…`
③`?page=3` 無車+鏡有車時回不回第 1 頁 ④網址帶壞車參數「**視同無車、套 session 車+順手把網址修乾淨**」。

⇒ 📌 **「壞參數視同無車」講的是【車要不要套、網址要不要修】,不是【列表要撈多少件】。**
`docs/plans/2026-09-20-vehicle-url-silent-drop-plan.md` 那一題(選錯車靜默給全站商品)
**不在這個拍板的射程內** —— 它要另外問。
