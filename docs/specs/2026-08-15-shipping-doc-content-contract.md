...
> ⛔ **更正(`Q-C5`=丙,Sean 2026-08-17 逐字 `q3: 丙`)**:**追蹤碼那一項作廢** ——
> 出貨明細單不印追蹤碼,追蹤碼只走LINE／Email(`Q-C9`)。**金額與抬頭那兩項不受影響。**
> ⇒ 這道守門仍然要被改寫,但**理由少了一項**。原文零刪除。
...
> 🔴🔴 **更正(C-212 2026-08-16 深夜,Sean 拍板推翻)**:上面那句規格**已作廢**。
> **Sean 2026-08-16 逐字「改成: 日期 這兩個字就好」**(`Q-C6`)⇒ 那格叫 **`日期`**,兩個字,
> 不是「出貨日」、不是「列印日」。**「日期」不宣稱任何事,所以不需要分流,也不需要標籤共用的論證。**
> ⚠️ 原文自帶「不准刪、不准簡化」⇒ **本更正加在原句【之後】,原句一個字沒動。**
> 🔴 **第一版我把這個框插進原句【中間】,把它劈成兩半、還留下一行孤兒 `。**`** ——
> 而我在 commit body 裡逐字宣稱「加框不改字、原文保留供對照」。**那句宣稱是假的,R2 抓到。**
> 📎 落檔 `docs/specs/2026-08-16-shipping-doc-open-questions.md`(錨點 `Q-C6` · 已答)。
