...
1. **英文公司名的法定登記證** —— 有沒有句點是**事實問題**,repo 裡查不到登記證。
   ...
   **有句點版逐處(自己量,不轉抄)**:
   ...
   ⇒ **這題不是「哪個字對」,是「這兩個位置該不該一致」,而且現況比想像的亂。**
   **要 Sean 看登記證。** ⚠️ **登記證上到底有沒有句點 = 未確認。**
   數法(分母與逐行分類都寫出來,不只報一個 0):
   ```bash
   git grep -rniE '登記證|營利事業|統一發票證明|公司登記' -- docs apps packages \
     | grep -v '^design-reference'          # ⇒ 10 行
   printf '營利事業登記證' | grep -ciE '登記證|營利事業'   # 正向對照 ⇒ 1
   ```
   **那 10 行逐行看,沒有一行是登記證本身**:`site-config.ts:13` 講的是**中文**公司登記名、
   `legal-content.ts:9` 是「資訊由 Sean 口頭提供」的註記、其餘 8 行是
   `docs/specs/research-2026-07-25/` 底下的**稅務申報研究**(營所稅申報書表結構)。
   ⇒ **repo 內沒有任何一份文件能回答英文登記名有沒有句點。**
   在他看之前:
   紙上照 08-15 拍板印**有句點**;**`LEGAL_NAME_EN` 不准動**(改它連動
   `apps/storefront/src/lib/org-jsonld.ts:41` 與 `apps/storefront/src/data/legal-content.ts:90`
   服務條款正文 ⇒ 命中鐵則 12⑤ 對外不可回收)。
