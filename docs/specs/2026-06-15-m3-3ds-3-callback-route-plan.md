...
5. outcome → 變體(displayId 一律取自 §3 歸屬讀的 orders.display_id、不依賴 settleCharge):
     ...
     pending                    → CheckoutSuccess variant=processing  + <ClearCartOnSuccess/>(可能已扣款 → 清品項)
     no_attempt                 → CheckoutSuccess variant=processing  (🔴 必然未扣款 → **不清車**;codex K2 r1 must-fix、Sean A)
     failed                     → CheckoutSuccess variant=failed      (PCM 自撰 + 返回購物車 CTA;🔴 不清車)
```

- 🔴 **清車政策(Q4=A;codex K2 r1 must-fix 後最終版)**:`pending`(Record unreachable / auth_or_pending / unverified、鎖仍持、**可能已扣**)→ **清品項**(對齊既有 `useChargePayment` processing 清車、防殘車誘導重複扣款);`paid` → 清品項;**`no_attempt` → 不清車**(🔴 codex K2 r1:settleCharge 契約下 no_attempt ⟺ attempt=failed〔markFailed 僅 TapPay final-failed〕或從未建立 → **必然未扣款** → 清車零安全收益、卻摧毀失敗付款〔webhook vs redirect 競態〕的車);**`failed` → 不清車**(markFailed 已釋鎖、可立即重結帳;Sean D4)。即**清車 ⟺ 可能已扣款(paid/pending);未扣款態(no_attempt/failed)保留車**。
...
- [ ] 終態:paid→訂單已成立+清品項 / pending→處理中+清品項(可能已扣) / no_attempt→處理中+**不清車**(必然未扣款) / failed→付款未完成+不清車(CTA 返回購物車)。
...
- **執行側 K2(commit 前)**:PASS 0 must-fix;採納 2 consider → ① 應用層縱深 `.eq('customer_user_id', userId)`(§5.1 step3)② no_attempt 邊界文件化。
- **🔴 審查側獨立 K2 r1(amend ba56530、Sean 拍 A)**:逮真破口 —— `no_attempt` 一律清車**錯**。trace settle-charge.ts:no_attempt ⟺ attempt=failed(markFailed 僅 TapPay final-failed)或從未建立 → **必然未扣款** → A4「可能已扣款→清車」前提不成立、清它無安全收益卻摧毀失敗付款的車。**最小修**:no_attempt 改不清車(pending 仍清);改 §5.1 step5 + 清車政策 + Q4 字面對齊 A4 真意(=清車 ⟺ 可能已扣款)。failed-vs-never 顯真 failed 留讀路徑/3DS-4。
...
- **Q4 🔴 清車政策**(codex 關卡1 提、Sean 拍 A;**codex K2 r1 再修 no_attempt**):清車 ⟺ **可能已扣款** —— `paid`+`pending` 清品項(對齊既有 processing 清車、防可能已扣款殘車誘導重複扣款);`no_attempt`(必然未扣款)+`failed`(已釋鎖)**不清車**。〔關卡1 原 A 含 no_attempt 清車、K2 r1 trace settleCharge 證 no_attempt 必然未扣款 → 修為不清車、對齊 A4 真意。〕
