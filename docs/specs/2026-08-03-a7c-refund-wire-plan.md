...
3. **probe**:✅ **全案收案(08-03 20:0x 殘項兩發跑完、預測全中)**。消耗鍵=`6002`(**恆久、跨請款不重置、無 TTL** ⇒ 維持結論 2 且強化為「rotation-always 必要 + at-most-once 可依賴」);新鍵=`0`(正向鏈閉合;同發驗證 `amount` 6→5 與 `refunded_amount` 0→1 兩語意)。詳 §2.3b P6/P6a/P6b。**⇒ 原「RW2c 前不得開工 / RW2d 不開工」兩道 probe 閘門解除**;仍未證者=正式商戶請款時點(不寫進 UI)、併發 n=1。
