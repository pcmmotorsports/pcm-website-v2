...
⚠️ 而「已經有人用過的券, 還能不能改折抵金額」—— **本 spec 判【不能】**, 理由:
   ...
   ⇒ 🔴 **而這是我判的, 不是 Sean 拍的** —— 見 §5-1, 我列成待確認。
```
...
⚠️ 碼的產生方式 = 員工自己打(Q5甲 的配套)
   ⇒ 🔴 後台要提示「不要用連號或簡單的碼」—— 這是 `Q-新2甲` 的配套, 而 Q-新2 未答
   ⇒ **提示文字先不寫死**, 等他答完再定
```
...
```
5-1 🔴 「已經有人用過的券能不能改折抵金額」—— §3-2 我判【不能】, 而【那是我判的不是 Sean 拍的】
    ⇒ 它影響員工日常操作 ⇒ 建議併進下一批問他
5-2 🔴 `discount_type='percent'` 那一半 —— 卡在 Q-新1(進位規則, 是錢)
    而【欄位要建、UI 不給選】:
