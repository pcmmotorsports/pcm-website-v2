...
🛑 ~~折扣方式:本版【只有「折 NT$ N 元」一個選項】~~
   ⇒ **2026-08-29 標作廢:`a 甲`(四捨五入)已答 ⇒ `percent` 那一半【已解鎖】。**
   ⚠️ **而【已解鎖】不等於【已落地】** —— 兩個選項的 UI 到今天還沒有人寫。
   🔴 **而線4 當初刻意留著不改的理由【仍然成立, 保留】**:
      改 UI 選項會連動「折扣方式」那顆 select 的驗證與 RPC 參數形狀
      ⇒ 那屬實作片(2c), 要主視窗劃面之後一起做, 不是在規格上改一行字。
   ✅ 而「它仍然是一個 select 不是寫死」那一句**不受影響**
