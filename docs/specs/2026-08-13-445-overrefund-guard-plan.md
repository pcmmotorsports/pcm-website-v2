...
>
> v7 的整個主體建立在「一張單可以**同時**已付款且有取消紀錄」上。**經由 RPC,那不可能。**
>
> | 方向 | 擋在哪 | 逐字 |
> |---|---|---|
> | **取消端**:只能取消未付款、且零非-failed 刷卡嘗試的單 | `20260804180000_..._a8a1:199-203`<br>`20260805100000_..._a8a2:360-364`(兩處親讀比對) | `IF v_order.payment_status <> 'unpaid'::public.payment_status OR EXISTS (SELECT 1 FROM public.payment_charge_attempts a WHERE a.order_id = p_order_id AND a.status <> 'failed') THEN RAISE EXCEPTION` |
> | **付款端**:有任何取消紀錄就拒絕確認付款 | `confirm_order_payment` **三代都帶**:`20260804150000:93-97` / `20260810160000:403-404` / **`20260810170000:369-370`(時間戳最大=live)** | `IF v_order.cancelled_at IS NOT NULL OR EXISTS (SELECT 1 FROM public.order_cancellations c WHERE c.order_id = p_order_id) THEN RAISE EXCEPTION` |
>
> 🔴 `20260810160000:569-585` 還把付款端那道閘的**字面**釘進結構斷言(缺字面即 RAISE)⇒ **它是被守著的,不是順手寫的。**
>
> **`payment_status` 寫入點已數完**(量法 `grep -rn "SET payment_status" supabase/migrations/*.sql` = **6 行**):
> 1 行是註解 / 3 行是同一支 `confirm_order_payment` 的三代(全帶閘)/ 1 行是初代已被 REPLACE / 1 行是 `admin_finalize_order_refund` 的 `paid → refunded`(不製造 paid)。
> **應用層零繞過**:`grep -rn "payment_status" apps packages --include="*.ts" --include="*.tsx" | grep -iE "update\(|insert\(|upsert\("` → **exit 1 零命中**;
> `order_cancellation_items` 在 apps/packages 的 27 處命中**逐條看過全是讀取面**(SELECT 內嵌 / mapper / types / 測試),零 INSERT。
>
> ⚠️ **強度收斂(重要)**:正確說法是「**經由 RPC 互斥**」,**不是絕對不可能** ——
> `20260611120000:230` 自己就逐字寫著 service_role「可繞過 `confirm_order_payment` 直接 UPDATE orders SET payment_status='paid'」。
> 與 §8 第 1 條(繞過 RPC 直接 INSERT 擋不到)**同一層級**。
> ⚠️ 應用層那條掃描抓的是 supabase-js 的 `.update(/.insert(/.upsert(` 形狀,**手寫 SQL 字串掃不到**。
>
> ### ⇒ 對施工的指示
>
> **步 5 要求 `payment_status ∈ (paid, partiallyRefunded)`(`20260812170000:598`)⇒ 任何走得到
> `admin_initiate_order_refund` 的單,`Σ order_cancellation_items` 恆為 0**
> ⇒ **取消減項是 no-op**,卻買進一個 JOIN、第三處 NULL 源、5 格驗收、+60 分估時。
>
> | 標記 | 意思 |
> |---|---|
> | **🏁 已撤(Sean 2026-08-14 本人,逐字「q- 撤版: A」)** | 建立在假 BLOCKER 上、**不要實作**。內容原樣保留(誤判紀錄是這次最貴的資產)|
> | **✅ v8 保留** | 與取消無關、獨立成立 |
>
> **🏁 已撤的有**:§4-2 的減項 / §2-9 / §2-9a / §4-3a 第 3 點 / §5-445b-1 的 JOIN / 驗收格 **42-46** /
> §8 的 `部分取消`·`運費折扣`·`finalize 標籤` / §10 的 445b 上修 / backlog `#479`·`#480`(已改標 ⛔ 不可達)。
> 🔴 **`445c` 不在撤除之列** —— Sean 是在知道「**445c 獨立成立、照做**」的前提下撤的;
> 他當初拍 A 的目標(**員工一眼知道能退多少**)沒變也沒受影響,由 445c 承接。**不要連它一起撤。**
> **✅ 保留的有**:day-0 入口 fail-closed / 格 11 降級 / §4-5 逐列數更正 / §11 堆 A 標過期 / §4-5 代價句摺起 / §8 編號修 / **445c 的 disclosure** / 格 47 / 格 48 / §8 的 `部署順序`·`步 5 NULL-fail-open`·`guard IS NULL 死碼`。
> **445a-1 的 code(5 檔)完全不受影響** —— 純回傳碼管線,不碰式子。
>
> ### 🔴 這次誤判的形狀(進坑集,主視窗執筆)
>
> 我量了三件事、**三件全是真的**(`orders.total` 有 CHECK / 全樹零 `UPDATE ... SET total` / cancel 只寫三欄),
> **結論卻是假的** —— 我量的是**機制**,該問的是「**這條路走得到嗎**」。
> 🔴 同一天我用「**這格紅的時候是誰讓它紅的**」審了 41 格,
> 卻從沒對自己的 BLOCKER 問對稱那句:「**這個場景是誰讓它發生的**」。
> ⇒ **恆綠格 = 紅不起來的守門;假 BLOCKER = 發生不了的災難。同一個形狀的正反面,都是沒問可達性。**
> ⚠️ **A 軸(opus, fresh context)也沒抓到**,還把它標成「已量到」⇒ **四輪三個模型同一個盲點,不是誰粗心,是問題沒被問出來。**

---

> 🔴 **v7 的存在理由(⏸ 全段前提已被上面推翻,保留備查)—— 這一版改的是「守門式本身」,不是措辭。**
>
> **R4**(第四輪對抗審查;角度換成「拿驗收表反推」+「只審 v5→v6 delta」,兩軸都不是前三輪的框架)
> 抓到一條 **BLOCKER**:**部分取消不下修 `orders.total`** ⇒ 守門線 = 訂單**原始**總額,
> 而「該退的錢」已經少掉被取消的部分。1000 元的單取消 700 之後,守門式算出 1000、
> 445c 面板把 1000 顯示給員工看 ⇒ **多退 300 元真的出得去**。
> 【量到】`admin_cancel_order` 對 `orders` 只寫 `cancelled_at/cancelled_reason/updated_at`
> (`20260805100000:462-467`);全樹零 `UPDATE ... SET total`
> (量法 `grep -rniE "set[^;]*\btotal[[:space:]]*=" supabase/migrations/*.sql` = **0 行輸出**)。
>
> 🏁 **`Q-445-R4-1` = A**(Sean 2026-08-14,推翻 R 窗推薦的 B):**守門式扣掉已取消金額。**
> 他的理由逐字:「可以讓員工**一眼知道可以退剩下多少錢**比較重要…對於真的要部分退款的員工/我來說
> **非常重要**…目前畫面打開,要知道退多少其實**沒這麼直覺**。」
> ⇒ 連帶兩件:①**445c 不能砍**(那個數字是他要的東西本體)
> ②「一眼知道」= **打開就看得到**、不是按了才知道 ⇒ 445c 多一個 disclosure(§5-445c,**+30 分,成本不藏**)。
>
> ⚠️ **R4 另抓到 8 個恆綠格**(§6 裡「就算印通過也證不到 §1」的格),逐格處置見 §6 與 §14。
> 🔴 **v6→v7 差異短表在 §14**(給不讀 900 行的人看)。
>
> 2026-08-14 · R 窗 · **R1 codex FAIL(33)→ v2 → R2 換模型 FAIL(1 BLOCKER + 13 MF)→ v3 → R3 兩條換模型線 FAIL(26 MF)→ v4 折 22 ⏸ 5 → `Q-445-R3-1` 拍板 → v5/v6 → R4 兩軸 FAIL(BLOCKER 1 + 35 MF + 8 恆綠格)→ `Q-445-R4-1` 拍板 → 本版**
...
### 2-9 🆕(v7)取消那一側的事實 —— **它存的是數量,不是錢**

| 事實 | 依據 | 強度 |
|---|---|---|
| `order_cancellation_items.cancelled_quantity` 是 **`integer` 數量**,表上**沒有任何金額欄** | `20260730130000:223` 逐字 `cancelled_quantity integer NOT NULL`(親讀) | 【量到】 |
| 單價來源 = `order_items.unit_price`(結帳當下凍結) | `20260604120000:147` `unit_price integer NOT NULL CHECK (unit_price >= 0)`;`:149` `CHECK (line_total = unit_price * quantity)` | 【量到】 |
| ⇒ **已取消金額必須 JOIN `order_items` 算**,不能直接 SUM 一個欄 | 上兩列 | 【量到】 |
| `order_cancellations` 表頭**沒有 status/狀態欄** ⇒ 每一列都是既成事實,**不需要狀態過濾** | `20260730130000:68-102` 逐欄親讀 = `id / order_id / reason_code / reason_detail / idempotency_key / payload_hash`(+ 時間欄) | 【量到】 |
| 兩道複合 FK 夾住「明細品項 ∈ header 的訂單」⇒ 跨單串接進不來 | `20260730130000:233-236` | 【量到】 |

#### 2-9a 🔴 一個 A 案本身沒解決的問題:**`orders.total` 不等於 Σ 品項**

`orders.total = subtotal + shipping_fee − discount_total`(`20260604120000:26` 逐字 CHECK),
而 `subtotal = Σ line_total`(`:101` 逐字註解)。
⇒ **`總額 − Σ(取消數量 × 單價)` 在「整張單都取消」時不會等於 0**,而是 `shipping_fee − discount_total`。

| 情況 | 式子算出來 | 方向 |
|---|---|---|
| 全取消、有運費無折扣 | `guard = shipping_fee`(>0)| 允許把運費退掉 —— **可能正確,也可能該連運費一起退**(那要 `guard = total`) |
| 全取消、折扣 > 運費 | `guard < 0` | **任何退款都被擋死** ⇒ 太嚴 |

🔴 **這是業務題,不是技術題,我不自己選。** 本片先取**保守方向**(算出來多少就是多少、負數就擋),
理由:兩種偏差**都是 fail-closed**(擋住合法退款,不是放行超退),而 §1 要防的是後者。
⇒ **列入 §8 誠實邊界 + 開 backlog**,並在 §6 配一格把行為釘住,免得日後有人「順手」放寬。

🏁 **已立案 = backlog `#479`**(2026-08-14,號由主視窗集中發)。

```
Q-445-R4-2(不擋本片、可事後補;backlog #479):整張單取消時,運費與折扣算誰的?
A. 現況(本片採用)—— guard = total − Σ(取消數量×單價);全取消時剩 shipping_fee − discount_total,
   可能為負(此時任何退款被擋)。fail-closed,不會多退,但可能擋到「連運費一起退」這個合法動作。
B. 全取消時 guard = total —— 運費也可退。要多一條「是不是全取消」的判斷。
C. 運費永不退 —— guard 再減 shipping_fee。
⚠️ 這題不影響 §1(三個選項都不會讓 100 元退出 99999),所以不擋本片。
```
...
## 🏁 **`Q-445-R4-1` 已由 Sean 本人撤回(2026-08-14,逐字「q- 撤版: A」)—— 下面這個減項不實作**

**取消與付款經由 RPC 互斥** ⇒ 任何走得到本 RPC 的單(步 5 要 `payment_status ∈ (paid, partiallyRefunded)`,
`20260812170000:598`)`Σ order_cancellation_items` **恆為 0** ⇒ **這個減項是 no-op。**

🔴 **撤板時 Sean 知道的前提(不得誤讀成「445c 也撤了」)**:他當初拍 A 的目標是
「**員工一眼知道能退多少**」—— **那個目標沒變、也沒受影響**,因為 **445c 獨立成立、照做**。
**他是在知道 445c 保留的前提下撤的。**

⇒ **v8 的實際施工式子 = 回到 v6 那行**:`可退額 = 訂單總額 − GREATEST(TapPay 已退, 帳本佔額度合計)`

~~🏁 **v7(`Q-445-R4-1` = A)改寫成:**~~

~~**可退額 = (訂單總額 − 已取消金額) − GREATEST(TapPay 已退, 帳本佔額度合計)**~~

~~v6:可退額 = 訂單總額 − GREATEST(TapPay 已退, 帳本佔額度合計)~~
🔴 v6 那條的被減數是**原始**總額,部分取消之後它比「該退的錢」高 ⇒ R4 的 BLOCKER,見檔頭。
...
- 🆕 **已取消金額**(v7)= `Σ (oci.cancelled_quantity × oi.unit_price)`,
  **要 JOIN 才算得出來**(見 §2-9)。🔴 **也必須 `COALESCE(..., 0)`** —— 沒有取消紀錄的單,
  `SUM()` 對零列回 **NULL** 不是 0,漏了 COALESCE 就是把**沒取消過的單**全部擋死。
  與 §4-3a 第二處 NULL 源**同型、同一個坑在同一片裡出現第二次**。
  ⚠️【未量】「沒取消過的單佔多數」我沒有查正式庫的分佈 —— 但**不需要那個數字**:
  零取消列在結構上就是可達狀態(取消是選配動作),一發就夠了。
...
🔴 **445b 的一條硬要求(2026-08-14 關卡 2 codex 標 `unreachable?`,我判該留)**:
`remaining = 0` 且 `blocked_by = 'amount'` 這個組合**不得由 445b 吐出來** ——
表單只收正整數(`refund-section.tsx` 的 `pattern='[1-9][0-9]{0,9}'`),
上限是 0 時**沒有任何合法金額可以「降低後重送」**,而 `amount` 那句文案叫他做的正是這件事
⇒ 員工會照著做,然後一直失敗。
**445b 要嘛在 `guard = 0` 時回既有的 `REFUND_NOTHING_LEFT`(語意本來就是它),
要嘛新增第 4 個 `blocked_by` 值**(那會讓 `EXCEEDS_FAILURE_CODE` 的 `Record` 編譯期紅,是預期行為)。
⚠️ **codex 標它 `unreachable?` 是對的**:445b 還不存在,無法從實際分支證明這個配對會發生。
**我留著它不是因為證實了,是因為它是 445b 的設計約束** —— 寫在這裡讓寫 445b 的人一定會看到。

**步 6b 的式子:**

```
guard := (訂單總額 - COALESCE(已取消金額, 0))
         - GREATEST(p_record_refunded_before, COALESCE(帳本佔額度合計, 0))

   已取消金額 = (SELECT SUM(oci.cancelled_quantity * oi.unit_price)
                  FROM order_cancellation_items oci
                  JOIN order_items oi ON oi.id = oci.order_item_id
                 WHERE oci.order_id = <order>)        -- 🔴 零列回 NULL ⇒ 外面那層 COALESCE 不可省
...
🔴 **v7 補一條 §4-3a 的量具沒掃到、R4-C 軸換量具才看到的東西**(**不是**第三處 NULL 源,是同族的另一個形狀):
步 5 的兩道業務閘 `:595` `IF v_ps = 'refunded'` 與 `:598` `IF v_ps NOT IN ('paid','partiallyRefunded')`
**是 NULL-fail-open 結構** —— `v_ps` 若為 NULL,兩道**一起**靜默跳過(`NULL NOT IN (...)` 也是 NULL)。
今天不可達,靠的是 `orders.payment_status` 的 **NOT NULL**(`20260604120000:99` 逐字
`payment_status      payment_status     NOT NULL DEFAULT 'unpaid',`)—— **那個約束不是本片建的。**
⇒ 與 §4-2a 第 1 條(GREATEST 不低估靠別片的索引)**同型**,一併列進未陳述假設。
【量具:掃函式體 `:480-699` 全部 25 條 `IF`/`ELSIF`,不只掃 `v_frozen` 這個名字;
量法 `awk 'NR>=495 && NR<=699' <2f檔> | grep -n 'IF \|ELSIF '`】
...
3. 🆕 **(v7)`SUM(cancelled_quantity * unit_price)` 對零取消列回 NULL** —— **第三處,而且是本片自己新加的。**
   零取消列 = 沒有取消過的訂單。漏 COALESCE ⇒ `guard` 變 NULL ⇒ 走 `guard IS NULL` 分支
   ⇒ **沒取消過的單一筆都退不了**。**同一個坑在同一片裡出現第二次**(第 2 點是帳本、這點是取消)
   ⇒ §6 為它單獨配一格,不與第 2 點合併(合併就會出現「一個突變同時紅兩格、拿掉一個 COALESCE 卻沒有格子紅」)。
...
🔴 **v7 更正(R4-B MF#2:上一版這裡逐列數錯,而且錯在會誤導施工者的方向)。**
~~本表的第 2、3、5 列已重開;仍然成立的是第 1 列與第 4 列。~~ **實際上第 1-5 列全部重開:**

| 列 | v6 宣稱 | 實際 |
|---|---|---|
| 1 「UI 上界與守門線同源」硬條件 = 刪除 | 仍成立 | ❌ **重開** —— §5-445c 硬條件 1 逐字「顯示的必須是**完整守門式**的結果」= 同源要求回來了 |
| 2 445c 整片 = 不做 | 重開 | ✅ 重開 |
| 3 §6-23/24/25 = 刪除 | 重開 | ✅ 重開(換成新的 34-41 + v7 的 47/48)|
| 4 §8-5 窗口二 = 不存在 | 仍成立 | ❌ **重開** —— §8-5 現在逐字「窗口回來了」。**照 v6 這句施工的人會判「沒有窗口」,實際有** |
| 5 §10「同一工作段」硬約束 = 刪除 | 重開 | 🟡 半開:仍是「建議」不是硬約束 |
| 6 B5 說明文字 = 維持原狀 | 未提 | ✅ 維持(§8 另有劃界)|

**真正仍然成立的只有一句**:**表單本身不加 `max`、不做即時提示**(那才是 `Q-445-R3-1`=A 的內容)。
🔴 **兩個拍板不衝突,別讀成互相推翻**:`Q-445-R3-1`=A 講的是「**不要在輸入框上設限**」;
第三案 = A 講的是「**開面板時把數字顯示給他看**」。**一個是輸入限制,一個是資訊揭露。**

🔴🔴 **v7 刪除下面這段(R4-B MF#3)** —— 它與 §4-6「開面板就顯示」**直接互斥**,
而 §13 `:909` 早就自己標了作廢卻沒刪本文 ⇒ 兩種相反的實作都能引用本 plan 過關。
**現行事實:第三案 = A 之後,員工在按送出之前就看得到上限。**

~~🔴 **A 案付出的代價,寫在明處**:員工**要按一次送出**才知道上限。~~
<details><summary>(v5 原文,僅備查)</summary>
...
1. 新守門函式 `pcm_order_refund_guard_remaining(uuid, bigint)`(吃 order_id + record_refunded_before),
   **內部三個 `COALESCE(..., 0)`**(帳本合計 / 🆕 已取消金額 / §4-3a 列的兩處),
   🆕 **已取消金額要 JOIN `order_items` 取 `unit_price`**(§2-9:取消表只存數量)。
   ⚠️ 這支函式現在讀**三張表**(`orders` / `order_refunds` / `order_cancellation_items`+`order_items`)
   ⇒ ACL 與效能都要重看,不是「多加一個減項」而已。
...
| 檔 | 改什麼 |
|---|---|
| 🆕 **退款區塊的 disclosure(新元件)** | 🔴 **v7 新增,R4 抓到的**:今天**沒有「點開退款面板」這個事件** —— `refund-section.tsx:1` 是 `'use client'`,在 `order-detail.tsx:476-480` **直接掛載常駐**;全檔 4 個 `useState` 全是表單欄位,`grep -n "details\|summary\|dialog\|open" <該檔>` **零命中**。⇒ 觸發點要新做。🔴 **Sean 的驗收是「一眼知道」= 打開就看得到** ⇒ 這個數字**不能藏在收合裡**,disclosure 只用來界定「什麼時候去查」,不是「什麼時候給看」 |
| `refund-read.ts` | 新增一支包 `pcm_order_refund_guard_remaining(uuid, bigint)` 的 read 函式。🔴 **要自帶授權閘** —— 它是新的進入點,`refund-actions.ts:93` 的 `authorizeAdminMutation()` 管不到它(R4-B MF#8) |
...
### 🆕 v7 新增格(42-48;**刻意接在後面不重新編號** —— 1-41 已被 §11/§12/R4 報告大量按號引用)

> ## 🏁 **格 42-46 已撤(Sean 2026-08-14 撤 `Q-445-R4-1`)—— 不要實作**(v8)
> 五格全部驗「取消有沒有被扣掉」,而**取消與付款經由 RPC 互斥** ⇒ 這五格的 fixture
> (已付款 + 有取消列)**構造不出來** ⇒ 它們不是驗收格,是**五個恆綠格**。
> 🔴 諷刺點記在這裡:**本輪的角度就是恆綠獵人,而我親手加了五個恆綠格進去。**
> **✅ 格 47 / 48 不受影響**(47 去掉「部分取消」那個 fixture 條件後仍成立;48 是授權,與取消無關)。

42. ☐ 🔴 **取消扣得到**(`Q-445-R4-1`=A 的核心驗收 / R4 的 P1 缺格):
    單 `total=1000`、刷卡全額、帳本零列、Record 已退 0、**取消 700 元品項**
    ⇒ `guard` **= 300**(不是 1000);員工送 1000 ⇒ `REFUND_EXCEEDS_REMAINING`。
    **這格紅的時候是誰讓它紅的**:把式子的 `− COALESCE(已取消金額,0)` 拿掉。
43. ☐ 🔴 **零取消列的單不被擋死**(§4-3a 第三處 NULL 源;**這格若紅 = 沒取消過的單全退不了**):
    無任何 `order_cancellation_items` 列 ⇒ `guard` = 訂單總額 − TapPay 已退,**不是 NULL**。
    突變 = 拿掉**取消那一層**的 COALESCE(必須與格 10 分得開:格 10 的突變是拿掉**帳本**那層)。
44. ☐ 取消金額**用單價算不是用列數算**:一列 `cancelled_quantity=3`、`unit_price=100` ⇒ 扣 **300**。
    突變 = 寫成 `COUNT(*)` 或漏乘 `unit_price`。
45. ☐ **跨單不汙染**:A 單有取消、B 單沒有 ⇒ 算 B 單的 `guard` 時**不減 A 的取消額**
    (`oci.order_id` 條件寫錯就會踩到;兩道複合 FK 擋的是寫入,擋不了讀取寫錯 WHERE)。
46. ☐ **全取消 + 折扣 > 運費 ⇒ `guard` 為負 ⇒ 擋**(`Q-445-R4-2` 的現況行為,§2-9a)。
    🔴 **這格是把「已知偏嚴」釘住**,不是宣稱它是對的 —— 沒有這格,日後有人「順手」把負值 clamp 成 0 就是靜默放寬。
47. ☐ 🔴 **445c 的面板值 = 有扣取消的那條線**:造一張**同時有** Portal 場外已退、一筆 `manual_failed`、
    **與部分取消**的單 ⇒ 面板顯示值 == RPC 實際會擋的線。
    ⚠️ **這格取代格 34 的角色**:格 34 只比「面板值 == helper 回傳值」,而面板本來就是呼叫那支 helper
    ⇒ 它證的是**接線**,不是 helper 算對了(R4-B MF#6)。helper 本身的正確性由格 18/19/42/43/44 守。
48. ☐ 🔴 **445c 的新 read 有授權閘**:未授權身分帶別人的 `orderId` 打那支 read ⇒ 拒絕。
    (R4-B MF#8;`refund-actions.ts:93` 的 `authorizeAdminMutation()` 只守 action,不守新的 read。)

### 🔴 v7 對既有格的處置更正(R4 抓到 8 個恆綠格)

| 格 | R4 的判定 | v7 處置 |
|---|---|---|
| **11** | **恆綠。** `orders.total` NOT NULL + `:534` 已 RAISE 掉 NULL 參數 + 各層 COALESCE ⇒ **經 RPC `guard` 恆非 NULL**,刪掉 `guard IS NULL OR` 兩行也沒有任何輸入能讓它紅。而 plan `:614` 稱它是「本輪最重的負測」 | **降級**:改成「**直呼 helper** 的負測」,並在 §8 明寫 `guard IS NULL` 分支在 RPC 路徑是**死碼**(留著是 defence-in-depth,不是被驗過的守門)。🔴 正中 repo 教訓「負測構造不出來 = 先懷疑那條守門是 no-op」 |
| 8 | 恆綠(被格 9 嚴格蘊含)| 併進格 9,不單獨列 |
| 26 | 條件式恆綠(oracle 是 2f harness,而格 29 允許把它「世代凍結」)| 若選凍結,**445b 必須自帶**「advisory lock 仍在步 2 後步 3 前」的後置斷言(R4 的 P6) |
| 28 | 部分恆綠(10 處同步無單一 oracle)| 拆成「有 oracle 的」與「靠人核的」兩堆,後者明標**不是驗收格** |
| 29 / 31 / 38 | **程序格,不是驗收格**(「有沒有寫一段字」/ 別片的工作 / 「有沒有改測試檔」)| 移出 §6,改列進 §5 的施工步驟 |
| 37 | 條件式恆綠:只寫負向掃描,取值失敗時 `textContent=''` ⇒ 恆真(`refund-ledger-section.test.tsx:111-113` 自己記著這件事)| **沿用同一條正規式時必須連正向錨一起抄**,不能只抄 regex |
...
🔴 **先修編號**:上面那串實際是 `1 2 3 4 5 7 8 6 7`、`7` 出現兩次
(量法 `awk 'NR>=693 && NR<=727' <本檔> | grep -oE '^[0-9]+\.' | sort | uniq -d` → `7.`)。
⇒ 跨段引用「§8-7」指不到唯一一條。**v7 起一律用下面的具名條,不用號碼引用 §8。**

> 🔴🔴 **v8:下面前三條(`部分取消` / `finalize 標籤` / `運費折扣`)全部 🏁 已撤** ——
> 三條同一根因,而**取消與付款經由 RPC 互斥 ⇒ 三個場景都不可達**(檔頭逐字證據)。
> **`部分取消` 回到 v6 的狀態:守門線不扣取消量,而那不是缺口,因為走得到退款的單不會有取消列。**
> ✅ **後三條(`guard IS NULL 死碼` / `步 5 NULL-fail-open` / `部署順序`)獨立成立、保留。**

| 條 | 內容 | 變動 |
|---|---|---|
| ~~**`部分取消`**~~ 🏁 **已撤** | ~~守門線不扣取消量~~ | ~~🏁 **不再是誠實缺口** —— `Q-445-R4-1`=A 已補進式子(§4-2 / §6-42)~~ **v8:`Q-445-R4-1` 已由 Sean 撤回;本條不是缺口也不需補式子** |
| ~~**`finalize 標籤`**~~ 🏁 **已撤**<br>(backlog **`#480`** = ⛔ 不可達)| ~~🔴 **這個才是 v7 的新缺口**~~:`admin_finalize_order_refund` 步 7 用 `v_sum >= v_total` 決定 `payment_status` 標 `refunded` 還是 `partiallyRefunded`(`20260803150000:772-782`)。**它是狀態分類器,不是擋門**(R4 我原本寫「也拿 total 當上界」講強了,已更正)。⇒ 取消過的單退到「該退的錢」時,`v_sum < v_total` 仍成立 ⇒ **會被標成 `partiallyRefunded` 而不是 `refunded`**。**錢是對的,標籤是錯的。** ⚠️ **若不同批修,這條必須寫在明處** | 🆕 |
| ~~**`運費折扣`**~~ 🏁 **已撤**<br>(backlog **`#479`** = ⛔ 不可達)| ~~全取消時 `guard = shipping_fee − discount_total`,可能為負 ⇒ 擋掉合法退款(§2-9a、`Q-445-R4-2`)。**fail-closed,不會多退**~~ **v8:沒有取消減項就沒有這個殘值** | 🏁 已撤 |
| **`guard IS NULL 是死碼`** | 經由 RPC,`guard` 恆非 NULL(`orders.total` NOT NULL + `:534` RAISE + 三處 COALESCE)⇒ `guard IS NULL OR` 兩行**在 RPC 路徑上永遠不會觸發**。留著是 defence-in-depth,**不得宣稱它「被驗過」**(R4-A 格 11) | 🆕 |
| **`步 5 NULL-fail-open`** | 步 5 兩道業務閘(`:595`/`:598`)是 NULL-fail-open 結構,安全性寄生在**別片建的** `orders.payment_status NOT NULL` 上(§4-3a) | 🆕 |
| **`部署順序`** | 🔴 **445a 必須先 deploy,才准 apply 445b**。反過來:RPC 開始吐第 9 碼而 app 不認得 ⇒ `refund-repository.ts:211` 丟 `RefundCallerBugError` ⇒ **每一筆被擋下的超退在員工眼裡變成「系統呼叫異常」**,直接打穿 §0-E 第 2 條。既有 memory `feedback_app-layer-must-not-ship-before-migration-apply` 只管另一個方向(R4-A S3) | 🆕 |
...
| 片 | v6 估 | **v7 估** | 差在哪 |
|---|---|---|---|
| **445a-1**(回傳碼管線)| —(v6 沒拆)| **已完成,實耗約 45 分** | 5 檔 +236 −3;三綠全綠(vitest 456 檔 / 7644 過);含**三發突變實測**證明守門有牙 |
| **445a-2**(數字進訊息 + 可點錨點)| 併在 445a | **40-60 分** | 🔴 **必須排在 445b 之後** —— 445b 落地前寫數字 = 講一個比實際可退額高的數 |
| **445a-3**(刪短路 + `refund-ledger-section` 失敗顯示分支)| 併在 445a | **35-50 分** | 唯一有真行為變化的一片(新增 fail-closed 依賴)|
| **445b** | 未知,下界 180 分 | **未知,下界 240 分** | +60:取消那一側要 JOIN、多三格驗收(42/43/44/45/46)、helper 從讀兩張表變**讀四張表** ⇒ ACL 與效能要重看 |
| **445c** | 90-150 分 | **120-180 分** | **+30 = disclosure 新元件**(R4 抓到今天沒有「點開面板」這個事件)+ 新 read 的授權閘(格 48)|

🔴 **`Q-445-R4-1`=A 讓本片變大了,這個成本寫在明處、不藏。** Sean 是在知道「A 案要動守門式」之後拍的。

<details><summary>(v5 原估,僅備查)</summary>
...
### 堆 A —— ⚠️ **本表已過期,v7 更正在下方**(R4-B MF#4)

🔴 **下面這張表寫的「不存在 / 刪除」是 v5 的狀態,第三案 = A 之後已經不成立**;
表下那句「**若**第三案拍下去,A2/A3/A4 會重開」也是過期字面 —— **它已經拍了**。
⇒ **照這張表施工的人會把 445c 與 B4 的 oracle 改動刪掉,而 §12-A 把它們列為必做。** 兩處互相打架。

| 條 | v5 寫 | 🏁 v7 現況 |
|---|---|---|
| A2 §4-5 的論證 | 改寫成「不做前台上界」 | **部分作廢**:表單仍不加 `max`,但「不做 445c」已被推翻 |
| A3 445c 的資料來源 | 「不存在(片界回兩片)」 | ❌ **存在**:445c 復活,片界 = **三片**,資料來源 = 開面板時的 G0 + 守門函式(§4-6)|
| A4 §8-5 窗口二 | 「不存在」 | ❌ **存在**:445b apply 後、445c 上線前那段窗口回來了(§8-5)|
| B4 §6-24 措辭 oracle | 「該格刪除」 | ❌ **必做**:oracle 要先從**檔案粒度**改成**字串粒度**(§5-445c 硬條件 3)|
| B5 說明文字 | 維持原狀 | ✅ 不變(§8 `B5` 條劃界)|

<details><summary>(v5 原表,僅備查)</summary>
...
🏁 **上面那個「若」已經發生了(2026-08-14 第三案 = A)⇒ 以本節開頭的 v7 更正表為準。**
...
7. ☐ **Sean 批准 v6**(鐵則 8)
8. 🏁 **R4 已跑完(2026-08-14),而且是 plan 層的最後一輪**(主視窗判停令 `R-102-A`:
   `R1/R2/R3` 全部在同一層 = 讀散文找散文的錯,Sean 07-29 的判停條件「都在同一層打轉 = 方向問題」早已成立)。
   ⇒ **不會有 R5。金流對抗審查不降級,但改成 commit 前審 diff**(鐵則 12 關卡 2)——**審散文無終點,審 diff 有終點。**
   - **A 軸** adversarial-reviewer(opus)· 角度 = **恆綠獵人(審 §6 那 41 格,不審散文)** → FAIL 16 MF / 3 nit / **恆綠格 8**
   - **B 軸** codex `gpt-5.6-sol` 背景 · 角度 = **放寬獵人(只審 v5→v6 delta,那段從沒被審過)** → FAIL 19 MF / 6 nit
   - **C 軸** R 窗自量 · 重量 §4-3a 那個「只掃 `v_frozen` 一個名字」的量具 → 結論**沒翻案**,但補一條未陳述依賴
   - 三軸對前三輪自陳的三個交辦(①量具太窄 ②證據非空即佔會不會擋合法重試 ③E2 修法沒實測)逐條處理,見 §14
9. 🏁 **`Q-445-R4-1` = A**(2026-08-14):守門式扣掉已取消金額 ⇒ **本 v7 的主體**。
10. ☐ **`Q-445-R4-2`**(運費/折扣歸屬,§2-9a)—— **不擋本片**,三個選項都不影響 §1。
11. ☐ **Sean 批准 v7**(鐵則 8)。⚠️ 時機題已拍 **B**:等 A 案改進 plan 之後再一次請批
    ⇒ **就是現在這一版**。
12. ☐ 鐵則 12 **關卡 2**:445a-1 的 diff 送 codex 唯讀對抗審查(**未跑,commit 前必跑**)。

<details><summary>(v6 原文:R4 的交辦,僅備查)</summary>
...
## §14-0 🔴 **v7 → v8 差在哪**(**先讀這張,下面那張 v6→v7 有一半已作廢**)

| v7 的哪一條 | v8 判決 | 為什麼 |
|---|---|---|
| **①守門式減取消金額** | **🏁 已撤** | 取消與付款**經由 RPC 互斥** ⇒ 對走得到退款的單恆為 0 = **no-op**,卻買進 JOIN + 第三處 NULL 源 + 5 格 + 60 分 |
| ②取消側偵察(§2-9)| 🏁 已撤 | 同上 |
| ③第三處 NULL 源(§4-3a-3)| 🏁 已撤 | 隨減項消失 |
| ⑧新增格 42-46 | 🏁 已撤 | fixture 構造不出來 ⇒ **五個恆綠格** |
| ⑬估時 445b 240 | **回 180** | 同上 |
| ④**445c 的 disclosure** | ✅ **保留** | 「今天沒有『點開面板』事件」是 `refund-section.tsx` 的結構事實,**與取消無關**;而且這是 Sean 要的東西本體 |
| ⑤格 48 授權閘 | ✅ 保留 | 新進入點的授權 |
| ⑥**day-0 入口 fail-closed** | ✅ 保留 | 證據是 `tappay-reference.md:174` P4 + 表單預設 `full`,**一個字不牽涉取消** |
| ⑦格 11 降級 | ✅ 保留 | 減項撤掉後**更成立**(少一個 NULL 源)|
| ⑨§4-5 逐列數 / ⑩§11 堆 A / ⑫§8 編號 | ✅ 保留 | 純文件自相矛盾,獨立 |
| ⑪§8 改寫 | **拆開逐條判**:`部分取消`·`運費折扣`·`finalize 標籤` **🏁 已撤**(三條同一根因、同樣不可達);`部署順序`·`步 5 NULL-fail-open`·`guard IS NULL 死碼` **✅ 保留** | 不整段留也不整段丟 |
| backlog `#479` / `#480` | **改標 ⛔ 不可達,不刪** | 誤判經過寫進條目 |

**445a-1 的 5 檔 code 不受影響**(純回傳碼管線,不碰式子;三綠仍綠)。

---

## §14 🆕 **v6 → v7 差在哪**(短表;⚠️ **第 1/2/3/8/13 條已被 §14-0 作廢**)

| # | 改了什麼 | 為什麼 | 誰抓到的 |
|---|---|---|---|
| 1 | 🔴 **守門式多一個減項:扣掉已取消金額**<br>`(總額 − 已取消) − GREATEST(TapPay已退, 帳本佔額)` | 部分取消**不會**下修 `orders.total` ⇒ 1000 元的單取消 700 後,守門線仍是 1000、面板也顯示 1000 ⇒ **多退 300** | R4-A(**BLOCKER**);`Q-445-R4-1` = **A** |
| 2 | **取消金額要 JOIN 才算得出來** —— 取消表存的是**數量**(`cancelled_quantity`),不是錢 | `20260730130000:223` 親讀;單價在 `order_items.unit_price` | v7 偵察(§2-9)|
| 3 | 🔴 **第三處 NULL 源**:取消那層也要 `COALESCE(...,0)`,否則**沒取消過的單全部退不了** | 「零列 SUM 回 NULL」這個坑**在同一片裡出現第二次** | v7 自查(§4-3a)|
| 4 | **445c 不能砍,而且要多做一個 disclosure**(+30 分) | 今天**沒有「點開退款面板」這個事件**(`refund-section.tsx` 是常駐 client 元件,`grep details\|summary\|dialog` 零命中);Sean 要「**一眼知道**」 | R4-B MF#7 |
| 5 | **445c 的新 read 要自帶授權閘**(新增格 48) | 它是新的進入點,`refund-actions.ts:93` 的授權閘管不到 | R4-B MF#8 |
| 6 | 🔴 **day-0 不能只給文案,入口要 fail-closed** | 未請款的**全額退**實測**會成功**(`tappay-reference.md:174` P4:`0 Success`)⇒「必回 10024」只對**部分退**成立;而表單**預設就是全額退** ⇒ 員工看到「還不能退」按下去、**錢真的會動** | R4-B MF#10/#11 |
| 7 | **格 11 降級**:`guard IS NULL` 在 RPC 路徑是**死碼** | `orders.total` NOT NULL + `:534` RAISE + 三處 COALESCE ⇒ 刪掉那兩行也沒有輸入能讓它紅。而 v6 稱它「本輪最重的負測」 | R4-A(8 個恆綠格之一)|
| 8 | **新增 7 格驗收(42-48)、6 格處置更正** | 41 格裡**沒有一格提到取消** | R4-A 任務三 |
| 9 | **§4-5 表的逐列數改對**:v6 說「第 1、4 列仍成立」,實際**兩列都已被自己推翻** | 照 v6 施工的人會判「沒有窗口」,實際有 | R4-B MF#2 |
| 10 | **§11 堆 A 標為過期**、v5「員工要按一次才知道上限」那段**摺起來** | 同一份文件兩處互相打架,兩種相反實作都能引用它過關 | R4-B MF#3/#4 |
| 11 | **§8 改寫**:`部分取消` 不再是缺口;新增 `finalize 標籤`(**錢對、標籤錯**)/`運費折扣`/`guard IS NULL 死碼`/`步 5 NULL-fail-open`/**`445a 必須先 deploy 才准 apply 445b`** | 反向部署會讓每一筆被擋的超退變成「系統呼叫異常」 | R4-A S3 + R4-C |
| 12 | **§8 編號修**(實為 `1 2 3 4 5 7 8 6 7`、`7` 重複)⇒ 改用具名條 | 「§8-7」指不到唯一一條 | R4-B nit |
| 13 | **估時上修**:445b 下界 180→**240**、445c 90-150→**120-180** | 成本不藏 | — |

### 🔴 這一版**沒有**做的(明講,不是漏)

- **`Q-445-R4-2`**(全取消時運費/折扣算誰的)**未拍** —— 現況取保守值(可能為負 ⇒ 擋),
  **三個選項都不影響 §1**,所以不擋本片,但已配格 46 把行為釘住。
- **`finalize` 標籤偏差**未修 —— 若不同批修,它就是 §8 的新缺口(已寫在明處)。
- **445b 之後 harness 會紅幾格**仍**未量**(要拋棄式庫;§5-445b-0 就是為此設的)。
- **plan 全文的行號**未重新校對 —— v7 新增了約 200 行,**v6 以前寫下的行號引用會漂**;
  R4-A 已逐條複核過約 40 處並抓到兩個偏差(`:342` 四座標錯三個 / `:144` 的 `:657`→實際 `:653`),
  **那兩處我沒有回頭改 v6 本文**(改了會與 R4 報告對不上)⇒ 引用前一律以字面 anchor 為準,不信純行號。
