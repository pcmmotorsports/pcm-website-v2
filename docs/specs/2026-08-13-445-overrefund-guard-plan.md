...
> 版本史:v1 `5a7497c1` / v2 `e3573f8b` / v3 `188e185d` / v4 `ca858336` / **v5 = 本版**。
>
...
>
> 🏁 **v5 的存在理由**:`Q-445-R3-1` = **A(不做 445c)**,Sean 2026-08-14 拍板。
> v4 掛著的 5 條 ⏸ 全部定案(§4-1 / §4-5 / §5-445c / §6-445c / §8-5 / §10 / B5)。
> 🔴 **同時處理 Sean 拍板時附帶的一句假設**(逐字:「a(但是前台應該也會有還有多少可以退之類的金額對吧)」)
> —— 那句話的字面命中措辭鐵律,而它問的是一個**該被正面回答**的問題 ⇒ 新增 **§4-6** 專門回答。
>
...
⚠️ **本表的定案有一個未結的追問**:Sean 隨後問「點開退款面板時才去打 api 顯示金額」——
那不是 v4 §12-A 評估的 B 案(render 期每頁都打)。**若那個第三案被拍下去,本表的第 2、3、5 列會重開。**
主視窗正在回題,見 §4-6 開頭。**在拍板之前不要照本表把 445c 相關的字從 plan 裡刪乾淨。**
...
### 4-6 ⏸ **待 Sean 追問拍板 —— 本節只放已量到的事實,結論一律不寫**

Sean 拍 A 時附了一句:「a(但是前台應該也會有還有多少可以退之類的金額對吧)」,
隨後又追問:「那如果每次**點開退款項目後**,後台馬上去打 api 顯示正確金額,不就知道可以退多少錢了?」

🔴 **那個追問把題目換了,所以本節的結論不能先寫**:v4 §12-A 評估過的 B 案是
「**render 期**打 Record」= 每開一張訂單頁都打;他問的是「**點開退款面板時**才打」——**觸發點窄一個量級**。
⇒ 主視窗正在把這個第三案回給 Sean 拍。**在他答之前,本節不下任何「所以前台不能/不用顯示」的結論。**

以下 1-5 是**與拍板無關、已經量到的事實**,拍板之後直接拿來用:
...
⏸ **這個洞要怎麼補,取決於追問的拍板結果**,本節不寫結論。已知的兩個候選:
①送出被擋時回「本單目前可受理的退款上限:N 元」(§4-4,已在 445a 範圍內);
②點開退款面板時打一次 Record(Sean 追問的第三案,片界影響未評估)。
...
| 表單填寫中 | ⏸ **待追問拍板**(A 案下是「沒有上限提示」;第三案下這裡會有一個數字)| ⏸ | ⏸ 若顯示的數字來源是 Record 或完整守門函式,§2-7 那條禁令的**理由**不適用(見本節第 2 點),但要另外決定它叫什麼 |
...
### ~~445c~~ —— 🏁 **不做**(`Q-445-R3-1` = A,Sean 2026-08-14)

片界 = **445a + 445b 兩片**。445c 的四個檔案改動全部取消,理由與代價見 §4-5 / §4-6。
...
### ~~445c~~ —— 🏁 **不做**(v3/v4 的 §6-23/24/25 三格刪除,見 §4-5)
...
5. 🏁 **兩片之間只剩一個窗口**(`Q-445-R3-1`=A 之後,原本的窗口二不存在)——
   ...
   · ~~445b apply 之後、445c 上線前~~ **沒有 445c ⇒ 沒有這個窗口**。
   · ✅ **已證實的好消息**:步 6 / veto / 步 6b 全在步 8 INSERT 之前(`20260812170000:605-657`)
     ⇒ 被擋 = **零寫入、無半寫狀態、無資料面回退窗口**。【量到】
7. 🔴 **員工在按送出之前看不到本片那條守門線**(A 案的已知代價,§4-5 / §4-6)。
   訂單頁上那個「帳本未登記額」是**另一條、比較寬的**式子(只算本帳本、不含 Portal 場外退),
   而且**第一筆退款時它根本不顯示**(`refund-ledger-section.tsx:59` 零列早退)。
   ⇒ 本片**不得**在任何地方宣稱「員工事先看得到可退上限」。
...
- ~~445c~~:**不做**(`Q-445-R3-1`=A)⇒ v4 的「445b 與 445c 必須同一工作段完成」硬約束**刪除**,445b 可獨立排期。
...
### §12-A 🏁 `Q-445-R3-1` = **A(不做 445c)**,Sean 2026-08-14 拍板 —— 5 條已定案(§11 堆 A)

⏸ **但有一個未結的追問**:Sean 隨後問「每次**點開退款項目後**,後台馬上去打 api 顯示正確金額」——
那是**第三案**,觸發點比下表的 B 案(render 期每頁都打)窄一個量級。主視窗正在回題。
**在他答之前,下表只當歷史紀錄讀,不要照它刪東西。** 追問要處理的事實已整理在 §4-6(含第 3b 點:
Record 只給得出守門式 `GREATEST` 的前半,帳本佔額度那半 Record 看不到)。
...
6. ⏸ **Sean 的追問未答**:「點開退款項目後,後台馬上去打 api 顯示正確金額」——
   主視窗正在回題。**答案可能讓 A2/A3/A4 重開**(§4-6 開頭、§12-A)。**在他答之前 §4-6 不寫結論。**
7. ☐ **Sean 批准 v5**(鐵則 8)
...
| **(v5)** A 案「員工要按一次才知道上限」是唯一代價 | **🔴【推的·且已被追問動搖】** 沒有實測操作;Sean 的第三案可能讓這句失效 ⇒ §4-6 不下結論 |
