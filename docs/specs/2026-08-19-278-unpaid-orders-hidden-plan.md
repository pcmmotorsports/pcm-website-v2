...
```
> ## ⛔ 2026-08-19 G2 **撤回下面整段 —— 那個「矛盾」不成立**(先讀這段再讀下面)
> ```
> 主視窗當場 grep MAIN-055 的原始輸出：
>   :606  被藏起來的 8 筆：unpaid 3 筆 + 已取消 5 筆
>   :672  未收未定（unpaid）3 筆：Z6QDV9 / WCYCW5 / 2SQH2P
>   :673  已取消             5 筆：9G5Y6V / HR92W9 / F5SP3R / F9Q4DQ / 7F4TF8
> ```
> ⇒ 🔴 **`8 = 3 unpaid + 5 已取消`,不是 `8 = unpaid 數`** ⇒ **沒有矛盾。**
> ⇒ ✅ **「那 3 筆全是自己人」仍然成立**(三筆逐列具名),`§N-1` 的依據**沒有被推翻**。
>
> 📌 **我錯在哪(留著,因為它是本檔主題的又一格)**:我把兩個數字放在一起對了,
> **而漏了第三個 —— 那個拆分。** ⇒ 🔴 **對帳的失敗方式不只是「沒對」,還有「對了而分母不完整」。**
> ⚠️ 而成因不全在我:**那個拆分只活在 `MAIN-055` 裡,本檔引的「差 8 筆」不帶拆分**
> ⇒ **同一個數字、兩份載體,一份帶拆分一份不帶** —— 而只讀不帶的那份,推得出一個假矛盾。
>
> ### 🟡 而它撈到一個【真的、還沒人查】的東西(結論不是那個矛盾)
> ```
> :830-831 那顆開關藏的是 channel=tappay 【且】 status=unpaid
> 而被藏的 8 筆裡有 5 筆是【已取消】 ⇒ 🔴 那一句 code 解釋不了它們
> 兩種可能（不猜）：①另有一道過濾在藏已取消的（沒有人查過）
>                  ②或「加參數 ⇒ 18 筆」同時放行兩類（:830-831 只是其中一道）
> ```
> ⇒ **主視窗 2026-08-19 裁:不查。** 理由=**不影響本片的答案**(那 5 筆是已取消、不是等付款的客人),
>   且 Sean 明天的動線不會撞到它。⇒ **記在這裡,下一班有空再看。**
>
> ---
>
> ### ~~🔴🔴 2026-08-19 G2 自查:這個 8 與本檔下面那個 3,算術上不可能同時成立~~(**已撤回,見上**)
