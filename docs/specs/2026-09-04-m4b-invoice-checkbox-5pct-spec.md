...
· **實作** = `supabase/migrations/20260904224500_m4b_invoice_requested_false_is_final.sql`(**未 apply, 等他貼**)
...
· 🔬 拋棄式 PG 17.10 實跑**七格全過**:世界一 / 重貼被前置閘擋(**而訊息說得出「你已經貼過了, 不用做任何事」**)/
  零寫入 / `false→true` 被擋 / `true→false` 放行 / 改別欄不受影響 / replica 仍被擋 / 排在後面的 `BEFORE` trigger 偷改仍被擋。

### 🔴🔴 **而這道鎖守的是【紀錄】, 不是【結果】—— 兩件事要分開讀**
(2026-09-04 codex 對抗審查兩條 must-fix, 本窗逐條驗證屬實)
```
✅ 它守得住   「決定不開發票」這個【紀錄】不會被人翻回「要開」
🔴 它守不住   一張 invoice_requested = false 的單, 【仍然開得出發票】
```
🔬 **兩條既有的路, 都不讀那一欄**:
· **後台 workflow RPC** 可以把 `invoice_status` 改成 `issued` 並填發票號碼與金額
  (該支掃 `invoice_status`/`invoice_number`/`invoice_amount` ⇒ **45 處**;掃 `invoice_requested` ⇒ **0**)
· **`record_pending_invoice()`** 付款後只檢查 `payment_status='paid'`
  (該支掃 `invoice_requested` ⇒ **0**)⇒ 仍會替 `false` 的單建【待開票紀錄】
⇒ 🎯 **兩條路上旗標都維持 `false` ⇒ 那道 trigger 完全不會叫。**

📌 **⇒ 而這是【範圍題】不是 bug**:「守住那個決定的**紀錄**」與「守住那個決定的**結果**」是兩件事,
而 Sean 批的字面(「事後把不開改成開了 ⇒ 擋下」)**兩種讀法都成立**。
🛑 **⇒ 已端他(主視窗 `-94` 推薦【擴】, 理由:那 5% 跟著那個勾走 ⇒ 開了票價錢就不對)。**
⚠️ **⇒ 在他答之前, 不要把這支 migration 讀成「不會開出發票」** —— 它沒有那個作用。

### 🔴 而第 2 步(手動建單的勾選)之前, **還有四處非改不可**
```
