...
> **狀態:Sean 2026-07-23 已批准開工(「做」)。S1a-S2 已收工(S2 = code+branch 驗證、待 Sean db push,見 §5 / slice plan `docs/specs/2026-07-23-m3-s2-sweeper-pgcron-plan.md`);S3 以後未動。仍不 push;碰 DB/flag/env 一律停下交 Sean 手動。**
...
S2(③)sweeper 搬 pg_cron〔建設,flag 留 S4〕+ anomaly-alert 備妥  ← DB/平台；安全網
...
### S2(③)sweeper 搬 Supabase pg_cron + anomaly-alert〔建設;flag 留 S4〕

> 🔴 **2026-07-23 Q1=A 更正 + 已實作**:S2 slice 真權威 = `docs/specs/2026-07-23-m3-s2-sweeper-pgcron-plan.md`。**S2 只建 pg_cron 基礎設施 + Supabase branch 實跑驗證;flag(`CRON_SWEEPER_ENABLED`/`ANOMALY_ALERT_ENABLED`)+ Resend/LINE 告警 env 全留 S4 一起開**(下方「打開/實發」為原始設計字面、已被 Q1=A 取代)。實測 postgres 非 superuser → **不 REVOKE net/cron/vault**(見 slice plan §11)。

**為什麼**:Vercel 現況每日一次(且方案有頻率上限)→ 黑洞漏 webhook 時最久卡一天;Q4=A 改 pg_cron 分鐘級。Q5 一起備妥雙扣告警。
**改什麼**:
...
- 🔴 **Q1=A:S2 不開 flag** —— `CRON_SWEEPER_ENABLED`/`ANOMALY_ALERT_ENABLED` + Resend/LINE 告警 env 全留 **S4**(否則 ENABLED=true 缺管道會 fail-closed throw)。
...
**驗收(Q1=A 更正)**:S2 = pg_cron 觸發 → route flag-off **200 no-op** 連通驗證(net._http_response 200 + `{skipped:'sweeper_disabled'}`;branch + prod runbook ⑥ 6h 內)。**真掃描 stuck attempt/webhook inbox + 告警實發留 S4**(flag-on 後)。
