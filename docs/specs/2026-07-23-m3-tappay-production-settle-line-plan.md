...
| g | migration `20260624120001`(find_active_sibling_own)、`20260621120000`(claim_order_poll_settle)已 db push | Sean | 需確認正式站已 apply |
...
```
S1(②)超時出口 + by-cartSessionId 反查            ← code + 1 migration；先做、四層審
...
**改什麼**:
- `useChargePayment.tsx`:`chargePaymentAction` 呼叫加 client 逾時(Promise.race,值待定=見決策附註)。逾時 → 先 best-effort 呼新 `reconcileCartSession`(短逾時);拿到 paid/failed → 顯真實終態;否則 → 落 `unknown` 終態(現有政策:clear + 保留 cart_session_id + 不釋鎖 + 勿重複付款文案)。**overlay 因離開 `submitting` 自動掀開 → 客人有出口。**
- 新 server action `reconcileCartSession(cartSessionId)`:→ 新 by-cartSessionId 節流 RPC → `find_active_sibling_own`(own-only auth.uid())取 orderId → `settleCharge`(重用、零新寫入)→ 回 paid/failed/pending。可同時掛在 `unknown` 態下的「查詢付款結果」按鈕(節流)。
- 新 migration:by-cartSessionId 節流 RPC(比照 `claim_order_poll_settle` by-orderId 版、SECURITY DEFINER、GRANT authenticated、節流秒數常數)。
**影響面**:結帳付款鏈(client)+ 1 支新 server action + 1 支新 RPC。付款安全契約(getPrime/charge/3DS/inflight/終態鎖)**不改語意**、只加逾時分支。
**驗收**:逾時→掀 overlay 有出口(agent-browser 實測);reconcile own-only(他人 cartSessionId 回 none)、節流生效、settleCharge 對 pending/查不到不動訂單(單元 + 交易模擬);三綠 + 突變。
**Rollback**:逾時分支為純新增,移除即回舊行為;RPC 為新物件,不 apply 即未生效(charge 鏈不依賴它)。
...
翻三 flag(3DS/sweeper/anomaly)回 false = 全線回到今日「不可刷卡」態;S1 RPC/pg_cron 為新物件、不啟用即無影響;訂單一律只標記不刪 → 無資料破壞可回滾。

## 8. 待 Sean 手動(Claude 不代做)

db push(S1 RPC、L1 migration)/ 設 env / 換正式 merchant / TapPay 後台登記 notify URL / 翻 flag / push / deploy 正式站 / TapPay 後台退款。
...
- 節流:`claim_order_poll_settle(p_order_id,p_throttle_seconds)` by-orderId(migration `20260621120000`);**無 by-cartSessionId 版 → S1 需新建**。
