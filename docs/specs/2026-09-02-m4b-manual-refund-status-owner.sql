-- 🛑🛑🛑 **不得貼 —— codex R2 仍判 FAIL(10 must-fix + 2 nit)。2026-09-02 05:xx。** 🛑🛑🛑
--
--    🔴 **最重的一條是【我自己新加的】**:§2 在 `admin_void_manual_refund` 已經鎖住
--      退款子列之後,才去呼叫會鎖 `orders` 父列的同步器 ⇒ **子→父 反向鎖序**
--      ⇒ 與另一個先鎖父列的交易互等 ⇒ PostgreSQL 以 `40P01` 終止其中一筆。
--      ⚠️ 而 harness 全部是單執行緒 ⇒ **22 格全綠而它一格都看不到**。
--    🔴 其餘重的:更正判定那支 RPC(`admin_correct_order_refund_verdict`)也沒接同步器;
--      §2 的替換手法只數兩個文字錨、沒釘 body 指紋;兩支檔的前置閘都沒驗 ACL。
--
-- ✅ **而【不貼】是安全的,這一格要講清楚**:今天那個狀態本來就在說謊(退了款仍寫已付款)。
--    不貼 = 維持現況,**不會新增任何失效模式**;而貼了會在金流路徑上加一個死結可能。
--    ⇒ 📌 所以這不是「來不及」,是「這一版會讓事情變得更複雜而不是更安全」。
--
-- 🔵 **正式庫實量(唯讀,2026-09-02 05:2x)**:`order_manual_refunds` **總列數 0**
--    ⇒ 受影響的既有訂單 **0 筆** ⇒ **今天沒有任何一張單因為這個缺口在說謊。**
--    ⚠️ 而那個 0 有前提:訂單表今早被 `⟦b4-PURGE1⟧` 清空過 ⇒ 它是「現在沒有」,
--      不是「從來沒發生過」。負對照(現造狀態值)⇒ 0 ⇒ 尺不是恆零。
--    ⇒ ⇒ **所以風險是未來式:Sean 明早【新登記】一筆退款時才會踩到。**
--
-- 📋 **交接**:做到哪 / 差什麼 / 下一個人從哪接 ⇒ `docs/handoff/2026-09-02-manrefund-status.md`
-- ════════════════════════════════════════════════════════════════════════
...
--   ③ 🔴 **卡片退款那條路的篩選規則【有改】—— 而前一版檔頭寫「一個字都沒改」是錯的。**
--      改的是:多算一種列 —— 一筆被人工判定「退款失敗」、而**事後被更正成『錢其實動了』**的列。
--      (那種列在「還能退多少」那支函式裡本來就算, 而狀態這一側漏掉它
--       ⇒ 兩邊對同一筆錢給不同答案 ⇒ 客人會看到互相矛盾的畫面。)
--      ⚠️ **所以就算一筆人工退款都沒有, 只要有那種被更正過的卡片列,**
--        **狀態也可能在下次同步時改變。**
...
--   ⛔ ~~本檔前一版寫「還原-人工退款狀態-20260902.sql」~~ —— **那個檔名不存在**。
--   桌面檔名由 `hand-to-sean.sh` 決定, 而它還沒跑過 ⇒ 不要先寫一個猜的名字。
--   **整支貼下去就對,不用挑段落。**
...
--      ⇒ 📌 **同一個結果,而爆炸半徑小一個量級。**
--
-- ⚠️ **貼本檔【之前】真正的呼叫端只有兩個**(🔴 貼【之後】是三支函式、四個呼叫位置 ——
--    本檔 §2 又接了作廢那支的兩個 return 點 ⇒ 這個數字要帶時間限定, 否則下一個人會少數兩處)(全 repo 產線 SQL 實查;⛔ ~~本檔前一版寫「5 處」~~ 那是把
