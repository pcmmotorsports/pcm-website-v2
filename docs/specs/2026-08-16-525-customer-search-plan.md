...
> 做 UI 的人一定會打開 plan,不一定會打開 migration。

### ① 🔴 `truncated` 必須接起來,而且要顯示給員工
**migration 的 `COMMENT` 逐字**:
> 「命中超過上限時只回前 `p_limit` 筆並 `truncated=true`
...
而 cookie **跨分頁共用** ⇒ 同一瀏覽器開兩個客戶列表分頁、各搜各的**會互相蓋掉**
(這條是**訂單側已認列、刻意不修**的缺陷,本片一併繼承)。
⇒ **緩解就是讓那個看不見的狀態變成【看得見、關得掉】。**
⚠️ **沒有它的話,員工會看到一個「莫名其妙只有幾筆」的列表而不知道為什麼。**

### ③ 清除搜尋**走同一支 action**(送空字串),不做前端捷徑
`keyword-search-action.ts` 的 `empty` 與 `invalid` **共用同一個出口 = 刪 cookie**。
🔴 **前端另做一條 `router.replace` 之類的路 = 生出第二條語意不同、而且沒有人守的路徑。**

### ④ 搜尋表單必須帶 `return_to`,且呼叫端要把 `page` 歸 1
不帶的話 PRG 會退回 `/customers` ⇒ **搜尋一次就把會員等級篩選洗掉**。
(`safeListReturnTo` 只做 fail-closed 驗證,**它不會幫你補**。)

### ⑤ 驗收必須有的兩格(否則等於沒驗到本片的動機)
- **零訂單的客人搜得到** —— 🔴 `#525` 存在的理由
- **翻頁不掉搜尋詞** —— 那是 cookie 當載體的**唯一理由**
