...
## §9 🔴🔴 定位那 1,400ms —— **而答案是:它不是「詞數」,是【中文字】。我先前的結論錯了。**

> 主視窗-87 2026-09-03 04:4x 指派:把那 1,400ms 定位到一層。判準寫在跑之前
> (`scratchpad/search-1400-plan.md`):**每一層都要量【兩個世界】,一個對詞數不敏感的層不論多慢都不是答案。**

### 9-a ⛔ 先作廢我自己的結論
⛔ ~~「斷崖在 1 ⇒ 2 個詞,不在 8」~~(本檔 §10-b 與 `…multiword-sku-plan.md` §10-b)
🔴 **那是我的【測資】造成的假象** —— 我當時的一詞樣本是 `rsv4`(**純 ASCII**),
而兩詞與八詞樣本**每一個都含中文**(`油箱貼` / `碳纖維`)。
📌 **⇒ 我以為我在變動「詞數」,實際上我同時把「有沒有中文」也變了 —— 而那才是真正的變數。**

### 9-b 🎯 拆開之後的實測(線上,每格 4 發取中位數,`cache:'no-store'` + cache-buster)
```
一詞 ASCII        rsv4                     115 ms
兩詞 全 ASCII     rpm rsv4                 318 ms
兩詞 全 ASCII     gilles rsv4              293 ms
八詞 全 ASCII     aa bb … hh               654 ms
一詞 ASCII 長     zzqprbxx9137never        304 ms
──────────────────────────────────────────────────
🔴 一詞 中文      油箱貼                 2,351 ms
🔴 一詞 中文      碳纖維                 1,761 ms
🔴 **一個中文字**  貼                     1,705 ms   ← 一個字
🔴 兩詞 混中文    rsv4 油箱貼            1,725 ms
🔴 八詞 只含一個中文 aa bb … gg 貼        1,741 ms
```
🎯 **⇒ 八個 ASCII 詞 = 654ms;而【一個中文字】= 1,705ms。**
⇒ 📌 **真正的變數是「這串輸入裡有沒有多位元組字元」,不是「有幾個詞」。**

### 9-c ⇒ 那也解釋了為什麼前面兩層都被「排除」了 —— **兩次都用錯了測資**
```
① 主視窗的 EXPLAIN(182ms · 對詞數不敏感)—— 🔴 **它用的是哪一組字, 未確認**
   若是 ASCII, 它量到的就是上面那條 115–654ms 的線 ⇒ 它排除的是【不存在的嫌疑】
② 我的拋棄式庫(21ms · 對詞數不敏感)—— 🔴 **鑽機是 `LC_ALL=C` / `--locale=C` 起的**
   ⇒ **C locale 之下 ILIKE 對 UTF-8 幾乎不做 case-folding** ⇒ 中文在本機【本來就便宜】
   ⇒ 📌 而本 repo 記過同族:「`pg_trgm` 對中文在 macOS BSD libc 抽零 trigram 而正式站 glibc 正常
        ⇒ EXPLAIN 類斷言在本機無判別力」—— **同一個坑,換一個受詞(從 trigram 換成 ILIKE)。**
```
🛑 **⇒ 所以「不是 DB」那個結論【站不住】** —— 它是用**看不見這個現象的兩把尺**得到的。

### 9-d ⇒ 而「不要建索引」那個結論也要跟著撤
⛔ ~~「不要建索引 —— DB 只佔 11%,且對詞數不敏感」~~
🔴 那句的依據是 182ms,而**那一發很可能沒有中文**。
✅ **而 `pg_trgm` GIN 索引正是為這種 `%…%` 比對設計的** —— 也正是 Gemini 與 Codex **各自**建議的方向。
⇒ 📌 **繞了一圈,外部意見一開始就指對了地方,而我用一組錯的測資把它排除掉了。**

### 9-e ⚠️ 而我【沒有】證明成因,只證明了相關 —— 明寫
我證的是「**含中文 ⇒ 慢 6–15 倍**」這個**關聯**。
**我沒有證的**:它慢在 `ILIKE` 的 case-folding、慢在 collation、還是慢在別的地方。
⇒ ✅ **要證成因,要對【正式庫】用【含中文的字串】跑一發 `EXPLAIN (ANALYZE, BUFFERS)`,
   並與同形狀的 ASCII 查詢對比** —— 那要 DB access,**我沒有**。
🛑 **在那一發跑出來之前,不要動手改任何東西** —— 我已經因為一組錯的測資推翻過一次結論。
