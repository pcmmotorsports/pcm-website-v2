...
🔴 **這一句決定了方案的形狀**:
**內嵌(embed)的列數沒有辦法翻頁**,把 200 調到 1000 只是走到那道真牆前面;
**要真的撈得完,那份品項必須改成【頂層查詢】** —— ⚠️ 「PostgREST 的 embed 不支援獨立分頁」
這一句我是**照既有 docstring 的敘述推的、沒有當場對官方文件查證** ⇒ **未確認,動工前要先查一次**。
