...
三件現況實查(每條附行號,**座標為 2026-08-17 落地前**):

1. **路 A 的文案是一句假話** —— `shipping-doc.tsx:64` 逐字「請重新整理後再列印」,
   而上限是固定值 ⇒ **叫他去做一件永遠不會成功的事。**
2. **「列印」鈕在紅字【上面】、不受擋**(`shipping-doc.tsx:293`)
   ⇒ 他按得下去,印出來的紙上只有那一行紅字。**守門裝在沒有事的那條路上。**
3. **路 B 是 Next.js 內建的英文 404** —— admin 沒有自訂 404 頁
...
📌 **揀貨單不是出口** —— 它截斷時照印,只加一句
`components/print/picking-doc.tsx:122-126` 逐字「**下面看到的不是全部…不要拿這張去揀貨**」
⇒ **它只是換一句話叫他別用。**
