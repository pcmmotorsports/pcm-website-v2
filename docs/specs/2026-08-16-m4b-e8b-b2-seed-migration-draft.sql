...
-- ───────────────────────────────────────────────────────────────────────────
--  1. 三列
--     🔴 uuid 與上面 DECLARE 的三個必須一致 —— 改的時候【九處都要改】
--        (原註解寫「六個」是我數錯,關卡2 R2 指出。量法:
--         `grep -c '00000000-0000-0000-0000-00000000000' <本檔>` 回 9,
--         分佈在三段:DECLARE 三處、INSERT 三處、落地斷言三處。)
--        ⚠️ 漏改落地斷言那三處 ⇒ 拿真 uuid 比佔位字面 ⇒ **會紅**,方向是安全的,
