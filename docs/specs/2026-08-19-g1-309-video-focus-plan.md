...
## ⑩ 實作清單補一格(2026-08-18,座標更正的下游)

`BrandPageMedia.tsx:108` 的註解逐字寫著「設計稿 `:1926` 的 `poster.remove()`」,而**今天實查是 `:2029`**。
- **本輪刻意不改那一行** —— 動 `.tsx` 就要跑三綠,為一行註解跑不划算;而**這片本來就會動那個檔**。
- ⇒ **實作時一併改**:`:1926` → `:2029`,並把「鐵則 1 拍板路徑 ⇒ backlog #309」那半句
  改成「**主視窗 2026-08-18 裁定不算偏離(代裁人 MAIN,可推翻),見 plan §⑨**」。
- 🔴 backlog 那兩處(`#309` 條目本文與「相關」列)**已在 `docs/phase-1-backlog.md` 改掉了**,
  所以 **`.tsx` 這一處是目前唯一還寫著 `:1926` 的載體** —— 漏了它,下一個人會拿舊座標去核。

## ⑪ 🔴 驗收條件補一條(主視窗指定,不能只寫在論證裡)

> **修法必須照 design 自己的借還語彙**(`brand-page.html:1927/1933/1940` 那套
> 「開啟前存 `lastFocus`、關閉後還回去」),**不另發明一套**。
> 判別句:**我寫的這段焦點程式,能不能在 `brand-page.html` 裡找到同型的前例?**
> 找不到 ⇒ 我在發明,停下來。
