...
### 7.5a 裁決與落地(主視窗 `P-275-A` 裁 **B**,2026-08-09 22:4x)

**收 L4b**,但把未確認銘刻成字面。理由:態 5 有正向對照撐;`-1` 是**構造不出**而非還沒試
⇒ 不收=無限期等一條不存在的路;而既有已上線的 cart dedup 路徑本來就吃同一句假設,
**擋 L4b 不會讓正式站更安全,只會少一層守門**。三件已落地:

1. `packages/use-cases/src/settle-charge.ts` `classifyRecordStatus` 檔頭註解已銘刻三段口徑 + 失效條件。
2. backlog **#353** 已立案(`-1` 前提未驗 + 真實發卡行行為差異,含「不修痛在哪」)。
3. 🔴 **轉述口徑鐵線**:講「前提已驗」**只能指態 5**;`-1` **一律帶「未確認」**;
   任何轉述都保留「只證 sandbox、不等於真實發卡行」。落 memory
   `project_l4-stale-3ds-url-premise-verified-only-for-cancel`。
