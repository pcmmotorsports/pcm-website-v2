...
⇒ **有相當機率他看到的是「顏色沒吃到」而不是「沒做」。** 本項**不排片**,改成一個 5 分鐘的確認動作:
部署後拿 `E-505-STOP` 的探測靶驗線上產物,再請他看一次現況。**確認之後若仍有差,再回來開片。**
...
| 已知坑 | `:1047` 逐字:「⋯ 那格:內容 24px,欄寬 30 扣掉 7+7 內距只剩 16 ⇒ **實測被切**。給 **38** + 縮內距」 | 我們 `col-ops` 現為 **34px**(`globals.css`),內距已縮到 `0.125rem` 兩側 |
...
🔴🔴 **S4 有一條不只是「同檔」的真相依**:OD `:1047` 實測 ⋯ 那格要 **38px**,我們現在是 **34px**。
而 `Q-E2`=A 之後欄寬是**固定值、零和的**,品名欄只剩約 **1 個中文字**的餘裕
(`globals.css` 片5 那段 + `E-504-STOP`)⇒ **S4 要多的 4px 必須從別的欄扣,那是要 Sean 再拍一次的事。**
**⇒ 排隊順序:S1 → (Q2 拍板) → S2 → (欄寬 4px 拍板) → S4。S4 不要排在最前面。**
...
- **Q3 操作欄的 4px**:S4 要把 `col-ops` 34→38,錢只能從別欄出。
