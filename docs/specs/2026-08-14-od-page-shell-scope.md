...
>
> ## 🔴🔴 2026-08-15 現況更正 —— **本檔 §1/§2 有四處已經不成立,往下讀前先看這段**
>
> **原句一字不刪**(留痕才看得出被推翻的是哪一句);更正由 C 窗 2026-08-15 實查,主視窗裁定寫進原檔、不重寫一份。
>
> ⚠️ **本次更正的範圍(先講限度,免得被讀成整份都重驗過)**:
> **只驗「我們這側的現況」。OD 側的所有字面仍是轉引本檔 08-14 的內容、未開 `overview-desktop.html` 原檔複驗**
> —— 而**本檔自己記過引錯表一次**(見 §1-4 那條刪節線)⇒ **要引用 OD 側字面,請先開原檔。**
>
> | # | 本檔原本寫的 | 2026-08-15 實查 |
> |---|---|---|
> | 1 | **S1 工具列 chip「不存在」**(§1-2) | 🔴 **已完成** —— `apps/admin/src/components/orders/order-filter-chips.tsx`,現有兩顆(`全部`/`未到貨`),即 `#484` 片B-1 |
> | 2 | **S4「⋯ 選單不存在」**(§1-4) | 🔴 **已完成,但形狀被改了** —— `orders-table.tsx:76-77` 逐字:**動手前 hit test 證偽了零 JS `<details>` 彈出選單,現況 3/3 點不到** ⇒ **本檔 §1-4 引的 OD `<details class="km">` 方案已被實測否決**,做成 ⋯ 但不是彈出選單(`#486` 乙案) |
> | 3 | **「我們側欄 7 項」**(§1-1) | **現在 9 項**(多了 `商品`、`操作紀錄`)⇒ **對齊 OD 要拿掉的入口比 08-14 更多** |
> | 4 | **指標 `app-sidebar.tsx:23-36` 的 `NAV_ITEMS`**(§1-1) | 🔴 **假行號** —— `#27` D1c-1 已把清單搬到 `apps/admin/src/components/layout/nav-items.ts`;照原指標去看會找不到 |
>
> ### 🔴 所以這條線真正剩下的只有兩件
> **S1 / S4 已完成;S3(退貨中)、S5(側欄)本檔已明寫不排。**
> ⇒ **剩 `S2`(待處理 chip,不需要 Sean 再拍)與 `Q1`(側欄,決策題)。**
> ~~**本檔 §2 的估時表(S1 40 + S2 20 + S4 45 = 105 分)已過期,實際只剩 S2 的 20 分。**~~
> 🔴🔴 **上面那句是錯的,而且是我自己寫的(2026-08-15 稍後自查更正,原句留痕)**:
> **「S2 = 20 分」本身就是一個過期估值**,我從 `#485` backlog 抄過來、沒有驗。
> **真值見 `docs/specs/2026-08-15-1-today-view-plan.md` §0**,那份 plan 的第一句就在推翻它:
> 「待處理」是**跨兩軸的 OR**,而 **chip 殼 / filter 值物件 / adapter 下推三層都是單軸 AND 的形狀**
> ⇒ 拆成 **片1(40 分)+ 片2(35 分)+ 片3(20 分)≈ 95 分**,不含 plan 與審查。
> ⚠️ **一份專門修過期字面的段落,自己帶進了一個過期字面** —— 主視窗逐字:
> **「那不是諷刺,那是這個病的本體:它不挑載體,連正在修它的那份檔都照樣中。」**
>
> ### 🔴 本段什麼時候會過期(交件自帶,常設條款 2026-08-15)
> | 這一項完成時 | 要回來改本段的哪一句 |
> |---|---|
> | **S2(待處理 chip)三片做完** | 上面「剩下的只有 S2 與 Q1」⇒ 改成**只剩 Q1**;`95 分` 那個估值改成**實際耗時** |
> | **Q1(側欄)Sean 拍板** | 「Q1 決策題未拍」⇒ 改成拍板結果;若拍「對齊 OD」則 §1-1 整段要重寫 |
> | **有人開 `overview-desktop.html` 原檔複驗** | 本段最前面那條限度(「OD 側字面仍是轉引」)⇒ 改成已複驗或列出差異 |
> | **側欄再加/減任何一項** | 「現在 9 項」那個數字(數法 `grep -oE "key: '[a-z-]+'" nav-items.ts \| wc -l`) |
>
> ### ⚠️ 「待處理」拍板的代號:**本檔以下一律不引字母**
> 原本各處寫「拍 C」,而**同一個拍板在 commit 記錄裡是 `Q-待處理=B`**
> (`4ffda20b` / `a01457be`,兩顆都在 `origin/dev` 上、可達)。
> **兩份選項清單編號不同、行為描述一致** ⇒ **改引行為與出處**:
> **「還沒收錢 = `unpaid` ∪ `partiallyPaid`」(出處 commit `4ffda20b` / `a01457be`)**。
> 🔴 **通則(主視窗 2026-08-15 裁定)**:**決策代號(甲/乙/A/B/C)是那一次對話的座標,不是永久識別碼。**
> **跨檔引用要引「行為」與「出處 commit」,不要引字母** —— **字母會被拿去查選項表,而選項表可能不是同一份。**
