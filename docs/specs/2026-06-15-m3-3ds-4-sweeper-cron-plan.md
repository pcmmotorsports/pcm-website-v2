...
- **port 擴**:`IWebhookInbox` 加 `claimDueEvents(limit): Promise<{recTradeId,orderNumber,attemptCount}[]>` / `markProcessed(recTradeId, claimedCount)` / `markRetry(recTradeId, claimedCount, reasonCode)`;`IChargeAttemptStore` 加 `claimStuckUnsettled(ageSeconds, limit): Promise<{attemptId,orderId,settleCount}[]>`(🔴 原子 claim、非 list)+ `markSettleRetry(attemptId, claimedCount, reasonCode)` + `flagNonUnpaidActive(limit): Promise<number>`。
- `sweepSettlements(deps, opts)` use-case:
  ...
  - ③ `flagNonUnpaidActive()` → 標記 refunded/partiallyPaid 殘留 attempt + 回 count(>0 告警)。
