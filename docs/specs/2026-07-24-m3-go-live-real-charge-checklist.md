...
>
> ## ✅ 2026-07-24:兩項皆已驗證通過 → 審查結論升為 **GO-with-cautions**
> 檢查 A(8 支 RPC)= Sean 實跑 8/8;檢查 B(SITE_URL)= Claude 由 robots/metadata 獨立反推證實。
> **F1、F2 兩個阻擋首刷的條件皆解除。** 剩餘 cautions(不阻擋、但要知道)見步驟 7/7.5/8:
> ①currency 閘從沒跑過真回應(最可能首刷就中、錢在可退)②背景兜底全關 ③3D 分頁別留著 ④flag 須字面 `true`。
...
✅ **2026-07-24 已驗證通過(Claude 獨立驗證、非採信宣稱)**:
- Sean 實開 `https://shop.pcmmotorsports.com/checkout/callback?order=00000000-…` → 正常渲染
  「付款處理中」頁、非 404、網址列未跳離該網域。
- 🔴 **值本身的獨立佐證**(不靠 Sean 抄 Vercel):`curl https://shop.pcmmotorsports.com/robots.txt`
  回 `Host: https://shop.pcmmotorsports.com` + `Sitemap: https://shop.pcmmotorsports.com/sitemap.xml`,
  且非「全擋」狀態。依 `seo.ts:8-10`「未設 → robots 全擋 + sitemap 空」的休眠設計反推 →
  **`NEXT_PUBLIC_SITE_URL` 確實已設,且值 = `https://shop.pcmmotorsports.com`**,與客人瀏覽網域逐字相同。
  首頁 org-jsonld 絕對網址(`…/#store`、`…/pcm-logo.png`)同 base 交叉印證。
- ⇒ **檢查 B 通過、審查 F2(唯一靜默掉錢路徑)解除。**(點時間驗證:日後改網域須重驗。)
