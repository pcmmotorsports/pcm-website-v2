# 靜默截斷:讓它不會發生 —— plan(鐵則 8)【✅ 已完成 2026-08-17】

> ✅✅✅ **狀態:【已完成】**(2026-08-17)。~~已批准,施工中~~
>
> **落地的 commit**:`f57832c8`(四支查詢改明示上界 + fitment 兩步改一步 `!inner`)
> ＋ `eb4bc586`(同分類那層改下推查詢,收掉 codex 第一輪最後一條 must-fix)。
> 🔴 **這兩顆是【沒有安全中間點】的一組** —— 停在 `f57832c8` 會拿到一個
> 四綠全過、而「`score 100` 那層會被池的前 N 筆擠掉」**還在**的狀態,**且它零症狀**。
>
> 🔴 **為什麼一做完就回來改這一行**(而不是留給下一個人發現):
> **越新的計畫書越危險** —— 它比 `2026-08-16` 那份新,**更容易被當成現況引用**。
> 而本檔 §8 還寫著「我需要的批准」⇒ **讀到那裡的人會以為這件事還沒開始。**
> 📎 通則:**一條線做完之後,最先過期的是它自己的計畫書。**
> **鐵則 8 批准 = Sean 逐字「A: 甲 = 批,現在做」**(經主視窗轉;主視窗同時裁 Q2 修法照做、Q3 範圍取**乙**)。
> 📌 **批准寫在這裡而不是只留在信裡** —— 2026-08-16 發生過「兩份已批 plan 的檔頭仍寫著尚未批准」。
>
...
## 8. ~~我需要的批准~~ ⇒ **全數已批並已完成(2026-08-17)**

> **原文保留於下**,因為它記錄了「當時哪些是未定的」——
> 刪掉就看不出這一片是**帶著三個未決開工**的。

```
1  鐵則 8 批准（動 packages/ports 介面 + 跨 3 檔）  ⇒ ✅ Sean 批（Q1 = 甲）
2  §2-d 修法（count 取池大小 + 明示 .limit(N)）      ⇒ ✅ 主視窗裁；⚠️ 實作【只做後半】，
                                                       count 未加（理由見 commit f57832c8 body③）
3  範圍：甲=只修推薦兩支 / 乙=甲＋66 筆人工分類      ⇒ ✅ 主視窗裁【乙】
                                                       ⚠️ 而【66 筆人工分類尚未做】—— 見下
```

### ⬜ 這一片**沒有做完的那一半**(不要因為狀態寫「已完成」就當全清)

**§3 那 66 筆「沒有明示上限的查詢」的人工分類,我沒有做。**
主視窗當時裁的「乙」包含它,而實際落地的只有推薦引擎那四支 + 商品搜尋。
⇒ **它仍是一份要人去看的清單**,分類產出要分三堆(`真缺陷` / `不是缺陷（附理由）` / `不確定`),
🔴 **第三堆不准為了好看清空**;報告裡要把「這支腳本掃不到什麼」(`.rpc(`、裸 SQL)寫在數字旁邊,
免得 `66` 被當成分母。

### 📌 由本片衍生、已另立的

```
#603  fetchAllPaginated 的 PAGE_SIZE 等於而非小於 max-rows（含 F-S3 第二份獨立實作）
#605  客戶列表分頁排序缺唯一鍵次序
#606  vitest 的 @ alias 只指 storefront ⇒ admin 頁面寫不出頁面層測試
Wallet  另立 docs/specs/2026-08-17-wallet-ledger-pagination-plan.md（已完成，commit a155229b）
訂單歷史 SupabaseOrderAdapter.listSummariesByCustomer 同病、【未做】
        ⇒ 等 Sean 答「客人前台要不要也分頁」（它還被 storefront 用）
```
