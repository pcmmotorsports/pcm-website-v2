...
片 2  展開列內容:訂貨段 + 到貨登錄段搬進去
      資料現成、容器現成、不動 server ⇒ 風險低，動的是 DOM 位置與 props
      🔴 零新增:品項層備註【Sean 08-17 裁定不做】（「備註留在訂單層就好」）
...
### 9-d-3 片 2 的「改什麼」條列(F1–F4 已收進;這一份是可以直接照做的版本)

> **狀態:等 Sean 批。** 本節只是把上面四條變成動作,**沒有動任何 code**。
> 🔴 **F3 是 Sean 的題,不是這張條列能解的** —— 主視窗已把代價講給他,他若說「採購那塊留在訂單層」,
> **本節第 1、2 條就整個作廢**。⇒ **先等那一句,再動手。**

**1. 搬(片 2 本體)** —— `item-procurement-section.tsx` 的**每一輪迴圈內容**搬進對應品項的展開列
```
...
**2. 🔴 三個【訂單層】的東西留在訂單層 —— 不跟著品項走(F1;這條不軟化)**
```
...
**6. 🔴 作廢列的「—」要有一句話說為什麼(逐項表 `#17`;主視窗裁定不另立案、收進片 2)**
```
