...
| 7 | 真相非零但摘要列缺失 | `:377-387` | **fail-closed:停用該單取消**(K1-6;不是只給提示)。🔴 **關鍵在「真相非零」四個字**——真相為零的缺列 RPC 是放行的,呼叫端立場見 §1.3 的 2026-08-09 修訂(五項前提) |
...
> 🔄 **2026-08-09 修訂(主視窗裁決 `E-007-A` Q1=A;鐵則 8 的批=該信)。**
> ~~缺列 ⇒ 停用取消(1.2 #7),不得當成「到貨 0、取消 0」的事實~~ —— **舊字面已作廢**,理由三條:
>
> 1. **一律停用是「誤加的閘」,不是安全**:A4a 的四支 trigger 只掛
>    `order_item_procurement` / `_receipts` / `order_cancellation_items`(`20260803140000:714-717` 的 DROP 清單即全集),
>    摘要列**惰性建立**(`:179-180`)⇒ **從未採購也從未取消過的品項,那一列永遠不會存在**(穩態,非時序 race)。
>    而 RPC 的摘要在場閘只在「receipts 或 cancellation 兩個和非零」時 RAISE(`20260805100000:376-387`)
>    ⇒ 同一張單 **RPC 放行、UI 永久拒絕** ⇒ 把取消 UI 的主要情境(未付款、未採購、客人要求取消)整個關掉。
> 2. **連帶讓毀損訊號失去判別力**:`quantity_summary_missing` 同時是 `mappers/order.ts:557` 偵測 C7 不變式
>    被違反的唯一顯示出口;日常天天亮 ⇒ 員工學會忽略 ⇒ 真毀損那天沒人看
>    (memory `feedback_fix-that-moves-gate-earlier-erases-evidence`)。
> 3. **新立場不是「補 0」,是權威訊號的窮舉**(memory `feedback_guard-reads-non-authoritative-cache` 的精神保留 ——
>    推 0 的來源不是快取):`procurements` **零筆且未截斷** ⇒ 無採購列 ⇒ 收貨列 FK 掛不上 ⇒ 到貨必 0;
>    取消歷程**可讀且完整**時 `order_cancellation_items` 就是真相表本身 ⇒ 已取消量 = Σci;
>    買了幾件取自 `order_items.quantity` 真相值。
>
> ⇒ **現行立場**:摘要缺列時,**若**「零採購列 + 採購未截斷 + 取消歷程可讀完整 + 品項清單未截斷 +
> **該品項 Σci 恰為 0**」**五項全成立**,推 `{instock: 0, cancelled: 0}`;
> **任一項不成立即維持 fail-closed**(停用取消)。
> 🔴 **第五項是確認輪抓到的**(初版只寫四項):RPC 那道閘逐字是 `receipts > 0 **OR** Σci > 0`
> 且缺摘要 ⇒ RAISE ⇒ 「零採購但取消過 2 件、又沒有摘要列」**RPC 必拒**,推值放行等於做出
> 「按鈕亮著、送出必失敗」。
> 🔴 **五項是充分條件、不是等價條件**(確認輪 R3 MF-i 打掉我原本寫的 ⟺):
> 五項全成立 **⇒** 真相為零 ⇒ RPC 的閘不觸發(方向安全,與 §1.2 #7 一致、不矛盾);
> **反向不成立** —— 例如「採購 parent 有列但零收貨、Σci=0、又缺摘要列」時 RPC 放行,而 UI 因
> 第一項不成立仍擋。⚠️ 但那個差集**正好是異常情境**:A4a 的 trigger 掛在 `order_item_procurement` 上,
> 有採購列就該有摘要列 ⇒ 「有採購列卻缺摘要」= A4a 沒建列 = 該擋。⇒ **過嚴的那一格剛好落在該擋的地方**。
> 落地在 `apps/admin/src/lib/orders/cancel-view.ts` 的 `ZERO_INFERENCE` 段(該段有正負測與突變守門)。
> 修法後 `quantity_summary_missing` 只在**兩種真異常**時亮:①**有採購或取消活動、卻沒有摘要列**
> ②**採購內嵌投影讀不全**(`procurementTruncated`,含「內嵌鍵整個沒回來 = 投影退版」)。兩者的指路都是「通知系統維護」。
> 🔴 **另兩個「看不全」的情況一律不報這個碼**(確認輪 R1 MF2 + R2 MF-a;兩者都會讓**正常資料**誤亮):
> ①`itemsTruncated`(品項數觸及 200 上限的正常大單)②`cancellations_unreadable`
> (某品項合法取消過 100 次就會讓外層歷程被截 ⇒ **整單**推不出 0/0,連從未有活動的品項也跟著缺上限)。
> 兩者各有自己的碼且都已擋住整張單 —— **抑制的只是碼,不是 fail-closed 的實質**(上限仍是 `null`、仍勾不動)。
> 🔴 **`procurementTruncated` 刻意不比照抑制**(確認輪 R3 MF-ii):它**沒有**對應的碼,抑制它會讓
> 「上限算不出來」卻一個拒因都不吐 ⇒ `canCancel` 變 `true` = **fail-open**。該不變式由測試釘住。
>
> ⚠️ **「五項 ⇒ 真相為零」的隱含前提**(確認輪 R2 nit + R3 nit):它假設各層截斷**看得出來**。
> 相關上限共四個 —— 取消歷程外層 100 / 取消明細內層 200 / 品項 200 / 採購列 —— 都刻意低於實測的
> 伺服器 `max-rows`(production 1000);若專案日後把 `max-rows` 調到低於其中任一個常數,
> 伺服器會先截斷而旗標判不出來 ⇒ 該推論靜默失效。
> 同 `mappers/order-cancellations.ts` 與 `PAYMENT_CHARGE_ATTEMPTS_EMBED_LIMIT` 檔內已記的同族殘餘風險。
>
> ⚠️ **同時失去的東西(誠實記載)**:`mappers/order.ts:557` 把「C7 不變式被違反」的摘要列也壓成 `null`;
> 在五項全成立的品項上,那種**毀損的快取列**會被推 0/0 蓋過去、不再顯示成異常。
> ⇒ 本 plan **不得再宣稱「C7 毀損一定看得見」**。行為面無害(RPC 同樣以真相判定、同樣放行),
> 但要讓它可見得另立管道(對帳報表),不在取消 UI 的職責內。
