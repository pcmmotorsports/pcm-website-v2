# Plan:訂單品項的 `max-rows` / 內嵌截斷修法(**✅ Sean 已批;A 條已交,B/C 見下**)

> ## 🔴 狀態(2026-08-18 更正 —— 原檔頭寫著「等 Sean 批」而他早就批了)
>
> ~~**狀態:等 Sean 批(鐵則 8:跨 3+ 檔 + 動查詢)。批准前一行 code 都不動。**~~
> **⇒ 原句保留劃掉,不刪。批准【早於】本次更正,所以那句在被讀到的每一天都是假的。**
>
> ✅ **Sean 2026-08-17 拍板 `D2` = 甲**,逐字:
> 「調大 max-rows ＋ 程式改成撈到盡且撈不完要講出來」
> **出處**:`~/.claude/projects/-Users-sean-1-pcm-website-v2/memory/project_0817-night-four-rulings-and-env-literals.md`
> (主視窗轉述時指 `:136`;⚠️ **那個行號是轉來的,我沒開檔核**)。
>
> ### 🔴🔴 而本檔裡有【兩種不同級】的批准,不要混
> ```
> D2（調大 max-rows + 程式改撈到盡）  ← Sean 2026-08-17 拍板
> Q-maxrows-1（先做 A 還是 A+B 一起） ← §4，Sean 未逐題回答；A 已依推薦先做
...
> ### 各條現況
> ```
> A 建箱候選   ✅ 已交（commit e0c343db，2026-08-18）—— 含真資料驗證：彈窗 201/201
> B 揀貨單     ⏳ 未開始
> C 訂單明細頁 ⏳ 有獨立 plan：docs/specs/2026-08-18-m4b-order-detail-items-exhaustive-plan.md
>              🔴 那一份【仍在等 Sean 批】—— 它是【新的】鐵則 8 觸發（7 支檔 + 動共用型別），
>                 不是本檔這一份的延續。兩份的批准狀態不同，不要互相推論。
> ```
>
...
> · 本機 `db-max-rows` 是【我自己設的 100】,與正式站 Dashboard 的值是【兩個不同的東西】,**不互相引用**。
>   (~~原寫「正式站的 **1000**」~~ ⇒ **2026-08-18 起是 `2000`**,Sean 親手調、V 窗實測。
>    🔴 **這一條的重點從來不是那個數字,是「兩個不同的東西」** —— 數字換了,規矩沒換。)
> · 「本機量到 X」不得寫成「線上是 X」。
...
`:411` 更直接:**「零餘裕」**。
> ## 🔴🔴 2026-08-18:上面那兩句引文的【事實部分】已不成立
>
> ⚠️ **本段是 I 窗與 B 窗兩份更正 merge 後【合併成一份】的**(2026-08-18 B 窗解衝突時併)——
> 原本兩段各自講同一件事、只是措辭不同。**留兩份會讓下一個人以為是兩件事。**
>
> `db-max-rows` 已從 1000 調到 **2000** ⇒ `PAGE_SIZE = 1000` **不再等於**伺服器上限、**不再零餘裕**。
> ```
> 量法（V 窗 2026-08-18 對正式站）：products?select=id&limit=5000
>          ⇒ HTTP 206、content-range 0-1999/19777
>          🔴 分母 19,777 > 2000 ⇒ 量到的是【天花板本人】，不是資料不夠
> 效度：正式站 / storefront anon key / 那一刻
> 🔴 主視窗、I 窗、B 窗【三方均未自驗】—— 全部是轉錄 V 窗的量測
> ```
> **那兩處 code 註解已就地更新**(B 窗 `8989d1da`,原句劃掉保留):
> ```
> packages/adapters/src/supabase/SupabaseOrderAdapter.ts:411    「零餘裕」
> packages/adapters/src/supabase/SupabaseOrderAdapter.ts:888-889「等於傳聞中的 db-max-rows(1000)」
> ```
> ⚠️ **而本節論證的「嚴格大於 vs 大於等於」是一條【準則】,它一個字都沒變** ——
> 準則不因為它舉的例子今天不再觸發而失效。
> 🔴 **規則沒過期,例子過期了。**
> 🔴🔴 **而餘裕是【設定】給的、不是程式保證的** —— Dashboard 改回去它就再次歸零,
> 而**不會有任何東西紅** ⇒ **本節這條準則的必要性因此【更強,不是更弱】。**
...
2. ~~🔴 **正式站現在的 `db-max-rows` 是多少,我們【不知道】** —— 那是問題清單 `B1`。~~
   ...
   **⇒ 2026-08-18 已解決,而且是【量到的】不是讀來的**:
   ```
   Sean 回報他把 max-rows 從 1000 調到 2000（口頭）
   V 窗實測：storefront anon key 打正式站 REST products?select=id&limit=5000
             ⇒ HTTP 206、content-range 0-1999/19777、實回 2000 列
             🔴 分母 19,777 > 2000 ⇒ 量到的是【天花板本人】，不是資料不夠
   效度限定：正式站 / storefront anon key / 2026-08-18 那一刻
   ```
   ⚠️ **這是 V 窗量的,不是我量的** —— 我引用它,而它的效度限定要跟著走。
...
- ~~**零實作。** 本檔是 plan,鐵則 8 ⇒ 等 Sean 批。~~
  **⇒ 2026-08-18 更正:Sean 已批 `D2`=甲,而 A 條已實作並交出(`e0c343db`)。**
  **本檔仍是 plan(它自己不含 code),但「等批」那半是假的。** 見檔頭。
- ~~**正式庫今天沒有超過 200 項的單可量**(Sean:都還沒對外開放、都是測試單)⇒ 本檔**沒有現況母體數字**。~~
  **⇒ 仍成立(正式庫沒有那種單),但 2026-08-18 起【不再是「量不到」】**:
  可以在拋棄式環境種一張 201 品項的真單,用真後台量。做法
  `docs/runbooks/local-admin-with-real-data-probe.md`;A 條就是那樣驗的。
  ⚠️ **那證的是行為,不是正式站的資料分布** —— 母體數字仍然沒有。
- **production 的 PostgREST 版本未比對**;V-041 的 `100` 是**本機自設**,與正式站的 `2000` **不互相引用**。
  🔴 **2026-08-18 更正:正式站現值是 `2000` 不是 `1000`** —— Sean 親手調的,
  V 窗實測(REST `206` / `content-range 0-1999/19777`,分母 19,777 > 2000 ⇒ 量到的是天花板本人)。
  ⚠️ 本檔 §2-e 用 `1000` 算出來的「最低可接受 1001 / 建議 2000」**推導過程仍有效**,
  而**結論已經被實現了**。⇒ 那一節從「待辦」變成「已完成」,不要再照它去按面板。
  🔴 而 A 條的修法**刻意不吃那個餘裕**:終止條件是「拿到 0 列」
  ⇒ **沒有任何數字承擔正確性,1000 還是 2000 都不影響它。**
