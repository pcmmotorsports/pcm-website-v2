...
> 🔴🔴 **2026-08-18:上面那兩句【引用的是 code 註解,而那兩處註解現在是假的】。**
> `max-rows` 已從 1000 調到 **2000** ⇒ `PAGE_SIZE = 1000` **不再等於**伺服器上限、**不再零餘裕**。
> ```
> 字面債住址（描述【世界的狀態】，一動就假而零機械訊號）：
...
> ```
> ⚠️ **而上面那段論證【仍然有效】** —— 它證的是「頁大小等於伺服器上限時,短頁判準零判別力」,
> 那是一條**規則**,不是那個數字。**規則沒過期,例子過期了。**
> 📎 **本檔刻意不代改那兩處 code 註解** —— 改 `.ts` 是另一片(要跑三綠),
> 已寫進 `#629` 那族。**在這裡留一個指路,比在這裡動手好。**
