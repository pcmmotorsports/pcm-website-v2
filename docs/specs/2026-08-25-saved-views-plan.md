...
>    **三個事實錯誤**, 不是原文。要看原文去 git:`git show <本次 commit>^:docs/specs/2026-08-25-saved-views-plan.md`。
...
```
📌 **兩份白名單都會漂, 而它們漂掉之後【一個會被發現、一個不會】。** 那才是選甲的真理由。

### 6-2-2 那份鍵白名單長什麼樣 —— **它必須是【推導】出來的,不是手抄的**
...
⇒ 可存的 = 11 − 2 = **9**
```
🔴 **而「9」這個數字【不可以寫進程式碼】** —— 要寫的是那個減法本身:
```ts
const SAVED_VIEW_PANEL_KEYS = [ORDER_PANEL_PARAM, CUSTOMER_PANEL_PARAM] as const;
const SAVED_VIEW_KEYS = ORDER_LIST_URL_KEYS.filter((k) => !SAVED_VIEW_PANEL_KEYS.includes(k));
```
⇒ 日後有人往 `ORDER_LIST_URL_KEYS` 加一個 key ⇒ **它自動流進可存集合**。
⚠️ 而那正是危險的地方:**新 key 可能是另一個「面板狀態」而不是篩選條件** ——
   自動流進去 = 靜默做了一個沒有人做的決定。
🔴 ⇒ **要讓「加一個 key」這件事【逼出一個決定】**,而不是自動放行。做法(實作片決定,兩條都可行):
```
甲 型別逼:把 SAVED_VIEW_KEYS 寫成【逐一列舉】的 union, 並加一格
   「列舉的長度 + panel 那兩個 === ORDER_LIST_URL_KEYS.length」的斷言
   ⇒ 加 key 而沒有分類 ⇒ **tsc / 測試當場紅**
乙 測試逼:保留 filter 推導, 另加一格把 SAVED_VIEW_KEYS 的內容釘成快照
   ⇒ 加 key ⇒ 快照不合 ⇒ 紅, 而修它的人必須看一眼那個 key 屬於哪一類
👉 **我傾向甲**(它在 tsc 就紅, 不必等測試跑), 而這一格**不需要 Sean**, 屬實作片。
```

### 6-2-3 🔴 過濾要做在【讀】那一側,寫那一側只是順手
...
⚠️ 而「丟掉」要留痕:讀到一個不在白名單裡的 key ⇒ **不是靜靜丟**, 至少要有一格測試證明它被丟了
...
· 日期那格(`6-2a`)不受影響 —— 它是值的語意問題, 不是鍵的分類問題
```
...
>    ⇒ **剩下的理由【只有】那四題。** 而那次重寫**沒有動到本表任何一欄**(逐條見 `§10-1`)。
...
# 🛑 §11 `§6-2` 第二版的 `code-reviewer` R1 = **FAIL**,**而下面這些一條都還沒修**
