...
| `update` | `UPDATED` · **`UPDATED_OVERWROTE`** · `NO_CHANGE` · `NOT_FOUND` · **`NAME_TAKEN`** | `NAME_TAKEN` 2026-08-28 加(codex 關卡2 F3):改名也會撞唯一索引,而它原本讓 DB 例外直接冒到畫面 ⇒ 📌 **回傳碼合約要涵蓋【使用者做得到的每一種事】,而不是【我寫到哪一支時想到的】** |
...
> ```
> supabase/migrations/20260828080000_m4b_b4views1_saved_order_views.sql   597 行(2026-08-28 當場 wc -l)
...
```bash
D=supabase/migrations/20260828080000_m4b_b4views1_saved_order_views.sql
...
# ✅ §14-30 帶資料的 rollback —— **而最重要的一格與資料量無關**

> 2026-08-28 02:21(當場 `date`)。主視窗給三個候選,我挑**甲**,理由:那是我自己剛開的口,
> 而**開出缺口的人接著去填它**成本最低(拋棄式流程 / 11 發驗收條 / `RBHERE` 都現成)。
> 交付 `docs/specs/2026-08-25-saved-views-rollback-with-data.sh`
...
⚠️ 而真實 N 的範圍本身也沒有人量過:`Q-檢視-9 = 甲`(不限制數量),
   後台現在 2 個人在用(來源 memory,**非本片量測**)
   ⇒ 100,000 遠超現實, 它在那裡是**上界探針**不是預測。
```
🔴 **灌資料那一步自己也有量具**(主視窗點名):灌完當場 `count(*)`,**不相信 INSERT 的回傳**;
   對不上 ⇒ 那一輪作廢。三輪實得 `0 / 1000 / 100000`,全部等於想灌的數。

## 量到什麼
```
N=0        down rc=0  18ms   之後表=不存在
N=1000     down rc=0  22ms   之後表=不存在
N=100000   down rc=0  19ms   之後表=不存在   ⇒ **完全持平**
```
### 🔴 而持平是一個危險的結果 ⇒ 我替碼錶做了正對照
```
持平有兩個世界:①DROP TABLE 真的不掃列(只解檔案連結)②**我的碼錶不會動**
⇒ 這兩個世界在畫面上是同一組數字。
✅ 正對照:同樣的資料量餵 `DELETE FROM`(它【會】逐列掃)
   N=1,000 ⇒ **15ms** · N=100,000 ⇒ **54ms**(3.6 倍)
⇒ 碼錶會動 ⇒ **DROP 那三格的持平是真的。**
📌 **一組持平的數字, 在「這件事真的不受影響」與「我的尺壞了」之間, 印同一個東西。**
```

## 🔴🔴 而最重要的一格與 N 無關:**有人在用後台時,`DROP TABLE` 會卡住**
```
一個連線開著交易在讀那張表 ⇒ 另一個連線跑 down
⇒ DROP TABLE 拿不到 ACCESS EXCLUSIVE ⇒ **擋在那裡**
✅ 被擋下來之後【表還在】—— 整支交易回滾, 沒有做一半
✅ 負對照:沒有人在讀的時候同一發 down 過得去 ⇒ 這不是「什麼都擋」
```
🔴 **維運上的意思**:半夜沒人用時 20ms 就結束;**上班時間有人開著後台,它會掛在那裡**。
而 `psql` 預設**沒有 `lock_timeout`** ⇒ 它**無限等**,畫面上就是一個不動的游標
⇒ 📌 **而「很慢」與「卡在鎖上」在那個畫面上是同一件事。**
✅ 已寫進 runbook 的 ②-b,並在 down-script 前面加 `SET lock_timeout = '5s';`。

## ⇒ runbook 的「不涵蓋」那一列改了一格,而**沒有整列刪掉**
```
✅ 從「有資料之後的 rollback 未量」⇒ 改成「三格量過, 而**那是拋棄式 PG 的數字**」
🛑 仍不涵蓋:**正式庫**上帶資料的 rollback(本機磁碟、無其他負載 ⇒ 不是正式環境的數字)
📌 **量到了一格, 不等於那一整列可以劃掉。**
```

---

# 🔎 §14-31 **本節自己的量法體檢**(2026-08-28 09:4x,當場 `date`)

> 主視窗指派「丙」。**方法照 `-c8` `⟦b4-BOARD1⟧` 那套,不自創**
> (問① 還沒做的世界印什麼 / 問② **對 ❌ 格要翻面**:已經做了的世界這把尺會印非 0 嗎 /
>  五種失效 / 蓋章要帶當場跑出來的值,只有日期的章在「我跑了」與「我看一眼」印同一句話)。
> 🔴 **狀態欄我一格都沒改** —— 那是判斷不是量測,撈到的丟主視窗裁。

## 分母怎麼來的(不是「我想得到的量法」)
```
範圍 = 本檔 §14 起(`:1918`)到檔尾, 共 3,292 行
字集 = grep -c / grep -l / grep -rn / wc -l / ls supabase   ⇒ **38 行**
⚠️ 而這個字集**自己的窄無法被自己量到** —— 用敘述講的量法(「開檔看」「逐條核」)撈不到
   ⇒ 📌 **38 是【可執行的那一種】的分母, 不是【所有宣稱】的分母。**
負對照 `負對照zzzz` ⇒ 0(尺會動)
```

## 結果:重跑 30 格 ⇒ **仍成立 18 · 過期 10 · 尺與答案不符 2**

### 🔴 第五種(尺是活的、形狀對的,而**答案過期**)—— 10 格
| 格 | 寫的 | 09:4x 實得 | 為什麼漂 |
|---|---|---|---|
| `FOR UPDATE` | 2 | **6** | 🔴 見下,這一格最值得看 |
| `clock_timestamp` | 1 | **2** | 我後來加的**解釋註解**自己含那個字 |
| 草稿行數 | 597 | **703** | 檔長大了 |
| `authorizeManagerMutation` | 0 | **2** | B-0 落地了(`§14-26` 已記,而**這一行原地仍寫 0**)|
| `Q-檢視-zz` | 1 | **2** | 我把「我弄壞負對照」這件事又寫了一次 |
| 「三支」 | 23 | **40** | 換路後我一直在談這兩個數 ⇒ 談論本身推高了它 |
| 「四支」 | 5 | **32** | 同上 |
| `upsert` | 16 | **27** | 同上 |
| `uuid` | 3 | **9** | 同上 |
| `GRANT SELECT` | 8 | **12** | 同上 |

### 🔴🔴 `FOR UPDATE` 那一格:**我裝的守門,把我的尺變成恆真**
```
現在 6 個命中, 濾掉註解之後仍是 6:
  :190 :264  ← 真的鎖列(update / delete 各一)= 原本寫的那個 2
  :413 :414 :416 :419  ← **我後來加的「碼錨 鎖列」**(它的工作就是檢查 `FOR UPDATE` 在不在)
📌 **我裝了一道「檢查 FOR UPDATE 在不在」的守門, 而那道守門自己讓「數 FOR UPDATE」這把尺失去判別力。**
⇒ 把兩支真的鎖列全部拿掉 ⇒ 這把尺仍然印 **4**, 而 4 ≠ 0 ⇒ **看起來還在。**
⇒ ✅ 正解是分開數:`grep -c '     FOR UPDATE;'`(帶縮排的語句)⇒ 2 · 碼錨那幾行另計
🔴 而這與 `clock_timestamp` 是同一族的第 **6** 次:**尺數到「寫著這件事的文字」而不是這件事本身。**
   而這一次的來源特別諷刺:**那段文字是我為了防這件事而寫的。**
```

### ⚠️ 第一種(尺與它旁邊的答案**對不起來**)—— 2 格
```
① `grep -cE "^\s*(ALTER TABLE|DROP |TRUNCATE )" <本片migration>` ⇒ 寫 **0**, 實得 **2**
   實得的兩行是 `ALTER TABLE … ENABLE ROW LEVEL SECURITY` 與 `ALTER TABLE … OWNER TO postgres`
   ⇒ 🔴 **那條量法寫在草稿還不存在的時候** —— 它防的是「破壞性語句」,
     而它的字集**把本片必要的 `ALTER TABLE` 一起撈進來** ⇒ 期望值 0 從一開始就不可能成立
   ⇒ 這不是過期, 是**期望與設計互相矛盾而沒有人跑過它**

② `grep -rn 'authorizeManagerMutation' apps/admin/src --include='*.ts' | 排除定義檔` ⇒ 寫 **3 個呼叫端**
   照那條命令當場跑 ⇒ **31**
   🔴 **我寫下的那個 3 來自一條更窄的命令**(只掃 `staff-actions.ts`), 而我寫下的是寬的那條
   ⇒ 📌 **命令與它旁邊的數字不是同一次量測** —— 而下一個人會跑命令、拿到 31、以為東西壞了
```

## ✅ 仍成立的 18 格有一個共同點,值得寫下來
```
它們幾乎全部走 `CODE()`(先濾掉 `^\s*--` 再量)⇒ **設計時就把註解排除掉的那些, 一格都沒漂。**
📌 **而漂掉的那幾格, 全部是【對整份檔案裸數】的。**
⇒ 這不是巧合:一份活的規格會一直長註解, 而註解裡會出現它在談的那些字面。
```

## 🛑 我沒有做的(刻意)
```
· **狀態欄一格都沒改** ⇒ 撈到的丟主視窗裁
· **原地的數字一個都沒改** ⇒ 改了就看不出它曾經是什麼、漂了多少
  (照 `§14-Z` 那條:關掉的用刪除線、原文不刪)
...
# ✅ §14-33 codex 關卡2 兩輪 —— 6 條 must-fix,而**折完之後覆蓋三次都沒動**

> 2026-08-28 11:0x(當場 `date`)。鐵則 12③(migration)⇒ 由**這個窗自己**跑 codex,唯讀。
> 零留痕:HEAD 未動 · **12 支檔逐字相同** · 無新增檔 · **負對照(改一個位元組)⇒ 判得出不同**。

## 兩輪帳
```
R1  FAIL / 4 條 —— 全接受、全修
R2  FAIL / 2 條 —— 而 R1 的 F2 / F3 兩條它**主動驗過沒被擊破**
🛑 照 skill 輪次規矩【不跑第三輪】⇒ 那兩條「已修而未經第二輪」的狀態端 Sean 決定
```
| # | 打什麼 | 為什麼是真的 |
|---|---|---|
| F1 | identity sequence 的權限沒撤 | 它會弄壞 T16 —— 本片**唯一**那個「證明 INSERT 有沒有跑過」的觀察點 |
| F2 | 重播判定靠「哪個唯一索引先報」 | 🔴 一發**正常的重播**會被回報成「名字重複」⇒ 使用者去改名 ⇒ **真的建出第二張** |
| F3 | update 改名撞唯一索引 | DB 例外直接冒到畫面;`NAME_TAKEN` 原本**只有 create 產得出來** |
| F4 | 授權讀 `staff` 而只鎖 views | 📌 **鎖住「要改的那一列」不等於鎖住「准你改的那個理由」** |
| R2-1 | sequence 名字**撞名**時查到錯的物件 | 見下,這一格最值得看 |
| R2-2 | `list` 也有授權 TOCTOU | 我修 F4 時只修**有寫入**的三支 —— **而讀也會** |

## 🔴🔴 R2-1:**我驗過的是【打錯】,它問的是【撞名】**
```
我等複審時自己量過「sequence 名字打錯會怎樣」⇒ ERROR, 大聲失敗, 不會安靜失效 ✅
🔴 而 codex 打的是另一個世界:**名字沒錯, 而那個名字指到別的東西。**
⇒ 📌 **「我驗過這一格」與「我驗過的是這一格會出的那個錯」是兩件事。**
   而前者會讓人**停止在那一格找** —— 這比「沒驗」更難抓, 因為那裡有一發真的量測擋著。
✅ 實測造出那個世界:先放一個同名孤兒 sequence ⇒ 真正掛在 `id` 上的被命名為
   **`admin_saved_order_views_id_seq1`** ⇒ 寫死名字的版本會去撤【孤兒】那支。
```

## 🔴🔴 而**折完 findings 之後,測試格數三次都沒動**
```
R1 四條修完  ⇒ 34 ⇒ 34
R2 兩條修完  ⇒ 39 ⇒ 39
📌 **修 must-fix 而測試數不變 ⇒ 三綠全綠 ⇒ 看起來像「修得很乾淨」。**
⇒ 而三次都是我自己【數格數】才發現的, 沒有任何一道閘會問這件事。
✅ 補到 **39 格** + 突變 **24 ⇒ 29 發** + 並發世界 **5 ⇒ 6 個**
```
### 而補覆蓋的過程,harness 自己壞了四次
```
① 搬檔後兩支 .py 的路徑沒跟上 —— 而我「六層全重跑」時**沒跑到產生器** ⇒ 它一直是壞的
② R2 的修法改了字面 ⇒ **八發突變的錨對不上** ⇒ 產生器印了八個 ⚠️,
   🔴 **而 runner 只走「目錄裡有幾支」⇒ 它印「有問題 0」**
   ⇒ 📌 **產生器說少了八發, 而跑的人說一切正常 —— 兩個都在同一次執行裡。**
   ✅ 修法用線B 2026-08-28 給的那條:**比【我餵幾條】與【它跑幾支】**, 不同 ⇒ 整輪不算數
③ 抓「紅在哪一格」的那段我連改三次都錯(只認 `ERROR:` ⇒ 空白 / 也認 🔴 ⇒ 標籤被當失敗 /
   加「結尾非 ✅」⇒ 抓到彙總行)
   📌 **同一個字元同時當【嚴重性標記】與【結果】用, 那把尺就分不出它們。**
④ NC2 的錨帶著縮排, 而 F3 把 UPDATE 包進子區塊 ⇒ 對不上
   📌 **一個以【字面 + 縮排】當錨的東西, 會被一個純結構的重構弄壞, 而重構本身完全正確。**
```

## 🔴🔴 而最大的一格是這個:**先前每一發打 REVOKE 的守門都是恆綠的**
```
MF1(拿掉 sequence 的 REVOKE)⇒ **恆綠**。而它恆綠的理由是真的:
   拋棄式 PG 上的新物件**本來就沒有權限可撤** ⇒ 「有 REVOKE」與「沒 REVOKE」印同一個結果。
📌 **一個「乾淨」的測試環境, 會讓所有【防止髒東西】的守門都失去判別力。**
✅ 修法:在 bootstrap 裡把 Supabase 那個前提造出來 ——
   `ALTER DEFAULT PRIVILEGES … GRANT ALL ON TABLES / SEQUENCES / FUNCTIONS TO anon, authenticated, service_role`
   (依據:正式庫 2026-08-14 實測 `pg_default_acl` ⇒ 四個角色各 `arwdDxtm`)
⇒ MF1 現在紅在斷言 7c-3;而**正對照仍全綠** ⇒ 那些 REVOKE 是真的在做事。
🔴 **這一改的射程比它本身大**:它讓 `7c` / `7e` 那一整族從「無事可證」變成「有事可證」。
```

## 現況(11:04,同一個版本上)
```
apply rc=0(含 22+ 道碼錨與斷言 + 撞名世界)· 39 格行為 rc=0
突變 29 發 + 負對照 ⇒ **29/29 紅在指定那一格**, 恆綠 0 · 紅在語法錯 0
並發 6 個世界 rc=0(NC1-NC5 五發各紅在指定世界)
rollback 空表 / 帶資料 rc=0 · 合約 rc=0 · 管理者定義 rc=0
🛑 未 apply(`q5 = 甲`:Sean 自己在 SQL Editor 跑)· 未 push
給他貼的那份 `~/pcm-mailbox/SQL給Sean-saved-views建表-20260828.sql`
  ⇒ 檔內的 migration 與現行**逐字相同**(當場比對)
```

## ✅ §14-33b 第三輪 = **PASS**(Sean 2026-08-28 11:0x 批准的破例,不是照規矩跑的)
```
🔴 落檔強度:skill 寫死「round2 仍 FAIL ⇒ 停、raise Sean」——
   本輪是【他明文批的】, 不得寫成「照輪次規矩跑的」。
```
兩條修法**都不需改碼**,而它給了兩個**射程限制**,兩個我都自己補掉了:
```
① 測試缺口:MF1 證的是「動態斷言抓得到未撤權」, 而要複現舊版那個【安靜全綠】,
   要把 REVOKE 與斷言【兩邊都】改回寫死名字。
   📌 **只改一半的突變, 打的是「兩者不一致」, 不是「兩者一起錯」** ——
      而真正會出事的世界是後者:兩邊一起指向孤兒, 誰都不會叫。
   ✅ 補 MF5(兩邊一起改回)⇒ 實跑紅在撞名世界 ⇒ 突變 **29 ⇒ 30 發**
② 它說「依禁搜規則, 無法核實白名單外是否有【先鎖 saved view 再更新 staff】的反向路徑」
   ✅ 這一格我自己查得到, 而且有正對照:
      正對照 `admin_audit_log`          ⇒ **704** 支(尺是活的)
      受測   `admin_saved_order_views`  ⇒ **0**(排除本片自己那支檔)
      負對照 `zzz_never_referenced`     ⇒ **0**
   ⇒ 🔴 **這張表今天才出生, 沒有任何既有交易碰得到它** ⇒ 反向鎖序在現況下構造不出來。
   ⚠️ 而第一次量的時候我**只跑了受測與負對照, 兩個都是 0** ——
      📌 **兩個 0 之間沒有任何資訊。加正對照才知道那把尺會動。**
```
### 而它主動確認的一格值得記
```
`FOR SHARE` 的代價它算給我:**後台列表可能短暫被員工編輯阻塞, 反過來高頻列表也可能延後員工異動。**
⇒ 而它判「鎖只維持單次 RPC 交易 ⇒ 屬可接受的正確性代價」。
🔴 我照收, 而**寫下來讓它被看見** —— 這不是「沒有代價」, 是「代價被算過而接受」。
```

## 現況(11:11)
```
突變 **30 發** + 負對照 ⇒ **30/30 紅在指定那一格**, 恆綠 0 · 紅在語法錯 0
39 格行為 · 6 個並發世界 · rollback 空表/帶資料 · 合約 · 管理者定義 —— 全綠
🔴 三輪對抗審查:R1 FAIL 4 · R2 FAIL 2 · **R3 PASS**;零留痕三輪都驗過(含負對照)
```
