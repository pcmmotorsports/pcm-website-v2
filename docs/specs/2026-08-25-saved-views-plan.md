...
與 `apps/admin/src/lib/staff/staff-form.ts:37-40` 上線後仍會宣稱 `is_manager`「不是權限旗標」
...
📌 而它**改變的是要看哪裡**:①那類要問「這一片能不能讓人看到他本來看不到的單」
   ⇒ 我上面那一發(`staff_id` 命中 0)就是為了答它 —— **答案是不能, 而它是量到的。**
...
· `§13-8` 的鐵則 12① 我提了異議而仍照命中處理 ⇒ **若主視窗判我的異議成立, 那一格可以縮**
```
