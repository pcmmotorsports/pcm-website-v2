...
| `Q-檢視-10` `sort_order` 誰能拖 | **表定稿(片1)** | 🛑 **未答** | Sean(選項已備,`§14-16`) |
| `Q-檢視-13` 離職員工的私人檢視 | **可能擋表** | 🛑 **未答** | Sean(選項已備,`§14-16`) |
...
## 不擋表、但擋後面片的(不要拿它們擋片1)
```
Q-檢視-8   私人與共用同名時 UI 怎麼分   ⇒ 擋 片3(UI)
Q-檢視-9   數量上限                     ⇒ 擋 片3(版面)  ⚠️「不擋表」是推的不是量的
Q-檢視-12  固定期間 / 每次算最新         ⇒ 擋 片2
```
