...
| **B3** `cancel × unvoid` | A8a2 部分取消(抬 `cancelled`)與 unvoid(抬 `shipped`)併發 | `cancelled + shipped > quantity` ⇒ **撞 C6′**;或可取消量算在過期值上 | 兩者不得合計超出 `quantity`;拒絕訊息要指路 | 拿掉其中一側的鎖 ⇒ 撞 C6′(🔴 **W6b-2 開工時照 B2 更正的同一標準先驗此靶可構造**——單列 writer 下「鎖」可能同樣無物可拿,屆時靶改打 C6′ 本身+轉譯層;設計已備 `B-289-STOP` ⑥) |
