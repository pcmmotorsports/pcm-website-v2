...
| **W7** | ~~突變矩陣(每條斷言一發靶)+ 覆蓋帳~~ 🔴 **08-08 追加式更正(B-296 ⑤-1 主視窗裁准)**:實交=harness 級突變帳+零靶欠款清單(#341/#342)+**跑過帳** `w7-coverage.sh`(record/check,守四種真實失效:零靶族/承諾不存在的靶/跨線釘值過期/跑都沒跑);**沒有逐條靶**(379 格逐條一發靶不合理,R3 拍掉「凍結帳本」第一版)。詳 `docs/reviews/2026-08-08-w-line-mutation-matrix.md` | 否 | 45 分 |
