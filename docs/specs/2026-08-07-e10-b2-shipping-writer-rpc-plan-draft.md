# B2 出貨 writer RPC 片 —— plan **v4.2**(2026-08-07,🏁 **已定稿**)
...
| **W7** | ~~突變矩陣(每條斷言一發靶)+ 覆蓋帳~~ 🔴 **08-08 追加式更正(B-296 ⑤-1 主視窗裁准)**:實交=harness 級突變帳+零靶欠款清單(#420〔原 #341,2026-08-11 改號〕/#342)+**跑過帳** `w7-coverage.sh`(record/check,守四種真實失效:零靶族/承諾不存在的靶/跨線釘值過期/跑都沒跑);**沒有逐條靶**(379 格逐條一發靶不合理,R3 拍掉「凍結帳本」第一版)。詳 `docs/reviews/2026-08-08-w-line-mutation-matrix.md` | 否 | 45 分 |
| **W7b** | 🔴 **交棒 6 清償**(關卡1 **F5**:v1 把它列進範圍表卻**零片認領**)。~~a8a2 已釘 `ORDER BY oi.id` + NKU ⇒ 清償便宜,**補一格**斷言取鎖序,不是重寫。~~ 🔴 **本列已被本檔 `:707` R3-D4 推翻,勿照做**(a8a2 檔內已有三個 ORDER BY 結構錨,再補一格=第二個恆真錨):W7b=**跨函式行為格**(a8a2 × `shipments_summary_recompute_ac` 真併發),依賴 W3c+W4(§2a 列為準);開工入口=`B-293-STOP` ⑦。🔴 開任何片前先 grep 片名在本檔全部命中,勿只讀本表一列(§ 字面過期第五例) | 否 | ~~20 分~~ 重估 |
...
| **B2** `shipment INSERT × unvoid` | A 對品項 X 建箱掛品項並出貨;B 同時 unvoid 另一張也含 X 的包裹 | 兩邊各自前緣檢查都過,合起來 `shipped > instock` ⇒ **C9 紅在誰身上不確定**;或反序取鎖 ⇒ `40P01` | 兩者不得同時成功到超出 instock;紅的那一筆要**帶轉譯後的人話**(Q8=A) | ~~拿掉前緣檢查 ⇒ 出現 raw `23514`;拿掉鎖序 ⇒ 出現 `40P01`~~ 🔴 **08-07 W6b-1 實證更正(主視窗裁,證據=`B-289-STOP` ②③+`scripts/w6b1-ship-vs-unvoid.sh` 落地版)**:①Q8=A 之後轉譯層存在,拿掉前緣只會走 C9 轉譯路、**看不到 raw** ②兩支 writer 皆 `WHERE id =` 單列、**無鎖序可拆**(該面是 W6a 打的)⇒ 靶改打**承重牆本身**:拿掉 C9 ⇒ 兩邊都成功、`shipped 6 > instock 3`;轉譯層打成回 NULL ⇒ 輸方露 raw。真併發 4/4 前緣互看不見=C9+轉譯層才是不變式的承重牆 |
| **B3** `cancel × unvoid` | A8a2 部分取消(抬 `cancelled`)與 unvoid(抬 `shipped`)併發 | `cancelled + shipped > quantity` ⇒ **撞 C6′**;或可取消量算在過期值上 | 兩者不得合計超出 `quantity`;拒絕訊息要指路 | ~~拿掉其中一側的鎖 ⇒ 撞 C6′~~ 🔴 **08-07 W6b-2 實證更正(主視窗裁,證據=`B-290-STOP` ②+`scripts/w6b2-cancel-vs-unvoid.sh`)**:①無鎖可拿(單列 writer)②**B3 這對根本沒有危險面**——取消讀 instock/cancelled 寫 cancelled、復原讀 instock/shipped 寫 shipped,互不讀對方會改的值 ⇒ ΔC+箱量≤Q 代數恆成立、C6′ 摸不到(與 `…s2a…sql:178`「代數冗餘」註解一致)③C6′ 被 C7 嚴格遮蔽(拿掉 −instock 項撞 C7、再拿掉 C7 才 C6′ 單獨紅=非 no-op)⇒ 靶落地=C6′ 本身+C7+可取消量 `−instock` 項+轉譯層。🔴 **真正有陳舊面的是「取消 × 到貨(receipts)」**(instock 共同軸),§6a 原四組漏列 ⇒ W6b-3 補(B-290 ④-1) |
| **B4** 冪等重放 oracle | 同鍵、同 payload 的兩個請求併發 | 第二個撞鍵表 `23505` **被誤判成撞號** ⇒ 重產號 ⇒ **新建一箱**(§1d M1) | 恰一組產物;第二個**轉重放** | ~~把 §1d 的 conname 分派拿掉 ⇒ 新建第二箱~~ 🔴 **08-07 深夜 W6c 探針實證更正(主視窗裁,證據=`B-292-STOP` ③,prosrc 定位實測)**:分派點有兩個——**建箱迴圈的分派**(`…w3a…sql:171`)拿掉**不會**產生第二箱(鍵表 23505 不在該迴圈冒出);真正造得出第二箱的是 **claim 的分派**(`…w2…sql` `pcm_b2_shipping_idem_pk`)回 NULL=被當首次認領。⇒ W6c 落地**兩發都打**:①claim 分派回 NULL ⇒ ~~第二箱~~ ②建箱迴圈分派拿掉 ⇒ 證它守的是另一面(撞號),不同族。🔴 **W6c 實跑第四次更正(`B-293-STOP` ②)**:「新建第二箱」作為**持久結果**是錯的——第二箱會在 `record()` 撞 freeze trigger(shipment_id 不可改指)P0001 整筆回滾 ⇒ **牆有兩層**;疊加消融實證:只拆 claim 分派=輸家變硬錯、shipments 仍 +1;**claim 分派+freeze trigger 都拆**=兩箱都提交、鍵只指一箱=**孤兒箱**(要不要第三道=待 Sean,見 STATUS) |
...
3. 🏁 **v4.2 更新(主視窗 `B-185-A` ① 裁定;只更新事實、非改設計,定稿地位不變、不開輪)**:
   §1c-1 的 DDL **已在拋棄庫實測** —— PG 17.10、埠 54367、自建自拆、工作樹零留痕、**`PASS=17 FAIL=0`**。
   涵蓋:DDL 語法 / 正向鏈 / 五條 CHECK 各自紅在**自己**的 conname / **B2 巢狀洞實測證實擋不到**(誠實邊界屬實、
   不是保守措辭)/ 兩發 trigger / `tgtype` 基準 `block_delete=11`・`block_truncate=34` /
   TRUNCATE 突變靶翻面(拿掉那發 ⇒ 函式仍在、trigger 數 >=1 **兩種斷言全綠**,**只有 tgtype 集合比對紅**)/
   三角色 ACL / RLS。骨架在 scratchpad,**正式版隨 W0b commit**。
   🔴 **這行更新不使下列三件消失**:①五條 CHECK **只有負測、沒有消融靶**
   ②跑在**裸 PG 不是 Supabase** ⇒ ACL 三格證不了正式站的最終權限
   ③`pcm_b2_is_blank` 是抽字面**重建**、非 replay migration ⇒ **與正式站那支的等價性未驗**(W0b 正式版改 replay 驗)。
4. 🏁 **v4.1 已定稿**(主視窗 `B-184-A` 口頭確認)。本行取代原「定稿候選」字面。
