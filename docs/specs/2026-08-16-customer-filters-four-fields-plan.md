...
>
> ✅🔴 **2026-08-26 加註(線4 `-1d`)—— 本檔上面那三個字【已被 Sean 本人改掉】**
> ```
> ~~不願透露~~ ⇒ **不透露**(Sean 2026-08-26 `q3=乙` 定案)
> ```
> **本檔原句留著不刪 —— 它是本檔作者當時如實記的,不是記錯。**
> 經過:同一個決定 Sean 2026-08-26 又被問了一次(`~/pcm-mailbox/DECISIONS-Sean-2-20260826.md` 的 `Q11`),
> 他答的一樣是甲,而字面不同 ⇒ 端回去請他定 ⇒ 他選了「不透露」。
> ⇒ 📌 而**重問這一題的是線4**,原因是**寫新規格時沒有 grep 既有拍板** ——
>    紀律逐字「先 grep 既有拍板不重問」寫在 `docs/working-style.md`,**而它沒有機制。**
>    **這一次的代價是 Sean 多答了一題。**
>
> ⚠️ **本檔其餘內容未動、未過期判定** —— 它仍是「等 Sean 批」。而它的年齡那一片
> 與 `2026-08-26-customer-gender-birthday-spec.md` §1-4 **是同一件事的兩份寫法**,
> 開工前必須先合併, 見該檔 §1-3b。
>
