...
>
> 🔴🔴 **2026-08-26 加註(線4 `-1d`,主視窗 `-96` 裁)——【字面待統一,不要照本檔的字先寫下去】**
> **同一個決定 Sean 2026-08-26 又被問了一次**(`~/pcm-mailbox/DECISIONS-Sean-2-20260826.md` 的 `Q11`),
> 他答的一樣是甲,而字面是「**不透露**」不是本檔的「**不願透露**」。
> ⇒ **兩次都是他答的、兩次都是甲 ⇒ 不用「較新為準」蓋掉任一組, 端回去問他。**
> ⇒ 🔴 **在他回話之前,本檔與 `2026-08-26-customer-gender-birthday-spec.md` 兩處都不得先寫死。**
> ⇒ 📌 而重問這一題的是線4, 原因是**寫新規格時沒有 grep 既有拍板** ——
