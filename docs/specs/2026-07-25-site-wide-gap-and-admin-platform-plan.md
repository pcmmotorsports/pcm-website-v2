...
2. 🟡 **首頁大圖是 Unsplash 圖庫圖**,網址和文案都寫死 → 你想換,現在做不到。✅ **授權我已親查**(可商用、免標註),但它是**熱連結**——原作者刪圖我們首頁就破,而且不會有人通知我們。
...
**要你早上決定的事** → §9,共 **6 題**(v1 六題中:退刷線題移出本檔、「最新商品」題因前提錯誤刪除;新增「發布模型」「手動商品邊界」「價格窄路」「內容改動頻率」)。

---

## 0b. 🔴 Sean 2026-07-25 拍板(§9 六題全答 + 追加需求;本節為權威、覆蓋 §4/§7 相衝字面)

| 題 | 拍板 | 對規劃的影響 |
| --- | --- | --- |
| Q1 施工順序 | **A** 先修搜尋(E0)、再開內容管理 | E0 排第一 |
| Q2 圖片存哪 | **A** 沿用 ADR-0004 = **Supabase Storage** | E3 不必再比較 R2;不推翻既有 ADR |
| Q3 Unsplash 首頁大圖 | **A** 等內容管理時換自有照片;在那之前先轉存自有空間避免熱連結破圖 | E4 含「轉存」小步;授權已查證合規 |
| Q4 內容發布模型 | **A** **草稿 → 預覽 → 按發布才上線 → 可一鍵回上一版**;AI 改的一律先當草稿 | 🔴 決定 `site_content` schema:需版本表 + 狀態 + 發布/回滾 RPC |
| Q5 手動商品邊界 | 🔴 **B 完整商品**(公開可搜尋、可下單、可管庫存、可下架)——**推翻 Claude 推薦的 A** | 🔴 **E6b 大幅擴張**:等於做一套完整商品後台 + 處理與供應商同步的邊界。Sean 知情選 B |
| Q6-1 手動商品價格 | **A** 後台專屬表單、每次設價留紀錄(誰/何時/多少/為什麼) | 對應藍圖 `M-4a-13`「改金額紅線」;仍**排除**在內容 override 之外、走獨立窄路 |
| Q6-2 首頁內容改動頻率 | **B** 一季 1-3 次 = **L2** | 🔴 內容管理**非鐵則 9 強制**、可排在搜尋與商品之後;E2 屬「Sean 要的便利性」非規則要求 |

### 追加需求(Sean 同批提出)

1. 🔴 **搜尋要「模糊搜尋」且涵蓋:內文、標題、料號、車款、年份、類似關鍵字**;並指示「**參考我們報價單的方式,或者有可以帶入的現成工具**」。
   ⇒ **E0 範圍實質擴張**:原 plan 的 E0 只接既有 `searchByKeyword()`(ILIKE 三欄 title/subtitle/description),**達不到這個要求**(無料號、無車款年份、無模糊/近似)。→ **另立搜尋引擎方案偵察與 plan**(進行中),E0 改為依該 plan 執行。
2. **首頁大圖要能加連結或影片**(Sean 問「可以吧?」→ 可以)。實查:`HomeHero.tsx:29-32` **已有連結**(`<Link href="/products?filter=new">`「Discover the collection」)、`:40` 另有 scroll anchor;圖為 `<img>`(`:13-15`)包在 `.ed-hero-media` 內、外覆 `.ed-hero-tint`。
   ⇒ **影片可行且改動小**(`<img>` → `<video muted autoplay loop playsinline poster>`,版面與 tint 層不動)。🔴 **內容 schema(E2)必須預留欄位**:媒體型別 `image|video`、媒體 URL、poster URL、eyebrow、標題行(多行)、連結文字、連結目標、meta、以及「整張圖是否可點」旗標。
   🔴 **四項實作注意**:①手機自動播放必須 `muted` + `playsinline`(否則 iOS 不播)②影片拖慢 LCP → 短 loop 5-10s、≤2-3MB、必附 poster fallback ③Supabase Storage 影片**每次載入都計頻寬**(首頁流量大需評估)④須尊重 `prefers-reduced-motion`(無障礙)。

### 施工順序(依拍板重排)

**E0 搜尋(依新搜尋 plan)→ E5 權威鎖地基 → E6a 同步商品 override 後台 → E6b 完整手動商品(Q5=B)+ 價格窄路(Q6-1) → E7 AI 審核 → E3 圖片上傳 → E2 內容管理(L2、含發布模型)→ E4 首頁媒體 → E1/E9/E8 穿插**
(理由:Q6-2=B 把內容管理降為 L2 → 讓位給搜尋與商品;E3 圖片上傳提前到 E2 之前因 E4 首頁換圖/影片依賴它。)
...
🔴 **兩個要揭示的問題**:
1. **首頁 8 元件檔頭皆無 L1/L2/L3 分級註解** → 從未經鐵則 9 分級盤點。⚠️ **v2 修正推論**:鐵則 9 說的是「**若**內容是 L3 → 必後台 CRUD + 先寫 PRD」(`PHASE-1-NORTHSTAR.md:196-206`),**不是**「想做後台就自動屬 Phase 1」。⇒ 首頁內容是不是 L3,取決於 **Sean 實際想多常改**(→ §9-Q6-2)。
2. **HomeHero 用 Unsplash 圖** → ✅ 授權已親查合規(§9-Q3);真正風險=**熱連結破圖**。
...
🔴 **v3 補充交叉證據(2026-07-25 更新進度地圖時發現,強化上表判定)**:Phase 1 藍圖(`~/.claude/skills/pcm-roadmap/roadmap-data.json`,結構真權威=`docs/PHASE-1-MILESTONES.md`)的 **M-4a 員工後台 13 個 slice 中,已明列且狀態為 todo**:

- **`M-4a-06` 後台商品列表 + 編輯(內文 / 圖片 / 售價 / 經銷價)** ← 就是 Sean 要的商品管理
- **`M-4a-07` 後台商品待審核 + 通過 / 退回** ← 就是 E7 的 AI 提案審核流骨架
- `M-4a-02`~`05` 後台版面/儀表板/收件匣/數據分頁 ← E8 dashboard
- `M-4a-13`「改金額紅線 —— 員工不能改金額,要通知你」← **與 §9-Q6-1 價格窄路同一件事、且藍圖早已把它獨立成一條**

⇒ **「後台商品管理」與「審核流」本來就在 Phase 1 藍圖裡、不是新的範圍擴張**;真正屬擴張的只有 **E5 同步覆寫層(override)** 與 **E6b 手動可販售商品** 這兩項藍圖未涵蓋的機制。上表 E6a 因此可下修為「Phase 1 內(對應 M-4a-06/07)」、E7 亦然。
⚠️ **同時揭示**:M-4a 13 slice 目前 **只完成 3**(`M-4a-01` 骨架 / `M-4a-08` 訂單列表 / `M-4a-10` 會員列表);Sean 這次要的東西**幾乎就是 M-4a 尚未動工的那 10 個 slice**。
