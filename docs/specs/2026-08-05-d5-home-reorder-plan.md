...
| — | 頁尾 | `HomeFooter` | 石墨 `#202225` | **D7 做**,本片不碰;✅ D7 已於 2026-08-05 由全站重設計線 0b 完成 |
...
| ~~**D5b**~~ | ~~N°02 最新商品:區塊編號 + 條件標題~~ 🔴 **整片取消**(§5 Q5 撤案);區塊編號在實作時已提前併入 D5a | — | — | — |
...
| **D5f** | N°06 授權代理:純文字牆 → 20 家 logo 磚牆 | `BrandIndex.tsx` + CSS + test | 45 分 | D5a、**Q2**(本欄原誤植 Q5,以 §5 拍板表為準) |

~~**D5a 可以立刻開工**(零決策依賴)。D5c/D5d/D5e/D5f 各自卡一題,見 §5。~~
🔴 **2026-08-05 更新:五題已全數拍板**(見 §5 開頭的拍板結果表),D5b 取消、其餘各片可依序開工。
...
2. **頁尾不是 `© 2026`,是 `© MMXXVI`**(羅馬數字,`HomeFooter.tsx:76`)。
   ...
   ✅ **已解決**:0b 執行 D7 時改成 OD 全站頁尾逐字的
   「© {年份} PCM MOTOR PARTS LTD. 版權所有」+「統一編號 90003020」——
   R2-3 表列明訂「MMXXVI 羅馬數字寫法作廢」,所以那個視覺變更是拍過板的、不是誤傷。
...
## 5 · 🔴 停下等 Sean 拍板(五題)—— **已於 2026-08-05 全數答覆**

> **拍板結果**(主視窗轉達,信 `/Users/sean_1/pcm-mailbox/C-84-A.md`;下方原題目保留供追溯,不代表待辦):
>
> | 題 | 答案 | 對應片 |
> |---|---|---|
> | Q1 | **B — 不報數**。字面用 Sean 已過目的例句「原廠正式代理,每件附序號與保固卡」為預設,肉眼驗可再改 | D5d ✅ |
> | Q2 | **A — `brands-trim` 去背原色 20 檔** | D5f |
> | Q3 | **C — 先實測真站分類數**,結果寫信回報、Sean 再選 12 或 8,批了才動格數 | D5c(前置量測) |
> | Q4 | **A — 完整做**。20 家文案由 Sean 去 Open Design 生;施工端先做「資料驅動殼 + 每 3 天輪播」,當期固定 RIZOMA ⇒ 自行拆 D5e-1 / D5e-2 | D5e |
> | Q5 | **撤案** ⇒ **§3 的 D5b 整片取消**(條件標題不做、`garage` 傳參也不需要了)。§3 表格那一列與本節 Q5 一併作廢 | ~~D5b~~ |
> | Q6 | **A — placeholder 改弱化色**(不在本檔切片表內、獨立輕量片) | ✅ 已做 |
>
> 🔴 **母計畫有一處字面因 Q1=B 而過期**(本檔不改他人檔案,在此標註):
> `docs/specs/2026-08-03-storefront-home-brand-page-wire-plan.md:271` 寫「D5(數字對齊 20)」
> —— Q1=B 之後**不報任何數字**,照著做會做錯。
> (同檔 `:111` 逐字是「數字必須對齊到 20**或改成不報數**」= 本來就含 B 案、**未過期**;
> R2 抓到我上一版把它一起說成過期,是超出事實的宣稱,已更正。)
> 真正的落檔處 = `docs/design-storefront-manifest.yaml` 的
> `HomeStatement.business_overrides.brandCountClaimRemoved`。
