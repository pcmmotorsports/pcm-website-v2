...
### §4-1 🏁 `473b-2` 已完成(2026-08-14),但**留下一個已知的單向可見**

- ✅ 已做:異常清單納入卡住的列(**兩支獨立查詢**,不是加寬一支 —— 合成一支會讓 ②類累積後餓死 ①類,關卡2 codex must-fix)、卡住的列**零按鈕**、文案敘述「當初的判定」而非「錢的狀態」。
- ⚠️ **未做(刻意不擴片,要一個 backlog 號)**:訂單頁帳本區塊的「⚠ 已進退款異常清單」徽章掛在 `isRefundException`(processing-only,`refund-ledger-view.ts`)⇒ **卡住的列在清單裡、訂單頁卻不說**。
  **不修未來會痛在哪**:員工從訂單頁看不出這筆退款已被列管,只有主動去開異常清單才知道 —— 而異常清單是「值班才會開」的頁面,訂單頁才是他每天在看的那頁。⇒ 兩邊說法不一致時,他會相信訂單頁。
  **修法**:訂單頁對卡住的列掛第二種徽章(文案不同:那列**沒有動作可做**),與 `473c` 的「哪個是現況」顯示同片做較省。
