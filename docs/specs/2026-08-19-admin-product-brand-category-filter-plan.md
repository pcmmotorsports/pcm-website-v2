...
## §0 🔴 開工前第一步:兩格我沒有證據,**不准用推的補**

這兩格的答案會改變 plan 本身,所以它們是**第一個動作**,不是「順便驗一下」。

| # | 要量什麼 | 怎麼量 | 答案怎麼改變這份 plan |
|---|---|---|---|
| 0-a | **填充率** | Sean 那批查詢已含此發:`select count(*), count(brand_id), count(distinct brand_id), count(category_id), count(distinct category_id) from products;` | 🔴 有分類的列數佔比低 ⇒ **這片不該開**,該做的是補資料(可能牽到上架管線)。**先看數字再決定要不要動工** |
| 0-b | **分類有幾層**(現有字面看起來是兩層,**未量**) | `select max(jsonb_array_length(segments)) from categories;` 併 `select count(*) filter (where parent_category_id is null) from categories;` | 回 2 ⇒ §3 的「大類/子類兩顆下拉」成立;回 ≥3 ⇒ **§3 要重寫**(三層以上不能用兩顆下拉表達) |

⚠️ 現有 migration 裡看得到的字面全是兩層(`'引擎部品 · 排氣管'`),**而那是樣本不是全集** ——
`init_brands_categories.sql:40` 的 `parent_category_id` 自參照**允許任意深度**。
...
```
① §0-a 填充率 —— 等 Sean 貼查詢結果
② §0-b 分類層數 —— 等同一批查詢
③ 「品牌數有多少」⇒ 20 個 vs 2000 個是不同的 UI(下拉 vs 搜尋式選擇器)
   ⇒ 同一發 SQL 的 count(distinct brand_id) 就答得出來
④ G3 分頁片的收工 HEAD —— 本片從那裡起手
```
