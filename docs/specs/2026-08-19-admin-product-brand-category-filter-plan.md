...
✅ **一條過期註解已修**(2026-08-19 同日,單獨一顆 commit):
`apps/storefront/src/lib/category-taxonomy.ts` 的檔頭一度寫著「真分類為**單層**、無子類」
(2026-07 的字面)—— 而它就在「怎麼建分類樹」那支檔的檔頭,**下一個做分類的人一定會讀到**。
已改成現值 + **一發可跑的檢查**(`count(*) filter (where parent_category_id is not null)`),
讓下一個人不必信檔裡寫的字。舊字面**刪掉不留劃線** —— 它在檔頭,留著的話順讀的人先讀到錯的。
