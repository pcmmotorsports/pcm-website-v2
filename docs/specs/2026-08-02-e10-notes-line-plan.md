...
### 7.1 guard / mutant manifest(**[27][44][45][47][48] + F1/F2/F4/F5/F8 + 關卡2 R1 折入**:**64 格**、9 分區、有限互斥)

**格 = 守門;格數 = 64 = 14(A)+7(B)+11(C)+8(D)+**7(E)**+10(F)+3(G)+3(H)+1(I)**
(E 區因關卡2 [11] 加 **E⑦ RETURN 字面集合恰 14 碼** ⇒ 6→7)。
**突變 = 73**,推導 = 帶突變的格 60(64 − F⑥縱深 1 − H 自檢 3)各 1 = 60,
D⑧ 改帶 13(+12)、G① 帶 2(拿掉 + 註解化,+1)⇒ 73。
🔴 **v3/v4 原本寫的「63 格 / 74 突變」是我算錯的字面** —— 實作實際只有 71 個突變(折入後 73);
harness 現在同時釘 `EXPECTED_CELL=64` / `EXPECTED_MUT=73` / `EXPECTED_TOTAL=157` **三數**
(關卡2 [10]:只釘總數的話「刪一個 mutant + 補一個無關 ok」照樣過)。增減守門必先改本表再改三個釘數。
宣稱修正(**F5**):~~每格一專屬紅點~~ → **除 F⑥(縱深,紅點歸 F⑤)與 H 區(自檢在 §0)外,每格有專屬紅點**。
