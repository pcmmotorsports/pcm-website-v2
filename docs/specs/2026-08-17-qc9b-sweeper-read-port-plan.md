...
>
> ## 🔴🔴 而在動手之前:**這條線今天沒有收件人**(2026-08-18 補,`#633`)
>
> **本檔原本一個字都沒提這件事** —— 而**實作的人打開的是本檔,不是母 plan**。
> ⇒ 所以在這裡放一份指標:
> ```
> 寫入端兩處把收件信箱【寫死成 null】，而 code 自己的註解說那是刻意分階段：
>   packages/adapters/src/supabase/mappers/order.ts:144-146
>   apps/storefront/src/app/checkout/charge-actions.ts:266-267
> 🔴 在 B-4 落地之前，之後每一張新單也都會是 null
> ⇒ 本片做完之後，寄信時去讀那個欄位會【永遠讀到 null】
> ⇒ 那不是「少數客人收不到」，是【一個都收不到】，而畫面與測試都不會紅
> ```
> 🔴 **它的形狀**:**它接通了,而它送 `null`** —— 比「沒接通」難發現,**因為每一層都會回報成功**。
> **判別句:我驗的是「這條路有沒有跑到底」,還是「跑到底的時候帶著真的值」?**
>
> 🔴 **所以本片的卡點【不只是 composition 那一行的授權】**:
> **就算那一行批了、做完了、測試全綠,也驗不到「客人真的收到」。**
> ⇒ **驗收條款的硬規定在 `#633`,不要在這裡自己發明一個** ——
> **「`email_outbox` 有列 = 成功」在 `recipient_email` 是 `null` 時【照樣綠】。**
> 📎 完整證據、旗標為什麼不是那個閘、DB 現況 0/13:**全部在 `#633`**,本檔不複製一份
> (複製了就會漂)。
>
> ---
>
