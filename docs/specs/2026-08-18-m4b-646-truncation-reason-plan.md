...
> # ✅ **狀態:Sean 已批 2026-08-18 18:3x —— 可以開始做。**
>
> **他的逐字**:`q5: A: 現在做 —`
> **送到他面前的選項原文(引用這一板必須連這段一起引)**:
> ```
> Q5 這件事現在做，還是排到上線後？
>    A: 現在做 —— 它屬於「員工的一天」，而那是你定的上線判準
>       代價：要動到訂單取消那條路的安全檢查 ⇒ 高風險片，要跑對抗審查，大概吃掉一個窗一段時間
>       風險可回收：單一 commit，不對就 revert，沒有資料庫變更
>    B: 排到上線後 —— 現在只有你在用後台，痛的人只有你
> ```
> 主視窗轉述他採納的理由 = 本檔 §2 那句:**「一個『有時候對』的指示,比一個一直錯的指示更難被發現是壞的。」**
>
> 🔴 **他批的是「可以開始做」,不是免審。** 片型 = **高風險片(全 9 步)**、鐵則 12 ① 命中
> (碰 order 取消路徑的 fail-closed 閘 `zeroInferable`)⇒ **commit 前跑 codex 關卡2,不降級。**
> 🔴 **技術面甲/乙由主視窗裁定 = 乙**(2026-08-18 19:0x),不必再問任何人。
>
> ~~2026-08-18 18:4x 由 G2 寫,【尚未批准】~~(歷史狀態,保留)。本檔原本不裁定甲/乙、兩案並列。
...
⑤ 四綠 + vitest 全綠
⑥ 🔴 **角色②(型別判別欄)不得跟著消失** —— 逐字從 §6 搬進驗收表(主視窗 2026-08-18 19:1x 指示:
   「風險段寫了不等於驗收表有」):
   乙改形狀之後，`AdminOrderLine`（列表投影的品項）**仍然餵不進取消邏輯**，
   而這件事要用 `cancel-view.test.ts` 那條 `@ts-expect-error` **實證,不能用推的**;
   🔴 突變自驗要跑 `tsc --noEmit`（**vitest 看不到這一格**）——
   突變 = 把那道型別差抹掉 ⇒ `@ts-expect-error` 變成「未使用的指令」⇒ TS2578 ⇒ typecheck 轉紅。
```
