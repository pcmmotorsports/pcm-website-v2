...
⇒ 前提:§10-A 已裁 A(2026-08-19,**代裁、Sean 可推翻**)。若之後改判 B,這一段跟著作廢。
```
...
### 7-依據 🔴 **每一步的體積依據**(主視窗指定:要依據不要更準的數字)

> 規則:**答不出依據的那一步,明寫「這一步是猜的」。** 一個好看的總數沒有用。
> 🔴 **本節不重估總數。** 下面的數字就是 §7 那些數字,只是把它們的來源攤開。

| §5 步 | 體積依據(**量到的**) | 有沒有可抄的既有實作 |
|---|---|---|
| **0** 移檔 | `git mv` 1 支;連帶要改的紙 **7 處**(數法:`bash scripts/literal-sweep.sh '20260819010000_m4a_close_manual_review_attempt'` ⇒ 總計 7 命中 / 掃 4127 檔) | ✅ 慣例已存在(`scripts/` 底下多支刻意不放 migrations 的 `.sql`) |
| **1a** 告警函式 | 🔴 **整支要重貼 ~105 行**(`104` CREATE → `208`;PG 沒有「只改一個子查詢」的語法 —— 該檔 `:210` COMMENT 自己寫著) | ✅ 前一版本體可對照 |
| **1b** app 層鏈 | **6 個下游檔**(清單見 §6-2)+ `gen types` + 更新 `l5b0-verify.sh:51` 的指紋 | ⚠️ 逐欄 `parseCount` 是機械活,可對照既有欄位 |
| **2** admin action + 留痕 RPC | 🔴🔴 **見下方「這一步的依據推翻了它自己的估時」** | 🔴 **admin 側零** |
| **3** C 族出口 RPC | 抄本 `20260624120010`(`close_released_attempt`)**全檔 174 行**(數法 `wc -l`)⇒ 新的那支同量級 | ✅ 形狀完整可抄(§10-① 已認定它是對的形狀) |
| **4** R 族決策落地(已裁 A) | 清單頁多一段說明文字 + 寫出那支 RPC 怎麼呼 | ✅ 純文案與文件 |
| **5** 清單頁 | 🔴 **這一步是猜的** —— 我沒有盤點 admin 既有清單頁的樣板密度,也沒量過要幾個元件 | ❓ 未查 |
| **6** 沙盒刷一筆走完補入帳 | 🔴 **這一步是猜的** —— 沒有人跑過那條 path,連「刷一筆要多久」都沒有樣本(G3 逐字:實測 = 0) | ❌ 無樣本 |

#### 🔴🔴 步 2 的依據**推翻了它自己的估時**(~60 分)
```
§3-2 寫「與 reconcile-actions.ts:101 同一條路,逐字重用」——
而那支檔在【storefront】,不在 admin:
  find apps packages -name reconcile-actions.ts   ⇒ apps/storefront/src/app/checkout/reconcile-actions.ts (118 行)

更硬的一格:**admin 側沒有任何東西碰過 settleCharge**
  grep -rl settleCharge apps/admin/src      ⇒ 0     ← 雙向表演:
  grep -rl settleCharge apps/storefront/src ⇒ 17    ← 同一把尺對該命中的東西會動
而它的組裝根 getSettleChargeDeps 只活在 storefront:
  apps/storefront/src/lib/payment/composition.ts
```
⇒ **步 2 不是「照抄一支既有 action」,是【在 admin 側建一個還不存在的組裝根】。**
...
#### 本節的證據等級
```
· 表格裡標「量到的」那幾格,數法都寫在同一格,可重跑
· 步 5 / 步 6 明寫【這一步是猜的】—— 不是謙虛,是真的沒有依據
· 步 2 標【依據不足】—— 它比「猜的」更糟:原本的數字有一個【錯的】依據
· 🔴 總計 ~11 小時**沒有改**(主視窗指定不重估)⇒ 它現在是一個
  【已知至少一格站不住】的數字。引用它要連這一句一起帶。
```
...
### 10-A-裁 ✅ **已裁:A**(2026-08-19)

> 🔴 **【代裁,代裁人 = 主視窗 MAIN,Sean 可推翻】** —— 標成代裁是刻意的:
> **那個標記就是 Sean 推翻它的入口。** 不要讀成「Sean 拍過了」。

```
裁定：A —— 維持 owner-only,後台清單【只顯示、不給鈕】
理由：採用 G4 的理由,不是保守 ——
      B 要推翻的是【寫在 COMMENT 裡的刻意設計】
      (20260624120010_m3_3ds_r1c3_close_released_attempt.sql:139
       逐字「Phase 1 受控人工流程」),而【沒有「情況已改變」的證據】。
```
🔴 **而那個「沒有證據」的限定要留著,它是這一裁的解除入口**:
```
G4 自標:「沒找到」= 未窮舉 —— 我只讀了那支 migration 的 COMMENT 與 guard,
        沒有掃過其後所有相關 migration ⇒ 【未確認】。
```
**解除條件(寫出來,否則它會變成永久預設值)**:
```
有人拿出「Phase 1 受控人工流程」已不適用的證據 —— 例如 Sean 說員工要能自己清 R 族
⇒ 那時才重開 B。**在那之前不要再問這一題。**
```

### 10-A-驗 🔴 選 A 之後多出來的驗收條件(不是 no-op)
```
清單頁對 R 族那幾列【必須明說「這一族要找 Sean 處理」】——
**不能只是把鈕拿掉。**
⇒ 否則員工看到一列不會動的東西,只會多打一通電話。
📎 這條接 memory `project_admin-ux-operation-intuitiveness`(操作直覺化=後台常設準則):
   驗收句是「不用人教能做對嗎」,而「一列沒有鈕、也沒有字」答不出那一題。
```
...
🔴🔴 **而 GR 在 R2(GR-048)明說它這輪【也沒複驗】那四條** —— 理由是那是它 R1 自己給的敘述,
   **由它複驗等於自驗**。⇒ 這四條的座標到現在是 **零獨立查核**。
   ~~**不是「應該對」,是沒有人查過。**~~ ✅ **2026-08-19 已由 G4 逐條核完,見 §15-1b。**

### 15-1b ✅ **S5 / S6 / S7 / S8 的座標,2026-08-19 由 G4 逐條開檔核完**
> 由來:GR 在 R2(GR-048)自標「那是我 R1 自己給的敘述,**由我複驗等於自驗**」;
> 主視窗裁「那四條由 G4 在動手前逐條核」。**這一格從「零獨立查核」變成已核。**
> 🔴 **結論:GR 沒有一條說錯。** 而有**兩個數字要補掃描範圍**,否則下一個人重跑會對不上。

| 條 | GR 說的 | 我量到的 | 判 |
|---|---|---|---|
| **S5** | `manual_reviewed_at` 全樹只 1 個檔 | `git grep -l` ⇒ **2**(那支死 migration + **本 plan 自己**) | ✅ 結論成立,分母要標範圍 |
| **S5** | 負對照 `attempt_manual_review_count` ⇒ 5 個檔 | 全樹 **14**;`-- 'supabase/migrations/*'` ⇒ **5** | ✅ **GR 的分母是「只掃 migrations 目錄」** |
| **S7** | MAIN-052 §4-b 授權三句 | `:157/:158/:159` 三句逐字都在 | ✅ |
| **S7** | 測試卡與流水帳骨架已在 | `docs/reference/tappay-sandbox-test-cards.md` 存在,123 行 | ✅ |
| **S8** | 舊 plan `:78` 甲乙丙 = 三互斥方案 | `:78` 逐字「甲(推薦)…\| 乙 新增「已處理」欄 \| 丙 直接清 `needs_manual_review`」 | ✅ |
| **S8** | 同檔 `:86`「丙 的致命處」 | 逐字「🔴 **丙 的致命處**…三個月後有人查『這筆為什麼消失』——**查不到**」 | ✅ |
| **S8** | `triage:70` 甲/乙 = 兩資料族 | 逐字 `CASE WHEN a.status = 'pending' THEN '甲' ELSE '乙' END` | ✅ |
| **S8** | 「甲+乙+丙」只命中 MAIN-052 與 G4-008 兩封 | 現在 **5 封** | ⚠️ 見下 |
| **S6** | §5/§14-4 順序矛盾、§7 不含移檔 | 已在本輪改寫中修掉(§5 第 0 步 / §7 已含) | ✅ 已消滅 |

#### ⚠️ 那個「兩封 → 五封」不是 GR 數錯
```
多出來的三封是 GR-043 / GR-044 / MAIN-053 —— **全部是【討論這件事本身】的信**。
⇒ GR 量的那一刻是對的;數字長大是因為【我們在談它】。
📌 這是「訃聞會被掃進待辦數」的同族(literal-sweep §5 也警告過):
   **同一個字面,在『還沒改』與『正在被討論／已標作廢』兩種狀態下長得一樣。**
⇒ 下一個人重跑這一發要先分堆,不要直接讀成「還有五個地方沒改」。
```

#### 🔴 而 S5 的結論**比 GR 寫的更強**(這一格是我核出來的)
```
manual_reviewed_at 全樹 2 個檔,而那 2 個是【死 migration】+【本 plan 自己】
⇒ **零個 app 層 / script / adapter 消費者。**
對照 attempt_manual_review_count 全樹 14 個檔,分布:
  supabase/migrations 5 / docs/specs 4 / scripts 2 / packages/adapters 2 / packages/use-cases 1
⇒ 真正在用的欄位長這樣;而 manual_reviewed_at 一個消費者都沒有
⇒ **「它是遺物」不只是推論,是量到的。**
```
...
## 19. 本片挖出的三個判別句(**候選區,尚未升進 `docs/patterns/`**)

> 🔴 **狀態:未立法。** 放這裡是為了不讓它們只活在訊息裡;
> 要不要升進 `docs/patterns/guard-and-instrument-traps.md` **由主視窗裁**(那支檔別人正在動,我不碰)。
> 三條都有**當天的實例與量法**,不是感想。

### 19-1 **我掃的地方,是這件事會發生的地方嗎?**
```
實例:我要查「這支 cron 有沒有人排」⇒ 去掃 vercel.json
     find . -name vercel.json ⇒ 3 份;逐份 grep -c crons ⇒ 全 0
     ⇒ 我下了「沒有人排」。
🔴 那個 0 是【真的】—— 而排程在 **pg_cron**:
     20260723120000_m3_s2_settle_sweep_pgcron.sql:128
     cron.schedule('pcm-settle-sweep', '*/2 * * * *', …)  ← 每 2 分鐘,一直都在
```
📌 **這不是「掃得不夠廣」,是【掃錯地方】** —— 掃再廣的 `vercel.json` 也不會出現 pg_cron。
⇒ 落筆寫「查無 / 沒有人 / 零命中」前,先答:**這件事【會發生在哪裡】?我掃的是那裡嗎?**
⇒ 答不出來 ⇒ 只能寫「**我掃的那個地方沒有**」,不能寫「沒有」。

### 19-2 **我引的那一行,`sed -n '<行>p'` 印出來是不是我說的那段字面?**
```
實例:同一份 migration,我引錯【五次】(fable 指名三處在 code、我自己又掃出兩處在 plan)
     我寫 :13 / :11 / :13-17  ⇒ 實際 :92 / :90 / :94
🔴 病灶:那份檔前 40 行**全是設計散文**,而散文與 SQL 講【同一件事】
     ⇒ 引錯了**讀起來完全對** ⇒ **零機械訊號**
```
⇒ 機械判準:`sed -n '<行>p' <檔>` 印出來**以 `--` 開頭 ⇒ 我引的是註解,不是 code**。
⇒ 這條**可以做成機制**(引用格式 `檔:行` 時自動印那一行),而我沒做 —— 那是機制題。

### 19-3 **這一發檢查讀的東西,和那一發 commit 讀的東西,是同一份嗎?**
```
實例:我跑了規定的兩道 —— `git diff --cached --stat`(印 30,通過)+ pathspec commit
     而 commit 進去 **70 行**,多的 40 行是別的窗在中間寫的
🔴 病灶:`--cached` 讀【index】;而 `git commit -- <path>` 取【工作樹】
     ⇒ **兩道檢查與那一發 commit,讀的是兩個不同的世界。**
```
⇒ 配對規則:用 pathspec commit ⇒ 檢查要用 **`git diff --stat -- <路徑>`(工作樹)**;
  `--cached` 對它**零判別力**。
⚠️ **而配對之後仍有窗口**(檢查與 commit 之間別人還是能寫)
  ⇒ 🔴 **真正治本的是【共用檔別多人同時寫】,不是再加一道檢查。**
  ⇒ 這一條**我不自己改規範**,只提供病例與判別句。

### 19-4 **我這個數字,有沒有第二把【不同原理】的尺對過?**
```
實例(2026-08-19,我自己抽驗 17 顆 commit body 時):
  前 11 顆:grep 配 **實跑 vitest 37 passed** ⇒ 兩把不同原理 ✅
  後 6 顆:grep 配「grep 一個不存在的字面回 0」
          ⇒ 🔴 **那仍然是 grep 對 grep** —— 我當時以為做了雙向表演
          ⇒ 而**雙向表演可以在同一把尺上完成**,那時它證的是【尺會動】,不是【尺量得對】
  補救:改成**開檔讀那一節** ⇒ grep 回 1 只證明「那串字在」;
        開檔讀才證明「**那段話長成它宣稱的形狀**」
```
🔴 **而「查核」這個動作有職業病**(主視窗指出,同一小時兩個實例):
**查核者傾向用他熟的那把尺,而熟的尺是為【別的事】磨的。**
```
G4 數測試 grep -c "  it("  ⇒ 29   實跑 ⇒ **37**(pattern 綁死縮排)
GR 數圍欄 只認頂格         ⇒ 40   含縮排 ⇒ **42**
⇒ 兩個人都在【檢查有沒有造假】的時候用了窄尺
🔴 危險的方向是固定的:**查核用的尺出錯,結論會偏向「沒問題」**(少數到 = 少發現)
```
⇒ 判準:**grep 對 grep 不算;grep 對【實跑】或【開檔讀】才算。**

### 19-5 四條的共同形狀
```
① 掃錯地方   ② 引錯層(散文 vs 本體)   ③ 讀錯世界(index vs 工作樹)
⇒ 全部都是【量具對準了錯的東西】,而**三者的輸出看起來都正常**:
  0 命中是真的 0、行號格式是對的、--cached 的數字是真的。
🔴 **沒有一個會報錯。** 這正是 `docs/patterns/guard-and-instrument-traps.md` 的母題:
  **錯的那次和對的那次長得一樣。**
```
