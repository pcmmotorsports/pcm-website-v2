...
**⚠️ 觸發門檻**:一張單的品項數要超過專案的 `max-rows`(~~repo 記載 production 實測 1000~~ ⇒ **2026-08-18 起是 `2000`**)。
> 🔴 **值不寫在這裡** —— 以 `packages/adapters/src/supabase/mappers/order.ts:406` 為準,那裡是被大家引用的來源。
> 這裡只留一個指標,**下次它再變的時候不必回頭改這一行**。
⇒ **比明細那條(我方常數 200)難踩得多。**
