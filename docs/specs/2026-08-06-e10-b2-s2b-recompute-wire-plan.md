...
> 狀態:**v2;關卡1 已跑 codex R1(FAIL 21+4)與 R2(FAIL 19+1),兩輪 findings 全折入;R3 待跑(換模型換角度)。不得開工**。
...
| **S2b-4a** | 非 M | `a4a-verify` 判別力修復(N8 錨、收尾零殘留閘、`proacl` 假綠) | 否 | 40 分 |
...
| 23b | 收尾零殘留閘擴成**六函式 / 五 trigger** / 四軸 oracle / 候選全集(數字理由同項 21b) |
...
| 片 | 認領的驗收項 |
|---|---|
| **S2b-4a** | 23、23b(先做,見 §2 封窗案 A) |
| **S2b-1** | 6、6b、7、8、9、9b、17、**小線過期註解的同批改寫** |
| **S2b-2a** | 10、10b、11、12、12b、**12c**、14、15、18、20 |
| **S2b-2b** | 19、**29**、§5 兩環境突變矩陣 |
| **S2b-3a** | 21(五份真相式同步,拆兩半見 §1.1) |
...
- 本檔狀態:**v2 已跑關卡1 R1(codex,FAIL 21 must-fix + 4 nit,全折入),R2 待跑**。
  v1 曾寫「尚未送審 / 小線先送」—— 那是 v1 時點的事實,v2 已不適用。
...
**v2 狀態:關卡1 已跑四輪** —— codex R1(FAIL 21+4)、codex R2(FAIL 19+1)、**Fable R3 換模型換角度**(FAIL 9+4,五條翻案條件我親驗全成立)、Fable R3-複驗(FAIL 6+4)。**四輪 findings 全折入;下一輪複驗待跑。** 小線已收工(`cdf7e89`)。
...
—— v2:v1 起草 + `B-138-A` 五項輸入增補 + 關卡1 **四輪**(codex ×2、Fable ×2)共 **55 條 must-fix + 13 條 nit 全折入**;複驗待跑 ——
