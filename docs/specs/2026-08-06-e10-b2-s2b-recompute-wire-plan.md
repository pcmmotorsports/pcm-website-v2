...
migration 兩塊,**共 6 塊、每塊恰 6 行**。

🔴🔴 **v3.2 更正(3a 後段 R1 must-fix 5:上一版這裡與本節上方正面打架)**:
前段我在這裡寫「後段的守門一律**自己 grep 取集合**再驗計數」——**那句與本節上方「禁止用全樹 grep
決定集合」直接矛盾**,而上方那條是有論證的(標記字串也出現在 plan 檔與守門腳本自己身上)。
⇒ **以「寫死集合」為準**。後段實作(`scripts/b2s2b-truth-sync.py` 的 `SITES`)照這條做,
並**另外凍結 key 集合**(不是只凍個數)—— R1 實測:只凍個數時,「刪一格 + 重複另一格」
會讓守門零輸出全綠、那一塊從此無人守。**位置在檔內用「第幾個標記區塊」定位,不寫行號。**

**負測**:把 plan 檔也丟進集合 ⇒ 該格必須紅在「**位置 key 集合**不符」(v3.2 已從「集合大小」改成 key 集合;
大小被 key 集合蘊含,而只凍大小會被「刪一格 + 重複另一格」繞過 —— 3a 後段 R1 實測),而不是紅在 md5 不等。
...
| 25a-c | 三支 S1 harness 新格(**v2-R2 把「逐支明列」真的列出來**;codex:上輪只落了要求字面)。**25a** `scripts/b2s1a1-verify.sh`(`shipments`):新 cell = 「`AFTER UPDATE OF shipped_at` 那支重算 trigger 存在且結構逐字」;mutant = 改 `tgtype` 成 BEFORE;預期計數 = 現值 +1 格 +1 靶。**25b** `scripts/b2s1a2-verify.sh`(guards):新 cell = 「X1/X3 仍在且本線未放寬它們」(§0.3 前提的回歸點);mutant = 拿掉 X3;+1 格 +1 靶。**25c** `scripts/b2s1b-verify.sh`(`shipment_items`):新 cell = 「本表**沒有**重算 trigger」(Q1=A 的負向釘死);mutant = 加一支;+1 格 +1 靶。🔴 三支的**具名 key 集合**都要同批更新(W1),不是只加計數。<br>🔴🔴 **2026-08-07 S2b-4c 實測:上面那句 W1 的前提是錯的** —— 三支**都沒有具名 key 集合、也沒有任何凍結計數**(實查 `EXPECT*` 各 0 命中、`ok()` 只吃訊息不吃 key、收尾只印 `PASS=/FAIL=` 不做斷言)⇒「同批更新」沒有對象。retrofit key 集合要改**每一個 `ok` 呼叫點**(數十處/檔),是另一件事;**本片不假裝做了**,缺口進 STOP 由主視窗裁。<br>🔴 **25a 指定的靶「改 `tgtype` 成 BEFORE」構造不出來** —— PG 文法不接受 `CREATE CONSTRAINT TRIGGER … BEFORE`(與環境A 矩陣同一件事,二度實測)⇒ 換成**多掛一個 INSERT 事件面**(同樣打 tgtype 面、構造得出),實跑翻面成功。<br>🔴🔴🔴 **本片最重要的發現(plan 完全沒預料):本線讓三支 S1 harness 共 30 條斷言轉紅。** 交付版實跑(全前綴庫,port 54367):`b2s1a1` **59/1**、`b2s1a2` **21/25**、`b2s1b` **56/4**。<br>**乾淨消融(R1 must-fix 5 要求的隔離)**:`DROP`/`DISABLE` 都會擾動 trigger 集合本身 ⇒ 改成**只把 `pcm_a4a_shipments_summary_recompute()` 的本體換成 `RETURN NULL`**(trigger 照在、照啟用、catalog 一字不差)⇒ 三支**全綠 60/0、46/0、60/0**;還原本體後立刻回到 59/1、21/25、56/4。⇒ **那 30 條紅純粹來自重算的「行為」,不是任何 catalog 變動。**<br>病根:三支的 fixture 建 `shipment_items` + 設 `shipped_at` 時**沒有任何到貨**(receipts)⇒ 四軸重算算出 `shipped > instock = 0` ⇒ C9 擋。S2b 之前 `shipped` 恆 0,C9 從不發火。🔴 **不是本線的 bug —— 是那三支的 fixture 早於 C9 這條規則**(Sean Q1=A 拍的強制點),但確實是本線造成的回歸,而且今天沒有任何守門在看(三支不在本線的 gate 裡)。**處置進 STOP 由主視窗裁**(照 R4:修法含 disable/繞過 = 不屬執行者可拍的板);因此本片**也不替三支加凍結計數**——現在加等於把壞狀態凍起來。<br>🔴 **另有一條紅本片已自行修掉、沒有上呈**:`b2s1a2` 凍結的 `shipments` trigger 三元組是**七支**,而本線新增了第八支 `shipments_summary_recompute_ac:O:17` ⇒ 該格當時就紅,而且**紅得完全正確**。修法只是把新 trigger 加進凍結集(一行字面),不含 disable/繞過、不需上呈。🔴 我第一版把它跟另外 25 條 C9 的紅混寫成「紅點全是 23514」= **字面 vs 事實偏離**(R1 抓到),已更正:**25 條是 C9,1 條是這個凍結集,已修 ⇒ 現在是 25 條**。|
...
🔴 (a)(b)(c) 三格**都必須跑出預期觀察值才算過**;不接受「構造不出來也算完成」(codex:那是同一項無論結果如何都能登記完成)。<br>🔴 **2026-08-07 實測:三格都跑出預期觀察值,無一項以「構造不出來」結案。** 落在 `scripts/b2s2b-verify.sh` 的 `B29a-stale-high-reachable` / `B29b-line-self-heals` / `B29c-real-overship-still-red`。(a)=`step2=ok|step3=23514/oiqs_shipped_le_instock`(超額出貨零違規提交、紅在下一筆無辜採購更正)、(b)=`step2=ok|step3=ok|0/2`、(c)=`23514 / oiqs_shipped_le_instock` 紅在出貨那一筆。🔴 **(a) 與 (b) 是<u>同一段 body</u> 跑在兩個基準上**(pre-S2b 副本 vs post),body 逐字相同、觀察值不同 ⇒ **基準 pin 本身就是判別式**,(a) 因此不另設外部突變靶(它的對照組就是 (b))。🔴 **(b)(c) 已掛進 `beh_oracle`** ⇒ 靶⑤ 紅 (b)、靶① 紅 (c);掛進去之前這兩格**零靶** |
...
### §4.19 🔴 項 19 的實測記錄(2026-08-07;**不改 §4 表格內的驗收字面**)

> 落地在 `scripts/b2s2b-verify.sh` 的 `barrier_oracle` + 四發靶
> (`BMUT-0` / `BMUT-L1` / `BMUT-L2A-INCONCLUSIVE` / `BMUT-L2B`),commit 見該檔檔頭第八片。

**① 編排(三個真 psql session、真 COMMIT、真死結偵測)**
情境 A:兩個包裹含同一對品項、`shipment_items` **實體列序刻意相反**;阻擋者握住迭代序第一顆,
S1、S2 依序以 `pg_blocking_pids` **觀察到真的被擋住**後才放行阻擋者。情境 B:先寫者未提交時後寫者進場。

**② 靶 L1(NKU 鎖移到讀真相之後)= plan §5 點名「必須真的翻面」的那一靶 ⇒ ✅ 真的翻面**,
紅點 `BAR-shipped BAR-stale`。機制:read committed 下鎖若排在讀之後,後到者會先讀到看不見對方
未提交出貨的真相、等到鎖之後再把那個過期值寫回去,蓋掉先提交者算對的值。

**③ 靶 L2a(plan 指定的「拿掉 `ORDER BY`」)⇒ ❌ 翻不了面,期望紅點是空集合。**
PG 把 `DISTINCT` 規劃成 `Sort + Unique`(EXPLAIN 實跑,harness 以 `BAR-PLAN-SHAPE` 釘住該形狀),
輸出序等同 `ORDER BY order_item_id`;兩個 session 跑**同一支函式** ⇒ 順序只是值集合的函式、兩邊恆等,
反序取鎖**構造不出來**。⇒ **今天擋住反序取鎖的是 `DISTINCT` 的排序,不是 `ORDER BY` 那幾個字。**
`ORDER BY` 仍留著(把保證寫成明文、且是 `DISTINCT` 哪天被拿掉時的唯一防線),但**不得宣稱 harness 守得住它**。

**④ 靶 L2b(連 `DISTINCT` 一起拿掉)⇒ ✅ 真的丟 `40P01`。** 它存在的理由是給 ③ 的空集合背書:
沒有它,「L2a 全綠」與「這套編排根本抓不到死結」不可分。

**⑤ 🔴🔴 待裁事項(執行者不自行認定)**
§4 表格內項 19 逐字寫「若翻面測不出 `40P01`,該格判**紅**、不得算過」。實測是測不出,
但**不是**該條括號裡設想的「被別的鎖序意外救了」,而是**指定的翻面動作根本沒有移除排序**。
⇒ **項 19 算不算通過,由主視窗裁定**。harness 讓**機器可讀的三個面**自己承載這件事(R2 must-fix F1):
① key 名 `BMUT-L2A-INCONCLUSIVE`;② 該格走 `inconc`、**不計入 `PASS`**,另有一組凍結的待裁 key 集合
(所以它也刪不掉);③ 收尾**離開碼 3**(0=全過 / 1=有紅 / 3=無紅但有待裁)。
🔴 這**不是**替主視窗判它紅,是**拒絕在裁定之前把它記成過**;裁定回來後改一行即可歸位:
要記成過 ⇒ key 去掉 `-INCONCLUSIVE` 後綴並移進 `EXPECT_PASS_KEYS`;要判紅 ⇒ 改期望紅點集合。

**⑥ 整格另標 `inconclusive`(§8.1 第二列;項 19 是唯一預先授權的一格)** —— 舉證:
出貨 writer RPC 今天不存在 ⇒ 兩個 session 的 `UPDATE shipments` 是 **owner 直寫**。
證不到:①未來出貨 RPC 進 trigger 前會不會先取別的鎖 ②A8a2 多品項取鎖順序(§9 交棒 6)。
🔴 §8.1 末段那句「項 19 與項 26 **都** inconclusive 才進 STOP」的條件**未觸發** ——
項 26 是真的 DISABLE trigger 後觀察摘要不動、且有 S1/S2/S3 三發靶,不是 inconclusive。

**⑦ 🔴 凍結值是「後驗」的,不是 W1 開工前凍結(R3 must-fix 3)**
`BMUT-L2A-INCONCLUSIVE` 的期望紅點(空集合)與 `EXPECT_INCONC*` 這組凍結值,
都是**先實測、後寫下**的。§2.1 片級 DoD 的 W1 要求「計數器與具名 key 集合開工前先凍結」——
這一組不符合那條,`-INCONCLUSIVE` 後綴不改變這件事。**明記於此與 commit body,不當成 W1 凍結引用。**
(其餘三發靶 `BMUT-0 / L1 / L2b` 的期望值同樣是實測後凍結;差別在它們的期望**方向**開工前就寫在 plan §5,
而 L2a 的期望方向被實測**推翻**了。)

**⑧ 🔴🔴 兩條與本檔字面相衝的讀法(R3 提出,一併呈給主視窗,不由執行者選邊)**
- **讀法甲(R3 主張)**:§8.1 第三列說「該格**構造不出來** ⇒ ❌ 不算通過,須寫出哪一道擋住它 + 附實跑輸出」。
  §4.19 ③ 自己就用了「構造不出來」並附了 EXPLAIN ⇒ **plan 其實已經裁定為「不算通過」**;
  再開一個「待裁」態,等於把**已有裁定**的情況重標成**沒有裁定**,正是 §8.1 v2-R3 補洞① 要擋的重新標籤。
  ⇒ 保守做法應是**判紅**(離開碼 1)+ 註明「可由主視窗改判為過」。
- **讀法乙(執行者當時的判斷)**:§8.1 三列講的是**該格**(項 19)的可構造性,而該格**是**構造出來了
  (L1 翻面、L2b 翻面、對照組全綠);構造不出來的是**其中一發靶**。⇒ 不適用第三列,故留待裁。
🔴 **這一題的裁定權不在執行者**,兩種讀法都寫在這裡,由主視窗擇一;
現況(離開碼 3、不入 PASS 帳)是**兩者之間的保守中點**,不是對這題的答案。

🔴🔴 **主視窗已裁:Q1 = C —— 維持第三態(離開碼 3、不入 PASS 帳);終局判定綁 apply 前 preflight**
(`B-159-A`,2026-08-07)。理由逐條落檔,免得未來讀 plan 的人以為是執行者自己選的:
- **甲(算過)不採**:⑥ 自己寫了整格 `inconclusive` 的真因 = writer RPC 不存在、owner 直寫模擬
  證不到未來的前置鎖。把這格記成 PASS = **宣稱 > 事實**。
- **乙(判紅)不採**:紅的語意是「編排抓不到反序取鎖」,但 L2b 實測**真的丟 `40P01`** = 編排抓得到;
  判紅同樣是字面 ≠ 事實,只是反向。R3 引 §8.1 補洞① 的「構造不出來 ⇒ 不算通過」——
  **C 也沒把它算通過**,爭點只剩離開碼的形狀,而 `b2s2b-verify.sh` 零自動化消費者
  ⇒ 兩種讀法**無行為差**,取誠實的那一個。

**⑨ 🔴 §8.1 還有第二個升級條款,而它<u>成立</u>(R3 must-fix 2)**
§8 有另一句:「**若它(項 19)最終是本線唯一的併發證據,依 §8.1 末條進 STOP**」(本檔 §8 該列)。
項 26 是**單 session** 的 DISABLE 觀察,**不是併發證據** ⇒ 該條件成立。
⇒ **已依規定進 STOP 呈報主視窗**(不是「未觸發」)。⑥ 只評估了「19 與 26 都 inconclusive」那個版本,
單看它會漏掉這一條 —— 兩條併陳於此。

🔴🔴 **主視窗已裁(`B-159-A`,2026-08-07):本線的併發驗證<u>正式移交</u>「出貨 writer RPC 片」(§9 交棒 1)。**
項 19 這一格**重新定性 = 編排可行性證明**(三 session 真併發骨架 + 真 `40P01` 判別力已證),
**不是**本線的併發證據。真併發證據等 writer RPC 存在時在真路徑上做,
且 **§9 交棒 10(批次多列 UPDATE 的逐列發火鎖序面)併進同一片的射程 —— 兩面同一片證,不拆**。
apply 前 preflight 會列「併發證據 = deferred,交棒 1 + 10 未結」讓 Sean 知情裁 apply。
...
| **蓄意改註解時** | 同批更新腳本內的凍結常數,並在 commit body 寫明改了哪一句、為什麼 —— 與小線 `MIG_SHA_FROZEN` 的契約同形 <br>🔴🔴 **2026-08-07 S2b-5 已落地**:`scripts/b2s2b-verify.sh` 的 `6h/8` 段,**七格** —— `CFP-SAMESOURCE`(**三邊**同源:姊妹檔 `fp_sql()` / 本檔凍結清單 / 本檔 `cfp_rows_sql` 實作,逐字同序;且該支 `COMMENT_FP` = 本檔 expected-before)/ `CFP-DISPATCH`(常設格,分流依 `shipments_summary_recompute_ac` 是否存在,**與 COMMENT 文字完全無關**)/ `CFP-BEFORE-BRANCH`(對 `b2s2b_pre` 實跑,否則 before 那半永遠沒被執行過)/ `CFP-DIMENSIONS`(七段全部真的取得到 COMMENT,零個退化成 `-`;某段退化只會讓指紋漂一次、重凍後永久失明)/ 負測 ⓐⓑ 兩發翻面 + ⓒ 一發靜態不等式。🔴 **plan 原本要求的「ledger 尾」我沒有另開一格** —— 本檔第 318 行的 `NEWEST_TS` 閘早就在同一份 `ls` 上 `die`,另開一格是**恆真**的(唯一可紅路徑不可達),R1 抓到後拿掉並留指標。凍結值:expected-before `f5798812892bb484b52083ad068a48a6`(= 小線值,獨立算出後與 `b2s2a-verify.sh:185` 逐字相符)、expected-after `f2794c052c359f08c366429090e8fa53`。🔴 負測ⓐ 第一版**沒有翻面** —— 蓋寫插在第 6 節之前會被該節再設回正確值,錨改到檔尾才是「最後一句跑到的 COMMENT」。 |
...
| helper 把 NKU 鎖移到讀 SUM 之後 | 項 19 barrier —— 🔴 **S1 消融 #25 的回歸點**(S1 拿掉 NKU 時 harness 全綠),本靶**必須真的翻面**。**✅ 2026-08-07 實測真的翻面**:紅點 `BAR-shipped BAR-stale`(靶 `BMUT-L1`) |
| 迭代拿掉 `ORDER BY` | 項 19 —— 🔴🔴 **2026-08-07 實測:這一靶翻不了面**(詳見 §4.19);判別力改由同一套編排下的新靶 `BMUT-L2B`(連 `DISTINCT` 一起拿掉)背書 |
...
| 10 | 🔴🔴 **併進交棒 1「出貨 writer RPC 片」同一片證(主視窗 `B-159-A` 裁定,不拆)** —— 任何「一句 UPDATE 改多列 `shipments`」的片(批次出貨) | **本線完全沒測到的第三個併發面**(2026-08-07 R3 換角度抓到)。本片的 trigger 是 `AFTER … FOR EACH ROW`(`20260806180000:361-365`)⇒ 一句 `UPDATE public.shipments SET shipped_at=… WHERE <多列>` 會**逐列**發火,而**跨 shipment 的取鎖順序 = 該 UPDATE 的列序,沒有任何排序保證**。兩個這種語句併發、若各自選到不同的掃描順序(索引 vs 循序),就是真 `40P01`。🔴 同檔 backfill(`:263`)特地做了全域 `ORDER BY x.order_item_id` 並註明「不製造新的鎖序」——**證明這條規律當時就知道,但只補在 backfill、trigger 路徑沒補、也沒列進交棒**。⇒ 批次出貨那片必須:①單句多列 UPDATE 前先以確定序取列(或改逐列送)②補一格 barrier 負測。**本線不宣稱已擋。** |
