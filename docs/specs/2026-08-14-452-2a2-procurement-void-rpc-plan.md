...
| 母 plan §0-C 的 **12 個讀者面** | 🔴 **v2 更正:「本片不碰應用層」已經是假的** —— §3.1 要動 2 個檔 3 行(碼集放寬)。精確版 = **本片只碰「碼集」那一面,不碰任何顯示面** ⇒ **13 支讀者面**(實查數,非母 plan 的 12)全部留給後續片,已立案 **`#476`**。⚠️ 這代表 **2a-2 apply 之後、`#476` 之前,後台畫面不會顯示「這筆撤了」**,而訂購數已把它扣掉 ⇒ **畫面列 3 件、總數說 0 件** |
