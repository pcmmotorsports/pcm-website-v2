...
2. **車庫預填**:登入會員有唯一車或 primary 車 → 預填+標示 ~~「來自你的車庫」~~ ⇒ **現字面「來自您的車庫」**
   (2026-08-18 Sean 拍板「顧客站用您 / 後台用你」,W2 整批換過;實作在 `CartVehicleField.tsx:45`)。
...
> 🔴 **2026-08-18 字面更正(W2)**:Sean 當日拍板「**顧客站用「您」/ 後台用「你」**」⇒ 本節與 §上方
> hint 文案裡的「你」已換成「您」(`ProductFitmentCheck.tsx` 四態 / `CartVehicleField.tsx:45` / `CartView.tsx:115`)。
> **舊字面留痕不刪**;grep code 請用「您」版本。
> ⚠️ **只換人稱**:「錯誤的✓比空白更糟」那條紅線、以及「禁模糊/相似度匹配、禁 AI 猜」的車種鐵律,**都沒有動**。
