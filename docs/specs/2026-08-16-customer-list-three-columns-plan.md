...
### 2.2 訂單數 / 最後下單 —— **主視窗 2026-08-16 裁 A(原本是我選的、無人裁)**

> **狀態(2026-08-16 二次更新):已裁 = A(排除已取消)。主視窗裁,不送 Sean。**
> 理由=消費金額已排除已取消 ⇒ **兩欄並排必須共用一條規則**,否則員工要學兩套。
> 🔴 **未來選項(不是缺口,是另一個欄位)**:要「他最近有沒有來」的行為訊號,
> 應新增「**最後互動**」欄,而不是把取消單併回「最後下單」。
...
⚠️ 這條**同樣要寫進 `COMMENT`**,標「**主視窗 2026-08-16 裁、Sean 未逐條答、要改直接改**」。
🔴 **本行原本寫「A 窗選、無人裁」—— 08-16 裁下之後就地更正。**
   (改了上面的狀態塊卻沒改這行與小標,是「改了前件、後件沒跟著翻」;`literal-sweep.sh '無人裁'` 抓到的。)
