...
> ✅ **狀態:已回核放行(`D-221-A`,2026-08-08 03:05)。本片範圍 = Q27 + §2-2 + Q28②。**
> **Q28① 已裁定拆出去另開一片**(單獨 plan、單獨過審)⇒ 本檔 §4 全段**僅存查**、不在本片範圍。
...
| 鐵則 8(提 plan) | ✅ 本檔即是 |
| 鐵則 12 | Q27 不命中六類。⚠️ **本欄原寫「Q28① 命中⑥共用元件行為」= 分類錯,已更正**(`D-221-A` ①-1):⑥ 的字面是「共用元件 **`packages/ui`** 行為改動」,而 `products-url-state.tsx` 在 `apps/storefront/src/components/`(實查)⇒ **命中的是鐵則 8**(動全站真相來源 + 跨 3 檔因果),不是 12⑥。**從嚴辦理不變**(自評加審可以、不得免審),但**別讓後人拿這條當 12⑥ 判例**。|
...
### ✅ 3-1 裁定 = (a);**我先前讀錯了那句「待補」,如實記錄**

`D-221-A` ①-2 裁定:**Q28② 的規格是完整的,可做,不用等 Sean。**

🔴 **我的誤讀**:我把「⚠️ Sean 第 2 點斷句未完,待補」讀成「②(清除車輛)那條未完」。
**錯了。** 關鍵在同一行的標題自己就寫了:「Q28(Sean 逛 3111 後指示,**功能追加二連發之一**)」——
這條記的是**兩則訊息裡的第一則**,①② 都出自那一則;「Sean 第 2 點」= **第二則從未送達的新題**,
與 Q28② 的畫面無關。
⇒ **我引了那句警語,卻沒有跟同一行的標題對照。** 同族病根:引用片段而不讀完整脈絡。

**裁定內容(照做)**:本地視覺照現行「更改車款」行為(`setSel(null)` 回選車入口、可立刻選新車),
**加上**清除全站 `vehicle-context`。這是對拍板字面的最小忠實讀法;(b)(c) 屬規格外加戲。
⚠️ `D-221-A` 附帶:「若你實查後認為 (a) 與『清除全站狀態』字面有衝突,STOP 提出」。

#### (以下為 v1 原文,保留備查)

### ⛔【已被上方取代,勿引用】3-1 v1:這條的指令本身標著「未完」,我不打算猜
...
## ⛔ §4 Q28①【已裁定拆出去,**不在本片範圍**,以下僅存查】

`D-221-A` ①-1 裁 **A**:Q28① 另開一片、單獨 plan、單獨過審。
**本片不做這一題** —— 以下分析原樣保留,給那一片開工時當偵察結果直接用。
...
⇒ **動它 = 動全站選車的真相來源**,風險與體積都遠高於 Q27。
⚠️ **分類更正(本段原寫「命中鐵則 12 ⑥」= 錯,`D-221-A` ①-1 糾正)**:⑥ 的字面限定「共用元件 **`packages/ui`**」,
而 `products-url-state.tsx` 在 `apps/storefront/src/components/` ⇒ **命中的是鐵則 8**,不是 12⑥。
**從嚴辦理不變**(加審可以、不得免審)。🔴 **下一片接手時請用這個更正後的分類**,別繼承錯的。
...
## §6 ✅ 三件已全數回核(`D-221-A`,2026-08-08 03:05)

| 件 | 裁定 |
|---|---|
| Q28① 拆不拆 | ✅ **裁 A(照推薦)** —— 另開片、單獨 plan、單獨過審;本檔 §4 僅存查 |
| Q28② 規格是否完整 | ✅ **完整、可做,不等 Sean**;採 **(a)**。我先前對「待補」的解讀有誤,更正記於 §3-1 |
| §2-2 規格外回填 `sel` | ✅ **核准**,兩個條件:①**判定邏輯(`commit`/比對)不動**——回填只餵顯示層 ②`onPickYear` **當新功能測**(含突變)|

⚠️ 主視窗同時收下我對來源的更正並自報帳:`D-220-A` 寫「兩題規格以 `D-218-A` 為準」是**沒 grep 就引用**
(`grep -c 'Q28' D-218-A.md` = 0,雙方各自實跑證實)。權威來源定案:
**Q27 = `D-218-A`;Q28 = memory `project_site-redesign-content-pages-decisions.md:105`**。

#### (以下為 v1 原文,保留備查)

### ⛔【已全數回核,勿再當待決引用】§6 v1:待回核的三件
