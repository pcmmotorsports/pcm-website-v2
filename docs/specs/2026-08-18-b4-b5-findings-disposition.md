...
| F17 | nit | ✅ **已修**(`be8d4649`) | ~~`:62` 的 `grep -q`~~ ⇒ 實際在 `:84`(R1 `F23` 修行號);已改成「**恰一支且含第 9 參**」,overload 並存另回 `rc=5`。⚠️ ~~「腳本只在 notify-email 分支」~~ 已作廢:`55f391b5` 收進 dev 了。 |
...
## 🔴 R2(2026-08-18 14:0x,adversarial-reviewer 對【重寫後的 B-4 plan】)

`FAIL:must-fix 12 / consider 7 / nit 7` —— **不是上面那 24 條回來,是新的 26 條**(標的換了)。
已折進 B-4 plan(逐條在該檔內留痕):
```
must-fix  F1 證據從「兩筆」改成「一筆」+ 補一條更強的推論鏈
          F2/F3/F4 三個【既有測試檔】會編不過或必紅 ⇒ 範圍表從五處變九處
          F5/F6 🔴 cutoff 死結（flag 永不開 ⇒ PRD 的 cutoff 取不到值）⇒ 新增 §11 決策題 Q-G4-5
          F7 null 分支【上游產不出來】⇒ §5 改寫、§6 #3 換成真的釘子
          F8 原 #6 是重複格（既有已有 9 格）F9 兩發突變不是局部的 ⇒ 突變表重寫
          F10/F14 「只在 notify-email 分支」已因收割作廢
          F11 硬閘要寫死「只有 exit 0 才可部署」 F12 要指定落點（commit body + STATUS）
consider  F13 用 session email 的理由是【推的】不是量的 F15 cardholder 要改兩處
          F16 mock 少 addressEmail 而 typecheck 不會紅 F17 只斷言鍵存在擋不住
          F18 resolver 簽章要收 undefined F19 後台顯示與實際收件人可能分岔
nit       F20~F23 行號 F24/F25 五處註解字面會變假 F26 推論鏈可以更強
```
🔴 **R2 之後仍未 PASS** —— 依規則 R1 FAIL ⇒ 修完跑 R2 確認,而這一輪是「修完」不是「確認過」。
**下一輪要換角度、換模型**(規則:第 3 輪起同模型會在同一框架內找更細的問題)。
