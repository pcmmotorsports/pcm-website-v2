...
> 🔴🔴 **先讀這句,否則你會誤用這張表:這張表不是全集。**
> `orders` 的 `Insert` 實際有 **35 個鍵**(數法 §5-2),而 §1 只有 **11 列** —— 我合併了同性質的欄(快照類、結構欄),
> 而且**有欄位根本沒被列到**。**沒列到的欄 ≠ 檢查過了、≠ 沒問題。**
> 要拿這張表當「某欄能不能改」的依據,先確認那欄真的在表上;不在表上 = **還沒查**。
>
...
- **刪一列**:🔴 **不該做,理由是「會把數量記錄一起抹掉、帳就對不起來」。**
  複合 FK 是 `ON DELETE CASCADE`(`20260730150000:94`,結構斷言在 `:335` 逐字比對整串定義)
  ⇒ **刪掉一列品項,摘要那列會被資料庫連帶刪掉**,而摘要裡存的是
  `ordered_quantity`(已向供應商訂了幾個)/ `instock_quantity`(已到貨幾個)/
  `cancelled_quantity`(已取消幾個)/ `shipped_quantity`(已出貨幾個)。
  ⇒ **這四個數字會一起消失、沒有任何地方留著**:那批貨還是訂了、還是到了、還是出了,
  但系統上再也查不到 —— **帳對不起來,而且是靜默的**(沒有錯誤訊息)。
  **正解走取消線**(`admin_cancel_order` 的部分取消):取消是**留著紀錄再加一筆**,刪除是**把紀錄抹掉**。
  ⚠️ 「不該做」這個結論**沒有 Sean 拍板背書**,列給他裁;但上面那個機制理由是可驗的,不是偏好。
