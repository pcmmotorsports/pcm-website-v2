...
GET /api/auth/line/callback
    ↓ 1. 驗 state
    ↓ 2. POST LINE token endpoint 換 access_token
    ↓ 3. GET LINE profile endpoint 拿 userId / displayName / email
    ↓ 4. 用 Supabase Admin API:
         - 查 auth.users 是否已有 line_user_id（meta 內找）
         - 無 → createUser({ email, raw_user_meta_data: { name, provider: 'line', line_user_id } })
                → handle_new_auth_user trigger 自動建 customers row
         - 有 → 直接拿該 user
    ↓ 5. 用 supabase.auth.admin.generateLink 產生 magic link、或自簽 JWT 發 session cookie
...
6. **OpenID Connect** → 啟用 `profile` + `email` scope(取 user 資訊)
