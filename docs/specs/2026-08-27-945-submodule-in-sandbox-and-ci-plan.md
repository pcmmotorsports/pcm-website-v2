...
> 🛑 **而檔尾還有兩節是【給下一個人的停止訊號】, 從這裡就要看得到**:
>   · `§13-7d` —— **不要動 `.github/`、不要端兩步紙**(`docs/runbooks/2026-08-27-945-deploy-key-two-steps.md`)
>   · `§13-7c` —— R3 對那份兩步紙列了 7 條 must-fix, **一條都沒修**(刻意:上面那題若答 A, 整份紙消失)
> 🔴 理由與 `2026-08-25-saved-views-plan.md` 檔頭那條相同:**住在檔尾的警告活不過一次 `grep`。**
...
⇒ **15+1skip ↔ 16 passed** —— 兩個世界印不同的值, 不是兩個綠。
  ...
  ⚠️ **那格 2026-08-27 已改名**(`§13-8`)⇒ 現名「🔴 漂移(B, 有稿才跑):…」;拿舊名去 grep 會查無。
...
> 🔴 **§13-7b 那一題已經關了** —— 主視窗 `pcm-website-v2-5b` 2026-08-27 裁,
> 依 Sean 同日「這兩個問題應該是你們要自己決定的,用你建議的方式繼續就好」的授權。
> **落檔字面:「Sean 授權主視窗決定,主視窗裁 A」—— 不是「Sean 拍了 A」。**
> ⚠️ 本節是被審查逼出來的:上一版我在測試檔裡寫「見 `§13-7b`」,而那一節**只有題目沒有答案**,
> 同檔 `§13-7d` 還逐字寫著「要先關的是 §13-7b 那一題」⇒ **跟著指標走的人會看到一題還沒關。**
> 📌 **「我知道它裁了」與「檔案知道它裁了」是兩件事, 而只有後者對下一個人成立。**

```
裁決  A(反恆真)。B(抓漂移)不做成 CI 格。
理由(主視窗給的三條, 第三條決定性):
  1 A 買到的正是我們在意的:證明「刻意不渲染 X」不是空話 ⇒ **一份 fixture 就夠**
  2 A 一裁, `Q-945-1`(鑰匙)與 `Q-945-3`(fork 處置)**兩題同時消失**
    ⇒ 省下的不是我們的工, 是 **Sean 的注意力**
  3 B 這件事本 repo 已經拍過一次同樣的板, 理由在 `scripts/od-drift-check.py` 檔頭:
    稿住在 repo 外面 ⇒ CI 上那個檔不存在 ⇒ **紅在環境而不是紅在漂移, 那種紅會被學會忽略**
    (⚠️ 那支查的是 OD 稿不是 design-reference ⇒ **不是先例套用, 是採用它的論證**。
      主視窗自己標了這一點。)
⇒ 連帶作廢:`Q-945-1` = 甲 deploy key(已無適用對象)· `Q-945-3` 三選項 · 兩步紙
   ⚠️ 三份都**留著不刪**, 而各自標了作廢原因 —— 它們記著「我們為什麼曾經覺得需要鑰匙」。
```

## 13-8a 做出來的東西

```
新  apps/storefront/src/components/account/tabs/wallet-design-contract.fixture.txt
    從原稿【機械抄】的三行 + 來源 / design commit sha / 重抄指令 / 核對法
改  apps/storefront/src/components/account/tabs/WalletTab.test.tsx
    · 新增「反恆真」一格 —— **不 skip, 每一台機器都跑**
    · 原本那格改名成「漂移(B, 有稿才跑)」, 仍 skipIf, 並**反過來驗 snapshot 每一行仍在活稿上**
    · 🔴 兩處共用 `TOKEN_RULER`, 而它是**第二版**(見下)
改  docs/design-storefront-manifest.yaml  walletTierCardOmitted 的守門形狀敘述 + 更動日期
```

## 13-8b 🔴 審查在這一片抓到一個**已經存在的壞尺**,而我照抄了它

```
原本兩處都用 `new RegExp('\\b' + token + '\\b')`, 而註解逐字寫著
「`toContain` 在原稿改名成 `-v2` 時仍然綠 ⇒ 所以改用 `\b`」。
🔴 實測:`/\bwal-tier-card\b/.test("wal-tier-card-v2")` ⇒ **true**
   —— `-` 不是 `\w` ⇒ `d` 與 `-` 之間本來就是一個邊界。
⇒ **那把尺在它自己的註解宣稱要擋的那個世界, 是綠的。** 而那個註解在 repo 裡放了一天。
⇒ 第二版 `(?<![\w-])token(?![\w-])`, 四個世界實測:
     真值命中 true · `-v2` false · 截斷字 `wal-tier` 對真值 false · 舊尺在後兩者皆 true
📌 而它為什麼活得下來:**當時的突變複驗只演了「改我方常數」那個方向**(那個方向兩把尺都會紅)
   **沒有演「稿被改名」那個方向** —— 而那正是註解宣稱它在擋的那一個。
   ⇒ 動作版:**一把尺宣稱擋 X ⇒ 就要有一發突變【製造 X】。** 不是製造「某種錯」。
```

## 13-8c 驗收:四發突變,各紅各的格(2026-08-27)

```
發1 元件裡真的渲染一個被禁的字面   ⇒ 1 failed | 16 passed  紅「沒有立即儲值鈕…」那格
發2 把 DELIBERATELY_OMITTED 打錯字 ⇒ 2 failed | 15 passed  紅 A 與 B(量具接上了)
發3 手改 fixture 塞一行稿上沒有的  ⇒ 1 failed | 16 passed  紅 B 的繫繩(snapshot 不能手打)
發4 把 fixture 的 class 改成 `-v2` ⇒ 2 failed | 15 passed  **舊的 `\b` 尺在這一格會綠**
還原:三支檔 `shasum -a 256 -c` 全 OK。

🔴 而最重要的那一發不是突變, 是**世界**:
  沒有 submodule 的沙箱(preflight + `GIT_SSH_COMMAND=/bin/false`)
  ⇒ `Tests 16 passed | 1 skipped (17)` ⇒ **只有 B 那格 skip, A 跑了。**
  改動前的同一個世界:正對照 skip ⇒ **那個省略斷言是恆真的, 而畫面上是綠的。**
```

## 13-8d 這片的天花板(附不出檢查的就明寫)

```
· A 只證「那三個字面【曾經】在稿上」—— snapshot 過期它不知道。
  繫繩(B 那格反查 snapshot)**本身也是 skipIf 的** ⇒ **沒有稿的地方, 我們只能相信 snapshot。**
  📌 這不是疏漏, 是這個設計換來的:要在 CI 上證「現在」, 就得把稿帶進 CI —— 而那正是被裁掉的路。
· fixture 檔頭的 design commit sha **仍然沒有任何一行碼讀它**(繫繩比的是內容不是 sha)
  ⇒ 它是給人看的, 不是給機器看的。**寫在這裡, 免得下一個人以為它在守什麼。**
· B 那格的覆蓋率是「有稿的人」, 不是「每個人」。A 才是每個人都有的那一半。
```

## 13-9 `code-reviewer` R2 = **FAIL**(12 條)⇒ 全修,而其中三條是我自己剛立的規矩

```
🔴 R2 最有價值的三條, 都不是「新錯」, 是【改寫留下的碎片】:
F1  `:289` 舊指示「class 走 \b」原封留著, 而同一格上面剛寫完「不要換回 \b」
    ⇒ **一支檔裡兩條互相矛盾的指示, 而壞的那條是祈使句** ⇒ 下一個人照它改回去, 四發突變全綠
F2  `:283` 是上一版段落被改寫後留下的**斷句**, 主詞被換掉了 ⇒ 讀者會把它接到新尺頭上
F3  `:193` 「下面那格正對照拿 design 原稿當量具」—— 拆成兩格之後那句漂了(R1 抓過一次, 我沒修完)
📌 **改寫比新增危險**:新增的東西讀者知道要看, 而改寫留下的碎片**看起來像原本就在那裡**。

F5 🔴 `TOKEN_RULER` **沒有 escape** ⇒ token 含 regex 特殊字元時:`wal.tier` 命中 `walXtier`(靜靜過)
   ⇒ 補 `escapeRe`。而它是被**上一輪的修法**放大的:rest spread 讓那組 token 對未來新增開放。
F6 🔴 分堆用 `const { depositButtonLabel, ...rest }` ⇒ **只擋掉現在這一個中文欄位**;
   日後新增第二個中文標籤會自動流進 class 那把尺, 而 `(?<![\w-])會員等級(?![\w-])` 對「會員等級卡」
   回 **true** ⇒ 那個洞換個入口回來。
   ⇒ 改成**由內容分堆**(ASCII-only 走 token 尺 / 含非 ASCII 走行錨尺)+ 一格「兩堆加起來 = 總數」。
F4 F10 manifest:reason 結尾指向 `§13-7`(沒有答案的那一節)⇒ 改 `§13-8`;
   另兩條同守門的 override(`walletTxRunningBalanceOmitted` / `walletBalanceMetaDiscountClaimRemoved`)
   也補上「守門形狀最後更動」⇒ **守門形狀變了, 而三條 override 只有一條記著。**
F7 繫繩只跳空行不夠 ⇒ 加下限。⚠️ **而我第一版的下限自己是錯的**:寫「長度 >= 8」,
   當場把 `            立即儲值` 判紅 —— **CJK 一個字就是一個 char** ⇒
   📌 **一個用字元數表達的「夠長」, 對中文與對英文不是同一個門檻。**
   ⇒ 拿掉長度那一半, 只留「這一行要含至少一個被守的字面」。
F9 `DESIGN_WALLET_JSX` 的 docstring 仍寫「正對照量具」⇒ 它現在是**漂移**量具。
F8 fixture 當時還是未追蹤 ⇒ **忘了 `git add` 它, A 會在每一台機器 ENOENT 硬紅** ⇒ 本 commit 已收。
F11 🔴 **沒有一發突變改 `design-reference` 那一側** —— 而那正是 B 宣稱擋的世界。已補(發5)。
F12 三綠時點綁不到被審那份 bytes ⇒ 本節的三綠是**修完之後重跑**的。
```

### 13-9a 驗收(修完重跑;每一發都核了【紅的理由】不只核紅)

```
發1  元件真的渲染被禁字面        ⇒ 1 failed | 16 passed   紅「沒有立即儲值鈕…」那格
發2  打錯 DELIBERATELY_OMITTED   ⇒ 2 failed | 15 passed   紅 A 與 B
發3  fixture 塞一行稿上沒有的    ⇒ 1 failed | 16 passed   紅 B 的繫繩
     理由逐字「契約 snapshot 有一行在活的 design 原稿上逐字找不到」✅
發3b fixture 塞一行 `  }`        ⇒ 1 failed | 16 passed   紅 B 的下限
     理由逐字「有一行不含任何被守的字面 ⇒ 它對任何原稿都恆真」✅
     (⚠️ `}` 本身在 JSX 裡到處都有 ⇒ 只有那道下限抓得到它, `toContain` 會過)
發4  fixture class 改 `-v2`      ⇒ 2 failed | 15 passed   舊的 `\b` 尺在這一格會綠
發5  🔴 **活稿** class 改 `-v2`  ⇒ 1 failed | 16 passed   **只紅 B**, A 綠(正確:漂移該由 B 報)
     submodule 動前 porcelain=0 / 還原後 porcelain=0 / 主樹對 submodule 的看法 = 空
還原:三支檔 + design 原稿 `shasum -a 256 -c` 全 OK。
三綠 2026-08-27 13:37 `TURBO_FORCE=1` ⇒ 0 / 0 / 0
無 submodule 沙箱 ⇒ `Tests 16 passed | 1 skipped (17)` ⇒ 只有 B skip、A 跑了
```

### 13-9b ⚠️ 這一片是在一個**工作被吞掉**的當下做完的,記一行

```
2026-08-27 13:24 一顆 author `t <t@t>` 的 commit `4dc32874 base` 落在 dev 上,
我這三支檔隨即從工作樹消失, 而 `git status` 只剩一行未追蹤的 fixture。
⇒ 東西在 `stash@{1}`(lint-staged automatic backup)裡, 用 `git show 'stash@{1}:<path>'` 逐支取回,
  逐支 `shasum` 比對 stash 版相同、與 HEAD 版不同(負對照)。**沒有跑 `git stash pop`** ——
  那個 stash 裡混了**另一個窗**的五支檔。
📌 而真正該記的不是我救回自己的東西, 是:**我去看了那個 stash 裡【還有誰的】。**
   那一步不是必要的, 而它是這件事沒有變成事故的原因(另一個窗完全不知道自己的檔不見了)。
🔴 **`lint-staged` 的自動 stash 會被自動丟掉** ⇒ 這種取回是有時限的, 不是「有空再處理」。
```
