...
> 🔴🔴 **2026-08-26 起,本檔與另外兩份是【同一件事的三份寫法】,而矛盾已裁在:**
> **[2026-08-26-customer-filters-merged-plan.md](2026-08-26-customer-filters-merged-plan.md) —— 開工前先讀那一份。**
> 本檔內容**未刪、未取代**,推導仍以本檔為準;**而與另外兩份衝突的地方以合併檔的裁決為準。**
...
## 🔴🔴 1-3b **這一片撞到一份更早的 plan,而我寫它的時候不知道它存在**

**2026-08-26 主視窗派工時要我「先查有沒有已經寫好的生日篩選 plan」。查了。有。**

```
docs/specs/2026-08-16-customer-filters-four-fields-plan.md
  逐字標題「客戶篩選四欄 plan(年齡 / 性別 / 居住地區 / 登入方式)—— 鐵則 8 + 鐵則 9,**等 Sean 批**」
  :4 逐字「提出:A 窗,2026-08-16」·「狀態:未動手,零程式改動」
量法 grep -rln '生日' docs/specs/*.md ⇒ 7 支(分母 = docs/specs 共 343 支)
負對照 'zzz生日20260826' ⇒ 0
🔴 而【本檔與 2026-08-25-customer-filter-plan.md 對它的引用次數 = 0】
   量法 grep -c 'customer-filters-four-fields' <兩支> ⇒ 0 / 0(負對照 'zzz-four-fields' ⇒ 0)
```

⇒ 🔴 **我寫這一片的時候,對它一無所知。而它就在同一個目錄裡。**
⇒ 📌 **這正是我今晚寫的那份索引在講的病**(`2026-08-26-index-tonight.md` §1-a):
   **東西找得到,只是沒有一條路指過去** —— 而**寫索引的那個人,自己就踩了一次。**

### 🔴 1-3b-① 最重的一格:性別那一題【Sean 十天前就答過了】,而我又問了一次

```
2026-08-16  A 窗記:  `Q2=甲`  男 / 女 / **不願透露**
2026-08-26  我  記:  `Q11=甲` 男 / 女 / **不透露**
量法 grep -rl '不願透露' docs/ ⇒ 2 支 · grep -rl '不透露' docs/ ⇒ 2 支 · 負對照 '不欲透露' ⇒ 0
```
⇒ **同一個決定、同一個「甲」、兩個日期、兩組字面。**
⇒ 🔴 **我浪費了 Sean 一題。** 而 CLAUDE.md 的決策題紀律逐字寫著「**先 grep 既有拍板不重問**」——
   **我沒 grep。**
⇒ ⚠️ **而現在有兩個字面在檔案裡,沒有任何機制會讓它們對齊。**
   enum 值是英文(`'male'|'female'|'undisclosed'`)⇒ **只有畫面上那三個字受影響**。

## ✅ **字面已定案 —— Sean 2026-08-26 `q3=乙`:三個選項 = 男 / 女 / 不透露**

```
~~不願透露~~(08-16 `Q2=甲`)  ⇒  **不透露**(Sean 2026-08-26 `q3=乙` 定案)
```
🔴 **舊字面留痕不刪** —— `2026-08-16-customer-filters-four-fields-plan.md:5` 的「不願透露」
**是那份檔的作者當時如實記的,不是他記錯。** 靜靜改掉會讓他以為自己記錯了。
⇒ 該檔已加同一行訃聞,**兩處一致**。
⇒ enum 值仍是英文 `'male'|'female'|'undisclosed'`,**不受字面影響**。

### 🔴 1-3b-② 那份 plan 有一個我答不出來的數字,**而它就是 E=丙 的量級**

`2026-08-16-customer-filters-four-fields-plan.md:15` 逐字:
> `客戶總數 11、全部是自己測試用、有填生日 2 ⇒ 真實客戶 0`

⚠️ **同檔 :16-18 自己標了【未確認】**(逐字:「這三個數字我沒有當場量…來源是主視窗轉述的先前盤點」)
⇒ **所以這不是一個可以拿來下結論的數字,是一個【可以拿來問的數字】。**
⇒ 🔴 **而它直接指向 E=丙 的成本問題**:如果有填生日的真的只有 2 個人,
   **「兩個都要」做出來之後,兩種篩法篩的是同一批 2 個人。**
⇒ 📌 **本檔 §3 原本寫「我沒查、講不出量級」** —— 那句要更正成:
   **「有一個轉述的數字,而它自己標未確認」**。兩者不同:前者是沒有,後者是有而不能信。
   **開工前第一件事是重量它**(那份 plan `:144` 自己也這樣寫)。

### 🔴 1-3b-③ 它還有一題,而 `E=丙` 讓那題變成必答

同檔 `:91` 逐字:
> **兩個要他拍的**:①**分組怎麼切** ②**沒填生日的人**歸哪一堆

⇒ **`E=丙` 的「幾歲到幾歲」就是它的「年齡」那一片** ⇒ **這兩題現在是我的了,而 Sean 沒被問過。**
⇒ 而 ② 特別重要:**沒填生日的人在「這個月生日」與「幾歲到幾歲」兩種篩法下都會【消失】** ——
   而畫面上看不出來少了誰。**這與 §0 的 OAuth 那一段是同一個病的第二個入口。**

### 1-3b-④ 另外一支也撞到,而它是【編輯】不是【篩選】
```
docs/specs/2026-08-14-25-customer-profile-edit-plan.md(C 窗 2026-08-14,待 Sean 批)
:63 逐字「**C1** | 姓名/電話/生日 | **8**(檔)| 標準片 | **鐵則 8 命中**」
```
⇒ 🔴 **「把生日接上去」已經有人寫過 plan 了,8 支檔、等批。**
⇒ 本檔 §1-3 寫「生日在會員中心本來就該有,我們只是還沒接」—— 那句沒錯,
   **而「還沒接」的下一步不是新寫,是【去批那一份】。**
⚠️ 同檔 `:81` 還記了一格我沒想到的:
...
3-3 ✅ **這一格【關掉了】** —— Sean 2026-08-26 `q4=甲`:**現在做。**
    🔴 **而他是在【知道真實客戶可能是 0】的情況下選的**(主視窗端題時就把那格寫進去了)
    ⇒ 🔴 **不得再拿「客戶 0」回頭質疑這個拍板, 也不得把它寫成待確認的顧慮。**
    📌 那個數字本身仍留著當背景, 而它只回答「上線後多久才有用」, **不再是成本問題**:
      `2026-08-16-customer-filters-four-fields-plan.md:15` 逐字
      「客戶總數 11、全部是自己測試用、有填生日 2 ⇒ 真實客戶 0」(同檔 :16-18 自標未確認)

3-7 ✅ **已定案**(Sean 2026-08-26 `q3=乙`)= **男 / 女 / 不透露**。
    舊字面「不願透露」留痕在兩處(見 §1-3b-①),**不刪** —— 那不是誰記錯。

3-8 🔴 **兩題我沒問過而 `E=丙` 讓它們變成必答**(來源 `2026-08-16-...:91` 逐字):
    ① 年齡分組怎麼切 ② **沒填生日的人歸哪一堆**
    ⇒ ② 特別重要:那些人在兩種篩法下都會【消失】而畫面上看不出來少了誰
