...
## 4-b 🔴🔴 硬順序:丙的【地址那一半】不得走在 `#39` 前面

**Sean 2026-08-18 傍晚確認 PCM【有】在做客人自取**(逐字與選項原文在 `#39` 條目)。
而今天「自取」在系統裡沒有自己的表示 —— 它被歸在 `carrier_code = 'other'` 這個大雜燴桶
(同桶還有自送/站到站),真正的資訊只在 `carrier_note` 那段**自由文字**。

```
⇒ 在有正式的「自取」模式之前，**任何對【地址】收嚴的規則都會把自取單擋死**。
⇒ 本片已把甲乙改窄成「只擋 name」⇒ 現行版本不衝突。
🔴 而丙（DB CHECK）也**只准收嚴 name** —— 地址那一半【在 `#39` 落地之前不得再收嚴】。
```
⚠️ **這一段刻意寫在條目/plan 裡而不是只寫在信裡**:下一個做丙的人不會讀今天的信,
而「順手把三個鍵一起收嚴」是這一片從第一天就有人想做的事(backlog 原文就是那樣寫的)。
