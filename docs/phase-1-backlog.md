...
> 🔴 **已知撞號(08-10 S 窗盤出、主視窗立 #376 重編號片待排)**:#308(:8091 typeahead vs :8146 alt/h1;且 08-04 STATUS 曾有第三個 #308 設計稿瑕疵已結案)、#341(:9136 拆檔 vs :9271 W 線靶族)、#342(:9158 快速加購 vs :9294 冪等覆蓋)、#343(:9178 購物車幽靈行 vs :9309 customer_addresses RLS)。**引用這四號必附行號**,重編號前不得再派這四號。
...
- **狀態:** 🔶 **2026-08-11 拆兩半(依賴不同、非縮範圍)**
  ...
  - **269-b `?filter=new`(新品)= 🔶 段一(DB)已落檔、段二(接線)卡 apply**
    ...
    (不在 `CATALOG_SORT_VALUES` 白名單、選了靜默回退成 recommend)—— 269-b 落地時把它加進白名單即可;
    **269-b 若被取消,那一行要跟著拿掉**。
...
### #308. 🔍 採購表單的供應商選單沒有 typeahead —— 純 `<select>`,家數長大後會難找
...
- **狀態:** 🔶 **兩題已拍板並落地(2026-08-05 / D3c-4);本條的根因未修、不關閉**
...
- **現況**:0b 已把五支 CSS 的頁首偏移收斂進 `--shell-header-h`,但 `account.css:84`、
...
### #343. 🧹 購物車清不掉 `found:false` 的殘行(版本偏移/下架商品的幽靈行)
...
### #341. 🎯 W 線 harness 的零靶族與單成員靶族(W7 突變矩陣盤出的欠款)
...
- **排程**:D 窗 #347 搜尋線收尾後接手(同屬 admin orders 域,避免與 347-3 UI 片互踩)。
...
- **進度**:① 完成(08-10 凌晨 `885c3d01`,白話層僅覆蓋可證一碼一義的碼);**② 完成(08-10 `e05508d9`:彈窗改 DB 口徑 avail=instock−already+出不了品項標原因 blockedReason 四值+數量框停用)**;**排序 Sean 拍 A(08-10 晚:能出的排前面、出不了的沉底,server 端 sort)=D 窗待做小片**;③④未動(D 窗續)。
...
### #362. 🔥 `scripts/a7bm-verify.sh:42-45` workdir 閘 `/tmp/?*` 放行 `/tmp//` ⇒ `rm -rf` 可清整個 /tmp(B 窗 08-10 順帶回報立案)
...
### #376. 🔢 backlog 撞號重編號片:#308/#341/#342/#343 各兩條+#308 歷史第三用(S 窗 08-10 盤出,主視窗立案)

- **來源**:S 窗 backlog 全掃(381 條親數、89 條開內文)抓到四組同號異文;主視窗複驗屬實(行號見檔頭警示),且 STATUS 08-04 曾有第三個 #308(設計稿瑕疵,已結案)=同號被回收三次。
- **要做什麼**:四組的後到條目改派新號(改派當下 grep repo+信箱重查最大號);每條改派處留「原誤取 #N」註記;引用面 triage=repo 內 docs/code 引用逐處按內容對號改寫,歷史信件與 STATUS 不回改(加不動歷史原則)、必要處補行號消歧。
- **不修會痛在哪**:引用 #341-#343 的每一處都是二義的——派工、收割帳、審查 findings 引錯條=白工或漏做;S 已實測「拿代號找東西先對線」成本已在發生。
- **止血(已做)**:檔頭警示+這四號凍結不再派發;引用必附行號。
- **排程**:純 docs 片、待派(不擋任何施工線)。
- **編號註**:立案當下最大=#375(grep repo+信箱兩邊 0 命中 #376)。
...
- **狀態:** ⏳ 未修(**先於 #269-b 存在**,由 #269-b 的 codex R2 對抗審查掃出並立案)
...
- **問題(兩條,同一支 RPC `search_catalog_by_vehicle`):**
  1. **OFFSET 分頁沒有跨請求快照。** 每一頁是一個獨立請求、各自的快照。翻頁期間若有新商品進來
     ...
     擋不住「底下的資料集在兩次請求之間換了」。
  2. **`total` 只搭在回傳列上。** RPC 用 `count(*) OVER ()` 把總數黏在每一列;
     OFFSET 超過最後一頁時回 **0 列** ⇒ 呼叫端
     `apps/storefront/src/lib/products.ts:386` 的 `rows.length > 0 ? Number(rows[0]?.total ?? 0) : 0`
...
- **優先級:** 🟡 中低(單跑恆綠;只在多窗並行、load 高時逾時 5-6s > vitest 預設 5s)
...
### #396. 🧟 `scripts/op1p-verify.sh` 已過期 —— 對 OP2b 之後的世界零覆蓋
...
### #399. 🔢 A7-t migration 的 trigger 數量斷言過期 ⇒ A7-t 零可跑回歸覆蓋

- **狀態:** ⏳ 待修(2026-08-11 B 窗實跑發現;W-a 因此擋下 a7t-verify 收編)
- **優先級:** 🟠 中高(正式站不受影響,但**一條已上線守門鏈的回歸覆蓋現在是 0**)
- **事實(乾淨全套 migration 庫實測)**:
  `20260730140000_…a7t_cancellation_consistency_triggers.sql:284` 斷言
...
- **🔴 要改的是兩條斷言,不是一條**(2026-08-11 code-reviewer 抓到我原本只點名一條):
  · `:283` `IF v_cnt <> 2` —— 數量斷言
  · `:288` `IF v_actual IS DISTINCT FROM (…v_expected…)` —— **名稱集合精確相等**
...
| **#396** | `op1p-verify.sh` | 依賴一個**後來被 DROP 的物件**(dormant gate,OP2b 移除) | 見 #396 |
| **#399** | `a7t-verify.sh` | 對世界的**精確集合斷言**(trigger 數恰 2 + 名稱集合相等) | 見 #399 |
