...
- 🏁 **第三個獨立來源複驗(W7 2026-08-18,本機 dev server / 另一棵樹 / 另一組資料 / 真瀏覽器)**
  ```
  1440 → 容器 1246 · 14 欄 · 表格 1412 · 藏 166 · 越界的是 狀態/發票/操作
  ⇒ 與上表 1440 那一列【逐字相同】,而我量的時候沒有先看這張表(我是為了另一件事去量的)
  1728 → 容器 1534 · 表格 1534 · 藏 0        ← 🆕 上表沒有這一列
  ⇒ 套上面那條公式 1728 − 194 = 1534 ✅ 命中,公式在第六個點上仍成立
  ```
  🔴 **而 1728 這一列是有意義的,不只是多一個數:**
  `~/pcm-mailbox/A-224-STOP-20260818.md` §3-⑥ 記著桌機基準已改成 **1728**(Sean 自己的機器)。
  ⇒ **死區 [900, 1412) 的上緣是 1412,而 Sean 的容器是 1534 —— 他在死區外面。**
  ⇒ 🔴🔴 **這條缺陷【對拍板的人是隱形的】**:唯一會撞到它的是「用 1440 或 1600 外接螢幕的人」,
     而那些人正是**員工**,不是回報鏈的起點。上面「不會回報成 bug」那句因此更強一階:
     **不只是員工不會回報,是連能拍板的人都看不到症狀。**
  ⇒ 這會改變**丙案(死區內加可見提示)**的權重:它不解根因,但它是**唯一一個在 Sean 看不到的地方
     仍然會說話**的做法。**這只是把代價講清楚,不是替他挑。**
  ⚠️ **我的量測限度**:①只量 1440 與 1728 兩點,1024/1280/1600 我**沒有量**(引用那三列請仍用主視窗真站那次)
     ②本機資料是 7 張單、品名都短 —— 這不影響欄寬(`table-layout: fixed` + 固定 px),但**資料格內容的截斷情形不能拿我這次當分母**
     ③量法:`document.querySelector('.orders-grid')` 的 `clientWidth` / `scrollWidth` 與 `thead th` 逐格 `getBoundingClientRect().right` 對容器右緣
...
### #631 · 訂單列表 embed limit 500 未修 —— 四條路只修了三條

- **發現者:** A 窗 2026-08-18(B 窗 `pcm-website-v2-fb` 自行 grep 核實後指出這是第四條路)
- **事實(可重跑):**
  - `ADMIN_ORDER_LIST_ITEMS_EMBED_LIMIT = 500`(`packages/adapters/src/supabase/mappers/order.ts:389`)**原封不動**
  - 母 plan 涵蓋 **A 建箱 / B 揀貨單 / C 詳情** —— 🔴 **D【列表】沒有人修**
  - C 條(訂單明細品項改撈到盡)已落地在 B 窗 `products` 分支(`a22f6e2d` / `31c11acc` / `743e59c2`),**只修詳情那一邊**
- **不修未來會痛在哪:**
  - 第一張 **501 品項**的單進來時,**訂單列表那半仍然會靜默算錯件數**
  - 🔴 **「靜默」是重點** —— 它**不會紅、不會報錯**;畫面上「未知」與「算錯的數字」長得都很正常
  - 而列表是員工每天第一眼看的那一頁
- **今天的量測與它的失效條件:**
  - V 窗 2026-08-18 真站唯讀實測:**品項 > 500 ⇒ 0 張 / > 200 ⇒ 0 張 / 最多的那張 ⇒ 2 項**
  - 正向對照:`orders` 13 筆、`order_items` 16 列且逐列撈回實得 16(沒被 `db-max-rows` 夾)
  - 🔴 **這個 0 的失效條件:第一張真實大單進來的那天,而 repo 內【零機械訊號】** ——
    沒有守門在看「今天最大的單有幾項」。Sean 說過「現在沒有正式訂單、都是自己測試的」。
- **為什麼現在才立案(不是漏做,是刻意的克制):**
  - 🔴 **B 窗明說它不順手做** —— 修這一條是**新的鐵則 8 觸發**(動 adapter 投影 + 影響列表每一列)
  - 那是克制不是漏做,**寫在這裡免得日後被讀成「當時沒人發現」**
- **與 `#629` 的關係(🔴 刻意不合併):**
  - `#629` = 「code 依賴一個住在**伺服器設定**裡的數字」(`db-max-rows`)
  - 本條 = 「**我們自己寫死的** embed limit 沒被修完」
  - ⇒ **病根不同、修法不同。合併會讓「修完 #629」被誤讀成「這條也修了」。**
- **驗證方法(B 窗給的,直接可用):**
  - 🔴 **一次只讓一個東西壞**:要驗列表這一態就種 **501** 項;而 C 條之後**詳情不再截斷** ⇒ 對照組正好乾淨
  - (B 窗驗 C 條時刻意種 201 而**不跨 500**,理由同上:兩個病同時在場就分不出症狀是誰造成的)
- **🔴 怎麼種出可驗的樣本(T① 2026-08-18 落檔,已驗檔案存在 54 行):**
  `~/pcm-mailbox/probe-501-item-order-recipe.md`
  - `§0` 環境(指到 B 窗 runbook §1-§4 + T① 踩到的埠與坑)
  - `§1` 🔴 **一次只讓一個東西壞**的判準(200 那條種 201、500 那條種 501,為什麼要分開)
  - `§2` 種 501 項單的可貼 psql / `§3` 會擋你的真 constraint 與 trigger 清單 / `§4` 觸發後看哪裡
  - ⇒ **從「T① 那一班有過一張單」變成「任何人都種得出來」**
  ⚠️ T① 的拋棄式環境**已拆**(processes pgrep 驗死、worktree remove、埠 free)⇒ **要驗得照配方重種**
- **連動:** 訂單列表截斷態的 tooltip 文案已於 `3a2a8200` 改成三段式(不再說「請重新整理」);
  🔴 **而那句的第三段「點進詳情看得到全部」要等 C 條進 dev 之後才寫得出來** —— 已標暫緩、非漏掉

### #637 · 訂單右側面板在窄視窗把列表壓成 18px —— 已裁定的「不分割」只做在點擊路徑,URL 路徑沒做

- **發現者:** 前任 A 窗 2026-08-18 量到 `18px` 但**刻意沒判**(逐字「我刻意沒有去挑一個看起來合理的寬度」);
  W7 2026-08-18 依鐵則 1 查真權威 + 定位機制 + 補上「哪一條路走得到這個狀態」。
- **事實(可重跑;量測環境跟著數字走)**
  - 環境:`pcm-bmw-m` @ `1de98760` 自己的 dev server `localhost:3002`、2026-08-18 10:0x–10:1x CST、
    路由 `/orders`、面板**開啟**(網址帶 `panel=<id>`)、真瀏覽器(playwright,非 jsdom、非產圖工具)
  - 🔴 **這一欄要跟著數字走**:量測時 `document.cookie` = **空**、`--workspace-panel-width` = **300px**
    (那是元件第一次掛載時依當下視窗算的預設值,`workspace-shell.tsx` **刻意沒有 resize 監聽**
    ⇒ 我一路縮放視窗時它不會跟著變)。⇒ 表中每一列的 `panel` 欄之所以都是 320,
    是 CSS `min-width: 320px` 把 300 頂上去的,**不是使用者存過 320**。
  - ⚠️ 換成沒縮放過的新分頁,預設寬 = 視窗一半(1440 ⇒ 720)⇒ 表中 1440 那列的 `content` 會更小;
    **而 390 那列不受影響**(390 的一半 = 195,一樣被 `min-width: 320px` 頂到 320)⇒ **18px 這個結論不變。**
  - 逐檔量到的退化曲線(單位 px):

    | viewport | `.workspace-row` | `.workspace-panel` | `.workspace-content` | `.orders-grid` |
    |---|---|---|---|---|
    | 1440 | 1296 | 320 | 972 | 924 |
    | 900 | 756 | 320 | 432 | 384 |
    | 768 | 644 | 320 | 320 | 272 |
    | 600 | 600 | 320 | 276 | 228 |
    | 390 | 390 | 320 | 21 | **18** |

  - 機制(不是猜的,`app/globals.css` 那條規則自己寫得出來):
    `.workspace-panel { width: var(--workspace-panel-width); min-width: 320px; max-width: calc(100% - 480px) }`
    ⇒ 依 CSS 規範 `min-width` 在 `max-width` **之後**套用 ⇒ **兩個下限打架時 `min-width` 贏**
    ⇒ row 寬 < `320 + 480 = 800` 時,**被犧牲的是列表那一欄**,而且沒有下限
  - 🔴 **`800` 是那條規則自己的兩顆常數加起來,不是我挑的**
    (`MIN_PANEL_WIDTH = 320` / `MIN_CONTENT_WIDTH = 480`,`lib/layout/workspace-panel.ts`)
- **🔴 真權威怎麼說(鐵則 1,查法與分母都寫在這裡)**
  - OD `list_projects` 當場列 ⇒ **9 個專案**,`designSystemId = bmw-m` 的有 2 個;
    訂單後台的權威 = `pcm-admin-order-ui`。
  - 該專案 `search_files "@media"` ⇒ **2 處命中**,都在 `overview-desktop-bmw-m.html:322` / `:326`,
    內容是 `.cmp3` 欄數與 `.wrap` 左右留白 ⇒ 🔴 **OD 對「右側面板在手機怎麼辦」查無規格。**
  - `design-reference/` 是**顧客站**的設計系統(`filter-drawer.css` / `account.css` …),沒有後台面板 ⇒ 同樣查無。
  - ⇒ **答案不在 OD,在本 repo 的既有拍板:**
    `docs/specs/2026-08-10-350-workspace-shell-plan.md:98` **Q5** 逐字:
    「小螢幕沒有分割的空間,點編號照舊整頁進詳情」;
    `lib/layout/workspace-panel.ts:54` 註解逐字:「真正的小螢幕解法是**不分割**」。
  - ⇒ 🔴 **要做什麼早就裁定了。缺的不是設計決定,是那個裁定只落在一條路上。**
- **🔴 為什麼「點擊」那條路沒事(這一條決定了嚴重度,不要漏)**
  - `orders-table.tsx` 同一格渲染**兩個連結**:`a[data-nav='panel']` → `/orders?…&panel=<id>`、
    `a[data-nav='page']` → `/orders/<id>`;顯隱由 `globals.css` 與卡片化**同一條容器斷點**決定。
  - 390 實測:`panel` 連結 `display:none`(寬 0)、`page` 連結 `display:block`(寬 58)
    ⇒ **手機上點單號拿到的是整頁詳情,Q5 有被實作。**
  - ⇒ 走得到 18px 的只有**繞過那格的路**:①桌機開著面板把視窗縮小 ②把帶 `panel=` 的網址
    存書籤 / 傳給手機上的同事 / 上一頁回到它。
- **不修未來會痛在哪**
  - 員工之間傳一條訂單網址是**最自然的動作**,而桌機複製出來的網址**就是帶 `panel=` 的那條**
    ⇒ 收到的人在手機上打開,看到 320px 詳情 + **18px 寬的一條列表**。
  - 🔴 **它不會報錯、不會紅、`body.scrollWidth` 也沒有溢出**(390 實測 = 390)
    ⇒ **repo 內零機械訊號**,只有人看到才知道。
  - ⚠️ 中間地帶不是「壞掉」而是「變窄」(900 ⇒ 列表 384px 卡片模式,還讀得到)
    ⇒ **不要把這條寫成「窄視窗全壞」,壞的是 row < 800 之後沒有下限這件事。**
- **修法方向(🔴 命中鐵則 8:動共用版面 ⇒ 要 plan 等批,W7 沒有自己改)**
  - 語意上要的是:**row 放不下兩欄時 ⇒ 面板佔滿、列表讓位**(= Q5 的「不分割」),
    而不是現在的「面板佔 320、列表讓到剩 18」。
  - 🔴 **觸發條件要的是 row 寬、不是 viewport 寬** —— row = viewport − 側欄,
    而側欄在 768 以下是 off-canvas(實測 vw 600 ⇒ row 600)、以上會吃掉 124–144px(實測 vw 768 ⇒ row 644、
    vw 900 ⇒ row 756、vw 1440 ⇒ row 1296),**展開態還會更寬** ⇒ 一條 viewport `@media` 天生對不準。
  - ⚠️ **而 `.workspace-row` 加 `container-type: inline-size` 是有代價的**:它連帶 `contain: layout`
    ⇒ 該元素會變成 `position: fixed` 子孫的 containing block,而建箱彈窗正是 `fixed inset-0 z-50`
    ⇒ **會動到彈窗的定位**。`workspace-shell.tsx` 檔頭已經為了同一類「順手加樣式改掉全站行為」
    的坑寫過一整段(捲動模型那條)⇒ **這條不能順手做。**
  - ⇒ 兩案給 Sean:**甲**=容器查詢(準,但要先驗彈窗定位沒被改掉)/ **乙**=保守 viewport 斷點
    (不準但零風險,只在明顯放不下時生效)。**W7 推薦先量甲的副作用再決定。**
- **🔴 我沒驗的(逐條附缺哪一道檢查)**
