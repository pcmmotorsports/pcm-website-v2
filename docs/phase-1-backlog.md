...
> 🔴 **已知撞號(08-10 S 窗盤出、主視窗立 #376 重編號片待排)**:#308(**採購表單供應商 typeahead** vs **品牌頁 band logo alt/h1 重複**;且 08-04 STATUS 曾有第三個 #308 設計稿瑕疵已結案)、~~#341(拆檔 vs W 線靶族)~~ **已解決:2026-08-11 W 線那條改號 #420**、#342(**卡片快速加購只會導頁** vs **冪等層部分接線零覆蓋**)、#343(**購物車 `found:false` 幽靈行** vs **`customer_addresses` 可被客人直接寫入**)。**引用這三號必附標題關鍵字**(不是行號),重編號前不得再派。
>
> ⚠️ **本行原本釘行號,2026-08-13 全數過期**(:8091/:8146/:9158/:9294/:9178/:9309 → 實際 :8327/:8382/:9663/:9805/:9683/:9820)。本檔一直在長,行號釘了就會爛 ⇒ 改釘**標題關鍵字**。
...
- **狀態:** 🟠 **報價單側已結案、我方剩 3 筆(2026-08-13 同步後實查定案)**。
