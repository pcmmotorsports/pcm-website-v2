...
### #380. 🐛 admin 側邊欄「可隱藏」按鈕消失,補回(Sean 2026-08-10 深夜正式站肉眼驗回饋)

- **來源**:Sean 實測「側邊欄位原本可隱藏按鈕現在不見,請補回」。疑=某次 WorkspaceShell/側欄改版(350a 收窄或其後)把 collapse toggle 拿掉或藏死。
- **要做什麼**:診斷先行(git log 找按鈕何時消失、是刪了還是 CSS 蓋掉)→ 補回=輕量-標準片照診斷結果判。
- **不修會痛在哪**:員工要更大工作區時收不起側欄;既有能力倒退=體驗回歸。
- **排程**:優先於 #379(這是 bug 回歸不是品味);窗醒後派。
