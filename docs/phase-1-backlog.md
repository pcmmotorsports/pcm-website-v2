...
- **不修會痛在:**
  ...
  - **🔴 與 `#445` 硬綁(這是真正的時限):** `#445` plan v3 §4-2 把 `manual_failed` 算成**佔額度**(保守方向,為了不漏掉「人判錯、錢其實動了」的情形)。**一旦 `#445b` 上線,這條就從「紀錄改不了」升級成「那張訂單之後永遠退不了款」** —— 而解鎖入口不存在。⇒ **`#445b` 的 apply 停點之前必須先有出口**,否則等於拿一個永久卡單換一道超退防線。
