...
- 🔴 **不修未來會痛在哪(鐵則 10,這條是本條目的核心):**
  ...
  **沒有任何守門會叫** —— X8 不擋它,CHECK `shipments_shipped_needs_tracking` 也只要求「非空」。
  ⇒ 症狀會出現在**客人那一端**:他拿著~~紙上的~~舊單號去查,查到別的貨或查無。
  🔴 **更新(`Q-C5`=丙,2026-08-17)**:**紙上已經不印追蹤碼了** ⇒ 客人手上的舊單號
  **來自LINE／Email**(`Q-C9`),不是那張紙。**病沒有消失,載體換了而且更難收**:
  紙還在自己人手上,**LINE 訊息寄出去收不回來**(鐵則 12⑤)。
- **現行唯一救濟:** 作廢重開新箱 —— 而**那會換箱號,已經印出去、跟著貨走的那張紙就白印了**。
  (⚠️ 這一句仍然成立:**箱號**照印,丙拿掉的只有追蹤碼那一列。)
- **要決定的(修之前先答,不要直接開工):**
...
- **不修未來會痛在哪:** ~~打錯一碼的紙**印得出來、看起來完全正常**~~,而客人拿去 HCT 網站查
  ...
  🔴 **更新(`Q-C5`=丙,2026-08-17)**:紙上**已經不印追蹤碼**了 ⇒ 載體改成**LINE／Email**(`Q-C9`)。
  **本條沒有變輕,是變重了** —— 紙至少還在員工手上、有機會被人看一眼;
  **LINE 訊息寄出去就收不回來**(鐵則 12⑤),而錯的那一碼與對的那一碼在寄出前長得一模一樣。
  ⇒ 這道格式守門要在 `Q-C9` **寄信之前**就位,不是之後。
...
- **⚠️ 一條【被我讀了 code 之後推翻】的成因假說,留著防重踩:**
  ...
  **開檔實查:不是。** `stageAndRunGate()` 的 `git add` 帶 `{ cwd: scratch }`,
  ...
  🔴 **後續(C 窗自陳,同日)**:那個相關性**也不成立** —— 它補出 A/B/C 三次
  **髒樹而全綠**的紀錄(見下方六筆表)。⇒ 這條的正確讀法是
  **「機制錯 + 相關性也錯」**,不是「相關性對、只是理由要換」。
- **🔴🔴 C 窗補的六筆原始觀察(2026-08-17,`print-docs` 樹上)—— 而它們把 C 窗【自己那條相關性】也推翻了:**

  | # | 時刻 | duration | 結果 | 工作樹 |
  |---|---|---|---|---|
  | A | 11:52:25 | `87.91s` | `508 passed (508)` · **0 failed** | **髒**(4 個未 staged 改動) |
  | B | 12:07:58 | `77.22s` | `508 passed (508)` · **0 failed** | **髒** |
  | C | 12:18:00 | `62.88s` | `8513 passed` · **0 failed** | **髒** |
  | D | 12:50:09 | `76.62s` | **1 failed** / `509` 檔 | **髒**(2 改動 + 3 未追蹤) |
  | E | ~12:52 | (未記) | **5 failed** | **髒**(同 D) |
  | F | 12:54:31 | `142.80s` | **4 failed** | **髒**(同 D) |
  | G | ~12:57 | (未記) | `8521 passed` · **0 failed** | **全部 `git add`** |

  | H | 13:35 | `82.87s` | **1 failed**(`好 .sql staged → 放行`) | **全部 `git add`** |

  🔴 **A / B / C 三次【樹是髒的而且全綠】,而 H 一次【全部 staged 卻紅了】**
  ⇒ **「髒樹 ⇒ 紅」兩個方向都不成立,C 窗那條相關性也錯了。**
  📎 **H 是後來補上的第七筆,而它比 A/B/C 更硬**:A/B/C 只推翻「髒 ⇒ 必紅」,
  **H 直接推翻「staged ⇒ 綠」** —— 也就是我原本拿來當結論的那一筆(G)只是碰巧。
  ⚠️ H 之後單獨重跑該檔 ⇒ `9 passed`,與前幾次同一形狀。
  ⚠️ **它原本只有 D/E/F/G 四筆**(髒紅、staged 綠),而 A/B/C 一直都在同一個 session 的上面
  —— **它挑了離結論最近的那一段當樣本**,那才是病根,不是機制寫錯。
  📎 I 窗推翻的是機制;**這一段推翻的是相關性本身**,兩件事都要記,因為
  **「相關性還在、只是解釋錯了」比「相關性也不成立」更容易讓人繼續往錯的方向修。**

  **改往哪個方向看(仍是【推論】,未量)**:
  ```
  F 是最慢的一次（142.80s ≈ 平常的兩倍），也是紅最多的一次
  ...
  ⇒ **若成因是全域的(負載 / 並行度),修 gate 那支只會讓症狀換一個檔出現。**
  ⇒ 上面「甲 加 testTimeout / 乙 重寫 harness」兩個選項**都預設病在這支檔**;
  **在成因確認之前,那個前提本身還沒被驗。**
  📎 每次紅的**那一組不一樣**(D 紅 1 格、E 紅 5 格含跨檔、F 紅 4 格)⇒ 與 timeout 相符、與邏輯錯不符。
...
- 🔴 **2026-08-17 A 窗另外四筆,而它帶來了本條第一個【可證偽的假說】:**
  A 窗同日跑四次全套:**508 全綠 / 5 檔紅 / 3 檔紅 / 13 檔紅**,**全部是 `timed out`,而失敗的檔每次都不同**。
  🔴 **它同時量到 `load average 230.13`。**(這四筆與那個 load 數字**是 A 窗量的,不是 I 窗**。)
  ⇒ 加上 I 窗那六筆(慢的四次都有紅、快的兩次全綠),**假說可以寫成一句會被推翻的話**:
  ```
  假說：機器 load 高 ⇒ 個別測試撞到 vitest 預設 5000ms 逾時
  驗它的方法（控制變因，不用設計）：等機器閒下來、確定沒有其他窗在跑，再跑一次全套。
    · 閒的時候仍然紅 ⇒ 假說被推翻，要回頭找真成因
    · 閒的時候全綠、忙的時候紅 ⇒ 假說成立，修法就是 vitest.config.ts 一個 testTimeout
  ```
  🔴🔴 **2026-08-17 15:0x 一筆【反例】,I 窗量的 —— 它讓假說變弱,不要漏掉它:**
  ```
  跑前 load1m=173.40   跑後 load1m=140.92   duration 51s   ⇒ 512 檔【全綠】
  對照同一小時內：load1m≈196 ⇒ 2 檔紅 76.5s ／ A 窗 load≈230 ⇒ 13 檔紅
  ```
  ⇒ **`load average` 173 之下它跑得又快又全綠** ⇒ **「1 分鐘 load 高 ⇒ 會紅」不成立。**
  📎 **可能真正相關的是「那一刻有沒有別的 CPU 重活在跑」,而 `load average` 是過去一分鐘的平均、不是當下。**
  ⇒ **假說要重寫成一個量得到的東西**(例:跑測試那 60 秒內有沒有其他 `pnpm`/`next build` 行程),
  **而不是抄一個 `uptime` 數字。**
  ⚠️ **這一筆刻意留著而不是把假說刪掉** —— 它是這條目前唯一的反例,
  **刪掉它、下一個人會用前面那幾筆「確認」一個已經被打過一次的假說。**
  ⚠️ **在跑那一次之前,它仍然只是相關性。** 本條**不得**被引用成「成因已查明」。
  📎 **「失敗的檔每次都不同」是這條假說最有力的一面** —— 若病在某一支測試檔,不會每次換人。
...
### #611 · 單筆請求上限擋不住跨請求放大 —— 出貨候選單沒有速率限制

- **狀態:** ⏳ 待執行(**不擋任何人**;它是 `MAX_SHIPMENT_CANDIDATE_ORDERS = 50` 那道單筆上限的**已知殘餘**)
  ⚠️ **那道上限沒有自己的 backlog 號** —— 它是 2026-08-17 C 窗直接落地的一片,
  座標 `apps/admin/src/lib/shipping/shipment-candidates.ts`,錨點 `git grep -n MAX_SHIPMENT_CANDIDATE_ORDERS`。
  📎 本行原本寫「`#610` 那道單筆上限」—— **`#610` 是我落筆時憑感覺寫出來的號,沒有配過**,已更正。
- **病:**
  ```
  loadShipmentCandidates 現在有 MAX_SHIPMENT_CANDIDATE_ORDERS = 50，
  而那只綁住【單一請求】的 fan-out。

  被盜的 admin session 平行送 20 個請求 × 每個 50 個不同 id
    ⇒ 1000 次 findAdminOrderDetail。
  ```
  🔴🔴 **危害的描述【已經被收窄過一次】,不要再擴回去**(codex 2026-08-17 R2 開檔核出來的):
  本條初稿寫「成交價 + 客人 PII 照樣一次撈走」—— **那是假的**。
  這支 action 回的是**刻意很窄的 DTO**(零金額欄),`recipient` 只有 **`details[0]` 那一張單的**收件資料
  (`shipment-candidates.ts` 錨點 `recipient: details[0]!.shippingAddress`)。
  ✅ **已證實的危害只有兩件**:① 無界 DB fan-out ② 一次拿到大量訂單的品項 / 料號 / 單號對映。
  📎 **為什麼專程收窄**:誇大的威脅會把力氣導去做「不要回傳金額」——**它本來就沒回**,
  而真正該做的是速率限制。**論證寫得越有說服力,搬走的人越不會回頭核它。**
  🔴 **這條的價值在於它推翻了一句寫在規格裡的話。** E 窗原規格第 4 條逐字
  「長度一上限、fan-out 自動有界(≤N),**不需要另外處理**」——
  **那句話只在「攻擊者只發一個請求」這個前提下成立,而那個前提沒有人寫出來過。**
  (codex 2026-08-17 對抗審查 R1 抓到;C 窗已把該註解改掉,原句留痕。)
- **⚠️ 等級與範圍(不要放大也不要縮小):**
  - **不跨授權邊界**、仍然要有 admin session ⇒ 它是**帳號被盜之後的放大器**,不是入口。
  - **那道上限沒有變成白做**:它把「一個請求 = 無限」變成「一個請求 = 50」,
    攻擊者要達到同樣的量得**自己發 N 倍請求** —— 而那正是速率限制看得見的形狀。
- **要決定的(修之前先答):**
  ```
  ① 限在哪一層？ Vercel 邊緣 / middleware / server action 內自己數
     ⚠️ 平台方案是 Vercel Hobby（memory reference_pcm-platform-plans-vercel-hobby-supabase-pro）
        ⇒ 平台級速率限制不一定拿得到，先查證再設計，別假設有
  ② 母體是什麼？ 每個 admin 帳號 / 每個 session / 全域並行上限
  ③ 這是不是只有出貨候選單的問題？ ← 幾乎確定不是
  ```
  🔴 **③ 是重點:任何「吃陣列 + `Promise.all` map」的 server action 都是同一個形狀。**
  只修這一支 = 修了一個症狀。開工前先盤點全站同形狀的入口,再決定要不要做成共用的一層。
- **不修未來會痛在哪:** 帳號被盜時,**這是把「看得到後台」放大成「用少數幾個請求把大量訂單的
  品項 / 料號 / 單號對映一次抓走,並把 DB 打到抖」的那一段**(⚠️ **不是**「拖走成交價與 PII」,見上方收窄)。
  而它**沒有任何症狀** —— 沒有 audit log(出貨線兩層都沒留痕,見 `Q-AUDIT-1`)、沒有告警,
  只有 DB 負載會抖一下,而那時沒有人在看。
- **📎 順帶記一條【不是本片造成、但同一段路上】的既有缺口**(codex R2):
  `shipment-dialog.tsx:137` 用 `void onRefreshCandidates?.()` ⇒ **沒有接住 rejection**。
  一般的 DB / 網路錯誤在那裡就會變成 unhandled rejection,**與 50 這道閘無關**。
  修速率限制那一片時順手看一下,不要另開一條線。
- **⚠️ 誠實邊界:** 「20 × 50 = 1000」是**推論**,我**沒有實際發過並行請求試**;
  推論的依據是 `fetchShipmentCandidates` 是無狀態 server action、每個請求各自跑一次 `Promise.all`。
- **由來:** 2026-08-17 · C 窗做「出貨候選單單筆上限 50」那一片時,codex 對抗審查 R1 抓到 · 由 C 窗立案。
- **發號兩道閘都跑了:** `sh scripts/reserve-backlog.sh` ⇒ `RESERVED #611`(git ref CAS,不是下限);
  信箱佔位掃描 `grep -rn '#611' ~/pcm-mailbox/*.md docs/` ⇒ **零命中**,
  附正向對照 `grep -rc '#606' ~/pcm-mailbox/*.md | grep -v ':0$'` ⇒ 命中 3 檔以上 ⇒ **那個 0 是量出來的**。

### #612 · admin 繳稅註解清理:alias 修好後的過期字面

- **狀態:** ⏳ 待執行(輕量、純註解/文案,不動行為)
- **病:** `#606` 之前 apps/admin 寫不出 `@/` import 的測試,大量檔案帶著「改相對路徑繳稅」類註解。
  alias 修好(vitest projects per-app alias)之後,**那些註解全部變成過期字面** ——
  下一個人讀到會以為現在還要繳那個稅、繼續把 `@/` 改成相對路徑。
- **量(2026-08-17 落筆當下、vitest-alias 樹上重跑;歷史的「33/15」兩數主視窗已裁【量法已佚失、不再引用】,見 STATUS 08-17 稽核表 row 4,本條不沿用):**
  ```
  grep -rln '相對路徑\|不要用 @/\|alias.*storefront\|vitest.*alias' apps/admin/src | wc -l
  ⇒ 35 支(同命令可重跑復驗)
  ⚠️ 這支 pattern 會自命中:凡提到 alias 的檔都算,含 #606 自己新寫的測試檔
     (35 支裡 12 支檔名含 test)⇒ 它是【候選清單產生器】不是繳稅檔計數;
     清理時逐支開檔親讀病灶句再判,不整批 sed(特徵所在行 ≠ 病灶所在行)
  ```
- **為何不在 #606 同片清:** plan §3⑤ 明寫 —— 混進去會讓 #606 的 diff 大到看不出重點(它動的是全域測試解析設定)。
- **不修未來會痛在哪:** 過期註解會誘導人手動把 `@/` 改回相對路徑=重新開始繳已經廢止的稅;
  且與 shadcn 生成物(固定產 `@/`)方向相反,同一個 repo 兩種指向的「規矩」並存,新人無從判斷哪個是真的。
...
- **病:** L3 片5 把五欄拉寬之後,六顆欄寬**全部貼著今天資料的上緣**。
  它們被同一個綠勾蓋住(某輪報告的「六顆欄寬完全相同 ✅」)⇒ **打勾了就沒人會再看**,
  而其中一顆的餘裕只有 **1px**。
- **六顆的現況(宣告寬口徑;每一條都附 `globals.css` 的行號,可重跑 `grep -n 'col-<x>' apps/admin/src/app/globals.css`):**

  | 欄 | 宣告寬 | 實測需求 | 餘裕 | 出處 |
  |---|---|---|---|---|
  | `col-invoice` | 56 | **55** | 🔴 **1px** | `:851` 逐字「片5:44 → 56。實測需求 = 55px」 |
  | `col-oid` | 132 | 131 | 1px | `:774`(`PCM-2026-0104` 舊格式 13 碼) |
  | `col-brand` | 188 | 187 | 1px | `:807`(`EVOTECH PERFORMANCE`,brands 表全庫最長) |
  | `col-vehicle` | 180 | **229** | 🔴 **−49px 已截** | `:795`(`2012 Ducati Hypermotard 1100 Evo` = 213,需欄寬 229) |
  | `col-title` | 154 | 128 | 26px | `:820`「**實測 0/18 列被截**,今天最長品名 = `品牌貼紙（白色）`」 |
  | `col-amount` | 104 | 103 | 1px | `:839`(`NT$ 128,000` = 103px;單價欄 `:832` 同一個字串) |

- **為什麼這是債不是 bug:** 今天全部(除車型欄外)都放得下。
  **明天進一個更長的品牌 / 更長的品名 / 六位數以上金額,就會截,而截了沒有任何東西會紅。**
  `col-brand` 與 `col-title` 的註解自己都寫了「明天進更長的就會再截」。
- **不修未來會痛在哪:** 員工在列表上看到被截的料號或品牌,會**點進去確認**,或更糟 —— **照著截斷的字去對單**。
  車型欄今天就已經在截 —— **`globals.css:796` 逐字「兩個 oracle 一致:文字寬法 **2/11**、`scrollWidth` 法也 **2/11**」**,那是既有事實、不是預測(而且它自帶兩個獨立量法互核)。
- **修法不是「再加寬」:** 表格宣告總寬已 **1412px**,而容器 1294px ⇒ 已經要橫向捲動(溢出 118px)。
  再加寬只會讓捲動更長。**這是 Sean 的取捨題**:哪幾欄可以截、哪幾欄不能截、要不要放棄「一屏看完」。
- **量測層級要跟著走(踩過):** 上表全是**宣告寬**(含內距)。
  另有一組**內容盒**口徑(扣 7+7 內距),例如發票欄 `39 / 40` —— **兩組不可互相引用**。
...
- **要先做的事(這一條就是本條的驗收):**
  ...
  ```
- 🔴 **2026-08-17 傍晚新增:一條【讀出來的】機制,不是推的**
  一直紅的那支 `scripts/check-syntax-nonts.gate.test.ts`,**每一格都 spawn 一個 `lint-staged` 行程**
  (`scripts/check-syntax-nonts.gate.test.ts:42-48`:`spawnSync('git', ['add', …])` + `spawnSync(LINT_STAGED, …)`)。
  ⚠️ **它用的是 `mkdtemp` 拋棄式 repo(`cwd: scratch`,`:60-64`)⇒ 不碰真 repo 的 index**
     —— 我原本懷疑「七窗共用 `.git` 互相干擾」,**開檔讀完就排除了**,那個懷疑是錯的。
  ⇒ 真正的形狀:**`vitest` 的 5000ms 預設門檻,對「要開子行程」的格子在高負載下不成立**。
  ⇒ 這不是「flake」,是**門檻與那格做的事不相稱** —— 它與其他格根本不是同一種東西。
  ⇒ 🔴 **可測的下一步(比重跑有用)**:量那幾格的 `spawnSync` 實際耗時分佈,
     而不是量 load average。門檻該不該分開,由那個分佈決定。
  📎 這一條符合本條目自己的要求:**先量得到,再談假說。**
- ✅ **2026-08-17 夜 T 窗:①②兩件的【儀器】已落地**(`scripts/dev-four-greens.sh`,分支 `four-greens-drift`)
  —— 🔴 **這是儀器不是答案。它沒有解釋任何一次紅,它只讓下一次紅【自己帶著條件】。**
  ```
  ① 每一發記【並行 vitest 行程數】(開始/結束兩個瞬時值)+ 起訖時戳
     ⇒ 印在畫面上，同時落 logs/four-greens/<目錄>/RUN-CONTEXT
  ② 每一項的【完整輸出】改留在 repo 內的 logs/…/<項目>.full.log
     ⇒ 在此之前它只在 run-rc 的 mktemp（/var/folders），macOS 會清掉
  ```
  🔴 **為什麼記行程數不記 Duration**(V 窗抽的,原字採用):Duration **部分內生** ——
  timeout 自己會把時間拉長 ⇒ **慢可能是果不是因**。並行行程數是**外生**的。
  🔴 **當場配了負測**:格20/21 起一個帶唯一標記的行程 ⇒ 計數 0→1→0
  (突變 M12 計數器恆 0 ⇒ 格20 紅;M13 恆 1 ⇒ 格21 紅)。
  **一個永遠回 0 的計數器不會報錯**,它會讓每一發都記下「並行 0 個」而看起來完全正常。
  🔴 **平台眉角(落筆當場實測,不是查來的)**:**macOS 的 `pgrep` 沒有 `-c`**
  —— `pgrep -c vitest` ⇒ 印 usage、`rc=2`。只能 `pgrep -f '[v]itest' | wc -l`。
  ⚠️ **射程限定**:記的是**開始與結束兩個時點的瞬時值**,不是期間最大值 ——
  中間衝上去又掉下來這兩個數看不到,要看得到得改成持續取樣,不在該片範圍。
  ⚠️ **②的成本與界線**:`logs/` 仍在 `.gitignore` 裡 ⇒ **痕跡只活在跑的那台機器上,進不了 git**。
  本次**刻意不改成追蹤**(四綠全量 log 進 git = 每次收割都塞幾百 KB 噪音);
  要留給後人的那幾份仍走**手動搶救**(今晚 I 窗已把 24 支搬進 `docs/probes/2026-08-17-vitest-red-runs/`)。
  🔴 判這一刀的依據是**「這份 log 給誰看」**:尾 N 行服務「當下看紅在哪」的人,
  全量服務「三週後查因果」的人 —— **兩個讀者都要,而在此之前只有前者被服務到。**
- 🔴🔴 **【作廢】原「第一筆帶著條件的樣本」(2026-08-17 21:41)—— 量具特異性不足,那個數字不是訊號。**
  **作廢理由(22:0x 同一個窗實測,兩個世界):** 第一版 pattern 寫 `pgrep -f '[v]itest'`,
  **它量到的不是「vitest 在跑」,是「誰的命令列提到 vitest 這個字」**:
  ```
  真的有 vitest 在跑時   舊尺 23~25   新尺（認 vitest.mjs）2   ← 新尺 = 真的 run 數
  沒有 vitest 在跑時     舊尺 ≥3      新尺 0                  ← 舊尺有一個「地板」
  假命中實例（pgrep -fl 逐一列出來的）：
    · 別窗的 zsh -c … grep -E "^typecheck|^lint|^build|^vitest…"   ← grep 的 pattern 裡有那個字
    · 別窗一支叫 vitest-sampler.sh 的腳本                          ← 檔名裡有那個字
  ```
  🔴 `[v]itest` 那個括號技巧**只避開 `pgrep`/`grep` 自己,避不開【別人命令列裡的那個字】** ——
  在八個窗同時工作的機器上,那個「別人」隨時都在。
  ⇒ **`起跑前=2` 不可讀成「2 個並行 run」,也不可換算** —— 只能重量。
  ⚠️ **同一把壞尺量的還有 C 窗 22:02 那發的 `起跑前=12`** ⇒ **一併作廢**
  (對照:2 個真 run 就讓舊尺讀到 23~25 ⇒ `12` 對應的真 run 數**可能是 1、也可能是 0**)。
  🔴 **⇒ 落筆當下,帶【可信】並行數的樣本是【零筆】。** 我原本以為有 1 筆,而那對我不利,照實寫。
  🔴🔴 **而「改認 `vitest.mjs` 就修好了」也是錯的 —— 第二版同樣自我命中,一晚三版:**
  ```
  第一版  pgrep -f '[v]itest'        ⇒ 量到「誰的命令列提到 vitest」
          假命中：別窗 grep 的 pattern、一支叫 vitest-sampler.sh 的腳本
          I 窗複驗補了主污染項：13 個命中裡 10 個是【同一發 run 的 worker】
          🔴 而 worker 數會【跟著 run 數一起動】⇒ 舊尺看起來「有反應」
             ⇒ 一把會跟著世界動、但【單位是錯的】尺，比完全不動的尺更難被發現
  第二版  pgrep -f '[v]itest\.mjs'   ⇒ 🔴 仍然自我命中。I 窗量到【零個真 run 而讀到 5】：
          3 個是它自己的 zsh（同一條命令裡 echo 了含該字面的解釋文字）
          2 個是別窗的 codex（**prompt 文字裡有那個字面**）
          🔴 我自己也做過同一件事：我請 codex 審這把尺，prompt 裡就寫著那個字面
  ⇒ 【x】那個括號技巧只避開 grep/pgrep 自己，避不開「有人正在談論它」
  🔴🔴 這把尺的地板高度 =【現在有多少人正在打字提到它】
       而討論強度【在事故當天最高】⇒ 噪音與調查力度正相關
  ```
  📎 **I 窗抽的判別句(主視窗核可,原字採用)**:
  > **命令列是【任何人都能寫進去的字串】,而執行檔名與檔案系統不是。**
  > **凡是從 `ps`/`pgrep` 的【命令列】數東西,你數的是「誰提到過它」,不是「它在不在跑」。**

  ✅ **第三版(現行,主視窗裁定走這條):不從 `ps` 數,改數【標記檔】。**
  跑 vitest 那一段自己在 `logs/four-greens/.running/` 開一支以 PID 命名的檔,跑完刪掉
  ⇒ **那不是字串比對,是「只有真的有那一發 run 才會存在的東西」**;
  **它不需要任何人記得任何事,也不受任何人寫了什麼字影響。**
  **兩道防呆一起上**:`trap` 刪自己那支 + 讀的時候 `kill -0` 驗 PID 還活著
  (🔴 `trap` 對 SIGKILL 無效,少了第二道,一次被 kill 的 run 會讓這個數**永遠多算 1**)。
  **五格守門 + 四發突變**:
  ```
  格25-a 零個 run ⇒ 必須 0        （🔴 前兩版最大的病：它們有地板）
  格25-b 兩發獨立 run ⇒ 必須 2    （🔴 單位那一格 —— 它解掉了「我報 2 / I 窗報 1」那個分歧：
                                    字串尺量的數字沒人分得出是「兩發 run」還是「一發兩個命中」）
  格25-c 不算自己                  格25-d PID 已死的不算        格25-e 而且順手清掉
  突變 M23 恆 0 / M24 不驗 PID / M25 不排除自己 / M26 不清死檔 ⇒ 各紅對應那幾格
  ```
  ⚠️ **這一版換了一種瞎,寫在這裡**:**只數【走這支腳本】的 run**;
  別窗手打 `npx vitest run …` 沒有標記檔 ⇒ **算不到**。
  ⇒ 它答的是「**還有幾個窗在跑四綠**」,不是「機器上總共有幾發 vitest」——
  **欄位名已照這個改**(`其他窗四綠vitest並行數(不含自己)`),**語意寫在欄位名裡,不留給讀的人猜。**
  📎 教訓全文 `docs/patterns/traps-inbox/T-20260817.md` 第一則(第五、六、七個實例)。

  **以下是那一發的原始紀錄,刻意留著不刪**(它的 `Duration` / 逾時 / 失敗清單仍然可用,
  **只有並行數那三個值作廢**)——
  🔴 **不刪的理由**:刪掉的話,下一個人會以為那個數字從來沒有被寫進分析過,
  **而它被寫進去了、還被兩個窗引用了一輪。**
  ```
  logs 目錄        4d7cb736-20260817T214126-72761
  全量已搶救       docs/probes/2026-08-17-vitest-red-runs/4d7cb736-…-FULL-with-runcontext.txt
                   （logs/ 在 .gitignore；檔尾附了 RUN-CONTEXT 三行）
  RUN-CONTEXT      並行vitest行程數 開始=1 【vitest起跑前=2】 結束=1
                   起訖 21:41:26 → 21:43:45
  結果             Test Files 1 failed | 517 passed；Tests 2 failed | 8581 passed
  失敗檔           apps/admin/src/lib/shipping/order-shipments.test.ts（2 格）
  第 1 格          🔴 Test timed out in 5000ms   ← 本條目「症狀 A」逐字
  第 2 格          AssertionError: expected null not to be null（同 describe 的【正向對照】）
  ```
  **判它不是回歸的三個依據(逐條可重跑,不是推論):**
  ```
  ① 本窗那兩顆 commit 動到 apps/ 的檔數：git diff --name-only <base>..HEAD | grep -c '^apps/' ⇒ 0
  ② 單獨跑那一支檔（不併發整套）：npx vitest run <該檔> --project admin ⇒ rc=0，16 passed
  ③ 那兩支被測檔最近的異動是別人的片（ee7b1f35 / 65b76981），不在本窗界線內
  ```
  ⚠️ **我沒有證到的那一格,明寫**:第 2 格是 `AssertionError` 不是逾時,
  我**推測**它是第 1 格逾時之後同檔 module mock 狀態被帶壞的連鎖(本條目「症狀 B」那一族),
  **而我沒有構造出那條連鎖** ⇒ **標未確認,不要當成已解釋。**
  🔴 **這一發的價值不在「又紅了一次」,在於它是第一發【紅的時候我們知道機器上有幾支 vitest 在跑】。**
  在此之前那個數字只能事後用 `uptime` 猜,而前一任已自標「相關不是因果」。
  ⚠️ **一筆不是分佈**:`起跑前=2` 這個值**還不能拿來下任何結論**,它是樣本 1/N。
  **要等累積到能比較「紅的那幾發 vs 綠的那幾發」才有意義** —— 那正是這條目原本要的東西。
- 📊 **2026-08-17 夜 T 窗:把現有樣本全部量了一次。結論是【資料不足下結論】,而過程本身有三個發現。**
  問題(主視窗給的):**是「`5000ms` 門檻太低」還是「四綠不該並行跑」?**
  🔴 **兩者修法相反** —— 前者改 vitest 設定(影響所有人、**可能掩蓋真的效能回歸**),
  後者是排隊機制(不改任何測試,但要協調)。**答錯會讓真的回歸從此看不見。**

  **資料集(`docs/probes/2026-08-17-vitest-red-runs/`,25 發;數法 `ls -1 *.txt | wc -l`):**
  ```
  紅 7 / 綠 18                 （紅率 28%）
  Duration  紅 min 77.7 / 中位 138.8 / max 302.6 秒
            綠 min 37.5 / 中位  55.0 / max 109.7 秒     ⇒ 有位移，而【區間重疊】
  逾時格數  紅 1,1,1,3,7,11 …   綠 全為 0
  ```
  ⚠️ **`Duration` 部分內生**(V 窗原字)—— 每個逾時格自己就加 5 秒。
  粗調(減去 `5s × 逾時格數`)後紅方 `72.7 ~ 247.6`、中位約 `105`,**仍高於綠方中位 55**,
  🔴 **而那個減法本身是估的**:vitest 平行跑檔,逾時不會單純序列相加 ⇒ **標估算,不當量測**。

  🔴 **發現一:那個目錄名說謊,而它差點讓我漏掉對照組。**
  目錄叫 `vitest-red-runs`,而裡面 **18/25 是綠的**。
  我一開始照名字假設「沒有綠可以比」——**打開來數才發現對照組一直都在**。
  ⇒ **目錄名是個字面,而它與內容不符;`README.md` 沒有更正它。**

  🔴 **發現二:我為了回溯併發數而造的量具,是瞎的 —— 而它兩個世界都印 0。**
  想法:每發都有起始時戳與 `Duration` ⇒ 算區間重疊數,當併發的代理指標。
  ```
  區間重疊數   紅:min=0 中位=0 max=0     綠:min=0 中位=0 max=0
  ```
  **兩個世界同一個數 ⇒ 零判別力。** 成因:**這 25 發全是【同一個窗】的循序 run**,彼此天生不重疊;
  真正造成併發的是**別的窗那些從來沒被搶救的 run**。
  ⇒ **若我沒去看那個分佈,就會寫出「紅的那幾發重疊數是 0 ⇒ 併發不是原因」** —— **完全錯的結論。**

  ✅ **發現三:唯一站得住的訊號是【時間叢聚】,而它指向環境不是門檻。**
  ```
  7 發紅裡有 5 發落在 16:15–16:55 這【40 分鐘】內（16:15 / 16:25 / 16:35 / 16:44 / 16:55）
  另兩發：15:20、21:41
  ```
  **若 `5000ms` 只是對這幾支測試訂得太低,紅應該散佈在整個晚上,而不是擠在 40 分鐘裡** ——
  而且**同一支檔單獨跑會過**(21:41 那發實測 `16 passed`)。⇒ **指向環境,不指向門檻。**
  ⚠️ **這是【叢聚論證】,不是因果**:同一個 40 分鐘裡也有綠的(16:49 綠、`Duration 109.74s` = 全場最長的綠)。

  🔴 **同夜 C 窗三筆(主視窗轉來的,原字照收;⚠️ 我沒有親自跑,值屬 C 窗):**
  ```
  1  products/[id]/page.test.tsx                     14 passed / 1.78s   （單獨跑）
  2  lib/shipping/order-shipments.test.ts            16 passed / 0.435s  + 連鎖 1 格
  3  lib/orders/order-keyword-search-wiring.test.tsx 21 passed / 1.14s   + 連鎖 1 格
  三發都：紅在它沒動過的檔 / Test timed out in 5000ms / 單獨跑全過
  ```
  🔴🔴 **而 C 窗抓到一個【把整族範圍撐大】的字面問題,比那三筆數字重要:**
  > `order-shipments.test.ts` **不是 jsdom 元件測試,是純 TS 單元。**

  ⇒ **`F-2` 那個「會 spawn 子行程的那幾格 vs jsdom 元件測試」的二分【不夠】** ——
  **第三類:純 TS 單元也會逾時。** ⇒ **「只給會 spawn 的那幾格單獨門檻」(上面的選項丙)
  在這個新事實下涵蓋不到全部** ⇒ 丙的範圍要重估,不能照 `F-2` 的二分直接做。
  📌 我自己 21:41 那發紅的**也是** `order-shipments.test.ts`(純 TS 單元)⇒ **兩個窗、同一支檔、同一類。**

  🔴 **連鎖現象:從「我的推測」升級成「跨窗四次觀察」,而機制仍未確認。**
  ```
  我 21:41      逾時 1 格 → 下一格 AssertionError: expected null not to be null
  C 窗第 2 發   逾時     → 連鎖 1 格：expected null not to be null
  C 窗第 3 發   逾時     → 連鎖 1 格：expected to be called 1 times, but got 2 times
  ```
  ⇒ **形狀一致:逾時之後【下一格】以一個看起來像「守門壞了」的斷言錯誤跟著紅。**
  🔴 **兩個訊息都讀起來像真的缺陷**(回了 null / 被呼叫兩次)——
  **而成因在上一格逾時,不在被指名的那個守門。**
  ⚠️ **仍未確認的是機制**:我猜是 module mock 狀態被帶壞(症狀 B 那一族),
  **四次觀察證明了這個形狀會重複發生,沒有證明那個機制** —— **沒有人構造出那條連鎖。**
  ⇒ **對讀報告的人的實務結論(這一條現在就可用)**:
  **看到逾時之後那一格的斷言錯誤,先問「上一格是不是逾時了」,不要直接去修被指名的守門。**

  🔴 **⇒ 現在的答案:資料不足,不選邊。** 帶【可信】並行數的樣本:~~1 筆~~ **已作廢**
  (21:41 那筆量具特異性不足,見本條目上方)+ **修好之後的第 1 筆 = 22:09,`起跑前=1`,而它【綠】**。
  **C 窗那三筆【沒有並行數】** ⇒ **不能算進分佈。**
  🔴 **「四筆樣本」與「四筆【帶並行數】的樣本」是兩回事,不要混著數** ——
  這正是本條目自己在防的那種事:**數字離開量測現場就要帶著它的範圍走。**
  **我不會為了給出一個答案而拿一筆資料下結論** —— 那正是 `#608` 的 load average 假說死掉的方式。

  **要收集什麼才夠(具體,不是「再多看看」):**
  ```
  1 紅【與綠】都要帶 RUN-CONTEXT，且要來自【多個窗】
    ✅ 已自動化：儀器已在 dev 上，任何窗跑四綠都會落 RUN-CONTEXT
    🔴 ⚠️ 而 dev 上【22:0x 之前】那版的 pattern 是壞的（認字不認 runner 身分）
       ⇒ 修法進 dev 之前任何窗量到的並行數都不可信，不要收進分佈
    ⇒ 綠的那些【也要搶救】—— 只存紅的等於只留了一半的分佈（本輪就是這樣差點沒有對照組）
  2 樣本量：紅、綠【各 ≥10 筆帶並行數】才比得出分佈；且要同一台機器、同一段時期
  🔴 2b 第三類（純 TS 單元）也要進樣本 —— C 窗那一發把 F-2 的二分打破了，
     若只收「會 spawn 的」與「jsdom 的」，分佈會漏掉一整類
  3 決定性的那一發是【控制實驗】不是觀察：
    刻意維持 N 支併發 vitest（N=0/1/2/4）各跑一輪完整套件，記紅率
    ⚠️ 照 #608 的判準：不夠閒的時候不要跑 N=0 那組（紅了會誤推翻，綠了也證不了什麼）
  ```
  **三個選項與代價(等資料夠了再選,而這是決策題不是我可以拍的):**
  ```
  甲 排隊機制（四綠不並行）  不動任何測試、不掩蓋回歸；代價=協調成本、窗要等
  乙 調高全域門檻            一行改完；🔴 代價=把「真的變慢」一起藏起來 ⇒ 動驗證本身
  丙 只給【會 spawn 子行程的那幾格】單獨的門檻
                             比乙窄；#618 原文已寫「門檻該不該分開，由那個分佈決定」
                             代價=要先有那個分佈，而那正是現在缺的東西
  ```
  🔴 **我的建議是【先不要動】,而理由不是保守**:乙與丙都是**改驗證本身**,
  而本 repo 的紀律把「想動驗證」列為**停止訊號**。**甲不需要等資料就不會變壞,乙丙需要。**
  ⇒ **若最終要走乙或丙,那是 Sean / 主視窗的決策題,不是我自己改。**
- ✅ **修好量具之後的【第一筆可信樣本】(2026-08-17 22:09,T 窗)**
  ```
  logs 目錄   c245211d-20260817T220931-67702
  RUN-CONTEXT 開始 2026-08-17 22:09:31 CST / 結束 22:11:22 CST
              並行vitest行程數 開始=0 【vitest起跑前=1】 結束=0
  結果        四項 rc 全 0（typecheck / lint / build / vitest 皆綠）
  ```
  🔴🔴 **【這一筆也作廢】** —— 它是**第二版**量的,而第二版同樣自我命中(見上方三版對照)。
  `起跑前=1` 這個值**可能全部來自污染**(當晚我自己的 codex prompt 裡就有那個字面)。
  ⇒ **落筆當下,帶【可信】並行數的樣本仍然是【零筆】。** 第三版(標記檔)進 dev 之後跑的才算。
  ⚠️ 這一發的 `Duration`、四項 rc、逾時格數**仍然可用** —— **只有並行數那三個值作廢。**
  ⚠️ **它證不了任何事**,它只是**分佈的第 1 筆**。四格都要有樣本才比得出來
  (並行低/高 × 紅/綠),而現在只有「低 × 綠」那一格有 1 筆。
  🔴 **不要拿它支持任何假說** —— 尤其不要拿它說「並行低所以綠」:**單格一筆無法排除巧合。**
- ✅ **`build` 並行數也進 `RUN-CONTEXT` 了(2026-08-17 22:4x)**
  🔴 **這一欄新增的理由必須先講清楚,免得被誤讀(主視窗指定寫進條目)**:
  > **它不是在追那個負載假說。** 落筆當下四格(並行低/高 × 紅/綠)**只有「低×綠」有樣本**,
  > **任何為了支持假說而加的量測都會變成「找資料救假說」。**
  > ⇒ **正當理由只有一個:把儀器補完整。** `RUN-CONTEXT` 原本只看得到 vitest,
  > 而 **`build` 與 vitest 搶的是同一批 CPU**,它是 C 窗指的兩個候選變數之一。
  > **加它是為了涵蓋另一個【已知的資源競爭者】,不是因為已有證據指向 build。**
  **做法與 vitest 那半同款、不打折**:同一支標記檔機制(**刻意不複製第二份** ——
  這支檔的歷史上已經發生過兩次「函式算一份、變數存一份」),兩個子目錄分開。
  三道邊界全上:`trap` 刪自己 / 驗 PID 還活著 / **比對行程啟動時間**(擋 PID 回收)。
  **欄位名把語意寫死**:`其他窗四綠build並行數(不含自己)`。
  **守門 7 格 + 突變 3 發**:
  ```
  格26-a 零個 build ⇒ 0（地板）      格26-b 兩發獨立 build ⇒ 2（單位）
  格26-c 🔴 build 的標記不得被算進 vitest 那一欄（兩種互不干擾）
  格26-d build 也擋 PID 回收          格26-j/k 釘呼叫點  格26-l 那兩個值真的餵進 run_context
  突變 M30 拿掉呼叫點 / M31 恆 0 / M32 兩種共用同一個目錄 ⇒ 各紅對應那幾格
  ```
  🔴 **「瞬時 CPU」仍然不加**,理由不變:**它要取樣不是取點,成本跳一級**,
  而現在連 run 數都還沒有第二格樣本。
  **實測(22:41 那發,四項全綠)**:
  ```
  ...
  跑完兩種標記目錄殘留各 0 支
  ```
- ✅ **2026-08-17 22:5x:改成【被動收集】,並附判停條件(主視窗裁定「不要刻意製造負載」)**
  🔴 **不製造的理由值得原字留著,因為它決定了工具的形狀:**
  > **你造的負載形狀是你選的**(N 發並行你的靶),
  > **而真實的負載形狀是八個窗在做不同的事**(build / vitest / codex / playwright 混著)。
  > **若紅與「形狀」有關而不只與「數量」有關,造出來的那一格會是一個【不存在的世界】。**
  📎 同族 memory `feedback_green-in-a-world-that-cannot-happen` 的**鏡像**:
  那條講「綠在一個不可能發生的世界」,這裡是**紅在一個不會發生的世界**。
  ⇒ **等樣本的成本是 0**(八個窗持續在跑四綠、儀器已經準了);**造樣本的成本是可能得到一個不存在世界的答案。**

  **工具(唯讀,不跑任何東西)**:`scripts/four-greens-context-table.sh` —— **17 格 selftest**。
  把 `logs/four-greens/*/RUN-CONTEXT` 撈出來對上 rc,印「並行數 × 紅綠」四格表。
  ```
  sh scripts/four-greens-context-table.sh              # 掃本樹
  sh scripts/four-greens-context-table.sh --selftest
  ```
  🔴 **它刻意不下結論**,而且**自己把判停條件印在結果旁邊**(寫在檔頭沒人讀)。
  **配套改動**:`RUN-CONTEXT` 多一行 `rc typecheck=N lint=N build=N vitest=N` ——
  🔴 **不是多餘的**:沒有它,任何要做這個統計的人都得去 parse `run-rc.sh` 印的那句中文
  (`rc=0 ✅（…）`)⇒ **統計工具綁死在那句措辭上,措辭一改、統計靜默失準。**
  守門=格23-e(四個 rc 原字進檔且**位置不得對調**,餵 0/1/2/3 分得出來)。

  **🔴 判停條件(主視窗指定 —— 兩個出口都是結論)**
  ```
  四格都有樣本                ⇒ 可以開始談結論
  跑了幾小時仍然只有「低×綠」  ⇒ 🔴 那本身是一個發現
                               （紅根本不常發生，而先前那三發紅可能是別的原因）
  🔴 不要讓「等不到」變成「去製造」。
  ```
  ...
  **落筆當下實跑一次的結果(唯讀,不是預測):**
  ...
  帶 RUN-CONTEXT 的發次 12
    ...
    不進四格（四種原因分開數、不補 0、不猜）：
      欄位名未知 7（前兩版量具的舊欄位名 ⇒ 那些數字已作廢）
      僅vitest且rc未知 3   僅vitest 0   rc未知 1
  ```
  ⚠️ **「補 0 會讓『沒量到』長得像『量到 0』」** —— 所以那四種分開具名,不合併成一個標籤。
  🔴 **而我第一版就把其中兩種印成同一個「格式未知」** ⇒ **同一個標籤兩種意思**,
  **那是本輪一直在修的病發生在這支新工具自己身上**;格5b/5c 兩格是為它加的。
...
- ✅ **同時做了一件不需要結論的**(V 窗指的那條):紅的時候**並行數印在失敗清單【前面】**,
  ...
  突變 M20/M21/M22 各紅對應那一格。
  🔴 那三行**刻意不下結論**(不寫「所以是負載造成的」)—— **一筆不是分佈。**
...
> **改字面的理由**:F-2 的價值在「**spawn 族只蓋 9/25**」,不在「其餘 16 個都是 jsdom」——
...
② F-3/F-4 給了一個【可證偽】的統一假說:主事件 = 飢餓逾時,B/C 是下游污染
   證偽方式 = 找到一發「B 出現而同檔沒有前置 timeout」⇒ 統一假說倒
   現有資料 0 次,而【樣本數就是那個未確認的分母】,不要當成已證實
...
- **F-4 獨立重現【兩次】** —— 第 2、3 發的連鎖格:
  ```
  第 2 發  expected null not to be null
           🔴 而那一格的名字是「正向對照:剛好等於上限（沒有第 N+1 筆）⇒ 正常回,不誤擋」
  第 3 發  expected "vi.fn()" to be called 1 times, but got 2 times
  ```
  🔴 **兩個都讀起來像「守門壞了 / 呼叫次數不對」,而成因在【上一格逾時、狀態沒被重設】。**
...
```
· 沒有跑第四次（主視窗規矩:兩次夠判）
· 【沒有量機器負載】⇒「八窗同跑」是【推的】,依據是 Duration 量級差與紅在三個不同檔
  —— 不是量到的
· 三發的完整輸出在 run-rc.sh 自己的 mktemp（/var/folders/…,會被系統清）,
  repo 內 logs/four-greens/ 只有尾 60 行 ⇒ 主視窗指示留著不清
· #601 因此未 commit，等主視窗裁
```
...
⇒ 目前【零筆】帶可信並行數的樣本。這兩個數【不可換算】,只能重量。
```
...
三發紅 + 一發靜默下綠那組資料仍然成立。
⚠️ **而它的「12」現在作廢** ⇒ **這個反例剩下的可用部分只有 `Duration`**:
...
> **而我把「儀器量的」讀成了「可信的」。**
⇒ **判別句:「這個數字是量出來的」只回答了【誰產生它】,沒有回答【它有沒有判別力】。**
...
```
A 紅 305ms → 格 B 量到恰好 ['stray-1-from-A']（腿打進下一格,mockReset 擋不住
                                              —— reset 在格起點,腿在格中落地）
          → 格 C 自打 1 發、斷言 1 發 ⇒ 紅,錯誤原字
            expected "vi.fn()" to be called 1 times, but got 2 times  ← 與四次觀察逐字同形
對照組:A await 自己的腿 ⇒ A 綠 1403ms、B 綠量到 0、C 綠恰 1
```
...
② 寫測試規則:起了 async 腿的格【必須 await 它】（對照組證明 await 乾淨）
③ 🔴 mockReset 移到 afterEach【救不了】（腿落在 reset 之後）,別往那裡修
...
```
a/b（逾時打壞 mock registry ／ vitest teardown 重設）⇒ 排除
    治療組格 A 逾時 304ms 後,三個觀測點【全部仍拿到 mock】:
      ① 下一格自己呼叫 ⇒ 'mocked'
      ② importActual 投影常數仍 500
      ③ 🔴 連格 A 那條逾時後才落地的 in-flight 腿,拿到的也是 mock（不是真貨）
    對照組全綠、格 A 909ms（≥900 = 真跑到時點,對照有效性驗過）
c（setup 沒跑完被誤讀成 mock 失效）⇒ 排除,靠【簽名不同】
    beforeEach 逾時的失敗訊息是「Hook timed out in 300ms」、只掛那一格、後續格 mock 完好
    ⇒ 它產不出「requireEnv not set + 真 client 堆疊」那個形狀
    ⇒ 兩者在錯誤訊息上【分得開】,「長得一樣」實測不成立
```
...
```
三個 "timed out" 逐條【往上找最近的 FAIL 檔名】（開檔讀,不用猜）:
  行 116 ⇒ order-keyword-search-wiring.test.tsx
  行 141 ⇒ order-shipments.test.ts
  行 154 ⇒ order-shipments.test.ts
帳:16（order-shipments 全紅）+ 2（wiring,該檔 21 tests | 2 failed）= 18
   ⇒ 對上該輪總 Tests 行「18 failed」 ✅
```
...
22:09 起跑前=1   🔴 第二版      ⇒ 也作廢 —— 它【上一顆 commit 才把它寫成「第一筆可信的」】
可信樣本 = 第三版跑的三發（22:21 / 22:30 / 22:34,全部 0/0/0、全綠）
```
⚠️ **一筆不是分佈**:四格(並行低/高 × 紅/綠)**只有「低×綠」有樣本**
...
race + 冷快取（每 run 前 rm -rf node_modules/.vite/vitest）⇒ 🔴 2/3 run 命中
     且【只在 r1 = 真首解析】:被測模組拿到真 dep ⇒ THREW requireEnv(...) not set
     ← 症狀 B 的【原字形狀】
     同一發【直接 import 那條仍拿到 mock】= 現場「mock 有註冊、走 under-test 的格才打到真貨」
race + 暖快取   ⇒ 0/3
serial + 冷快取 ⇒ 0/3
serial + 暖快取 ⇒ 0/1（50 發）
```
...
① 測試檔避免「async factory + importActual 拿同一模組」
   ⇒ 改用 factory 的 importOriginal 參數（官方 API,可能正是繞開這個競態的那條）
② 冷快取 + 高並行的班表（全陣同時開跑）= 症狀高發窗
   ⇒ 【collect 時段錯開】比【改門檻】有效
```
...
```
· 為什麼【單獨跑】永遠是綠的      ⇒ 單獨跑那一刻快取已暖,不是「因為沒有別人在跑」
· 為什麼【靜默那一發】綠了        ⇒ 那是當晚第 4 發,快取早暖了
```
⇒ **這兩件不再是負載的證據。**
...
現場那檔的併發源（觸發路徑）      ⇒ 🔴 未證（serial + 冷 全乾淨,而現場格子是逐條 await）
```
⇒ 正確口徑:「**找到一個足以產生這兩種症狀的機制,而還沒證明現場走的就是這條路。**」
...
```
git show --stat 31fa9b7e ⇒ STATUS.md + docs/patterns/guard-and-instrument-traps.md   非 .md 檔數 0
git show --stat 9946d24e ⇒ STATUS.md                                                  非 .md 檔數 0
（數法 `git show --name-only --format='' <c> | grep -v '^$' | grep -vc '\\.md$'` ⇒ 皆 0）
```
⇒ **「merge 改了模組 ⇒ 該鏈快取失效 ⇒ 冷」不成立。**
⇒ **真實 run 的冷/暖由什麼決定 = 未解。**
⚠️ 而 `chain4` 實測 **disk 冷/暖在跨檔構造下都 0 命中** ⇒ **disk 快取甚至不是跨檔情境的閘。**
...
2 跨檔 2 worker（P mock 檔 + Q 無 mock 靜態 import 同鏈）⇒ 暖 0/10、冷 0/10
3 跨檔 7 檔（P + 6 個併發產生器）× 冷        ⇒ 0/10
4 讀 config 排除:root vitest.config 無 setupFiles/globalSetup ⇒ 候選 b 死;
                 現場檔無 top-level import 被 mock 模組        ⇒ 候選 c 死
```
...
> (`merge --stat` 檔類、`node_modules/.vite/vitest` mtime、worker 數)。
...
其他窗四綠 build  並行數(不含自己)  開始=0  vitest 起跑前=0
...
2. 🔴 **負載假說再失一隻腳** —— 第八節第 2 層說它失去了兩個支持;
   **本節是【正面反例】:並行數為 0 的世界裡,紅照樣發生。**
   ⇒ **口徑**:負載**不是必要條件**。(⚠️ 這**不排除**它是**加速因子** —— 那需要「高×紅」那一格,仍缺。)
3. **`F-4`(逾時 → 下一格假缺陷)在野外被再次觀察到**,而且**這一次它與構造的結果逐字對得上**。
   ⚠️ **而 `F-4` 的因果地位仍照第八節第 1 層**:它是**下游症狀**,不是原因。
4. ⚠️ **這一發的失敗檔 `order-keyword-search-wiring.test.tsx` 【不在】候選 4 的形狀名單裡**
...
唯一夠格的證據 = 用同一套 2×2 harness:
    改寫前 race + 冷 ⇒ 2/3      改寫後必須 ⇒ 0/3
```
...
> ```
> docs/probes/2026-08-17-vitest-red-runs/6e292a68-20260817T223236-59394-RED.txt              124 行
> docs/probes/2026-08-17-vitest-red-runs/3f06df74-20260817T225716-45587-RED-F4-inthewild.txt 134 行
...
2. **三發是三個不同的形狀**(瀏覽器/伺服器、`F-4` 逾時鏈、單純斷言)
   ⇒ 🔴 **不要把它們算成「同一個 bug 的三次」** —— 只有第 2 發帶 `F-4` 形狀。
...
3 把 log 保存進 repo（.txt,不是 .log —— .gitignore:49 有全域 *.log）
4 然後才重跑
```
...
- **狀態:** 📋 **登記給 Sean,不動。** 🔴 那兩個數字(`≤ 30 非空行` 且 `≤ 15,000 字元`)是他拍的判準
  (`~/.claude/rules/00-work-rules.md` §4 精簡觸發值那一族)⇒ **改它要問他,我不自己動。**
- 🔴 **本條【只報已量到的成本,不提修法】** —— 修法方向會影響他怎麼判,提了等於替他拍板。
- **已量到的成本(2026-08-17 一天,收割窗連續四次):**
  ```
  落 119🏁 之前 → 先把 117 與 116 逐字搬進附屬區（一次搬兩條，因為搬一條還是超標）
  落 120🏁 之前 → 先把 119 搬下去
  落 121🏁 之前 → 先把 120 搬下去
  落 122🏁 之前 → 先把 121 搬下去
  ⇒ 四次落檔 = 四次「落檔 + 搬檔」，而搬檔本身要複驗零刪除（每次 grep -c 對三條）
  ```
  🔴 **不是偶發** —— 它跟「單日收了幾批」成正比,而 2026-08-17 這天收了 12 批。
  **落 122 之後主表剩餘餘裕:29 行 / 30、14,997 字元 / 15,000 ⇒ 只剩 3 個字元。**
  (量法在 STATUS 附屬區「🧭 主表兩把尺」那節;⚠️ 那節自己警告過**字元 vs byte 不是同一個東西**。)
- **為什麼今天沒出事:** 每一次都真的搬了,而且都複驗過零刪除
  ⇒ **這條記的是【流程摩擦】不是【已發生的錯誤】**,不要讀成「STATUS 壞了」。
- **不修未來會痛在哪:** 那個摩擦落在**每一次落檔的當下**,而落檔是收割窗唯一的產品
  ⇒ 摩擦越高,越可能出現「這次先不落、等一下一起寫」——
  **而「等一下一起寫」就是今天 118🏁 補記在治的那個病**(收了沒落檔,在 repo 上看起來和沒收過一樣)。
...
- **狀態:** ✅ **已實作**(2026-08-17 夜 T 窗,分支 `four-greens-drift`)。R1 那五條**逐條配了突變、每一發都指名哪一格會紅**
  —— 對照表在下面「本版怎麼收的」,數法 `sh scripts/dev-four-greens.sh --selftest` ⇒ `PASS=39 FAIL=0`(舊版 22 格)。
  🔴 **這一行本身犯過 V 窗 R3-1 點名的病**:格數改了而它沒跟著改(29/30 卡了一輪)——
  **表會被抄走,前後文不會** ⇒ 現在每個數字旁邊都貼著它的數法。
  之後跑了**兩輪** codex 對抗審查:R1 FAIL(7 條)、折完 R2 FAIL(2 條折得不完整 + 3 個新洞),
  兩輪折完共補 5 發突變 ⇒ **12 發(M0–M11)**。🔴 **R3 沒跑** —— 規矩是第 3 輪起要換角度換模型,
  而本窗兩輪用的是同一支 codex(`command -v codex` ⇒ `/opt/homebrew/bin/codex`,R1/R2 皆 `gpt-5.6-sol`,
  值取自 `/tmp/cx620r2.log` 的 session 標頭;**是否有別的模型可用未查**);R2 剩下的兩條(hex 常數過得了格14-0、`sed` 範圍假設)已改成
  **收窄標籤 + 寫明前提**,不是靠再審一輪關掉的。要不要開 R3 是主視窗裁量。
  下面「為什麼撤回」那五條**保留原文不刪**——它們是這一版每一格存在的理由,刪了就沒人知道格子在守什麼。
  ⚠️ **本版仍有一格沒守住,寫在最後一項「本版的射程限定」,不要讀成整條關掉。**
...
- 🔴 **三次同一支檔 ⇒ 那不是作者的問題,是【那支檔的形狀】**(主視窗抽,原字採用):
  ```
  這支工具的守門一直守不住，而三次的成因都是
  「格子驗的是中間值 / 常數換掉照樣綠」

  🔴 下一次動它之前，先問：
     這支工具有沒有一個【純函式核心】可以被獨立餵值？
     沒有 ⇒ 先把那個核心抽出來，再寫格子。
  ```
  📎 **依據是一個已經發生過的好處,不是一個道理**:2026-08-17 T 窗把定位器重構成**純函式**,
  **而那讓 V 窗可以把它移植出來實跑** ⇒ **可測性與可審性同源**。
- ✅ **本版怎麼收的(2026-08-17 夜,分支 `four-greens-drift`)**:照上面「修法方向」把判定抽成吃**字串**的
  `drifted_between()` 與 `drift_report()`,格子餵構造好的指紋去打它;`tree_fingerprint()` 的第三欄從
  **行數**換成兩個 `cksum`(未提交檔案清單 + `git diff HEAD` 內容),三欄各自帶 `nohead` / `GITFAIL-*` 失敗字面。
  selftest **22 → 39 格**(數法 `sh scripts/dev-four-greens.sh --selftest` ⇒ 合計那一行 `PASS=39 FAIL=0`)。

  **突變對照表 —— 我跑的 20 發(數法:數下表 `M` 開頭的列,`M0`–`M19`)。**
  預期紅的格是【動手前】寫下的。🔴 **操作符原文一起收進來** ——
  V 窗 R3-2b 逐字:「**活下來的是對照表,不是操作符**」⇒ 沒有它,「其餘各發沒有 M9 式互污」
  這句話**只能從紅格分佈推,不能重放驗證**。`$B` = 未突變的基準副本,`$T` = 被測檔。

  | 突變 | 操作符(可重放) | 對應 finding | 預期紅 | 實得 |
  |---|---|---|---|---|
  | M0 不突變(正向對照) | — | — | 全綠 | ✅ `PASS=39 FAIL=0` |
  | M1 `drifted_between` 恆 `false` | `override 'drifted_between() { false; }'` | R1 F3 | 格18 | 格18 + 格18-b |
  | M2 `drifted_between` 恆 `true` | `override 'drifted_between() { true; }'` | R1 F3 | 格17 | 格17 |
  | M3 指紋換成常數 | `override 'tree_fingerprint() { echo CONSTANT; }'` | R1 F1 | 格15 | 格14-0 + 格14-2 + 格15 |
  | M4 指紋退回**行數**版 | `override 'tree_fingerprint() { echo "$(git rev-parse HEAD …) dirty=$(git status --porcelain … \| wc -l …)"; }'` | R1 F4 | 格15 | 格14-2 + 格15 |
  | M5 指紋塌成 `nohead` | `override 'tree_fingerprint() { echo "nohead 0 0"; }'` | R1 F5 | 格14-0 | 格14-0 + 格14-2 + 格15 |
  | M6 刪掉呼叫行 | `grep -v '^  drift_report "$FP_START"' "$B" > "$T"` | R1 F3 剩下那一跳 | 格19 | 格19 |
  | M7 指紋換成**非 `nohead`** 的常數 | `override 'tree_fingerprint() { echo "CONSTANT 1 1"; }'` | codex F1 | 格14-0 | 格14-0 + 格14-2 + 格15 |
  | M8 指紋退回**管線版** | `override 'tree_fingerprint() { echo "… \| cksum \| tr -d \" \" …"; }'` | codex F2 | 格14-2 | 格14-2 |
  | M9 刪掉螢幕上的 `echo` | `awk '/^drift_report\(\) \{/{inside=1} inside&&/^  echo/{next} /^\}/{inside=0} {print}' "$B" > "$T"` | codex F4 | 格18-b | 格18-b |
  | M10 呼叫行搬到函式之外 | `awk '/^  drift_report "$FP_START"/{call=$0;next} /^summary_table\(\) \{/{print;print call;next} {print}' "$B" > "$T"` | codex F3 | 格19 | 格19 |
  | M11 拿掉 `-uall` | `sed 's/--porcelain -uall 2>…/--porcelain 2>…/' "$B" > "$T"` | codex R2 | 格15-b | 格15-b |
  | M12 計數器恆 0 | `override 'count_matching() { echo 0; }'` | `#618` 儀器 | 格20 | 格20 |
  | M13 計數器恆 1 | `override 'count_matching() { echo 1; }'` | `#618` 儀器 | 格21 | 格21 |
  | M14 不傳 `RUNRC_FULL_LOG` | `sed 's/RUNRC_FULL_LOG=/IGNORED_VAR=/' "$B" > "$T"` | `#618` 全量 log | 格22 | 格22 |
  | M15 `run_context` 只寫時戳 | `override 'run_context() { mkdir -p "$4"; printf "開始 %s\\n" "$1" > "$4/RUN-CONTEXT"; }'` | `#618` 儀器 | 格23 | 格23 |
  | M16 `drift_report` 不回 3 | `grep -v '^  return 3   ' "$B" > "$T"` | V 窗 R3-3 | 格18-c | 格18-c |
  | M17 `four_greens` 不接那個 rc | `grep -v '^    return 3$' "$B" > "$T"` | V 窗 R3-3 | 格19-b | 格19-b |
  | M18 弄壞 `git status` 呼叫 | `sed 's/--porcelain -uall 2>…/--porcelain -uall --bogus-flag 2>…/' "$B" > "$T"` | V 窗 V1 等價 | 格14-1 | 格14-1 + 格15-b |
  | M19 拿掉 `run_context` 的絕對路徑閘 | `awk '/^  case "$rx_dir" in$/{skip=4} skip>0{skip--;next} {print}' "$B" > "$T"` | 今晚實際生出 `7/` 那個病 | 格23-b + 格23-c | 格23-b + 格23-c |

  **V 窗(Fable)R3 自己跑的兩發 —— 🔴 我沒有重跑,原字照收,來源不合併進上表:**

  | 突變 | 目的 | V 窗回報的結果 |
  |---|---|---|
  | V1 弄壞 `status` 呼叫 | 格14-1 零突變 | 恰紅 14-1(+15-b 連動,真壞該紅) |
  | V2 指紋摻 `/dev/urandom` | 格16 零突變 | 恰紅 16、其餘 29 綠 |

  🔴 **V1/V2 存在的理由比結果重要**:R3-2 指出我原本 12 發的實得欄**只覆蓋 8/10 個新格**,
  **格14-1 與格16 是零突變 = 判別力未量**,而 commit 標題卻寫「每一格都擋得住」——
  **宣稱大於量法**。我補了 M18(V1 等價、我自己跑的);**格16 那一格我採信 V 窗的 V2,沒有重跑**。

  🔴 **harness 自己踩了【六個】坑,全留著 —— 這一區比上面那張表值錢**
  (數法:數下面 `①`–`⑥` 六個編號)。**共同形狀:突變跑起來了、報告印出來了,而它量的不是我以為的那件事。**
  ① **M9 第一版用 `grep -v '<字面>'` 刪整份檔案** ⇒ **連格18-b 自己的 pattern 那一行也一起刪掉**
  ⇒ 那格是紅了,而**紅的理由被污染**(突變同時改到了測試本身)。
  ② **報告對 M0 無條件印「🔴🔴 零紅 = 沒有判別力」**,而 M0 是**正向對照、零紅正是預期**
  ⇒ **量具自己在誤報**(`CLAUDE.md` 終端機紀律第三條的原形:輸出的標籤要由結果決定,不能無條件印)。
  ③ 🔴 **selftest 在 repo 根目錄生了一個 `7/` 目錄,而它印的是全綠。**
  成因:我改 `run_context()` 的參數個數時,呼叫端與函式體有一瞬間對不上(4 個 vs 5 個)
  ⇒ 目錄參數收到的是 `"7"`(某個並行行程數)。**shell 沒有參數個數檢查,參數錯位不會有任何東西叫。**
  ⇒ 修法不是「小心一點」,是**絕對路徑閘**(`run_context` 收到相對路徑就 `rc=2`)+ 格23-b/格23-c + 突變 M19。
  ④ 🔴 **`M15` 的操作符過期了,而它看起來還在動**:它照【舊的 4 參數簽章】寫,
  函式改 5 參數之後它拿 `$4` 當目錄 ⇒ **突變自己在 repo 裡生目錄**。
  ⇒ 與 ① 同族:**突變操作符也會過期,而過期的操作符照樣「跑得動」。**
  ⑤ 🔴 **`M19` 第一版的 `awk` 只刪 3 行、把 `esac` 留下來** ⇒ 語法錯 ⇒ 整支跑不起來
  ⇒ 報告印出**「零紅 = 那道斷言沒有判別力」**,而真相是**腳本根本沒跑**。
  🔴 **抓到它的不是報告,是它同時留下的兩個垃圾目錄** —— 報告那一欄在這個世界裡是說謊的。
  ⑥ 🔴 **格23-b 的靶會活到下一發**:它真的生出目錄,而我沒讓那一格收乾淨
  ⇒ **M16/M17/M18 三發都被它污染、各多紅一格**。修法=那一格檢查完自己 `rm -rf`
  (= memory `feedback_mutation-harness-needs-both-pre-and-post-checks` 的最小版本)。

  R1 F2(負向對照測的是字串運算不是那支函式)沒有單獨一發 —— 它**不是一個可以突變的行為**,
  是舊格15 那行 `[ "$fp1" != "${fp1%dirty=*}dirty=999" ]` 本身;本版把那一格整格換掉(改成餵真 repo 的兩個世界),
  **舊寫法已不存在**,數法 `grep -c 'dirty=999' scripts/dev-four-greens.sh` ⇒ `0`。
- ⚠️ **本版的射程限定(數法:數下面 `①`–`⑤` 五個編號,共 **5** 條;其中 ④ 已被 V 窗 R3 打掉、已改)**:
  ① **格19/格19-b 只證「那兩行還在 `four_greens()` 函式體裡」,不證它執行過** ——
  🔴 **不過這一條在 2026-08-17 19:10 那一發被【觀察到】了一次**:真跑一次四綠,
  畫面印出 `RUN-CONTEXT` 那一段、`DRIFT` 沒有誤亮(樹沒動),四項 rc 皆 0
  ⇒ **那一發是執行過的證據,而它是一次觀察不是一道守門** —— 下一次改壞了,仍然沒有格子會紅。
  ② `tree_fingerprint` 的內容欄走 `git diff HEAD`,**看不到未追蹤檔的內容** ⇒ 只動未追蹤檔內容
  而檔名清單不變的那個世界,DRIFT 仍不會亮。
  ③ **ABA**:期間被改動、收尾前又改回原狀 ⇒ 頭尾指紋相同 ⇒ 不亮(codex R1 F7 抓的)。
  頭尾兩點取樣天生看不到中間,要看得到得改成持續監看,不在本片範圍。
  ④ ~~**DRIFT 亮了整支的 `rc` 仍是 0**~~ **已作廢 —— V 窗 R3-3 打掉,本版改掉了。**
  🔴 它抓到的不是「不夠好」,是**這個功能當時買不到任何東西**:四個載體
  (`I-008 §⑤` 收割判準 A/B/C、`T-004 §②`、兩本 runbook)逐一 `grep DRIFT` **零命中**,
  全是 `#620` 自述 ⇒ **真漂移那天:警告印在滾走的輸出裡、檔落在沒人查的目錄裡、收割照收。**
  ⇒ 本版給 DRIFT 一個**和「有項目紅」分得開的** exit code:**`rc=3`**(`rc=1` 才是四項有紅),
  `sh scripts/dev-four-greens.sh && git merge …` 這種寫法會當場停住。守門=格17-c/格18-c/格19-b,
  突變 M16/M17 各紅一格。
  🔴 **而機制只做了一半**:另一半是**收割判準要加一列** `test ! -e <該發 logdir>/DRIFT` ——
  那份判準在 `I-008` 那封信裡、不在本 repo,**由收割窗自己加**(主視窗已同步通知)。
  **兩邊要對上;只有我這邊改完,不等於那條路被守住。**
  ⑤ 🔴 **`cksum` 兩欄改用 `tr ' ' '-'` 之後的碰撞,【沒有格子守】**(V 窗 R3-4 的修法本身)——
  要構造一組真的碰撞需要湊出兩份 `git status` 輸出讓 `cksum` 的兩個欄位錯位相等,
  **成本遠高於這條風險**,標為誠實缺口。⚠️ 反過來說:把它改回 `tr -d ' '` **不會有任何一格紅**。
...
- **狀態:** ⏳ 待執行。🔴 **這條記的是【顯示與能力對不上】,不是「作廢功能未完成」** ——
  功能沒做完是排程問題(它本來就排在 2a-2);**畫面已經先把結果畫出來了**才是這一條。
- **現況(2026-08-17 A 窗當場量,每條附數法):**
  ```
  顯示層【已經在畫】
    item-procurement-section.tsx:169   {voided && <span>(已作廢)</span>}
    item-procurement-section.tsx:96-104 VoidReasonCell —— 三態:有原因 / 太長收合 / (沒有帶回來)
  前端入口【零命中】
    grep -rlnE 'voidProcurement|procurementVoid|void_procurement|作廢這筆採購' components lib | wc -l ⇒ 0
    🔴 量具對照(同一把 grep 查已知存在的出貨作廢入口):
    grep -rlE 'voidShipment|shipment-void' components lib | wc -l ⇒ 11
    ⇒ 上面那個 0 不是「這把 grep 什麼都找不到」
  欄位【已經在資料庫裡】
    grep -c '20260813120000' supabase/APPLIED.tsv ⇒ 1（對照:20260803160000 ⇒ 1）
  ```
- **建表 migration 自己逐字寫過這件事**
  (`20260813120000_m4b_e10_452_procurement_void_schema.sql:19`):
  > 「其中 (a) 的那兩個新欄位:**目前沒有任何一支程式會去填它們** —— 撤銷的按鈕與後端在下一片(2a-2)才做。」
  同段下一行也寫著唯一的例外:**有人拿最高權限直接下 SQL**(Supabase 後台的 SQL 編輯器)。
- 🔴 **所以它不是「等一個永遠不會來的資料」,精確版是:**
  **那個狀態【只有工程師手動改資料庫】才生得出來,而畫面對員工完全沒有說這件事。**
  ⇒ 員工看到「(已作廢)」時**沒有任何入口可以做同一件事、也沒有入口可以還原**;
  他唯一能做的是找工程師 —— 而畫面上沒有一個字告訴他這件事。
- **不修未來會痛在哪:** 這一段顯示**不會紅、不會被任何守門抓到**(型別對、測試對、三綠全綠) ——
  它只會在**某個工程師手動撤過一筆之後**第一次現身,而那一刻員工看到的是一個
  「系統顯然支援作廢」的畫面,然後**找不到那顆鈕**。
  🔴 更貴的一半:他會**因此推論「作廢這件事系統管得住」**,進而放心去下第二筆採購 ——
  而在 2a-2 之前,撤銷的正確性(數量回沖、唯一索引)全部靠那次手動 SQL 有沒有做對。
- **兩種收尾都可以,擇一(不是二選一的技術題,是排程題):**
  1. **做完 2a-2** —— 撤銷按鈕 + 後端上線,顯示層就對得上能力了(顯示層一行都不用改)。
  2. **把顯示先拿掉**,直到 2a-2 —— 拿掉 `:169` 那顆徽章與 `:96-104` 那一列,
     或改成一句對員工說得通的話(例:「這筆已由工程師撤銷,如需還原請聯絡工程師」)。
     ⚠️ 選 2 的代價:**手動撤過的那些列會變成靜默消失**(數量已經不算它了,而畫面不解釋為什麼)
     —— `#476` 修的正是「兩個數字互相矛盾而沒有一行字解釋」,拿掉顯示等於把那條 bug 換個位置放回來。
  🔴 **我的建議是 1**,而這是排程判斷、不是我可以拍的板。
- 🔴 **收尾動作(兩種收尾【都】要做,不是擇一)——「—」旁邊要有一句話說為什麼**
  (2026-08-17 收進本條;原本掛在片 2「改什麼」第 6 條,而**片 2 已被 Sean 拍掉整片作廢** ——
  主視窗裁定不另立案、掛回本條,理由:同一個根、同一段畫面,另開會長出第二個真相。)
  ```
  現況  item-procurement-section.tsx:198  作廢列的到貨登錄整格 = 「—」
        同列左邊有「(已作廢)」徽章、下一列有作廢原因
        ⇒ 推得出來，而【沒有一個字在講「所以這裡不能登錄到貨」】
  動作  「—」旁邊（或用 title）補一句，例:「這筆採購已作廢，不能再登錄到貨」
  ```
  🔴 **為什麼現在就要記,而不是等它痛**:
  > **撤銷採購上線之前它極少見**(「已作廢」目前只有工程師手動下 SQL 才生得出來,見本條上方)——
  > **而本條做完之後它會變成常見狀態。**
  ⇒ **它今天不會痛,所以今天不會有人修它。** 這句不要在整理時被當成贅字刪掉。
  ⚠️ 選收尾 2(把顯示拿掉)的話,這一條**跟著那個顯示一起消失** ——
  但**選 1 的話它是必要的**,因為那時候員工**真的會看到**作廢列。
...
### #625 · 主樹未追蹤檔可被任一窗的 `git stash -u` 靜默移走並重建,而事後查不出來(reflog 對 stash+pop 是瞎的)

- **狀態:** ⏳ 待執行。🔴 **本條【不指控任何人、也不宣稱它發生過】** ——
  它回答的是主視窗指定的那個問題:**「有沒有一支腳本【能】寫出這種東西」**,不是「是誰寫的」。
  前者我構造出來了,後者**以現有量具永遠答不出來**(見下方「那把尺是瞎的」)。
- **觀察到的現象(唯讀,`ls -la`,沒有動任何一個檔):**
  ```
  /Users/sean_1/pcm-website-v2 的三個未追蹤檔（I-008 §⑨-1 明令不清不 ignore）
    0                    0 bytes       mtime Aug 17 19:14
    information_schema   0 bytes       mtime Aug 17 19:14
    admin-orders.png     171,283 bytes mtime Aug 17 19:14
  ```
  🔴 **三個檔共用同一個時點,而其中一個是 171KB 的截圖。**
- 🔴 **先打掉一個聽起來很合理的假說(主視窗提的,我實測推翻):**
  假說=「檔名 `0` 來自 shell 重導向打錯(`2>0`、`[ "$n" > 0 ]` 那類)」。
  **不成立,而理由不需要掃任何一支腳本**:
  ① **重導向一次只生一個檔**,解釋不了三個檔共用時點;
  ② 若 `admin-orders.png` 曾被重導向寫過,它會**被截成 0 bytes**,而它是完好的 171KB。
  ⇒ **形狀不是「有人寫了一個檔」,是「有東西把這三個檔【整批】重建了一次」。**
  (掃描仍留痕:`grep -rE '\[[^]]*[^-<>=!]>[[:space:]]*[0-9]'` 掃 119 支 `.sh` ⇒ 13 檔命中,
   **逐一開檔看,全部是 SQL 字串裡的 `> 0`,零個是 shell 重導向** ⇒ 這把尺對本題零命中且**字集偏寬**。)
- ✅ **構造得出來(三個世界,自造拋棄式 repo,不是主樹):**

  | 世界 | 未追蹤檔的 mtime | 檔還在不在 | 內容 |
  |---|---|---|---|
  | `status` / `checkout` / `merge`(負向對照) | **不變** | 在 | 不變 |
  | `git stash -u`(不 pop) | — | 🔴 **不見了** | — |
  | `git stash -u` + `git stash pop` | 🔴 **重設為當下** | 在 | **一個位元都沒變** |

  ⇒ **第三列與觀察到的現象完全同形**:三個檔都在、內容沒變(0 bytes 的還是 0 bytes、
  PNG 還是 171,283 bytes)、共用一個新的 mtime。
  ⚠️ **同形 ≠ 就是它**。其他能造出同一形狀的還有 `touch`、備份還原腳本、`cp` 來回。
  **我沒有排除它們,也沒有排除的辦法。**
- 🔴🔴 **本條真正的價值在這裡:那把尺是瞎的**
  我原本要寫「主樹 `git stash list` 空、`.git/logs/refs/stash` 不存在 ⇒ 沒有人 stash 過」。
  **落筆前先去驗那把尺**(拋棄式 repo,`stash -u` + `pop` 之後看那個檔在不在):
  ```
  stash 之前          .git/logs/refs/stash  不存在
  stash -u + pop 之後 .git/logs/refs/stash  🔴 仍然不存在
  ```
  ⇒ **`pop` 把 stash 條目丟掉時,連 ref 與它的 reflog 一起收掉** ——
  **那把尺在「發生過」與「沒發生過」兩個世界印同一個東西 ⇒ 零判別力。**
  ⇒ **「主樹沒有 stash 紀錄」這句話證不了任何事,不得寫成「沒有人 stash 過」。**
  📎 這正是 `feedback_absence-read-as-verified`:**什麼都沒有,被讀成檢查過了。**
- **不修未來會痛在哪:** 任一窗在主樹跑 `git stash -u`(對抗審查者要看乾淨樹時的常見動作),
  **會把別人的未追蹤檔整批移走**;忘了 `pop`(或 `pop` 撞衝突)⇒ **那些檔就沒了**,
  而**事後沒有任何痕跡可查** —— 連「有沒有發生過」都問不出答案。
  🔴 `I-008 §④-4` 已記著前一任拿其中一個當突變靶且**原始大小未量** ⇒ **現在也沒有基準可以比對**。
  ⇒ **這三個檔現在處於「不知道它們該長什麼樣」的狀態,而規則只說了不要動它們。**
- **修法(已有現成設計,不是從零想):** `T-003-STOP-20260817.md §④ diff 2` 的
  `block-git-history-rewrite.js`(PreToolUse hook)**正是為這條路設計的**:
  擋 `git stash` / `reset --hard` / `checkout -- .` / `clean -f`,
  🔴 **而刻意放行 `stash pop` 與 `stash list`**(主視窗當時裁定:`stash pop` 正是事故當天救回東西的動作,
  擋它 = 下次不可復原)。
  ⚠️ **它卡在 `settings.json` 需 Sean 親准**,`T-003` 明寫「不請任何窗代動 —— 那正是權限洗白」。
  ⇒ **本條不新增機制,它是那份待批 diff 的【第二個實證理由】。**
