...
- **落點待定:** 可能在報價單 repo(Sean 2026-08-11 Q11 逐字「如果要做應該也是做在那邊,因為那邊才可以帶入商品資訊」),與 2026-08-12 Q6=A「不做庫存」的關係**待 Sean 釐清**(主視窗已問、未回)。
...
- **不修會痛在:**
  ...
  - **bug 可追蹤性:** 卡住的列不在異常清單裡(只查 `processing`)⇒ 沒有任何畫面會告訴值班「這張單被一列 `manual_failed` 擋住了」,症狀會以「退款鈕按了就被擋、看不出原因」出現。
- **預期解法(三選一,未拍板):**
  ...
  3. 只做「唯讀救濟」:異常清單納入 `manual_failed`,讓值班至少看得到、由工程師手動 SQL 解(**這條不解卡單,只解看不見**)。
- **估時:** 方案 1 = 90-150 分(動 schema + 對抗審查 + 回退);方案 3 = 30-45 分。
...
- **🔴🔴 前置(這條是本片真正的成本):** OD `:1047` 實測 ⋯ 那格要 **38px**;我們 `col-ops` 是 **34px**。而 `Q-E2`=A(2026-08-14 Sean 看圖拍「維持固定寬」)之後**欄寬是零和的**,品名欄只剩約 **1 個中文字**的餘裕 ⇒ **那 4px 要從別欄扣,得 Sean 再拍一次。**
...
- **依賴:** 4px 拍板;**排在 `#484` `#485` 之後**(它與欄寬片同檔,`#484` 不同檔)。
