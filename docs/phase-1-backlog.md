...
- **要做什麼(日後)**:獨立的進貨/庫存管理面(供應商進貨單、不掛訂單的庫存量、出貨時從庫存扣帳)——屬 9 大藍圖的庫存管理塊,**不在 Phase 1 範圍**(鐵則:不做 9 大藍圖)。#352 的「現貨/自有庫存」虛擬供應商(memory `project_m4b-352-spot-stock-supplier-decision`)是它的過渡貼片:現貨仍要等訂單成立後才能以該單品項名義補記。
