...
- **狀態:** **卡在決策**,不是卡在工。⏸ **而且現在還多一層前置**:它要吃片 B 的 chip 殼,而片 B 排在 A1 apply → A2 之後。
...
- **狀態:** ⏸ **等 Sean 拍板** —— 2026-08-14 動手前 hit test **證偽了原規格**(零 JS `<details>` 彈出選單)。
  4px 前置那條已於同日撤回(實測推翻),但**冒出一個更根本的**:**選單在這張表裡點不到。**
...
### #489. 🧭 訂單列表「車種」欄 2/11 被截 —— **確認屬實,但修不掉(欄寬零和)**
