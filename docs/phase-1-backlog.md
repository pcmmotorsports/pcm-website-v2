...
- **狀態:** 🔶 **2026-08-11 拆兩半(依賴不同、非縮範圍)**
  ...
  - **269-b `?filter=new`(新品)= ⏳ 待 plan** —— Sean 定義「新品 = 本週新增的商品」、**要做真的篩選**。
...
### #338. 🔎 供應商單號搜尋沒有供應商維度 — 結果可能混入另一家供應商的訂單

- **狀態:** ⏳ 待執行(2026-08-07 A10c2 立案;A9b2-A 階段 C must-fix 4 的產品面缺口)
- **現況**:`order_item_procurement.supplier_order_no` 在 DB 層**沒有跨供應商唯一性**
...
### #391. 🧾 SORT_OPTIONS 與 CATALOG_SORT_VALUES 兩份清單零對帳守門
- **來源**:#269-a 審查週邊發現(2026-08-11):UI 清單五項中兩項不在 server 白名單(sale 已移除、new=269-b 待做真),兩清單間無任何測試比對;「兩份清單無人對帳」第二實例(第一=#333 跨檔競爭者)。
- **修法**:一格測試斷言 SORT_OPTIONS 每項 value ∈ CATALOG_SORT_VALUES ∪ 顯式標註的「已知假清單」(269-b 落地後該集合應歸零);輕量片。
- **不修會痛在哪**:下一個人往下拉加選項不會知道要動兩處,客人再拿到靜默 no-op 排序。
