...
- **狀態:** 🟡 **部分落地**(2026-08-16 C 窗)。**不要標成已修** ——
  ```
  ✅ 已做：長度 10 碼【擋】；字元集與檢查碼【只警告】；兩個入口都接上
           （建箱彈窗 shipment-dialog / 標記出貨 shipment-mark-shipped-button）
  ❌ 沒做：檢查碼沒有升成【擋】 —— 刻意的，見下方「升級點」
  ❌ 沒做：sf（順豐）完全不驗 —— 規則我方未查證，【不是】刻意豁免
  ```
- **🔴 升級點(有人拿真貨號對過一次之後才做):**
  ```
  ① 檢查碼 warn → block。升級當下 lib/shipping/tracking-number.test.ts
     那格「只 warn 不 block」會紅 —— 那是對的，改它。
  🔴 ② 而在那之前，先用這條【便宜的證偽器】驗規則本身：
        MOD(x,7) 的值域是 0-6 ⇒ 照規則，真貨號【尾碼永遠不會是 7/8/9】。
        Sean 手上任何一張真託運單尾碼是 7/8/9 ⇒ 規則錯了，整條要重查，不要升級。
     ⚠️ 反過來也要留意：若規則錯，現況會對【約三成】的正確貨號跳警告
        ⇒ 員工學會無視琥珀色 ⇒ warn-only 等於零。這是本條最可能的失敗方式。
...
- **裝在哪一層(這條的重點,不是「要不要驗」)**:
  🔴 **不裝在列印端。** 那是最後一刻才發現,而那時貨已經要出門了。
  ⇒ 裝在**建箱 / 標記出貨的表單**(`components/orders/shipment-dialog.tsx` 的 `shipBlocker` 旁邊),
    員工還在鍵盤前面的那一刻擋住。
