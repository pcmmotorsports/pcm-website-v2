...
- 🔴 **不修未來會痛在哪(鐵則 10,這條是本條目的核心):**
  ...
  **來自簡訊／Email**(`Q-C9`),不是那張紙。**病沒有消失,載體換了而且更難收**:
  紙還在自己人手上,**簡訊寄出去收不回來**(鐵則 12⑤)。
...
- **不修未來會痛在哪:** ~~打錯一碼的紙**印得出來、看起來完全正常**~~,而客人拿去 HCT 網站查
  ...
  🔴 **更新(`Q-C5`=丙,2026-08-17)**:紙上**已經不印追蹤碼**了 ⇒ 載體改成**簡訊／Email**(`Q-C9`)。
  **本條沒有變輕,是變重了** —— 紙至少還在員工手上、有機會被人看一眼;
  **簡訊寄出去就收不回來**(鐵則 12⑤),而錯的那一碼與對的那一碼在寄出前長得一模一樣。
...
(`gate.test` 7 + `check-syntax` CLI 2);其餘 **16 個**在 jsdom 元件測試,**純 render 逾時 5s**,
與 spawn 無關,形狀是 **worker 飢餓**。
