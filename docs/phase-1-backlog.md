...
- **狀態:** ✅ **已實作**(2026-08-17 夜 T 窗,分支 `four-greens-drift`)。R1 那五條**逐條配了突變、每一發都指名哪一格會紅**
  —— 對照表在下面「本版怎麼收的」,數法 `sh scripts/dev-four-greens.sh --selftest` ⇒ `PASS=30 FAIL=0`(舊版 22 格)。
...
- ✅ **本版怎麼收的(2026-08-17 夜,分支 `four-greens-drift`)**:照上面「修法方向」把判定抽成吃**字串**的
  ...
  **行數**換成兩個 `cksum`(未提交檔案清單 + `git diff HEAD` 內容),三欄各自帶 `nohead` / `GITFAIL-*` 失敗字面。
  selftest **22 → 29 格**。
  **突變對照表(十發,每一發都實跑;預期紅的格是【動手前】寫下的):**

  | 突變 | 對應 finding | 預期紅 | 實得 |
  |---|---|---|---|
  | M0 不突變(正向對照) | — | 全綠 | ✅ `PASS=30 FAIL=0` |
  | M1 `drifted_between` 恆 `false` | R1 F3 | 格18 | 格18 + 格18-b |
  | M2 `drifted_between` 恆 `true` | R1 F3 | 格17 | 格17 |
  | M3 指紋換成 `echo CONSTANT` | R1 F1 | 格15 | 格14-0 + 格14-2 + 格15 |
  | M4 指紋退回**行數**版 | R1 F4 | 格15 | 格14-2 + 格15 |
  | M5 指紋塌成 `nohead` | R1 F5 | 格14-0 | 格14-0 + 格14-2 + 格15 |
  | M6 刪掉 `four_greens` 的呼叫行 | R1 F3 剩下那一跳 | 格19 | 格19 |
  | M7 指紋換成**不是 `nohead` 的**常數 | codex F1 | 格14-0 | 格14-0 + 格14-2 + 格15 |
  | M8 指紋退回**管線版**(git 失敗被 `cksum` 吃掉) | codex F2 | 格14-2 | 格14-2 |
  | M9 刪掉 `drift_report` 螢幕上的 `echo` | codex F4 | 格18-b | 格18-b |
  | M10 呼叫行**搬到 `four_greens` 之外** | codex F3 | 格19 | 格19 |
  | M11 拿掉 `git status` 的 `-uall` | codex R2 | 格15-b | 格15-b |

  🔴 **harness 自己也踩了兩個坑,留著**:①M9 第一版用 `grep -v '<字面>'` 刪整份檔案,
  **連格18-b 自己的 pattern 那一行也一起刪掉** ⇒ 那格是紅了,而**紅的理由被污染**(突變同時改到了測試本身)。
  ②報告對 M0 無條件印「🔴🔴 零紅 = 沒有判別力」,而 M0 是**正向對照、零紅正是預期** ⇒ **量具自己在誤報**
  (`CLAUDE.md` 終端機紀律第三條的原形:輸出的標籤要由結果決定,不能無條件印)。
...
- ⚠️ **本版的射程限定(四條,明寫不假裝已守)**:
  ① **格19 只證「呼叫那一行還在 `four_greens()` 函式體裡」,不證它執行過** —— 要證後者得真跑一次四綠(15-60 秒 × 4)。
  ...
  ④ **DRIFT 亮了整支的 `rc` 仍是 0**(原條目列為 nit,本版沿用)—— 呼叫端不會因為 DRIFT 而失敗,
  它只印警告與落痕跡檔。改 `rc` 語意會撞上檔頭既有的「量具沒起來 vs 量到紅同一個 rc」那條,不在本片範圍。
