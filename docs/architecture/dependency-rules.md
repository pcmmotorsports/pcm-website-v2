...
| 5 | apps storefront → apps medusa | ✅ catch(M-0-03 用 apps tmp .ts) | ⏳ → 🟡 storefront 部分解開(M-1-03-main-d-pre 補 apps/storefront/tsconfig.json + 進 import resolver glob、boundaries Rule 5 已生效於 storefront;apps/api / apps/admin / apps/sync-engine 仍純殼、待 M-4a-01 / M-5-01 補) |
| 6 | ui → domain | ✅ catch | ✅ catch |
| 7 | schemas → domain | ✅ catch | ✅ catch |

**apps/* 部分加入 project glob**(2026-05-08 / M-1-03-main-d-pre 解開 storefront 部分):
- ✅ apps/storefront 補齊 `tsconfig.json`(對齊 packages/* 模板 extends `tsconfig.base.json`)+ 進 import resolver glob、boundaries Rule 5 對 storefront 真實生效
- ⏳ apps/api / apps/admin / apps/sync-engine 仍純殼、待 M-4a-01(admin)/ M-5-01(sync-engine)真寫 .ts 時各自補 tsconfig.json 同時加進 import resolver glob
...
M-1 裝 Next.js 後 apps/storefront 已有大量 .tsx(現 100+ 個)→ 本 flag 從「為讓純殼 lint task 跑通」的必要設定降為保險用(無副作用、可保留;仍空殼的 apps/api 受益)。(#25)
...
4. ⚠️ apps 之間預設**不可互相 import**(`boundaries/dependencies` from='apps' 的 `allow.to` 不含 apps、default disallow;見 §3 表 Rule 5 / §2.3)。新 app 若確需 import 既有 app,須顯式在 from='apps' 規則補 allow,否則被擋(維運盲點、對稱 §6.1 步驟 3)。(#24)
...
| 2026-05-08 | §5.1 storefront 部分解開:apps/storefront/tsconfig.json 補齊(對齊 packages/* 模板 extends `tsconfig.base.json`)+ 進 `import/resolver.typescript.project` glob;Rule 5 對 storefront 真實生效(對齊 backlog #54 Supersede storefront 部分解開);apps/api / apps/admin / apps/sync-engine 仍純殼、留 M-4a-01 / M-5-01 補 | Claude Code(M-1-03-main-d-pre slice) |
