-- ⛔⛔ **2026-09-05 裁【不貼】** —— 出口擋會把一張【真的已經退完】的單鎖死
--    (路徑:人工全退 → 作廢造成 stale `refunded` → 之後真的在 TapPay Portal 全退而 PCM 沒記帳
--     ⇒ 這道閘判不足、拒絕;而 `admin_cancel_order` 也擋刷卡單 ⇒ **兩條取消路都走不通**)。
--    ⇒ **成因修在片③**(`20260905010000` 那道早退拿掉 + 作廢端呼叫同步器 ⇒ 狀態照事實降回)。
--    ⇒ 本檔**留在分支當「第二層保險候選」**, 不是作廢 —— 片③ 上線後若仍要縱深, 從這裡接。
--    🔵 裁決者=主視窗 `-f8`(工程題;片③ 的方向 Sean 2026-08-22 `Q-B=甲` 已拍「允許回 paid」)。
--    🛑 而那條鎖死路徑是**讀碼推出來的, 沒有重現過** —— 見板列 ⟦0a-CARDCANCELNOREFUND⟧。
--    📎 findings 來源:codex 框架輪(換角度, 同模型 gpt-5.6-sol effort high)2026-09-05。
-- ══════════════════════════════════════════════════════════════════
-- ══════════════════════════════════════════════════════════════════
