...
```
Q3:要不要把「提修法之前先實測那個修法」寫成固定步驟?

    背景:我第一版提的「改法甲(放寬正規式)」實測完全沒用,而我是在 codex 指出之後才去量的。
          今天我三次遇到「以為修好了 / 以為紅了」而實際沒有,三次都是加了前提斷言或實測才發現。

    甲(推薦)= 要,但只限【改守門 / 量具 / 掃描器】的修法。
        理由:量具的修法「有沒有用」本來就是一個可以當場跑出數字的問題,成本幾乎是零。
    乙 = 要,所有修法都適用。
        ⚠️ 太貴 —— 不是每個修法都便宜到可以先試,寫太寬會沒人遵守。
    丙 = 不要,規則已經夠多了(對齊 09-09 的減法版)。
