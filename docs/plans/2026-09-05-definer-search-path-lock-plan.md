...
> 唯讀普查來源 `e2597dc1a`;授權射程:唯讀已批、**apply 未批** ⇒ 本檔只規劃。
> 🔴🔴 **數字會動 —— 引用前自己重跑一發,不要抄本檔**。同一夜量了兩次就變了:
>
> | 量測時刻 | `public` 函式總數 | DEFINER | 已鎖成空 | **待鎖** | 待鎖中 SR 叫得動 |
> |---|---|---|---|---|---|
> | 09-05 早(貼板前) | 169 | 117 | 78 | **39** | 22 |
> | 09-05 貼完那 10 支後 | 176 | 122 | 84 | **38** | 22 |
>
> 📌 **少掉的那一支是 `pcm_sync_order_refund_payment_status`** —— `20260905010000` 順手把它鎖成 `""` 了。
>    ⇒ 它同時是**一發活生生的「鎖了沒壞」證據**(貼完至今正式庫沒有因它出錯)。
>    🛑 而**分母也在動**(169→176)⇒ 只更新分子會得到一個內部不一致的比例。
...
⇒ **待鎖範圍 = ⛔ ~~39~~ ⇒ 38 支,不是 46。**(舊字面留著,讓搜 39 的人同一發撞到訂正)

## 1. 分堆(⛔ ~~39~~ ⇒ **38 支**;09-05 貼完那 10 支後重量。名單自己重跑,不要用舊檔)

| 堆 | `search_path` 現值 | 支數 | 其中 `service_role` 叫得動 | 處置 |
|---|---|---|---|---|
| **A(優先)** | `public, pg_temp` | ⛔ ~~36~~ ⇒ **35** | **20** | 鎖成 `''` |
