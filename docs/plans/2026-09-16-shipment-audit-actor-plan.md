...
### ③ 部署順序:**板先貼、碼後推** —— 而且中間有一段空窗

新簽章算「新函式」⇒ 碼先上線會找不到它。部署時序閘本來就會擋這種情況(它盯 `.rpc(`)。
⇒ **這片程式在 Sean 貼板之前不能推。**

🔴🔴 **但「板先貼、碼後推」只解決一半 —— 這一段是 2026-09-16 貼完板之後才補的,原版漏了。**

「板先貼、碼後推」這條規矩**是為【新增函式】寫的**:那種情況碼先上會叫不到還不存在的東西,所以板要先。
**而這一支是【改簽章】,兩個方向都有空窗**:

| 順序 | 空窗期間會怎樣 |
|---|---|
| 板先貼(我們選的) | **舊碼叫不動新函式** —— 舊碼不送 `p_actor` / `p_request_id`,而那兩個是必填(無 DEFAULT)⇒ 沒有任何多載滿足得了這個呼叫 |
| 碼先推 | 新碼叫不動舊函式 —— 舊函式收不到那兩個參數 |

⇒ **真正的規矩是:改簽章要把「貼板」與「推碼」當成同一次動作** ——
中間的時間要短,而且**要事先知會正在用後台的人**。

**2026-09-16 實際發生了**:板 199 貼上正式庫之後、碼推上去之前,
正式站的建箱 / 掛品項 / 標出貨 / 作廢 / 還原是**壞的**。
這不是估計,是兩個讀數推出來的:
1. 正式庫 `pg_get_function_arguments` 實查:`admin_void_shipment` 5 個參數**零個有預設值**;
   `admin_mark_shipment_shipped` 只有 `p_tracking_number` 有 ⇒ **`p_actor` / `p_request_id` 必填**。
2. 正式站部署的那顆 `bd4fd24f7`(`git show` 讀出來的):呼叫時**只送** `p_idempotency_key` /
   `p_shipment_id` / `p_tracking_number`,**沒有** `p_actor`。
⇒ 必填的沒送 ⇒ 一定失敗(錯誤碼叫什麼名字不影響這個結論)。
...
## ⚠️ 貼板要挑時段(2026-09-16 補)

這片是 **`DROP FUNCTION` 再重建**五支出貨函式。
`DROP` 會等它拿到鎖,而在那個瞬間**如果有員工正在建箱 / 標出貨 / 作廢**,那個動作會失敗。

⇒ **貼板之前要先確認沒有人在用後台。**
站上已經有真客人(2026-09-15 上線),所以:
- 挑出貨量低的時段貼(例:一早或午休後,避開下午出貨尖峰)。
- 貼之前問一句「現在有沒有人在後台出貨」。
- 五支在**同一支 migration、同一個交易**裡換完 ⇒ 不會出現「換到一半」的狀態;
  最壞情況是那一發整個失敗回捲,舊函式原封不動,員工重按一次即可。
