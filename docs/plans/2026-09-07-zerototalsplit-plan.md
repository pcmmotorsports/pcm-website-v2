...
# 🔴 關卡 1 · codex R2 `VERDICT: FAIL` ⇒ 收斂與**還沒收斂的那一格**(2026-09-07)

> rc=0 / 產出 6149 行 / 回聲字 `SPLITPLAN2R0907` 命中 4。**輪數上限 2 輪已用完** ⇒ 依規矩不自己開第三輪, 整理決策題上呈。

## R2 逐條判定(它自己給的, 照抄不美化)
關閉:**R1-1 · R1-2**;核心關閉:**R1-6**(而它註明「作者的正式庫讀數不算本輪獨立驗證」);
部分關閉:**R1-4**;**未關閉:R1-3 · R1-5**。

## ✅ 我採納 R2 的設計, 而它把 R1-3 與 R1-5 一起解掉
R2 `#3` 給的第三種排法 —— **新支的前置閘改成三態, 不是釘死一個舊 md5**:
```
現行 body = 舊版(087a886d…)  ⇒ 替換
現行 body = 目標版(f134e95d…) ⇒ 不替換, 只驗屬性與 ACL 後通過
現行 body = 其他任何值        ⇒ 拒絕, 人來看
```
⇒ 📌 **這樣 `76 → 新支` 也成立** ⇒ **順序限制消失**(R1-5 的「必須排在 76 之前」我**再收回一次**),
而且 R1-3 那個「76 覆蓋 rollback」的反例也弱化 —— 因為新支可以在任何時點**重跑一次**把狀態拉回目標版。
🛑 **而 R2 說得對的地方要留著**:三態閘**分不出「尚未升級的舊版」與「刻意 rollback 回去的同一舊版」** ——
兩者 body 一模一樣。⇒ **那不是閘能答的問題**, 要靠貼板紀律(rollback 之後要有人把該列改回 open)。**明寫, 不假裝閘擋得住。**

## 🔴 R2 `#5` 新抓到的行為反例 —— **我開檔驗過, 成立**
76 的 P5 新分支(`:1220-1221`)逐字 `OR (o.total = 0 AND pay.rows_n = 0 AND o.ps = 'paid' AND o.payment_method = 'zero_total')`
⇒ **它沒有檢查 `att.uncovered_n`**, 而**同一個 P5 的第一分支有**(`pay.rows_n > 0 AND att.uncovered_n = 0`)。
⇒ 一張 `total=0 · paid · zero_total · 零收款列` **而存在未入帳 charged attempt** 的單, 其餘 P 值皆真時 **會被判 `settled`**;
**舊一代會要求人工處理。** ⚠️ 未證實正式庫存在該狀態。
⇒ **新支必須帶這一格的負測, 或證明該狀態不可達。**(這一條在抽出來之前就要處理 —— 它跟著 body 一起搬。)

## 📋 R2 說「還不足以動手」, 缺四格。**三格我填了, 一格要拍板。**
1. ✅ **新版本號、實際貼序、四種狀態怎麼處理** ⇒ 上面三態閘那段(舊版替換 / 目標版驗證不替換 / 未知拒絕 / rollback 靠紀律+板列)。**版本號排序與人工貼序分開定義**:版本號取當日次一個空號即可, **貼序無限制**。
2. ✅ **確切鎖鍵、取鎖位置與交易邊界** ⇒ 鍵 `hashtextextended('admin_compute_order_settlement', 0)`(**與函式同名**, 與現有四個家族都不撞);位置 = **`BEGIN` 之後、前置閘之前**;`SET LOCAL lock_timeout = '5s'`;`BEGIN`/`COMMIT` 自帶。
   🛑 **而不受它保護的入口要明寫**:任何**沒有拿同一把鍵**的替換路徑(76 現況、舊貼板副本、SQL Editor 手改)—— advisory lock 是**合作機制**, 它擋不住不合作的人。
3. ✅ **驗收案例** ⇒ ①三態閘各餵一次(舊版/目標版/現造第三種 body)②失敗時整筆還原(前置閘紅 ⇒ 函式 body 不變)③R2 `#5` 那個 `uncovered_n` 反例的負測 ④兩處行為變更(voided 排除 / 0 元 P5)各一發。
4. 🔴 **要拍板的那一格(我不自己選)**:
   > **76 要「就地修訂」還是「替代交付」?** —— 就地修訂 = 動一支既有 migration 檔 ⇒ 要重新審查、重出貼板 sha;
   > 替代交付 = 76 維持原樣不再貼, 另出一支只做券那半的新片。
   **Q:76 怎麼處理?**
   **A: 甲 = 替代交付(76 不再貼, 券那半另出新片)| 乙 = 就地修訂 76(改既有檔、重審、重出 sha)**
   ⚠️ **我沒有推薦** —— 這一格牽動的是「一支已經審過的 migration 還能不能改」的制度問題, 不是技術判斷。

## 🛑 R2 的射程(照抄)
唯讀核對 `agent/line-db@3aeda41f6` 的 plan、相關 SQL 與紀錄;兩代 body MD5 重算吻合。
未連正式庫、未執行 migration、未跑行為或雙 session 測試;**作者的 trigger 線上驗證不算本輪獨立驗證**。
