...
### 6.4 ✅ 已答:刪除理由 **選填**(Sean 2026-09-13 逐字「乙 = 可以不填」)

```
Q: 刪除備註要不要「理由必填」?
A: 甲｜必填 —— 對帳與客訴時看得到「為什麼刪」,而刪除本來就該是少見動作
   乙｜選填 —— 少一個攔路欄,員工比較會用          ← ✅ Sean 選這個
```
交辦檔 ④ 寫的是「軟刪除(`deleted_at` + `deleted_by` + 原因)」,**沒有說必填**。

**他選乙的理由**(這一段要留著,下一個想「順手加必填」的人需要讀到):
**必填會製造假理由** —— 想不出理由的員工會打「.」或「刪除」,
而**一個被亂填的必填欄,比一個空的選填欄更糟:它看起來像個理由。**
一個空的 `deleted_reason` 老實說「沒人寫」;一個「.」會讓讀對帳的人以為有人交代過。

⇒ 實作落點:CHECK 只釘 `deleted_at` 與 `deleted_by` 同生同滅(名字 `order_notes_deleted_pair_together`,
刻意不叫 triple);理由正規化後為空 ⇒ 存 `NULL`,**不是錯誤碼**;固定碼因此是 **7 個不是 8 個**。

⚠️ **兩段插曲要留痕,因為它們各自是一個形狀:**
- 本 plan 原本把這一格列為「擋著實作」,理由是「事後補約束會撞既有列」——
  而 `order_notes` **0 列** ⇒ 那個前提不成立 ⇒ **擋路自己失效,不是有人改口**。
- 而 Sean 其實**當天早上就答了乙**,訊息沒轉到施工窗 ⇒ migration 第一版做成必填。
  📌 **在貼正式庫之前改回來 ⇒ 零對外**。那正是「寫檔、不貼、等他對編號回一句」這條流程擋下來的東西。
