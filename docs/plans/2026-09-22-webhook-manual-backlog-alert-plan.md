...
在既有的「付款異常提醒」（每天台北時間 09:00 與 21:00 各執行一次的 `check-anomaly-alerts`，同時送 Email 與 LINE）多加一組數字：「付款通知需人工確認」。另外在後台首頁「等你處理」那一區加一行。
...
- 實際效果：告警一天跑兩次（cron `0 1,13 * * *`，台北時間 09:00 與 21:00；`20260915120000_m4b_anomaly_alert_twice_daily.sql`），所以從卡住到 LINE 響，最長約 60 小時（48 小時門檻加上最多 12 小時等下一次執行）。後台首頁則是員工一登入就看得到。（2026-09-22 Fable 補審 nit 更正：原寫「每天早上 9 點跑一次、最長約 72 小時」。）
...
- 乙：有一筆就響（下一次排程，台北 09:00 或 21:00 就通知）。最快知道，但付款通知偶爾卡一下也會響。
- 丙：24 小時。介於兩者之間。
