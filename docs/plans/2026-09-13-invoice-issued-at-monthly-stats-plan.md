...
**⑦ ⛔ ~~登記那一格「預設今天」~~ —— P2 落地時改成【不開立的單一律不預填】(codex R1+R2 各一條 must-fix)**
根因一句:**任何不是員工自己打的日期,都會被表單原樣送出去,而 RPC 分不出「打的」與「自動送的」。**
· 預填既有值 ⇒ 9/28 開 → 作廢 → 10/5 重開只改狀態 ⇒ 自動回送 9/28 ⇒ **重開的金額歸回 9 月**
· 預填今天   ⇒ 9/30 23:59 開表單、10/1 00:01 登記 ⇒ 自動送 9/30 ⇒ **10 月的發票算進 9 月**
⇒ ✅ 落地版:**只有「已開立」的單預填既有日期**(改號碼不該逼他重打);`voided` / `not_issued` 一律空,
變成已開立時必須自己打,留空送出 ⇒ RPC P9I01。**`max` 也拿掉**(跨午夜沒重載的 `max` 會把合法的今天擋在 RPC 之前)。
🛑 **這偏離規格 §2-a「預設今天」的字面,已回報主視窗**;代價只是他多打一個他手上那張紙就寫著的日期。

**⑧ 舊表單 `order-edit-form.tsx` 補 `key={detail.version}`**(codex R2 must-fix):
它是 server component,沒有 key 的話 revalidate 之後 React 只更新 props,uncontrolled 的 select / date **留著舊 DOM 值**
⇒ 別人剛作廢、甲只改號碼按存 ⇒ 送出 `version=8 + issued + 9/28` = 用舊日期重開。
形狀照小抄彈窗那張 form(它早就有)。突變(拿掉 key)⇒ 那一格當場紅。
