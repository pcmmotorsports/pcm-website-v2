...
> 🛑🛑 **[2026-09-05 · codex 關卡1 = FAIL,8 must-fix + 1 nit]**
> **findings 全文**:`docs/reviews/2026-09-05-vehicle-taxonomy-rpc-codex-gate1.txt`
> 🔴 **而第①條不是細節,是【這一案現在不該做】**(逐字):
> > 流量為零、兩條客戶等待路徑皆已解除、機制仍未解 ⇒ 現在做只增加 schema／部署風險;
> > 「先不做」即可取得相同的當前客戶收益,**plan 未先訂背景成本門檻**。
> 🔵 **而它不是【獨立】得出這個結論的** —— 它讀了本檔 §0 自己寫的那句
> 「證據今天不存在 ⇒ 本案可以緩」。⇒ 📌 **它做的是把作者的自我懷疑升級成 must-fix**,
> 不是第二次獨立量測。而**它多指出一格作者漏的:本 plan 沒有訂「痛到什麼程度才做」的門檻。**
> ⏸ **⇒ 等 Sean 一個字**(甲 先不做而現在就訂門檻 / 乙 照原訂做)。
> 🛑 **在那之前【不折】其餘 7 條** —— 折它們是為了一個 codex 說現在不該做的案子。
>
> ---
>
> ✅ **[2026-09-05 15:0x · Sean 拍板 = 甲]** 逐字:
> 「甲(車款清單,根據我請報價單那邊做了,你可以給我提示詞我再給他確認一次)」
> ⇒ **甲 = 先不做,而現在就訂門檻。** 板列 `⟦search-VEHTAXSLOW⟧` 已改 `parked`。
> ⚠️ **括號裡那半是新資訊,不是理由的重述** —— 他說**報價單專案那邊已經做了**車款清單相關的東西,
> 而**我們不知道那是什麼**;提示詞由主視窗 `-f8` 寫在
> `~/pcm-mailbox/提示詞-給報價單側-車款清單-20260905.md`,Sean 轉貼。
>
> ### 📏 解封門檻(codex #① 說「plan 未先訂背景成本門檻」,這就是那一格)
> ```
> 指標 = view `public.vehicle_taxonomy_public` 的列數
>        今天 12,053 列 / PAGE_SIZE 1000 ⇒ 13 頁
> 解封條件(或):列數 > 16,000(= 17 頁)   ‖   冷撈 > 15s
> ```
> 🔬 **今天的讀數(隨時重跑得到)**:12,053 列 · 13 頁 · 冷 11.8~12.6s · 暖 33~40ms。
> 🔵 **為什麼兩個條件而不是一個**:列數是線性推頁數的、隨時量得到;
> 🛑 **而我沒有量過「16,000 列那天冷撈會是幾秒」—— 那是【外插不是量到】**
> ⇒ 所以寫成【或】,讓直接量得到的時間那一格可以先叫。
> 🛑 **這個門檻證不到什麼**:15s 是我訂的,不是從任何讀數推出來的;
> 而**觸發之後仍要重跑本 plan 的其餘 7 條 must-fix**,不是直接開工。
> ⚠️ 而那 7 條**沒有過期**:它們指的是 plan 的內容,而 plan 沒有改。
