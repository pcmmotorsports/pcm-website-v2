# plan · ⟦b4-REFUND10016⟧ 乙:讓值班分得出「錢動了」與「錢沒動」
...
|---|---|---|---|---|
| `:429`(`wire.status === 0`) | 受理了,回應格式壞掉 | **可能已經出去** | **不要重退**,先對帳 | 重退 ⇒ **退兩次** |
| `:499`(`wire.status ≠ 0`) | 拒絕了 | **沒動** | 要重退 | 不退 ⇒ 客人的錢一直沒回來 |
...
① 兩個 throw 各自帶對的 outcome —— 而【不是抄我要寫的那行碼】:
   期望值從 §1 那張表推(:429 = 錢可能已出去 = accepted_malformed;:499 = 錢沒動 = unknown_wire_status)
