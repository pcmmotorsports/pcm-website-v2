...
## 11. codex R2(片 2a+2b,2026-09-10)—— **零 must-fix**,2 條 should-fix

### ✅ should-fix ① 已修 —— 而它打中的是我最重要的兩個設計決定

codex 在記憶體裡對七支各做兩種突變,**兩種都是 121 格全過**:
...
📌 **那兩件事正是我在 §10 宣稱「解掉了」的東西,而沒有任何一格在守它們。**
🎯 一個「解法」如果退回去不會紅,**那它與沒解只差在我記得**。

✅ 七支各補兩格,而**現在那兩個突變各紅 7 格(一支一格)**:

| 突變 | 修之前 | 修之後 |
|---|---|---|
| 不寄的列加回 `countNewEvents` 分母 | 121 格全過 | **7 failed / 7 檔** |
| 落痕迴圈移到 cap 之後 | 121 格全過 | **7 failed / 7 檔** |

### ⏳ should-fix ② 沒修,而理由要寫出來

> 片 2a 的「不會回掃描面」測試**只比對手抄清單** —— 假設 SQL 放行清單誤加 `manual_no_recipient`,本測試仍會通過。

🔴 **codex 是對的。** 那一格今天證的是「這個常數沒有撞到我手抄的那五個字串」,
**不是**「正式庫那張 view 真的會排除它」。

🛑 **而我沒有修它,因為正確的修法要一個【跑得起 view 的拋棄式 PG】的測試骨架,而這個 repo 今天沒有** ——
`migrations-replay-from-zero.sh` 是**整批 apply**,不是「落一列再問 view 有沒有它」。
⇒ 📌 **硬做 = 在這一片裡順手蓋一個測試基建**,那超出範圍而且會蓋掉這一片的 review 焦點。

✅ **留成已知缺口,而把它的形狀寫死在這裡**:
```
今天守得住   常數不等於那五個放行碼(手抄清單, 出處是 2026-09-10 唯讀正式庫 pg_get_viewdef)
今天守不住   有人【改 SQL】把 manual_no_recipient 加進放行清單
             ⇒ 部分退款那些列會被重新掃到 ⇒ insert 撞既有鍵回 duplicate ⇒ 每輪重撈
             ⇒ 🔴 **那正是本片要修的病, 原地復活, 而不會有任何一格紅。**
關法         要一個能對真 view 下 SQL 的測試骨架(落一列前有候選 / 落下後候選消失)
```

### 🔵 codex 核對過而沒有問題的(摘)

- 七支語意:`effectiveEmail` 的收窄與 `noRecipient` 計數與改版前一致。
  ⚠️ 一個防禦情境差異:部分退款那支,「手動留白 **且** 退款金額/時間無效」的列現在會先計 `unusableAmount`(以前計 `noRecipient`)—— 既有 DB 約束正常時造不出這種資料。
- cap 前寫入不是無上限(每支一輪最多掃 50,scanner 上限 200);逐列獨立寫,不延長同一筆交易。
  ⚠️ **未做 60 秒時限的負載驗證。**
- 失敗不是「不重試」:寫入失敗 ⇒ 沒有終態 ⇒ 下一輪 scanner 會再試;持續性錯誤仍可能佔住掃描窗口,而 `errors` 會讓 route 回報失敗。
- PII:確實**多了一份 email 副本**在 outbox 上;本次改動沒有把它送進 result / log / 前台,而**這不等於完整合規結論**。
- 沒有任何一條路會讓這些列被寄出去或被重排。
- 部署順序正確;反向部署會撞 CHECK ⇒ 落痕失敗並持續重撈。
