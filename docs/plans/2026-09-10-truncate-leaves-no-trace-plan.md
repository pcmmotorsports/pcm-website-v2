...
> 🔴 **本節是第三版。** 第一版建立在「三支 trigger = 完整方案」的錯前提;第二版加了丙,而丙的好處也是推的。
> **三個選項的描述都改過,並加了第四個。**

### 🔵 先回答你會第一個問的那件事:**這不影響你刪測試單**

四個選項**沒有一個會擋 `DELETE`**(實測)。
而你 2026-09-04 那次刪測試單是 **`23 ⇒ 1`** —— **那不是 `TRUNCATE`**(`TRUNCATE` 會變成 0)。
⇒ 📌 **不管你選哪個,你以後照樣刪得掉測試單,而且刪的那條路本來就有留痕了。**

---

```
Q:訂單被 TRUNCATE 清光時, 要不要補留痕 / 要不要擋?

    現況(都是量的):
      · 貼板 103 已上線, 擋得住【一列一列 DELETE】—— 刪一張單留 3 列, 單號金額都在。
      · 🔴 但 TRUNCATE 一列都不會留。
      · 🔴 而 TRUNCATE orders CASCADE 會遞迴清到【26 張表】, 其中【14 張碰錢】
        (退款 8 · 收款扣款 4 · 發票 1 · 重複扣款異常 2)。
        ⚠️ CASCADE 的範圍不看 FK 是不是 ON DELETE CASCADE —— RESTRICT 的也照清。

    甲 = 補三張表的留痕(orders / order_items / order_legal_consents)。
        給的:那三張被 TRUNCATE 時, 每一列的完整內容都抄得下來(實測, 單號金額都在)。
        🔴 不給的:另外【23 張照樣安靜地全部消失】, 包含所有退款與收款紀錄。
        🛑 而這一格是它最大的問題:事後你會看到「三張表被清了」而【看不到退款資料被清了】
           ⇒ 📌 **它比不做更容易誤導, 因為它看起來像有在記。**
        代價:留痕表只增不減 · WAL 與備份 · 持鎖時間變長
              · 留痕寫失敗會讓整個操作失敗(沿用你 09-07 答 Q37 拍的 fail-closed)。

    乙 = 補到 26 張表都留痕。
        給的:CASCADE 掃到的每一張都留痕。
        代價:🔴 26 支 trigger 要維護, 而【每次有人加一張參照 orders 的新表, 就多一個沒被涵蓋的洞】,
              而【沒有任何東西會叫】⇒ 那是一份會持續腐爛的清單。
              持鎖與寫入量也放大一個量級。

    丙 = 不記, 改成【擋住 TRUNCATE】。
        給的:實測擋得住, 連資料庫擁有者都擋, 而且錯誤訊息講得出下一步。
        🔴 代價一(最重要):**它是減速丘, 不是牆。**
              `SET session_replication_role = replica` 【一行】就繞過去了,
              而下得了 TRUNCATE 的身分本來就下得了那一行。
              ⇒ 它擋得住【手滑】與【不知道有這規矩的人】, 擋不住【決定要做的人】。
        🔴 代價二:**它也要逐表掛** —— PostgreSQL 直接拒絕「一支守全庫」的寫法
              (實測 `ERROR: event triggers are not supported for TRUNCATE TABLE`)
              ⇒ 跟乙同一個腐爛問題。
        🔴 代價三:一樣擋不住 DROP TABLE / DROP SCHEMA(實測表直接沒了)。
        🔴 代價四:會擋到【我們自己的驗證腳本】—— 掃到 12 處 / 10 支, 其中幾支會套用我們的
              migration ⇒ 那道擋 trigger 跟著進去 ⇒ 自己擋掉自己。
              🛑 確切幾支會壞我證不到, 只抽樣了四支。

    丁 = 不做, 而把它寫成一條【給人看的規矩】。
        做法:runbook 一句 ——「清訂單資料一律用 DELETE, 不要用 TRUNCATE, 因為 DELETE 那條路有留痕」。
        給的:零程式 · 零維護 · 零腐爛 · 不擋任何驗證腳本 · 不增加任何鎖與寫入。
        🔴 代價:**沒有任何東西會擋, 完全靠人記得。**
        🔵 而它誠實 —— 甲與丙都是【看起來有防護】而實際擋不住決定要做的人。
           📌 如果最後的答案就是「靠人」, 那就明講, 不要包裝成機制。
...
**① 甲與丙的真實防護力,其實很接近丁。**
· 甲擋不住任何事(它只記,而且只記 26 分之 3)。
· 丙一行就繞得過。
⇒ 📌 **三者都擋不住「決定要做的人」,而甲丙多了維護成本與誤導風險。**

**② 而「這件事會不會再發生」我證不到。**
· 【量的】五個指定應用角色(`anon`/`authenticated`/`service_role`/`pcm_readonly`/`authenticator`)
  對那三張表**沒有直接的 `TRUNCATE` 權限**。
· 🛑 **但這推不出「唯一做得到的是你」** —— 我沒排除其他角色、可切換的角色身分、管理連線、高權限函式。
· 🛑 **也證不到 09-04 那次用的是 `TRUNCATE`** —— `23 ⇒ 1` 反而不像(那會變成 0)。
⇒ 📌 **所以這一題不是「補一個已知會再發生的洞」,是「要不要替一個沒發生過的形狀先付成本」。**

### 🔵 一格保持「推的」,不去補

`pg_restore --disable-triggers` 本來就是設 `session_replication_role = replica`
⇒ **丙不會在災難還原那天擋路**。
⚠️ **這是【推的】,我沒有真的跑過一次 `pg_restore`。**
🛑 而**不值得為它去跑一次** —— 它不改變上面四個選項的任何排序。
