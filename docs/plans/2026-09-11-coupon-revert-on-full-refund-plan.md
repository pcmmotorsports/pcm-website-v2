...
> ## 🔴 2026-09-11 **本節整段訂正過。舊字面是我量錯的,而它已經到過 Sean 面前。**
> ⛔ ~~「扣券的路今天在正式庫上是通的」~~ · ⛔ ~~「`create_order` 兩個 overload 都呼 `redeem_coupon`」~~
> ⛔ ~~「擋著它的那道牆已經倒了」~~
> 🎯 **錯法**:我用 `position('redeem_coupon' in prosrc) > 0` 問「它呼不呼」,而那些命中**全部是註解**。
> 📌 **而我在同一個晚上、更早的一個小時,才剛用「剝掉行註解再數」抓到另一處同樣的錯**
> 　 (那一處 9 支 ⇒ 7 支)—— **然後我沒有回頭把同一把尺套回這一格。**
> 🛑 **一個修法只套用在你當下打開的那個問題,而同族的另一處沒有東西會提醒你。**
> 　 (這句話就寫在 `supabase/migrations/20260831160000…:466-467`,是**別人踩過同一件事留下的**。)

**今天:券一次都扣不掉,而退回的路也不存在。要做的是【補上一個目前不會發生的前置】,不是【止血】。**

```
🔬 剝掉行註解重量(全庫, create_order 兩個 overload 都算)
   誰的【碼】裡有 redeem_coupon
     coupon_redeem_order_problem   raw 1 ⇒ real 0
     create_order (overload A)     raw 5 ⇒ real 0
     create_order (overload B)     raw 5 ⇒ real 0
   ⇒ 🔴 全庫真呼叫端 = 0        尺自檢 raw_total 11 ⇒ real_total 0(剝註解這步確實有作用)
   🟢 正對照 pcm_order_refundable_remaining ⇒ real 4  ← 這把尺印得出非零, 所以那個 0 是真的
   create_order 兩個 overload:calls_redeem f · touches_redemptions f · touches_coupon_id f
                               只有 uses_coupon_code_param = t(參數收得到, 而碼裡不用它)

✅ 真的變了的只有一格:redeem_coupon 這支函式【今天在正式庫上】(09-02 的 plan 說它未 apply)
🔴 而【沒有任何東西呼它】⇒ 券一次都扣不掉
⇒ 📌 plan 09-02 §② 那句「表在、view 在、而沒有東西寫」【今天仍然成立】。

🔴 不存在的那一半(這一格沒有變, 三次量法都一致)
...
🛑 **今天曝險 = 0,而它有【兩層】,兩層都不是閘:**
```
① 沒有任何東西呼 redeem_coupon ⇒ 券扣不掉
② coupons 0 列 · coupon_redemptions 0 列 ⇒ 就算呼得到也沒有券可扣
```
📌 **兩層都是【當下的事實】,不是機制** —— 它們不會在有人建券、或有人接上呼叫端的那一秒叫。

🔵 **所以「現在做」站得住的理由只有一個,而它不是我給的**:
　 退券仍然是 `20260901021000` / `20260901030000` 的**前置條件** —— 那是 **Sean 自己 2026-09-02 拍 🅐** 的理由。
...
> ## 🔴 2026-09-11 codex R1 之後訂正 —— **上面「已退總額 >= 訂單應收總額」那個字面兩半都不精確**
> **(A)「已退總額」我第一版拿錯了尺** ⇒ 我用 `pcm_order_refundable_remaining <= 0`,
> 　 而那一支答的是「**還能再退多少**」,它把 `status='processing'`(**還在飛的卡退**)也扣掉。
> 　 🔴 反例:`total=1000` · `confirmed=400` · `processing=600` ⇒ `remaining=0` ⇒ 判「整筆退了」⇒ 退券,
> 　 **而實際只出去 400**;那 600 後來若失敗,券已經不可逆地還回去了。
> 　 ✅ 改成抄匯流點自己的三段帳本(`20260907140000:165-180`),並補上它的 `v_moved > 0`
> 　 (那一半專門擋**全額折抵的零元單**:`0 >= 0` 會成立)。
>
> **(B)「訂單應收總額」這個詞不成立** ⇒ 實作取的是 `orders.total`,而
> 　 `20260908060000:711` 只在**全數取消**時更新取消欄位,**沒有重算 `total`**;
> 　 有效金額另有 view 在算。
> 　 ⇒ 📌 **所以不能說「重用既有 helper ⇒ 已涵蓋部分取消的應收」** —— 那句話是我加上去的,不成立。
> 　 ⚠️ 而 codex 判它今天**證不到可達的錯退券案例**(現有部分取消 + 扣券前置的限制下)⇒ 列為 nit,
> 　 　 **而不是「沒問題」** —— 它是「今天走不到」,不是「不會錯」。
>
> 🎯 **這兩格合起來的母題**:
> ## **「重用既有的尺」這句話本身不夠 —— 還要問【那把尺回答的是哪一個問題】。**
...
### 為什麼
> 🔴 **本節 2026-09-11 訂正過。** ⛔ ~~「扣券路徑今天已通 ⇒ 名額正在被吃掉」~~ —— 見 §0。
```
🛑 它【不是】在止血 —— 今天券一次都扣不掉(全庫零呼叫端), 名額沒有正在被吃。
✅ 它是在【補一個前置條件】, 而那個前置是 Sean 自己 2026-09-02 拍 🅐 定下的:
     「券的退回機制 = 扣券上線的前置條件」(板列 ⟦b4-COUPONREVERT⟧, 態 doing)
✅ 而它今天卡住兩支 migration:20260901021000 / 20260901030000 的第一道閘查的就是它
     ⇒ 本支不貼, 那兩支【誰先誰後都貼不下去】
🔵 而它要解的那個病仍然是真的, 只是【還沒有分母】:
     後台券清單那句 reverted_at IS NULL 排除的永遠是 0 筆
     ⇒ 📌 扣券真的上線那天, 沒有退回路徑 = 名額只減不增, 而那張表會一路顯示得很正常。
```

### 影響
> 🔴 **字面訂正**:貼這一支**不是**「解除一個活風險」,是**「補一個目前不會發生的前置」**。
```
...
🔴 而它解除了兩支 migration 的封印:
     ...
     ⇒ 它一貼, 那兩支就【變成貼得下去】—— 而那兩支才是讓扣券真的跑起來的東西
   ⇒ 🛑 **所以貼它之前, Q1(呼叫端)必須已經有答案。**
      ⚠️ 而這一句的理由 2026-09-11 換過:
         ⛔ ~~「否則會出現扣得動而退不了的視窗期」~~ —— 那個視窗期今天不存在,
             因為就算那兩支貼了, 也還要有人去接 create_order 那一端才會真的扣。
         ✅ 真正的理由:**那兩支一貼, 扣券就只差最後一根線** ⇒
...
🔴 而 UNIQUE 那一格會咬到讀這份的人:
   coupon_redemptions_one_per_order 是 UNIQUE (order_id)【整表的】, 不是部分唯一
   ⇒ 一張單退券之後, 那個位子仍然被佔著, **它永遠不能再兌換第二次**
   ⇒ 回去的是【那張券的名額】, 不是【這張單的資格】。已寫進函式 COMMENT。
```
...
1. 🔴🔴 **這一格已經從「證不到」升級成「證錯了」,留著不刪。**
   我原本寫:「『扣券路徑是通的』是讀 `prosrc` 字串位置推的 ⇒ 它證的是【碼在】不是【它會動】」。
   ✅ 那句限定**寫對了**,而 🛑 **我還是把結論當成事實往上報了** —— 而它連【碼在】都不是,
   　 因為那些命中**全部在註解裡** ⇒ 連「碼在」都是假的。
   > 📌 **一句誠實的限定詞,擋不住我拿那個結論去做事。**
   > 🎯 **限定詞是給讀的人用的,而寫的人自己會跳過它。**
   訂正後的讀數在 §0。**我仍然沒有跑過任何一筆真的下單或退款。**
