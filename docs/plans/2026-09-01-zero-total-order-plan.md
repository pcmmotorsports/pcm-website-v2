...
> 🔴 **[2026-09-11 訂正]**「今天通了」**不成立** —— 3b(`20260901021000` / `20260901030000`)
> **兩支都還沒貼**(`supabase/APPLIED.tsv` 逐字各 ⇒ 0)。正確的條件句是:**那兩支貼上去之後**那條路才通。
> 🛑 而還有第三道:那兩支自己有 fail-closed 閘(`021000:315` / `030000:379`)——
> 退券路徑 `public.coupon_revert_on_full_refund` 不存在就 `RAISE EXCEPTION` ⇒ **今天貼會當場失敗**
> (Sean 2026-09-02 拍 🅐:「券的退回機制 = 扣券上線的**前置條件**」,`021000:263`)。
> 📌 **⇒ 本 plan 寫的是【片做完之後】的世界,而「今天」這兩個字會被讀成現況。舊字面留著不刪。**
> 而(3b 貼上之後)客人會撞到「付款失敗,請稍後再試」——**而再試一百次都不會成功。**
>
