...
> 🔴🔴 **[2026-09-06 訂正 · 本節 §1.2 與 §1.3 的【結論】已被推翻,而讀數本身沒有錯]**
> 量測正本:`~/pcm-mailbox/量-抽屜正式站-20260906.md`(正式站 + preview 各 4 發,含正對照與射程)。
>
> 🎯 **`router.replace` 從來沒有被丟掉** —— RSC 請求 **6–72 ms** 就發出去了,
> 而**回應要 3.4–8.5 秒**;Next 只在 transition commit 時才寫網址 ⇒ 那段期間網址不動。
> ```
> 正式站落地   3371 · 3378 · 5487 · 6259 ms
> preview 落地 4476 · 7332 · 7619 · 8526 ms
> ```
> ⇒ 🛑 **本 plan 的 §2(兩個候選機制)· §3(v1 與 v2 修法)全部作廢** —— 它們修的病不存在。
> ⇒ 📌 病移到板列 `⟦search-CATSWITCHSLOW⟧`:**效能 + 零載入回饋**,不是競態。
> 🔵 **舊字面全部留著不刪**,讓拿舊結論來搜的人同一發撞到這裡。
...
⛔ ~~**上表把嫌疑縮到「細項那一步」**~~ ⇒ 🔴 **2026-09-06 訂正:那個縮法縮到的是雜訊。**
**只點大分類也一樣慢**(正式站 5487 / 3371 ms · preview 8474 / 7552 ms),**而且更慢**。
⇒ 🎯 差別不在「點了幾層」,在**那一發等了幾秒**;而上表三格的等待時間都遠短於 3.4 秒。

### 1.3 而「開著抽屜時網址不動」是**正常的**,不是病
只點大分類、**不按查看**,等 1/2/3 秒 ⇒ 網址一直是空的;**按了查看才寫**。
⛔ ~~⇒ 📌 **寫入是掛在【關閉】上的** —— 所以「開著時沒反應」不是線索。~~

🔴🔴 **2026-09-06 訂正:這兩句都不成立,而它們錯的方式不一樣。**
· **「按了查看才寫」** ⇒ ⛔ 實測:**點大分類的當下網址就變了**(抽屜還開著,沒按查看)。
  而機制上也不可能:`FilterDrawer.tsx:411` 那顆鈕 `onClick={onClose}`、**零 dispatch**,
  而 effect 的 deps 是 `[cascade, extras, restoreSources, router]` ⇒ **關閉一個都不動。**
· **「等 1/2/3 秒 ⇒ 網址一直是空的」** ⇒ 🔴 **在 3.4–8.5 秒的延遲底下,等 3 秒【必然】看到空的。**
  📌 **那不是觀察,是量程不足** —— 而它讓後面每一輪推理都建立在「東西不見了」上面。
