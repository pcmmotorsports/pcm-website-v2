...
> ✅ **第四輪跑完了 —— 碼階段 codex 審 diff:FAIL, 11 must-fix + 2 nit, 逐條處置在 §14。**
...
🔴🔴 **[2026-09-06 04:0x 訂正 —— 讀數對, 而我把它接到了錯的結論上]**
⛔ ~~下面那段「這是拍板沒有被完整實作」~~ **收回。** 逾時自動取消不寄 = **Sean 2026-09-03 拍過的【乙】**,
逐字寫在**正式庫的 COMMENT** 裡(`20260903040000:100-102`):「`expire_unpaid_orders`(一次上限 **500 張**)【不涵蓋】——
Sean 逐字『**不寄, 只有員工按下取消才寄**』」。
🛑 **我引的是同一天【另一題】的答案**:`project_0903-cancel-email-yes` 答的是「**要不要有這封信**」(甲),
而**【射程】是同一天的乙** —— 📌 **同一天、同一條線、兩題兩答, 我只讀了一題。**
🔴 **而這個錯指向一個會傷到客人的動作**:照我原本寫的修法去拆射程閘 ⇒ **一輪 500 張 ⇒ 上百封不可回收的信**
寄給一批早就忘記這件事的人。⇒ **那段修法方向逐條作廢。**
🔵 **留下來的是那個【客人體驗】問題**:這封匯款信會叫他「5 天內匯」而第 6 天單子安靜消失 ——
**那是一題要端 Sean 的新題**(R3-C3 也指到同一處:提醒信與自動取消要不要寄是同一題), **不是一個沒做完的實作**。

⛔ ~~🔴🔴 **而它是【拍板沒有被完整實作】, 不是設計取捨**~~ **(見上方 04:0x 訂正 —— 收回)** ——
...
# §14 🔴🔴 codex R1(碼階段, 鐵則 12⑤ 那一輪)—— **FAIL:11 must-fix + 2 nit**

> 🔬 **兩道閘的值(艦隊 2026-09-06 新立)**:`^ERROR: …"status":400` ⇒ **0 命中** · `^model:` ⇒ **`gpt-5.6-sol`** · `rc=0` · log **1,016,046 bytes**
> 🔵 **而 codex 自己標了效度限定, 逐字轉**:「Vitest 在載入前即被唯讀沙箱禁止建立暫存目錄, 顯示 `3 failed / no tests`;**這不是 repo 測試紅燈, 也不能當成測試通過**。」⇒ 📌 **它沒有跑測試。**
>
> 🔬 **條數自檢(§10-0 那道)**:**審查者 11 + 2 = 13** · **本節處置編號 = 13**(#1–#13)⇒ **13 vs 13**。

## 14-A 已折進碼(八條)

**#2 / #3 【折】快照 vs 現況** —— 🔴 **本輪最重的一條, 而它打的是我以為守住的東西。**
我的重驗只問「那一列還在不在」⇒ 入列 12,800 之後後台改成 10,000 ⇒ view 仍回 mailable ⇒ **照舊寄出快照的 12,800/0/12,800**;收件人被改成 B 也一樣 ⇒ **信仍寄給快照裡的 A**。
⇒ 🎯 **我把「還該不該寄」與「我的快照還準不準」當成同一個問題, 而它們是兩個。**
✅ port 改成回**現況三值**;use-case 比 `total` / `balanceDue` / `recipientEmail`, 任一不同 ⇒ **不寄**, 標**自己那個碼** `bank_order_snapshot_stale`。
🛑 **不是「用新值寄」** —— 那等於在寄送當下重算一封信的內容, 而那封信的字面是 Sean 逐字核可、由整串 `toBe` 鎖著的。

**#10 【折】主旨沒進那道「全文逐字」的鎖** ⇒ 改核可的主旨, 我那一發照樣全綠。
✅ 而**鎖搬到 adapters 那一側**(`order-email-assembly.test.ts`), 理由是量出來的:
🔬 sweep 送的是 `job.subject`(**enqueue 當下就寫進 outbox 的那一份**)⇒ 在 sweep 那支測試裡鎖它, **鎖到的是 fixture 不是碼**。
🧬 突變驗過:主旨少四個字 ⇒ **1 failed**;還原 ⇒ 5 passed。

**#1 【折】`claimDue` 排除清單沒有它** ⇒ **拔掉 cutoff 停不了線**(已入列的照樣被認領寄出)。
✅ 新增 `allowBankOrderCreated` option + `buildExcludeEventTypes()` **聯集**(不是三元覆寫 —— 覆寫會讓「A 關著、B 開著」把 A 放出來)。
🔵 而那顆 option 是**必填** ⇒ 六個呼叫點當場紅 ⇒ 📌 **那是它必填的理由:一道 fail-open 的閘比沒有閘更糟。**

**#6 【折】route 上膛註解還寫「DB 四支」** ⇒ 45e 是後來加的, 而**重驗就靠它**。少了 45e 照這段上膛 ⇒ 重驗查一支不存在的 view ⇒ 那些列燒完 attempts 進死信。

**#8 【折】bank 狀態沒進「未上膛」log** ⇒ 「正常零封」與「整線沒啟用」印同一個 200。
🔴 **而我補的不只那一行輸出, 還有【觸發條件】** —— 只有匯款線沒上膛時, 少了條件那一行**永遠不會被看到**。📌 **加了輸出而沒加條件 = 那個輸出在【唯一需要它的世界】裡不存在。**

**#11 【折】探針那發突變不是單點** —— 同時拿掉 `IS NOT NULL` / `> 0` / `<= total` 三條。
✅ 拆成 21a/21b/21c 各一條 + 21d 全拿(留著當對照)。
🔴🔴 **而拆完當場發現一個更深的空白**:原 fixture 的 `paid_total` 是**恆 0 的 stub** ⇒ `balance_due` 恆 = `total`
⇒ 📌 **`> 0` 與 `<= total` 那兩條守門【在那個 fixture 裡沒有世界可以測】**, 而我那一發同時拿掉三條, **剛好把這個空白蓋住了**。
✅ 補兩筆:**付清(0)** 與 **多付(負數)**;`paid_total` 改成可設。⇒ 探針 **28 → 44 格**。
🔵 而 21b 的標籤我寫錯過一次(寫「退款單漏進來」而讀數是 1)—— **標籤要由讀數決定**, 已訂正。

**#12 【折 nit】測試 fake 的 `markSkipped*` 預設回 true** ⇒ 非預期呼叫會安靜通過。✅ 改成**預設 reject**, 只有該呼叫它的那一格才換成成功。

**composition 那格(不是 codex 抓的, 是我自己的全套紅)**:`getSweepEmailOutboxDeps` 的回傳鍵被一格**全等**斷言鎖著, 而我加了 `bankOrderMailable`。
✅ 更新期望值**並寫理由**:🔴 **它與 `paidContext` 那一次方向相反** —— 那一支接上去讓客人**收到不一樣的信**;這一支接上去**開始擋東西**, 而漏掉它的症狀是**一封都不寄**。

## 14-B 【折了一半】—— 而另一半要一支新的 migration(#4 / #5 / #7)

三條是**同一個機制**:`skipped_*` 落一列 ⇒ pending view 的 anti-join **status-agnostic 且鍵在 `order_id + event_type`** ⇒ **那張單再也排不進來**。
```
#4  金額暫時錯 ⇒ 標終態 ⇒ 修回正數後仍被擋      ← 本片新造的一族
#5  只有合成信箱 ⇒ skipped_no_real_email ⇒ 補上真信箱後仍被擋   ← 既有機制, 本片多一族列踩它
#7  退版 ⇒ 舊碼撞 default ⇒ 燒完 attempts ⇒ 同上
```
🛑🛑 **而主視窗給的方向【只解一半】, 這一句要寫出來**:
> 逐字:「`dedup_key` 帶 `orders.version` 或帶快照 hash, 否則 anti-join 永久擋住」

🔴 **`dedup_key` 救不了它** —— **anti-join 根本不看 `dedup_key`**(它 `WHERE e.order_id = m.order_id AND e.event_type = 'bank_order_created'`)。
⇒ ✅ 要讓修正後的新快照重排, **anti-join 必須排除 `skipped_*` 那些列** ⇒ **那是一支新的 migration(45f)**, 而它不是我能自己排的。
⚠️ **而在那之前, 現在這個方向仍然是對的**:📌 **少寄一封 < 寄一個錯的金額給錯的人。** 碼裡那一格已經明寫這個已知缺口。

## 14-C 【端 Sean】(#9)

期限句的「(含)」與 cron 判準對不上 —— 🔬 `20260903080000:161-163` 逐字 `o.created_at < now() - interval '5 days'` ⇒ **時間戳比較**
⇒ 9/5 12:00 下單, **9/10 12:00 之後**就可能被取消, 而信讓客人以為整個 9/10 都可以。
🔴 **而它不是我引進的** —— 訂單頁那句話已經這樣寫, 而它旁邊的推導**只證明了「9/10 00:00 還沒到期」**, 卻被用在「9/10 整天」這個更寬的結論上。
📌 **一個正確的推導, 用在一個比它寬的結論上。**
✅ 兩案已進 Sean 明早清單(`Q-匯款到期`:甲 改字帶時刻 / 乙 cron 改日界)。🛑 **我不改那句字面** —— 改了就是我替他決定文案。

## 14-D 【不折, 而寫下來】(#13 nit)

`markSent` 失敗 + 停擺 >24h ⇒ idempotency 到期 ⇒ 真的重寄。
🔵 **這是既有的 at-least-once 風險, 不是本片新造的**;而**本片這封信承受得住收兩次**(它是唯讀資訊:帳號 + 金額 + 期限, 不是動作指令)。
⚠️ 而**那個「承受得住」有前提**:如果哪天有人在這封信裡加一個「點這裡確認」的動作, 這句話就不成立了。

## 14-E 這一輪買到的一句

> **我做了一道「寄送前重驗」, 而它問的是【這張單還該不該收到這封信】——**
> **它從來沒問【我手上這一份還準不準】。**
>
> 📌 兩個問題長得很像, 而只有第二個問得出「後台把金額改了」與「收件人換了」。
> 🎯 判別句:**一道重驗如果只回 yes/no, 它就答不出「值變了」** —— 要值, 就得回值。
