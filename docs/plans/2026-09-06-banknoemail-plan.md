...
> ✅ **`-f8` 04:3x 裁:C1 採納** —— 金額走 `job.payload` 快照, 02:xx 裁的乙(`orders.version`)**收回**
> (handoff 記「裁示早於 C1」)。落地與自審差異在 **§12**;順帶量到的另一件在 **§13**。
> 🛑 **不跑第四輪 plan 審** —— 第四輪 = **碼階段 codex 審 diff**(鐵則 12⑤ 本來就必跑)
> ⇒ 📌 **在那之前, 本檔的狀態是「三輪審 + 一次自審」, 不是「四輪審」。**
...
| 12 | ⛔ ~~**混版信守門(`orders.version` 不一致 ⇒ 不寄重排)**~~ 🔴 **04:3x 收回 —— C1 讓那個問題【消失】, 不是被解掉。**
改成:**金額走 `job.payload` 快照**(既有三封信同形);payload 只放 **`balanceDue` + `display_id` + `created_at`**,
🛑 **帳號常數不得進**(payload 會落 DB)| **只有一次讀 ⇒ 沒有兩次查詢之間** |
| ~~12x~~ | ~~🔴 **混版信守門(`-f8` 03:0x 裁乙, 定案)**~~ —— 讀完 context 之後複讀 `orders.version`,
...
# §12 🔵 C1 落地 —— **「C1 改了哪些節」自審差異**(`-f8` 04:3x 採納, 不跑第四輪 plan 審)

> **裁示逐字**:「C1 採納:金額走 `job.payload` 快照(既有三封信同形), 我 02:xx 裁的乙(`orders.version` 檢查)**收回**,
> 記進 handoff『裁示早於 C1』;約束照你寫的(帳號常數不進 payload;快照下單當下的 `balanceDue` + `display_id` + `created_at`;
> MF8 寄送前重驗問 `balanceDue` 仍在)。」
> 🔵 **第四輪 = 碼階段的 codex 審 diff**(鐵則 12⑤ 本來就必跑)⇒ **不另開一輪 plan 審。**

## 12-A 改了哪些節(逐節, 而不是「都改好了」)

| 節 | 改前 | 改後 | 這一改**取消**了什麼 |
|---|---|---|---|
| **§3-4** | 金額走 `IPaidEmailContext` + 一句「名字是 `Paid…` 而它不看 `payment_status`(量過)」 | 金額走 **`job.payload` 快照** | ⇒ 「型別名字會騙下一個人」那段**整段不需要了** |
| **§5-12** | 混版信守門(`orders.version` 不一致 ⇒ 不寄重排)| **刪**(裁示收回) | ⇒ 🔴 **不是「做了更好的」, 是【那個問題消失了】** —— 快照只有一次讀 |
| **§5**(新)| — | payload 要放 **`balanceDue` + `display_id` + `created_at`**;🛑 **帳號常數不得進**(payload 會落 DB)| — |
| **§5-11c** | 寄送前重驗問 `balanceDue` | **不動**(裁示明寫仍在)| ⇒ 快照解的是「兩次讀不一致」, **解不掉「下單後他匯了一半」** |
| **§6** | 世界 ⑩⑪ + 混版信那格 | 混版信那格**刪**;新增 **快照 vs 現況分歧**那格 | — |
| **§8** | 混版信列在待答(R1 掉的那條)| **刪那一條**, 而**保留它的病歷** | ⇒ 📌 **一條被解掉的待答, 它的病歷要留著** —— 否則下一個人會重新發現它然後重新解一次 |
| **§10-B⑤** | 「`IPaidEmailContext` 沒有 `created_at`」| **失效**(快照自己帶)| — |

## 12-B 🔴 C1 **不**解什麼(三條, 寫出來免得被當成銀彈)

1. **它解不掉「下單之後金額真的變了」** —— 快照是**下單當下**的數字。客人隔天匯了一半 ⇒ 快照仍是舊的
   ⇒ 🛑 **`balanceDue` 那道寄送前重驗非留不可**, 而**裁示也是這樣寫的**。
2. **它解不掉 30 天之後查不到那封信** —— 那是 `⟦b4-NOSENTBODY⟧`, 不同的問題。
3. **它把一個「讀取一致性」問題換成一個「快照新鮮度」問題** —— 兩者都真, 而**後者有解**(重驗), 前者沒有便宜的解。
   📌 **這才是採納它的理由, 不是「它比較簡單」。**

## 12-C ⚠️ 這次自審**證不到**什麼
本節是**我自己**核我自己的差異(`-f8` 裁不跑第四輪)⇒ 🛑 **它不是第二意見。**
✅ **真正的第二意見在碼階段** —— 鐵則 12⑤ 本來就要 codex 審 diff, **那一輪才是這片的第四輪**。
⇒ 📌 **在那一輪之前, 本 plan 的狀態是「三輪審過 + 一次自審」, 不是「四輪審過」。**

---

# §13 🔴🔴 順帶量到的一件, 而它**不屬本片** —— 已開列 `⟦b4-EXPIREDNOCANCELMAIL⟧`

我在 §8 寫「第 6 天單子安靜消失」時, `-f8` 要我開檔驗它成不成立。**成立, 而它比我寫的更明確。**

🔬 **逐條量到的**
1. **逾期自動取消那條路做了什麼** ⇒ `20260903080000:186` 逐字 `cancelled_reason = 'payment_expired'`,
   而全檔 `INSERT INTO` **只有一處**, 目標是 `public.sweeper_heartbeat`(`:220`)。
   🟢 正對照同檔 `UPDATE` ⇒ **4** · 🔵 負對照現造字 ⇒ **0**。
   ⇒ 🎯 **它不寫 `order_cancellations`。**
2. **誰寫 `order_cancellations`** ⇒ `grep -ln 'INSERT INTO public.order_cancellations'` ⇒ **5 支, 全部是【員工取消】那一族**
   (`admin_cancel_order` / `partial_cancel` / `cancel_gate_noncard` / `cancel_reason_neutral` / `reject_reserved_reason`)。
   🔵 負對照現造表名 ⇒ **0**。
3. **取消信的掃描面要什麼** ⇒ `20260905210000:294-296` 逐字 `AND EXISTS (SELECT 1 FROM public.order_cancellations oc WHERE oc.order_id = o.id)`。
4. **而那支 view 的 COMMENT 自己就寫了射程** ⇒ `20260905030000:92` 逐字「未付款、**被【員工】取消**、…」。

🎯 **⇒ 結論:5 天逾期自動取消的單【不會 enqueue 任何取消信】。我那句「安靜消失」成立, 不用改。**

🔴🔴 **而它是【拍板沒有被完整實作】, 不是設計取捨** ——
`memory project_0903-cancel-email-yes` 記的 Sean 逐字決定:
> 「一張【還沒付款】的訂單被我們取消時(**缺貨 / 客人一直沒付款**), **要寄一封信告訴客人**」

📌 **「客人一直沒付款」就是逾期自動取消那條路** —— 它**被他點名了**, 而今天只有「員工手動取消」那一半做出來。
🛑 **⇒ 那是【取消信線】漏掉的一個入口, 而取消信線是我的線** ⇒ 這一條掉在我自己頭上。

⚠️ **而我不在本片修它**:本片是匯款成立信;那一條是取消信線的下一片(`-f8` 已排)。
📌 **兩件事共用一個症狀(客人沒收到信), 而修法在兩條不同的線上** —— 混在一起會讓兩片都變大。
