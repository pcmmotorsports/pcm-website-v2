# `⟦b4-MGR0-RPC⟧`(板列 ⛔ ~~`docs/launch-todo.md:756`~~ ⇒ 🔴 **行號會漂,用錨定位**:`bash scripts/board-row-by-anchor.sh b4-MGR0-RPC`;2026-09-09 當場量是 `:831`)· 管理者權限 TOCTOU · plan(鐵則 8 + 12②③;**只 plan, 零改動**)

> 🔴🔴 **本檔是【現行版】。**前一份 `docs/plans/2026-08-29-mgr0-rpc-toctou.md`(84 行)**已被本檔取代** ——
> 它**不含** §7 的鑽機重現、§8/§9 的兩輪 codex FAIL、§11 的分母查證、§12 的五格規格。
> 📌 **兩份原本互不指涉**,而板列指的是**舊那一份** ⇒ 2026-09-09 `-sync` 三處都補上指標。

---

# 0. 🧑 **給 Sean 的一頁**(2026-09-09 `-sync` 寫;主視窗 A 指定的形狀 —— 他讀這一段就夠)

## 🛑 先講三件他該先知道的,不藏

1. **這份 plan 的關卡 1 對抗審查【被打回兩次】**(codex `VERDICT: FAIL` × 2,§8 §9)。
   兩輪上限用完,依規矩停在【已調查、未施工】(主視窗 A 2026-09-07 裁乙)。
   🔵 而**第二輪抓到的兩件都是我自己的錯**(RPC 有個三值邏輯的洞 · 我的鑽機形狀與真碼不同)——
   那五格缺的規格**已經在 §12 補完了**(2026-09-09),**而補完之後沒有再送審**。
2. **原本推的「丙」已經被否決**(§8-1 §8-2)——我一度說它「純前端片、不受鐵則 12 阻擋」,**那句是錯的而且已經轉述出去**。
3. **今天要你點頭的只有一件:鐵則 8**(新建 DB 物件要你批)。**不是要你現在貼。**

## 白話三句

**① 這件不做,客人會怎樣?** —— **客人不會怎樣。**這個洞在後台員工管理那一頁,
   而**非管理者連門都進不來**(要先過入口閘)。
   🔴 **會怎樣的是你**:出事那天翻稽核紀錄,它會寫「**管理者 A 做的**」,
   而**事發當下 A 已經不是管理者了** ⇒ **你查不出是誰做的,而紀錄看起來很正常。**

**② 做了會動到什麼?** —— 後台的**新增員工 / 改員工資料 / 停用員工**這三個動作,
   從「先問一次再寫」改成「**問跟寫綁在同一筆交易裡**」。
   要**新建三支 DB 函式**(`SECURITY DEFINER` RPC)＋改三處後台碼。
   ⚠️ **不動客人看得到的任何一頁。**

**③ 出事怎麼退?** —— 分兩層,兩層都不用改資料:
   · **碼那一層**:後台切回舊路徑(舊路徑**不刪**,留一段時間)⇒ 一次部署就回去了。
   · **DB 那一層**:那三支函式**只是新增**,舊的寫入路徑一個字都沒動 ⇒
     退的時候**不用 DROP 也不用改任何一列資料**,放著不叫就等於沒有它。
   📎 順序與曝險逐格寫在 **§12-5**。

## 🔬 今天的觸發率(2026-09-09 01:2x 唯讀實查正式庫,不是估的)

```
員工總數 6 · 是管理者 2 · 在職 3 · 🔴 在職管理者 = 1
🟢 正對照:逐列分得出四堆(f/f 2 · f/t 2 · t/f 1 · t/t 1)⇒ 那把尺不是恆真
```
🎯 **要踩到這個洞,需要【兩個管理者同時動】—— 一個在寫,另一個當下把他降級。**
**而今天在職的管理者只有 1 個** ⇒ **正常操作踩不到它。**
🛑 **而這【不是】「所以沒問題」**,兩個理由:
   · 它只說**今天的觸發率**,不說**成不成立** —— 洞是真的,已經在拋棄式 PG 上重現過(§7-1)。
   · **你加第二個管理者的那一天,這句話就失效了** ⇒ 📌 **而不會有任何東西提醒你。**

## 鐵則 8 那四件在哪

| 要什麼 | 在哪一節 |
|---|---|
| 要改什麼 | §2(三處窗口,逐窗量過)＋ §12-1(三支 RPC 的介面與可改欄位) |
| 為什麼 | §1(病)＋ §3(沒有第二道網)＋ §7-1(**在拋棄式 PG 上重現成功**) |
| 預期影響面 | §12-2(權限 · owner · search_path)＋ §12-4(稽核的保證邊界) |
| rollback | §12-5(migration / 應用切換 / 舊路徑退場的順序與曝險) |

## ❓ 要你回的

```
Q:MGR0-RPC(後台稽核紀錄會記錯人)現在怎麼走?
A: 甲 = 批准開工(規格已補完, 而它會再走一次對抗審查才動手)
   乙 = 先不做, 停在「已調查、未施工」, 等有第二個管理者那天再說
```
🔵 **我不替你選** —— 差別是**排程**不是對錯:洞是真的而今天踩不到,
而「今天踩不到」這件事**會在你加第二個管理者那天安靜地失效**。
