...
## 0. 結論 —— 兩句,分開講(2026-09-09 傍晚結案)

1. 🔴 **DB 層**:`anon` 對 `net` 兩表授權全開、鏈路每一格都通、我們的 cron wrapper 的 `Authorization: Bearer <cron_secret>` 會以明文進 `net.http_request_queue.headers`(§1)。
   🛑 **而這一層我們沒有權限收** —— `postgres` 在 `net` 的 `nspacl` 裡沒有 grant option,`REVOKE` 會印 `WARNING` 然後是個空動作(§2)。
2. ✅ **對外層**:**Data API 沒有曝露 `net`** ⇒ 拿公開 anon key 的人打不到(§5,Sean 親自從面板抄的)。

⇒ 🎯 **合起來:今天擋著的是【面板那一格】,不是 DB 的授權。**

### 🔴 而這是我們今天接受的【殘餘風險】,不是「已修復」
那一格**是一個可以被人在面板上點掉的設定**,不是資料庫層的保護。
⇒ 📌 **誰在面板上加了 `net`(或未來某個功能需要它),那個明文 token 就直接見光** —— 而 DB 那一層我們動不了,所以**不會有第二道**。
🛑 **不要把本列讀成「處理掉了」。** 它的狀態是:**風險還在,而擋著它的東西在面板上,不在庫裡。**
...
## 5. ✅ 那一題有答案了:**Data API 沒有曝露 `net`**

🛑 **這一格的來源是【Sean 2026-09-09 下午親自從面板抄的截圖】,不是我們量的。** 逐字轉錄(Supabase → Integrations → Data API → Settings → Exposed schemas,專案 `pcm-website-v2`,分支 `main` PRODUCTION):
```
Exposed schemas:「2 of 3 schemas exposed」
下拉清單完整三項,勾選狀態逐字:
  ✓ graphql_public
    pcm_cron        ← 未勾
  ✓ public
```
⇒ 🎯 **`net` 不在清單裡,連可選項都不是。** `pcm_cron` 也沒勾。
⇒ ✅ **所以 §0 第 2 句成立:外面的 Data API 進不來。**
🔵 **rollback 抄本**:曝露的兩個是 `graphql_public` 與 `public`,順序如上。**Sean 什麼都沒改**,所以這份抄本記的就是現況。

🛑 **這一格的射程**:它答的是「Data API 有沒有把 `net` 端出去」。它**不**答「有沒有別的路徑碰得到 `net`」(§3 掃過,所列搜尋未發現,而那一節自己列了三類看不到的路)。
...
### 路一 · 面板拿掉 `net` ⇒ 🎯 **不用做了**(面板本來就沒有 `net`,§5)。以下保留,供「哪天有人把它加回去」時照著還原
...
### 路二 · 開一張 Supabase support 單 ⇒ **建議,而做不做、誰去做由 Sean 決定**
⏬ **急迫性降一級**(面板既然沒開,外面進不來)—— 照實記,不自己升級也不自己刪掉。
...
## 9. 窗 C 接下來做什麼
- ✅ **本列結案**(2026-09-09 傍晚,Sean 答「面板沒有 `net`」)。窗 C 對這一列**沒有動任何 schema / GRANT / migration**,產出就是這份 plan。
- 🛑 **結案 ≠ 修好**:殘餘風險見 §0,擋著它的是面板那一格設定。
- 🛑 **板子 `docs/launch-todo.md` 凍結唯讀,窗 C 沒有改那一列的狀態** —— 結案的證據在本檔,不在板上。
