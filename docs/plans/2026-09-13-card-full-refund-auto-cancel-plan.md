...
> ## 🟢🟢 **狀態:2026-09-13 Sean 批准(逐字「4 你批」)。零實作維持。**
> ```
> ✅ Q3′ 甲 = 手動那顆鈕也加混軌前置閘(推翻本 plan 傾向)⇒ §3-quater ⑦-ter
> ✅ Q4  = 定案兩次 ⇒ 系統自動走「全額退款訂單自動取消」, 員工手動沿用原句(§7 Q4)
> ✅ Q5  = 表級 CHECK 要做, 含那個「預期的紅」⇒ §3-quater ⑦-quater
> ✅ Q6  = cron 動 vercel.json + 白名單, 一併批了
> ✅ Q-信 = 甲(自動取消信**不印原因**, 而那是**零改動**)⇒ §3-quater ⑦-hexa
> ✅ Q-碼 = 乙(給自動那條一個自己的碼)⇒ §3-quater ⑦-septa
>   🔴 **而主視窗給的落地方式(新碼映成 NULL)走不通, 會撞兩道閘** —— 正解在那一節
> ✅ Q-名 = `full_refund`, **只加 `admin_mark_order_cancelled`** ⇒ §3-quater ⑦-septa ④⑤
> ✅ ⑦-ter 那段文案**已定案 = Sean 的版本 A** ⇒ `docs/specs/2026-09-13-mixed-rail-cancel-block-copy.md`
>   🛑 那份 spec 開頭有他給的**後台文案四條原則** —— 那是**標準**, 後面每一段後台文案都照它。
> 🔴 **而寫它的時候撞到一條新的必做**:那道閘要回一個**可分辨的碼**,
>   否則既有那句**列舉式**的拒絕訊息會少一項而變成假的 ⇒ §3-quater ⑦-ter
> ```
...
🛑 **命名撞字警告(2026-09-13 收尾 grep 抓到)**:`refund_closeout` 這個字**只當那一列
   staff 的 id**。⛔ 我上一輪曾拿同一個字當 `reason_code` 的候選 —— **那會是一字兩用**,
   而本檔已經被「丙 指四件事」教訓過兩次 ⇒ 📌 **碼的名字另外挑** ——
   ✅ **已裁 = `full_refund`**(§7 Q-名)⇒ 兩個字不再撞。
...
```
  ...
  🔴 **而它有一個副作用, 一起寫**:同檔 `:191-197` 有一道斷言逐字
    ...
    (已 apply 的不會重跑 ⇒ 今天不會紅)。
  ⇒ ✅ **已裁 = 做(Sean 2026-09-13 答 Q5 甲)** —— ⛔ ~~本 plan 的立場:採 CHECK~~
    ⇒ 不再是立場, 是定案。副作用(重新 baseline 會紅)**寫成【預期的紅】**, 見 §3-quater ⑦-quater。
  🛑 **而那條退路的話留著**:若哪天有人把 CHECK 拿掉只留註解 ——
    **註解不是機制, 擋不住任何一次 UPDATE**, 不能讓它讀起來像一道閘。
```
...
Q:自動觸發的 cancelled_reason 要不要用一個保留字(而非沿用手動那句)?
  ...
  🟢 **✅ 已答(Sean 2026-09-13)= 甲, 而他給的字是另一句** ——
     定案 = **系統自動那條**用 `全額退款訂單自動取消`, **員工手動那條沿用原句** ⇒ **§7 Q4**。
...
🔵 而那封信**今天就在寄**(手動那顆鈕按下去就會)⇒ 本片不改變那件事, 只改變「誰觸發」。
  ⚠️ **而【信裡印不印取消理由】是另一件事** —— 今天那句自由文字不在白名單 ⇒ 不印
    ⇒ 📌 自動取消那封信會**完全沒有原因** ⇒ ✅ **已裁 = 就這樣**(Sean 答 Q-信 甲)
      ⇒ **§3-quater ⑦-hexa**(而落地方式是【零改動】)。
...
⇒ 🛑 **所以這一格的結論是:今天 0 張, 而【那不是機制上的保證】。**
  ...
  🔴 而**手動那顆鈕今天沒有這道閘** ⇒ ✅ **已裁 = 也要加**(Sean 2026-09-13 答 Q3′ 甲)
    ⇒ 📌 那是一個**改變既有行為**的決定, 而他是在知道這件事之後選的 ⇒ **§3-quater ⑦-ter**。
```
...
#### 🔴🔴 ⑦-ter. **手動那顆鈕也加閘 ⇒ 而【擋住之後員工要怎麼辦】**(Sean 2026-09-13 答 Q3′ 甲)

> **主視窗逐字**:「🛑 **一個擋住人而不告訴他下一步的閘, 會讓他去找繞路。**」

```
🔴 **這是【改變既有行為】** —— 今天那顆「標記取消」鈕, 員工對混軌單按得下去;
   加閘之後**按不動了**。⇒ 📌 **而那一刻他手上有一張真實的單、一個真實的客人在等。**

✅ **所以這一片多一個必做項:那道閘要【說話】, 不是只回一個錯誤。**
   🟢 **三件事全部由 Sean 親筆那一版答掉了**(定案逐字在 spec):
   ① **為什麼擋** ⇒「本訂單採用**複合付款**…系統暫不支援自動計算信用卡剩餘應退金額」
   ② **風險是什麼** ⇒「為避免產生**退款差額遺漏**」
   ③ **他下一步該做什麼** ⇒ ⛔ ~~這一格 plan 現在沒有答案, 候選:找 Sean / 走人工對帳 /
      先把卡那側退乾淨再回來按~~ —— 🟢 **已答**:「請記錄本筆訂單編號,
      **聯繫系統工程人員**協助結案」+ 指路「可至本頁『收款』區塊核對實際收款管道與金額」。
      📌 **三個候選他一個都沒選** —— 他給的是「回報 + 自己先看得到事實」。
   🔴 **而這三件的結構就是他那四條原則裡的【三段式】** ——
     📌 本 plan 當初是**列出三件要答**, 而他給的是**一個結構**。那不是同一個東西:
     清單可以每一項都答到而順序亂掉, 結構不會。

🔴 **文案本身要端 Sean** ⇒ **本 plan 不定字面。**
   ✅✅ **2026-09-13 已定案** ⇒ **`docs/specs/2026-09-13-mixed-rail-cancel-block-copy.md`**
   🔴 **而 B 窗寫的那一版被 Sean 退回重寫, 他自己給了兩版 ⇒ 定案 = 他的版本 A。**
   🛑 **那份 spec 的開頭有【後台文案四條原則】** —— 角色定位 / 三段式 / 術語簡化 / 120 字,
     而那是**標準不是這一段的意見**(後面還有下一步欄、發票、備註要寫)。
   ⚠️ 他那一版用**全形標點**, 照字面原樣 —— 要改先問他。

🟢 **而上面那格「下一步 plan 現在沒有答案」⇒ 已經有答案了, 而是【他自己寫的】**:
   **「請記錄本筆訂單編號,聯繫系統工程人員協助結案」**
   + 指路「可至本頁『收款』區塊核對實際收款管道與金額」。
   🔬 那句指路我當場核過:`payment-list.tsx:234` 有 `<h2>收款</h2>`,
     而該清單 `:59` 逐列印收款管道、`:79` 逐列印金額 ⇒ **指得到一個存在的地方。**

🔴🔴 **而寫文案的時候撞到一件【先前沒有人寫下來】的事 —— 它是接線問題, 不是文案問題**:
```
...
🔴 而那一句**列舉了它的原因**(`cancel-result-panel.tsx:181-198` 逐字):
   「『把這張單結掉』只適用於刷卡收款、而且錢已經全額退完的單,
     並且這張單先前沒有部分取消過。」
  ⇒ 📌 **加了混軌這道閘而不給它自己的訊息 ⇒ 那個列舉少一項 ⇒ 那句話變成假的**
    ⇒ 員工會對著一張「刷卡、已全退、沒部分取消過」的單, 讀到一句說它應該可以結掉的話。
🔴🔴 **而同檔 `:191-196` 有一段 codex R2 must-fix 的留痕**, 逐字:
   「**不可以斷言『這不是系統出錯』**…那支 RPC 的 `P0001` 同時涵蓋四族…SQLSTATE 分不出它們
    ⇒ **一句安慰的話, 在四分之二的世界裡是假的** —— 而它會讓員工不去回報。」
  ⛔ ~~所以主視窗要的第 3 件事(「不要讓他以為是自己做錯了」)在技術上要求那道閘可分辨~~
  🟢 **那個理由消失了** —— Sean 的原則 ③ 逐字「**不要在正文裡辯解**」把那句安慰話刪掉了。
  🔴 **而必做不變, 理由換成上面那條更硬的**:不是為了寫安慰話,
    是**為了不讓既有那句列舉式的訊息變成假的**。
⇒ ✅ **本片多一條必做**:混軌那道閘要回一個**與其他拒絕分得開**的碼
  (自己的 SQLSTATE 或自己的 outcome code), 否則文案交付不了。
```

⚠️ **而這一格有一個【本 plan 答不出來】的前提**:
   📌 **今天有多少張單會當場被這道閘擋住?** ——
   實查說混軌單 = **0 張**(§3-ter-2)⇒ 🟢 **今天上線不會擋到任何人**。
   🛑 **而那正是把閘加進去的最好時機** —— 等到有單被擋住那天才加, 第一個被擋的
     就是一張正在等的真單。
```

#### 🔴🔴 ⑦-penta. **Q4 定案帶出的三件實作細節 —— 而其中兩件是【同一個決定】**

> 主視窗 2026-09-13 交辦三件「一件都不要漏」。**三件我都開檔核過, 結論在下面。**
> 🛑 **而第 2、3 兩件查完發現是同一題 ⇒ 依交辦【列給主視窗, 不自己選】。**

**① 客人看不看得到 —— 🔴 主視窗那個前提【再次】要訂正**
```
> 主視窗逐字:「那句是【客人看得到的】—— 我們今天早先確認過 `cancelled_reason`
>   那一族會印進取消信。」
🛑 **那不成立, 而我第二次核它**(第一次在上一輪, 已 commit 555e18257):
  · `CUSTOMER_FACING_CANCEL_REASONS`(`packages/use-cases/src/order-email-copy.ts:328-334`)
    **只有五句寫死的**, 而「全額退款訂單自動取消」**不在裡面**。
  · `customerFacingCancelReason()`(同檔 `:337-341`)逐字:不在白名單 ⇒ 回 `null`。
⇒ 📌 **DB 存什麼 / 信印什麼是兩件事, 而它們今天【不相等】**:
    DB 存 = 「全額退款訂單自動取消」(Sean 的字, 稽核與後台看得到)
    信印 = **什麼都不印**(那一段整段不出現)
⇒ 🟢 **所以 A3 那道文案閘本來就不必開** —— 不是因為「Sean 自己寫的」,
  而是因為**那句話今天到不了客人眼前**。兩個理由都成立, 而後者才是機制。
⇒ ✅ **而它換來的那一題已經裁了**(Sean 答 Q-信 甲 = 不印, 就這樣)⇒ **§3-quater ⑦-hexa**。
```

**②③ `reason_code` 與拒絕清單 —— 🔴 查完發現是同一個決定**
```
🔬 **先講一個【被機制逼出來的答案】**:自動那條**只能走 `other`**, 那不是選擇。
  `admin_mark_order_cancelled`(`20260903093000:634-647`)那組 CASE 把六個非 other 的碼
  **全部映射成五句固定的客人用語**;只有 `'other' THEN NULL` 那一支
  會走到 `:668` 的 `v_reason_txt := v_detail` ⇒ 📌 **要讓 Sean 那句字真的存進
  `cancelled_reason`, `p_reason_code` 就【必須】是 `other`。**

🛑🛑 **而那直接撞上第 3 件** ——
  主視窗問:「全額退款訂單自動取消」要不要也進那份「員工不准填」的拒絕清單?
  🔴 **今天【不能】, 而理由是機械性的**:
```
  · 那道拒絕(`20260903093000:661-666`)住在 **`IF p_reason_code = 'other' THEN` 裡面**
  · 而自動那條**也是** `other` + **一模一樣那個字串**
  ⇒ 📌 **加了那道拒絕, 系統自己的那一發會被自己擋掉。**
  🔵 對照 `payment_expired` 為什麼沒這個問題:寫它的是 **L3 自動失效那支函式**
    (`…l3a_expire_unpaid_orders_fn.sql:174`, 直接寫欄), **不經過這支 RPC**
    ⇒ 拒絕清單擋得到員工, 擋不到系統。**而我們這一條走的是同一支 RPC。**
```
⇒ 🎯 **所以 ② 與 ③ 是同一題**, 兩個選項:
```
甲 = 自動那條沿用 `other` + Sean 那句
     🟢 本片不動任何既有 CHECK / 白名單, migration 最小
     🔴 **代價①**:「誰做的」只靠**那句 detail 的字面** —— 而那比一個碼脆
       (員工手打一模一樣的字, 系統分不出來)
     🔴 **代價②**:**那句字不能進拒絕清單**(會擋到系統自己)
       ⇒ 📌 **員工可以手打一句看起來像系統做的理由, 而今天沒有東西擋。**
乙 = 給自動那條一個**自己的 `reason_code`**(例 `refund_closeout`)
     🟢 「誰做的」變成一個**碼**, 不是一句字 ⇒ 穩
     🟢 而 `other` 那條路就可以把 Sean 那句加進拒絕清單(系統不再走 other)
     🔴 **代價**:要動兩處值清單 ——
       · 那支 RPC 的 CASE(`20260903093000:634-644`)要加一支
       · `order_cancellations.reason_code` 那個七值 CHECK **要不要跟著加**?
         ✅ **已裁 = 不加**(§3-quater ⑦-septa ②)
         🔬 **而這一格有一個事實**:自動那條**在 `order_cancellations` 寫零列**
           (§3 丙 + Q3 已裁)⇒ 📌 **那個 CHECK 其實管不到自動這條路**
           ⇒ 加不加它是「要不要讓員工也選得到這個碼」的問題, **不是本片的必要條件**。
     🔴 **代價②**:碼是新的 ⇒ 客人那側的 CASE 要決定它映射成什麼
       ⛔ ~~(映射成固定語 ⇒ 信會印那句固定語;映射成 NULL ⇒ 與甲 一樣不印)~~
       🔴🔴 **括號裡那句【後半是假的】, 我後來核出來的**:映射成 NULL ⇒ 撞
         `20260903093000:645-647` 那道「未知取消原因碼」**當場炸**, 根本走不到印不印。
         ⇒ ✅ 正解是**映成一句固定字面**, 而那句**不在白名單** ⇒ 仍然不印。
         ⇒ 📌 逐字在 **§3-quater ⑦-septa**。
```
🛑 **依交辦【不自己選】** ⇒ ✅ **Sean 2026-09-13 裁 = 乙(新碼)** ⇒ **§3-quater ⑦-septa**。
🔵 **而我要指出一件事實**(不是推薦, 是讓題目變簡單):
  **甲 的代價② 就是 Sean 09-12 為 NVB42Z 那封信加白名單時在處理的同一種病** ——
  「一個看起來像系統說的話, 其實是員工打的」。
```
...
```
✅ **自動取消那封信【不印任何取消原因】。** 客人收到「訂單已取消」而沒有一句說為什麼。

🔵 **他是【聽過理由之後】選的, 而那個理由要留著**(主視窗端他時講的):
   📌 **客人剛收到一筆全額退款 —— 他知道為什麼。**
🛑 **為什麼這句非留不可**:一封「沒有原因」的信**看起來像漏了東西**
   ⇒ 📌 下一個人會想「這裡是不是該補一句」⇒ **要讓他看得出那是【裁過的】, 不是漏的。**

🟢🟢 **而機制上它本來就會這樣** ——
   那句不在 `CUSTOMER_FACING_CANCEL_REASONS` ⇒ `customerFacingCancelReason()` 回 `null`
   ⇒ 整段不印。
⇒ 🔴 **所以這一答的落地方式是【零改動】, 不是【已處理】。**
   📌 兩者差很多:「已處理」會讓人去找那段碼在哪;**本片對信件路徑一行都不碰。**
```

#### 🔴🔴 ⑦-septa. **Q-碼 已裁 = 乙(新碼)—— 而主視窗給的落地方式【走不通】, 我核過了**

> ✅ **裁定逐字**:給自動那條一個自己的 `reason_code`。
> 🔵 **Sean 買的理由**(主視窗端他時用的, 逐字留著):
>   **甲 的代價就是他 09-12 為 NVB42Z 那封信加白名單時在處理的同一種病 ——
>   一個看起來像系統說的話, 其實是員工打的。**

**🛑 主視窗交辦 ① 的做法:「新碼映成 NULL, 走 `:668` 讓 Sean 那句進 `cancelled_reason`」**
```
🔴🔴 **那條路【走不通】, 而它會撞兩道閘, 不是一道**
  (`20260903093000`, 我逐行核過):
  · 第一道 `:645-647` 逐字:
      IF v_reason_txt IS NULL AND p_reason_code IS DISTINCT FROM 'other' THEN
        RAISE EXCEPTION 'admin_mark_order_cancelled: 未知取消原因碼';
    ⇒ 📌 **新碼映成 NULL ⇒ 它就【等於】一個未知的碼 ⇒ 當場炸。**
  · 第二道 `:670-671` 逐字:
      ELSIF v_detail IS NOT NULL THEN
        RAISE EXCEPTION 'admin_mark_order_cancelled: 非 other 不得填說明';
    ⇒ 📌 就算閃過第一道, **非 other 的碼不准帶 detail** ⇒ 再炸一次。
  ⇒ 🛑 **`:668` 那條 `v_reason_txt := v_detail` 只住在 `IF p_reason_code = 'other'` 裡面
    —— 新碼【永遠到不了它】。**

✅ **而正解比它簡單, 而且零改動那兩道閘**:
```
  **新碼在 CASE 裡直接映成【Sean 那句字本身】**, 與其他六碼同形:
      WHEN 'xxx' THEN '全額退款訂單自動取消'
  而呼叫端 `p_reason_detail` 傳 **NULL**。
  · ✅ 第一道閘:`v_reason_txt` 非 NULL ⇒ 過
  · ✅ 第二道閘:`v_detail IS NULL` ⇒ 過
  · ✅ `cancelled_reason` 拿到 Sean 那句字
  · ✅ 那句**不在** `CUSTOMER_FACING_CANCEL_REASONS` ⇒ **信仍然不印** ⇒ 滿足 Q-信 甲
```
🔬 **而還有一個【更硬的】理由必須走 CASE 而不是 detail**(同檔 `:158-160` 逐字):
  「冪等回放端拿 `orders.cancelled_reason` 跟**重算的映射**比, `IS DISTINCT FROM` 就 `RAISE`。」
  ⇒ 📌 **`cancelled_reason` 必須等於 CASE 重算得出的值** —— 而固定字面天生滿足它。
```

**② 要不要進 `order_cancellations` 的七值 CHECK ⇒ 🟢 同意主視窗:【不加】**
```
✅ **他的理由我同意**:那個碼的意義就是「系統做的」⇒ 讓員工選得到就毀了它的判別力。
🟢 **而我補一個他沒講的、而且更硬的理由**:
   `20260730130000:440-450` 有一道**逐字比對整條 allowlist** 的事後閘
   (它自己的註解逐字:「原寫法只 strpos 七個值 ⇒ **加入第八個值不會被抓到**」)
   ⇒ 📌 **加第八個值 ⇒ 那支 migration 重新 baseline 會紅** —— 與 §⑦-quater 那個
     staff CHECK「預期的紅」是同一個形狀。
   ⇒ 🟢 **不加 = 連碰都不碰它。** 少一個預期的紅。
🔵 **而這一格成立的前提寫清楚**:自動那條**在 `order_cancellations` 寫零列**
  (§3 丙 + Q3 已裁)⇒ 那個 CHECK **管不到自動這條路** ⇒ 不加不會讓任何事失敗。
```

**③ `other` 那條現在可以加拒絕了 ⇒ ✅ 做**
```
✅ 自動那條不再走 `other` ⇒ 把「全額退款訂單自動取消」加進
   `20260903093000:661-666` 那份拒絕清單**不會擋到系統自己**。
🔴🔴 **而這兩件事【綁在一起】, 那句話要寫死在碼的註解裡**(主視窗交辦, 我同意):
   📌 **這道拒絕之所以能成立, 是因為 Q-碼 選了乙。**
   🛑 **下一個人若把碼改回 `other`, 這道拒絕就會【擋死系統自己】** ——
     而那時候壞掉的長相是「自動取消整條路每一發都 RAISE」, 沒有人會聯想到這道拒絕。
⚠️ **射程照既有那道**:精確、大小寫敏感、不做正規化(同檔 `:164-168` 逐字說明為什麼)。
```

**④ 碼的名字 —— 🔴 我舉的 `refund_closeout` 【不合慣例】, 列候選不自己選**
```
🔬 **七值的命名慣例(我核的)**:`customer_request` / `out_of_stock` / `long_leadtime`
   / `price_change` / `duplicate_order` / `internal_error` / `other`
⇒ 📌 **它們全部在描述【為什麼取消】, 沒有一個在描述【誰做的】或【做了什麼動作】。**
⇒ 🔴 **而 `refund_closeout` 是「動作 / 角色」風** —— 它答的是「誰做的」, 偏離那一族。
   (⚠️ 它是我在上一輪隨手舉的例, **不是一個查過慣例之後的提案**。)
✅ **照慣例的候選**:
   · `full_refund`         —— 最貼那一族(為什麼取消 = 已全額退款);短、無歧義
   · `refund_completed`    —— 同義而多一個狀態味
   · `card_full_refund`    —— 多帶一個軌別;而本片今天只對卡 ⇒ 也可能是多餘的精確
✅✅ **已裁 = `full_refund`(主視窗 2026-09-13 裁, 技術判斷不端 Sean)**。
   **理由就是上面那條慣例** —— `full_refund` = 「因為已全額退款而取消」, 貼那一族。
   ⛔ ~~`refund_completed`~~ 多一個**狀態味**;
   ⛔ ~~`card_full_refund`~~ 的**軌別在本片是多餘的精確**
     ⇒ 🔴 **而多餘的精確會過期**:哪天匯款那條也要自動標, 那個名字就變成假的。
```

**⑤ 🔴 主視窗【沒有列到】的第五件 —— 而它會讓一句註解變假**
```
🔬 `20260903093000:632` 逐字:「步2 輸入驗(**七值映射逐字同 admin_cancel_order**;…)」
   —— 而 `admin_cancel_order` 那一支(同檔 `:125-175`)有**同一張七值表**。
⇒ 🛑 **只在 `admin_mark_order_cancelled` 加第八個碼 ⇒ 兩支不再逐字相同
  ⇒ 那句註解當場變成假的。**
⇒ 📌 而「一句自陳兩邊相同的註解, 而兩邊其實不同」正是本檔一路在抓的那個形狀。
✅✅ **已裁 = 甲(主視窗 2026-09-13)**:**只加在 `admin_mark_order_cancelled`。**
   ⛔ ~~乙 = 兩支都加~~ —— 那等於**讓員工那條路也認得這個碼**
     ⇒ 🔴 **與「不進七值 CHECK」是【同一個決定的兩面】, 方向相反。**
   📌 那個碼的意義就是「系統做的」⇒ **員工那個入口不該選得到它。**

🔴🔴 **而那句註解要改成【刻意不同】, 不是刪掉**(主視窗逐字, 我同意):
```
✍️ 新的註解要說出三件事:
   ① 兩支的七值表**曾經逐字相同**(不要讓人以為它們本來就各自獨立)
   ② 2026-09-13 起 `admin_mark_order_cancelled` **多一個 `full_refund`**
   ③ **為什麼**:員工那個入口不該選得到這個碼
🛑 **為什麼不能只把那句刪掉** ——
   📌 **一句「兩邊相同」的註解變假, 修法是【說出為什麼不同】, 不是把那句話刪掉。**
   刪掉的話下一個人會以為它們**本來就各自獨立**
   ⇒ 而那會讓「**要不要同步**」這個問題**整個消失**。
```
```

#### ⑦-quater. **那道表級 CHECK ✅ 已裁 = 做(Sean 2026-09-13 答 Q5 甲)**

```
✅ **做。** 定義與理由逐字在 §②-bis;它是**唯一 role-independent** 的機制
  ⇒ 連拿 `service_role` 直寫也擋, 而註解 / app 層守衛 / GRANT 全都擋不到。

🔴🔴 **而副作用【已知、已批, 而且要寫成「預期的紅」】**(主視窗逐字):
```
· `20260726120000_m4b_e8a1_staff_table.sql:191-197` 有一道斷言:
  `IF v_cnt <> 3 THEN RAISE EXCEPTION 'staff CHECK 異常 — 應恰有 3 個 CHECK…'`
· ⇒ 加第 4 個 CHECK 之後, **那支 migration 重新 baseline 會紅**。
· 🟢 **今天不會紅** —— 它已經 apply 過, 不會重跑。
⇒ 📌 **那不是壞掉, 是【基準要跟著加一】。**
⇒ ✅ **新 migration 裡要把這句寫進註解**, 而不是等下一個人撞到那行 RAISE 才去猜。
⚠️ 而**改那個 3 → 4 是改一支【已貼過】的 migration** ⇒ 那是另一個決定, 本片不做;
  本片只負責**把這件事寫在它會被看到的地方**。
```
```

#### 🔬 客人看不看得到 `cancelled_reason` —— **一個要訂正的前提**

> 🔴 **主視窗 2026-09-13 逐字**:「而這一格重要, 因為 **`cancelled_reason` 客人看得到**
> (我們今天早先確認過那一族會印進取消信)⇒ **字錯了是寄出去的。**」

```
🛑 **那個前提今天【不成立】, 而我當場核過** ——
```
· 白名單 `CUSTOMER_FACING_CANCEL_REASONS`(`packages/use-cases/src/order-email-copy.ts:328-334`)
  **只有五句、寫死的**:
    `依您要求取消` / `商品供貨中斷,已為您取消` / `交期無法配合,已為您取消`
    `訂單已取消,詳情請洽客服` / `重複訂單,已為您取消`
· 而 `customerFacingCancelReason()`(同檔 `:337-341`)逐字:
    不在白名單 ⇒ 回 `null` ⇒ **信裡那一段不印**。
· 🔬 **而那道白名單正是 09-12 為了 NVB42Z 那封信加的**(同檔 `:317-324` 逐字點名它)。
```
⇒ 📌 **所以 Sean 那句 `全額退款訂單自動取消` 不在白名單 ⇒ 【寄不出去】。**
⇒ 🟢 **「字錯了是寄出去的」今天不成立** —— 而這是好消息:**這一格沒有寄信風險**,
  而 Q4 已經定案兩次(§7 Q4), 不擋實作。
🔴 **而它換來另一件要寫下來的事**:自動取消那封信會**完全不印取消理由**。
  ⇒ ⚠️ **那是不是我們要的?** 客人收到「您的訂單已取消」而沒有任何原因。
    ✅ **已裁 = 甲, 不印**(Sean 2026-09-13)⇒ 見 **§3-quater ⑦-hexa**(含他買的理由)。
  🔵 而若要讓它印, 路只有一條:**把那句加進白名單**(而白名單與 DB 那組 CASE
    有一格測試逐字比對 ⇒ 兩邊要一起改)。
```
...
✅ **Q3′ 已裁 = 甲(Sean 2026-09-13 逐字「1 甲」)**:**手動那顆鈕也要加同一道閘。**
   ⛔ ~~本 plan 傾向乙(不加)~~ ⇒ 🔴 **他推翻了** —— 兩條路一致, 而那是**改變既有行為**。
   ⇒ 📌 **而那帶出一個新的必做項** ⇒ 見 §3-quater ⑦-ter。
```

```
✅✅ **Q4 已定案兩次, 第二次把待確認那格關掉了(Sean 2026-09-13 答【甲】)**:
```
系統自動那條(cron ⇒ RPC)⇒ cancelled_reason = 「全額退款訂單自動取消」  ← 他逐字給的
員工手動按那顆鈕         ⇒ 沿用既有那句「款項已全額退還,收尾把訂單標記為取消」
                            (`cancel-actions.ts:383`, **不動**)
```
🔴🔴 **他買的是【看理由就知道是人按的還是系統做的】** ——
  📌 **所以那兩句字【不可以被後面的人統一掉】。**
  🛑 而「統一文案」正是最容易被順手做掉的事 ⇒ **這句理由要跟著那兩個字面一起活著**
    (寫在 migration 與 `cancel-actions.ts` 的註解裡, 不是只寫在這份 plan)。

---

⛔ ~~以下為第一次定案時的留痕(那一格待確認已經被上面關掉)~~
✅ **Q4 已答(Sean 2026-09-13)—— 而他答的字與選項的描述【不一樣】, 逐字照登**:
```
他的逐字:  2 甲:全額退款訂單自動取消
```
🔴🔴 **⇒ 定案字面 = `全額退款訂單自動取消`**(逐字, 含全形零標點)。
⚠️ **而這一格標【待確認一格】**(主視窗 2026-09-13 提, 他正在問 Sean):
```
· 甲 的描述是「**沿用員工現在手打的那句**」= `款項已全額退還,收尾把訂單標記為取消`
· 而他給的是**另一句**、而且明顯是**系統口吻**(「自動取消」四個字員工不會那樣打)
⇒ 📌 那實際上**介於甲 與乙 之間**:不是保留字(不是代碼), 也不是沿用員工那句。
⇒ 🔴 **待確認的那一格**:這句是**取代**員工手打那句(手動那條也改), 還是**只用在自動那條路**?
```
🔵 **而不論哪一邊, 白名單那一格不變** —— 見下面〈客人看不看得到〉那格。
```

```
✅ **Q-信 已裁 = 甲(Sean 2026-09-13):不印, 就這樣。**
   🔵 他買的理由 + 為什麼那句理由非留不可 ⇒ **§3-quater ⑦-hexa**。
   🟢 落地方式 = **零改動**(本片對信件路徑一行都不碰)。
⛔ ~~以下為開題時的原文留痕~~
🔴 ~~**Q-信(新開, 未裁)**:自動取消那封信要不要**印出取消理由**?~~
   🔬 事實:Sean 那句 `全額退款訂單自動取消` **不在**信件白名單 ⇒ **今天不會印**
     ⇒ 客人收到「訂單已取消」而**沒有任何原因**。
   甲 = 就這樣 —— 不印(與今天其他自由文字理由的行為一致)← 本 plan 傾向
   乙 = 把那句加進白名單 ⇒ 🔴 白名單與 DB 那組 CASE 有一格測試**逐字比對**
        ⇒ **兩邊要一起改**, 而那會動到既有的取消信路徑。
   🔵 完整實查在 §3-quater〈客人看不看得到〉。
   🛑 **為什麼不編號**:本檔已經有一個「Sean 答 Q7 乙」(那是主視窗端題的號碼)
     ⇒ 📌 **再開一個 Q7 就是【丙 指四件事】那個病的第二次** ⇒ 這兩題改用名字。
```

```
✅ **Q-碼 已裁 = 乙(Sean 2026-09-13):給自動那條一個自己的碼。**
   🔴 **而落地方式與主視窗給的【不一樣】** —— 他說「映成 NULL」, 而那會撞兩道閘
   ⇒ 正解是**映成一句固定字面** ⇒ 逐字在 **§3-quater ⑦-septa**。
   ✅ **而 Q-名 也裁完了** = `full_refund`, **只加 `admin_mark_order_cancelled`**
   (主視窗 2026-09-13)⇒ 見下 / §3-quater ⑦-septa ④⑤。
⛔ ~~以下為開題時的原文留痕~~
🔴 ~~**Q-碼(新開, 未裁 —— 主視窗交辦「列出來不要自己決定」)**~~:
   自動那條的 `reason_code` 要走既有的 `other`, 還是給它一個自己的碼?
   甲 = 沿用 `other` + Sean 那句 —— migration 最小;
        🔴 而那句字**不能**進「員工不准填」拒絕清單(會擋到系統自己)
        ⇒ 員工可以手打一句看起來像系統做的理由。
   乙 = 新碼(例 `refund_closeout`)—— 「誰做的」變成碼不是字, 而 `other` 那條
        就能把 Sean 那句加進拒絕清單;代價是要動那支 RPC 的 CASE。
   🔬 **完整依據(含為什麼 ② ③ 是同一題)在 §3-quater ⑦-penta。**
```

```
✅✅ **Q-名 已裁(主視窗 2026-09-13, 技術判斷不端 Sean)—— 兩格都答了**:
   ① **碼名 = `full_refund`**(= 因為已全額退款而取消, 貼那一族的命名慣例)
      ⛔ ~~`refund_completed`~~(多一個狀態味)/ ⛔ ~~`card_full_refund`~~
        (軌別在本片是多餘的精確, 而**多餘的精確會過期**)
   ② **範圍 = 只加 `admin_mark_order_cancelled`**, 並把那句「逐字同」的註解
      **改成【刻意不同】而不是刪掉**。
   🔬 完整依據與新註解要寫什麼, 在 §3-quater ⑦-septa ④⑤。
```
...
· 🔴 **§3-quater 新增的、我沒查 / 未裁的**(三輪審點名, 照實列):
  ...
  ③ ✅ **Q3 已裁 = 加閘, 而且是必要條件**(codex 二輪 must-fix, §3-quater ⑦-bis)——
     ⛔ ~~本行原本寫「仍未裁」~~。✅ **而 Q3′ 也裁完了 = 甲, 手動那顆鈕也加**
     (Sean 2026-09-13)⇒ 連帶的必做項在 §3-quater ⑦-ter。
