...
> 🔴 **新開 Q-名(未裁)**:碼叫什麼 + 要不要也加進 `admin_cancel_order` ⇒ §7
> ```
...
🛑 **命名撞字警告(2026-09-13 收尾 grep 抓到)**:`refund_closeout` 這個字**只當那一列
   ...
   而本檔已經被「丙 指四件事」教訓過兩次 ⇒ 📌 **碼的名字另外挑**, 見 §7 Q-名。
...
✅ **兩條路, 列進 §7 Q-名 一起裁**:
   甲 = **只加在 `admin_mark_order_cancelled`**, 而**把那句註解改掉**,
        並寫明為什麼只有一支有(員工那個入口不該選得到這個碼)← 本 plan 傾向
   乙 = 兩支都加 ⇒ 註解維持真;🔴 而那等於**讓員工那條路也認得這個碼**
        ⇒ 與「不進七值 CHECK」那個決定**方向相反**。
```
...
✅ **Q-碼 已裁 = 乙(Sean 2026-09-13):給自動那條一個自己的碼。**
   ...
   🔴 **剩下的是 Q-名**(碼叫什麼 + 要不要也加進 `admin_cancel_order`)⇒ 見下。
⛔ ~~以下為開題時的原文留痕~~
...
```
🔴 **Q-名(新開, 未裁)—— 兩格一起裁**:
   ① 新碼叫什麼?(🔴 我上一輪舉的 `refund_closeout` **不合七值的命名慣例**,
      那一族全在描述「為什麼取消」而不是「誰做的」)
      候選:`full_refund`(最貼那一族)← 本 plan 傾向 / `refund_completed` / `card_full_refund`
   ② 那個碼要不要**也加進 `admin_cancel_order`**(員工那條路)?
      甲 = 只加 mark 那一支 + 改掉「七值映射逐字同 admin_cancel_order」那句註解 ← 本 plan 傾向
      乙 = 兩支都加(註解維持真, 而員工那條路也認得這個碼 ⇒ 與「不進七值 CHECK」方向相反)
   🔬 完整依據在 §3-quater ⑦-septa ④⑤。
```
