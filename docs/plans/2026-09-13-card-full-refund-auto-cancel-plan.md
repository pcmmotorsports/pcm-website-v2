...
> ✅ Q4  = cancelled_reason 定案字面「全額退款訂單自動取消」⇒ §7 Q4(⚠️ 待確認一格)
> ✅ Q5  = 表級 CHECK 要做, 含那個「預期的紅」⇒ §3-quater ⑦-quater
> ✅ Q6  = cron 動 vercel.json + 白名單, 一併批了
> 🔴 **新開 Q7(未裁)**:自動取消那封信要不要印取消理由?⇒ §7 Q7
> ```
...
Q:自動觸發的 cancelled_reason 要不要用一個保留字(而非沿用手動那句)?
  ...
  🟢 **✅ 已答(Sean 2026-09-13)= 甲, 而他給的字是另一句** ——
     定案字面 `全額退款訂單自動取消`, 逐字與那一格待確認在 **§7 Q4**。
