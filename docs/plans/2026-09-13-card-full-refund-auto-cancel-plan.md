...
> ✅ ⑦-ter 那段文案**已寫成檔待核** ⇒ `docs/specs/2026-09-13-mixed-rail-cancel-block-copy.md`
> 🔴 **而寫它的時候撞到一條新的必做**:那道閘要回一個**可分辨的碼**, 否則
>   ① 既有那句列舉式的拒絕文案會變成假的 ② 「不是你操作錯誤」那句寫不下去 ⇒ §3-quater ⑦-ter
> ```
...
✅ **所以這一片多一個必做項:那道閘要【說話】, 不是只回一個錯誤。**
   訊息要講三件事(缺一件就會有人去找繞路):
   ① **為什麼擋** —— 這張單同時收過卡與非卡的錢, 而系統算不出卡上還剩多少沒退
   ② **風險是什麼** —— 直接標取消的話, 卡上可能還欠客人錢, 而**沒有任何一張表會顯示它**
   ③ **他下一步該做什麼** —— 🔴 **這一格 plan 現在【沒有答案】**, 而它要跟文案一起定
      (候選:找 Sean / 走人工對帳 / 先把卡那側退乾淨再回來按)

🔴 **文案本身要端 Sean** ⇒ **本 plan 不定字面。**
   ✅ **2026-09-13 已寫成檔**(主視窗改派:「你比我適合寫 —— 只有查過那個閘的人
   知道那句話是什麼」)⇒ **`docs/specs/2026-09-13-mixed-rail-cancel-block-copy.md`**
   ⇒ 🟡 狀態:草稿待核, 照 A3 先例(B 窗寫 ⇒ 主視窗端 ⇒ 他點頭才定案)。

🟢 **而上面那格「下一步 plan 現在沒有答案」⇒ 已經有答案了**:
   **「先不要按, 把單號告知系統維護」** ——
   📌 而那不是我新發明的窗口:**「告知系統維護」是這個後台既有的說法**,
   至少三處在用(`cancel-review-section.tsx:113` / `MARK_REJECTED_TEXT` /
   `MARKED_CANCELLED_UNVERIFIED_TEXT`)⇒ 員工聽得懂, 不用 Sean 另外決定一個流程。
...
🔴🔴 **而同檔 `:191-196` 有一段 codex R2 must-fix 的留痕**, 逐字:
  ...
  ⇒ 🛑 **所以主視窗要的第 3 件事(「不要讓他以為是自己做錯了」)在技術上【要求那道閘可分辨】**
    —— 分不開就寫不下那句話, 寫了就是重犯 codex 抓過的那個錯。
