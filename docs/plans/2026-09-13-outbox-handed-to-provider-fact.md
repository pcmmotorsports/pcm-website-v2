...
| **既有五族的行為** | ⛔ ~~**零改變**~~ 🔴 **那句過寬**(codex 2026-09-13 nit,合成驗過):**退休鍵那個【抉擇】確實只有 `bank_order_amount_changed` 在用**,而寄送前那一發寫入是**每一種信共用的新失敗點** —— 它持續失敗 ⇒ 那些信**完全不呼叫 sender**、回收重試燒完 `attempts` 後停在 `failed`。🔵 不會永遠卡 `sending`、不會無限重試(那是既有的失敗處置),**而「零改變」不成立** |
...
## 7-bis. 🔴 實作時被打掉的兩個前提(2026-09-13 落地,codex R1+R2)

> **本節存在的理由**:plan 被批准之後,實作又打掉了它自己的兩個假設。
> 📌 **一份與碼不一致的 plan,比沒有 plan 危險** —— 下一個人會照 plan 推論。

**① ⛔ ~~「既有每一列都是 NULL ⇒ 不需要回填」~~ —— 假的。**
舊碼送出而 `markSent` 落表失敗的列也是 NULL,而它的答案是**「可能送過」**。
⇒ 我先改成「回填 `attempts > 0` 的列」,**而那個判準也是假的**:
`admin_requeue_dead_email` 把 `attempts` 歸零、且不碰 `sent_at` / `claimed_at`
⇒ 🎯 **一列「送過、死掉、被救回來」的舊列,在每一個欄位上都與全新的一模一樣 —— 它身上沒有證據。**
⇒ ✅ **落地版 = 回填【每一列】。** 判準不是「證明它送過」,是**「證明不了它沒送過」**。
   回填之後 `NULL` 只剩一個意思:**出生在本欄之後而還沒被交出去**(可證)。
   ⚠️ 代價:既有列日後快照過期一律不退休鍵(少寄一封)——**而今天那個代價是零**,
   因為唯一會讀這一欄的型別由 migration 前置閘④ 保證一列都沒有。

**② ⛔ ~~「先貼庫、後上碼」就夠~~ —— 不夠。**
舊碼若在**貼完庫、碼還沒上**的那一段送出一封而落表失敗 ⇒ 那一列是 NULL ⇒ 新碼讀成「沒送過」
⇒ 退休鍵 ⇒ **寄第二封**(codex 合成實跑:2 把不同冪等鍵)。
⇒ ✅ **落地版 = 四步**:① 確認那顆 env 沒上膛 → ② 貼庫(含回填)→ ③ 部署碼 → ④ **這時才上膛**。
🛑 **而 ①④ 是【人的順序】不是閘** —— migration 看不到 Vercel 的 env。
它今天成立是因為**那顆 env 從來沒被設過**;前置閘④ 就是在量那個世界還空不空。
