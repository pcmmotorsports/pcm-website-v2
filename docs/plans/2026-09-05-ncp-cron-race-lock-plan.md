...
> ## 🔴🔴🔴 **最終結論(2026-09-05 拋棄式 PG 實測之後):本 plan 的做法【不做】**
>
> 主視窗要我「用實測不用推」把檔頭三問答完。**答完之後這份 plan 的做法被自己的實測推翻。**
>
> ### 實測到的三件(PG 17, 兩條連線, `deadlock_timeout=300ms`)
> ```
> ① A 拿 FOR UPDATE 不提交 ⇒ B 的 INSERT 【真的被擋】(3 秒 lock_timeout 到期)
>    ⇒ 🟢 codex 那條「B 的 FK 要 KEY SHARE ⇒ 會等 A」**成立**, 而我原本的探針預期是錯的
>
> ② 舊順序(A: FOR UPDATE → advisory)+ 真實 trigger 形狀 ⇒ **零死結**
>
> ③ 🔴🔴 **我提的修法(A: advisory → FOR UPDATE)⇒ 死結, 而且是 B 被殺掉**
>    PostgreSQL 印的環(逐字):
>      Process 7976 waits for ExclusiveLock on advisory lock […]; blocked by 7988
>      Process 7988 waits for ShareLock on transaction 769; blocked by 7976
>      CONTEXT: SQL statement "SELECT pg_advisory_xact_lock(pcm_order_lock_key(NEW.order_id))"
>    ⇒ 📌 **A 先拿了 advisory 而在等 B 的列鎖;B 在 trigger 裡等那把 advisory。**
> ```
>
> ### ⇒ **所以這個做法不能做, 而理由是它把傷害搬到更貴的那一側**
> 舊順序下死結不發生;新順序下死結發生, **而 PostgreSQL 殺掉的是 B** ——
> 🔴 **B 是客人的那筆付款。** 舊順序最壞是「錢留在已取消的單上」(可補救、有紀錄);
> 新順序最壞是「**客人的付款交易被資料庫殺掉**」。
> 📌 **⇒ 一個修正競態的做法, 把一個【可補救的錯】換成一個【當場失敗的付款】。**
>
> ### 🛑 而我兩次都推錯, 兩次方向相反
> 送審前我推「不會死結」(codex 說會);實測後發現**舊的不會、新的會** ——
> 📌 **codex 與我都對了一半:環確實存在, 而它是【我的修法造出來的】, 不是原本就有的。**
> ⇒ 🔵 **這一格只有實測答得出來。** 兩個人讀同一份碼, 推出兩個相反的錯。
>
> ### ✅ 而板列要留什麼(病灶仍然在)
> `⟦b4-NCPCRONRACE⟧` **不關** —— 「已取消而錢在裡面」那個競態**沒有被修掉**。
> 本 plan 的貢獻是**排除一條路**, 並留下三個可用的讀數:
> 　① B 會被 A 的列鎖擋住(⇒ 窗口比想像小)
> 　② 舊順序不死結(⇒ 現況沒有死結風險, 不要為了「保險」去動它)
> 　③ **advisory 與列鎖混用時, 拿鎖順序反過來會死結** —— 下一個做法必須避開這一格
> 🔵 **下一條路的方向**(未驗, 寫給下一個人):讓 A 在取消前**重驗一次 `order_payments`**
> 　 並把取消與那次重驗放進**同一句 UPDATE 的 WHERE**(不加任何鎖)——
> 　 那是樂觀路線, 而它擋不住「UPDATE 之後才落帳」那一段。**它需要自己的一份 plan。**

---

> ## 🔴 **(已作廢的前一版結論)codex 關卡 1 之後:這一片【還不能動手】**
>
