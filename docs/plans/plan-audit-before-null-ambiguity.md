...
### ✅✅ 2026-09-08 **①【證到了】—— 而答案是 0, 那推翻我自己寫上板的字面**

> 🔵 我原本寫「我沒有 DB 連線」⇒ **那是我沒試過**。
> `bash scripts/readonly-prod-sql.sh` 從施工窗 worktree **跑得動**(它自己 `cd` 回主樹 source `.env.local`)。
> 📌 **「我沒有連線」與「我沒試過」印同一句話。**

```
🔬 正式庫唯讀(2026-09-08, 零改動)

那兩個 action 的稽核列                                    ⇒ **0 列**
  其中 before ->> 'order_cancelled_outbox_row' 是 null    ⇒ **0 列**

🟢 正對照 A  admin_audit_log 整張表                        ⇒ 119 列   ⇒ 尺量得到東西
🟢 正對照 B  表裡實際有哪些 action                          ⇒ 21 個相異值, 逐個列出
             (order_refund.finalize 14 · order.workflow.update 14 · … · order.cancel 1)
             🔴 **而沒有任何一個是 `email.order_cancelled.*`**
             ⇒ 📌 所以那個 0 不是我的 LIKE 問錯了, 是那兩個動作【從來沒有跑過】
⚪ 負對照   現造 action 名 / 現造字串                        ⇒ 0 / 0
```

#### 🔴 而這推翻我今天早些時候寫進落板文字、而 `ship` 已經貼上板的那句

```
⛔ ~~「這一列不是【上線前修掉就沒事】, 是【已經在寫分不出來的資料】。」~~
```

🎯 **那句是【推出來的】不是【量到的】** —— 我從「三顆 commit 已部署」推出「它已經在寫」。
⇒ 🛑 **部署了 ≠ 跑過。** 而那兩件事我當時沒有分開。
📌 **⇒ 同一個母題今天第 N 次:一個完成式的動詞(「已經在寫」), 而我只量到了它的前置條件。**

#### ✅ 訂正後的嚴重度

```
· 曝光期(碼在線上)   ⇒ 真的, 2026-09-06 起(不變)
· 實際寫下的模糊列數   ⇒ **0**
⇒ 🔵 **「回不去的」那一半【不成立】** —— append-only 裡沒有任何一列需要面對。
⇒ ✅ 修法只要防未來, 不必處理舊資料 ⇒ **這一列確實是「上線前修掉就沒事」。**
```

#### ⚠️ 而三個限定要跟著這個 0 走

```
① 這是【今天】的讀數。那兩個動作隨時可能被按第一次 ⇒ 🔴 這個 0 有到期日,
   而**到期的那一刻沒有訊號** —— 沒有人會來告訴你「第一筆寫下去了」。
② 我只查了 `admin_audit_log` 一張表。**有沒有別的地方也存了那個 before,我沒查。**
③ 整張表只有 119 列 ⇒ 📌 **那與別處記載的「正式庫今天 1 張訂單、0 個真客人」一致**
   ⇒ 🛑 **所以這個 0 主要說的是【系統還沒被真的用過】, 不是【那個修法不重要】。**
   ⇒ 真客人進來之後, 那兩個按鈕會開始被按。
```

### ⛔ 以下為舊字面(零刪除), 已被上面那一節取代
