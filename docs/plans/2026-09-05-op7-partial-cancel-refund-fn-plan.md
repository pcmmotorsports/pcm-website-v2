...
### 📮 拍乙之後【新長出來】的兩題(交主視窗進佇列)

```
Q-取消後反而欠錢:客人買 5,050 免運, 取消一件 51 元的 ⇒ 剩 4,999,
   照規則要補收 100 運費 ⇒ 他反而還欠我們 49 元。這時候怎麼辦?
A: 甲 | 乙
甲 = 就算了, 不跟他要(系統記 0, 不開待退款也不開應收)
乙 = 擋下來, 不讓員工這樣取消(叫他改用別的方式處理)
```
**推薦 甲。** 代價一句:甲 我們少收那幾十塊,而**乙 會擋掉一個合理的取消動作**,
而員工當下多半只是想幫客人退一件東西。

```
Q-手動單的運費:員工手動建單時運費是他自己打的(可以是任何數字)。
   這種單部分取消時, 運費怎麼算?
A: 甲 | 乙
甲 = 沿用他當初打的那個數字, 不重算
乙 = 也照顧客站的規則重算(滿 5,000 免運)
```
**推薦 甲。** 代價一句:甲 那個數字可能不合規則,而**乙 會把員工當初刻意打的運費改掉**
—— 他打那個數字通常是有原因的(談好的、老客戶、貨到付款)。
