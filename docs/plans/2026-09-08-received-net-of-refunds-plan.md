...
## §R 🔵 主視窗 `-1a` 2026-09-08 裁了三格 —— 而其中兩格【不是新題】

```
① 「兩處都改」⇒ 照做, 而這不是新題。
   🔬 Sean 拍乙的原句括號逐字:「(而看不出收過多少)」
   ⇒ 📌 他【已經知道】會失去那個資訊, 而他還是選了乙 ⇒ 兩處都改 = **照拍板**。
   🔴 只改頭條才是推翻拍板 —— 它製造同一頁兩個「已收」不一樣, 那正是本 bug 的形狀。
② 付款卡那格 ⇒ 裁【一起改】。理由:同一個詞在同一頁不能有兩個意思。
③ 淨額可以是負的 ⇒ 裁【顯示負數, 不夾住】。
   逐字理由:夾成 0 會把「我們多退了 500」藏起來, 而那是錢。
🛑 ④「已收足」吃未扣退款的 kind ⇒ **那是真 bug 不是判斷題, 修它。**
```

### 🔬 ③ 那個「今天有幾張是負的」我量了 —— **0 張, 而分母是 4**

`-1a` 要求端 Sean 之前先量。**唯讀查正式庫**(零寫入, 已掃過無 ERROR 行):
```
① 負淨額張數(收款 − 已退 < 0)     0
🟢 正對照 訂單總數                  4
🟢 正對照 有退款的張數(已退 > 0)   2
...
⚪ 負對照 已退為負的張數(恆應 0)   0
```
🛑 **那個 0 是真的, 而它從【4 張單】來的** ⇒ 📌 **這不是「這個問題不存在」, 是【還沒有材料】。**
   ⇒ 兩個正對照(有退款 2 / 有收款 2)證明**尺碰得到那個欄位**, 而 4 張單裡沒有一張退超額。
   ⇒ ✅ 照 `-1a` 的判準:**0 張 ⇒ 寫進 plan、量法附上**, 不驚動 Sean。

**量法(下一個人自己重跑, 不要抄這個數字)**:
```
🔴 我的唯讀角色【叫不動】 pcm_order_refundable_remaining
   ⇒ `permission denied for function`(它是 SECURITY DEFINER, 只 GRANT service_role)
   ⇒ 📌 **跑不動是【對的】, 不是壞掉** —— 而畫面那條路走 service_role, 它叫得動。
✅ 所以我【自己算】, 三段逐字對齊 `20260820100000:224-262`:
   order_refunds(processing + confirmed)
 + order_refunds(failed + manual_failed + 更正成 money_moved)
 + order_manual_refunds(voided_at IS NULL)
⚠️ 而「自己算」這件事**只限這一發唯讀查詢** —— 碼裡不准這樣做
   (守門 `refund-remaining-single-source.test.ts:1085-1091` 擋的正是 TS 層自己聚合)。
```
