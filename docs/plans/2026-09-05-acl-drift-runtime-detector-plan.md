...
> ⛔ ~~**本檔零 apply、零新 migration** —— 它只規劃。~~
> 🔵 **[2026-09-05 11:xx 訂正 —— 片一已經寫出來了]** `supabase/migrations/20260905140000_m4b_acl_drift_digest_table.sql`。
> 🔴 **而本 plan 的下列字面【已經被實作推翻】, 照它做會做錯**(codex R2 抓到 plan 與實作互相矛盾):
>   · ~~「七族」~~ ⇒ **八族**(`DEFACL` 是 plan 寫完之後才加的);當場量 1346 列。
>   · ~~「有沒有人手動動過權限」~~ ⇒ **兩個取樣時點【之間】的狀態差**。
>     10:00 GRANT、11:00 改回 ⇒ 兩次 00:00 快照相同 ⇒ 這支永遠不會叫。
>   · §5 rollback ~~漏了兩支函式~~ ⇒ **完整版寫在 migration 檔頭**,以那份為準。
>   · §6「成本沒量」⇒ **量了**:對正式庫唯讀連跑三發 **108 / 87 / 84 ms**。
>   · §4.3 那個沒拍板的岔路(repo hash 怎麼給 DB 讀)⇒ **片一完全繞開它**,留給片二。
> 📌 **以 migration 檔頭為準,不以本檔字面為準。** 本檔保留是為了留下當時的推理。
