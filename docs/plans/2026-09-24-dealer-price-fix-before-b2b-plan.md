...
- 商品頁取經銷價失敗時改顯示一般價，而結帳收的是經銷價（`apps/storefront/src/app/products/[slug]/page.tsx:204`、`ProductInfo.tsx:193`）。這是既有問題，Codex 實作審查 R1 指出、R2 同意不擋本次。**要在第一個經銷會員開通前另外修**：取不到就顯示「價格暫時無法取得」，不顯示一般價。
