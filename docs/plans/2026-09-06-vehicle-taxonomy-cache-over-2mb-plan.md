...
🟢 而車輛對照表【那一包被快取的東西】= §7.1 量的同形樹 365,774 bytes (0.36 MB) ⇒ 在 2 MB 之內
   ⚠️ §7.1 自己標的射程:那是【用 SQL 重建同一個形狀】量的, 不是 JS 那一棵
⛔ ~~而 wrs 2026-09-08 全撈序列化 1,062,838 bytes (1.01 MB) 也在 2 MB 之內~~
   🔴 **這一句我引錯了(2026-09-09 自己抓到)** —— wrs 那個數量的是【餵進去的那張表】,
   而 **wrs 本人逐字寫過「那對本列零判別力」**。⇒ 📌 **我把一個被標成零判別力的數字
   拿來當佐證, 而它剛好指向我要的方向。**兩個數差 3 倍, 因為它們是【兩個不同的東西】。
...
🔴 **本節整段作廢 —— 見檔頭訂正與 §7.2 §7.3。那 2.68 MB 是商品清單頁的, 不是車輛對照表的。**
...
**🔵 而我手上的四發讀數 —— 讀它們要很小心, 見下面那一格的訂正**:`/products` 那支儀器印過
```
...
**🛑 ⇒ 這一節的主結論(而不是註腳)—— 而它【不是】「有反面證據」**:
```
⛔ ~~那四發是【對 c′ 不利】的證據~~
✅ 那四發對 c′ 是【零判別力】—— 兩件不一樣, 而寫成前者會讓讀的人
   以為我們已經有反面證據, 而我們一發都沒有。

理由:那四發的 tax= 全部是【快取命中】(26-45 ms), 而 c′ 只在【cold】
      那個世界才有意義(862-1215 ms) ⇒ 兩邊沒有交集。

⇒ 所以現在的狀態逐字是:
   · 【命中】那個世界:車款樹清楚不是長桿(四發同名次)—— 而 c′ 不治這個世界
   · 【cold】那個世界:六件事各多久, 【零發讀數】
   · 唯一支持 c′ 的讀數(130 次 cold, 862-1215 ms)量的是【它自己多久】,
     不是【它是不是最慢的那一件】—— 那兩件事在 log 上長得一樣
🔴 ⇒ c′ 值不值得做, 今天【兩個方向都沒有證據】, 不是「證據說不值得」。
```
