...
> 🛑🛑 **先講一句對我自己的訂正, 因為它比下面的內容重要**:
> **這件事【不是我發現的】—— `§7.2` 與 `§7.3` 在 2026-09-06 就寫了**,
> 逐字「**本檔的標題是錯的:那棵樹不是 2.68 MB, 是 0.36 MB**」與「**那 2,679,379 bytes 是【別人的】**」,
> **而 §7.3 用的方法跟我今天用的是同一個**(讀那行 log 裡的 cache key)。
> 🔵 **我今天真正加的只有兩件**:①拿**正式站近 7 天**的資料重驗一次(方法相同、時點不同)
> ②把它**搬到檔頭**、並寫進**板列**。
>
> 🔴🔴 **而真正的病在這裡, 它才是要記的**:
> ```
> 09-06  §7.2 §7.3 寫下訂正 —— 而它住在一份 300 行 plan 的【中間】
> 09-08  -ship 依【板列列頭那句】把這一列判成 ⟨擋⟩(擋上線)
> 09-09  我讀到 §7.2 時, 那個 ⟨擋⟩ 已經掛了一天
> ```
> 📌 **⇒ 一個正確的訂正, 寫在正確的檔案裡, 而【判的人讀的是另一句】** ——
> 　 板列的列頭沒有跟著改, 而**列頭就是派工與判擋的人讀的那一句**。
> 🎯 **⇒ 所以修法不是「寫得更清楚」, 是【訂正要走到它推翻的那一句旁邊去】。**
> 　 (同族:memory `feedback_a-correction-living-in-the-middle-of-a-row`。)
...
**🔴🔴 而這不再是「一個旁證」—— 目前【所有】量到的讀數都說同一件事, 三發同方向**:`/products` 那支儀器印過
```
tax=26  cats=11  brands=12  garage=2  page=3081  total=3107
```
⇒ 在那一頁, **車款樹是 26 毫秒, 而最慢的是 `page=` 的 3,081 毫秒**。
而 2026-09-09 讀正式站 `dpl_HYRP…` 那三發(`⟦search-TAXONOMY2MB⟧` 那一發順手讀到的)逐字:
```
tax=36ms  cats=25ms  brands=26ms  garage=5ms  page=328ms
tax=45ms  cats=14ms  brands=26ms  garage=3ms  page=561ms
tax=37ms  cats=18ms  brands=20ms  garage=7ms  page=852ms
```
⇒ 🎯 **四發讀數, 四次都是同一個名次:`page=` 最慢, `tax=` 在所有項目裡數一數二快。**

**🛑 ⇒ 這一節的主結論(而不是註腳)**:
```
目前所有量到的讀數都說【車款樹不是長桿】;
而唯一支持 c′ 的那個讀數(首頁 130 次 cold, 每次 862-1215 ms)
量的是【它自己多久】, 不是【它是不是最慢的那一件】。
🔴 那兩件事在 log 上長得一樣, 而它們決定 c′ 值不值得做。
```
⚠️ **而上面四發的射程要跟著走**:四發**都是快取命中**的 `tax=`(26-45 ms);
　 c′ 針對的是 **cold** 的那 130 次 —— **cold 的時候別的五件多久, 一發都沒有人量過。**
　 ⇒ 📌 **所以「車款樹不是長桿」目前只在【快取命中】那個世界證到, 而 c′ 只在【cold】那個世界才有意義。**
　 **兩邊剛好沒有交集 —— 那正是為什麼這一題現在答不了。**
