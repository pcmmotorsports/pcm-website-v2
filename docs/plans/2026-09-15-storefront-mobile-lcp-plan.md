...
> 設計窗。主視窗 pcm-website-v2-b7 派工。鐵則 8(動 `app/layout.tsx` 全站字型策略、`next.config.ts`、可能升 Vercel 方案)⇒ **本檔批了才開工,只寫 plan、不寫碼**。
...
- 這份要 Sean 決定兩件事(§8):
  ...
  - **商品圖**:要用 Vercel 的自動縮圖,**得先把 Vercel 升到 Pro(每月 US$20)**。Sean 07-25 拍過「正式對外開賣前再升」,而網站 09-15 已經正式上線 —— 所以這題要重問。
...
| 事實 | 出處 |
|---|---|
| **Vercel = Hobby(免費)**;Hobby「restricted to non-commercial personal use only」 | memory `reference_pcm-platform-plans-vercel-hobby-supabase-pro`;`docs/runbooks/2026-08-18-admin-session-compromise.md:54` |
| **Sean 2026-07-25 拍 B:先不升 Pro,等「正式對外開賣前」再處理**;09-15 網站已正式上線 | 同 memory;memory `project_0915-site-launched` |
| Vercel Image Optimization:Hobby 含 **5K transformations / 月、300K cache reads、100K cache writes**;超量 ⇒ **新圖回 402 失敗(破圖)**,舊的快取照常 | https://vercel.com/docs/image-optimization/limits-and-pricing(last_updated 2026-08-11) |
...
- 成本(**估算,不是帳單**):
  - 前提 🔴:**要先升 Pro**。Hobby 每月 5K transformations,25,402 件 × 至少 1 種寬度就超過 ⇒ **超量的新圖直接破圖(402)**。
  - Pro 上的用量:假設每件 2 種寬度 ⇒ 最多 **50,804 transformations** ⇒ × $0.05 / 1K ≈ **$2.54**(只在 cache MISS 算,之後命中快取不再算)。
  - cache writes:假設縮完平均 30 KB(**未量,假設值**)⇒ 約 4 單位 × 50,804 ≈ 20 萬單位 ⇒ × $4 / 1M ≈ **$0.81**。
  - ⇒ 兩項合計約 **$3.4 / 月級**,落在 Pro 內含的 **$20 額度**裡;**真正的新增成本是 Pro 平台費 $20 / 月**。
- 碰到:`apps/storefront/next.config.ts`(鐵則 8)、`ProductImage.tsx`、`ProductGallery.tsx`、`ProductCard` 相關測試;**Vercel 方案**。
- rollback:`next.config.ts` 設 `images.unoptimized: true` 或 revert;方案可降回 Hobby(`/docs/plans/pro-plan` Downgrading 一節)。
...
### 推薦:**A(Vercel Image Optimization)**,前提是 Sean 同意升 Pro
- 理由:一次涵蓋全部 17 個 host、零跨 repo、零 schema;Pro 內含額度估算足夠(§4-A);順便解快取時效。
- 若 Sean 暫不升 Pro ⇒ 先做 **D** 當過渡(只救 Shopify 那 21%),其餘等升級。
...
- 費用:商品圖 A = Vercel Pro $20 / 月(並同時解掉 Hobby「禁商業用途」的既存合規風險,memory 同條)。
...
3. 商品圖:preview deployment(Pro)上跑目錄頁 Lighthouse,圖片傳輸量要從 4.3 MB 明顯下降;抽 17 個 host 各 1 張確認沒有破圖(特別是 402)。
...
Q2:商品圖要不要讓 Vercel 自動縮圖?(要先把 Vercel 升 Pro,每月 US$20)
A:  甲 升 Pro、做自動縮圖(推薦)—— 所有品牌的圖一次變小;順便解掉 Hobby 不能商業使用的問題
    乙 先不升,只把 Shopify 那 21% 的圖縮小(免費、純程式)
    丙 都不動
    ⚠️ 07-25 你拍過「正式對外開賣前再升 Pro」,而 09-15 網站已正式上線 —— 所以重問。
```

## 9. 估時
- 字型甲:~2 小時(改 layout / tokens + 截圖比對 + 本機 before / after)
- 商品圖甲:~3 小時(next/image 接線 + remotePatterns + preview 驗 17 host)+ Sean 升級方案的時間
