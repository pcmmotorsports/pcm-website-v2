...
## 附錄 · 三代收窄片(`20260905190000`)的兩個【沒選】的做法,與為什麼

> 2026-09-05 · R3(adversarial-reviewer opus / 假設審查角度)F7 與 F8 提出。
> 🔴 **它們不是 findings 的修法,是【這一片該不該長這樣】的替代案** ——
> 而 **兩份 plan 在此之前零處評估過它們**(R3 逐字:「查無:兩份 plan 零處評估過」)。
> 🎯 **所以本節的價值不是結論,是【把沒被想過的那兩格填上】。**

### 甲(**已採用**):加一條 `customers_update_service_role` policy
上線的就是這個。**今天更小、可逆**(一條 policy,DROP 就回到今天)。

### 乙(**未選**):後台改客戶基本資料改走一支 `SECURITY DEFINER` 函式
抄 `admin_set_customer_tier` / `admin_adjust_wallet` 的形狀,**本片整支不需要**,
而且**順便補上稽核**(那兩支都有 `admin_audit_log`,而基本資料修改**今天沒有稽核**)。
🔴 **未選的理由(主視窗 `-f8` 2026-09-05 裁)**:
- 它要**多一支 SECURITY DEFINER 函式**(= 一個新的 DB 物件、新的 `EXECUTE` 授權面)**加上 TS 那半要改**
  ⇒ 比「一條 policy」大一級,而**今天要的是把 harden 的前置鋪好**,不是改設計。
- 甲**可逆**(DROP POLICY);乙上線之後,呼叫端已經改成呼叫 RPC ⇒ **退回去要退兩邊**。
🔵 **而乙的那個附加價值是真的**:**基本資料修改沒有稽核**這件事,甲不會解決它。
⇒ 📌 **那是另一列該開的事**,不是本片縮不縮的理由。**寫在這裡免得它跟著本片一起被忘掉。**

### 丙(**未選**):不獨立一片,直接併進 `⟦b9-RLSHARDEN⟧` 那一刀
**少一次貼、少一次記帳。**
🔴 **未選的理由**:那一刀本來就是全場最大的一次改動,**把前置塞進去會讓它的
「出事了是哪一格」變難答**。⇒ 📌 **前置與那一刀分開,正是為了讓 harden 當天
如果壞了,可以先問「是前置沒鋪好,還是這一刀本身」。**
⚠️ **而丙的代價是量得到的**:多一次貼 = 多一次 Sean 的手 + 多一列帳本。**它不是零成本。**
