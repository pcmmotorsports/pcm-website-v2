...
# ✅ §12 · **第三版(動手用的那一份)** —— 只改 R2 點名的那幾段

> ⛔ **§10-2 整段作廢**(兩個條件都是死的, 見 §11-1)。§10-1 的「union 進 `categories=`」**保留**。
> 🔵 本節是**動手時照著寫的那一份**;前面 §0-§11 是演進史, **不要照它們做**。

## 12-1 判別「使用者自選」的**最終形狀**

```
① lastWrittenCategoryRef 初值【用進站網址播種】
   在 :152-155 的 !initialized.current 區塊裡:
     lastWrittenCategoryRef.current = parseCategoryFromUrl(params, restoreSources.categories)
   ⇒ 沒播種的話, 第一波 derived "X" !== null ⇒ 誤判自選(R2 must-fix 2)

② 一律先正規化成 null:  const cat = category ?? null;
   ⇒ 否則 undefined !== null 恆真, cascade 為空的每一波都被判自選(R2 must-fix 3)

③ 🔴 null 一律【不算】自選
   ⇒ 這一條同時擋掉「清除全部」那一波(R2 must-fix 4:本案自己造出的膠囊復活)

④ 還原波判準【不用 pendingRestoreRef】(它在 :180 已被設 false, 見 §11-1①)
   改用:filterKey 由【空】變【非空】 且 新值 === 網址解得出來的那顆
   ⇒ 那正是「state 剛追上 URL」的定義, 而它在寫入點還讀得到

⑤ 🔴 保留既有的 categoryAxisSuppressed 等值早退, 不被新判別法整個取代(R2 must-fix 4)
```
**寫入**:`params.set(CATEGORIES_PARAM, union(現有, cat).join(','))`;**不碰 `category=`**;**不 delete 任何東西**。

## 12-2 🔴 測試怎麼寫 —— **先寫【該紅的世界】, 綠了再寫正向**(主視窗指定的順序)

**第一個世界(必須殺得死「判別法恆真」)**:R2 指出既有 ㉓㉕ 殺不死它, 因為它們的 union 是 **no-op**。
⇒ **要造的世界 = 還原波, 而還原出來的那顆【不在】`categories=` 裡面**:
```
setUrl('?categories=A%2CB&category=<第三顆>&page=2')
三 render idiom 照 :640-647(㉖/⑭):initialProps { category: null } → rerender 成還原出來的那顆
斷言  expect(hoisted.replace).not.toHaveBeenCalled()
恆真的實作 ⇒ 寫成 A,B,第三顆 且 page 被刪 ⇒ 🔴 紅
```
**第二個世界(正向, 照 :575-589 ㉔ 的形狀)**:
```
?categories=A%2CB → 三 render 讓還原窗口先消化 → 把 cascade 換成【第三顆】
斷言  qs(url).get('categories') 逐字 === 'A,B,第三顆'
      qs(url).get('category')   === null        ← §10-1「不碰 legacy 單槽」
```
🔴 **斷言一律走 `qs(url).get(...)`, 不比對解碼後的字串**(檔頭 `:99-103` 已警告逗號會被編成 `%2C`)。

## 12-3 驗收表(取代 §10-3 列 3/6/7)

| 列 | 改成 | 為什麼 |
|---|---|---|
| 3 | **指名格 × 指名突變**:12-2 第一個世界那一格, 對「判別法恆真」這個突變**必紅** | 原本那條沒有任何一格殺得死(R2) |
| 6 | 改成可判定的不變式:**「清除全部那一波, hook 不得送出任何 `replace`」** | 原本要驗兩個 `replace` 的落地順序, 而 router 被 mock ⇒ **根本沒有順序**(R2) |
| 7 | **指名格 × 指名突變 ⇒ 必紅**(不寫「既有 25 格全綠」) | 不改這片也全綠 ⇒ 零判別力(R2) |
| 新 | **回退觸發訊號**:深連結帶 `?page=N` 進站後 **page 消失** ⇒ 那就是要回退的訊號 | 本案的回歸是**安靜的**, 沒訊號就沒有人會啟動 rollback(R2) |

## 12-4 仍未驗(動手時逐條驗)
`useDeepLinkRestore` 會不會從 `categories=` 還原 · `categoriesFromParams` 的白名單過濾 ·
`cascadeFilterReducer` 的 toggle 字面(§10-5 `f`)· R1 的 `c`(已繞開設計)`e` `f`。

---

# §13 · 實作完成 + 真瀏覽器實走 —— **五格過, 而撞到一個【與本片無關】的既有異常**

## 13-1 ✅ 實作(只動 `use-catalog-filter-url-sync.tsx` 一支檔)
`lastWrittenCategoryRef`(**只在 `router.replace` 之後寫**)+ 初值用進站網址播種 + `toUrlCategory` 正規化
+ 四條件判別(`cat !== null` · 非上次寫出去的 · 非網址還原出來的 · 且 `categories` 存在)⇒ `union` 進 `CATEGORIES_PARAM`。
🔵 **`typecheck` 當場抓到一個真錯**:`parseCategoryFromUrl` 回的是**物件**不是字串, 我兩處拿它跟字串比
(TS2322 / TS2367)⇒ 📌 **那一紅是好的:兩個「分類」在型別上本來就不是同一種東西, 而我用同一個名字想它。**

## 13-2 🟢 真瀏覽器實走(鑽機重起後, 樹 = 本片)
```
驗收①④  ?categories=操控部品,車架 (6 件/2 顆)
         → 點「碳纖維部品」 ⇒ 網址 +碳纖維部品 · 共 15 件 · 3 顆
         → 再點「腳踏後移」 ⇒ 網址 +腳踏後移   · 共 19 件 · 4 顆   ← 第一顆【沒有消失】
驗收③    刪一顆膠囊 ⇒ 網址少一顆、膠囊 3, 等 1.5 秒【沒有復活】
驗收⑥    「清除全部」⇒ 網址清空 · 膠囊 0 · 108 件全回來, 等 2 秒【沒有被寫回】
```
🔵 **而「清除全部」我第一發【點錯東西】** —— 頁面上有**兩顆**同名按鈕(`fs-clear` 在側欄 · `ac-clear-all` 在膠囊列),
我抓到第一顆 ⇒ 三個數都沒動 ⇒ **看起來像「清除全部壞掉」**。**今晚第二次點錯東西。**

## 13-3 🔴 **而實走時撞到一個異常, 我【沒有】把它算成本片的成果或缺陷**
```
操控部品 單獨   共 4 件
車架     單獨   共 2 件
兩個一起        共 4 件   ← 🔴 聯集少了 2;換順序打(車架在前)仍是 4 件, 膠囊兩顆都在
```
🛑 **這與本片無關** —— 本片只動**寫入**那一側, **一個字都沒動讀取**;而上面三發**全是直接打網址**, 沒有經過我的碼。
⚠️ **而我【也不能說它是既有缺陷】** —— 因為 §9-3 我在**鑽機重起之前**用同一種形狀量到 `2 + 4 = 6`(聯集成立)。
📌 **兩次的差別是【鑽機重起換了隨機種子】** ⇒ **跨重起的件數不可比**, 而我一度差點拿它去推論。
🔵 **我沒有繼續追** —— 追它要開 DB, 而我試了兩把尺(`psql` 連不上 · PostgREST 那個 port 回 404)
**兩把都沒接上, 連對照組都查無** ⇒ 那是**尺沒接上**不是查無 ⇒ **停手, 不猜工具設定。**
⇒ 🛑 **這一格記為【未解, 待查】** —— 要判它是不是缺陷, 得在**同一個種子**裡量單獨值與聯集值。

## 13-4 🔴 一條**沒有測試守著**的分支(誠實記, 不假裝)
`cat === null` 不算自選 —— 我把它拿掉跑一輪 ⇒ **29 格全綠**。
🔬 探針斷言印出 `{ n: 0, first: null }` ⇒ **那一波根本沒送出任何 `replace`** ⇒ **擋住它的是更前面的等值早退, 不是本條。**
⇒ 📌 **那不是「測試沒寫」, 是那條路在【router 被 mock】的世界裡到不了**(R2 逐字說過同一件事)。
✅ 已把這一段**寫進碼的註解**, 並在此明寫:**它的驗證欠在真瀏覽器那一層。**

## 13-5 讀數總表
```
測試      products-url-state.hooks.test.tsx  29 格, 連跑兩發一致(㉔ 訂正 + 新增 ㉘㉙)
突變      ①判別法恆真 ⇒ 3 紅   ②拿掉 union ⇒ 2 紅   ③拿掉 null 擋 ⇒ 🔴 全綠(見 13-4)
還原      hook 檔 sha256 前 16 = 18dd30c4e48f8d3a(每次突變後都比對過)
related   餵 hook ⇒ 8 支檔 158 綠
三綠      TURBO_FORCE=1 三發皆 0 cached
```
⚠️ **編號**:新格是 **㉘㉙** —— **㉗ 檔裡本來就有**(我第一版撞號, 而**發現它的過程是我用 `git checkout` 把自己的測試改動清掉之後重寫**;那個動作我今晚記過一次危險, 而我又做了)。

---

# §14 · R3(codex, 第三個模型)FAIL 6 條 —— **折了 4 條, 2 條【明寫沒守住】**

🟢 **三輪三個角度, findings 三次零重疊** ⇒ 換模型換角度是有效的, 不是同一層打轉。

| # | R3 說什麼 | 我做了什麼 |
|---|---|---|
| ① | 瀏覽器上一頁後 ref 仍是舊頁值 ⇒ 再選同一顆被判未變、漏 union | ✅ **折了**:網址上沒有 committed 那顆時 **ref 退回**;新增 **㉛** 釘住 —— 突變(拿掉退回)⇒ **只有 ㉛ 紅** |
| ③ | `router.replace()` 只代表**已呼叫**;Next 靜默忽略時 ref 仍前進 ⇒ **同一選擇永久被吞** | ✅ **折了**:改成 **pending / committed 兩段**, **觀察到網址落地才升** |
| ④ | ㉙ 只查第 1 次呼叫 ⇒「先送對的、再多送一個錯的」仍綠 | ✅ **折了**:`toHaveBeenCalledTimes(1)` + 查 **最後一次**(㉙㉚㉛ 都改) |
| ⑤ | 不同意我「`cat===null` 單元層測不到」—— 同一波**改別的軸**可繞等值早退 | 🟡 **他對了一半**, 見下方 |
| ② | union 沒去重上限、分類名含逗號 | 🛑 **不撿**(主視窗裁;可能超片) |
| ⑥ | 缺 vehicle 讓路後落地 + back/forward 組合 | 🟡 **㉛ 覆蓋了 back 那半**, 讓路那半沒做 |

## 14-1 🔴 **⑤ 的結果:他推翻了我一半, 而我照做之後【它還是綠的】**
我寫過「`cat===null` 那道擋在單元測試層**構造不出會紅的世界**」。
**R3 說那是因為我只讓分類變 —— 同一波改別的軸就繞得過等值早退。他是對的**:我照他的形狀造出 **㉚**,
那一波**確實走到寫入段**(`replace` 1 次、網址帶 `price`)。
🛑 **而拿掉那道擋之後 ㉚ 仍然綠** —— `join(',')` 對 `null` **不產生尾逗號**, `categories` 照樣印 `A,B`。
⇒ 📌 **本條今天仍然沒有測試守著, 而理由換了一個**:
   ⛔ ~~到不了那個世界~~ ⇒ ✅ **到了那個世界, 而它的錯誤【不留下可觀察的痕跡】。**
✅ **㉚ 留著當【回歸鎖】**(擋未來有人把 union 改成會產出空值的寫法), **不是突變殺手** —— 兩者不混。

## 14-2 🔴 **另一個突變也全綠, 一併明寫**
把 `pendingWrittenCategoryRef` 改回**直接記 committed** ⇒ **31 格全綠**。
成因:**單元測試裡 `router` 是 mock 的, `replace` 永遠「成功」** ⇒ 兩種寫法在那個世界**行為相同**。
📌 **它守的是【Next 靜默忽略】那個世界, 而那個世界在 mock 掉 router 之後【不存在】。**
⇒ **驗證欠在真瀏覽器 / E2E 那一層, 不假裝有。** 兩條都已寫進**碼的註解**。

## 14-3 讀數
```
測試   31 格, 連跑兩發一致(新增 ㉚㉛;㉙㉚㉛ 都帶 toHaveBeenCalledTimes)
突變   ①判別法恆真 ⇒ 5 紅   ②拿掉 union ⇒ 3 紅   ③拿掉 null 擋 ⇒ 🔴 全綠(14-1)
       ④拿掉 ref 退回 ⇒ 🎯 只有 ㉛ 紅   ⑤直接記 committed ⇒ 🔴 全綠(14-2)
還原   hook sha256 前 16 = 62b388bedd93c599(每次突變後比對)
三綠   TURBO_FORCE=1 三發皆 0 cached
```
⚠️ **而我今天【第二次】用 `git checkout` 清掉自己未 commit 的測試** —— 第一次清掉 ㉔訂正+新格,
這次清掉 ㉚。已寫一條硬規則給自己(`scratchpad/RULE.md`):**插探針前先 `cp` 備份, 還原用備份不用 git。**
📌 判別句:**這支檔裡有沒有我還沒 commit 的東西?有 ⇒ `git checkout` 就是刪除鍵。**
