...
1. **新 server action `queryHctUnknownAction({ shipmentId })`**(放 `shipment-submit-hct-action.ts` 旁,或自己一檔):
   ...
   - `found` ⇒ 與 #5 **同一段寫法**:`recordHctSubmit(submitted, edelno, raw)`(實作沒有另抽函式:就是那 1 次呼叫,參數逐字相同)。
...
| 型 | 查到 | 查無 | 查不到答案 |
|---|---|---|---|
| 甲 | 「新竹有這張單(貨號 X),已記成已送出,不要重送。標籤:同一天按『重新取得標籤』;隔天救回 ⇒ 作廢這箱、重開一箱再送(舊的新竹單可以打電話請新竹取消,不取消也沒關係)」(R1 F1:查回包沒有標籤圖;Sean 2026-09-15 定案) | 「新竹查無。**查詢還沒驗證過**,請照 runbook 打電話確認後再決定要不要放回草稿」 | 「沒拿到答案,不要重送,稍後再查」 |
| 乙 | 同上 | 「新竹查無,但新竹**曾經回過話** ⇒ 不自動處理、不放回草稿。請先打電話給新竹確認:新竹確認沒有這張單 ⇒ 把這一箱作廢(舊箱留紀錄),重新開一箱再送新竹」(Q2 定案) | 同上 |
...
```
✅ **定案(Sean 2026-09-15 逐字)**:「甲 = 這一箱作廢, 重新開一箱, 再送新竹叫一次車」
⇒ 不給放回草稿;出口 = 作廢這箱(舊箱留紀錄)→ 重開一箱 → 重新送新竹。
作廢 RPC 不看 `hct_status`(`20260808100000` `admin_void_shipment`)⇒ 卡在 unknown 的箱作廢得掉。片 A 查無文案照這條寫。
- Q1 定案:甲(做片 A)。Q3 定案:甲(同意真打兩發;開開關與真打時間另外端 Sean,片 B 入口先寫好不打)。
