...
`:179` UPDATE public.orders SET payment_status = v_target::public.payment_status

**允許集合(逐字)**

`:145` IF v_ps NOT IN ('paid', 'partiallyRefunded', 'refunded') THEN<br>`:179` UPDATE public.orders SET payment_status = v_target::public.payment_status
