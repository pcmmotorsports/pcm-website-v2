...
`:630` INSERT INTO public.order_cancellations (order_id, actor, idempotency_key, reason_code, reason_detail, payload_hash)<br>`:635` INSERT INTO public.order_cancellation_items (cancellation_id, order_id, order_item_id, cancelled_quantity)<br>`:640` INSERT INTO public.order_cancellation_items (cancellation_id, order_id, order_item_id, cancelled_quantity)<br>`:666` SET cancelled_at = pg_catalog.now(),

**允許集合(逐字)**

`:348` FROM public.order_cancellations<br>`:369` IF (v_order.cancelled_at IS NOT NULL) <> (NOT EXISTS (<br>`:376` IF v_order.cancelled_at IS NULL THEN<br>`:401` JOIN public.order_cancellations c ON c.id = (g.after->>'cancellation_id')::uuid<br>`:438` IF (v_order.payment_status <> 'unpaid'::public.payment_status<br>`:444` AND NOT (v_order.payment_status = 'partiallyPaid'::public.payment_status<br>`:450` AND NOT (v_order.payment_status = 'paid'::public.payment_status<br>`:456` WHERE pa.order_id = p_order_id AND pa.status <> 'failed') THEN<br>`:485` OR v_audit.before->>'payment_status' NOT IN ('unpaid', 'paid', 'partiallyPaid')<br>`:509` IF v_closed AND v_order.cancelled_at IS NULL THEN<br>`:517` IF v_order.cancelled_at IS NOT NULL THEN<br>`:525` OR EXISTS (SELECT 1 FROM public.order_cancellations c<br>`:546` IF (v_order.payment_status <> 'unpaid'::public.payment_status<br>`:552` AND NOT (v_order.payment_status = 'partiallyPaid'::public.payment_status<br>`:558` AND NOT (v_order.payment_status = 'paid'::public.payment_status<br>`:564` WHERE a.order_id = p_order_id AND a.status <> 'failed') THEN<br>`:630` INSERT INTO public.order_cancellations (order_id, actor, idempotency_key, reason_code, reason_detail, payload_hash)
