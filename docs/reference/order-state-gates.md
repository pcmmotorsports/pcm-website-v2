...
| `admin_record_manual_refund` | **3** | 20260820021000_m4b_e10_d1_record_manual_refund.sql:141<br>20260823020000_m4b_refund_notify_p2a_record_calls_sync.sql:290<br>20260905280000_m4b_cardcancel_p1_manual_refund_card_confirm.sql:89 | `20260905280000_m4b_cardcancel_p1_manual_refund_card_confirm.sql:89` |
...
| `admin_update_order_item_amount` | **3** | 20260815040000_m4b_e10_13_slice1_admin_update_order_item_amount.sql:325<br>20260816040000_m4b_e10_13_518_p2c13_detail.sql:45<br>20260905360000_m4b_pricecopytax_p2_manual_order_computes_tax.sql:972 | `20260905360000_m4b_pricecopytax_p2_manual_order_computes_tax.sql:972` |
...
`:603` INSERT INTO public.order_cancellations (order_id, actor, idempotency_key, reason_code, reason_detail, payload_hash)<br>`:608` INSERT INTO public.order_cancellation_items (cancellation_id, order_id, order_item_id, cancelled_quantity)<br>`:613` INSERT INTO public.order_cancellation_items (cancellation_id, order_id, order_item_id, cancelled_quantity)<br>`:639` SET cancelled_at = pg_catalog.now(),

**允許集合(逐字)**

`:321` FROM public.order_cancellations<br>`:342` IF (v_order.cancelled_at IS NOT NULL) <> (NOT EXISTS (<br>`:349` IF v_order.cancelled_at IS NULL THEN<br>`:374` JOIN public.order_cancellations c ON c.id = (g.after->>'cancellation_id')::uuid<br>`:411` IF (v_order.payment_status <> 'unpaid'::public.payment_status<br>`:417` AND NOT (v_order.payment_status = 'partiallyPaid'::public.payment_status<br>`:423` AND NOT (v_order.payment_status = 'paid'::public.payment_status<br>`:429` WHERE pa.order_id = p_order_id AND pa.status <> 'failed') THEN<br>`:458` OR v_audit.before->>'payment_status' NOT IN ('unpaid', 'paid', 'partiallyPaid')<br>`:482` IF v_closed AND v_order.cancelled_at IS NULL THEN<br>`:490` IF v_order.cancelled_at IS NOT NULL THEN<br>`:498` OR EXISTS (SELECT 1 FROM public.order_cancellations c<br>`:519` IF (v_order.payment_status <> 'unpaid'::public.payment_status<br>`:525` AND NOT (v_order.payment_status = 'partiallyPaid'::public.payment_status<br>`:531` AND NOT (v_order.payment_status = 'paid'::public.payment_status<br>`:537` WHERE a.order_id = p_order_id AND a.status <> 'failed') THEN<br>`:603` INSERT INTO public.order_cancellations (order_id, actor, idempotency_key, reason_code, reason_detail, payload_hash)
