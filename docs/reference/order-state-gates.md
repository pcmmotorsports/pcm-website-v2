...
`:642` INSERT INTO public.order_cancellations (order_id, actor, idempotency_key, reason_code, reason_detail, payload_hash)<br>`:647` INSERT INTO public.order_cancellation_items (cancellation_id, order_id, order_item_id, cancelled_quantity)<br>`:652` INSERT INTO public.order_cancellation_items (cancellation_id, order_id, order_item_id, cancelled_quantity)<br>`:678` SET cancelled_at = pg_catalog.now(),

**允許集合(逐字)**

`:360` FROM public.order_cancellations<br>`:381` IF (v_order.cancelled_at IS NOT NULL) <> (NOT EXISTS (<br>`:388` IF v_order.cancelled_at IS NULL THEN<br>`:413` JOIN public.order_cancellations c ON c.id = (g.after->>'cancellation_id')::uuid<br>`:450` IF (v_order.payment_status <> 'unpaid'::public.payment_status<br>`:456` AND NOT (v_order.payment_status = 'partiallyPaid'::public.payment_status<br>`:462` AND NOT (v_order.payment_status = 'paid'::public.payment_status<br>`:468` WHERE pa.order_id = p_order_id AND pa.status <> 'failed') THEN<br>`:497` OR v_audit.before->>'payment_status' NOT IN ('unpaid', 'paid', 'partiallyPaid')<br>`:521` IF v_closed AND v_order.cancelled_at IS NULL THEN<br>`:529` IF v_order.cancelled_at IS NOT NULL THEN<br>`:537` OR EXISTS (SELECT 1 FROM public.order_cancellations c<br>`:558` IF (v_order.payment_status <> 'unpaid'::public.payment_status<br>`:564` AND NOT (v_order.payment_status = 'partiallyPaid'::public.payment_status<br>`:570` AND NOT (v_order.payment_status = 'paid'::public.payment_status<br>`:576` WHERE a.order_id = p_order_id AND a.status <> 'failed') THEN<br>`:642` INSERT INTO public.order_cancellations (order_id, actor, idempotency_key, reason_code, reason_detail, payload_hash)
