...
### 7.3a 🔵 **這一節的內容【已經夠了】—— 而 2026-09-08 有人差點在它旁邊再寫一份**

> **寫這一格的理由,是一次沒發生的重複。**
> 2026-09-08 線 `-ship` 判板列 `⟦0e-RLSLIVE1⟧` 時量到同一個順序約束(先補 policy、再收 `BYPASSRLS`),
> 而主視窗 A 指示「把它寫進這份檔,因為這裡才是做強化的人會讀到的地方」。
> 🔴 **而我開檔之後發現 §7.3 逐字已經有了** —— 連「45 張同時且靜默」「空資料看起來像正常資料」都在。
> ⇒ 📌 **「這份檔裡還沒有這一條」是一個【宣稱】,而我們兩個都把它當成了【前提】。**
> 🎯 **⇒ 今晚同一形狀的第四次**(前三次:沒有人讀過那三份新竹文件〔真〕· 沒有人跑過那道檢查〔假〕·
>   還沒有人拍過跨 repo 範圍〔假,判過 24 次〕)⇒ **要往這份檔加東西之前,先讀一次它。**

✅ **而那一輪【真正是新的】只有一格,補在這裡**(其餘與 §7.3 重複,刻意不抄第二份):

**那 45 張裡,`staff` 那張是【後台自己登入用的】** ——
`apps/admin/src/lib/shipping/../staff-repository.ts` **五處**引用(`:27,56,68,90,111`,**讀出來的、非實跑**)。
🛑 **⇒ 收 `BYPASSRLS` 而漏補它那條 policy ⇒ 失敗形狀不是「某一頁空了」, 是【沒有人登得進後台】。**
⚠️ **而「整個後台鎖死」這半仍然是【推的】** —— 要證它得真的拿掉 `BYPASSRLS` 跑一次後台,
而 📌 **那是本題的觀測對象、不是可以調的旋鈕** ⇒ 沒有人做過, 也不建議為了證它而做。
🔵 **⇒ 它的可執行版是:§7.3 那個盤點跑出來的清單裡, `staff` 是【第一個】—— 不是【其中之一】。**
🎯 **為什麼要寫成「第一個」**(主視窗 A 2026-09-08 指定的字面):**那個失敗會把【修它的路】一起關掉** ——
其餘 44 張漏補的症狀是「某一頁空了」, 而你還進得去後台去修它;
🛑 **`staff` 漏補的症狀是【沒有人登得進後台】** ⇒ **你發現的時候, 已經進不去了。**
📌 ⇒ 一份「要補的清單」若照表名字母序或張數大小排, 這一格會落在中間 —— 而它與別的不是同一種東西。
