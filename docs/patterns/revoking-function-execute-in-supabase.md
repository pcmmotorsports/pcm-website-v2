...
>
> ```
> 40 張開了 RLS 的表【沒有】給 service_role 一條可用的 SELECT 政策。
> 今天靠 service_role 的 BYPASSRLS 撐著 ⇒ 讀得到, 一切正常。
> 🔴 你這一動（收掉 BYPASSRLS / RLS 嚴格化 / 換 service_role 的用法）
>    ⇒ 客戶後台【整個空白】, 而搜尋（SECURITY DEFINER 繞 RLS）照樣打得到人
>    ⇒ **系統看起來活著, 而資料看起來不見了。**
> ```
> **正本 = 板子 `⟦b9-Q15GAP⟧`**;範圍與數法 = `docs/specs/2026-08-25-q15-service-role-select-scope.md`
