...
> **「突變後綠」有三種成因,而它們印出來一模一樣**:
> ① 斷言沒判別力(我們以為在找的)② 突變根本沒被執行到(打在死分支)③ 突變被執行了,
> 但效果被別處冗餘遮蔽(A′3 實錘:SKU 在三個渲染點各出一次——shipping-doc.tsx:436/:471/:499,
> 只切主表那個 ⇒ 頁面上 SKU 仍在、toContain 照綠)。
...
| 綠 | EXECUTED | **① 或 ③**:先掃「同一觀察物還在哪些地方被產出」(A′3=grep 其他渲染點),排除遮蔽才准寫「斷言沒判別力」 |
