...
### 🔴🔴 **而上面那條 `wait` 擋的是【睡死】—— 它的分母裡【沒有】第二種死法**(⟦0a-FOREGROUNDTIMEOUT⟧)

**2026-09-02 同一天三個窗各死一次, 而三次都不是睡死**:

```
-e3  16:4x  codex 跑滿 10 分鐘 ⇒ 撞到自己的【前景逾時】⇒ SIGTERM(rc=143)⇒ codex 跟著死(pgrep 回 0)
-0a  16:34  announce-and-push ⇒ 10 分鐘逾時砍掉 ⇒ 推送預告停在「(推中)」而遠端一個字沒動
-f3  同日   codex 對抗審查 ⇒ Exit code 143 / Command timed out after 10m 0s
```

🔴 **而 `-f3` 那一發最毒**:那一發 codex **其實已經跑完了** —— log 尾有「審查完成」與四個 ✓,
**逾時砍在它寫完之後** ⇒ 📌 **它差一點被讀成「它沒跑完」而整片重跑。**

🎯 **⇒ 兩種死法在畫面上都是「它沒有跑完」, 而成因與修法不同**:
```
睡死        stdin 沒導掉 ⇒ 它在等輸入      ⇒ 修法 `< /dev/null` + `wait`
被自己砍掉  前景逾時到了 ⇒ SIGTERM(143)   ⇒ 🔴 `wait` 擋不到它 —— 連 wait 自己一起被砍
```
🛑 **⇒ 所以上面那條規則不是寫得不好, 是【它的分母裡沒有這一種】。**

### 🔴 **[2026-09-05 補 · 線【身分】`-auth` 實跑撞到]** 143 那一格【有一個比換背景更早的出路】

⛔ ~~我當時把 `rc=143` 讀成「這個 harness 的前景上限是 2 分鐘, 那條紀律做不到」~~
⇒ 🔴 **那句是錯的:Bash 工具有 `timeout` 參數, 上限 600000 ms(10 分鐘)。**
⇒ ✅ **所以 `wait "$PID"` 那一發【帶 `timeout`】就好** —— 紀律不必改, 改的是我沒帶參數。
　　(codex 這一發實測 R1 產出 3,681 行、耗時 8 分鐘以上 ⇒ **預設 2 分鐘必砍, 10 分鐘夠。**)
🛑 **而 143 的毒不在被砍, 在【被砍的時候 codex 還活著】** ——
　 `rc=143` 與「codex 失敗了」長得一樣, 而真相是**它在背景繼續跑**。
　 ⇒ 📌 **拿到 143 的第一個動作是 `pgrep -f 'codex exec'`, 不是重跑。**

### 🔴🔴 **[2026-09-05 同一發] 背景那條路有【第三種死法】, 而它比零位元組更毒**

我改用 `nohup codex … &` 包在一個會結束的 Bash 呼叫裡 ⇒
**harness 回報「任務完成 exit code 0」, 而 codex 本體還在跑。**
🎯 **⇒ 那個「完成」通知說的是【wrapper 退出】, 不是【工作完成】** —— 兩者印同一句話。
⚠️ **我一夜撞兩次**;第一次是靠輸出只有 41 行才發現不對 ⇒
　 📌 **若那次輸出剛好夠長, 我會把一份沒跑完的審查當成 PASS 交出去。**
✅ **修法(照優先序)**:
```
① wait "$PID" + timeout 參數(≤600000ms)        ← 最好, 通知就是真完成
② run_in_background 的【同一個 shell】裡起 + wait  ← 超過 10 分鐘時
③ 另一發 until ! pgrep -f 'codex exec'; do sleep 15; done
   ⚠️ 而 ③ 有已知殘餘風險:`pgrep -f` 會撈到【別窗】的 codex(本檔上面那條紀律已記)
      ⇒ 八窗齊跑時它會讓你多等別人那一發 ⇒ 所以 ③ 是最後手段, 不是預設。
```

✅ **原修法仍然成立**:會超過幾分鐘的東西**一律 `run_in_background`**, 讓它**跨 turn 活著**。

⚠️ **而背景不是免費的 —— 它換來另一個失敗形狀, 一起寫**:
`-f3` 同日改用背景跑 codex ⇒ **stdin 被接壞** ⇒ 它 **`rc=0` 而只印了 prompt、零輸出**
(memory 已記過「codex 空 prompt 也是 rc=0」)。
📌 **⇒ 換到背景之後,「跑完了」與「根本沒開始」又變成同一個 rc。**
⇒ 🟢 **所以背景那條路要加一格**:收到結果後先 `wc -c` 那個輸出檔 —— **零位元組 ⇒ 它沒跑, 不是它沒發現問題。**

🟢 **而 `-0a` 那一發的好消息要一起寫(否則這一段讀起來像「什麼都會壞」)**:
它的失敗方式是**乾淨的** —— 遠端未動 · 鎖沒卡住 · 無半套
⇒ **`announce-and-push` 的互斥鎖與預告落檔在被砍時仍然自洽** ⇒ 那支腳本的設計是對的。

🛑 **本節【沒有】做的事**:`CLAUDE.md` 那一句(codex-adversary 紀律的「等待方法」)**仍然只寫了 `wait`** ——
改它屬於「`CLAUDE.md` 本體」⇒ **要 Sean 拍**, 本窗不自行動。
⇒ 📌 **在它被改之前, 讀 `CLAUDE.md` 那一句的人不會被路由到這一段** —— 這是一個**已知的缺口**,
   寫在這裡是為了讓「它為什麼還沒修」有落點, 不是為了假裝它修好了。
