...
| [`search-synonym-dictionary.md`](search-synonym-dictionary.md) | 搜尋俗稱字典這條線的收攏檔 —— 字典為什麼不能被比對規則取代(26/32 列字面無關)· 一列合法而永遠沒效果的四種死法 · 三件量不到的事 · EMPTY 快照靠誰在擋。**每格帶射程與失效條件。** |
...
| `admin-auth-is-delegated-to-quote.md` | 🔴 **「報價單登入了就能進後台」是刻意的 SSO,不是漏掉的閘**(Sean 2026-07-12 批准的提案)。**查這件事之前先讀它**:cookie domain 那條路**實查零命中**,而真正的機制是一條換票協定;含兩個含意(**誰能登入報價單誰就能進後台** / **後台 session 裡沒有「這是誰」**)|
...
| 你手上的問題 | 去哪 |
|---|---|
| 🔴 **我正要跑一個盤點 / 掃描,而還沒開始** | **先讀下面那份「30 秒清單」的三題**(🔴 **Q0 先看分母合不合理** ／ Q1 零斷言 ⇒ 跑正向對照 ／ Q2 有斷言 ⇒ 印出來看命中是不是那一種 ／ Q3 用成敗決定下一步 ⇒ 不接管線)。<br>來源 = 2026-08-19 一晚**八個**量具坑,全部當場自抓,每個附實例與數字。 |
...
>
> (2026-08-19 G6 立;來源 = 一個晚上**八個**量具坑,**全部當場自抓**、每個都有實例與數字。
> 🔴 **每一題給的是【正向的作法】,不是「小心 X」** —— 因為「小心」不會被執行,一條命令會。)
>
> ### 🔴 Q0 **我剛跑完一發,而它印出了一個結論** ← **這一題比 Q1 更早**
> ```
> ⇒ **先看分母 —— 那個數字合理嗎?**
>    「我掃了幾支檔」「這張表有幾列」「候選集有幾個」——
>    分母不合理(尤其是 **0**)⇒ **結論作廢,先修尺。**
> ```
> **為什麼它排在正向對照【前面】**:
> ```
> 正向對照證的是「**尺會動**」;分母證的是「**尺有東西可以量**」。
> 🔴 而尺沒東西可量的時候,**它照樣會動** —— 動了一個空集合,而那看起來就像結論。
> ```
> **實例(2026-08-19,今晚第八種壞尺,而方向是最危險的那種)**:
> ```
> 我用 `/usr/bin/ls` 列 migration —— **那支不存在**(是 `/bin/ls`)
> ⇒ 分母 0 ⇒ 印出「✅ 零支待 apply」
> ⇒ 而實際上有 **2 支**待 apply,其中一支是【已裁定不得 apply、apply 一定會炸】的那支
> 🔴 抓到它的不是正向對照,是那一行「**repo migration 版本數 0**」明顯不可能
> ```
>
...
2. **窗做完一件、回合結束,它就「不在」了** —— 而「不在」跟「在等人」長得一樣。
   自律只縮短那道縫,**偵測必須是外部的**。
3. **一個問題被記錄在【最懂它的人】那裡,而答得出它的是【另一個人】** ——
   實錘:`three-ds-flag.ts:19-23` 那段寫著兩個 flag 的狀態有爭議、**而它連受眾都指名了**
   (逐字「誰讀誰就把這段關掉」)⇒ **它仍然待了 13 天**,因為那段話問的是
   「Vercel 面板上那兩個值是多少」,而**只有 Sean 進得去那個面板**。
   🔴 **⇒ 這不是「寫在沒有流量的地方」,是【寫在正確的地方,而那個地方的讀者答不出那個問題】。**
   ⇒ 判別句:落筆寫下一個待解問題時多問一句 **「答得出這個問題的人,會來讀這支檔嗎?」**
   不會 ⇒ **那個問題要有第二個落點,而第二個落點要在【答得出的人會去的地方】** ——
   🛑 **不是把它寫得更詳細、更醒目,那只是讓看不到的人看得更清楚。**
   ⚠️ 天花板:**知道誰該答**與**那個人真的收到**之間還缺一個動作,**而沒有人負責那個動作**。
   全文 `docs/patterns/traps-inbox/D-20260830.md` 的 D-17(`-e4` 2026-08-30 寫)。

## 病史簿(casebook)族 —— 2026-09-02 常載檔瘦身產生
> 🔴 這四支的內容【原本住在 `CLAUDE.md`】(每一輪對話全文載入)。
> 搬出理由:那些字是**病史·實錘·射程**, 而查表的人只需要**症狀句**。
> 🎯 `CLAUDE.md` 留症狀 + 一行指標;細節在這裡。**一個字都沒刪。**

| 檔 | 它答什麼 | 從哪裡搬來 |
|---|---|---|
| `routing-casebook.md` | 路由表 22 列的病史(列印多一頁 / greenlight / OD 稿 / probe / what-happened-to …) | `CLAUDE.md` 路由表 |
| `ironrules-casebook.md` | 鐵則 1(真權威)· 6(檔案大小)· 11(三綠與測試分母)的實錘與舊字面 | `CLAUDE.md` 鐵則 1-12 |
| `zsh-and-bash-traps.md` | 13 條 zsh/bash 寫法各自怎麼壞(`$?` 覆寫 / `local X=$(...)` 吞 rc / `echo \c` / `&&` 被合法的零截斷 …) | `CLAUDE.md` 終端機紀律 |
| `checklist-casebook.md` | 開工前/收工前 25 格勾選各自「為什麼有這一格」 | `CLAUDE.md` 快速自檢清單 |
| `board-merge-rules.md` | 六窗同寫一張表時合併會壞在哪(merge 自生重複 / 去重刪掉後長的 / 最長非超集 / 相似度不遞移 / 裸行號漂) | 2026-09-07 一夜實錘, 新立 |

🛑 **四支共同的射程**:它們都答不出「你的情況適不適用」——
每一支檔頭都寫了**「這個檔在什麼情況下會【變成假的】」**, 引用前先讀那一格。
