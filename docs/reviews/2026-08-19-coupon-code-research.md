...
## 7. ✅ **補查:`plan v6 §3.2` 找到了 —— 而它旁邊就有一個推翻的前例**

### 7-1 它在哪
```
docs/specs/2026-06-04-m3-checkout-plan.md
  檔頭:「M-3 結帳 + 會員分級定價 — 動手前計畫 **v6**(鐵則 8 關卡1 用)」
  §3.2 在 :74;優惠券那句在 :76
掃描分母:git grep -ln "plan v6" ⇒ **38 檔**(負向對照「鐵則 12」同尺 ⇒ 522 檔,尺會動)
```
...
premiumStore 再折(暫=store)/ 經銷商自動申請審核+管理 UI(手動設 tier、M-2-06/M-4a 延)/
儲值金 / 優惠券 / 合作店家取貨 / 完成頁安裝 CTA / ATM(隱藏)/ 發票自動開立(只收存)/
免運門檻套 NT$5,000(L2)。偏離一律註解+commit body+manifest override 揭示。
```
🔴 **注意兩件**:
```
① 優惠券**沒有自己的理由** —— 它在一個【九項並列的批次清單】裡
② 節名是「不做 / **先留位**」 ——「先留位」= **刻意保留位置**,不是「不要」
```

### 7-3 🔴🔴 **當初的理由:同一份檔在下一段自己講了**
同檔 `:78` 的 Sean override 註記逐字:
> 「🆕 **2026-06-11 Sean override:TapPay webhook(notify)移出「不做」→ 改做**
>  (理由=金流穩健優先、**原排除僅 Phase-1 省事非硬擋**)」

⇒ **「原排除僅 Phase-1 省事非硬擋」這七個字就是答案** ——
```
§3.2 整批的排除理由 = **控制 Phase-1 範圍**,不是任何技術或商業上的硬擋。
⇒ 優惠券不是「評估過覺得不該做」,是「**這一期先不做**」。
```
📌 **而那一段本身就是【從這張清單裡撈一項出來改做】的前例**:
TapPay webhook 當初也在同一張清單上,而 Sean 2026-06-11 把它移出來、理由寫在括號裡。
⇒ **格式現成的**:要動優惠券,照那個形狀寫一行 override 註記即可。

### 7-4 那個理由今天還成立嗎 —— 🔴 **我不判,我只把當初的前提列出來**
```
當初的前提                              今天
① Phase-1 要控制範圍                     ⇒ 仍在 Phase 1(Sean 08-18「不間斷把網站完成」)
② 優惠券與【儲值金】綁在同一批決定       ⇒ 🔴 而儲值金 Sean 08-18 才剛說「**現在不開**」
   (cart.css:8 那句「#202 wallet HOLD」把兩者寫在一起)
③ 當初結帳線還沒上線                     ⇒ 現在上線了、而且 Sean 本人刷卡驗過
④ 當初沒有後台                           ⇒ 現在後台重建中(M-4b),而券是 L3 必後台 CRUD
```
⇒ 🔴 **②那格要特別給他看**:優惠券與儲值金是**同一批**被 HOLD 的,
  而他**昨天才說儲值金不開** ⇒ **他可能沒意識到自己正在拆開那一批。**
  ⚠️ 而**拆得開不開我沒查**(CSS 那兩塊的耦合程度)—— 標未確認。
...
而 design-reference/components/ 全部 20 個元件**沒有一個是後台**
  (App / Header / HomePage / ProductPage / CheckoutPage / WalletTab …)
```
⇒ **design-reference 本身就只有顧客站** ⇒ 後台的券管理頁**當然沒有**,不是漏掉。
⇒ 🔴 依鐵則 1 的「真權威解析」:**後台 UI 的真權威是 `docs/design/admin-design-system.md`**
  (BMW M 設計語言,Sean 2026-08-16 擴到全後台)—— **不是 design-reference。**
⇒ 所以後台那一頁**要照後台的設計系統做**,而顧客站那一格**要照 design-reference 直接搬**。
📌 **這兩半的真權威不同,不要混。**
