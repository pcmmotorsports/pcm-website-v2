...
| 材料 | 是否自帶 |
|---|---|
| migration **全檔逐字**(檔頭註解 / `BEGIN` / helper / 守門 / `DROP` / `CREATE` **含函式體全文** / ACL / `COMMENT` / `NOTIFY` / 斷言 / `COMMIT`) | ✅ 全文,共 651 行(§6) |
...
- **網頁版 Codex**:**NO-GO,4 must-fix + 3 nit** → 全數處置,銷案表 = plan §15.4。**至今仍未取得任何一輪 GO**
