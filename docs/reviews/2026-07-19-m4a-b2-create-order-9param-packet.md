...
| 材料 | 是否自帶 |
|---|---|
| migration **全檔逐字**(檔頭註解 / `BEGIN` / helper / 守門 / `DROP` / `CREATE` **含函式體全文** / ACL / `COMMENT` / `NOTIFY` / 斷言 / `COMMIT`) | ✅ 全文,共 654 行(§6) |
...
5. **跨檔案的指紋公式一致性無程式強制**:B-2 migration 與 rollback SQL 各有一支 `pg_temp` helper,
   單檔內是結構性保證,**跨檔只靠人工比對 + 兩邊實跑得出相同指紋**。
6. **本機 PG 17.10 vs prod 17.6**:§7 的 byte-level 與指紋相符已大幅降低版本差風險,**但不是證明**。
7. **B-2 apply 後必填尚未生效**:第 9 參仍 `DEFAULT NULL`,`authenticated` 直呼 RPC 可省略 → 必填收緊是 B-6。
...
- 🔴 **CLI 關卡2 輪數上限已用完** → **Sean 拍 A:不開 round 3,改走本包給網頁版 Codex**(= 鐵則 12 本來就要走的關)
- **網頁版 Codex**:**NO-GO,4 must-fix + 3 nit** → 全數處置,銷案表 = plan §15.4
- **Fable 對抗審查**(Sean 指定,受審凍結版 `f650a50`):**NO-GO,2 must-fix + 2 nit** —— 🔴 **migration SQL 本體零 must-fix**,兩條皆文件/SOP 層 → 全數處置,銷案表 = plan §15.5。Fable 明示「F1+F2 修完即轉 GO」
...
| **15** | 🔴 **rollback 守門負向 A**:竄改既存 9-param 的 COMMENT 後跑 rollback | 本機 | ✅ **在 `DROP` 之前**失敗;9-param 仍在、8-param 未被誤建 |
| **16** | 🔴 **rollback 守門負向 B**:對既存 9-param `GRANT EXECUTE … TO anon` 後跑 rollback | 本機 | ✅ 同樣被擋 |
| **17** | 🔴 **斷言⑧ 負向**:改 migration 產物(COMMENT)但常數保持舊值 | 本機 | ✅ 明確觸發「斷言⑧失敗」、整批回滾、8-param 基線完好 |
| **18** | `CALLED ON NULL INPUT` / `NOT LEAKPROOF` 顯式化後指紋是否改變 | 本機 | ✅ 兩個指紋**皆未改變** → 常數不需重取 |
