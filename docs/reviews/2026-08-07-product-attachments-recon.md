...
**PDF 與影片** —— 🔴 **兩個來源、兩個截面日期,分欄並列(2026-08-07 下午更正)**:

> **為什麼要分欄**:原始版本只用了左欄(config 註解、**07-10** 截面)並據此寫出「evotech 完全零附件」,
> 而右欄(報價單 plan 的 MCP 實查、**07-19**)顯示 evotech 那時已有 6 份 —— **9 天內附件補上了**。
> 錯誤字面留著就是下一次過期引用的種子,故改成雙欄,讓讀者看得見「哪一格是哪一天的事實」。

| 供應商 | PDF:07-10 config 註解 | 🔴 PDF:07-19 MCP 實查(單群最多 / ≥2 份群數) | 影片:07-10 | 出處 |
|---|---|---|---|---|
| 🔴 **evotech** | ~~**0**~~ **已過期** | **6 / 73 群** | 0(未複核) | config `:146` / plan `:37` |
| **akrapovic** | 635 群有 | **12 / 572 群**(平均每群 **4.8**、213 群 ≥6 份) | 0 | config `:236-242` / plan `:25,:34` |
| lightech | 2,019 群有 | 2 / 50 群 | 0(Vimeo 預期、晚到自然補) | config `:164` / plan `:38` |
| cncracing | 1,008 群有 | 3 / 103 群 | 55 群(Vimeo) | config `:138` / plan `:36` |
| gbracing | 有(數量未載明) | 3 / 146 群 | — | config `:113` / plan `:35` |
| bonamici | 有(數量未載明) | 1 / 0 群(全是單份) | 有 | config `:124` / plan `:39` |
| kspeed | 0 | 該表未列 | **57 群** | config `:272-277` |
| ebc | **0 —— 刻意不給,非缺口** | 該表未列 | **45 群**(YouTube) | 見下方 ⚠️ |
| eazigrip / samco / motogadget / front3d / materya | 皆 0 | **該表未列 ≠ 確認為零** | 皆 0 | config `:146-147` |
| rpm | — | 該表未列 | — | config `:102` `syncInstallResources: false` |

🔴 **讀這張表的三個限制**:
1. **右欄那份 plan 的表只列了 6 家**(它當時只關心「多份 PDF 被錯標」的受害者)。
   其餘各家標「該表未列」—— **那不等於確認為零**,只代表那份文件沒查它。
2. **影片欄兩個截面都沒有複核** —— 全部停在 07-10 config 註解。evotech 的 PDF 都變了,影片同樣可能變。
3. ⇒ **不得用本表做「某家永遠沒有附件」的決策**。要用就重查 DB;
   顯示邏輯一律走「有才顯示」,不要用名單寫死。
...
🔴 **2026-08-07 下午更正:evotech 不屬於這一類。** 它在 07-19 的 MCP 實查已有 PDF
(單群最多 6、73 群 ≥2 份);我原本把它歸進「來源本身就沒有」是**引用了 07-10 的過期截面**。
⇒ 這一類剩下的成員也**只是「該截面為零」,不是「確定不會有」** —— 見 §1-Q3 表的三個限制。
...
3. 🔴 **§1-Q3 的所有數字都是「文件裡記載的實查值」,不是我當下查 DB 得到的。**
   ...
   🔴 **這條預告的風險已經實現了一次(2026-08-07 下午,寫 OD 需求包查證時抓到)**:
   我依 07-10 截面寫的「evotech 完全零附件」,對照 07-19 的 MCP 實查是**錯的**(它已有 6 份 PDF)。
   ⇒ 已更正 §1-Q3(改成雙欄截面)與斷法 C。**教訓不是「這次改對了」,是「截面數字會在被引用的那一刻剛好過期」**
   —— 差一步就寫進給 Sean 帶去 OD 的設計依據裡。
