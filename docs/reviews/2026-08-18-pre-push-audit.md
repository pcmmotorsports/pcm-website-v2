# 推前總驗收 —— 今晚(2026-08-17 之後)改了行為的 commit 四欄對帳

> 起因:Sean 已批 apply、也早授權推,213+ 顆隨時上 pcm-admin production。而今晚改了十幾處(含錢/權限/出貨),沒有人回頭確認每一處【真進了 dev】【有負測】【被第二方驗過】【血半徑進了 code 註解】。
> 執行:T①(socket 32075),2026-08-18。**唯讀盤點,不改任何被審的檔。**

---

## 0. 分母(可重跑)

```bash
# 產品執行期行為 = apps/** + packages/** (非測試) + supabase/migrations/**,排除 scripts/(工具儀器)與純 docs
git log origin/dev..dev --since="2026-08-17 00:00" --pretty=%H --name-only \
 | awk '/^[0-9a-f]{40}$/{h=$0;next}
        /^(apps|packages|supabase\/migrations)\//&&!/\.(test|spec)\.[jt]sx?$/&&!/__tests__/{print h}' \
 | sort -u
```
- 產品執行期行為 commit = **27 顆**(含 scripts/ 工具儀器則 51 顆;那些不碰顧客面)。

---

## 🔴 column a 的分母陷阱(比表本身重要,寫在最前面)

> **column a(「進 dev 了嗎」)若拿【dev】當分母,它對分母裡每一顆【恆真】—— 零判別力,而表看起來完全正常。**
> 這是「恆真格」那一族的最高階版本:不是一格測試恆真,是**一張表的一整欄恆真**。
> ⇒ 要回答「有沒有東西【沒進】dev」,那一欄必須換**另一個分母**:各窗今晚宣稱改過的 commit(不論在哪個 branch),再逐顆 `git merge-base --is-ancestor <h> dev`。
> **本表的 a1210748 就是這樣抓到的 —— 去 dev 分母【之外】找。**

---

## 1. column a —— 不在 dev 的(推 dev 不會帶上)

| commit | 面 | 在哪 | 判定 |
|---|---|---|---|
| 🔴🔴 **a1210748** | **錢** settle-charge 對帳「已退金額欄缺不翻 0」 | `sso-security-log-tests` | **NOT in dev** —— 缺口 |
| a22f6e2d | C 條資料層(adapters) | `products` | NOT in dev —— **施工中正常** |
| 7f6d0ac1 | 六列表頁 max-w-6xl | `bmw-m-headline` | NOT in dev —— **施工中正常** |
| de7cc9ef | 出貨入口鈕改名 | `bmw-m-headline` | NOT in dev —— **施工中正常** |

**a1210748 細節(可回放)**:
- `git merge-base --is-ancestor a1210748 dev` ⇒ 否;merge-base 停在 15d8b19c。
- dev 現行 `packages/use-cases/src/settle-charge.ts:418` 仍是 fail-open:`if ((tr.refundedAmount ?? 0) !== 0) …` —— `?? 0` 就是病灶(欄缺→0→閘不響)。
- dev 版 grep `refunded_amount_missing`(修法招牌字)⇒ 0 命中 ⇒ 修法確不在。
- **射程**:這是【對帳/告警】閘(回 triage 訊號),**不是資金移動閘** ⇒ 不直接掉錢,但一類異常退款【不被告警】。仍屬鐵則 12①。
- **成因(主視窗自陳)**:I 窗押著它等 V 窗複驗;V 窗早 PASS;而「押著的理由消失」時沒有東西通知 I 窗 ⇒ 一顆錢的修法停在分支。→「暫緩要寫成有到期條件的依賴,而到期時沒有東西會叫」。

**🔴 91df7f90 —— column-a 的第二種形狀:「修法的兩半分屬兩顆 commit,只有一半進 dev」**
- fa958b0c(出貨單【擋印】)✓ in dev;91df7f90(擋印之後【給對的診斷訊息】:面8「讀不到任何品項」移到面5「不要自己拼網址」之前)🔴 **NOT in dev**。
- ⇒ 推上去的話:**擋是擋住了,而值班會被指去查一個不存在的問題(以為自己拼錯網址,真正的病是系統讀不到品項)。**
- 🔴 **與 a1210748 形狀不同**:a1210748 是【整顆沒進】⇒ 症狀=「沒修」;91df7f90 是【半顆沒進】⇒ 症狀=**【修了一半而看起來修好了】** —— 後者更難發現。

**🔴 現況更新(audit 之後):** a1210748 已被收割進 dev(`9ac8fa11` 收 pcm-613 8 顆),**而它把 dev 弄紅了** —— `packages/use-cases/src/sweep-settlements.test.ts` 13 格 AssertionError,成因=作者只同步了自己那支的 fixture、姊妹檔沒動(與 15d8b19c 同族:fixture 造了 production 不會出現的形狀)。主視窗已命令作者【先答「sweep 那條路的紀錄來源」再改,不准動 fixture】。⇒ **column-a 現有三筆:a1210748(已收但引發紅)/ 91df7f90(半顆)/ 原本那幾筆施工中的。**

> ⚠️ 「正常(施工中)」與「缺口」分開講 —— 這張表不是四條紅字的恐慌清單。

---

## 2. column b —— 負測是不是真的

方法:`git show <hash> -- <test>` 讀 diff 判「會不會對舊 code 紅」(比「有沒有測試檔」強;比「親手拆」弱)。

| commit | 面 | 負測判定(讀 diff) | 親手拆紅 |
|---|---|---|---|
| 7305cf8d+b9a6b1e2 | tier prod 硬閘 | ✅ 真(翻掉舊斷言) | ✅ T① 親拆,①③兩格紅(V 窗獨立複驗一致) |
| 15d8b19c | 錢/退款帳本 | ✅ 真([5d] 舊斷言翻成 table 為 null,舊斷言留註解備查) | ✅ **5681 親拆三次:2 failed/25 passed([5d]+[5e])。🔴 但 commit body 寫「1 failed」是錯的** —— 更正在 STOP §⑤-b,收割引 STOP 別引 body |
| fa958b0c | 出貨單守門 | ✅ 真(不擋→擋印 + 反向格擋恆綠) | ✅ **C 窗親拆:2 failed/61 passed,與 body「2 格紅」一致**;兩格紅=設計(反向對照) |
| 251c2307 | 權限/RLS migration | ✅ 真(628-verify.sh M1/M2 REVOKE 突變) | ✅ **E 窗親跑:M1 轉紅在【元資料層】(非行為層,刻意——codex R2-MF3 把元資料守門重排到行為前,權限還在的世界先在 §2 被擋、REINDEX 不跑)** |

> 🔴 **251c2307 口徑(E 窗主動說,推前必帶)**:`628-verify.sh` PASS=32 **≠「正式庫 apply 會過」**。本機基線是照 repo DDL 重建的**三欄節錄版**,驗不到正式庫 owner/grantor/RLS/default ACL/Supabase 角色拓樸。限定寫在 migration 檔頭「誠實界」段。
> 📌 **E 窗 harness 設計值得記**:每發突變斷言【紅在哪一格】(逐格 grep 點名),不是【紅幾格】⇒ 結構上沒有「該紅 N 格」這個字面可與實際不一致 ⇒ V 窗抓的那類漂移在這支不存在。「紅了」與「紅對地方」是兩件事,數格數只驗前者。

> **column b 加問(今晚同族三次)**:commit body/註解寫的「拆掉會紅 N 格」,跟【現在】實際紅幾格一樣嗎?三次實例:
> - T① tier.test 原寫「第一格必須翻紅」,實測 ①③兩格 ⇒ b9a6b1e2 已修。
> - **15d8b19c body 寫「1 failed」,實測「2 failed」** ⇒ 5681 更正在 STOP §⑤-b(commit body 改不動)。成因逐字:「那個 `1 failed` 是 `[5e]` 還不存在時(26 格)量的,加完 [5e] 沒重跑 M1,拿舊數字描述最終狀態。」
> - 🔴 **5681 給的判別句最準(這族的根)**:**「不是數錯了,是【量測的時點】與【描述的對象】不是同一個狀態 —— 而它天生沒有訊號:那個數字當時是對的,是世界(檔案)變了而數字沒跟著變。判別句:我這個數字,是在【現在這個檔】上量的嗎?中間我改過它嗎?」**
> - 🟢 反例(結構上免疫)= E 窗 628-verify.sh:每發突變斷言【紅在哪一格】(逐格 grep 點名),不用「N 格」制 ⇒ 沒有那個會漂的字面。
> **親手拆**:各原窗自拆自己那顆(它們 worktree 有可跑狀態;T① 的 branch 落後 dev-merge、本地拆不動),回一行。

---

## 3. column c —— 第二方審查

> 🔴🔴 **這一欄最重要的不是那四顆,是【審查證據活在哪裡】。**
> reviewer-gate 的 marker **不留歷史**;三個月後**沒有任何機械方式**回答「這顆審過沒」。今晚主視窗能逐顆對帳,只因為它**還記得那幾封訊息**,而它會被壓縮。
> **判別句:這顆的審查證據,活在哪裡?活在 commit body ⇒ 永久;活在訊息 / marker ⇒ 會消失。**
> ⇒ 錢/權限/schema 的審查結論**必須寫進 commit body**(誰審的、判定、findings 折了幾條),不能只在訊息裡。

| commit | 審過沒(主視窗 review 帳) | body 有記? |
|---|---|---|
| 7305cf8d | codex + code-reviewer(opus)PASS | ✅ 有記 |
| 1fd291d6 | codex(收五條) | ✅ 有記 |
| 8cb69570 | 寫/驗分離:B 窗寫、E 窗驗 | ✅ 有記 |
| 15d8b19c | ✅ codex R3 PASS + V 窗 PASS(兩層) | 🔴 **body 沒記** |
| 251c2307 | ✅ codex 三輪 13 條全折(harness 禁 Agent 故未跑 code-reviewer;主視窗裁「apply 前必補」仍成立=apply 前置非推前置) | 🔴 **body 沒記 codex** |
| 🔴 fa958b0c | ~~**無人審**~~ ⇒ **已補審 R2 PASS**(見本檔「C 窗三片審查」段;原判定=C 窗自判輕量片跳審,判定人=自己,沒有第二方) | —— |
| 92ac213c | 🟡 E 窗規格層第二方;修法本身回驗無紀錄 | 待補 |

### 🔴 fa958b0c = 真 column-c 缺口 + 一個規則的洞(C 窗主動自曝)

> ✅ **缺口已於 2026-08-18 補審關閉(R2 PASS,見本檔「C 窗三片審查」段);🔴 而【規則的洞】仍開著。** 下段為當時的原文,逐字保留。

- **無人審**:C 窗依 `CLAUDE.md` 片型分級自判輕量片(不碰六類 + 單一元件)⇒ code-reviewer 可跳。合規,但**沒有第二個人看過**。
- 🔴 **規則的洞**:這一片**翻掉了一格既有測試的斷言**(原釘「`null` ⇒ 不擋」→ 改「`null` ⇒ 擋印」)。**「翻既有測試斷言」不在片型分級的六類裡,所以規則沒擋它** —— 而翻既有測試的語意 = 行為語意改動,理應觸發審查。C 窗自評「那大概是規則的洞,不是我該利用的空間,要補審我不反對、該補」。
- 「三條依據寫得完整」≠「有第二個人查證過那三條」—— 是兩件事。
- **建議**:推前對 fa958b0c 補跑一次 code-reviewer(叫得動 Agent 的窗);並考慮把「翻既有測試斷言」加進觸發審查的條件(機制優先律候選)。

---

## 4. column d —— 血半徑限定進了 code 註解,不只 commit body

> **為什麼重要(現成最佳論證)**:`lessons:1065` 那條 —— 原文 runbook(`docs/reviews/2026-08-07-e-batch-apply-runbook.md:164-166`)有「PGRST202 = 具名參數對不上」的限定,而 lessons **摘要弄丟了它**,摘成「函式不在」,活了 11 天並被當通則引用。
> ⇒ **限定會在轉載時掉,而掉了之後沒有人知道它掉過。** commit body 不會被讀第二次,code 註解會 ⇒ 血半徑限定必須進 code 註解。

範圍=27 顆裡的錢/權限/schema/出貨 9 顆。方法:`git show <h> -- <非測試檔>` 撈新增註解行,比對 body 的血半徑限定句有沒有【同一句】進 code 註解。

| commit | 面 | body 的血半徑限定 | 進了 code 註解? |
|---|---|---|---|
| 7305cf8d | tier | 硬閘≠#215、cookie 認證化未做/等 M-2-08 | ✅ a(tier.ts 註解) |
| 1fd291d6 | 錢 | 半份資料/兩種語意並存/EMBED_LIMIT=500 | ✅ a(orders-table.tsx JSDoc) |
| fa958b0c | 出貨 | 9 條路構造不出、【不宣稱不可能】、網路層/PostgREST 逾時未涵蓋 | ✅ a(shipping-doc.tsx 逐字有) |
| e0c343db | 出貨 | 200=Sean 口述業務上緣、URL 長度≠列數、max-rows 未量 | ✅ a(shipment-candidates/repository 註解,含 codex R2 的 URL≠列數) |
| 251c2307 | 權限/schema | 未 apply、三支要一起跑、MAINTAIN=PG17-only、本機節錄版 | ✅ a(SQL 檔頭「誠實界」段) |
| 8cb69570 | 權限/schema | 未 apply、寫/驗分離、產生器問題、supabase_admin 仍自帶授權 | ✅ a(SQL 註解豐富) |
| 🌟 92ac213c | 權限/schema | **正式庫 service_role 有沒有 BYPASSRLS 未查(本機 false),但本片不依賴那答案(兩道都顯式)** | ✅ **a 模範** —— 效度限定 + 「缺的那道仍列著:正式庫查 pg_roles.rolbypassrls」都在 SQL 註解 |
| 62acdf57 | schema/契約 | 三個契約決定(linesTruncated fail-closed / null=讀不到 / 不傳維持 fail-closed) | ✅ a(IShippedEmailContext.ts JSDoc) |
| 🔴 15d8b19c | 錢 | **「出貨單 shipping-doc.tsx:81 的新判準我沒核完、刻意不動」** | 🔴 **b —— 只在 body,code 註解 0 命中** |

**b 類(只有一顆)排序**:15d8b19c 的「shipping-doc 未核」是【我沒覆蓋哪個 sibling】型限定 —— 掉了之後下一個人會假設**所有截斷面都 fail-closed 了**、不去查 shipping-doc(=錯的決定,不只是不知道)。⚠️ **但它已自解**:fa958b0c 隨後就把 shipping-doc 那個面改成 fail-closed ⇒ 錯誤決定的窗口已關,現況嚴重度 LOW。

**column d 結論(正向)**:這一欄艦隊做得好 —— 9 顆裡 8 顆把血半徑限定寫進了 code 註解,migration 那幾顆(92ac213c/8cb69570)尤其把「未查/效度限定」直接放進 SQL,**正是 `lessons:1065` 沒做到的那件事的反面模範**。唯一的 b(15d8b19c)還自己解掉了。no c(全都有血半徑限定)。

---

## 5. 真畫面驗證(3001 主樹 dev、真資料、GET-only)

> 前四欄全部是【讀 repo】能回答的,沒有一欄回答:**這些改動在跑起來的畫面上,真的是我們以為的樣子嗎?**
> 方法:對主樹 dev admin server(3001,bypass on)只做 GET,沒 kill/沒起第二台。資料很小=7 張訂單、2 張出貨單。PII 只用 UUID / PCM 顯示碼 / 計數,不抓姓名。
> 🔴🔴 **一句話**:**靜態表四欄全綠,不代表那幾個分支的畫面被人看過。**

### 🔴 這一輪自己的盲面(最該記):我用來「看」的工具,看得到什麼?

> **判別句**:**我用來「看」的那個工具,看得到的是【文字】、【版面】、還是【像素】?三者的盲面不同。**
> curl 拿的是 server 渲染的 HTML ⇒ 看得到**文字**,看不到**版面/像素**。所以「去看真畫面」這件本身也只做到一半 —— C 堆就是這個限定的產物,**沒有混進 A 堆當成驗過**。

### A 堆 —— 今天畫面可見【且比對 body 一致】(5)

| commit | 面 | 驗法 |
|---|---|---|
| 498e85ef | 訂單文案(移除「這期」「退款線」) | 🔴 **先證取消/採購區塊今天有渲染(206 命中)才讀「舊語彙 0 殘留」** —— 沒有那一步,「0 殘留」與「那區今天沒出現」分不開(= 零命中要有分母的【畫面版】) |
| 854efe61 | 簡訊→LINE | 全頁「簡訊」0 殘留 |
| 17c98ce2 | 揀貨單「一項都不用揀時別印『全部勾完才算揀完』」 | 3 張單都 pickableCount=0,新句渲染、舊恆真句 0(今天正好觸發得到且對) |
| 566bfcdf | 續頁抬頭 | 揀貨單(多箱那張)+ 兩張出貨單都渲染 |
| a330bf16/0484f97d/ebcff98d | 紙上內部符號 🔴/⇒ | 全頁 0 殘留(給倉庫的紙乾淨) |

### 🔴 B 堆 —— 推之後【沒有人看過的畫面】(給推上去之後的人,不是給我們的)

> 🔴 **共同性質(要跟這 5 顆一起講,否則「沒人看過」讀起來比實際嚴重)**:它們全是 **fail-closed / 邊界分支** ⇒ 失敗方向是**【擋了不該擋的】,不是【放了不該放的】**。寫錯的症狀是「有人做不了事」= 會被抱怨、會被發現;不是「有東西靜靜溜過去」。

| commit | 沒人看過的畫面 | 要看到得構造什麼資料 |
|---|---|---|
| 15d8b19c | 退款帳本【整區失敗、一列都不印】 | 一張退款列數 > 顯示上限的單(`rowsTruncated`) |
| fa958b0c | 出貨單【擋印 + 診斷訊息】 | 一張 `reportedTotal` 讀不到(對帳訊號缺席)的出貨 |
| 1fd291d6 | 訂單列表金額欄/膠囊「未知」 | 一張 `itemsTruncated`(單筆 > 500 品項)的單 |
| e0c343db | 建箱候選第 201 項之後 | 一張 > 200 品項的單 |
| ea4fc0c4 | 揀貨單 B 態(缺的列印進表身) | 一張有缺列狀態的單 |

（今天 7 張單、2 張出貨都到不了這些觸發條件 ⇒ 全 0。）

### 🟢 實測結果(2026-08-18 鑽機跑通:detached worktree@dev + 拋棄式 PG/PostgREST/proxy/admin:3031,構造觸發資料後真的開頁)

> ⚠️ **鑽機效度限定(§5)**:本機 RLS/GRANT/BYPASSRLS 自補,證不了正式站權限。**且我在 throwaway 補了 migration `20260803150000` 的三欄(`kind`/`failed_detail`/`provider_refund_id_evidence`,`kind` 填 `'partial'`)—— 那三欄的值是我們自己填的、不是正式庫現況**,任何依賴 `kind` 語意的結論要帶這限定。**Screen 4 的截斷邏輯不受影響**(app 層、PostgREST 直查 `0-100/101` 證實)。

每格兩問:(a) 畫面對嗎 (b) 值班知道下一步嗎。🔴 **(b) 是這輪抓到東西的那一問。**

| 螢幕 | (a) 畫面 | (b) 值班知道下一步嗎 |
|---|---|---|
| 訂單列表膠囊(1fd291d6) | ✅ 501 項單狀態欄印「未知」 | 🔴 **錯**:tooltip「這次沒有完整載入…請重新整理」而 itemsTruncated 是結構性、重整無用 |
| 揀貨單 B 態(ea4fc0c4) | ✅ 200 列 + 3 列「?」+「這張表沒有結尾」 | ✅ **對**:「固定限制,不是暫時的狀況…聯絡負責人…不要拿這張去揀貨」 |
| 退款帳本整區失敗(15d8b19c) | ✅ 101 筆(>100)⇒ 一列都不印 | ✅ **最完整**:「看不出這張單有沒有退款…請不要用這一頁判斷…不要在這個狀態下發起退款…通知系統維護」 |
| 出貨單 reportedTotal=null(fa958b0c 面6-b) | ✅ 擋印、不印品項表 | ✅ 對:「讀不到品項總數…系統的問題,不是你操作錯…聯絡負責人…不要拿這張紙出貨」 |
| 建箱候選第 201(e0c343db) | ✅ B 窗 §7 已驗(建箱 501/501 vs 明細 200/201) | — (彈窗=client,B 窗 playwright 驗過,引它) |

🔴🔴 **本輪最大發現(operator guidance 這個病有完整證據)**:三個 fail-closed 畫面【同一個底層條件 itemsTruncated/結構性上限】,而給值班的話 **2 對 1 錯** —— 訂單列表膠囊講「請重新整理」(錯),揀貨單 B 態與退款帳本講「固定限制/看不出什麼/聯絡負責人」(對)。⇒ **這把 Screen 1 從「可能是設計」變成「量到的漏套」,而且範本在同 repo 有兩個。** 判別句:**一個孤例分不出「刻意」與「漏掉」;同 repo 的第二個正例才分得出來。** 已直達 A 窗(34888)四段骨架。

🔴 **reportedTotal=null 的兩態(fa958b0c 面6-b 的邊緣)**:它是【基礎設施故障態、非資料態】(V 窗 9 條資料路徑造不出=對)。而「沒 count」有兩種:①response 無 content-range ⇒ supabase-js count=null ⇒ reportedTotal=null ⇒ **面6-b 乾淨觸發**;②content-range=`0-200/*` ⇒ `parseInt('*')`=NaN 而 `typeof NaN==='number'` 為 true ⇒ reportedTotal=NaN ⇒ 面6-b【滑過】、改由面6 觸發,畫面印「資料庫說有 **NaN** 項」漏給值班看。兩者都擋印(fail-closed 安全),但②的 (b) 有瑕疵。是否在正式站發生取決於真實故障回哪種 content-range,無法從正式站證實。
> 🔴 **②是真 bug,已立 `#634`(寄 B 窗)**:病根=守門 `typeof count === 'number'` 想擋「沒有數字」,實際擋的是「不是 number 型別」,而 `NaN` 是 number ⇒ 滑過。修法一行 `Number.isFinite(count)`。負測必須斷言【畫面不含 `NaN` 字樣 + 走面6-b】,不是「有擋印」(修前也綠、零判別力)。**病名:註解說得出「要有數字」,斷言只表達得出「型別是 number」。**
> 🔴 **交接句**:`reportedTotal=null` 是【基礎設施故障態,不是資料態】—— V 窗當時寫「造不出來」不完整,正確是「用資料造不出來,因為它不在資料那一層」(前者讀起來像「大概不會發生」,後者說「發生時來自別的地方」)。

### ⚠️ C 堆 —— curl 驗不到,需【真瀏覽器/截圖】(3)

| commit | 為什麼 curl 答不了 |
|---|---|
| fc065f3f/8a881975/c401c41c | 整幅阻印【版面】,且只有 guard 觸發才顯示 |
| 8e576886 | 量測管線狀態錨點 = 視覺量測 |
| 416a4791 | 頁碼離紙邊 5.1mm(印表機不可印區那帶)= 尺寸 |

⇒ 待辦:Design session 或開真瀏覽器截一輪。curl 只證頁面 200 + 文字在。

---

## 既存債(這輪審出、非本批引入)

- 🟡 **面8 訊息口徑不一致**:`apps/admin/src/components/print/shipping-doc.tsx:145` 面8「讀不到任何品項」仍寫「請重新整理」,而面6/面6-b 已立「系統問題不叫使用者重試」的口徑。C 窗這三片(fa958b0c/8e576886/91df7f90)【沒動它】⇒ 屬既存債,列供日後統一,不算本輪 FAIL。
  - 🔴 **統一口徑時要逐面判,不是一律禁詞根(C 窗補)**:面8 是【投影出問題】,重整【真的可能好】;而面6-b 是【固定上限】,重整一百次同一個數字。⇒ 判準=「這一面重整有沒有用」,不是機械地禁掉「請重新整理」。

## C 窗三片審查(fresh code-reviewer opus,2026-08-18)

- **實質三角度 PASS**:① 91df7f90 動守門【順序】無對稱誤攔(窮舉 lines{空/非空}×items{空/非空}×reportedTotal{null/對上/對不上},面8 只在真空單 reportedTotal===0 才發,shipping-doc.tsx:135-146)② 三片無矛盾,且 8e576886【不在】shipping 守門鏈(它動 picking-doc + page-measure.test.tsx)⇒ 三片不是同一條鏈 ③「9 格未修」清單措辭誠實。
- 🔴 **must-fix(字面 vs 事實)→ ✅ 已解 R2 PASS**:`page-measure.test.tsx` 檔頭給下一個人的可重跑 grep 宣稱「9 行」,實跑 15 —— 多的 6 行是檔頭自己的字面清單被自己 pattern 掃到(留痕掃到自己的訃聞)。實體斷言數 9 是對的,錯的是那條檢查。C 窗 `0bf90df3` 挑修法①(加 `expect(` 錨)。**T① 直接複驗(字面帳=確定性計數,非判斷,依 §1 不為幾個 tool call 的機械檢查另派 subagent)**:新命令 = 9 ✓、且【結構免疫】——限定在註解行時匹配 0(註解無 `expect(`)⇒ 檔案再長也不會回膨脹;檔頭並保留舊 bug(15、6 自match)當雙向對照。三綠過、該檔 6 passed(C 窗量)。R1 FAIL→修→R2 PASS,本片審查關閉。

## 小結(推前)

> 🔴 **2026-08-18 10:5x CST W1 收割時更正:本節【兩條都已關閉】,而本檔一度自己跟自己打架。**
> 追加「## C 窗三片審查」那段的人沒有回頭改本節 ⇒ 讀本節的人會以為缺口還開著。
> **原文逐字保留在下面的刪除線裡,零刪除** —— 改的是【現況判定】,不是作者當時的紀錄(他當時寫的是對的)。

**兩個推前要處理的 —— ✅ 兩條都已關閉(2026-08-18 10:5x CST W1 實查)**:
1. ~~🔴🔴 **a1210748(錢的 fail-open 修法)not-in-dev** —— 推 dev 不會帶上。已單報,I 窗去收。成因=「押著等複驗、複驗過了沒人解押」。~~
   ✅ **已進 dev**。量法(可重跑):`git merge-base --is-ancestor a1210748 dev` ⇒ rc=0;
   修法招牌字 `grep -n refunded_amount_missing packages/use-cases/src/settle-charge.ts` ⇒ `:380 :390 :437`。
   ⚠️ 本檔上文寫「它把 dev 弄紅了(`sweep-settlements.test.ts` 13 格 AssertionError)」—— **那個紅也已解**
   (`9fda4719`「sweep 的 fixture 補 refunded_amount=0」,`git rev-list --count dev..sso-security-log-tests` ⇒ 0)。
   證據=W1 2026-08-18 10:19 CST @ `3c48e938` 的四綠:`Tests 8645 passed`、**0 failed**、rc=0。
2. ~~🔴 **fa958b0c 無人審** —— 且翻了既有測試斷言(規則的洞)。推前補一次 code-reviewer。~~
   ✅ **已補審**,而且**就寫在本檔自己的「## C 窗三片審查(fresh code-reviewer opus,2026-08-18)」段**,
   逐字「R1 FAIL→修→R2 PASS,本片審查關閉」;三片 = `fa958b0c` / `8e576886` / `91df7f90`。
   ⚠️ **來源屬性(不得美化)**:這一條 W1 是【讀本檔那一段】得到的,**沒有複驗那次 code-reviewer 審得對不對**。
   🔴 **仍未關的是那個【規則的洞】本身**:「翻既有測試斷言」不在片型分級的六類裡 ⇒ 規則沒擋它。
   補審關掉的是這一片,不是那個洞。機制優先律候選,尚無編號。

**已澄清、非缺口**:
- 其餘三顆 not-in-dev(a22f6e2d/7f6d0ac1/de7cc9ef)= 施工中正常。
- 15d8b19c / 251c2307 = **審過了(codex+V窗 / codex 三輪),只是審查帳沒進 body**。
- column b 三顆全部【原窗親拆確認紅】:15d8b19c 2 格、fa958b0c 2 格、251c2307 元資料層(刻意)。

**比那些 commit 本身更重要的兩條(寫進制度)**:
- **column a 分母陷阱**:一整欄在自己的分母上恆真=零判別力(見檔頭)。
- **column c 載體律**:審查證據活在 commit body ⇒ 永久;活在訊息/marker ⇒ 會消失(reviewer-gate marker 不留歷史)。錢/權限/schema 的審查結論必須寫進 body。
- **column b 時點漂移**:數字「當時對」但檔案變了沒回頭重量(15d8b19c 的「1 vs 2 failed」);逐格點名(不用 N 格制)結構免疫(E 窗 628-verify.sh)。

**附註**:
- 15d8b19c 的 body「1 failed」是錯的,更正在 `~/pcm-mailbox/T-008-STOP-20260817.md §⑤-b`;收割 merge body 引 STOP、不引 commit body(I 窗已照辦)。
- 251c2307 的 `628-verify.sh PASS=32 ≠ 正式庫 apply 會過`(本機三欄節錄版,驗不到正式庫 owner/grantor/RLS/ACL/角色拓樸)。
- 8e576886(量測管線 refactor)body 有一句歸因錯誤(V 窗在 C 窗施工中工作樹上 `cp` 還原蓋掉了 C 窗的還原,C 窗誤記成「腳本沒吃到」)—— C 窗走信+STOP 更正,那段期間的突變量到的是半成品狀態、V 窗自己作廢。
- 檔名相撞已解:5681 把它的 traps-inbox 檔改名 `T2-20260818.md`(883fabb2),本窗的 `T-20260818.md`(da070870)不用動,`git merge-tree` 已 rc=0。
