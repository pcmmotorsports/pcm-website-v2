...
## 2026-06-21 querystatus-fix `22f780b` 關卡2 最終 sign-off = ✅ PASS(審查側、寫審分離 ROLE=A)

**commit** `22f780b`「fix(use-cases): 修 settleCharge queryStatus=2 誤判致 3DS 授權單卡 pending」(9 檔 +108 -23、worktree m3-3ds-qsfix、未 push 未 merge)。

**背景(真刷暴露)**:2026-06-21 sandbox 3DS 真刷 PCM-2026-0018 / rec `D202606210tqU8C`(TapPay 後台=已授權 Auth Code 849647=record_status 0)→ 訂單卻永遠卡 pending/待付款。root cause = settle-charge.ts L85 `queryStatus !== 0` 把 TapPay 查詢成功態 top status=2(官方逐字「已無更多分頁」、查詢成功、≠交易狀態)誤殺成查詢失敗 → 到不了 classifyRecordStatus → **S1「授權即成立」實際從未生效**。bug 被 test L151-153「queryStatus=2→unverified」固化、三綠恆綠無人察覺;今天真刷打臉。

**三角度全收斂 PASS、0 must-fix、0 安全破口**:

1. **Claude fresh-context 審(不可變 git show)**:
   - 核心:L85 改 `!isQuerySucceeded(queryStatus)` 白名單 {0,2}、named helper fail-closed docstring;**count===1 + records.length===1 + recordMatchesOrder 三重縱深全保留**(同一 gate);observability console.warn 零 PII(orderId/queryStatus/count、不改裁決、仍 fail-closed pending)。
   - 🔴 脊椎零變更:settle-charge.ts diff 僅 L85 gate + helper + 註解;classifyRecordStatus / recordMatchesOrder / settlePaid / markFailed 零出現在 diff。record_status 仍成立權威、status 只管查詢成功。
   - 字面 vs 事實:active docstring(domain types L182-186 / wire L69·93·104 / PollOrderStatus)改正準確、語氣對齊 §8 不過度斷定;凍結 plan(設計包 §5.1/§6、S2 callback plan L134)加 supersede 註記非改寫。
   - 測試:R1-R5b(root cause 正向 + record_status 全 table 逐態 it.each + 白名單嚴格 qs=99 fail-closed + count/records 縱深拆兩)+ adapter A1(raw status=2 有紀錄不丟 trade_records)/A2 改名;改 L151-153 移除 bug 固化。
2. **fresh 三綠(審查側獨立重跑、worktree m3-3ds-qsfix)**:typecheck 7/7 + lint 10/10 + **full vitest 134 檔 1395 測全綠**(對齊執行側報告)。
3. **codex K2 跨模型(read-only、零留痕)= PASS、0 finding、6 維**:核心安全(白名單 fail-closed + 三重縱深未弱化)/ 脊椎零變更(自跑 git diff 證)/ 字面 vs 事實 / 測試覆蓋 / observability 零 PII / PCM 鐵則(無 schema·env·deploy、金額整數、零經銷價/卡資料洩漏、commit 對齊 diff)。

**STATUS 7 欄**:當前 slice 準確述 querystatus-fix;最近 commit `cb43cc3` 是 `22f780b` 可達祖先(merge-base 驗);前序 S2b ✅ 已 merge+push、本地 f6db42c+cb43cc3 領先 origin 2。

**forward note(非阻擋)**:codex K2 + 審查側皆確認殘留舊「queryStatus=2 查無」語意只在 **out-of-scope 凍結/歷史檔**(webhook master-plan L131、本 review-log 早期條目)、plan §3c 明確未列、非 active contract、commit body 誠實標「不動」。可未來順手清、非本 fix 範圍。

**🔴 重大 forward 觀察(fix plan §8、待 Sean 重刷實證)**:修 L85 後若 callback **首次** settleCharge 即成立(無需等輪詢/webhook/sweeper),則證明設計包 §5.1「Record 同步延遲」歸因錯誤、真因純為 L85 bug → 後續可 revisit S2 輪詢 / S2b 主動 settle 的必要性(可能可簡化)。本 fix 不動 S2/S2b、僅記觀察。

**sign-off**:✅ PASS。審查側無保留。待 Sean:① merge `m3-3ds-qsfix`→dev(--no-ff、9 檔含 STATUS/settle-charge/測試/字面)② storefront `pnpm build && pnpm start` rebuild ③ 重刷一筆 sandbox 3DS(或對 PCM-2026-0018 輪詢)→ 訂單應幾秒內「待付款」→「處理中」(payment_status=paid)。純 code、migration/db push 無涉。
