...
```
branch      : dev
HEAD        : 🔴 **本包一律不寫死 commit hash 與 ahead 數**(見下方說明);以實跑為準:
                git log --oneline -3   /   git rev-list --count origin/dev..HEAD
origin/dev  : 9694561(此值只在 push 後才變,故可寫)
狀態        : **已 commit、未 push、未 apply**(B-1 本體 + 依 Codex 各輪 findings 的修正 commit)
...
| 🔴 突變驗證 ×3 | 逐項證明 DO 3b2 **6 項核對**不是假綠:拔 `^[!-~]+$` → RAISE `MUTATION_CAUGHT`;拔 email 形狀 regex → 偵測到;拔合成域精確等值 `<>` → 偵測到;**正常版三次皆未誤報**(`PROBE_RESULT` 三項皆 t)。⑤⑥兩項為 **Codex 第二輪補提**(前版只核四項) |
...
> 第一輪 Codex must-fix #3 + 第二輪補正:**本節逐段標示「完整」或「節錄」,不做籠統的「全文」宣稱**。
> 全部對 prod(project `bmpnplmnldofgaohnaok`)執行;寫入型者皆以**結尾無條件 RAISE 原子回滾**、事後查零留痕。
>
> | 段 | 內容 | 完整度 |
> |---|---|---|
> | §7.1 | 第二輪交易模擬(15 條斷言) | ✅ **完整 SQL + 逐字輸出** |
> | §7.2 | 述詞矩陣第二批(12 個重音/類 ASCII 對抗樣本) | 🟡 **完整 SQL**;輸出為**節錄關鍵欄**;第一批 23 樣本**未逐字列出**(結構相同) |
> | §7.3 | 第三輪(加入 3b2 六項核對)+ 突變驗證 ×3 | ✅ **完整 SQL + 逐字輸出** |
...
### 7.3 突變驗證 ×3(證明 DO 3b2 六項核對不是假綠)—— 完整 SQL 與逐字輸出

> 第三輪模擬 = §7.1 的 15 條 **＋** §6 檔內 DO 的 3b2 六項核對,輸出為
> `ERROR: P0001: SIM_ALL_PASS_V3 全數通過(含新增 3b2 述詞內容核對 4 子項、且未誤報),本 RAISE 為刻意回滾`
> (⚠️ 該次訊息寫「4 子項」是當時只有四項;第二輪 Codex 補提後已增為 **6 項**,下方即 6 項版的突變驗證。)

```sql
DO $$
DECLARE v_def text; v_ok boolean; v_mutA boolean; v_mutB boolean;
BEGIN
  ALTER TABLE public.orders ADD COLUMN notification_email text;

  ALTER TABLE public.orders ADD CONSTRAINT c_probe CHECK (
    notification_email IS NULL OR (
          notification_email = btrim(notification_email)
      AND notification_email ~ '^[!-~]+$'
      AND octet_length(notification_email) <= 254
  ...
  SELECT pg_get_constraintdef(oid) INTO v_def FROM pg_constraint
   WHERE conrelid='public.orders'::regclass AND conname='c_probe';
  v_ok := (position('^[^@]+@[^@]+' IN v_def) > 0 AND position('<> ''line.pcmmotorsports.local''' IN v_def) > 0);
  ALTER TABLE public.orders DROP CONSTRAINT c_probe;

  -- 突變 A:拔掉 email 形狀 regex
  ALTER TABLE public.orders ADD CONSTRAINT c_probe CHECK (
    notification_email IS NULL OR (
          notification_email = btrim(notification_email)
      AND notification_email ~ '^[!-~]+$'
  ...
  SELECT pg_get_constraintdef(oid) INTO v_def FROM pg_constraint
   WHERE conrelid='public.orders'::regclass AND conname='c_probe';
  v_mutB := (position('<> ''line.pcmmotorsports.local''' IN v_def) = 0);

  RAISE EXCEPTION 'PROBE_RESULT 正常版兩項皆命中=% | 突變A(拔 email 形狀)被偵測=% | 突變B(拔合成域等值)被偵測=% | 三者皆 t 才算斷言有效',
    v_ok, v_mutA, v_mutB;
END
$$;
```
...
**逐字輸出**:
```
ERROR:  P0001: PROBE_RESULT 正常版兩項皆命中=t | 突變A(拔 email 形狀)被偵測=t | 突變B(拔合成域等值)被偵測=t | 三者皆 t 才算斷言有效
CONTEXT:  PL/pgSQL function inline_code_block line 45 at RAISE
```

**另一次突變(拔 `^[!-~]+$`)逐字輸出**:
```
ERROR:  P0001: MUTATION_CAUGHT ✅ 3b2 有效 — 偵測到弱化版 CHECK 缺「可列印 ASCII」成分,斷言正確 RAISE
```

**事後零留痕實查**(含臨時 `c_probe`):
```
residual_column=0 / residual_any_check=0 (orders_notification_email_valid 與 c_probe 皆無)
orders_total=30 / orders_column_count=30 / remote_latest_migration=20260717020000
```
