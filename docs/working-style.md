...
### 2.1a 🔴 一個字回得完 —— 判別「這是決策題還是告知」(2026-08-20 Sean 指示落檔)

**判準:我這則訊息,Sean 可以用【一個字】回完嗎?**
```
可以 ⇒ 它是決策題
不行 ⇒ 🔴 它是【告知】—— 而**告知不會產生決定，它只會產生「我講過了」**
```

**Why(2026-08-20 兩次實錘,主視窗)**:
- 「這條可以劃掉了」—— 那句是錯的(那份改動存成 patch、工作樹已還原,根本不在程式碼裡)。
  🔴 **而 Sean 什麼都不必回 ⇒ 那句錯話沒有任何回饋路徑**,直到 40 分鐘後另一個窗實查才發現。
- 「3 顆欠審要不要補審」寫在一份清單裡,而**清單沒有跟著「推」那句指令一起到** ⇒
  他在讀到之前就推了 ⇒ **那三顆的選擇被時序吃掉了 —— 不是他選了甲,是他沒有機會選。**

**兩條連帶的規矩**:
1. 🔴 **凡是「他決定之前要知道」的東西,必須寫在【叫他決定的那句話裡】** ——
   不是附件、不是「我等下發你」、不是另一個檔。
   **那個「之前」不是文件品質的一部分,是排程的一部分** ⇒ 清單寫得再好都救不了。
   判別句:**如果他只讀我這則訊息的前三行就動手了,他會不會漏掉什麼?**
2. ⚠️ **「你可以劃掉了 / 你不用做什麼」這類句子最危險** ——
   它們既是**決策觸發**(讓他停止查證),又**不需要回應**(所以錯了沒有人會說)。
   ⇒ 這種句子要附**證據的落點**(檔案:行號 / 我怎麼量的),不能只有結論。
