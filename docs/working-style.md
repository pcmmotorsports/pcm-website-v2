...
- Supabase migration 兩階段、commit 進 git ≠ DB 已套用、Claude.ai 寫指令引「落地」字面時必明確區分:「commit 落地」= migration SQL 進 git / 「apply 落地」= DB 已套用
- slice 指令涉 view / table / column 行為時、必含驗證階段(commit-only / 含 apply);若需 apply 落地、必含 Step「Sean 手動跑 supabase db push」(對齊四方分工、Code 不代跑)
- commit body 字面紀律:commit message 寫「新增 X view」時、body 必揭示 apply 狀態
- 違反偵測:Code 偵察揭示「commit 落地但 apply 未落」立即 raise multi-select(對齊鐵則 11「事實 > 字面」)
- 跨專案適用:適用所有 Sean 用 Claude Code + Supabase migration 的專案
- **教訓來源:** M-1-05 刀 1 spike 撞 migration drift、刀 1.5(`e2ac99a`)補 apply 修法 / lessons §12-31

**第 41 條:Claude.ai 不憑印象推 MCP / CLI 工具行為(對應 lessons §12-32)**

- 寫指令引 MCP / CLI 工具行為前必驗 schema + flag、不憑印象(特別 timestamp / version 處理、flag 必要性、副作用)
- 真權威 = 當下實測 schema + dry-run(MCP tool list parameter schema / CLI `--help` 字面 / 工具行為實測);不憑訓練資料推
- 違反偵測:Claude.ai 寫指令憑印象 → Code raise multi-select、Sean 拍板再執行
- enforce:Code Step 1 確認字面、Claude.ai 寫指令字面前完成 schema + flag grep
- 跨專案適用:適用所有用 MCP / CLI 工具的專案(Supabase / Vercel / GitHub MCP / 第三方 CLI 通用)
- **教訓來源:** M-1-05 刀 1.5 揭示 MCP apply_migration timestamp 倒掛、改用 Supabase CLI db push 修法 / lessons §12-32

**第 42 條:Claude.ai 寫指令字面前必先 grep callsite 真權威(對應 lessons §12-33)**

- 寫指令字面涉「mapper 拆分 / 投射欄位列舉 / type 簽名 / 既有 method 字面」前必 grep callsite 三層真權威:domain / port type(`packages/domain/src/**/types.ts` + `packages/ports/src/I*.ts`)/ DB 投射(`supabase/migrations/*.sql` + `information_schema.columns`)/ adapter mapper 簽名(grep `map.*ToDomain` / `map.*ToSupabase`)
- 不憑「概念上應該夠用」印象推、屬具體技術細節必字面驗
- 違反偵測:Claude.ai 寫死投射欄位 / type 簽名 / mapper 字面前未標 `<待 Code 字面確認>` 占位 → Code raise multi-select、Sean 拍板才執行
- 跨專案適用:適用所有 Sean 用 Claude Code 開發的專案
- **教訓來源:** M-1-05 刀 2 Sub-slice 2-3 Option A 收斂 detail view 事故源、Claude.ai 規劃憑「list view 投射欄位夠用」印象未 grep callsite / lessons §12-33

**第 43 條:Claude.ai 寫立法字面前必 view 末條編號(對應 lessons §12-34)**

- 寫「立法新條」字面前必 view 末條編號:working-style §6.3 `grep "^**第" docs/working-style.md | tail -3` / lessons §12 `grep "^### 12-" docs/lessons-learned.md | tail -3` / ADR `ls docs/decisions/` 拿最大 / backlog `grep "^### #" docs/phase-1-backlog.md | tail -3`
- 不憑「上次落地到 N、下次 N+1」印象推、跨 session 期間可能有其他 slice 落地中間編號
- 立法字面內引「對應條」編號時必同樣 grep view 驗
- 違反偵測:Claude.ai 寫立法字面前未標 `<待 Code view 確認末條編號>` 占位 → Code raise multi-select、Sean 拍板再寫
- 跨專案適用:適用所有有編號制立法的專案
- **教訓來源:** M-1-05 刀 3-a 規劃憑印象推「第 40 + 41 條」事故源(本次推對僥倖)/ lessons §12-34

**第 44 條:Claude.ai 列 multi-select 跨選項字面格式 / 詳簡度 / 三視角描述必統一(對應 lessons §12-35)**

- 列 multi-select 跨選項字面統一三層:格式(標題 / 段落 / 列舉項 / 術語固定)/ 詳簡度(跨選項長度相當、不容詳簡不一)/ 三視角(每選項擴充性 + 可維護性 + bug 可追蹤性 各一句、不缺、不混用其他維度替代)
- 違反徵兆:Sean 多輪比對 / Code raise multi-select 校準字面差異 = 跨選項字面不一致
- 跨專案適用:適用所有 Sean + Claude.ai 用 multi-select 拍板協作的專案
- **教訓來源:** M-1-05 刀 2 Sub-slice 2-3 multi-select 拍板、Claude.ai 規劃端回報跨選項不一致(立規型、具體事故無 git 實據)/ lessons §12-35

**第 45 條:Claude.ai 寫立法字面前必 view 既有條目格式真權威(對應 lessons §12-36)**

- 寫「立法新條」字面前必 view 既有條目格式真權威:lessons §12 末三條完整字面 `view docs/lessons-learned.md` 涵蓋 §12-(N-2) 至 §12-N 條尾 / working-style §6.3 末三條 `view docs/working-style.md` 涵蓋 第 (M-2) 至 第 M 條條尾 / ADR 末條 `view docs/decisions/000N-*.md` / backlog 末條 `view docs/phase-1-backlog.md` 末條條尾
- view 範圍含「條尾 marker」(下一條起始 / 章節分隔 / 檔尾)、確認插入點精確
- view 後 verbatim 比對立法字面:模板段數 / bullet 結構 / 段落 marker(`**N:**` 同行 vs 換行)/ 條間 marker(`---` 或空行)、任一不對齊即重寫
- 不憑「上次落地是 N 段」/「印象中是 bullet」推、跨 session 期間既有格式可能微調必字面驗
- 立法字面內引「跨檔對應條」(lessons §12-N ↔ working-style 第 M 條)必雙向 view 驗對應存在
- 違反偵測:Claude.ai 寫立法字面前未標 `<待 Code view 確認模板>` 占位 → Code raise multi-select、Sean 拍板採 C 路線(Claude.ai 重寫、不允 B 路線 Code 代排越四方分工)
- 跨專案適用:適用所有有編號制立法 + 模板格式的專案(lessons / ADR / RFC / patterns / backlog 通用、不限 PCM)
- **教訓來源:** M-1-06 a11y polish slice amend(`61d7fc0` → 新 hash)4 處同族事故源:CSS 評估容器結構誤判 / 9 處 arrow grep pattern 範圍不足 / amend slice grep pattern 命中 0 meta / amend Step 2 立法字面 7 段 prose 結構錯誤 meta(本條自身誕生過程符合本條規則、自落地驗證)/ lessons §12-36
