...
>
> ⛔ ~~**今天的實際暴露 = 0** —— `HCT_SUBMIT_ENABLED` / `HCT_API_ENDPOINT` 沒設~~
> 🔵 2026-09-15:新竹已開送(2026-09-10 第一箱送成功)⇒ 這一份現在是真的會用到的。
>
...
⛔ ~~而「查」這件事今天【只能打電話】 —— 新竹那個服務只講 SOAP,我們的查詢功能還沒接~~
🔵 **2026-09-15 起:先在那一箱按「向新竹查詢貨號」**(甲、乙兩型都有,只查不送;
plan `docs/plans/2026-09-15-hct-carrier-replied-exit-plan.md` 片 A)。
- 查到 ⇒ 系統自動記成「已送出」並帶貨號 ⇒ **不要重送**。
  ⚠️ 查回來的資料**沒有標籤圖**:同一天 ⇒ 在訂單明細頁按「重新取得標籤」;
  **隔天才救回的箱 ⇒ 沒有按鈕拿得到標籤 ⇒ 把這一箱作廢、重新開一箱再送新竹**(拿新貨號、新標籤)。
  舊的新竹單可以打電話請新竹取消(新竹沒有取消 API),**不取消也沒關係**(Sean 2026-09-15:「我們本地重新做一張就好…或者可以取消更好」)。
- 查無 ⇒ 🛑 **查詢還沒對新竹真打驗證過**,「查無」不能當成新竹沒收到的證據 ⇒ **照舊打電話確認**。
  乙型(新竹回過話)查無 ⇒ 不給放回草稿;打電話給新竹,**新竹確認沒有這張單 ⇒ 把這一箱作廢(舊箱留紀錄)、重新開一箱再送新竹**(Sean 2026-09-15 Q2 甲)。
- 查不到答案 / 顯示未開通 ⇒ 打電話問就是查,那不是繞過流程。
