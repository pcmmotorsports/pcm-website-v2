...
❌ 不能證 **「在後台做了一個動作 ⇒ 客人那一端看得到」** —— 見下方那一格,這是【工具的邊界】不是你沒走完
```

### 🔴🔴 **跨端走查(後台做動作 ⇒ 顧客站看結果)今天做不到 —— 這是【已知的】,不要重新發現一次**

> **2026-09-04 線 `-db` 實測、主視窗 `-94` 拍板「丙:不做,而把它明寫在這裡」。**

```
admin-probe      PG 55534    ← 後台那一端的世界
storefront-probe PG 55533    ← 客人那一端的世界
⇒ 兩個【不同的資料庫】, 各自 initdb、各自種子。
```

🎯 **⇒ 你在後台鑽機出的貨,客人鑽機那一頁【看不到】—— 因為那張單根本不在它讀的那個庫裡。**

🛑 **⇒ 而這一格最貴的地方是它的形狀**:兩台都會正常起來、兩邊畫面都對、**沒有任何東西會報錯** ——
　 你只會看到「客人那頁沒有那張單」,而它與**功能壞掉**長得一模一樣。

✅ **所以撞到的人該寫的是**:「**我沒走到 ⑥,因為兩台是兩個庫**」,
　 **不是**「⑥ 走不通」。📌 **那兩句話派出方向相反的工。**

🔵 **為什麼今天不修**(2026-09-04 量的,理由留著):
```
· 不是刻意隔離 —— storefront-probe/up.sh 檔頭逐字「把同一條鏈接到 apps/storefront」、
  :150 逐字「與 admin-probe/up.sh 同一段, 逐字相同」⇒ 它是【抄出來的】
  (找「刻意隔離」的理由 ⇒ 零命中;正對照找「拋棄式」⇒ 3 命中, 尺會動)
· 而「讓一台指向另一台的 PG」不是一行 env —— up.sh 沒有「PG 已經在跑就沿用」那條分支,
  它只會偵測 `Address already in use` 然後報紅
· 🔴 而 down.sh 會刪資料目錄 ⇒ 兩台指向同一顆 PG 時,【先收的那台會把另一台的世界刪掉】
```
📎 **完整評估與三條修法** ⇒ 板列 `⟦01-TWOPROBESTWODBS⟧`。
...
## 🔴🔴 驗收順序不是隨意的 —— 它由【重跑會清掉什麼】決定(2026-09-04 線 `-ship`)

**現場**:票 **15 分鐘**到期。我把四格走完(訂貨 → 到貨 → 出貨 都成功),
而**最後那一格**(「勾了確認之後收款鈕會不會亮」)走到時票剛好過期。

🛑 **而「重跑一次補做」對這一格【結構上】不成立** —— `up.sh` 會 `rm -rf` 資料目錄並重種:
```
重跑 ⇒ 種子回到原樣 ⇒ 我前面走出來的那些狀態(採購 / 到貨 / 出貨)全部消失
⇒ 「留到最後補」= 那一格永遠排在一個【會被清掉的東西】後面
```

### ✅ 排序規則

> **1.** 不依賴前面步驟的格子(純畫面狀態、按鈕灰不灰)⇒ **排最前面**,票一到手就做。
> **2.** 依賴種子累積狀態的格子(要先下採購才驗得到到貨)⇒ 排後面,而它們**本來就必須連著跑完**。
> **3.** 🔴 **腳本先寫好, 再起鑽機** —— 起站到票到期只有 15 分鐘,
>    而「起好了才開始想怎麼點」會把那 15 分鐘花在寫 selector 上。

### ⚠️ 兩個同日踩到的環境坑

- 🔴 **工具逾時會把 dev server 一起殺掉**:它是那個 shell 的子行程 ⇒ 一發 2 分鐘的逾時砍掉整組,
  下一發 `up.sh` 直接 `rc=1`。✅ **修法:給足逾時, 或先 `down.sh` 再 `up.sh`。**
- 🟢 **而 `up.sh` 把「本次沒有起任何東西(rc=1)」印在【最後一行】** ——
  它檔內逐字寫著理由:**會改變結果意義的訊息必須是最後一行**。
  📌 那讓我沒有把「既有那一份」的埠讀成自己的。**這是那支腳本做對的地方, 抄它。**
