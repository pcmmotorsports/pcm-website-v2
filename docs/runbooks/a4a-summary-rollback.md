...
> 🔴 **本節與步驟 ⑦ 用版本號指稱 migration(`20260730150000` 這種),而你要打開的是【檔案】。**
> **貼之前先把版本號換成檔名,並確認它只對到一支**:
> ```bash
> for v in 20260730150000 20260803140000 20260806100000 20260806180000; do
>   printf '%s -> ' "$v"; ls -1 supabase/migrations/${v}_*.sql | wc -l
> done
> ```
> 2026-08-25 實跑 ⇒ **四行都印 1**。印 **0** ⇒ 那支檔不在這棵樹上(版本號打錯,或你在別的 branch)。
>
> 🔴 **要逐行看,不要只看 rc** —— 負對照實測(餵 `29990101000000`):
> zsh 印一行 `no matches found: …` **然後照樣印 `0`,而整個迴圈 `rc=0`**。
> ⇒ **打錯版本號不會讓這個迴圈失敗**,它只會在一堆 `1` 中間夾一個 `0`。
> ⚠️ 兩件本檔**沒有實測**、不要靠它們接住你:
> ① bash 對 glob 不匹配的行為與 zsh **不同**(zsh 會出聲,bash 不會)——**你的 shell 是哪個,自己確認**。
> ② 版本號打錯之後 `psql -f` 會怎麼表現。
> (2026-08-25 補。⚠️ 原句我寫成「四行全 0 也只是安靜地印四個 0」—— **那是我沒跑就寫的,zsh 會出聲。已改成實測值。**)
