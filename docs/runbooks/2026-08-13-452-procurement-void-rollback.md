...
> 🔴 **貼之前先看「回退當天必做」那三條(它們寫在本檔 §「回退之後」,而那是一百多行之後的事)**:
> 跑完 SQL **不等於**回退完成 —— 那三條逐字說:①`20260813120000` 在 history 上**仍然是 applied**
> 而 schema 上已經不在 ⇒ 必須在 `supabase/APPLIED.tsv` 與當日 handoff 寫明「已回退」;
> ②`scripts/b2s2b-truth-sync.py` 的 `helper-452` 位置要從 `SITES` 拿掉;③原因寫進當日 handoff。
> **跳過去**:在本檔搜尋 `回退不是失敗`(數法 `grep -n '回退不是失敗' <本檔>`,2026-08-25 實測 ⇒ **2 行**)。
> 🔴 **第一個命中就是你正在讀的這一行** —— **你要的是第二個**(那一節的末句)。
> ⚠️ 上面是**指路,不是摘要**——**三條的完整字面在那一節,以那裡為準。**
>
> 📌 **這一格自己踩過兩次,值得留著**:錨原本用「history 上」⇒ 本段也寫了那四個字 ⇒ 讀者會先命中這裡;
> 換成新錨之後**一模一樣的事又發生了**,因為**指路的那句話必須把錨唸出來才叫指路**。
> 🔴 **⇒ 一個寫在文件裡的 grep 指路,結構上不可能不命中自己。**
> 能做的不是找一個更好的錨,是**把命中數與順序講明白**(就是上面那一行)。
