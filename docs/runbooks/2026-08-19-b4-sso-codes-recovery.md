...
>
> 🔴🔴 **2026-08-19 降級:本檔已從【主要路徑】變成【純保險】。**
> 原因:B4 的三個動作改寫成**單一 `ALTER TABLE` 語句**,而單一語句的原子性
> **不取決於任何外層** ⇒ 本檔預設要救的那個「有欄位、沒有 CHECK」的半套狀態,
> **在現行寫法下構造不出來**(失敗路徑實測:兩欄跟著整句回滾)。
> ⇒ **量測與理由的正本在 migration 檔頭**:
>   `docs/specs/2026-08-19-m4b-e8b-b4-sso-codes-columns-migration-draft.sql`(commit `1ef78a7c`)
>   —— **本檔刻意不抄那份量測**,抄了就有兩份會各自過期。
> ⚠️ **而本檔仍然要留**:apply 管道若做了預期外的事,它是唯一的回頭路。
