...
## 🔔 觸發式待辦:新竹給了 `rtn_code` 對照表的那一天(2026-09-16 立)

**現況**:我們**沒有**新竹失敗碼的對照表。
`apps/admin/src/lib/shipping/hct-client.ts:538` 逐字:
> 「`rtn_code` 我只在範例裡看過 `1`(成功)—— 失敗時它是什麼**未知**(plan §7-3)」

⇒ 所以 `:540-542` 對任何非 `1` 的值一律回 `{ kind:'unknown', reason: 'rtn_code_<值>' }`,
畫面印「新竹的回答我們看不懂(rtn_code_XXXXXXX)」。
🔬 **2026-09-16 真的發生了一次**:Sean 叫車,新竹回 `rtn_code_120000004`,
而那個碼**在我們手上的 API 文件裡零命中**(文件第 8 節自己就叫「未確認清單(禁止當事實實作)」)。

### 🔴 觸發條件與第一件事
**只要拿到新竹的 `rtn_code` 對照表(向站所索取、或新版文件裡出現),第一件事就是回來做這兩樣:**
1. 把對照表落成一支查表(位置:`hct-client.ts` 旁,與 `classifyHctUnknown` 同層),
   讓 `rtn_code_*` 這種 reason **能翻成人話**,而不是把代碼原樣丟給員工。
2. 分類要跟著改:**現在所有非 `1` 都歸成「不知道」** —— 有了對照表才分得出
   「**確定失敗(可以重按)**」與「**真的不知道(不能重按)**」。
   ⚠️ 那個差別是**能不能再按一次叫車**,而按錯的代價是**叫兩次車**。
