...
> ### ⛔ **而「`total` 是 null ⇒ 客人看不到有幾筆」是假的**(2026-09-19 a1 自陳更正)
> ⛔ ~~客人看不到搜到幾筆~~ ⇒ **客人看得到。**
> ```
> 🔬 /search?q=rpm 頁面逐字:「共 1110 件,顯示前 25 件」
> ```
> 🔴 **成因**:本節量的 `/api/search` 是 **header 快搜下拉**那一支(固定 8 筆、`total` 恆 `null`),
> 　 **不是客人打開的那一頁**。搜尋頁的筆數是另一條路來的。
> 🎯 **⇒ 我拿【一支 API】的盲區,寫成了【客人那一頁】的結論** ——
> 　 那是 `guard-and-instrument-traps.md` 分類軸的**第③種:尺準、對象也對,而量的是別的東西**。
> ✅ **⇒ 本節那個 `total: null` 仍然是真的,而它只管得到快搜下拉。要答「客人看不看得到筆數」⇒ 去開 `/search?q=…` 那一頁。**
...
### 🔴🔴 **參數名打錯【不會報錯】,它把全部 25,410 件當成篩選結果給你**(2026-09-19,a1 對帳時第一發就踩到)

```
🔬 /products?brands=rpm-carbon   ⇒ 25,410 件   ← 🔴 完全不篩, 而畫面【完全正常】
🔬 /products?brand=rpm-carbon    ⇒  1,106 件   ✅ 而網址自己補成 …&pbrands=rpm-carbon
⚪ 獨立分母:/api/search?q=rpm 的 brands facet ⇒ RPM CARBON count = 1106  ⇒ 兩邊對得上
```
🛑 **危險的不是打錯字的人** —— 是**任何拿它來對帳的人,會拿到一個看起來完全合理的錯數字**。
　 📌 25,410 不像錯誤訊息,它像一個答案。

### ⚪ 而客人走不到那個網址(2026-09-19 a1 實查)⇒ **這是【對帳的人】的坑,不是客人的 bug**

```
🔬 五個入口 + sitemap 全掃 [?&]brands= 的命中數:
   /  ⇒ 0    /products ⇒ 0    /brands ⇒ 0    /products?filter=new ⇒ 0    /sitemap.xml ⇒ 0
⚪ 判別力對照:同一份 HTML 掃 pbrands= ⇒ 首頁 2 命中 ⇒ 尺會動, 不是對什麼都回 0
```
⇒ ✅ **站內沒有任何連結產生 `brands=`** ⇒ 它只會出現在**手打網址的人**手上,而那通常是在對帳。
🛑 **這把尺抓不到什麼**:我只掃了那五個入口的 **HTML 字面**;
　 **前端 JS 動態組出來的網址、以及站外連進來的**,都不在射程裡。

📌 **通則(比這個參數名大)**:**一個篩選條件「沒生效」與「篩出全部」印出來是同一個畫面。**
⇒ ✅ **量任何篩選之前先問:沒篩的時候是幾件?** 兩個數字一樣 ⇒ 你量到的是「沒篩」,不是「篩完剛好全中」。
