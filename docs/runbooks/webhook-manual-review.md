...
1. **訂單已經是「已付款」，而且讓它變成已付款的那筆扣款，就是這則通知的交易。**
   判斷方式：找出這張訂單已確認的扣款紀錄，`payment_charge_attempts.rec_trade_id` 必須**等於**這則通知的 `rec_trade_id`。
   證據：訂單單號、付款狀態、那一列扣款紀錄的編號、狀態與交易編號（兩個交易編號要一起列出）。
   **交易編號不相等就不算**，一律改用第 2 種（到 TapPay 查這則通知那筆交易的明細），查不出最終結果就保留並通知 Sean。
   原因：客人第一次付款 3DS 卡住（交易 X 轉人工），重刷成功（交易 Y）後訂單會變成已付款；但 X 也可能已經扣到錢
   （`packages/use-cases/src/settle-charge.ts:103-108`）。看到「已付款」就把 X 結案，重複扣的那筆就不會有人發現。
