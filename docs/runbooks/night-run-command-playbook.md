...
- **STOP 即掛哨(施工窗標準動作,2026-08-06 增補;2026-08-11 改基準)**:每封 STOP 寫完的最後一動=掛信箱小哨兵(run_in_background,樣板見尾註 B;**基準=掛哨當下 `touch` 的 `.sentinel-base-<窗>`**、mtime 比對、pattern 涵蓋本窗**所有**信型並排除自己會寫的型)。~~基準=自己剛寫的那封 STOP 檔~~ **已於 2026-08-11 廢止**(那樣會漏掉「寫信那幾十秒內送達」的信,實錘見尾註 B 增補 1)。哨兵推播→全量讀所有新信→接工前先重掛(新基準)。施工中不輪詢(工作即狀態),每片收工時全量掃一次即可。壓縮/重啟後第一動=重讀自己最新 STOP+全量掃信箱,**不信任何舊哨兵**。
...
BASE=$MB/.sentinel-base-<窗代號>          # 專屬基準檔:每座哨兵一支、不與別座共用
LOG=$MB/.sentinel-<窗代號>-<號>.log
touch "$BASE"                              # 🔴 掛哨當下才建 ⇒ 晚於「寫信那幾十秒」
...
要點:`-name '<窗>-*.md'` 涵蓋 -A/-Q/任何後綴,**不得寫死號段開頭數字**(舊樣板 `<窗>-1*.md` 在號段長到 2xx/3xx/4xx 後=恆不觸發且完全靜默,2026-08-10 D 窗實測抓到);**掛之前先證錨點匹配得到**:`find "$MB" -maxdepth 1 -name '<窗>-*.md' | wc -l` 命中 0 = pattern 錯,不准掛;mtime 比對天然免疫撞號蓋信與截斷;`exit 0` 喚醒即死=單發哨兵,**絕不兩座共用狀態**(A-14 吞信事故 —— 這也是基準檔要**帶窗代號、且每座各自 `touch`** 的原因:兩座共用一支基準檔時,後掛的那座會把先掛那座的基準往前推)。

🔴 **2026-08-11 四處增補(當日四窗各踩一條;都是機制版,不是靠人記得)**:

1. **基準改成「掛哨當下 `touch` 的專屬檔」,不再用「自己剛寫的那封 STOP」**(E 窗 E-285 §3)。
   ~~舊字面:基準=自己剛寫的 STOP(存在且不會再被摸)~~ —— 它有一個死角:
   **在你寫那封 STOP 的那幾十秒內送達的信,mtime 早於你的 STOP** ⇒ `find -newer` 永遠不命中。
   實錘:E 窗 `E-284-A`(16:57)比自己的 `E-284-STOP`(16:58)早一分鐘,整封漏讀,
   是 Sean 叫「收信」後改用 `ls -lt` 全量掃才發現。
   ⇒ `touch` 版把基準點推到**寫信之後、掛哨當下**,那段死角就不存在。

2. **排除「自己會寫的信型」,而不是列舉「對方會寫的信型」**(D 窗 D-479 §3)。
   正向列舉(只認 `-A`)對**新出現的信型全盲** —— 對方哪天發一個 `-URGENT` 就靜默漏掉;
   反向排除最多只是多叫一次(看到自己的信),而**多叫的代價遠小於漏叫**。
   ⚠️ **排除清單要跟著你實際寫的信型走**:上面列的是 2026-08-11 實際出現過的五型
   (STOP / PLAN / ACK / Q / NOTE);**多用一種新型就要補一條**,漏補的症狀是自觸發
   —— 不致命,但會把「有新信」這個訊號稀釋成雜訊。

3. **判準先用檔名前綴,內文關鍵字只當補充**(S 窗 S-035 §3)。
   檔名前綴是**協定保證**的(收件人寫在檔名裡);內文關鍵字是**寫信的人當下的措辭**
   ⇒ 拿後者當主判準,等於把「別人怎麼措辭」變成你的推播條件。
   內文只適合用來**分流已經確定是給你的信**(例如先讀含「STOP」「擋發」的),不適合用來認領。

4. **處理完新信的最後一動 = 補掛哨兵**(B 窗 B-446 §4)。
   固定動作序:**讀完所有新信 → 做事 → 寫回信 → 同一條背景命令重掛**。
