...
**string literal union 抽 type alias 規則(M-1-02-audit Q3 落地):**

entity 內 string literal union ≥ 2 個 consumer 必抽 type alias、不留 inline。

- ❌ `availability: 'in-stock' | 'out-of-stock'` inline 在 Product entity、ProductCard / ProductPage / sync-engine seed / admin form 各處 hard-code `=== 'in-stock'`
- ✅ `export type ProductAvailability = 'in-stock' | 'out-of-stock';` + Product 用 `availability: ProductAvailability;`、各 consumer import 同一 type

理由:跨 layer single source of truth、avoid silent drift(consumer 漏 case 不報錯);未來新增第 3 個 enum value 改 type alias 一處、TS 編譯期 catch 全 consumer。

教訓來源:M-1-02 catalog Product.availability 落地時 inline union(M-1-02-audit Q3 抓出)、M-1-02-audit L1 立即修為 ProductAvailability type alias。
...
**ports JSDoc contract vs adapter implementation TODO 規則(M-1-02-audit Q6 落地):**

ports JSDoc 只描述 contract(method 做什麼、輸入輸出、業務語意);adapter implementation TODO(樂觀鎖 / idempotency / audit trail / cache 等)寫進 adapter class JSDoc、不寫 ports。

- ❌ `IProductRepository.save` JSDoc 寫「TODO M-1-03: MedusaProductAdapter 落地時補樂觀鎖 / idempotency / audit trail」(leak adapter 實作關注、誤導 InMemoryProductRepository 開發者「我也要實作?」)
- ✅ `IProductRepository.save` JSDoc 只寫「儲存 product entity(create / update 統一入口)、輸入 Product、輸出 Product」;adapter-specific TODO 寫進 `MedusaProductAdapter.save` JSDoc

理由:port = contract、adapter = implementation;contract 不該綁定特定 adapter 的 implementation concern(in-memory adapter 不需樂觀鎖、test mock adapter 也不需);避免 leak abstraction。

教訓來源:M-1-02 IProductRepository.save 加 JSDoc 含 3 條 adapter TODO(M-1-02-audit Q6 抓出);M-1-03 真實 adapter 落地時把 TODO 從 ports 搬到 MedusaProductAdapter。
...
| 2026-05-04 | §3.2 加「string literal union 抽 type alias 規則」(Q3 規範類落地、M-1-02 catalog availability 教訓);§3.3 加「ports JSDoc contract vs adapter implementation TODO 規則」(Q6 規範類落地、M-1-02 IProductRepository.save 教訓) | Claude Code(M-1-02-audit) |
