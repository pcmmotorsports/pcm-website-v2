...
- 🔴 **Branch(2026-09-08 Sean 拍「改」訂正;⛔ ~~`main`←production(Sean 手動 merge)~~ 是【錯的】)**:**`dev` = pcm-admin 的 production** —— Vercel 面板逐字「To update your Production Deployment, push to the **dev** branch」,Source 那格 = `dev` / `0a53037`。⇒ 🛑 **推 `dev` 就是上線**(`admin.pcmmotorsports.com` 當場重新部署)。`dev`←主開發(slice 都在 dev、線性、暫不開 feature branch);`main` 今天完全被 `dev` 含住(`main` 獨有 0 顆),**它不是 production**。🔴 **舊字面留刪除線** —— 照舊句讀會以為「推 dev 反正 production 在 main」而那正是它危險的地方。📌 **成因**:哪一支 branch 部署【設在 Vercel 面板上】,`vercel.json` 與 `apps/admin/vercel.json` 兩支都查無 production branch 鍵⇒ **從 repo 看不到** ⇒ 一句寫在規則檔裡的事實,不等於一個量到的事實。
