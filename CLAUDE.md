...
| 🔴 **列印出來的紙【多了一頁】/ 紙上看不到頁碼 / 換一台印表機就不一樣 / 「另存 PDF 是一頁而真印表機是兩頁」** | `apps/admin/src/app/print/print-a4.css` 檔頭那段 **2026-08-18 實測**(量法=印成 PDF → 70dpi PNG → 量墨水外接框離紙邊多遠, `gs -r70`, ±0.4mm;四張圖 p1/p2/p7/p14 一致):**左 11.6 · 右 11.6 · 上 13.4–14.5 · 🔴 下 5.1mm**。⚠️ **一般辦公室雷射印表機的硬體不可印區常見落在 4–6mm** ⇒ **5.1mm 正好在那條帶上** ⇒ 那行頁碼可能【整行被切掉】,而後果不是難看:**員工不知道這張單有第 2 頁**。🔴 而 2026-08-29 量到另一半:`.pd-sheet` 的 `min-height:271mm` + 頁尾 `margin-top:auto` ⇒ **內容永遠恰好 1024px(=271mm), 不論幾項** ⇒ **餘裕是零**;實測可印高 271mm ⇒ 1 頁、**269mm ⇒ 連只用 68% 的那張也變 2 頁** ⇒ 那是 **2mm 寬的懸崖**。📌 **⇒ 這一行的觸發情境刻意寫【症狀】不寫【主題】** —— 8/18 那份量測寫得很好、也在正確的檔裡,而 2026-08-29 三個窗花了數小時重新發現它的一半:**沒有人會去查「列印邊界實測」, 而每個人都在查「為什麼變兩頁」**。 |
