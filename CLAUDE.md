...
- 🔴🔴 **不知道現在該做什麼(壓縮後 / 接手 / 手上空了)⇒ 讀 `docs/NEXT.md`** —— 它**不列任何一件事**,它只教你當場印一份:`python3 scripts/launch-dispatch-table.py`。📌 **為什麼不列**(量到的支數 + 那句逐字 → `docs/NEXT.md` 檔頭):🛑 **repo 與信箱裡那些檔名含「清單/上線/待辦/todo」的檔(2026-09-08 量的那一批)【全部是歷史快照,不是待辦】** —— 要用其中一份先問「它是哪一顆 sha 上的」,答不出來就當歷史讀。
...
| 🔴 **寄信線要停下來 / 怎麼關掉寄信 / 停了信會不會丟** | `docs/runbooks/email-sweep-kill-switch.md`(Sean 09-07 QB-1 甲;開關 = pg_cron 排程本身, 一句 unschedule, 不是 env 不是碼;信累積不丟;走貼板流程 Sean 說貼才貼) |
...
| 🔴🔴 **我想知道「這支 migration 貼進正式庫了沒」/ 我 grep `APPLIED.tsv` 拿到 0** | ⛔ **帳本的 0 不是答案** —— `supabase/APPLIED.tsv` 檔頭逐字「不在本表上**什麼都不代表**」。✅ 改用 `bash scripts/is-migration-applied.sh <版本號>`(產帶正負對照的唯讀 SQL);`scripts/migrations-not-in-ledger.sh` 列差集;**全量**問 `python3 scripts/prod-vs-vc-functions.py`(唯讀, 答「正式庫有沒有跑跟版控不同的邏輯」)。🛑 「物件在不在」對 `CREATE OR REPLACE` **零判別力**。📎 病史 → 板列 `⟦01-LEDGERFALSENEG⟧` |
...
- 🔴 **Branch(2026-09-08 Sean 拍「改」訂正;而【兩個 app 兩條規矩】,不可寫成一條)**:**① `pcm-admin`(`admin.pcmmotorsports.com`)的 production branch = `dev`**。⇒ 🛑 **推 `dev` = 後台當場上線**。**② storefront(`shop.pcmmotorsports.com`)的 production = `main`,Sean 手動 FF**。⇒ ⛔ ~~`main`←production / `dev`←主開發~~ **那句只對 storefront 成立、對 admin 是錯的**(完整版 📎 下方指標檔)。`dev`←主開發(slice 都在 dev、線性、暫不開 feature branch)。📎 **成因(設在 Vercel 面板上、repo 看不到)與射程教訓 → `docs/reference/environment-values-and-what-stands-on-them.md`**
...
🔴 **而「卡住」不在上面那張表裡 —— 卡住不等於收工**(2026-09-08 Sean 拍板,逐字「可以繼續安排我不用決策的工作啊」):
不能推 / 不能貼 / 不能部署,擋的是**送出去**,擋不了**做**。要說「手上空了」之前,先答:①有沒有量得到而沒人量過的 ②有沒有我自己標「等 Sean」而他其實不用決定的 ③有沒有別人交來而沒人複量的。**答不出=還沒找完。**(判準全文 `~/.claude/rules/00-work-rules.md` R7)
