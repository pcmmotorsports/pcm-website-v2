...
| 寫或審任何 `SECURITY DEFINER` / `GRANT` / `REVOKE`(含改既有 SECDEF 函式的參數型別) | `docs/patterns/revoking-function-execute-in-supabase.md`(**兩道 REVOKE 少一道都是開的**、四臂實測、`has_function_privilege` vs `proacl`、改參數型別會讓舊 REVOKE 全部失效) |
