...
**slice 結束前**:☐ **寫一行心跳**(`bash /Users/sean_1/pcm-website-v2/scripts/heartbeat.sh …`)☐ 🔴 **順序:先 `git add`、【再】寫 reviewer 標記、才 commit** ☐ 標記一律走 `bash scripts/write-reviewer-marker.sh "<片名+輪次或跳審理由>"`,不要直接寫 `.git/` ☐ 肉眼驗(「肉眼驗✅」是 Sean 專屬用詞)☐ 三綠(動 .ts/.tsx 加 build、不 disable/skip)☐ **改過任何對外字面 → 拿舊字面跑 `bash scripts/literal-sweep.sh '<舊字面>'` 再 commit** ☐ 命中鐵則 12 → codex 對抗審查已跑、未 push ☐ 動前台元件 → 補/更新 smoke test ☐ commit 字面vs事實一致、偏離寫 body ☐ 精準 add、格式對 ☐ STATUS 7 欄更新(同 commit)☐ 收尾對帳(Sean 拍板逐條 vs 已落檔,漏的補寫 memory `project_*.md`)☐ busboy-end ☐ 不 push
