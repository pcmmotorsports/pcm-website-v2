...
- 🔴🔴 **不知道現在該做什麼(壓縮後 / 接手 / 手上空了)⇒ 讀 `docs/NEXT.md`** —— 它**不列任何一件事**,它只教你當場印一份:`python3 scripts/launch-dispatch-table.py`。📌 **為什麼不列**(量到的支數 + 那句逐字 → `docs/NEXT.md` 檔頭):🛑 **repo 與信箱裡那些檔名含「清單/上線/待辦/todo」的檔【全部是歷史快照,不是待辦】** —— 要用其中一份先問「它是哪一顆 sha 上的」,答不出來就當歷史讀。
