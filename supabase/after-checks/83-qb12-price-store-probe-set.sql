-- ═══════════════════════════════════════════════════════════════════════════
-- ⛔⛔ **本檔已作廢 —— 不要跑, 不要貼。**(2026-09-07 12:4x)
-- ═══════════════════════════════════════════════════════════════════════════
-- **為什麼**:Sean 當天改口 —— 逐字「**幹嘛做假商品, 直接讓正式站有經銷價功能**」
--   ⇒ 要的是**灌真經銷價**(Q16 資料側), **不是設一個 87 的假價來驗**。
--   ⇒ 📌 這三支解的是一個**已經不存在的題目**。
--
-- 🛑 **本檔【不刪除】, 理由**:它記著「當時為什麼要這樣驗、以及那條路長什麼樣」。
--   刪掉之後, 下一個想「設個測試價來驗」的人會**從頭再走一次同樣的路**。
--   ⇒ 而**留著就必須讓它跑不動** —— 一句註解擋不住複製貼上。
-- ═══════════════════════════════════════════════════════════════════════════

-- 🔴🔴 **硬擋:本檔已作廢** —— 這一段在任何其他敘述之前, 貼下去就停。
--   📌 **註解會被跳過, `RAISE` 不會。** 這是「機制優先於提醒」那一條。
DO $voided83$
BEGIN
  RAISE EXCEPTION '83 系列已作廢(2026-09-07:Sean 改口要灌真經銷價, 不設測試價 87)⇒ 本檔不得執行。要驗經銷價請走 Q16 那條路。';
END $voided83$;
-- ═══════════════════════════════════════════════════════════════════════════
