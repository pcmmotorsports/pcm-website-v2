...
SELECT count(*) AS 幾個多載
 ...
 WHERE n.nspname||'.'||p.proname = 'public.pcm_sync_order_refund_payment_status';

\echo '=== ② 🔴 判準:函式體含不含「order_manual_refunds」==='
\echo '    預期:貼【前】f · 貼【後】t'
SELECT COALESCE(bool_or(pg_catalog.pg_get_functiondef(p.oid) LIKE '%order_manual_refunds%'), false) AS 含判別字面
  FROM pg_catalog.pg_proc p JOIN pg_catalog.pg_namespace n ON n.oid=p.pronamespace
 WHERE n.nspname||'.'||p.proname = 'public.pcm_sync_order_refund_payment_status';
...
\echo '    · order_manual_refunds 正式庫 0 列 ⇒ 貼完不會有單被翻動, 那是預期不是沒生效'
