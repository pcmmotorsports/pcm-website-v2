...
SELECT count(*) AS 幾個多載
 ...
 WHERE n.nspname||'.'||p.proname = 'public.begin_charge_attempt';

\echo '=== ② 🔴 判準:函式體含不含「superseded_by_card」==='
\echo '    預期:貼【前】f · 貼【後】t'
SELECT COALESCE(bool_or(pg_catalog.pg_get_functiondef(p.oid) LIKE '%superseded_by_card%'), false) AS 含判別字面
  FROM pg_catalog.pg_proc p JOIN pg_catalog.pg_namespace n ON n.oid=p.pronamespace
 WHERE n.nspname||'.'||p.proname = 'public.begin_charge_attempt';
...
\echo '    · 那段 UPDATE 的四個收窄條件已有檔案層守門(supersede-bank-order-contract.test.ts 九格)'
