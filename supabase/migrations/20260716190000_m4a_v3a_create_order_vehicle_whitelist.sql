  ...
  LOOP
    ...
    IF v_veh IS NOT NULL AND pg_catalog.jsonb_typeof(v_veh) = 'object' THEN
      ...
      IF v_veh_ok AND v_veh->>'kind' = 'dict' THEN
        IF coalesce(pg_catalog.btrim(v_veh->>'brand'), '') <> '' AND pg_catalog.length(v_veh->>'brand') <= 200
      ...
      ELSIF v_veh_ok AND v_veh->>'kind' = 'free' THEN
        IF coalesce(pg_catalog.btrim(v_veh->>'raw'), '') <> '' AND pg_catalog.length(v_veh->>'raw') <= 200
