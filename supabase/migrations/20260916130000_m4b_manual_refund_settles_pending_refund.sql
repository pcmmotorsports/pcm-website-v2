...
-- ⟦Q2 甲⟧ 登記人工退款 ⇒ 自動結清同單同軌的未結待退款;作廢那筆退款 ⇒ 解除結清、重開。
-- plan `docs/plans/2026-09-15-pending-refund-settlement-plan.md` §3。
-- Sean 2026-09-16 逐字:「Q2 A: 甲 = 要，同一張單、同一種付款方式就自動關掉；分次退的留下差額(要改資料庫，改完先過審查、你批了才貼)」
-- pcm:idempotent: yes
--   理由:兩支都是 CREATE OR REPLACE;前置閘的 md5 接受「改前」與「本支」兩個值 ⇒ 重貼不會紅。
--
-- ── 為什麼 ─────────────────────────────────────────────────────────────────
--   鑽機 PCM-2026-1009(路 3):部分取消 5,080 ⇒ 系統開待退款 5,080;員工登記退款 5,080
--   ⇒ **待退款沒人結(settled_at 全 repo 零寫入者)** ⇒ 對帳 view 判 rail_mismatch ⇒ 每日告警多一張假異常。
--
-- ── 本支動什麼 ──────────────────────────────────────────────────────────────
--   ① admin_record_manual_refund(8 參):稽核寫完之後、同步狀態之前,結清同單同軌那一列未結待退款。
--      相等 ⇒ 結清。退得少 ⇒ 結清 + 另開差額列(同 cancellation_id)。退得多 ⇒ 只結清(多的不猜;多退的錢帳本已扣,「該開」算出來是 0 ⇒ 對帳 view 不報)。
--      那條軌沒有未結列 ⇒ 不動。
--   ② admin_void_manual_refund(3 參):作廢時找被這筆結清的那一列 ⇒ 解除結清;
--      同單同軌若已有差額列 ⇒ 差額列作廢(merged_on_refund_void)、金額併回;然後 pcm_partial_cancel_recompute 照 Q13 重算。
--      🔴 鎖單從 FOR NO KEY UPDATE 改成 FOR UPDATE:recompute 自己取 FOR UPDATE,
--         同交易內從 NO KEY 升級會等別人的 KEY SHARE(例:排程補開待退款插列時的 FK 檢查)⇒ 可成環。一開始就拿 FOR UPDATE 就沒有升級。
--   🛑 簽章一個字都沒改;SET search_path = '' 逐字抄回;零 GRANT 零 REVOKE。
--   🔵 對帳 view、兩支算額函式、排程都不動。
--
...
--    🔵 本支【兩支都沒有改】—— `pcm_order_refundable_remaining` 只出現在照抄回來的 record body(步5 額度守門)。
--
-- ── 已知天花板(寫白)────────────────────────────────────────────────────────
--   · 排程 pcm_cron.late_payment_pending_refund_sweep 不鎖單就插列:它與登記退款同時跑時,
--     可能開出一列沒扣到這筆退款的待退款 ⇒ 對帳 view 會報出來(與今天同一族,不是本支新開的洞)。
--   · 分次退之後再作廢會留下一列 merged_on_refund_void 的作廢列 ⇒ 那個排程對「有作廢列」的已取消單整張跳過
--     (它的 R1-F2 設計)⇒ 那張單之後的晚到匯款不會自動補開。極少見;對帳 view 仍會報 missing_row。
--   · 那個排程對已取消單插待退款時先占 unique index、再做 FK 檢查等 orders;同一毫秒員工作廢退款把舊列改回未結
--     ⇒ 兩邊互等 ⇒ PG 判 40P01 砍掉其中一邊(排程下一輪再跑 / 員工重送即可),不會寫錯。
--   · 作廢「沒有結清過任何列」的退款不重算(只有解除結清那條才叫 recompute):
--     recompute 不看有沒有取消過,對「沒取消、客人多付」的單會開出待退款 ⇒ 違反 Sean 09-05「多匯不翻狀態只標字」。
--
-- ── ROLLBACK ═══════════════════════════════════════════════════════════════
--   `supabase/rollbacks/20260916130000-rollback.sql`(內含兩支 20260912040000 的完整 body)。
...
DECLARE
  v_n integer;
BEGIN
  IF pg_catalog.to_regclass('public.order_pending_refunds') IS NULL THEN
    RAISE EXCEPTION '前置閘①:找不到 public.order_pending_refunds';
  END IF;
  IF pg_catalog.to_regclass('public.order_pending_refunds_live_order_rail_key') IS NULL
     OR pg_catalog.to_regclass('public.order_pending_refunds_settled_ref_key') IS NULL THEN
    RAISE EXCEPTION '前置閘②:待退款的兩個 partial UNIQUE 不見了 ⇒ 本支「同單同軌最多一列未結」的假設要重讀';
  END IF;
  IF (SELECT pg_catalog.count(*) FROM pg_catalog.pg_constraint
       WHERE conrelid = 'public.order_pending_refunds'::regclass
         AND conname IN ('order_pending_refunds_settled_needs_ref',
                         'order_pending_refunds_settled_xor_voided',
                         'order_pending_refunds_settled_same_order')) <> 3 THEN
    RAISE EXCEPTION '前置閘③:待退款結清那三道約束不齊';
  END IF;
  IF pg_catalog.to_regprocedure('public.pcm_partial_cancel_recompute(uuid)') IS NULL THEN
    RAISE EXCEPTION '前置閘④:pcm_partial_cancel_recompute(uuid) 不存在(20260914070000 沒貼?)';
  END IF;
  IF pg_catalog.to_regprocedure(
       'public.admin_record_manual_refund(uuid,uuid,text,text,integer,text,timestamp with time zone,boolean)') IS NULL
     OR pg_catalog.to_regprocedure('public.admin_void_manual_refund(uuid,text,text)') IS NULL THEN
    RAISE EXCEPTION '前置閘⑤:兩支退款 RPC 有一支不存在 ⇒ 停下';
  END IF;
  -- 🔴 body 必須是 20260912040000 那一代(或本支自己)—— 別人先換過一代,本支整段照抄會把它蓋掉。
  IF (SELECT md5(p.prosrc) FROM pg_catalog.pg_proc p
       WHERE p.oid = 'public.admin_record_manual_refund(uuid,uuid,text,text,integer,text,timestamp with time zone,boolean)'::regprocedure)
     NOT IN ('7e1454063b86d6b14bb6d8da738ae8b9', '71ac9c1313a3d41fcb0c92eeaf1cbcb1') THEN
    RAISE EXCEPTION '前置閘⑥:admin_record_manual_refund 的 body 不是 20260912040000 那一代 ⇒ 有人換過,停下合併';
  END IF;
  IF (SELECT md5(p.prosrc) FROM pg_catalog.pg_proc p
       WHERE p.oid = 'public.admin_void_manual_refund(uuid,text,text)'::regprocedure)
     NOT IN ('b5b8d4320d005ff235c513f5ec525fb7', '92094aa979ca3f61779015c762b1ed7d') THEN
    RAISE EXCEPTION '前置閘⑦:admin_void_manual_refund 的 body 不是 20260912040000 那一代 ⇒ 有人換過,停下合併';
  END IF;
  -- 🔴 結清只寫在這兩支 ⇒ 資料庫裡寫 order_manual_refunds 的函式必須恰好是它們(plan §3.1 反方)。
  SELECT pg_catalog.count(*) INTO v_n FROM pg_catalog.pg_proc p
   WHERE p.prosrc ~* '(insert\s+into|update)\s+public\.order_manual_refunds'
     AND p.oid NOT IN ('public.admin_record_manual_refund(uuid,uuid,text,text,integer,text,timestamp with time zone,boolean)'::regprocedure,
                       'public.admin_void_manual_refund(uuid,text,text)'::regprocedure);
  IF v_n <> 0 THEN
    RAISE EXCEPTION '前置閘⑧:另有 % 支函式寫 order_manual_refunds ⇒ 它們寫的退款不會結清待退款,先讀再決定', v_n;
  END IF;
  -- 🔴 plan §3.5:不補舊資料。貼板當下若已有「未結而之後登記過同軌退款」的列 ⇒ 停,給人看。
  --    只在【還沒貼過本支】時數:貼過之後「作廢併回」本來就會讓舊 opened_at 的未結列與較晚的同軌退款並存
  --    ⇒ 不分的話本支重貼會紅,pcm:idempotent 那句就是假的(審查 R1)。
  IF (SELECT md5(p.prosrc) FROM pg_catalog.pg_proc p
       WHERE p.oid = 'public.admin_record_manual_refund(uuid,uuid,text,text,integer,text,timestamp with time zone,boolean)'::regprocedure)
     = '7e1454063b86d6b14bb6d8da738ae8b9' THEN
    SELECT pg_catalog.count(*) INTO v_n FROM public.order_pending_refunds r
     WHERE r.settled_at IS NULL AND r.voided_at IS NULL
       AND EXISTS (SELECT 1 FROM public.order_manual_refunds m
                    WHERE m.order_id = r.order_id AND m.rail = r.rail
                      AND m.voided_at IS NULL AND m.created_at > r.opened_at);
    IF v_n <> 0 THEN
      RAISE EXCEPTION '前置閘⑨:已有 % 列未結待退款在開單之後登記過同軌退款 ⇒ 本支不補舊資料,停下讓人看', v_n;
    END IF;
  END IF;
END $$;
...
DECLARE
  ...
  v_settled record; -- ⟦Q2 甲⟧ 被這筆退款結清的那一列待退款
  v_live    record; -- ⟦Q2 甲⟧ 同單同軌此刻未結的那一列(分次退開的差額)
  v_new     bigint;
BEGIN
  ...
  END IF;
  -- 🔴 ⟦Q2 甲⟧ 20260916130000 從 FOR NO KEY UPDATE 改成 FOR UPDATE:下面解除結清後會呼叫
  --    pcm_partial_cancel_recompute, 它自己取 FOR UPDATE ⇒ 同交易從 NO KEY 升級要等別人手上的 KEY SHARE
  --    (例:排程補開待退款插列的 FK 檢查)而那個交易可能正等我們的待退款列 ⇒ 成環。一開始就拿滿就沒有升級。
  PERFORM 1 FROM public.orders o WHERE o.id = v_oid FOR UPDATE;
  ...
  -- 🔴🔴 ⟦Q2 甲⟧(20260916130000):這筆退款結清過待退款 ⇒ 解除結清、把它結掉的份加回去。
  --    · 結掉的份 = LEAST(退款金額, 那一列金額)(退得多時多的那段本來就沒算進待退款)。
  --    · 同單同軌此刻若有未結列(分次退開的差額)⇒ 那一列作廢(merged_on_refund_void)、金額併進被解除的那列
  --      ⇒ 同單同軌仍只有一列未結。鏈狀分次退也成立:只動「被這筆結的那一列」與「此刻那一列未結」。
  --    · 然後照 Q13 重算一次(整單取消 / 刷卡 / 稅算不出的單 recompute 自己會跳過 ⇒ 併回的金額就是結果)。
  --    · 冪等重放(步 3)走不到這裡:第一次作廢時已解除。
  SELECT r.id, r.rail, r.amount_at_cancel
    INTO v_settled
    FROM public.order_pending_refunds r
   WHERE r.settled_manual_refund_id = p_refund_id
     FOR UPDATE;
  IF FOUND THEN
    v_new := LEAST(v_row.refund_amount, v_settled.amount_at_cancel);
    SELECT r.id, r.amount_at_cancel
      INTO v_live
      FROM public.order_pending_refunds r
     WHERE r.order_id = v_oid AND r.rail = v_settled.rail
       AND r.settled_at IS NULL AND r.voided_at IS NULL
       FOR UPDATE;
    IF FOUND THEN
      UPDATE public.order_pending_refunds
         SET voided_at = pg_catalog.now(), void_reason = 'merged_on_refund_void'
       WHERE id = v_live.id;
      v_new := v_new + v_live.amount_at_cancel;
    END IF;
    UPDATE public.order_pending_refunds
       SET settled_at = NULL, settled_manual_refund_id = NULL, amount_at_cancel = v_new
     WHERE id = v_settled.id AND voided_at IS NULL;
    GET DIAGNOSTICS v_hit = ROW_COUNT;
    IF v_hit <> 1 THEN
      RAISE EXCEPTION 'admin_void_manual_refund: 解除結清影響 % 列(期望恰 1)⇒ 停下', v_hit;
    END IF;
...
BEGIN
  FOR r IN
    SELECT p.proname, p.prosecdef, p.proconfig::text AS cfg, p.proacl::text AS acl
     ...
     WHERE p.oid IN ('public.admin_record_manual_refund(uuid,uuid,text,text,integer,text,timestamp with time zone,boolean)'::regprocedure,
                     'public.admin_void_manual_refund(uuid,text,text)'::regprocedure)
  LOOP
    IF NOT r.prosecdef THEN
      RAISE EXCEPTION '事後閘①:% 不再是 SECURITY DEFINER', r.proname;
    END IF;
    IF r.cfg IS DISTINCT FROM '{"search_path=\"\""}' THEN
      RAISE EXCEPTION '事後閘②:% 的 search_path 變成 %', r.proname, r.cfg;
    END IF;
    IF r.acl IS DISTINCT FROM '{postgres=X/postgres,service_role=X/postgres}' THEN
      RAISE EXCEPTION '事後閘③:% 的 EXECUTE 權限變成 %(本支零 GRANT 零 REVOKE)', r.proname, r.acl;
    END IF;
  END LOOP;
  IF (SELECT pg_catalog.count(*) FROM pg_catalog.pg_proc p
       WHERE p.proname IN ('admin_record_manual_refund', 'admin_void_manual_refund')
         AND p.pronamespace = 'public'::regnamespace) <> 2 THEN
    RAISE EXCEPTION '事後閘④:兩支 RPC 長出多載';
  END IF;
END $$;
