...
--    而它【重跑安全】的理由是結構的, 而且是【兩道】, 都在同一個 BEGIN…COMMIT 裡(:84 / :285):
--      第一道 :106 前置閘②(欄已存在 ⇒ RAISE EXCEPTION, forward-only) ← 第二次跑【在這裡就停了】
--      第二道 :161 ADD COLUMN(欄已存在 ⇒ 42701)                     ← 第一道被拿掉時才輪到它
--    ⇒ 任一道炸 ⇒ 整包回滾 ⇒ :166 那段 UPDATE 跑不到 ⇒ 零損害、而且大聲。
--    📌 「重跑會大聲失敗」與「重跑是空操作」在這道閘眼裡都算安全, 而它們不是同一件事 —— 這裡是前者。
--    🛑 前置閘② 與 ADD COLUMN 是【兩道】, UPDATE 要兩道都過才跑得到。不要拿掉前置閘②、
--       不要把 ADD COLUMN 改成 IF NOT EXISTS —— 【兩件都做了】UPDATE 才會在第二次跑到, 上面那句才變假。
--       少一道還擋得住, 而少一道之後這段話讀起來會像還有兩道。
--    🔬 主視窗第一版只寫了 ADD COLUMN 那一道 —— B 窗 2026-09-13 核出前置閘② 才是先炸的那道, 已改。
--
