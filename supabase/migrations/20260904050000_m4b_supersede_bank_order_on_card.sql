-- ============================================================
-- 20260904050000 · 客人改用刷卡時, 自動取消【每一張】未付款的匯款單(⛔ 舊字面 ~~那張~~ 是單數, 而碼從第一版就是全部取消;Sean 2026-09-04 第二十四題拍【甲 全部一起取消】⇒ 現行行為自此有授權)
...
BEGIN
  ...
  -- ══ ⑨b 【片 B】客人改用刷卡 ⇒ 取消【每一張】未付款的匯款單(舊字面 ~~那張~~;拍板見檔頭)══
  --
...
BEGIN
  ...
  --   ⚠️ **射程(2026-09-05 逐項實測, 不是推論)** —— 這一招對五種輸入的實際結果:
  --      ```
  --      正常碼                              ⇒ true   ✅
  --      行註解裡 `-- superseded_by_card`     ⇒ false  ✅ 這是它唯一擋得住的
  --      🔴 塊註解裡 `/* superseded_by_card */` ⇒ **true**  ⇒ **假綠**:碼刪光了它還說在
  --      🔴 字串常值裡 `'superseded_by_card'`  ⇒ **true**  ⇒ **假綠**:同上
  --      🔴 字串裡有 `--`(`'a -- b', superseded_by_card`)⇒ **false** ⇒ **假紅**:碼在而它說不在
  --      ```
  --      🎯 **⇒ 兩種失效方向【相反】** —— 前兩種讓壞碼過關, 第三種讓好碼被擋。
  --      ⛔ ~~原本寫「本檔沒有塊註解」~~ **那是一個【會被下一個人弄假】的假設** ——
  --         他加一段 `/* */` 說明就成立了, 而**沒有任何東西會叫**。
  --      🛑 **所以不要把這一格讀成守門** —— 它是【便宜的早期訊號】, 真正的判別力在探針的
  --         資料世界格子(格1-19), 那些格子問的是「它有沒有做事」, 而不是「那個字在不在」。
  --      🔵 codex 關卡2 R2 把這格判 PARTIAL 是對的(⟦b4-PIECEBGATEGAPS⟧ ②④)。
