...
--         改名 ⇒ `030000` 的 `DROP` 不再指向本支建的那一支 ⇒ `public.orders` 上同時活著兩支綁同一支函式的 trigger。
--         ⛔ ~~⇒ 一次付款扣兩次券~~ 🔴 **2026-09-12 Fable 審打掉這個理由, 我複驗成立**:
--         `redeem_coupon`(`20260831160000:242-249`)先查 `coupon_redemptions WHERE order_id`, 同券同人同折抵就早退不再 INSERT;
--         加上 `coupon_redemptions_one_per_order UNIQUE (order_id)`(`20260829150000:182`)⇒ **第二支是 no-op**(折抵對不上才丟例外)。
--         🎯 **⇒ 真正的理由是:那個「沒扣兩次」靠的是【別支函式的冪等】, 而兩支檔的自檢只用 tgname 查自己那一支,
--         沒有一句在問「這張表上是不是只有一支」⇒ 多出來的那一支不會有任何人叫。**
--   ③ **兩支的 trigger 函式體是不是同一支** ⇒ ✅ 同一支 `public.coupon_redeem_on_paid`。
--      實測(拋棄式 PG 17.10, 比【資料庫實際存進去的樣子】不是原始碼字面):
--        · 兩支的 **WHEN 子句逐位元組相同**(各 328 bytes)
--        · 兩支的 **函式體早退條件逐位元組相同**
--        · 而函式體整體 **+23 / -1 行** ⇒ `030000` 確實改了東西, 不是空跑
--      🟢 那個 +23/-1 同時是正對照:**尺分得出兩版**, 所以上面那兩個「相同」是真的。
--   ④ **`public.orders` 上還有沒有別的 trigger 會跟它搶順序** ⇒ 【量的】唯讀查正式庫, 有 **1 支**:
--        order_pending_refund_open_au              盯 cancelled_at   ⇒ 不會一起叫
--        pcm_e13_orders_subtotal_guard             盯 subtotal       ⇒ 不會一起叫
--        zzz_pcm_invoice_requested_false_is_final  **盯全部欄**      ⇒ 🔴 **會一起叫**
--      · 同事件內 PostgreSQL 依 **trigger 名排序** 觸發 ⇒ `trg_…` < `zzz_…`
--        ⇒ **本支的 trigger 先叫, `zzz_` 那支後叫** ——`zzz_` 前綴顯然就是為了排最後, 本支不打亂它。
--      · ⚠️ 而 `zzz_` 那支的 `tgenabled = 'A'`(ALWAYS)⇒ **它在 `session_replication_role = replica`
--        之下照樣叫**, 而本支是預設的 `'O'` ⇒ **那種模式下本支不叫而它會叫**。
--
--   ⚠️ **上面那幾個行號是【加了本段之後】重量的** —— 本段替兩支檔頭各加了 34 行,
...
--   ✅ **2026-09-12 唯讀實查:那支函式【在正式庫上】了**(貼板 125 `20260901020500` 建的;126 是接線那一發)⇒ 那道閘今天過得去。
...
BEGIN
  ...
  -- ── ⟦b4-COUPONREVERT⟧ 券的【退回】路徑必須先存在 ────────────────────────────
  --   Sean 2026-09-02 拍 🅐:「券的退回機制 = 扣券上線的【前置條件】」(-0a 轉、-f3 落)。
  --
  --   🔴 **為什麼是一道會 RAISE 的 SQL, 而不是一句檔頭警告**:
  --     `20260829150000:50` 逐字「本檔【不由任何窗 apply】—— apply 是 Sean 的手」
  --     ⇒ 他是在 Supabase SQL Editor 一支一支貼的
  --     ⇒ **我們任何 husky / CI 閘都在【我們 commit】那一刻叫, 到不了那裡。**
  --     ⇒ 而檔頭警告 —— 同 repo 2026-09-02 `a920143e` 才剛量到反例:
  --       一道寫在正確位置、而且**被引用了**的警告, 沒有擋住引用它的人。
  --     ⇒ ⇒ 📌 **能在【貼下去那一刻】叫的只有【貼下去會 RAISE 的 SQL】。**
  --        那不是選了一個形狀, 是**只有一個形狀到得了那一刻**。
  --
  --   🔴 **為什麼用【具名函式】而不是掃 `prosrc` 找 `reverted_at =`**:
  --     `pg_proc.prosrc` **包含註解** ⇒ 一支只在註解裡提到 `reverted_at` 的函式會讓本閘假綠。
  --     ⇒ 那正是本 repo 2026-09-02 記過的同一個病(板 `⟦f3-STATEGATESCOMMENT⟧`:
  --       一張表把 `COMMENT ON` 的字串當成狀態改動抓進去)。**不要用字面尺去問行為。**
  --     ⚠️ **代價明寫**:本閘綁死一個名字 ⇒ 若退回路徑最後叫別的名字,
  --       **改名那一片必須同 commit 改這一行** —— 而它不改的話, 症狀是 apply 被擋(會叫、不會靜默)。
  --       🔵 那是刻意選的方向:**寧可誤擋, 不可漏放** —— 漏放的後果在下面那段訊息裡。
  --
  --   🛑🛑 **本閘【證不到】什麼(codex R1 must-fix①, 2026-09-02)—— 這一段與修法一樣重要**:
  --     它證的是「**有一支叫這個名字、吃一個 uuid 的 function 存在**」, 而**不是**:
  --       · 它真的會寫 `reverted_at`(一支空殼 `BEGIN NULL; END` 照樣通過)
  --       · 呼叫端有權限叫它 · 它被接到任何退款流程上
  --     ⇒ 📌 **它擋的是【完全沒有人做這件事】那一種, 不是【做了一半】那一種。**
  --     ✅ 已收窄的兩格:`prokind = 'f'`(擋掉同名 PROCEDURE)+ `pg_catalog.uuid`(擋掉
  --       特殊 `search_path` 下的同名自訂型別)—— 兩格都是 codex 抓的。
  --
  --   🔴🔴 **排序約束(codex R1 must-fix②)—— 不寫下來它是一個【永遠解不開】的死結**:
  --     建立 `coupon_revert_on_full_refund` 的那支 migration **必須排在本檔【之前】**
  --     (版本號更小), **或與本檔合併成同一支**。
  --     ⇒ 因為它今天還不存在 ⇒ 它自然會拿到一個**更晚**的時戳 ⇒ 而 `db push` 依版本號排序
  --       ⇒ **會先撞上本閘而中止, 永遠到不了那支 prerequisite。**
  --     🛑 而 Sean 手貼時可以自己決定順序 ⇒ **那條路不會卡, 而 `db push` 那條會**
  --       ⇒ 兩條路的行為不同, 所以這句話要寫在檔案裡, 不是只寫在交接訊息裡。
  -- 🔵 **第二格(codex R2 must-fix①)—— `prokind='f'` 之後仍會放行一支【空殼函式】**:
  --   `BEGIN NULL; END` 照樣通過存在性檢查 ⇒ 加一格看**函式本體真的提到 `reverted_at`**。
  --   🔴 **而它先剝掉 `--` 註解才比對** —— 直接比 `prosrc` 會被註解騙:
  --     `prosrc` 包含註解, 而註解最會講那件事(本 repo 2026-09-02 板 `⟦f3-STATEGATESCOMMENT⟧`)。
  --     🔵 剝註解這個技法 repo 裡已有先例(`scripts/sql-comment-strip-audit.py`, 以及
  --       `APPLIED.tsv` 20260902020000 那一列:「不剝註解會數到 PCM01 五次而判成它根本沒改掉」)。
  --   🛑 **而這一格【仍然證不到】三件事, 明寫**:
  --     ① 那支函式**跑起來真的會寫**(它只證明本體裡有這個字, 不證明那一行會被執行到)
  --     ② 呼叫端**有權限**叫它  ③ 它**被接到**任何退款流程上
  --     ⚠️ 還有一格窄的:本剝法只剝 `--` 行註解, **不剝 `/* */` 區塊註解、也不管字串常數**
  --       ⇒ 一支把 `reverted_at` 寫在區塊註解或字串裡的函式仍會假綠(已知, 未修)。
  --   ⇒ 📌 **本閘擋的是【完全沒有人做】與【交一個空殼】, 不是【做了一半】。**
  IF NOT EXISTS (
...
-- ══════════════════════════════════════════════════════════════════════════
-- **誰拍的**:主視窗 `-f1` 2026-09-07 裁【丙】(線【資料】`-db` 量完後給的判斷)。
--
-- 🎯 **為什麼**:`20260907040000`(貼板 71, **已貼**)重建了 `create_order`,
--    而它建的那支 **10 參數簽章與本片這一段【逐字相同】**:
--      `p_lines, p_address_id, p_shipping_method, p_invoice, p_cart_session_id,`
--      `p_terms_version, p_client_ip, p_client_ua, p_notification_email, p_coupon_code`
--    ⇒ 本片若照原樣貼, 那句 `CREATE OR REPLACE` 會**精準命中 71 剛建的那支**,
--      把 body 換成 2026-09-01 的舊版 ⇒ 📌 **71 的經銷未稅那段算錢邏輯會被靜靜蓋掉。**
--    (`⟦db-NOROLLBACKARTIFACT⟧` 那個形狀:`CREATE OR REPLACE` 沒有「刪掉就回去了」。)
--
-- 🔬 **量到的(可重跑)**:三支的參數區各自剝掉註解之後逐字比對 ⇒ 71:668 / 本片 / 貼板 76
--    三者的 10 個參數名與順序**完全相同**;而 71:145 那支是 **11 參數**(多 `p_payment_channel`),
--    不受影響。
--
-- 🛑 **而【收工驗收】那一段也一起拿掉了, 那不是順手, 是【非拿不可】**:
--    它逐條斷言的是 create_order 的 body, 其中至少兩條對 71 的 body **必然紅**:
--      · 「函式體沒有呼叫 redeem_coupon ⇒ 券碼收得下卻不會生效」—— 71 的碼裡 `redeem` = **0**
--      · 「3a 那道『尚未啟用』封鎖還在 ⇒ 本片沒有解除它」—— 71 逐字**還有**那道 RAISE
--    ⇒ 📌 留著它 = 這一片貼不下去。
--
-- 🔴🔴 **⇒ 本片現在【只做一件事】:收款成功時扣券的那支 trigger(第 2 節)。**
--    ⛔ ~~原檔頭寫的「本片做什麼(四件)」的 ①②③~~ **全部是 `create_order` 那一段的**,
--      隨這次改寫一起失效;**只有 ④ 留下來**。舊字面留在下面不刪,讓照它找碼的人撞到這一段。
--
-- 🛑🛑 **這次改寫留下一個【半套】, 明寫在這裡不要當成做完**:
--    券的基礎設施(本片的 trigger + `20260901020000` 的欄位 + `redeem_coupon`)會落地,
--    而**結帳仍然拒收券碼** —— 71 的 body 裡那句
--    那句「優惠券結帳尚未啟用(券片3b 未上線)」的例外 **還在**
--    (原字面在 71 的 body 裡, 這裡不重打 —— 靜態檢查⑥ 不剝註解,
--     註解裡寫出那個 DDL 形狀會被算成碼而假紅;本檔 :5 已經有一處同型的,
--     那一處【我改之前就在】, 我不再多加一處)。
--    ⇒ ✅ 那是**安全**的(明確拒絕, 不是靜默算錯錢), ⇒ 🔴 **而券功能等於沒有打開。**
--    ⇒ 要打開:把本片原本那段券邏輯**併進 71 的新 body**, 那是**另一片**, 不是這次貼板。
--       (主視窗 2026-09-07 說這一格他端 Sean。)
--
-- 🔵 **被拿掉的那一段去哪了**:它**沒有另存**, 而它**不需要另存** ——
--    原文完整留在 git 歷史裡(本檔這一顆 commit 的 parent), 而**正式庫現在跑的是 71 的版本**;
--    71 貼之前的前一代由主視窗手動存在 `~/pcm-mailbox/貼板-0906/71-前一代-20260907-*.sql`。
--
-- 🔵 `NOTIFY pgrst, 'reload schema'` **留著**(下面), 而它的理由改了:
--    原本是「換簽之後要刷 cache」, 而本片不再換簽;留著是因為本片新增了一支函式,
--    刷一次 cache 不會有害, 而少刷一次的失敗長得像「函式不存在」。
-- ══════════════════════════════════════════════════════════════════════════
