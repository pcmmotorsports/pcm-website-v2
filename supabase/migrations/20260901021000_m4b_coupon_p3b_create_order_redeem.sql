...
BEGIN
  ...
  --
  --    ⚠️ **我沒有量完的一件, 標未確認**:「同一張單再扣一次會怎樣」。
  --      `20260831160000:243-258` 有一條冪等路(同單同券同帳號、折抵一致 ⇒ 回 `valid:true`),
  --      **而它前面還有一道閘** —— `:190` `coupon_redeem_order_problem()` 非 NULL 就先 `RAISE`。
  --      🛑 ⛔ ~~前兩版我在這裡寫過兩段對周邊函式行為的推論, 兩段都被逐條打掉~~
  --         ⇒ **不再在這裡複述別支函式的行為**;要知道請開那支檔。
