-- 20260915060000 · M-4b #953 P2 · 三支寫 orders.total 的 RPC 改呼叫 public.pcm_order_total(), 不再各自寫一份等式。
--
-- 🛑🛑 **未貼。** 貼的人是 Sean(或他明文授權的那一個編號)。**前提:20260915030000(P1, 板 161)先貼**, 前置閘⓪ 釘它。
-- plan:`docs/plans/2026-09-14-953-order-total-equation-single-source-plan.md` P2(Sean 2026-09-14 批 Q1 甲 / Q2 甲:改金額那道「有稅不給改」的閘【不拆】)。
-- 版本號:`scripts/migration-version-free.sh 20260915060000` 2026-09-14 掃過(沒人用);貼的當天重掃。
--
-- ═══════════════════════════════════════════════════════════════
-- 三支各疊在哪一代上(本檔的函式文字是**程式從那一代逐 byte 抽出**, 只換 v_total 那一段;生成器 = 本檔 commit body 記的 gen-p2.py)
-- ═══════════════════════════════════════════════════════════════
--   create_order(11 參)                 ← 20260907040000:145 (live;20260913090000 只 DROP 10 參那支, 不動這支)   本體 md5 42a9132a887b1e39995452009dd7cb47
...
-- 回退:`supabase/rollbacks/20260915060000-rollback.sql`(三支整支貼回上一代;先於 P1 的回退 —— P1 的 rollback 會掃到這三支本體而拒跑, 那是設計)。
-- SET LOCAL lock_timeout = '5s';  ← 回退那支自己也夾這一行。
-- 驗收:`supabase/after-checks/20260915060000-rpc-total-single-source.sql`(拋棄式 PG:三支本體含 pcm_order_total、舊字面不在、md5 = 新代;
--   行為格用 admin_update_order_item_amount 改一次價 ⇒ total 與 pcm_order_total() 逐位元同)。
...
BEGIN
  ...
  IF v_src IS NULL THEN RAISE EXCEPTION '前置閘①:找不到 7 參 admin_update_order_item_amount'; END IF;
  IF pg_catalog.md5(v_src) = '4b3d6e086edadb27b896f161c10b61dd' THEN RAISE EXCEPTION '前置閘①:admin_update_order_item_amount 已是本檔那一代 ⇒ 本支貼過了, 停'; END IF;
  IF pg_catalog.md5(v_src) <> '665d49b5a5cc3246bcdba0e3ffc85042' THEN
    RAISE EXCEPTION USING MESSAGE = '前置閘①:admin_update_order_item_amount 本體 md5 是 ' || pg_catalog.md5(v_src) || ', 本檔抄的是 20260915040000 那一代 665d49b5a5cc3246bcdba0e3ffc85042 ⇒ 停下人工對齊';
  END IF;
END
$gate_pre$;

-- ══ ① create_order(11 參)—— 整支 = 20260907040000:145 逐 byte + v_total 那一段 ═══════════════════
...
-- ══ ② admin_create_manual_order(13 參)—— 整支 = 20260914140000:105 逐 byte + v_total 那一段 ═════════
...
BEGIN
  ...
  -- #953 P2(20260915060000):subtotal 那道先驗(要 ::integer 餵函式);total 那道留在原位、同一個錯碼。
  IF v_subtotal > 2147483647 THEN
  ...
  END IF;
  -- #953 P2:總額等式只住 public.pcm_order_total()(20260915030000)—— 而且【多帶第四項 tax_total】。
  --   今天上面 pcm_e13_no_edit_when_taxed 那道閘讓有稅的單進不來 ⇒ tax_total 恆 0 ⇒ 行為不變;
  --   那道閘哪天拿掉(要一起重算稅, 另一片), 這一行不用再改 —— 這就是 #953 說「B2 的地雷」拆掉的那一格。
  v_total := public.pcm_order_total(v_subtotal::integer, v_ord.shipping_fee, v_ord.discount_total, v_ord.tax_total);
  IF v_total < 0 THEN
...
DECLARE
  ...
  v_sig text; v_new text; v_old_literal text;
BEGIN
  ...
  LOOP
    ...
    IF v_src IS NULL THEN RAISE EXCEPTION '事後閘①:% 不見了', v_sig; END IF;
    IF pg_catalog.md5(v_src) <> v_new THEN
      RAISE EXCEPTION USING MESSAGE = '事後閘①:' || v_sig || ' 本體 md5 是 ' || pg_catalog.md5(v_src) || ', 編檔時算的是 ' || v_new;
    END IF;
    IF pg_catalog.strpos(v_src, 'public.pcm_order_total(') = 0 THEN
      RAISE EXCEPTION '事後閘②:% 本體沒有呼叫 public.pcm_order_total(', v_sig;
    END IF;
    -- 舊字面【當成程式碼】不得再出現:去掉每一行 `--` 之後的部分再找(註解裡提到它沒關係)
    ...
    END IF;
  END LOOP;
  -- 事後閘⑤:三支的 EXECUTE 面沒變(anon 一律不能;其餘照各代原樣, 這裡只釘「沒有變寬」的那一格)
  IF pg_catalog.has_function_privilege('anon', 'public.admin_create_manual_order(uuid, uuid, text, text, text, text, jsonb, jsonb, integer, jsonb, text, text, jsonb)', 'EXECUTE')
     OR pg_catalog.has_function_privilege('anon', 'public.admin_update_order_item_amount(uuid, uuid, integer, integer, text, text, text)', 'EXECUTE') THEN
    RAISE EXCEPTION '事後閘⑤:後台兩支 RPC 對 anon 開著 EXECUTE';
  END IF;
END
$gate_post$;
