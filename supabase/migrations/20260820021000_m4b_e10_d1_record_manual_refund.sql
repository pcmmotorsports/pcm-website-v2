...
--
-- ── ⚠️ 錯誤訊息不得指向【不存在的動作】(2026-08-20 W5 盲審 R3 前,我自己先量到)──────
--   我原本在 actor 那則訊息裡寫「請先用原本的 request_id **查**那筆是否已登記」。
--   實測:`grep -rn --include='*.ts' --include='*.tsx' "order_manual_refunds" packages apps` ⇒ **0**
--   (正向對照:同族 `order_refunds` ⇒ 24;負對照一個不存在的表名 ⇒ 0)
--   ⇒ **沒有任何畫面查得到本表** ⇒ 那句話把員工推去一個**不存在的動作**。
--   ⇒ 已改成「沒有畫面可查 ⇒ 請找系統維護確認」。
--   📌 這一族 repo 記過:**文案指向一個不存在的流程** —— 而它比說錯話更貴,
--     因為員工會先花時間去找那個不存在的東西,然後自己想辦法(通常就是重送)。
--
-- ── ⚠️ 錯誤訊息的三個坑,而第二個是我【八行前才剛修掉的同一個】(W5 盲審 R3)──────
--   ① **判準的主詞**:我原本寫「問『我手上實際把錢交回去幾次?』」——
--      而這則訊息的場景正是「原經手人停用、**換人**重送」⇒ 接手的人答案是**零次**,
--      `一次`/`兩次` 兩個分支都不成立 ⇒ 他只能猜,而開場白正是「不要靠感覺」。
--      ⇒ 改成「**這張單實際退回去給客人的錢,發生過幾次?**」——**主詞是那筆錢,不是讀的人。**
--   ② 🔴 **我在 :59 自己寫下「訊息不得指向不存在的動作」,然後在八行後寫了「要更正既有那筆請走沖銷」**
--      —— 而**沖銷入口對本表不存在**(那是另一片)。更糟的是:舊版寫「走沖銷(D2)」,
--      那個括號至少標示了「這是另一片」,而**我把它拿掉了** ⇒ 讀起來像一個現在就能點的東西。
--      📌 這是「我為前一段寫下的理由,沒有套用到下一段」的**第三次**(前兩次見 traps 那條)。
--   ③ **不要把會過期的量測寫進永久的訊息**:我原本寫「(全 repo 零 app code 讀本表)」——
--      登記畫面上線那天那句就是假的,**而不會有東西紅**。
--      ⇒ 改成讓讀的人自己判得出來:「**你在後台如果找不到…那就是還沒有**」——
--        它不會過期,因為它描述的是**讀者當下看得到的東西**,不是我某天量到的數字。
--
-- ── 🔴 RPC 是唯一的守門點 ────────────────────────────────────────────────
