-- ============================================================
-- 經銷品牌折扣接進價格:商品頁 / 購物車、經銷目錄、結帳三處同一個算法(B2B 計畫 §10.3,片 E2)
-- ============================================================
-- plan:docs/plans/2026-09-23-b2b-subdomain-plan.md §10.3(Sean 2026-09-25 新增,「依推薦」;免運看折後小計 Sean 拍板)
-- 退回檔:supabase/rollbacks/20260925040000-rollback.sql(檔內第一行就是 SET LOCAL lock_timeout)
-- 前提:20260925030000(dealer_brand_discounts)先貼。
--
-- 算法只有一支:dealer_discount_apply(經銷價, 折扣%) = round(經銷價 × (100 − 折扣%) ÷ 100);折扣是 NULL 就原價。
--   查折扣(只對 tier = 'store', 後台「車行」)有兩個入口, 都接到上面那一支:
--   · dealer_discounted_amount(會員, 品牌, 經銷價) —— 商品頁、購物車、結帳(一次查幾件)
--   · 經銷目錄 view 用 LEFT JOIN 查(一次要算整個目錄;逐列呼叫實測慢 4–6 倍, 2026-09-25 拋棄式庫 2.6 萬件)進位 = 四捨五入到元(round(numeric) 遇 .5 進位,
--   與商品匯入 roundTwd 的 Math.round 對正數相同;Sean 2026-09-10 稅務進位也是四捨五入)。
-- 接進三處(品牌一律取【母商品】products.brand_id):
--   ① get_effective_prices 商品半與變體半 —— 商品頁、購物車
--   ② products_list_dealer —— 經銷目錄(只由 SECURITY DEFINER 的 search_catalog_by_vehicle_dealer 讀)
--   ③ create_order —— 收錢。另外把「讀等級」搬到 advisory lock 之後並 FOR SHARE 鎖客人那一列:
--      後台存折扣 / 改等級都 FOR UPDATE 同一列 ⇒ 一張單裡每一件用同一版折扣(Codex 計畫 R1/R2)。
-- 三支的底都是【正式庫現役定義】(2026-09-25 唯讀 pg_dump / pg_get_functiondef 取出, 前置閘比對 md5),
--   只改上面說的那幾行, 其餘逐字不動(下架判斷、優惠券、稅、search_path)。
-- 🔵 前台窗(pcm-website-v2-40)之後的 D1 會以本版為底改這三樣, 版本號排在本檔之後(2026-09-25 兩窗對好)。
-- ============================================================
...
BEGIN
  ...
  IF pg_catalog.to_regclass('public.dealer_brand_discounts') IS NULL THEN
    RAISE EXCEPTION '前置閘①:找不到 dealer_brand_discounts ⇒ 先貼 20260925030000。';
  END IF;
  IF pg_catalog.to_regprocedure('public.dealer_discounted_amount(uuid, uuid, integer)') IS NOT NULL
     OR pg_catalog.to_regprocedure('public.dealer_discount_apply(integer, numeric)') IS NOT NULL THEN
    RAISE EXCEPTION '前置閘②:dealer_discounted_amount / dealer_discount_apply 已存在 ⇒ 停(可能已貼過)。';
  END IF;
  IF (SELECT pg_catalog.md5(p.prosrc) FROM pg_catalog.pg_proc p WHERE p.oid = 'public.create_order(jsonb, uuid, text, jsonb, uuid, text, text, text, text, text, text)'::regprocedure)
     IS DISTINCT FROM '53f803ed322abf33bf1bc31beae7b982' THEN
    RAISE EXCEPTION '前置閘③:create_order 本體不是 2026-09-25 量到的那一版(20260915100000)⇒ 有人改過, 停, 重新以現役為底。';
  END IF;
  IF (SELECT pg_catalog.md5(p.prosrc) FROM pg_catalog.pg_proc p WHERE p.oid = 'public.get_effective_prices(uuid[], uuid[])'::regprocedure)
     IS DISTINCT FROM 'cc73a38bf5775da7b402c9c1c9fad140' THEN
    RAISE EXCEPTION '前置閘④:get_effective_prices 本體不是 2026-09-25 量到的那一版(20260924100000)⇒ 停。';
  END IF;
  -- 🔴 整支 view 的指紋, 不只看價格那一格(Codex E2 R1):別人加了排除條件而保留價格那一格, 只看一格會被本檔蓋掉
  IF v_vfp IS DISTINCT FROM '684c5b40c960bdc1a690d33b90add602' THEN
    RAISE EXCEPTION '前置閘⑤:products_list_dealer 不是 2026-09-25 量到的那一版(20260924100000)⇒ 停, 重新以現役為底。';
  END IF;
END
$pre$;

-- ── 1. 算價(唯一的公式)與查折扣 ─────────────────────────────────
-- 🔵 兩支都刻意【不是】SECURITY DEFINER、不帶 SET:
--    · dealer_discount_apply 是純運算 ⇒ PostgreSQL 會把它內嵌進查詢計畫(目錄 view 一次算整個目錄)。
--    · 它們只在三支 SECURITY DEFINER 函式與 view 裡面被叫 ⇒ 以 owner 身分執行;任何客人角色都沒有 EXECUTE。
--    · 名稱全部帶 schema, 不依賴 search_path。
-- 進位:round(numeric) 遇 .5 進位(對正數與 JS Math.round 相同);先算單價再乘數量(create_order 的 v_line_total)。
CREATE FUNCTION public.dealer_discount_apply(p_amount integer, p_percent numeric)
RETURNS integer
LANGUAGE sql
IMMUTABLE
AS $fn$
  SELECT CASE
    WHEN p_amount IS NULL OR p_percent IS NULL THEN p_amount
    ELSE pg_catalog.round(p_amount::numeric * (100 - p_percent) / 100)::integer
  END
$fn$;

CREATE FUNCTION public.dealer_discounted_amount(p_user uuid, p_brand uuid, p_amount integer)
RETURNS integer
LANGUAGE sql
STABLE
AS $fn$
  SELECT public.dealer_discount_apply(p_amount,
    (SELECT d.percent
       FROM public.dealer_brand_discounts d
       JOIN public.customers c ON c.user_id = d.customer_user_id AND c.tier = 'store'::public.member_tier
      WHERE d.customer_user_id = p_user AND d.brand_id = p_brand))
$fn$;

REVOKE ALL ON FUNCTION public.dealer_discount_apply(integer, numeric) FROM PUBLIC, anon, authenticated, service_role;
REVOKE ALL ON FUNCTION public.dealer_discounted_amount(uuid, uuid, integer) FROM PUBLIC, anon, authenticated, service_role;

-- ── 2. 經銷目錄 view(以 20260924100000 為底, 只改價格那一格)────────────
...
SELECT
  ...
  -- 🔴 20260925040000(B2B 計畫 §10.3):再套這位經銷會員對這個品牌的額外折扣(沒設定就原價)。
  --    本 view 只由 SECURITY DEFINER 的經銷目錄 RPC 讀 ⇒ auth.uid() 是那位登入的客人。
  --    折扣用下面的 LEFT JOIN 查(不逐列呼叫 dealer_discounted_amount:整個目錄算下來慢 4–6 倍);
  --    公式仍是同一支 dealer_discount_apply。只有 tier = 'store' 才有 dc 那一列。
  public.dealer_discount_apply(coalesce(b.price_store, v.price_general), CASE WHEN dc.user_id IS NOT NULL THEN dd.percent END) AS price_general,
  v.supplier_slug,
  v.card_image,
  v.fits,
  v.brand_name,
...
-- 主鍵 (customer_user_id, brand_id) 與 customers 主鍵 ⇒ 兩個 LEFT JOIN 都最多一列, 不會讓商品重複。
LEFT JOIN public.dealer_brand_discounts dd
  ON dd.customer_user_id = (SELECT auth.uid()) AND dd.brand_id = v.brand_id
LEFT JOIN public.customers dc
  ON dc.user_id = dd.customer_user_id AND dc.tier = 'store'::public.member_tier
...
-- ── 3. get_effective_prices(以正式庫現役為底, 只改商品半與變體半的 store 那一格)────
...
BEGIN
  ...
  SELECT 'product'::text, p.id,
         CASE
           ...
           WHEN v_tier = 'store' THEN public.dealer_discounted_amount(v_uid, p.brand_id, coalesce(
  ...
  SELECT 'variant'::text, v.id,
         CASE
           ...
           WHEN v_tier = 'store' THEN public.dealer_discounted_amount(v_uid, pp.brand_id, coalesce(v.price_store, v.price_general))
...
-- ── 4. create_order(以正式庫現役為底:讀等級搬到鎖之後 + FOR SHARE、多取 p.brand_id、單價套折扣)────
...
BEGIN
  ...
  LOOP
    ...
    IF v_tier = 'store'::public.member_tier THEN
      ...
      v_unit_price := public.dealer_discounted_amount(v_uid, v_variant.brand_id, coalesce(v_variant.price_store, v_variant.price_general));
    ELSE
...
DECLARE
  ...
  r      text;
  v_cfg  text;
  -- 🔴 收權斷言清單:本檔建出來或換掉的【可授權物件】(migration-static-checks ③ 會數)
  v_relations text[] := ARRAY['public.products_list_dealer']::text[];
  v_functions text[] := ARRAY['public.dealer_discount_apply(integer, numeric)', 'public.dealer_discounted_amount(uuid, uuid, integer)', 'public.get_effective_prices(uuid[], uuid[])', 'public.create_order(jsonb, uuid, text, jsonb, uuid, text, text, text, text, text, text)']::text[];
BEGIN
  -- 🔵 view 的指紋:pg_get_viewdef 會隨 search_path 決定要不要寫 schema ⇒ 固定成空字串量, 量完換回來
  --    (不另建暫存函式:同一連線連跑本檔與退回檔會撞名, Codex E2 R2)。
  v_sp := pg_catalog.current_setting('search_path');
  ...
  -- 兩支算價函式:沒有任何客人角色能叫;不是 DEFINER
  FOREACH r IN ARRAY ARRAY['public.dealer_discount_apply(integer, numeric)', 'public.dealer_discounted_amount(uuid, uuid, integer)']::text[] LOOP
    IF pg_catalog.has_function_privilege('anon', r, 'EXECUTE')
      ...
      RAISE EXCEPTION '事後閘①:% 被開了 EXECUTE ⇒ 停。', r;
    END IF;
    IF (SELECT p.prosecdef FROM pg_catalog.pg_proc p WHERE p.oid = r::regprocedure) THEN
      RAISE EXCEPTION '事後閘①b:% 不該是 SECURITY DEFINER ⇒ 停。', r;
    END IF;
  END LOOP;

  -- 兩支被換掉的:仍是 DEFINER + search_path 空字串;EXECUTE 維持原樣(authenticated 可, anon 不可)
  FOREACH r IN ARRAY ARRAY['public.get_effective_prices(uuid[], uuid[])', 'public.create_order(jsonb, uuid, text, jsonb, uuid, text, text, text, text, text, text)']::text[] LOOP
    ...
    IF NOT pg_catalog.has_function_privilege('authenticated', r, 'EXECUTE')
       OR pg_catalog.has_function_privilege('anon', r, 'EXECUTE') THEN
      RAISE EXCEPTION '事後閘③:% 的 EXECUTE 變了 ⇒ 停。', r;
    END IF;
    IF (SELECT pg_catalog.strpos(p.prosrc, 'public.dealer_discounted_amount(') FROM pg_catalog.pg_proc p WHERE p.oid = r::regprocedure) = 0 THEN
      RAISE EXCEPTION '事後閘④:% 沒有接上 dealer_discounted_amount ⇒ 停。', r;
    END IF;
  END LOOP;
  IF pg_catalog.strpos(pg_catalog.pg_get_viewdef('public.products_list_dealer'::regclass, true), 'dealer_discount_apply(') = 0 THEN
    RAISE EXCEPTION '事後閘④b:products_list_dealer 沒有接上 dealer_discount_apply ⇒ 停。';
  END IF;
  -- 本檔寫出來的三個指紋(退回檔的前置閘比對這三個;之後有人以本版為底改了任一個, 退回檔就會停)
  IF v_vfp IS DISTINCT FROM '42ddb5f87a1096361f42a6db13255918'
     OR (SELECT pg_catalog.md5(p.prosrc) FROM pg_catalog.pg_proc p WHERE p.oid = 'public.create_order(jsonb, uuid, text, jsonb, uuid, text, text, text, text, text, text)'::regprocedure) IS DISTINCT FROM '77c7ab9cf4dc26404af6dbbe723a1d42'
     OR (SELECT pg_catalog.md5(p.prosrc) FROM pg_catalog.pg_proc p WHERE p.oid = 'public.get_effective_prices(uuid[], uuid[])'::regprocedure) IS DISTINCT FROM 'c316058adcad20679d7503b8b0967bb2' THEN
    RAISE EXCEPTION '事後閘⑦:寫出來的三個本體指紋與拋棄式庫量到的不同 ⇒ 停(退回檔會對不上)。';
  END IF;
  IF has_table_privilege('anon', 'public.products_list_dealer', 'SELECT')
     OR has_table_privilege('authenticated', 'public.products_list_dealer', 'SELECT') THEN
    RAISE EXCEPTION '事後閘⑤:products_list_dealer 被開了 SELECT ⇒ 停。';
  END IF;

  -- 真的叫一次:沒有折扣 ⇒ 原價;NULL 金額 ⇒ NULL(不變成 0)
  IF public.dealer_discounted_amount('00000000-0000-0000-0000-000000000000'::uuid, '00000000-0000-0000-0000-000000000000'::uuid, 1000) IS DISTINCT FROM 1000
     OR public.dealer_discounted_amount('00000000-0000-0000-0000-000000000000'::uuid, '00000000-0000-0000-0000-000000000000'::uuid, NULL) IS NOT NULL
     -- 手算:7.5% ⇒ 1000→925、1234→1141(1141.45)、999→924(924.075)、1500→1388(1387.5 進位)
     OR public.dealer_discount_apply(1000, 7.5) <> 925 OR public.dealer_discount_apply(1234, 7.5) <> 1141
     OR public.dealer_discount_apply(999, 7.5) <> 924 OR public.dealer_discount_apply(1500, 7.5) <> 1388
     OR public.dealer_discount_apply(0, 7.5) <> 0 OR public.dealer_discount_apply(1000, NULL) <> 1000 THEN
    RAISE EXCEPTION '事後閘⑥:dealer_discounted_amount 自測不對 ⇒ 停。';
  END IF;
  RAISE NOTICE '✅ 品牌折扣已接進 get_effective_prices / products_list_dealer / create_order。';
END
$post$;
