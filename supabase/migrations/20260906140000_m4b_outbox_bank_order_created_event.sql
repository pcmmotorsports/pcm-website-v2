...
-- ══════════════════════════════════════════════════════════════════
-- 🔬 **線上實況(2026-09-06 唯讀複量, 零寫入;DB now() = 2026-09-05 18:4x UTC)**
-- ══════════════════════════════════════════════════════════════════
-- 起因:帳號窗 `ada19656b` 量到「`20260902120000`(`…_v2`)**不在線上**, 而舊 check 仍 = 1」
--       ⇒ 主視窗要我確認本支的前置閘釘的那一代對不對。
--
-- 逐字讀數(`pg_get_constraintdef`):
--   conname = `email_outbox_event_type_check`
--   def     = CHECK ((event_type = ANY (ARRAY['order_created'::text, 'order_shipped'::text,
--             'order_cancelled'::text, 'order_unpaid_cancelled'::text, 'shipment_tracking_corrected'::text])))
--   `_v2` / `_v3` / `_v4` / `_v5` 這些名字 ⇒ **一個都不在**(`string_agg` 回空)
--   `event_type` 這一欄上的 check/fk ⇒ **只有一條**, 就是上面那個
--   🟢 正對照:`public.email_outbox` 存在 = 1 · 🔵 負對照:現造約束名 = 0
--
-- 🎯 **⇒ 本支的四道前置閘與線上實況【完全對得上】, 不撞。**
--
-- 🔴🔴 **而帳號窗那個「_v2 不在線上」是【假陰性】, 這一句要留著**:
--    `20260902120000` 的最後一步逐字是 `RENAME CONSTRAINT … _v2 TO email_outbox_event_type_check`
--    ⇒ 📌 **`_v2` 這個名字【本來就不該在】—— 它是中繼名, 不是終態名。**
--    ⇒ 🛑 **拿「找不到 _v2」當「那支沒貼」= 拿一個【設計上必然消失的東西】當存在性證據。**
--    ✅ 而**內容**證明它貼了:`'order_cancelled'` 在 def 裡, 而**那個值正是 `20260902120000` 加的**。
--    ⇒ 📌 同一族:`⟦01-LEDGERFALSENEG⟧`(帳本的 0 什麼都不代表)。**問【裡面是什麼】, 不要問【叫什麼名字】。**
