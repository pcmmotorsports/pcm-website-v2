-- 20260914140000_m4b_orders_vehicle_snapshot_manual_order.sql —— #956 乙:後台建單「車種」一格(第 9 代:12 參 → 13 參)+ orders.vehicle_snapshot
--
-- 🛑 未貼(寫好不貼;貼是 Sean 一次一個編號)。plan `docs/plans/2026-09-14-956-manual-order-vehicle-plan.md`。
--
-- ══ Sean 拍的 ══════════════════════════════════════════════
-- 2026-09-14 #956 拍乙(看 ~/pcm-mailbox/0914-品味題/956-B-一格.png):一格「車種」照 Excel 寫法打, 對到字典就帶入, 沒有就照存。
-- 存法:一張單一台車(08-28 plan 甲)⇒ 寫在 orders 上, 不是每個品項一台。
--
-- ══ 做什麼 ═════════════════════════════════════════════════
...
--
-- ══ 形狀:DROP 12 參 + CREATE 13 參(逐字照第 8 代那次)══════════
-- 不用 CREATE OR REPLACE:簽章含參數列, 加參數 = 第二支多載, 舊 12 名呼叫 PGRST203。
-- 🔴 DROP 帶走 ACL 與 COMMENT ⇒ §6 三行 REVOKE/GRANT + §6b COMMENT ON 一定要重貼。
-- 🔴 `DEFAULT NULL` ⇒ 舊 TS 送 12 個名字仍唯一命中 ⇒ DB 先貼、UI 後上, 中間態合法。
--
-- ══ 回滾 ═══════════════════════════════════════════════════
-- `supabase/rollbacks/20260914140000_down.sql` = DROP 13 參 + 逐字貼回第 8 代 + 它的 ACL / COMMENT + DROP COLUMN(欄是本支加的;顧客站恆 NULL, 手動單填過的車會跟著掉 —— 回滾前先匯出)。
-- 🔴 順序 = 先 revert code 再回 DB(DB 回舊版時新 UI 送 13 個名字 ⇒ PGRST202)。
...
DO $pre$
DECLARE
  v_old oid := pg_catalog.to_regprocedure('public.admin_create_manual_order(uuid,uuid,text,text,text,text,jsonb,jsonb,integer,jsonb,text,text)');
  v_new oid := pg_catalog.to_regprocedure('public.admin_create_manual_order(uuid,uuid,text,text,text,text,jsonb,jsonb,integer,jsonb,text,text,jsonb)');
  v_src text;
  v_secdef boolean; v_strict boolean; v_vol "char"; v_config text[]; v_owner text;
BEGIN
  IF v_old IS NULL THEN
    RAISE EXCEPTION '前置閘一:找不到 12 參那一代 —— 這台庫上的 admin_create_manual_order 不是我以為的形狀(20260914030000 貼了沒?), 停下人工對齊';
  END IF;
  IF v_new IS NOT NULL THEN
    RAISE EXCEPTION '前置閘二:13 參那一代【已經存在】—— 本檔貼過了, 或有人先建了一支多載, 停下人工對齊';
  END IF;
  ...
  -- 🔴 本體指紋:本檔的函式體是從第 8 代逐字抄的(20260914030000 後置閘二那一版)。
  IF pg_catalog.md5(v_src) <> '3021a009074b0caa3f1ceb5cd5c90cdd' THEN
    RAISE EXCEPTION USING MESSAGE =
      '前置閘三:函式體 md5 不是第 8 代(3021a009074b0caa3f1ceb5cd5c90cdd;實得 ' || pg_catalog.md5(v_src) || ')⇒ 有人動過它, 本檔的抄本基準已失效, 停下人工對齊';
  END IF;
  IF NOT (v_secdef AND NOT v_strict AND v_vol = 'v'
    ...
    RAISE EXCEPTION USING MESSAGE =
      '前置閘四:函式屬性不是第 8 代那一組(prosecdef=' || v_secdef::text || ' proisstrict=' || v_strict::text
      || ' provolatile=' || v_vol::text || ' proconfig=' || COALESCE(v_config::text, 'NULL') || ' owner=' || v_owner || ')';
  END IF;
  IF pg_catalog.strpos(v_src, 'manual_dict') > 0 THEN
    RAISE EXCEPTION '前置閘五:函式體裡已經有 manual_dict ⇒ 這一片貼過了, 拒重貼';
  END IF;
  IF pg_catalog.strpos(v_src, 'tier_overridden') = 0 THEN
    RAISE EXCEPTION '前置閘六(正對照):連 tier_overridden 都找不到 ⇒ 這把尺在錯的東西上';
  END IF;
  IF pg_catalog.strpos(v_src, 'zzz_never_a_literal') > 0 THEN
    RAISE EXCEPTION '前置閘七(負對照):現造字面竟然命中 ⇒ 這把尺壞了';
  END IF;
  IF EXISTS (SELECT 1 FROM pg_catalog.pg_attribute a WHERE a.attrelid = 'public.orders'::regclass AND a.attname = 'vehicle_snapshot' AND NOT a.attisdropped) THEN
    RAISE EXCEPTION '前置閘八:orders.vehicle_snapshot 已存在 ⇒ 本支貼過了';
  END IF;
  IF pg_catalog.to_regclass('public.order_items') IS NULL THEN
    RAISE EXCEPTION '前置閘九:找不到 order_items(抄 CHECK 的來源)';
  END IF;
END
$pre$;

-- ── §1 orders.vehicle_snapshot(形狀 CHECK 逐字抄 20260716180000:31-49)──
ALTER TABLE public.orders ADD COLUMN vehicle_snapshot jsonb;
COMMENT ON COLUMN public.orders.vehicle_snapshot IS
  '#956 乙(20260914140000):【手動單】這張單一台車(選填;NULL = 沒填)。形狀 {kind:''dict'',brand,model,year?,source:''manual_dict''} 或 {kind:''free'',raw,year?,source:''manual_text''}(判別式與 order_items.vehicle_snapshot 同款;source 標得出「這台不在字典」)。'
  ' 唯一寫入 = admin_create_manual_order 第 9 代白名單重組。🔴 顧客站的單這欄【恆 NULL】—— 它的車在品項上(order_items.vehicle_snapshot);顯示端訂單級有就印它, 沒有才退回品項。純 metadata, 不驅動金流 / 取價。';
ALTER TABLE public.orders
  ADD CONSTRAINT orders_vehicle_snapshot_shape
  CHECK (
...
-- ── §2 換函式:DROP 12 參(第 8 代)→ CREATE 13 參(第 9 代)──
DROP FUNCTION public.admin_create_manual_order(
  uuid, uuid, text, text, text, text, jsonb, jsonb, integer, jsonb, text, text);
...
BEGIN
  ...
  END IF;
  IF pg_catalog.strpos(v_src, 'tier_overridden') = 0 THEN
    RAISE EXCEPTION '後置閘六(正對照):第 8 代的 tier_overridden 不見了 ⇒ 抄本不是第 8 代';
  END IF;
  ...
  FOREACH r IN ARRAY v_functions LOOP
    IF pg_catalog.has_function_privilege('anon', r, 'EXECUTE') OR pg_catalog.has_function_privilege('authenticated', r, 'EXECUTE') THEN
      RAISE EXCEPTION '後置閘九:anon / authenticated 叫得動 %', r;
    END IF;
    IF NOT pg_catalog.has_function_privilege('service_role', r, 'EXECUTE') THEN
      RAISE EXCEPTION '後置閘十:service_role 叫不動 % ⇒ 後台建單會 42501', r;
    END IF;
  END LOOP;
  IF pg_catalog.obj_description(v_new, 'pg_proc') IS NULL THEN RAISE EXCEPTION '後置閘十一:COMMENT 沒貼回'; END IF;
  IF NOT EXISTS (SELECT 1 FROM pg_catalog.pg_constraint c WHERE c.conrelid = 'public.orders'::regclass AND c.conname = 'orders_vehicle_snapshot_shape') THEN
    RAISE EXCEPTION '後置閘十二:orders_vehicle_snapshot_shape CHECK 沒掛上';
  END IF;
  IF pg_catalog.to_regprocedure('public.zzq_no_such_fn_20260914e()') IS NOT NULL THEN
    RAISE EXCEPTION '後置閘 自檢:負對照命中 ⇒ 量具可疑';
  END IF;
  RAISE NOTICE '第 9 代貼好了:admin_create_manual_order 13 參(p_vehicle)+ orders.vehicle_snapshot。';
END
$post$;
