...
--
-- 🔴🔴 **[R1-F6]** `20260905070000` 逐字寫著:
--    「**所以這裡【不加 FOR UPDATE】—— 那個競態改由事後掃描器收**」
--    ⇒ 📌 **070000 把本支當成【那個競態的唯一修法】, 不是兜底。**
--    ⇒ 🛑 **所以本支【不可以】因為「它只是兜底」而被關掉** —— 關掉它,
--       那一行明文交出去的世界就沒有人接了。
--    ⚠️ **座標用【引文】不用行號**(⛔ ~~原本寫 `070000:168`~~)—— codex R3 ⑦ 那一輪,
--       另一個窗拿 `:169` 去開檔, 讀到的是一行 SQL:**兩棵樹的同一支檔行號差 5**
--       (本支加的那五行只在 `agent/line-mail` 上)⇒ 📌 **多窗共寫的檔, 行號在別人手上是假的。**
--    🔵 「那個競態」= 下面的**競態乙**(兩筆付款同時打同一張已取消的單), 不是甲。
--
-- 🛑 **它【不是】在修一個【新】的洞** —— 而**那裡有【兩個】不同的競態, 我原本把它們寫成一個**。
--
-- 🔴🔴 **[codex R3 ⑦ 說本檔有三套互相矛盾的敘事 —— 它抓對了症狀, 而病因不是「哪一套是假的」]**
--    🔬 我去逐行讀了 `20260905070000`(**不是憑記憶**), 而答案是:**那是兩個競態, 各有各的接手者。**
--
--    ┌ 競態甲:**取消 vs 付款**(一筆付款進來時, 取消正在飛)
--    │   ✅ 接手者 = **四支正式取消端全部先 `FOR UPDATE`** ⇒ 付款的 INSERT 被 FK 擋得夠久,
--    │      等它進得來時取消已經提交 ⇒ trigger 讀得到 ⇒ **它到不了線上。**
--    │   📍 出處 = `20260905070000` 的「[R4-F3 之後回頭訂正這整段的【射程】]」那一段,
--    │      逐字:「那三個讀數描述的困境**到不了線上**」。
--    │   🔵 前提由 `scripts/cancelled-at-writers-lock-gate.test.ts` 守著(檔在, 當場 `ls` 過)。
--    │   ⇒ **本支【不是】甲的接手者。**本支對甲只是「前提被改壞時還有人收」。
--    │
--    └ 競態乙:**兩筆付款同時打同一張已取消的單**
--        🛑 接手者 = **本支。就是這一支, 沒有別人。**
--        📍 出處 = `20260905070000` 的「⇒ 而我試了那個排法, 它【被實測推翻】」那一段:
--           advisory lock + `FOR UPDATE` 的排法在**探針世界 E** 造成**真死結、只有一筆進得去**
--           ⇒ 🛑 **客人的錢掉了一筆**;成因是 FK 的 `KEY SHARE` 在**任何 trigger 之前**就被拿走
--           ⇒ advisory lock 來得太晚。⇒ 該段結論逐字:
--           「**所以這裡【不加 `FOR UPDATE`】—— 那個競態改由事後掃描器收**」。
--        🔼 **`pcm_pending_refund_open_for` 的函式體裡確實一個 `FOR UPDATE` 都沒有**(當場 grep 過)。
--
--    ⚠️ **而 `20260905070000` 自己也有一段過期的字面**:那支檔開頭附近寫著
--       「`FOR UPDATE` 是這一片的樞紐 —— 拿掉它整個修法失效」,而**碼裡沒有那個 `FOR UPDATE`** ——
--       那是後來被自己的訂正段推翻、卻**沒有被劃掉**的舊設計。
--       📌 讀那支檔的人**先撞到那一句**(它排在前面)⇒ 它會讓人以為甲乙是同一件事。
--       🛑 **那不是我的檔, 我不改它** —— 我把座標寫在這裡, 讓撞到的人知道那不是他讀錯。
--
--    🎯 **我原本錯在哪(留著, 免得下一個人再繞一次)**:
--       我把「甲有人接」與「乙交給我」寫成同一組條列 ⇒ 讀起來像**同一個競態的兩種說法**
--       ⇒ 而**任何一個讀者都只會挑一種相信**。📌 **矛盾不在事實裡, 在我的排版裡。**
--
-- 🔵 **而「兩邊互指」這句話要限定射程**:`20260905070000` 指回本支的那五行
--    **只在 `agent/line-mail` 上**(當場量:我的樹 670 行 / `origin/dev` 665 行;
--    `origin/dev` 那一版 grep `20260905180000` ⇒ **0 命中**)。
--    ⇒ 📌 **在 dev 上它今天是【單向】的** —— 只有本支指過去。合進 dev 之後才會變雙向。
...
⚠️ ⛔ ~~而那個世界原本我寫成「取消與請款同時在飛」~~ —— **講錯了**(codex R3 ⑦ 之後逐行讀 070000):
那一個由**四支取消端各自的 `FOR UPDATE`** 接住;交給本掃描器的是**併發付款**那一個。檔頭有兩者的分界。而 `20260905070000` **今天還沒 apply**
