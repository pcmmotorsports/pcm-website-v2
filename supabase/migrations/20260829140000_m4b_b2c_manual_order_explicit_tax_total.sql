-- ============================================================
-- 🛑🛑 **這支現在【不要 apply】。而擋住它的不是它自己。**
-- ============================================================
--   本檔依賴 `20260828100000`(B1, 加 `orders.tax_total`), 而 **B1 被兩件事擋著**:
--     `#955` 狀態逐字「⏳ 待執行(2026-08-28 立;主視窗裁 `A-MF1 = 甲 + 一道硬條件`)」
--     `#958` 狀態逐字「🔴 **B1 的 apply 前置** —— 第一個理由已解, **第二個仍在 ⇒ B1 仍然不 apply**」
--     (2026-08-29 線C 開 `docs/phase-1-backlog.md` 逐條讀的;負對照 `^### #99955\.` ⇒ 0)
--   ⇒ **那兩件關掉之後**, 才照本檔最下面「Sean 要做什麼」那一節的順序 apply。
--
-- 🔴 **為什麼這句寫在【最前面】而不是最後**:一句正確的 apply 指示, 在一個它不成立的前提下,
--    **讀起來完全正常**。而人讀指示讀開頭 —— **把擋住整段的條件寫在最後 = 寫在沒有人看的地方。**
-- ============================================================
