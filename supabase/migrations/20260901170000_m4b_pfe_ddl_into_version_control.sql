...
-- ------------------------------------------------------------
-- pcm:ddl-into-vc: public.product_fitments_effective
-- pcm:never-apply
...
--     ⚠️ **不要把它挪到 20 行以外** —— 那支腳本只讀檔頭前 20 行, 而那是刻意的:
--       一支檔中段【提到】這個字面(例如在解釋這個機制)不該把它自己變成 never-apply。
--     📌 而它存在的理由是量到的:2026-09-02 這支檔曾被排進「要 Sean 手貼」的清單
--       —— 因為帳本上它是 `0`, 而那個 `0` 對當時 24 支裡的 23 支都是對的。
