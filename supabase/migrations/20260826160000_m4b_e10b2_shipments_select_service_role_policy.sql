...
--    📌 而這一格的形狀:**一句被貼回來的話, 在檔案上與一句他自己說的話長得一模一樣。**
...
-- ============================================================================
-- 🟡 **本片狀態 ⇒ 看下面的「審查紀錄」節, 那裡才是唯一一份。這裡【刻意不重述】。**
--    🔴 **理由是量到的:這一格在同一個晚上過期了【兩次】。**
--      ~~第一版「還沒有過第二輪審查」~~ ⇒ 寫於 2026-08-27 清晨, 而同一天又跑了三輪。
--      ~~第二版「最新這一輪的 codex 那一關還沒跑」~~ ⇒ **它跑了**(cf 代跑, FAIL 1 must-fix + 3 nit,
--        已全折), 而我更新了審查紀錄卻【沒有回頭改這一句】。
--    📌 **一句重述「跑到第幾輪」的狀態句, 每跑一輪就過期一次, 而它自己不會知道。**
--       **而重述它的地方越多, 過期的地方就越多** —— 這一格今晚同時活在本檔、`STATUS.md`、`CURRENT.md` 三處。
--    ⇒ **修法不是再寫一次現況, 是讓這一格不需要被更新:指過去, 不重述。**
--
-- ── ⚠️ 「這支檔能不能 apply」—— 這一格【不要憑本節推】────────────────────────
--    能不能 apply 是**兩個獨立條件**, 分開看:
--      ① 技術上跑不跑得過正式庫 ⇒ ✅ **可以**(2026-08-27 照正式庫量到的五角色形狀實跑, rc=0)
--      ② 審查上該不該送 ⇒ 見「審查紀錄」節。**兩者不會互相蘊涵。**
--    🔴 而 apply 本身**永遠是 Sean 的手**(不可逆、對外可見 ⇒ CLAUDE.md R3)。
--    ⚠️ **`STATUS.md` 與 `CURRENT.md` 引用本檔 `:7` 的那兩處已經過期**(2026-08-27 b4 窗抓到):
--       `:7` 現在是拓撲那一行;「不要 apply」只剩 `:73` 而那句說的是**相反**的意思。
--       📌 **一個引用【行號】的句子, 在被引的檔改動的那一刻就過期, 而兩邊都不會有訊號。**
--
-- 時序(不要壓成一句):
...
BEGIN
  ...
  FOREACH t IN ARRAY ARRAY['shipments', 'shipment_items'] LOOP
    ...
    -- 🔴🔴 **Sean 2026-08-27 選【乙】**(選項字面由我擬, 不是他的逐字 —— 見檔頭那格)。
