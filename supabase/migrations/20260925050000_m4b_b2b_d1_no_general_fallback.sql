-- 20260925050000 · B2B D1:經銷價缺的時候不再退回一般價,create_order 依站別擋
--
-- 計畫:docs/plans/2026-09-23-b2b-subdomain-plan.md「D1 實作計畫」(Codex R1 四項必修已併入、Fable R2 PASS)。
-- 以後台窗 E2(20260925040000,commit e8292e7a2,分支 agent/shop-6)為底,改三樣:
--   ① products_list_dealer:經銷目錄價格 = 品牌折扣(基準款變體 price_store),缺經銷價 ⇒ NULL
--   ② get_effective_prices:store 的商品級與變體級缺經銷價 ⇒ NULL(非 store 不變;兩道 WARNING 原本就只在「一般價也壞」時出聲,不用改)
--   ③ create_order:store 缺經銷價 ⇒ 既有「變體無有效單價」拒絕;依 x-pcm-site 標頭擋錯站(Sean Q3 甲、主視窗裁甲)
-- 🔴 前置:E2 必須先貼(前置閘比 E2 的三個指紋與函式屬性)。
-- 🔴 退回:supabase/rollbacks/20260925050000-rollback.sql;**退回前先把經銷站下線**(否則缺價商品會以一般價加 5% 成交)。
--    要退 E2 必須先退 D1。
-- 🔴 上線閘:L4 空窗要等「D1 已貼」且「兩站部署都含 apps/storefront/src/lib/supabase/server.ts 帶 x-pcm-site 那顆」才算關掉。
-- 只寫檔,由 Sean 貼(主視窗整理步驟)。
...
BEGIN
  ...
  IF (SELECT pg_catalog.count(*) FROM pg_catalog.pg_proc p WHERE p.proname = 'create_order' AND p.pronamespace = 'public'::regnamespace) <> 1 THEN
    RAISE EXCEPTION '前置閘①:create_order 不是剛好一支(可能有舊多載被還原)⇒ 停。';
  END IF;
  IF (SELECT pg_catalog.md5(p.prosrc) FROM pg_catalog.pg_proc p WHERE p.oid = 'public.create_order(jsonb, uuid, text, jsonb, uuid, text, text, text, text, text, text)'::regprocedure) IS DISTINCT FROM '77c7ab9cf4dc26404af6dbbe723a1d42'
     OR (SELECT pg_catalog.md5(p.prosrc) FROM pg_catalog.pg_proc p WHERE p.oid = 'public.get_effective_prices(uuid[], uuid[])'::regprocedure) IS DISTINCT FROM 'c316058adcad20679d7503b8b0967bb2'
     OR v_vfp IS DISTINCT FROM '42ddb5f87a1096361f42a6db13255918' THEN
    RAISE EXCEPTION '前置閘②:三樣不是 E2(20260925040000)那一版 ⇒ E2 還沒貼,或有人改過;停,重新以現役為底。';
  END IF;
  -- 🔴 只比本文擋不住「只用 ALTER FUNCTION 改設定」;CREATE OR REPLACE 會把設定整組換掉 ⇒ 屬性也要一樣(Codex D1 計畫 R1 必修 2)
  IF (SELECT p.prosecdef::text || '|' || pg_catalog.array_to_string(p.proconfig, ',') || '|' || p.provolatile::text || '|' || pg_catalog.pg_get_function_arguments(p.oid)
        FROM pg_catalog.pg_proc p WHERE p.oid = 'public.create_order(jsonb, uuid, text, jsonb, uuid, text, text, text, text, text, text)'::regprocedure)
       IS DISTINCT FROM 'true|search_path=""|v|p_lines jsonb, p_address_id uuid, p_shipping_method text, p_invoice jsonb, p_cart_session_id uuid, p_terms_version text, p_client_ip text, p_client_ua text, p_payment_channel text, p_notification_email text DEFAULT NULL::text, p_coupon_code text DEFAULT NULL::text'
     OR (SELECT p.prosecdef::text || '|' || pg_catalog.array_to_string(p.proconfig, ',') || '|' || p.provolatile::text || '|' || pg_catalog.pg_get_function_arguments(p.oid)
        FROM pg_catalog.pg_proc p WHERE p.oid = 'public.get_effective_prices(uuid[], uuid[])'::regprocedure)
       IS DISTINCT FROM 'true|search_path=""|s|p_product_ids uuid[] DEFAULT NULL::uuid[], p_variant_ids uuid[] DEFAULT NULL::uuid[]' THEN
    RAISE EXCEPTION '前置閘③:create_order / get_effective_prices 的屬性(SECURITY DEFINER、search_path、volatility、參數與預設值)與預期不同 ⇒ 有人用 ALTER 改過,停。';
  END IF;
  IF pg_catalog.to_regprocedure('public.dealer_discounted_amount(uuid, uuid, integer)') IS NULL
     OR pg_catalog.to_regprocedure('public.dealer_discount_apply(integer, numeric)') IS NULL THEN
    RAISE EXCEPTION '前置閘④:找不到 E2 的 dealer_discounted_amount / dealer_discount_apply ⇒ 停。';
  END IF;
END
$gate$;
...
-- 🔴 20260925050000(B2B D1,Sean Q3 甲):缺經銷價(基準款變體 price_store 是 NULL)⇒ 價格 NULL,不再退回一般價。
SELECT
  v.id,
  v.title,
  v.subtitle,
  ...
  v.availability,
  v.fitments,
  -- 🔴 唯一與 products_list_public 不同的一格。基準款變體沒有經銷價 ⇒ ~~退回商品一般價~~ D1(20260925050000)起價格 NULL(不能買)。
  ...
  public.dealer_discount_apply(b.price_store, CASE WHEN dc.user_id IS NOT NULL THEN dd.percent END) AS price_general,
  v.supplier_slug,
  v.card_image,
  v.fits,
  v.brand_name,
...
BEGIN
  ...
  END IF;
  -- 一般價退路真的拿掉了、站別判斷真的接上了
  --   (只看經銷價那幾個呼叫的參數;get_effective_prices 的 WARNING 判準刻意保留 coalesce(v.price_store, v.price_general):它問的是「一般價也壞了嗎」)
  IF pg_catalog.strpos((SELECT p.prosrc FROM pg_catalog.pg_proc p WHERE p.oid = 'public.create_order(jsonb, uuid, text, jsonb, uuid, text, text, text, text, text, text)'::regprocedure), 'v_variant.brand_id, coalesce(') > 0
     OR pg_catalog.strpos((SELECT p.prosrc FROM pg_catalog.pg_proc p WHERE p.oid = 'public.create_order(jsonb, uuid, text, jsonb, uuid, text, text, text, text, text, text)'::regprocedure), 'pcm_wrong_site') = 0
     OR pg_catalog.strpos((SELECT p.prosrc FROM pg_catalog.pg_proc p WHERE p.oid = 'public.get_effective_prices(uuid[], uuid[])'::regprocedure), 'brand_id, coalesce(') > 0
     OR pg_catalog.strpos(pg_catalog.upper(pg_catalog.pg_get_viewdef('public.products_list_dealer'::regclass, true)), 'COALESCE(B.PRICE_STORE') > 0 THEN
    RAISE EXCEPTION '事後閘④:一般價退路沒拿乾淨或站別判斷沒接上 ⇒ 停。';
  END IF;
  IF v_vfp IS DISTINCT FROM 'f45a37de325ecd907c3713401c61d14a'
     OR (SELECT pg_catalog.md5(p.prosrc) FROM pg_catalog.pg_proc p WHERE p.oid = 'public.create_order(jsonb, uuid, text, jsonb, uuid, text, text, text, text, text, text)'::regprocedure) IS DISTINCT FROM '74716d30da57db777cd319ee48652a03'
     OR (SELECT pg_catalog.md5(p.prosrc) FROM pg_catalog.pg_proc p WHERE p.oid = 'public.get_effective_prices(uuid[], uuid[])'::regprocedure) IS DISTINCT FROM 'c40796dbfc8c46a9592ea8ca7bf1701d' THEN
    RAISE EXCEPTION '事後閘⑤:寫出來的三個本體指紋與預期不同 ⇒ 停(退回檔會對不上)。';
  END IF;
END
$post$;
