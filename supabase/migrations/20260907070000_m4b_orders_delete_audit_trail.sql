...
--
-- ══ 🔴🔴 貼這一支【同一批】要有一份 SOP —— 而它已經寫好了 ═══════════════════
--    📎 `docs/runbooks/deleted-order-audit-lookup.md`(副手窗 `B` 2026-09-08 寫)
--
--    🔬 **為什麼補這一行**(讀數,不是提醒):2026-09-08 量到 ——
--       `orders_deleted_log` 在 repo `docs/` 命中 **0 支檔**
--       (🟢 正對照 `sweeper_heartbeat` 32 支 · `order_pending_refunds` 18 支 · ⚪ 負對照現造表名 0)
--       而 `~/pcm-mailbox/` 那 8 支逐支看檔名, **全部是工作紀錄與盤點, 沒有一支是 SOP**。
--       🔴 **本檔自己也沒有寫「誰該來查」** —— 檔內三處 `orders_deleted_log` 的註解
--          講的是 IDENTITY 序列與 `service_role` 不該有 INSERT。
--    🛑 **一個沒有人知道要去查的稽核表, 與沒有稽核表, 在出事那天印同一個東西。**
--    ✅ **Sean 2026-09-08 拍板「做啊」, 而【那一行 SOP 是這個拍板的一半, 不是附加項】**
--       (主視窗 A 給他的選項逐字是「做, 而同一批補一行 SOP」)。
--
--    🔵 那份 SOP 答三句:**什麼時候會想到它** / **去哪裡查** / **查得到什麼、查不到什麼**;
--       其中最重要的一節是「**查到零列的時候先分清楚是哪一種零**」——
--       📌 **「我讀不到」與「它沒有那一列」印同一個東西:什麼都沒有。**
--    ⚠️ **本入口刻意【不】進 `CLAUDE.md` 路由表**:當日量到常載 `CLAUDE.md` = **23,878 字元**
--       (警告線 22,000 已過 / 硬上限 26,000)⇒ 入口改走**本檔頭 + 板列 `⟦db-ORDERDELETENOTRACE⟧`**。
-- ═══════════════════════════════════════════════════════════════════════════
