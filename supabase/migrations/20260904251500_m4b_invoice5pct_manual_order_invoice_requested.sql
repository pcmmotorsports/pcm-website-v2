...
-- ⟦b4-INVOICE5PCT⟧ 第 2 步的後半:**手動建單那顆「這張單要開發票」勾選, 真的落到 DB。**
--
-- 🔴 **這支與前半【成對】, 同一顆 commit** —— 少了它, 前台那顆勾選是**會說謊的欄位**:
--    員工把它取消勾選, 而 `orders.invoice_requested` 仍靠 DEFAULT 落成 `true`
--    ⇒ 🔴 **與他勾的相反**, 而畫面上看不出來。(codex R1 must-fix #1)
--
-- ── 這是 `admin_create_manual_order` 的**第④代** ───────────────────────────
--    ①`20260824020000`(create) ②`20260829140000`(cor, 帳本未記)
--    ③`20260831180000`(cor, 帳本已記 = live)  ④本檔
--    🔴 **本體是從第③代【原樣抄】的**(`20260831180000:100-692`, 用程式切, 不是手打),
--       只動四處, 每一處都在原地留了 `第④代` 註解:
--         ① DECLARE 加 `v_invoice_requested`
--         ② `v_invoice` 組完之後驗 `p_invoice.requested` 並取值(**NULL 拒絕**)
--         ③ 指紋 `v_canonical` 加 `invoice_requested`
--         ④ `INSERT INTO public.orders` 顯式寫入該欄
--    ⇒ 📌 要 diff 的人:`sed -n '100,692p' <第③代> ` 與本檔的函式段比, 差異應**只有那四處**。
--
-- 🛑 **`CREATE OR REPLACE` 是對的, 不是偷懶**:簽名一個字沒動 ⇒ 不會產生第二個 overload;
--    而 `CREATE OR REPLACE` **保留既有 ACL** ⇒ 本檔不再 `GRANT`(下面有一格斷言去看它真的還在)。
--
-- ⚠️ **本檔未 apply。** Sean 手貼;貼完要在 `supabase/APPLIED.tsv` 補一列。
--
-- ── 🔵 **本檔【真的被執行過】——量到的, 不是讀過去的**(2026-09-04 夜, 拋棄式 PG 17)──
--    環境:`initdb` 空庫 + `member_tier` enum + 一張只有 `id / invoice_requested` 的 orders stub。
--    ✅ 前置閘 1 / 前置閘 2 / CREATE OR REPLACE / 事後斷言 1 / 2 / 3 **全部真的跑過**, 最後印 `COMMIT`。
--    ✅ **而每一格都做過「該紅時會不會紅」**:
--       · 前置閘 1 ⇒ 拿掉 orders ⇒ 紅在我寫的那句(⚠️ 這一格**改過**:原本用 `::regclass`,
--         它自己先炸, 我的訊息**永遠印不出來** ⇒ 改 `to_regclass`)
--       · 驗證那道閘 ⇒ 三個世界逐一量:鍵不存在 / 鍵是 JSON null / 鍵是 true
...
--       **那句話錯了**(codex R4 打穿)。錯在**先上碼**那個方向:
--         ① 新畫面讓員工**取消勾選** ② 新碼送出 `requested:false`
--         ③ 舊函式(第③代)**忽略這個鍵** ④ DB 走 DEFAULT 寫成 **`true`**
--       ⇒ 🔴 **那不是「與今天一樣」** —— 今天沒有那顆勾選, 沒有人被承諾過任何事;
--          而那個世界裡**畫面接受了「不開」, 資料庫寫下了「開」**。
--       ⇒ 📌 我當初把「行為與今天相同」與「沒有人被誤導」當成同一件事, 而它們不是。
--
--    ✅ **正確的順序, 而它只有一個**:
--       **1) 貼這支 migration → 2) 部署碼。**
--       · 只貼 migration、碼還沒上 ⇒ 舊碼不送該鍵 ⇒ 缺鍵視同 `true` ⇒ **行為與今天相同**(安全)
--       · 兩邊都到齊    ⇒ 勾選真的生效
--       🛑 **反過來(先上碼)⇒ 上面那條路 ⇒ 會產生與員工選擇相反的資料。**
--    ⚠️ 相容期那個設計**仍然有價值** —— 它讓「先貼 migration」這一步是安全的
--       (否則貼下去舊碼會建單全掛)。它省掉的是「同時」, 不是「順序」。

BEGIN;

-- ── 前置閘 1:那一欄要真的存在, 而且是 boolean ────────────────────────────
--    🔴 沒有它, 下面的 INSERT 會在**執行時**才炸, 而那時已經有人在建單了。
--    🔴🔴 **`to_regclass` 不是 `::regclass`** —— 這是實測改的:
--       `'public.orders'::regclass` 在表不存在時**自己先丟 `relation does not exist`**
--       ⇒ 我下面那句 RAISE **永遠印不出來**, 而人看到的是一句沒有出路的原生錯誤。
--       (2026-09-04 在拋棄式 PG 上餵一次空庫量到的 —— 不是想到的。)
--       `to_regclass` 表不存在時回 `NULL` ⇒ 訊息才輪得到我寫的那一句。
DO $gate$
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_catalog.pg_attribute a
     WHERE a.attrelid = pg_catalog.to_regclass('public.orders')
       AND a.attname = 'invoice_requested'
       AND a.atttypid = 'pg_catalog.bool'::pg_catalog.regtype
       AND NOT a.attisdropped
  ) THEN
    RAISE EXCEPTION '前置閘 1 失敗:public.orders.invoice_requested 不存在或不是 boolean。'
                    '⇒ 先確認建它的那支 migration 已 apply,不要在這裡補建。';
  END IF;
END
$gate$;

-- ── 前置閘 2:現在庫裡**只有一個** admin_create_manual_order, 而且簽名就是我要換的那個 ──
--    🔴 兩個世界要印不同的東西:多一個 overload ⇒ 這裡就會紅, 而不是安靜地換掉其中一個。
DO $gate$
DECLARE
  v_n   integer;
  v_args text;
...
BEGIN
  ...
  IF v_n <> 1 THEN
    RAISE EXCEPTION '前置閘 2 失敗:public.admin_create_manual_order 有 % 個定義(預期 1)。'
                    '⇒ 有 overload 存在時不得 CREATE OR REPLACE,先把多的 DROP 掉。', v_n;
  END IF;
  ...
  SELECT pg_catalog.pg_get_function_identity_arguments(p.oid) INTO v_args
    ...
    JOIN pg_catalog.pg_namespace n ON n.oid = p.pronamespace
   WHERE n.nspname = 'public' AND p.proname = 'admin_create_manual_order';
  IF v_args <> 'p_customer_user_id uuid, p_manual_request_id uuid, p_actor text, '
               'p_order_source text, p_payment_channel text, p_shipping_method text, '
               'p_ship_to jsonb, p_invoice jsonb, p_shipping_fee integer, p_lines jsonb' THEN
    RAISE EXCEPTION '前置閘 2 失敗:簽名不是預期的那一個(實際:%)。'
                    '⇒ 本檔的本體抄自第③代,簽名不同代表庫裡不是第③代,停下來對版。', v_args;
  END IF;

  -- 🔴🔴 **簽名相同【不代表】庫裡是第③代**(codex R2 must-fix)——
  --    第①②③代**簽名一個字都不差** ⇒ 上面那一格對「庫裡其實是第②代」印同一個綠,
  --    而本檔會把它整支覆蓋掉, 順手把第③代帶進來的東西當成「本來就有」。
  --    ⇒ ✅ 改成認**本體裡的一句第③代獨有的字**。
  --    🔵 那句話為什麼是第③代獨有的:機械比過第②代與第③代的**非註解行**,
  --       第③代多出 19 行, 而這一句在其中(權威規格那一段是第③代才加的)。
  --    ⚠️ **射程**:它證明「庫裡有第③代那一段」, **不證明**「庫裡逐字等於第③代」——
  --       中間若有人手貼過一支同簽名的 hotfix 而沒動那一段, 這裡照樣放行。
  --       ⇒ 📌 那是**已知的殘餘風險**, 寫在這裡而不是被靜靜帶過。真要關掉它需要
  --       把第③代的本體雜湊寫死進來, 而那會讓任何合法的空白調整都變成假紅。
  SELECT pg_catalog.pg_get_functiondef(p.oid) INTO v_def
    FROM pg_catalog.pg_proc p
    JOIN pg_catalog.pg_namespace n ON n.oid = p.pronamespace
   WHERE n.nspname = 'public' AND p.proname = 'admin_create_manual_order';
  IF pg_catalog.strpos(v_def, '是 product_variants.spec 那一列要先修好') = 0 THEN
    RAISE EXCEPTION '前置閘 2 失敗:庫裡那一版【不是第③代】(找不到第③代獨有的那一段)。'
                    '⇒ 本檔的本體抄自第③代 20260831180000。庫裡若是第①/②代,'
                    '覆蓋上去會把第③代的權威規格檢查一起帶進來,而那是另一片的東西。'
                    '⇒ 停下來對版,不要當成「反正比較新」。';
  END IF;
END
$gate$;
...
-- ── 事後斷言 1:換完之後仍然**只有一個**定義(證明沒有多長出 overload)────────
DO $post$
DECLARE v_n integer;
BEGIN
  SELECT count(*) INTO v_n
   ...
   WHERE n.nspname = 'public' AND p.proname = 'admin_create_manual_order';
  IF v_n <> 1 THEN
    RAISE EXCEPTION '事後斷言 1 失敗:換完後有 % 個定義。', v_n;
  END IF;
END
$post$;

-- ── 事後斷言 2:新本體裡**真的有**那四處 ─────────────────────────────────
--    🛑 這一格量的是**函式定義的字面**, 不是行為 —— 它擋的是「我以為我改了而 sed 沒命中」。
--    🔵 而它會不會叫得出來:把下面任一個字串改一個字元, 這一格就紅 ⇒ 它不是恆真。
DO $post$
DECLARE
  v_def text;
  v_missing text := '';
BEGIN
  ...
  IF pg_catalog.strpos(v_def, 'v_invoice_requested boolean') = 0 THEN
    v_missing := v_missing || ' [DECLARE]';
  END IF;
  IF pg_catalog.strpos(v_def, 'p_invoice.requested 送了但不是') = 0 THEN
    v_missing := v_missing || ' [驗證]';
  END IF;
  IF pg_catalog.strpos(v_def, '''invoice_requested'', v_invoice_requested') = 0 THEN
    v_missing := v_missing || ' [指紋]';
  END IF;
  IF pg_catalog.strpos(v_def, '        invoice_requested,') = 0 THEN
    v_missing := v_missing || ' [INSERT-欄名]';
  END IF;
  -- 🔴🔴 **欄名與值要【分開驗】**(codex R2 must-fix):只驗欄名的話,
  --    把 VALUES 那一格改成寫死的 `true`, 上面三格**照樣全綠** ⇒ R1 那條原樣復發
  --    (員工取消勾選而單子仍被記成「要開發票」), 而沒有任何東西會叫。
  IF pg_catalog.strpos(v_def, '        v_invoice_requested,') = 0 THEN
    v_missing := v_missing || ' [INSERT-值]';
  END IF;

  IF v_missing <> '' THEN
    RAISE EXCEPTION '事後斷言 2 失敗:新定義裡找不到這幾處 ⇒%。'
                    '🛑 這代表本檔的四處改動有一處【沒有進到庫裡】,而其餘三處已經進去了'
                    '⇒ 那是半套,不要當成「大致成功」。', v_missing;
  END IF;
END
$post$;

-- ── 事後斷言 3:ACL 沒被 CREATE OR REPLACE 弄掉 ──────────────────────────
--    🔴 `service_role` 要還有 EXECUTE(它是唯一呼叫端);`anon` / `authenticated` **不得有**。
--    ⚠️ 射程:`has_function_privilege` 對「透過 PUBLIC 拿到的」也回 true
--       ⇒ 下面第二格另外去看 `proacl` 裡有沒有 PUBLIC 那一列(grantee = 0)。
DO $post$
DECLARE
  v_oid oid;
  v_acl aclitem[];
BEGIN
  ...
  IF NOT pg_catalog.has_function_privilege('service_role', v_oid, 'EXECUTE') THEN
    RAISE EXCEPTION '事後斷言 3 失敗:service_role 失去了 EXECUTE ⇒ 後台建單會全掛。';
  END IF;

  IF v_acl IS NULL THEN
    RAISE EXCEPTION '事後斷言 3 失敗:proacl 是 NULL ⇒ 那代表【誰都看得到】(PUBLIC 預設),'
                    '而本函式是 SECURITY DEFINER。停下來,不要當成「還沒設」。';
  END IF;

  IF EXISTS (SELECT 1 FROM pg_catalog.aclexplode(v_acl) x WHERE x.grantee = 0) THEN
    RAISE EXCEPTION '事後斷言 3 失敗:proacl 裡有 PUBLIC(grantee = 0)。';
  END IF;
  ...
  IF pg_catalog.has_function_privilege('anon', v_oid, 'EXECUTE')
    ...
    RAISE EXCEPTION '事後斷言 3 失敗:anon / authenticated 拿得到 EXECUTE。';
  END IF;
END
$post$;
