...
-- 🔴🔴 **那句話【執行不了】**(codex 2026-09-06 must-fix ④, 我開檔核過):
--    `20260906620000:146` 逐字是 **`CREATE FUNCTION`**, 不是 `CREATE OR REPLACE`
--    ⇒ 🛑 重貼會因**同簽章函式已存在**而失敗, 而**新述詞仍然生效**
--    ⇒ 📌 **一個【看起來有】而其實跑不動的回退路徑, 比明說沒有更糟** ——
--      災難當下照著做的人會以為自己退回去了。
-- ✅ **真正跑得動的回退**(要退的時候原地貼這一段, 它與本檔上面那支只差【沒有那條新述詞】):
--    把本檔的 `CREATE OR REPLACE FUNCTION … $fn$ … $fn$;` 整段複製出來,
--    **刪掉兩處 `AND NOT EXISTS ( … admin_audit_log … )`**(pending 與 oldest 各一處), 再貼。
--    🔵 那樣仍然是 `CREATE OR REPLACE` ⇒ 貼得上去;而收權與 COMMENT 照本檔那幾行一起帶。
--    ⚠️ **退回去之後**:已經按過「已電話通知」的那些單會**重新出現在提醒裡**
--      (稽核那一筆還在, 只是沒有人讀它了)—— 那是預期的, 不是資料壞掉。
