-- 20260926100000 · 後台刪除會員與停用會員(第一支:欄位、權限、客戶列表、五支既有函式的停用檢查、4 支新函式)
--
-- 計畫:~/pcm-mailbox/計畫-後台刪除停用會員-20260926.md 第 8 版(Sean 2026-09-26 Q22 甲批准)。
--   Sean 決定:沒有訂單、儲值金、退款、經銷紀錄的帳號可以刪除;有紀錄的只能停用(不能登入、不出現在會員列表,
--   訂單與帳務保留);只有老闆(staff.is_manager)能操作;每次操作寫 admin_audit_log;刪除時稽核留 email 與姓名。
-- 這一支做的事:
--   ① customers 加 disabled_at / disabled_by / disabled_reason / disabled_version;會員可讀自己的 disabled_at
--   ② admin_customer_list_v 最後加 disabled_at(前 12 欄原樣)
--   ③ admin_search_customers 加 p_status(預設 'active');先 DROP 舊簽章, 只留一版
--   ④ create_order、dealer_application_submit、dealer_application_update_mine、admin_create_manual_order 在鎖住會員列後擋停用;
--      admin_dealer_application_decide 改成先鎖會員列再鎖申請列, 停用的會員不能核准
--   ⑤ 新函式:admin_customer_delete_eligibility、admin_disable_customer、admin_enable_customer、admin_delete_customer
-- 🔴 前置:正式庫這幾支必須是 2026-09-26 唯讀取出的那一版(前置閘比 md5(prosrc))。
-- 🔴 順序:本支 → 20260926100100(儲值金流水外鍵改擋下)→ 20260926100200(驗證外鍵);三支都貼完才推程式。
-- 🔴 退回:supabase/rollbacks/20260926100000-rollback.sql;**網站程式先退回並部署完成才能退這支**
--    (顧客站還在讀 disabled_at 時收回權限或刪欄位, 全站會登不進去)。
-- 只寫檔, 由 Sean 貼。

BEGIN;

-- 加欄位與改 view 要拿 customers 的最高等級鎖, 而每個登入請求都讀這張表 ⇒ 等不到鎖就整支失敗, 挑離峰重貼。
SET LOCAL lock_timeout = '5s';
SET LOCAL statement_timeout = '60s';
SET LOCAL transaction_timeout = '90s';

-- ── 0. 前置閘:正式庫必須是這支檔案依據的那一版 ──
DO $gate$
DECLARE
...
BEGIN
  FOR r IN SELECT * FROM (VALUES
      ('public.create_order(jsonb, uuid, text, jsonb, uuid, text, text, text, text, text, text)', '74716d30da57db777cd319ee48652a03'),
      ('public.dealer_application_submit(text, text, text, text, text, text, text, text)', 'e9578fa9e96290f498bfbb29bbcfaea6'),
      ('public.dealer_application_update_mine(uuid, text, text, text, text, text, text, text, text)', '784b0badc796a092423aeb20789d13d6'),
      ('public.admin_dealer_application_decide(uuid, text, text, text, text, text, timestamp with time zone)', 'd97f50e5de5bcbd9d56f44477f3219ee'),
      ('public.admin_create_manual_order(uuid, uuid, text, text, text, text, jsonb, jsonb, integer, jsonb, text, text, jsonb)', 'c280e7e6f35660fc454e1754833c3981'),
      ('public.admin_search_customers(text, integer)', '77a953d8734500dc56f6b914051b638b')
    ) AS t(sig, fp)
  LOOP
    IF pg_catalog.to_regprocedure(r.sig) IS NULL THEN
      RAISE EXCEPTION '前置閘:找不到 %(正式庫不是本檔依據的版本)⇒ 停。', r.sig;
    END IF;
    IF (SELECT pg_catalog.md5(p.prosrc) FROM pg_catalog.pg_proc p WHERE p.oid = pg_catalog.to_regprocedure(r.sig)) IS DISTINCT FROM r.fp THEN
      RAISE EXCEPTION '前置閘:% 的內容和本檔依據的版本不同(有人改過)⇒ 停, 重新取定義再改。', r.sig;
    END IF;
  END LOOP;
  FOR r IN SELECT unnest(ARRAY['create_order', 'dealer_application_submit', 'dealer_application_update_mine',
                               'admin_dealer_application_decide', 'admin_create_manual_order', 'admin_search_customers']) AS n
  LOOP
    IF (SELECT pg_catalog.count(*) FROM pg_catalog.pg_proc p WHERE p.proname = r.n AND p.pronamespace = 'public'::regnamespace) <> 1 THEN
      RAISE EXCEPTION '前置閘:% 不是剛好一支(可能有舊多載)⇒ 停。', r.n;
    END IF;
  END LOOP;
  PERFORM pg_catalog.set_config('search_path', '', true);
  v_vfp := pg_catalog.md5(pg_catalog.pg_get_viewdef('public.admin_customer_list_v'::regclass, true));
  PERFORM pg_catalog.set_config('search_path', v_sp, true);
  IF v_vfp IS DISTINCT FROM '52a26f8ee89b33789c1eeb0942bcdf19' THEN
    RAISE EXCEPTION '前置閘:admin_customer_list_v 不是 20260901010000 那一版(12 欄)⇒ 停。';
  END IF;
  IF EXISTS (SELECT 1 FROM pg_catalog.pg_attribute a WHERE a.attrelid = 'public.customers'::regclass
               AND a.attname IN ('disabled_at', 'disabled_by', 'disabled_reason', 'disabled_version') AND NOT a.attisdropped) THEN
    RAISE EXCEPTION '前置閘:customers 已經有停用欄位(這支可能已經貼過)⇒ 停。';
  END IF;
  IF pg_catalog.to_regproc('public.admin_disable_customer') IS NOT NULL THEN
    RAISE EXCEPTION '前置閘:admin_disable_customer 已存在(這支可能已經貼過)⇒ 停。';
  END IF;
END
$gate$;

-- ── 1. 欄位 ──
-- 加可為空的欄與「NOT NULL DEFAULT 常數」的欄只改目錄, 不重寫整張表。
-- disabled_version:停用與恢復成功時各加 1, 永遠不歸零。後台送出時帶畫面載入時的版本號,
--   不同就回 STALE(Codex 加審必修:用 disabled_at 比對擋不住「停用 → 恢復 → 舊停用請求重送」)。
ALTER TABLE public.customers
  ADD COLUMN disabled_at      timestamptz,
  ADD COLUMN disabled_by      text,
  ADD COLUMN disabled_reason  text,
  ADD COLUMN disabled_version integer NOT NULL DEFAULT 0;

COMMENT ON COLUMN public.customers.disabled_at IS
  '停用時間(20260926100000)。有值 = 已停用:不能登入、不能建單或申請經銷、預設不出現在後台會員列表。只能經 admin_disable_customer / admin_enable_customer 改。會員可讀自己的這一欄(網站登入與每次請求的檢查要用)。';
COMMENT ON COLUMN public.customers.disabled_by IS
  '停用的員工 staff.id(20260926100000)。只給後台看, 會員讀不到。';
COMMENT ON COLUMN public.customers.disabled_reason IS
  '停用原因(20260926100000)。只給後台看, 會員讀不到。';
COMMENT ON COLUMN public.customers.disabled_version IS
  '停用狀態版本號(20260926100000)。停用與恢復成功時各加 1、永遠不歸零;後台送出時帶畫面上的版本號, 不同就回 STALE。';

-- ── 2. 會員讀自己的 disabled_at ──
-- 會員讀自己那一列只能讀逐欄開放的欄位(20260914040000:76-78);網站用會員自己的連線讀等級(site-access.ts),
--   沒開這一欄的話, 多讀 disabled_at 會被拒絕, 全站會被判成「查不到」而登不進去。
-- ACL-GATE-EXEMPT: public.customers -- 網站用會員本人的連線判斷帳號是否停用, 只開 disabled_at 一欄(20260926100000, Sean 2026-09-26 Q22 甲批准計畫)
GRANT SELECT (disabled_at) ON public.customers TO authenticated;

-- ── 3. 客戶列表 view:前 12 欄原樣(20260901010000:157), 最後加 disabled_at ──
...
SELECT
  ...
  c.gender                                              AS gender,
  -- 20260926100000 新欄:列表「狀態」篩選用(預設只看正常的會員)
...
COMMENT ON COLUMN public.admin_customer_list_v.disabled_at IS
  '停用時間(20260926100000)。有值 = 已停用;後台客戶列表預設只列 disabled_at IS NULL 的會員。';

-- ── 4. 關鍵字搜尋加狀態參數 ──
-- 只用 CREATE OR REPLACE 加有預設值的參數會並存兩版, PostgREST 報 PGRST203, 兩個呼叫端同時壞掉 ⇒ 先 DROP 舊簽章。
-- DROP 之後舊呼叫端不帶 p_status 仍能用(預設 'active' ⇒ 行為變成排除已停用)。
DROP FUNCTION public.admin_search_customers(text, integer);

CREATE FUNCTION public.admin_search_customers(p_query text, p_limit integer DEFAULT 100, p_status text DEFAULT 'active'::text)
 RETURNS jsonb
 LANGUAGE plpgsql
...
COMMENT ON FUNCTION public.admin_search_customers(text, integer, text) IS
  'M-4b #525:後台客戶搜尋。**三維度**(會員姓名 / Email / 會員電話),回 {"ids": uuid[], "truncated": bool}，ids = customers.user_id（該表 PK）。'
  '20260926100000 加 p_status:active(預設,排除已停用)/ disabled(只列已停用)/ all;不認得的值回空清單。手動建單挑客人傳 active,客戶列表依篩選傳。'
...
-- ── 5. 五支既有函式加停用檢查(以 2026-09-26 正式庫現行定義為底, 只改停用檢查與鎖的順序;簽章與權限不變)──
-- 改動前的定義存在 supabase/rollbacks/20260926100000-rollback.sql(退回時照原樣重建)。
...
BEGIN
  ...
  END IF;
  -- 20260926100000:先鎖會員列、再碰申請列(與 admin_dealer_application_decide 同一個順序, 避免死結);
  --   FOR SHARE 與 admin_disable_customer 的 FOR UPDATE 互斥。停用的會員不能申請或修改申請。
  SELECT c.disabled_at INTO v_disabled_at FROM public.customers c WHERE c.user_id = v_uid FOR SHARE;
  IF NOT FOUND THEN
    RAISE EXCEPTION 'dealer_application_submit:查無會員資料' USING ERRCODE = '28000';
  END IF;
  IF v_disabled_at IS NOT NULL THEN
    RAISE EXCEPTION 'dealer_application_submit:pcm_customer_disabled(帳號已停用)' USING ERRCODE = '42501';
  END IF;
...
BEGIN
  ...
  SELECT c.disabled_at INTO v_disabled_at FROM public.customers c WHERE c.user_id = v_uid FOR SHARE;
  IF NOT FOUND THEN
    RAISE EXCEPTION 'dealer_application_update_mine:查無會員資料' USING ERRCODE = '28000';
  END IF;
  IF v_disabled_at IS NOT NULL THEN
    RAISE EXCEPTION 'dealer_application_update_mine:pcm_customer_disabled(帳號已停用)' USING ERRCODE = '42501';
  END IF;
...
BEGIN
  ...
  -- 20260926100000:鎖的順序一律「先會員列、再申請列」(與 dealer_application_submit / _update_mine 相同;
  --   原本先鎖申請列再鎖會員列, 兩邊同時進行會互相等待而被資料庫中止)。
  --   ① 不加鎖讀出申請屬於哪位會員 ② 鎖會員列(保留 FOR UPDATE:核准要改等級)
  --   ③ 鎖申請列 ④ 鎖住後重新確認 ⑤ 核准才改等級。
  SELECT a.user_id INTO v_uid FROM public.dealer_applications a WHERE a.id = p_application_id;
  IF NOT FOUND THEN
    RETURN 'NOT_FOUND';
  END IF;
  -- 婉拒也讀等級, 稽核紀錄的快照才是真的(Codex R1:原本婉拒寫成 tier=null)
  SELECT c.tier::text, c.disabled_at INTO v_tier, v_disabled_at FROM public.customers c WHERE c.user_id = v_uid FOR UPDATE;
  IF NOT FOUND THEN
    RETURN 'NOT_FOUND';
  END IF;
  ...
  IF NOT FOUND THEN
    RETURN 'NOT_FOUND';
  END IF;
  IF v_app.user_id IS DISTINCT FROM v_uid THEN
    RETURN 'STALE';
  END IF;
  ...
  IF p_decision = 'approve' THEN
    IF p_expected_tier IS DISTINCT FROM v_tier THEN
      RETURN 'STALE';
    END IF;
    -- 20260926100000:停用的會員不能核准(婉拒照常可以)。後台顯示「這位會員已停用，請先恢復再審核。」
    IF v_disabled_at IS NOT NULL THEN
...
-- ── 6. 刪除條件(內部用;三支後台函式共用同一份清單)──
-- 會參照會員的表共 11 張(計畫第三節);這裡列「有就不能刪」的那些, 加上 customers 自己的儲值金與等級。
-- 地址、車款、收藏、LINE 綁定不算紀錄, 隨帳號一起刪。清單會過期 ⇒ scripts/customer-disable-delete-migration.test.ts 比對外鍵。
CREATE FUNCTION public.pcm_customer_delete_blockers(p_customer_user_id uuid)
RETURNS text[]
LANGUAGE sql
STABLE
SET search_path = ''
AS $fn$
  SELECT pg_catalog.array_remove(ARRAY[
    CASE WHEN EXISTS (SELECT 1 FROM public.orders t WHERE t.customer_user_id = p_customer_user_id) THEN 'orders' END,
    CASE WHEN EXISTS (SELECT 1 FROM public.shipments t WHERE t.customer_user_id = p_customer_user_id) THEN 'shipments' END,
    CASE WHEN EXISTS (SELECT 1 FROM public.coupon_redemptions t WHERE t.user_id = p_customer_user_id) THEN 'coupon_redemptions' END,
    CASE WHEN EXISTS (SELECT 1 FROM public.customer_wallet_ledger t WHERE t.customer_user_id = p_customer_user_id) THEN 'wallet_ledger' END,
    CASE WHEN EXISTS (SELECT 1 FROM public.payment_charge_attempts t WHERE t.customer_user_id = p_customer_user_id) THEN 'payment_charge_attempts' END,
    CASE WHEN EXISTS (SELECT 1 FROM public.payment_double_charge_anomalies t WHERE t.user_id = p_customer_user_id) THEN 'payment_double_charge_anomalies' END,
    CASE WHEN EXISTS (SELECT 1 FROM public.dealer_applications t WHERE t.user_id = p_customer_user_id) THEN 'dealer_applications' END,
    CASE WHEN EXISTS (SELECT 1 FROM public.dealer_brand_discounts t WHERE t.customer_user_id = p_customer_user_id) THEN 'dealer_brand_discounts' END,
    CASE WHEN EXISTS (SELECT 1 FROM public.customers t WHERE t.user_id = p_customer_user_id
                        AND (t.wallet_balance <> 0 OR t.total_deposit <> 0)) THEN 'wallet_balance' END,
    CASE WHEN EXISTS (SELECT 1 FROM public.customers t WHERE t.user_id = p_customer_user_id
                        AND t.tier <> 'general'::public.member_tier) THEN 'dealer_tier' END
  ], NULL)
$fn$;

REVOKE ALL ON FUNCTION public.pcm_customer_delete_blockers(uuid) FROM PUBLIC;
REVOKE ALL ON FUNCTION public.pcm_customer_delete_blockers(uuid) FROM anon, authenticated, service_role;

COMMENT ON FUNCTION public.pcm_customer_delete_blockers(uuid) IS
  '會員不能刪除的原因清單(20260926100000),空陣列 = 可以刪。代碼:orders / shipments / coupon_redemptions / wallet_ledger / payment_charge_attempts / payment_double_charge_anomalies / dealer_applications / dealer_brand_discounts / wallet_balance / dealer_tier。只給 admin_customer_delete_eligibility 與 admin_delete_customer 內部呼叫, 沒有任何角色可直接執行。';

-- ── 7. 查可否刪除(唯讀, 給後台詳情頁顯示原因)──
CREATE FUNCTION public.admin_customer_delete_eligibility(p_customer_user_id uuid)
RETURNS jsonb
LANGUAGE plpgsql
STABLE
SECURITY DEFINER
...
BEGIN
  IF p_customer_user_id IS NULL THEN
    RAISE EXCEPTION 'admin_customer_delete_eligibility: 缺 customer_user_id';
  END IF;
  PERFORM 1 FROM public.customers c WHERE c.user_id = p_customer_user_id;
  IF NOT FOUND THEN
    RETURN pg_catalog.jsonb_build_object('found', false, 'deletable', false, 'reasons', '[]'::jsonb);
  END IF;
  v_blockers := public.pcm_customer_delete_blockers(p_customer_user_id);
  RETURN pg_catalog.jsonb_build_object(
    'found', true,
    'deletable', pg_catalog.cardinality(v_blockers) = 0,
    'reasons', pg_catalog.to_jsonb(v_blockers));
END;
$fn$;

-- ── 8. 停用 ──
CREATE FUNCTION public.admin_disable_customer(
  p_actor text, p_customer_user_id uuid, p_expected_version integer, p_reason text, p_request_id text)
RETURNS text
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = ''
AS $fn$
DECLARE
  -- 空白字元集照抄 admin_set_customer_tier(20260914130000):Unicode White_Space 全集 + 零寬字元, 共 31 個。
  v_ws constant text := E' \t\r\n\f\013' || U&'\0085' || U&'\00A0' || U&'\1680' || U&'\180E'
    || U&'\2000' || U&'\2001' || U&'\2002' || U&'\2003' || U&'\2004' || U&'\2005' || U&'\2006'
    || U&'\2007' || U&'\2008' || U&'\2009' || U&'\200A' || U&'\200B' || U&'\200C' || U&'\200D'
    || U&'\2028' || U&'\2029' || U&'\202F' || U&'\205F' || U&'\2060' || U&'\3000' || U&'\FEFF';
  v_actor       text := pg_catalog.btrim(coalesce(p_actor, ''), v_ws);
  v_request_id  text := pg_catalog.btrim(coalesce(p_request_id, ''), v_ws);
  v_reason      text := pg_catalog.btrim(coalesce(p_reason, ''), v_ws);
  v_is_manager  boolean;
  v_disabled_at timestamptz;
  v_version     integer;
BEGIN
  IF pg_catalog.char_length(v_ws) <> 31 THEN
    RAISE EXCEPTION 'admin_disable_customer: v_ws 字元集長度異常(預期 31)';
  END IF;
  IF v_actor = '' OR v_request_id = '' OR p_customer_user_id IS NULL OR p_expected_version IS NULL THEN
    RAISE EXCEPTION 'admin_disable_customer: 參數不正確';
  END IF;
  IF v_reason = '' THEN
    RAISE EXCEPTION 'admin_disable_customer: 停用原因必填';
  END IF;
  IF pg_catalog.char_length(v_reason) > 200 OR v_reason ~ '[[:cntrl:]]' THEN
    RAISE EXCEPTION 'admin_disable_customer: 停用原因非法';
  END IF;
  -- 只有在職的老闆可以停用(伺服器端已先查一次, 這裡是資料庫端的第二道)
  SELECT s.is_manager INTO v_is_manager FROM public.staff s WHERE s.id = v_actor AND s.is_active FOR SHARE;
  IF NOT FOUND OR NOT coalesce(v_is_manager, false) THEN
    RAISE EXCEPTION '無權執行此操作';
  END IF;

  -- 鎖會員列:與建單、經銷申請、手動建單的 FOR SHARE 互斥 ⇒ 停用提交前後兩種世界只會有一種
  SELECT c.disabled_at, c.disabled_version INTO v_disabled_at, v_version
    FROM public.customers c WHERE c.user_id = p_customer_user_id FOR UPDATE;
  IF NOT FOUND THEN
    RETURN 'NOT_FOUND';
  END IF;
  -- 同一個請求重送(同一個 request_id 已經停用過這位會員)⇒ 已經做完, 不重做
  IF EXISTS (SELECT 1 FROM public.admin_audit_log a
              WHERE a.request_id = v_request_id AND a.action = 'customer.disable'
                AND a.target = 'customer:' || p_customer_user_id::text) THEN
    RETURN 'NO_CHANGE';
  END IF;
  -- 畫面載入之後有人停用或恢復過 ⇒ 不改, 請員工重新整理再決定
  ...
  UPDATE public.customers
     SET disabled_at = pg_catalog.now(), disabled_by = v_actor, disabled_reason = v_reason,
         disabled_version = v_version + 1
   WHERE user_id = p_customer_user_id;

  INSERT INTO public.admin_audit_log (actor, action, target, before, after, reason, request_id, source_app)
  VALUES (v_actor, 'customer.disable', 'customer:' || p_customer_user_id::text,
          pg_catalog.jsonb_build_object('disabled', false, 'disabled_version', v_version),
          pg_catalog.jsonb_build_object('disabled', true, 'disabled_version', v_version + 1),
          v_reason, v_request_id, 'admin');

  -- 刪掉登入工作階段 ⇒ 刷新權杖跟著刪(auth.refresh_tokens → auth.sessions ON DELETE CASCADE, 2026-09-26 正式庫查證)
  --   ⇒ 這位會員拿不到新的存取權杖;已發出的存取權杖到期前(預設 1 小時)仍有效, 見計畫第五節「已知上限」。
  DELETE FROM auth.sessions s WHERE s.user_id = p_customer_user_id;
...
-- ── 9. 恢復 ──
CREATE FUNCTION public.admin_enable_customer(
  p_actor text, p_customer_user_id uuid, p_expected_version integer, p_reason text, p_request_id text)
RETURNS text
LANGUAGE plpgsql
SECURITY DEFINER
...
AS $fn$
DECLARE
  -- 空白字元集照抄 admin_set_customer_tier(20260914130000)
  v_ws constant text := E' \t\r\n\f\013' || U&'\0085' || U&'\00A0' || U&'\1680' || U&'\180E'
    || U&'\2000' || U&'\2001' || U&'\2002' || U&'\2003' || U&'\2004' || U&'\2005' || U&'\2006'
    || U&'\2007' || U&'\2008' || U&'\2009' || U&'\200A' || U&'\200B' || U&'\200C' || U&'\200D'
    || U&'\2028' || U&'\2029' || U&'\202F' || U&'\205F' || U&'\2060' || U&'\3000' || U&'\FEFF';
...
BEGIN
  IF pg_catalog.char_length(v_ws) <> 31 THEN
    RAISE EXCEPTION 'admin_enable_customer: v_ws 字元集長度異常(預期 31)';
  END IF;
  IF v_actor = '' OR v_request_id = '' OR p_customer_user_id IS NULL OR p_expected_version IS NULL THEN
    RAISE EXCEPTION 'admin_enable_customer: 參數不正確';
  END IF;
  IF v_reason = '' THEN
    RAISE EXCEPTION 'admin_enable_customer: 恢復原因必填';
  END IF;
  IF pg_catalog.char_length(v_reason) > 200 OR v_reason ~ '[[:cntrl:]]' THEN
    RAISE EXCEPTION 'admin_enable_customer: 恢復原因非法';
  END IF;
  SELECT s.is_manager INTO v_is_manager FROM public.staff s WHERE s.id = v_actor AND s.is_active FOR SHARE;
  IF NOT FOUND OR NOT coalesce(v_is_manager, false) THEN
    RAISE EXCEPTION '無權執行此操作';
  ...
  SELECT c.disabled_at, c.disabled_by, c.disabled_reason, c.disabled_version INTO v_row
    FROM public.customers c WHERE c.user_id = p_customer_user_id FOR UPDATE;
  IF NOT FOUND THEN
    RETURN 'NOT_FOUND';
  END IF;
  IF EXISTS (SELECT 1 FROM public.admin_audit_log a
              WHERE a.request_id = v_request_id AND a.action = 'customer.enable'
                AND a.target = 'customer:' || p_customer_user_id::text) THEN
    RETURN 'NO_CHANGE';
  END IF;
  IF v_row.disabled_version <> p_expected_version THEN
    RETURN 'STALE';
  END IF;
  IF v_row.disabled_at IS NULL THEN
    RETURN 'NO_CHANGE';
  END IF;

  UPDATE public.customers
     SET disabled_at = NULL, disabled_by = NULL, disabled_reason = NULL,
         disabled_version = v_row.disabled_version + 1
   WHERE user_id = p_customer_user_id;

  INSERT INTO public.admin_audit_log (actor, action, target, before, after, reason, request_id, source_app)
  VALUES (v_actor, 'customer.enable', 'customer:' || p_customer_user_id::text,
          pg_catalog.jsonb_build_object('disabled', true, 'disabled_version', v_row.disabled_version,
            'disabled_at', v_row.disabled_at, 'disabled_by', v_row.disabled_by, 'disabled_reason', v_row.disabled_reason),
          pg_catalog.jsonb_build_object('disabled', false, 'disabled_version', v_row.disabled_version + 1),
          v_reason, v_request_id, 'admin');
...
-- ── 10. 刪除 ──
CREATE FUNCTION public.admin_delete_customer(
  p_actor text, p_customer_user_id uuid, p_reason text, p_request_id text)
RETURNS text
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = ''
...
BEGIN
  IF pg_catalog.char_length(v_ws) <> 31 THEN
    RAISE EXCEPTION 'admin_delete_customer: v_ws 字元集長度異常(預期 31)';
  END IF;
  IF v_actor = '' OR v_request_id = '' OR p_customer_user_id IS NULL THEN
    RAISE EXCEPTION 'admin_delete_customer: 參數不正確';
  END IF;
  IF v_reason = '' THEN
    RAISE EXCEPTION 'admin_delete_customer: 刪除原因必填';
  END IF;
  IF pg_catalog.char_length(v_reason) > 200 OR v_reason ~ '[[:cntrl:]]' THEN
    RAISE EXCEPTION 'admin_delete_customer: 刪除原因非法';
  END IF;
  SELECT s.is_manager INTO v_is_manager FROM public.staff s WHERE s.id = v_actor AND s.is_active FOR SHARE;
  IF NOT FOUND OR NOT coalesce(v_is_manager, false) THEN
    RAISE EXCEPTION '無權執行此操作';
  ...
  -- 同一個請求重送:已經刪掉了 ⇒ 回 DELETED, 不回「查無此會員」讓員工誤會沒刪成
  IF EXISTS (SELECT 1 FROM public.admin_audit_log a
              WHERE a.request_id = v_request_id AND a.action = 'customer.delete'
                AND a.target = 'customer:' || p_customer_user_id::text) THEN
    RETURN 'DELETED';
  END IF;

  -- 🔴 一定要 FOR UPDATE(不能 FOR NO KEY UPDATE):新增訂單、儲值金流水等的外鍵檢查對會員列取 KEY SHARE,
  --   只有 FOR UPDATE 會擋住它們 ⇒ 「按下去的那一刻剛好有人下單」只會有一邊成功。
  SELECT c.email, c.name, c.tier::text AS tier, c.created_at INTO v_row
    FROM public.customers c WHERE c.user_id = p_customer_user_id FOR UPDATE;
  IF NOT FOUND THEN
    RETURN 'NOT_FOUND';
  END IF;
  -- 鎖住之後重新檢查一次(鎖之前完成的寫入, 這一句都看得到)
  IF pg_catalog.cardinality(public.pcm_customer_delete_blockers(p_customer_user_id)) > 0 THEN
    RETURN 'HAS_RECORDS';
  END IF;

  -- 稽核留 email 與姓名(Sean Q21 甲):刪除後無法從別處查回這位會員是誰
  INSERT INTO public.admin_audit_log (actor, action, target, before, after, reason, request_id, source_app)
  VALUES (v_actor, 'customer.delete', 'customer:' || p_customer_user_id::text,
          pg_catalog.jsonb_build_object('email', v_row.email, 'name', v_row.name, 'tier', v_row.tier,
                                        'created_at', v_row.created_at),
          NULL, v_reason, v_request_id, 'admin');

  -- 刪登入帳號 ⇒ customers、地址、車款、收藏、登入身分、工作階段依外鍵 CASCADE 一起刪;
  --   若仍有「擋下」類紀錄(訂單、出貨、優惠券、儲值金流水)⇒ 外鍵擋下, 整筆交易回滾(稽核也一起回滾)。
  DELETE FROM auth.users u WHERE u.id = p_customer_user_id;
  GET DIAGNOSTICS v_n = ROW_COUNT;
  IF v_n <> 1 THEN
    RAISE EXCEPTION 'admin_delete_customer: 登入帳號刪除筆數是 %(預期 1)⇒ 整筆回滾', v_n;
  END IF;
...
-- ── 11. 權限:四支新函式只給 service_role(後台伺服器)──
REVOKE ALL ON FUNCTION public.admin_customer_delete_eligibility(uuid) FROM PUBLIC;
REVOKE ALL ON FUNCTION public.admin_customer_delete_eligibility(uuid) FROM anon, authenticated;
GRANT EXECUTE ON FUNCTION public.admin_customer_delete_eligibility(uuid) TO service_role;
REVOKE ALL ON FUNCTION public.admin_disable_customer(text, uuid, integer, text, text) FROM PUBLIC;
REVOKE ALL ON FUNCTION public.admin_disable_customer(text, uuid, integer, text, text) FROM anon, authenticated;
GRANT EXECUTE ON FUNCTION public.admin_disable_customer(text, uuid, integer, text, text) TO service_role;
REVOKE ALL ON FUNCTION public.admin_enable_customer(text, uuid, integer, text, text) FROM PUBLIC;
REVOKE ALL ON FUNCTION public.admin_enable_customer(text, uuid, integer, text, text) FROM anon, authenticated;
GRANT EXECUTE ON FUNCTION public.admin_enable_customer(text, uuid, integer, text, text) TO service_role;
REVOKE ALL ON FUNCTION public.admin_delete_customer(text, uuid, text, text) FROM PUBLIC;
REVOKE ALL ON FUNCTION public.admin_delete_customer(text, uuid, text, text) FROM anon, authenticated;
GRANT EXECUTE ON FUNCTION public.admin_delete_customer(text, uuid, text, text) TO service_role;

COMMENT ON FUNCTION public.admin_customer_delete_eligibility(uuid) IS
  '後台詳情頁用:這位會員可不可以刪除(20260926100000)。回 {found, deletable, reasons[]},原因代碼見 pcm_customer_delete_blockers。唯讀。僅 service_role 可執行。';
COMMENT ON FUNCTION public.admin_disable_customer(text, uuid, integer, text, text) IS
  '停用會員(20260926100000)。只有在職老闆;原因必填 ≤200 字。回 OK / NO_CHANGE(已停用, 或同一個 request_id 重送)/ STALE(版本號和畫面不同)/ NOT_FOUND。同一筆交易:鎖會員列、寫欄位、版本號 +1、寫稽核 customer.disable、刪 auth.sessions。僅 service_role 可執行。';
COMMENT ON FUNCTION public.admin_enable_customer(text, uuid, integer, text, text) IS
  '恢復會員(20260926100000)。只有在職老闆;原因必填 ≤200 字。回 OK / NO_CHANGE / STALE / NOT_FOUND。同一筆交易:鎖會員列、清欄位、版本號 +1、寫稽核 customer.enable(before 留停用時間、停用者、停用原因)。僅 service_role 可執行。';
COMMENT ON FUNCTION public.admin_delete_customer(text, uuid, text, text) IS
  '刪除會員(20260926100000)。只有在職老闆;原因必填 ≤200 字;有任何紀錄(pcm_customer_delete_blockers)就回 HAS_RECORDS 不刪。回 DELETED(含同一個 request_id 重送)/ HAS_RECORDS / NOT_FOUND。同一筆交易:鎖會員列、重新檢查、寫稽核 customer.delete(留 email、姓名、等級、建立時間)、刪 auth.users(其餘依外鍵 CASCADE)。無法復原。僅 service_role 可執行。';
...
DECLARE
  r record;
  v_sig text;
  -- 本支新建的可授權物件(scripts/migration-static-checks.sh 規則③:清單要列齊)
  v_functions text[] := ARRAY['public.admin_customer_delete_eligibility(uuid)', 'public.admin_disable_customer(text, uuid, integer, text, text)', 'public.admin_enable_customer(text, uuid, integer, text, text)', 'public.admin_delete_customer(text, uuid, text, text)', 'public.admin_search_customers(text, integer, text)', 'public.pcm_customer_delete_blockers(uuid)']::text[];
BEGIN
  -- 欄位與權限:會員只讀得到 disabled_at, 讀不到停用者與原因
  IF NOT pg_catalog.has_column_privilege('authenticated', 'public.customers', 'disabled_at', 'SELECT') THEN
    RAISE EXCEPTION '事後閘:authenticated 讀不到 customers.disabled_at';
  END IF;
  IF pg_catalog.has_column_privilege('authenticated', 'public.customers', 'disabled_by', 'SELECT')
     OR pg_catalog.has_column_privilege('authenticated', 'public.customers', 'disabled_reason', 'SELECT')
     OR pg_catalog.has_column_privilege('authenticated', 'public.customers', 'disabled_version', 'SELECT') THEN
    RAISE EXCEPTION '事後閘:authenticated 讀得到停用者、原因或版本號(不應開放)';
  END IF;
  IF pg_catalog.has_column_privilege('authenticated', 'public.customers', 'disabled_at', 'UPDATE') THEN
    RAISE EXCEPTION '事後閘:authenticated 可以改 disabled_at';
  END IF;
  -- view 仍是 security_invoker、13 欄、最後一欄是 disabled_at
  IF NOT EXISTS (SELECT 1 FROM pg_catalog.pg_class c WHERE c.oid = 'public.admin_customer_list_v'::regclass
                   AND 'security_invoker=true' = ANY (c.reloptions)) THEN
    RAISE EXCEPTION '事後閘:admin_customer_list_v 的 security_invoker 不見了';
  END IF;
  IF (SELECT pg_catalog.max(a.attnum) FROM pg_catalog.pg_attribute a WHERE a.attrelid = 'public.admin_customer_list_v'::regclass AND a.attnum > 0) <> 13
     OR NOT EXISTS (SELECT 1 FROM pg_catalog.pg_attribute a WHERE a.attrelid = 'public.admin_customer_list_v'::regclass AND a.attnum = 13 AND a.attname = 'disabled_at') THEN
    RAISE EXCEPTION '事後閘:admin_customer_list_v 欄位不是「原 12 欄 + disabled_at」';
  END IF;
  -- 搜尋函式只剩一版
  IF (SELECT pg_catalog.count(*) FROM pg_catalog.pg_proc p WHERE p.proname = 'admin_search_customers' AND p.pronamespace = 'public'::regnamespace) <> 1 THEN
    RAISE EXCEPTION '事後閘:admin_search_customers 不是剛好一版(PostgREST 會報 PGRST203)';
  END IF;
  -- 新函式與搜尋函式:會員與訪客不能執行;後台可以(pcm_customer_delete_blockers 例外:後台也不能直接執行)
  FOREACH v_sig IN ARRAY v_functions LOOP
    IF pg_catalog.to_regprocedure(v_sig) IS NULL THEN
      RAISE EXCEPTION '事後閘:斷言清單裡的 % 不存在', v_sig;
    END IF;
    IF pg_catalog.has_function_privilege('anon', v_sig, 'EXECUTE') OR pg_catalog.has_function_privilege('authenticated', v_sig, 'EXECUTE') THEN
      RAISE EXCEPTION '事後閘:% 對 anon 或 authenticated 開放執行', v_sig;
    END IF;
    CONTINUE WHEN v_sig = 'public.pcm_customer_delete_blockers(uuid)';
    IF NOT pg_catalog.has_function_privilege('service_role', v_sig, 'EXECUTE') THEN
      RAISE EXCEPTION '事後閘:service_role 不能執行 %', v_sig;
    END IF;
    IF NOT EXISTS (SELECT 1 FROM pg_catalog.pg_proc p WHERE p.oid = v_sig::regprocedure AND p.prosecdef
                     AND p.proconfig @> ARRAY['search_path=""']) THEN
      RAISE EXCEPTION '事後閘:% 不是 SECURITY DEFINER 或 search_path 沒鎖', v_sig;
    END IF;
  END LOOP;
  IF pg_catalog.has_function_privilege('service_role', 'public.pcm_customer_delete_blockers(uuid)', 'EXECUTE')
     OR pg_catalog.has_function_privilege('authenticated', 'public.pcm_customer_delete_blockers(uuid)', 'EXECUTE') THEN
    RAISE EXCEPTION '事後閘:pcm_customer_delete_blockers 不應開放執行';
  END IF;
  -- 改過的五支:簽章不變、仍是 SECURITY DEFINER、內容已含停用檢查、會員原本能執行的仍能執行
  FOR r IN SELECT * FROM (VALUES
      ('public.create_order(jsonb, uuid, text, jsonb, uuid, text, text, text, text, text, text)', true),
      ('public.dealer_application_submit(text, text, text, text, text, text, text, text)', true),
      ('public.dealer_application_update_mine(uuid, text, text, text, text, text, text, text, text)', true),
      ('public.admin_dealer_application_decide(uuid, text, text, text, text, text, timestamp with time zone)', false),
      ('public.admin_create_manual_order(uuid, uuid, text, text, text, text, jsonb, jsonb, integer, jsonb, text, text, jsonb)', false)
    ) AS t(sig, member_exec)
  LOOP
    IF NOT EXISTS (SELECT 1 FROM pg_catalog.pg_proc p WHERE p.oid = r.sig::regprocedure AND p.prosecdef
                     AND pg_catalog.strpos(p.prosrc, 'disabled_at') > 0) THEN
      RAISE EXCEPTION '事後閘:% 沒有停用檢查或不是 SECURITY DEFINER', r.sig;
    END IF;
    IF pg_catalog.has_function_privilege('authenticated', r.sig, 'EXECUTE') IS DISTINCT FROM r.member_exec THEN
      RAISE EXCEPTION '事後閘:% 的會員執行權限和改之前不同', r.sig;
    END IF;
  END LOOP;
END
$post$;
