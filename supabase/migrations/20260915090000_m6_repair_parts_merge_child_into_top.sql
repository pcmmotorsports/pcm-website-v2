-- pcm:idempotent: yes
--   理由:本支寫資料(UPDATE products.category_id + DELETE 一列 categories),而**重跑安全**——
--   第二發走冪等出口:子類 `維修零件 · 維修零件` 已被第一發刪掉 ⇒ `v_child_id IS NULL` ⇒ RAISE NOTICE 後 RETURN,
--   零寫入(`:57-62`)。拋棄式 PG 2026-09-14 實跑四段驗過:套用 → 重跑(跳過)→ 回滾 → 再套用。
--   ⚠️ 宣告的責任在我:重跑不寫、也不會把別的分類搬錯(前置閘①②③ 各自 fail-closed)。
-- ============================================================
