...
--   顧客站目錄每頁上限 100 → 1000(每頁選 200 / 500 / 1000 時不再跳過商品)
-- M-4b · 後台窗 shop-6(主視窗 pcm-website-v2-90 派工, Sean 選甲)· plan docs/plans/2026-09-22-catalog-page-limit-1000-plan.md
-- 🛑 未貼(寫好不貼;plan 批准與貼板由 Sean 決定)。
--
-- ══ 為什麼 ═══════════════════════════════════════════════════
-- 每頁選項是 Sean 09-12 指定的 100 / 200 / 500 / 1000(01b5b72bc), 前端照選項送 p_limit;
-- 而兩支目錄函式把回傳筆數上限寫死成 100(p_limit 再大也只回 100 列)。
-- ⇒ 選 200 時第 2 頁從第 201 件開始, 但第 1 頁只拿到 100 件 ⇒ 第 101-200 件客人永遠看不到。
--    選 500 / 1000 只有 1 頁 100 件, 其餘整批不見(正式站 MT-09 SP 489 件, 選 500 只看到 100)。
--
-- ══ 改什麼 ═══════════════════════════════════════════════════
-- 只改 4 處上限字面 100 → 1000, 其他一個字不動:
--   search_catalog_by_vehicle(14 參)兩處(沒選車 / 有選車)、search_catalog_by_vehicle_dealer(14 參)兩處。
-- 本體從 20260916260000 逐字抄;本機算的 md5(prosrc) 與 2026-09-22 正式庫唯讀讀到的相同
--   (9242dea9… / e577ec35…)⇒ 抄的就是正式庫正在跑的那一代。簽章不變 ⇒ CREATE OR REPLACE, 不 DROP。
-- SET 子句跟著本體原樣帶著;事後閘③逐字驗 owner / SET / ACL / DEFINER。
-- 🔵 不動 11 參數的舊版 search_catalog_by_vehicle:ACL 只剩 postgres(anon / authenticated 叫不到), 前端一律送 14 參。
--
-- ══ 效能(2026-09-22 正式庫唯讀實測, 見 plan 第 2 節)══════════════
-- p_limit 1000 最慢約 1.05 秒(沒選車全站);anon 的 statement_timeout 是 3 秒。
--
-- ══ 上線順序 ═══════════════════════════════════════════════════
-- 純 DB, 前端不用改(前端本來就照每頁選項送 p_limit)⇒ 碼不用跟著推。
-- 🔴 貼完手動跑 NOTIFY pgrst, 'reload schema';(函式本體換了)。
-- 🔴 貼完同批跑 pcm_acl_approve_latest(p_note 帶 20260922130000):2 支換一代。
-- 鎖:兩支 CREATE OR REPLACE FUNCTION, 不動表。
--
-- ══ rollback ═══════════════════════════════════════════════════
-- SET LOCAL lock_timeout = '5s';
-- (上一行 = 退回檔開頭那句, rollback-locktimeout-gate 要它在本段第一行)
-- supabase/rollbacks/20260922130000-rollback.sql:兩支回 100(= 20260916260000 那一代本體, md5 釘)。
-- ============================================================

BEGIN;
SET LOCAL lock_timeout = '5s';
-- 事後閘②會叫兩次目錄(沒選車全目錄 + 選車 Panigale V4)⇒ 給 120s 上界
SET LOCAL statement_timeout = '120s';

-- ── 前置閘:兩支必須是 20260916260000 那一代(md5 of prosrc)──
DO $pre$
BEGIN
  IF (SELECT pg_catalog.md5(p.prosrc) FROM pg_catalog.pg_proc p
     ...
     IS DISTINCT FROM '9242dea94a4ff272efdf485603d821bb' THEN
    RAISE EXCEPTION '前置閘a:search_catalog_by_vehicle(14 參) 不是 20260916260000 那一代 ⇒ 有人改過或已經貼過, 停下對齊';
  END IF;
  IF (SELECT pg_catalog.md5(p.prosrc) FROM pg_catalog.pg_proc p
     ...
     IS DISTINCT FROM 'e577ec35d5545dcda3174fc1bb8407f3' THEN
    RAISE EXCEPTION '前置閘b:search_catalog_by_vehicle_dealer(14 參) 不是 20260916260000 那一代, 停';
  END IF;
  ...
  IF (SELECT pg_catalog.pg_get_userbyid(p.proowner) || '|' || p.prosecdef::text || '|' || p.proconfig::text || '|' || p.proacl::text
    ...
    RAISE EXCEPTION '前置閘c:search_catalog_by_vehicle 的 owner / SET / ACL / DEFINER 與 2026-09-22 讀到的不同 ⇒ 有人改過設定, 停下對齊';
  END IF;
  IF (SELECT pg_catalog.pg_get_userbyid(p.proowner) || '|' || p.prosecdef::text || '|' || p.proconfig::text || '|' || p.proacl::text
    ...
    RAISE EXCEPTION '前置閘d:search_catalog_by_vehicle_dealer 的 owner / SET / ACL / DEFINER 與 2026-09-22 讀到的不同, 停';
  END IF;
END
$pre$;
...
-- ── 事後閘①:本體是預期那一版(md5), 而且上限字面是 1000 ──
DO $post1$
BEGIN
  IF (SELECT pg_catalog.md5(p.prosrc) FROM pg_catalog.pg_proc p
       WHERE p.oid = pg_catalog.to_regprocedure('public.search_catalog_by_vehicle(text[],text,text,integer,integer,integer,text,text,text[],integer,integer,timestamp with time zone,text[],text)')) IS DISTINCT FROM '55d387e8901911dd16e993391bfe72d7' THEN
    RAISE EXCEPTION '事後閘①a:search_catalog_by_vehicle 本體不是預期那一版';
  END IF;
  IF (SELECT pg_catalog.md5(p.prosrc) FROM pg_catalog.pg_proc p
       WHERE p.oid = pg_catalog.to_regprocedure('public.search_catalog_by_vehicle_dealer(text[],text,text,integer,integer,integer,text,text,text[],integer,integer,timestamp with time zone,text[],text)')) IS DISTINCT FROM 'e1760be9748ff230576e979ef518f685' THEN
    RAISE EXCEPTION '事後閘①b:search_catalog_by_vehicle_dealer 本體不是預期那一版';
  END IF;
END
$post1$;

-- ── 事後閘②:沒選車、選車各叫一次 p_limit = 5000(比上限大)⇒ 拿到的列數 = LEAST(1000, 總數) ──
...
BEGIN
  ...
  IF v_total IS NULL OR v_total <= 1000 THEN
    RAISE EXCEPTION '事後閘②a:沒選車全站只有 % 件, 驗不到上限 1000 ⇒ 停下看資料', v_total;
  END IF;
  IF v_n <> 1000 THEN
    RAISE EXCEPTION '事後閘②b:p_limit=5000 拿到 % 列, 應為 1000', v_n;
  END IF;
  ...
  IF v_total IS NULL OR v_total <= 1000 THEN
    RAISE EXCEPTION '事後閘②c:選車 Ducati Panigale V4 只有 % 件, 驗不到上限 1000 ⇒ 停下看資料', v_total;
  END IF;
  IF v_n <> 1000 THEN
    RAISE EXCEPTION '事後閘②d:選車 Ducati Panigale V4 p_limit=5000 拿到 % 列, 應為 1000', v_n;
  END IF;
  RAISE NOTICE '事後閘② ok:上限 1000 在沒選車與選車兩段都生效(Panigale V4 共 % 件)', v_total;
END
$post2$;
...
BEGIN
  IF (SELECT pg_catalog.pg_get_userbyid(p.proowner) || '|' || p.prosecdef::text || '|' || p.proconfig::text || '|' || p.proacl::text
        FROM pg_catalog.pg_proc p WHERE p.oid = pg_catalog.to_regprocedure('public.search_catalog_by_vehicle(text[],text,text,integer,integer,integer,text,text,text[],integer,integer,timestamp with time zone,text[],text)'))
     IS DISTINCT FROM 'postgres|false|{"search_path=public, pg_temp"}|{postgres=X/postgres,anon=X/postgres,authenticated=X/postgres,service_role=X/postgres}' THEN
    RAISE EXCEPTION '事後閘③a:search_catalog_by_vehicle 的 owner / SET / ACL / DEFINER 與貼之前不同';
  END IF;
  IF (SELECT pg_catalog.pg_get_userbyid(p.proowner) || '|' || p.prosecdef::text || '|' || p.proconfig::text || '|' || p.proacl::text
        FROM pg_catalog.pg_proc p WHERE p.oid = pg_catalog.to_regprocedure('public.search_catalog_by_vehicle_dealer(text[],text,text,integer,integer,integer,text,text,text[],integer,integer,timestamp with time zone,text[],text)'))
     IS DISTINCT FROM 'postgres|true|{"search_path=\"\""}|{postgres=X/postgres,authenticated=X/postgres}' THEN
    RAISE EXCEPTION '事後閘③b:search_catalog_by_vehicle_dealer 的 owner / SET / ACL / DEFINER 與貼之前不同(經銷價不能給 anon)';
  END IF;
  RAISE NOTICE '事後閘③ ok:兩支 owner / SET / ACL / DEFINER 與貼之前相同';
END
$post3$;
