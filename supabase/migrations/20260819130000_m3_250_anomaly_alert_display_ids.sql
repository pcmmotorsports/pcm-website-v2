...
-- ── 4. 五鍵都在、都是 array;外加一次【對帳快照】(⚠️ 不是守門,看下面那段) ──
--
-- 🔴🔴 **這一節【不是】fail-closed 守門 —— 名字改了,因為原本的名字在誤導人。**
--    兩位審查者(W6 R3 / W3 R4)獨立指出同一件事:
--    **它平常跑的環境就是【空庫或該類 0 筆】⇒ 幾乎不可能紅過,而且不會。**
--    ⇒ 一個幾乎不會紅的檢查,寫著 `fail-closed assert` 會讓下一個人以為那一格有人守著。
--    ⇒ **該做的是改它的名字,不是想辦法補強它。**(補強它要有資料,而 apply 期沒有。)
--
-- **它真正能做到的兩件(這兩件恆成立,與資料無關)**:
--    ① 五個鍵都回得出來 ② 五個都是 array
--    ⇒ 那擋得住「拼接把某個鍵弄丟了」與「型別錯」,兩者都突變驗過。
-- **它做不到的**:證明述詞同步。**證過述詞的是拋棄式 PG 那一發**(五類各種一筆真資料,
--    斷言「計數 = 陣列長度」且單號逐字正確)—— 那一發有資料,這一發沒有。
--    ⚠️ **而那一發【不可重跑】**:它跑在一次性的拋棄式叢集上,不在 repo 裡。
--       ⇒ 它證的是**當時那一版**的述詞同步,不是「這兩支永遠同步」。要再證一次得重搭環境。
-- ⚠️ ~~原句「這一格擋的是述詞不同步」~~ **作廢**(關卡2 抓的:它與上面那個新名字正面互斥)。
--    真話是:**述詞不同步這種壞法 apply 不會紅、三綠不會紅、告警照常寄**,
