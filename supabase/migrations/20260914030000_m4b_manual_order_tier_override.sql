-- 20260914030000_m4b_manual_order_tier_override.sql —— 手動建單可替【這張單】選會員等級(第 8 代:11 參 → 12 參)
--
-- 🛑 未貼(寫好不貼;貼是 Sean 一次一個編號)。plan `docs/plans/2026-09-14-manual-order-tier-override-plan.md`。
--
-- ══ Sean 拍的 ══════════════════════════════════════════════
-- 2026-09-14 逐字「手動建立訂單的時候可以選擇車行會員還是經銷的選項才對」。
-- 主視窗假設(已回 Sean):預設帶客人現在的等級、只改這張單的 `tier_at_checkout`、客人帳號等級不動。
--
-- ══ 做什麼 ═════════════════════════════════════════════════
-- · `admin_create_manual_order` 加第 12 參 `p_tier text DEFAULT NULL`(白名單 general / store / premiumStore;
--   NULL = 照客人現在的)。函式體 = 第 7 代(`20260910090000:213-1141`)逐字 + 四處加碼:
--     ① G1 白名單 RAISE ② G3 之後 `v_tier := COALESCE(p_tier::public.member_tier, v_tier)`
--     ③ 冪等指紋 `v_canonical` 加 `'tier'` = **p_tier 原值**(NULL 就是 NULL;主視窗裁, 理由同第④代 invoice_requested;
--        代價:apply 前的舊 request_id 原樣重送會被判 P858B —— 那段代價 `20260910090000:783-795` 已述, 這裡再加一個鍵而已。
--        🔴 不是 v_tier:那會讓「沒帶 tier + 客人等級中間被改 + 同鍵重送」變成 P858B, codex R1 must-fix ①)
--     ④ audit `after` 加 `tier_at_checkout` / `tier_overridden`
-- · 零價格連動:`v_tier` 只出現在 G3 / INSERT / 指紋 / audit(plan §0 實查);premiumStore 不觸發取價 / 稅。
--
-- ══ 形狀:DROP 11 參 + CREATE 12 參(逐字照 `20260905130000` 加第 11 參那次)══════════
-- 不用 CREATE OR REPLACE:簽章含參數列, 加參數 = 第二支多載, 舊 11 名呼叫 PGRST203。
-- 🔴 DROP 帶走 ACL 與 COMMENT ⇒ §6 三行 REVOKE/GRANT + §6b COMMENT ON 一定要重貼。
-- 🔴 `DEFAULT NULL` ⇒ 舊 TS 送 11 個名字仍唯一命中 ⇒ **DB 先貼、UI 後上**, 中間態合法。
--
-- ══ 回滾 ═══════════════════════════════════════════════════
-- `supabase/rollbacks/20260914030000-rollback.sql` = DROP 12 參 + 逐字貼回第 7 代 + 它的 ACL / COMMENT。
-- 🔴 順序 = 先 revert code 再回 DB(DB 回舊版時新 UI 送 12 個名字 ⇒ PGRST202)。
--
-- ══ 驗證(拋棄式 PG = admin-probe 55560, 2026-09-14)═══════════════════
-- apply 綠(前置 / 後置閘全過)· down 綠(md5 回到 e703456a…)· 再 apply 綠 · 負對照(白名單外 / 重貼 / 指紋)見 commit body。
...
DO $pre$
DECLARE
  v_old oid := pg_catalog.to_regprocedure('public.admin_create_manual_order(uuid,uuid,text,text,text,text,jsonb,jsonb,integer,jsonb,text)');
  v_new oid := pg_catalog.to_regprocedure('public.admin_create_manual_order(uuid,uuid,text,text,text,text,jsonb,jsonb,integer,jsonb,text,text)');
  v_src text;
  v_secdef boolean; v_strict boolean; v_vol "char"; v_config text[]; v_owner text;
BEGIN
  IF v_old IS NULL THEN
    RAISE EXCEPTION '前置閘一:找不到 11 參那一代 —— 這台庫上的 admin_create_manual_order 不是我以為的形狀, 停下人工對齊';
  END IF;
  IF v_new IS NOT NULL THEN
    RAISE EXCEPTION '前置閘二:12 參那一代【已經存在】—— 本檔貼過了, 或有人先建了一支多載, 停下人工對齊';
  END IF;
  SELECT p.prosrc, p.prosecdef, p.proisstrict, p.provolatile, p.proconfig, pg_catalog.pg_get_userbyid(p.proowner)
    INTO v_src, v_secdef, v_strict, v_vol, v_config, v_owner
    FROM pg_catalog.pg_proc p WHERE p.oid = v_old;
  -- 🔴 本體指紋:本檔的函式體是從第 7 代逐字抄的(簽章相同 ≠ 同一代, `20260905130000:97-104` 那個坑)。
  IF pg_catalog.md5(v_src) <> 'e703456a3766558851d57c1932c87c1b' THEN
    RAISE EXCEPTION USING MESSAGE =
      '前置閘三:函式體 md5 不是第 7 代(20260910090000 後置閘那一版 e703456a3766558851d57c1932c87c1b;實得 '
      || pg_catalog.md5(v_src) || ')⇒ 有人動過它, 本檔的抄本基準已失效, 停下人工對齊';
  END IF;
  -- 🔴 屬性也要比(同 `20260910090000:191-206`):本體沒動而屬性被修補過 ⇒ 硬貼會把修補抹掉。
  IF NOT (v_secdef AND NOT v_strict AND v_vol = 'v'
    ...
    RAISE EXCEPTION USING MESSAGE =
      '前置閘四:函式屬性不是第 7 代那一組(prosecdef=' || v_secdef::text || ' proisstrict=' || v_strict::text
      || ' provolatile=' || v_vol::text || ' proconfig=' || COALESCE(v_config::text, 'NULL') || ' owner=' || v_owner || ')';
  END IF;
  -- 防重貼(字面判準)
  IF pg_catalog.strpos(v_src, 'tier_overridden') > 0 THEN
    RAISE EXCEPTION '前置閘五:函式體裡已經有 tier_overridden ⇒ 這一片貼過了, 拒重貼';
  END IF;
  -- 正對照 / 負對照
  IF pg_catalog.strpos(v_src, 'v_notification_email') = 0 THEN
    RAISE EXCEPTION '前置閘六(正對照):連 v_notification_email 都找不到 ⇒ 這把尺在錯的東西上';
  END IF;
  IF pg_catalog.strpos(v_src, 'zzz_never_a_literal') > 0 THEN
    RAISE EXCEPTION '前置閘七(負對照):現造字面竟然命中 ⇒ 這把尺壞了';
  END IF;
END
$pre$;

-- ── 換函式:DROP 11 參(第 7 代)→ CREATE 12 參(第 8 代)──
...
-- ═══ §6 ACL:逐字照 `20260905130000:806-815`, 只換簽章 ═══
-- 🔴 DROP 帶走了 ACL ⇒ 這三行是唯一來源。三道 REVOKE 不是兩道的加強版:PUBLIC 與具名角色是兩套授權。
REVOKE ALL ON FUNCTION public.admin_create_manual_order(
  uuid, uuid, text, text, text, text, jsonb, jsonb, integer, jsonb, text, text) FROM PUBLIC;
REVOKE ALL ON FUNCTION public.admin_create_manual_order(
  uuid, uuid, text, text, text, text, jsonb, jsonb, integer, jsonb, text, text) FROM anon, authenticated;
GRANT EXECUTE ON FUNCTION public.admin_create_manual_order(
  uuid, uuid, text, text, text, text, jsonb, jsonb, integer, jsonb, text, text) TO service_role;

-- ═══ §6b COMMENT ON —— DROP 也把它帶走了(`20260905130000:817-830` 同款坑)═══
-- 內容 = 第 5 代那份 + 第 8 代加的三處(白名單 / 蓋過 / 指紋與 audit 多的鍵)。
...
-- ── 後置閘(照 `20260910090000:1146` 形狀 + `20260905130000:832-903` 的簽章 / 多載 / 參數名 / ACL / COMMENT 斷言)──
DO $post$
DECLARE
  v_new oid;
  v_src text;
  ...
  v_names text;
BEGIN
  v_new := pg_catalog.to_regprocedure('public.admin_create_manual_order(uuid,uuid,text,text,text,text,jsonb,jsonb,integer,jsonb,text,text)');
  IF v_new IS NULL THEN
    RAISE EXCEPTION '後置閘一:12 參那一代沒建起來';
  END IF;
  SELECT p.prosrc, pg_catalog.array_to_string(p.proacl, ',') INTO v_src, v_acl
    FROM pg_catalog.pg_proc p WHERE p.oid = v_new;

  -- 🔴 貼進去的就是我編檔時算過的那一版 —— 逐字元(md5 對 prosrc, 含 $fn$ 內側兩個換行)。
  IF pg_catalog.md5(v_src) <> '3021a009074b0caa3f1ceb5cd5c90cdd' THEN
    RAISE EXCEPTION USING MESSAGE =
      '後置閘二:新本體 md5 是 ' || pg_catalog.md5(v_src) || ', 而編檔時算的是 3021a009074b0caa3f1ceb5cd5c90cdd ⇒ 貼進去的不是我驗過的那一份';
  END IF;

  -- 四處新碼都在
  IF pg_catalog.strpos(v_src, 'p_tier NOT IN (''general'', ''store'', ''premiumStore'')') = 0 THEN
    RAISE EXCEPTION '後置閘三:找不到白名單那一行 ⇒ 壞的 tier 進得來';
  END IF;
  IF pg_catalog.strpos(v_src, 'v_tier := COALESCE(p_tier::public.member_tier, v_tier)') = 0 THEN
    RAISE EXCEPTION '後置閘四:找不到 COALESCE 那一行 ⇒ 選了也沒用';
  END IF;
  IF pg_catalog.strpos(v_src, '''tier'',             p_tier') = 0 THEN
    RAISE EXCEPTION '後置閘五:指紋裡沒有 tier(要的是 p_tier 不是 v_tier)⇒ 只改等級再送會被判成同內容';
  END IF;
  IF pg_catalog.strpos(v_src, 'tier_overridden') = 0 THEN
    RAISE EXCEPTION '後置閘六:audit 裡沒有 tier_overridden';
  END IF;
  -- 🔵 第 7 代的東西一個字沒少(殘差 / 未稅價別 / 通知信箱)
  IF pg_catalog.strpos(v_src, 'v_taxed_residual') = 0 OR pg_catalog.strpos(v_src, 'v_price_tax_mode := ''inclusive''') = 0
     OR pg_catalog.strpos(v_src, 'v_notification_email') = 0 THEN
    RAISE EXCEPTION '後置閘七:第 7 代的字面不見了 ⇒ 本片動到了它不該動的東西';
  END IF;

  -- 恰一支(多載會讓呼叫端拿到 function is not unique)
  SELECT pg_catalog.count(*) INTO v_dup FROM pg_catalog.pg_proc p
    JOIN pg_catalog.pg_namespace n ON n.oid = p.pronamespace
   WHERE n.nspname = 'public' AND p.proname = 'admin_create_manual_order';
  IF v_dup <> 1 THEN
    RAISE EXCEPTION '後置閘八:public.admin_create_manual_order 有 % 支(期望恰好 1)', v_dup;
  END IF;

  -- 參數名(PostgREST 走具名呼叫)與預設值個數(兩顆 DEFAULT NULL:notification_email / tier)
  SELECT pg_catalog.string_agg(a, ',' ORDER BY o) INTO v_names
    FROM pg_catalog.unnest((SELECT p.proargnames FROM pg_catalog.pg_proc p WHERE p.oid = v_new)) WITH ORDINALITY AS t(a, o);
  IF v_names IS DISTINCT FROM 'p_customer_user_id,p_manual_request_id,p_actor,p_order_source,p_payment_channel,p_shipping_method,p_ship_to,p_invoice,p_shipping_fee,p_lines,p_notification_email,p_tier' THEN
    RAISE EXCEPTION '後置閘九:參數名字串是 [%] —— 與預期不符', v_names;
  END IF;
  IF (SELECT p.pronargdefaults FROM pg_catalog.pg_proc p WHERE p.oid = v_new) <> 2 THEN
    RAISE EXCEPTION '後置閘十:預設值個數不是 2 —— 舊的 11 名呼叫端會壞';
  END IF;

  -- ACL / COMMENT 回來了(DROP 帶走的兩樣)
  IF v_acl IS NULL OR v_acl NOT LIKE '%service_role=X/%' THEN
    RAISE EXCEPTION USING MESSAGE = '後置閘十一:proacl 裡看不到 service_role(收到 ' || COALESCE(v_acl, '<null>') || ')';
  END IF;
  IF pg_catalog.has_function_privilege('anon', v_new, 'EXECUTE') OR pg_catalog.has_function_privilege('authenticated', v_new, 'EXECUTE') THEN
    RAISE EXCEPTION '後置閘十二:anon / authenticated 拿得到 EXECUTE';
  END IF;
  IF pg_catalog.obj_description(v_new, 'pg_proc') IS NULL THEN
    RAISE EXCEPTION '後置閘十三:新函式沒有 COMMENT —— DROP 帶走的第二樣東西沒補回來';
  END IF;
  IF (SELECT p.prosecdef AND p.proconfig @> ARRAY['search_path=""']::text[] FROM pg_catalog.pg_proc p WHERE p.oid = v_new) IS NOT TRUE THEN
    RAISE EXCEPTION '後置閘十四:不是 SECURITY DEFINER + search_path 空';
  END IF;

  -- ⚪ 負對照
  IF pg_catalog.strpos(v_src, 'zzz_never_a_literal') > 0 THEN
    RAISE EXCEPTION '後置閘十五(負對照):現造字面竟然命中 ⇒ 上面那幾格不算數';
  END IF;

  RAISE NOTICE '第 8 代貼好了:admin_create_manual_order 12 參(p_tier), 只改這張單的 tier_at_checkout。';
END
$post$;

-- ── 收權斷言(靜態檢查③認得的 `v_functions` 形狀;內容與上面後置閘十一 / 十二同一件事, 這裡是那把尺讀的形狀)──
DO $assert$
DECLARE
  v_relations text[] := ARRAY[]::text[];
  v_functions text[] := ARRAY['public.admin_create_manual_order(uuid,uuid,text,text,text,text,jsonb,jsonb,integer,jsonb,text,text)']::text[];
  r text;
  v_bad text := NULL;
BEGIN
