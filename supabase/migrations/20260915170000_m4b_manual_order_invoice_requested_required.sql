-- 20260915170000_m4b_manual_order_invoice_requested_required.sql —— 手動建單「開發票」缺鍵改回 RAISE(不再替員工猜 true / +5%)
--
-- 🛑 未貼(貼板 Sean 一張一點名)。Sean 2026-09-16 批 Q10:plan `docs/plans/2026-09-15-invoice5pct-missing-key-raise-plan.md`(B 窗 20bc4817c)。
--
-- ══ 病 ═════════════════════════════════════════════════════════════════
-- admin_create_manual_order 現行代(20260915060000:915):`v_invoice_requested := COALESCE((p_invoice ->> 'requested')::boolean, true);`
-- ⇒ 呼叫端沒送 `requested` 時 DB 自己填 true(開發票、含稅列 +5%), 與「員工明確勾了要開」在 orders.invoice_requested 上是同一個 true。
-- 那是 2026-09-04 第④代的相容期設計, 板列 ⑩ 寫明「相容期結束要另一支 migration 改回 RAISE, 還沒有人排」。
-- ══ 相容期結束了(量到的)══════════════════════════════════════════════
-- 唯一 TS 呼叫端 manual-order-repository.ts:307 一律送 `requested: values.invoiceRequested`(boolean);表單沒勾 ⇒ hidden 'off' ⇒ false。
-- ⚠️ codex R1 nit 更正:「沒有任何呼叫端不送」說太滿 —— 要分三層講:
--   · 正式應用程式:只有 manual-order-repository.ts 一支, 一律送(上面那行)。
--   · 現行驗證探針 / after-check:docs/probes/2026-08-24-858-manual-order-rpc-r2-negatives.sql 的 pg_temp.mk3、
--     supabase/after-checks/130000-after.sql 與 130000-after-sqleditor.sql(codex R2 抓)原本沒送 ⇒ 同 commit 補 "requested":true。
--     20260915060000-rpc-total-single-source.sql 有送鍵但釘舊 md5 ⇒ 版本限定, 本代套上後重跑那支會紅是預期, 不當回歸依據。
--   · 固定歷史版本的探針:scripts/spec1-apply-probe.sh:128(冪等案例)、scripts/probe-invoice5pct-tax-gate.sh:179(缺鍵案例)沒送,
--     但它們測的是自己那一代, 不會因本檔而壞 ⇒ 不動。supabase/tests 零呼叫。
-- 🔴 行為改變要講清楚:用舊代【沒送鍵】建成的單, 若原封不動用同一個 p_manual_request_id 重送, 新代會在指紋比對之前就拒(不回 idempotent)。
--    原訂單仍在;要重送 ⇒ 其餘內容不變、同 request id 補 `requested: true`(舊代缺鍵等同 true ⇒ 指紋正規化後相同, 回 idempotent)。
--    ⚠️ 補成 false 指紋就不同 ⇒ 會撞 P858B(codex R2 nit)。不要換新 request id 重建。唯一呼叫端從 2026-09-04 起就送鍵, 實務上遇不到。
-- ══ 做什麼 ═════════════════════════════════════════════════════════════
-- · 本代 = 20260915060000 那一代本體【逐字】+ 只換 requested 那一段:缺鍵 ⇒ RAISE;送了不是 boolean ⇒ 照舊 RAISE;其餘 ⇒ 直接 ::boolean。
-- · 簽章不動 ⇒ CREATE OR REPLACE;🔴 SET 子句整組照寫 SET search_path = ''。ACL 重發基準並事後斷言。
-- · 缺鍵那道在任何寫入之前(G6 品項迴圈與 INSERT orders 都在它後面)⇒ 擋下時整筆零寫入。
-- ══ 前提 ═══════════════════════════════════════════════════════════════
-- 前置閘:本體 md5 = 20260915060000 那一代 d97986f066c64f0c7f8baaf9cc5202f8 ⇒ 升;= 本代 dc97e3eee72204f893fa95e8355e8f5b ⇒ 冪等;其他 ⇒ 停(有人又改了一代, 同名 RPC 版本晚的必須是聯集)。
-- 回滾:supabase/rollbacks/20260915170000_down.sql = 貼回 20260915060000 那一代逐字。零資料改動。
...
BEGIN
  ...
  END IF;
  IF pg_catalog.md5(v_src) = 'd97986f066c64f0c7f8baaf9cc5202f8' THEN
    RAISE NOTICE '前置閘⓪:20260915060000 那一代在(md5 相符)⇒ 升本代';
  ELSIF pg_catalog.md5(v_src) = 'dc97e3eee72204f893fa95e8355e8f5b' THEN
    RAISE NOTICE '前置閘⓪:已是本代(md5 相符)⇒ 照樣重跑(冪等)';
  ELSE
    RAISE EXCEPTION '前置閘⓪:admin_create_manual_order md5(prosrc) = % 不是 20260915060000 那一代也不是本代 ⇒ 有人又改了一代, 停下人工對齊(版本晚的必須是聯集)', pg_catalog.md5(v_src);
  END IF;
  IF pg_catalog.to_regprocedure('public.zzq_no_such_fn_20260915170000(uuid)') IS NOT NULL THEN
...
DO $post$
DECLARE v_oid oid := pg_catalog.to_regprocedure('public.admin_create_manual_order(uuid, uuid, text, text, text, text, jsonb, jsonb, integer, jsonb, text, text, jsonb)'); v_src text; v_bad text;
BEGIN
  ...
  IF pg_catalog.strpos(v_src, 'p_invoice.requested 必填(true / false)') = 0 THEN
    RAISE EXCEPTION '事後閘③:函式體沒有缺鍵 RAISE ⇒ 貼到的不是本代';
  END IF;
  IF pg_catalog.strpos(v_src, 'COALESCE((p_invoice ->> ''requested'')::boolean, true)') <> 0 THEN
    RAISE EXCEPTION '事後閘③:缺鍵猜 true 那一行還在';
  END IF;
  IF pg_catalog.strpos(v_src, 'pcm_order_total') = 0 OR pg_catalog.strpos(v_src, 'v_vehicle') = 0 OR pg_catalog.strpos(v_src, 'p_tier') = 0 THEN
    RAISE EXCEPTION '事後閘③:20260915060000 那一代的 pcm_order_total / 車輛 / 等級 少了一樣 ⇒ 聯集漏了';
  END IF;
  IF pg_catalog.strpos(v_src, 'P9Z99_20260915170000') <> 0 THEN
    RAISE EXCEPTION '事後閘③b(負對照):現造的字串居然在定義裡 ⇒ 這把尺壞了';
  END IF;
  RAISE NOTICE '20260915170000 貼好了:admin_create_manual_order 缺 p_invoice.requested ⇒ RAISE。';
END
$post$;
