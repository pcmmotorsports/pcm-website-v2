-- pcm:ddl-into-vc: public 的 pcm_readonly 角色與它的 GRANT
--   【`pcm:ddl-into-vc` 那行】🔴 給機器讀的(⟦0e-DDLINTOVC-MARK⟧;`-f8` 2026-09-06 裁甲/乙)。
--     它說的是【物件在正式庫上早就有了】⇒ 「角色存在」對「這支檔跑過沒」是**零判別力**。
--     🛑 **這一行是 2026-09-06 補的, 而它訂正了一個已經造成後果的空白**:
--       `supabase/APPLIED.tsv:504`(commit `ada19656b`)記本支時, 判別依據逐字
--       「pg_roles 裡 rolname = pcm_readonly 的列數 = 1」—— **那正是本標記要消滅的推論**。
--       結論可能是對的, 而理由不成立。⇒ 該列已改成補版控型的固定格式, 原文一字未刪。
--     ⚠️ **不要挪到檔頭 20 行以外** —— 每一支讀它的工具都只讀前 20 行, 而掉出去是靜默的。
--     🔴🔴 **本支【刻意】不帶另一個標記**(那是另一個軸:「要不要貼」, 兩者不可互相推導)。
--       ⛔ **而這一行原本逐字寫著那個標記的字面, 結果把本檔【變成】了那一型** ——
--         `scripts/state-gates.sh` 用**子字串**比對檔頭前 20 行 ⇒ 它把這句解釋讀成了標記,
--         `docs/reference/order-state-gates.md` 的分母當場從「排除 4 支」變成「排除 5 支」。
--       🎯 **一句說明「本檔沒有這個標記」的話, 讓本檔有了那個標記。**(2026-09-06 當場撞到並修)
--       📌 而 `lib-migration-header-marks.sh` 的檔頭正好寫著這條:「一支檔中段【提到】這個字面,
--         不該把它自己變成那一型」—— 我寫了那句話, 然後在同一夜自己踩進去。
--       ⇒ **要提到某個標記字面時, 把它拆開寫**(例:pcm 冒號 never-apply), 不要留完整字面。
-- ═══════════════════════════════════════════════════════════════════════════
...
-- 🔴 **「不記帳本」那半已於 2026-09-06 被 -f8 裁【乙】推翻**(⟦0e-DDLINTOVC-MARK⟧):
--    補版控型一律記進 `supabase/APPLIED.tsv`, 理由是**第二欄綁 sha ⇒ 檔內容一改就自動變回
--    PENDING、部署閘正確重燃**;而檔頭標記對內容改動完全不敏感。
--    🛑 不記的代價是量到的:它永遠 PENDING ⇒ `deploy-order-gate` 擋掉每一次碰到這些物件的
--    app 改動, 每次印「可能誤擋…要 Sean 在場」—— Fable R3 逐字:**那句話訓練的是 --no-verify**。
--    ✅ 「不 apply 到正式庫」那半**維持不變**。舊字面留刪除線, 讓搜它的人撞到訂正。
-- ═══════════════════════════════════════════════════════════════════════════
