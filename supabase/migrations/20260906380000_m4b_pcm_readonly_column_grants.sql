...
--
-- ═══ 段六 · 🔴 若走【手動 psql】一定要帶 `-v ON_ERROR_STOP=1` ═══════════════
--   🔬 2026-09-06 `-db` 拋棄式 PG 實測:沒帶它 ⇒ 交易中途 ERROR、**整包 ROLLBACK**,
--     而 **`psql` 自己回 rc=0** ⇒ 📌 **「整包回捲了」與「跑完了」在離場碼上同形。**
--   🔵 而**三條路各自的狀況不同**, 寫清楚免得下一個人以為都一樣:
--     ① `scripts/apply-paste-board.sh:522` **已經帶了**(而且 rc 取 `PIPESTATUS[0]`, 不是管線最後一段)⇒ 走它不必自己記。
--     ② **SQL Editor**(Sean 走這條):中途錯它會在畫面上紅 —— 而**本檔對這一半也守得住**,
--        因為驗收那段 `SELECT` 在 `COMMIT` **之前** ⇒ 前面任何一句錯掉, **那張 t/f 表就不會出現**。
--     ③ **手動 `psql -f`** ⇒ 🛑 **只有這一條靠人記得帶旗標。**
--   ⇒ 🎯 **所以這一行寫在檔頭, 不寫在某則會隨 session 消失的通告裡。**
-- ═══════════════════════════════════════════════════════════════════════════
