...
-- 稽核 P0-2 逾期匯款單補登記 · DB 第 1 片:期限 helper + 手動建單拿客人層級 advisory lock
-- pcm:rule1-exception: 新 helper pcm_bank_transfer_due_at 用 CREATE OR REPLACE 是為了重貼冪等(回滾後重套、同版重跑);撞名的危險由前置閘②擋:同名函式已存在而 md5 不是本片那一版 ⇒ RAISE(verify ⑥ 另測前置閘判別力)
--
-- 🛑 未貼(貼板 Sean 一張一點名)。plan:docs/plans/2026-09-15-expired-bank-order-late-registration-plan.md(R3,Sean 2026-09-15 11:3x 批甲)§2 第 2、3 項、§4、§7.1。
--
-- ══ 做什麼 ═════════════════════════════════════════════════════════════
-- ① 新增 public.pcm_bank_transfer_due_at(created_at):匯款單「逾期取消的那個 00:00」。
--    本體取自 pcm_cron.expire_unpaid_orders(20260906600000:264-272)bank_transfer 那一支,只把 o.created_at 換成參數。
--    SQL STABLE(時區名查表)、SECURITY INVOKER、search_path 空字串、owner postgres、四個角色都收回 EXECUTE ——
--    只給下一片的收款 RPC(postgres 擁有的 SECURITY DEFINER)在內部呼叫。
--    主視窗裁 Q1 甲:不動排程本身;前置閘釘住排程那一代的 md5(只看本體,COMMENT 改了不擋),排程換代時這支停下。
-- ② admin_create_manual_order 第 13 代 = 20260915170000 那一代本體【逐字】,只在 G7 INSERT orders 之前加一行
--    pg_advisory_xact_lock(hashtextextended(p_customer_user_id::text, 0))。
--    key 與 create_order(20260915100000:262)、begin_charge_attempt(20260904050000)同一個算法。
--    為什麼:下一片的補登記收款要判「客人是否在期限後已另下新單」(Sean Q2 乙),而會建出 orders 列的只有
--    create_order(已拿這把鎖)與本函式(原本零鎖)⇒ 本函式不拿,補登記看不到正在建立的手動單(codex plan R2 M3)。
--    鎖在 G7 之前的輸入驗證與冪等早退之後(G7 之後仍有規格驗證,那種失敗會先等鎖);拿鎖之後不鎖任何既有訂單列(plan §7.1)。
--    簽章不變 ⇒ CREATE OR REPLACE;SET 子句整組照寫;ACL / proconfig / SECURITY DEFINER / owner 改前改後逐一比。
--
-- ══ 前提 ═══════════════════════════════════════════════════════════════
-- 前置閘:admin_create_manual_order md5 = dc97e3eee72204f893fa95e8355e8f5b(20260915170000)⇒ 升;= c280e7e6f35660fc454e1754833c3981(本代)⇒ 冪等重跑;其他 ⇒ 停。
--         proconfig 必須恰好是 {search_path=""}(多了別的設定 ⇒ CREATE OR REPLACE 會把它洗掉 ⇒ 停)。
--         pcm_bank_transfer_due_at 不存在或 md5 = 9c4775d65626d189fe3881ae6fe16b8b;pcm_cron.expire_unpaid_orders md5 = 7e1e6764def6738440a1012cbea44f05。
-- 回滾:supabase/rollbacks/20260915233000_down.sql(下一片若已上,要先回滾下一片 —— 回滾檔的前置閘會擋)。
-- 驗證:bash scripts/20260915233000-verify.sh(拋棄式 PG = 正式庫 schema dump,Sean Q41 甲)。

BEGIN;
SET LOCAL lock_timeout = '5s';

CREATE TEMP TABLE _p02a_attr_before ON COMMIT DROP AS
SELECT p.proacl::text AS acl, p.proconfig::text AS cfg, p.prosecdef AS secdef, pg_catalog.pg_get_userbyid(p.proowner) AS owner
  FROM pg_catalog.pg_proc p WHERE p.oid = pg_catalog.to_regprocedure('public.admin_create_manual_order(uuid, uuid, text, text, text, text, jsonb, jsonb, integer, jsonb, text, text, jsonb)');

DO $pre$
DECLARE v_src text; v_cfg text[]; v_hel text; v_exp text;
BEGIN
  SELECT p.prosrc, p.proconfig INTO v_src, v_cfg FROM pg_catalog.pg_proc p WHERE p.oid = pg_catalog.to_regprocedure('public.admin_create_manual_order(uuid, uuid, text, text, text, text, jsonb, jsonb, integer, jsonb, text, text, jsonb)');
  IF v_src IS NULL THEN
    RAISE EXCEPTION '前置閘①:找不到 13 參 admin_create_manual_order';
  END IF;
  IF v_cfg IS DISTINCT FROM ARRAY['search_path=""'] THEN
    RAISE EXCEPTION '前置閘①:admin_create_manual_order 的 proconfig = % 不是恰好 {search_path=""} ⇒ CREATE OR REPLACE 會洗掉多出來的設定, 停', v_cfg;
  END IF;
  IF pg_catalog.md5(v_src) = 'dc97e3eee72204f893fa95e8355e8f5b' THEN
    RAISE NOTICE '前置閘①:20260915170000 那一代在(md5 相符)⇒ 升本代';
  ELSIF pg_catalog.md5(v_src) = 'c280e7e6f35660fc454e1754833c3981' THEN
    RAISE NOTICE '前置閘①:已是本代 ⇒ 冪等重跑';
  ELSE
    RAISE EXCEPTION '前置閘①:admin_create_manual_order md5 = % 不是 20260915170000 那一代也不是本代 ⇒ 有人又換了一代, 停下合併(版本晚的必須是聯集)', pg_catalog.md5(v_src);
  END IF;
  ...
  IF v_hel IS NOT NULL AND v_hel <> '9c4775d65626d189fe3881ae6fe16b8b' THEN
    RAISE EXCEPTION '前置閘②:pcm_bank_transfer_due_at 已存在而本體不是本片那一版(md5 = %)⇒ 停', v_hel;
  END IF;

  SELECT pg_catalog.md5(p.prosrc) INTO v_exp FROM pg_catalog.pg_proc p WHERE p.oid = pg_catalog.to_regprocedure('pcm_cron.expire_unpaid_orders(integer)');
  IF v_exp IS DISTINCT FROM '7e1e6764def6738440a1012cbea44f05' THEN
    RAISE EXCEPTION '前置閘③:pcm_cron.expire_unpaid_orders md5 = % 不是 20260906600000 那一代 ⇒ 逾期規則換代了, helper 的算法可能跟它不一樣, 停下重對', v_exp;
  END IF;

  IF pg_catalog.to_regprocedure('public.zzq_no_such_fn_20260915233000(uuid)') IS NOT NULL THEN
...
-- ── ① 期限 helper ──
CREATE OR REPLACE FUNCTION public.pcm_bank_transfer_due_at(p_created_at timestamptz)
RETURNS timestamptz
LANGUAGE sql
STABLE
SECURITY INVOKER
...
$fn$;
REVOKE ALL ON FUNCTION public.pcm_bank_transfer_due_at(timestamptz) FROM PUBLIC, anon, authenticated, service_role;
ALTER FUNCTION public.pcm_bank_transfer_due_at(timestamptz) OWNER TO postgres;
COMMENT ON FUNCTION public.pcm_bank_transfer_due_at(timestamptz) IS
  '稽核 P0-2(20260915233000):匯款單逾期取消的界 = 下單的台北日曆日 + 5 天 + 1 天的 00:00(Asia/Taipei)。'
  '本體取自 pcm_cron.expire_unpaid_orders(20260906600000)bank_transfer 那一支;排程換代時要一起對。'
  '只給收款 RPC 內部呼叫,四個角色都沒有 EXECUTE。';

-- ── ② admin_create_manual_order 第 13 代 ──
CREATE OR REPLACE FUNCTION public.admin_create_manual_order(
  ...
  -- 第 8 代(20260914030000):這張單的會員等級。NULL = 照客人現在的(舊版表單 / 舊 TS 送 11 個名字仍唯一命中)。
  p_tier text DEFAULT NULL::text,
  -- 第 9 代(20260914140000):這張單一台車(#956 Sean 2026-09-14 拍乙)。NULL = 沒填(舊 TS 送 12 個名字仍唯一命中)。
  p_vehicle jsonb DEFAULT NULL::jsonb
)
RETURNS jsonb
LANGUAGE plpgsql
SECURITY DEFINER
...
END;
$fn$;
REVOKE ALL ON FUNCTION public.admin_create_manual_order(uuid, uuid, text, text, text, text, jsonb, jsonb, integer, jsonb, text, text, jsonb) FROM PUBLIC;
REVOKE ALL ON FUNCTION public.admin_create_manual_order(uuid, uuid, text, text, text, text, jsonb, jsonb, integer, jsonb, text, text, jsonb) FROM anon, authenticated;
GRANT EXECUTE ON FUNCTION public.admin_create_manual_order(uuid, uuid, text, text, text, text, jsonb, jsonb, integer, jsonb, text, text, jsonb) TO service_role;
ALTER FUNCTION public.admin_create_manual_order(uuid, uuid, text, text, text, text, jsonb, jsonb, integer, jsonb, text, text, jsonb) OWNER TO postgres;

DO $post$
DECLARE
  v_oid oid := pg_catalog.to_regprocedure('public.admin_create_manual_order(uuid, uuid, text, text, text, text, jsonb, jsonb, integer, jsonb, text, text, jsonb)');
  v_hoid oid := pg_catalog.to_regprocedure('public.pcm_bank_transfer_due_at(timestamptz)');
  v_src text;
  v_bad text;
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_catalog.pg_proc p WHERE p.oid = v_oid AND p.prosecdef
    ...
    RAISE EXCEPTION '事後閘①:admin_create_manual_order 不是 SECURITY DEFINER / proconfig 恰好 search_path 空字串 / owner postgres';
  END IF;
  IF EXISTS (
    SELECT 1 FROM pg_catalog.pg_proc p, _p02a_attr_before b
     WHERE p.oid = pg_catalog.to_regprocedure('public.admin_create_manual_order(uuid, uuid, text, text, text, text, jsonb, jsonb, integer, jsonb, text, text, jsonb)')
  ...
  ) OR NOT EXISTS (SELECT 1 FROM _p02a_attr_before) THEN
    RAISE EXCEPTION '事後閘①:admin_create_manual_order 的 ACL / proconfig / SECURITY DEFINER / owner 改前改後不同(不准放寬也不准收緊)';
  END IF;
  SELECT p.prosrc INTO v_src FROM pg_catalog.pg_proc p WHERE p.oid = v_oid;
  IF pg_catalog.md5(v_src) <> 'c280e7e6f35660fc454e1754833c3981' THEN
    RAISE EXCEPTION '事後閘②:admin_create_manual_order md5 = % 不是本代 c280e7e6f35660fc454e1754833c3981', pg_catalog.md5(v_src);
  END IF;
  IF pg_catalog.strpos(v_src, 'pg_advisory_xact_lock(pg_catalog.hashtextextended(p_customer_user_id::text, 0))') = 0 THEN
    RAISE EXCEPTION '事後閘②:函式體裡沒有客人層級 advisory lock';
  END IF;

  IF v_hoid IS NULL THEN
    RAISE EXCEPTION '事後閘③:pcm_bank_transfer_due_at 不在';
  END IF;
  IF NOT EXISTS (SELECT 1 FROM pg_catalog.pg_proc p WHERE p.oid = v_hoid AND NOT p.prosecdef AND p.provolatile = 's'
                  AND p.proconfig = ARRAY['search_path=""'] AND pg_catalog.pg_get_userbyid(p.proowner) = 'postgres'
                  AND pg_catalog.md5(p.prosrc) = '9c4775d65626d189fe3881ae6fe16b8b') THEN
    RAISE EXCEPTION '事後閘③:helper 不是 INVOKER / STABLE / search_path 空字串 / owner postgres / 本片那一版';
  END IF;
  SELECT pg_catalog.string_agg(COALESCE(r.rolname, 'PUBLIC'), ', ') INTO v_bad
   ...
   WHERE p.oid = v_hoid AND a.grantee <> p.proowner;
  IF v_bad IS NOT NULL OR pg_catalog.has_function_privilege('anon', v_hoid, 'EXECUTE')
     OR pg_catalog.has_function_privilege('authenticated', v_hoid, 'EXECUTE')
     OR pg_catalog.has_function_privilege('service_role', v_hoid, 'EXECUTE') THEN
    RAISE EXCEPTION '事後閘③:helper 除 owner 外有人拿得到 EXECUTE(%)', v_bad;
  END IF;

  -- ④ 算得對不對:四個台北時刻同一天 ⇒ 同一個界;前一天 23:59:59 ⇒ 早一天;UTC 16:30 = 台北隔天 00:30 ⇒ 晚一天
  IF public.pcm_bank_transfer_due_at('2026-09-05 00:00:00+08') <> '2026-09-11 00:00:00+08'::timestamptz
     OR public.pcm_bank_transfer_due_at('2026-09-05 00:01:00+08') <> '2026-09-11 00:00:00+08'::timestamptz
     OR public.pcm_bank_transfer_due_at('2026-09-05 12:00:00+08') <> '2026-09-11 00:00:00+08'::timestamptz
     OR public.pcm_bank_transfer_due_at('2026-09-05 23:59:59+08') <> '2026-09-11 00:00:00+08'::timestamptz
     OR public.pcm_bank_transfer_due_at('2026-09-04 23:59:59+08') <> '2026-09-10 00:00:00+08'::timestamptz
     OR public.pcm_bank_transfer_due_at('2026-09-05 16:30:00+00') <> '2026-09-12 00:00:00+08'::timestamptz THEN
    RAISE EXCEPTION '事後閘④:pcm_bank_transfer_due_at 算出來的界不對';
  END IF;
  IF public.pcm_bank_transfer_due_at('2026-09-05 12:00:00+08') = '2026-09-10 00:00:00+08'::timestamptz THEN
    RAISE EXCEPTION '事後閘④(負對照):錯的答案居然相等 ⇒ 這把尺壞了';
  END IF;
  RAISE NOTICE '20260915233000 貼好了:pcm_bank_transfer_due_at 在;admin_create_manual_order 建單前拿客人層級鎖。';
END
$post$;

-- ── 收權斷言(migration-static-checks 規則③:新物件要列進清單)──
DO $acl$
DECLARE
  v_functions text[] := ARRAY['public.pcm_bank_transfer_due_at(timestamptz)']::text[];
  r text;
BEGIN
  FOREACH r IN ARRAY v_functions LOOP
    IF pg_catalog.to_regprocedure(r) IS NULL THEN
      RAISE EXCEPTION '收權斷言:% 不存在', r;
    END IF;
    IF pg_catalog.has_function_privilege('anon', pg_catalog.to_regprocedure(r), 'EXECUTE')
       OR pg_catalog.has_function_privilege('authenticated', pg_catalog.to_regprocedure(r), 'EXECUTE')
       OR pg_catalog.has_function_privilege('service_role', pg_catalog.to_regprocedure(r), 'EXECUTE') THEN
      RAISE EXCEPTION '收權斷言:% 對 anon / authenticated / service_role 仍可執行', r;
    END IF;
  END LOOP;
END
$acl$;
