...
--  ✅ **跨塊那一維, 兩種形狀都驗過了**(而這是本檔最容易被漏掉的一維):
--       🟢 品牌塊 + 四欄塊  G4  ['akrapov','排氣管']                    ⇒ **555 筆**, 兩向 0 差
--       🟢 料號塊 + 四欄塊  G5b ['fcap03','油箱蓋']                     ⇒ 1 筆, 兩向 0 差
--                           G5c ['FCAP-03','油箱蓋']                    ⇒ 1 筆, 兩向 0 差
--                           G5d ['fcap2b002','快拆']                    ⇒ 1 筆, 兩向 0 差
--     🛑 ⛔ ~~G5 `['fcap06','碳纖維']`~~ ⇒ **它兩邊都回 0 列 ⇒ 零判別力, 不是「驗過了」**
--        —— 那是兩個空集合互相對帳。**留著這一行**, 因為下一個人很可能會再挑一組這種詞。
--        🔵 G5b/c/d 取代它的方式就是「**有結果**」:先去查真的對得上的商品名再組詞
--           (`FCAP-03 ⇒ 油箱蓋` · `FCAP2-B002 ⇒ FCAP2 快拆式油箱蓋`, 主視窗 2026-09-04 實查)。
--  🛑 **本檔沒有比對「改前 / 改後客人看到的排序」** —— 本函式只回 id, 排序在呼叫端
--  🛑🛑 **「把品牌那塊併回 OR」這個回歸, 本檔【沒有任何東西擋得住】**:
--     它**行為零改變** ⇒ 集合差全 0、三綠全綠、每一格事後閘都過 ——
--     🔴 **它只會讓中文搜尋從 21ms 掉回 243ms, 而沒有任何一個測試會紅。**
--     ⇒ 📌 唯一守得住它的是【效能斷言】, 而我們沒有(正式站的 EXPLAIN 不在 CI 裡)。
--     ⇒ ⇒ 🔵 今天守它的只有:本檔 `bh` 那段註解 + `scripts/storefront-search-split-reconcile.sh` ⑤ 那格
--        (鑽機層:fn_old 含 b.name ⇒ trgm +0 · 新案 ⇒ +4)。**那是提醒, 不是閘。**。
--
-- ══════════════════════════════════════════════════════════════════════════
