...
--
--   🔴 **那三句逐字寫在這裡，不要靠任何暫存檔**（2026-08-19 補；原本寫「改好的版本已備妥」，
--     而它備在寫這支檔的那個 session 的 scratchpad —— **那個路徑不保證還在**
--     ⇒ 下一顆會拿到一個指向不存在檔案的指標）。
--     `apps/storefront/src/data/legal-content.ts` 要改成（🔴 **本檔的 hash 就是對這三行算的**）：
--   [三句1] titleSuffix: ' — PCM MOTOR PARTS LTD',
--   [三句2] 'PCM MOTOR PARTS LTD(派達有限公司)網路交易服務條款:企業經營者資訊、契約成立、商品交付、付款方式、退貨與契約解除權、瑕疵擔保與消費爭議處理。',
--   [三句3] 'PCM MOTOR PARTS LTD(派達有限公司)隱私政策:個人資料之蒐集機構、目的、類別、利用期間與對象、您得行使之權利,以及 Cookie 與第三方服務說明。',
--     ⚠️ **行號會漂**（別人會改那支檔）⇒ **對字面找，不要對行號**：
--     找 `titleSuffix:` 那一行、以及兩句 description 的開頭字串。
--     ⚠️ 而**改完必須重跑 hash 測試對照本檔的值** —— 不相同代表那支檔在這期間被別人動過。
--
