...
-- ⇒ 🔴 **客人一定收到信, 而員工沒有任何方法阻止。**
--
-- 🔴🔴 **而上面那句話有射程, 它會在一個【具體事件】之後變假**(R3-C4):
--    片 D(`20260905130000`, `-account` 做)給 `admin_create_manual_order` 加第 11 參
--    ⇒ **那天起手動單【可以有】`notification_email`** ⇒ 「表單根本沒那個欄位」逐字變假。
--    🛑 **而它變假的那一刻不會有任何東西叫** —— 這一段是註解, 沒有測試讀它。
--    ⇒ 📌 **片 C(分流)必須把這個射程帶進去**:分流判準**不能只看 `order_source`** ——
--       否則一張**員工有填 email 的手動單**會被判成「留白 ⇒ 不寄」而**壓掉他填的那個值**。
--       ✅ 正確判準是 **`order_source` 是 manual_* 【而且】`notification_email` 為空 ⇒ 不寄**。
--       (兩個條件, 不是一個。而今天只看第一個也對, 因為第二個恆真 —— 那正是它危險的地方。)
--
...
-- ✅ **回退腳本已經寫好而且【跑過】**:`scripts/20260905080000-down.sql`
--    ⛔ ~~(220 行)~~ ⛔ ~~(251 行)~~ —— 🔴 **這個數字我寫了兩次, 兩次都在下一次編輯後過期**
--    (R3-C2 抓到第一次;而我改成 251 之後, 補 R3-C3 那段又讓它變成 261)。
--    ⇒ 📌 **修法不是把數字改對, 是【不要在散文裡寫它】** —— 一個沒有東西在維護的計數,
--       每一次編輯都讓它更假, 而它讀起來永遠一樣有把握。要行數就 `wc -l` 當場數。。
