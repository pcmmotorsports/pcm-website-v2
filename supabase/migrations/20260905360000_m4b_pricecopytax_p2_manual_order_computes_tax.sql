-- ══════════════════════════════════════════════════════════════════
-- M-4b · ⟦b4-PRICECOPYTAX⟧ 片二:手動建單【真的算稅】(RPC 第 6 代)
-- ══════════════════════════════════════════════════════════════════
-- 真權威 plan:`docs/plans/2026-09-05-manual-order-tax-computed-plan.md`(過 codex 關卡1 兩輪, 12 must-fix + 2 nit 全折)。
-- Sean 2026-09-05 逐字批:手動單算稅 `甲`。鐵則 8(已批)+ 12①(錢)③(DB 結構)。
-- ⛔ **本檔不由施工窗 apply** —— apply 要 Sean 在場。
--
-- ── 這一支做四件事(而它們必須在同一個交易裡)────────────────────
--   ① `orders` 加 `price_tax_mode`(封閉兩值 + CHECK + DEFAULT 'inclusive')
--   ② `admin_create_manual_order` 第 6 代:**真的算稅**, 並顯式寫 `price_tax_mode='exclusive'`
--   ③ `admin_order_list_v` 尾巴加第 44 欄 `price_tax_mode`
--   ④ 🔴 `admin_update_order_item_amount`:**有稅的單擋下改價**(見該處註解;這是刻意的功能缺口)
-- 🔵 **四支全是 `CREATE OR REPLACE` / `ADD COLUMN`, 零 `DROP`** ⇒ ACL 與 `COMMENT ON` 全部原地保留。
--
-- ── 🔴 為什麼要 `price_tax_mode` 這一欄 ─────────────────────────
-- 本片之後, 手動單的 `unit_price`/`subtotal` 是**未稅**, 而顧客站的是**含稅**(Sean 0829)。
-- 🛑 **而今天沒有任何一個欄位說得出來一張單的 `subtotal` 含不含稅** ——
--    `tax_total = 0` **分不出**「舊單(價含稅)」與「新單但真的免稅」。
-- ⇒ 誰會被咬到:任何**拿 `subtotal` 反推稅**、或**拿 `unit_price` 比價**的人(報表/對帳/退款/CSV)。
--
-- 🔴 **DEFAULT `'inclusive'` 會讓沒改到的 writer 靜靜冒充含稅** ⇒ 所以 plan §9-1 要求**先盤點所有 writer**。
-- 🔬 **盤點做了(2026-09-05 唯讀正式庫, 走 `pg_proc.prosrc`, 不是 grep 檔案)**:
--    今天活著的 `orders` writer = **3 支** —— `admin_create_manual_order`(11 參)
--    · `create_order`(**10 參與 11 參兩支多載**, 見 `20260904020000` 檔頭「兩支並存」)
--    🔬 正對照:同一把尺量 `order_items` 的 writer ⇒ 3(尺會動);負對照 假表 ⇒ 0
--    ⇒ ✅ **另外兩支是顧客站, 而顧客站的單價【真的含稅】** ⇒ **DEFAULT 對它們是對的, 不會說謊。**
--    ⚠️ **而非函式的 writer(直接下 SQL / dashboard)這把尺看不到** ⇒ 那一格**未查**。
--
-- ── 🔴 稅怎麼算(而型別決定了合不合法)─────────────────────────
--   稅基 = subtotal + shipping_fee − discount_total   (本函式的 discount_total 寫死 0)
--   tax  = ROUND(稅基::numeric × 0.05)
--   total = 稅基 + tax
-- 🔴 **先轉 `numeric` 再 `ROUND`**:`numeric` 的 `ROUND` 對 `.5` 是**遠離零**(四捨五入),
--    而 `double precision` 是**銀行家捨入**(進偶數)。營業稅尾數依財政部法規是四捨五入
--    ⇒ 📌 **選錯型別 = 不合法, 那不是風格。**
--
-- ── Rollback ──────────────────────────────────────────────────
--   ✅ 可跑的版本:`supabase/rollbacks/20260905360000-rollback.sql`
--   🔴🔴 **而【只回函式】會再次少收稅** —— 畫面仍叫員工填未稅而系統又當含稅。
--      回滾是**三個動作**:①函式回第 5 代 ②`git revert` 那顆 UI 文案 ③🔴 **不要 `DROP COLUMN`**
--      (已寫進去的值會消失, 那些單就再也分不出來)。詳見 rollback 檔與 plan §8。
-- ══════════════════════════════════════════════════════════════════
...
-- ══ 前置閘 ════════════════════════════════════════════════════════════
DO $pre$
DECLARE v_n integer; v_args text; v_cols integer;
BEGIN
  -- ⓪ 本檔已套用過?(forward-only)
  IF EXISTS (SELECT 1 FROM information_schema.columns
              WHERE table_schema='public' AND table_name='orders' AND column_name='price_tax_mode') THEN
    RAISE EXCEPTION '前置閘⓪:orders 已經有 price_tax_mode ⇒ 本檔已套用過, forward-only 拒重跑';
  END IF;

  -- ① 建單 RPC 必須是【一支且 11 參】(第 5 代)
  ...
  IF v_n <> 1 OR v_args <> '11' THEN
    RAISE EXCEPTION '前置閘①:admin_create_manual_order 有 % 支(引數數 %)—— 期望剛好 1 支 11 參(第 5 代)。'
                    '🛑 多載會讓呼叫端拿 is not unique。', v_n, v_args;
  END IF;

  -- ② view 必須是 43 欄(`20260905230000` 貼過了)
  SELECT count(*) INTO v_cols FROM information_schema.columns
   WHERE table_schema='public' AND table_name='admin_order_list_v';
  IF v_cols <> 43 THEN
    RAISE EXCEPTION '前置閘②:admin_order_list_v 有 % 欄, 期望 43 ⇒ 20260905230000 沒貼或又被動過。'
                    '🛑 本檔要在它尾巴附加第 44 欄, 基線對不上就不能貼。', v_cols;
  END IF;

  -- ③ 改價那支必須是【一支】—— 而上一版寫成 `IS NULL AND NOT EXISTS` ⇒ 錯簽章/多載也放行
  --    🔴 codex 2026-09-05 抓到, 它對:那個 `AND` 讓「有任一同名函式」就過
  --    ⇒ 舊多載留著、本檔再建一支 ⇒ **PostgREST 呼叫變歧義**。
  ...
  IF v_n <> 1 THEN
    RAISE EXCEPTION '前置閘③:admin_update_order_item_amount 有 % 支, 期望剛好 1 支 ⇒ 基線對不上', v_n;
  END IF;
END
$pre$;

-- ══ ① orders 加 price_tax_mode ════════════════════════════════════════
-- 🔴 **封閉值域 CHECK 是必要的, 不是保險** —— 沒有它, 拼錯字也進得去,
--    而一個拼錯的值會被每一個讀它的人當成「不是這兩種」而**各自處置**。
ALTER TABLE public.orders
  ADD COLUMN price_tax_mode text NOT NULL DEFAULT 'inclusive';

ALTER TABLE public.orders
  ADD CONSTRAINT orders_price_tax_mode_domain
  CHECK (price_tax_mode IN ('inclusive', 'exclusive'));

COMMENT ON COLUMN public.orders.price_tax_mode IS
  '這張單的 unit_price / subtotal 是含稅還是未稅。'
  'inclusive = 含稅(顧客站 create_order, 以及本欄加上去之前的【所有】既有單);'
  'exclusive = 未稅、稅另計(2026-09-05 起的後台手動單)。'
  '🔴 DEFAULT 是 inclusive ⇒ 任何沒有顯式寫這一欄的 writer 寫進來的單會【自稱含稅】。'
  '⇒ 新增任何 orders 的 writer 時, 必須決定它寫哪一個值 —— 不寫等於宣稱含稅。'
  '🔬 2026-09-05 盤點:今天活著的 writer 3 支(admin_create_manual_order 寫 exclusive;'
  'create_order 兩支多載走 DEFAULT, 而顧客站的價【真的含稅】⇒ 對)。';

-- ══ ② 建單 RPC 第 6 代 ════════════════════════════════════════════════
-- 🔵 參數列**一個字未動**(仍 11 參)⇒ `CREATE OR REPLACE` 是【取代】不是多載
--    ⇒ ACL 與 `COMMENT ON` 原地保留, 不必重貼。
-- 🔬 函式本體**程式抽出、零手抄**:`20260905130000:120-805` 原樣, 只插五處
--    (`DECLARE` 加 `v_tax` / 稅算式 + 負稅基閘 + 溢位加 `v_tax` /
--     INSERT 欄位加 `price_tax_mode` / `tax_total` 的 `0` 換成 `v_tax` / 值列尾加 `'exclusive'`)。
...
DECLARE
  ...
  v_tax         bigint;
  -- 🔴 **這個值出現在兩個地方**(INSERT 的值列 + audit payload)⇒ 收成一個常數。
...
--     ④ 後來被 ⑦c 取代 —— 理由見那一段。)
  ...
  o.tax_total,
  o.invoice_requested,
  -- 🔴 **第 44 欄, 而它【必須在最後】** —— `CREATE OR REPLACE VIEW` 只能在尾巴附加。
...
-- ══ ④ 改價那支:有稅的單擋下來 ════════════════════════════════════════
-- 🔬 本體**程式抽出、零手抄**:`20260816040000_m4b_e10_13_518_p2c13_detail.sql:45-211` 原樣, 只插一道 4d-2。
...
-- ══ 事後斷言 ══════════════════════════════════════════════════════════
DO $post$
DECLARE v_n integer; v_cols integer; v_oid oid;
BEGIN
  -- ① 欄在, 而且 **NOT NULL + DEFAULT 都要對**(codex #8:上一版只問「欄在不在」)
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns
                  WHERE table_schema='public' AND table_name='orders' AND column_name='price_tax_mode'
                    AND is_nullable = 'NO'
                    AND column_default LIKE '%inclusive%') THEN
    RAISE EXCEPTION '斷言①:price_tax_mode 不存在, 或它可以是 NULL, 或 DEFAULT 不是 inclusive';
  END IF;

  -- ② CHECK 在, 而且**掛在對的表上、述詞是那一條、而且 validated**(codex #8)
  --    🔴 只問 `conname` 存不存在 ⇒ 一條**掛在別張表**或**述詞被改掉**的同名約束照樣過。
  IF NOT EXISTS (
        SELECT 1 FROM pg_catalog.pg_constraint c
          JOIN pg_catalog.pg_class t ON t.oid = c.conrelid
          JOIN pg_catalog.pg_namespace n ON n.oid = t.relnamespace
         WHERE c.conname = 'orders_price_tax_mode_domain'
           AND c.contype = 'c'
           AND n.nspname = 'public' AND t.relname = 'orders'
           AND c.convalidated
           AND pg_catalog.pg_get_constraintdef(c.oid) LIKE '%inclusive%'
           AND pg_catalog.pg_get_constraintdef(c.oid) LIKE '%exclusive%') THEN
    RAISE EXCEPTION '斷言②:封閉值域 CHECK 不見了 / 掛在別張表 / 述詞被改 / 沒 validated';
  END IF;

  -- ③ 🔴 CHECK 真的會擋 —— 而**這一格上一版是零判別力的**
  --    ⛔ ~~`UPDATE public.orders SET price_tax_mode='zzz' WHERE false`~~
  --    🔴 codex 2026-09-05 抓到, 它對:**`WHERE false` 不會評估任何一列的 CHECK**
  --       ⇒ **沒有例外也算通過** ⇒ CHECK 改成恆真或整個失效, 八道照樣全綠。
  --    ⇒ 📌 **我寫了一格「看起來在驗行為」的斷言, 而它在兩個世界印同一個東西。**
  --    ✅ 改成:**對一列真的下 UPDATE**, 而整個包在 SAVEPOINT 裡回滾 ⇒ 零留痕。
  --    ⚠️ 而它要求庫裡**至少有一列** —— 零列時**明說量不到**, 不要靜靜當成過。
  DECLARE v_probe uuid;
  BEGIN
    SELECT id INTO v_probe FROM public.orders LIMIT 1;
    IF v_probe IS NULL THEN
      RAISE WARNING '斷言③:orders 零列 ⇒ 這一格【量不到】CHECK 會不會擋(不是通過)';
    ELSE
      BEGIN
        UPDATE public.orders SET price_tax_mode = 'zzz_not_valid' WHERE id = v_probe;
        -- 走到這裡 = CHECK 沒擋
        RAISE EXCEPTION '斷言③失敗:拼錯的值寫得進去 ⇒ 封閉值域 CHECK 沒有作用';
      EXCEPTION
        WHEN check_violation THEN NULL;  -- ✅ 預期:被擋
      END;
      -- 🔴 正對照:合法的值必須寫得進去(否則我擋的是【全部】而不是【拼錯的】)
      BEGIN
  ...
  -- ④ 建單 RPC 仍是【一支 11 參】(沒有變成多載)
  SELECT count(*) INTO v_n FROM pg_catalog.pg_proc p
    JOIN pg_catalog.pg_namespace n ON n.oid=p.pronamespace
   WHERE n.nspname='public' AND p.proname='admin_create_manual_order';
  IF v_n <> 1 THEN
    RAISE EXCEPTION '斷言④:admin_create_manual_order 變成 % 支 ⇒ 多載, 呼叫端會拿 is not unique', v_n;
  END IF;

  -- ⑤ 函式體裡真的有稅 —— 🔴 **而這一格【只搜 token】, 它的判別力就是這麼窄**(codex #8)
  --    ⛔ **註解裡寫 `v_tax` 也會讓它綠**;不可達的分支也會。
  --    ⇒ 📌 **它證的是「這支函式的原始碼裡有那幾個字」, 不是「稅算對了」。**
  --    ✅ 收窄一格:搜的是**剝掉行註解之後**的 body, 而且要求那兩個**可執行的形狀**都在
  --       (`v_tax :=` 的指派、`'exclusive'` 的字面)。
  --    🛑 **而它仍然證不到算式對不對** —— 那要真的建一張單, 見檔尾那段。
  IF NOT EXISTS (
        SELECT 1 FROM pg_catalog.pg_proc p
          JOIN pg_catalog.pg_namespace n ON n.oid=p.pronamespace
         WHERE n.nspname='public' AND p.proname='admin_create_manual_order'
           AND pg_catalog.regexp_replace(p.prosrc, '--[^\n]*', '', 'g') LIKE '%v_tax :=%'
           AND pg_catalog.regexp_replace(p.prosrc, '--[^\n]*', '', 'g') LIKE '%v_price_tax_mode constant text := ''exclusive''%') THEN
    RAISE EXCEPTION '斷言⑤:建單函式體(剝註解後)沒有 v_tax 的指派或 exclusive 的字面 ⇒ 第 6 代沒貼進去';
  END IF;
  ...
  SELECT count(*) INTO v_cols FROM information_schema.columns
   WHERE table_schema='public' AND table_name='admin_order_list_v';
  IF v_cols <> 44 THEN
    RAISE EXCEPTION '斷言⑥:view 有 % 欄, 期望 44(43 + price_tax_mode)', v_cols;
  END IF;
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns
                  WHERE table_schema='public' AND table_name='admin_order_list_v' AND column_name='price_tax_mode') THEN
    RAISE EXCEPTION '斷言⑥b:view 沒有 price_tax_mode';
  END IF;

  -- ⑦ view 的 security_invoker 與唯讀 ACL 沒掉(CREATE OR REPLACE 應該保留, 而這一格證它)
  ...
  IF NOT EXISTS (SELECT 1 FROM pg_catalog.pg_class c
    ...
    RAISE EXCEPTION '斷言⑦:security_invoker 不見了';
  END IF;
  IF NOT pg_catalog.has_table_privilege('pcm_readonly', v_oid, 'SELECT') THEN
    RAISE EXCEPTION '斷言⑦b:pcm_readonly 讀不到這支 view 了';
  END IF;

  -- ⑧ 改價那支真的有那道稅閘
  -- ⑧ 改價那支的稅閘 —— 同樣**剝註解**再搜, 而且要求**判準那一行**在(不是只有錯誤碼)
  IF NOT EXISTS (
        SELECT 1 FROM pg_catalog.pg_proc p
          JOIN pg_catalog.pg_namespace n ON n.oid=p.pronamespace
         WHERE n.nspname='public' AND p.proname='admin_update_order_item_amount'
           AND pg_catalog.regexp_replace(p.prosrc, '--[^\n]*', '', 'g') LIKE '%pcm_e13_no_edit_when_taxed%'
           AND pg_catalog.regexp_replace(p.prosrc, '--[^\n]*', '', 'g') LIKE '%price_tax_mode = ''exclusive''%') THEN
    RAISE EXCEPTION '斷言⑧:改價那支沒有稅閘, 或它的判準不是 price_tax_mode ⇒ 稅基 0 的未稅單會穿過去';
  END IF;
END
$post$;

-- 🛑 **上面八道斷言【證不到】稅算得對不對** —— 它們看的是 catalog 的形狀與函式體的字面。
--    要證明算式對, 得真的建一張單;而那要 `service_role` 身分 ⇒ 貼後對帳另檔。
--    📌 **「rc=0 八道全過」與「稅是對的」是兩個宣稱。**

-- 🔴 orders 加欄 + view 加欄 + 函式體改 ⇒ PostgREST 的 schema cache 要重載。
