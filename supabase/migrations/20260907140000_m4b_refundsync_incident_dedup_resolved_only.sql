-- 🔴🔴 **貼之前先讀這一句(主視窗 A 指定放在板頭)**
-- **今天 no-op:`public.pcm_incident.resolved_at` 全 repo 沒有任何寫入端**
-- (3 處 `SET resolved_at` 都在另一張表 `payment_double_charge_anomalies`;
--  正對照同一把尺換 `voided_at` = 5 處 ⇒ 那個 0 不是尺沒接上)
-- ⇒ 本支**先關掉未來的漏報**, 在有人做「結案」那件事的那天**自己生效**。
-- 🛑 **字面紀律**:貼完講這一支, 一律說「**先關掉未來的漏報**」, **不要說「修好了」** ——
--    「修好了」會讓人以為有一個正在發生的漏報被止住了, 而今天沒有那條路。
-- ⚠️ 完整的「它證不到什麼」在本檔最末一節, 不要只讀這五行。
