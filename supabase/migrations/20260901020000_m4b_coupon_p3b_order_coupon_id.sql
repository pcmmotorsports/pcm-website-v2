...
--
-- ── 🛑🛑 先讀這一段:**`d1-export.ts` / `d1-restore.ts` 【不是】我們的備份系統。**────
--    檔名讀起來像備份, 而它們是**一次性的 cohort 匯出** —— `d1-export.ts:1-3` 逐字
--    「D1a1:cohort 匯出腳本產生器」,`d1-cohort.ts:1-2` 逐字「D1c 會永久刪除的 **26 張訂單**
--    + 必須留下的 **3 張**」, cohort 是**寫死的 29 個 uuid**(我數過)。
--    沒有排程在跑它:`.github` 提到 `d1-export` ⇒ 0、`package.json` ⇒ 0
--      ⚠️ 而那兩個 0 的正對照**不合格**(`.github` 提到 `npx tsx` 也是 0 ⇒ 同族全 0)
--      ⇒ 只能說到「這兩個地方沒有」, 不能說「不存在」。
-- 🔴 **⇒ 而【我們的常態備份是什麼】= 未確認, 而它不在 repo 裡。**
--    主視窗 `-24` 2026-09-01 用 Supabase MCP 查過:`get_project` **不吐備份 / PITR 設定**
--    (那住在 dashboard 的 Database → Backups)⇒ **到它那裡就停了。已端進 Sean 的早報。**
--    ⇒ 📌 **「未確認」不是「沒有」。**而在他答之前, `d1-export` / `d1-restore` 一個字都不要動。
-- 📌 **這一段寫在這裡的用途:擋住下一個人重走我今晚那條路** ——
--    他會看到那個檔名, 然後以為那是備份, 然後對著一條凍結的災難還原腳本動手。
--    **而我會發現, 只是因為我去讀了它的檔頭第一行。**
--
-- ── 🔴 備份還原(B1 踩過的那個坑;而下面這一段只涵蓋 d1 那一支)────────────────
...
COMMENT ON COLUMN public.orders.coupon_id IS
  ...
  '🔴 【券是誰扣的】:trigger public.coupon_redeem_on_paid()(落點 20260901021000), '
  '   在 unpaid⇒paid 那一次轉換時呼 redeem_coupon 真的扣名額。**不是 confirm_order_payment 做的** —— '
  '   ⛔ ~~全 repo 至少五處不同函式會 SET payment_status=''paid'', 只補一支會漏掉後台手動收款那條路。~~ '
  ...
  '     🎯 ⇒ 而下面那句錯的註解【引用的正是那支檔】。⇒ 它被讀到了, 而它沒有擋住任何事。'
  '   🔴 而數字那一格:【五處不同函式】是錯的, 真值是【2 支函式】—— '
  '     ① public.confirm_order_payment(共 4 代 CREATE OR REPLACE;newest = live = 20260810170000)'
  '     ② public.settle_zero_total_order(20260901030000:1497, 零元單)'
  '   🎯 而【它錯在哪個機制】才是要寫下來的那半, 因為下一個人會用同一把 grep 拿到同一個錯數字:'
  '     · migration 目錄每一代各留一份檔 ⇒ grep 對【同一支函式的四代】印四個命中 = 結構性多報'
  '     · 20260820020000:169 與 :560 是【自檢 fixture】不是生產寫入點 —— 那兩處自己的註解逐字寫著'
  '       「把【借來的】那一列【暫時】改成目標形狀」⇒ 而 grep 讀得到那一行, 讀不到它旁邊那一行。'
  '   🟢 而【結論沒有被推翻, 反而更強】:兩支都不是後台手動收款 ⇒ 匯款那條路今天連一個寫入點都沒有 —— '
  '     那正是「員工登錄匯款之後客人收不到付款信」的成因(2026-09-02 未修, 修法待 Sean 批)。'
  '   🔵 ⇒ 所以【把扣券做成 trigger】這個決定仍然成立, 只是它原本給的理由是假的。本次不改設計。'
...
-- 🔴🔴 **而它【不再是理論】—— 2026-09-01 我自己在拋棄式 PG 上撞到了整整一次:**
--    021000 的驗收要測前置閘③, 我 drop 掉 `coupon_id` 欄 ⇒ 那兩張測試單的
--    `discount_total = 300` 留著而券的身分沒了 ⇒ 再跑本檔 ⇒
--      `ERROR: check constraint "orders_discount_needs_coupon" of relation "orders" is violated by some row`
--    ⇒ 這**就是**還原那條路的形狀:**資料還在、而它已經不滿足新的守門。**
--    ⇒ 我當時的處置是「把那兩張的折扣歸零」—— 那在拋棄式 PG 上沒有代價,
--      **而在正式庫上那叫【改了客人的訂單金額】** ⇒ 所以上面那三個選項要 Sean 拍, 不是誰順手做掉。
--    📌 **一個被寫成「已知風險」的東西, 在我自己的測試環境裡半小時內就發生了一次。**
--       ⇒ 而它之所以沒有變成事故, 只是因為那裡的資料不值錢。
