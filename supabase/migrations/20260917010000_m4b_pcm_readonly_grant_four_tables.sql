...
--   🔴 而【M-B 那一題還沒人拍】—— 見下面〈pcm_net_exposure_snapshot 這一張是一題〉。
--
...
--
-- ═══ 🛑🛑 `pcm_net_exposure_snapshot` 這一張是一題(R2 M-B, 等 Sean 拍)═══════
...
--
--   🛑🛑 **所以這一題要 Sean 拍**:他 2026-09-17 只被問過 `supplier_inbound_emails` 那一張(Q20 乙),
--      **沒被問過這一張**。端給他要編號時要把這一題明講。
--   🔴 **而不管他怎麼拍, `20260908030000:105-107` 那三行字面貼完就是假的**
--      (那張表存在的目的就是「量曝露面」—— 在它身上留一句過期的權限字面是最壞的位置)
--      ⇒ 拍乙的話, **同一顆 commit 要順手更正那三行**。
--
