...
--
-- ⚠️ **射程延伸(明寫,不藏)—— 而它與上面那一格【不同級】, 兩者不要混**:
--    折扣(Q39)與運費階梯這兩條拍板, 原情境都是**已付款後退錢**,
--    而這裡用在**未付款的匯款單** ⇒ **「未付款也照同一條嗎」沒有被單獨問過**
--    ⇒ 主視窗 B 2026-09-07 判:照做並標明。
--    🔵 對照:上面 `Q82`(總額上升要不要照寄)**是單獨問過、單獨答的** ——
--    📌 **一個是【延伸】, 一個是【拍板】, 而它們在檔案裡長得一樣** ⇒ 所以要各自標。
--
-- 🔴 **那個「總額反而變大」的邊界 —— Sean 2026-09-07 答 `Q82 = 照寄`**:
--    原單 subtotal 5,000(免運)⇒ 取消一件 50 ⇒ subtotal 4,950 跌破門檻 ⇒ 運費 +100
--    ⇒ **total 5,050,比取消前【多 50】** ⇒ 📌 **客人取消了東西,卻被叫去匯更多錢。**
--    ✅ **他的答案是「照寄」** ⇒ 🛑 **不做任何人工攔截那一層**;算式不變。
--    🔴🔴 **而這一格【是被明確問過的】, 不是我們從舊拍板外推的** ——
--      📌 差別要留著:09-05 那個「取消會補收運費」講的是**退錢**的情境,
--      而**「總額上升」那一格是 2026-09-07 單獨端出去、單獨答的**。
--      ⇒ 下一個人不會把它讀成「我們自己延伸的」而回頭去質疑它。
--
