...
--
-- ⛔⛔ ~~**草稿。未 apply。**~~ 🟢 **[2026-09-06 `-mail` 訂正(R1 opus nit)]已 apply** ——
-- 　 `grep -c '^20260906200000' supabase/APPLIED.tsv` ⇒ **1**(帳本 `:505`;🔵 負對照現造 `^20260906209999` ⇒ **0**)。
-- 　 🛑 **而帳本是自陳帳**(`scripts/deploy-order-gate.sh:61` 逐字「更新了卻沒真 apply…攔不到」)
-- 　 ⇒ **這一行說的是「帳上記了」, 不是「我去正式庫看過」。**
-- 　 📌 **舊字面留刪除線**:搜「草稿。未 apply。」的人要撞到這個訂正。 🔴 **排在 45f(`20260906190000`)之後。**
--
-- ══════════════════════════════════════════════════════════════════
-- 為什麼(而它只做拍板的那一半)
-- ══════════════════════════════════════════════════════════════════
...
①【舊資料】那封信寄出去時我們還沒開始記;
②【新資料而沒拿到】provider 沒回 / 回應超過大小上限 / 讀不完整 / 型別或格式不合。
🛑 **分界訂正(opus R2 · MF2 + C4)**:⛔ ~~「靠 `sent_at` 與 45g 的 apply 時刻比對」~~ **那是錯的**,
兩個獨立的理由:
(a) **分界是【碼上線時刻】, 不是本支 migration 的 apply 時刻。**而鐵則要求的順序是【先貼庫、後上碼】
⇒ 那個部署窗裡的每一列都 `sent_at > apply`, 而它們**全都是①** ⇒ 照舊字面稽核會把①整批讀成②,
📌 **方向與這段註解本來要防的那個誤讀【正好相反】。**
(b) **那個時刻沒有可信記錄可比** —— apply 時刻與上線時刻都沒有落在任何一張表上
(`supabase/APPLIED.tsv` 檔頭逐字:不在表上什麼都不代表)。
✅ **改用資料自己答得出來的那個分界**:`SELECT min(sent_at) FROM public.email_outbox
WHERE provider_message_id IS NOT NULL` —— 它是**可觀測的**, 不需要任何外部紀錄。
⚠️ **而它是【下界】不是精確時刻**:上線後最早那幾封若剛好都沒拿到 id, 這個值會**偏晚**
⇒ 它右邊的 NULL 一定是②, 左邊的**大多是①而混得進②** ⇒ 要斷言請只用右半邊。
📌 而稽核時把 ② 誤讀成 ① 會得到「我們一直沒開始記」這個假結論;把 ① 誤讀成 ② 則會得到
「這個功能壞了」這個假結論 —— **兩個方向都有, 而舊字面只防了其中一個。**$c$;
