...
-- M-4b-01 員工權限「改金額紅線」server-side enforce · P2(只 .sql)。
-- plan:~/pcm-design/docs/plans/2026-09-14-m4b-01-manager-redline-server-side-enforce-plan.md(設計窗 6c59deee4)
-- Sean 2026-09-14 拍:Q1 甲(改品項金額只有管理者能改)· Q2 甲(其他七件碰錢的不動)。
--
-- ══ 做什麼 ═════════════════════════════════════════════════
-- 五支 RPC 在 DB 那一層驗 `staff.is_manager AND is_active`(逐字抄 admin_staff_create 20260912050000:146-154),
-- 不是 ⇒ RAISE '無權執行此操作'(與 staff RPC 同句)。為什麼要 L3:後台 key 是 service_role, PostgREST 直打不經 TS 那道閘。
-- ① admin_update_order_item_amount(7 參)—— 第 5 代 = 第 4 代(20260909080000:122)逐字 + 閘(參數檢查之後、鎖列之前)
-- ② admin_requeue_dead_email —— 🔴 簽章 (uuid) → (uuid, text):原版沒有 p_actor。DROP 舊 + CREATE 新 + ACL 逐字搬。
--    ⚠️ TS 端 dead-letter-actions.ts 要多帶 p_actor(P3, 另一顆);P3 未合之前貼本檔 ⇒ 重排壞掉(PGRST202)。順序:貼本檔 → 合 P3。
-- ③ admin_soft_delete_order_note(5 參)—— 第 2 代 = 第 1 代(20260913020000:163)逐字 + 閘(actor 驗完之後, 用 trim 過的 v_actor)
-- ④ record_manual_cancel_notice(4 參)—— 第 2 代 = 第 1 代(20260906920000:62)逐字 + 閘(invalid_args 之後、鎖單之前)
-- ⑤ revoke_manual_cancel_notice(4 參)—— 第 2 代 = 第 1 代(20260906930000:56)逐字 + 閘(invalid_args 之後、鎖列之前)
-- 🔴 CREATE OR REPLACE 會把 SET 子句整組換掉 ⇒ 每支 header 逐字抄回(SECURITY DEFINER + SET search_path = '')。
-- 🔴 ①③④⑤ ACL 不動(CREATE OR REPLACE 保留);② 簽章變 = ACL 重做, 貼完跑 pcm_acl_approve_latest(0914 拍甲)。
--
-- ══ 回滾 ═══════════════════════════════════════════════════
-- `supabase/rollbacks/20260915040000-rollback.sql`:四支貼回上一代整支 + ② DROP 2 參版、貼回 1 參版 + 原 ACL。
-- 回滾後改金額回到「任何員工都能改」(今天的狀態)。退 ② 之前 P3 那顆 TS 要先 revert。

BEGIN;
SET LOCAL lock_timeout = '5s';

-- ── 前置閘:五支本體 md5 都要是本檔抄本的那一代(拋棄式 PG 2026-09-14 實得 = repo 重建同值)──
DO $pre$
DECLARE
  ...
  i int; v_sig text; v_md5 text; v_src text;
BEGIN
  FOR i IN 1 .. pg_catalog.array_length(v_expect, 1) LOOP
    v_sig := v_expect[i][1]; v_md5 := v_expect[i][2];
    SELECT p.prosrc INTO v_src FROM pg_catalog.pg_proc p WHERE p.oid = pg_catalog.to_regprocedure(v_sig);
    IF v_src IS NULL THEN
      RAISE EXCEPTION '前置閘一:% 不在 ⇒ 上一代還沒貼, 或簽章不是我以為的', v_sig;
    END IF;
    IF v_src LIKE '%無權執行此操作%' THEN
      RAISE EXCEPTION '前置閘二:% 已含管理者閘 ⇒ 這一片貼過了, 拒重貼', v_sig;
    END IF;
    IF pg_catalog.md5(v_src) <> v_md5 THEN
      RAISE EXCEPTION USING MESSAGE = '前置閘三:' || v_sig || ' 本體 md5 不是 ' || v_md5 || '(實得 ' || pg_catalog.md5(v_src) || ')⇒ 有人動過它, 本檔抄本基準已失效, 停下人工對齊';
    END IF;
  END LOOP;
  IF pg_catalog.to_regprocedure('public.admin_requeue_dead_email(uuid,text)') IS NOT NULL THEN
    RAISE EXCEPTION '前置閘四:admin_requeue_dead_email(uuid,text) 已經在 ⇒ 貼過了, 拒重貼';
  END IF;
  IF NOT EXISTS (SELECT 1 FROM pg_catalog.pg_attribute WHERE attrelid = 'public.staff'::regclass AND attname = 'is_manager' AND NOT attisdropped) THEN
    RAISE EXCEPTION '前置閘五:staff.is_manager 不在 ⇒ 20260829193000 還沒貼';
  END IF;
END
$pre$;

-- ── ① admin_update_order_item_amount 第 5 代 ─────────────────────────
...
BEGIN
  ...
  -- ── M-4b-01 管理者紅線(20260915040000):逐字抄 admin_staff_create 20260912050000:146-154 ─────
...
COMMENT ON FUNCTION public.admin_update_order_item_amount(uuid,uuid,integer,integer,text,text,text) IS
  '改品項單價(第 5 代 20260915040000):管理者限定(M-4b-01, Sean 09-14 拍甲)—— 非 staff.is_manager AND is_active ⇒ 無權執行此操作。其餘與第 4 代 20260909080000 逐字同。';

-- ── ② admin_requeue_dead_email 1 參 → 2 參(DROP + CREATE + ACL 逐字搬自 20260831040000:156-200)──
DROP FUNCTION public.admin_requeue_dead_email(uuid);
...
BEGIN
  ...
  END IF;
  IF p_actor IS NULL OR pg_catalog.btrim(p_actor) = '' THEN
    RAISE EXCEPTION 'admin_requeue_dead_email:缺 actor';
  END IF;
  -- ── M-4b-01 管理者紅線(20260915040000):逐字抄 admin_staff_create 20260912050000:146-154 ─────
  SELECT s.is_manager INTO v_is_manager
    FROM public.staff s WHERE s.id = p_actor AND s.is_active
...
ALTER FUNCTION public.admin_requeue_dead_email(uuid, text) OWNER TO postgres;
COMMENT ON FUNCTION public.admin_requeue_dead_email(uuid, text) IS
  'M-4b ⟦b4-MAILDEAD⟧:把一封已經放棄的信(attempts >= max_attempts)原地翻回 pending 重排。'
  '第 2 代 20260915040000:多 p_actor, 管理者限定(M-4b-01)—— 非 staff.is_manager AND is_active ⇒ 無權執行此操作。'
...
REVOKE ALL ON FUNCTION public.admin_requeue_dead_email(uuid, text) FROM PUBLIC;
REVOKE ALL ON FUNCTION public.admin_requeue_dead_email(uuid, text)
  FROM anon, authenticated, payment_confirmer;
GRANT EXECUTE ON FUNCTION public.admin_requeue_dead_email(uuid, text) TO service_role;

-- ── ③ admin_soft_delete_order_note 第 2 代 ───────────────────────────
...
BEGIN
  ...
  -- ── M-4b-01 管理者紅線(20260915040000):逐字抄 admin_staff_create 20260912050000:146-154 ─────
  --    codex R1 must-fix:放在【全部參數檢查之後、鎖 orders 之前】—— 非管理者的無效輸入仍回 INVALID_INPUT / REASON_TOO_LONG, 不變成權限例外。
  SELECT s.is_manager INTO v_is_manager
...
-- ── ④ record_manual_cancel_notice 第 2 代 ────────────────────────────
...
-- ── ⑤ revoke_manual_cancel_notice 第 2 代 ────────────────────────────
...
DO $post$
DECLARE
  -- 收權斷言清單(靜態閘 ③ 數的是這個陣列;可授權物件 1 = 新簽章的 ②;①③④⑤ 是 OR REPLACE 保留 ACL, 一起列進來多驗不擋)
  v_functions text[] := ARRAY[
    'public.admin_update_order_item_amount(uuid,uuid,integer,integer,text,text,text)',
    'public.admin_requeue_dead_email(uuid,text)',
    'public.admin_soft_delete_order_note(uuid,uuid,text,text,text)',
    'public.record_manual_cancel_notice(uuid,text,text,text)',
    'public.revoke_manual_cancel_notice(uuid,uuid,text,text)'
  ]::text[];
  r text; v_src text; v_config text[]; v_secdef boolean;
BEGIN
  FOREACH r IN ARRAY v_functions LOOP
    ...
    END IF;
    IF v_src NOT LIKE '%無權執行此操作%' THEN
      RAISE EXCEPTION '後置閘二:% 本體沒有管理者閘', r;
    END IF;
    IF NOT v_secdef OR v_config IS NULL OR NOT (v_config @> ARRAY['search_path=""']) THEN
      RAISE EXCEPTION '後置閘三:% 不是 SECURITY DEFINER + SET search_path = ''''(CREATE OR REPLACE 把 SET 子句換掉了)', r;
    END IF;
    ...
    END IF;
  END LOOP;
  IF pg_catalog.to_regprocedure('public.admin_requeue_dead_email(uuid)') IS NOT NULL THEN
    RAISE EXCEPTION '後置閘六:1 參版 admin_requeue_dead_email 還在 ⇒ 兩支多載並存(⟦db-SAMETRIGGERNAME⟧ 那一族)';
  END IF;
  -- 正對照:範本函式含同一句;負對照:現造字面不含。
  SELECT p.prosrc INTO v_src FROM pg_catalog.pg_proc p WHERE p.oid = pg_catalog.to_regprocedure('public.admin_staff_create(text,text,text,boolean,text)');
  IF v_src IS NULL OR v_src NOT LIKE '%無權執行此操作%' THEN
    RAISE EXCEPTION '後置閘七:正對照 admin_staff_create(text,text,text,boolean,text) 不在或不含那一句 ⇒ 這把尺壞了';
  END IF;
END
$post$;
