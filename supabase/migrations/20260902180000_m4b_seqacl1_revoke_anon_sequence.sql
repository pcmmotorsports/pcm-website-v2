...
--     🔴 **而冒號後面【不可以是空的】** —— 空的不算合法標記, 而那支腳本會出聲說「標記不完整」
--        並把本支留在 ④。📌 那是刻意的:**一個打錯的標記與一支真的待套檔, 不可以印同一個結果。**
--
-- ══ 🔴🔴 為什麼是【⑩ 不是 ⑨】—— 兩者的差別不是分類 ═══════════════════════════
--   ⑨(`never-apply`)那幾支在**複製正式庫現況** ⇒ 它們結構上**永遠不會**變成「要貼」。
--   🔴 **而本支是一支【真的要 apply】的安全片** —— 它被自己的前置閘擋下, 因為那四支序列
--     **已經被【不知道是誰】收乾淨了**(2026-09-02 20:0x;而「是誰收的」證不出來,
--     理由是結構性的:**正式庫的 ACL 沒有歷史**)。
--   ⇒ 🎯 **所以有人把權限改回去, 本支【會重新變成要貼】** —— 而 ⑨ 不會。
--   ⇒ ⇒ 📌 **那就是為什麼這個標記【必須帶一句可執行的複查方法】**:
--      一個靜態標記只記得【當初為什麼不貼】, 它答不出【它今天有沒有重新變成要貼】。
--   🛑 **而加這個標記【不會】讓 `⟦b4-SEQACL1⟧` 那一列變成 done** —— 那一列自己寫著
--      「**不得標【已解決】**:根因(default ACL)沒有被任何人修掉 ⇒ 下一支新序列還是會漏」。
--      ⇒ **標記管的是「這支檔要不要貼」, 不是「那件事解決了沒」。兩件事不要綁在一起。**
--
-- ══ 🔴 一句 Sean 讀得懂的話 ═══════════════════════════════════════════════════
