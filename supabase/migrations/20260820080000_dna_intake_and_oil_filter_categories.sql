...
--      實查(2026-08-20,W5 盲審打掉我原本那條命令後重量):
--        `git grep -c 'CREATE TABLE.*category_taxonomy_map'` ⇒ **全 repo 零檔命中**
--        (正向對照 `CREATE TABLE` ⇒ 37 檔、負向對照 `category_zzz_map` ⇒ 0 ⇒ 尺是活的)
--      🔴 **不要用 `grep -rl 'category_taxonomy_map'` 當判準,它有兩個假訊號**:
--        ① 掃 `supabase/migrations` ⇒ 命中 **1**,而那 1 就是**本檔自己**(:10 :20-24)
--        ② 掃全 repo ⇒ 命中 **5**,其中 `docs/security/2026-08-17-quote-db-*.md` 兩支逐字列著
--           「category_taxonomy_map | RLS=true | 欄數=5」—— 那兩份的檔頭第一行寫的是
--           **「報價單庫 production schema 盤點」**,量的是 `pcm-quote-v2`,**不是顧客站這一庫**。
--        ⇒ 下一個人照原本那條命令重跑,會拿到 1 或 5,**兩個都會讀成「表在」**。
...
--      🔴 **不跟進不會讓任何東西不可達 —— 而那句話有前提**(W5 2026-08-20 盲審要求寫成條件式):
--        來源側是純 `ADD COLUMN`、真權威留在報價單 `20260820120000` ⇒ 值沒有遺失。
--        ⇒ 正確寫法是「**不可達 = 否,前提是報價單那側繼續保有那個欄位**」,
--          不是「不可達 = 否」。前提哪天不成立,這句就要跟著失效。
...
-- ② 六個子類都在,而且 parent 掛對
--    🔴 **射程(W5 2026-08-20 盲審打掉我原本那句)**:它證的是「**這 6 列都在、且掛對**」,
--    而它抓得到的失敗只有一種 —— **upsert 沒跑到**(JOIN 找不到 parent ⇒ 那列插不進去 ⇒ count < 6)。
--    **它抓不到「掛錯 parent」**,而且是**不可能抓到**,因為:
--      · 清單內的 6 列 —— §2 的 upsert 剛把 parent 修成正確值 ⇒ 這裡不可能看到掛錯的
--      · 清單外的列(例如既有的 `進氣系統 · 別的` 掛去別處)⇒ parent 不等 ⇒ **根本不進 count**
--    ⇒ 我原本在這裡寫「掛錯 parent 的那一列 raw_path 照樣長得對」,那句話與下面 §3③ 直接矛盾,
--      而**留在這裡的那句比較顯眼** ⇒ 已改寫。
--    📌 可搬走的:**拿掉一個恆真守門時,它的兄弟句子要一起檢查** —— 我移了程式碼(§3③),
--      而宣稱留在註解裡繼續生效,長得像一個還在守的東西。
--
--    ⚠️ 分母釘死在這 6 個 raw_path 字面(W5 nit-1):原本用 `LIKE '進氣系統 · %'`,
--    分母是「進氣系統底下**所有**子類」而期望值是「我插的那 6 列」
--    ⇒ 日後有人加第 7 個子類、又單獨手動重跑本檔 ⇒ **紅在 7 <> 6**,而那不是缺陷。
--    (依序重放沒事;檔頭那句「可重播」在**手動重跑**這一格上比它的字面窄。)
--
--    ✅ **兩個世界都表演過**(2026-08-20 拋棄式 PG 17.10,port 55621,同一個 template 複製出的庫):
--      · 同一個已有第 7 個子類的庫 ⇒ **舊版(`754dbe65` 的 LIKE 寫法)紅在「實得 7」**、
--        **新版(本檔的 IN 字面清單)綠** ⇒ 差異來自這次的改動,不是來自別的東西。
--      · 而閘沒有被改鬆:把 §2 插入的 `進氣系統 · 管束配件` 打成 `…管束配件XX`(② 的清單不動)
--        ⇒ **紅在「實得 5」** ⇒ 它仍抓得到「該插的那一列沒插成」。
--      · 抽掉大類「引擎與冷卻」⇒ 紅在 ①(那是 ① 的射程,不是 ② 的)。
SELECT count(*) INTO v_subs
  FROM categories c
 WHERE (c.raw_path IN ('進氣系統 · 空氣濾芯','進氣系統 · 進氣上蓋','進氣系統 · 進氣套件',
                       '進氣系統 · 濾芯保養品','進氣系統 · 管束配件')
        AND c.parent_category_id = v_intake_id)
