...
--
-- ② **訂單數 / 最後下單 也排除已取消**
--    → **主視窗 2026-08-16 裁(Q-最後下單口徑 = A)。Sean 未逐條答。要改直接改。**
--    理由(主視窗原文):**消費金額既然排除已取消,兩欄並排就必須共用一條規則** ——
--    不然員工要學兩套(「消費金額不算取消的,但最後下單算」),而畫面上看起來是同一組資訊。
--    ⇒ **一句話能解釋整列:「已取消的訂單不算。」**
--
--    ⚠️ **反方成立,但它屬於另一個欄位**(主視窗原文):
--       「他最近有沒有來」是行為訊號 ⇒ **那該叫「最後互動」,不叫「最後下單」**。
--       **本片不做,列為未來選項。** 改成「含取消」的成本 = view 一行 + 重跑 probe。
--    📎 這條原本是「A 窗選、無人裁」,2026-08-16 已由主視窗裁下 —— **本段就是那次更新**。
--       (自陳缺口不就地關掉的話,會以「誠實」的外觀繼續活著。)
--
