-- 20260915070000_m4b_order_workflow_ship_to.sql —— 後台改單第 5 代:編輯個資彈窗可改 收件人 / 電話 / 地址
--
-- 🛑 未貼(寫好不貼;貼是 Sean 一次一個編號)。主視窗 2026-09-14 批甲(Sean 在正式站走到「改客人地址改不了」)。
--
-- ══ 病 ═════════════════════════════════════════════════════════════════
-- orders.shipping_address_snapshot 全 repo 零 UPDATE 寫入者(只有建單 INSERT);出貨彈窗那三格是唯讀顯示 ⇒ 後台沒有任何路能改一張單的收件資料。
-- ══ 做什麼 ═════════════════════════════════════════════════════════════
-- · admin_update_order_workflow 第 5 代 = 第 4 代(20260913060000:96-383)本體【逐字】+ 白名單三鍵 + ship_to 那一段 + NOOP 比對多一欄 + SET 多一欄 + audit before/after 多整包快照。
-- · 三鍵全送才更新整包(那道 CHECK 要 exact {name,phone,line} 全字串);三個都必填、擋零寬、長度上限 60 / 30 / 200。
-- · 已建的箱(shipments.recipient_snapshot)不動 —— 出貨當時的快照;要換箱上的收件人 ⇒ 沒印標籤的重設回 draft 再建, 印了的打電話新竹。
-- ══ 形狀 ═══════════════════════════════════════════════════════════════
-- CREATE OR REPLACE(簽章不變);🔴 它會把 SET 子句整組換掉 ⇒ 本支照寫 SET search_path = '';ACL 由第 1 代設、REPLACE 保留、事後閘斷言。
-- 前置閘:第 4 代要在(函式體有 invoice_title);已是第 5 代 ⇒ 冪等重跑。
-- 回滾:supabase/rollbacks/20260915070000_down.sql = 貼回第 4 代本體(逐字)。已改過的快照留著(那是員工真的改的)。
...
BEGIN
  ...
  IF v_oid IS NULL THEN
    RAISE EXCEPTION '前置閘⓪:admin_update_order_workflow 不在, 拒繼續';
  END IF;
  ...
  IF v_src NOT LIKE '%''invoice_title''%' OR v_src NOT LIKE '%''invoice_tax_id''%' THEN
    RAISE EXCEPTION '前置閘⓪:函式體沒有 invoice_title ⇒ 第 4 代(20260913060000)沒套過, 先貼它;本支是它的聯集';
  END IF;
  ...
  END IF;
  IF v_src LIKE '%''ship_to_name''%' THEN
    RAISE NOTICE '前置閘⓪:已是第 5 代 ⇒ CREATE OR REPLACE 照樣重跑(冪等)';
  ELSE
    RAISE NOTICE '前置閘⓪:第 4 代在 ⇒ 升第 5 代';
  END IF;
  IF NOT EXISTS (SELECT 1 FROM pg_catalog.pg_attribute a WHERE a.attrelid = 'public.orders'::regclass AND a.attname = 'shipping_address_snapshot' AND NOT a.attisdropped) THEN
    RAISE EXCEPTION '前置閘①:orders.shipping_address_snapshot 不在';
  END IF;
END
$$;
...
BEGIN
  ...
  IF v_src NOT LIKE '%''ship_to_name''%' OR v_src NOT LIKE '%''ship_to_phone''%' OR v_src NOT LIKE '%''ship_to_line''%' THEN
    RAISE EXCEPTION '事後閘④:函式體沒有 ship_to_* ⇒ 貼到的不是第 5 代';
  END IF;
  IF v_src NOT LIKE '%''invoice_title''%' OR v_src NOT LIKE '%''invoice_tax_id''%'
     OR v_src NOT LIKE '%''invoice_issued_at''%' OR pg_catalog.strpos(v_src, 'P9I01') = 0 THEN
    RAISE EXCEPTION '事後閘④:函式體少了第 3 / 4 代的東西 ⇒ 聯集漏了一半';
  END IF;
  ...
  v_res := public.admin_update_order_workflow(
    ...
    '{"ship_to_name":"x","ship_to_phone":"0912","ship_to_line":"y"}'::jsonb,
    'postgate', 'postgate');
