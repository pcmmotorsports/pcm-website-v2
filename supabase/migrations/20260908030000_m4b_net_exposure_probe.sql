...
--
-- 🔴🔴 **2026-09-17 更正:上面那三行【自 20260917010000 起不再成立】。**
--   ⬆️ **原句留著不改** —— 它記的是 2026-09-08 當下的真值, 抹掉它等於抹掉一個做過的決定。
--   ⇒ 現況:`pcm_readonly`(唯讀稽核帳號)對本表**有表級 SELECT**。
--   理由:本表存在的目的就是「量曝露面」, 而**讀它的人正是稽核者**;
--     原本逼人去用 Supabase 管理 API(權限比唯讀角色大很多)才看得到 —— 那個習慣本身是風險。
--   🔬 而上面 `:102-103` 的表 COMMENT 逐字寫「pcm_readonly 是否讀得到【未確認】」
--     ⇒ 📌 **那是留白, 不是禁止** —— `20260917010000` 把那個未決收掉了(Sean 2026-09-17 拍乙)。
--   🛑 **而『不給任何人』這句對【其他角色】仍然成立**:anon / authenticated / service_role 照舊零 GRANT。
--     ⇒ 下一個人不要把這一段讀成「這張表開放了」。
