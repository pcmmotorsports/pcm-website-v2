...
--     🔵 **出處**:那兩句是**主視窗 B 從 `docs/patterns/revoking-function-execute-in-supabase.md`
--        轉給我的, 而【接到了錯的受詞上】** —— 兩句本身是真的, 它們講的不是函式 `EXECUTE`。
--        📌 B 自陳:「**我把真的話接到了錯的受詞上**」。**寫出處是為了讓下一個人不要從本檔再抄一次。**
--        🎯 **而它的方向值得注意:結果是【斷言比我以為的更可信】** ——
--        **這種錯不會有人抱怨, 只會讓人多做一輪驗證、或改掉一個本來正確的判準。**
...
--  ⑤ 🔴🔴 **最重要的一條, 別讓它被上面幾條埋掉**:
--     **codex 那一輪自陳「本輪為靜態核對及記憶體關聯模擬, 未實跑 PostgreSQL / 正式庫」。**
--     ⇒ 📌 **那七道靜態守門 + codex 那一輪, 沒有任何一發真的送一筆資料進去。**
--     ⇒ 🛑 **「codex 過了 + 七道守門綠」不等於「驗過了」** —— 行為驗收在前端片的 `V1`-`V5` 與突變,
--        以及**貼完之後我自己跑的那三格權限複驗**(不靠代貼的人回報綠)。
