...
--      🔵 2026-08-29 訂正(線I):**那一片後來被做掉了。** `scripts/rpm-delta.ts:98` 現在是
--         `newP < 0`(不是 `<= 0`)⇒ **0 已經放行**。落點 `1651fe76`「匯入閘放行 0 元贈品」,
--         `git merge-base --is-ancestor 1651fe76 dev` ⇒ rc=0(在 dev 上)。
--   ② 結帳 `create_order`                ⇒ **本檔**
--   ③ 顯示 storefront `Price.tsx`        ⇒ 已於 `0ed3cf16` 做完
--   ~~⇒ ①的第二道還在 ⇒ **贈品仍然上不了架。本檔不會改變那件事。**~~
--   🔵 2026-08-29 訂正(線I):**這句今天是假的,而它是本次訂正的主因。**
--      三道 0 元閘(上架 / 單價 / 整車)**都已在正式庫的 live 函式上**:
--        `supabase/APPLIED.tsv` 末列 · 本檔 `20260825130000` 第 3 欄 apply 日期 = `2026-08-27`,
--        Sean 本人 SQL Editor 貼;落點 `cf19474d`(補帳)+ `eb683bae`(追記已驗 live 函式)。
--        而該列追記記載 2026-08-27 09:4x **Sean 貼回 `pg_proc` 唯讀查詢四列** ——
--          ①單價閘「新版:0 元單價已放行」②整車閘「新版:整車 0 元閘已在」
--          ③上架閘「新版:0 元可上架」④**負對照「查無(尺是活的)」**
--        ⇒ 那不是「只證明他跑了」, 是量到函式體換了。
--      數法(可重跑, **不需要任何 DB access**):
--        awk -F'\t' '$3=="2026-08-27"' supabase/APPLIED.tsv | wc -l  ⇒ 1  (就是本檔)
--        正對照 $3=="2026-08-25" ⇒ 2 (尺是活的) · 負對照 $3=="2026-08-99" ⇒ 0
--   🔴 **為什麼這句錯得比一般過期句嚴重**:它寫在【下一個做這片的人一定會打開】的位置,
--      而它說的是「整件事沒做」⇒ 讀到的人會停止查證, 而**停止查證不留痕跡**。
--      📌 判別句:我寫「這個還沒做 / 這個我量不到」的當下,
--         有沒有去確認【那句話本身】是不是也過期了?
--   ⚠️ 而本次訂正**只動這幾行註解, 一行 SQL 都沒有動**(證明見 commit body)。
--   ⚠️ 平台 `supabase_migrations` 那本帳**仍未登記本檔**(SQL Editor 手貼不會寫進去)——
--      那一格**本次不處理**, 見 `APPLIED.tsv` 該列原文。
--
