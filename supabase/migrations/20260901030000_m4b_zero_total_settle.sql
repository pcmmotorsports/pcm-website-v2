...
--         改名 ⇒ 本支的 `DROP` 不再指向 `021000` 建的那一支 ⇒ `public.orders` 上同時活著兩支綁同一支函式的 trigger。
--         ⛔ ~~⇒ 一次付款扣兩次券~~ 🔴 **2026-09-12 Fable 審打掉這個理由, 我複驗成立**:
--         `redeem_coupon`(`20260831160000:242-249`)同券同人同折抵會早退不再 INSERT, 加上
--         `coupon_redemptions_one_per_order UNIQUE (order_id)`(`20260829150000:182`)⇒ **第二支是 no-op**。
...
--   ✅ **2026-09-12 唯讀實查:那支函式【在正式庫上】了**(貼板 125 `20260901020500` 建的;126 是接線那一發)——
...
--    ① `coupon_redeem_on_paid` 這支函式 **正式庫裡不存在**
--       (`pg_proc` 裡名字帶 coupon 的只有 `coupon_redeem_order_problem` 與 `redeem_coupon`)
--    ② 本支與 `20260901021000` **兩支都不在 `supabase/APPLIED.tsv`**(帳本當時 377 列)
--    ③ `payment_status` 五個值今天各幾張:unpaid 2 · paid 1 · **partiallyPaid 0** ·
--       refunded 1 · **partiallyRefunded 0**(合計 4 = orders 總數)
--    ⚪ 而那幾個 0 先證明過尺是活的:public schema 205 支函式 · 非內部 trigger 79 個 ·
--       點名 `create_order` / `expire_unpaid_orders` / `search_catalog_by_vehicle` 都在 ·
--       現造名字 ⇒ 0 · 帳本正對照 `20260712203000` ⇒ 在。
--
-- 🎯 **所以時序長這樣, 而它決定了「什麼時候修」**:
--      今天          trigger 不存在 ⇒ **這個缺陷不可能發生**
--      貼下去那一刻  trigger 出生   ⇒ **缺陷同時出生**
--    📌 **⇒ 它不是「上線後要修的 bug」, 是【貼這一支之前要先修好的東西】。**
--    📌 **⇒ 修的時機比修的內容更重要, 而現在時機還在我們手上。**
--
-- 🛑 **貼之前要有人回答的兩題(本註解【不替任何人拍板】)**:
--    ① 口徑:`partiallyPaid ⇒ paid` 到底該不該扣券?(那是業務題, 要 Sean)
--    ② 若該扣:WHEN 子句與函式體**兩處的條件必須逐字相同** —— 改一處等於沒改。
--       🔴 而這一句我**沒有驗過**:我原本要拿正式庫的 `prosrc` 去比 WHEN 子句,
--          **而兩邊都不存在** ⇒ 那個比對沒有對象。
--          缺的那一道檢查 = **在拋棄式 PG 上把這兩支貼起來再比。未跑。**
--
-- ⚠️ **三個受詞都成立而不衝突, 不要拿其中一句去推翻另一句**:
--      「觸發條件已達成」 受詞 = 碼會不會寫出 `partiallyPaid`  ⇒ 成立(`20260904230000` 會)
--      「今天 0 張」      受詞 = 今天有沒有那種單              ⇒ 成立
--      「trigger 不存在」  受詞 = 正式庫有沒有那個東西          ⇒ 成立
--      📌 **只有第三句回答了「今天會不會出事」。**
--
-- 🔵 **為什麼這段話寫在【這裡】而不是只寫在板上**(主視窗 A 2026-09-08 裁「甲」):
--    綁在時間上會漏, 綁在**貼板流程**上不會 —— 而貼的人一定會打開這支檔。
--    ⇒ 📌 **要求要住在它會被讀到的那個時刻。**
-- ══════════════════════════════════════════════════════════════════════════════
--
