...
BEGIN
  ...
  FOREACH v_t IN ARRAY v_tables LOOP
    ...
    -- ④-a2 🔴🔴 **【你是誰】—— R3 MF1。本片第三次栽在同一個形狀上, 所以這次寫長一點。**
    --
    -- 🎯 **形狀**:檔頭 `:84` 量了「五張 owner 都是 `postgres`」, 而**那個前提沒做成閘**。
    --   R1 MF1 補的是「**acl 上那一列**的 grantor 是不是 owner」;
    --   🔴 **而沒有人問過「【現在跑這支的人】能不能當 owner」** ——
    --   實查:本片與還原檔裡 `current_user` / `pg_has_role` / `session_user` 各出現 **0 次**。
    --
    -- 🔬 **PG 的規則**:`REVOKE` / `GRANT` 只動得到「**執行者當得成的那個 grantor**」的列,
    --   而**動不到時不報錯、也不印 WARNING**(本檔 `:533` 那一格量的就是這個)。
    --   ⇒ 執行者若只有 `WITH GRANT OPTION`、不是 owner / 不是 superuser / 不是 owner 的成員:
    --     · 五道 `✅ 前置閘④[…]:要收的那條在` **全印**(它們問的是 acl, 不是身分)
    --     · 五句 REVOKE **零 WARNING**
    ...
    IF NOT pg_catalog.pg_has_role(
             ...
             'USAGE') THEN
      RAISE EXCEPTION '前置閘④-a2[%]:你現在是【%】, 而 % 的 owner 是【%】—— 你不是它、不是它的成員、也不是 superuser ⇒ 你下的 REVOKE **動不到 owner 授出的那一列, 而且不會報錯也不會印 WARNING** ⇒ 停下, 換一個當得成 owner 的身分再跑。',
...
BEGIN
  ...
  --      原始碼裡的例外句 = **28 句**(本輪加了 ④-a2)
  --      ⛔ ~~跑起來會被評估的次數 = **79 格**~~
  --   🔴🔴 **79 是【錯的】—— 而這一格比那個數字本身重要。**
  --     R2 nit1 就告訴過我, 我重數了、**算對了、也講對了**, 却沒有寫回這支檔 ⇒ R3 nit1 又抓了一次。
  --     📌 **一個我算對過、講對過、而沒有落到檔案裡的數字 —— 那等於沒算。**
  --     🛑 **同一個數字被兩輪審查各抓一次, 而中間那一次我還自己量過。**
  --   🔴 **錯在哪**:後置「迴圈外 8」裡有 **4 句其實在 over-revoke 的 pairs 迴圈裡**
  --     ⇒ 它們跑的次數 = **名單長度 n**, 不是 1。
  ...
  --
  -- ✅ **照實的版本**:
  --   **在 2026-09-18 夜那份讀數仍然成立、而且本檔單獨貼的前提下, 那 100 格(今天 prod 形狀)【一格都不會叫】。**
  ...
  --   這 100 格守的是「前提哪天翻掉」—— **一道守著前提翻掉的閘, 在前提還成立時本來就該恆綠。**
  --   🔴 **而 R1 那一輪實際燒紅過其中 10 格**(9 發負對照 + 多 grantor 那一發)
  --   ⇒ 📌 **「今天不會叫」與「它永遠不會叫」是兩件事, 而前者是燒出來的, 不是推出來的。**
  RAISE NOTICE '✅ 後置閘全過(屬於後置的那幾格;總格數依 over-revoke 名單長度而定 —— 名單空 80 格 / 今天 prod 形狀 100 格, 見本區塊末註解)。🛑 而在 09-18 夜那份讀數仍成立的前提下, 這 100 格【本來就一格都不會叫】—— 它們守的是「讀數從量完到貼下去之間漂掉了」。⇒ **貼之前請現場重量一次五張的 relacl(含 grantor)與全域那 81。**';
