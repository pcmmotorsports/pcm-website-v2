-- ⟦b4-PARTPAIDNOCANCEL1⟧ 收了訂金的匯款單可以【整單】取消 —— 20260908060000 的重寫版(第 2 版, 2026-09-14)
--
-- ── 為什麼有第 2 版(舊檔 20260908060000 留著不動、不貼)─────────────────
-- ① 🔴 **舊檔的前置閘永遠不會過**:它用 `pg_get_function_identity_arguments(oid) = 'uuid, uuid, text, text, text, jsonb'`
--    找函式, 而那個函式的參數【有名字】⇒ 該函式實際回的是
--    `p_order_id uuid, p_idempotency_key uuid, p_actor text, p_reason_code text, p_reason_detail text, p_items jsonb`
--    ⇒ 查到 0 列 ⇒ 前置閘拋「不存在」。拋棄式 PG 實跑(2026-09-14, 第 8 代 20260903093000 在線上時):
--    `psql:...20260908060000...:249: ERROR: 前置閘:public.admin_cancel_order(...) 不存在`。
-- ② 🔴🔴 **同一個字串也出現在舊檔的後置斷言①(ACL)** ⇒ 那道斷言【恆綠】(篩到 0 列 ⇒ v_bad 為 NULL ⇒ 不叫)。
--    fail-closed 的前置閘擋住了一道 fail-open 的後置斷言 —— 兩個 bug 互相遮住。
-- ③ 舊檔沒有函式體後置 md5、沒有 proconfig 斷言、沒有 rollback 檔。
-- ✅ 本版:前置 / 後置都改用 `to_regprocedure('public.admin_cancel_order(uuid,uuid,text,text,text,jsonb)')`;
--    後置加 md5 + proconfig `{search_path="", lock_timeout=5s}`;前置加 `order_pending_refund_open_au` trigger
--    在且啟用(舊檔 codex R3 ③ 記著未修的那一格);rollback 在 `supabase/rollbacks/20260914050000-rollback.sql`。
-- 函式體本身(三處述詞放寬 partiallyPaid 整單)與舊檔【逐字相同】, 基準仍是第 8 代 20260903093000
--   (線上 prosrc md5 `bd7c79ba2ccc792d3dab5f3b54335582`, 拋棄式 PG 2026-09-14 實查, 與舊檔前置閘期望值相同)。
--
-- ── 前置條件今天的讀數(2026-09-14)──────────────────────────────────
-- ① `#787` 人工退款入口解封 ⇒ ✅ `manual-refund-entry-gate.ts:224` `MANUAL_REFUND_ENTRY_BLOCKED_BY_787 = false`, 已在 dev。
-- ② `order_pending_refunds.settled_at` 有寫入端 ⇒ 🔴 **仍然沒有**(全 migrations / apps / packages 剝註解後 0 個寫入端;
--    拋棄式 PG `pg_proc.prosrc ~ 'settled_at\s*='` ⇒ 0 支)。
--    📌 **而這一格【不是本片獨有的洞】**:今天 `paid` 的匯款單整單取消【已經放行】(20260820030000 起),
--    它同樣會由 `order_pending_refund_open_au` 開一列待退款、同樣沒有人標得掉。本片只是讓 `partiallyPaid`
--    走同一條已經開著的路。待退款【金額】那一半 app 端讀的是 `pcm_pending_refund_amounts`(收款 − 人工退款的淨額,
--    不看 settled_at)⇒ 登記人工退款後畫面上的「還欠多少」會正確歸零;只有 `order_pending_refunds` 那一列的
--    `settled_at` 留白。⇒ 貼不貼是 Sean 的決定, 本檔不再自己擋門(舊檔頭「兩條都勾才可以貼」是主視窗 A 09-08 的讀法)。
--
-- ── 以下是 20260908060000 的原檔頭(逐字保留, 其中「不可套用 / 本檔」等字指的是那一版)──
...
--    🔴 **上面那句是【過期字面】, 舊字面留著不刪**(主視窗 A 2026-09-08 批改, 副手窗 `B` 落筆)。
--    ✅ **實情:三輪 26 條** —— R1 **8** must-fix / R2 **10** / R3 **10**, 逐條開檔驗過皆屬實。
--       來源:commit `59665be3b` body 逐字;而**本檔身內自己就引用了** R2 must-fix ⑤⑥⑦⑩ / R2 MF2 / R3
--       ⇒ 📌 **檔頭與檔身當時就已經不一致, 而不一致的方向是【檔頭少報】。**
--    🔬 **其中 R2 的 ⑧⑨ 是【R1 的修法造出來的】;R3 的 ⑤ 是【R2 的修法造出來的】。**
--    📌 **為什麼值得改**:檔頭是**貼的人唯一保證會讀到的地方**, 而它原本說「只跑過 R1」
--       ⇒ 他會以為審查深度只有一輪 ⇒ 因此加審(浪費)或因此不信任這支檔(更糟)。
--       🛑 **一份權威的過期字面, 比沒有那份字面糟。**
--
-- ── 🛑 兩條前置與它們【現在】的狀態(2026-09-08 16:3x 複量;檔頭是權威 ⇒ 寫在這裡)──────
--    ☑ **① `#787` 人工退款入口解封 ⇒ ✅ 已達成**(寫本檔時還沒有 ⇒ 那一格的舊讀數在板列 ⟦b4-PARTPAIDNOCANCEL1⟧, 留著)。
--       🔬 量法(主視窗 A 當場跑, 不是轉述):`git merge-base --is-ancestor aea57bdef origin/dev` ⇒ **是**;
--          `git branch -a --contains aea57bdef` ⇒ auth/db/front/refund/ship **五條線都含它**;
--          `git show origin/dev:apps/admin/src/components/orders/manual-refund-entry-gate.ts` 內
--          `MANUAL_REFUND_ENTRY_BLOCKED_BY_787` ⇒ **`false`**。
--       ⚠️ **仍未證的那半是【部署】本身。**
--       🔴 **而問這一格一律問 `origin/dev`, 不要問工作樹** —— 落後的樹上那個值仍是 `true`,
--          而**「我的樹落後」與「它沒做」在 grep 上長得一模一樣**。
--    ☐ **② `order_pending_refunds.settled_at` 有寫入端 ⇒ 🔴 仍未達成。**
--       🛑 **而量這一格有一個陷阱, 它往【放行】的方向錯**:
--          `grep -rl 'SET settled_at' supabase/migrations/` ⇒ **1 支** —— 看起來像達成了。
--          而**那唯一的 1 就是本檔自己的一行【註解】**(特徵字面:那行註解逐字寫著
--          「全 migrations `SET settled_at` ⇒ 0」;⚠️ **此處刻意不寫行號** —— 本次檔頭加了 20 幾行,
--          **只加註解的 diff 會位移檔內每一個行號**, 而寫死的行號會安靜地變成假的)。
--          ⇒ 🎯 **一句記錄「這裡是 0」的註解, 讓下一把尺量到 1。**
--          ✅ **量法**:剝行註解再數, 或直接開檔看那一支是誰。
--          🟢 正對照 `SET payment_status` ⇒ **21 支** · ⚪ 負對照 `SET settled_atZZQ` ⇒ **0** ⇒ 尺會動。
--          📎 memory `feedback_comments-read-as-code-by-grep`。
--    🛑 **兩條【都勾】才可以貼 ⇒ 本檔今天仍然不可套用, 而擋門【不是授權】** —— 不必問 Sean,
--       也**不要把它補成貼板包**:建了包會讓一支不可套用的 migration **長出可以被貼的形狀**。
--    ⏰ **今天的嚴重度(唯讀複量 2026-09-08 16:3x)**:正式庫 `orders` 共 **4** 張(🟢 正對照, 尺會動)·
--       **`partiallyPaid` 0 張** · `order_pending_refunds` **0** 列 · ⚪ 負對照 `WHERE 1=0` ⇒ **0**。
--       ⇒ 📌 **今天一張那種單都沒有 ⇒ 這一片解的是【上線那天】的缺陷, 不是今天的。**
--       🔴 **而 UI 那一側已經放行了**(`cancel-view.ts` 內 `paid` 與 `partiallyPaid` 走同一條 rail)
--       ⇒ **鈕畫得出來、按得下去, 而正式庫那道舊閘會拒, 回的是通用訊息** ⇒ **上線前必關。**
...
BEGIN
  -- 🔴 v2:不用 identity_arguments 字串比對(參數有名字, 那個字串永遠對不上 ⇒ 舊檔恆擋)。
  ...
  END IF;
  -- v2:proconfig 也要是我以為的那組(CREATE OR REPLACE 會整組換掉 SET 子句, 這裡先確認基準)
  IF (SELECT p.proconfig FROM pg_catalog.pg_proc p
       WHERE p.oid = pg_catalog.to_regprocedure('public.admin_cancel_order(uuid,uuid,text,text,text,jsonb)'))
     IS DISTINCT FROM ARRAY['search_path=""', 'lock_timeout=5s']::text[] THEN
    RAISE EXCEPTION '前置閘:admin_cancel_order 的 proconfig 不是 {search_path="", lock_timeout=5s} ⇒ 停下';
  END IF;
  -- v2(舊檔 codex R3 ③):本片放行的取消靠 orders 上這支 trigger 開待退款列;它不在或停用 ⇒ 取消成功而待退款靜默不開。
  --   codex R1 must-fix:只排 'D' 會放過 'R'(只在 replica 模式觸發), 同名接錯函式也會過 ⇒ 釘 O/A + 函式 + tgtype
  --   (17 = ROW|AFTER|UPDATE, 來源 20260901080000:495-498 `AFTER UPDATE OF cancelled_at ON public.orders FOR EACH ROW`)。
  IF NOT EXISTS (SELECT 1 FROM pg_catalog.pg_trigger t
                  WHERE t.tgrelid = 'public.orders'::regclass AND t.tgname = 'order_pending_refund_open_au'
                    AND t.tgenabled IN ('O', 'A')
                    AND t.tgfoid = pg_catalog.to_regproc('public.pcm_pending_refund_on_cancel')
                    AND t.tgtype = 17) THEN
    RAISE EXCEPTION '前置閘:orders 上的 order_pending_refund_open_au 不在、未啟用、或不是 AFTER UPDATE ROW 接 pcm_pending_refund_on_cancel ⇒ 停下';
  END IF;
END
$$;
...
BEGIN
  ...
  END IF;
  IF pg_catalog.strpos(v_old, '20260914050000') > 0 THEN
  ...
  ELSE
    EXECUTE pg_catalog.format(
      ...
      v_old || ' 🔴 2026-09-14(⟦b4-PARTPAIDNOCANCEL1⟧, 落點 20260914050000;20260908060000 未貼、前置閘寫壞、由本版取代):允許集合再放寬一格 —— '
...
BEGIN
  ...
  END IF;
  -- v2 後置斷言③:換上去的函式體與 proconfig 是我以為的那一份(md5 由拋棄式 PG 套完實查)
  IF (SELECT pg_catalog.md5(p.prosrc) FROM pg_catalog.pg_proc p
       WHERE p.oid = pg_catalog.to_regprocedure('public.admin_cancel_order(uuid,uuid,text,text,text,jsonb)'))
     <> '90d6420f86e6039819c8e1ec168bb6f7' THEN
    RAISE EXCEPTION '後置斷言③:admin_cancel_order 換上去的函式體 md5 不是預期值 ⇒ 停下';
  END IF;
  IF (SELECT p.proconfig FROM pg_catalog.pg_proc p
       WHERE p.oid = pg_catalog.to_regprocedure('public.admin_cancel_order(uuid,uuid,text,text,text,jsonb)'))
     IS DISTINCT FROM ARRAY['search_path=""', 'lock_timeout=5s']::text[] THEN
    RAISE EXCEPTION '後置斷言③:admin_cancel_order 的 proconfig 不是 {search_path="", lock_timeout=5s} ⇒ 停下';
  END IF;
  IF NOT EXISTS (SELECT 1 FROM pg_catalog.pg_proc p
                  WHERE p.oid = pg_catalog.to_regprocedure('public.admin_cancel_order(uuid,uuid,text,text,text,jsonb)')
                    AND p.prosecdef AND pg_catalog.strpos(p.prosrc, 'partiallyPaid') > 0) THEN
    RAISE EXCEPTION '後置斷言③:admin_cancel_order 不是 SECURITY DEFINER 或函式體沒有 partiallyPaid ⇒ 停下';
  END IF;
END
$$;
