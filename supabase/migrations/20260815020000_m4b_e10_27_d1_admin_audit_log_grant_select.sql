...
BEGIN
  ...
  DECLARE
    v_expected CONSTANT text[] := ARRAY[
      'admin_audit_log_select_service_role',  -- 20260904270000
      'admin_audit_log_insert_service_role'   -- 20260905090000
    ];
    v_actual text[];
  BEGIN
    SELECT coalesce(array_agg(polname ORDER BY polname), '{}')
      INTO v_actual
    ...
    IF v_actual <> (SELECT array_agg(x ORDER BY x) FROM unnest(v_expected) x) THEN
      RAISE EXCEPTION 'D0 異常 — admin_audit_log 的 policy 集合與期望不符;拒繼續。期望 % / 實際 %',
        v_expected, v_actual;
    END IF;
  END;
