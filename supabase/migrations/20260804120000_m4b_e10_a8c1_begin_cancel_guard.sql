...
--   + Sean 2026-08-04 拍板 Q1=A(pcm-mailbox A-09-A:genesis 死結列已知可用性面、COMMENT 更正、backlog #321)。
...
--   ⇒ 兩向零新增收款路徑。根治=mark 家族三支統一 orders-first(backlog #321)。
...
--
-- 🔴 前置閘(plan §3.8;防 #321 先落地被本檔倒寫):四支 functiondef md5 必=已知值,
...
BEGIN
  ...
  IF v <> 'fd87db5732a12c4688babc6a46c1e508' THEN
    RAISE EXCEPTION 'A8c1 前置閘:mark_charge_attempt_failed 定義漂移(md5=%);#321 已動它?停下人工對齊', v;
  END IF;
  ...
  IF v <> '7334ef0e2ee7f89a093bd8a7bda98828' THEN
    RAISE EXCEPTION 'A8c1 前置閘:mark_charge_attempt_charged 定義漂移(md5=%);#321 已動它?停下人工對齊', v;
  END IF;
  v := md5(pg_get_functiondef('public.close_released_attempt(uuid,text)'::regprocedure));
  IF v <> '717bcf80e4d913f1fd047ea2d7c1a667' THEN
    RAISE EXCEPTION 'A8c1 前置閘:close_released_attempt 定義漂移(md5=%);#321 已動它?停下人工對齊', v;
  END IF;
END
$$;
...
COMMENT ON FUNCTION public.mark_charge_attempt_charged(uuid, uuid, text) IS
  'M-3 3DS R1b1c PF-X1 麵包屑主軌(released→charged 放寬 + 雙扣 genesis):status IN (pending,released)→charged + rec_trade_id。released = late success 對帳收斂(§2.6)→ 同交易建 open anomaly(payment_double_charge_anomalies、ON CONFLICT old_attempt_id DO NOTHING、所有 NOT NULL 欄齊填、缺則 RAISE fail-closed 寧不收斂不漏記;refund_target=舊 attempt rec 絕不指向新單、amount=orders.total integer)。🔴 已知死結面(E10-A8c1 2026-08-04,Sean 拍板接受、backlog #321):genesis 路徑(鎖 attempt→UPDATE charged→anomaly FK 等 orders KEY SHARE)× begin(orders-first)= 40P01 單方 victim;倖存 begin 必 order_locked(order_lock_idx 述詞含 released)⇒ 零新增收款路徑;victim=本函式時 late-success 收斂延後重試。基線雙鍵驗 + FOR UPDATE + charged 同 rec 冪等 no-op + 跨單重複 rec 通用 RAISE 逐字保留。只 payment_confirmer 可呼(ACL 沿用基線、不改)。';
