-- 回滾 · `20260905360000`(片二 手動建單真的算稅)
--
-- 🔴🔴 **誰跑、用什麼跑**:Sean 在 Supabase SQL Editor 整支貼一次。**任何 session 不得代跑。**
-- 🟢 純 SQL、無 psql 反斜線指令 ⇒ 貼得進 SQL Editor。整支包在一個交易裡。
--
-- ══ 🔴🔴 只跑這一支【不夠】—— 回滾是三個動作 ══════════════════════════════
--   ① **本檔**:函式回第 5 代 · 改價那支回原版 · view 回 43 欄
--   ② 🔴🔴 **同時要把 UI 那句話退回「請填含稅」** ——
--      只回函式 ⇒ **畫面仍叫員工填未稅, 而系統又當它含稅** ⇒ **那正是本片要修的那個少收 5%。**
--      ⇒ 📌 **回滾一半, 會回到一個比動手前【更糟】的狀態**(動手前兩邊是一致的)。
--      ⇒ 落點:`agent/line-account-taxcopy` 上那顆改文案的 commit ⇒ `git revert` 它。
--   ③ 🔴 **不要 `DROP COLUMN price_tax_mode`** ——
--      已經寫進去的 `'exclusive'` 會消失, 那些單就**再也分不出來**。
--      **欄位留著**(有 DEFAULT、有 CHECK, 對舊碼無害 —— ⚠️ 那句是判斷, 我沒有逐一開檔看呼叫端)。
--      🔵 而回滾之後新建的單會拿到 DEFAULT `'inclusive'`, 而它們**真的是含稅的**(函式回去了)⇒ 對。
--
-- 🛑 **已經用新語意建出來的單不會被回滾** —— 它們的 `subtotal` 是未稅、`price_tax_mode='exclusive'`。
--    📌 **那正是那個欄位存在的理由** —— 回滾之後仍然分得出它們是哪一種。
--    ⇒ ⇒ **回滾的單位是碼, 而傷害的單位是資料。**
--
-- 🔬 三支函式/view 的本體都是**程式抽出、零手抄**(第 5 代 `20260905130000:120-805` ·
--    改價 `20260816040000_m4b_e10_13_518_p2c13_detail.sql:45-211` · view `20260905230000`)。
-- ══════════════════════════════════════════════════════════════════════════

BEGIN;

-- ① 建單 RPC 回第 5 代(參數列相同 ⇒ CREATE OR REPLACE 是取代;ACL/COMMENT 原地保留)
...
-- ③ view 回 43 欄
-- 🔴 `CREATE OR REPLACE VIEW` **不能減欄** ⇒ 這一段會失敗(`cannot drop columns from view`)。
--    ⇒ 所以 view 那半**要 `DROP VIEW` + `CREATE`**, 而那會帶走 ACL 與 COMMENT。
--    🛑 **本檔【刻意不做 view 的回滾】** —— 多一欄不會弄壞任何既有呼叫端(它們逐欄列出自己要什麼),
--       而為了少一欄去 DROP 一支 view, 買到的風險(ACL/COMMENT 掉了而沒人發現)比留著那一欄大。
--    ⇒ 📌 **這是取捨, 不是漏做。** 要真的減欄 ⇒ 另寫一支, 並把 ACL 與 COMMENT 明文貼回。
...
BEGIN
  ...
  IF v_n <> 1 OR v_args <> '11' THEN
    RAISE EXCEPTION '回滾斷言①:admin_create_manual_order 有 % 支(引數數 %)⇒ 期望 1 支 11 參', v_n, v_args;
  END IF;
  -- 🔴 函式體裡不可以再有 v_tax / exclusive(擋住「我以為回滾了而它沒回」)
  IF EXISTS (SELECT 1 FROM pg_catalog.pg_proc p JOIN pg_catalog.pg_namespace n ON n.oid=p.pronamespace
              WHERE n.nspname='public' AND p.proname='admin_create_manual_order'
                AND (p.prosrc LIKE '%v_tax%' OR p.prosrc LIKE '%exclusive%')) THEN
    RAISE EXCEPTION '回滾斷言②:建單函式體裡還有 v_tax 或 exclusive ⇒ 第 5 代沒貼回去';
  END IF;
  IF EXISTS (SELECT 1 FROM pg_catalog.pg_proc p JOIN pg_catalog.pg_namespace n ON n.oid=p.pronamespace
              WHERE n.nspname='public' AND p.proname='admin_update_order_item_amount'
                AND p.prosrc LIKE '%pcm_e13_no_edit_when_taxed%') THEN
    RAISE EXCEPTION '回滾斷言③:改價那支還有稅閘 ⇒ 原版沒貼回去';
  END IF;
  -- 🔵 而 price_tax_mode 那一欄【應該還在】—— 它是刻意留的
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns
                  WHERE table_schema='public' AND table_name='orders' AND column_name='price_tax_mode') THEN
    RAISE EXCEPTION '回滾斷言④:price_tax_mode 不見了 —— 本回滾【不該】刪它, 已經寫進去的值會消失';
  END IF;
END
$post$;
...
-- 🛑 **跑完這一支之後【還沒回滾完】** —— 上面 ② 那個 UI 文案的 `git revert` 是另一半,
--    而它不在資料庫裡。⇒ 📌 **一支跑得完的回滾檔, 不代表回滾完成了。**
