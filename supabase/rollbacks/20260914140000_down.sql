-- #956 乙 第 9 代 回滾:DROP 13 參 + 逐字貼回第 8 代(20260914030000:83-1030)+ 它的 ACL / COMMENT + DROP CHECK / COLUMN。
-- 🔴 順序 = 先 revert code 再回 DB(DB 回舊版時新 UI 送 13 個名字 ⇒ PGRST202)。
-- 🔴 DROP COLUMN 會把手動單填過的車一起丟掉(顧客站的單這欄恆 NULL)⇒ 回滾前先匯出:
--    SELECT display_id, vehicle_snapshot FROM public.orders WHERE vehicle_snapshot IS NOT NULL;
BEGIN;
SET LOCAL lock_timeout = '5s';
DO $pre$
BEGIN
  IF pg_catalog.to_regprocedure('public.admin_create_manual_order(uuid,uuid,text,text,text,text,jsonb,jsonb,integer,jsonb,text,text,jsonb)') IS NULL THEN
    RAISE EXCEPTION '回滾前置閘:13 參那一代不在這台庫上, 沒有東西可回滾';
  END IF;
END
$pre$;
DROP FUNCTION public.admin_create_manual_order(
  uuid, uuid, text, text, text, text, jsonb, jsonb, integer, jsonb, text, text, jsonb);
...
BEGIN
  ...
  FOREACH v_role IN ARRAY ARRAY['anon','authenticated'] LOOP
    IF pg_catalog.has_function_privilege(v_role, 'public.admin_create_manual_order(uuid,uuid,text,text,text,text,jsonb,jsonb,integer,jsonb,text,text)', 'EXECUTE') THEN
