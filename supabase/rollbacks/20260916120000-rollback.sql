-- 20260916120000-rollback.sql —— 退回 20260916120000_m4b_catalog_universal_and_noimage_last.sql
--
-- 做什麼:三支函式換回前一代本體(逐字取自 20260909070000 / 20260912010000,prosrc md5 = 2026-09-15 正式庫),再 DROP helper。
-- 🔵 順序:先換回函式(它們在本體內叫 helper)再 DROP helper。
-- 🔵 TS 沒改 ⇒ 退回不需要先退程式。
-- ⚠️ 前置閘釘 120000 那一代的 md5 ⇒ 本檔不能重跑(第二次會被前置閘擋 = 已退過)。
-- 🔴 退完同批跑 pcm_acl_approve_latest(p_note 帶 20260916120000 與「rollback」)。

BEGIN;
SET LOCAL lock_timeout = '5s';

-- 前置閘(R1 nit):現在必須是 20260916120000 那一代;以後有新一代蓋過去時誤跑本檔 ⇒ 停,不靜靜把新一代也退掉
DO $pre$
BEGIN
  IF pg_catalog.to_regprocedure('public.pcm_card_image_is_placeholder(text)') IS NULL
     OR (SELECT pg_catalog.md5(p.prosrc) FROM pg_catalog.pg_proc p WHERE p.oid = pg_catalog.to_regprocedure('public.search_catalog_by_vehicle(text[],text,text,integer,integer,integer,text,text,text[],integer,integer,timestamp with time zone,text[])')) IS DISTINCT FROM '6b73da535ffaf5628b995f048efd88f6'
     OR (SELECT pg_catalog.md5(p.prosrc) FROM pg_catalog.pg_proc p WHERE p.oid = pg_catalog.to_regprocedure('public.search_catalog_by_vehicle_dealer(text[],text,text,integer,integer,integer,text,text,text[],integer,integer,timestamp with time zone,text[])')) IS DISTINCT FROM 'd9ab09783d69dc0a082d7640c59deb3a'
     OR (SELECT pg_catalog.md5(p.prosrc) FROM pg_catalog.pg_proc p WHERE p.oid = pg_catalog.to_regprocedure('public.catalog_facet_counts(text[],text[],text,text,integer,text[],text[])')) IS DISTINCT FROM '37665499408108cbf2f1dc4fd7de42a5' THEN
    RAISE EXCEPTION '退回前置閘:現在不是 20260916120000 那一代(沒貼過、已退過、或已被更新的一代蓋過)⇒ 停下人工對齊';
  END IF;
END
$pre$;
...
DROP FUNCTION IF EXISTS public.pcm_card_image_is_placeholder(text);

DO $post$
BEGIN
  IF (SELECT pg_catalog.md5(p.prosrc) FROM pg_catalog.pg_proc p WHERE p.oid = pg_catalog.to_regprocedure('public.search_catalog_by_vehicle(text[],text,text,integer,integer,integer,text,text,text[],integer,integer,timestamp with time zone,text[])')) IS DISTINCT FROM '1757ca8bd92a6d8c10f08fbcb1338fe1'
     OR (SELECT pg_catalog.md5(p.prosrc) FROM pg_catalog.pg_proc p WHERE p.oid = pg_catalog.to_regprocedure('public.search_catalog_by_vehicle_dealer(text[],text,text,integer,integer,integer,text,text,text[],integer,integer,timestamp with time zone,text[])')) IS DISTINCT FROM 'eaee6f2f418312775746676d51300a35'
     OR (SELECT pg_catalog.md5(p.prosrc) FROM pg_catalog.pg_proc p WHERE p.oid = pg_catalog.to_regprocedure('public.catalog_facet_counts(text[],text[],text,text,integer,text[],text[])')) IS DISTINCT FROM 'aae85942d04afaa0cc2f3a89a83e1275' THEN
    RAISE EXCEPTION '退回後置閘一:三支函式沒有全部回到前一代 md5';
  END IF;
  IF pg_catalog.to_regprocedure('public.pcm_card_image_is_placeholder(text)') IS NOT NULL THEN
    RAISE EXCEPTION '退回後置閘二:helper 還在';
  END IF;
END
$post$;
