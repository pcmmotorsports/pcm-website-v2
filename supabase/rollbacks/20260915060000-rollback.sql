-- 20260915060000-rollback.sql
--
-- 三支 RPC 整支貼回上一代(create_order ← 20260907040000:145 / admin_create_manual_order ← 20260914140000:105 /
...
BEGIN
  ...
  IF v_src IS NULL OR pg_catalog.md5(v_src) <> '4b3d6e086edadb27b896f161c10b61dd' THEN
    RAISE EXCEPTION USING MESSAGE = '回退前置閘:admin_update_order_item_amount 不是 20260915060000 那一代(md5 ' || COALESCE(pg_catalog.md5(v_src), 'NULL') || ')⇒ 停下人工看';
  END IF;
END
$gate_pre$;

-- ① create_order ← 20260907040000:145(CREATE 改 CREATE OR REPLACE, 其餘逐 byte)
...
-- ② admin_create_manual_order ← 20260914140000:105(同上)
...
-- ③ admin_update_order_item_amount ← 20260915040000:60(原本就是 CREATE OR REPLACE)
...
BEGIN
  SELECT p.prosrc INTO v_src FROM pg_catalog.pg_proc p WHERE p.oid = pg_catalog.to_regprocedure('public.create_order(jsonb, uuid, text, jsonb, uuid, text, text, text, text, text, text)');
  IF pg_catalog.md5(v_src) <> '42a9132a887b1e39995452009dd7cb47' THEN RAISE EXCEPTION '回退事後閘:create_order md5 不是上一代 42a9132a887b1e39995452009dd7cb47'; END IF;
  SELECT p.prosrc INTO v_src FROM pg_catalog.pg_proc p WHERE p.oid = pg_catalog.to_regprocedure('public.admin_create_manual_order(uuid, uuid, text, text, text, text, jsonb, jsonb, integer, jsonb, text, text, jsonb)');
  IF pg_catalog.md5(v_src) <> 'c7924d9222e88ae33d042e9730ce1199' THEN RAISE EXCEPTION '回退事後閘:admin_create_manual_order md5 不是上一代 c7924d9222e88ae33d042e9730ce1199'; END IF;
  SELECT p.prosrc INTO v_src FROM pg_catalog.pg_proc p WHERE p.oid = pg_catalog.to_regprocedure('public.admin_update_order_item_amount(uuid, uuid, integer, integer, text, text, text)');
  IF pg_catalog.md5(v_src) <> '665d49b5a5cc3246bcdba0e3ffc85042' THEN RAISE EXCEPTION '回退事後閘:admin_update_order_item_amount md5 不是上一代 665d49b5a5cc3246bcdba0e3ffc85042'; END IF;
END
$gate_post$;
