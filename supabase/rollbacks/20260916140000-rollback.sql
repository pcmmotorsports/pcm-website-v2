-- 20260916140000-rollback.sql —— 退回 20260916140000_m4b_search_relevance_order.sql
--
-- 做什麼:搜尋函式 DROP + CREATE 回 20260916110000(板 192)兩欄那一代並還原授權;目錄兩支回 20260916120000 那一代。
-- 🔵 TS 沒改 ⇒ 不用先退程式。前置閘釘 140000 那一代 md5 ⇒ 本檔不能重跑。
-- 🔴 退完同批跑 pcm_acl_approve_latest(p_note 帶 20260916140000 與「rollback」)。
...
DO $pre$
BEGIN
  IF (SELECT pg_catalog.md5(p.prosrc) FROM pg_catalog.pg_proc p WHERE p.oid = pg_catalog.to_regprocedure('public.storefront_search_product_ids(text[])')) IS DISTINCT FROM '92639ddd57e8aa92300ef70d210f5840' THEN
    RAISE EXCEPTION '退回前置閘一:搜尋函式不是 20260916140000 那一代 md5 對不上 ⇒ 基底不是預期那一代, 停下對齊';
  END IF;
  IF (SELECT pg_catalog.md5(p.prosrc) FROM pg_catalog.pg_proc p WHERE p.oid = pg_catalog.to_regprocedure('public.search_catalog_by_vehicle(text[],text,text,integer,integer,integer,text,text,text[],integer,integer,timestamp with time zone,text[])')) IS DISTINCT FROM '58e35a8b6f40022ac7545a2488f0dcec' THEN
    RAISE EXCEPTION '退回前置閘二:search_catalog_by_vehicle 不是 20260916140000 那一代 md5 對不上 ⇒ 基底不是預期那一代, 停下對齊';
  END IF;
  IF (SELECT pg_catalog.md5(p.prosrc) FROM pg_catalog.pg_proc p WHERE p.oid = pg_catalog.to_regprocedure('public.search_catalog_by_vehicle_dealer(text[],text,text,integer,integer,integer,text,text,text[],integer,integer,timestamp with time zone,text[])')) IS DISTINCT FROM '1c3f0386b9d20f12f12d5df44a744648' THEN
    RAISE EXCEPTION '退回前置閘三:search_catalog_by_vehicle_dealer 不是 20260916140000 那一代 md5 對不上 ⇒ 基底不是預期那一代, 停下對齊';
  END IF;
END
$pre$;

DROP FUNCTION public.storefront_search_product_ids(text[]);
...
ALTER FUNCTION public.storefront_search_product_ids(text[]) OWNER TO postgres;
-- ACL-GATE-EXEMPT: public.storefront_search_product_ids -- DROP+CREATE 之後還原授權, 集合與正式庫現行 proacl 相同 anon/authenticated/service_role(2026-09-15 唯讀讀到);anon 必須留, 它就是客人的搜尋(20260916140000)
...
DO $post$
BEGIN
  IF (SELECT pg_catalog.md5(p.prosrc) FROM pg_catalog.pg_proc p WHERE p.oid = pg_catalog.to_regprocedure('public.storefront_search_product_ids(text[])')) IS DISTINCT FROM 'ba579fb932a3a8a72946600292405e08' THEN
    RAISE EXCEPTION '退回後置閘一:搜尋函式沒回到 192 那一代 md5 對不上 ⇒ 基底不是預期那一代, 停下對齊';
  END IF;
  IF (SELECT pg_catalog.md5(p.prosrc) FROM pg_catalog.pg_proc p WHERE p.oid = pg_catalog.to_regprocedure('public.search_catalog_by_vehicle(text[],text,text,integer,integer,integer,text,text,text[],integer,integer,timestamp with time zone,text[])')) IS DISTINCT FROM '6b73da535ffaf5628b995f048efd88f6' THEN
    RAISE EXCEPTION '退回後置閘二:search_catalog_by_vehicle 沒回到 120000 那一代 md5 對不上 ⇒ 基底不是預期那一代, 停下對齊';
  END IF;
  IF (SELECT pg_catalog.md5(p.prosrc) FROM pg_catalog.pg_proc p WHERE p.oid = pg_catalog.to_regprocedure('public.search_catalog_by_vehicle_dealer(text[],text,text,integer,integer,integer,text,text,text[],integer,integer,timestamp with time zone,text[])')) IS DISTINCT FROM 'd9ab09783d69dc0a082d7640c59deb3a' THEN
    RAISE EXCEPTION '退回後置閘三:search_catalog_by_vehicle_dealer 沒回到 120000 那一代 md5 對不上 ⇒ 基底不是預期那一代, 停下對齊';
  END IF;
  IF NOT pg_catalog.has_function_privilege('anon', 'public.storefront_search_product_ids(text[])'::regprocedure, 'EXECUTE') THEN
    RAISE EXCEPTION '退回後置閘四:anon 叫不動搜尋';
  END IF;
END
$post$;
