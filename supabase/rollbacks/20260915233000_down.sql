-- 20260915233000 回滾:admin_create_manual_order 貼回 20260915170000 那一代逐字(拿掉客人層級鎖);刪 pcm_bank_transfer_due_at。零資料改動。
-- ⚠️ 下一片(補登記收款 RPC)若已上,它會呼叫 helper ⇒ 先回滾下一片;本檔前置閘②會擋(以本體字面搜尋,不是完整相依分析)。
BEGIN;
SET LOCAL lock_timeout = '5s';

CREATE TEMP TABLE _p02a_down_attr_before ON COMMIT DROP AS
...
DO $pre$
DECLARE v_src text; v_users text;
BEGIN
  ...
  IF v_src IS NULL OR pg_catalog.md5(v_src) <> 'c280e7e6f35660fc454e1754833c3981' THEN
    RAISE EXCEPTION '回滾前置閘①:admin_create_manual_order md5 = % 不是 20260915233000 那一代 c280e7e6f35660fc454e1754833c3981 ⇒ 不回滾, 停', COALESCE(pg_catalog.md5(v_src), '(函式不在)');
  END IF;
  SELECT pg_catalog.string_agg(p.oid::regprocedure::text, ', ') INTO v_users
    FROM pg_catalog.pg_proc p
   WHERE pg_catalog.strpos(p.prosrc, 'pcm_bank_transfer_due_at') > 0
     AND p.oid IS DISTINCT FROM pg_catalog.to_regprocedure('public.pcm_bank_transfer_due_at(timestamptz)');
  IF v_users IS NOT NULL THEN
    RAISE EXCEPTION '回滾前置閘②:還有函式在呼叫 pcm_bank_transfer_due_at(%)⇒ 先回滾那一片', v_users;
  END IF;
END
$pre$;
...
DROP FUNCTION IF EXISTS public.pcm_bank_transfer_due_at(timestamptz);
...
BEGIN
  ...
  IF pg_catalog.md5(v_src) <> 'dc97e3eee72204f893fa95e8355e8f5b' THEN
    RAISE EXCEPTION '回滾事後閘①:md5 = % 不是 20260915170000 那一代 dc97e3eee72204f893fa95e8355e8f5b', pg_catalog.md5(v_src);
  END IF;
  IF EXISTS (
    SELECT 1 FROM pg_catalog.pg_proc p, _p02a_down_attr_before b
  ...
  ) OR NOT EXISTS (SELECT 1 FROM _p02a_down_attr_before) THEN
    RAISE EXCEPTION '回滾事後閘①:admin_create_manual_order 的 ACL / proconfig / SECURITY DEFINER / owner 改前改後不同(不准放寬也不准收緊)';
  END IF;
  IF pg_catalog.to_regprocedure('public.pcm_bank_transfer_due_at(timestamptz)') IS NOT NULL THEN
    RAISE EXCEPTION '回滾事後閘②:pcm_bank_transfer_due_at 還在';
  END IF;
END
$post$;
COMMIT;
