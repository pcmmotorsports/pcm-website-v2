-- ═══════════════════════════════════════════════════════════════════════════
-- 🛑🛑 **先讀這 12 行。本檔是【另一支還原檔的前置】。**
-- ═══════════════════════════════════════════════════════════════════════════
--
-- 🔵 **本檔退完之後, `supabase/rollbacks/20260918020000-rollback.sql` 才重新可用。**
--    原因:本片改寫了 `20260918020000` 也改過的**同一則** `home_banners` 表 COMMENT
--    ⇒ 從本片貼下去那一刻起, 那支還原檔單獨跑會撞它自己的前置閘③,
--      而那道閘的訊息會說「**中間有人改成第三種字面**」—— **那個「第三者」就是本片。**
--    ⇒ 🎯 順序:**本檔 → 20260918020000 的還原檔**。本檔把說明退成 `dcfda5c8…`,
--      而那正是那支檔前置閘要的現值(逐字命中)。
--
-- 🔴🔴 **而反過來說, 那代表**:
--    **退 212 今天 = 連 213 一起退 = 那個 90 天的洞當場打開**
--    ⇒ 有人只是想退掉 212 那幾句說明, 今天**一定會連本片一起退** ⇒ 而本片是安全修補。
--    ⇒ 🛑 **退本片的代價不只是本片。** 這一行寫在最上面, 因為半夜要退的人只會讀最上面那幾行。
--
-- 🔵 **往後的規矩(2026-09-18 立, 主視窗批)**:**一片一句 —— 不要讓兩片各改同一則 COMMENT。**
--    這一整段就是共用一則的代價:後貼的那片會**靜靜弄死前一片的還原檔**, 而沒有東西會說。
-- ═══════════════════════════════════════════════════════════════════════════
