-- 20260912040000-rollback.sql —— 退回 20260912040000_m4b_manrefundnoaudit_rpc_writes_audit.sql
--
-- 退回之後 = 兩支 RPC 回到【不寫稽核】的那一版(逐字抄自 20260909090000 / 20260905440000)。
-- 🔴 **完整 body 內含在本檔**(Sean 2026-09-11 Q4 甲:回捲檔不可寫「請去某檔複製」)。
-- 🛑 **已經寫下的稽核列【不刪】** —— admin_audit_log 是 append-only;
--    那些列記的是真的發生過的事, 退回一支函式不會讓它們變成沒發生。
--    ⇒ 本檔**零 DELETE**。要清那些列是另一個決定, 而它需要一個比「我改壞了」更強的理由。
-- ⚠️ 回退之後「誰做的」又只剩 order_manual_refunds.actor 查得到(後台稽核那一頁看不到)。
-- 🔬 驗收:退回後 `pg_get_functiondef(...) ilike '%admin_audit_log%'` 兩支皆 false;
--    而 `select count(*) from admin_audit_log where action like 'order_refund.manual_%'` **不變**。
