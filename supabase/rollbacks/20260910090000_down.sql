...
--
-- ══ 🔴 而這一支【不用 md5 當前置閘】,理由要寫清楚 ═══════════════════════════
-- forward 用 md5 釘樁(「一定要是那一版才准改」),而 **rollback 不能那樣寫** ——
-- 那會讓「本體後來被別人合法改過」變成【退不掉】
-- ⇒ 📌 **一道為了安全而寫的閘,在災難當下擋住逃生門。**
-- (2026-09-10 codex R1 在 `20260909120000` 上抓過同一個形狀。)
-- ✅ **本支的前置閘只問一件事:那一片真的貼過嗎**(函式體裡有沒有 `v_taxed_residual`)。
--
-- ══ 🔬 本體怎麼來的 ═══════════════════════════════════════════════════════
