-- supabase/rollbacks/20260909050000-rollback.sql
--
-- 把兩支 catalog RPC 還原成 `20260909050000` **之前**那一版(批次日規則還在)。
-- 🔴 **一句話就跑得完**:整支貼進 psql 即可。內容 = 改動前的函式定義**原樣**
--    (公開那支來自 `20260909010000:333-737`、經銷那支來自 `20260909040000:161-587`),
--    不是重寫、不是散文。⇒ 板列 ⟦db-NOROLLBACKARTIFACT⟧ 要的就是這個。
-- 🔵 兩支都含 `p_terms`(13 參數)⇒ 還原之後關鍵字搜尋照樣在。
--
-- ⚠️ **驗收不要只看 `?filter=new` 的件數** —— 那個數字對 `p_terms` **零判別力**:
--    就算還原時把整段關鍵字濾除弄丟、只留參數簽章,新品件數照樣完全正確。
--    ⇒ 對帳查詢見 `docs/plans/2026-09-09-drop-new-arrivals-batch-day-rule-plan.md` §4 那四格
--      (兩個分支 × 關鍵字有命中 / 無命中)。
-- ═══════════════════════════════════════════════════════════════════════════════
