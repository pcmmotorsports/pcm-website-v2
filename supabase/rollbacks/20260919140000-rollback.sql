...
--
-- ══ 🛑 本檔【不過】`migration-static-checks.sh` 規則③ —— 這是知情的 ═════════
-- 手動跑 `bash scripts/migration-static-checks.sh <本檔>` ⇒ **rc=1**,訊息
--   「可授權物件 1 個, 斷言清單列了 0 個 ⇒ 有漏列」。
-- ⚪ **不是本輪造成的**:這支檔從出生就是這樣(HEAD 版本同樣 rc=1)。
-- ⚪ **而它平常不會叫**:repo 裡沒有任何 CI 或 `.husky` 在跑那支閘(2026-09-19 全樹實 grep),
--    它是手動腳本;四輪審查沒人跑過它, 因為那支閘平常沒人拿去掃 `rollbacks/`。
-- 🛑 **刻意不補(主視窗 2026-09-19 裁甲)**:唯一的補法是往本檔加一份 `v_relations` 宣告,
--    而**正片那一份也只被一個長度檢查讀到** ⇒ 加了等於再擺一個量不到東西的儀式。
--    📌 那正是板 216 R4 講的「邊際安全 0」那個形狀, 也是 CLAUDE.md〈做完就停〉的「不加閘」。
-- ⇒ **下一個人看到那個 rc=1 不必修它 —— 先回來讀這一段。**
--
-- ══ 做什麼 ═══════════════════════════════════════════════════════════════
