-- 20260916110000-rollback.sql —— 退回 20260916110000_m4b_storefront_search_fold.sql
--
-- 退回之後 = 20260911150000 的狀態:函式 DEFINER、不折疊、沒有整串優先;索引與 pcm_search_fold 拿掉。
-- 🔴 函式本體 = `20260911150000_m4b_search_ids_definer_uses_indexes.sql` 逐字(md5 892b6c6e2d47acd550e57aad7eaeb88a,
--    = 正式庫 2026-09-15 唯讀讀到的現行代);事後閘驗這個 md5。
-- 🔴 順序是承重的:先把函式換回不引用 pcm_search_fold 的那版 ⇒ 再 DROP 索引(它依賴 pcm_search_fold)⇒ 最後 DROP 函式。
-- 🔵 CREATE OR REPLACE 會把 SET 子句整組換掉 ⇒ 下面照 20260911150000 寫完整那組(SECURITY DEFINER + search_path '')。
-- 前台 TS 零改動 ⇒ 回滾不用重部署。
-- 🔴 鎖(R1 nit):`DROP INDEX` 拿 ACCESS EXCLUSIVE ⇒ 會短暫擋【客人的讀取】(不只寫入), 排隊期間新的讀取也會卡住,
--    最多到 lock_timeout 5 秒 ⇒ 挑客人少的時段跑, 不要在出事當下的尖峰硬跑(先確認問題真的在這支)。
...
DO $pre$
BEGIN
  IF to_regprocedure('public.pcm_search_fold(text)') IS NULL AND to_regclass('public.products_search_fold_trgm_idx') IS NULL THEN
    RAISE EXCEPTION '前置閘①:pcm_search_fold 與索引都不在 ⇒ 20260916110000 看起來沒貼過 ⇒ 停, 不要蓋掉現行函式';
  END IF;
  -- ② 現行函式本體必須正是 20260916110000 那一版(R1 nit:之後若有更新一代也在用 fold, 不要把它蓋回 0911)
  --    md5 = 拋棄式 PG 套完 20260916110000(R2 F1 修正版)後讀的 prosrc(2026-09-16 s110c)
  IF md5((SELECT p.prosrc FROM pg_catalog.pg_proc p WHERE p.oid = 'public.storefront_search_product_ids(text[])'::regprocedure)) <> 'ba579fb932a3a8a72946600292405e08' THEN
    RAISE EXCEPTION '前置閘②:現行函式本體 md5 不是 20260916110000 那一版 ⇒ 有更新的一代 ⇒ 停, 先查再決定回到哪一版';
  END IF;
END
$pre$;
...
DROP INDEX IF EXISTS public.products_search_fold_trgm_idx;
DROP FUNCTION IF EXISTS public.pcm_search_fold(text);

DO $post$
BEGIN
  IF md5((SELECT p.prosrc FROM pg_catalog.pg_proc p WHERE p.oid = 'public.storefront_search_product_ids(text[])'::regprocedure)) <> '892b6c6e2d47acd550e57aad7eaeb88a' THEN
    RAISE EXCEPTION '事後閘①:函式本體 md5 不是 20260911150000 那一版 ⇒ 還原沒成功';
  END IF;
  IF NOT (SELECT p.prosecdef FROM pg_catalog.pg_proc p WHERE p.oid = 'public.storefront_search_product_ids(text[])'::regprocedure)
     OR (SELECT p.proconfig FROM pg_catalog.pg_proc p WHERE p.oid = 'public.storefront_search_product_ids(text[])'::regprocedure) IS DISTINCT FROM ARRAY['search_path=""'] THEN
    RAISE EXCEPTION '事後閘②:DEFINER / search_path 不是 20260911150000 的形狀';
  END IF;
  IF to_regclass('public.products_search_fold_trgm_idx') IS NOT NULL OR to_regprocedure('public.pcm_search_fold(text)') IS NOT NULL THEN
    RAISE EXCEPTION '事後閘③:索引或 pcm_search_fold 還在';
  END IF;
  IF (SELECT p.proacl FROM pg_catalog.pg_proc p WHERE p.oid = 'public.storefront_search_product_ids(text[])'::regprocedure) IS NULL
     OR NOT (has_function_privilege('anon', 'public.storefront_search_product_ids(text[])', 'EXECUTE')
    ...
    RAISE EXCEPTION '事後閘④:EXECUTE 名單不是 anon / authenticated / service_role';
  END IF;
END
$post$;
