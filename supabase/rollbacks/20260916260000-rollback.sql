-- 20260916260000-rollback.sql
--   退回 20260916240000 那一代:通用區重新把整個「維修零件」櫃擋掉(含 gilles 的替換零件)
-- M-4b · 前台窗 pcm-website-v2-ce
--
-- 🔴 **退了會怎樣**:gilles 那 683 群替換零件**重新消失**(客人選了車就看不到)。
--    那正是 Sean 2026-09-17 拍甲要解掉的狀態 ⇒ **退這一支等於退掉他那個拍板。**
-- 🔵 三支本體是 20260916240000 那一代從正式庫原樣 dump 的, md5 釘在前置閘裡。
...
-- ── 前置閘:三支必須是 20260916260000 那一代, 才輪得到退 ──
--   🔴 已經是舊的還硬退 = 把別人的新版本蓋掉。
DO $pre$
BEGIN
  IF (SELECT pg_catalog.md5(p.prosrc) FROM pg_catalog.pg_proc p
     ...
     = '0533d6ce6263455cc04dcacfbbf27cec' THEN
    RAISE EXCEPTION '退回閘:search_catalog_by_vehicle 已經是 20260916240000 那一代 ⇒ 不用退, 停';
  END IF;
END
$pre$;
...
-- ── 事後閘:退回去之後, gilles 的維修零件必須【重新消失】 ──
--   🔴 少了這一格, 一支「什麼都沒退」的 rollback 也會全綠。
DO $rbpost$
DECLARE
  v_n bigint;
BEGIN
  ...
  IF v_n <> 0 THEN
    RAISE EXCEPTION '退回事後閘:通用區還看得到 % 群維修零件 ⇒ 沒退乾淨', v_n;
  END IF;
  RAISE NOTICE '退回事後閘 ok:維修零件重新被擋在通用區外';
END
$rbpost$;

-- ── owner / ACL / SET 照樣要在(理由同正向那支)──
DO $rbacl$
DECLARE
  r record;
BEGIN
  FOR r IN
    SELECT p.proname, pg_catalog.pg_get_userbyid(p.proowner) AS owner, p.proconfig, p.proacl
  ...
  LOOP
    IF r.owner <> 'postgres' THEN RAISE EXCEPTION '退回 ACL 閘:% 的 owner 是 %', r.proname, r.owner; END IF;
    IF r.proconfig IS NULL THEN RAISE EXCEPTION '退回 ACL 閘:% 的 SET 子句不見了', r.proname; END IF;
    IF r.proacl IS NULL THEN RAISE EXCEPTION '退回 ACL 閘:% 的 ACL 是 NULL', r.proname; END IF;
  END LOOP;
END
$rbacl$;
...
-- 🔴 退完手動跑:NOTIFY pgrst, 'reload schema';
