-- ═══════════════════════════════════════════════════════════════════════════
-- 🛑🛑 **先讀這 15 行再跑。2026-09-18 起本檔【不能單獨跑】。**
-- ═══════════════════════════════════════════════════════════════════════════
--
-- 🔴 **你如果看到這個錯誤:**
--       「退回前置閘③:現值 md5 是 …, 既不是本片寫的 dcfda5c8… 也不是貼前的 2f46412f…
--         ⇒ **中間有人改成第三種字面** ⇒ 退下去會把【他的】東西蓋掉 ⇒ 拒退」
--    ⇒ 🎯 **不是有第三者, 是 `20260918030000` 也改過同一句。先跑它的還原檔。**
--       📌 那句錯誤訊息會把你送去找一個不存在的人 —— 而**一個指錯方向的錯誤訊息,
--          比沒有訊息更花時間**。所以這一段寫在最上面。
--
-- ✅ **正確的退法(兩支, 照這個順序)**:
--       ① `supabase/rollbacks/20260918030000-rollback.sql`   ← 先跑這支
--       ② 本檔                                                ← 它的前置閘這時才會過
--    🔬 兩支接得起來, 是逐字命中不是勉強:
--       213 的還原檔把說明退成 `dcfda5c8…` ⇒ 而那正是本檔前置閘要的現值。
--
-- 🔴🔴 **而那個順序有代價, 你要先知道再決定退不退**:
--    **退 212 今天 = 連 213 一起退 = 那個 90 天的洞當場打開**
--    ⇒ 本片只是**幾句說明的更正**, 而它的退路現在綁著一個**真的安全修補**
--      (`20260918030000` 擋的是「信件草稿放過 90 天會自己掙脫『要配到商品』那道閘」)。
--    ⇒ 🛑 **所以「退 212」今天不是一個小動作。** 值不值得, 那是人要判的, 不是這支檔判的。
--
-- 🔵 **往後的規矩(2026-09-18 立, 主視窗批)**:
--    **一片一句 —— 不要讓兩片各改同一則 COMMENT。**
--    共用一則的後果就是這一整段:後貼的那片會**靜靜弄死前一片的還原檔**, 而沒有東西會說。
--    ⚠️ 而它管不到已經貼的這兩片 ⇒ 這兩片靠上面那段話。
-- ═══════════════════════════════════════════════════════════════════════════
