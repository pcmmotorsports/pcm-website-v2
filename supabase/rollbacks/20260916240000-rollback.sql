-- 20260916240000-rollback.sql —— 退回 20260916240000(通用區排除六櫃)
--
-- 做什麼:三支函式的本體換回 20260916220000 那一代(逐字取自 2026-09-16 正式庫 pg_get_functiondef)。
-- 🔵 **簽章不變 ⇒ CREATE OR REPLACE ⇒ owner / ACL 都保留**, 與正片同一個理由。
-- 🔴 退完仍要 NOTIFY pgrst, 'reload schema';(函式體換了)
-- 🔴 退完同批跑 pcm_acl_approve_latest(p_note 帶 20260916240000 與「rollback」)。
-- ⚠️ 前置閘釘「現在是 240000 那一代」⇒ 本檔不能重跑(第二次會被擋 = 已退過)。
-- 🔵 TS 不用退 —— 本片碼沒改。
...
BEGIN
  IF (SELECT pg_catalog.md5(p.prosrc) FROM pg_catalog.pg_proc p
     ...
     = 'f968fc5b9226bab9c2e9e0dddc1d0781' THEN
    RAISE EXCEPTION '退回前置閘:公開列表已經是 20260916220000 那一代 ⇒ 240000 沒貼過或已經退過, 停';
  END IF;
END
$pre$;
...
DECLARE
  ...
  f text; v_oid oid;
BEGIN
  -- ① 三支 prosrc 必須回到 20260916220000 那一版
  IF (SELECT pg_catalog.md5(p.prosrc) FROM pg_catalog.pg_proc p
       WHERE p.oid = pg_catalog.to_regprocedure('public.search_catalog_by_vehicle(text[],text,text,integer,integer,integer,text,text,text[],integer,integer,timestamp with time zone,text[],text)'))
     IS DISTINCT FROM 'f968fc5b9226bab9c2e9e0dddc1d0781' THEN
    RAISE EXCEPTION '退回事後閘①:公開列表的 prosrc 不是 20260916220000 那一版';
  END IF;
  IF (SELECT pg_catalog.md5(p.prosrc) FROM pg_catalog.pg_proc p
    ...
    RAISE EXCEPTION '退回事後閘②:經銷列表的 prosrc 不對';
  END IF;
  IF (SELECT pg_catalog.md5(p.prosrc) FROM pg_catalog.pg_proc p
    ...
    RAISE EXCEPTION '退回事後閘③:側欄件數的 prosrc 不對';
  END IF;
  -- ② owner / ACL / SET / DEFINER 四樣照樣驗(同正片的理由)
  FOREACH f IN ARRAY v_functions LOOP
    ...
    IF (SELECT pg_catalog.pg_get_userbyid(p.proowner) FROM pg_catalog.pg_proc p WHERE p.oid = v_oid) <> 'postgres'
       OR (SELECT p.proacl FROM pg_catalog.pg_proc p WHERE p.oid = v_oid) IS NULL
       OR pg_catalog.has_function_privilege('public', v_oid, 'EXECUTE') THEN
      RAISE EXCEPTION '退回事後閘④:% 的 owner 或 ACL 不對', f;
    END IF;
  END LOOP;
  IF (SELECT p.proconfig FROM pg_catalog.pg_proc p
    ...
    RAISE EXCEPTION '退回事後閘⑤:經銷那支的 SET search_path 或 SECURITY DEFINER 沒回來';
  END IF;
  IF pg_catalog.has_function_privilege('anon', pg_catalog.to_regprocedure('public.search_catalog_by_vehicle_dealer(text[],text,text,integer,integer,integer,text,text,text[],integer,integer,timestamp with time zone,text[],text)'), 'EXECUTE') THEN
    RAISE EXCEPTION '退回事後閘⑥:🔴 anon 拿到經銷目錄執行權';
  END IF;
  RAISE NOTICE '退回完成:三支回到 20260916220000 那一代, owner / ACL / SET / DEFINER 都驗過';
END
$post$;
