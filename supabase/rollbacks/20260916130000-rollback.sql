-- 20260916130000-rollback.sql —— 退回 20260916130000_m4b_manual_refund_settles_pending_refund.sql
--
-- 退回之後 = 兩支 RPC 回到 20260912040000 那一版(登記退款不結清待退款、作廢不解除結清;void 鎖單回 FOR NO KEY UPDATE)。
-- 🔴 **完整 body 內含在本檔**(Sean 2026-09-11 Q4 甲:回捲檔不可寫「請去某檔複製」)。
-- 🛑 **已結清的列、分次退開的差額列、作廢併回的列【不動】** —— 本檔零 UPDATE 零 DELETE。
--    理由:那些列與帳本對得上(對帳 view 用「非卡淨收 − 剩餘應收」算該開多少,結清列不算未結)。
--    把它們改回未結 ⇒ 正確退過的單又變成 rail_mismatch 假異常;而且差額列存在時改回會撞
--    order_pending_refunds_live_order_rail_key。plan §3.7 原寫「UPDATE 回未結」,實作時改為不動,理由即此。
-- ⚠️ 退回之後新登記的退款又不會結清 ⇒ 對帳 view 會再報 rail_mismatch(改前的行為)。
-- 🔬 驗收:事後閘比對兩支 md5 = 20260912040000 那一代(7e1454063b86d6b14bb6d8da738ae8b9 / b5b8d4320d005ff235c513f5ec525fb7)。
...
BEGIN
  IF (SELECT md5(p.prosrc) FROM pg_catalog.pg_proc p
     ...
     NOT IN ('71ac9c1313a3d41fcb0c92eeaf1cbcb1', '7e1454063b86d6b14bb6d8da738ae8b9') THEN
    RAISE EXCEPTION '前置閘①:admin_record_manual_refund 不是 20260916130000(或已退回)那一代 ⇒ 有人換過,停下';
  END IF;
  IF (SELECT md5(p.prosrc) FROM pg_catalog.pg_proc p
       WHERE p.oid = 'public.admin_void_manual_refund(uuid,text,text)'::regprocedure)
     NOT IN ('92094aa979ca3f61779015c762b1ed7d', 'b5b8d4320d005ff235c513f5ec525fb7') THEN
    RAISE EXCEPTION '前置閘②:admin_void_manual_refund 不是 20260916130000(或已退回)那一代 ⇒ 有人換過,停下';
  END IF;
END $$;
...
BEGIN
  FOR r IN
    SELECT p.proname, md5(p.prosrc) AS h, p.prosecdef, p.proconfig::text AS cfg, p.proacl::text AS acl
  ...
  LOOP
    IF r.h NOT IN ('7e1454063b86d6b14bb6d8da738ae8b9', 'b5b8d4320d005ff235c513f5ec525fb7') THEN
      RAISE EXCEPTION '事後閘①:% 退回後 md5=% 不是 20260912040000 那一代', r.proname, r.h;
    END IF;
    IF NOT r.prosecdef OR r.cfg IS DISTINCT FROM '{"search_path=\"\""}'
       OR r.acl IS DISTINCT FROM '{postgres=X/postgres,service_role=X/postgres}' THEN
      RAISE EXCEPTION '事後閘②:% 的 SECURITY DEFINER / search_path / ACL 變了(%, %, %)', r.proname, r.prosecdef, r.cfg, r.acl;
    END IF;
  END LOOP;
END $$;
