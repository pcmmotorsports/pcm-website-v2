-- 20260925050000 的退回:三樣還原成 E2(20260925040000)那一版(逐字抄 E2,函式屬性同 E2)。
--
-- 🔴🔴 **貼之前先把經銷站下線**(Vercel 把 b2b.pcmmotorsports.com 從專案拿掉,確認經銷站不能再建單):
--    退回後三處重新退回一般價、create_order 不再依站別擋 ⇒ 經銷站還開著時,缺經銷價的商品會以一般價成交(刷卡另加 5%)。
--    已建立訂單的付款回呼與 settle 照常完成(它們讀訂單快照,不呼叫 create_order)。
-- 🔴 要退 E2 必須先貼這一支(E2 的退回檔在三樣被改過時會停)。
...
BEGIN
  ...
  IF (SELECT pg_catalog.count(*) FROM pg_catalog.pg_proc p WHERE p.proname = 'create_order' AND p.pronamespace = 'public'::regnamespace) <> 1 THEN
    RAISE EXCEPTION '退回前置閘①:create_order 不是剛好一支(可能有舊多載被還原)⇒ 停。';
  END IF;
  IF (SELECT pg_catalog.md5(p.prosrc) FROM pg_catalog.pg_proc p WHERE p.oid = 'public.create_order(jsonb, uuid, text, jsonb, uuid, text, text, text, text, text, text)'::regprocedure) IS DISTINCT FROM '74716d30da57db777cd319ee48652a03'
     ...
     OR v_vfp IS DISTINCT FROM 'f45a37de325ecd907c3713401c61d14a' THEN
    RAISE EXCEPTION '退回前置閘②:三樣不是 D1(20260925050000)那一版 ⇒ 有人改過,停。';
  END IF;
  ...
  IF (SELECT p.prosecdef::text || '|' || pg_catalog.array_to_string(p.proconfig, ',') || '|' || p.provolatile::text || '|' || pg_catalog.pg_get_function_arguments(p.oid)
    ...
    RAISE EXCEPTION '退回前置閘③:create_order / get_effective_prices 的屬性(SECURITY DEFINER、search_path、volatility、參數與預設值)與預期不同 ⇒ 有人用 ALTER 改過,停。';
  END IF;
  IF pg_catalog.to_regprocedure('public.dealer_discounted_amount(uuid, uuid, integer)') IS NULL
    ...
    RAISE EXCEPTION '退回前置閘④:找不到 E2 的 dealer_discounted_amount / dealer_discount_apply ⇒ 停。';
  END IF;
END
$gate$;
...
BEGIN
  ...
  IF pg_catalog.has_table_privilege('anon', 'public.products_list_dealer', 'SELECT')
    ...
    RAISE EXCEPTION '退回事後閘③:products_list_dealer 被開了 SELECT ⇒ 停。';
  END IF;
  ...
  IF v_vfp IS DISTINCT FROM '42ddb5f87a1096361f42a6db13255918'
    ...
    RAISE EXCEPTION '退回事後閘⑤:寫出來的三個本體指紋與預期不同 ⇒ 停(退回檔會對不上)。';
  END IF;
END
$post$;
