-- 20260915170000 回滾:admin_create_manual_order 貼回 20260915060000 那一代逐字(缺 requested ⇒ 又變回猜 true)。ACL 重發基準。零資料改動。
BEGIN;
...
BEGIN
  ...
  IF v_src IS NULL OR pg_catalog.md5(v_src) <> 'dc97e3eee72204f893fa95e8355e8f5b' THEN
    RAISE EXCEPTION '回滾前置閘:md5(prosrc) = % 不是本代 dc97e3eee72204f893fa95e8355e8f5b ⇒ 不回滾, 停', COALESCE(pg_catalog.md5(v_src), '(函式不在)');
  END IF;
END
$pre$;
...
BEGIN
  ...
  IF pg_catalog.md5(v_src) <> 'd97986f066c64f0c7f8baaf9cc5202f8' THEN
    RAISE EXCEPTION '回滾事後閘:md5(prosrc) = % 不是 20260915060000 那一代 d97986f066c64f0c7f8baaf9cc5202f8', pg_catalog.md5(v_src);
  END IF;
END
$post$;
COMMIT;
