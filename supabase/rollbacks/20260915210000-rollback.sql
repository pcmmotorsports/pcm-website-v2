-- 20260915210000-rollback.sql —— 退回 20260915210000_m4b_unpaid_cancel_cutoff_by_cancelled_at.sql
-- 兩個物件退回改前逐字定義:view = 20260907230000:327-390, function = 20260903070000:71-156(CREATE → CREATE OR REPLACE)。
-- 🔴 同時要 revert TS 那半(scanner 拿掉 created_at 那一行), 否則寄信端與告警端口徑又分岔。
-- 🔴 退回的代價:「部分取消 → 逾時自動取消」的單會回到誤寄;cutoff 前成立、之後取消的單回到收不到信。

BEGIN;
SET LOCAL lock_timeout = '5s';

-- 🔴 只退「本檔目標那一代」(或已經退回的舊版 = 重跑);其他指紋 ⇒ 有人又改過, 不准蓋掉(與正檔前置閘同理, codex R2 MF3)
DO $pre$
...
BEGIN
  ...
  IF v_view NOT IN ('fba321f731525877fe90800f9001e099', '2f728f34a5551441714d7e217f8f55f9')
     OR v_fn NOT IN ('e7c1cc4796e74ef6da08c6142cfa6cb1', '0db807b1f0f8864a1d7c6c3793dfa9e6') THEN
    RAISE EXCEPTION USING MESSAGE = '退回前置閘:定義指紋 view=' || coalesce(v_view, 'null') || ' fn=' || coalesce(v_fn, 'null')
      || ' 不是 20260915210000 目標也不是舊版 ⇒ 有人又改過, 停下';
  END IF;
END
$pre$;
...
BEGIN
  ...
  IF pg_catalog.strpos(v_def, 'payment_expired') <> 0 THEN
    RAISE EXCEPTION '退回後置閘:view 仍含 payment_expired ⇒ 沒退回去';
  END IF;
  RAISE NOTICE '[20260915210000-rollback] 退回完成';
END
$post$;
