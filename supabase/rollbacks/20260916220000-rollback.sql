-- 20260916220000-rollback.sql —— 退回 20260916220000_m4b_catalog_fit_scope.sql
--
-- 做什麼:DROP 三支帶 p_fit_scope 的新簽章, 建回 13 / 13 / 7 參那一代本體
--   (本體逐字取自 2026-09-16 正式庫 `pg_get_functiondef`, prosrc md5 釘在前置閘), 並還原 ACL。
-- 🔴 DROP + CREATE 同一交易;建回的是【裸 CREATE】⇒ 撞名當場紅, 不會靜靜蓋掉。
-- 🔴 退完同批跑 pcm_acl_approve_latest(p_note 帶 20260916220000 與「rollback」)。
-- ⚠️ 前置閘釘新簽章存在 ⇒ 本檔不能重跑(第二次會被擋 = 已退過)。
-- 🔵 **要先退程式再退板**:前台送 p_fit_scope 的碼若還在線上, 退完會 PGRST202。
...
BEGIN
  IF pg_catalog.to_regprocedure('public.search_catalog_by_vehicle(text[],text,text,integer,integer,integer,text,text,text[],integer,integer,timestamp with time zone,text[],text)') IS NULL
    ...
    RAISE EXCEPTION '退回前置閘一:新簽章不在 ⇒ 20260916220000 沒貼過或已經退過, 停';
  END IF;
  IF pg_catalog.to_regprocedure('public.search_catalog_by_vehicle(text[],text,text,integer,integer,integer,text,text,text[],integer,integer,timestamp with time zone,text[])') IS NOT NULL THEN
    RAISE EXCEPTION '退回前置閘二:13 參那支已經在了 ⇒ 有人先建了, 停下對齊';
  END IF;
END
$pre$;

DROP FUNCTION public.search_catalog_by_vehicle(text[],text,text,integer,integer,integer,text,text,text[],integer,integer,timestamp with time zone,text[],text);
DROP FUNCTION public.search_catalog_by_vehicle_dealer(text[],text,text,integer,integer,integer,text,text,text[],integer,integer,timestamp with time zone,text[],text);
DROP FUNCTION public.catalog_facet_counts(text[],text[],text,text,integer,text[],text[],text);

CREATE FUNCTION public.search_catalog_by_vehicle(p_categories text[], p_brand text DEFAULT NULL::text, p_model text DEFAULT NULL::text, p_year integer DEFAULT NULL::integer, p_offset integer DEFAULT 0, p_limit integer DEFAULT 25, p_sort text DEFAULT 'recommend'::text, p_category text DEFAULT NULL::text, p_brand_slugs text[] DEFAULT NULL::text[], p_price_min integer DEFAULT NULL::integer, p_price_max integer DEFAULT NULL::integer, p_new_since timestamp with time zone DEFAULT NULL::timestamp with time zone, p_terms text[] DEFAULT NULL::text[])
...
CREATE FUNCTION public.catalog_facet_counts(p_category_keys text[], p_brand_keys text[], p_brand text DEFAULT NULL::text, p_model text DEFAULT NULL::text, p_year integer DEFAULT NULL::integer, p_selected_categories text[] DEFAULT NULL::text[], p_selected_brand_slugs text[] DEFAULT NULL::text[])
...
DECLARE
  v_functions text[] := ARRAY[
    'public.search_catalog_by_vehicle(text[],text,text,integer,integer,integer,text,text,text[],integer,integer,timestamp with time zone,text[])',
    'public.search_catalog_by_vehicle_dealer(text[],text,text,integer,integer,integer,text,text,text[],integer,integer,timestamp with time zone,text[])',
    'public.catalog_facet_counts(text[],text[],text,text,integer,text[],text[])'
  ]::text[];
  f text; v_oid oid;
BEGIN
  -- ① prosrc 必須回到貼板前量到的那一版
  IF (SELECT pg_catalog.md5(p.prosrc) FROM pg_catalog.pg_proc p
    ...
    RAISE EXCEPTION '退回事後閘①:公開列表的 prosrc 不是貼板前那一版';
  END IF;
  IF (SELECT pg_catalog.md5(p.prosrc) FROM pg_catalog.pg_proc p
    ...
    RAISE EXCEPTION '退回事後閘②:經銷列表的 prosrc 不是貼板前那一版';
  END IF;
  IF (SELECT pg_catalog.md5(p.prosrc) FROM pg_catalog.pg_proc p
    ...
    RAISE EXCEPTION '退回事後閘③:側欄件數的 prosrc 不是貼板前那一版';
  END IF;
  -- ② 新簽章不能還在
  IF pg_catalog.to_regprocedure('public.search_catalog_by_vehicle(text[],text,text,integer,integer,integer,text,text,text[],integer,integer,timestamp with time zone,text[],text)') IS NOT NULL THEN
    RAISE EXCEPTION '退回事後閘④:新簽章還在';
  END IF;
  -- ③ SET / SECURITY / ACL
  IF (SELECT p.proconfig FROM pg_catalog.pg_proc p
       WHERE p.oid = pg_catalog.to_regprocedure('public.search_catalog_by_vehicle_dealer(text[],text,text,integer,integer,integer,text,text,text[],integer,integer,timestamp with time zone,text[])'))
     IS DISTINCT FROM ARRAY['search_path=""'] THEN
    RAISE EXCEPTION '退回事後閘⑤:經銷那支的 SET search_path 沒回來';
  END IF;
  IF (SELECT p.prosecdef FROM pg_catalog.pg_proc p
       WHERE p.oid = pg_catalog.to_regprocedure('public.search_catalog_by_vehicle_dealer(text[],text,text,integer,integer,integer,text,text,text[],integer,integer,timestamp with time zone,text[])')) IS NOT TRUE THEN
    RAISE EXCEPTION '退回事後閘⑥:經銷那支掉了 SECURITY DEFINER';
  END IF;
  ...
  FOREACH f IN ARRAY v_functions LOOP
    ...
    IF (SELECT p.proacl FROM pg_catalog.pg_proc p WHERE p.oid = v_oid) IS NULL
      ...
      RAISE EXCEPTION '退回事後閘⑧:% 的 PUBLIC 沒收掉', f;
    END IF;
  END LOOP;
  RAISE NOTICE '退回完成:三支回到 13 / 13 / 7 參那一代, ACL 與屬性都驗過';
END
$post$;
