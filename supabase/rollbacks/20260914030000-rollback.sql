-- 20260914030000-rollback.sql —— 退回 20260914030000_m4b_manual_order_tier_override.sql(第 8 代 → 第 7 代)
--
-- 🔴 順序 = 先 revert code(UI 不再送 p_tier)再跑本檔:DB 回 11 參時, 新 UI 送 12 個名字 ⇒ PGRST202 建不出單。
-- 🔴 已用第 8 代建出的單, `tier_at_checkout` 留著(那是資料, 不是本檔的事);audit 列 append-only, 零 DELETE。
-- 🔴 反向的指紋代價(codex R1 nit ④):第 8 代建的單指紋含 tier, 退回第 7 代之後原樣重送那些 request_id ⇒ 永久 P858B。
--    與 apply 那一頭同一種代價, 方向相反;重送只發生在按下去那一刻的補救, 影響面 = 退回那一刻正在飛的請求。
-- 本體 = `20260910090000:213-1141` 逐字(第 7 代);ACL 與 COMMENT 照 `20260905130000` §6 / §6b(DROP 會帶走)。
...
BEGIN
  IF v_new IS NULL THEN
    RAISE EXCEPTION '退回前置閘一:找不到 12 參那一代 ⇒ 沒貼過 20260914030000, 不必退';
  END IF;
  -- 🔴 要退的必須【就是第 8 代那一版】(codex R1 must-fix ②):簽章相同的後續修補(第 9 代)被本檔 DROP 掉會回報成功。
  ...
  IF pg_catalog.md5(v_src) <> '3021a009074b0caa3f1ceb5cd5c90cdd' THEN
    RAISE EXCEPTION USING MESSAGE =
      '退回前置閘二:12 參那一代的本體 md5 不是第 8 代(3021a009074b0caa3f1ceb5cd5c90cdd;實得 ' || pg_catalog.md5(v_src)
      || ')⇒ 它已經被後面的版本改過, 本檔不知道怎麼退, 停下人工對齊';
  END IF;
  IF v_old IS NOT NULL THEN
    RAISE EXCEPTION '退回前置閘三:11 參那一代【還在】⇒ 兩支多載並存, 本檔不覆寫任何一支, 停下人工對齊';
  END IF;
END
$pre$;
...
BEGIN
  SELECT p.prosrc, pg_catalog.array_to_string(p.proacl, ',') INTO v_src, v_acl
    FROM pg_catalog.pg_proc p WHERE p.oid = pg_catalog.to_regprocedure('public.admin_create_manual_order(uuid,uuid,text,text,text,text,jsonb,jsonb,integer,jsonb,text)');
  IF v_src IS NULL THEN RAISE EXCEPTION '退回後置閘一:11 參那一代沒建回來'; END IF;
  IF pg_catalog.md5(v_src) <> 'e703456a3766558851d57c1932c87c1b' THEN
    RAISE EXCEPTION USING MESSAGE = '退回後置閘二:本體 md5 不是第 7 代那一版(實得 ' || pg_catalog.md5(v_src) || ')';
  END IF;
  IF v_acl IS NULL OR v_acl NOT LIKE '%service_role=X/%' THEN
    RAISE EXCEPTION '退回後置閘三:proacl 裡看不到 service_role';
  END IF;
  IF pg_catalog.to_regprocedure('public.admin_create_manual_order(uuid,uuid,text,text,text,text,jsonb,jsonb,integer,jsonb,text,text)') IS NOT NULL THEN
    RAISE EXCEPTION '退回後置閘四:12 參那一代還在';
  END IF;
  IF (SELECT pg_catalog.count(*) FROM pg_catalog.pg_proc p JOIN pg_catalog.pg_namespace n ON n.oid = p.pronamespace
    ...
    RAISE EXCEPTION '退回後置閘五:public.admin_create_manual_order 不是恰好 1 支';
  END IF;
END
$post$;
