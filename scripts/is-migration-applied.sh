...
#    · 🔴 **`COMMENT ON …`(2026-09-08 線 -db 實測第三個受詞)** ⇒ 註解**不留歷史**
#      ⇒ 一支更晚的 `COMMENT` 把同一格覆蓋掉之後,「前一支跑過沒有」**從 DB 端永遠答不出來**。
#      實例:`staff.is_manager` 有三支改過同一格(`20260726120000` len 48 /
#      `20260828070000` len 702 / `20260829193000` len 1132),而線上逐字等於**最後那一支**
#      ⇒ 📌 而那也意味著**它不重要** —— 現況由最後那支決定,前面那支貼不貼都不改變任何東西。
#    · 🔴 **純 `REVOKE` / `GRANT`(第四個受詞)** ⇒ 它不建任何物件 ⇒ 本支產不出有判別力的查詢。
#      而它有一個**更壞**的性質:`REVOKE` 在「revoker 有權限而**無轉授權**」時只印 `WARNING`,
#      **`rc` 仍是 0、ACL 一格沒動**(2026-09-08 線 -db 拋棄式 PG 三發實測;
#      revoker 對該物件**零權限**時才是 `ERROR`)。
#      ⇒ 🛑 **那種片的事後斷言不能問「有沒有出錯」, 必須問「ACL 現在長什麼樣」。**
#      ⇒ ✅ **正面實例**:貼板 94(`20260905120000`)的事後斷言問的就是 ACL
#         ⇒ 三個 `REVOKE` 都跑了而 24 項權限仍在 ⇒ 它**擋住了自己**、整筆回滾、正式庫零改動。
