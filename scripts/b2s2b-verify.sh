...
# RE-PIN 2026-08-10 (347-3a): 20260810110000 -> 20260810120000 = admin_search_orders date range
#    (search RPC only; DROP+CREATE of admin_search_orders, no shipping table/function touched;
#    grep recompute|order_item_qty|oiqs|shipment on the new migration = 0 hits
...
# RE-PIN 2026-08-10 (OP2b): 20260810120000 -> 20260810130000 = order_payments reversal
#    invariants (A9 inverse-amount + immutable columns + no-delete) and DROP of the
#    dormant gate, all in one transaction. order_payments is the OP1 receipts ledger;
...
#    Ledger: full re-record done (w7-coverage record all); MD5_HELPER_4AXIS unchanged.
# RE-PIN 2026-08-10 (L5b-1): 20260810130000 -> 20260810140000 = payment_refunds +
#    payment_refund_events two-table refund ledger (P5B02 no-cycle guard, composite FK,
#    append-only triggers). New tables only, zero touch on shipping/receipt objects;
#    grep recompute|order_item_qty|oiqs|shipment on the new migration = 0 hits (verified)
#    => shipping oracles unchanged, no md5 re-measure needed. Main-window re-pin, P-line migration.
#    Ledger: full re-record done (w7-coverage record all); MD5_HELPER_4AXIS unchanged.
# RE-PIN 2026-08-10 (3c-3): 20260810140000 -> 20260810150000 = COMMENT-only fix on
...
#    零 DML 零表 DDL 只建 view ⇒ 與 S2b 被測面無交集、預期 MD5_HELPER_4AXIS 不變(未重定義被釘 helper);主視窗重釘後 record all 由 SA-* 說話。
# 🔴 重釘 2026-08-10 晚(#352-a1):20260810220000 -> 20260810230000 = 到貨登錄 schema 地基(D 窗)。
# 🔴 重釘 2026-08-11(#352-a2):20260810230000 -> 20260810233000 = 到貨登錄兩支 writer RPC(D 窗)。
#    🔴 該片與 S2b 被測面**無交集**(receipt 寫入 RPC vs shipped 重算)⇒ 預期 MD5_HELPER_4AXIS 不變;
#    預期不是量測——主視窗重釘後當場整支重跑(w7 收割線),由 SA-* 與 helper 指紋格自己說話。
#    🔴 收穫:本檔釘值不叫 LINE_TIP ⇒ D 窗七支重釘與主視窗 grep LINE_TIP 親驗**雙雙漏掉它**,
#    w7 record all 才抓到(RECEIPT-b2s2b 紅)——「換個錨名就逃過重釘檢查」,親驗改 grep 舊前綴字面本身。
...
#    於新 migration = **0 命中**(主視窗收割時實跑)⇒ MD5_HELPER_4AXIS 未動。帳:同批重錄(record all)。
# 🔴 重釘 2026-08-10(OP-A12):20260810200000 -> 20260810210000 = admin_reverse_manual_payment
#    (人工軌沖銷入口 + OP5/A12 兩支開權;只碰 orders/order_payments/staff 三張表,零 DDL 於出貨面)。
#    grep recompute|order_item_qty|oiqs|shipment 於新 migration = 0 命中(實跑)⇒ MD5_HELPER_4AXIS 未動。
# 🔴 重釘 2026-08-10(OP5):20260810160000 -> 20260810200000 = admin_record_manual_payment
#    (人工軌 bank_transfer/cash 收款登錄 RPC;只碰 orders/order_payments/staff 三張表,零 DDL 於出貨面)。
#    對新 migration 跑本檔要求的 grep recompute|order_item_qty|oiqs|shipment = **0 命中**(實跑,不是推的)
#    ⇒ 出貨面 oracle 不受影響、不需重量 md5;MD5_HELPER_4AXIS 未動。帳:同批重錄。
# 🔴 重釘 2026-08-10(OP3):20260810150000 -> 20260810160000 = confirm_order_payment 的 card 腿
#    (B 窗原鏈 130000->160000;merge dev 後前兩跳=上面 L5b-1/3c-3 兩段,主視窗收割時聯集改齊)
...
# 🔴 這 10 格(對照 + T1-T9)已抽到 `scripts/b2s2b-tsync-cells.sh`(2026-08-21 W5)——
#    它們**完全不碰 DB**,而埋在本檔裡就只有「起完一整座 cluster 的人」跑得到。
#    抽出來是為了讓它們也能獨立跑(`sh scripts/b2s2b-tsync-cells.sh --selftest`)。
#    🔴 **是 source 不是複製** —— 同一份程式碼、兩個呼叫端,不會漂移。
. "$(dirname "${BASH_SOURCE[0]}")/b2s2b-tsync-cells.sh"
b2s2b_tsync_cells
