...
⚠️ 其他射程限制
   ...
   · 整行被 ~~…~~ 劃掉的視為【已撤回】, 不計
   · 上下文窗 = 前 3 行 + **後 3 行**。落點寫在更遠的地方 ⇒ 本工具看不到 ⇒ 會誤報
     ⛔ ~~第一版是「前 3 行 + 後 1 行」~~ ⇒ 🔴 而【回填一個號的自然形狀就是寫在那句話下面】
     ⇒ 2026-09-01 我自己回填三處, 其中一處的號落在第 2 行下面 ⇒ **本工具照樣報它沒落點**
     ⇒ ⇒ 📌 **那把尺會懲罰它自己想鼓勵的那個行為。**
"""
    ...
    for f in files:
        ...
        for i, line in enumerate(lines, 1):
            ...
            if STRUCK.search(line):
                ...
                continue
            # 🔴 後面要看 3 行, 不是 1 行(2026-09-01 實測抓到的):
            #    【回填】一個號的自然形狀就是【寫在那句話的下面】——
...
def selftest() -> int:
    ...
    cases = [
        ...
        ('🔴 回填寫在下面 2 行', '// 已回報主視窗立 backlog。\n// 一句中間的話\n// 已落 backlog #671', 'landed'),
