...
# ══ 收割問句(主視窗 `-f1` 2026-09-07 05:5x 裁:寫在這裡, 不改 CLAUDE.md)══════════
# 🔴 **為什麼放這一支**:每一片收工都會叫它一次 ⇒ 它是**唯一一個每片都會撞到的點**。
# 🔬 **問句的來源是量到的**(板列 `⟦15-HALFREVERT⟧`;2026-09-07 `-ship` 拿它稽核自己同一晚的三顆 commit):
#    **3 顆 / 5 處 / 抓到 2 處沒有證人** —— 而那三顆**當時每一顆都跑了突變、都全綠、
#    都回報過「突變各紅在不同的地方」**。
#    ⇒ 🎯 **所以它擋的不是「沒做突變的人」, 是【做了突變而以為做完了的人】。**
# 🛑 **走 stderr, 不走 stdout** —— 呼叫端常寫 `> /dev/null`(把 marker OK 那行吃掉),
#    問句走 stderr 才活得下來。⚠️ **而 `2>&1` 連它一起吃掉** ⇒ 📌 **這一行的到達率不是 100%,
#    它是【呼叫端怎麼寫】的函數** —— 要它真的每片都被看到, 呼叫時不要把 stderr 也導掉。
{
  echo "──────────────────────────────────────────────────────────────"
  echo "❓ 收工問句(板列 ⟦15-HALFREVERT⟧, 一句, 答不出來就還沒做完):"
  echo "   這一片改了幾處會被斷言的東西?  ⇒  我有沒有【那麼多發】突變, 每一發【只退回一處】?"
  echo "   🔴 「全部退回會紅」證不出這件事 —— 它在【全改對】與【只改對一處】兩個世界印同一個紅。"
  echo "──────────────────────────────────────────────────────────────"
} >&2
