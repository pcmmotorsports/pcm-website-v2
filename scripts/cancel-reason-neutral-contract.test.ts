...
describe('取消原因:兩格遮罩 = 中性句(Sean 2026-08-30「2 乙」)', () => {
  ...
  const SHAPES: ReadonlyArray<readonly [string, number]> = [
    ...
    // 尺自檢:一個現造字面必須數到 0(這格與上面兩個 0 的差別是它不是【曾經對過又改掉】的字面)。
    // 🔴 2026-08-30 換字(線C `-b4`,`-48` 指名;原字面 `zzq6641`):
    //    ~~原註解「它從來沒存在過」~~ **當時是真的,而 2026-08-30 量到它已流進**
    //    **9 支 repo 檔 + 14 支交件** —— 它變成全隊慣用的負對照字 ⇒ 那句話不再成立。
    // 📌 機制:**一個負對照字面的效力來自「沒有人用它」,而它一旦好用大家就會開始用**
    //    ⇒ **效力與普及度成反比** ⇒ 所以負對照的字面要【每次現造】,
    //      不要挑一個「我印象中沒人用」的固定字。
    // ⚠️ 而本格的效力【不靠】那個字全世界沒人用 —— 靠的是**它不在這支 migration 的函式體裡**;
    //    換字當下已驗:`qx7v2m9k4b` 在 repo 0 支、信箱 0 支(正對照 `zzq6641` ⇒ 9 支)。
    ['qx7v2m9k4b', 0],
  ];
  ...
  it('負對照:一個不存在的字面必須數到 0（恆回真的尺在這裡會紅)', () => {
    expect(countOf(functionBody(latestDefining()!), 'qx7v2m9k4b')).toBe(0);
  });
});
