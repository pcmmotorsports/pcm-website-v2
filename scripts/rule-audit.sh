...
audit() {  # $1=ledger 路徑;回傳非 0 表示有紅
  ...
  while IFS="$_US" read -r id rule file pat expect since source || [ -n "$id" ]; do
    ...
    if [ "$expect" = "==0" ]; then
      ...
      # 🔴 字面存活 ≠ 語義存活:艦隊慣例是【劃線留痕、不刪字】⇒ 被廢止的規則會永遠綠。
      # 🔴🔴 判定方式:**把 `~~…~~` 那幾段【拿掉】之後,pattern 還在不在** ——
      #    不可以用「整行含不含 ~~」判(GR 2026-08-18 抓到的反向假紅):
      #    一行更正常常【同時含】劃線的舊句與現行的新句
      #    ⇒ 用整行判會把那種更正誤判成訃聞,而**三週後第一顆這種紅會被當成真廢止**。
      _alive=$(grep -F -- "$pat" "$file" | sed 's/~~[^~]*~~//g' | grep -cF -- "$pat" || true)
      if [ "$_alive" = "0" ]; then
        printf '🔴 %s  命中 %s 行,而【拿掉 ~~ 劃線段之後一行都不剩】⇒ 這條看起來已被廢止\n     %s\n' "$id" "$_n" "$rule"
      else _ok=1; fi
    else
  ...
  # 🔴 這一句的字面在自檢裡被 `grep -v '^🔴 綠的意思是'` 排除(它是說明不是 finding)——
  #    **兩處同一份字面、零互釘**:改這裡的開頭字就要同時改那裡(GR-050 F4)。
  printf '🔴 綠的意思是【那段字面還在那個檔裡而且不在劃線區】,不是【那條規則有在被遵守】。\n'
  printf '   規則還在而全隊都無視 ⇒ 它照樣是綠的。(G6 2026-08-18 提:守門要自己聲明射程)\n'
  # 🔴 綠數 0 也要紅,而且要【說為什麼】—— 一個紅了而沒有句子的判定,跟本工具要治的病同形
  if [ "$_red" = "0" ] && [ "$_green" = "0" ]; then
    printf '🔴 一列都沒有驗到(綠 0/紅 0)⇒ 這張表可能被清空或表頭被竄成 0。**這不是通過。**\n'
  fi
  [ "$_red" = "0" ] && [ "$_green" -gt 0 ]
}
...
if [ "${1:-}" = "--selftest" ]; then
  ...
  _w() {  # $1=標題 $2=表 $3=期望rc ;印判定
    ...
    # 🔴 這裡原本是【紅句前綴的白名單】(`^🔴 R0|^🔴 表頭|^🔴 列數`)——
    #    而 audit() 對「整張表清空」印的是 `🔴 一列都沒有驗到…`,三個前綴一個都不中
    #    ⇒ 那句紅【存在於 $T/o 裡】,只是被這一行濾掉 ⇒ 世界四紅得沒有句子。
    #    (GR 2026-08-19 實跑抓到;負向對照=世界二/三/五/八 都印得出指名的紅句。)
    # ⇒ 改成【收所有 ^🔴,只排掉那句固定的說明】:以後新增的紅句【預設會出現】,
    #    而不是預設消失。白名單的失敗方向是「安靜漏掉」,黑名單是「多印一行」—— 後者看得見。
    _f="$T/f"
    grep -E '^🔴|^列 ' "$T/o" | grep -v '^🔴 綠的意思是' > "$_f" || true
    # 🔴 **不設顯示上限**(GR-050 F1)——原本 `| head -3` 配一句「… 還有 N 行(完整輸出 $T/o)」,
    #    而那個路徑 ①`trap 'rm -rf "$T"' EXIT` 會把整個目錄刪掉 ②每個世界共用同一個 `$T/o`
    #    ⇒ 訊息印在世界四底下,而檔裡裝的是世界九的輸出 —— **指路指到別的房間**。
    #    自檢輸出本來就短(實測每個世界最多 2 行)⇒ 全印最省,順帶讓截斷那條死分支消失(F3)。
    sed 's/^/    /' "$_f"
    # 🔴 **顯示層自己的負向對照**(GR-050 F2):`_w` 原本唯一判定是 rc,
    #    而 `$_f` 從頭到尾沒被檢查 ⇒ 有人把過濾改回白名單,九個世界照樣全綠 ✅ ——
    #    **那正是 2026-08-19 剛發生過的那一次**(紅得沒有句子而 rc 是對的)。
    #    ⇒ 該紅的世界必須【印得出至少一行 🔴】,否則本支自檢自己算壞掉。
    if [ "$3" = "1" ] && ! grep -q '^🔴' "$_f"; then
      printf '    🔴 這個世界該紅,而顯示層一行紅都沒印出來 ⇒ 過濾器把紅句吃掉了\n' >&2
      _rc=顯示層壞掉
    fi
    [ "$_rc" = "$3" ]
  }
  ...
  echo
  D9="$T/fix.md"
  printf '~~舊句:我自己發明的前置檢查是有效的~~ ⇒ 現行:我自己發明的前置檢查,結構上保護不了它\n' > "$D9"
  awk -F'\t' -v f="$D9" 'BEGIN{OFS="\t"} /^R008/{$3=f} {$1=$1; print}' "$LEDGER" > "$T/m9.tsv"
  _w "== 世界九(GR 反向假紅):一行【同時含】劃線舊句與現行新句 ⇒ 必須【綠】,不可誤判訃聞 ==" "$T/m9.tsv" 0 || P=1
  echo
  if [ "$P" = "0" ]; then
    echo "✅ 九個世界都對:規則在/更正留痕=綠;不見/掉列/空表/空pattern/整條劃線/==0被違反/欄位位移 = 紅且 rc=1"
    exit 0
  fi
