...
#
# ══ 🔴🔴 **射程宣告:它答得出【兩層】問題, 而它不會告訴你你在問哪一層** ══════════
#    這一段在最上面, 因為挑錯層的關鍵字會拿到一個【看起來很乾淨的零命中】。
#
#      第一層  問「**他答過沒**」        ⇒ 關鍵字用【**他會講的話**】
#      第二層  問「**這件事還在不在**」  ⇒ 關鍵字用【**碼裡的識別字**】
#
#    🛑 **而本檔的五個載體裡有三個是【碼】** ⇒ 它同時服務兩層 ⇒ **而它不會分辨你在問哪一層。**
#    ⇒ 📌 **所以第三句規矩是:【先想清楚你在問哪一層, 再挑字】。**
#
#    🔴 **實例(2026-09-01 線 `-5b`, 而它是【拿第二層的措辭去問第一層】)**:
#      它要查 `coupon_p2_redeem_rpc.sql:87` 的「已交主視窗端成決策題」還算不算數,
#      而它拿「**做法錯了要換**」去查 —— **那是我們的話, 不是 Sean 會講的話**
#      ⇒ 零命中 ⇒ **而它拿那個零命中當答案。**
#      🔵 而正解是第二層:拿碼裡的識別字(`coupon_redeem_order_problem`)去掃
#      ⇒ 當場撈到 `20260831155000` 那支 migration 存在、而 RPC `:190` 真的在呼叫它
#      ⇒ ⇒ **那一條不是「判不出來」, 是【它描述的那個世界已經不存在了】。**
#
#    🔴 **同一天的另外兩條, 三條共用同一句「已回報主視窗」, 而真相是三種【相反】的東西**:
#      · 做完了而沒有人回頭劃 · 已修而未生效(修法未 apply)· 真的沒有落地
#      📌 **⇒ 那句話的資訊量是零 —— 它在三個相反的世界裡讀起來一模一樣。**
#      ⇒ **而分辨它們要【兩層都問】:第一層問有沒有人拍過, 第二層問碼裡還在不在。**
#
#    🛑 **而「挑錯層」這件事, 本檔的 `--selftest` 【驗不到, 而且不可能驗得到】** ——
#      它不知道你想問哪一層, 而**兩層用的都是同一組 grep**。
#      ⇒ 📌 **所以這一段只能是【給人讀的宣告】, 不能是一道檢查。**
#      🔴 而那正是它寫在檔頭【最上面】的理由:一個驗不到的失效模式,
#         它唯一的防線就是【被讀到】—— 而寫在下面等於沒寫。
#
# 用法:
...
sweep() {
  ...
  #    而它單獨印, 讀的人自己決定要不要點進去。
  # 🟢 **「全掃會不會慢到沒有人跑它」那個代價量了(2026-09-07 `tidy`;⟦f3-ASKGATEGLOB1⟧ 標為未量)**:
  #    `~/pcm-mailbox` 實際 **6,875 支 .md**(全部檔 7,446)⇒ 這一發 grep 連跑三次 **312 / 161 / 156 ms**;
  #    整支工具 **5.0 秒(冷) / 1.8 秒(熱)**。⇒ 📌 **代價可忽略, 放寬不必再猶豫。**
  #    🟢 正對照:`"開站前必須做完"` ⇒ ②b 命中 **10 支**, 含當初漏掉的那份
  #      `清單-開站前必須做完-20260901.md`(它既不叫 `*決策*` 也不叫 `*等Sean*`, 正是 ② 那個 glob 白名單漏掉的檔)。
  #    ⚪ 負對照:現造關鍵字 ⇒ ②b **0 支** ⇒ **這一段有判別力, 不是恆非零。**
  #    ⚠️ 射程:數字是**本機**量的, 而它會隨信箱長大;**檔數與耗時要一起讀** —— 只帶耗時的話,
  #      下一個人不知道那是幾支檔跑出來的。
