...
#
# 🔴 **一句話界線(2026-08-16 跑完三題之後收斂)**:
#    **它答「這個【識別字】被哪些程式碼位置用到、各自有沒有對應測試」。**
#    🔴 **值經過 DB / 產物 / 瀏覽器計算之後的下游,它一律看不到。**
#    實測三題各斷在不同地方 ⇒ 適用邊界是立體的,不是一條線:
#      · `TIER_LABEL`   純 code,沒斷        ⇒ 工具答得完整(撈到兩張表分岔)
#      · 經銷價          斷在【build 產物】  ⇒ 工具答不了(正確答案是空集合,而它對空集合 exit 1)
#      · actor cookie   斷在【DB】          ⇒ 工具答一半(看得到守門,看不到它守的顯示點)
#
# 🔴 **它只回答這一個問題,不是通用工具。**
...
echo "  🔴🔴 6. **若某支【測試】出現在 ② 的「只有渲染字才抓得到」那一格,"
echo "         而它對應的【顯示點】不在 ① 的清單裡 ⇒ 這條鏈中間【斷過】"
echo "         (值離開了程式碼:DB / build 產物 / 瀏覽器計算)。**"
echo "         那個顯示點本工具看不到,要另外找觀察點。"
echo "         ⇒ 這個不對稱是本工具【唯一能指向自己盲區】的訊號,看到就停下來想。"
echo "         實例:actor 那題 —— audit-log-table.test.tsx 在那一格,"
echo "         而 audit-log-table.tsx 不在清單裡(它讀的是 DB 撈回來的一列)。"
