...
#
# ══ 🔴 設計準則:【問錯名字時,「查無」就是正確答案】(2026-08-31 線DB 實例)═══════
#   有人要改 `#866` 那道上限閘的訊息,先問了 `pcm_manual_refund_rail_cap_enforce`
#   —— 那是**從檔名推出來的名字**(檔名是 `..._rail_cap_enforce.sql`)。
#   ⇒ 本工具印 **「查無」**。而真名是 `pcm_manual_refund_rail_cap_guard`。
#   🔴 **而【隔壁那一支 migration】裡有另一支叫 `pcm_manual_refund_rail_cap`**
#      (`20260824010000` 建的算式函式;`_guard` 是 `20260824011000` 建的 trigger 函式 —— 不同檔、不同用途)
#      ⇒ 若本工具做「幫你找最接近的那個」, 它會回那一支
#      ⇒ ⇒ 📌 **而那個人會拿到一個【看起來完全合理】的答案, 然後 CREATE OR REPLACE 錯的函式。**
#   ⇒ 🛑 **所以不要加模糊比對 / 相似名建議 / 「你是不是要找…」。**
#      在一把【要被拿去抄簽章】的工具上, 那不是便利功能, 是一個**製造錯誤**的功能:
#      **它把「我問錯了」這個會讓人停下來的訊號, 換成一個會讓人繼續走的答案。**
#   🔵 而正確的補救是使用者自己做的:看到「查無」⇒ 開那支 migration 找真名 ⇒ 再問一次。
#      **兩次查詢, 換一次不會錯抄。**
#
...
#
# ══ 🔴🔴 本工具【自己犯了兩次】它要防的那個病(-48 要求寫進檔頭, 而理由是對的:
#    這比任何說明更能讓下一個人相信它懂自己在講什麼)═══════════════════════════
#   本工具存在的理由是:**「掃不到」與「真的沒有」印同一個答案**。而 R2 抓到它兩次:
#     ① `-d "$MIG"` 檢查原本排在 dispatch 之【後】⇒ 在一棵沒有 migrations 的樹上,
#        `--raw` 回 **rc=0 且零輸出**。成因是量到的:**BSD grep 對不存在的 `-r` 目標
#        回 rc=1 不是 2** ⇒ 我寫的 `-gt 1` 不觸發 ⇒ **「查無」與「我根本沒去查」被壓成同一格。**
#     ② `APPLIED.tsv` 不存在時, 帳本欄**整欄印「未記」而零警告** ——
#        而本檔自己就寫著「『這東西從來沒被建過』那個結論太醒目, 會被當成真的發現」。
#   📌 **⇒ 兩條是同一句話的兩次。**⇒ 已各補一道出聲(⑭ 的回歸格 / 帳本檔缺失的警告行)。
#   🔴 **而兩條都不是讀出來的, 是 reviewer【真的去跑】才現形的** ——
#      ①靠一棵合成的空樹, ②靠把 `APPLIED.tsv` 移走。
#      ⇒ **一支工具讀起來對, 與它在【它自己防的那個世界裡】表現對, 是兩件事。**
#
