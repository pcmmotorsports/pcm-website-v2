...
# ============================================================
#
# 🔴🔴 2026-08-30 起本檔的【對客文字期望值已過期】(Sean 拍「2 乙」+ 逐字「寫很官方就好」):
#    price_change 與 internal_error 的對客文字已改成「訂單已取消,詳情請洽客服」,
#    不再是「依您要求取消」。本檔 D4/D6 那兩列 fixture 與 I2 的「同映射對」都要跟著改:
#      · 同映射對改成 price_change -> internal_error（兩者現在同文字）
#        ~~customer_request -> price_change~~ 已失效：它們不再同文字 ⇒ 那一對會被
#        【文字比對】順手抓到，而 I2 要測的是 hash 與不變式 ⇒ 用不同文字的一對 = 這格
#        失去判別力而仍然綠。
#    ⚠️ 本檔【沒有】跟著改，理由是它的身分閘早就合法失效(md5 pin 是舊版函式本體)——
#       改一支跑不起來的 harness 只會讓下一個人以為它可以跑。
#    ⇒ 誰寫繼任的 a8a3/a8a4-verify，請從這裡抄期望值，不要從下面那張表抄。
#       落點 supabase/migrations/20260830020000_m4b_e10_cancel_reason_neutral.sql
#
set -uo pipefail
