...
# ⛔ ~~`MIG_DIR = 'supabase/migrations'`~~ —— **2026-09-16 刪掉這個死常數。**
#    它宣告一個白名單目錄, 而本閘【實際的分母】是:
#      · staged 那條路 ⇒ 任何 staged 的 `.sql`(見 `staged()`, 2026-09-05 放寬)
#      · `--audit`     ⇒ `git ls-files '*.sql'`(實跑 726 支)
#    🔴 **而它零引用** —— 全檔只出現在宣告那一行, 沒有任何地方用它。
#    📌 **它曾經誤導過一次判斷**(2026-09-16:有人讀到這一行就回報「本閘沒有掃
#       `supabase/rollbacks/`」, 而實燒之後三發全紅 —— 洞不存在)。
#    ⇒ 🎯 **一個死的宣告, 讀起來與一個活的設定【逐字相同】** —— 這就是留它的成本。
