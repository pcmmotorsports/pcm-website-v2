...
#      🟢 **[2026-09-07 `-ship`, 主視窗 `-f1` 批]這一條【沒有被推翻】, 而它有一個不牴觸的落點**:
#        `scripts/harvest-chain.sh` 的 **`REPORT_ONLY`** 家族 —— 那一族逐字「**只報不擋**」,
#        `verdict` 不把它的 rc 算進放行判定 ⇒ 🎯 **上面那句話擋的是「假紅會擋人」, 而那裡的紅不擋人。**
#        而它**在 `EXPECT_GATES` 裡** ⇒ **少跑一支抓得到**(那正是本檔 17 天沒人按的那個病)。
#        🔴 **所以「必跑」那兩個字仍然成立** —— 我們沒有讓它變成必跑, 我們讓它變成**常跑而不擋**。
#        ⚠️ 而代價照抄不掩蓋:**一個真紅在那裡不會停下任何東西**, 要有人去讀那一行。
#
...
# ✅✅ **[2026-08-31 11:43 `-15`]:兩個庫【都】乾淨了 —— `both` ⇒ rc=0、30 格 PASS、FAIL 0。**
#    授權:**Sean 本人 2026-08-31 在本窗逐字打「Q-探針 = 甲。可以對正式站跑那個唯讀曝露探針。
#    這是我 Sean 本人在你這個視窗打的字,不是轉述。」** ⇒ 下方那道「授權存疑」已解除。
#    兩庫的 E 白名單實得逐字(它自己講的, 不是抄的):
#      `Only the following schemas are exposed: public, graphql_public`
#    🔵 quote 那側先前停在 C 格(404)⇒ 已校準成 `suppliers`(見下), 現在 15 格全過。
#    🛑 **而「今天乾淨」不等於「明天乾淨」** —— 本檔是【偵測】不是【牆】:
#       偵測頻率 = 暴露窗, 而目前沒有任何東西會叫人跑它(見下方那一節)。
...
# ✅ **[2026-08-31 11:4x 解除]:Sean 本人在本窗逐字授權(見檔頭上方)。下面這段留著當歷史 ——
#    它記的是【擋對了的那一次】,而一條從來沒擋過東西的限制與一條被繞過的限制長得一樣。**
...
run_db() {  # run_db <site|quote>
  case "$1" in
    ...
    quote)
      ...
      # 🔵🔵 **[2026-08-31 11:4x `-15`]:這一格【已校準】** —— Sean 本人在本窗逐字授權
      #    「Q-探針 = 甲。可以對正式站跑那個唯讀曝露探針」之後,實測選出來的。
      #    ⚠️ ~~原句:「報價單庫沒有 customers 這張表 ⇒ 這一格至今未校準,而我不猜該用哪張表」~~
      #       —— 那句對,而**它的解法不是猜,是【問那個庫】**:唯讀 GET,七張候選表各打一發。
      #    實測(唯讀 GET `?select=*&limit=0`,同一把 anon key,2026-08-31 11:4x):
      #      suppliers 401 · products 401 · quotes 404 · customers 404
      #      orders 404 · users 404 · profiles 404
      #      負對照 現造 `zznosuchtable_zz` ⇒ **404**
      #    📌 ⇒ **401 與 404 在這個庫上分得開** —— 而那正是 C 格要證的那件事
      #       (「404 不是打錯路徑的通用回應」)。選 `suppliers`:存在、而 anon 被擋。
      #    🔴 **為什麼不選 products**:它也回 401,兩者都合格;選 suppliers 是因為那個庫的
      #       PostgREST hint 自己建議 'public.suppliers' ⇒ **它是那個庫自己講出來的名字**,
      #       而 products 是我從網站庫的詞彙外推的。**用別人家的詞彙做的尺會很穩定地印零。**
      c_path='/rest/v1/suppliers?select=*&limit=0'
