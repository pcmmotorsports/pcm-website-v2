...
for N in "$@"; do
  echo "════════ #${N} ════════"
  # 🔴 標題分隔符有兩種:`### #123.` 與 `### #123 ·`(2026-08-19 G2 量:601 條裡 78 條用「·」)
  #    只認「.」⇒ 那 78 條(≈13%)會被誤報成「③ 查不出來」。射程印在輸出裡。
  START="$(grep -nE "^### #${N}[ ]?[.·]" "$BACKLOG" | head -1 | cut -d: -f1)"
  if [ -z "$START" ]; then
    echo "  ③ 查不出來 —— $BACKLOG 裡找不到 '### #${N}.' 或 '### #${N} ·'(號碼打錯,或那條不存在)"
    continue
  fi
  # 條目範圍 = 本標題到下一個 '### #' 之前
...
🔴 **它只認【完整路徑】,不認裸檔名** —— 而 backlog 是人寫的,兩種寫法都有。
   實數(2026-08-19,可重跑):
     帶完整路徑的檔名(去重)  **572**   grep -oE '(apps|packages|supabase|scripts|docs)/…' | sort -u | wc -l
     出現過的檔名(含裸檔名)  **613**   grep -oE '\b[A-Za-z0-9._-]+\.(tsx|ts|css|sql|sh)\b' | sort -u | wc -l
     ⇒ **41 支檔只以裸檔名出現,本腳本對它們零判別力**
   🔴 **而這個不修是刻意的**:改成比對裸檔名之後,`page.tsx`(在 backlog 出現 **72** 次)、
     `types.ts`(**69**)、`index.ts`(**40**)會讓它**永遠在響** —— 響到沒人看,比瞎區更糟。
