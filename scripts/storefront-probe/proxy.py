...
# 🔴 效度限定(runbook §8-f):替身【不驗密碼】—— 密碼欄任何字串都收。
#    ⇒ 不要拿這條鏈驗任何「擋不擋得住」的題目,它在那些題目上恆綠。
# ⚠️ ~~「任何字串都登得進去」~~ **這半句 2026-08-21 起已不成立**,被本檔 `:139-160` 推翻:
#    登入與註冊分家之後,**登入只認【auth.users 裡已經存在】的 email**;不存在 ⇒ 400 `invalid_grant`,
#    而 storefront 把它顯示成「登入失敗,請稍後再試」。
#    ⇒ 現成可用帳號:`probe@example.com`(甲)/ `probe2@example.com`(乙,換帳號那族用)。
#    🔴 為什麼要劃掉而不是刪掉:2026-08-23 有【兩個窗】照這句話推論「所以應該登得進去」,
#       其中一個因此把兩件驗收記成「環境壞了、量不到」。**一句曾經為真、被同一支檔別處推翻的話,
#       比沒有那句話更糟 —— 它讀起來像權威。**(來源:窗 A 讀碼指出;補寫於 2026-08-23)
