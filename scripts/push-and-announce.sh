...
#       —— 那要真推才走得到,仍等 Sean。
# ── 🔴 推【被驗過的那一顆】,不是推 tip(2026-08-18 19:3x 主視窗裁,而它一直是本檔的設計)──
#   本腳本第一個參數吃的是**完整 sha**,不是分支名 ⇒ `git push origin <sha>:dev`
#   ⇒ **推出去的就是那發四綠涵蓋到的那一顆**,而那之後別人繼續 commit **完全不影響它**。
#
#   🔴 **為什麼要寫在這裡**:主視窗與我都短暫地掉進同一個沒有人規定過的預設 ——
#      「推 = 推 dev 最新那顆」⇒ 於是需要一道「全窗停止 commit」的凍結令來讓「最新」不動。
#      那道令 2026-08-18 當晚由 Sean 當場問倒(逐字大意:「我們應該沒有要停手的意思」),已撤回。
#   ⇒ **「四綠之後又多了 N 顆」不是缺陷,是下一輪的工作量。** 不要為了讓它變 0 而叫任何人停手。
#   📎 形狀:那是**在一條本來就不會出事的路上裝機制**
#      (memory `feedback_a-guard-on-a-safe-path-is-net-negative`)。
#      判別句:**這個機制擋的那條路上真的會出事嗎?** 這裡不會 —— 推指定 sha 時別人有沒有 commit 無關。
#
#   ⚠️ **而「被驗過」不等於「是對的」** —— 那發綠答的是「這棵樹編得起來、測試綠」。
#      第三個參數(四綠 sha)印進信裡的那段就是在講這件事,不要跳過它。
SHA="${1:-}"
WHO="${2:-}"
GREEN="${3:-}"
