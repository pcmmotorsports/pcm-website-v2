...
#
# 天花板/範圍: 用起訖索引切片改檔時抓「靜默吃掉鄰居」;比對【宣告的預期 Δ】vs【實得 Δ】,不符就還原 + 非零退出。
#   這份清單是我想得到的那些, 而我最可能漏掉的是「呼叫端把預期 Δ 也算錯 —— 兩個錯的數對得上就一起放行」。
# 天花板/量具: 行數對得上但改錯內容,它一律放行 —— Δ 是【數量】不是【語意】。
#   這份清單是我想得到的那些, 而我最可能漏掉的是「增 N 刪 M 對得上、但改的是別的地方」。
set -uo pipefail
