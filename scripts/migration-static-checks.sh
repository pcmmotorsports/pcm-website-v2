...
# 🔴 標題不用反引號 —— 雙引號裡的反引號會被 shell 當【命令替換】吃掉(CLAUDE.md 明文禁)
#    而它不報錯:那一段會靜靜地變成空字串, 句子照樣讀得通。
echo "── (8) manual_failed 那個共用邊界的配對句(2026-09-08 加)──────────────"
# 🔴 為什麼在這裡而不是另掛一支:掛在本支上, 它就自動吃到入口
#    `migration-new-file-static-checks.sh` 那一刀(**已落地的檔一律不掃**)——
#    🛑 而那一刀非有不可:實測【22 支既有 migration】碰到那兩支函式,
#       而它們都已 apply、連註解都不可改 ⇒ 另掛一支會擋死全隊。
#    📌 ⇒ 接在既有的過濾後面, 不自己發明第二把「哪些檔該掃」的尺。
if python3 "$(dirname "$0")/manual-failed-pairing-gate.py" "$F" > /tmp/mfpg.$$ 2>&1; then
  echo "✅ 沒有碰到那兩支共用邊界的函式,或碰到了而檔頭帶了配對句"
else
  cat /tmp/mfpg.$$
  RC=1
fi
