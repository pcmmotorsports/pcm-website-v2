...
#
# ══ 🔴🔴 「我餵幾條 vs 它跑幾支」在這裡是【嚴格相等】═══════════════════════════
#   ⛔ ~~餵 basename(去副檔名), 而 vitest 的位置參數是【子字串過濾】⇒ 它跑的 ≥ 我餵的~~
#      **2026-09-05 改掉**。舊做法有兩個病:
#      ① **多撈**:餵 `coupon` 也撈到 `coupon-cap-*` ⇒ 第四個數對不上, 只能寫成不等式
#      ② 🔴 **有的樹上它會【整批落空】**:線【信】`-mail` 同一支腳本 **餵 53 ⇒ 跑 1, rc=2**
#         (本支的 `judge_ran` 當場擋下 —— 那一格是 2026-09-05 才補的, 它第一次承重就接住了)
#         而同一小時我這棵樹 餵 55 ⇒ 跑 74。**同一支腳本, 兩棵樹印相反。**
#   ✅ 改成餵【完整相對路徑】—— 一條路徑是它自己的子字串, 而**不會是別人的**
#      ⇒ 📌 **第四個數從「≥」變成「=」, 那是一個【更強的】判準, 不是同一個判準換寫法。**
#      🔬 2026-09-05 實測(本樹):餵 55 ⇒ **跑 55**、788 格、20 秒(basename 版 55 ⇒ 74、21 秒)。
#   🛑 **而我沒有查出 `-mail` 那棵樹為什麼會落空** —— 換成路徑之後那個世界不再發生,
#      而**「不再發生」與「我知道為什麼」是兩件事**。若日後路徑版也落空, 那個成因還在等人。
#
# ══ 天花板 ═══════════════════════════════════════════════════════════════════
...
judge_ran() {
  ...
  if [ "$_ran" -ne "$_fed" ]; then
    echo "🔴 它跑的($_ran)【不等於】我餵的($_fed)"
    echo "   ⇒ 餵的是完整相對路徑 ⇒ 一條路徑只會匹配它自己 ⇒ 正常情形【嚴格相等】。"
...
SQLEOF
  ...
  [ "$?" = "2" ] && echo "  ✅ judge_ran:跑 9 ≠ 餵 52(少)⇒ 2" || { echo "  🔴 judge_ran 對「跑得比餵的少」沒反應"; ok=1; }
  # 🔴 餵路徑之後【多】也是壞掉 —— basename 版時「多」是正常的, 那一格判準跟著換了。
...
# 🔴 餵【完整相對路徑】不餵 basename(見檔頭「嚴格相等」那節)
NAMES="$(list_scan_tests)"
FED=$(printf '%s\n' "$NAMES" | grep -c . )
if [ "$FED" -lt 1 ]; then
