...
#
# ══ 🔴 「那四道」是哪四道 —— 具名(`-auth` 2026-09-08 補;**舊字面留著**)═══
#   ⛔ **原句只寫「那四道」而沒有寫是哪四道** ⇒ 而本檔具名的自證標記有 **10 種**
#      (見下面 `KEYS_FROZEN` 那一行)⇒ 🛑 **想複查的人查不了 —— 不是他沒查, 是那句話沒有可查的形狀。**
#   ⇒ 📌 **一條沒有人能判斷它壞了沒的紀律, 與沒有那條紀律, 在往後每一天印同一個答案。**
#
#   ✅ **對照(每一條都附【我判定的依據】, 而依據強弱不同 —— 不要把它們讀成一樣確定)**:
#     ④ ⇒ `W6A-ORDERED-NO-DEADLOCK`
#          🟢 **確定** —— ④ 那一行**自己就寫了這個名字**。
#     ① ⇒ `W6A-SYNC-REACHED`
#          🟢 **強** —— 該格的斷言文字自己在講 ① 那個坑:
#          `:419` 逐字「至少有一輪**兩邊都卡在迴圈中途的同步點**上 ⇒ 編排真的把交錯製造出來了」
#          而 ① 的病正是「barrier 只設在進入 RPC 那一刻 ⇒ 兩邊沒重疊過」。
#     ② ⇒ `W6A-MUTANT-RAN`
#          🟢 **強** —— `:418` 逐字「突變輪至少有一輪**兩邊都真的執行了**復原 ⇒ 下面兩格量到的是真的併發面」
#          而 ② 的病正是「突變函式每次呼叫都錯、什麼都沒跑到,而自證只看**定義文字**」。
#     ③ ⇒ `W6A-FIXTURE`
#          🟡 **這一條是【我的判讀】, 不是原文寫的** —— ③ 的病是「自證**假設了箱子的狀態**
#          (硬寫死作廢→復原)」, 而 `:253` 那一格**去驗**箱子的狀態
#          (逐字「兩張含**相同兩個品項**的包裹都已『出貨→作廢』」)⇒ 假設 → 改成驗證, 對得上。
#          ⚠️ **而我排除掉的候選要寫出來**:`W6A-RESTORED`(`:444` 講的是**還原突變過的 trigger**,
#          不是箱子)· `W6A-REPLAY`(`:210` 講 migration 重放)· `W6A-BARRIER`(`:288` 講 barrier 本身 = ①那一族)。
#          🔴 **知道③ 是哪一道的人:如果不是 `W6A-FIXTURE`, 請直接改這裡。**
#
#   🛑 **而這一段【不改變】上面那句紀律** —— 它只是把「那四道」變成查得到的名字。
#      📎 十種標記的完整清單是下面的 `KEYS_FROZEN`(它本身就是一份凍住的契約)。
#
# ══ 🔴 v1 編排為什麼沒有判別力(留著當反例)═══════════════════
