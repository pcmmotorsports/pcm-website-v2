...
const PINNED_SHIPMENTS_COLUMNS: readonly string[] = [
  ...
  // 🔴 **2026-09-10 新增兩欄(⟦ship-DISPATCHORDER⟧, `20260910130000`)。同樣不是順手加名單。**
  //    先答這道閘要人答的那一句:「客人看得到這一欄, 可以嗎?」
  //    ✅ **答:可以, 而理由與上面那一格同一條**:
  //    ① 兩個都是**時點**, 意思是「我們什麼時候向新竹叫這一箱的車」——
  //       在**客人自己那一列**上(RLS 只讓他看自己的箱), 不牽涉任何別人;
  //    ② 叫到車之後我們**主動寄出貨信告訴他**貨要走了 ⇒ 藏這兩個時點沒有保護到什麼;
  //    ③ 敏感度比同名單上已開放的 `hct_raw_response`(新竹回的整包原始回應, 含標籤圖)
  //       **低得多**。
  //    🛑 **而有一個後果要寫出來, 不要假裝沒有**:
  //      `attempted` 有值而 `dispatched` 空 = **叫車途中中斷**
  //      ⇒ 📌 **客人理論上看得出「這一箱出過狀況」, 而他無從得知是什麼、也無從行動。**
  //      ⇒ 我判**可接受**(它不會讓他做出錯誤決定, 而那個狀態本來就要有人去處理),
