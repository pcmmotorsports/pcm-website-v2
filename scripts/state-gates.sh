...
HEADER
  ...
  for f in "${APPLICABLE[@]}"; do
    ...
    awk -v FN="$base" -v WP="$WRITE_PAT" -v GP="$GATE_PAT" '
      ...
      # 🔴 **整行是一個 SQL 字串字面 ⇒ 跳過**(⟦f3-STATEGATESCOMMENT⟧, 2026-09-07)。
      #   病:本表被引用的用途是「這條路上有沒有閘」= **盤點**;而它把 `COMMENT ON … IS` 底下
      #     那些**一行一個字串字面**的註解當成「這個函式改什麼狀態」抓進來 ⇒ 實錘三筆,
      #     其中一筆抓到的是**一句被劃掉的錯註解**(`20260901020000…:183`), 印在權威位置上。
      #   ⚠️ 既有的 `$0 !~ /^[ \t]*--/` 只擋【行首 `--`】;那些字串**不以 `--` 開頭**
      #     ⇒ 那道濾網結構上看不到它們。
      #
      #   🎯 **判準(`4dcc92e8f` 留下的)+ 它的射程(A 2026-09-07 補, 免得下一個人套錯)**:
      #     **守門容得下誤報(有人被叫住 30 秒), 盤點容不下(數字會變成別人的分母)。**
      #     **判別:那個位置有沒有東西在【機器讀】它。**
      #     ⇒ runbook 是**人讀的散文** ⇒ 那裡的刪除線是**對的**, 不要去改;
      #       `order-state-gates.md` 是**盤點** ⇒ 不是。
      #     🛑 **不要把「剝刪除線」當成全域規則。**
      #
      #   ⛔ ~~第一版我寫「`COMMENT ON` 到敘述結尾 `;` 之間整段跳過」~~ ⇒ **剝過頭, 而是【量】出來的**:
      #     那個狀態旗標找不到結尾就一路吃到檔尾 ⇒ 資料行 75⇒67 · `SET payment_status` 32⇒27,
      #     **連 `IF v_order.cancelled_at IS NOT NULL` 這種真的閘都被吃掉**
      #     ⇒ 📌 **那正是本列列尾警告的「比較危險的那個方向」(漏報), 而它靠兩個數才看得見。**
      #   ✅ **改成無狀態的窄規則**:整行 trim 之後**以單引號開頭** ⇒ 那是字串字面, 不是碼。
      #     真的 SQL 述句不會以單引號開頭 ⇒ **這條吃不到碼, 也不會失控吃到檔尾。**
      #   🔵 `\047` = 單引號的八進位 —— **直接寫單引號會把整個 awk 程式的 shell 單引號提前關掉**
      #     (我第一版就是這樣炸的:`syntax error near unexpected token`)。
      /^[ \t]*\047/ { next }
      $0 ~ WP && $0 !~ /^[ \t]*--/ {
