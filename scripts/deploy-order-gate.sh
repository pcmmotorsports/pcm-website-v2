...
comment_only_drift() {  # $1=路徑 $2=帳本記的 raw sha $3=被推的 rev;回 0 = 已降級(且已印警告)
  ...
  # 🔵 `notfound` 時**只印一句指路**, 邏輯一個字不動(仍然維持 PENDING、仍然不猜)。
  #    🔴 為什麼要這一句(2026-09-08 線【DB】`-db` 量到, 主視窗 A 裁「改訊息不改邏輯」):
  #      四支 notfound 逐支去比對之後, **三支的帳本 sha 與 `~/pcm-mailbox/貼板-*/` 底下
  #      同版本號那一份【逐字相同】** ⇒ 📌 **帳本記的是【真的被貼下去的那一份】,
  #      而 repo 那一份後來被改了。⇒ 帳本不是壞的, 它比 repo 準。**
  #      (第四支 `20260902060000` 的 sha 掃遍信箱 3326 支 .sql 一份都不命中 ⇒ 那是另一種情況。)
  #    🛑 **而本支【不去】掃那個目錄, 那是刻意的** —— 貼板那批檔**不在版控**
  #      (主視窗 2026-09-08 00:2x 裁:「那支檔不進 repo;壽命 = 這批落完」)
  #      ⇒ 一道閘去依賴一個**會被清掉**的位置, 它會安靜地退回 notfound,
  #        而那時候**沒有人知道它曾經找得到**。
  #    ⇒ 📌 **一道閘可以誠實地說「我看不到那裡」, 而那比它去看一個會消失的地方好。**
  #    🎯 母題:**一把尺只認得一個位置, 而東西住在三個位置。**
  if [ "$kind" = "notfound" ]; then
    echo "gate:    ⇒ **git 歷史裡**找不到帳本記的那一版 ⇒ 維持 PENDING, 不猜。" >&2
    echo "gate:    🔵 而下一步【不是】重記 sha —— 先去 \`~/pcm-mailbox/貼板-*/\` 找同版本號那一份比一次:" >&2
    echo "gate:       帳本可能記的是【當時真的被貼下去的那一份】, 而 repo 那一份後來被改了。" >&2
    echo "gate:       (2026-09-08 實測:四支 notfound 裡 **三支**是這個原因;第四支掃遍信箱不命中。)" >&2
    echo "gate:    🛑 本閘**刻意不去掃那個目錄** —— 它不在版控、會被清掉;閘去依賴它會安靜地失效。" >&2
  fi
  [ "$kind" = "drift" ] || return 1
