...
--
-- 🛑🛑 **這道閘【證不到】什麼**(R3-C3, 明寫不假裝):
--    · 上面那行跑法**把旁路字面直接印在檔案裡** ⇒ 📌 **要繞過它的人不必自己想, 只要複製。**
--      ⇒ 它不是「防止繞過」, 它是**讓繞過變成一個明示的動作** —— 而那正是它全部的價值。
--    · 🔴 **`ALTER DATABASE … SET pcm.code_reverted = 'yes'` 或 `ALTER ROLE … SET` 那一層,
--      本閘【永遠綠而零訊號】** —— 一旦有人那樣設過, 之後每一次跑都自動通過, 而沒有人會知道。
--    · 有人為了讓某個自動化腳本跑通而把 `SET` 塞進去 ⇒ **它就永遠是 `yes`**。
--      ⇒ 判別句:**這道閘今天擋過誰?** 答不出一個人名, 它可能已經被洗成裝飾了。
--    · 它問的是「你有沒有宣告退過碼」, **不是「碼真的退了」** —— 後者本閘結構上答不出來
--      (SQL 那一端看不到部署狀態)。
