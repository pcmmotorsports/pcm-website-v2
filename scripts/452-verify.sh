...
#
# ── 🔴 這把尺量不到什麼(上面那段講的是【範圍】,這段講的是【量具本身】)──────────
#   ⚠️ 兩段都要在。**一份只寫了「範圍」的天花板,會被讀成「已經有天花板了」**,
#      而缺的那半正是綠燈的可信度。(2026-08-27 立;成因見下)
#
#   🔴 **本 harness 只在【本機 PG 17.x】上跑過** —— `:91` 逐字強制
#      `initdb --version | grep -q ' 17\.'`,`:95` 起的是本機拋棄式庫。
#      ⇒ 「本機 PG 17.x 上全綠」與「正式站那個 PG 上全綠」是**兩個宣稱**,
#        而本檔的綠燈只支撐前者。
#      ⚠️ **而正式站的 PG 版本【未確認】—— 沒有人查過。**
#      ⇒ 缺的那一道檢查很具體:**到正式庫跑一次 `select version()`**,
#        然後回來把這一段改寫成「兩邊同版 / 不同版而差異在哪」。
#      📌 **標「未確認」而附得出缺哪道檢查 = 可以被下一個人接手的狀態;空著不是。**
#        (這一句本來差點沒被寫下來 —— 因為寫它的人想「等我知道正式站版本再寫」,
#         而那會讓它永遠不被寫。**「我不知道 X」本身就是天花板的一部分。**)
#
#   ⚠️ 連帶:`initdb` / `pg_ctl` 走的是 **PATH 上那一支**,而本機與 CI 的 PATH 可能不同支
#      ⇒ 「PG17」這個條件擋得住大版本落差,**擋不住同大版本內的小版本差異**。
# ============================================================
set -uo pipefail
