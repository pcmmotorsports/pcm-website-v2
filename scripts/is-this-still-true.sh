...
sweep() {
  ...
  grep -n -- "$KW" "$REPO/docs/launch-todo.md" 2>/dev/null \
    | awk '{
        ...
        # 🔴🔴 **態【不得寫死 a[2]】**(2026-09-08 `-auth`;主視窗 A 批)。
        #    板子**在同一支檔裡有不只一種表** —— 37 件那張的表頭是
        #    | 行 | 錨 | 誰受什麼傷 | 派給 | ⇒ 那些列的 a[2] 是【行號】不是【態】。
        #    🔬 實測(改之前):`b4-NEWAPI1` ⇒ L1650 印 態= 671
        #    ⇒ 📌 **671 看起來像一個答案**, 而它是行號。
        # 🛑 而這與那個【裸豎線】的病是**兩件事**:豎線那半 2026-09-02 就修了(上面那段就是),
        #    而症狀沒有消失 ⇒ **兩個不同的病共用同一個症狀。**
        # ✅ **修法用【封閉集】**:態只認 open / doing / parked / done 四個值,
        #    在整列裡找那一格;找不到 ⇒ 印一句說得出【為什麼沒有】的話,
        #    **不回退去猜一欄** —— 答不出來要長得像答不出來, 不能長得像一個答案。
        #    (那句判準是 `scripts/board-dispatch-triage.py` 寫的, 這裡沿用它的形狀。)
        # 🔵 為什麼不像參照實作那樣讀表頭:本段吃的是 `grep -n` 的輸出 ⇒ **一列一列來,
        #    看不到它上面的表頭** ⇒ 封閉集是這一層唯一問得出來的判準。
        line=$0
        gsub(/\\\|/, "\001", line)
        n=split(line, a, "|")
        ...
        for (i=1;i<=n;i++) {
          v=a[i]; gsub(/^[ \t]+|[ \t]+$/, "", v)
          if (v=="open" || v=="doing" || v=="parked" || v=="done") { st=v; break }
        }
        printf "%s 態=%s | %s\n", a[1], st, substr(a[4],1,110)
      }' > "$TMP"
