...
[ -f "$MIG" ]  || die "找不到 $MIG"
[ -f "$DOWN" ] || die "找不到 $DOWN"
# 🔴 **`mktemp -t <prefix>` 是 BSD 方言 —— 在 GNU coreutils 上【必炸】**(理由同 `l4a1-verify.sh`)。
#    2026-08-27 實例:`.husky/commit-msg:76` 同款寫法讓 CI 紅了 7 格(修在 `7efbe93d`)。
#    ⇒ 模板形,兩邊都認得。**仍然是【檔案】不是目錄** —— 下面 `cp "$MIG" "$BASE"` 靠這一點。
#    ⚠️ **本檔【沒有人在 Linux 上跑過】** —— 見檔頭「缺哪一道檢查」那節。
BASE="$(mktemp "${TMPDIR:-/tmp}/e13base.XXXXXX")"
cp "$MIG" "$BASE"
