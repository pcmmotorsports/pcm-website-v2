...
#
# 天花板/範圍: 只掃 docs/probes/ 一層(maxdepth 1)的 .sh/.sql;不掃 scripts/ 底下的探針、不掃巢狀目錄。
#   只驗「標記在不在」,不驗「標了 yes 的探針真的自給自足、跑不跑得過」(那要人看檔頭理由行)。
#   這份清單是我想得到的那些, 而我最可能漏掉的是「有人把需要 CI 認領的探針放在 docs/probes/ 以外」。
# 天花板/量具: 只讀前 5 行找標記字面;標記在第 6 行以後 = 看不到。正則是前綴字面比對,
#   判不出 yes/no 宣告與現實符不符(標 yes 卻其實連不上 DB 的探針,本閘照 PASS、要 CI 跑起來才炸)。
#   這份清單是我想得到的那些, 而我最可能漏掉的是「標記語法對、但語意在騙人的那種寫法」。
# ============================================================================
set -u
