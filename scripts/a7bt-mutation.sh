...
#
# ── 這支腳本在回答哪一個問題 ────────────────────────────────────────────
#   T3a(120 案例)與 T3b(29 案例)證明的是「**這些壞資料現在進不去**」。
...
#
# ── 🔴 本檔的突變覆蓋面**擋不住什麼**(Fable R3 F16 / F17 / F18 / F20,逐條登記)────────
#   · **F16 trigger 綁定沒有專屬 mutant**:②那一類只涵蓋走 `TG_ARGV` 的五支。
#     其餘六支 trigger(含掛在 `order_cancellation_items` 上的第 11 支)是「函式被 DROP TRIGGER
#     解綁」這個形狀 —— 本檔改的是**函式本體**,`DROP TRIGGER` 之後那些 ID 仍然存在於
#     函式裡、只是永遠不會被呼叫 ⇒ **解綁這個突變形狀本檔測不到**。
#     (`a7bt-acl-rollback-lock.sh` 的結構指紋會抓到 trigger 定義消失,但那是「有沒有被改」,
#      不是「拿掉之後負測會不會轉綠」。)
#   · **F17 同一個 ID 多條負測只突變第一條**:`seen` 去重是刻意的(一條足以證明承重),
#     但代價是**其餘那幾條沒有各自的承重證明**。`a7bt_c5_item_not_cancelled` 有三條案例、
#     只有第一條進過突變。
#   · **F18 plan↔實跑的 SQLSTATE 欄不是獨立第二來源**:plan 的矩陣列是**從 matrix.tsv 產生**的,
#     而 matrix.tsv 的 SQLSTATE 欄又是 `case_red` 由 ID 前綴**推導**出來的 ⇒ 兩邊同源、必然相等。
#     真正有判別力的是 `expect_red` 那一格:它把**資料庫實際回的 SQLSTATE** 與推導值比對
#     ⇒ 推導錯了會在那裡轉紅。plan 比對的價值只剩「列數與 ID 沒有被同步刪掉」。
#   · **F20 `count_gate` 的能力小於它的自述**:它分開檢查 `CASE_N` 與 `PASS`
#     ⇒ 刪一條案例會被 `CASE_N` 抓到;但**只動斷言**時「刪一條 + 加一條無關的」淨值不變、穿得過去。
#     真正擋住案例被刪的是 `CASE_N` 與 §7.2 逐列 diff,不是 PASS 那個總數。
#
# ── 🔴 不證明什麼 ───────────────────────────────────────────────────
...
printf '  T1 具名守門 %s 個,本檔突變過 %s 個;其餘 %s 個沒有突變(= §7.2.2 的被支配 7 個 + T2 已關的 5 個等待型閘門 + `a7bt_isolation_not_read_committed`(它的負測走自訂外殼、不在 §7.2 矩陣裡 ⇒ 本檔看不到它),三者都**沒有承重證明**)\n' \
...
EXPECT_MUT_TOTAL=102   # 突變過的具名物件總數
EXPECT_MUT_T1=92      # 其中 a7bt_*(T1 守門)
EXPECT_MUT_M=10       # 其中 orj_* / order_refund*(A7b-M 的約束與索引)
EXPECT_ALL_T1=105      # T1 具名守門總數(= T3a 覆蓋率的同一份口徑)
EXPECT_GREEN=57       # 突變後乾淨轉綠(= 該守門是唯一失敗點)
