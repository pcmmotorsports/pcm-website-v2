...
#      🔴 格**名集合**另凍結在檔尾 `KEYS_FROZEN`(backlog #420-1(原 #341-1,2026-08-11 撞號改號)):只守總數的話,
...
OUT="$(QF "$D/w0b.sql")"
case "$OUT" in
  *ERROR*) bad DDL-SYNTAX "DDL 跑不起來:$OUT"; exit 1 ;;   # 🔴 收尾交給 trap teardown(原本這裡是無條件 rm -rf = fail-open)
...
# 🔴 backlog #420-1(原 #341-1,2026-08-11 撞號改號)(W7d-3 補):本檔原本是全線 18 支裡**唯一沒有 KEYS_FROZEN 的**。
