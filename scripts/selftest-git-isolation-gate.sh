...
echo "── 分母 $n 支 · 違規 $bad(其中新的 $newbad)· 量不到 $slow · 自己先死掉 $early(不擋, 而它們未被驗過)"
  ...
  # ══ 🔴🔴 這一行 2026-09-06 被【兩個窗從兩個不同的世界】各釘了一次 ══════════════
  #   `-auth` 在他的樹上量到 **3**(分母 113), 而 `-ship` 在自己的樹上量到 **8**(分母 116),
  #   而**兩份名單互不相交** —— 因為各自的樹裡都少了對方剛修好的那一批。
  #   ⇒ 📌 **那不是誰量錯了, 是【分母是你站的那棵樹】** —— 兩個都是真讀數。
  #   ⇒ 🛑 **所以合併之後 3 與 8 都不對**, 下面這個值是在【合過的樹上】重量的。
  #   ⇒ ⚠️ 引用這個數字的人:它綁著「哪一棵樹、哪一個時點」, 不要跨樹搬。
  #
  # ── `-auth` 那一發(留著, 它記的是 GITFREE 那一輪)──────────────────────────
  #   🔬 ⛔ ~~**10 這個數字帶著量法**:2026-09-06 全量掃(分母 106 · 耗時 379s)當場數的;
  #      名單在板列 ⟦02-EARLYDEATHUNVERIFIED⟧ 的 7 + 3 兩堆。~~
  #   🔬 **3 這個數字帶著量法**:2026-09-06 `-auth` 把 ⟦02-GITFREE-*⟧ 那 7 支修完後全量重掃
  #      (分母 **113** · `早死 3`)當場數的。剩的 3 支 = ⟦02-SELFRED-*⟧ 那堆(`latest-definition-of.sh` ·
  #      `rule-audit.sh` · `spec-1a-recheck.sh`)—— 它們**不帶 `GIT_DIR` 也紅**, 與本閘無關。
  #   ⚠️ **中間量到 4 不是 3** —— 多的那一支是 `ledger-col2-census.py`, **不在原本那 13 支名單裡**
  #      (分母同時 108 ⇒ 113 ⇒ 它是後來新增的檔)。同型, 已一併修 ⇒ 才回到 3。
  #      📌 **那一發示範了這個數字的用途**:它不只在「有人修好」時變動, **也在「多了一支新的」時變動**,
  #        而兩者在收尾那一行上長得一樣 ⇒ 撞到它的人一定要去看名單, 不要只改期望值。
  # ── `-ship` 那一發(留著, 它記的是為什麼要跟著改小)────────────────────────
  #   🔬 **10 這個數字帶著量法**:2026-09-06 全量掃(分母 106 · 耗時 379s)當場數的;
  #      名單在板列 ⟦02-EARLYDEATHUNVERIFIED⟧ 的 7 + 3 兩堆。
  # 🔴 **10 ⇒ 8(2026-09-06 `-ship`;而這是【本閘自己叫我做的】** —— `:315` 逐字
  #    「變少 ⇒ 有人修好了 ⇒ 把本檔的 EXPECT_EARLY 一起改小(否則下一次變多會被吃掉)」)。
  #  🔬 當發讀數:分母 **116** · 違規 **0**(其中新的 0)· 量不到 **0** · 自己先死掉 **8**。
  #  🔵 掉的那兩支來自 ⟦02-GITFREE-*⟧ / ⟦02-SELFRED-*⟧ 那一輪的修復(不是我這三支 ——
  #     我修的 latest-definition-of / rule-audit / spec-1a-recheck **不在**那 8 支名單裡)。
  #  🛑 **不改小的代價是【安靜的】**:期望停在 10 ⇒ 下一次真的多壞一支時 8+1=9 仍 ≠ 10
  #     ⇒ 它照樣印「變了」而沒有人分得出是**修好了一支**還是**壞了一支**
  #     ⇒ 📌 **一個沒跟著現實走的期望值, 會把下一次的退步吸收掉。**(今晚 R017 同型。)
  #   🔬 **0 這個數字帶著量法**:2026-09-06 `-ship` 在【合過 39f 的樹上】重量一發 ——
  #      分母 **117** · 違規 **0**(其中新的 0)· 量不到 **0** · **自己先死掉 0**, rc=0。
  #      (跑法 `SGI_EXPECT_EARLY=0 bash scripts/selftest-git-isolation-gate.sh` ——
  #       ⚠️ 那個 env 只是**不讓它拿舊期望值來比**, 實際數字是它自己數出來印在上面那一行的。)
  #   🎯 **為什麼 3 與 8 都不對, 而 0 是對的**:兩窗各自修好了對方名單上的那一批
  #      ⇒ 合起來之後兩份名單**同時清空**。而在合併之前, 誰都量不到這個 0。
  #   🛑 **這一格從此是【零容忍】** —— 多壞一支就 1 ≠ 0 ⇒ 它會叫。而那正是它該有的靈敏度。
  EXPECT_EARLY="${SGI_EXPECT_EARLY:-0}"
