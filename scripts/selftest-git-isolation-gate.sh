...
#      這台機器上的 `grep` 是 **`ugrep`**(不是 GNU grep)。**編碼不合法的檔它整支跳過** ——
#      零輸出、`rc=1`, **與「這支檔裡沒有 `--selftest`」印一模一樣的東西**。
#      ⇒ 一支壞編碼的腳本會**安靜地掉出分母**, 而本閘照樣印「違規 0」。**分母縮水零訊號。**
#      自造已知答案(2026-08-25 實測, 本機 `ugrep 7.8.4`):
#        `printf '第一行 --selftest 在這裡\n第二行\n' > good.txt`
#        `LC_ALL=C cut -c1-8 good.txt > bad.txt`   <- 切在中文字中間
#        good ⇒ `grep -c ''` **2** / bad ⇒ **空 rc=1**, 而 `/usr/bin/grep -c ''` 對 bad 仍回 **2**。
#      ⚠️ **不是每次切都會發生** —— 切在乾淨字邊界仍是合法 UTF-8, 那一發會「重現失敗」,
#         **而失敗的那一發看起來就像「這個病不存在」。**
#      判別法(免費):`file -b <檔>` 出現 `Non-ISO extended-ASCII` ⇒ 對 ugrep 全盲;
#         救法 `/usr/bin/grep` / `awk` / `python3`。
#      現況(2026-08-25 夜量, 用 python3 逐檔 UTF-8 解碼, 不用 grep):
#         `scripts/` 底下 `.sh`+`.py` **181 支, 解不開 0 支**(負對照:餵上面那支 bad.txt ⇒ `UnicodeDecodeError`)。
