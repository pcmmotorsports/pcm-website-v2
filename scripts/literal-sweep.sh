...
if [ "${1:-}" = "--selftest" ]; then
  ...
  FAIL=0
  # 🔴🔴 **`RAN` 是 2026-08-31 加的, 而它補的是【這支自檢分不出「某一格根本沒跑到」】**
  #    (`-30` 實跑量到, 不是讀出來的):把世界② 的斷言換成 `if false; then … fi`
  #    ⇒ **rc=0、印「selftest: 全過」, 而 PASS 從 10 格變 9 格**
  #    ⇒ 📌 兩發唯一的差別是那個 9 與 10, **而這支腳本從來不數它** ——
  #      它只【印】PASS, 不【算】PASS ⇒ **少一格與零失敗在輸出裡逐字相同。**
  #    ⚠️ 而這一支不是普通的 harness:`CLAUDE.md` 的收工清單逐字要求
  #      「改過任何對外字面 ⇒ 拿舊字面跑 literal-sweep.sh 再 commit」⇒ **全隊都在用它。**
  #    🛑 **為什麼是【計數】不是【具名格】**:這支的世界①有 7 格是同一個 for 迴圈跑出來的
  #      ⇒ 同構的世界沒辦法一個一個點名。**同構 ⇒ 計數;各自不同 ⇒ 具名。**
  #      (`-eb` 的 authz-failure-visibility.sh / migration-reset-role-guard.sh 是後者。)
  RAN=0
  pass() { RAN=$((RAN + 1)); echo "PASS  $1"; }
  fail() { RAN=$((RAN + 1)); echo "FAIL  $1"; FAIL=1; }
  ...
  # ── 🔴 收工前先問【跑了幾格】—— 而期望值**跟著資料走, 不寫死一個數字**。
  #    3 = 迴圈外的三個世界(限度 2 的行為探針 / 世界② 七類全印 / 限度整段逐字)
  #    ${#ST_FILES[@]} = 世界① 每一支合成檔一格(它由答案卷的 cat 條數決定, :125 已驗 = 7)
  #    ⚠️ **為什麼不寫 10**:寫死的話, 有人在答案卷加一類 ⇒ 這一格恆紅 ⇒ **它會被拿掉**。
  #      綁著 `${#ST_FILES[@]}` ⇒ 加一類的時候期望值自己跟著加。
  #    🛑 **而那個 3 仍然是寫死的** —— 迴圈外新增一個世界時要手動 +1。
  #      這是已知缺口, 而它比整個數字寫死小一級:**改動的位置就在這一行的正上方幾十行內。**
  EXPECT_RAN=$((3 + ${#ST_FILES[@]}))
  # 🔴 比較抽成函式, 是為了能【對它自己做一發負對照】——
  #    2026-08-31 實測:把上面那行判斷放寬成 `[ "$RAN" -lt 1 ]` ⇒ **rc=0、印「全過(10/10)」**
  #    ⇒ 📌 一道【被放寬的守門】在沒有東西該紅的那一天, 與一道好的守門印同一個字。
  #    ⇒ ✅ 所以要餵它一個【該紅的值】看它叫不叫 —— 而那要它是一個叫得動的東西。
  # 🛑 **這一段【殺不掉】的兩個突變(2026-08-31 實跑, 寫出來不假裝殺光)**:
  #    ① **把下面那發負對照整格拿掉** ⇒ rc=0 全過。
  #       —— 這是所有負對照共有的性質:**在沒有東西該紅的那一天, 有它跟沒它印同一個字。**
  #       再加一道守門去守它 = 無窮後退 ⇒ **停在這裡, 而把它寫下來。**
  #    ② **把 `EXPECT_RAN` 寫死成 `10`** ⇒ rc=0 全過。
  #       —— 今天 `${#ST_FILES[@]}` 就是 7, 兩者同值 ⇒ **今天是等價突變。**
  #       🔴 而它是【延遲引爆】的那一種:有人在答案卷加一類的那一天才開始說謊,
  #       **而那一天不會有任何事情發生** —— 它只會安靜地少數一格。
  #       ⇒ 綁 `${#ST_FILES[@]}` 讓那張預約單作廢, 而不是等它到期。
  ran_ok() { [ "$1" -eq "$2" ]; }
  if ran_ok "$((EXPECT_RAN - 1))" "$EXPECT_RAN"; then
    echo "🔴🔴 那道『跑了幾格』的守門自己壞了:餵它【少一格】它還說 OK" >&2
    echo "   ⇒ 這一發的綠不算數 —— 而它會在【真的少跑一格】的那天安靜地放行。" >&2
    FAIL=1
  fi
  if ! ran_ok "$RAN" "$EXPECT_RAN"; then
    echo "🔴🔴 selftest 只跑了 $RAN 格(期望 $EXPECT_RAN)⇒ **有世界沒跑到, 而它不會 FAIL**" >&2
    echo "   ⇒ 這一發的綠不算數。不要拿它當證據。" >&2
    FAIL=1
  fi
  st_cleanup
  trap - EXIT INT TERM HUP
  if [ "$FAIL" -eq 0 ]; then echo "selftest: 全過($RAN/$EXPECT_RAN 格)"; exit 0; else echo "selftest: 有 FAIL"; exit 1; fi
fi
