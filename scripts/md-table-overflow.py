...
🔴🔴 **它讀的是【index(staged)】那一份, 不是你工作樹那一份** —— pre-commit 走 `--staged-md`。
   ⇒ **改完檔案沒有 `git add` ⇒ 這道閘照樣紅, 而你會以為你的修法沒有用。**
   實錘(2026-08-31 兩個窗各撞一次, 線DB / `-a0`):修好了、閘還在紅 ⇒ `git add` 之後就過。
   📌 **⇒ 我量的那一份, 不是它量的那一份。** 而兩份在「我改好了」這句話底下長得一樣。
   ⇒ 自己複驗一列時要對齊來源:`git show :<檔> > /tmp/x.md` 再餵它, 不要餵工作樹那份。
