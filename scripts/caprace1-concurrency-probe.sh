#!/usr/bin/env bash
# 🔴🔴 **[2026-09-07 `-db`] 本支測的是【2026-08-31 那一版】的設計, 而那一版已經不是現況。**
#    現況:`20260902020000` 已在正式庫(超額**不再擋**, 改 `RAISE WARNING`),
#    而 `20260907180000`(待貼)再加上 `cap_state` / `over_cap_by` 兩欄的標記。
#    ⇒ 📌 **本支跑綠, 證不到那兩支的行為對不對。**
#    🛑 **別把它的綠當成新版的證據** —— 它問的是一個已經被換掉的問題。
#    ⏰ 要嘛改到新版(驗 12 條, 見 `docs/plans/2026-09-07-caprace1-over-cap-plan.md` §K),
#       要嘛刪掉。**在那之前, 它是一個會給人錯誤安心的綠。**
#    (誰標的:線【資料】`-db`;codex 鐵則12 審查指出 `:32` 仍是舊版。)
