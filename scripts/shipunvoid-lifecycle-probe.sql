...
--
-- 🔵🔵 **雙向指標:這一件有【兩支】probe, 而它們不重複**
--    `scripts/shipunvoid1-apply-probe.sh` —— 世界【更多】(W1/W2/W3 必死正對照/W4/W4b codex 那個
--      並發順序/W5 兩輪作廢復原, PASS=14), 而 schema 是它自陳的【最小 stub】。
--    本檔 —— 世界【較少】(四格 + 一發突變), 而它跑在 replay 全部 migration 的庫上
--      ⇒ 真 CHECK / 真 trigger / 真 oiqs 摘要重算都在。
--    🎯 **⇒ 兩支是同一個問題的兩個保真度, 不是同一支的兩份。要判行為看那支, 要判 schema 相容看這支。**
--
