...
const SERVICE_ROLE_ONLY_TABLES = [
  ...
  'shipments',
  'shipment_items',
  // 逐批到貨明細(供應商到貨時間 / 登錄者 / 溢收件數)—— `20260729020000:235-238`
  // 🔴 **這一條在 2026-08-10(#352-a1)之前是靠「子字串巧合」被涵蓋的**:
  //    `order_item_procurement` 是 `order_item_procurement_receipts` 的前綴,而本檔用
  //    `source.includes(table)` 比對 ⇒ 提到 receipts 的檔案本來就會被上面那條攔下。
  //    那是**巧合、不是設計**:上面那條哪天改名或收窄,receipts 會**無聲**失去防護。
  //    ⇒ 明列一條,讓它有自己的守門、不寄生在別人的字面上。
  //    ⚠️ **誠實說明本條今天的判別力**:因為前綴涵蓋仍在,本條**今天被上面那條嚴格蘊含**
  //       —— 也就是**現在寫不出一個「只紅本條、不紅上面那條」的負測**。
  //       它買的是**未來**:上面那條一旦改名/收窄,本條就是唯一還站著的那道。
  //       (不要因為「構造不出負測」就把它刪掉 —— 那正好把它要防的事變成必然。)
  'order_item_procurement_receipts',
  // 到貨登錄的冪等帳(帶 actor / request_id / 到貨與溢收件數)—— `20260810230000`(#352-a1)
  // 🔴 backlog #329 點名的失效路徑 =「新表忘登記 = 零防護零訊號」⇒ 建表同一片就登記。
  //    ⚠️ 這張表名**不含** `order_item_procurement` 子字串 ⇒ 拿不到上面那份巧合涵蓋,
  //       沒有這一行就是真的裸奔。
