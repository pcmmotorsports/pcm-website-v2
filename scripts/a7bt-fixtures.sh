...
PORT="${PORT:-54329}"
# 🔴 PORT 可注入是為了多視窗並行(2026-08-05 B-211-STOP:寫死 54329 讓第二個視窗完全跑不了),
#    但注入值必須落在**本機拋棄庫慣用段 543xx** —— 這道範圍閘擋的是「把 harness 指向正式站」
#    (Supabase direct 5432 / pooler 6543 都不在 543xx)。
#    ⚠️ 它擋不住什麼:有人在 127.0.0.1 的 543xx 自己開一個非拋棄庫並放上 `.d1t2-harness` 標記
#    —— 但那與原本硬編 54329 時的情況**完全相同**(硬編也只要在那個 port 開庫+放標記就繞得過)
#    ⇒ 判別力沒有下降,真正的防線是「URL 的 host 寫死 loopback + workdir 標記 + database=postgres」
#    這三道,本閘只是把第一道從單值放寬成合理範圍。
case "$PORT" in
  543[0-9][0-9]) ;;
  *) echo "🔴 PORT=$PORT 不在本機拋棄庫慣用段 543xx ⇒ 拒絕執行(防 harness 指向正式站)" >&2; exit 1 ;;
esac
