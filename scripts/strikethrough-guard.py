...
def selftest():
    cases = [
        ...
        ("🔴 圍欄區塊裡的 ~~ 同上",
         "```\n~~ 這是範例\n```\n~~舊值~~ ⇒ 值 9",                          "值 9",        1, 0),
        # 🔴🔴 下面兩格是 2026-09-07 `selftest-guards-what` 打出來的:
        #    上面那兩格【餵了反引號, 而它們分不出 CODE 有沒有生效】——
        #    把 CODE 換成永不匹配之後, 劃線區間確實變了(14-20 ⇒ 3-16),
        #    **而「值 9」兩種情況都落在區間【外】** ⇒ 期望值相同 ⇒ 兩格照樣綠。
        #    ⇒ 📌 **一格「餵了那個形狀」的斷言, 若目標字沒落在【會變的那一段】,
        #       它測不到那個形狀。**(與本 repo traps「fixture 沒踩到邊界」同族。)
        #    ✅ 改法:目標字選在【只有弄瞎時才會被算進劃線】的位置。
        ("🔴 反引號沒被遮蔽時, 它會跟後面的真劃線配成一對(目標落在【差異區】)",
         "講 `~~` 這個標記 · ~~舊值~~ ⇒ 值 9",                              "這個標記",    1, 0),
        ("🔴 圍欄同上(目標落在差異區)",
         "```\n~~ 這是範例\n```\n那一行 ~~舊值~~ ⇒ 值 9",                 "那一行",      1, 0),
    ]
    bad = 0
