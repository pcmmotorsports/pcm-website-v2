...
def mask_code(text):
    """把行內程式碼與圍欄區塊換成【等長】佔位字元 —— 位置不變, 內容失效。

    🔴 為什麼(2026-09-01 10:0x, 線索是 `-a0` 掃出 `docs/handoff/CURRENT.md` 錯開區 98%):
       那支檔那五根「落單」的 `~~` **全部住在反引號裡** —— 它們是在【討論】這個記號,
       不是在【用】它。而本工具原本把它們算進配對 ⇒ 奇偶被算歪 ⇒ 整份錯開。
       ⇒ 📌 **一份【在講某個標記】的文件, 對一把【數那個標記】的尺是最壞的輸入。**
       ⇒ ⇒ 而本 repo 的文件大量在講自己的標記 ⇒ 這不是邊緣案例, 是常態。
    ⚠️ 等長替換是刻意的:所有回報的位置/行號都靠原文偏移, 換長度會全部漂掉。
    """
...
def struck_spans(text):
    ...
    """
    marks = [m.start() for m in re.finditer(r"~~", mask_code(text))]
    out, dropped = [], []
...
def selftest():
    cases = [
        ...
        ("🔴 落單的 ~~ 不得毒到它之後的全部(否則 cry wolf ⇒ 工具被刪掉)",
         "~~ 落單\n" + "x" * 2500 + "\n值 9 ~~ 尾",                         "值 9",        1, 0),
        # 🔴 下面兩格:反引號/圍欄裡的 `~~` 是【在講這個標記】, 不是在用它。
        #    線索是 `-a0` 掃出 `docs/handoff/CURRENT.md` 錯開區 98%,
        #    而那五根「落單」全在反引號裡;`-2d` 獨立用三個世界量到同一件事。
        ("🔴 行內程式碼裡的 ~~ 不參與配對(否則它會跟真刪除線配成一對)",
         "講 `~~` 這個標記 · ~~舊值~~ ⇒ 值 9",                                "值 9",        1, 0),
        ("🔴 圍欄區塊裡的 ~~ 同上",
         "```\n~~ 這是範例\n```\n~~舊值~~ ⇒ 值 9",                          "值 9",        1, 0),
    ]
    bad = 0
