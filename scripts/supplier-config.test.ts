...
describe('⟦DBK 製造商品牌⟧ perRowBrand', () => {
  it('只有 dbk 開、只准 termignoni / ohlins(Sean 01:0x「只搬真的是該品牌做的」);其他供應商一家都沒開', () => {
    expect(getSupplierConfig('dbk').perRowBrand).toEqual({ allowedSlugs: ['termignoni', 'ohlins'] });
...
describe('getSupplierConfig', () => {
  ...
  it('should register exactly the pilot set + 品牌放量 8 家(2026-07-10)+ akrapovic(07-19)+ extreme/kspeed(07-24)+ dna(08-20)+ gilles(08-27)+ dbk(09-04)+ rizoma(09-04)+ wrs(09-04)', () => {
    ...
    // 2026-09-04 補 wrs(第 20 家真供應商;supplierSlug=brandSlug=handlePrefix='wrs',拼法未分岔)
    //   ⛔ ~~writeAllowed=false 起手,零寫入。~~ ⇒ ✅ **2026-09-08 Sean 拍板「開」⇒ 翻 true**
    //     (那一秒本檔的 matrix 對帳測試當場紅, 就是它提醒補 `rpm-sync.yml` 的)。舊字面留刪除線。
    //   🔬 **2026-09-08 重量, 下面 09-04 那組數字全部過期**(源頭每天在動):
    //     1,131 變體 ⇒ **1,207** · 568 群 ⇒ **624** · 沒有真照片 33 件 ⇒ **81 件 / 涉 71 群**
    //     (`DISTINCT image_url` 仍 = 1、仍是 PCM 自己的卡 ⇒ 仍不擋上架;
    //      🟢 正對照:有圖的 1,126 列 `DISTINCT image_url` = **1,126** ⇒ 這把尺不是恆等於 1)
    //     · ⛔ ~~缺描述 0~~ ⇒ 🔴 **624 群裡 44 群整群零描述**, 而**連原文也是空的**
    //       (`description_zh` 空且 `description_origin` 也空 = 53 列;`有原文而無中文` = **0**)
    //       ⇒ 📌 **那不是翻譯, 是要寫。** Sean 09-08 逐字「A 照樣上架, 那 44 群之後補」。
    //   🔴 **而下面那段【授權】的敘述留著、不要刪** —— 它記的是 09-04 為什麼停, 而
    //     「先做 A 再做 B 不含 B 可以做」這個判別下一家還會用到。它與 dbk / rizoma 的差別在【授權】不在資料:
    //   Sean 對 dbk 拍「甲 上」· 對 rizoma 拍「`q3: 上`」= **上架授權**;
    //   而對 WRS 他拍的是 `q4: 甲,` =「**先做品牌形象區**」⇒ 🛑 **那是順序, 不是上架授權。**
    //   📌 **一個「先做 A 再做 B」的拍板不含「B 可以做」, 而那兩者讀起來很像。**
    //   🔬 preflight 實測(2026-09-04 單一時點):1,131 變體 / 568 群 · 價缺 0 · 明文 http 0 ·
    //     v2 已分類 = 總數 · 缺中文名 0 · 缺描述 0 · 8 大類 15 子類 · 有 pdf 的群 **0** ·
    //     沒有真照片 **33** 件(`DISTINCT image_url`=1, 逐字 `quote.pcmmotorsports.com/no-photo.png`
