...
//   · 突變①  把印出那段(log/warn)整段拿掉 ⇒ **5 格紅**
//   · 突變②  在印之前插 `if (r.noChinese === 0) return r;`(= 製造「該叫才叫」)
//            ⇒ **2 格紅**:「零件時仍要印 0 列」**與**「--group/--limit 要標篩選後」兩格
//   ⛔ ~~「①4 格紅 · ②恰好 1 格紅 ⇒ 精準擊殺」~~ —— **那是 R1 修完前的讀數, 已被 R2 證偽。**
//      🔴 成因:我後來補了 scopeNote 與 ids 兩格測試, **而沒有回頭重跑①②** ——
//         📌 **改了尺之後, 先前那些突變證據當場作廢, 而它不會出聲。**
//      🔵 而②打紅兩格【不是壞事】:那兩格共用同一條印出路徑, 所以本來就該一起紅。
//         錯的是我把「1 格」寫成一個【精準】的性質, 而那個數字自己會過期。
...
describe('🔴🔴 接線守門 —— 這一發在 rpm-import.ts 把呼叫刪掉時會紅', () => {
  ...
  it('🔴 有把 FULL_MODE 傳進去 —— 不傳的話 --limit 會印出假的全綠', () => {
    // 🔴 R2 抓到:寬鬆版 `[\s\S]{0,60}FULL_MODE` 對 `!FULL_MODE` 也會綠 ——
    //    而那正是這道閘要防的那個假全綠。釘住確切形狀, 不留驚嘆號的空間。
    expect(code).toMatch(/printTitleLanguageReport\(\s*productRows\s*,\s*FULL_MODE\s*\)/);
  });
});
