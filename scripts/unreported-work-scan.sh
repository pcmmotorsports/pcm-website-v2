...
#
# ── 🔴🔴 紀律:**這支【不掛成 pre-commit / pre-push 守門】**(2026-08-21 裁定,主視窗)──
#   這不是待決,是已經裁掉的。要重開請照本段最後那一行的門檻,不要重新論證一次。
#
#   **裁定理由 = 當天量到的精確度,不是感覺:**
#     2026-08-21 實跑 5 個紅格,逐格開檔核對「那裡是不是真的有未交工作」:
#       主樹 admin-orders.png ✅真  /  pcm-vitest-alias ✅真
#       pcm-site-redesign ❌假(已搶救進 dev `48521e09`,樹上那份只多 9 行路標)
#       pcm-w3-guards     ❌假(9 項全部已被 dev 取代)
#       pcm-w4-review2    ❌假(3 項同上)
#     ⇒ **樹層級精確度 2/5 = 40%。十次有六次是誤報。**
#     逐格證據:`~/pcm-mailbox/G-c0-D3可掛清單v2-20260821.md` §0
#              `~/pcm-mailbox/G-c0-F-81-兩棵樹合併結論-20260821.md` §2(G+F 兩窗各自跑過)
#
#   **為什麼 40% 不能有阻擋權(而這不是「誤報很煩」而已):**
#     誤報不會讓人更小心,它讓人**學會繞過** —— 被擋下的人會改用別的路做同一件事,
#     而那條路【完全合規】⇒ 守門的覆蓋率靜靜下降,**而下降本身沒有訊號**。
#     ⇒ 一道會誤報的閘,擋掉的不是壞東西,是**使用它的人**。
#     (同族實錘:2026-08-20 reviewer-gate 三態;memory `feedback_a-false-positive-reroutes-people-out-of-the-guards-view`)
#
#   🔴 **而誤報【不是 bug】,修不掉 —— 這格決定了它永遠不該掛:**
#     本工具量的是「這些檔案沒有 commit」，照這個定義它一格都沒錯。
#     使用者讀到的是「這裡有還沒做完的工作」。
#     兩者只在【那份工作被別的路徑交付掉】的時候分岔 ——
#     而在一支多窗艦隊裡,**那正是常態不是例外**。
#     ⇒ 精確度不會靠修 bug 變好,它是這個定義的結構性上限。
#
#   **現行用法(裁定同時給的替代):** 當作**每日一封信**給主視窗,由人判。
#     誤報的代價從「擋住 commit」降成「多讀一行」。
#     ⚠️ 而這個替代自己有一個洞:**沒有人盯的信會被略過** —— 那是已知代價,不是沒想到。
#
#   ── 🔴 什麼時候才值得重開這一題(門檻寫死,免得憑感覺重議)──────────
#     ① 精確度連續量 **3 次 ≥ 80%**(每次都要逐格開檔核對真假,不是憑紅格數推),且
#     ② 那 3 次分屬 **不同日期**(同一天量三次 = 一個證據複印三份),且
#     ③ 量測時**艦隊仍在多窗運作**(單窗下這個誤報洞本來就不存在 ⇒ 那時的高精確度不算數)
#     三條全中才重議。**任一條答不出來 ⇒ 維持不掛。**
#     🔴 重議時要一起答:「上面那個【定義分岔】怎麼解掉」——
#        答不出來就只是換一個人重踩同一格。
#
#
# ── exit code(對齊 where-is.sh / migration-ledger-divergence.sh 的 ⛔ ~~1/2/3~~ )──
#   🔵 **2026-09-02 訂正(舊字面留著加刪除線,讓搜舊句的人同一發撞到)**:
#     `migration-ledger-divergence.sh` 現在有**第四個碼 `4`**(標記不完整 / 矛盾 / 過期;
#     `-c7` 依 `-15` 的 never-apply 第二種態規格加的)。
#   🛑 **而【本支仍然只有 1/2/3】** —— 變的是那一支,不是這一支。
#     ⇒ 📌 所以這一行原本的意思(「本支的碼與那兩支同一套語意」)**對 `where-is.sh` 仍然成立**,
#       而對 `migration-ledger-divergence.sh` 只涵蓋它的前三個。
#   🔴 **而那個 `4` 在它自己那邊有一個【今天就失效】的射程**:它的 ②⑦ 現在就在紅
#     ⇒ `rc` 被 `3` 蓋過去 ⇒ **rc 分不出是哪一種** ⇒ 那邊要讀訊息不要讀 rc。
#     ⇒ ⇒ 🔵 **所以不要照抄那個 `4` 進本支** —— 它在那邊是一個【明知會被蓋掉】的碼。
#   0 = 掃完了,沒有撈到未交的東西
...
#      時回 2(`migration-ledger-divergence.sh` 的 `usage; exit 2` 三處;查法
#      `grep -n 'usage; exit 2' scripts/migration-ledger-divergence.sh`)⇒ **今天不可達,不是我漏測。**
#      🔴 2026-08-27:這裡原本寫 `:87,90,93` —— 而別人往那支加了註解之後,
#         87 行變成了 `usage() {`。**指標寫數字就會漂,而漂掉時零訊號。**改成字面錨。
