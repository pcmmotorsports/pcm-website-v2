...
run_context() {
  ...
  mkdir -p "$rx_dir"
  {
    ...
    printf '  🔴🔴 上面那兩欄【並行數】射程比欄位名窄:標記檔落在自己這棵工作樹，而「其他窗」在別棵樹\n'
    printf '     ⇒ 那兩欄幾乎構造上必然是 0(2026-08-18 I 窗實測)，不是量到機器閒\n'
    printf '     ✅ 本欄不同：ps -A 是全機，當場驗過看得到 5 棵不同工作樹的行程\n'
  } > "$rx_dir/RUN-CONTEXT"
  echo
  ...
  echo "  🔴🔴 而上面那兩欄【並行數】2026-08-18 被 I 窗證實射程比欄位名窄很多:"
  echo "     標記檔目錄是 \$(pwd)/logs/four-greens/.running ⇒ 落在【自己這棵工作樹】,"
  echo "     而「其他窗」照定義就在別棵樹 ⇒ 那兩欄幾乎【構造上必然是 0】,不是量到閒。"
  echo "     ⇒ 本欄與它們不可互相佐證;引用那兩欄的舊分析要重看理由。"
