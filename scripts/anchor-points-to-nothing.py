...
def main():
    ...
    if noanchor:
        ...
        print(f'  🟡 另有 {len(noanchor)} 個:那一列【存在而沒有錨欄】(錨寫在標題欄裡)'
              f' ⇒ **不要開新列**, 那會製造重複。')
        # 🔴 R2 nit(2026-09-07):修法**不只一種**, 而它們在這裡分不出來 ——
        #    ① 錨欄是 `—`(空)⇒ 把錨搬進錨欄
        #    ② **錨欄已經被【別的錨】占用**(實例 `b4-LOCK1`:錨欄現為 `⟦db-ROLLBACKLOCKWAIT⟧`,
        #       `-tidy` 2026-09-07 補錨時改名)⇒ 🛑 **那時要改的是【碼裡的指標】, 不是搬錨。**
        #    ⇒ 📌 **開檔一看即知, 而本支看不出來** —— 所以它問, 不替你決定。
        print('     ⚠️ 兩種修法, 開檔才分得出來:錨欄是空的 ⇒ 把錨搬進去;'
              '錨欄已被別的錨占用 ⇒ 改【碼裡的指標】。')
