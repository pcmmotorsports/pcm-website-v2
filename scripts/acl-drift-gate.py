...
def self_protect():
    ...
    🔴🔴 **[2026-09-05 自我訂正 —— 我第一版把這件事寫得比事實嚴重]**
    ⛔ ~~一顆把 R5 那幾行拿掉的 commit, 本閘照樣讓它過~~ · ⛔ ~~刪是最便宜的繞法~~
    🔬 **實查之後有【兩道既有機制】**(`grep -rn 'acl-drift-gate' package.json .husky/`):
      ① `.husky/acl-drift-gate.sh` **每顆 commit 都跑**, 而且 **fail-closed**:
         這支 `.py` 不見了 ⇒ `exit 2`(擋下)· 沒有 python3 ⇒ `exit 2`
         ⇒ 📌 **刪掉它【不是】最省事的關法, 它會當場擋下。**
      ② `package.json:64` 的 lint-staged:staged 到本檔 ⇒ **當場跑 `--selftest`**
         ⇒ 📌 **【改】它就要讓那 129 格全過。**
    🎯 **⇒ 真正的缺口比原本寫的窄很多**:不是「改它沒人管」, 是
       **【規則與測它的測試住在同一支檔】** ⇒ 同一顆 commit 裡把規則砍掉、把守它的格子一起砍掉,
       **兩道機制都不會叫**。(那正是本 repo 記過的形狀。)
    ⇒ 🔴 **這一格【沒有】在本次修掉** —— 修法都在**閘外**:把期望格數釘在別的檔 / CI 一支 job /
       「動 `scripts/` 要第二個人核准」。⇒ 板列 `⟦b9-GATESELFEDIT⟧`。
    ✅ **而今天多守著它的是【人】**:動本閘 = 動驗證本身 = `00-work-rules` R4 的立即停止訊號
       ⇒ 要提 plan、要主視窗裁、要兩輪對抗審查。本次就是那樣走的。**而那不是機制。**
    ⚠️ **上面兩道機制我是【讀碼推出的, 沒有實跑】** —— 我沒有真的做一顆「砍規則 + 砍格子」的 commit。
    """
