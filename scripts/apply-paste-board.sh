...
preflight() {
  local num root paste hits n ver mig s_paste s_mig led undo_hit
  num="$1"; root="$2"
  ...
  # ── 前置⑧(⚠️ **warn-only** —— 這支腳本裡【唯一一格不 return 1】的前置)────────
  # 板列 ⟦mail-PASTEUNDOWARN⟧;A 2026-09-07 拍(機制優先律), 主視窗 B 派。
  #
  # 🛑🛑 **不要順手把它改成 `return 1`。**
  #    前置 ①-⑦ **一律 fail-closed**, 而這一格【刻意】不是 ⇒ 那個不一致是**設計**不是漏寫。
  #    📌 下一個讀到這裡的人:照鄰居的形狀「補上 return 1」= **提早轉擋** ——
  #      一個看起來像修 bug 的動作, 實際是跳過一個【還沒到】的決策點。
  #    ✅ **轉擋的條件寫在板列**:warn 跑過一批而期間沒有誤報 / 漏報 ⇒ **另開片**改。
  #
  # 🔴 **來由**:一支貼板沒有同號 `Nr_` 還原檔, 而**貼之前沒有任何東西會說**
  #    ⇒ 災難當天才發現無路可退(A 轉述 84 R3 F4:重貼 79 被自己的 P4 擋)。
  # 🔬 **分母(2026-09-07 14:2x 當場數 `貼板-0906/`)**:主板 **34** 支 · 有還原檔 **18** 支
  #    · **無 16 支**(45/51/52/55/57/58/59/60/62/63/64/65/66/83/85/86)
  #    ⇒ 🟡 **它今天會對將近一半的板叫** —— 那正是先 warn 不擋的理由。
  #
  # ⚠️ **它答不出什麼**:只問「**有沒有一支非空的同號檔**」—— 答不出那支還原檔
  #    【內容對不對】、【跑不跑得動】、或【還原得回來嗎】。
  #    📌 所以它是**提醒**, 不是保證;把它讀成「有 Nr_ 就安全」比沒有這一格更糟。
  # 🔵 **`-s`(非空)不是 `-e`(存在)**(主視窗 B 2026-09-07 一個字的改法):
  #    ⛔ 用 `-e` 的話**一個 0 位元組的檔就讓它閉嘴** —— 而那正是「看起來有做」的形狀。
  # 🔴 **而「沒有檔」與「檔在而是空的」印【不同的話】** —— 兩者要去做的事不一樣:
  #    前者是**還沒寫**, 後者是**寫到一半或被清空**(而後者更該停下來看)。
  #    ⇒ 📌 一個把兩種成因印成同一句的警告, 會讓人往錯的方向找。
  undo_hit=$(find "$(dirname "$paste")" -maxdepth 1 -type f -name "${num}r_*" 2>/dev/null | head -1)
  if [ -z "$undo_hit" ]; then
    note "🟡 前置⑧:找不到同號還原檔 ${num}r_*(在 $(basename "$(dirname "$paste")"))"
    note "   🛑 **這一格只警告, 不擋** —— rc 不變, 貼照樣往下走。"
    note "   ⇒ 災難當天沒有這支檔就沒有回頭路;要補請放同一個貼板目錄, 檔名 ${num}r_<版本號>_還原_災難用.sql"
  elif [ ! -s "$undo_hit" ]; then
    note "🟡 前置⑧:同號還原檔 $(basename "$undo_hit") **存在而是空的(0 位元組)**"
    note "   🛑 **這一格只警告, 不擋** —— rc 不變, 貼照樣往下走。"
    note "   ⇒ 這比「沒有」更值得停一下:它在檔案清單上看起來【已經有了】。"
  fi
  # 🔵 **有還原檔時【一個字都不印】**(A 指定的形狀)——
  #    ⇒ selftest 那兩格就是拿「印不印 🟡 前置⑧」當判別式, 多印一行會讓那個判別式變鈍。
...
'
  ...
  # ── ⑮ 前置⑧(還原檔 warn-only)—— 板列 ⟦mail-PASTEUNDOWARN⟧ ──────────────
...
'
  ...
  ck "⑮d 負對照:不可以印成「找不到」" "$(grep -c '找不到同號還原檔' "$D/out" | head -1)" "0"

  # ⛔ **未覆蓋(明寫)**:這一格答不出那支還原檔**內容對不對 / 跑不跑得動 / 還原得回來嗎** ——
  #    一支只有一個空白字元的檔就過得了 `-s`。⇒ 把它讀成「有 Nr_ 就安全」比沒有這一格更糟。
  #
  # 🔴🔴 **【一個會讓這四格假綠的世界 —— 而它 2026-09-07 真的發生過】**
  #    這四格靠 `mk NN <版本號>` 造貼板檔, 而**前置① 與 前置⑥ 會先擋**:
  #    · 前置① 擋「同一個**編號**命中多支」· 前置⑥ 擋「同一個**版本號**已在 APPLIED.tsv」
  #    ⇒ 📌 **撞到任一個, 這四格【根本走不到前置⑧】** —— 而畫面上是「前置①/⑥ 擋下」,
  #      一個**指向別的地方**的錯誤訊息。
  #    🔬 實錘:合 `origin/dev` 那天, 別條線的格用了 `20991024/25000000`,
  #      而我這四格也用同一個版本號 ⇒ `⑮a`/`⑮b` 被**前置⑥**擋掉、rc=1。
  #    🛑 **而我事前查過「零碰撞」** —— 我的尺只問「**同一個編號**對到幾個版本號」,
  #      **沒問反方向**「同一個**版本號**被幾個編號用」。
  #    🎯 **一把單向的尺, 在它沒看的那個方向上是全盲的 —— 而它給我的是一個綠色的答案。**
  #    ✅ **加格的人請兩個方向各查一次**, 兩邊都要 1 對 1。
