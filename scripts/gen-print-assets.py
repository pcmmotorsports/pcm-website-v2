...
// 🔴 而症狀是【圖不見了, 不是錯誤】—— 不報錯、三綠全綠(那兩顆 `<img>` 沒有掛任何失敗處理)。
//
// ⚠️ **來源標記(2026-08-29 code-reviewer 抓到, §6-b「量到 vs 推出」)**:
//   · **量到的**:那兩顆 `<img>` 的失敗處理屬性, 字面命中 **0**
//     ⇒ 它證的是【拿不到圖時不會有東西叫】, **不是**【圖真的被 303 了】。
//     **分母(2026-08-29 R2 要求補;數字要帶著範圍走)**:
//       範圍 = `apps/admin/src/components/print/` 整個目錄(含測試)· 量於 2026-08-29
//       · 量的是【字面】不是行為 · 失敗處理的字面命中 ⇒ **0**
//       · 負對照(一個一定存在的字面)⇒ **三位數** ⇒ 尺是活的
//     🔴🔴 **而這一格自己踩了一次, 留著當標本**:我第一版把那兩個 `grep` 指令
//        【逐字寫進本檔】⇒ 而其中一條含著它要找的那個字 ⇒ **重跑時它把自己數進去**,
//        那個 `0` 當場變成 `3`, 而三筆全是這段註解自己。
//        📌 **⇒ 一段【記錄某個東西不存在】的註解, 會讓下一次量測找到那個東西。**
//        ✅ ⇒ 所以這裡【不寫可複製的 grep 指令】, 只寫範圍與結論;
//           要重量的人自己組字串, 而**組的時候記得排除本檔**。
