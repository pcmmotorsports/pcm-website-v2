...
#
# 🔴🔴 **canonical: `scripts/const-echo-scan.sh`**
#    **若你手上這一份不在這個路徑上, 它是【副本】** ——
#    **去讀 canonical, 不要用副本判定本尺的能力。**
#    驗法(兩行, 各自答一個問題):
#      git ls-files --error-unmatch scripts/const-echo-scan.sh   # rc=0 ⇒ 你在 canonical 那棵樹上
#      git diff --quiet -- scripts/const-echo-scan.sh            # rc=0 ⇒ 你手上這份 = HEAD 那一份
#    ⚠️ 在 scratchpad / 非 repo 底下第一行會錯 —— **那個錯就是訊號**, 不是工具壞了。
#    📌 **為什麼加這幾行**(2026-08-29,而它是實錘不是預防):
#       有人拿一份 **87 行的 scratchpad 副本**, 判定隔壁那把尺(`naked-cells.py`)
#       「沒有宣告 throw 盲區」—— **而 canonical 有宣告, 還給了實例、每次跑都印。**
#       🔴 他沒有做錯任何事:**那份副本沒有任何東西告訴他它是副本。**
#    ⚠️ **而本檔的副本數是量到的, 不是估的**:2026-08-29 掃 scratchpad + repo 兩側,
#       本檔有 **8 份 sha256 相同的副本**(全部是 worktree 複本)—— 是這一族裡最多的一支。
#       ⇒ **越多副本的檔, 越需要這一段** —— 而它原本一段都沒有。
#    🔴 **為什麼【不寫 sha256 / 不寫行數】(設計選擇, 沿用 `naked-cells.py` 那半的理由)**:
