 ...
 *     ⏱️ **560 是 2026-08-15 當時的讀數;2026-09-17 唯讀實查 = 1,089。**
 *     🔵 **本段的結論不依賴這個數字** —— 它只是在說「有這麼一批」,而「刻意不改」的理由是
 *        【改讀取範圍會動到那兩道閘的分母】,與它是 560 還是 1,089 無關。
 *     🟢 **而那兩道閘【沒有】綁死在這個數字上**(2026-09-17 查過才敢在別人的檔上加這一段):
 *        `SOURCE_MISSING_RATIO_ABORT` / `VARIANT_DELETE_RATIO_ABORT` 都是**比例**,
 *        分母在執行當下從活的資料算(`computeSourceMissing` / `computeVariantOrphans`)
 *        ⇒ 全檔搜 `560` **只有上面那一行**,零引用。
 *     📌 **原值刻意留著不改** —— 蓋掉它就看不出它長了多少;而「它會長」正是下一個人要知道的事。
