...
-- 備援路徑(工程端、手上有連線字串時):
--   `psql "<連線字串>" -v ON_ERROR_STOP=1 -f scripts/452a-down.sql`
--   ⚠️ 原本這裡寫 `psql "$DATABASE_URL"` —— **`.env.local` 裡沒有 `DATABASE_URL` 這個名字**
--      (數法:`grep -c '^DATABASE_URL=' .env.local` = 0;也沒有任何 `*DB*` 名字)
--      ⇒ 照抄會連不上,而且錯在「連線」這一層、看起來卻像腳本壞掉。已改成不指定變數名。
--
-- ── 🔴 為什麼「貼 Dashboard」這條路成立(兩條都可重數;數法連同陷阱一起寫在下面)──
--   🔴 **一律只數「可執行區間」= `^BEGIN;$` 那行到 `^COMMIT;$` 那行**,不要數整個檔 ——
--      本註解區塊自己就含 `:'name'`、`:CONSTRAINT_NAME` 這些樣式字,數整檔會把它們一起數進去
--      (我第一版就是這樣寫的:寫完「0」,重數得 15,**註解當場變成自己的反例**)。
--      取區間:`B=$(grep -n '^BEGIN;$' 檔) ; C=$(grep -n '^COMMIT;$' 檔) ; sed -n "$B,$Cp" 檔 | …`
--
--   ① **可執行區間內沒有任何 psql 變數** ⇒ 不依賴 `-v`,貼進 Dashboard 語意不變。
--      · `:'name'` 形式(psql 會代換成帶引號字面):區間內 `grep -c ":'[A-Za-z_][A-Za-z0-9_]*'"` = **0**
--      · psql 反斜線指令(`\set` / `\if` / `\gexec`…):全檔 `grep -cE '^\\[a-z]'` = **0**
--      · ⚠️ `:name` 形式**粗數會得 10**,不是 0 —— **逐行開檔看過,10 個全部在 `--` 註解或單引號
--        字串裡**(典型:中文冒號後面接英文字、`RAISE` 訊息內文、`<>:CONSTRAINT_NAME` 這種敘述),
--        **沒有一個在可執行 SQL 的裸露位置** ⇒ psql 不會代換到它們。
--        🔴 這一行刻意把「粗數 10」寫出來:只寫「0」的話,下一個人重數得 10 會以為本註解在說謊。
--   ② **整檔包在單一交易裡** ⇒ Dashboard 沒有 `ON_ERROR_STOP` 也不會半套用。
--      · 頂層 `grep -c '^BEGIN;$'` = **1**、`grep -c '^COMMIT;$'` = **1**(行號會漂,刻意不寫)
--      · 那兩行之外**沒有任何非註解、非空白的行**(前段與後段各 grep 一次,輸出皆為空)
--      · (檔內其他 `BEGIN` 都是 plpgsql 區塊的 `BEGIN`,不是交易控制)
--   ⚠️ 兩條都是 **2026-08-14 R 窗對本檔當下內容**數的。**改過本檔就要重數**,不要沿用。
--
