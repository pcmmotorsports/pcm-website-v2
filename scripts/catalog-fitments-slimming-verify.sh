...
✅ 2026-09-07 03:2x 實查:**兩半都在 main** ⇒ 上面那句不成立了, 這一格今天量得到。
   · app 半:apps/storefront/src/lib/products.ts 的 blob sha **main == dev**(a3c4f9269703b6ec…),
     該檔呼叫 get_vehicle_taxonomy 命中 7 次(兩邊同)
   · DB  半:貼板 52 已貼, 而 main 與 dev **共用同一個正式庫**
   · 交叉:production 部署 dpl_3RcCdEezGBnJit1YUBjk3T2zqjNP 的 githubCommitSha = 5b91190d0 = origin/main
🔴 **而【由哪一顆帶進 main 的】我沒追** —— 只證了「今天在」, 沒證「什麼時候進去的」。
🛑 舊字面留刪除線不刪:照這支腳本跑的人會讀到它, 而它會讓人以為量了也沒用。
HINT
}
