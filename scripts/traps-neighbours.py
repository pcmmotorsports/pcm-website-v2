...
def main():
    ...
    print('⚠️ 這是【候選】不是【判定】。本工具已知的【八】種失效(①-⑥ 量於 2026-08-22、⑦ 量於 2026-08-28、⑧ 量於 2026-08-29):')
    ...
    print('      🔴 **[2026-09-07 tidy 實證一次 —— 而它是【前 6 名 0 命中】那一種]**:')
    print('         草稿 = 母題級通則「零命中的正對照是【同一個字在別的地方】」;')
    print('         本工具前 6 名 **一條都不是同族**(最高 0.2881, 全是別的母題)。')
    print('         🔵 **`0.2881` 這個數要留著** —— 下一個人看到它才知道那不是「有點像」,')
    print('            是**完全沒撈到**:同族那條的真實分數在名單外, 而榜首只是別的母題的雜訊。')
    print('         ✅ **改 `grep -n \'^## .*零命中\'` 標題行 ⇒ 當場撈到正本 `:12110`**')
    print('            「零命中的第一個問題永遠是:這份檔用不用這個詞」(W5 2026-08-19)⇒ 同族, 已併入。')
    print('         📌 **⇒ 下一個人會相信那前 8 名就是全部** —— 而這一發證明它可以整批不中。')
    print('         🎯 **可機械執行的補救**:草稿若是【母題級通則】(講的是【判準本身】不是某支檔),')
    print('            **一律另跑一發 `grep -n \'^## \' <正本> | grep <你那句話的關鍵詞>`** —— 那一發吃標題,')
    print('            而母題的標題常常就是那句話本身。')
    print('   ⑧ 🔴🔴 **反向版本:排名【最高】的那一條也可能不是同族** —— 而它比 ① 更難擋')
    print('      (2026-08-29 `-c8` 判 13 條時量到, 主視窗 `-06` 定為第八種):')
    print('      一份草稿的第一名 **0.4658**(該輪全表最高)⇒ 開檔讀 ⇒ **不同族**')
    print('      (那條講「收了證據而沒往下想」, 草稿講「數錯東西」)。')
    print('      🔴 **而它是那 13 條裡【唯一一條高分而判不併】的** ——')
    print('      **照分數自動決定的話, 恰好是分數最高的那一條會被錯併。**')
    print('      📌 **⇒ 高分是【值得開檔】的訊號, 不是【同族】的結論。**')
    print('      ⚠️ 這與 ① 是同一件事的兩端:①【低分而是同族】會被放行, ⑧【高分而不同族】會被錯併;')
    print('         **而兩端的解法一樣 —— 開檔那一步不能省, 而不是把門檻調高或調低。**')
    print('      🔴🔴 **而 ⑧ 還有另一半(2026-08-29 同一輪第二個實例, 主視窗 `-06` 批)**:')
    print('      **分數告訴你「這裡有東西」, 它【不告訴你】「所以要開新的」還是「所以要併」。**')
    print('      **兩個實例並排 —— 這一格不是修辭, 是量到的**:')
    print('        0.4658(該輪最高)⇒ 開檔讀 ⇒ **判【不併】**(不同族)')
    print('        0.2010(只有中等)⇒ 開檔讀 ⇒ **判【併】**, 而它就是最好的落點')
    print('      ⇒ **分數對「開新條 / 併進既有」這個問題【完全沒有判別力】。**')
    print('      📌 **而那個決定有另一把判準, 不在本工具裡**:【錯了怎麼辦】——')
    print('        **選【併】選錯了, 下一個人可以拆開;選【新】選錯了, 另一半會永遠留在 inbox。**')
    print('        **兩個選項的錯誤成本不對稱, 而那才是判準。**')
    print('      🔴 **而分數的【順序】也不告訴你任何事**(2026-08-29 第三個實例):')
    print('        同一份草稿 ⇒ 第一名 **0.3625** 開檔讀 ⇒ 判【不併】;')
    print('        第二名 **0.3129** 開檔讀 ⇒ **它就是落點** —— **兩者只差 0.05。**')
    print('        📌 **那個 0.05 沒有任何意義, 而排名讀起來像它有。**')
