 ...
 * 🔴🔴 **為什麼是 4 —— 算式寫在這裡，因為沒有算式的常數，下一個人只會把它改大**：
 *    手機 hero 內距左右各 20 ⇒ 可用寬 **350px**；ticks 排下來的總寬
 *    = 亮起那條 **46** + 其餘每條 **34** + 每個間隙 **8**（`home.css` 的 `.b-hero-tick` / `gap`）
 *    ⇒ **8 格** = 46 + 7×34 + 7×8 = **340** ≤ 350 ✅
 *    ⇒ **9 格** = 46 + 8×34 + 8×8 = **382** > 350 ❌
 *    ⇒ 上限 **8 格**；而既有實拍照片固定佔 **4 格**（`HomeHero` 的 `SLIDES`）⇒ **大圖最多 4 張**。
 *    🛑 **改大這個數字之前先把上面重算一次** —— 第 5 張大圖一上，ticks 就會換行或溢出。
 * 🔵 出處：OD `pcm-banner-v2 / home-carousel-mixed-v1.html` §七逐字要求「那個 4 不可以裸著放」；
 *    那份稿的 46 / 34 / 8 是 2026-09-16 在正式站用 `getBoundingClientRect()` 量的，不是猜的。
 *    ⚠️ 這三個數字的真權威是 `apps/storefront/src/styles/home.css` —— **那邊改了，這段算式就過期了**。
 */
