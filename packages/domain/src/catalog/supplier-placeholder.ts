...
> = [
  ...
  // 🛑🛑 **2026-09-06 · 線 `front`:有一個【看起來該加而絕對不能加】的候選, 寫在這裡免得下一個人加它。**
  //   `https://img.tpl.one/w/products/DNA/500x500_blank/<SKU>.jpg` —— 路徑段逐字含 `_blank`,
  //   而 `⟦front-SNAPSHOTPLACEHOLDER⟧` 那一列曾把它標成「這道濾網接不住的已知缺口」。
  //   🔬 **量了(preview 四頁型錄, 零 DB)**:18 個相異網址全在那個路徑段底下,
  //      取 6 張下載比 sha ⇒ **5 個不同的 hash**(唯一相同的那一對是同商品兩尺寸);
  //      正對照(非 `_blank` 的一張真圖)hash 也不同。**而我開圖看過** ——
  //      `AK-AL155-70-S3.jpg` 是一顆 DNA 空氣濾芯的實拍。
  //   ✅ **⇒ `_blank` 指的是【白底】不是【沒有圖】。加它 = 一次濾掉 18 張以上的真商品照片。**
  //   📌 這正是本檔上方 `bild-` 那一段逐字警告過的壞法(「客人看到一張佔位圖蓋住一張真照片,
  //      而那個壞法不會有人回報」)⇒ 🎯 **那個警告在這裡提前發生了, 攔下它的是量測不是紀律。**
  //   ⚠️ 射程:18 個是【我打的那四頁】掃到的, 不是全庫;本列原本點名的 `HD-PANAM.jpg` 不在其中, 我沒親眼看過那一張。
  //      而結論不依賴它 —— 只要那個路徑段底下存在多張相異的真照片, 綁它就是危險的。
];
