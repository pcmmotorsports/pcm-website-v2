// #10:貨運商代碼 → 中文標籤。**共用的唯一一份。**
//
// ⚠️ ~~原檔頭寫「三個消費端」~~ **那個數字已過期**(2026-08-22 實測 **5 支**:
//    `shipment-dialog.tsx` / `shipment-section.tsx` / `print/shipping-doc.tsx` /
//    `lib/shipping/shipping-doc-dispatch.ts` / `lib/shipping/tracking-number.ts`,
//    再加寄信那條新的)。
// 🔴 **本檔之後不再寫消費端的【數量】** —— 寫死的數字過期時零機械訊號,而數法不會過期:
//    `grep -rn 'CARRIER_LABEL\|carrierLabelOf\|CARRIER_OPTIONS' --include='*.ts' --include='*.tsx' apps packages`
//
// 🔴 **2026-08-22 從 `apps/admin/src/lib/shipping/carrier-label.ts` 搬到這裡(M-4b E4-b)。**
//    ~~「三個消費端」~~ 已過期:出貨通知信(`packages/use-cases` → storefront)是第四個,
//    而它**不可能** import `apps/admin` ⇒ 留在 admin 的話,信件那一側只能再手抄一份。
//    ⇒ **下面那 20 行正是在講「多抄一份會發生什麼」** —— 所以搬的是【值與理由一起】,不只值。
//    ⚠️ `apps/admin/src/lib/shipping/carrier-label.ts` **仍然存在**,內容是一行 re-export
//    加一段指標 —— 因為下面那 20 行的讀者是「將來要新增貨運商的人」,而**他會先打開 admin**。
//    ⇒ 那支檔整支消失的話,他找不到這段理由。
//
// 🔴 **代碼的【集合】權威在 DB,不在本檔**:
 ...
 * (`supabase/migrations/20260805170000_m4b_e10_b2_s1a1_shipments.sql`)。
 *
 * 🔴 **2026-08-22 從 `apps/admin/src/lib/shipping/shipment-repository.ts:39` 搬來這裡。**
 *    搬的理由與下方 `CARRIER_LABEL` 同一個:寄信那條鏈整條在 `packages/` 與 storefront,
 *    而 `packages` 不可反向 import app ⇒ 留在 admin 的話,信件那一側只能再手抄一份
 *    ⇒ **那就是下面那 20 行在講的第六份**。
 *    ⚠️ `shipment-repository.ts` 現在從本檔 re-export 它 —— 寫入路徑那側**一個字都沒改**。
 */
export type CarrierCode = 'hct' | 'sf' | 'other';

/**
 * 代碼 → 中文標籤。**鍵的集合要與 DB 的 CHECK 完全一致**(守門在 `apps/admin/src/lib/shipping/carrier-label.test.ts` —— 🔴 **它不在本檔旁邊**,
 * 因為它同時要讀 `supabase/migrations/` 去對 DB 的 CHECK;寫完整路徑是 Fable 2026-08-22 R2 F4:
 * 只寫檔名的話,**domain 的讀者在本目錄找不到它,會判「這個值域沒有守門」**)。
 *
