 ...
 *
 * ── 🔴🔴 誠實邊界:**這個搜尋沒有供應商維度**(階段 C must-fix)────────────
 ...
 * 🔴 **本片不解**(要解需要讓呼叫端同時帶供應商 = A10c2 的畫面決策 + 可能的 filter 擴充);
 * ⇒ **A10c2 必須在結果列上顯示供應商**,讓人眼能消歧,並把「加供應商 filter」列進 backlog。
 * 本條寫在這裡而不是只寫在 plan,是因為 plan 會過期、而這個 docstring 就在呼叫端手邊。
 */
