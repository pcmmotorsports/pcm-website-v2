...
const SQL_ALLOWLIST: Record<string, { count: number; why: string }> = {
  ...
  //    ✅ **答:兩者都不是 —— 它【一處都不是我寫的】。**
  //       本支是 `admin_record_manual_refund` 的 `CREATE OR REPLACE`,而**本體是從正式庫
  //       `pg_proc.prosrc` 原樣取下來的**(舊 md5 `dcfec025…`,檔頭有宣告),
  //       我只做 **4 處具名替換,而 4 處【全部是同一個 `RAISE EXCEPTION` 裡的字串】**。
  //    🔬 **機械證明(檔頭也有)**:把那 4 段新字串換回舊的 ⇒ **逐字等於舊本體**
  //       ⇒ 我沒有動到別的地方。
  //    🔬 **而行號直接對得出來**:
  //       · 我的 4 處替換落在 `:271-274`
  //       · `refund_amount` 落在 `:205 :215 :216 :305 :311 :352 :354 :360 :362`
  //       ⇒ 📌 **交集是空的。**
  //    🔬 **零聚合**:本檔 `grep -c 'SUM('` ⇒ **0**
  //       (🟢 正對照:同尺量 `20260907200000` ⇒ **1** ⇒ 尺會動。
  //        ⚠️ 而我第一版拿 `20260908080000` 當正對照 ⇒ **它也是 0** ⇒ **那把尺當時是死的**,
  //        我換了一支才敢寫這一行。)
  //    🛑 **而共享的邊界仍要寫下來**(這道閘的精神):本支**不改** `v_remaining` 怎麼來的
  //       —— `:352` 那句 `IF p_refund_amount > v_remaining` 一個字沒動,
  ...
  '20260909090000_m4b_cardalreadyrefunded_confirm_label.sql': {
    ...
    count: 3,
    why:
      '本檔是 admin_record_manual_refund 的 CREATE OR REPLACE,而本體從正式庫 prosrc 原樣取下' +
      '(舊 md5 dcfec025… / 新 3296b558…,檔頭宣告)。我只改 4 處字串,全部在同一個 ' +
      'RAISE EXCEPTION 裡(混合單那句話對「卡已退成功」是錯的,Sean 2026-09-09 拍甲)。' +
      '那 3 處 refund_amount 全是【抄下來的既有碼】:參數宣告、正整數守門、冪等比對、' +
      '上限判斷、INSERT 欄位 —— 我一處都沒碰(替換落 :271-274,refund_amount 落 :205/:215/' +
      ':216/:305/:311/:352/:354/:360/:362,交集為空;檔頭另有機械證明)。' +
      '本檔 SUM( 命中 0(🟢 正對照 20260907200000 ⇒ 1)⇒ 零聚合、不回「已退 / 還能退」、' +
      '不新增任何額度算式;:352 的 v_remaining 仍讀 pcm_order_refundable_remaining。',
  },
  ...
  // ── 2026-09-07 · 線【帳號】`account` 補(⟦b4-TAPPAYDIRECT⟧ 貼板 84;**作者就是我**)──
  //    🔴 **登記, 不是放寬** —— 這道閘的判準我一個字都沒動。
  //    🔬 **`count` 用【這道閘自己的尺】量的**:它報「2 處」, 我照抄。
  //      (本檔前面那筆的 why 記著「用自己的 grep 填出 7 而正確是 2」的事 —— 不重蹈。)
  '20260907130000_m4b_tappaydirect_c1a_isolation_before_idempotent.sql': {
    count: 2,
    why:
      // 🔴 **這支【整段沿用】前一代 `20260907100000` 的 body, 只在最前面插入一段隔離級別斷言。**
      //   兩處 `refund_amount` 因此是**前一代原本就有的那兩處**, 不是本片新增的邏輯:
      //     · `:280` 冪等指紋比對 `r.refund_amount = p_amount` —— 問「這是不是同一次重送」
      //     · `:320` INSERT 的**欄名清單** —— 它在寫一筆退款, 不在讀「還能退多少」
      //   ⇒ 🎯 **兩處都沒有聚合、沒有 SUM、沒有自己算「已退 / 還能退」** ——
      //     本片一個字都沒碰 `pcm_order_refundable_remaining` 那條路。
      '本檔是 20260907100000(補登 RPC)的 CREATE OR REPLACE, 只在 BEGIN 之後插入一段' +
      '隔離級別斷言(不是 read committed 就 RAISE P7B09), 其餘 body 逐字沿用、程式抽的不是手打。' +
      '2 處 refund_amount 都是前一代原本就有的:一處是冪等指紋比對(r.refund_amount = p_amount,' +
      '問「是不是同一次重送」), 一處是 INSERT 的欄名清單(在寫一筆退款)。' +
      '兩處都沒有聚合也沒有自己算「已退 / 還能退」⇒ 不是 #473b-1 要防的繞路。' +
      '🔬 逐字性有正對照:兩代 body 做 diff 是純插入, 而抽出來的前代 md5 = ' +
      '88ca8531a02616e47d5d969b6f49dc59 / 7433, 與本片前置閘 P1 鎖的期望值逐字相同,' +
      '而那個值是從正式庫唯讀 md5(prosrc) 取回來的。' +
      '審查:codex R1(補審貼板 80)+ codex R2 + opus R3 換模型換角度 + codex R4 窄範圍,' +
      '四輪 must-fix 共 12 條全折;本片未貼, 貼要走 Sean「貼 84」授權。',
  },
  ...
  '20260907050000_m4b_tappaydirect_a3_correct_backfill.sql': {
    count: 3,
    // 🔴🔴 **作者就是我**(線【帳號】)—— 寫在這裡免得下一個人以為有第三方審過。
    //    第三方那一半:codex `gpt-6-astra` 對抗審查(鐵則 12①③)有跑, 見該 migration 的 commit body。
    why:
      '本檔是【更正一筆補登的金額】(同交易 void 舊列 + 用同一個 DR 碼補新列), 它**不回答「還能退多少」**。' +
      // ✅ 三處各是什麼(逐處開檔核過, 不是憑 grep 推):
      // 🔴 行號會隨 diff 移動 —— 這三個是 **codex R1 三條修完之後**重新量的(2026-09-07),
      //    不是寫 plan 時的那一版。引用前請用 `grep -n refund_amount <該檔>` 當場核。
      '三處各自是:`:95` 把舊列金額讀進變數(拿去做「新舊相同就拒絕」與 void_reason 的文字);' +
      '`:124` 冪等比對的**等值判斷**(重送時比內容, 不是算式);`:205` INSERT 的欄位清單(寫一列, 不是讀一個答案)。' +
      // ✅ 結構性反面證據:它把「還能退多少」整件事**委託出去**, 自己零算式。
      '本檔零 SUM(、零加減、零 refundable/remaining(剝註解後量);餘額改由既有 ' +
      '`pcm_sync_order_refund_payment_status` / `pcm_order_refundable_remaining` 自己重算。' +
      // 🔬 而那不是推的 —— 拋棄式 PG 2026-09-07 實跑:訂單 21,820,補登記 100 之後
      //   remaining = 21,720;更正成 500 之後 remaining = 21,320 且 card_refunded = 500。
      //   ⇒ 📌 **被保護的那支算出來的數只扣了一次 500**(不是 600、不是 400)
      //     ⇒ 「另一份算式看不到更正」那個形狀在本檔上量不到。
      '🔬 實跑量到(拋棄式 PG 2026-09-07, codex 三條修完後重跑):訂單 13,830、補登記 100 ⇒ remaining 13,730;' +
      '更正成 500 ⇒ remaining 13,330、card_refunded 500 —— 那個數是【被保護的那支】算的, **只扣一次**。' +
      // 🔴 可證偽的那一半:
      '🔴 這一筆哪天失效:本檔若開始出現 SUM( / 對 refund_amount 做加減 / 自己回傳一個金額給呼叫端 ⇒ 立刻作廢。' +
