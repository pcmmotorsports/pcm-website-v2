...
export type AdminOrderFilter = {
   ...
   *
   * 🔴 **第三項是 `#841`(2026-08-23)加的復活條件,而它讓隱藏面【變窄】**:
   *   ```
   *   舊  隱藏 ⟺ tappay ∧ unpaid                          ← 這一行原本就寫在這裡,是 Sean 拍板的字面
   *   新  隱藏 ⟺ tappay ∧ unpaid ∧(淨額 = 0 ∨ 已取消)    ← 新的是舊的【真子集】
   *   ⇒ 今天被藏的一張都沒有多跑出來,只多放行「收到匯款而且沒取消」那一種。
   *   ```
   *   病:客人刷卡失敗 → 改匯款 → 員工登錄收款(每一步都成功)→ 回列表,那張單不見了。
   *   🔴 **規則的唯一定義在 `order-hidden-rule.ts`**(SQL 側與 UI 側都從那裡拿)——
   *      本段是**說明**,不是第二份真相;改規則去那裡,回頭記得同步這段文字。
   *
   ...
   *      而**走它的是拍板,不是疏忽**。
   *
   * ✅ **而 RPC 這個選擇仍然正確 —— 理由整組換掉**
   *    (~~四個都與 PII 無關~~ 🔴 **三個與 PII 無關, 第④個正好相反** ——
   *      codex 2026-08-26 抓到:不 `RAISE` 的目的**就是**不讓 PII 進 log。
...
export type AdminOrderDetail = {
   ...
   */
  openPendingRefundTotal?: number | null;
...
export type MemberOrderDetailItem = {
   ...
   *      ⇒ **同一把尺, 我原本沒套在自己身上。**
   * 🛑 **顯示端只印【有沒有】**, ⛔ ~~不印日期也不印數量~~ ——
   *    🟢 **2026-09-06 Sean Q6 拍甲推翻了【數量】那一半**(⟦b9-SHIPUI⟧ ①):
   *    顧客可看「每件出了幾件 / 共幾件」⇒ 見下一欄 `shippedQuantity`。
   *    ⚠️ **而【日期】那一半沒有被推翻**, 逐件仍不印時間。
   */
  shipped: boolean;
  /**
   * ⟦b9-SHIPUI⟧ ① **這一件【實際出了幾件】**(2026-09-06;Sean Q6 拍甲)。
   *
   * 🔴 **它修的那個病**:`allItemsShipped` 原本只問「有沒有出過」——
   *    一筆 `quantity: 5` 只出 2 件會被算成**已全出** ⇒ 分批小字被抑制
   *    ⇒ 🛑 **客人以為到齊了, 於是【不會來問我們】剩下那 3 件**
   *    ——「其餘商品出貨時會再通知您」那句話存在的理由, 被反轉成了它的相反。
   * 🔴 **它推翻的那條政策, 而且【只推翻一格】**:原本「數量摘要不給顧客站(洩漏出貨節奏)」
   *    ⇒ Sean 甲之後:顧客**可看**【每件出了幾件 / 共幾件】,
   *    **仍不可看**箱數、出貨批次時間、追蹤號以外的節奏。
   *    ⇒ 📌 那道界線由 `MEMBER_ORDER_DETAIL_SELECT` 的 forbidden-token 守門執行, 不是靠記得。
   * 🔵 **資料不擴投影到新表** —— 就在既有的 `shipment_items` 內嵌裡加一欄。
   * ⚠️ **有效的箱**與 `shipped` 同源(`shipped_at` 非空【且】`deleted_at` 為空);
...
export type MemberOrderDetail = {
  ...
  paidAt: string | null;
  /**
   * ⟦b9-SHIPUI⟧ **最早的一次出貨時間**(未出貨 ⇒ null)。進度軸「已出貨」那一階用它。
   *
   * 🔴 **來源是包裹真相(`shipment_items → shipments.shipped_at`), 不是 `fulfillmentStatus`** ——
   *    稿 `order-detail-page.html:172` 逐字:「`fulfillment_status` 是 stale 出貨軸,
   *    拿它點亮『已出貨』等於**對客人說謊**」。2026-09-02 實查一張真出貨的單, 那一欄仍是
   *    `notOrdered`(只有 item 層動了)⇒ **那不是理論風險。**
   * 🔴 **作廢的箱(`shipments.deleted_at` 非空)不算** —— 否則一張被作廢的出貨單會點亮客人的
   *    進度條, 而他手上沒有貨。
   * ⚠️ 分批出貨時取**最早**那一次:客人問的是「開始出了沒」, 而第一箱就回答了那個問題。
   */
  shippedAt: string | null;
  /**
   * ⟦b9-SHIPUI⟧ **撈到的每一個品項都至少在一個有效的箱裡**。
   *
   * 🔴 **它決定那句分批小字要不要出現**(Sean 2026-09-02 拍丙:亮「已出貨 MM-DD」+
   *    小字「其餘商品出貨時會再通知您」)—— 全部出完時那句話是**假的**。
   *    ⛔ **上一行那個 `MM-DD` 已被他自己推翻**:2026-09-02 Q31 拍甲「跟鄰居一致(`2026-09-02`)」
   *       (落點 `~/pcm-mailbox/拍板-20260902-上午.md:244`)⇒ 日期走 `YYYY-MM-DD`。**本欄不帶格式**,
   *       它只回 ISO 字串;格式化是 storefront 那一層的事。
   * 🛑 **而 `itemsTruncated` 時它一律是 `false`(保守)**:我看不到全部品項 ⇒ 不能宣稱「全部出完」。
   *    ⇒ 那個方向的錯是**多印一句話**;反過來會讓客人以為東西都出了。
   *    ⛔ ~~「多印一句**無害**的話」~~ —— **那兩個字是錯的**(codex 對抗審查 2026-09-02):
   *       `OrderDetailView.test.tsx` 那格的失敗訊息**自己就逐字寫著**
   *       「全部出完了還說『其餘商品出貨時會再通知您』⇒ 客人會為了**等一批不存在的貨**而不聯絡我們」
   *       ⇒ 📌 **同一片裡兩句註解互相打架, 而它們各自讀起來都合理。**
   *    ✅ **正確的說法:兩個方向都會傷人, 而我們選的是【比較輕的那一邊】** ——
   *       白等一批不存在的貨, 客人還會來問;而以為到齊了, 他就不會來問了。
   *       🛑 **這是一個取捨, 不是一個沒有代價的選擇。**
   *    ⚠️ **而它有一個【真的會發生】的入口**:恰好 200 個品項而且全部出完
   *       ⇒ `itemsTruncated` 為 true ⇒ 這裡算 false ⇒ **印那句話, 而根本沒有其餘商品**。
   * ⚠️ **零品項時也是 `false`** —— `[].every()` 回 `true`, 而「沒有品項」不是「全部出完」。
   *
   * 🔴🔴 **已知限制(code-reviewer 2026-09-02 抓;`-fc` 收下而【刻意不修】)**:
   *    它問的是「這個品項**有沒有**進過一個有效的箱」,**不問數量**。
   *    ⇒ 一筆 `quantity: 5` 而只出了 2 件的品項 ⇒ 這裡算 `true`
   *    ⇒ ⇒ **分批小字會被抑制, 而客人以為到齊了** —— 正是上面那段警告的方向。
   * 🛑 **而修法不自由, 所以不在本片修**:要算數量就得把 `shipment_items.shipped_quantity`
   *    拉進顧客站投影,而那撞到一條既有政策 ——
   *    **數量摘要對顧客站永久零滲入,理由是它洩漏我們的出貨節奏。**
   *    ⇒ 📌 **那是 Sean 的板, 不是實作的板。已交主視窗端他。**
   * 🔴🔴 **而【擋它的是誰】要講清楚(code-reviewer R2 抓,`-fc` 開檔複驗)**:
   *    ⛔ ~~本段第一版寫「`SupabaseOrderAdapter.test.ts` 的 forbidden-token 那格擋著」~~ —— **那是假的**。
   *    那個迴圈逐字只 assert **`ORDER_LIST_SELECT`**(`expect(\`ORDER_LIST_SELECT:${…}\`)`),
   *    `MEMBER_ORDER_DETAIL_SELECT`(= 本片這條路)**不在它的分母裡**
   *    ⇒ 真把 `shipped_quantity` 加進明細投影,**那道閘不會紅**(只有 byte-equal 那格會紅)。
   *    ⇒ ⇒ **政策的【精神】成立,而【機制】不成立。擋住明細這一側的是 Sean 的板,不是一道測試。**
   *    📌 **「機制擋著」與「人的拍板擋著」是兩個宣稱** —— 前者可以被下一個人 grep 反駁掉,
   *       而他會連帶把「所以不修」一起推翻。
   * 🔵 **在他拍板之前, 這個欄位的語意逐字是「每個品項都至少出了一件」, 不是「都出完了」。**
   */
