 ...
 *
 * ── 🔴🔴 **本模組沒有字元集守門,而那不是疏漏** ──────────────────────────
 * ⚠️ #347-B 之後這是**唯一的搜尋維度**;原本另有兩個(A9b1 訂單編號、A9b2 供應商單號),
 *    它們各自有字元集守門(只放行 `[A-Z0-9-]` / 擋掉 `,` `(` `)` `"` `\`),
 ...
 *    不是「把守門拿掉、威脅還在」。下面這段解釋為什麼本維度從一開始就不需要它。
 *
 ...
 *
 * 🔴 **為什麼是擲、不是把壞掉的部分濾掉**(此判準原本引 `supplier-order-no-search.ts`,
 *    該檔已隨 #347-B 刪除;判準本身不變,理由是既有判準):
 ...
 * 原本的同款先例寫在 `order-number-search.ts`,該檔已隨 #347-B 刪除 ⇒ 本檔是現存唯一一份)。
 * 這一組**整組都是隱形字元** ——
