 ...
 *
 * ⚠️ 這支檔會在**任何人改動那【九】個數字時變紅**。那是它的用途,不是它壞了:
 *    改門檻是一個**會改變線上告警行為**的決定 ⇒ 它應該要有人按一下確認。
 *    ⇒ 改的時候把下面的期望值一起改,**而那一次改動就是那個「按一下」**。
 */
describe('🔴 【九】支排程的門檻是【唯一來源】,而這裡把值釘住', () => {
  it('【九】支的名字與門檻逐格釘死(改動 = 改變線上告警行為, 要有人按一下)', () => {
    expect(
   ...
   */
  it('剛好【九】支 —— 少一支 = 那支排程再也沒有人在看,而它不會自己出聲', () => {
    expect(CRON_JOB_WHITELIST.length).toBe(9);
  });
   ...
   */
  it('失敗計數無意義的名單 = 【四支】純 SQL 的(它們寫不出失敗心跳)', () => {
 ...
 *    ⚠️ 碼裡今天是【九】支(2026-09-05 三條線各加一支:`pcm-acl-digest` /
...
describe('⟦b9-HBSEMANTIC⟧ 週期對照表 —— 而它不解析 cron 運算式', () => {
   ...
   */
  it('🔴 【九】支分成兩種語意 —— 八支答【它停了嗎】, 一支答【它準時嗎】', () => {
    ...
    expect(byMeaning.停了嗎).toHaveLength(8);
