 ...
 *
 * ⚠️ 這支檔會在**任何人改動那【十】個數字時變紅**。那是它的用途,不是它壞了:
 *    改門檻是一個**會改變線上告警行為**的決定 ⇒ 它應該要有人按一下確認。
 *    ⇒ 改的時候把下面的期望值一起改,**而那一次改動就是那個「按一下」**。
 */
describe('🔴 【十】支排程的門檻是【唯一來源】,而這裡把值釘住', () => {
  it('【十】支的名字與門檻逐格釘死(改動 = 改變線上告警行為, 要有人按一下)', () => {
    expect(
      CRON_JOB_WHITELIST.map((w) => [w.jobName, w.staleMinutes] as const),
    ).toEqual([
      // 🔴 2026-09-14 Sean 改口 26 ⇒ 14 小時(與晚班同一顆);理由見 cron-jobs.ts:58 那段。
      ['pcm-anomaly-alert', 14 * 60],
      ...
      ['pcm-settle-retry', 30],
      // 🔵 2026-09-05 匯款兜底(20260905180000)。門檻 30 = 排程 */10 連漏三輪才叫,
      //    與 `pcm-capture-recheck` 同一把尺(它也是 */10)。
      ['pcm-late-payment-sweep', 30],
      // 🔵 2026-09-08 加(⟦b9-PROBESCHED⟧ 片 1 · 20260908030000)。門檻 2880 與 `pcm-acl-digest`
      //    同一把尺:兩支都是 `0 0 * * *`, 2880/1440 = 2 ⇒ 連漏兩天才叫。
   ...
   */
  it('剛好【十】支 —— 少一支 = 那支排程再也沒有人在看,而它不會自己出聲', () => {
    expect(CRON_JOB_WHITELIST.length).toBe(10);
  });
   ...
   */
  it('失敗計數無意義的名單 = 【五支】純 SQL 的(它們寫不出失敗心跳)', () => {
    // 🔵 2026-09-05 一天加了三支, 而那是 merge 的產物 —— 四支都是純 SQL、
    //    同一個物理限制(同交易裡寫的失敗心跳會跟著回捲)。
    //    🔴 它們彼此的差別只有【碰不碰錢】:`expire` 與 `settle-retry` 碰,
    //       `acl-digest` 不碰, `late-payment-sweep` 碰 —— **後果不同, 而【量不到】是一樣的。**
    expect([...FAILURE_COUNT_MEANINGLESS]).toEqual([
      'pcm-expire-unpaid-orders',
      'pcm-acl-digest',
      'pcm-settle-retry',
      'pcm-late-payment-sweep',
      // 🔵 2026-09-08 加(⟦b9-PROBESCHED⟧ 片 1)。同一個物理限制, 而它【不碰錢】——
      //    與 `acl-digest` 同一類:後果比 expire / settle-retry 輕, 而【量不到】是一樣的。
      //    🔴 不變式 = **這個 Set 的插入序要與 `CRON_JOB_WHITELIST` 的相對序一致**
  //       (`PgAnomalyAlertReaderAdapter.test.ts:1411-1412` 比的就是那個)。
  //       ⛔ ~~新增一律 append~~ —— 那句今天剛好成立而它不是規則:
  //       有人把新 job 插在白名單中段再 append 到這裡, 照那句做會紅。
      'pcm-net-exposure',
    ]);
  });
 ...
 * 就算那一片不做, 這張表也答得出「今天這幾支的 schedule 是什麼、而它的週期是幾分鐘」。
 *
 * ## 它擋什麼
 ...
 *    全表(六支, 分母是全部不是抽樣)—— 🔵 **這個「六支」刻意不改**:
 *    它是 **2026-09-02 那一發量測的讀數**,不是今天的現況。
 *    📌 把它改成七,等於竄改一個帶時點的觀察 —— 而下一個人會以為那天量到七支。
 *    ⚠️ 碼裡今天是【十】支(六 + 2026-09-05 三條線各加一支:`pcm-acl-digest` /
 *       `pcm-settle-retry` / `pcm-late-payment-sweep`),而**那要重撈一次才算數**。
 *       ⇒ **那六支的 schedule 全部與下表相同**。
...
describe('⟦b9-HBSEMANTIC⟧ 週期對照表 —— 而它不解析 cron 運算式', () => {
  ...
  const PERIOD_MINUTES_BY_SCHEDULE: Readonly<Record<string, number>> = {
    ...
    '*/10 * * * *': 10,
    '0 * * * *': 60,
      // 🔵 2026-09-14:`pcm-anomaly-alert` 從 '0 1 * * *' 改成一天兩班(migration `20260915120000`)。
      //    ⇒ 週期 1440 ⇒ **720**。而 '0 1 * * *' 已經沒有人用 ⇒ 下面那一格會叫 ⇒ 一起拿掉。
      //    🔴 這裡寫 720 是**兩班之間的間隔**(01:00 與 13:00 剛好等距)——
      //       這張表不解析 cron 運算式, 而**不等距的兩班沒有單一個「週期」可寫**;
      //       哪天改成 `0 1,20 * * *` 這種不等距的, 這一格要回來重想, 不能照抄。
    '0 1,13 * * *': 720,
      // 🔵 每天 00:00(⟦b9-ACLDRIFT5⟧ 片一)
      '0 0 * * *': 1440,
  };
   ...
   * 🔵 後來 merge 進來的兩支(`pcm-settle-retry` 30/10、`pcm-late-payment-sweep` 30/10)
   *    比值都是 **3.0** ⇒ **都不踩線**, 上面那段訂正不受影響。
   *
   * 🔴 **[2026-09-14 —— 分子分母【一起】動了, 而比值幾乎沒變]**
   *    `pcm-anomaly-alert` 改成一天兩班(`0 1,13 * * *`, migration `20260915120000`)
   *    ⇒ 分母 1440 ⇒ **720**;Sean 同日改口把門檻 26 小時 ⇒ **14 小時**
   *    ⇒ 分子 1560 ⇒ **840** ⇒ 比值 1.08 ⇒ **1.17**, **仍然 < 2 ⇒ 它還是那支答「它準時嗎」的**。
   *
   *    🔴 **而門檻【必須】跟著改, 這不是順手收緊 —— 不改的話這次改動會讓偵測變晚**
   *       (codex R1/R2 MF1;我第一版在這裡寫「絕對值沒有變壞」, **那句話是假的**):
   *       ```
   *       台北 9/16 21:05 整支停掉(而且沒有失敗心跳)
   *         舊(一天一班 · 26 小時):最後成功 9/16 09:00 ⇒ 9/17 11:00 才 stale(停掉後 14 小時)
   *         兩班而門檻不動(26 小時):最後成功 9/16 21:00 ⇒ 9/17 23:00 才 stale(停掉後 26 小時)← 晚 12 小時
   *         兩班 + 門檻 14 小時      :最後成功 9/16 21:00 ⇒ 9/17 11:00 就 stale(停掉後 14 小時)← 追平
   *       ```
   *       📌 **多一班會刷新「最後成功時間」** —— 所以動排程就是在動分母, 而分母動了分子不動,
   *          那個「幾小時沒心跳才算異常」的數字就換了意思。
   ...
   */
  it('🔴 【十】支分成兩種語意 —— 九支答【它停了嗎】, 一支答【它準時嗎】', () => {
    ...
    expect(byMeaning.停了嗎).toHaveLength(9);
    // 🔴 釘住那個【恰好 2.0】—— 它是分界開始承重的那一刻。
    expect(2880 / 1440).toBe(2);
    // 🔴 釘住 2026-09-14 那次「分子分母一起動」:三個值擺在一起, 是要讓下一個人一眼看出
    //    **不是有人把門檻放寬了, 是週期減半、門檻跟著減半**。
    expect(1560 / 1440).toBeCloseTo(1.083, 3); // 舊:26 小時 / 一天一班
    expect(1560 / 720).toBeCloseTo(2.167, 3); // 🛑 若當初只改排程不改門檻 ⇒ 會翻到「它停了嗎」那一邊
    expect(840 / 720).toBeCloseTo(1.167, 3); // ✅ 現值:14 小時 / 一天兩班 ⇒ 仍是「它準時嗎」
  });
});
