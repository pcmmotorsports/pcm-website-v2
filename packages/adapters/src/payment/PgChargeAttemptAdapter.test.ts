...
//      🔴🔴 **[2026-09-11 再訂正 —— 上面那句訂正【自己也是錯的】,而它在這支綠測試裡躺了 10 天]**
//        📌 **一句錯的斷言住在一支綠的測試裡,三綠永遠不會紅。** 2026-09-11 它讓兩個窗各走錯一次。
//        🔬 【量的】以「`redeem_coupon(` 的**呼叫語法**」當尺(排掉註解標記)⇒ repo 真呼叫端 **2 處**:
//          `20260901021000_..._create_order_redeem.sql:573` · `20260901030000_..._zero_total_settle.sql:562`
//        ① 🔴 **呼它的不是 `create_order`,是 trigger 函式 `coupon_redeem_on_paid`** ——
//           `20260901020000:180-181` 的 COMMENT 逐字:「在 unpaid⇒paid 那一次轉換時呼 redeem_coupon」
//           ⇒ 那正是 Sean `20260829150000:7` 拍的乙:「**付款成功才算**」,`:11`「不綁建單」。
//        ② 🔴 **而 `create_order` 呼它是【設計上明文禁止】的** —— `20260901003000:655-656` 是一道
//           fail-closed 閘:`IF v_args LIKE '%public.redeem_coupon(%' THEN RAISE EXCEPTION '片3 fail-closed…'`
//        ③ 🛑 **而那兩支【未貼】** —— `supabase/APPLIED.tsv` 逐字 `20260901021000` ⇒ 0 · `20260901030000` ⇒ 0
//           ⇒ 📌 **正式庫那一側真呼叫端是 0;repo 這一側是 2。兩個都對 —— 受詞是哪一個世界。**
//           🛑 **出處分開記**:repo 那個 2 是本則作者自己量的(尺與行號在上面);
//              **正式庫那個 0 是【另一個窗唯讀查正式庫】量的,本則作者沒有量過。**
//              ⇒ 而那正是本段在講的事:一句話沒有出處,下一個人分不出它屬於哪一個世界。
//        🔵 而**下面那段「我錯在兩個地方」仍然成立、也仍然是最有用的一段** ——
//           它講的是**那把尺為什麼騙人**,而那一格與上面被劃掉的結論無關:
//           逐字「**我在自己那條落後的分支上量 —— 一把尺量到 0,而它量的是一棵【比世界舊】的樹,
//           而那個 0 讀起來完全正常。**」🛑 修結論的人請不要順手刪掉它。
//      ⇒ 而我錯在**兩個地方**,兩個都是分母:
