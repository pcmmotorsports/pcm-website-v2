...
describe('mapVariantRow', () => {
  ...
  describe('🔴 D-4 引信:有人【從這兩支 mapper】接上經銷價時要發得出聲音', () => {
    ...
    // ⚠️ **算覆蓋率的人請讀這段:下面這格【不算獨立的一道】。**
    //    它對 `store` 的斷言與既有那格(本檔上方「經銷價防護」)**同生同死** ——
    //    同一天紅,而且**誘發同一個「把 0 改成真值」的修法**,那正是本組宣稱要防的那個修法。
    //    ⇒ **保留它是為了下面那段 C4 的敘事**(「真而空」與「假的」要的修法完全不同),
    //      **不是為了多一道守門。** 📌 寫在這裡,是因為**下一個算覆蓋率的人不會讀我們的對話。**
