...
export type CreateOrderRpcArgs = {
   ...
   *
   * 🔴 **2026-09-18 訂正:舊那一支【已經不在了】。** 唯讀實查正式庫 `pg_catalog.pg_proc`
   *    (`nspname='public'` / `prokind='f'`)⇒ `create_order` **只有 1 支**,11 參那一代。
   * ✅ **結論不變:這個鍵仍然【不可以是 optional】。** 但**理由換了** ——
   *    今天少送它不會靜靜掉回舊那支,而是 **PostgREST 直接 `PGRST202` 拒收**(找不到相符簽章)。
   * ⇒ 📌 **差別在哪很重要**:舊理由說的是「**會靜靜存成錯的付款方式**」(壞資料、不會叫),
   *    新事實是「**整張單建不出來**」(會叫、而且立刻)。
   *    下一個人如果照舊理由去評估風險,會把一個**吵的**故障當成一個**靜的**故障。
   */
