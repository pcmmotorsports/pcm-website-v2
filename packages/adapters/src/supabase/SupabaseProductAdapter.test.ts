...
describe('searchByKeyword — 料號完全命中排第一(Sean 2026-09-06)', () => {
  ...
  function makeRpcMock(opts: {
    ...
    const client = {
      ...
      from() {
        // 🔴🔴 **兩條查詢共用這個 builder, 而它們的【結尾那一個方法】不同** ——
        //   完全命中那一發:`.select('id, external_id').ilike(…).order(…).limit(…)`
        //   撈那一頁那一發:`.select(BIG).in(…).order(…)`
        //   ⇒ 📌 用 `sawIlike` 記住走的是哪一條, 否則 `.order()` 不知道該回 builder 還是回結果。
        //   ⚠️ 我第一版讓 `.ilike()` 直接回 Promise ⇒ 實作後面接的 `.order().limit()` 打在 Promise 上
        //     ⇒ 三格當場紅, 而**那個紅是 mock 壞掉, 不是實作壞掉** —— 兩者在報表上長得一樣。
        const b: Record<string, unknown> = {};
        let sawIlike = false;
        const exactRows = () => {
          const rows = (opts.exactIds ?? []).map((id) => ({
            id,
            external_id: opts.exactExternalId ?? 'AZ203',
          }));
          if (opts.exactError) return null;
          const rx = new RegExp(
            '^' +
              (captured.pattern ?? '')
                .split('%')
          ...
          // 🔴 **照 ILIKE 的語意真的過濾** —— 原本收下 pattern 就丟掉、無條件回 exactIds
          //   ⇒ DB 端的過濾從來沒被模擬過 ⇒「候選撈不回來」那一族對每一格都不可見(R2 must-fix)。
          return rows.filter((r) => rx.test(r.external_id));
        };
        b.select = () => b;
        ...
        b.order = () => {
          if (sawIlike) return b;
          // 🔴 回**真的列且照 id 升冪** —— PostgREST 帶 `.order('id')` 就是這樣回的
          //   ⇒ 這樣「實作有沒有照 pageIds 重排」才量得到(舊版回 `[]` ⇒ res.items 恆空)。
          return Promise.resolve({
  ...
  it('🔴 `%` 與 `_` 不得變成萬用字元 —— 這個函式整個的意義就是【完全相等】', async () => {
    ...
    // 🔵 現行 pattern 是**字元之間插 `%`**(涵蓋 DB 那一列帶分隔號的寫法)⇒ `AZ23` ⇒ `%A%Z%2%3%`。
    //   而 `%` `_` 已在正規化那一步被吃掉 ⇒ 客人打的那兩個字元不會變成萬用字元。
    expect(captured.pattern).toBe('%A%Z%2%3%');
  });

  it('🔴🔴 鏡像方向:客人打 `AZ203` 而【DB 那一列帶分隔號】(`AZ-203`)也要提前', async () => {
    // 🛑 這一格是 R2 逼出來的, 而它**在上一版必紅** ——
    //   上一版 `.ilike('external_id', '%AZ203%')` 比的是**原始欄** ⇒ `AZ-203` 撈不回來
    //   ⇒ 那句嚴格相等永遠沒機會跑 ⇒ **提不了前, 而畫面完全正常。**
    // 🔬 而那種列是量到的:`20260904180000_…:36-38` 逐字列出正式庫 `01-0110058` / `01022.4501-01`。
    const { client } = makeRpcMock({
      ids: ['a-zdm130', 'c-az203'],
      exactIds: ['c-az203'],
      exactExternalId: 'AZ-203',
    });
    const res = await new SupabaseProductAdapter(client).searchByKeyword('AZ203', {
      limit: 20,
      offset: 0,
    });
    expect(res.items[0]!.id, 'DB 那一列帶分隔號就提不了前 ⇒ 鏡像方向還是破的').toBe('c-az203');
  });

  it('🔵 1-2 字 ⇒ 那一發【根本不發】(它會撞 db-max-rows 而 206 是成功、error 看不到)', async () => {
    const { client, captured } = makeRpcMock({ ids: ['a', 'b'], exactIds: [] });
    await new SupabaseProductAdapter(client).searchByKeyword('AZ', { limit: 20, offset: 0 });
    expect(captured.pattern, '1-2 字也去打了那一發 ⇒ 可能拉回 2000 列而真正那筆不在裡面').toBeUndefined();
  });
