...
describe('searchByKeyword — 列的順序由【上游】決定, 不由 PostgREST 的 id 升冪決定', () => {
  // ⛔ ~~**這一格守的是一道【接縫】, 而它今天是 no-op**~~ ⇒ ✅ **2026-09-07:它已經是真的守門。**
  //   ⛔ ~~今天 `pageIds` 來自 `[...brandIds].sort()` ⇒ 它就等於 id 升冪 ⇒ 這一格【現在必綠】~~
  //   ⇒ **兩個前置都滿足了**:①db 的 `62`(`20260906950000`)已貼(`supabase/APPLIED.tsv:548`)
  //     ②`SupabaseProductAdapter.ts:587` 的 `.sort()` 已拿掉。
  //   🔬 **而「它會紅」是量到的, 不是推的**:把 `.sort()` 加回去 ⇒ 本格紅在
  //     `expect(captured.pageIds).toEqual(ids)`(`['a','b','c']` vs `['c','a','b']`)。
  //   🛑 **舊字面留刪除線不刪** —— 搜「現在必綠」的人要在同一發撞到「它已經不是了」。
