 ...
 * 既有共用 helper `fetchAllPaginated` 的 `PAGE_SIZE` 正是 `1000`
 * (`helpers/product-query-support.ts:56`;2026-08-18 當場核過**仍是 1000** = **code 事實,沒變**)。
 * ~~= **零餘裕**~~
 * 🔴 **2026-08-18 更正:「零餘裕」那句現在是【假的】。** `db-max-rows` 已從 1000 調到 **2000**
 * (Sean 親手調;V 窗 2026-08-18 對正式站實測 `products?select=id&limit=5000`
 *  ⇒ HTTP `206` / `content-range 0-1999/19777`;分母 19,777 > 2000 ⇒ 量到的是天花板本人。
 *  **效度:正式站 / storefront anon key / 那一刻。🔴 本段的兩位改動者(I 窗、B 窗)【均未自驗】,
 *  都是轉錄 V 窗的量測。**)
 * ⇒ `1000 < 2000` ⇒ 現在**有一倍餘裕**。
 * 🔴🔴 **而餘裕是【設定】給的,不是程式保證的** —— Dashboard 改回 1000 它就**再次歸零**,
 * 而**不會有任何東西紅**(沒有測試、沒有型別、沒有一條 grep 看得到那個遠端設定)。
 * ⇒ **上面那條規則(頁大小等於伺服器上限 ⇒ 短頁判準零判別力)【沒有過期】,過期的是例子。**
 * ⇒ 而下面那句「刻意不寫成 `max-rows - 1`」的理由**因此更強,不是更弱**。
 * ⚠️ **而這裡刻意不寫成 `max-rows - 1`** —— 那會把正確性綁在一個**遠端隨時可改**的值上
 * (而且官方文件**沒有說它對內嵌資源怎麼算**)。
 * ⇒ 挑一個**明顯更小的固定值**。
...
export class SupabaseOrderAdapter implements IOrderRepository {
  ...
  async findOrderDetailForCustomer(
    ...
    // ══ 🔴🔴 **部分取消過的單:兩個金額欄【一起】收成 null** ═══════════════════════
    //
    // 🛑 **根因一句**:`balance_due = o.total − 已收`, 而 **`orders.total` 在部分取消時
    //    【不會跟著降】** —— 那是契約不是 bug(`20260804120000:11` / `:89`、`20260804150000:68`
    //    三處逐字;全樹 `SET total =` 零處可執行碼)。正因為它不降, 才另外有一支
    //    `pcm_order_effective_amounts_v`(`20260907210000`)在算 `effective_total`,
    //    而**匯款通知信 2026-09-07 已經改讀它**(`20260907220000:40-41`)。
    //
    // 🔴🔴 **而「所以信是對的」這句話【是假的】, 我寫過一次**(Fable 2026-09-12 審 F2;我複核成立):
    //    ⛔ ~~客人拿不到網頁上的數字也沒關係, 信裡那個是扣好的。~~
    //    那封信**只在建單時寄一次**(排信面對 `email_outbox` 做 anti-join、**不看 status**:
    //    `20260906180000:133-136`)⇒ 而部分取消**發生在建單之後** ⇒ 兩種時序都拿不到正確的數:
    //      · 取消在寄出**之後** ⇒ 客人手上那封信是**取消前的全額**(沒有任何路會重寄)
    //      · 取消在寄出**之前** ⇒ 送信前那道快照比對(`sweep-email-outbox.ts:1922-1923`)現在讀的是
    //        effective ⇒ 對不上 ⇒ 標 stale **不寄**, 而 anti-join 讓它**不會重排** ⇒ **一封都沒有**
    //    ⇒ 🛑 **所以本段把數字收掉之後, 那個客人【沒有任何正確金額的來源】, 只剩客服。**
    //      Sean 拍甲時聽到的前提是「信裡是對的」—— **那個前提錯了, 而錯的是我** ⇒ 已回報主視窗重端。
    //      📌 **這一段【仍然要留】**:印一個會讓他匯多的數字, 比不印數字糟。
    //
    // ⇒ **兩側各錯一個方向, 而兩個方向都傷客人**:
    //    · `balanceDue`(還要付多少)⇒ **比真正該付的【多】** ⇒ 客人照頁面匯 ⇒ **匯多了**
    //    · `overpaidTotal`(多付多少)⇒ **比真正該退的【少】** ⇒ 我們少退給他
    //    🔬 誤差是 repo 早就量過的:`20260909100000` 檔頭逐字「兩件各未稅 1000、原稅 100、
    //       已收 2100、**取消一件** ⇒ view 說多收 **1100** · 真正該退 **1050** · 誤差 50」。
    //
    // ⚠️ **這個世界到得了**:部分取消要求 `unpaid` + 刷卡 attempt 全 failed 或零筆
    //    (`20260805100000:359-364`)⇒ **匯款單沒有刷卡 attempt ⇒ 正好落在允許的那一格**;
    //    而多付也不翻狀態(`20260904230000`)⇒ 單子就留在 `unpaid`。中間沒有任何閘。
    //    🔵 今天的分母是 0 張(正式庫唯讀 2026-09-12)—— 而**那一天不會有訊號**。
    //
    // 🔴 **一個判準、一個來源**(主視窗 2026-09-12 要求):兩欄是**同一個 `balance_due` 的兩側**,
    //    而讓它們不可信的是**同一件事** ⇒ 📌 這裡只有一個布林, 兩欄一起吃它。
    //    各寫一份的話兩邊以後一定會走鐘。
    //
    // 🛑 **這一段【不是 TODO, 它是現行規格】** —— Sean 2026-09-12 拍甲:「有取消件(或問不到
    //    取消件數)⇒ 不印金額, 改印第一層那句不帶數字的提醒」, 而他知道代價(客人在網頁上
    //    看不到還要付多少;而信裡那個數是扣好的)。
    //    plan:`docs/plans/2026-09-12-partcancel-storefront-balance-plan.md` §3 甲。
    //    ⇒ 🔵 **只有在 `member_order_balance_v` 改成讀 `pcm_order_effective_amounts_v`
    //      (plan 的乙)之後, 這一段才該回來重讀** —— 而那要先解「用過券的未付款單顯示
    //      原價還是折扣價」那一題(`effective_total` 會把券整張作廢算回原價 ⇒ 可能比原本還高)。
    //      **在那之前不要拿掉它:它今天擋的是一個真的會讓客人匯多的數字。**
    const cancelledItemsMakeAmountsUntrustworthy =
      // 🔵 `null` = **問不到**(那支 RPC 降級)⇒ 一律當成「可能有取消」。
      //    🛑 **不知道 ≠ 沒有取消** —— 讀成後者會讓降級期間的部分取消單照印錯的數字。
      cancelledByItemId === null ||
      // 🔵 看的是【件數 > 0】不是【有沒有那個鍵】:沒有取消的單那支 RPC 回空物件 / 全 0。
      Object.values(cancelledByItemId).some((n) => n > 0);
    if (cancelledItemsMakeAmountsUntrustworthy) {
      // 🔴 `null` = **算不出來** ⇒ 顯示端印第一層那句不帶數字的提醒(含「請不要再匯款」)。
      //    ⛔ 不准補 0 —— 0 的意思是「剛好付清」, 那是一個具體斷言。
      balanceDueAmount = null;
      overpaidAmount = null;
    }
  ...
  async listOrderSummariesForAdmin(
    ...
    // Q5 乙(2026-09-14):客人身分 = view 的 `tier_at_checkout`(下單當下的等級)。多值 IN,同來源 / 管道那兩軸。
   ...
   * (`helpers/product-query-support.ts:56`;2026-08-18 核過仍是 1000 = **code 事實**)。
   * ~~**等於**傳聞中的伺服器 `db-max-rows`(1000…**我未自己量**)~~
   * 🔴 **2026-08-18 更正:那句「等於」現在是【假的】** —— `db-max-rows` 已是 **2000**
   * (Sean 親手調;V 窗量法 `products?select=id&limit=5000` ⇒ HTTP `206` /
   *  `content-range 0-1999/19777`。**I 窗與 B 窗均未自驗,轉錄 V 窗**)。
   * ⇒ 今天 `1000 < 2000`,那支 helper **暫時**不會第一頁就停。
   * 🔴🔴 **「暫時」兩個字是重點**:餘裕來自一個 **Dashboard 上點一下就能改的設定**,
   *    改回去的那一刻它**再次歸零,而不會有任何東西紅**。
   * 🔴 **而這一段的論證與那個數字是多少【無關】** —— 它講的是
   * 「**只要**伺服器上限低於頁大小,每一頁都會不滿」,那個「只要」在 2000 之下同樣成立,
   * 只是今天沒有觸發。
   * ⇒ **下面這條規則不受影響,它講的是機制不是那個數字:**
