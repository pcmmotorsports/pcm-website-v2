...
//
// ══════════════════════════════════════════════════════════════
// 🔵 **2026-08-31 第二次重釘(`643f185cc39c` ⇒ `e032afbe64b9`), 而【翻掉它的那次改動沒有作者】**
// ══════════════════════════════════════════════════════════════
//   成因(量到的):線 account 在 `12ffc026` 寫下這道閘時它是**綠的**;dev 那一側 `f49f1eae`
//   上**這支檔根本不存在** ⇒ 兩邊都沒有紅。合併 `a77766ce` 之後才紅。
//   翻掉它的是 `82c9c3ac docs(adapters): 散文指向那個 toBe` —— 對落點①③ 兩支 `.ts`
//   `git show --numstat --format='' 82c9c3ac` ⇒ 各 `6 0`(6 加、**0 刪**), 非註解新增行 **0**。
//   📌 **⇒ 一個零行為改動的註解 commit, 把另一條線的閘翻紅了。兩位作者都沒做錯事。**
//   ⇒ 而它活過 5 次收割(`a77766ce`→`551b2e91`→`9e0a770c`→`8df2de0e`)、失敗值從頭到尾沒變過,
//     **因為沒有人認領** —— 合併造成的紅沒有作者。
//
//   🛑 **而【看過了】這一次是真的做了, 三段各一句話寫在下面** —— 讀了哪幾行:
//     SupabaseOrderAdapter.ts:62-154 · mappers/order.ts:163-216 · IOrderRepository.ts:71-84
//   ① adapter 那段講的是:這串 select 取哪些欄、每欄為何過鐵則 12 —— 摘要欄 + 內嵌 `order_items`;
//      **禁**經銷價/cost/tier/PII;`unit_price` **仍然不取**;`line_total` / `product_snapshot` /
//      variant images 自 2026-08-29 Sean 拍板(卡片商品列)起進投影;`cancelled_at` / `cancelled_reason`
//      是 `#249` 加的, 而 `cancelled_reason` **投影出來只給伺服器用**(原文不進瀏覽器)。
//   ③ mapper 那段講的是:`SupabaseOrderListRow` 只取投影的那些欄, 並**刻意留著**已失效的原句
//      (加刪節線), 讓搜舊字面「不含 product_snapshot」的人同一發撞到現況。
//   ④ ports 那段講的是:`listSummariesByCustomer` 回 `OrderListItem[]`;「列表不含 items[]」那句
//      2026-08-29 起不成立, 而**繞過 `#217` 的理由仍然成立** —— `items[]` 是唯讀顯示投影
//      (title/brand/imageUrl/quantity/lineTotal)、**沒有 `productId`** ⇒ 不必重建 `Order.items[]`。
//   ⇒ 🔴 三段講的是不是碼在做的同一件事:**是**(零刪除、零事實漂移;
//     `ORDER_LIST_SELECT` 字面一字未動, 而它的 byte-equal 守門本次 114 passed)。
//
//   ⚠️ **而本次不是只重釘 —— 先做了一次純位置搬移**(見同 commit):
//   `82c9c3ac` 那 6 行插入的位置把兩處的「上一行↔下一行」切開了(落點① 切掉續行、
//   落點③ 切掉縮排子項)⇒ 句子還讀得通, 而讀到的是**另一個父親**。
//   已把那 6 行各自往下搬回被切斷的那一行之後, **零字改動、只換位置**。
