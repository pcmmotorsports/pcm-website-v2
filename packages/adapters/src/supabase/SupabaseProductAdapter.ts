...
export class SupabaseProductAdapter implements IProductRepository {
  ...
  async searchByKeyword(
    ...
    // 🔴 **2026-09-18 訂正:它貼了。** `supabase/APPLIED.tsv` ⇒ `20260903050000` **貼於 2026-09-03**。
    //    ⇒ 「在等 Sean」那半也過期了 —— 他早就拍過, **不要再端他一次**。
    //    🔵 **而下面那兩行的設計仍然成立、一個字都不改**:函式不在時結果要【變差】而不是【消失】。
    //       📌 那是**降級策略**, 不是「等上線」的暫時措施 —— 它守的是「哪天那支函式又不在」那一天。
    ...
    if (rpcPage !== null) {
      ...
      //
      // ⛔ ~~**今天這一段是【no-op】, 而那是刻意的**~~ ⇒ ✅ **2026-09-07:它已經是真的在做事了。**
      //   ⛔ ~~`pageIds` 是從 `[...brandIds].sort()` 切出來的, 所以順序現在就等於 id 升冪~~
      //   ⛔ ~~**而 `.sort()` 今天【不能拿掉】**~~ ⇒ **兩個前置都滿足了, 逐條**:
      //     ① db 的 `62`(`20260906950000`)**已貼進正式庫** —— `supabase/APPLIED.tsv:548`
      //        逐字 `2026-09-06` · sha `8b824f53…`(本窗開檔核過, 不是轉述)
      //        ⇒ RPC 在函式內排序(完全命中優先), 對外簽章仍 `id uuid`。
      //     ② 上面 `:587` 的 `.sort()` **已拿掉**(`const ordered = [...brandIds];`)。
      //   ⇒ 🎯 **⇒ 這一段從【接縫】變成【唯一把上游順序送到畫面的那一步】** ——
      //     它今天若被拿掉, 輸出會退回 PostgREST 的 id 升冪, 而**每一個訊號仍然是綠的**。
      //   🛑 **舊字面刻意留刪除線不刪** —— 搜「今天這一段是 no-op」的人會同一發撞到這裡;
      //     刪掉只會讓引用它的地方變成沒有訂正的孤本。
      //   ⚠️ **而那兩個前置的順序不可對調, 那不是歷史** —— 先 `62` 再拿 `.sort()`:
      //     反過來做(RPC 還沒 `ORDER BY` 而 TS 已不排)⇒ 列序任意
      //     ⇒ **分頁會重複與漏商品**(第 1 頁與第 2 頁是兩次獨立呼叫)。
