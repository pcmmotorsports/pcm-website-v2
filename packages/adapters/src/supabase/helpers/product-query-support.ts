...
export function partNumberPattern(term: string): string | null {
  ...
  const segments = term
    ...
    .filter((seg) => seg !== '');
  // 🔴🔴 **這裡原本有一道 `if (segments.length < 2) return null;` —— 它是【死碼】。**
  //    code-reviewer 2026-09-03 證明它構造不出來:走到這一行 ⟺ 上面兩道守門都過了
  //    ⟺ 這串是 ASCII 且**同時含字母與數字** ⇒ 那兩者之間**必然**有一個字母↔數字交界
  //    ⇒ 切完**恆 ≥ 2 段**。
  // 🛑 而更該記的是:**我為它寫的測試假裝蓋到了它** ——
  //    我餵 `'a1'.slice(0, 1)`, 而那是 `'a'` ⇒ 它被上面那道「沒有數字」的守門攔掉,
  //    **從來沒有到過這一行**。⇒ 📌 一格「看起來在測邊界」的測試, 測的是另一道守門。
  //    ⇒ ⇒ 而拿掉那個 `if` **零測試會紅** —— 那正是「測試假綠」的定義。
  // ✅ 拿掉它。而拿掉之後行為零改變(它從來沒有執行過);
  //    若日後有人放寬上面的守門, 一段的情況只會多送一個與原 pattern 等價的 clause ⇒ 不會錯。
