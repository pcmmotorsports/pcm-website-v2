...
export async function listCategories(
  ...
  const { data, error } = await supabase
    ...
    //    改前 vs 改後, 客人看得到的順序:
    //      🔴 **頂層 31 個 ⇒ 2 列互換** —— 第 9/10 位 **`碳纖維部品` ⇄ `燈具方向燈`**
    //         ⇒ 🛑 **那是首頁看得到的** ⇒ 主視窗 2026-09-04 批准。
    //         ⇒ 📌 **⇒ 寫在這裡是為了「哪天有人問首頁分類順序怎麼變了」時, 答案找得到。**
    //      ✅ 有貨的子分類 84 個 ⇒ **0 列移位**
    //      🔵 而**扁平清單** 117 列裡有 81 列移位 —— 📌 **那份扁平順序不是被渲染的那一份**
    //         (`lib/category-taxonomy.ts` 的樹是它唯一的出口, 扁平陣列不外流)。
    // 🔴 **`name` 在 `id` 之前, 而順序不能反** —— 理由是 `id` 買不到的那一半:
    //    ⛔ ~~上一版只有 `.order('id')`~~ ⇒ 🛑 而 `id` 是 `gen_random_uuid()`
    //      (`supabase/migrations/20260505130758_init_brands_categories.sql:39`)
    //      ⇒ **重灌 seed / 重建那一列 ⇒ 前台順序靜靜換一套**
    //    ⇒ 🎯 **⇒ 那表示「只用 id」的「每次都一樣」帶著一個看不見的括號:(只要沒有人重建那一列)**
    //    ⇒ ⇒ 📌 **⇒ 而那個括號不會出現在任何畫面上、任何測試上。**
    //    ✅ 而 `name` 跨得過重建。(2026-09-04 主視窗拍乙)
    // 🔵 而 admin 早就用 name 破平手:`apps/admin/src/lib/products/product-taxonomy-options.ts:107`
    //    逐字 `a.sort_order - b.sort_order || a.name.localeCompare(b.name)`
    //    🛑 **而可以說的只有「兩邊都用 name 破平手」, 不是「兩邊順序相同」** ——
    //      JS 的 `localeCompare` 與 PostgREST 的 `order=name.asc`(走 DB collation)
    //      **對中文不保證同一種排序**, 而我沒有量過它們一不一樣。⇒ 📌 射程寫在這裡。
    .order('name', { ascending: true })
    // 🔴🔴 **`id` 這一格【不能省】** —— `name` 也不唯一:
    //    🔬 正式站唯讀實查 2026-09-04:`(sort_order, name)` 這一對**還有 3 組平手** ——
    //       `20 / 水管束環` ×2 · `10 / 防爆水管組` ×2 · `10 / 維修零件` ×2
    //    🔬 而 `ORDER BY sort_order, name` 的 md5 是 `3ed9c3aa…`, 加上 id 之後變 `fe90a410…`
    //       ⇒ **那三組今天仍然是照「沒有人寫下來的東西」排的。**
    //    ⇒ 🎯 **⇒ 所以最後這一格買的是【全序】, 而前一格買的是【跨得過重建】。兩格各買一半。**
