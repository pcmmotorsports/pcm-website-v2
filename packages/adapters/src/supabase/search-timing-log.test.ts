...
describe('搜尋那條路的計時量具', () => {
  ...
  it('🔴 route:四條腿各自要有數字, 否則只量得到其中一條', () => {
    ...
    // 🔴🔴 **2026-09-14:與上面 adapter 那一格【同一個病、同一個修法】—— 不是把守門調鬆。**
    //   ⛔ ~~`expect(countInfo(route)).toBe(1)`~~ 數的是**整支檔的 `console.info` 總數**,
    //   而本格的標籤說的是「**那發 Promise.all 的四條腿**各自要有數字」。那兩件事在今天之前剛好相等,
    //   ⇒ 📌 **又一個靠巧合成立的等式** —— 品牌俗名退路(`13cb51958`:文字 0 筆且俗名解出品牌 ⇒ 改列
    //     該品牌目錄第一頁)在**另一個分支**印了 `path=brand-synonym`(**不是那四條腿的計時行**),
    //     它就紅了, 而訊息說「四條腿少了一條」—— **那句話是錯的:四條腿都還在(上面那個 for 迴圈剛驗過)。**
    // ✅ 改成直接釘【那一行計時行本身】:四條腿少一條、或那一行被拆成兩發 ⇒ 紅;
    //    **而在別的分支加一行資訊不會誤報**(與 adapter 那一格逐字同形)。
    const timingLines = (route.match(/\[api\/search\] qlen=/g) ?? []).length;
    expect(timingLines, '四條腿的計時行不是恰好一行 ⇒ 要嘛不見了, 要嘛被拆成兩發而數字對不起來').toBe(1);
    expect(countInfo(route), 'route 一行 console.info 都沒有 ⇒ 整條路零儀器').toBeGreaterThanOrEqual(1);
  });
