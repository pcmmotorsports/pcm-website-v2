...
describe('#906:refund_id 出得來嗎(唯一變因 = wire.status)', () => {
  // 🔴🔴 **這個值是【編的】,不是抄來的 —— 而它上一版是抄來的**(2026-08-29 線C 自查後改)。
  //    上一版我用的是一顆**真的** DR 編號(2026-08-22 一筆真交易的,出處在信箱的規格輸入檔)。
  //    ⇒ 我當時的理由是「用真實形狀比較不會寫錯」—— 而那個理由本身沒有錯,
  //      錯的是**真實形狀不需要真實資料**:`DR` + 8 位日期 + 6 碼英數,編一個就有了。
  //    📌 而它是怎麼滲進來的:規格檔 §四把那顆編號落檔(那是對的,它本來只活在訊息裡),
  //      而我讀那份檔 ⇒ 抄進 fixture ⇒ **三步裡沒有一步是錯的**。
  //    ✅ 現值的負對照(2026-08-29 當場跑):`DR20991231ZZQ641` 在 repo ⇒ 0 檔、信箱 ⇒ 0 檔;
  //      而同一把尺量那顆真值 ⇒ 信箱 **1** 檔 ⇒ **尺是活的**。
  //    ⚠️ 年份刻意用 **2099** —— 讓它一眼看得出是編的,而不是「看起來很像真的」。
  const DR = 'DR20991231ZZQ641';
