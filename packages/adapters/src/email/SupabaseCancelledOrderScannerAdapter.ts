/**
 * SupabaseCancelledOrderScannerAdapter —— 取消信掃描面的 Supabase 實作。
 *
 * 🔵 它查 `public.pcm_cancelled_email_pending`(`20260905310000`)。
 *    **射程住在那支 view 裡, 不在這裡** —— 這一支只負責翻頁與解析。
 * 🛑 而那支 view **刻意不看 `order_cancellations`**:`admin_mark_order_cancelled` 不寫那張表
 *    ⇒ 看它的話, 走那條路取消的單零命中, 而「零」與「沒有單要寄」印同一個數字。
 */
import 'server-only';
import type { SupabaseClient } from '@supabase/supabase-js';
import type {
  ICancelledOrderScanner,
  ListCancelledWithoutEmailInput,
  ListCancelledWithoutEmailResult,
} from '@pcm/ports';
 ...
 * ⇒ 射程全文與來源座標在 port 檔頭(`ICancelledOrderScanner`),**這裡不重寫一份**。
 */
/**
 ...
 */
export type CancelledOrderScannerClient = SupabaseClient<Database>;
 ...
 */
export class CancelledScanQueryError extends Error {
  constructor(public readonly stage: 'orders' | 'customers', public readonly code: string) {
    ...
    this.name = 'CancelledScanQueryError';
  }
}
...
async function safeQuery<T>(
  ...
  try {
    ...
    throw new CancelledScanQueryError(stage, 'rejected');
  }
  if (outcome.error !== null) {
    throw new CancelledScanQueryError(stage, outcome.error.code || 'unknown');
  }
  return outcome.data;
}
...
//    那份型別是從**正式庫**產生的, 而 `20260905310000` **還沒貼**
//    ⇒ 當場量:`pcm_unpaid_cancelled_email_pending` 在裡面(1)、本支 **0**。
// ⇒ 🛑 所以 `.from(PENDING_VIEW)` 過不了 `SupabaseClient<Database>` 的多載。
// ✅ **窄轉型只包住那一個呼叫**, 不是把整個 client 放寬:
//    ⇒ 📌 放寬 client = 這支檔之後**所有**打錯的表名都不會紅。
// ⏰ **什麼時候拿掉**:Sean 貼了 `20260905310000` 並重新產生 types 之後 ——
//    那時這個 cast 會變成**多餘而無害**, 而留著它的代價是「下一個人以為這支 view 不在型別裡」。
const PENDING_VIEW = 'pcm_cancelled_email_pending';

export class SupabaseCancelledOrderScannerAdapter implements ICancelledOrderScanner {
  constructor(private readonly client: CancelledOrderScannerClient) {}

  async listCancelledWithoutEmail(
    ...
    if (!Number.isSafeInteger(input.limit) || input.limit < 1 || input.limit > MAX_LIMIT) {
      throw new CancelledScanQueryError('orders', 'bad_limit');
    }
    ...
    const page = await safeQuery('orders', () =>
      this.client
        ...
        //    / outbox anti-join / 手動留白 —— **五個條件都在 view 裡**了。
        // 🔴 是 `payment_method` 不是 `payment_channel`(codex R2 ④;DB COMMENT 逐字「算錢勿用本欄」)。
        .select(
          'order_id, display_id, cancelled_at, cancelled_reason, created_at, notification_email, customer_email, order_source, refunded_amount, refund_kind',
        )
        // 🔴 **cutoff 留在這裡** —— 它是參數, 烤不進 view。
        // 🔴🔴 **而 `.gte('created_at', cutoff)` 這一條【已經刪掉了】**(codex R1 ②)——
        //    ⛔ ~~原樣從兄弟片(未付款取消)搬過來, 理由寫著「不順手動它」~~
        //    🛑 **那個「無害」只在兄弟片成立**:未付款單 1 天就 expire ⇒ 建立與取消幾乎同時
        //      ⇒ 兩個 cutoff 篩出同一群。
        //    🔴 **本片不是** —— 一張刷卡付掉的單可以放好幾週, 然後今天才被取消。
        //      `created_at` 在 cutoff 之前 ⇒ **它永遠進不了這一批** ⇒ 那封信一輩子不會寄,
        //      而症狀是「掃到 0 筆」—— 與「今天沒有單要寄」印同一個東西。
        //    ⇒ 📌 **逐字搬一個條件, 會把它【在別處才成立的無害理由】一起搬過來。**
        //    ✅ 這條線該看的只有 `cancelled_at`(port 合約也只講它)。
    ...
    return {
      rows: rows.map((o) => ({
        ...
        // 🔴 `?? 0` / `?? 'partial'` 是 **fail-safe 的方向**:讀不到就當「沒退到滿」
        //    ⇒ sweep 只在 'full' 時印退款那一段 ⇒ 📌 **讀不到金額時它【不印】, 而不是印 0。**
        refundedAmount: o.refunded_amount ?? 0,
        refundKind: o.refund_kind ?? 'partial',
