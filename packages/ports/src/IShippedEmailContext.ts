...
export type ShippedEmailContext = {
   ...
   *    ⇒ 📌 **不能拿號碼去問這個問題**:A 改成 B、B 改成 C、再改回 B ——
   *      第一封與第三封的**號碼相同而它們是兩件事**, 拿號碼比會兩封都寄。
   * 🛑 **比較用【時刻】不用【字串相等】** —— 工作單那側帶的是 SQL 算的 20 位數字串,
   *    而這一欄是 DB 直接回的 ISO 字串, **兩者格式不同是刻意的**:
   *    要求兩邊字串相同 = 要求兩邊各格式化一次 = 那正是最會漂的做法。
   */
  trackingCorrectedAt: string | null;
  /** 這一箱裝了什麼。 */
