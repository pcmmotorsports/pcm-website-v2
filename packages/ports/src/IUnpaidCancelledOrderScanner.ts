...
/**
 * IUnpaidCancelledOrderScanner —— 「未付款被【員工】取消、而還沒排過通知信」的窄查詢 port。
 *
 ...
 */
export type UnpaidCancelledOrderWithoutEmail = {
  orderId: string;
  displayId: string;
  /** 取消時刻(ISO 8601)。 */
  cancelledAt: string;
...
export type ListUnpaidCancelledWithoutEmailResult = {
  rows: UnpaidCancelledOrderWithoutEmail[];
  scannedPages: number;
  truncated: boolean;
};

export interface IUnpaidCancelledOrderScanner {
  listUnpaidCancelledWithoutEmail(
