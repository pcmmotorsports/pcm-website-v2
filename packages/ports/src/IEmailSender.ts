...
export type SendEmailInput = {
  ...
  subject: string;
  /**
   * 純文字內文(對齊 EmailAlertNotifierAdapter 慣例)。
   *
   * 🔴 **仍然必填 —— 而那是 2026-09-01 加 `html` 那一片刻意保住的**:
   *    加一個選填欄**不該改變必填欄的地位**。做成「html 與 text 二選一」會讓
   *    「這封信沒給 html」變成一個【要處理的狀態】,而它今天不是。
   *    ⇒ 而 `text` 同時是**退化路徑**:收信端不顯示 HTML 時(純文字客戶端、
   *      Gmail 的 plain-text 檢視、或本層把畸形 html 丟掉的那一發),它是唯一讀得到的那一份。
   *      ⛔ ~~第一版把「無障礙讀屏」列進這個清單~~ **那是錯的**(codex nit):
   *      **讀屏讀得了寫得好的 HTML** —— 它不屬於「不支援 HTML」那一類。
   *      ⚠️ 而所引的 mailbox 檔只支持「純文字版對到達率有價值」,**沒有支持讀屏這個例子**。
   */
  text: string;
  /**
   * HTML 內文(選填)。給了就與 `text` 一起放進同一個 POST body。
   * ⚠️ **「收信軟體會挑一份顯示」這件事我沒有來源** —— repo 內與所引的 mailbox 檔都查無
   *    Resend 同收 `text` + `html` 的行為描述(2026-09-01 grep 過)。⇒ **標【未確認】。**
   *    它對片1(只開管道)不構成影響,而**片2 會照著它做** ⇒ 那一片動手前要自己查一次。
   *
   * 🔴 **選填的規矩與 `attachments` 逐字相同,而它不是巧合 —— 那是同一題**:
   *    **不給 `html` 時,送出去的 POST body 不得出現 `html` 這個 key**。
   *    給空字串 / `undefined` 一律當作沒給。
   *    ⇒ 理由與下面那格一樣:**既有呼叫端一個字都沒改,而它們寄出去的東西要逐位元不變。**
   *    ⚠️ 而這一欄是 `text` 那格**原本**寫的「(對齊 EmailAlertNotifierAdapter 慣例;
   *      **HTML 版留 E3 需要時擴欄**)」預告的那一欄 ⇒ 走的是設計時就留好的路,不是在旁邊挖一條。
   *      🔴 **而那句話在本片重寫 `text` 註解時被我刪掉了**(code-reviewer 抓到:指標指向自己)
   *      ⇒ 所以這裡把它**原文引在這一格裡**,而不是指過去。
   *
   * 🛑 **本層【刻意不驗】html 的內容** —— 不 sanitize、不量長度、不判它是不是合法 HTML。
   *    **而「刻意不驗」與「忘了驗」在碼上長得一樣,差別只在有沒有這一行。所以寫的是為什麼:**
   *    ⇒ ① 內容是**產出側的契約** —— 同一條寫在 **adapter** 裡(`ResendEmailSenderAdapter.ts`
   *         那句「刻意**不驗 base64 格式**:那是產出側的契約」)。⚠️ **不在本檔**,別在這裡找。
   *    ⇒ ② Gmail 會在信超過某個大小時把它剪掉 —— 而那條**屬於模板那一片**,不屬於管道:
   *         它要量的是「這封信組出來多大」,而本層只知道「有人給了我一個字串」。
   *         🔴 **而那個數字要照原文的形狀引,不要寫成平述句**:
   *         來源 `~/pcm-mailbox/58-片B前置查證-附件與到達率-20260823.md` 逐字是「**約** 102 KB」,
   *         而同檔下一段自己標「**我沒有另外向 Google 查證 ⇒ 單一來源**」。
   *         ⛔ ~~本註解第一版寫成「Gmail 在 102KB 會把信剪掉(OD 交接檔照抄)」~~ —— **兩處都錯**:
   *           它不是 OD 交接檔(是窗 58 親讀 Resend 官方文件的查證),而「約」與「單一來源」都被我吃掉了。
   */
  html?: string;
  /**
