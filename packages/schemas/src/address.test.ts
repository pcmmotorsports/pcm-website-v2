...
describe('AddressInput 發票跨欄位規則(U3a canonical schema)', () => {
  ...
  it('🔴 電話必填(Sean 2026-09-04 拍甲)—— 空字串與純空白都要擋', () => {
    ...
    // ⚠️ **而這個值下游有一條長度線**:新竹那支 `hct-trans-data.ts` 的 `HCT_MAX.phone`。
    //    ⛔ ~~`= 15`(`ertel1` Char(15))⇒ 這個 20 字元的值送新竹時會被靜靜截成 15~~
    //    🔴 **2026-09-09 訂正:那個 15 是錯的, 它是【出貨人】電話的上限。**
    //       V15 規格第 12 頁逐字 `ertel1 收貨人電話1 String(20)`;
    //       第 13 頁逐字 `ettel1 出貨人電話1 String(15)` ⇒ **兩個欄位被拿混了。**
    //       ⇒ `HCT_MAX.phone` 已改為 **20** ⇒ 下面這個**剛好 20 字元**的值現在**不會被截**。
    //    📌 **而這一格的立場沒有變**:它是「schema 該放行的值」, 不是「一路無損的理想值」——
    //       21 字元以上仍然會被截(會列進 `truncated`, 不炸)。別把這一格讀寬。
    //    ⚠️ 另有一條**格式**線兩邊今天都沒擋:V15 第 11 頁「電話規則」逐字**不可加國碼**、
    //       **不可填分機** ⇒ 下面這個 `+886 … #12` schema 放行、長度也過, 而**新竹那端可能拒收**。
    //       要不要在我們這側擋是 Sean 的決定(2026-09-09 已端給他)。
