...
import type { IEmailOutbox, ICancelledOrderScanner } from '@pcm/ports';
import { enqueueOrderCancelledEmails } from './enqueue-order-cancelled-emails';
...
function deps(rows: ReturnType<typeof row>[]) {
  ...
  const scanner: ICancelledOrderScanner = {
    listCancelledWithoutEmail: vi.fn(async () => ({
      rows,
      scannedPages: 1,
      truncated: false,
    })),
...
describe('enqueueOrderCancelledEmails', () => {
  it('🔴 正常一筆 ⇒ 排一封, 而 payload 帶取消時刻與理由', async () => {
    const d = deps([row()]);
    const r = await enqueueOrderCancelledEmails(d, OPTS);
  ...
  it('🔴 notification 空 ⇒ 退化到 customerEmail', async () => {
    ...
    await enqueueOrderCancelledEmails(d, OPTS);
...
describe('片 C:手動建單留白 = 不寄', () => {
  ...
  for (const src of MANUALS) {
    it(`① ${src} + 留白 ⇒ 不寄、計 noRecipient(不得退回 customers.email)`, async () => {
      ...
      const res = await enqueueOrderCancelledEmails(d, OPTS);
      expect(d.outbox.enqueue).not.toHaveBeenCalled();
...
// 🔴🔴 **這一族守的是【金額有沒有一路帶到 outbox】** —— 而它是本片與另外四支唯一不同的地方。
//    🛑 少了它:`refundedAmount` 掉在半路 ⇒ enqueue 進去的 payload 沒有金額
//    ⇒ sweep 那一段 `if (refundKind === 'full')` 仍然成立 ⇒ **信裡印「退款金額 NT$ 0」**。
//    📌 **一個掉在半路的數字, 比一個沒有的數字糟** —— 後者不印, 前者印一個錯的。
describe('退款金額要一路帶到 outbox', () => {
  function argOf(d: ReturnType<typeof deps>) {
    return (d.outbox.enqueue as unknown as { mock: { calls: unknown[][] } }).mock
      .calls[0]![0] as Record<string, unknown>;
  }

  it('🔴 refundedAmount / refundKind 出現在 enqueue 的參數裡', async () => {
    const d = deps([row({ refundedAmount: 5000, refundKind: 'full' })]);
    await enqueueOrderCancelledEmails(d, OPTS);
    const arg = argOf(d);
    expect(arg.refundedAmount, '🔴 金額掉在半路 ⇒ 信裡會印「退款金額 NT$ 0」').toBe(5000);
    expect(arg.refundKind).toBe('full');
  });

  it('🔵 換一組值也要跟著換 —— 不是寫死 5000/full', async () => {
    // 🛑 少了這格:一個把 refundedAmount 寫死成 5000 的實作, 上面那格照樣綠。
    const d = deps([row({ refundedAmount: 1234, refundKind: 'partial' })]);
    await enqueueOrderCancelledEmails(d, OPTS);
    const arg = argOf(d);
    expect(arg.refundedAmount).toBe(1234);
    expect(arg.refundKind).toBe('partial');
  });

  it('🟢 正對照:同一發裡 eventType 是 order_cancelled(證明上面兩格不是在看別支)', async () => {
    const d = deps([row()]);
    await enqueueOrderCancelledEmails(d, OPTS);
    expect(argOf(d).eventType).toBe('order_cancelled');
  });
});
...
describe('⟦auth-MANUALORDERLIMITBURN⟧ 那兩個承重條件的守門', () => {
  ...
  it('🔴 cap 擋下整批時, 痕跡【已經落下去了】(落痕迴圈必須排在 cap 之前)', async () => {
    ...
    await expect(enqueueOrderCancelledEmails(d, OPTS)).rejects.toThrow();
    const trace = outbox.enqueueManualNoRecipient as unknown as { mock: { calls: unknown[][] } };
