...
describe('⟦5b-SHIPPEDNUMNOTRECORDED1⟧ 每一種信都要表態:它印不印追蹤號碼', () => {
  ...
  const PRINTS_TRACKING_NUMBER: Record<EmailOutboxEventType, 'records-number' | 'no-number'> = {
    ...
    // 部分取消補寄信(2026-09-13):講的是【新的應付金額】—— 未付款的單, 還沒出貨。
    // 🔵 照判別句走過, 而**判準不是「它今天有沒有模板」**:本型別在 sweep 裡今天是
    //    fail-closed throw(文案未核可)⇒ 一個字都不印。
    //    ⇒ 📌 那會讓「'no-number' 因為它不印任何東西」變成一句**恆真而無資訊**的話。
    //    ✅ 所以我答的是**那封信【將來】會印什麼**:三行金額 / 匯款資訊 / 期限 / 訂單頁連結 / LINE
    //      (Sean 2026-09-13 親手給的那一份, `~/pcm-mailbox/0913-Sean文案-部分取消補寄信.md`)
    //      —— 零貨運欄位、零箱號 ⇒ 'no-number'。
