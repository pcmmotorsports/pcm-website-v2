...
export type CheckAnomalyAlertsResult = {
   ...
   * ```
   * 📌 **四者互斥**:`disarmed` 為真時其餘三個恆假(沒上膛就沒有讀過)。
   *
   * 🔵 **[codex R3 nit:我原本在這裡多寫了「而且【窮盡】」, 而那個字不成立]**
   *   🛑 **正常上膛、資料新鮮、`rowsSeen = 30`** ⇒ `disarmed`/`unknown`/`failed`/`empty`
   *     **四個全是 false** ⇒ 📌 **那是第五態(「正常」), 而它沒有自己的欄位。**
   *   ✅ **修法是改這句話, 不是加第五個欄位** —— 一個 `fitmentOk` 只會是
   *     `!disarmed && !unknown && !failed && !empty` 的**副本**, 而副本會與本尊分岔,
   *     🔴 **分岔的時候不會有任何東西叫**(本檔已經為了同一件事把 `*ForMessage` 收成單一來源)。
   *   ⇒ 🎯 **正確的契約**:**四者互斥, 而【四者皆偽】就是「正常」那一態** ——
   *     下游要判「正常」請寫那個合取式, 不要再去找一個欄位。
   */
...
/**
 * 最後一道:整封超過 LINE 的上限時把 body 削小,**而結尾三行永遠留著**。
 *
 * 🔴🔴 **[codex R3 must-fix A①:這一行原本寫「【只】截單號清單那一段」, 而那句不實。]**
 *   ⛔ ~~「只截【單號清單】那一段」~~ —— 實作**從尾端整行 pop 整個 body**,
 *     單號行、標題行、診斷文字、刷卡三格**一律吃**。
 *   🛑 **⇒ 一句寫得比實作嚴格的註解,會讓下一個人以為某些東西被守住了** ——
 *     而它守住的只有 `footer`。📌 這與 R3 抓到的另外兩條(:1730 的窮舉、`Promise.allSettled`
 *     那句只涵蓋兩階段中的一個)是**同一族:註解宣稱的射程比碼寬**。
 *   ✅ 現在**分三刀**, 而三刀各自的承諾與代價寫在函式體裡:
 *     ① 整段丟掉【可犧牲】的監控區塊 ② 只丟【明細行】(標題與筆數留著)
 *     ③ 地板:仍超長才從尾端 pop —— **這一刀會吃掉刷卡三格, 而我明寫它。**
 *
...
function fitToLineBudget(subject: string, body: readonly string[], footer: readonly string[]): string {
  ...
  /**
   * 🔴🔴 **第二刀(codex R3 must-fix A②):只丟【明細行】—— 標題與筆數永遠留著。**
   *
   * ⛔ ~~原本第二刀就是 `kept.pop()` 從尾端整行丟~~
   * 🛑 **而 `chargeBlock` 排在 body 的【最後面】** ⇒ 監控區塊丟完仍超長時(既有 30 筆
   *   單號、`display_id` 末段 `{4,}` 沒有上界 ⇒ 做得到)⇒ **尾端 pop 第一個吃掉的就是它**
   *   ⇒ 📌 **⟦板 931⟧ 那句「刷卡三格【告警日也要有】」在最需要的那天靜靜地不成立。**
   * ✅ **明細行 = 以【兩個半形空白】開頭**(`orderLines` 逐字 `  ${id}`)——
   *   而 `chargeBlock` 那幾行開頭是「過去」與**全形**空白「　其中」⇒ **這把尺分得開它們。**
   *   ⚠️ **代價明寫**:有人把 `orderLines` 的縮排從兩個半形空白改掉 ⇒ 這一刀對它失效,
   *     而**沒有東西會紅** ⇒ 所以測試釘的是「截完之後刷卡三格還在」, 不是釘那個縮排。
   * 🔵 少幾個單號仍然可用(`note` 那句已經說了「只列出一部分」);少了筆數就不知道多嚴重。
   */
  const isDetailLine = (l: string) => l.startsWith('  ');
  const fits = () => [...kept, note, ...footer].join('\n').length <= budget;
  for (;;) {
    if (fits()) break;
    let i = -1;
    for (let k = kept.length - 1; k >= 0; k -= 1) {
      if (isDetailLine(kept[k]!)) { i = k; break; }
    }
    if (i === -1) break;
    kept.splice(i, 1);
  }
  /**
   * 🔴 **第三刀 = 地板, 而它【會】丟掉刷卡三格 —— 我不承諾它不會發生。**
   *
   * 走到這裡代表:可犧牲區塊全丟了、明細行一行不剩, 而**只剩標題與筆數還是超長**。
   * 🛑 **我沒有量過它可不可能發生** —— 現行內容下我估是幾百字 vs 4,600 預算,
   *   而**「估算」不是「量到」**(`00-work-rules` §6-b 第 3 條)
   *   ⇒ 📌 留一刀讓訊息一定寄得出去, 並且**把代價寫在這裡** ——
   *   不寫一句「不可能發生」去關掉下一個人的檢查動作。
   */
 ...
 * ⛔ ~~畫面那半已經在(`apps/admin/src/components/orders/cancel-result-panel.tsx`)~~
 * 🔴🔴 **2026-09-09 訂正 —— 那個指標【指錯檔】, 而且它替一個【形狀不同的東西】背書。**
 *    🔬 量法與讀數(窗 B, 兩把 grep 一致, 帶兩個正對照):
 *      · `cancel-result-panel.tsx`(22,255 bytes)對 `TapPay` ⇒ **0**
 *        (包裝 grep 0 · `/usr/bin/grep` 0 · 🟢 正對照同檔找「取消」⇒ **46 行** ·
 *         同尺同字在 `cancel-review-section.tsx` ⇒ **6 行** ⇒ **尺是活的, 那個 0 不是量具死掉**)
 *      · ⛔ ~~該檔的文案「全是」把這張單結掉那條路的~~ **過強**(codex 2026-09-09 nit):
 *        該檔還有一般取消成功文案(`CANCELLED_MATCH_TEXT`)與帳本核對 / 失敗 / 重試訊息。
 *        ✅ 收窄成:**`:189` 與 `:217` 那兩處**屬於「錢已全額退完之後結單」那條路,
 *        而那正好是本段擔心的「錢還沒退」的反面。
 *
 * ✅ **真正的落點是 `apps/admin/src/components/orders/cancel-review-section.tsx` 的 `payment_card_rail`**,
 *    而**它的形狀是【擋】不是【事後提醒】**。畫面逐字(窗 B 2026-09-09 在鑽機上造一張帶 card
 *    收款列的單、開取消區**實際看到**的, 不是讀碼推的):
 *      標題「這張單是刷卡付款的,要先把錢退回原卡片才能取消」
 *      內文「刷卡的錢不能只把訂單取消掉就算數,要退回原本那張卡。請到本頁最下方的「退款」裡處理;
 *            錢退完之後這張單才能取消。現金或匯款收的單則可以直接在這裡取消。」
 *    ⇒ 📌 取消表單不渲染(那一區只剩標題 + 這兩句 + 影響範圍);整單與逐品項共用同一個判定
 *      (`order-cancel-block.tsx` 的 `showForms`)⇒ **沒有部分取消那條繞路。**
 *
 * 🔴🔴 **而【擋的射程】要照實寫 —— 我第一版寫「有任何一筆 card 收款列就擋, 不是看 payment_status」,**
 *    **那句【不成立】**(codex 2026-09-09 實跑原函式量出下表;`cancel-view.ts:250` 只由
 *    `:758` 的 `paid` / `partiallyPaid` 分支呼叫):
 *      payment_status            一般取消表單     結單表單
 *      paid / partiallyPaid      不渲染           不渲染
 *      partiallyRefunded         不渲染           不渲染
 *      refunded                  不渲染           **可渲染**
 *      unpaid(扣款嘗試 clear)   **兩種都渲染**   不渲染
 *    ⇒ 🛑 **`unpaid + 有 card 收款列 + 扣款嘗試 clear` 那一格是【沒被 card 檢查蓋到的】**,
 *      SQL 那側同形(`20260908060000_m4b_partpaid_cancel_gate.sql:593` 的允許條件也讓 `unpaid` 跳過)。
 *      ⚠️ **而它【未被證實是營運漏洞】** —— 沒有人證明正常刷卡流程產得出那個狀態。
 *      📌 **寫在這裡是因為它是這道擋的邊界, 不是因為它是一個已知的洞。**
 *
 * 🔵 這道擋是 **Sean 2026-09-05 拍乙**「刷卡單先退款才准取消」的落地
 *    (`docs/plans/2026-09-05-card-cancel-blocks-until-refunded-plan.md` 標題逐字)。
 *
 * 🎯 **⇒ QB-11 乙的「畫面那半」是被一個【更強的東西】取代了, 不是沒做。**
 * ⚠️⚠️ **而「沒退完根本取消不了」這句【我收回】**(codex nit ①):**人工退款【登記】就足以把狀態
 *    推成 `refunded`**, 而那條路只要員工勾「我確認卡上那筆沒有退成功」——
 *    **它不向 TapPay 核實**(`20260905280000:194` / `20260907140000:169` / `20260903093000:734`)
 *    ⇒ 📌 **登記全額人工退款 ⇒ 狀態變 refunded ⇒ 結單鈕就開了, 而錢可能一毛都沒動。**
 *    🔵 那是**人工登記可信度**的邊界(它與 ⟦mail-PAYMENTNOCAP⟧ 同族), 不是本次新增的洞。
 * ✅ **本段能宣稱的, 收成一句**:
 *    **`paid` / `partiallyPaid` 且收款明細含 card 時, 後台會把整單與部分取消表單都藏起來並顯示退款指引;
 *      repo 的取消 RPC 也有對應阻擋(未核對正式庫部署版本)。**
 *    🛑 **這【不等於】已證明所有入口都無法在真的退錢之前取消。**
 *
 * ⇒ 以下是**日報那半**。
 *
...
export function buildAnomalyAlertMessage(
  ...
  if (fitmentSync.readFailed) {
     ...
     *
     * 🔴🔴 **[codex R3 must-fix A③:那句窮舉【少了一種】, 而少的那種會被主動排除]**
     *   ⛔ ~~「兩種可能:① 從來沒跑過 ② 留痕還沒裝」~~
     *   🛑 **而第三種:留痕【曾經有過而被清掉】**(有人手動清空 / 保留政策把舊列刪光)
     *     ⇒ 那時 `rowsSeen = 0`, 而它與①②**在資料上完全一樣**。
     *   🎯 ⇒ 📌 **一句「只有這兩種」的窮舉, 比不寫更糟** —— 它會讓收信的人
     *     **主動排除掉真正的成因**, 然後往兩個不存在的方向查。
     *   📌 這是 R2 那條「窮舉字面不實」的**第二個受詞** —— 我當時修了一處,
     *     而**同一個病在另一處**;下次改窮舉句要 grep 全檔的「兩種可能 / 只有」。
     */
    fitmentBlock.push(
      '【車款搜尋同步:留痕是空的】',
      '🔵 那張同步留痕表【一列都沒有】—— 所以我們今天【什麼都沒量到】, 而不是「量到了而沒事」。',
      '   ⇒ 三種可能, 而這封信分不出來:① 那條同步從來沒跑過 ② 這套留痕根本還沒裝上去'
        + ' ③ 留痕【曾經有過而被清掉了】(手動清空 / 保留政策刪光)。',
