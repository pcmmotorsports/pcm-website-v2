 ...
 * · 舊版(留 sending 等 lease 回收)⇒ 🔴 **兩個時點差一整輪回收, 不要合成一句**(2026-09-20 訂正):
 *   **attempts 用完 ≈ 4h20m**(每回來一次 = 租約 3600s + 退避 5 分 = 1h05m, 要等 4 次)·
 *   **而進死信要再等最後一次回收 ≈ 5h20m**。
 *   ⇒ 逐行推導在**本檔下方**那段以逐字「**而【三個時點要分開,不要合成一句】**」開頭的區塊。
 *   🛑 **這裡【刻意不寫行號】** —— 2026-09-20 實際踩到:我在本檔上方插了幾行註解,
 *      而我寫的引用是同一支檔的行號 ⇒ **插行把它推歪了**(原寫 `:2558-2570`, 實際已變 `:2563-2576`),
 *      而第一版訂正給的 `:2563-2572` **仍然切掉結論句**。
 *      📌 **⇒ 同檔自我引用 + 會插行 = 那個行號一定會錯, 只是時間問題。用句子當錨。**
 *   ⛔ ~~原本這裡寫「大約第 5 小時才用完」~~ —— 它把【用完】配上一個接近【死信】的數字。
 * · 🎯 **⇒ 上面那句「更早變成死信」比的是【進死信】** ⇒ 新版 **75 分**(失敗即死信, 兩個時點同一刻)
 *   vs 舊版 **≈5h20m**。📌 **而那個不對稱本身就是這個取捨的一部分。**
