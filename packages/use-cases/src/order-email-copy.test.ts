...
describe('會員中心那句【有沒有把條件掛在「有沒有帳號」上】', () => {
  ...
  it('🔴 條件必須掛在【有沒有 PCM 會員帳號】上, 不是隨便一個「若您有」', () => {
    ...
    const clauses = noSpace(ORDER_MEMBER_CENTER_SENTENCE)
      .split(/[。！？]/u)
      .filter((c) => c.length > 0);
    expect(
      clauses.some(
        (c) => c.includes(noSpace('若您有 PCM 會員帳號')) && c.includes('會員中心'),
      ),
      '🔴 條件與「會員中心」那句話【不在同一句】⇒ 條件被句號切走了 ⇒ 剩下的仍是一句對手動單客人為假的斷言',
    ).toBe(true);
  });
  ...
  it('🔵 負對照:把條件與敘述拆成兩句 ⇒ 上面那格【必須】看得出來', () => {
    ...
    const clauses = 拆成兩句.split(/[。！？]/u).filter((c) => c.length > 0);
    expect(
