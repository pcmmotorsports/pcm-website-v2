 ...
 *
 * 🔬 **數字**:本函式產出 5 / 10 / 20 / 40 分 ⇒ 第 5 次(`max_attempts` 預設 5)累計約在
 *    **第 75 分鐘**用完;⛔ 而**舊版**(失敗留在 `sending` 等 lease 回收, 每次 ≥1 小時)
 *    約在**第 5 小時**。⇒ 📌 **死信門檻從約 5 小時往前搬到約 75 分鐘。**
 *    🟢 來源交叉:codex `gpt-6-astra` 2026-09-07 獨立算出同一組(`sweep-email-outbox.ts` 的
 *    docstring 逐字「新版(退避 0 / 5 / 15 / 35 / 75 分)」)。
 *
 * 🎯 **⇒ 取捨方向(本段就是把它寫下來的那一句)**:
 ...
 *
 * 🛑 **而 `:54` 的 2 小時封頂在這裡【咬不到】** —— 5 次之內最大單次延遲是 40 分 < 2 小時
